...
Расширения `x-yc-apigateway-rate-limits` и `x-yc-apigateway-rate-limit` устарели и больше не поддерживаются. Чтобы ограничить скорость обработки запросов, используйте [интеграцию с {{ sws-full-name }}](sws.md).

{% endnote %}

{% note warning %}

Расширения предназначены для равномерного распределения нагрузки на приложение при легитимном трафике. При DoS-атаках корректная работа не гарантируется. Для защиты приложений используйте [профиль безопасности {{ sws-name }}](sws.md).
