# Создание триггера для {{ message-queue-short-name }}, который передает сообщения в функцию {{ sf-name }}
...
- Консоль управления
    ...
    1. В блоке **Настройки {{ message-queue-short-name}}** выберите очередь сообщений и сервисный аккаунт c правами на чтение из нее.
