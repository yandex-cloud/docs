...
__Контекст вызова__ — это объект, который принимается [обработчиком запросов](handler.md) в качестве второго аргумента.
...
* `token` — параметры для аутентификации в API сервисов Яндекс.Облака.

Контекст также содержит вспомогательные методы:
* `getRemainingTimeInMillis()` — возвращает время оставшееся на выполнение текущего запроса в милисекундах;
...
С помощью контекста вызова вы можете аутентифицироваться в API сервисов Яндекс.Облака, а также управлять получением тела HTTP-запроса (параметр `body`).

### Аутентификация в API сервисов Яндекс.Облака {#token}
...
### Получение тела HTTP-запроса {#getpayload}

`getPayload()` — это вспомогательный метод, который упрощает получение тела HTTP-запроса (параметр `body`). В зависимости от содержимого тела запроса и заголовка, функция обрабатывает данные разными методами:
