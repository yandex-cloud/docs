...
|[YC CLI](https://cloud.yandex.ru/docs/cli/)| Для работы с API Яндекс.Облака. | 
...
Сервис {{ sf-name }} автоматически захватывает поток стандартного вывода `stdout` скрипта и интерпретирует его как ответ на запрос. Поток стандартного вывода ошибок `stderr` отправляется в централизованную систему журналирования, доступную в Яндекс.Облаке. Туда же сохраняются служебные записи о начале и окончании выполнения функции и обо всех ошибках, которые произошли во время выполнения. Подробнее о формате журналов читайте в разделе [{#T}](logging.md).
