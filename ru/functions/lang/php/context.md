...
Контекст вызова является объектом, который предоставляет дополнительную информацию о свойствах версии функции. Для того чтобы получить эту информацию в своей программе, воспользуйтесь следующими методами:
...
* `getToken()` — возвращает параметры для аутентификации в API сервисов Яндекс.Облака.

## Использование контекста вызова {#use}

С помощью контекста вызова вы можете аутентифицироваться в API сервисов Яндекс.Облака.
