...
__Контекст вызова__ — это объект, который содержит некоторые параметры [контекста выполнения](../../concepts/runtime/execution-context.md) и параметры вызова самой функции. Принимается [обработчиком запросов](handler.md) в качестве второго аргумента.
...
* `get_remaining_time_in_millis()` — возвращает время оставшееся на выполнение текущего запроса в милисекундах.

## Использование контекста вызова {#use}

Контекст вызова нужен, чтобы функция имела доступ к информации об окружении: могла контролировать расход памяти, отслеживать время на выполение запроса и [аутентифицироваться](#token) в API сервисов Яндекс.Облака.
