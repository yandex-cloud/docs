...
**Контекст вызова** — это объект, который содержит некоторые параметры [контекста выполнения](../../concepts/runtime/execution-context.md) и параметры вызова самой функции. Принимается [обработчиком запросов](handler.md) в качестве второго аргумента.
...
Контекст выполнения нужен, чтобы функция имела доступ к информации об окружении: могла контролировать расход памяти, отслеживать время на выполение запроса и [аутентифицироваться](#token) в API сервисов Яндекс.Облака.
