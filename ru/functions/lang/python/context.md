...
* `function_folder_id` — каталог, в котором находится функция.
* `function_name` — идентификатор функции.
* `function_version` — идентификатор версии функции.
* `deadline_ms` — метка времени в миллисекундах в формате Unix, когда истечет таймаут выполнения [функции](../../concepts/function.md). Функция `get_remaining_time_in_millis()` возвращает интервал между этой меткой и текущим моментом.
