...
- Также для создания триггера вам потребуется роль `{{ roles-cr-puller }}` на реестр, события которого триггер обрабатывает.
