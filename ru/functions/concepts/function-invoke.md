...
Результат, который возвращает функция, также должен представлять собой JSON-документ, содержащий код ответа HTTP, заголовки ответа и содержимое ответа. {{ sf-name }} автоматически обработает этот JSON, и пользователь получит данные в виде стандартного HTTPS-ответа.
...
* `body` — содержимое запроса в виде строки. Данные могут быть закодированы в формате Base64 (в этом случае {{ sf-name }} установит параметр `isBase64Encoded: true`).
...
curl \
  --request POST \
  --data "hello, world!" \
  "https://{{ sf-url }}/<идентификатор_функции>?a=1&a=2&b=1"
```
...
curl -i \
  --request POST \
  --data "hello, world!" \
  "https://{{ sf-url }}/<идентификатор_функции>?customHeaderPolicy=stripAll"
```
...
Пример функции для вызова внешнего API при срабатывании алерта приведен в разделе [{#T}](../../monitoring/operations/alert/create-channel-function.md).
