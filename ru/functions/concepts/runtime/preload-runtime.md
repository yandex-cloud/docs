...
Как правило предзагружаемыми являются среды выполнения с последней мажорной версией. Подробнее в таблице [{#T}](index.md#runtimes). Если в сервис {{ sf-name }} добавлена новая мажорная версия среды выполнения, как предзагружаемая, предыдущая версия спустя какое-то время перестает быть предзагружаемой. Пользователям, использующим старую версию, придет уведомление и будет дано время, чтобы перейти на новую мажорную версию.
