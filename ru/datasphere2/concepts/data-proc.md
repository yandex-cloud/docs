 ...
 * подсеть будущего или уже существующего кластера {{ dataproc-name }}. {% if product == "yandex-cloud" %}В рамках интеграции доступны только подсети, созданные в зоне доступности `{{ region-id }}-a`.{% endif %}
