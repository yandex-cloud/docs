...
description: '{{ VLK }} — высокопроизводительная СУБД для данных типа ключ-значение, работающая в оперативной памяти. Сервис {{ mrd-name }} позволяет легко создавать кластеры хостов {{ VLK }} с высокой отказоустойчивостью. Основная сущность, которой оперирует сервис {{ mrd-name }}, — кластер баз данных.'
---

# Взаимосвязь ресурсов в {{ mrd-name }}


{{ VLK }} — высокопроизводительная СУБД для данных типа «ключ-значение», работающая в оперативной памяти. Сервис {{ mrd-name }} позволяет легко создавать [кластеры](../../glossary/cluster.md) хостов {{ VLK }} с высокой отказоустойчивостью.
