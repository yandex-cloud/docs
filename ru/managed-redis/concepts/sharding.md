...
Если при опросах большинство мастер-хостов не смогло получить ответ от опрашиваемого хоста, то считается что хост вышел из строя. Если из строя вышел мастер-хост, то мастером будет назначена одна из его реплик. Если все реплики вышли из строя или переключиться на них не удалось, запросы к шарду перестанут приниматься. При этом потеря целого шарда не означает отказа всего {{ RD }} Cluster — остальные шарды по-прежнему будут доступны для записи и чтения данных.
