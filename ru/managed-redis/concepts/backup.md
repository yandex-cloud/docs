...
Резервные копии в {{ mrd-name }} создаются на основе консистентного снимка образа памяти процесса, который получается в результате копирования исходного процесса {{ RD }} с помощью системного вызова `fork()`. Из-за особенностей реализации `fork()` в Linux запись данных на хост кластера {{ mrd-name }} в момент резервного копирования приведет к увеличению потребления памяти, т. к. в память процесса добавятся новые, измененные данные (механизм [Copy-on-Write]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/Копирование_при_записи){% endif %}{% if lang == "en" %}(https://en.wikipedia.org/wiki/Copy-on-write){% endif %}).
