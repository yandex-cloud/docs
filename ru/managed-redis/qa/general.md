...
Вы можете подключаться к базам данных {{ mrd-short-name }} способами, стандартными для СУБД, с одним ограниченим: хосты {{ mrd-short-name }} доступны только с виртуальных машин {{ yandex-cloud }}, подключенных к той же сети.
