...
{{ mrd-short-name }} берет на себя трудоемкие задачи администрирования инфраструктуры {{ mrd-short-name }}:
...
{{ mrd-short-name }} доступен всем зарегистрированным пользователям Яндекс.Облака.
...
Вы можете подключаться к базам данных {{ mrd-short-name }} способами, стандартными для СУБД, с одним ограниченим: хосты {{ mrd-short-name }} доступны только с виртуальных машин Яндекс.Облака, подключенных к той же сети.
...
{{ mrd-short-name }} поддерживает {{ RD }} 5.
