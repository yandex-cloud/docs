...
Каждая версия {{ RD }}, поддержка которой была включена в {{ mrd-name }}, остается доступной, пока она поддерживается производителем. Как правило, это 24 месяца с даты выпуска версии. Подробнее см. в [документации {{ RD }}](https://redis.io/docs/about/releases/).

С 1 июня 2022 года прекращена поддержка {{ RD }} версий 5.0 и 6.0. Создать кластер с этими версиями невозможно. Рекомендуется самостоятельно обновить кластер до более новой версии {{ RD }}. Если у вас возникли трудности при обновлении, обратитесь в [техническую поддержку]({{ link-console-support }}).

В момент релиза версии 7.0 пользователям кластеров {{ mrd-name }}, использующих версии 5.0 и 6.0, будет отправлено уведомление с предложением обновиться до минимальной поддерживаемой версии — 6.2. Через месяц после этой даты кластеры {{ RD }} версий 5.0 и 6.0 будут автоматически обновлены до версии 6.2.
