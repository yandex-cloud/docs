# Создание [!KEYREF RD]-кластера

[!KEYREF RD]-кластер — это один или несколько хостов базы данных, между которыми можно настроить репликацию. Репликация работает по умолчанию в любом кластере из более чем 1 хоста: хост-мастер принимает запросы на запись, асинхронно дублируя изменения в репликах.

Количество хостов, которые можно создать вместе с [!KEYREF RD]-кластером, зависит от выбранного типа хоста:
...
## Как создать кластер [!KEYREF RD] {#create-cluster}
...
1. Выберите сервис **[!KEYREF mrd-name]**.
...
1. Выберите окружение, в котором нужно создать кластер (после создания кластера окружение изменить невозможно):
    ...
    - <q>prestable</q> — для тестирования, в том числе самого сервиса [!KEYREF mrd-short-name]. Prestable-окружение обновляется чаще, из-за чего в нем раньше исправляются уже известные проблемы, но могут происходить обратно несовместимые изменения.
...
Допустим, нужно создать [!KEYREF RD]-кластер со следующими характеристиками:
