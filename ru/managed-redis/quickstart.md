...
[Запросите доступ](https://puncher.yandex-team.ru/) к макросу `_PGAASINTERNALNETS_`, чтобы подключаться к созданным кластерам. Для подключения к кластерам {{ RD }} нужен доступ к портам 26379 (Sentinel) и 6379 (Redis).
