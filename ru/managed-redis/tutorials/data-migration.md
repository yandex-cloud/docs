...
1. Подключитесь к кластеру-источнику любым удобным способом.
...
1. Запустите процесс создания логического дампа (в примере приведены не все параметры):

    ```bash
    ./redis-dump-go \
        -host <IP-адрес или FQDN кластера {{ RD }}> \
