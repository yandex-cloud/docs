...
description: На данной странице приведен список сервисных ролей для управления доступом к модулю Контроль {{ k8s }}® ({{ kspm-name }}) в сервисе {{ sd-name }}.
---

# Сервисные роли для модуля Контроль {{ k8s }}® ({{ kspm-name }})

С помощью сервисных ролей [модуля Контроль {{ k8s }}®](../concepts/kspm.md) ({{ kspm-name }}) вы можете управлять доступом пользователей к ресурсам модуля и их настройкам, а также к данным, содержащимся в результатах контроля и алертам.
