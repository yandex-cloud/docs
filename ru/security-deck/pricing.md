...
|| Модуль контроля конфигурации ({{ cspm-name }}) | Количество ресурсов в окружении, которые проходят проверку по включенным стандартам:<ul><li>виртуальная машина {{ compute-full-name }};</li><li>бакет {{ objstorage-full-name }};</li><li>кластеры баз данных&nbsp;^1^;</li><li>кластер {{ k8s }};</li><li>облачная сеть {{ vpc-full-name }};</li><li>MCP Gateway;</li><li>проект {{ ml-platform-name }}.</li></ul> | Рассчитывается ежемесячно.</br>Тарифицируются все стандарты, кроме [базовых правил безопасности облачной платформы {{ yandex-cloud }}](concepts/standard-compliance/yc-security-baseline.md).</br>Каждое окружение тарифицируется отдельно, даже если ресурсы в них пересекаются.</br>Неактивные ресурсы также тарифицируются.</br>Цена не зависит от продолжительности календарного месяца. ||
|| Модуль управления уязвимостями ({{ vuln-man-short-name }}) |
* Хранение образов в окружении {{ sd-name }}
* Операции сканирования |
* Если образ хранится в {{ sd-name }}, тарифицируются время хранения образа в окружении и сканирования. Первые 10 сканирований не тарифицируются. Оплата взимается только за каждое последующее сканирование, которое тарифицируется как повторное.
* Если образ хранится в [{{ cloud-registry-name }}](../cloud-registry/), тарифицируются только сканирования.
...
### Модуль управления уязвимостями ({{ vuln-man-short-name }}) {#vuln-example}

#### Сканирование в {{ sd-name }} {#vuln-sd-only}
...
- Расчет в рублях {#prices-rub}
  ...
  {% include [rub-vuln-sd-only](../_pricing_examples/security-deck/rub-vuln-sd-only.md) %}

- Расчет в тенге {#prices-kzt}

  {% include [kzt-vuln-sd-only](../_pricing_examples/security-deck/kzt-vuln-sd-only.md) %}

{% endlist %}



#### Сканирование в {{ cloud-registry-name }} {#vuln-cr-only}
...
- Расчет в рублях {#prices-rub}
  ...
  {% include [rub-vuln-cr-only](../_pricing_examples/security-deck/rub-vuln-cr-only.md) %}

- Расчет в тенге {#prices-kzt}

  {% include [kzt-vuln-cr-only](../_pricing_examples/security-deck/kzt-vuln-cr-only.md) %}
...
Например, команда использует {{ sd-name }} для централизованного ежедневного контроля (11 сканирований), а также раз в неделю запускает сканирование в {{ cloud-registry-name }} для локальных нужд реестра (4 сканирования в месяц). Итоговая стоимость составит:
...
- Расчет в рублях {#prices-rub}
  ...
  {% include [rub-vuln-combined](../_pricing_examples/security-deck/rub-vuln-combined.md) %}

- Расчет в тенге {#prices-kzt}

  {% include [kzt-vuln-combined](../_pricing_examples/security-deck/kzt-vuln-combined.md) %}
...
Если инфраструктура требует постоянного мониторинга образов, экономически целесообразно использовать {{ sd-name }}. Для единоразовых или редких проверок локальных реестров (до трех раз в месяц на один образ) оптимальнее использовать {{ cloud-registry-name }}.
