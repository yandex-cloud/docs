...
- Консоль управления
    ...
    1. В списке сервисов выберите **{{ lockbox-name }}**.
    1. Нажмите кнопку **Создать секрет**.
    1. В поле **Имя** введите имя секрета.
    1. (опционально) В поле **{{ kms-short-name }} ключ** укажите существующий ключ или [создайте новый](../kms/operations/key.md#create).
...
- API
    ...
    1. [Получите IAM-токен](../iam/operations/iam-token/create.md), необходимый для аутентификации и сохраните его в переменную:
        ...
        Также вы можете получить IAM-токен сервисного аккаунта изнутри ВМ, к которой он привязан. Для этого выполните запрос в [сервис метаданныx](../compute/operations/vm-info/get-info.md#request-examples). Пример c использованием утилиты [jq](https://stedolan.github.io/jq/):
