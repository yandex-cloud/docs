...
{{ lockbox-full-name }} — это сервис для централизованного хранения секретов, таких как пароли, [SSH-ключи](../../glossary/ssh-keygen.md) и токены. Секреты состоит из набора версий, в которых хранятся данные. Версия содержит пару ключа и значения. Ключ — это несекретный идентификатор значения, по которому вы будете его идентифицировать. Значение — это секретные данные.
