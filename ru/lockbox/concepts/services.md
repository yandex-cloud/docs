...
Если для работы [функции](../../functions/concepts/function.md) {{ sf-name }} требуются чувствительные данные, такие как пароли доступа к БД или [статические ключи доступа](../../iam/concepts/authorization/access-key.md), используйте секреты {{ lockbox-name }} для передачи таких данных в функцию. Это позволит предотвратить несанкционированный доступ третьих лиц к чувствительным данным.
