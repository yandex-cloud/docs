...
_{{ coi }}_ — [образ](../../compute/concepts/image.md) [виртуальной машины](../../compute/concepts/vm.md), оптимизированный для запуска Docker-контейнеров. Образ включает в себя: Ubuntu LTS, Docker и демона для запуска Docker-контейнеров.
...
* При создании ВМ с {{ coi }} нельзя создать загрузочный диск из [снимка диска](../../compute/concepts/snapshot.md).
* По умолчанию при создании ВМ из образа, создается диск, размер которого равен размеру образа. Но так как назначение ВМ с {{ coi }} — это запуск на ней Docker-контейнера, может возникнуть проблема недостаточного свободного места при развертывании Docker-контейнера. Чтобы этого избежать, явно укажите необходимый размер загрузочного диска с помощью флага `--create-boot-disk size=<размер диска в ГБ>`.
...
Инструкции для запуска Docker-контейнеров и конфигурации сервисов указываются в файле спецификации `docker-compose.yaml`, согласно [документации Docker](https://docs.docker.com/compose/compose-file/). Сложные конфигурации из двух и более файлов не поддерживаются.
...
`volumes` | Описание [томов](../../container-registry/concepts/docker-volume.md), используемых в Docker-контейнере.
