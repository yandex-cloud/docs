...
- Консоль управления
    ...
    1. В открывшемся окне **Настройка Docker-контейнера** задайте параметры, используя подсказки:
       ...
       * Укажите [**Docker-образ**](../concepts/index.md#docker-image), на основе которого будет запущен Docker-контейнер на виртуальной машине. Вы можете сделать это одним из способов:
          ...
          * Указать имя образа вручную. Для этого нажмите **Укажите ссылку** и введите имя образа. Вы можете указать образ из сервиса {{ container-registry-full-name }} или любой публично доступной библиотеки Docker-образов (например,  [Docker Hub](https://hub.docker.com)).
