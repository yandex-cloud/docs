...
При отправке данных в сервис переменная автоматически преобразуется в значение — ответ на вопрос, результат теста или технические данные.{% if audience == "internal" %} Если требуется, вы можете задать [фильтры для дополнительного преобразования переменных](send-request.md#filters).{% endif %}
