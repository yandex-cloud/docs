...
Поле заполнения формы или выполнения теста пользователь переходит на страницу результата, содержание которой можно настроить:
...
1. В блоке **Сообщение после отправки формы** задайте заголовок и текст сообщения.
...
1. Выберите способ перехода на сайт: 

    - **По кнопке** — чтобы перейти на сайт, пользователю нужно будет нажать кнопку. 

    - **Отложенный** — переход произойдет автоматически через 5 секунд после отправки ответа. При этом также можно перейти на сайт по кнопке.
    
1. В поле **Текст на кнопке** задайте название кнопки перехода на сайт.

1. Если вы публикуете форму на сайте с помощью [кода для вставки](publish.md#section_c21_gdb_42b), вы можете включить опцию **Перенаправлять на форму в том же фрейме**:
