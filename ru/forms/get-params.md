...
GET-параметры позволяют добавлять в [URL-адрес]({{link-wikipedia-url}}) формы какие-либо атрибуты. С помощью GET-параметров вы можете автоматически заполнять поля формы, которую вы публикуете по ссылке или [встраиваете на страницу с помощью iframe](publish.md):
