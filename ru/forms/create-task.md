...
1. Задайте тип, приоритет задачи и настройте другие параметры.
    ...
    **Формат ввода данных**. В полях **Автор**, **Исполнитель**, **Теги**, **Компоненты** и подобных при вводе значений не работает подсказка.
    ...
    - Если же форму заполняет не авторизованный пользователь, то в поле **Автор** записывается <q>Робот сервиса {{ tracker-name }} Робот</q> (`yndx-tracker-cnt-robot@`).
...
Возможная причина ошибки в том, что из формы в поля задачи **{{ ui-key.startrek.blocks-desktop_st-field-utils.field--author }}**, **{{ ui-key.startrek.blocks-desktop_st-field-utils.field--assignee }}** или **{{ ui-key.startrek.blocks-desktop_st-field-utils.field--followers }}** передаются неверные данные. Заполните эти поля следующим образом:

- Чтобы добавить сотрудника вручную, указывайте логин пользователя, например `ivanov`.

- Чтобы в поле **{{ ui-key.startrek.blocks-desktop_st-field-utils.field--followers }}** указать несколько пользователей, введите их логины через запятую (например, `ivanov,petrov`). 
...
Если в поле **{{ ui-key.startrek.blocks-desktop_st-field-utils.field--author }}** возникает ошибка, но поле при этом заполнено верно, убедитесь, что у заполнившего форму пользователя есть [доступ на создание задач в указанной очереди {{ tracker-short-name }}](#access).
