...
1. Добавьте группу действий или используйте существующую. В группу можно добавить несколько действий и задать для них общие условия выполнения.
    ...
    - Чтобы создать новую группу, внизу страницы нажмите кнопку {% if audience == "external" %}**Добавить группу уведомлений**{% else %}**Добавить группу действий**{% endif %}.
...
{% if audience == "internal" %}Чтобы вернуться к настройке вопросов, вверху страницы нажмите ссылку **В конструктор**.{% endif %}
...
1. Выберите группу действий и включите опцию {% if audience == "external" %}**Действия выполняются при условии**{% else %}**Выполнять действия при условии**{% endif %}.
...
1. Чтобы действие выполнялось в зависимости от того, какой у пользователя выбран язык интерфейса:

   1. В поле **Если значение** выберите **Язык**.

   1. Выберите операцию сравнения: **равно** или **не равно**. 
   
   1. Выберите язык, совпадение с которым требуется проверять.
...
1. В поле **Уведомлять об ошибках интеграций** укажите пользователей, которым будут отправляться сообщения. 
