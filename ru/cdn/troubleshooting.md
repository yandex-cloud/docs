...
Поддержку протокола [WebSocket](https://ru.wikipedia.org/wiki/WebSocket) можно включить только для [EdgeCDN-провайдера](./concepts/providers.md) и только для юридических лиц.

Чтобы запросить включение WebSocket, обратитесь в [техническую поддержку]({{ link-console-support }}). В обращении подробно опишите сценарий использования WebSocket, какие задачи вы будете решать, а также примерный объем передаваемого трафика.