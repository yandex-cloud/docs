...
- Один источник
  ...
  Если источник — бакет, выберите вариант **Свое значение** и укажите значение `<имя бакета>.{{ s3-storage-host }}` (или `<имя бакета>.{{ s3-web-host }}`, если бакет настроен для [хостинга статического сайта](../../storage/concepts/hosting.md)).
  
  В остальных случаях настройка зависит от того, какие запросы принимает источник.
  
  > Например, в [сценарии сине-зеленого и канареечного развертывания](../solutions/blue-green-canary-deployment.md) L7-балансировщик настроен так, чтобы принимать запросы с двумя значениями заголовка `Host`. Для CDN-ресурса выбран вариант **Как у клиента**.
...
- Группа источников
  ...
  1. Выберите вариант **Свое значение** и укажите значение `<имя бакета>.{{ s3-storage-host }}` (или `<имя бакета>.{{ s3-web-host }}`, если бакет настроен для [хостинга статического сайта](../../storage/concepts/hosting.md)).
  ...
  Если в группе больше одного бакета, рекомендуем использовать вместо них L7-балансировщик с бакетами в виде бэкендов (см. пример настройки в [сценарии сине-зеленого и канареечного развертывания](../solutions/blue-green-canary-deployment.md)).
