...
* Выпустить TLS-сертификаты от Let's Encrypt<sup>®</sup> для каждого доменного имени.
* Использовать для всех доменных имен один сертификат.

Сертификаты должны быть созданы или загружены в {{ certificate-manager-full-name }}. Это могут быть как [сертификаты от Let's Encrypt®](../../certificate-manager/concepts/managed-certificate.md), так и [пользовательские сертификаты](../../certificate-manager/concepts/imported-certificate.md).
