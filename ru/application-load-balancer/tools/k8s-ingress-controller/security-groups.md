...
* [Подключаться](../../../managed-kubernetes/operations/node-connect-ssh.md) к узлам по {% if lang == "ru" and audience != internal %}[SSH](../../../glossary/ssh-keygen.md){% else %}SSH{% endif %} и управлять кластером через {% if lang == "ru" and audience != "internal" %}[API](../../../glossary/rest-api.md){% else %}API{% endif %}, `kubectl` и другие утилиты можно только из CIDR `203.0.113.0/24` \[Упр\].
