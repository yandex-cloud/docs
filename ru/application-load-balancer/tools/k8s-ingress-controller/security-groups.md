# Настройка групп безопасности Ingress-контроллера {{ alb-name }} для {{ managed-k8s-name }} 

{% include [security-groups-note](../../_includes_service/security-groups-note.md) %}

Для корректной работы Ingress-контроллера нужно настроить группы безопасности кластера и групп узлов {{ managed-k8s-name }} и балансировщика {{ alb-name }}. Для кластера, групп узлов и балансировщика можно использовать разные группы безопасности (рекомендуется) или одну и ту же группу.

В группах безопасности должны быть настроены:

* Все стандартные правила, описанные в соответствующих разделах документации:

  * для кластера и групп узлов — в разделе [{#T}](../../../managed-kubernetes/operations/security-groups.md) документации {{ managed-k8s-name }};
  * для балансировщика — в разделе [{#T}](../../concepts/application-load-balancer.md#security-groups). Последнее правило, для исходящего трафика на ВМ бэкендов, должно разрешать соединения в подсе́ти [групп узлов](../../../managed-kubernetes/concepts/index.md#node-group) кластера или его группу безопасности.
