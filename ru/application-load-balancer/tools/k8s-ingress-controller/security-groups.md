...
* [Подключаться](../../../managed-kubernetes/operations/node-connect-ssh.md) к узлам по {% if lang == "ru" and audience != internal %}[SSH](../../../glossary/ssh-keygen.md){% else %}SSH{% endif %} и управлять кластером через API, `kubectl` и другие утилиты можно только из CIDR `203.0.113.0/24` \[Упр\].
