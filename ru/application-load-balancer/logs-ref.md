...
Подробнее читайте в разделе [Логирование](./concepts/monitoring.md#logging).
...
`smartwebsecurity` | Информация о результате проверки запроса в [{{ sws-name }}](../smartwebsecurity/concepts/index.md). Возвращается, если к виртуальному хосту [подключен профиль безопасности](../smartwebsecurity/operations/host-connect.md). Подробнее о логировании правил SWS в разделе [{#T}](../smartwebsecurity/operations/configure-logging-alb.md).
...
&emsp;&emsp; `rule_name` | Имя правила {{ sws-name }}.
&emsp;&emsp; `rule_type` | Тип правила {{ sws-name }}.
&emsp;&emsp; `verdict` | Решение по правилу, например `ALLOW`, `DENY` или `CAPTCHA`.
&ensp; `matched_rule` | Правило {{ sws-name }}, по которому принято решение об обработке запроса.
&emsp;&emsp; `dry_run` | Признак режима `Только логирование`.
&emsp;&emsp; `rule_name` | Имя правила {{ sws-name }}.
&emsp;&emsp; `rule_type` | Тип правила {{ sws-name }}.
&emsp;&emsp; `verdict` | Решение по правилу, например `ALLOW`, `DENY` или `CAPTCHA`.
&ensp; `profile_id` | Идентификатор профиля безопасности {{ sws-name }}.
&ensp; `unique_key` | Уникальный ключ запроса в {{ sws-name }}.
