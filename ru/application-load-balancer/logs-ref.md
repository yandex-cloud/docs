...
`time` | Дата и время запроса — получения балансировщиком первого байта от клиента. Записывается согласно [ISO 8601]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/ISO_8601){% endif %}{% if lang == "en" %}(https://en.wikipedia.org/wiki/ISO_8601){% endif %} в формате: `<YYYY>-<MM>-<DD>T<hh>:<mm>:<ss>Z`, где `Z` — время по Гринвичу, UTC.
...
`request_processing_times` | Информация о продолжительности обработки запроса.
