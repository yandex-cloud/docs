...
Сценарий описывает запись [логов](../concepts/application-load-balancer.md#logging) — сообщений с информацией о каждом входящем запросе к балансировщику {{ alb-full-name }} — в базу данных {{ PG }}.

Для логирования работы балансировщика автоматически создается [лог-группа](../../functions/concepts/log-group.md) в {{ cloud-logs-full-name }}. Поставка логов из этой лог-группы в БД будет настроена в сценарии с помощью ресурсов сервиса {{ sf-full-name }} — [триггера](../../functions/concepts/trigger/cloudlogs-trigger.md) и вызываемой им [функции](../../functions/concepts/function.md).

БД будет создана в сценарии с помощью сервиса {{ mpg-full-name }}.

{% note info %}

Некоторые шаги сценария нельзя выполнить в [консоли управления]({{ link-console-main }}). [Получить идентификатор лог-группы](#get-log-group-id) и [создать триггер](#set-up-sf-create-trigger) с этим идентификатором можно только через интерфейс командной строки (CLI) или API.
...
* разрешают балансировщику отправлять логи в {{ cloud-logs-name }}, а кластеру {{ PG }} — получать их из {{ cloud-logs-name }}.
...
- Консоль управления
  ...
  1. В [консоли управления]({{ link-console-main }}) откройте сервис **{{ mpg-name }}**.
...
В качестве веб-серверов для сайта будет выступать [группа виртуальных машин](../../compute/concepts/instance-groups/index.md) {{ compute-name }}. В этом сценарии серверы будут реализованы на LEMP-стеке (Linux, nginx, MySQL, PHP; подробнее см. в сценарии использования [{#T}](../../solutions/web/lamp-lemp.md)).
...
- Консоль управления
  ...
  1. В блоке **Сетевые настройки** выберите **Cеть** `alb-logging-network`, [созданную ранее](#create-network), и ее подсети.
...
Вместе с балансировщиком создается [лог-группа](../../functions/concepts/log-group.md), в которую записываются логи. Идентификатор лог-группы нужно будет использовать при создании триггера для {{ cloud-logs-name }} в {{ sf-name }}.
...
- API
  ...
  Используйте вызов gRPC API [LoadBalancerService/Get](../api-ref/grpc/load_balancer_service.md#Get) или метод REST API [get](../api-ref/LoadBalancer/get.md). Идентификатор лог-группы будет указан в поле `log_group_id`.
...
- Консоль управления 
  ...
  1. В [консоли управления]({{ link-console-main }}) откройте сервис **{{ sf-name }}**.
...
### Создайте триггер для {{ cloud-logs-name }} {#set-up-sf-create-trigger}
