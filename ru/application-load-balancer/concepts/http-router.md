...
* передачу запроса в [группу бэкендов](backend-group.md) типа **HTTP** или **gRPC** для обработки. В этом случае есть возможность настроить таймауты на обработку запроса, поддержку WebSocket или изменение URI перед передачей запроса в бэкенды.