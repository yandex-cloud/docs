...
{{ alb-name }} поддерживает [взаимную TLS-аутентификацию](https://en.wikipedia.org/wiki/Mutual_authentication#mTLS) (mTLS) для HTTPS-обработчиков. При использовании mTLS клиент проверяет сертификат балансировщика, а балансировщик — сертификат клиента, прежде чем принять трафик.
