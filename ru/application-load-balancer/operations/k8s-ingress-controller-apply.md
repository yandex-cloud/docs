# Создать или изменить ресурсы {{ alb-name }} по конфигурации для Ingress-контроллера в {{ managed-k8s-name }}

[Ingress-контроллер {{ alb-name }} для {{ managed-k8s-full-name }}](../tools/k8s-ingress-controller/index.md) управляет ресурсами {{ alb-name }} по вашей конфигурации — файлам в формате YAML с описаниями ресурсов {{ k8s }}: `Ingress`, `Service`, `HttpBackendGroup` и т. д. Подробнее о конфигурации см. в [справочнике Ingress-контроллера](../k8s-ref/index.md).
