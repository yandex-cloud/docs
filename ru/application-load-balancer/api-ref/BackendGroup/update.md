...
stream | **object**<br>New list of stream (TCP) backends that the backend group will consist of. <br> includes only one of the fields `http`, `grpc`, `stream`<br><br><p>A stream (TCP) backend group resource.</p> 
stream.<br>backends[] | **object**<br>New settings for the Stream backend.<br><p>A stream (TCP) backend resource.</p> 
