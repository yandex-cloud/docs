---
editable: false
---

# Правила тарификации для {{ alb-full-name }}

На стадии [Preview](../overview/concepts/launch-stages.md) использование сервиса {{ alb-name }} не тарифицируется.

После выхода общедоступной версии сервиса (перехода на стадию General Availability) начнется учет потребляемых ресурсов, но до **23 сентября 2021 года** включительно потребление ресурсов {{ alb-name }} не будет тарифицироваться. С **24 сентября 2021 года** потребление станет платным.

## Из чего складывается стоимость использования {{ alb-name }} {#rules}

При работе с сервисом {{ alb-name }} вы платите за фактическое использование вычислительных ресурсов каждого активного балансировщика. Использование сервиса тарифицируется почасно.

Использование балансировщика измеряется в _ресурсных единицах_. Одна ресурсная единица включает в себя:

* 1000 запросов в секунду (RPS);
* 4000 одновременно активных соединений;
* 200 новых соединений в секунду;
* 22 МБ (176 Мбит) трафика в секунду.

Количество ресурсных единиц за час работы балансировщика рассчитывается по показателю с наибольшим потреблением относительно порога. При расчете количество единиц округляется до большего целого значения.

Для каждой зоны доступности количество ресурсных единиц вычисляется отдельно. Минимальное количество единиц в час для одной зоны — 2. Работа балансировщика в зонах, где прием трафика отключен, не тарифицируется.

При расчете количества ресурсных единиц используются максимальные значения показателей за час.

### Пример расчета стоимости {#example}

Балансировщик, размещенный в одной зоне доступности, работал в течение часа со следующими показателями:

* 1500 RPS;
* 6000 одновременно активных соединений;
* 50 новых соединений в секунду;
* 2 МБ трафика в секунду.

Расчет стоимости за этот час и за месяц, состоящий из 720 часов с такими же показателями: 

> RPS: 1500 / 1000 = 1,5 ~ 2
> Активные соединения: 6000 / 4000 = 1,5 ~ 2
> Новые соединения: 50 / 200 = 0,25 ~ 1
> Трафик: 2 / 22 = 0,0909... ~ 1
> Минимальное количество ресурсных единиц в зоне: 2
>
> Количество ресурсных единиц: 2
>
> {% if region == "ru" %}2 × 1,3889 = 2,7778 ₽
>
> Итого за час: 2,7778 ₽
> Итого за месяц: 2,7778 × 720 = 2000,0016 ₽{% endif %}
> {% if region == "kz" %}2 × 8,3334 = 16,6668 ₸
>
> Итого за час: 16,6668 ₸
> Итого за месяц: 16,6668 × 720 = 12 000,096 ₸{% endif %}
> {% if region == "int" %}2 × 0,017806 = 0,035612 $
>
> Итого за час: 0,035612 $
> Итого за месяц: 0,035612 × 720 = 25,64064 ${% endif %}

Здесь {% if region == "ru" %}1,3889 ₽{% endif %}{% if region == "kz" %}8,3334 ₸{% endif %}{% if region == "int" %}0,017806 ${% endif %} — стоимость одной ресурсной единицы.

## Цены {#prices}

{% if region == "ru" %}

{% include [rub-lcu.md](../_pricing/application-load-balancer/rub-lcu.md) %}

{% endif %}

{% if region == "kz" %}

{% include [kzt-lcu.md](../_pricing/application-load-balancer/kzt-lcu.md) %}

{% endif %}

{% if region == "int" %}

{% include [usd-lcu.md](../_pricing/application-load-balancer/usd-lcu.md) %}

{% endif %}