...
Ресурс `Service` определяет [сервис {{ k8s }}](../../managed-kubernetes/concepts/index.md#service). {% if product == "yandex-cloud" %}Для инструментов {{ alb-name }} для {{ managed-k8s-name }} — [Ingress-контроллера](../tools/k8s-ingress-controller/index.md) и [Gateway API](../tools/k8s-gateway-api/index.md) —{% endif %}{% if product == "cloud-il" %}Для [Ingress-контроллера](../tools/k8s-ingress-controller/index.md) {{ alb-name }} для {{ managed-k8s-name }}{% endif %} сервисы представляют собой бэкенды, между которыми распределяется входящий трафик. 
