...
К хостам кластера {{ mos-name }} с [ролью](../concepts/hosts-roles.md#data) `DATA` можно подключиться:
...
* Добавьте путь к SSL-сертификату в конфигурацию вашего приложения для работы с {% if lang == "ru" and audience != "internal" %}[API](../../glossary/rest-api.md){% else %}API{% endif %}.
