...
В кластерах с несколькими хостами `DASHBOARDS` особый FQDN может некоторое время (до 10 минут) указывать на недоступный хост. Это связано с тем, что на обновление DNS-записи для особого FQDN требуется время. Если запрос завершился ошибкой, повторите его позднее.
