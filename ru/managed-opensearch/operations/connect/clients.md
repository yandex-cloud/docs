...
В этом разделе представлены настройки для подключения к хостам кластера {{ mos-name }} с помощью [инструментов командной строки](#command-line-tools), [{{ OS }} Dashboards](#dashboards) и из [Docker-контейнера](#connection-docker). Примеры подключения из приложения — в разделе [Примеры кода](./code-examples.md).
