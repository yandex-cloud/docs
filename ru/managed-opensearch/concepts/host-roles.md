...
Для обеспечения отказоустойчивости создать кластер только из двух хостов с ролью `DATA` невозможно. Для этого добавьте группу хостов типа `{{ OS }}` с ролью `MANAGER`.
