...
Каждая версия {{ ES }}, поддержка которой была включена в {{ mes-name }}, остается доступной, пока она поддерживаются производителем, как правило — 18 месяцев с даты ее выпуска. Детальную информацию о сроках прекращения поддержки отдельных версий см. в [календаре производителя](https://www.elastic.co/support/eol).
