...
С точки зрения {{ OS }}, документ — это набор полей, где каждое поле — это пара `ключ: значение`. Индекс хранит документ в оптимизированном виде, чтобы обеспечить возможность быстрого поиска по полям в документе. Оптимизация достигается за счет того, что каждое поле документа имеет определенный тип — это позволяет эффективно хранить данные этого поля в индексе. Подробнее о такой оптимизации в [документации {{ OS }}]({{ os.docs }}/opensearch/mappings/).
...
Подробнее об устройстве индексов в [документации {{ OS }}]({{ os.docs }}/opensearch/index-data/).
...
_Кодеки_ индексов определяют способ сжатия и хранения полей индекса на диске. Список поддерживаемых кодеков для {{ OS }} в [документации {{ OS }}]({{ os.docs }}/im-plugin/index-codecs/#supported-codecs).
