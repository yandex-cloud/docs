...
* _Действия_, которые нужно выполнить при переходе индекса в определенное состояние. Действия выполняются в том порядке, в котором они указаны.
    ...
    В {{ mos-name }} в дополнение к [стандартным действиям {{ OS }}]({{ os.docs }}/im-plugin/ism/policies/#ism-supported-operations) возможно также [изменение кодека индекса](indexing.md#codec-change).
...
Подробнее о политиках в [документации {{ OS }}]({{ os.docs }}/im-plugin/ism/policies/).
