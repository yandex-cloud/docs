...
* Стандартные локальные диски (`local-hdd`) — более медленные, но вместительные диски. Доступны только для хостов с процессорами на платформах Broadwell или Cascade Lake и имеющих не меньше восьми vCPU. `local-hdd` имеет фиксированный размер: 10240 ГБ для Broadwell и 12800 ГБ для Cascade Lake.

{% endif %}

{% if product == "cloud-il" %}

При создании кластера вы используете хранилище на локальных SSD-дисках (`local-ssd`). Размер такого хранилища — от 10 до 2048 ГБ.
