...
{{ mmg-short-name }} берет на себя трудоемкие задачи администрирования инфраструктуры {{ mmg-short-name }}:
...
{{ mmg-short-name }} доступен всем зарегистрированным пользователям Яндекс.Облака.
...
Для хранилища на NVMe-дисках количество хостов ограничено при создании: для {{ MG }}-кластеров можно создать не меньше 3 хостов.
...
{{ mmg-short-name }} поддерживает {{ MG }} версий 3.6 и 4.0.
