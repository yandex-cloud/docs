...
- Консоль управления
  ...
  1. Укажите параметры хоста:
      ...
      * зону доступности;
      * подсеть (если нужной подсети в списке нет, {% if audience != "internal" %}[создайте ее](../../vpc/operations/subnet-create.md){% else %}создайте ее{% endif %});
...
- CLI
  ...
  1. Запросите список подсетей кластера, чтобы выбрать подсеть для нового хоста:
     ...
     Если нужной подсети в списке нет, {% if audience != "internal" %}[создайте ее](../../vpc/operations/subnet-create.md){% else %}создайте ее{% endif %}.
...
1. Хост перестает принимать запросы на запись. Если хост был первичной репликой (`PRIMARY`), то {{ mmg-short-name }} попытается [сделать его вторичной репликой](https://docs.mongodb.com/manual/reference/method/rs.stepDown/#rs.stepDown) (`SECONDARY`).  В случае неудачи операция прерывается.

1. Инстанс MongoDB на хосте останавливается, и все данные удаляются.

1. Инстанс MongoDB снова запускается и заново скачивает данные с хостов-реплик.
...
Вы можете вручную перезагрузить хост кластера {{ mmg-short-name }}.
