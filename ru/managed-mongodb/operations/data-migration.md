...
Чтобы перенести существующую базу данных MongoDB в сервис {{ mmg-name }}, используйте утилиты `mongodump` и `mongorestore`: создайте дамп рабочей базы и восстановите его в {{ MG }}-кластере Облака.
...
2. Создать виртуальную машину в {{ compute-name }}, чтобы восстанавливать базу из дампа в инфраструктуре Яндекс.Облака (опционально).
3. Создать кластер {{ mmg-name }}, на котором будет развернута восстановленная база.
...
* Ваше оборудование или соединение с кластером {{ mmg-name }} недостаточно надежны.
...
1. В консоли управления [создайте новую виртуальную машину](../../compute/operations/vm-create/create-linux-vm.md) из образа Ubuntu 18.04. Нужное количество оперативной памяти и ядер процессора зависит от объема переносимых данных и требуемой скорости переноса.
   ...
   Виртуальная машина должна находиться в той же сети и зоне доступности, что хост-мастер кластера {{ mmg-name }}. Кроме того, виртуальной машине должен быть присвоен внешний IP-адрес, чтобы вы могли загрузить файл дампа извне Облака.

2. Установите клиент {{ MG }} и дополнительные утилиты для работы с СУБД:
...
## Создать кластер {{ mmg-name }} {#create-cluster}
