...
Чтобы перенести вашу базу данных в сервис {{ mmg-name }}, нужно непосредственно перенести данные, закрыть старую базу данных на запись и перенести нагрузку на кластер БД в Яндекс.Облаке.
...
2. При необходимости [создайте виртуальную машину](#create-vm) в {{ compute-name }}, чтобы восстанавливать базу из дампа в инфраструктуре Яндекс.Облака.
3. [Создайте кластер {{ mmg-name }}](#create-cluster), на котором будет развернута восстановленная база.
