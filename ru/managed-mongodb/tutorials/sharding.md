...
Шардирование кластера {{ mmg-name }} происходит с автоматическим созданием служебных хостов{% if audience != "internal" %}, которые [тарифицируются](../pricing.md) отдельно от основных хостов СУБД{% endif %}:
