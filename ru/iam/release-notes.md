...
* В вывод команды `yc iam api-key list` добавлено поле `MASKED KEY`, а в ответ методов [REST API](../iam/api-ref/ApiKey/list.md) и [gRPC API](../iam/api-ref/grpc/ApiKey/list.md) — поле `maskedSecret`. Оба поля содержат последние 6 символов секретной части ключа. {{ tag-cli }} {{ tag-api }}
