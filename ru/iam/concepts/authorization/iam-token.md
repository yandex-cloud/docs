...
Если вы получаете IAM-токен с помощью сервиса метаданных [изнутри виртуальной машины](../../../compute/operations/vm-connect/auth-inside-vm.md), то его время жизни может быть меньше, так как сервис метаданных не создает новый токен, а возвращает последний созданный. Новые токены создаются периодически.

Вместе с IAM-токеном сервис метаданных возвращает оставшееся время жизни токена. Учитывайте время жизни IAM-токена или запрашивайте токен чаще, например каждый час или при каждой операции.
