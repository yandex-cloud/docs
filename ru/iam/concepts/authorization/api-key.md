...
* `yc.monium.logs.write` — для записи [логов](../../../monium/logs/quickstart.md) в [{{ monium-name }}](../../../monium/index.yaml).
* `yc.monium.metrics.write` — для записи [метрик](../../../monium/metrics/overview.md) в [{{ monium-name }}](../../../monium/index.yaml).
* `yc.monium.traces.write` — для записи [распределенных трассировок](../../../monium/traces/index.md) в [{{ monium-name }}](../../../monium/index.yaml).
* `yc.monium.telemetry.write` — для записи любого вида телеметрии по протоколу [OTLP](https://opentelemetry.io/docs/specs/otlp/) (OpenTelemetry Protocol) в [{{ monium-name }}](../../../monium/index.yaml).
* `yc.postbox.send` — для отправки писем через API [{{ postbox-name }}](../../../postbox/index.yaml).
* `yc.cloud-registry.artifacts.manage` — для просмотра, публикации и удаления артефактов из [{{ cloud-registry-full-name }}](../../../cloud-registry/index.yaml).
* `yc.cloud-registry.artifacts.pull` — для просмотра и скачивания артефактов из [{{ cloud-registry-full-name }}](../../../cloud-registry/index.yaml).
* `yc.cloud-registry.artifacts.push` — для просмотра и публикации артефактов в [{{ cloud-registry-full-name }}](../../../cloud-registry/index.yaml).
...
При [просмотре списка](../../operations/authentication/manage-api-keys.md#list-api-keys) API-ключей отображаются последние шесть символов секретной части ключа. В {{ yandex-cloud }} CLI они показаны в поле `MASKED KEY`, в API — в поле `maskedSecret`, а в консоли управления — в столбце **{{ ui-key.yacloud.iam.folder.service-account.overview.column_key_masked-secret }}**.
