...
Если ключ утрачен или поврежден, восстановить его нельзя. В этом случае вы можете {% if product == "yandex-cloud" %}[перевыпустить](../../operations/compromised-credentials.md#api-key-reissue){% else %}перевыпустить{% endif %} ключ или создать новый.
