...
На этой странице можно узнать как вы управляете доступом к ресурсам и как [!KEYREF iam-short-name] проверяет права доступа к ним.

## Как проверяются права доступа?

Все операции с ресурсами в Яндекс.Облаке предварительно отправляются на проверку в [!KEYREF iam-short-name], например:

1. Пользователь просит сервис Compute Cloud создать новый диск в каталоге <q>default</q>.
1. Сервис спрашивает [!KEYREF iam-short-name], можно ли этому пользователю создать диск в этом каталоге.
1. [!KEYREF iam-short-name] проверяет, что пользователь — участник облака с каталогом <q>default</q> и имеет необходимые разрешения для создания диска в этом каталоге.
1. Если какого-то из разрешений у пользователя нет, операция не будет выполнена, и Яндекс.Облако сообщит об ошибке.
    Если все разрешения имеются, то [!KEYREF iam-short-name] сообщает об этом сервису.
...
О том, какие есть роли и какие разрешения в них входят, читайте в разделе [[!TITLE]](roles.md).
...
* `system` — [системная группа](system-group.md). Сейчас есть одна системная группа: `[!KEYREF subjects-allAuthenticatedUsers]` — все пользователи, которые зарегистрированы в Яндекс.Облаке и прошли [авторизацию](../authorization/).
...
Вы можете найти подробную информацию об управлении доступом для конкретного сервиса Яндекс.Облака в разделе <q>Безопасность и управление доступом</q> в документации соответствующего сервиса.
