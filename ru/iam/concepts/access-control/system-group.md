...
`allAuthenticatedUsers` — все пользователи, прошедшие [аутентификацию](../authorization/index.md). Это все зарегистрированные пользователи или сервисные аккаунты Яндекс.Облака: как из ваших облаков, так и из облаков других пользователей.

> Например, у вас есть образ диска с операционной системой и вы хотите поделиться им со всеми пользователями Яндекс.Облака. Для этого назначьте субъекту `allAuthenticatedUsers` роль `compute.images.user` на каталог с образом.

{% note alert %}

Назначение роли системной группе `allAuthenticatedUsers` открывает публичный доступ к вашим ресурсам. Данная роль дает права на ваши ресурсы **всем** пользователям, прошедшим аутентификацию в Яндекс.Облаке, а **не только пользователям из вашего облака**.
