...
Для выполнения операций в Яндекс.Облаке через API необходим IAM-токен. Чтобы получить [IAM-токен](../../concepts/authorization/iam-token.md) для сервисного аккаунта:
