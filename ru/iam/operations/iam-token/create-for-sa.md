...
* `kid` — идентификатор открытого ключа (ресурса `Key`), который был создан на [предыдущем шаге](#keys-create). Ключ должен принадлежать сервисному аккаунту, для которого запрашивается IAM-токен.
...
Создайте подпись, зашифровав строку из header и payload (в формате Base64Url), разделенных точкой (`.`):
