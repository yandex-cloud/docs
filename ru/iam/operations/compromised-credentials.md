...
{{ yandex-cloud }} автоматически [ищет секреты в открытом доступе](../../security/operations/search-secrets.md). В случае обнаружения секрета в открытом доступе владельцу организации или пользователю, который в соответствии с настройками организации принимает оповещения от сервиса {{ iam-name }}, придет письмо с адреса технической поддержки {{ yandex-cloud }}. Чтобы обеспечить безопасность ваших данных и инфраструктуры, внимательно следите за использованием секретов.
...
Если вам нужно максимально быстро пресечь угрозы со стороны скомпрометированного ключа, [заблокируйте](sa/suspend-reactivate.md#suspend) или [удалите](sa/delete.md) сервисный аккаунт.
