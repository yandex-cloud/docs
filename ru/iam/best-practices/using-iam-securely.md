...
* Назначайте минимально необходимые для работы роли. Например, чтобы в {{ compute-name }} разрешить использовать образы для создания виртуальных машин, [назначьте роль](../operations/roles/grant.md) `compute.images.user`, а не `editor` или выше.
* Старайтесь назначать [сервисные роли](../concepts/access-control/roles.md#service-roles) вместо [примитивных](../concepts/access-control/roles.md#primitive-roles) (`viewer`, `editor`, `admin`). Примитивные роли действуют для ресурсов любого из сервисов Яндекс.Облака.
...
## Защитите свой аккаунт на Яндексе {#safe-your-account}
