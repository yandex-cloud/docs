...
При работе через API вы будете использовать [!KEYREF iam-short-name] для получения IAM-токена. Этот токен необходим для авторизации через API.

Другие сервисы Яндекс.Облака используют API сервиса [!KEYREF iam-short-name], чтобы предоставить вам больше возможностей для управления доступом к ресурсам этих сервисов. Например, [!KEYREF compute-full-name] предоставляет дополнительную роль `compute.images.user` для контроля доступа к образам дисков.
...
В разделе [[!TITLE]](../quickstart.md) узнайте, как добавить нового пользователя в свое облако и назначить ему роль.
