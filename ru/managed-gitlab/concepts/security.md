...
{{ mgl-name }} предоставляет защиту вашего pipeline встроенными в {{ GL }} средствами.

Набор средств зависит от [типа лицензии {{ GL }}](https://about.gitlab.com/pricing/). В бесплатную лицензию входит ряд средств защиты, например, [Static Application Security Testing (SAST)](https://docs.gitlab.com/ee/user/application_security/sast/) и [Secret Detection](https://docs.gitlab.com/ee/user/application_security/secret_detection/index.html). Также вы можете разрабатывать собственные джобы на базе open source-инструментов.

Полный список средств защиты pipeline в зависимости от типа лицензии приведен в таблице:

Средства защиты pipeline | Free | Premium | Ultimate
--- | :---: | :---: | :---:
...
## Использование примеров безопасности pipeline {#security-pipeline-usage}

Доступны следующие варианты использования pipeline в ваших проектах:
* Создайте pipeline в отдельном проекте и подключите его к другим проектам с помощью [функции `include`](https://docs.gitlab.com/ee/ci/yaml/includes.html). Доступно для всех типов лицензий.
...
* Скопируйте секции pipeline в файл `.gitlab-ci.yml` ваших проектов.

## Дополнительные материалы {#additional-content}

Изучите примеры безопасности pipeline, подготовленные в рамках [{{ yandex-cloud }} Security Solution Library](https://github.com/yandex-cloud/yc-solution-library-for-security):
...
* Для безопасной интеграции используйте [Gitlab Agent for {{ k8s }}](https://docs.gitlab.com/ee/user/clusters/agent/).
...
* Чтобы исключить связанность {{ GL }} Runner и {{ k8s }}, используйте деплой с помощью [CI/CD tunnel](https://docs.gitlab.com/ee/user/clusters/agent/ci_cd_tunnel.html).

### Использование переменных {#using-env-vars}

* Чтобы ограничить доступ к переменным [используйте настройку `Protect variable`](https://docs.gitlab.com/ee/ci/variables/#protect-a-cicd-variable).
...
* По возможности отключите возможность `fork`.
...
### Безопасная конфигурация {{ GL }} Runners {#configuring-runners}
...
* Чтобы ограничить сетевой доступ к {{ GL }} Runners, используйте [группы безопасности](../../vpc/concepts/security-groups.md).
* Используйте безопасный механизм [назначения сервисных аккаунтов виртуальной машине](../../compute/operations/vm-connect/auth-inside-vm.md#link-sa-with-instance) для взаимодействия с облачным API изнутри джобов. Такой способ более безопасен, чем указание учетных данных через `env`.