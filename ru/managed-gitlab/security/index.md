...
Чтобы пользоваться сервисом, необходима {% if audience != "internal" %}[роль](../../iam/concepts/access-control/roles.md){% else %}роль{% endif %} `{{ roles.gitlab.editor }}` или выше на каталог, в котором создаются проекты. Роль `{{ roles.gitlab.viewer }}` позволит только просматривать список проектов и содержимое файлов, которые были загружены.
