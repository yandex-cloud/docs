---
title: Назначение правил доступа к таблицам в {{ mtr-full-name }}
description: Правила определяют, какие действия пользователи могут совершать над таблицами и представлениями в {{ mtr-name }}.
---

# Назначение правил доступа к таблицам в {{ mtr-name }}

Правила доступа к таблицам определяют, какие действия пользователи могут совершать над таблицами и представлениями (`VIEW`) в кластере {{ mtr-name }}. Чтобы правило применялось к представлению, укажите его имя в поле имени таблицы, а также соответствующие каталог и схему.
  ...
  1. В открывшемся окне задайте параметры правила:
     ...
     1. (Опционально) В поле **{{ ui-key.yacloud.trino.ClusterForm.label_table-privileges_2ikK4 }}** выберите разрешенные действия над таблицами:
        ...
        * `GRANT_SELECT` — использование данных таблицы или представления в представлениях с `SECURITY DEFINER`, которые читают другие пользователи.
        ...
        {% include notitle [view-creation](../../_includes/managed-trino/access-control-src.md#view-creation) %}
