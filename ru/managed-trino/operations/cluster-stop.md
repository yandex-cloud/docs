---
title: Остановка и запуск кластера {{ TR }}
description: 'При необходимости вы можете остановить кластер {{ TR }} и запустить его заново. При остановке кластера все данные в нем сохранятся: они снова станут доступны, когда вы запустите кластер.'
---

# Остановка и запуск кластера {{ TR }}

При необходимости вы можете остановить кластер {{ TR }} и запустить его заново. При остановке кластера все данные в нем сохранятся: они снова станут доступны, когда вы запустите кластер.
...
- CLI {#cli}
    ...
    Чтобы остановить кластер {{ TR }}, выполните команду:

    ```bash
    {{ yc-mdb-tr }} cluster stop <имя_или_идентификатор_кластера>
    ```
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [Cluster.stop](../api-ref/Cluster/stop.md) и выполните запрос, например, с помощью {{ api-examples.rest.tool }}:
        ...
        curl \
            ...
            --url 'https://{{ api-host-trino }}/managed-trino/v1/clusters/<идентификатор_кластера>:stop'
        ```
...
* CLI {#cli}
    ...
    Чтобы запустить кластер {{ TR }}, выполните команду:

    ```bash
    {{ yc-mdb-tr }} cluster start <имя_или_идентификатор_кластера>
    ```
...
* REST API {#api}
    ...
    1. Воспользуйтесь методом [Cluster.start](../api-ref/Cluster/start.md) и выполните запрос, например, с помощью {{ api-examples.rest.tool }}:
        ...
        curl \
            --request POST \
            --header "Authorization: Bearer $IAM_TOKEN" \
            --url 'https://{{ api-host-trino }}/managed-trino/v1/clusters/<идентификатор_кластера>:start'
        ```
