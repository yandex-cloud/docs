...
- REST API {#api}
  ...
  1. Создайте файл `body.json` и добавьте в него следующее содержимое:
      ...
      * `procedure` — процедуры, на которые распространяется правило. Если блок `procedure` не указан, правило распространяется на все процедуры.
        * `names` — список имен процедур.
        * `nameRegexp` — регулярное выражение. Правило распространяется на процедуры, имена которых соответствуют регулярному выражению.

        Блок `procedure` должен содержать либо вложенный блок `names`, либо параметр `nameRegexp`.
