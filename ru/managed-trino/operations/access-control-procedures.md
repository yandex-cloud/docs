  ...
  1. В открывшемся окне задайте параметры правила:
     ...
     1. (Опционально) Задайте полные имена процедур, к которым будет применяться правило, в формате `<имя_каталога>.<имя_схемы>.<имя_процедуры>`. Каждая часть имени задается с помощью отдельного поля.
        ...
        1. Выберите, в каком формате указать процедуры:
           ...
           * **{{ ui-key.yacloud.trino.rbac-catalog-match-by-name-regexp }}** — введите регулярное выражение для имен процедур.
           * **{{ ui-key.yacloud.trino.rbac-catalog-match-by-empty }}** — в поле `<имя_процедуры>` может быть любое значение.
  ...
  1. Создайте файл `access_control.yaml` и добавьте в него следующее содержимое:
     ...
     * `catalog`, `schema` и `procedure` — совместно определяют процедуры, к которым будет применяться правило. Каждый из параметров является опциональным.
       ...
       * `procedure` — процедуры, задаваемые одним из параметров:
       ...
       В {{ mtr-name }} полное имя процедуры формируется по шаблону `<имя_каталога>.<имя_схемы>.<имя_процедуры>`. Правило применяется к процедуре, только если ее полное имя соответствует всем заданным параметрам.
