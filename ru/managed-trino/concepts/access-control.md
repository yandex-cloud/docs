...
* Объекты кластера {{ mtr-name }}, к которым применяется правило.
* Привилегии пользователей — какие действия разрешено выполнять пользователям над объектами.

В параметре `users` указываются идентификаторы пользователей {{ iam-name }}, а в параметре `groups` — идентификаторы групп, а не логины пользователей или названия групп. Получить эти значения можно с помощью [SQL-запросов](#check-identity).
...
Режим проверки прав на таблицы-источники задается в команде [`CREATE VIEW`]({{ tr.docs }}/sql/create-view.html#security):
...
В {{ mtr-name }} при проверке доступа к таблицам-источникам в режиме `SECURITY DEFINER` группы владельца представления не учитываются. Если доступ выдан владельцу только через группы, чтение представления может завершиться ошибкой `Access Denied`, в том числе при запросе от самого владельца.
...
Чтобы узнать, какие идентификаторы {{ TR }} использует при проверке правил доступа, в сессии с ошибкой выполните запросы:
