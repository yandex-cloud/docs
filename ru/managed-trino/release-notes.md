...
* [Доступен новый коннектор](concepts/greenplum-connector.md): {{ GP }}/Cloudberry.
...
* Добавлена возможность [настраивать группы безопасности](operations/connect.md#configuring-security-groups) для подключения к кластеру, в том числе при работе с коннектором {{ GP }}.
...
* В консоли управления обновлен интерфейс для выбора подключений через {{ connection-manager-name }}.
