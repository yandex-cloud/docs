...
## HTTP-запрос
`POST /resource-manager/v1/folders`
...
Поле | Описание
--- | ---
cloudId | **string**<br>Обязательное поле. Идентификатор облака, в котором создается каталог. Чтобы получить идентификатор облака, используйте запрос [list](/docs/resource-manager/api-ref/Cloud/list).  Максимальная длина — 50 символов.
