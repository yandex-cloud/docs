...
## Как назначить роль на каталог {#access-to-user}
...
5. Выберите необходимую роль из списка. При необходимости
...
5. Назначьте пользователю роль `editor` на каталог `my-folder`. В субъекте укажите тип `userAccount` и ID пользователя:
...
Чтобы назначить роль другому пользователю, выберите пользователя на вкладке [Пользователи и роли](https://console.cloud.yandex.ru/iam) и нажмите кнопку **Настроить роли**.
...
Разрешите сервисному аккаунту `test-sa` управлять каталогом `my-folder` и ресурсами в нем:
...
2. Назначьте сервисному аккаунту `test-sa` роль `editor` на каталог `my-folder`. В свойстве `subject` укажите тип `serviceAccount` и ID `test-sa`. В URL запроса в качестве ресурса укажите ID `my-folder`:
