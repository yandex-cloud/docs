...
Удаление каталога может быть запрещено, если для этого каталога (либо для [облака](../../concepts/resources-hierarchy.md#cloud) или [организации](*organization), к которым этот каталог относится) создана [политика авторизации](*access_policies) `resourceManager.denyFolderRemoval`. Такой запрет действует даже в том случае, если пользователю назначена [роль](*roles), разрешающая удаление каталогов.
...
- {{ TF }} {#tf}
  ...
  1. Откройте файл конфигурации {{ TF }} и удалите фрагмент с описанием каталога.
      ...
      Подробнее о параметрах ресурса `yandex_resourcemanager_folder` в {{ TF }} читайте в [документации провайдера]({{ tf-provider-resources-link }}/resourcemanager_folder).
...
[*organization]: Организация — это высший ресурс в иерархии ресурсной модели {{ yandex-cloud }}, который объединяет ресурсы всех остальных сервисов. Также организации используются для управления пользователями и параметрами их аутентификации и авторизации. Подробнее читайте в разделе [{#T}](../../../organization/concepts/organization.md).
