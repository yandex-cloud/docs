...
Роль участника облака необходима пользователю с аккаунтом {% if product == "yandex-cloud" %}на Яндексе{% endif %}{% if product == "cloud-il" %}Google{% endif %} для выполнения операций с ресурсом в облаке, которое не принадлежит организации. Исключение — ресурсы с [публичным доступом](#public-access).
...
Когда пользователь (субъект) выполняет какую-либо операцию с ресурсом, сервис {{ iam-full-name }} ({{ iam-short-name }}) проверяет наличие необходимых прав доступа у пользователя на этот ресурс.
