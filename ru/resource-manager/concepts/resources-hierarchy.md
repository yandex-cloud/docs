...
Чтобы сделать облако или ресурс публичным, необходимо назначить на него роль для [системной группы](../../iam/concepts/access-control/system-group.md) `{{ subjects-allAuthenticatedUsers }}`.

> Например, можно предоставить любому пользователю доступ на просмотр всех ресурсов в облаке. Для этого достаточно назначить на это облако роль `{{ roles-viewer }}` для субъекта `{{ subjects-allAuthenticatedUsers }}` .
