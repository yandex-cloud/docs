...
Высокая доступность кластера основывается на стандартной потоковой репликации {{ PG }} и собственном оркестраторе. В стандартном {{ PG }} полностью бесшовное переключение мастера невозможно, так как контекст активных транзакций и соединений не может быть перенесен между хостами. Однако оркестратор обеспечивает согласованность данных и предотвращает появление двух мастер-хостов в кластере.
...
В кластере можно комбинировать автоматическое и ручное управление потоками репликации.

### Автоматическое управление потоками репликации {#replication-auto}

После создания кластера {{ PG }} из нескольких хостов в нем находятся один хост-мастер и реплики. Реплики используют хост-мастер в качестве источника репликации. При смене мастера источник репликации для всех хостов-реплик автоматически переключается на новый хост-мастер. Подробнее о переключении мастера в разделе [Выбор мастера при переключении](#master-failover).


### Ручное управление потоками репликации {#replication-manual}

При ручном управлении источником репликации для реплики служит не хост-мастер, а другой хост в кластере. Если заданный вручную источник репликации станет недоступен, реплика автоматически переключится на актуальный мастер, при этом продолжит считаться репликой с вручную заданным источником репликации. После восстановления источника репликации реплика снова переключится на него.
...
* Выбираться как наименее отстающие при использовании [особого FQDN](../operations/connect/fqdn.md#fqdn-replica).
...
### Выбор мастера при переключении {#master-failover}

Новый мастер выбирается следующим образом:

* После технического обслуживания и операций изменения кластера, требующих перезапуска хостов, новым мастером становится хост с максимальным приоритетом. Этим хостом может быть и предыдущий мастер. Если несколько хостов имеют максимальный приоритет, выбирается хост с наименьшим отставанием от мастера.
* При выходе из строя хоста-мастера новым мастером становится хост с наименьшим отставанием от мастера. Если несколько хостов имеют наименьшее отставание, выбирается хост с максимальным приоритетом.

Задать приоритет хоста можно при [создании хоста](../operations/hosts.md#add) или [изменении его настроек](../operations/hosts.md#update).

Хост не может быть назначен новым мастером, если он:

* не входит в кворум;
* находится в состоянии, отличном от `ALIVE`;
* отстает от мастера более чем на 240 секунд;
* имеет неизвестное отставание от мастера.
