...
Для минимизации рисков недоступности кластера на запись рекомендуем использовать кластер {{ mpg-name }} с тремя и более хостами.
...
При [обновлении версии {{ PG }}](../operations/cluster-version-update.md) кластер становится недоступным на запись. Реплики поочередно становятся недоступными для чтения. То есть, минимум одна реплика всегда доступна для кластера с тремя хостами, две — для кластера с четырьмя хостами и так далее. Кластер, содержащий один или два хоста, полностью недоступен на время обновления версии {{ PG }}. Учитывайте это при планировании обновления версии кластера.
