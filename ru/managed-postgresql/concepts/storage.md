...
О том, как правильно выбрать тип дисков, читайте в [статье](/blog/posts/2025/04/vm-disks).
...
При заполнении хранилища более чем на 97% {{ mpg-name }} переводит хост в режим `read-only`. При этом для всех баз данных через запрос `ALTER DATABASE` выставляется настройка `DEFAULT_TRANSACTION_READ_ONLY = TRUE`.
