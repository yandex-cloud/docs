...
Вы можете отключить режим read-only вручную двумя способами:

1. Обратитесь в [техническую поддержку]({{ link-console-support}}).

1. [Подключитесь к БД](../operations/connect.md) любым удобным способом и выполните запрос:

    ```sql
    ALTER DATABASE <имя базы данных> SET DEFAULT_TRANSACTION_READ_ONLY TO FALSE;
    ```

После отключения режима read-only удалите ненужные данные. Используйте операторы `DROP` и `TRUNCATE`. При использовании оператора `DELETE` строки отмечаются как удаленные, но не удаляются из базы физически. Для освобождения места в хранилище после удаления строк с помощью `DELETE` выполните запрос для очистки базы:
