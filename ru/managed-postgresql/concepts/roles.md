...
Чтобы назначить роль пользователю, [используйте](../operations/grant.md) CLI или {% if lang == "ru" and audience != "internal" %}[API](../../glossary/rest-api.md){% else %}API{% endif %} {{ yandex-cloud }}: назначение роли запросом `GRANT` отменяется при следующей операции с базой.
