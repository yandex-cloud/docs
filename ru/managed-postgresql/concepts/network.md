...
description: "При создании {{ PG }}-кластера вы можете задать сеть для самого кластера, подсети для каждого из хостов кластера и запросить публичный доступ, чтобы подключаться к кластеру извне {{ yandex-cloud }}."
---
...
FQDN (имя) хоста можно запросить со [списком хостов в кластере](../operations/hosts.md#list).
