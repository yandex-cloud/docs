...
* В разделе [Диагностика производительности](operations/performance-diagnostics.md) добавлено графовое представление планов запросов. Еще в разделе теперь можно посмотреть **Дерево блокировок**. С помощью этой функции пользователь может увидеть, какие запросы блокируют СУБД.
* Переработана форма создания кластера: улучшен дизайн, расположение функций стало удобнее, в интерфейс добавлены всплывающие подсказки.В раздел **Ресурсы кластера** добавлена опция [Зашифровать диск пользовательским ключом KMS](../compute/concepts/encryption.md#user-keys).
* При создании кластера через Terraform добавлена возможность задавать каскадные реплики.
* При создании или редактировании настроек кластера в консоли управления добавлена возможность одной кнопкой экспортировать Terraform-код. Опция также работает для уже созданных ресурсов сервиса.
* В публичный API добавлен [метод вывода доступных версий СУБД](api-ref/Versions/list.md).
* Добавлена опция [Балансировщик нагрузки для хостов](concepts/load-balancer.md) на стадии Preview.
* В форме создания кластера увеличен максимальный размер диска для автомасштабирования.
* Добавлены [расширения](operations/extensions/cluster-extensions.md) `pg_textsearch`, `pg_clickhouse`, `ip4r`, `prefix`, `insert_username`, `tcn`, `tsm_system_rows`, `tsm_system_time`.
* Добавлена поддержка новых параметров настройки СУБД: `checkpoint_warning`, `max_active_replication_origins`, `pg_hint_plan.hints_anywhere`.
...
* Расширена информация о [техническом обслуживании](./concepts/maintenance.md): теперь отображается оценка влияния операции.
* Добавлена поддержка [политик резервного копирования](./concepts/backup.md) на уровне каталога.
* В настройках пользователя добавлен параметр `idleSessionTimeout` для ограничения времени жизни неактивных сессий.
