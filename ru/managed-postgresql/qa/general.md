...
#### Что такое [!KEYREF mpg-short-name]? {#what-is}

[!KEYREF mpg-short-name] — это сервис, который помогает вам создавать, эксплуатировать и масштабировать базы данных [!KEYREF PG] в облачной инфраструктуре.

С [!KEYREF mpg-short-name] вы можете:
...
[!KEYREF mpg-short-name] берет на себя трудоемкие задачи администрирования инфраструктуры [!KEYREF mpg-short-name]:
...
Вы взаимодействуете с кластером БД в [!KEYREF mpg-short-name] как с обычной базой данных в вашей локальной инфраструктуре. Благодаря этому вы можете управлять внутренними настройками БД в соответствии с требованиями вашего приложения.


#### Какую часть работы по управлению и сопровождению баз данных берет на себя [!KEYREF mpg-short-name]? {#services}

При создании кластеров [!KEYREF mpg-short-name] выделяет ресурсы, устанавливает СУБД и создает базы данных.

Для созданных и запущенных баз данных [!KEYREF mpg-short-name] автоматически создает резервные копии, а также устанавливает исправления и обновления СУБД.

Также [!KEYREF mpg-short-name] обеспечивает репликацию данных между хостами БД (как внутри, так и между зонами доступности) и автоматически переключает нагрузку на резервную реплику в случае аварии.

#### Для каких задач стоит использовать [!KEYREF mpg-short-name], а для каких виртуальные машины с базами данных? {#mdb-advantage}

Яндекс.Облако предлагает два варианта работы с базами данных:

- [!KEYREF mpg-short-name] позволяет вам эксплуатировать шаблонные базы данных, не заботясь об администрировании.
...
#### Как начать работу с [!KEYREF mpg-short-name]? {#quickstart}

[!KEYREF mpg-short-name] доступен всем зарегистрированным пользователям Яндекс.Облака.

Чтобы создать кластер базы данных в [!KEYREF mpg-short-name], необходимо определиться с его характеристиками:
...
Для хранилища на NVMe-дисках количество хостов ограничено при создании: для [!KEYREF PG]-кластеров можно создать не меньше 3 хостов.


#### Как получить доступ к запущенному хосту базы данных? {#db-access}

Вы можете подключаться к базам данных [!KEYREF mpg-short-name] способами, стандартными для СУБД.
...
Под обслуживанием в [!KEYREF mpg-short-name] понимается:

- автоматическая установка обновлений и исправлений СУБД для ваших хостов БД;
- изменение класса хостов и объема хранилища;
- другие сервисные работы [!KEYREF mpg-short-name].


#### Какую версию [!KEYREF PG] использует [!KEYREF mpg-short-name]? {#dbms-version}

[!KEYREF mpg-short-name] поддерживает [!KEYREF PG] 10.
...
Через месяц после того, как версия СУБД становится неподдерживаемой, [!KEYREF mpg-short-name] автоматически оповещает владельцев кластеров БД, созданных с этой версией, по электронной почте.
...
В [!KEYREF mpg-short-name] стоимость потребления рассчитывается исходя из следующих параметров:
