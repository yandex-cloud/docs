...
Старые версии данных не удаляются автоматически. Если они больше не нужны, то можно удалить их, чтобы освободить занятое ими хранилище и устранить раздувание (bloat). Для этого обычно используются команды [VACUUM FULL]({{ pg-docs }}/sql-vacuum.html) или [CLUSTER]({{ pg-docs }}/sql-cluster.html). Однако эти команды требуют эксклюзивной блокировки таблиц, что не всегда удобно или допустимо.
