...
Перед тем, как пытаться импортировать данные, проверьте, совпадают ли версии СУБД у существующей базы данных и вашего кластера в Облаке. Если версии разные, восстановить сделанный дамп не получится. Чтобы мигрировать на {{ PG }} версии 11 или 12 , можно использовать [логическую репликацию](#logical_replication).

Этапы миграции:

1. [Создайте дамп переносимой базы](#dump).
1. [(опционально) Создайте виртуальную машину в Яндекс.Облаке и загрузите дамп БД на нее](#create-vm).
...
То есть для восстановления дампа {{ PG }} 10, {{ PG }} 11 и {{ PG }} 12 следует использовать `pg_restore 10`, `pg_restore 11`, `pg_restore 12` соответственно.
