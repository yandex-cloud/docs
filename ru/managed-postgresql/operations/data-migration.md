...
[Создайте кластер {{ mpg-name }}](./cluster-create.md) любой подходящей конфигурации. При этом:
...
* Включите те же [расширения {{ PG }}](./cluster-extensions.md), что и в кластере-источнике.

## Перенос данных с использованием сервиса {{ data-transfer-full-name }} {#data-transfer}

1. [Подготовьте кластер-источник](../../data-transfer/operations/prepare.md#prepare-source-pg).
1. [Подготовьте кластер-приемник](../../data-transfer/operations/prepare.md#prepare-target-pg).
...
Таким образом, для восстановления дампа {{ PG }} 10, {{ PG }} 11, {{ PG }} 12 и {{ PG }} 13 используйте `pg_restore 10`, `pg_restore 11`, `pg_restore 12` и `pg_restore 13` соответственно.
