...
О том, как использовать сертификат с помощью `libpq`, читайте в [документации PostgreSQL](https://www.postgresql.org/docs/current/static/libpq-connect.html#LIBPQ-PARAMKEYWORDS).
...
* Для ОС, использующих RPM-пакеты, дистрибутив PostgreSQL доступен в [yum-репозитории](https://yum.postgresql.org/).
...
[pgx в Go](https://github.com/jackc/pgx)), вы можете подключаться на специальный хост вида `c-<идентификатор кластера>.rw.mdb.yandexcloud.net`.
