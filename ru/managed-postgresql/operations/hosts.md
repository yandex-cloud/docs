...
Для каждого хоста в {{ PG }}-кластере можно изменить приоритет хоста и указать источник репликации. Подробнее см. в разделе [{#T}](../concepts/replication.md).

{% list tabs %}

- Консоль управления

  Чтобы изменить параметры {{ PG }}-хоста:
  1. Перейдите на страницу каталога и выберите сервис **{{ mpg-name }}**.
...
- API
  ...
  Чтобы изменить параметры {{ PG }}-хоста, воспользуйтесь методом API [updateHosts](../api-ref/Cluster/updateHosts.md) и передайте в запросе:
  ...
  Имя хоста можно запросить со [списком хостов в кластере](hosts.md#list-hosts), идентификатор кластера — со [списком кластеров в каталоге](cluster-list.md#list-clusters).
