...
{{ mpg-name }} предоставляет следующие особые FQDN:
...
Алиасы позволяют обращаться к мастеру, наименее отстающей реплике, а также к каждому конкретному хосту кластера {{ mpg-name }} по стабильному человекочитаемому имени, независимо от внутренних FQDN. Алиасы задаются пользователем и могут использоваться наравне с обычными и [особыми FQDN](#special-fqdns).

### Алиас кластера {#cluster-alias}

Алиас кластера всегда указывает на текущий хост-мастер в кластере {{ mpg-name }}. Имеет следующий вид:

```
c-<краткое_имя>-pg-<идентификатор_каталога>.rw.{{ dns-zone }}
```
...
```
h-<краткое_имя_хоста>-pg-<идентификатор_каталога>.{{ dns-zone }}
```
