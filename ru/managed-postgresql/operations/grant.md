...
Чтобы назначить **привилегию** определенному пользователю, [создайте](cluster-users.md) его с помощью консоли управления, CLI или API {{ yandex-cloud }}, затем выполните команду `GRANT` от имени владельца базы, например:

```postgresql
GRANT SELECT ON DATABASE db1 TO user2;
```

Чтобы назначить **роль** пользователю, используйте CLI или API {{ yandex-cloud }}: назначение роли запросом `GRANT` отменяется при следующей операции с базой.
...
1. От лица владельца БД выдайте новому пользователю привилегии `SELECT` и, при необходимости, `USAGE` на нужную базу данных или таблицу, например:
