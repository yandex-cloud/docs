...
1. Создайте подписку со строкой подключения к кластеру-источнику:
    ...
    Подробнее о создании подписок смотрите в [документации {{ PG }}]({{ pg-docs }}/sql-createsubscription.html).
...
1. Создайте дамп с последовательностями:
    ...
    Подробнее о паттернах смотрите в [документации {{ PG }}]({{ pg-docs }}/app-psql.html#APP-PSQL-PATTERNS).
