...
* Консоль управления {#console}
   ...
   Изменить тип диска для хостов-брокеров можно только через CLI, {{ TF }} или API.
   ...
   {% endnote %}
    ...
    1. В блоке **{{ ui-key.yacloud.mdb.forms.section_storage }}** укажите нужный размер диска.
...
* CLI {#cli}
    ...
    1. Чтобы изменить размер хранилища хостов {{ ZK }}, выполните команду (размер хранилища должен быть не меньше, чем значение `disk_size` в свойствах кластера):
        ...
        Тип диска для хостов {{ ZK }} нельзя изменить после создания.
...
* {{ TF }} {#tf}
  ...
  Чтобы изменить тип диска и увеличить размер хранилища для кластера:
    ...
    1. Измените в описании кластера {{ mkf-name }} значения параметров:
       * `disk_size` и `disk_type_id` в блоке `kafka.resources` для хостов-брокеров {{ KF }}.
          
          {% include [update-disk-type](../../_includes/mdb/mkf/update-disk-type.md) %}

       * `disk_size` в блоке `zookeeper.resources` для хостов {{ ZK }}.
