...
- Консоль управления
  ...
  1. В блоке **Хранилище**:
     ...
     1. Выберите объем хранилища, который будет использоваться для данных. Размер локального хранилища можно менять только с шагом 100 ГБ. При выборе размера учитывайте,что если хранилище заполнится на 95%, кластер перейдет в режим «только чтение».
  ...
  1. В блоке **Хосты** укажите количество [хостов-брокеров](../concepts/brokers.md) {{ KF }} для размещения в каждой выбранной зоне доступности.
     ...
     - Хосты кластера {{ KF }} будут равномерно расположены в выбранных зонах доступности. Количество зон и хостов в зоне следует выбирать исходя из требуемой модели отказоустойчивости и нагрузки на кластер. Например, для устойчивости к отказу `N` хостов понадобится кластер, состоящий из не менее, чем `2*N+1` хостов.
