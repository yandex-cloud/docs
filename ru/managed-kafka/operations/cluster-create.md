...
- Консоль управления
  ...
  1. В блоке **Хранилище**:
     ...
     1. Выберите объем хранилища, который будет использоваться для данных. Размер локального хранилища можно менять только с шагом 100 ГБ.
        ...
        Учитывайте, что если хранилище заполнится на 95%, кластер перейдет в режим «только чтение». Рассчитывайте и увеличивайте необходимый размер хранилища заранее.
  ...
  1. В блоке **Хосты** укажите количество хостов-брокеров {{ KF }} для размещения в каждой выбранной зоне доступности.
     ...
     - Хосты {{ KF }}-кластера будут равномерно расположены в выбранных зонах доступности. Количество зон и хостов следует выбирать исходя из требуемой модели отказоустойчивости и нагрузки на кластер. Так, например, для устойчивости к отказу `N` хостов следует разместить минимум `2*N+1` хостов.
     - Репликация работает по умолчанию в любом кластере из более чем одного хоста при настройке фактора репликации более `1`. Фактор репликации [настраивается после создания кластера](cluster-topics.md#create-topic).
