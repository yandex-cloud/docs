...
- Консоль управления
  ...
  1. В блоке **Настройки топика** задайте параметры:
     ...
     - **Максимальный размер раздела, байт** — максимальный размер, до которого может вырасти раздел, прежде чем {{ KF }} начнет удалять старые сегменты лога, если действует настройка хранения `delete`.
     ...
     - **Время жизни сегмента лога, мс** — время, по истечении которого {{ KF }} начнет удалять старые сегменты лога, если действует настройка хранения `delete`. 
