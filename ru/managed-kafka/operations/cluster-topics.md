...
- Консоль управления
  ...
  1. В блоке **Настройки топика** задайте параметры:
     - **Политика очистки лога**:
       ...
       - `CLEANUP_POLICY_COMPACT` —  сжимать сообщения в логе.
     ...
     - **min_insync_replicas** —  минимальное количество реплик, от которых необходимо дождаться подтверждения записи, чтобы запись сообщения в топик считалась успешной. Настройка имеет смысл, когда брокер ждет подтверждения записи от всех реплик в кластере. 
    
       Минимальное значение: `1` (по умолчанию).
       
       Подробнее см. [в документации {{ KF }}](https://kafka.apache.org/documentation/#offsets.commit.required.acks).
       
     - **max_message_bytes** — максимальный размер пачки сообщений, который {{ KF }} разрешает записать в топик производителю или прочитать из топика потребителю (в байтах, после сжатия, если оно включено).
