...
Рекомендуется обновлять на следующую версию относительно текущей, например, с версии 2.8 на 3.0. Обновление до более поздних версий следует производить поэтапно. Например, обновление версии {{ KF }} с 2.8 до 3.1 следует выполнять в такой последовательности: 2.8 → 3.0 → 3.1.
