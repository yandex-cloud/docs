...
- Консоль управления {#console}
  ...
  * В блоке **{{ ui-key.yacloud.kafka.field_connector-config-mirror-maker-source-cluster }}** укажите параметры для подключения к кластеру-источнику:
    * **{{ ui-key.yacloud.kafka.field_connector-alias }}** — префикс для обозначения кластера-источника в настройках коннектора.
      ...
      В консоли управления необходимо указать псевдонимы кластера-источника и кластера-приемника. Имя реплицированного топика в кластере-приемнике будет иметь вид `<псевдоним_кластера-источника>.<имя_топика>`. Чтобы сохранить исходные имена топиков, создайте коннектор с помощью CLI.
...
- CLI {#cli}
    ...
    * `--external-cluster` — параметры внешнего кластера:
    ...
    Чтобы сохранить исходные имена топиков при репликации, задайте пустые псевдонимы кластера-источника и кластера-приемника, а также пустой разделитель:
    ...
    {{ yc-mdb-kf }} connector-mirrormaker create <имя_коннектора> \
       ...
       --direction=<направление_коннектора> \
       --tasks-max=<лимит_задач> \
       --properties=replication.policy.separator='' \
       --replication-factor=<фактор_репликации> \
       --topics=<шаблон_для_топиков> \
       --this-cluster-alias='' \
       --external-cluster alias='',bootstrap-servers=<список_FQDN_хостов-брокеров>,security-protocol=<протокол_безопасности>,sasl-mechanism=<механизм_шифрования>,sasl-username=<имя_пользователя>,sasl-password=<пароль_пользователя>
    ```
