...
Если в кластере {{ mkf-name }} включена настройка **Управление топиками через API**, то для создания пользователя-администратора воспользуйтесь интерфейсами командной строки, {% if lang == "ru" and audience != "internal" %}[API](../../glossary/rest-api.md){% else %}API{% endif %} или {{ TF }}.
