...
* Разграничивают права доступа [производителей и потребителей](../concepts/producers-consumers.md) данных.
    Производитель или потребитель получает доступ только к тем [топикам](../concepts/topics.md), которые разрешены для его учетной записи. Вы можете использовать одну и ту же учетную запись для нескольких производителей или потребителей: первые получат права на запись в определенные топики, а вторые — на чтение.
...
- Консоль управления
  ...
  Чтобы создать учетную запись для производителя или потребителя в кластере:
  ...
  1. В поле **Топик** введите `*`.
  1. Нажмите кнопку **Добавить топик**.
...
- CLI
  ...
  1. Создайте учетную запись с ролью `admin`, действующей на все (`*`) топики кластера:
...
- API
  ...
  Чтобы создать [учетную запись администратора](../concepts/topics.md#management) для управления топиками в кластере с включенной настройкой **Управление топиками через API**, при создании пользователя передайте в параметре `userSpec` блок `permission` со следующими значениями:
