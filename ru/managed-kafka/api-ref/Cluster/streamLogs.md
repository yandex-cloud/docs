...
recordToken | Record token.  Set [recordToken](/docs/managed-kafka/api-ref/Cluster/streamLogs#query_params) to the `nextRecordToken` returned by a previous [streamLogs](/docs/managed-kafka/api-ref/Cluster/streamLogs) request to start streaming from next log record.  The maximum string length in characters is 100.
