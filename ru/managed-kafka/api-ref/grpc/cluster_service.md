...
role | enum **Role**<br>Host role. If the field has default value, it is not returned in the response. <ul><li>`ROLE_UNSPECIFIED`: Role of the host is unspecified. Default value.</li><li>`KAFKA`: The host is a Kafka broker.</li><li>`ZOOKEEPER`: The host is a ZooKeeper server.</li></ul>
