...
* Для {{ KF }} 4.x (начиная с версии 4.0) минимальное значение настроек [Log segment bytes](concepts/settings-list.md#settings-log-segment-bytes) на уровне кластера и [Segment bytes](concepts/settings-list.md#settings-topic-segment-bytes) на уровне топика увеличено до 1 048 576 байт (1 МиБ).
...
* Добавлена поддержка новых версий {{ KF }} 4.1 и 4.2. Эти версии доступны в окружении `PRODUCTION`. Подробнее об изменениях в [документации {{ KF }}](https://kafka.apache.org/community/downloads/#420).
...
* Добавлена возможность управлять настройкой брокера `transactional.id.expiration.ms`. Подробнее в разделе [{#T}](concepts/settings-list.md#cluster-settings).
