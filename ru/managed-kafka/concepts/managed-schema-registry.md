...
Также необходимо авторизовывать запросы к API-серверам с помощью HTTP-заголовка `Authorization`. В заголовке нужно указать [логин и пароль учетной записи {{ KF }}](../operations/cluster-accounts#create-account).

Возможность работы со схемами зависит от выбранного [способа управления топиками](./topics.md#management) и настроенных ролей учетной записи:

1. Если в кластере используются управляемые топики, то:

    * Учетной записи с ролью `ACCESS_ROLE_PRODUCER` для топика доступны любые операции над _субъектами_ ([subjects](https://docs.confluent.io/platform/current/schema-registry/develop/api.html#subjects)), связанными с этим топиком.
