...
Топик — это способ группировки потоков сообщений по категориям. [Производители](producers-consumers.md) [публикуют](../operations/connect.md) сообщения определенной катгории в топик, а [потребители](producers-consumers.md) [подписываются](../operations/connect.md) на этот топик и читают из него сообщения. Для каждого топика {{ KF }} ведет лог сообщений, который может быть разбит на несколько разделов.
