...
* С помощью [Admin API {{ KF }}](https://kafka.apache.org/documentation/#adminapi). Способ подходит, если у вас уже есть собственное решение для управления топиками и разделами. При использовании Admin API действуют ограничения:
    ...
    * Для управления топиками используется учетная запись администратора с ролью `ACCESS_ROLE_ADMIN`, которая действует для всех топиков кластера. Если такой учетной записи в кластере нет, [создайте](../operations/cluster-accounts.md) ее.

    * Права доступа для учетных записей [производителей и потребителей](./producers-consumers.md) настраиваются с помощью стандартных интерфейсов {{ yandex-cloud }}, а не Admin API {{ KF }}.
