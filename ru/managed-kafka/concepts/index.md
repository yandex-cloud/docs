...
Для обеспечения сохранности данных {{ mkf-name }} поддерживает репликацию. Данные в разделах топиков автоматически реплицируются между хостами-брокерами в соответствии с заданным [фактором репликации](brokers.md).

При выходе из строя одного из брокеров:

* Если реплики раздела находились на нескольких брокерах, один из фолловеров автоматически становится лидером.
* Если реплик не было, раздел становится недоступен до восстановления брокера.

Для критически важных данных рекомендуется использовать фактор репликации не менее 2. Подробнее об отказоустойчивых конфигурациях см. в разделе [Отказоустойчивый кластер](ha-cluster.md).
