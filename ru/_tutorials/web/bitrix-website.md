...
* ВМ на базе [Ubuntu 22.04 LTS](/marketplace/products/yc/ubuntu-22-04-lts) с доступом во внешнюю сеть, на которой будет установлен 1С-Битрикс.
...
- Консоль управления
  ...
  1. В сети `network-1` [создайте](../../vpc/operations/subnet-create.md) две подсети в разных зонах доступности со следующими параметрами:
  ...
  1. В сети `network-1` [создайте группы безопасности](../../vpc/operations/security-group-create.md):
     ...
     1. С именем `bitrix-sg-vm` для виртуальной машины в облаке. [Задайте правила](../../vpc/operations/security-group-add-rule.md) для этой группы безопасности в соответствии с таблицей:
...
- Консоль управления
  ...
  1. В блоке **Доступ** укажите данные для доступа к ВМ:
     ...
     * В поле **SSH-ключ** скопируйте ваш открытый SSH-ключ. Пару ключей для подключения по [SSH](../../glossary/ssh-keygen.md) необходимо создать [самостоятельно](../../compute/operations/vm-connect/ssh.md).
