...
1. Аутентифицируйтесь одним из способов:
    ...
    - Аккаунт на Яндексе, федеративный аккаунт или локальный пользователь {#yandex-account}

      Если вы используете аккаунт на Яндекс, федеративный аккаунт или аккаунт локального пользователя, то в значении параметра `token` укажите значение IAM-токен.
...
* переменной `YANDEXCLOUD_TOKEN` — параметр `token`.
