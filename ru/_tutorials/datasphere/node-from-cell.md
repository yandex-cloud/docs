...
В данном примере вы воспользуетесь моделью классификации изображений на основе [полносвязных слоев]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/Свёрточная_нейронная_сеть#Полносвязная_нейронная_сеть){% endif %}{% if lang == "en" %}(https://en.wikipedia.org/wiki/Convolutional_neural_network#Fully_connected_layers){% endif %}. Модель возвращает 10 значений вероятностей, которые отображают уверенность сети в том, что поданное на вход изображение соответствует определенному классу.
...
1. Создайте массив изображений в кодировке [base64]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/Base64){% endif %}{% if lang == "en" %}(https://en.wikipedia.org/wiki/Base64){% endif %}. Этот массив будет использоваться для классификации изображений с использованием обученной ML-модели. 
