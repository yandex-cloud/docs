...
Некоторые шаги сценария нельзя выполнить в [консоли управления]({{ link-console-main }}). [Получить идентификатор лог-группы](#get-log-group-id) и [создать триггер](#set-up-sf-create-trigger) с этим идентификатором можно только через интерфейс командной строки (CLI) или [API](../../glossary/rest-api.md).
