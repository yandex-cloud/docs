...
1. В вашем облаке должна быть запущена ВМ с [настроенным OpenVPN](../../tutorials/routing/openvpn.md) для безопасного соединения с серверами 1C. 
...
1. В поле **Имя** введите имя виртуальной машины: `server-1c`.
...
1. В поле **Публичный адрес** оставьте значение **Без адреса**. Доступ на виртуальную машину будет осуществляться через сервер OpenVPN.
...
1. В поле **Имя** введите имя виртуальной машины: `licensing-server-1c`.
...
1. В блоке **Сетевые настройки** выберите сеть и подсеть, к которым нужно подключить виртуальную машину. ВМ нужно создавать в той же подсети, где находится сервер OpenVPN.
