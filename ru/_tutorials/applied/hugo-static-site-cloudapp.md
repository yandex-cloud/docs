# Установка Hugo с помощью {{ cloud-apps-name }}


С помощью этого руководства вы установите и настроите статический сайт на [Hugo](https://gohugo.io/) — генераторе статических сайтов — используя сервис [{{ cloud-apps-name }}](../../cloud-apps/). Приложение развернет сайт в [{{ objstorage-name }}](../../storage/) и настроит доступ по домену с автоматическим выпуском SSL-сертификатов.
...
1. [Установите Hugo с помощью {{ cloud-apps-name }}](#install-hugo).
...
1. [Создайте]({{ link-src-docs }}/sourcecraft/operations/repo-create#create-from-template) репозиторий из [шаблона `cloudapp-hugo`](https://sourcecraft.dev/yandex-cloud-examples/cloudapp-hugo).
