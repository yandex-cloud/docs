...
При [создании внешней таблицы](../../managed-greenplum/operations/external-tables.md#pxf-examples) из таблицы, расположенной в бакете {{ objstorage-full-name }}, необходимо передать в запросе {% if audience != "internal" %}[статический ключ доступа](../../iam/concepts/authorization/access-key.md){% else %}статический ключ доступа{% endif %} для сервисного аккаунта. Это можно сделать с помощью [протокола `s3`](https://greenplum.docs.pivotal.io/6-20/admin_guide/external/g-s3-protocol.html) и конфигурационного файла, хранящегося на HTTP-сервере.
...
* Вручную
    ...
    1. В подсети кластера [включите NAT в интернет](../../vpc/operations/enable-nat.md) и [создайте группу безопасности](../../vpc/operations/security-group-create.md), разрешающую весь входящий и исходящий трафик со всех адресов.

    {% else %}

    1. В подсети кластера включите NAT в интернет и создайте группу безопасности, разрешающую весь входящий и исходящий трафик со всех адресов.
    ...
    1. Создайте виртуальную машину с Linux в той же облачной подсети, в которой расположен кластер {{ mgp-name }}.
...
* С помощью Terraform
    ...
    1. Если у вас еще нет {{ TF }}, установите и настройте его согласно инструкции.
    1. Скачайте файл с настройками провайдера. Поместите его в отдельную рабочую директорию и укажите значения параметров.
    ...
    1. Укажите в файле `greenplum-s3-vm.tf`:
        ...
        * Имя пользователя и путь к {% if lang == "ru" and audience != "internal" %}[SSH-ключу](../../glossary/ssh-keygen.md){% else %}SSH-ключу{% endif %} для доступа к виртуальной машине.
    ...
    1. Перейдите в [консоль управления]({{ link-console-main }}) и [включите NAT в интернет](../../vpc/operations/enable-nat.md) для подсети, в которой расположен кластер.

    {% else %}

    1. Перейдите в [консоль управления]({{ link-console-main }}) и включите NAT в интернет для подсети, в которой расположен кластер.
