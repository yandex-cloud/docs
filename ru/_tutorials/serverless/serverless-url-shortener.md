...
Сервис принимает обращения пользователей через публичный [API-шлюз](../../api-gateway/concepts/index.md). Пользователь получает c [хостинга](../../storage/concepts/hosting.md) HTML-страницу с полем для ввода URL. [Функция](../../functions/concepts/function.md) отправляет введенный URL на хранение в [serverless-базу данных](../../managed-ydb/concepts/serverless-and-dedicated.md#serverless), сокращает его и возвращает пользователю. При обращении пользователя по сокращенному URL, функция находит в БД полный URL и перенаправляет на него запрос пользователя.
...
1. [Создайте БД в {{ ydb-name }}](#ydb).
1. [Настройте функцию в {{ sf-name }}](#function).
1. [Опубликуйте сервис через {{ api-gw-name }}](#api-gw).
...
* Плата за обращения к базе данных (см. [тарифы {{ydb-full-name}}](../../managed-ydb/pricing/serverless.md)).
...
Чтобы создать базу данных {{ ydb-short-name }} и настроить ее для хранения ссылок:
