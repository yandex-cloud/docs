...
В этом руководстве вы создадите агент со стримингом ответа через [веб-сокеты](https://{{ lang }}.wikipedia.org/wiki/WebSocket) на [{{ sf-full-name }}](../../functions/) и [{{ api-gw-full-name }}](../../api-gateway/). Функция будет использовать [OpenAI Agents SDK](https://openai.github.io/openai-agents-python/) для работы с [{{ yagpt-full-name }}](../../ai-studio/quickstart/yandexgpt.md).
...
С помощью сервисного аккаунта функция получит доступ к секрету и {{ yagpt-name }}, а API-шлюз получит доступ к функции.
...
С помощью API-ключа функция получит доступ к {{ yagpt-name }}.
