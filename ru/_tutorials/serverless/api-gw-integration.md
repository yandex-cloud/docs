...
Пользовательская интеграция представляет собой [функцию](../../functions/concepts/function.md) {{ sf-name }} или [контейнер](../../serverless-containers/concepts/container.md) {{ serverless-containers-name }}, которые предназначены для решения типовой задачи. 
...
Разработайте функцию-интеграцию с {{ ydb-name }} для работы с [СУБД YDB](../../ydb/concepts/#ydb). Функция будет взаимодействовать с {{ ydb-name }} и обрабатывать внешние HTTP-запросы через API-шлюз с использованием [HTTP API](https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/Welcome.html), совместимого с [Amazon DynamoDB](https://aws.amazon.com/ru/dynamodb/). Язык кода функции — TypeScript, среда выполнения — Node.js 16.

Интеграция будет применена для реализации [CRUD]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/CRUD){% endif %}{% if lang == "en" %}(https://en.wikipedia.org/wiki/Create,_read,_update_and_delete){% endif %} API для работы с базой данных фильмов, развернутой в {{ ydb-name }}.
