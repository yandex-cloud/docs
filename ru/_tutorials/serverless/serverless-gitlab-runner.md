...
1. В разделе **Custom headers** нажмите **Add custom header** и добавьте заголовки:
    ...
    --- | --- | ---
    `Authorization` | `Api-Key <API-ключ>` | API-ключ сервисного аккаунта `gitlab-runner-caller`. Обязательно при закрытом доступе к контейнеру, иначе запрос будет отклонен. Подробнее в разделе [Аутентифицироваться при вызове приватного контейнера через HTTPS](../../serverless-containers/operations/auth.md).
