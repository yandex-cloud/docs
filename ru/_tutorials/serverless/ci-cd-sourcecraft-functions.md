В этом руководстве вы настроите [CI/CD](../../sourcecraft/concepts/ci-cd.md) между {{ sf-name }} и {{ src-name }}. Для этого вы создадите репозиторий, настроите развертывание функции и проверите результат.
...
В репозитории будут храниться параметры и код функции и настройки CI/CD-процесса.
...
В [секрете](../../sourcecraft/operations/secrets.md) в зашифрованном виде будет храниться авторизованный ключ сервисного аккаунта для доступа к функции.
...
* `.src.ci.yaml` с параметрами функции и настройками CI/CD-процесса.
...
- Интерфейс {{ src-name }} {#src}
  ...
  1. Аналогичным образом создайте файл `.src.ci.yaml`. Вставьте в него следующий код, указав идентификатор каталога, в котором хотите создать функцию:
...
- Консоль управления {#console}
  ...
  1. В [консоли управления]({{ link-console-main }}) перейдите в каталог, который указали в файле `.src.ci.yaml`.
...
- CLI {#cli}
  ...
  1. Получите список функций в каталоге, указанном в файле `.src.ci.yaml`:
      ```
