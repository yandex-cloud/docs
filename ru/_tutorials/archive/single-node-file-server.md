...
Вы научитесь создавать однонодовый файловый сервер с помощью [Samba](https://www.samba.org/) и [NFS]{% if lang == "ru" %}(https://docs.microsoft.com/ru-ru/windows-server/storage/nfs/nfs-overview){% endif %}{% if lang == "en" %}(https://docs.microsoft.com/en-us/windows-server/storage/nfs/nfs-overview){% endif %}, а также подключаться к нему с компьютеров на Linux, macOS и Windows.
