# Развертывание параллелльной файловой системы GlusterFS

[GlusterFS](https://ru.wikipedia.org/wiki/GlusterFS) — это распределенная, параллельная, линейно масштабируемая файловая система. Посредством горизонтального масштабирования позволяет обеспечить в облаке агрегированную пропускную способность в десятки ГБ/с и в сотни тысяч [IOPS](https://ru.wikipedia.org/wiki/IOPS).

По этому руководству вы создадите инфраструктуру из трех сегментов, имеющих общую файловую систему GlusterFS. Размещение [дисков](../../compute/concepts/disk.md) для хранения данных в трех разных [зонах доступности](../../overview/concepts/geo-scope.md) обеспечит высокую доступность и отказоустойчивость файловой системы.
...
1. [Разверните ресурсы](setup-environment).
...
1. Отредактируйте файл `variables.tf`, указав параметры разворачиваемых ресурсов:
   ...
   1. Если при создании пары SSH-ключей вы указали имя, отличного от имени по умолчанию, в секции `local_pubkey_path` значение `default` измените на `<путь_к_публичному_ключу_SSH>`.

## Разверните ресурсы {#setup-environment}
...
## Установите и настройте GlusterFS {#install-glusterfs}
   ...
   1. Выполните дополнительные настройки производительности:
      ...
      ```
   1. Примонтируйте папку общего доступа `regional-volume` к клиентским ВМ:
      ```bash
