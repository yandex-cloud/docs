В этом сценарии вы создадите [функцию](../functions/concepts/function.md) с приложением на [Node.js](https://nodejs.org/ru/), которое выполняет простой запрос к базе данных {{ ydb-short-name }}. Развертывание приложения осуществляется с помощью Bash-скриптов, для компиляции используется команда `tcs`.
