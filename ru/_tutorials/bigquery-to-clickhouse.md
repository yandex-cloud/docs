...
1. {% if audience != "internal" %}[Создайте сервисный аккаунт](../iam/operations/sa/create.md){% else %}Создайте сервисный аккаунт{% endif %} с ролью `storage.uploader` для доступа к бакету {{ objstorage-name }}.

1. {% if audience != "internal" %}[Создайте статический ключ доступа](../iam/operations/sa/create-access-key.md){% else %}Создайте статический ключ доступа{% endif %} для сервисного аккаунта. Сохраните идентификатор ключа и секретный ключ, они понадобятся далее.
