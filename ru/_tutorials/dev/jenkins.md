...
1. [Назначьте](../../iam/operations/sa/assign-role-for-sa.md) сервисному аккаунту [роль](../../iam/concepts/access-control/roles.md) `admin` на каталог, где будут выполняться операции:
   ...
   ```bash
   yc resource-manager folder add-access-binding <имя каталога> --role admin --subject serviceAccount:$SERVICE_ACCOUNT_ID
   ```
...
1. В поле **Имя** введите имя ВМ: `jenkins-tutorial`.
...
1. В блоке **Диски** укажите размер загрузочного [диска](../../compute/concepts/disk.md) 15 ГБ.
1. В блоке **Вычислительные ресурсы**:
   ...
   * Укажите необходимое количество vCPU и объем RAM:
     ...
     * **RAM** — 2 ГБ.
1. В блоке **Сетевые настройки** нажмите кнопку **Добавить сеть** и выберите, к какой [подсети](../../vpc/concepts/network.md#subnet) подключить ВМ. В блоке **Публичный адрес** назначьте ВМ публичный IP-адрес автоматически или выберите один из зарезервированных IP-адресов.
...
После этого в разделе **Образы** сервиса **{{ compute-name }}** появятся три новых образа:
