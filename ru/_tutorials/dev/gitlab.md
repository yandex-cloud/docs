...
После этого пользователи не смогут регистрироваться самостоятельно. Администратор может создавать учетные записи в разделе **Overview** → **Users**. Подробнее в [документации {{ GL }}](https://docs.gitlab.com/user/profile/account/create_accounts/).
...
1. Добавьте сценарий тестирования:
   ...
   1. Откроется страница с предложением добавить новый файл `.gitlab-ci.yml`, в котором в формате [YAML](https://yaml.org/) нужно описать сценарий. Добавьте текст сценария:
      ...
      * `pack` — на третьем этапе создается архив с исполняемым файлом, который можно будет загрузить через веб-интерфейс {{ GL }} после успешного завершения сценария. В блоке `artifacts` указаны файлы, доступные для загрузки.
      ...
      Задача `build` сохраняет исполняемый файл `hello` как артефакт. По умолчанию задачи следующих этапов скачивают артефакты предыдущих этапов, поэтому файл `hello` будет доступен задачам `test` и `pack`. Без этого артефакта обе задачи завершатся с ошибкой. Подробнее в [документации {{ GL }}](https://docs.gitlab.com/ci/jobs/job_artifacts/).
