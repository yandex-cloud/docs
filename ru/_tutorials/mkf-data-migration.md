...
### Перед началом работы {#before-you-begin-connector}
...
[Создайте для кластера-приемника коннектор](../managed-kafka/operations/cluster-connector.md#create) с типом `MirrorMaker` и настройками:
...
- С помощью {{ TF }}
    ...
    1. Укажите в файле `kafka-mirror-maker.tf`:
        ...
        * Идентификатор публичного [образа](../compute/operations/images-with-pre-installed-software/get-list) с Ubuntu без GPU. Например, для [Ubuntu 20.04 LTS](https://cloud.yandex.ru/marketplace/products/yc/ubuntu-20-04-lts).
...
- С помощью {{ TF }}
    ...
    1. Удалите конфигурационный файл `kafka-mirror-maker.tf`.
    1. Проверьте корректность файлов конфигурации {{ TF }} с помощью команды:
