...
- Консоль управления 
    ...
    1. В блоке **Выбор образа/загрузочного диска** перейдите на вкладку **{{ marketplace-name }}** и выберите образ {% if product == "yandex-cloud" %}[Drupal](/marketplace/products/yc/drupal-8){% else %}[Drupal](/marketplace/products/yc/drupal){% endif %}.
...
Для работы {% if lang == "ru" and audience != "internal" %}[VPN](../../glossary/vpn.md){% else %}VPN{% endif %} необходимо разрешить прием и передачу трафика на UDP-порты `500` и `4500` из внешней сети — это необходимо для работы IPSec-туннеля. Также необходимо разрешить передачу трафика между подсетями вашей виртуальной сети и сетью на удаленной площадке.
