...
Если ключ доступа к бастионному хосту на вашей локальной машине имеет стандартный путь и название `~/.ssh/id_rsa` (если нет, предварительно переименуйте его — параметр `-J` использует для промежуточного хоста стандартный путь поиска ключей и не позволяет указать для него отдельный ключ прямо в команде), то для упрощения SSH-доступа и его настройки добавьте параметр `-J` (ProxyJump) в команду SSH:
