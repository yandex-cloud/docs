...
С помощью туннелей на порт TCP или UDP и асимметричного шифрования можно создавать виртуальные сети. {% if lang == "ru" and audience != "internal" %}[VPN](../../glossary/vpn.md){% else %}VPN{% endif %} может быть использован, например, чтобы:
...
1. В блоке **Доступ** укажите данные для доступа на виртуальную машину:
   ...
   * В поле **Логин** введите имя пользователя {% if lang == "ru" and audience != "internal" %}[SSH](../../glossary/ssh-keygen.md){% else %}SSH{% endif %}, например, `yc-user`.
