...
Продукт [OpenVPN Access Server](https://cloud.yandex.ru/marketplace/products/yc/openvpn-access-server) совместим с [открытой версией](https://github.com/OpenVPN) OpenVPN и построен на ее основе. Он предоставляет клиенты для Windows, Mac, Android и iOS, а также позволяет управлять подключениями с помощью веб-интерфейса.
...
1. В блоке **Выбор образа/загрузочного диска**:
   ...
   * В списке публичных образов выберите [OpenVPN Access Server](https://cloud.yandex.ru/marketplace/products/yc/openvpn-access-server) и нажмите **Использовать**.
...
1. Для работы OpenVPN Access Server [добавьте](../../vpc/operations/security-group-update.md#add-rule) следующие правила в группу безопасности:
...
На сервере OpenVPN заранее создан пользователь `openvpn` с административными правами. Пароль генерируется автоматически при создании ВМ. Чтобы получить доступ к административной панели:

1. Подключитесь к виртуальной машине по SSH:

   ```
   ssh yc-user@<публичный IP-адрес ВМ>
   ```
...
1. Выполните команду:
   ...
   ```
   На экране отобразится пароль, который находится в текстовом файле `initial_ovpn_pass`.
...
1. Введите имя пользователя `openvpn` и пароль, полученный из файла `initial_ovpn_pass`.
