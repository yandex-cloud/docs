...
С помощью [VPC Stitching](../../cloud-router/concepts/vpc-stitching.md) вы можете организовать IP-связность между двумя и более [облачными сетями](../../vpc/concepts/network.md) {{ vpc-name }}, которые находятся в разных каталогах или облаках.
...
1. [Создайте транковое подключение](#create-trunk), если у вас еще нет подходящего подключения {{ interconnect-name }}.
...
* у вас есть роли [cic.editor](../../interconnect/security/index.md#cic-editor) и [cloud-router.editor](../../cloud-router/security/index.md#cloudrouter-editor) в каталогах с ресурсами {{ interconnect-name }} и {{ cr-name }} соответственно;
...
Создайте [обращение в поддержку]({{ link-console-support }}) для создания транкового подключения {{ interconnect-name }} с объемом данных 300 ТБ.
...
- Консоль управления {#console}
  ...
  1. В [консоли управления]({{ link-console-main }}) выберите каталог, в котором находится транковое подключение.
  1. [Перейдите]({{ link-console-main }}/link/interconnect) в сервис **{{ ui-key.yacloud.ui.constants.label_interconnect_aUMcv }}**.
  1. На панели слева выберите ![nodes-left](../../_assets/console-icons/nodes-left.svg) **{{ ui-key.yacloud.interconnect.private-connection.private-connections_daeaR }}** и нажмите **{{ ui-key.yacloud.interconnect.private-connection.create-private-connection_6K9tP }}**.
  1. В открывшемся окне:

      1. В поле **{{ ui-key.yacloud.interconnect.private-connection.TrunkConnectionSuggestOrCreate.title_oPBF7 }}** выберите созданное ранее транковое подключение.
      1. В поле **{{ ui-key.yacloud.interconnect.vlan-id_2wghK }}** укажите `4095`.
      1. В поле **{{ ui-key.yacloud.interconnect.peering-subnet_eYRTR }}** укажите `172.31.255.0/30`.
      1. В поле **{{ ui-key.yacloud.interconnect.peer-ip_pMdxo }}** укажите `172.31.255.1`.
      1. В поле **{{ ui-key.yacloud.interconnect.cloud-ip_6ESSe }}** укажите `172.31.255.2`.
      1. В поле **{{ ui-key.yacloud.interconnect.bgp-asn_3dSL7 }}** укажите `65534`.
      1. В блоке **{{ ui-key.yacloud.common.section-base }}** в поле **{{ ui-key.yacloud.common.name }}** укажите `vpc-stitching`.
...
- Консоль управления {#console}
  ...
  1. В [консоли управления]({{ link-console-main }}) выберите каталог, в котором хотите создать виртуальный маршрутизатор.
  1. [Перейдите]({{ link-console-main }}/link/cloud-router) в сервис **{{ ui-key.yacloud.ui.constants.label_cloud-router_kBGNL }}**.
  1. Нажмите **{{ ui-key.yacloud.cloud-router.router.create-router_v4cPC }}**.
  1. Введите имя виртуального маршрутизатора `ri-1`.
  1. В поле **{{ ui-key.yacloud.interconnect.private-connection.private-connections_daeaR }}** выберите соединение `vpc-stitching` или укажите его идентификатор.
  1. Нажмите **{{ ui-key.yacloud.common.create }}**.
...
Создайте [обращение в поддержку]({{ link-console-support }}) и попросите включить VPC Stitching для созданного приватного соединения.
...
- Консоль управления {#console}
  ...
  1. В [консоли управления]({{ link-console-main }}) выберите каталог, в котором находится виртуальный маршрутизатор `ri-1`.
  1. [Перейдите]({{ link-console-main }}/link/cloud-router) в сервис **{{ ui-key.yacloud.ui.constants.label_cloud-router_kBGNL }}**.
  1. В строке с виртуальным маршрутизатором `ri-1` нажмите значок ![ellipsis](../../_assets/console-icons/ellipsis.svg) и выберите ![pencil](../../_assets/console-icons/pencil.svg) **{{ ui-key.yacloud.common.edit }}**.
  1. В блоке **{{ ui-key.yacloud.cloud-router.router.routing-networks_42RL1 }}** выберите сети `Net-1` и `Net-2`.
...
- Консоль управления {#console}
  ...
  1. В [консоли управления]({{ link-console-main }}) выберите каталог, в котором находится приватное соединение `vpc-stitching`.
  1. [Перейдите]({{ link-console-main }}/link/interconnect) в сервис **{{ ui-key.yacloud.ui.constants.label_interconnect_aUMcv }}**.
  ...
  1. В строке с соединением `vpc-stitching` нажмите значок ![ellipsis](../../_assets/console-icons/ellipsis.svg) и выберите ![pencil](../../_assets/console-icons/pencil.svg) **{{ ui-key.yacloud.common.edit }}**.
  1. В поле **IPv4 StaticRoute prefix** добавьте все склеивающие анонсы из списка выше. Чтобы добавить очередной префикс, нажмите **{{ ui-key.yacloud.interconnect.private-connection.StaticRoutePrefixRow.add-prefix-button_n11CU }}**.
  1. Нажмите **{{ ui-key.yacloud.common.save }}**.
