...
Чтобы установить CSR 1000v и настроить к нему доступ по {% if lang == "ru" and audience != "internal" %}[SSH](../../glossary/ssh-keygen.md){% else %}SSH{% endif %}:
...
- Консоль управления
  ...
  1. В блоке **Выбор образа/загрузочного диска** перейдите на вкладку **{{ marketplace-name }}** и выберите образ {% if product == "yandex-cloud" %}[Cisco CSR](/marketplace/products/yc/cisco-csr){% else %}Cisco CSR{% endif %}.
