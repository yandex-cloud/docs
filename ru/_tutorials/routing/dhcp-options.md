...
Чтобы полностью пройти данный сценарий, необходимо {% if product == "cloud-il" %}[развернуть Active Directory](../../tutorials/windows/active-directory.md){% else %}развернуть Active Directory{% endif %}. Если вы используете собственные DNS-серверы, доступные для виртуальных машин облака, указывайте в сценарии свои значения DNS-суффикса и IP-адресов.
