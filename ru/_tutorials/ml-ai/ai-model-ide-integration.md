...
В стоимость поддержки инфраструктуры для подключения к модели из редактора кода входит плата за генерацию текста ([тарифы {{ ai-studio-full-name }}]({{ link-docs-ai }}ai-studio/pricing)).
