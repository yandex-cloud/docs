...
1. Настройте CNAME для своего домена:
   ...
   1. Подготовьте CNAME-запись таким образом, чтобы она указывала на скопированный ранее адрес в домене `.yccdn.cloud.yandex.net` или `.edgecdn.ru` в зависимости от используемого [провайдера CDN](../../cdn/concepts/providers.md). Например, если доменное имя сайта — `{{ domain-name-example }}`, создайте CNAME-запись или замените уже существующую запись для `cdn`:
