...
Также вы можете использовать {{ TF }} от имени {% if product == "yandex-cloud" %}[аккаунта на Яндексе](../iam/concepts/index.md#passport){% endif %}{% if product == "cloud-il" %}[аккаунта Google](../iam/concepts/index.md#passport){% endif %} или [федеративного аккаунта](../iam/concepts/index.md#saml-federation), однако этот способ является менее безопасным. Подробности см. в конце раздела.
