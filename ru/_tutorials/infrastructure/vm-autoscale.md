...
ВМ будут развернуты в двух [зонах доступности](../../overview/concepts/geo-scope.md), а нагрузка на них будет регулироваться с помощью [сетевого балансировщика нагрузки](../../network-load-balancer/concepts/index.md) [{{ network-load-balancer-full-name }}](../../network-load-balancer/).
...
1. Удалите группу ВМ:
   ...
   - Консоль управления {#console}
     ...
     1. Выберите сервис **{{ ui-key.yacloud.iam.folder.dashboard.label_compute }}**.
     1. На панели слева нажмите ![image](../../_assets/console-icons/layers-3-diagonal.svg) **{{ ui-key.yacloud.compute.switch_groups }}**.
     1. В строке группы ВМ `auto-group` нажмите значок ![image](../../_assets/console-icons/ellipsis.svg) и выберите **{{ ui-key.yacloud.common.delete }}**.
...
1. Удалите подсети:
   ...
   - CLI {#cli}
     ...
     1. Удалите подсеть в зоне доступности `{{ region-id }}-a`:
     ...
     1. Удалите подсеть в зоне доступности `{{ region-id }}-b`:
