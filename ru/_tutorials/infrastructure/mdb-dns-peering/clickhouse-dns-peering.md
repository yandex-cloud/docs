# Настройка {{ dns-full-name }} для доступа к кластеру управляемых баз данных из других облачных сетей
...
* Плата за кластер {{ mch-name }}: использование вычислительных ресурсов, выделенных хостам (в том числе хостам {{ ZK }}), и дискового пространства ([тарифы {{ mch-name }}](../../../managed-clickhouse/pricing.md)).
* Плата за ВМ: использование вычислительных ресурсов, хранилища и публичного IP-адреса (опционально) ([тарифы {{ compute-name }}](../../../compute/pricing.md)).
...
1. Подготовьте инфраструктуру:
    ...
    - С помощью {{ TF }}
        ...
        1. Укажите в файле `nets-vm-mch.tf`:
            ...
            * `image_id` — идентификатор публичного образа виртуальной машины. О том, как получить список доступных образов, смотрите в [инструкции](../../../compute/operations/images-with-pre-installed-software/get-list.md).
...
1. Настройте сетевую связность между облачными сетями `mch-net` и `another-net`, например, с помощью [IPSec-шлюза](../../../tutorials/routing/ipsec/index.md). Другие способы настроить сетевую связность смотрите в разделе [{#T}](../../../tutorials/routing/index.md).
