...
1. Вычисляется отношение `polygon` к `city`.
...
1. [Пригласите пользователя]({{ link-docs }}/organization/operations/add-account#send-invitation) с аккаунтом на Яндексе, добавьте [федеративного]({{ link-docs }}/organization/operations/add-account#add-user-sso) или [локального]({{ link-docs }}/organization/operations/add-account#local) пользователя.
...
Ссылку на дашборд при этом можно будет отправлять из браузера. Подробнее про права доступа к объектам {{ datalens-short-name }} смотрите в [документации](../../datalens/security/index.md#permissions).
...
1. Скопируйте код вставки `iframe` в светлой или темной теме.
