...
В этом сценарии будут {% if lang == "ru" and audience != "internal" %}[проанализированы открытые данные](../../glossary/data-analytics.md){% else %}проанализированы открытые данные{% endif %} по дорожно-транспортным происшествиям на дорогах РФ. Помимо выявления фактов статистики ДТП в России, в процессе анализа вы научитесь:
...
1. [Создайте подключение к файлу](#step1).
...
1. [Добавьте таблицу с геослоями регионов](#step9).
...
1. Добавьте поле для отображения геоточки.
   ...
   1. Нажмите **Создать**.
      ...
      В примере используются готовые данные в формате координат. {% if audience == "draft" %}В других случаях для конвертации данных в формат координат вы можете воспользоваться функцией [геокодинга](../../datalens/function-ref/GEOCODE.md).{% endif %}
