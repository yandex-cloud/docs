# Визуализация данных из {% if audience != "internal" %}CSV-{% endif %}файла
...
Для визуализации и исследования данных {% if audience != "internal" %}[подготовьте {{ datalens-short-name }} к работе](#before-you-begin), затем выполните следующие шаги:{% else %}выполните следующие шаги:{% endif %}
...
Создайте датасет на основе [подключения](../../datalens/concepts/connection.md) к {% if audience != "internal" %}CSV-{% endif %}файлу.
...
1. Если в рабочей области нет таблицы **SuperHeroes.csv**, то перетащите таблицу из панели выбора в рабочую область.
    ...
    {% if audience == "internal" %}![image](../../_assets/datalens/solution-01/06-Drag-Table-file.png){% else %}![image](../../_assets/datalens/solution-01/06-Drag-Table.png){% endif %}
