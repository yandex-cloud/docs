...
* Плата за каждую ВМ (узлы кластера, DNS-сервер, ВМ для управления кластером {{ managed-k8s-name }} без публичного доступа): использование вычислительных ресурсов, операционной системы и хранилища ([тарифы {{ compute-name }}](../../compute/pricing.md)).
* Плата за публичные IP-адреса для ВМ ([тарифы {{ vpc-name }}](../../vpc/pricing.md#prices-public-ip)).
