...
* Плата за кластер-приемник {{ mrd-name }}: использование вычислительных ресурсов, выделенных хостам, и дискового пространства (см. [тарифы {{ VLK }}](../../managed-valkey/pricing.md)).
...
- Вручную {#manual}
    ...
    1. [Создайте кластер {{ mrd-name }}](../../managed-valkey/operations/cluster-create.md) любой подходящей конфигурации.  При создании кластера {{ mrd-name }} укажите те же сеть и группы безопасности, что и для ВМ с веб-сервером. 
...
- {{ TF }} {#tf}
    ...
    1. Скачайте в ту же рабочую директорию файл конфигурации для подходящего типа кластера:
        ...
        * [redis-cluster-sharded-and-vm-for-php.tf](https://github.com/yandex-cloud-examples/yc-redis-as-php-session-storage/blob/main/redis-cluster-sharded-and-vm-for-php.tf) — для [шардированного](../../managed-valkey/concepts/sharding.md) кластера.
    ...
    * `/etc/php/7.2/fpm/` для NGINX.
    ...
    {% list tabs group=cluster %}
      ...
      - Шардированный кластер {#sharded}
        ...
        Где `<FQDN1>`, `<FQDN2>` и `<FQDN3>` — полные доменные имена [хостов-мастеров кластера](../../managed-valkey/operations/hosts.md#list). Например, для кластера из 3-х шардов с паролем `password` значение параметра `session.save_path` будет выглядеть так:
...
- Вручную {#manual}
    ...
    * [Удалите кластер {{ mrd-full-name }}](../../managed-valkey/operations/cluster-delete.md).
