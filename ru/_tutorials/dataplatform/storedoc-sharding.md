...
Оплачивается использование кластера {{ mmg-name }}: выделенные хостам вычислительные ресурсы, включая ресурсы дополнительных служебных хостов, а также объем хранилища и резервных копий ([тарифы {{ mmg-name }}](../../storedoc/pricing.md)).
...
Допустим, у вас уже есть кластер {{ mmg-name }} с шардами и базой данных `billing`. Необходимо включить шардирование для коллекций `payment` и `addresses`. Для примера в качестве ключа шардирования используется хеш индекса для `payment` и значение поля `addresses`.
