# Анализ производительности и оптимизация {{ SD }}
...
* [неэффективное выполнение запросов в {{ SD }}](#inefficient-queries),
* [блокировки](#locks),
...
1. Установите на внешнем хосте, имеющем сетевой доступ к хосту {{ SD }} (смотрите [{#T}](../../storedoc/operations/connect/index.md)), [утилиты](../../storedoc/operations/tools.md#monitoring-tools) получения данных о производительности {{ SD }} `mongostat` и `mongotop`.
1. Определите, для каких баз данных нужно выявить проблемы.
1. [Создайте пользователя](../../storedoc/operations/cluster-users.md#adduser) {{ SD }} с ролью [`mdbMonitor`](../../storedoc/concepts/users-and-roles.md#mdbMonitor) для этих баз. Это необходимо для использования утилит `mongostat` и `mongotop`.
...
Начните диагностику с выяснения характера нагрузки и определения проблемных коллекций. Воспользуйтесь встроенными [утилитами мониторинга {{ SD }}](../../storedoc/operations/tools.md#monitoring-tools). Далее проанализируйте производительность конкретных запросов с помощью [логов](../../storedoc/operations/tools.md#explore-logs) или [данных профилировщика](../../storedoc/operations/tools.md#explore-profiler).
...
В момент падения производительности проблему можно диагностировать в реальном времени с помощью [списка текущих запросов](../../storedoc/operations/tools.md#list-running-queries):
...
Чтобы выявить проблемные запросы в {{ SD }}:
...
{{ SD }} не предоставляет подробной информации по блокировкам. В частности, есть только косвенные способы узнать, чем заблокирован конкретный запрос:
...
Чтобы отслеживать степень заполнения хранилища на хостах кластера, [настройте алерт](../../storedoc/operations/monitoring.md#read-only-alert).
