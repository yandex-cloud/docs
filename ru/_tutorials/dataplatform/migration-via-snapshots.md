# Миграция в {{ mes-full-name }} с помощью снапшотов
...
- Вручную {#manual}
    ...
    1. [Создайте кластер-приемник](../../managed-elasticsearch/operations/cluster-create.md#create-cluster) {{ mes-name }} нужной вам конфигурации со следующими настройками:
    ...
    1. [Установите плагин](../../managed-elasticsearch/operations/cluster-plugins.md#update) `repository-s3` в кластер-приемник.
...
- {{ TF }} {#tf}
    ...
    1. Укажите в файле конфигурации `mes-migration.tf` в блоке `locals`:
        ...
        * [пароль для пользователя `admin`](../../managed-elasticsearch/operations/cluster-update.md#change-admin-password);
        * [редакцию кластера-приемника](../../managed-elasticsearch/concepts/es-editions.md);
    ...
    1. [Создайте статический ключ доступа](../../iam/operations/authentication/manage-access-keys.md#create-access-key) для сервисного аккаунта `sa-bucket`.
...
1. [Настройте доступ к бакету со снапшотами](../../managed-elasticsearch/operations/s3-access.md) для кластера-приемника. Используйте [созданный ранее](#before-you-begin) сервисный аккаунт.
...
1. Убедитесь, что все нужные данные перенесены в кластер-приемник {{ mes-name }}.
    ...
    Это можно сделать, например, с [помощью Kibana](../../managed-elasticsearch/operations/cluster-connect.md).
