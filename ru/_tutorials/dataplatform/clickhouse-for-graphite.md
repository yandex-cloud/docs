...
Если прореживание, агрегация или усреднение не требуются, то для хранения данных Graphite можно использовать любой [движок таблиц]({{ ch.docs }}{{ lang }}/engines/table-engines) {{ CH }}.
...
* плата за вычислительные ресурсы кластера, объем хранилища и резервных копий ([тарифы {{ mch-full-name }}](../../managed-clickhouse/pricing.md));
* плата за запущенную ВМ для управления БД ([тарифы {{ compute-full-name }}](../../compute/pricing.md)).
