...
Шардирование кластера {{ mmg-name }} происходит с автоматическим созданием служебных хостов{% if audience != "internal" %}, которые [тарифицируются](../../managed-mongodb/pricing.md) отдельно от основных хостов СУБД{% endif %}:
