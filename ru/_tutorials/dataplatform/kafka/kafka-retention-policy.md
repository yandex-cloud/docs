...
Потеря сообщений в топике {{ KF }} [группа потребителей](../../../managed-kafka/concepts/producers-consumers.md#consumer-groups) возникает из-за сочетания двух факторов:
...
* Плата за кластер {{ KF }}: использование вычислительных ресурсов, выделенных хостам (в том числе хостам ZooKeeper), и дискового пространства ([тарифы {{ KF }}](../../../managed-kafka/pricing.md)).
...
[Перейдите]({{ link-monitoring }}) в сервис **{{ monitoring-full-name }}** и проанализируйте поведение созданных ранее метрик:
