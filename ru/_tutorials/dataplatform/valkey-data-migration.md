...
- {{ TF }} {#tf}
    ...
    1. Скачайте в ту же рабочую директорию файл конфигурации для подходящего типа кластера:
        ...
        * [redis-cluster-sharded.tf](https://github.com/yandex-cloud-examples/yc-redis-data-migration-from-on-premise/blob/main/redis-cluster-sharded.tf) — для [шардированного](../../managed-valkey/concepts/sharding.md) кластера.
...
1. [Подключитесь к кластеру](../../managed-valkey/operations/connect/index.md) и запустите интерактивную сессию `screen`:
    ...
    - Подключение без TLS {#without-tls}
        ...
        **Подключение к шардированному кластеру**
        ...
        1. Запустите скрипт:
            ...
            Во время работы скрипта будут выводиться сообщения об ошибках вставки данных. Это нормальное поведение команды `redis-cli`, связанное с тем, что в шардированном кластере каждый шард хранит только часть данных. Подробнее см. в разделе [{#T}](../../managed-valkey/concepts/sharding.md).
