...
Для визуализации и исследования данных {% if audience != "internal" %}[подготовьте облако к работе](#before-you-begin), затем{% endif %} выполните следующие шаги:
...
1. В правом верхнем углу нажмите кнопку **Создать подключение**, введите название подключения `dtp_data` и нажмите **Создать**.
...
1. Выберите тип файла **CSV**.
   ...
   Скачайте на свой компьютер файл [**Регионы.csv**](https://{{ s3-storage-host }}/doc-files/Regions.csv). Затем в окне создания подключения загрузите его по кнопке **Выбрать CSV-файл**.
