...
Если вы не используете Cloudbase-Init и хотите изменить размер загрузочного диска в группе рабочих столов, чтобы он отличался от размера исходного образа:

1. Расширьте размер файловой системы загрузочного диска на рабочем столе. Например, с помощью утилиты `diskmgmt.msc`.

2. [Создайте новый образ](create-from-desktop.md) {{ cloud-desktop-name }} из этого рабочего стола.