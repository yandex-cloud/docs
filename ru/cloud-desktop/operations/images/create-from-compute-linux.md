...
Пользовательский образ создается на базе [виртуальной машины](../../../compute/concepts/vm.md) {{ compute-name }}.

Чтобы добавить в {{ cloud-desktop-name }} пользовательский образ:

1. [Создайте](../../../compute/operations/vm-create/create-linux-vm.md) или выберите уже созданную ВМ со следующими параметрами:
   * Доступ — по **SSH-ключу**. Доступ по OS Login в {{ cloud-desktop-name }} не поддерживается.
