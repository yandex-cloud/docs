...
При переносе зоны DNS метрики в [{{ monitoring-full-name }}](../../monitoring/) не переносятся. Те, что были в предыдущем каталоге, останутся в нем, а новые метрики будут создаваться уже в новом каталоге.
