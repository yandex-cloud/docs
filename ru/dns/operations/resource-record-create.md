...
- Terraform
  ...
  1. Опишите в конфигурационном файле параметры ресурсов, которые необходимо создать. Вы можете добавить несколько записей одновременно.
     ...
     * `private_networks` — для публичных зон указываются ресурсы {{ vpc-name }}, которым эта зона видна. Необязательный параметр.
     ...
     Параметры `yandex_dns_recordset`:
...
Значение ресурсных [TXT-записей](../concepts/resource-record.md#txt) не может содержать больше 255 знаков в одной строке и 1024 знака всего. Если запись содержит больше знаков, разделите её на несколько значений.
