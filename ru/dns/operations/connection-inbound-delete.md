...
Если входящее DNS-подключение используется в DNS-настройках приватных подсетей {{ baremetal-name }}, сначала [исключите его из настроек](../../baremetal/operations/configure-dns.md#disable-dns) всех таких подсетей. Пока подключение используется хотя бы одной подсетью, удалить его нельзя.
