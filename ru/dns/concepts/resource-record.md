...
При сохранении записей типа `AAAA` сервис производит нормализацию адреса, например, запись `2001:db8::` будет приведена к виду `2001:db8:0:0:0:0:0:0`.
...
* `RNAME` — доменное имя почтового сервера, обслуживающего зону. Значение по умолчанию — `md.cloud.yandex.net`.
