...
При создании TXT-записи API {{ dns-name }} автоматически разбивает длинные значения на текстовые блоки, учитывая уже заданное разбиение. Непрерывная последовательность символов без пробелов и кавычек считается отдельным блоком. Текст в двойных кавычках также считается одним блоком, даже если содержит пробелы.
...
* {{ objstorage-full-name }} — [запись ANAME](#aname) необходима, чтобы [привязать](../../storage/operations/hosting/own-domain.md) домена к бакету и помечена значком ![logo](../../_assets/storage/storage-logo.svg)
