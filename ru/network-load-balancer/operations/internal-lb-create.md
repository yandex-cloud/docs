...
- Terraform
    ...
    1. Опишите в конфигурационном файле параметры ресурса без блока `listener`:
       ...
       {% cut "Пример создания внутреннего сетевого балансировщика без обработчика с помощью Terraform" %}
...
- Terraform
    ...
    1. Чтобы создать внутренний сетевой балансировщик с [обработчиком](../concepts/listener.md), откройте файл конфигурации Terraform и добавьте блок `listener` в описании внутреннего сетевого балансировщика. Чтобы подключить целевую группу, добавьте блок `attached_target_group` с указанием на целевую группу в поле `target_group_id`.

       {% cut "Пример создания внутреннего сетевого балансировщика с обработчиком и подключенной целевой группой с помощью Terraform" %}
