...
* Наведите порядок в приёмнике {{ objstorage-name }} — подключайте политику очистки данных, чтобы автоматически удалять устаревшие, ненужные или избыточные данные из таблиц.
* Подключайте эндпоинты {{ ydb-name }} и {{ yds-name }} вручную.
* Сделайте {{ IBRG }} приёмником для CDC и отслеживайте, фиксируйте и передавайте изменения данных.
...
Настраивайте репликацию из [{{ KF }}](../operations/endpoint/source/kafka.md) и [{{ DS }}](../operations/endpoint/source/data-streams.md) в [{{ IBRG }}](../operations/endpoint/target/iceberg.md) с помощью консоли управления или API.
...
* Встречайте обновлённый приёмник данных {{ objstorage-name }}. Поставки данных в него стали доступны для всех пользователей. Вы можете записывать данные в бакет в формате JSON, CSV и Parquet.

* Новые возможности репликации данных из очередей {{ KF }} и {{ DS }} в приёмник данных {{ IBRG }}. Больше информации о поддерживаемых парах источников и приёмников данных в [матрице трансферов](../transfer-matrix.md).
...
* Поддержаны репликации данных из очередей [{{ KF }}](../operations/endpoint/source/kafka.md) и [{{ DS }}](../operations/endpoint/source/data-streams.md) в приемник данных [{{ IBRG }}](../operations/endpoint/target/iceberg.md).


* Приемник данных [{{ objstorage-name }}](../operations/endpoint/target/object-storage.md) обновлен и доступен для всех пользователей.
