...
1. Добавлена поддержка [источника {{ objstorage-name }}](../operations/endpoint/source/object-storage.md). Для подробной информации смотрите [матрицу доступных трансферов](../transfer-matrix.md). Эта функциональность находится на стадии Preview и предоставляется по запросу.
