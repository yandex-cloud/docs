...
- {{ MY }}
    ...
    1. Убедитесь, что источник использует подсистему хранения данных низкого уровня MyISAM или InnoDB. При использовании других подсистем, трансфер может завершиться с ошгибкой.
...
- {{ mpg-name }}
    ...
    1. Настройте пользователя с нужными правами. После старта трансфер подключится к источнику от имени этого пользователя:
        ...
        1. [Подключитесь к базе данных](../../managed-postgresql/operations/connect.md), которую нужно мигрировать, от имени владельца базы и [настройте привелегии](../../managed-postgresql/operations/grant.md#grant-privilege):
