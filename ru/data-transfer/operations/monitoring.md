...
{% if audience != "internal" %} Собирает значения метрик трансфера и отображает графики сервис [{{ monitoring-name }}](../../monitoring/concepts/index.md). {% endif %} Чтобы перейти к работе с метриками, дашбордами или алертами в сервисе {{ monitoring-name }}, нажмите кнопку **Открыть в Мониторинге** на панели сверху.
...
|---------------------------------------|:------------------------------------:|:-------:|:---------:|
| Максимальное отставание данных | `sinker.pusher.time.row_max_lag_sec` | `15`    | —         |
