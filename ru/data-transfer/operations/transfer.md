...
- {{ TF }} {#tf}
    ...
    1. Создайте конфигурационный файл с описанием трансфера.
       ...
       * (Опционально) Укажите объекты для переноса в блоке `data_objects`. Список включаемых таблиц передается через `include_objects`. В отличие от [настроек эндпоинтов](endpoint/index.md), в блоке нельзя исключать таблицы, но вы можете использовать эту настройку в сочетании с настройками эндпоинта. Это удобно для сценариев, когда один эндпоинт-источник участвует в нескольких трансферах и есть потребность настраивать список таблиц потрансферно.
...
Активация необходима каждому новому трансферу. Исключение: трансфер типа `INCREMENT_ONLY` или `SNAPSHOT_AND_INCREMENT`, [созданный](../operations/transfer.md#create) с помощью Terraform.
...
В процессе деактивации трансфера происходят действия по очистке артефактов запуска. Например:
