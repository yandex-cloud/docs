        ...
        * (Опционально) **Среда выполнения** — тип системы и параметры запуска трансфера:
            ...
            * **{{ yandex-cloud }}**:
                ...
                * **Параметры шардированного копирования** — параметры параллельного чтения из таблиц для увеличения пропускной способности трансфера. Для источника {{ PG }} параллельность доступна только для таблиц, которые содержат первичный ключ в [режиме serial](https://www.postgresql.org/docs/current/datatype-numeric.html#DATATYPE-SERIAL).
                    ...
                    * **Количество воркеров** — количество виртуальных машин в {{ compute-full-name }}, на которых будет запущен трансфер. Рекомендуется указывать значение от 2 до 8.
                    * **Количество потоков внутри воркера** — количество потоков для выполнения трансфера в инстансе. Рекомендуется указывать значение от 4 до 8.
{% else %}
        * **Среда выполнения** — тип системы и параметры запуска трансфера:

            * **YT**:

               * **Кластер YT** - Кластер YT, на котором будут запущены операции трансфера (`ARNOLD`, `HAHN`, `VANGA`).
               * **Пул YT** - Пул YT, на котором будут запущены операции трансфера. Оставьте поле пустым, если пул не известен.
               * **CPU** - Гарантированная доля CPU для каждой операции.
               * **RAM** - Гарантированный объем RAM для каждой операции. Если вам нужно больше RAM, чем можно выставить через эту форму, обратитесь к администраторам.
               * **Параметры копирования** — параметры параллельного чтения из таблиц для увеличения пропускной способности трансфера. Для источника {{ PG }} параллельность доступна только для таблиц, которые содержат первичный ключ в [режиме serial](https://www.postgresql.org/docs/current/datatype-numeric.html#DATATYPE-SERIAL).

                   * **Количество воркеров** — количество воркеров, запущенных для копирования.
                   * **Количество потоков внутри воркера** — количество потоков для копирования внутри одного воркера. Настройка работает только на этапе снапшота.

            * **Мульти YT** – добавьте несколько кластеров YT, на которых будут запущены операции трансфера.
{% endif %}
        * (Опционально) **Копирование** — регулярность активации трансфера типа _{{ dt-type-copy }}_:
...
- Консоль управления
    ...
    1. Измените параметры трансфера:
        ...
        * **Копирование** — регулярность активации трансфера типа _{{ dt-type-copy }}_:
