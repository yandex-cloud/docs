...
Смотрите также [рекомендации по настройке эндпоинта](#recommended-settings-queue) при [поставке данных в {{ CH }} из очередей](#scenarios).
...
- Консоль управления {#console}
    ...
    * Если в кластере-приемнике {{ CH }} включено шардирование и перенос выполняется в шардированную таблицу, записывайте данные в [локальные таблицы](../../../../managed-clickhouse/tutorials/sharding.md#uses), а не в распределенную таблицу. При этом выполняйте выборку перенесенных данных в приемнике из распределенной таблицы. Чтобы переопределить таблицу для записи, укажите ее в настройке приемника **{{ ui-key.yc-data-transfer.data-transfer.console.form.clickhouse.console.form.clickhouse.ClickHouseTargetAdvancedSettings.alt_names.title }}** → **{{ ui-key.yc-data-transfer.data-transfer.console.form.common.console.form.common.AltName.to_name.title }}**.
