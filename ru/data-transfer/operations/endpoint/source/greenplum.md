...
# Передача данных из эндпоинта-источника {{ GP }}/{{ CB }}

С помощью сервиса {{ data-transfer-full-name }} вы можете переносить данные из базы {{ GP }} и {{ CB }} и реализовывать различные сценарии переноса, обработки и трансформации данных. Для реализации трансфера:
...
## Сценарии передачи данных из {{ GP }} и {{ CB }} {#scenarios}
...
1. {% include [data-mart](../../../../_includes/data-transfer/scenario-captions/data-mart.md) %}
    ...
    * [Загрузка данных из {{ GP }} или {{ CB }} в витрину {{ CH }}](../../../tutorials/greenplum-to-clickhouse.md);
    * [Загрузка данных из {{ GP }} или {{ CB }}  в витрину {{ PG }}](../../../tutorials/greenplum-to-postgresql.md).
...
## Подготовка базы данных {{ GP }} и {{ CB }} {#prepare}

{% include [prepare greenplum db](../../../../_includes/data-transfer/endpoints/sources/greenplum-prepare.md) %}

## Настройка эндпоинта-источника {{ GP }} и {{ CB }} {#endpoint-settings}
...
- Консоль управления {#console}
    ...
    * **{{ ui-key.yc-data-transfer.data-transfer.console.form.greenplum.console.form.greenplum.GreenplumTableFilter.exclude_tables.title }}** — данные таблиц из этого списка передаваться не будут.
      ...
      * `<имя_таблицы>` — таблица в схеме по умолчанию.
      ...
      Имена включенных и исключенных таблиц должны соответствовать правилам именования идентификаторов в {{ GP }} и {{ CB }}. Двойные кавычки внутри имени таблицы не поддерживаются. Внешние кавычки используются только как разделители и будут удалены при обработке путей.
...
### Особенности работы с источником {{ GP }}/{{ CB }} {#advanced}

{{ data-transfer-name }} поддерживает {{ GP }} версий 6.28 и выше, и {{ CB }} версии Cloudberry 2.

Сервис выполняет операции с кластером в транзакциях с [уровнем изоляции]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-sql_commands-SET_TRANSACTION.html) `READ COMMITTED`.

{{ data-transfer-name }} поддерживает работу с включенным [параллельным копированием](../../../concepts/sharded.md) для источника {{ GP }} и {{ CB }}.

Во время работы с включенным параллельным копированием {{ data-transfer-name }} удерживает открытую транзакцию на хосте-мастере. При прерывании этой транзакции трансфер завершится с ошибкой.

При отключенном параллельном копировании трансфер переносит данные объектов {{ GP }} или {{ CB }} `TABLE`, `VIEW`, `FOREIGN TABLE`, и `EXTERNAL TABLE`. Данные из этих объектов трактуются как данные из обыкновенных таблиц и обрабатываются приемником соответственно. При включенном параллельном копировании трансфер переносит только таблицы (объекты `TABLE`), причем таблицы с [политикой распределения]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-sql_commands-CREATE_TABLE.html) `DISTRIBUTED REPLICATED` не переносятся.

Если в трансфере из {{ GP }}/{{ CB }} в {{ GP }}/{{ CB }} не используется прямое чтение с сегментов, количество потоков не может превышать минимальное число сегментов в участвующих кластерах.
