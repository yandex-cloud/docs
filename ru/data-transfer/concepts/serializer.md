...
* **dt.batching.max.size** — максимальный размер пакета сообщений в байтах.
    ...
    Настройка актуальна, если используется JSON-сериализация со {{ schema-registry-name }} (смотрите параметры **key.converter** и **value.converter**). При использовании {{ schema-registry-name }} сообщения, приходящие в очередь, могут стать очень маленькими. Пакетирование в таком случае позволяет увеличить пропускную способность очередей.
