...
Трансферы типа {{ dt-type-copy }} _из {{ CH }} в {{ CH }}_ не поддерживают работу с `VIEW`. В эндпоинте-источнике типа {{ CH }} `VIEW` должны быть включены в **{{ ui-key.yc-data-transfer.data-transfer.console.form.clickhouse.console.form.clickhouse.ClickHouseTableFilter.exclude_tables.title }}**, если **{{ ui-key.yc-data-transfer.data-transfer.console.form.clickhouse.console.form.clickhouse.ClickHouseTableFilter.include_tables.title }}** пуст или не указан. Если **{{ ui-key.yc-data-transfer.data-transfer.console.form.clickhouse.console.form.clickhouse.ClickHouseTableFilter.include_tables.title }}** непустой, в нем не должно присутствовать объектов `VIEW`.
...
* **Для таблиц, партиционированных методом наследования**:
    ...
    * При переносе базы данных из {{ PG }} в другую СУБД пользователь может включить в эндпоинте-источнике настройку [Объединить наследуемые таблицы](../operations/endpoint/source/postgresql.md#additional-settings). 

        Поведение трансфера при использовании настройки **Объединить наследуемые таблицы** зависит от значения этой настройки, а также таблиц, указанных в параметре **{{ ui-key.yc-data-transfer.data-transfer.console.form.migration.console.form.migration.PostgresMigrationTablesFilter.include_tables.title }}**:

        | **Указанные таблицы** | **Поведение трансфера при включенной настройке** | **Поведение трансфера при выключенной настройке** |
        |-----------------------|--------------------------------------------------|---------------------------------------------------|
        | Только родительская таблица | Будет перенесена родительская таблица с данными из всех таблиц-наследников | Будет перенесена только родительская таблица |
        | Все или некоторые таблицы-наследники | Будут перенесены данные из указанных таблиц-наследников. Для каждой таблицы-наследника будет создана отдельная таблица на приемнике |  Будут перенесены только указанные таблицы-наследники |
        | Родительская таблица и все таблицы-наследники | Будет перенесена родительская таблица с данными из всех таблиц-наследников | Будут перенесены родительская таблица и все таблицы-наследники указанной родительской таблицы |
        | Родительская таблица и некоторые таблицы-наследники | Трансфер завершится с ошибкой `not all child tables matched` |  Трансфер завершится с ошибкой `not all child tables matched` |
...
Для приемника {{ objstorage-name }} можно настроить политику очистки данных перед копированием. Политика очистки применяется при активации, повторной активации и перезапуске трансфера и определяет порядок работы с данными, которые уже есть в бакете. Доступны следующие политики очистки:

* **Не очищать** — данные не удаляются из бакета. Эта политика используется по умолчанию.
