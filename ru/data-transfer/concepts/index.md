...
Когда источник и приемник разных видов, при трансфере переносятся только объекты, содержащие данные. Например, при трансфере {{ PG }} → {{ CH }} пустые таблицы передаваться не будут.
