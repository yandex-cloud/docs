...
Если хосты в настройках эндпоинта указаны в виде доменных имен, то для их разрешения в IP-адреса будут использоваться {% if lang == "ru" and audience != "internal" %}[DNS-серверы](../../glossary/dns.md#dns-server){% else %}DNS-серверы{% endif %}, указанные в настройках DHCP выбранной подсети. Подробнее см. в разделе [IP-адреса и доменные имена в настройках эндпоинтов](#ip-addresses-and-domain-names).
...
{% if product == "yandex-cloud" %}* с помощью [{{ interconnect-full-name }}](../../interconnect/);{% endif %}
* с помощью промежуточной {% if lang == "ru" and audience != "internal" %}[ВМ](../../glossary/vm.md){% else %}ВМ{% endif %}, на которой настроена [маршрутизация трафика в {{ vpc-name }}](../../vpc/concepts/static-routes.md).
...
## Трансфер между Яндексом и внешним {{ yandex-cloud }} {#internal-to-external}
...
Чтобы переносить данные во внешнее Облако, например, в {{ mch-name }}, нужно выполнить следующее:
...
2. Запросите через [Puncher](https://puncher.yandex-team.ru/) доступ к выделенному project_id от макроса `_YTVANGANETS_` либо `_YTNETS_`. Для {{ mch-name }} укажите порты TCP {{ port-mch-http }} и {{ port-mch-cli }}. Информацию о портах для других БД см. в [документации MDB](../../mdb/access.md#network-access).
3. Создайте в облаке кластер {{ mch-name }} или другой требуемой БД, разместив его хосты в сети из п.1
...
## Трансферы между Яндексом и хранилищами в интернете {#internal-to-internet}
