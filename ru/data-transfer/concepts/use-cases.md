...
Сервис {{ data-transfer-name }} позволяет протестировать работу с сервисами {% if product == "yandex-cloud" %}[{{ mmg-full-name }}](../../managed-mongodb/), {% endif %}[{{ mmy-full-name }}](../../managed-mysql/) и [{{ mpg-full-name }}](../../managed-postgresql/) на реальных данных.
