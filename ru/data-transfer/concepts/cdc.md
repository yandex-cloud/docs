...
{{ data-transfer-name }} поддерживает CDC-режим для трансферов из баз данных {{ PG }}, {{ MY }} и {{ ydb-short-name }} в {{ KF }}, {{ IBRG }} и {{ yds-full-name }}^1^. При этом данные в приемник попадают в формате [Debezium](https://debezium.io/).
...
Готовность реализации для разных типов инсталляции {{ data-transfer-full-name }} может отличаться. Трансфер для источника {{ MG }} и приемника {{ KF }} пока не поддерживается (подробнее в [списке доступных трансферов](../transfer-matrix.md)).
