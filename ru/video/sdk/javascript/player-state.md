---
title: Состояния плеера {{ video-player-name }} в SDK для JavaScript
description: На странице описаны возможные состояния, которые может принимать видеоплеер {{ video-player-name }} в SDK для JavaScript.
---

# Состояние плеера

Вы можете получать информацию о состоянии видеоплеера и его [параметрах](#player-params) через [объект состояния](#player-state), содержащий поля со значениями состояний.
...
Значение скорости — число, например:
...
Области буферизации. Массив объектов формата:
...
Массив доступных текстовых дорожек (субтитров). Каждый элемент — объект формата:

```javascript
{
    /** @type {string} название дорожки, отображается в интерфейсе */
    name,
    /** @type {string} id дорожки, используется для смены через setTextTrack */
    value,
    /** @type {string} язык дорожки в формате ISO 639-1 (например, "ru", "en") */
    lang,
}
```
...
Текущая текстовая дорожка (субтитры). `null` — если субтитры отключены.

Формат объекта аналогичен элементам массива [textTracks](#state-textTracks).

Значение по умолчанию — `null`.

#### videoTracks {#state-videoTracks}

Массив доступных видеодорожек. Каждый элемент — объект формата:

```javascript
{
    /** @type {string} название дорожки, отображается в интерфейсе */
    name,
    /** @type {string} id дорожки, используется для смены через setVideoTrack */
    value,
    /** @type {number} битрейт видео в Kbps */
    bitrate,
    /** @type {number} [width] ширина видео */
    width,
    /** @type {number} [height] высота видео */
    height,
    /** @type {boolean} [auto] true — качество выбрано плеером автоматически, false — вручную или через API */
    auto,
    /** @type {number} [frameRate] количество кадров в секунду */
    frameRate,
    /** @type {string} [codecs] кодек */
    codecs,
}
```
...
Текущая видеодорожка. `null` — если плеер еще не выбрал дорожку (например, до готовности).

В режиме автоматического выбора возвращается объект дорожки с полем `auto: true`.

Формат объекта аналогичен элементам массива [videoTracks](#state-videoTracks).

Значение по умолчанию — `null`.

#### Полезные ссылки {#see-also}

[Interface: PlayerSdkState](../../api-ref/javascript/interfaces/PlayerSdkState.md) — описание параметров состояния плеера в справочнике API
