...
Суммаризация позволяет автоматически создавать краткое содержание видео, которое передает его основные моменты и ключевую информацию. При включении суммаризации к видео автоматически добавляются таймкоды. При необходимости можно скорректировать предложенные или добавить свои [таймкоды вручную](#timecodes).

### Нейроперевод {#stranslation}

Нейроперевод позволяет автоматически переводить видео на несколько языков и озвучивать его [разными голосами](https://habr.com/ru/news/905118/). Список поддерживаемых языков см. в [интерфейсе {{ video-name }}]({{ link-video-main }}).

### Субтитры {#subtitles}

Нейросетевые субтитры помимо текста, который произносится в видео, содержат разделение на спикеров. Речь нового спикера будет начинаться с новой строки и тире, что повышает удобство восприятия речи. Список поддерживаемых языков см. в [интерфейсе {{ video-name }}]({{ link-video-main }}).
