...
При расширенном шардировании роль координатора выполняет хост `COORDINATOR`. Чтобы обеспечить отказоустойчивость кластера, рекомендуется создать три хоста `COORDINATOR` в разных зонах доступности. Максимальное число хостов `COORDINATOR` — пять.
