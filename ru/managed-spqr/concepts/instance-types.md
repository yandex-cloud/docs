...
Хосты в кластерах {{ mspqr-name }} разворачиваются на базе виртуальных машин {{ compute-full-name }}. Эти виртуальные машины могут быть созданы на любой из платформ, которые поддерживает {{ compute-name }}. Подробное описание платформ приведено в разделе [Платформы](../../compute/concepts/vm-platforms.md). Размер хранилища не зависит от класса хостов.
...
Например, в `{{ region-id }}-d` недоступна платформа Intel Broadwell. 
