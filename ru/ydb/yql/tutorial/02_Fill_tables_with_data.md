...
Давайте заполним таблицы данными.
...
Для записи данных в таблицы используются операции [REPLACE](../reference/syntax/replace_into.md), [UPSERT](../reference/syntax/upsert_into.md) и [INSERT](../reference/syntax/insert_into.md). Поиск строки, которую нужно обновить, выполняется по первичному ключу.

REPLACE и UPSERT используются для вставки новых данных или обновления существующих. INSERT используется для вставки новой записи.

REPLACE и UPSERT более эффективны с точки зрения производительности, чем INSERT.
При выполнении INSERT, {{ ydb-short-name }} сначала выполняет операции чтения, чтобы проверить, что
такого первичного ключа не существует. Затем выполняется операция записи.

При выполнении REPLACE или UPSERT, {{ ydb-short-name }} выполняет слепую запись без предварительных чтений.
Рекомендуется использовать REPLACE для операций записи первичных данных, поскольку колонки, не участвующие в запросе, будут заполнены значениями по умолчанию.

В {{ ydb-short-name }} можно вставлять несколько колонок в один запрос REPLACE.
...
VALUES
    ...
    -- По умолчанию числовые литералы имеют тип Int32, если значение соответствует диапазону.
