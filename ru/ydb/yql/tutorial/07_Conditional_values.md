...
Задача: вывести все названия эпизодов первого сезона каждого сериала и отсортировать по имени.
...
FROM episodes
WHERE series_id IN (1,2)        -- IN указывает условие, согласно которому
                                -- значения должны попасть в результат.
                                -- Синтаксис:
                                ...
                                -- IN или NOT IN может привести к неожиданным результатам.
AND season_id = 1
GROUP BY
