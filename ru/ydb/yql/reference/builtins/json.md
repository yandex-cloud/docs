...
Предикат `like_regex` позволяет проверить строку на соответствие регулярному выражению. Синтаксис регулярных выражений такой же как в [HyperScan UDF](../udf/list/hyperscan.md) и [REGEXP](../syntax/expressions.md#_2).
...
1. `(1.3).ceiling()` возвращает `1`
2. `(1.8).ceiling()` возвращает `1`
3. `(1.5).ceiling()` возвращает `1`
4. `(1.0).ceiling()` возвращает `1`
