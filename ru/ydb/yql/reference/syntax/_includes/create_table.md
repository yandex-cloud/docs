...
Вызов `CREATE TABLE` создает {% if concept_table %}[таблицу](../../../../concepts/datamodel#table){% else %}таблицу{% endif %} с указанной схемой данных{% if feature_map_tables %}  и ключевыми колонками (`PRIMARY KEY`){% endif %}. {% if feature_secondary_index == true %}Позволяет определить вторичные индексы на создаваемой таблице.{% endif %}
...
Конструкция INDEX используется для определения {% if concept_secondary_index %}[вторичного индекса](../../../../concepts/secondary_indexes){% else %}вторичного индекса{% endif %} на таблице:
