...
* Dedicated
   ...
   1. В блоке **Сеть** выберите сеть и включите присвоение публичных ip-адресов узлам базы данных.
      ...
      * В открывшемся окне укажите имя новой сети и выберите опцию **Создать подсети**, если  {% if deploy != "arc" %} [подсети](../../vpc/concepts/network.md#subnet) {% else %} подсети {% endif %}  не были созданы ранее. Подсети будут созданы для каждой из  {% if deploy != "arc" %} [зон доступности](../../overview/concepts/geo-scope.md) {% else %} [зон доступности] {% endif %}.
