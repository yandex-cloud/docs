...
Записывает или удалеят элементы из таблиц.
...
Если из-за недостаточной пропускной способности не обработается ни один элемент, то метод вернет ошибку `ProvisionedThroughputExceededException`. Если будет обработан хотя бы один элемент, то метод завершится успешно, а необрабоатнные элементы вернутся в параметре `UnprocessedItems`.
