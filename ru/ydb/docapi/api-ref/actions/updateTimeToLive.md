...
Механизм проверки TTL сравнивает текущее время в формате [Unix time]{% if lang == "en" %}(https://en.wikipedia.org/wiki/Unix_time){% endif %}{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/Unix-время){% endif %} со временем, которое хранится в TTL-атрибуте элемента. Если оно меньше текущего времени, элемент помечается как устаревший и впоследствии удаляется.
