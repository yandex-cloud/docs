...
* `SHOW PREVIEW cluster.[path/to/table];` — предпросмотр данных таблицы. Логически мало чем отличается от `SELECT *`, но иногда может работать немного быстрее из-за дополнительного слоя кэширования.
{% if yt %}
Особенности:
