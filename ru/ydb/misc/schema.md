...
### Стандартный способ схематизации: мета-атрибуты schema и \_read\_schema {#standard-method-of-schematization-meta-attributes-of-schema-and-read-schema}
...
* Подробности о работе схем в YT описаны [здесь](https://wiki.yandex-team.ru//yt/design/schema).
...
### Альтернативная схема для таблицы — VIEW и UDF {#alternate-schema-for-able-view-and-udf}
В ряде случаев помимо основной схемы для таблицы удобно определить иметь одну или несколько дополнительных схем, которые чем-то отличаются от основной. Например, схемы, адаптированные под типичные паттерны использования или с дополнительной рекомендуемой пред- или постобработкой.
...
### Способы схематизации, поддерживаемые для совместимости {schema-supported-for-compatibility}
...
#### Схематизация таблиц в формате Key—Subkey—Value (YaMR-формат) {#schematicization-of-tables-in-key-subkey-value-format-yamr-format}
...
#### Приведение таблиц к формату Key—Subkey—Value (YaMR-формат) {#casting-tables-to-key-subkey-value-format-yamr-format}
...
#### Таблицы, содержащие сериализованные данные protobuf message {#tables-containing-serialized-data-protobuf-message}
...
#### Таблицы, не имеющие схемы {#tables-that-do-not-have-a-schema}
...
## Мета-атрибуты для схематизации данных {#meta-attributes-for-schematizing-data}
...
### (schema) schema — !!(зел)рекомендуется к использованию!! {#schema-green-recommended-for-use}
...
### \_read\_schema — не рекомендуется {#read-schema-not-recommended}
...
### (proto\_field) \_yql\_proto\_field {#proto-field-yql-proto-field}
...
### \_format — не рекомендуется {#format-not-recommended}
...
### \_yql\_view\_$viewname {#yql-view-viewname}
...
### \_yql\_row\_spec — не рекомендуется {#yql-row-spec-not-recommended}
...
## Как проверить, схематизированы ли ваши данные? {#how-to-check-if-your-data-is-schematized}
...
## Как разметить/переразметить свои данные? {#how-to-mark-redistribute-your-data}
...
## Какие данные сейчас схематизированы? {#what-data-is-schematized-now}
О схематизации данных в общей квоте logfeller `//logs` читайте [здесь](https://wiki.yandex-team.ru/logfeller/schema/yt/).
...
## Порядок обработки мета-атрибутов для схематизации {#order-of-processing-meta-attributes-for-schematization}
...
#### 1) Почему схем нет по умолчанию на «поколоночных» YT-таблицах, почему не сделать так же, как там, для всех таблиц? {#why-there-are-no-schemas-by-default-on-the-by-columns-yt-tables-why-not-do-the-same-for-all-tables}
...
#### 2) Какие типы сейчас доступны? {#what-types-are-now-available}
...
#### 3) Как залить данные с колонками разных типов? {#how-to-load-data-with-different-types-of-columns}
<https://yql.yandex-team.ru/?cluster=hahn&path=home/yql/examples/complex_types>
```SELECT * FROM hahn.[home/yql/complex_types];
```
#### 4) Что значат вопросики в превью схем? {#what-do-the-question-marks-mean-in-the-preview-schemes}
...
#### 5) Что делать, если я хочу обращаться в YQL к чужим данным, которые не схематизированы? {#what-to-do-if-i-want-to-access-other-peoples-data-in-yql-that-are-not-schematized}
...
#### 6) Почему способов «описать» данные так много? {#why-are-there-so-many-ways-to-describe-data}
...
#### 7) Почему есть \_read\_schema и schema? {#why-is-read-schema-and-schema}
