...
В столбце _Тип ошибки_ указан статус, с которым завершится выполнение запроса в случае ошибки. Подбробнее статусы описаны в разделе [Обработка ошибок в API](error_handling.md).
...
| Таблица | Максимальное количество копируемых таблиц | 1000 | Ограничение на размер списка таблиц операции консистентного копирования таблиц| MaxConsistentCopyTargets | GENERIC_ERROR |
...
## Ограничение при выполнении запросов

В таблице ниже перечислены ограничения, действующие при выполнении запросов. В столбце _Вызов_ указан вызов public api, обращение к которому завершится со статусом ошибки, указанным в столбец _Статус_.

<small>Таблица 3 — Ограничение при выполнении запросов</small>
