...
В профиле {{ ydb-short-name }} CLI вы можете задать эндпоинт, полный путь базы данных, а также способ авторизации пользователя, от имени которого будут выполняться команды. Использование профиля позволяет не указывать перечисленные параметры при каждом вызове команд {{ ydb-short-name }} CLI.
