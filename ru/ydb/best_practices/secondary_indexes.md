...
В разделе [Основные понятия / Вторичные индексы](../concepts/secondary_index.md) кратко описан подход, позволяющий реализовать в {{ ydb-short-name }} вторичные индексы на уровне приложения. Рассмотрим, как данный подход позволит повысить эффективность вывода списка из N сериалов, отсортированных в обратном порядке по количеству просмотров.

### Схема данных {#schema}

В примере используется схема данных, описанная в документе [Начало работы / Чтение и запись данных](../operations/crud.md).
