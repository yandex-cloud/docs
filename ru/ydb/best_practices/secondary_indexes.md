...
В разделе описано, как использовать [вторичные индексы](https://cloud.yandex.ru/docs/ydb/oss/public/develop/concepts/secondary_indexes) для выборки данных.

В общем случае транзакции с использованием глобального индекса являются [распределенными](https://cloud.yandex.ru/docs/ydb/oss/public/develop/concepts/transactions#distributed-tx). Запрос может быть выполнен как одношардовая транзакция в следующих случаях:

* точечное чтение по первичному ключу;
* точечное чтение по индексной колонке, если запрашиваемые данные представляют собой первичный ключ, его часть или имеется копия данных в индексе ([covering index](https://cloud.yandex.ru/docs/ydb/oss/public/develop/concepts/secondary_indexes#covering));
* точечная слепая запись в таблицу с [асинхронным индексом](https://cloud.yandex.ru/docs/ydb/oss/public/develop/concepts/secondary_indexes#async).
...
Возможно добавление вторичного индекса для существующей таблицы без остановки обслуживания. Подробнее об онлайн-создании индекса читайте в [инструкции](https://cloud.yandex.ru/docs/ydb/oss/public/develop/concepts/secondary_indexes#index-add).
