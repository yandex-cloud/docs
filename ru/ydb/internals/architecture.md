...
В процессе разработки **YDB** было решено использовать RSM гораздо более массово, чем RSM применялся в тот момент времени (для мастера метаданных, менеджера блокировок и т.п.). RSM в **YDB** отвечает за маленький кусочек системы, и за каждый кусочек системы, требующий надёжности, отвечает RSM.
 ...
 * **[State-storage](tablet_state_storage.md)** - (волатильно) хранит информацию о текущем мастере - используется для discovery и как хинт в процессе восстановления;
...
1. На уровене таблетки и BlobStorage:
  ...
  1.2. Применяются асинхронные standby фолловеры для мастеров таблеток, обеспечивающие близкое к нулю время переключения и готовность нового мастера принимать транзакции в случае сбоя текущего мастера таблетки.
