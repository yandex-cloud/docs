...
Хотя у таблетки должно существовать не более одного активного мастера, иногда полезно поддерживать дополнительно несколько реплик-фолловеров. Этим можно решить одну из нескольких задач:
...
При запуске таблетки (либо явной командой с **[Hive'a](tablet_hive.md)**, либо указав возможность запуска фолловеров в **[bootstrapper](glossary.md#bootstrapper)**) можно назначить новый инстанс таблетки не кандидатом, а слейвом.
  ...
  * Весь дальнейший лог транслируется мастером на фолловеров, а фолловеры применяют изменения к состоянию в памяти.
...
При обслуживании чтений слейвам может понадобиться загрузить данные с диска, полученные фрагменты складываются в локальном кеше и могут быть переиспользованы при апгрейде слейва до мастера. Альтернативно профиль запросов может отгружаться с мастера на фолловеров для синхронизации состояния кеша (полезно при использовании слейва как hot-standby).
