## Что такое BlobStorage {#what-is-blobstorage}
...
* Pdisk – непосредственно пишет данные на жесткий диск,
...
## Организация Blob Storage {#organization-of-blob-storage}

### Группы Blob Storage {#groups-of-blob-storage}
...
### Статическая и Динамическая конфигурация {#static-and-dynamic-configuration}
...
## Формат ключей записей в Blob Storage {#blob-storage-record-key-format}
...
Поля BlobSize хотя физически и участвуют в сравнении (т.к. внутри реализовано как сырое сравнение трех ui64), но фиксированы для конкретного набора остальных полей.
...
## API для взаимодействия с Blob Storage {#api-for-interacting-with-blob-storage}
