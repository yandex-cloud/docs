...
* Иначе алгоритм идет по всем существующим нодам и проверяет выполнение всех нижеперечисленных условий (по принципу "И"):
  ...
  * Если она может запустить данную таблетку?
    ...
    * Таблетка является мастером, для нее включен AllowFollowerPromotion и на ноде есть живой слейв.
    * Никто из мастеров/фолловеров таблетки еще не запущен на этой ноде.
...
* Если все условия выполнены, для ноды считается значение ее `usage` для данной таблетки:
  ...
  * Follower считается такой же таблеткой, как и Master. Со своими метриками и загрузкой.
