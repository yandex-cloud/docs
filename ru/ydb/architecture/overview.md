...
**На данный момент KiKiMR**:
  ...
  * реализует эффективное избыточное хранение данных;
...
Такая реализация RSM является базовым строительным блоком **KiKiMR** и называется **Таблеткой**.

В качестве вспомогательных механизмов используются:

 **State-storage** - (волатильно) хранит информацию о текущем мастере - используется для discovery и как хинт в процессе восстановления.

 **Hive** - one RSM to rule them all - служебная RSM, управляющая перезапуском управляемых таблеток, позволяет минимизировать конфликты и осведомлённо балансировать нагрузку по ресурсам кластера.
...
Все основные термины, понятия и определения полностью описаны в разделе **Glossary**. Для того, чтобы читателю было проще ориентироваться, ниже представлено краткое описание основных сущностей.

![kikimrstructure1.png](../_assets/kikimrstructure2.png)
                  (Рис.2 Набор компонент Kikimr)


 * **Tablet** - высокодоступная сущность, описываемая независимым куском данных, состояние которой полностью описывается упорядоченным логом команд. Таблетка состоит из пользовательского актора, экзекьютера и общетаблеточной части. Таблетки подразделяются на системные/служебные (например, Hive, Coordinator, etc) и не системные (например, Data Shard).
 * **Hive** - таблетка, интеллектуально распределяющая не системные таблетки по нодам и управляющая их жизнью.
 * **BlobStorage** - хранилище независимых иммутабельных объектов с адресацией по ключу, представляет собой набор BlobStorage групп.
 * **PDisk** - актор, управляющий физическим диском.
 * **VDisk** - актор, взаимодействующий с PDisk’ом, входит в одну из BlobStorage групп.
 * **Local** - актор, регистрируется в Hive и получает команды на запуск таблеток.
 * **DSProxy** - актор, играет роль интерфейса к операциям с BlobStorage.
 * **Warden (BSNodeController)** - актор, запускает акторы DSProxy, VDisk  и PDisk.
 * **TxProxy** - таблетка, клиентский интерфейс взаимодействия с **KiKiMR**.
 * **Coordinator** - таблетка, управляющая транзакциями и обеспечивающая глобальную упорядоченность, выбирается хешированием для конкретного Proxy.
 * **Mediator** - таблетка, применяющая транзакции на даташарды и обеспечивающая “движение” глобального времени.
 * **BlobStorage Controller** - таблетка, хранящая информацию обо всех VDisk’ах и BlobStorage группах.
 * **Scheme Shard** - таблетка, хранящая схему распределенной базы даных.
 * **Data Shard** -  таблетка, хранящая данные распределенной базы данных.
...
Информация о глобальной схеме хранится в собранных в дерево специализированных таблетках, которые называются **Scheme Shards**. Каждый узел глобальной схемы управляется конкретным Scheme Shard'ом. Непосредственно данные таблиц хранятся отдельно от схемы в шардированном виде в Data Shard'ах.

**Data Shard** - это таблетка, которая хранит часть данных глобальной таблицы (или просто таблицы) хранилища. Содержит в своей памяти индекс, соответствующий этой части таблицы (в зависимости от настроек - в диапазоне от полного in-memory представления индекса до минимального указания, где его в принципе искать) и использует лог таблетки (из **BlobStorage**) в качестве стабильной внешней памяти (адресуя конкретные элементы как [log-entry:shift:size]).
...
### Процесс загрузки KikiMR {#kikimr-download-process}
...
Таблетка BlobStorage Controller управляет динамической конфигурацией распределенного хранилища BlobStorage и контролирует все динамические BlobStorage группы в домене. Каждая BlobStorage группа состоит из множества VDisk-ов, объединенных в fail domains (например, к одному fail domain относятся VDisk-и из одной стойки с серверами), fail domains в свою очередь обьединены в Кольца (например, к одному кольцу относятся fail-domain-ы одного датацентра). Таким образом, группа состоит из Колец, каждое кольцо состоит и набора fail domains, каждый fail domain содержит множество VDisk'ов.
...
#### Transaction Proxy (TxProxy) {#transaction-proxy}
...
## Как достигается высокая производительность и надежность в KiKiMR {#how-to-achieve-high-performance-and-reliability-in-kikimr}
