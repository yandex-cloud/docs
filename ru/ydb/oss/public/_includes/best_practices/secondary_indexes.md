...
В разделе описано, как использовать [вторичные индексы](../../develop/concepts/secondary_indexes.md) для выборки данных.

В общем случае транзакции с использованием глобального индекса являются [распределенными](../../develop/concepts/transactions.md#distributed-tx). Запрос может быть выполнен как одношардовая транзакция в следующих случаях:

* точечное чтение по первичному ключу;
* точечное чтение по индексной колонке, если запрашиваемые данные представляют собой первичный ключ, его часть или имеется копия данных в индексе ([covering index](../../develop/concepts/secondary_indexes.md#covering));
* точечная слепая запись в таблицу с [асинхронным индексом](../../develop/concepts/secondary_indexes.md#async).
...
Возможно добавление вторичного индекса для существующей таблицы без остановки обслуживания. Подробнее об онлайн-создании индекса читайте в [инструкции](../../develop/concepts/secondary_indexes.md#index-add).
