...
При работе {{ ydb-name }} в режиме с выделенными инстансами вы оплачивете: 
...
* Объем, занимаемый резервными копиями баз данных по требованию, сохранёнными в сервисе {{ objstorage-name }}. Стоимость создания и хранение резервных копий рассчитывается по тарифам {{ objstorage-name }}.
   ...
   Yandex Database для каждой базы автоматические создаёт и бесплатно хранит 2 полные резервные копии за два последних дня. Плата за хранение автоматических резервных копий не взымается.
...
Все цены указаны с включением НДС. Цены за месяц указаны из расчета для месяца в 30 календарных дней. Для более коротких месяцев цена соответственно выше, для более длинных — ниже. Чтобы точно рассчитать стоимость для некоторых сценариев использования, воспользуйтесь [калькулятором](https://cloud.yandex.ru/services/ydb/#calculator).
...
Цены за время работы рассчитаны для [классов вычислительных ресурсов](../concepts/databases.md#compute-units). Чтобы точно рассчитать стоимость нужного кластера, воспользуйтесь [калькулятором](https://cloud.yandex.ru/services/ydb#calculator).
