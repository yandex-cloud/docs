---
title: Правила оценки стоимости YQL-запросов к {{ ydb-full-name }}
description: В статье содержатся правила оценки стоимости YQL-запросов к {{ ydb-name }}.
editable: false
---
...
Чтобы получить стоимость использования CPU, сумма времени CPU запроса делится на 1,5 мс, `( 475 + 514 + 4 062 + 870 ) / 1 500 = 3,95`. Результат округляется в меньшую сторону и переводится в RU:
...
* Вычисляется количество операций чтения с диска в записях и блоках, из них выбирается максимальное:
  ...
  * Прочитаны 16 байт, `16 / ( 4 × 1 024 ) = 0,004`, результат округляется в большую сторону, количество операций равно 1.
...
* Вычисляется количество операций записи на диск в записях и блоках, из них выбирается максимальное:
  ...
  * Записаны 2 456 байта, `2 456 / 1 024 = 2,398`, результат округляется в большую сторону, количество операций равно 3.
