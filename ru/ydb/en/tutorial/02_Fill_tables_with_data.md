...
Let's fill the demo tables with the demo data.

{% note info %}

In this example we assume that you have created demo tables on step [1.&nbsp;Create demo tables](01_Create_demo_tables.md).

{% endnote %}

There are [REPLACE](../../yql/reference/syntax/replace_into.md), [UPSERT](../../yql/reference/syntax/upsert_into.md) and [INSERT](../../yql/reference/syntax/insert_into.md) operators to write data in the {{ ydb-short-name }}.
REPLACE and UPSERT are used to insert a new record or update an existing one
by primary key value. INSERT is used to insert a new record.
REPLACE and UPSERT are more efficient than INSERT.
...
Contrary, while executing REPLACE or UPSERT, {{ ydb-short-name }} makes only blind write
(writing data without previously reading it in the same transaction).
It is recommended to use REPLACE on initial data write operations, because
columns that aren't participating in the query will be populated with the
default values.

{{ ydb-short-name }} allows you to insert multiple rows within a single REPLACE query.

```sql
REPLACE INTO series (series_id, title, release_date, series_info)
VALUES
    -- By default numeric literals have type Int32 if the value fits in range,
    -- otherwise they automatically expand to Int64.
    (
        1,
        "IT Crowd",
        CAST(Date("2006-02-03") AS Uint64),   -- CAST converts one datatype into
                                              -- another datatype. There is a
                                              -- possibility to convert string
                                              -- literal into simple type
                                              -- literal. Date() converts string
                                              -- literal in the ISO 8601 format into
                                              -- date.
