...
В режиме локального запуска для работы с базой данных предоставляется только {{ ydb-short-name }} API. API доступен на эндпоинте `grpcs://localhost:2135`, имя базы данных `/local`. Работа с базой выполняется с помощью консольного клиента {{ ydb-short-name }} ([YDB CLI](../quickstart/yql-api/ydb-cli.md)), предустановленного внутри Docker-образа. Посмотрите [примеры запросов](../quickstart/yql-api/ydb-cli.md#examples) к базе данных с помощью YDB CLI.
