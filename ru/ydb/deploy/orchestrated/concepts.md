---
sourcePath: core/deploy/orchestrated/concepts.md
---
# Обзор

Развертывание Yandex Database в {{ k8s }} — это простой и доступный способ установки Yandex Database. С {{ k8s }} вы можете использовать универсальный подход к управлению приложением в любом облачном провайдере. Это руководство содержит инструкции для развертывания YDB в [Yandex Managed Service for Kubernetes](yc_managed_kubernetes.md) и [AWS EKS](aws_eks.md).

YDB поставляется в виде Helm-чарта — пакета, который содержит шаблоны структур {{ k8s }}. Чарт может быть развернут в следующем окружении:
...
Helm-чарт устанавливает в кластер {{ k8s }} контроллер, который реализует необходимую логику для развертывания компонентов Yandex Database. Контроллер построен по паттерну [Оператор](https://kubernetes.io/docs/concepts/extend-kubernetes/operator/).
