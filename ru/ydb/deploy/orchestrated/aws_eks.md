---
sourcePath: core/deploy/orchestrated/aws_eks.md
---
# Развертывание Yandex Database в AWS EKS
...
1. Создайте кластер {{ k8s }}.
    ...
    Будет создан кластер {{ k8s }} с именем `yandex-database`. Флаг `--node-type` указывает, что кластер будет развернут с использованием инстансов `c5a.2xlarge` (8vCPUs, 16 GiB RAM), что соответствует нашим рекомендациям по запуску YDB.
