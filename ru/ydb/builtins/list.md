...
* ListFlatMap — возвращает список с результатами, но дополнительно пропускает «пустые» значения (NULL optional-типа). В дополнение к этому, если результат для каждого элемента является списком, объеденяет их (но только один уровень).
