...
База данных {{ ydb-short-name }} является региональной и располагается в регионе `ru-central1`. Регион `ru-central1` имеет три зоны доступности. Подробнее о регионах и зонах доступности читайте в разделе [Зоны доступности](https://cloud.yandex.ru/docs/overview/concepts/geo-scope).
