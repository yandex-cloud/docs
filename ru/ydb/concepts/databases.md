...
База данных {{ ydb-short-name }} является региональной и располагается в регионе `ru-central`. Регион `ru-central` имеет три зоны доступности. Подробнее про регионы и зоны доступности читайте в разделе {% if deploy != "arc" %}[{#T}](../../overview/concepts/geo-scope.md){% else %}зоны доступности{% endif %}.
{% endif %}
