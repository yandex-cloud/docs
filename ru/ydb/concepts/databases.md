...
База данных {{ ydb-short-name }} является региональной и располагается в регионе `ru-central1`. Регион `ru-central1` имеет три зоны доступности. {% if audience != "internal" %}Подробнее о регионах и зонах доступности читайте в разделе [{#T}](../../overview/concepts/geo-scope.md).{% endif %}
