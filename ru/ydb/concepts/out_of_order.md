...
* Приложение использует пару таблиц вида master/detail
* Приложение знает, что в master/detail запись попадает одновременно
* Приложение проверяет наличие записи в таблице master и находит объект
* Приложение читает дополнительные поля из таблицы detail без обращения к таблице master и не находит объект

В случае, если нужны дополнительные гарантии на порядок выполнения, рекомендуется в транзакциях чтения из detail таблицы выполнять также чтения из master таблицы (даже если эти данные непосредственно не нужны), в виде JOIN или отдельно стоящего проверочного SELECT.
