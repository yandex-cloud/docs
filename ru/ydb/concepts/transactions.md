...
Этот раздел описывает особенности реализации {% if audience == "external" %} YQL {% else %} [YQL](https://yql.yandex-team.ru/docs/ydb/) {% endif %} для YDB транзакций.

## Язык запросов
Основным средством создания, модификации и управления данными в YDB является декларативный язык запросов {% if audience == "external" %} YQL {% else %} [YQL](https://yql.yandex-team.ru/docs/ydb/) {% endif %}. YQL — это диалект SQL, который может считаться стандартом для общения с базами данных. Кроме того YDB поддерживает набор специальных RPC, например, для работы с древовидной схемой или для управления кластером.
...
Подробнее про поддерживаемые конструкции YQL можно почитать в {% if audience == "external" %} [документации YQL](../yql/reference/overview.md) {% else %} [документации YQL](https://yql.yandex-team.ru/docs/ydb/) {% endif %} .
...
Подробнее про поддержку YQL в YDB можно прочитать в {% if audience == "external" %} [документации YQL](../yql/reference/overview.md) {% else %} [документации YQL](https://yql.yandex-team.ru/docs/ydb/) {% endif %} .
...
В YDB поддерживаеются распределенные транзакции — такие транзакции, которые затрагивают более одного шарда одной или нескольких таблиц.
