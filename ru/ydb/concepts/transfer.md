...
В {{ ydb-name }} трансфер доступен только в [dedicated режиме работы](serverless-and-dedicated.md#dedicated) базы данных. Пример создания трансфера, работающего внутри одной базы данных описан в  статье [Трансфер — быстрый старт]({{ ydb.docs }}/recipes/transfer/quickstart).

Трансфер может читать данные из топиков, расположенных не только [базе данных]({{ ydb.docs }}/concepts/glossary#database), где он создаётся, но и в другой базе {{ ydb-short-name }} или [кластере]({{ ydb.docs }}/concepts/glossary#cluster) {{ ydb-short-name }}. Для чтения топика из другой базы данных при создании трансфера необходимо указать параметры подключения к этой базе. Целевая таблица должна находиться в той же базе данных, где создаётся трансфер.
...
{% note info %}
Действия необходимо производить в том облаке, в котором находится база с топиком источником
{% endnote %}
...
Для проверки корректной работы трансфера можно записать в топик несколько сообщений.
