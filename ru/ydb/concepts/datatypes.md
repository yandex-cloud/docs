...
| `Interval`    | точность до микросекунд, допустимые значения интервалов — не более 24 часов | Да | Нет | Нет | Да |
| `TzDate`      | точность до дней, содержит метку временной зоны | Да | Да | Нет | Да |
| `TzDatetime`  | точность до секунд, содержит метку временной зоны | Да | Да | Нет | Да |
| `TzTimestamp` | точность до микросекунд, содержит метку временной зоны | Да | Да | Нет | Да |
