...
Взаимодействие SDK с серверами YDB построено на базе протокола gRPC. Спецификация протокола взаимодействия с YDB открыта. Это позволяет реализовать SDK для любого языка программирования.{% if oss != true %} В дополнение к перечисленным выше OpenSource SDK внутри Yandex доступен [C++ SDK](../operations/start_cpp.md){% endif %}.
