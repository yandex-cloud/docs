...
Примеры запуска операции построения индекса с использованием cli представлены в разделе {% if audience != "external" %} [Начало работы - Использование консольного клиента {{ ydb-short-name }}](../getting_started/ydb_cli.md).
{% else %} [Использование консольного клиента {{ ydb-short-name }}](../quickstart/examples-ydb-cli#zapusk-operacii-dobavleniya-vtorichnogo-indeksa).
{% endif %}
