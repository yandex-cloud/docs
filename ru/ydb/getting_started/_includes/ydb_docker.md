...
В режиме локального запуска для работы с базой данных предоставляется только {{ ydb-short-name }} API. API доступен на эндпоинте `grpcs://localhost:2135`, имя базы данных `/local`. Работа с базой выполняется с помощью консольного клиента {{ ydb-short-name }} ([YDB CLI](../../reference/ydb-cli/install.md)), предустановленного внутри Docker-образа. 
...
Внутри образа доступна предсобранная версия [YDB CLI](../../reference/ydb-cli/install.md), с помощью которой можно выполнять различные команды, например:
