...
В [веб-интерфейсе YDB](https://ydb.yandex-team.ru/) для [аутентификации](../concepts/iam.md#authentication) используются сессионные cookies пользователя в домене yandex-team.ru. Подробнее этот метод аутентификации описан в разделе [Разграничение прав доступа](../concepts/iam.md#session_cookie). Пользователь, прошедший процедуру входа через [Яндекс.Паспорт](https://passport.yandex-team.ru/profile), аутентифицируется в веб-интерфейсе YDB автоматически.
...
* Для того, чтобы получить личный токен, следует нажать на иконку шестеренки в [веб-интерфейсе YDB](https://ydb.yandex-team.ru/), после чего нажать на кнопку "Give me my token". Полученный в результате персональный токен можно использовать в приложении.
* Для того, чтобы использовать роботного пользователя для доступа к БД потребуется:
    ...
    * войти в [Яндекс.Паспорт](https://passport.yandex-team.ru/profile) от имени робота;
    ...
    * настроить права доступа для робота используя рекомендации в разделе [Разграничение прав доступа](../concepts/iam.md).
...
Подробнее доступные виды прав описаны в документе [Разграничение прав доступа](../concepts/iam.md#ui_permissions).
...
Подробней про настройку прав доступа можно прочесть в разделе [{#T}](../concepts/iam.md).
