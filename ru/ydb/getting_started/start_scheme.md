...
| ```READ_REPLICAS_SETTINGS```                 | Настройка читающих реплик. ```PER_AZ``` означает использование указанного количества реплик в каждом AZ, ```ANY_AZ``` - во всех AZ суммарно. | String | ```"PER_AZ:<count>"```, ```"ANY_AZ:<count>"``` где ```<count>``` - число реплик | Нет                   |
