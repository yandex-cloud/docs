...
Подробнее про модель данных и схему YDB написано в разделе [Модель данных и схема](../concepts/datamodel.md).<br>Список типов данных и ссылки на их описание см. в разделе [Типы данных](../concepts/datatypes.md).
...
| Имя параметра                                | Описание                                                     | Тип    | Допустимые значения                                          | Возможность изменения |
| -------------------------------------------- | ------------------------------------------------------------ | ------ | ------------------------------------------------------------ | --------------------- |
| ```COMPACTION_POLICY```                      | Название предустановленного набора настроек compaction для таблицы | String | ```"default"```, ```"small_table"```, ```"log_table"```      | Да                    |
| ```AUTO_PARTITIONING_BY_SIZE```              | Режим автоматического партиционирования по размеру партиции  | Enum   | ```ENABLED```, ```DISABLED```                                | Да                    |
| ```AUTO_PARTITIONING_PARTITION_SIZE_MB```    | Предпочитаемый размер каждой партиции, в мегабайтах          | Uint64 | Натуральные числа                                            | Да                    |
| ```AUTO_PARTITIONING_MIN_PARTITIONS_COUNT``` | Минимальное количество партиций, при достижении которого перестаёт работать автоматическое слияние партиций | Uint64 | Натуральные числа                                            | Да                    |
| ```AUTO_PARTITIONING_MAX_PARTITIONS_COUNT``` | Максимальное количество шардов, при достижении которого перестаёт работать автоматическое разделение партиций | Uint64 | Натуральные числа                                            | Да                    |
| ```UNIFORM_PARTITIONS```                     | Начальное количество партиций для таблицы. Обязательное требование: первая колонка первичного ключа должна иметь тип Uint64 или Uint32 | Uint64 | Натуральные числа                                            | Нет                   |
| ```PARTITION_AT_KEYS```                      | Граничные значения ключей для разделения на партиции. Представляется списком граничных значений, разделенных запятыми и обрамленными в скобки. Каждое граничное значение может быть либо набором значений ключевых колонок (также разделенных запятыми и обрамленными в скобки), либо единичным значением, если указываются только значения первой ключевой колонки. Примеры: ```(100, 1000)```, ```((100, "abc"), (1000, "cde"))``` |        |                                                              | Нет                   |
| ```KEY_BLOOM_FILTER```                       | Использование фильтра Блума по ключу                         | Enum   | ```ENABLED```, ```DISABLED```                                | Да                    |
| ```READ_REPLICAS_SETTINGS```                 | Настройка читающих реплик (слейвов). ```PER_AZ``` означает использование указанного количества реплик в каждом AZ, ```ANY_AZ``` - во всех AZ суммарно. | String | ```"PER_AZ:<count>"```, ```"ANY_AZ:<count>"``` где ```<count>``` - число реплик | Нет                   |
...
Большинство параметров таблицы в YDB можно изменить командой ```ALTER```. В таблице 1 представлен полный список поддерживаемых параметров, где в последнем столбце отражена возможность их изменения.
