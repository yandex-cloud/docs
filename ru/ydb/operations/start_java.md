...
В этом разделе описана работа с помощью {{ ydb-short-name }} Java SDK. Вы узнаете как:
...
Для демонстрации работы с SDK мы подготовили приложение [basic_example](https://a.yandex-team.ru/arc/trunk/arcadia/kikimr/public/sdk/java/examples/src/basic_example). Чтобы успешно запустить [basic_example](https://a.yandex-team.ru/arc/trunk/arcadia/kikimr/public/sdk/java/examples/src/basic_example) потребуется:

* база данных (подробно о создании и управлении базой данных написано в разделе [Управление базами данных](create_manage_database.md);
* endpoint базы данных (информация об endpoint доступна на странице Info [консоли управления](https://ydb.yandex-team.ru) вашей БД);
...
```bash
./run.sh -j java ru.yandex.ydb.examples.basic_example.BasicExampleApp -e ydb-ru.yandex.net:2135 -d /ru/home/yourlogin/mydb
...
* драйвер {{ ydb-short-name }} отвечает за взаимодействие приложения и YDB на транспортном уровне;
* клиент {{ ydb-short-name }} работает поверх драйвера YDB и отвечает за работу с сущностями и транзакциями;
...
Конструкция `PRAGMA TablePathPrefix` добавляет указанный префикс к путям таблиц внутри БД. Она работает по принципу объединения путей в файловой системе: поддерживает ссылки на родительский каталог и не требует добавления слеша справа. Например:
...
Подробнее про PRAGMA в YQL написано в [документации YQL](../yql/reference/overview.md).
...
В большинстве случаев вместо явного использования [TCL](../concepts/transactions.md) вызовов Begin и Commit лучше использовать параметры контроля транзакций в вызовах ExecuteDataQuery. Это позволит избежать лишних обращений к {{ ydb-short-name }} и эффективней выполнять запросы. Фрагмент кода basic_example в листинге 19 демонстрирует явное использование вызовов ```beginTransaction``` и ```transaction.commit()```
