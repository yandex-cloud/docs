...
- Dedicated
  ...
  * `--async` — флаг асинхронного создания. Создание Dedicated БД может занимать значительное время, измеряемое минутами. Вы можете указать данный флаг для того, чтобы управление было возвращено сразу как только команда на создание БД будет принята облаком.
  ...
  >Примеры
  >
  >Создание минимальной одноузловой Dedicated БД {{ ydb-short-name }} с именем `dedb`, доступной из интернета:
  >
  >```bash
  ...
  Асинхронное создание трехузловой георезервированной Dedicated БД {{ ydb-short-name }} с хранилищем в 300 ГБ и вычислительным узлами по 64 ГБ RAM, с именем `dedb3`, доступной из интернета:
