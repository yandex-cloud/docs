...
Данные, которые до операции были доступны в таблице `logs/current_logs`, теперь находятся в `logs/old_logs`. Таблица `logs/current_logs` пуста потому, что по этому пути была перемещена таблица `logs/blank`. Чтобы повторить ротацию, необходимо создать заново таблицу`logs/blank`.
