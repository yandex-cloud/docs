---
sourcePath: overlay/reference/ydb-cli/profile/create.md
---
# Создание профиля

В профиле YDB CLI вы можете задать эндпоинт, полный путь к базе данных, а также способ авторизации пользователя, от имени которого будут выполняться команды. Заданные в профиле параметры можно не указывать при вызове команд YDB CLI.

С помощью профилей можно быстро переключать контекст выполнения команд YDB CLI. Для переключения на нужный профиль [активируйте](activate.md) его. Также указать профиль для выполнения команды можно с помощью глобального параметра `profile`.
