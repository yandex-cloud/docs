...
Каждому шарду для обработки запросов доступно не более одного ядра. Нагрузка, приводящая к потреблению выделенного ядра на 90% и более процентов может приводить к ухудшению времени выполнения запросов. На графике CPU Utilization диагностического дашборда показано количество шардов таблиц, загрузка которых превысила 70%. Количество шардов разложено по корзинам в соответствии с мгновенным потреблением выделенного ядра:
...
Повышенное (больше 90%) потребление CPU шардом, свидетельствует о близости потока запросов к максимальной пропускной способности шарда. Такая ситуация может привести к росту времени выполнения запросов к базе и росту очередей выполнения запросов.
...
Наиболее вредны запросы, не использующие индексы и просматривающие все строки таблицы. Количество ресурсов и времени, необходимое для выполнения такого запроса, растёт линейно или круче с ростом количества строк в таблице.
...
Одной из возможных причин высокого потребления памяти могут быть запросы к базе, читающие много данных. Для проверки такой гипотезы следует использовать системные таблицы. Пример запроса, который выводит top запросов по размеру прочитанных данных, приведён на листинге 3.
