...
Как правило, в обработке каждого запроса к базе данных участвуют один или больше _шардов_ таблиц. Подробнее про [шардирование таблиц](../concepts/datamodel#sharding) можно узнать из раздела [{#T}](../concepts/datamodel.md).
...
![Monitoring_CPU_Utilization_Chart](../../_assets/ydb/monitoring_cpu_utilization_chart.png)
...
![Diag_Dashboard_UserPool_Utilization_Chart](../../_assets/ydb/diag_dashboard_userpool.png)
...
![Diag_Dashboard_Node_Memory_Chart](../../_assets/ydb/diag_dashboard_nodes_memory.png)

<small>Рисунок 3 — примерный вид графика выделения памяти процессами базы</small>

Частые перезапуски процессов, обслуживающих базу, приведут к росту доли [ошибочных ответов](../ydb-sdk/error_handling.md#termination-statuses) `Unavailable`, что в свою очередь приведёт к росту доли ошибок в ответах приложения пользователю или к росту времени выполнения запросов.
...
Подробно коды ответов, возвращаемые в результате выполнения запроса, описаны в документе [{#T}](../ydb-sdk/error_handling.md).

## Блокировки {#locks}

На графике Transaction Locks Invalidation отображено количество транзакций в секунду, выполнение которых закончилось ошибкой `Transaction locks invalidated`, (TLI) – являющейся результатом нарушения [оптимистических блокировок](https://en.wikipedia.org/wiki/Optimistic_concurrency_control). Доступные режимы транзакций и блокировки рассмотрены в разделе [{#T}](..//concepts/transactions.md).
