...
Как правило, в обработке каждого запроса к базе данных участвуют один или больше _шардов_ таблиц. Подробнее про [шардирование таблиц](../concepts/datamodel#sharding) можно узнать из раздела [{#T}](../concepts/internal/datamodel.md).
...
Высокое потребление CPU шардом таблицы может быть связано с тем, что все записи попадают в один _шард_. Это может быть вызвано неоптимальным выбором первичного ключа. Например, в случае монотонного возрастания его значений, все записи будут попадать в последний _шард_. Рекомендации по проектированию первичного ключа читайте в  [документации](https://cloud.yandex.ru/docs/ydb/oss/public/deploy/best_practices/schema_design).
