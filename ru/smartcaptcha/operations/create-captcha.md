...
- Консоль управления
    ...
    1. Нажмите **Создать капчу** и укажите:
        ...
        * **Список сайтов** — в формате IP-адресов или доменных имен. Указываются без `http`, `htpps` и без `/` в конце, например, `example.com`. Доменные имена распространяются и на поддомены без символа wildcard (`*`).
