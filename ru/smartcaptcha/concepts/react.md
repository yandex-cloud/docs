...
`webview`               | `boolean` \| `undefined`                  | Запускается ли капча в вебвью. Свойство должно использоваться для страниц, которые будут встраиваться в webview, для повышения точности оценки пользователей. Если вы не уверены, нужно ли вам это свойство, то не указывайте для него никакого значения. 
