...
* _Условия для входящего трафика_ — набор параметров входящего трафика:
  ...
  * `Metadata` — дополнительные данные, передаваемые с фронтенда.
...
### Добавление и проверка метаданных {#metadata}

Параметр `metadata` позволяет передавать с фронтенда в капчу собственные дополнительные данные, например информацию о пользователе или результаты системы [антифрода](https://ru.wikipedia.org/wiki/Антифрод). Так можно настраивать правила показа разных типов заданий, например выдавать более сложную капчу для подозрительных пользователей.

Сохраняйте выданные значения `metadata` на бэкенде и сверяйте их при валидации токена капчи, чтобы защититься от подделки метаданных злоумышленниками,

Общая длина всех ключей и значений в `metadata` не должна превышать 512 символов.

Пример добавления капчи с метаданными:
...
{% cut "Схема проверки капчи с метаданными" %}

[Схема в отдельном окне](metadata-scheme.md)
...
* Не соответствует регулярному выражению
|
Дополнительные данные, передаваемые с фронтенда в формате `ключ: значение`, где:
* `ключ` — ключ параметра.
* `значение` — конкретное значение, префикс значения или регулярное выражение библиотеки PIRE.

Например, `user_type: premium` или `risk_score: high`.
| _и_ ||
|#