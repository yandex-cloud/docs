...
* `message` — сообщение о проверке, например, `Invalid or expired Token`.

Для корректной обработки ответов ориентируйтесь на поле `status`. Поле `message` используйте только для диагностики.
  
Примеры ответов и подробности обработки в разделе [Валидация пользователя](concepts/validation.md#service-response).
...
Читайте раздел [Расширенные настройки виджета](./concepts/widget-methods.md).
