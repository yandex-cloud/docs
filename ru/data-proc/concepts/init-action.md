...
URI скрипта может быть задан в схемах `https://` и `s3a://`, при этом во втором случае требуется выполнение хотя бы одного из условий:
...
* `MAX_WORKER_COUNT` — максимальное количество хостов типа `DATANODE` и `COMPUTENODE`.
* `MIN_WORKER_COUNT` — минимальное количество хостов типа `DATANODE` и `COMPUTENODE`.

Например, чтобы выполнить часть скрипта только на хостах с ролью `MASTERNODE`, используйте проверку значения переменной окружения `ROLE`:
