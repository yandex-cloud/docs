...
Декомиссии ресурсов под HDFS применяется при:
...
1. На хостах из списка `excluded` не запускаются новые задания. По мере того, как запущеные задания завершаются, хосты обновляются и перезагружаются.
1. Если задания не успевают завершиться до окончания таймаута декомиссии, они завершаются принудиительно и хост обновляется и перезагружается.
1. После перезагрузки хосты удаляются из списка `excluded`.
1. Сервис ждет, пока всех хосты перейдут в статус `Alive`.
