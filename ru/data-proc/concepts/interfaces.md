...
{{ dataproc-name }} позволяет создавать кластеры как с публичным доступом, так и только с внутренними адресами {{ yandex-cloud }}. Однако при любой конфигурации кластера сетевые и веб-интерфейсы компонентов рекомендуется делать недоступными извне. Для внешней связи с такими компонентами как HDFS NameNode, YARN ResourceManager и другими вы можете маршрутизировать трафик через промежуточную виртуальную машину, для которой выделен публичный IP-адрес.
