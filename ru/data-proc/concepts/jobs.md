...
При создании задания можно указать:

* _Аргументы_ — значения, передаваемые выполняемому скрипту.
...
* Подключитесь напрямую к узлу кластера. Подробнее см. в [продвинутых примерах](../solutions/job-overview.md#jobs-advanced-tutorials) работы с заданиями.
...
* Предоставьте доступ к нужным бакетам {{ objstorage-name }} для сервисного аккаунта кластера.
    ...
    * Бакет с правами на чтение и запись для хранения результатов выполнения заданий и логов. Укажите его при создании кластера.
...
По умолчанию логи заданий сохраняются в бакет, указанный при [создании кластера](../operations/cluster-create.md), по пути:

```http
s3a://<имя бакета>/<dataproc>/<идентификатор кластера>/jobs/<идентификатор задачи>/
```

Подробнее см. в разделе [{#T}](../operations/jobs.md).
