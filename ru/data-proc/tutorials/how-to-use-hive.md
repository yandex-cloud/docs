...
* В объекте {{objstorage-name}}, который доступен на чтение сервисному аккаунту кластера {{dataproc-name}}:
    ...
    {{ yc-dp }} job create-hive \
       --cluster-id=<идентификатор кластера> \
       --name=<имя задачи> \
       --query-file-uri="s3a://<ваш бакет>/hive-query.sql"
    ```

Результат исполнения запросов, а также дополнительную диагностическую информацию можно найти в бакете {{objstorage-name}}, который был указан при создании кластера: `s3://<ваш бакет>/dataproc/clusters/<идентификатор кластера>/jobs/<идентификатор задачи>/`.

Идентификатор задачи есть в выводе команды запуска задания YC CLI и есть в ответе {% if lang == "ru" and audience != "internal" %}[API](../../glossary/rest-api.md){% else %}API{% endif %} на запуск задания.


## Запуск задания с помощью Hive CLI {#run-job-hive-shell}

Чтобы запустить командную оболочку (CLI) для Apache Hive, подключитесь к хосту-мастеру по {% if lang == "ru" and audience != "internal" %}[SSH](../../glossary/ssh-keygen.md){% else %}SSH{% endif %} и выполните команду `hive`.
