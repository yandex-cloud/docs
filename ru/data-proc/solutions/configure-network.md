...
Согласно [концепции сети](https://cloud.yandex.ru/docs/vpc/concepts/network) в Яндекс.Облаке, хосты без публичных IP-адресов в кластере {{ dataproc-name }} не имеют доступа к ресурсам, находящимся за пределами виртуальной сети {{vpc-short-name}}. Чтобы взаимодействовать с узлами других сетей, интерфейсами сервисов Яндекс.Облака или узлами в интернете необходимо настроить публичный IP-адрес для хоста или воспользоваться NAT в интернет для нужной подсети.
...
* Вы можете контролировать весь исходящий трафик, и при необходимости развернуть не только NAT, но и [VPN между подсетью Яндекс.Облака и нужными ресурсами](../../solutions/routing/ipsec-vpn.md).
