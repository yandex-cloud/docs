...
Перенести в другую зону доступности можно только [легковесные кластеры](../concepts/index.md#light-weight-clusters). Миграция кластеров с сервисом HDFS пока недоступна. Кластеры с HDFS из зоны доступности `{{ region-id }}-c` техническая поддержка перенесет вручную, предупредив об этом их владельцев.
