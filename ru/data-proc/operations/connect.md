...
- UI Proxy
    ...
    Для использования [**UI Proxy**](../concepts/ui-proxy.md) [добавьте](../../vpc/operations/security-group-update.md#add-rule) в группу безопасности хоста главного подкластера правила, разрешающие входящий трафик через порт `{{ port-https }}`:
...
- Подключение с перенаправлением портов
    ...
    При использовании [перенаправления портов](../concepts/interfaces.md#routing), [добавьте](../../vpc/operations/security-group-update.md#add-rule) в группу безопасности промежуточной ВМ правила, разрешающие входящий и исходящий трафик через порты требуемых компонентов:
