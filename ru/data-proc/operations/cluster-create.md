...
В подсети, к которой будет подключен подкластер с управляющим хостом, {% if audience != "internal" %}[настройте NAT-шлюз](../../vpc/operations/create-nat-gateway.md){% else %}настройте NAT-шлюз{% endif %}. Это необходимо, чтобы подкластер мог взаимодействовать c сервисами {{ yandex-cloud }} или хостами в других сетях.
