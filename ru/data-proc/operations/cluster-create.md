...
- Консоль управления
  ...
  1. На этапе Preview доступна только одна версия образа {{ dataproc-name }} — 1.0. Выберите те компоненты, которые вы хотите использовать в кластере.
  ...
  1. Вставьте в поле **Публичный ключ** публичную часть вашего SSH-ключа. О том, как сгенерировать и использовать SSH-ключи, читайте в [документации {{ compute-full-name }}](../../compute/operations/vm-connect/ssh.md).
