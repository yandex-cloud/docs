...
В подсети, к которой будет подключен подкластер с управляющим хостом, {% if audience != "internal" %}[включите NAT в интернет](../../vpc/operations/enable-nat.md){% else %}включите NAT в интернет{% endif %}. Это необходимо, чтобы подкластер мог взаимодействовать c сервисами {{ yandex-cloud }} или хостами в других сетях.
...
- Консоль управления
  ...
  1. Для каждого подкластера можно настроить:
     ...
     * Подсеть сети, в которой расположен кластер.
      ...
      В подсети для подкластера с управляющим хостом должен быть включен NAT в интернет. Подробнее см. в разделе [{#T}](#setup-network).
