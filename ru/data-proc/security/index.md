...
Пользователь с ролью `{{ roles-viewer }}` может подключаться к хостам в кластере {{ dataproc-name }}, если его {% if lang == "ru" and audience != "internal" %}[SSH-ключи](../../glossary/ssh-keygen.md){% else %}SSH-ключи{% endif %} привязаны к этому кластеру.
