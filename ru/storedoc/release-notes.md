...
* Если реплика {{ SD }} сильно отстала, сервис теперь может быстрее актуализировать данные на ней через механизмы восстановления [резервных копий](./concepts/backup.md), не нагружая хост-мастер.
...
* В разделе **{{ ui-key.yacloud.mdb.cluster.monitoring.label_title }}** появилась вкладка **{{ ui-key.yacloud.mdb.cluster.switch_shards }}**. Она позволяет отслеживать метрики каждого шарда отдельно и быстро выявлять проблемные узлы.
* Добавлена возможность частичного восстановления кластеров {{ SD }} из резервной копии. Можно восстановить отдельные базы данных или коллекции, указав их в списках включения или исключения.
...
* Добавлена поддержка {{ MG }} версии 8.0. О том, как обновить кластер до последней версии, смотрите раздел [{#T}](operations/cluster-version-update.md).
...
Добавлена поддержка {{ MG }} версии 7.0. О том, как обновить кластер до последней версии, смотрите раздел [{#T}](operations/cluster-version-update.md).
...
* Добавлена поддержка {{ MG }} версии 6.0. О том, как обновить кластер до последней версии, смотрите раздел [{#T}](operations/cluster-version-update.md).
