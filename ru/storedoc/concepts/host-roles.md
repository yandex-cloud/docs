...
В {{ mmg-name }} есть четыре типа хостов: `MONGOD`, `MONGOINFRA`, `MONGOCFG` и `MONGOS`. Каждый тип хостов имеет собственную [роль и функции](#features) в кластере. [Тип и количество хостов](#configuration) в кластере {{ mmg-name }} зависят от выбранного типа шардирования.
