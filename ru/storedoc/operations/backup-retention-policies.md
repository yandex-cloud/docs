# Управление политиками резервного копирования в {{ mmg-name }}
...
- {{ TF }} {#tf}
    ...
    1. Подтвердите изменение ресурсов.
        ...
        Подробнее в [документации провайдера {{ TF }}]({{ tf-provider-resources-link }}/mdb_mongodb_backup_retention_policy).
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [BackupRetentionPolicy.Create](../api-ref/BackupRetentionPolicy/create.md) и выполните запрос, например, с помощью {{ api-examples.rest.tool }}:
        ...
        curl \
          ...
          --url 'https://{{ api-host-mdb }}/managed-mongodb/v1/clusters/<идентификатор_кластера>/retention_policies' \
          --data '{
                     "cron": {
...
- CLI {#cli}
    ...
    1. Получите список политик для кластера {{ mmg-name }}:
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [BackupRetentionPolicy.List](../api-ref/BackupRetentionPolicy/list.md) и выполните запрос, например, с помощью {{ api-examples.rest.tool }}:
        ...
        curl \
          --request GET \
          --header "Authorization: Bearer $IAM_TOKEN" \
          --url 'https://{{ api-host-mdb }}/managed-mongodb/v1/clusters/<идентификатор_кластера>/retention_policies?pageSize=<количество_результатов>'
        ```
...
- CLI {#cli}
    ...
    1. Удалите политику:
        ...
        ```bash
        {{ yc-mdb-mg }} backup-retention-policy delete <идентификатор_политики> \
           --cluster-id <идентификатор_кластера>
        ```
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [BackupRetentionPolicy.Delete](../api-ref/BackupRetentionPolicy/delete.md) и выполните запрос, например, с помощью {{ api-examples.rest.tool }}:
        ...
        curl \
          --request DELETE \
          --header "Authorization: Bearer $IAM_TOKEN" \
          --url 'https://{{ api-host-mdb }}/managed-mongodb/v1/clusters/<идентификатор_кластера>/retention_policies/<идентификатор_политики>'
        ```
