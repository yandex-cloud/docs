...
Вы можете обновить кластер {{ mmg-name }} только на следующую версию относительно текущей, например, с версии 4.2 на 4.4. Обновление до более поздних версий производится поэтапно. Например, обновление версии {{ SD }} с 4.2 до 6.0 выполняется в такой последовательности: 4.2 → 4.4 → 5.0 → 6.0.
...
- REST API {#api}
   ...
   1. После обновления все возможности {{ SD }}, у которых нет обратной совместимости с прежней версией, выключаются. Чтобы снять это ограничение, выполните еще один запрос и передайте номер новой версии {{ SD }} в свойстве `configSpec.featureCompatibilityVersion`.
      ...
      curl \
         ...
         --data '{
                  ...
                  "configSpec": {
                    "featureCompatibilityVersion": "<новая_версия_Yandex_StoreDoc>"
                  }
                }'
      ```
...
- gRPC API {#grpc-api}
    ...
    1. После обновления все возможности {{ SD }}, у которых нет обратной совместимости с прежней версией, выключаются. Чтобы снять это ограничение, выполните еще один запрос и передайте номер новой версии {{ SD }} в свойстве `config_spec.feature_compatibility_version`.
        ...
        grpcurl \
            ...
            -d '{
                  ...
                  "config_spec": {
                    "feature_compatibility_version": "<новая_версия_Yandex_StoreDoc>"
                  }
               }' \
            {{ api-host-mdb }}:{{ port-https }} \
            yandex.cloud.mdb.mongodb.v1.ClusterService.Update
