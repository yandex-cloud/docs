...
Для нового кластера необходимо задать все параметры, обязательные при создании, кроме типа кластера (резервную копию {{ SD }} не получится восстановить как кластер {{ PG }}).
...
- CLI {#cli}
  ...
  1. Посмотрите описание команды CLI для создания резервной копии {{ SD }}:
...
- CLI {#cli}
  ...
  Чтобы получить список резервных копий кластеров {{ SD }}, доступных в каталоге по умолчанию, выполните команду:
...
- CLI {#cli}
  ...
  Чтобы получить данные о резервной копии кластера {{ SD }}, выполните команду:
...
- {{ TF }} {#tf}
    ...
    1. Откройте актуальный конфигурационный файл {{ TF }} с планом инфраструктуры.
        ...
        Полный список доступных для изменения полей конфигурации кластера {{ SD }} приведен в [документации провайдера {{ TF }}]({{ tf-provider-mmg }}).

    1. Добавьте к описанию кластера {{ SD }} блок `backup_retain_period_days` в секции `cluster_config`:
