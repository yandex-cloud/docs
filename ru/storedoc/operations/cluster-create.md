...
- Консоль управления {#console}
  ...
  1. Задайте конфигурацию вычислительных ресурсов:
     ...
     1. В блоке **{{ ui-key.yacloud.mdb.forms.section_storage }}**:
         ...
         * Выберите размер хранилища, который будет использоваться для данных и резервных копий. Подробнее читайте в разделе [Резервные копии](../concepts/backup.md).
     ...
     1. (Опционально) В блоке **{{ ui-key.yacloud.mdb.cluster.section_disk-scaling }}** укажите желаемые настройки:
         ...
         * В поле **{{ ui-key.yacloud.mdb.resources.DiskAutoscalingFieldGroup.field_autoscaling_fsAon }}** задайте условия автоматического увеличения хранилища:
             ...
             * процент заполнения хранилища, при котором оно будет увеличено немедленно.
             ...
             Если заданы оба условия, то процент в первом условии должен быть меньше, чем процент во втором.
         ...
         * В поле **{{ ui-key.yacloud.mdb.resources.DiskAutoscalingFieldGroup.field_disk-size-limit_bK9Ng }}** укажите максимальный размер хранилища, который может быть установлен при автоматическом увеличении размера хранилища.
         ...
         Если вы включили увеличение хранилища в окно обслуживания, в блоке **{{ ui-key.yacloud.mdb.forms.section_additional }}** задайте время технического обслуживания.
     ...
     1. В блоке **{{ ui-key.yacloud.mdb.forms.section_host }}** добавьте хосты БД, создаваемые вместе с кластером:
         ...
         * Если хост должен быть доступен снаружи {{ yandex-cloud }}, включите опцию **{{ ui-key.yacloud.mdb.hosts.dialog.field_public_ip }}**.
         ...
         Чтобы обеспечить отказоустойчивость, для типов диска `local-ssd` и `network-ssd-nonreplicated` необходимо как минимум 3 хоста. Подробнее читайте в разделе [Хранилище](../concepts/storage.md).

         
         По умолчанию хосты создаются в разных зонах доступности. Подробнее об [управлении хостами](hosts.md).
...
- CLI {#cli}
  ...
  1. Укажите параметры кластера в команде создания (в примере приведены не все параметры):
      ...
      * `--host` — параметры хоста:
        ...
        * `secondary-delay-secs` — отставание вторичной реплики от первичной в секундах. Может быть полезно для восстановления данных в случае ошибочных операций.
  ...
  1. Опишите в конфигурационном файле параметры ресурсов, которые необходимо создать:
     ...
     * `host` — параметры хоста:
       ...
       * `host_parameters` — дополнительные параметры хоста:
         ...
         * `hidden` — скрытие хоста: `true` или `false`. Если хост скрыт, он будет доступен для чтения только для прямых подключений (например, чтобы делать с него резервные копии, не добавляя нагрузку на кластер).
         * `secondary_delay_secs` — отставание вторичной реплики от первичной в секундах. Может быть полезно для восстановления данных в случае ошибочных операций.
         * `priority` — [приоритет назначения хоста первичной репликой](../concepts/replication.md#master-failover).
     ...
     * `version` — версия {{ SD }}: {{ versions.tf.str }}.
     ...
     Чтобы настроить [автоматическое увеличение](../concepts/storage.md#auto-rescale) размера хранилища, добавьте к описанию кластера блок с настройками для нужного типа хостов:
     ...
     Более подробную информацию о ресурсах, которые вы можете создать с помощью {{ TF }}, читайте в [документации провайдера]({{ tf-provider-mmg }}).
    ...
    1. Создайте файл `body.json` и добавьте в него следующее содержимое:
        ...
        * `configSpec` — настройки кластера:
            ...
            * `mongod`, `mongoinfra`, `mongos`, `mongocfg` — [типы хостов](../concepts/host-roles.md).
              ...
              * `diskSizeAutoscaling` — параметры [автоматического увеличения](../concepts/storage.md#auto-rescale) размера хранилища:
        ...
        * `hostSpecs` — настройки хостов кластера в виде массива элементов. Каждый элемент соответствует отдельному хосту и имеет следующую структуру:
          ...
          * `hidden` — скрытие хоста: `true` или `false`. Если хост скрыт, он будет доступен для чтения только для прямых подключений (например, чтобы делать с него резервные копии, не добавляя нагрузку на кластер).
          * `secondaryDelaySecs` — отставание вторичной реплики от первичной в секундах. Может быть полезно для восстановления данных в случае ошибочных операций.
          * `priority` — [приоритет назначения хоста первичной репликой](../concepts/replication.md#master-failover).
  ...
  1. Создайте файл `body.json` и добавьте в него следующее содержимое:
        ...
        * `config_spec` — настройки кластера:
            ...
            * `mongod`, `mongoinfra`, `mongos`, `mongocfg` — [типы хостов](../concepts/host-roles.md).
              ...
              * `disk_size_autoscaling` — параметры [автоматического увеличения](../concepts/storage.md#auto-rescale) размера хранилища:
...
Кластер {{ mmg-name }} можно создать со [стандартным](#std-sharding) или [расширенным](#adv-sharding) шардированием. Подробнее о видах шардирования читайте в разделе [Особенности управления шардированием](../concepts/sharding.md#shard-management).
