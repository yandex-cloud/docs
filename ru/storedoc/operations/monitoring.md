...
- Консоль управления {#console}
  ...
  * **Queries on primary** — прирост количества команд и операций на первичной реплике шарда.
  ...
  * **Documents affected on primary** — прирост количества вставленных, обновленных, удаленных или возвращенных запросами документов на первичной реплике шарда.
  ...
  * **Readers/writers active queue per host, top 5** — количество операций чтения и записи для пяти наибольших очередей, ожидающих снятия блокировки, на каждом хосте шарда.
  ...
  В блоке **Latency**:
   
  * **Average operations time on primary** — среднее время выполнения команд и операций на первичной реплике шарда.
  ...
  * **Total operations time on Primaries** — суммарное время выполнения всех операций на первичной реплике шарда.
  ...
  * **Data size on primary, top 5 databases** — объем данных для пяти баз с наибольшим размером на первичной реплике шарда.
  * **Index size on primary, top 5 indexes** — размер индексов для пяти баз с наибольшим размером на первичной реплике шарда.
  ...
  * **CPU usage on Primaries** — доля использования CPU на первичной реплике шарда.
  ...
  * **Memory usage on Primaries** — использование оперативной памяти на первичной реплике шарда.
  ...
  * **Disk space usage on Primaries** — использование дискового пространства на первичной реплике шарда.
  ...
  * **WiredTiger checkpoint time on primary** — время создания чекпоинтов на первичной реплике шарда.
  * **WiredTiger cache state on primary** — использование кеша на первичной реплике шарда.
  * **WiredTiger transactions state on primary** — прирост количества транзакций на первичной реплике шарда.
  * **WiredTiger concurrent transactions on primary** — текущее количество тикетов параллельных транзакций на первичной реплике шарда.
  * **WiredTiger cache pages evicted on primary** — прирост числа освобожденных страниц кеша (измененных и неизмененных) на первичной реплике шарда.
...
- Консоль управления {#console}
    ...
    1. Если на графике несколько показателей, выберите запрос данных для формирования метрики и нажмите **{{ ui-key.yacloud_monitoring.dialog.confirm.button_continue }}**. Подробнее о языке запросов смотрите [документацию {{ monitoring-full-name }}](../../monitoring/concepts/querying.md).
