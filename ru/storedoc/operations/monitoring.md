...
- Консоль управления {#console}
  ...
  * **Queries on primary** — прирост количества команд и операций на мастере шарда.
  ...
  * **Documents affected on primary** — прирост количества вставленных, обновленных, удаленных или возвращенных запросами документов на мастере шарда.
  ...
  * **Readers/writers active queue per host, top 5** — количество операций чтения и записи для пяти наибольших очередей, ожидающих снятия блокировки, на каждом хосте шарда.
  ...
  В блоке **Latency**:
   
  * **Average operations time on primary** — среднее время выполнения команд и операций на мастере шарда.
  ...
  * **Total operations time on Primaries** — суммарное время выполнения всех операций на мастере шарда.
  ...
  * **Data size on primary, top 5 databases** — объем данных для пяти баз с наибольшим размером на мастере шарда.
  * **Index size on primary, top 5 indexes** — размер индексов для пяти баз с наибольшим размером на мастере шарда.
  ...
  * **CPU usage on Primaries** — доля использования CPU на мастере шарда.
  ...
  * **Memory usage on Primaries** — использование оперативной памяти на мастере шарда.
  ...
  * **Disk space usage on Primaries** — использование дискового пространства на мастере шарда.
  ...
  * **WiredTiger checkpoint time on primary** — время создания чекпоинтов на мастере шарда.
  * **WiredTiger cache state on primary** — использование кеша на мастере шарда.
  * **WiredTiger transactions state on primary** — прирост количества транзакций на мастере шарда.
  * **WiredTiger concurrent transactions on primary** — текущее количество тикетов параллельных транзакций на мастере шарда.
  * **WiredTiger cache pages evicted on primary** — прирост числа освобожденных страниц кеша (измененных и неизмененных) на мастере шарда.
