...
- Консоль управления {#console}
  ...
  1. Укажите параметры хоста:
     ...
     * [Подсеть](../../vpc/concepts/network.md#subnet). Если нужной подсети в списке нет, [создайте ее](../../vpc/operations/network-create.md).     

     * (Опционально) Выберите опцию **{{ ui-key.yacloud.mdb.hosts.dialog.field_public_ip }}**, если хост должен быть доступен извне {{ yandex-cloud }}.


     * Тип хоста и название шарда, если в кластере {{ mmg-name }} включено шардирование.
     * (Опционально) Отметьте хост скрытым, если он должен быть доступен для чтения только для прямых подключений. Например, чтобы делать с него резервные копии без дополнительной нагрузки на кластер.
     * (Опционально) Задайте отставание реплики от мастера в секундах. Может быть полезно для восстановления данных в случае ошибочных операций.
     * (Опционально) Укажите [приоритет назначения хоста мастером](../concepts/replication.md#master-failover). Настройка доступна, если хост не отмечен как скрытый.

  1. (Опционально) Добавьте теги в формате `ключ: значение`.
...
* Консоль управления {#console}
    ...
    1. Задайте новые настройки для хоста: 
        ...
        1. Отметьте хост скрытым, если он должен быть доступен для чтения только для прямых подключений (например, чтобы делать с него резервные копии без дополнительной нагрузки на кластер).
        1. Измените значение отставания реплики от мастера в секундах. Может быть полезно для восстановления данных в случае ошибочных операций.
        1. Укажите [приоритет назначения хоста мастером](../concepts/replication.md#master-failover). Настройка доступна, если хост не отмечен как скрытый.
        1. Добавьте теги в формате `ключ: значение`.
...
* {{ TF }} {#tf}
    ...
    1. Откройте актуальный конфигурационный файл {{ TF }} с планом инфраструктуры.
        ...
        Полный список доступных для изменения полей конфигурации кластера {{ mpg-name }} приведен в [документации провайдера {{ TF }}]({{ tf-provider-mpg }}).
...
Если после изменения хоста к нему невозможно [подключиться](connect/index.md), убедитесь, что [группа безопасности](../concepts/network.md#security-groups) кластера {{ mmg-name }} настроена корректно для подсети, в которую помещен хост.
...
Если хост является первичным в момент удаления, {{ mmg-name }} автоматически выберет новый мастер.
...
1. Хост перестает принимать запросы на запись. Если хост был мастером (`PRIMARY`), {{ mmg-name }} попытается сделать его вторичной репликой (`SECONDARY`). В случае неудачи операция прерывается.
...
* Если хост является [мастером](../concepts/replication.md).

При перезагрузке мастера не происходит его автоматического переключения. Чтобы избежать недоступности кластера {{ mmg-name }}, [переключите мастер](stepdown.md) кластера до его перезагрузки.
