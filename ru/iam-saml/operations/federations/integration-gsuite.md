...
В этой инструкции описано, как настроить аутентификацию в облаке через G-Suite от Google.
...
## Создайте SAML-федерацию {#create-federation}
...
- Консоль управления
    ...
    1. В поле **Ссылка на страницу для входа в IdP** скопируйте ссылку, указанную в поле **URL Системы единого входа** на странице **Сведения о поставщике услуг идентификации Google** в G-Suite. Это ссылка в формате:
        ...
        ```
    1. Вы можете включить опцию **Автоматически создавать пользователей**, чтобы новый пользователь автоматически добавлялся в облако после успешной аутентификации. Эта опция упрощает процесс заведения пользователей, но у пользователя будет только роль `resource-manager.clouds.member` и он не сможет выполнять никаких операций с ресурсами в этом облаке. Исключение — те ресурсы, на которые назначены роли системной группе `allUsers` или `allAuthenticatedUsers`.
...
1. На странице Связывание атрибутов, добавьте новое сопоставление:
   * В атрибуте приложения укажите укажите `name_id`.
   * В категории выберите **Общие сведения**.
   * В пользовательских данных выберите **Основной адрес эл. почты**.
   * Нажмите **Сохранить**.

После того, как вы закончили создание SAML-приложения, включите его:
1. Нажмите значок ![](../../../_assets/vertical-ellipsis.svg)
1. Выберите, кому будет доступна эта аутентификация: **Включено для всех** или **Включено для некоторых пользователей**.
