...
Вы можете настроить аутентификацию в вашем облаке через службу Active Directory Federation Services (AD FS).
...
1. [Настройте аутентификацию на сервере AD FS](#configure-sso-adfs).
...
1. [Создайте relying party trust](#configure-relying-party-trust)
1. [Настройте Claims Mapping](#configure-claims-mapping)

### Создайте relying party trust {#configure-relying-party-trust}

AD FS требует создавать _relying party trust_ для каждого поставщика услуг (Service Provider, SP), который будет использовать AD FS для аутентификации.

Создайте relying party trust для федерации, созданной в облаке:
...
1. Введите имя поставщика услуг и нажмите **Next**.
1. Введите имя поставщика услуг, например Yandex.Cloud, и нажмите **Next**.
...
1. На следующей странице введите в качестве идентификатора эту же [ссылку для входа в консоль] и нажмите **Add**. После этого нажмите **Next**.
1. Выберите политику управления доступом и нажмите **Next**.
...
1. В консоли управления AD FS в блоке **Relying Party Trusts** нажмите правой кнопкой мыши на созданном ранее relying party trust и выберите **Edit Claim Issuance Policy**.
1. В открывшемся окне нажмите **Add Rule**.
1. Выберите **Send LDAP Attributes as Claims** и нажмите **Next**.
1. На странице настройки соответствия заполните поля следующим образом:
    * В поле **Claim rule name** задайте имя правила, например `Map Email and Name ID`
    * В поле **Attribute Store** выберите `Active Directory`.
    * Добавьте строчку в списке **Mapping of LDAP attributes**:

        В **LDAP Attribute** выберите `User-Principal-Name` или `E-Mail-Addresses`.

        В **Outgoing Claim Type** выберите `Name ID`.

    ![image](../../../_assets/iam/federations/specify-claims-mapping.png)
...
1. Ведите ваши данные для аутентификации. По умолчанию, необходимо ввести UPN и пароль. Затем нажмите кнопку **Sign in**.
