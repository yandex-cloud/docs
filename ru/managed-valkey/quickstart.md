...
1. [Создайте виртуальную машину Linux](../compute/operations/vm-create/create-linux-vm.md#console_1).
    ...
    * **{{ ui-key.yacloud.compute.instances.create.field_security-groups }}** — оставьте поле пустым.
        ...
        Виртуальной машине будет автоматически назначена [группа безопасности по умолчанию](../vpc/concepts/security-groups.md#default-security-group) (с префиксом `default-sg`). Такая группа разрешает подключение к виртуальной машине по `SSH` и `RDP`, входящий трафик по `ICMP`, а также любой исходящий трафик. Для рабочих ресурсов используйте пользовательскую группу безопасности с минимально необходимыми правилами.
...
1. В [консоли управления]({{ link-console-main }}) выберите каталог, в котором нужно создать кластер {{ VLK }}.
...
Подробнее о создании кластера читайте в разделе [{#T}](./operations/cluster-create.md).
