---
title: Репликация и персистентность в {{ mrd-name }}
description: '{{ mrd-name }} использует стандартную репликацию {{ VLK }} совместно с агентом управления состоянием хоста rdsync.'
keywords:
  - репликация Valkey
  - Valkey
  - субд Valkey
...
# Репликация и персистентность в {{ mrd-name }}

{{ mrd-name }} использует стандартную репликацию {{ VLK }} совместно с агентом управления состоянием хоста [rdsync](https://github.com/yandex/rdsync), а также предустановленные настройки персистентности данных.
...
### Агент управления состоянием хоста rdsync {#rdsync}

В архитектуру {{ mrd-name }} интегрирован агент управления состоянием хоста [rdsync](https://github.com/yandex/rdsync), разработанный Яндексом.
...
Шардированный кластер с типом диска **local-ssd**, в котором на каждый шард приходится только один хост, создать нельзя.
...
Хост-мастер может быть сменен не только автоматически в результате сбоя, но и [вручную](../operations/failover.md).
...
Отключайте персистентность только если для работы вашего приложения не важна сохранность данных, например при использовании {{ mrd-name }} в качестве кеша. В таком случае последние записанные в {{ VLK }} данные будут храниться только в оперативной памяти и могут быть утеряны при аварийном завершении работы сервера.

{% endnote %}

Персистентность также можно включить только для реплик. В этом случае пропускная способность сервера увеличится, но риск потери данных уменьшится.

### Включение персистентности {#persistence-on}
...
### Включение персистентности только на репликах {#persistence-on-replicas}

С персистентностью, включенной только на репликах, действуют следующие настройки {{ VLK }}:

* На репликах — как при включенной персистентности.

* На мастере — как при выключенной персистентности.

Если роли хостов меняются, настройки обновляются автоматически.
