---
title: Классы хостов {{ RD }} до 20 июня 2023 года
...
Выделенный для хоста объем памяти также определяет параметр конфигурации `maxmemory` для хостов {{ RD }}: максимальный объем данных равен {{ mrd-memory-used }} доступной памяти. Подробнее в разделе [Управление памятью](../memory-management.md).
