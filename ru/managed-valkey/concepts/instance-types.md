...
Выделенный для хоста объем памяти также определяет параметр конфигурации `maxmemory` для хостов {{ VLK }}: максимальный объем данных равен {{ mrd-memory-used }} доступной памяти. Подробнее в разделе [Управление памятью](memory-management.md).
...
* **high-memory** (**hm1**, **hm2**, **hm3**, **hm4af**) — стандартные конфигурации для {{ VLK }}.
