...
Если соответствующая настройка отключена (по умолчанию), то {{ VLK }} использует IP-адреса в качестве адресов хостов. Если эта настройка включена, то IP-адрес хоста будет подменяться на его FQDN. Информация об этой настройке и сферах ее применения приведена в разделе [{#T}](../concepts/network.md#fqdn-ip-setting).
...
Подробнее о хранилище и автоматическом увеличении читайте в разделе [Хранилище в {{ mrd-name }}](../concepts/storage.md).
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [Cluster.Update](../api-ref/Cluster/update.md) и выполните запрос, например с помощью {{ api-examples.rest.tool }}:
        ...
        * `configSpec.redisConfig_<версия_{{ VLK }}>` — набор настроек {{ VLK }}. Укажите каждую настройку на отдельной строке через запятую.
            ...
            Список версий {{ VLK }}, доступных для параметра, приведен в [описании метода](../api-ref/Cluster/update.md#yandex.cloud.mdb.redis.v1.UpdateClusterRequest). Описание и возможные значения настроек читайте в разделе [{#T}](../concepts/settings-list.md).
