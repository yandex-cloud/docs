...
Информация об устройстве кластера {{ mrd-name }} приведена в разделе [Взаимосвязь ресурсов сервиса](../concepts/index.md).
...
- Консоль управления {#console}
  ...
  1. В блоке **{{ ui-key.yacloud.mdb.forms.new_section_resource }}**:
    ...
    1. В блоке **{{ ui-key.yacloud.mdb.forms.section_network }}** выберите:
       ...
       * Группы безопасности для сетевого трафика кластера. Может потребоваться дополнительная [настройка групп безопасности](connect/index.md#configure-security-groups) для того, чтобы можно было подключаться к кластеру.
...
- {{ TF }} {#tf}
  ...
  Чтобы создать кластер {{ mrd-name }}:
    ...
    Более подробная информация о ресурсах, которые можно создать с помощью {{ TF }}, в [документации провайдера]({{ tf-provider-mrd }}).
...
Если вы указали идентификаторы групп безопасности при создании кластера, то для подключения к нему может потребоваться дополнительная [настройка групп безопасности](connect/index.md#configure-security-groups).
