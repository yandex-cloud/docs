# Управление политиками резервного копирования в {{ mrd-name }}
...
- Консоль управления {#console}
    ...
    1. На странице **{{ ui-key.yacloud.mdb.BackupPolicy.title_backups_8fQgh }}** выберите вкладку **{{ ui-key.yacloud.mdb.BackupPolicy.label_backup-policies_s4xXC }}**.
    ...
    1. Укажите параметры новой политики:
        ...
        1. Выберите один из вариантов запуска резервного копирования:
            ...
            * **{{ ui-key.yacloud.mdb.BackupPolicy.value_type-weekly_69MML }}**. Для этого варианта дополнительно выберите один или несколько дней недели.
            * **{{ ui-key.yacloud.mdb.BackupPolicy.value_type-monthly_8xt6g }}**. Для этого варианта дополнительно выберите один или несколько месяцев и один или несколько дней месяца.
...
- {{ TF }} {#tf}
    ...
    1. Добавьте описание ресурса:
        ...
        ```hcl
        resource "yandex_mdb_redis_backup_retention_policy" "<имя_политики>" {
    ...
    1. Подтвердите изменение ресурсов.
        ...
        Подробнее в [документации провайдера {{ TF }}]({{ tf-provider-resources-link }}/mdb_redis_backup_retention_policy).
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [BackupRetentionPolicy.Create](../api-ref/BackupRetentionPolicy/create.md) и выполните запрос, например с помощью {{ api-examples.rest.tool }}:
        ...
        curl \
          ...
          --url 'https://{{ api-host-mdb }}/managed-redis/v1/clusters/<идентификатор_кластера>/retention_policies' \
          --data '{
                     "cron": {
...
- CLI {#cli}
    ...
    1. Получите список политик для кластера {{ mrd-name }}:
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [BackupRetentionPolicy.List](../api-ref/BackupRetentionPolicy/list.md) и выполните запрос, например с помощью {{ api-examples.rest.tool }}:
        ...
        curl \
          --request GET \
          --header "Authorization: Bearer $IAM_TOKEN" \
          --url 'https://{{ api-host-mdb }}/managed-redis/v1/clusters/<идентификатор_кластера>/retention_policies?pageSize=<количество_результатов>'
        ```
    ...
    1. Убедитесь, что запрос был выполнен успешно, изучив [ответ сервера](../api-ref/BackupRetentionPolicy/list.md#yandex.cloud.mdb.redis.v1.ListBackupRetentionPoliciesResponse).
...
- gRPC API {#grpc-api}
    ...
    1. Чтобы получить список политик для кластера:
        ...
        1. Убедитесь, что запрос был выполнен успешно, изучив [ответ сервера](../api-ref/grpc/BackupRetentionPolicy/list.md#yandex.cloud.mdb.redis.v1.ListBackupRetentionPoliciesResponse).
...
- CLI {#cli}
    ...
    1. Удалите политику:
        ...
        ```bash
        {{ yc-mdb-rd }} backup-retention-policy delete <идентификатор_политики> \
           --cluster-id <идентификатор_кластера>
        ```
...
- {{ TF }} {#tf}
    ...
    1. Удалите ресурс `yandex_mdb_redis_backup_retention_policy` с именем удаляемой политики.
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [BackupRetentionPolicy.Delete](../api-ref/BackupRetentionPolicy/delete.md) и выполните запрос, например с помощью {{ api-examples.rest.tool }}:
        ...
        curl \
          --request DELETE \
          --header "Authorization: Bearer $IAM_TOKEN" \
          --url 'https://{{ api-host-mdb }}/managed-redis/v1/clusters/<идентификатор_кластера>/retention_policies/<идентификатор_политики>'
        ```
