...
{{ certificate-manager-name }} запускает процедуру обновления сертификата за 30 дней до окончания срока действия сертификата. Статус сертификата измениться на `Renewing`.
...
- Консоль управления
    ...
    1. Выберите в списке сертификат в стататусе `Renewing`, который необходимо обновить.
