...
Для прохождения проверки вам необходимо добавить специальную DNS-запись c типом `TXT` или `CNAME`: 
* При добавление TXT-записи при автоматическом обновлении сертификата через 60 дней вам придется снова пройти проверку и добавить новое значение TXT-записи.
* При добавлении CNAME-записи необходимо пройти проверку только один раз. Вы можете делегировать {{ certificate-manager-name }} право ответа в DNS-зоне домена, используемой при проверке. В этом случае проверки при выпуске и последующем обновлении сертификата будут пройдены автоматически.
...
1. В списке сервисов выберите **{{ certificate-manager-name }}**.добавить CNAME-запись.
...
1. В [консоли управления]({{ link-console-main }}) выберите каталог, в котором был создан сертификат.
1. В списке сервисов выберите **{{ certificate-manager-name }}**.добавить TXT-запись.
1. В блоке **Проверка прав на домены** в поле **Значение** посмотрите значение для домена.
