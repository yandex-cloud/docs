...
При использовании TXT-записи вам придется пройти процедуру проверки через 60 дней в рамках автоматического обновления сертификата.     

При использовании CNAME-записи возможно пройти проверку только один раз. Для этого необходимо делегировать {{ certificate-manager-name }} право ответа в DNS-зоне домена, используемой при проверке. В этом случае проверки при выпуске и последующем обновлении сертификата будут пройдены автоматически.
