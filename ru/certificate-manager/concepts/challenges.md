---
title: Проверка прав на домен в {{ certificate-manager-name }}
description: 'Способы проверки прав на домен при выпуске сертификатов Let''s Encrypt в {{ certificate-manager-name }}: HTTP и DNS. Статусы проверок и условия автоматической проверки.'
---
...
{{ certificate-manager-name }} ожидает успешного завершения процедуры проверки по каждому домену из сертификата (статус всех проверок — `Valid`). После этого Let's Encrypt выпустит сертификат. Статус сертификата изменится на `Issued`, и вы сможете использовать его в сервисах, интегрированных с {{ certificate-manager-name }}.
...
1. [Перейдите]({{ link-console-main }}/link/certificate-manager) в сервис **{{ ui-key.yacloud.iam.folder.dashboard.label_certificate-manager }}** и нажмите на имя нужного сертификата.
...
При использовании CNAME-записи достаточно пройти проверку один раз. Для этого необходимо делегировать {{ certificate-manager-name }} право ответа в DNS-зоне домена, используемой при проверке. В этом случае проверки будут проходить автоматически.
...
* Сертификат находится в статусе `Renewing` — проходит процедуру [обновления](managed-certificate.md#renew).
* Сертификат [выбран](../../storage/operations/hosting/certificate#cert-manager) для HTTPS-конфигурации статического сайта [{{ objstorage-name }}](../../tutorials/web/static/index.md).
