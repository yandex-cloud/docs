...
Для получения и обновления сертификата от Let's Encrypt<sup>®</sup> пройдите процедуру проверку прав на каждый домен, указанный в сертификате. В {{ certificate-manager-name }} доступно два типа проверок: `HTTP` и `DNS`. При создании сертификата вы можете выбрать любой тип проверки.
