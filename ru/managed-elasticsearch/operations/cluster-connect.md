...
* Использовать специальный FQDN вида `c-<идентификатор кластера {{ ES }}>.rw.{{ dns-zone }}` (например, `https://c-e4ut2....rw.{{ dns-zone }}`). Хост для подключения выбирается случайным образом среди всех хостов с ролью _Data node_.
