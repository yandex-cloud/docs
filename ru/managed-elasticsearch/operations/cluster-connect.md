...
Для подключения к хосту потребуется его [FQDN](../concepts/network.md#hostname) — доменное имя. Вы можете использовать:

* FQDN конкретного хоста с ролью _Data node_, например `{{ host-name }}.{{ dns-zone }}`.
...
* Особый FQDN вида `c-<идентификатор_кластера>.rw.{{ dns-zone }}` (например, `c-{{ cluster-id }}.rw.{{ dns-zone }}`). Хост для подключения выбирается случайным образом среди всех хостов кластера с ролью _Data node_.
