...
1. Загрузите образы контейнеров:
   ...
   * `--image-bundle` — тип пакета образов. Используйте `full` для полного набора всех необходимых образов.
   ...
   Пакет `full` содержит только базовые компоненты {{ stackland-name }}. Для загрузки образов отдельно лицензируемых компонентов, таких как {{ speechsense-name }}, используйте отдельную команду с `--image-bundle speechsense`. Подробнее в разделе [{#T}](operations/speechsense/install-images.md).
