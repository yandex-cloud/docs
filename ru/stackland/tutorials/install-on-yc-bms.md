...
Если вместо ручной загрузки ISO-образа через KVM-консоль вы хотите использовать сетевую загрузку, обратитесь к руководству [Установка {{ stackland-name }} на {{ baremetal-full-name }} через PXE](install-on-yc-bms-pxe.md).
...
IP-адреса, назначенные серверам по DHCP, могут изменяться. Описание работы DHCP в [документации сервиса {{ baremetal-full-name }}](https://yandex.cloud/ru/docs/baremetal/concepts/dhcp#dhcp-private).
