---
title: Установка {{ stackland-name }} на {{ baremetal-full-name }} через PXE
---
...
* один бастионный сервер на Ubuntu 22.04 или выше. На нем запускается `sladm` со встроенными DHCP- и TFTP-серверами для сетевой загрузки;
...
Загрузчики `ipxe.pxe` и `ipxe.efi` распространяются под лицензией GPLv2. Исходный код iPXE доступен в пакете Ubuntu 24.04 (Noble): [ipxe](https://launchpad.net/ubuntu/noble/+source/ipxe).
...
Для запуска с параметром `--dhcp-interface` `sladm` должен иметь право открывать привилегированный UDP-порт 67, который используется DHCP-сервером. Если дополнительно указан `--pxe-folder`, `sladm` также открывает UDP-порт 69 для TFTP-сервера. Рекомендуемый способ — один раз выдать это право бинарному файлу `sladm`:

```bash
sudo setcap 'cap_net_bind_service=+ep' ./sladm
```

Если `sladm` расположен не в текущей папке, укажите путь к его бинарному файлу. Менее предпочтительный вариант — запускать команду установки через `sudo`.
...
* `--dhcp-interface` — сетевой интерфейс бастиона, подключенный к приватной подсети серверов кластера. Этот параметр можно использовать без `--pxe-folder`: в таком режиме сетевая загрузка не работает, но система, загруженная с ISO-образа, автоматически настроит сеть по DHCP;
* `--pxe-folder` — путь к папке с PXE-файлами. Используется вместе с `--dhcp-interface` для сетевой загрузки;
...
После запуска `sladm` сначала скачивает образы из `cr.yandex`. Это может занять продолжительное время. Функции установки и сетевой загрузки запускаются только после завершения скачивания.

Чтобы сетевая загрузка стала доступна сразу после запуска установки, заранее загрузите образы:
...
1. Подключитесь к [KVM-консоли](https://yandex.cloud/ru/docs/baremetal/operations/servers/server-kvm).
...
Если узлы уже установлены, загрузились с локальных дисков и доступны по Talos API, повторный запуск не требует PXE-загрузки. В этом случае не указывайте `--dhcp-interface` и `--pxe-folder`: DHCP- и TFTP-сервисы запускаться не будут.
