...
* **Федеративные пользователи** — учетные записи из корпоративного провайдера идентификации (IdP). Поддерживаются протоколы SAML и LDAP.
...
* **Доступ к платформенным сервисам** — реализуется механизмами самих платформенных сервисов, например, проверка происходит с помощью логина и пароля самой СУБД. В компоненте [{{ yt-name }}](concepts/components/ytsaurus.md) учетные данные администратора хранятся в Secret `ytadminsec`; для входа в веб-интерфейс пользователей также можно аутентифицировать через {{ iam-name }} по OAuth.
