...
В кластере {{ mch-name }} можно декларативно создавать базы данных, роли и пользователей с помощью отдельных кастомных ресурсов:

* `ClickhouseDatabase` — логическая база данных в кластере {{ CH }}.
...
Все три ресурса находятся в том же пространстве имен, что и кластер `ClickhouseCluster`. Изменения применяются на все хосты кластера {{ CH }}.
...
* `spec.name` — имя сущности в {{ CH }} (имя базы данных, роли или пользователя).

Ресурс с именем, не соответствующим этому шаблону, не пройдет валидацию.

### Порядок создания ресурсов {#order}

Рекомендуемый порядок создания: база данных, затем роли, затем пользователь. Роли ссылаются на базу данных по имени в {{ CH }}, а пользователь — на роли по имени ресурса `ClickhouseRole`.
...
* `ready` — ресурс успешно применен в {{ CH }}.
