...
Данные {{ iam-name }} хранятся в YDB. Начиная с {{ stackland-name }} 26.3 администратор кластера может сохранить резервную копию этой базы данных в локальный архив `tar.gz` и восстановить ее из архива с помощью скриптов.
