...
Если у вас есть [кластер](create-cluster.md) [{{ CH }}](../../concepts/components/clickhouse.md), вы можете создать в нем пользователя с помощью кастомного ресурса `ClickhouseUser`.
...
1. Откройте файл и вставьте конфигурацию ниже:
    ...
    {% include notitle [YAML-файл](../../_includes/yamls/operations/clickhouse/clickhouseuser.md) %}

    Где:

    * `spec.cluster` — название кластера `ClickhouseCluster`.
    * `spec.name` — имя пользователя в {{ CH }}.
