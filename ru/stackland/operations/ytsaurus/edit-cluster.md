# Изменить настройки кластера {{ yt-name }}

Если компонент [{{ yt-name }}](../../concepts/components/ytsaurus.md) включен, можно изменить параметры кластера через ресурс `YTsaurusConfig main`. На платформе допустим только один экземпляр ресурса; имя ресурса фиксированное — `main`.
...
* При изменении ресурсов, реплик и состава подкомпонентов оператор {{ YT }} перезапускает соответствующие поды. На время применения новой конфигурации часть запросов прерывается.
...
* Чтобы отключить компонент целиком, см. инструкцию [Отключить {{ yt-name }}](disable.md).
...
1. Дополнительно отслеживайте состояние кластера {{ YT }}:
...
* `spec.enabled` — флаг включения компонента. Значение `false` отключает компонент и удаляет кластер вместе с данными. Подробнее см. в инструкциях [Включить {{ yt-name }}](enable.md) и [Отключить {{ yt-name }}](disable.md).
* `spec.clusterName` — название кластера {{ YT }}, отображается в веб-интерфейсе и используется для адресации ресурсов в namespace `stackland-ytsaurus`.
...
* `spec.cluster.ui.enabled` — веб-интерфейс {{ YT }}.
...
* `spec.cluster.chyt.enabled` — CHYT, движок {{ CH }} поверх {{ YT }}. Перед включением убедитесь, что включен `strawberry`.
* `spec.cluster.spyt.enabled` — SPYT, движок {{ SPRK }} поверх {{ YT }}. Перед включением убедитесь, что включен `strawberry`.
* `spec.cluster.taskProxy.enabled` — Task Proxy для запуска внешних задач.
* `spec.cluster.taskProxy.instanceCount` — 1–10.
* `spec.cluster.tutorial.enabled` — встроенный туториал {{ YT }}. Создает каталог `//home/tutorial` с примерами.
...
* [Открыть веб-интерфейс {{ YT }}](connect-ui.md).
* [Подключиться к HTTP-прокси {{ YT }}](connect-api.md).
* [Отключить {{ yt-name }}](disable.md).
