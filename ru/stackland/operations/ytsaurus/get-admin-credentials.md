# Получить пароль и токен администратора {{ yt-name }}

После [включения компонента](enable.md) учетные данные администратора кластера {{ YT }} сохраняются в Secret `ytadminsec` в пространстве имен `stackland-ytsaurus`.

Секрет содержит три ключа:

* `login` — логин администратора, всегда `admin`.
* `password` — пароль администратора для входа в [веб-интерфейс {{ YT }}](connect-ui.md).
* `token` — токен для доступа к [HTTP-прокси {{ YT }}](connect-api.md) через `yt` CLI, SDK и сторонние клиенты.
...
Токен дает полный административный доступ к кластеру {{ YT }}. Храните токен в защищенном месте и не публикуйте его в системах контроля версий, журналах и сообщениях.
