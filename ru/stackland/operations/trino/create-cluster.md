...
1. Заполните поля:
    ...
    * **Правила запросов** — `user`, `queryOwner`, `privileges` (`execute`, `view`, `kill`). Если задан `queryOwner`, привилегия `execute` недоступна.
    ...
    В поле «Пользователь» значение можно ввести как регулярное выражение или выбрать из {{ iam-name }}. Для сервисного аккаунта укажите имя ресурса `ServiceAccount` в Kubernetes, а не идентификатор субъекта IAM. Если поле сопоставления (например, «Каталог», «Схема» или «Таблица») оставлено пустым, {{ TR }} подставляет `.*`.
...
- CLI
    ...
    * `external` — внешний адрес координатора для подключения через клиенты {{ TR }} (CLI, JDBC), полный URL вида `https://...`.
...
Для подключения по внешнему адресу используются TLS и вход в {{ iam-name }} через браузер. Для Trino CLI подготовьте хранилище доверенных сертификатов — truststore. Ниже приведены команды для Linux и macOS.
...
1. Получите сертификат удостоверяющего центра (CA) и создайте truststore. Если truststore уже настроен для этого кластера, используйте его при подключении.
    ...
    Если у вас нет доступа к Secret, получите сертификат CA у администратора {{ stackland-name }} и сохраните его в `~/.trino/stackland-ca.crt`.
...
1. Подключитесь к кластеру. В `--server` укажите значение `status.fqdn.external` — полный URL координатора вида `https://...`:
    ...
    Когда CLI предложит пройти аутентификацию, откройте указанную ссылку в браузере и войдите в {{ iam-name }}. Значение `current_user` соответствует логину IAM, который вы использовали при аутентификации OAuth. Параметр `--user` необязателен. При необходимости передайте `--user '<логин_IAM>'` как подсказку для формы входа или для выполнения запросов от имени другого пользователя, если это разрешено правилами в `spec.accessControl`.
