...
- CLI {#cli}
  ...
  1. Отредактируйте параметры выбранного типа каталога.
      ...
      Для каталогов {{ PG }} и {{ CH }} доступны параметры в блоках `spec.postgres` и `spec.clickhouse` соответственно:
      ...
      * `additionalProperties` — дополнительные настройки коннектора.
      ...
      Для каталога {{ IBRG }} доступны параметры:

      * `spec.stacklandRestCatalog.catalogRef` — имя ресурса `StacklandRestCatalog` в том же пространстве имен. При выборе другого каталога убедитесь, что он готов к работе, а пользователям {{ TR }} [назначены права](../rest-catalog/create-principal.md) на него.
...
- Консоль управления {#console}
  ...
  1. Измените параметры в блоке **Настройки каталога**. Поля **Имя** и **Тип** доступны только для чтения.
      ...
      Для {{ PG }} и {{ CH }} поле **Пароль** пустое. Если оставить его пустым и сохранить, существующее значение в Secret не изменится. Введите новое значение, чтобы обновить его.

      Для {{ IBRG }} можно выбрать другой **REST Catalog** в том же пространстве имен и изменить **Способ доступа**: **Внутренний** или **Внешний**. Внешний способ доступен при наличии внешнего адреса у выбранного каталога. Убедитесь, что пользователям [назначены права](../rest-catalog/create-principal.md#console) на новый каталог.
