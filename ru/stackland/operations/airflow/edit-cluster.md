---
title: Изменить настройки кластера {{ AF }}
description: Как изменить ресурсы, зависимости и другие параметры кластера {{ AF }}
---

# Изменить настройки кластера {{ AF }}

Настройки кластера [{{ AF }}](../../concepts/components/airflow.md) изменяются в ресурсе `AirflowCluster`.
...
Для Python-пакета можно указать `version` с ограничением версии, например `">=2.0,<3.0"`; для Debian-пакета задается только `name`. Установка пакетов не меняет версию самого {{ AF }} из `spec.version`: несовместимые зависимости приводят к ошибке установки.
