...
## Подготовить Object Storage к обновлению на 26.4 и выше {#storage-postgresql-upgrade}

Если в кластере используется {{ objstorage-name }} с метабазой PostgreSQL 15, перед обновлением {{ stackland-name }} на версию 26.4.0 или выше переведите метабазу на PostgreSQL 17. В версии 26.3 можно продолжать использовать PostgreSQL 15. Для новых установок {{ objstorage-name }} PostgreSQL 17 включен автоматически.

{% note warning %}

Во время обновления метабазы S3 API недоступен. Выберите время обслуживания с учетом приложений, которые используют {{ objstorage-name }}. Обновление метабазы начинается после явного подтверждения администратора.

{% endnote %}

1. Если обновление кластера уже запрошено, проверьте причину блокировки:

    ```bash
    kubectl get targetinstallationstate main -o yaml
    ```

    Если требуется обновить метабазу, ресурс переходит в `status.phase: Blocked`, а в `status.message` появляется сообщение о необходимости PostgreSQL 17.

1. В выбранное время обслуживания подтвердите обновление метабазы:

    ```bash
    kubectl annotate storageconfig main \
      stackland.yandex.cloud/approve-postgres-upgrade=true --overwrite
    ```

1. Отслеживайте состояние {{ objstorage-name }}:

    ```bash
    kubectl get storageconfig main -o yaml
    ```

    После успешного обновления хранилище возвращается в `Ready`, а аннотация подтверждения удаляется автоматически. Проверьте доступ приложений к S3 API. Если обновление кластера было заблокировано из-за версии метабазы, оно продолжится автоматически.

## Перед обновлением на 27.x {#pre-upgrade-27}

В {{ stackland-name }} 26.2 добавлена новая реализация Ingress-контроллера на основе Contour: предыдущая реализация на базе ingress-nginx больше не поддерживается. В {{ stackland-name }} 27 ingress-nginx будет исключен из поставки, поэтому мы предупреждаем вас об этих планах заранее. В релизах {{ stackland-name }} 26 ingress-nginx по-прежнему доступен для пользовательской нагрузки, но переход на новую реализацию нужно запланировать и осуществить до обновления на {{ stackland-name }} 27.

В большинстве случаев пользовательские манифесты `Ingress` с классом `stackland-default` продолжают работать, но специфичные для ingress-nginx аннотации `nginx.ingress.kubernetes.io/*` новый контроллер не поддерживает — их нужно убрать или заменить. Подробнее см. в разделе [{#T}](#check-user-ingress).
...
* `phase` — фаза обновления:
    ...
    * `Blocked` — обновление ожидает выполнения обязательных условий. Причина указана в `message`.
