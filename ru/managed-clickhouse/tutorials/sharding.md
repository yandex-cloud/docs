...
Шардирование обеспечивает [ряд преимуществ](../concepts/sharding#advantages) при высокой частоте запросов и работе с большими наборами данных. При этом создается распределенная таблица, которая маршрутизирует запросы к нижележащим таблицам. Обращаться к данным в шардированных таблицах можно как через распределенную таблицу, так и напрямую.
...
- Вручную
    ...
    1. [Создайте кластер](../operations/cluster-create.md) {{ mch-name }}:
        ...
        * **Тип хранилища** — выберите нужный тип хранилища.

            От выбранного типа хранилища зависит минимальное количество хостов в каждом шарде:

            * Два хоста, если выбрано хранилище на локальных SSD-дисках (`local-ssd`).
            * Три хоста, если выбрано хранилище на сетевых нереплицируемых дисках (`network-ssd-nonreplicated`).

            Дополнительные хосты для этих типов хранилищ необходимы для обеспечения отказоустойчивости.
    ...
    1. [Добавьте в кластер три хоста {{ ZK }}](../operations/zk-hosts.md#add-zk-host).
...
[Установите и настройте clickhouse-client](../operations/connect.md), чтобы иметь возможность подключаться с его помощью к базе данных.
