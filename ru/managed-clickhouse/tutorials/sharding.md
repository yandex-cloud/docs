...
По умолчанию Managed Service for ClickHouse создает первый шард вместе с кластером. Этот шард включает все хосты кластера. Чтобы начать использовать шардирование, [добавьте](../operations/shards.md#add-shard) нужное количество шардов и создайте таблицу на базе движка [Distributed](https://clickhouse.yandex/docs/ru/operations/table_engines/distributed/). В статье по ссылке описаны стратегии шардирования и инструкции по созданию таблиц нужного вида, а также ограничения распределенных таблиц.

Managed Service for ClickHouse автоматически создает [конфигурацию шардов](https://clickhouse.yandex/docs/ru/operations/table_engines/distributed/) в кластере. Вы можете [управлять этой конфигурацией](../operations/shards.md#shard-update).
