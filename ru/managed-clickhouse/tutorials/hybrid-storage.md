...
1. [Создайте кластер](../operations/cluster-create.md) {{ mch-name }} с сетевым хранилищем, включенным гибридным хранилищем и базой данных `tutorial`.
...
1. Познакомьтесь с тестовым набором данных:
   ...
   Таблица `tutorial.hits_v1` будет [настроена при создании](#create-table) таким образом, чтобы все «свежие» данные в таблице с 21 марта 2014 года и позже попали в сетевое хранилище, а более старые данные (с 17 марта по 20 марта 2014 года) — в объектное хранилище.   
...
1. Для данных в таблице проверяется значение `EventDate`:
   - если количество дней от текущей даты до `EventDate` меньше значения TTL (то есть время жизни еще не истекло), то эти данные остаются в сетевом хранилище;
