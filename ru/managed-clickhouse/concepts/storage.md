...
* `default` (по умолчанию) — кластер автоматически управляет размещением данных в зависимости от объема свободного места в кластерном хранилище и настроек [TTL](https://clickhouse.tech/docs/ru/engines/table-engines/mergetree-family/mergetree/#mergetree-table-ttl) (время жизни) для таблиц.
...
Политики хранения не оказывают влияния на [операции слияния]{% if lang == "ru" %}(https://clickhouse.com/docs/ru/engines/table-engines/mergetree-family/custom-partitioning-key/){% endif %}{% if lang == "en" %}(https://clickhouse.com/docs/en/engines/table-engines/mergetree-family/custom-partitioning-key/){% endif %} кусков данных:
...
* Хранилище на локальных SSD-дисках (`local-ssd`) — использует самые быстрые диски. Объем такого хранилища может составлять от 10 до 2048 ГБ.
