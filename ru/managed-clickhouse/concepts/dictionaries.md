...
Доступ к данным словаря осуществляется с помощью [набора функций]({{ ch.docs }}{{ lang }}/sql-reference/functions), которые выполняются быстрее стандартных SQL-запросов. Доступ к словарям осуществляется только на чтение, записать данные в словарь с помощью {{ CH }} невозможно. 
...
В качестве источников данных словарей могут выступать [встроенные словари {{ CH }}]({{ ch.docs }}{{ lang }}/sql-reference/statements/create/dictionary/embedded) или [внешние источники]({{ ch.docs }}{{ lang }}/sql-reference/statements/create/dictionary): HTTP(s) ресурс или другая СУБД: {{ MY }}, {{ CH }}, {{ SD }}, {{ PG }}.
...
Подробнее о функциях для работы со встроенными словарями читайте в [документации {{ CH }}]({{ ch.docs }}{{ lang }}/sql-reference/functions/ym-dict-functions).
