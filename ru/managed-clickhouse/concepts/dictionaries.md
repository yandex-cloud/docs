...
Доступ к данным словаря осуществляется с помощью [набора функций](https://{{ ch-domain }}/docs/ru/query_language/functions/ext_dict_functions/), которые выполняются быстрее стандартных SQL-запросов. Доступ к словарям осуществляется только на чтение, записать данные в словарь с помощью {{ CH }} невозможно. 
...
В качестве источников данных словарей могут выступать [встроенные словари {{ CH }}](https://{{ ch-domain }}/docs/ru/query_language/dicts/internal_dicts/) или [внешние источники](https://{{ ch-domain }}/docs/ru/query_language/dicts/external_dicts/): HTTP(s) ресурс или другая СУБД: {{ MY }}, {{ CH }}, {{ MG }}, {{ PG }}.
...
Подробнее о функциях для работы со встроенными словарями читайте в [документации {{ CH }}](https://{{ ch-domain }}/docs/ru/query_language/functions/ym_dict_functions/).
