...
Подробнее о {{ CK }} см. в [документации {{ CH }}]({% if lang == "ru" %}https://{{ ch-domain }}/docs/ru/operations/clickhouse-keeper/{% endif %}{% if lang == "en" %}https://{{ ch-domain }}/docs/en/operations/clickhouse-keeper/{% endif %}).
...
Чтобы создать реплицируемые таблицы на всех хостах кластера, отправьте [распределенный DDL-запрос](https://{{ ch-domain }}/docs/ru/sql-reference/distributed-ddl/):
