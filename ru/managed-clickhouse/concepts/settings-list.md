...
[Пользовательские настройки]{% if lang == "ru" %}(https://{{ ch-domain }}/docs/ru/operations/settings/#custom_settings){% endif %}{% if lang == "en" %}(https://{{ ch-domain }}/docs/en/operations/settings/#custom_settings){% endif %} можно переопределять, как переменные, в [профиле настроек]{% if lang == "ru" %}(https://{{ ch-domain }}/docs/ru/operations/access-rights/#settings-profiles-management){% endif %}{% if lang == "en" %}(https://{{ ch-domain }}/docs/en/operations/access-rights/#settings-profiles-management){% endif %}, сессии или запросе. Работа с профилями настроек доступна, если для кластера включено [управление пользователями через SQL](../operations/cluster-users.md#sql-user-management).
