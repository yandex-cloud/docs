...
В резервной копии хранятся данные только для движков семейства `MergeTree`. Для остальных движков хранятся только схемы таблиц. Подробнее про движки в [документации {{ CH }}]({{ ch.docs }}{{ lang }}/engines/table-engines).
...
* Резервные копии данных [локального и сетевых хранилищ](storage.md) содержатся в отдельном бакете {{ objstorage-name }} и не занимают место в хранилище кластера. При этом, если в кластере есть N свободных гигабайт места, то хранение первых N гигабайт резервных копий не тарифицируется.
...
Из резервной копии можно восстановить не только кластер целиком, но и отдельный [шард](../concepts/sharding.md).
