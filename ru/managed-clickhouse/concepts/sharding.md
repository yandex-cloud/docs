...
- В шард можно [добавить хосты](../operations/hosts.md#add-host).
  ...
  * В кластере с {{ CK }} можно сразу добавлять хосты: репликация уже работает.
  * В кластере с {{ ZK }} нужно [включить отказоустойчивость](../operations/zk-hosts.md#add-zk), если она еще не включена.

  Подробнее о репликации, {{ CK }} и {{ ZK }} см. в разделе [{#T}](replication.md).
