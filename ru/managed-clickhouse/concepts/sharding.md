...
Подробнее о работе с распределенными таблицами в [документации {{ CH }}]({{ ch.docs }}{{ lang }}/engines/table-engines/special/distributed).
...
- В шард можно [добавить хосты](../operations/hosts.md#add-host).
    ...
    * В кластерах с шардами из одного хоста нужно [включить сервис координации {{ CK }} или {{ ZK }}](../operations/update.md#enable-coordination), и только потом добавлять хосты в шард.
    ...
    Подробнее о репликации, {{ CK }} и {{ ZK }} в разделе [Репликация](replication.md).
