---
title: "Managed Service for ClickHouse. Ответы на вопросы"
description: "Что такое Managed Service for ClickHouse? Для каких задач стоит использовать Managed Service for ClickHouse, а для каких виртуальные машины с базами данных? Какую часть работы по управлению и сопровождению баз данных берет на себя Managed Service for ClickHouse? Ответы на эти и другие вопросы в данной статье."
---
...
Создание новых хостов с СУБД неподдерживаемых версий становится невозможным. Кластеры с неподдерживаемой версией {{ CH }} обновляются в соответствии с [политикой работы с версиями](../concepts/update-policy.md).
...
В [консоли управления]({{ link-console-main }}) перейдите на страницу каталога, выберите сервис **{{ mch-short-name }}** и нажмите на нужный кластер. В правой части экрана вы увидите стоимость использования кластера за месяц. {% if audience != "internal" %}Подробнее см. [{#T}](../pricing.md).{% endif %}
