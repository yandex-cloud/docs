...
{{ mch-short-name }} берет на себя трудоемкие задачи администрирования инфраструктуры {{ mch-short-name }}:
...
{{ mch-short-name }} доступен всем зарегистрированным пользователям Яндекс.Облака.
...
Для хранилища на NVMe-дисках количество хостов ограничено при создании: для {{ CH }}-кластеров можно создать не меньше 2 хостов.
...
{{ mch-short-name }} использует последнюю стабильную версию {{ CH }}.
...
Через месяц после того, как версия СУБД становится неподдерживаемой, {{ mch-short-name }} автоматически оповещает владельцев кластеров БД, созданных с этой версией, по электронной почте.
