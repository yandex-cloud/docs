...
Если в кластере используется [репликация](../concepts/replication.md), на вкладке **Мониторинг** появляются две вкладки:
...
Если репликация не используется, графики с вкладки **Clickhouse** показываются на общей вкладке **Мониторинг**.
...
### Состояния хоста {{CH}}
Перейдите на вкладку **Хосты**, смотрите столбец **Состояние**. 

Состояние | Описание состояния | Предлагаемые действия
:--- | :--- | :---
**ALIVE** | Хост работает в штатном режиме | Действий не требуется.
**DEGRADED** | Хост выполняет только часть своих основных функций | Выполните самостоятельную диагностику:<ul><li>Посмотрите список операций. Возможно, какая-то из них ещё не завершилась.</li><li>Убедитесь, что кластер не находится в процессе технического обслуживания.</li></ul>Если причины не удалось выяснить самостоятельно, составьте обращение в службу поддержки:<ul><li>Укажите id кластера, id хоста и id последних операций, которые на нём выполнялись.</li><li>По графикам доступности уточните, когда именно он перешел в состояние `DEGRADED`.</li></ul>
**DEAD** | Хост не способен выполнять свои функции | Составьте обращение в службу поддержки:<ul><li>Укажите id кластера, id хоста и id последних операций, которые на нём выполнялись.</li><li>По [графикам мониторинга](#monitoring-cluster) уточните, когда именно он перешел в состояние `DEAD`.</li></ul>
**HEALTH_UNKNOWN** | Состояние хоста неизвестно | Составьте обращение в службу поддержки:<ul><li>Укажите id кластера, id хоста и id последних операций, которые на нём выполнялись.</li><li>По [графикам мониторинга](#monitoring-cluster) уточните, когда именно он перешел в состояние `HEALTH_UNKNOWN`.</li></ul>
