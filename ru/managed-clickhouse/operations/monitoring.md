...
- {{ CH }}
  ...
  * **Max data parts per partition** — максимальное количество кусков данных в партиции. Лимит для этой величины определяется [настройками СУБД](../concepts/settings-list#setting-merge-tree). Приближение к лимиту указывает на чрезмерную нагрузку или низкую эффективность вставки данных.
