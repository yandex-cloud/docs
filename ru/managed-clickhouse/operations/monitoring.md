...
- {{ ui-key.yacloud.clickhouse.title_clickhouse }}
  ...
  * В блоке **Storage**:
    ...
    * **Inode usage, %** — процент использования inode для каждого хоста в подкластере {{ CH }}.
...
- {{ ui-key.yacloud.clickhouse.title_zookeeper }}
  ...
  * В блоке **Service Metrics**:
    ...
    * **Watches** — количество объектов Watch.
    ...
    Подробнее об объектах Znode, Ephemeral node и Watch в [документации {{ ZK }}](https://zookeeper.apache.org/doc/current/zookeeperOver.html). {#zookeeper-objects}
...
Текущий размер хранилища можно посмотреть в [детальной информации о кластере](cluster-list.md#get-cluster). Полный список поддерживаемых метрик приведен в [документации {{ monitoring-name }}](../../monitoring/metrics-ref/managed-clickhouse-ref.md).
