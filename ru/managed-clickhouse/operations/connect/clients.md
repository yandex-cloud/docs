...
description: "Следуя этой инструкции, вы сможете подключиться к базе данных в кластере {{ CH }} из специальных приложений, браузера и графических IDE."
---

# Подключение к кластеру {{ CH }} из приложений

В этом разделе представлены настройки для подключения к хостам кластера {{ mch-name }} с помощью [инструментов командной строки](#command-line-tools), из [графических IDE](#ide), [браузера](#browser) и [Docker-контейнера](#docker).

О подключении к хостам кластера из кода вашего приложения см. [Примеры кода](code-examples.md).

При необходимости перед подключением выполните [предварительную настройку](index.md): настройте группы безопасности кластера и получите SSL-сертификаты.
