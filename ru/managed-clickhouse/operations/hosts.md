...
Чтобы добавить новые хосты к кластеру из одного хоста, сначала [добавьте хосты {{ ZK }}](zk-hosts.md), которые обеспечат отказоустойчивость кластера.
...
- CLI
  ...
  1. Выполните команду добавления хоста:
     ...
     {% if audience != "internal" %} Идентификатор подсети необходимо указать, если в зоне доступности больше одной подсети, в противном случае {{ mch-short-name }} автоматически выберет единственную подсеть. {% endif %} Имя кластера можно запросить со [списком кластеров в каталоге](cluster-list.md#list-clusters).
