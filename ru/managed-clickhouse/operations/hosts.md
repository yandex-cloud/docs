...
На данный момент невозможно как добавить хост к кластеру из одного хоста, так и уменьшить количество хостов в кластере до 1. Решение о том, что данные в {{ CH }}-кластере должны реплицироваться, необходимо принять при создании кластера.
...
Количество хостов в кластерах {{ mch-short-name }} ограничено квотами на количество CPU и объем памяти, которые доступны кластерам БД в вашем облаке. Чтобы проверить используемые ресурсы, откройте страницу [Квоты](https://console.cloud.yandex.ru/?section=quotas) и найдите блок **{{ mch-full-name }}**.
