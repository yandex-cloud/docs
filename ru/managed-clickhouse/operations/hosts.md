...
Кластер, созданный с поддержкой механизма репликации [{{ CK }}](../concepts/replication.md#ck), может состоять только из одного или трех хостов, и добавлять новые хосты в него нельзя. Эта функциональность находится на стадии [Preview](../../overview/concepts/launch-stages.md). На стадии General Availability это ограничение будет снято.

{% else %}

Кластер, созданный с поддержкой механизма репликации [{{ CK }}](../concepts/replication.md#ck), может состоять только из одного или трех хостов, и добавлять новые хосты в него нельзя. Эта функциональность находится на стадии Preview. На стадии General Availability это ограничение будет снято.
