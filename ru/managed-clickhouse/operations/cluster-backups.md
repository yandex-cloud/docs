...
- CLI {#cli}
  ...
  1. Чтобы восстановить кластер, выполните команду в зависимости от резервной копии:
      ...
      * Если резервная копия создавалась сразу на все шарды кластера (актуальный вариант создания резервных копий), передайте в команде один идентификатор резервной копии:
      ...
      * Если резервные копии создавались отдельно на каждый шард кластера (устаревший вариант создания резервных копий), чтобы восстановить весь кластер целиком, передайте идентификаторы резервных копий всех шардов кластера:
