...
- CLI
  ...
  1. Запросите создание кластера из резервной копии:
      ...
      * Сеть `{{ network-name }}`.
      * Один хост класса `{{ host-class }}` в подсети `b0rcctk2rvtr8efcch63`, в зоне доступности `{{ zone-id }}`. 
      * База данных и пользователи из резервной копии.
...
- Консоль управления
  ...
  Время начала резервного копирования можно задать в [консоли управления]({{ link-console-main }}) при [создании](cluster-create.md) или [изменении](update.md#change-clickhouse-config) кластера в блоке **Дополнительные настройки**.
