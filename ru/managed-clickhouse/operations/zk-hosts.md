...
[Шарды](../concepts/sharding.md) из двух и более хостов с поддержкой [{{ CK }}](../concepts/replication.md#ck) отказоустойчивы по умолчанию. Если кластер был создан без поддержки {{ CK }}, его шарды из одного хоста не отказоустойчивы и не обеспечивают репликацию данных. Чтобы сделать шарды такого кластера отказоустойчивыми:

1. [Включите отказоустойчивость с использованием {{ ZK }}](#add-zk).
1. [Добавьте в шард дополнительные хосты {{ CH }}](./hosts.md#add-host).
