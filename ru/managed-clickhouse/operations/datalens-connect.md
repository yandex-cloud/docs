# Подключение к {{ CH }}-кластеру из {{ datalens-name }}
...
- Консоль управления
  ...
  1. Нажмите кнопку **Создать подключение**.
     ...
     Если вместо кнопки **Создать подключение** вы видите сообщение «{{ datalens-name }} еще не активирован», значит вы либо еще не активировали {{ datalens-name }}, либо подключили его в другом каталоге.
...
- Консоль управления
  ...
  В [консоли управления]({{ link-console-main }}) при [создании](#create-connector) или [изменении](#update-connector) подключения к кластеру:
    ...
    1. Введите пароль этого пользователя.
        ...
        Значения настроек **Порт HTTP-интерфейса**, **HTTPS**, **Время жизни кeша в секундах** и **Уровень доступа SQL запросов** оставьте без изменений.
...
Если для пользователя задана [настройка **Read only**](cluster-users.md#setting-readonly), убедитесь, что значение настройки — `0` или `2`.
В противном случае подключение работать не будет.
