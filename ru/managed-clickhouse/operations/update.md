...
Ниже описано, как изменить [настройки {{ CH }}, доступные в {{ yandex-cloud }}](../concepts/settings-list.md). Если вы хотите на уровне пользователя изменить настройки {{ CH }}, недоступные в {{ yandex-cloud }}, обратитесь к разделу [{#T}](change-query-level-settings.md).

Изменение некоторых [настроек на уровне кластера](../concepts/settings-list.md#dbms-cluster-settings) приводит к перезапуску серверов {{ CH }} на хостах кластера.
