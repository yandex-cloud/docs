...
Для некоторых [настроек на уровне кластера](../concepts/settings-list.md#dbms-cluster-settings) изменения вступают в силу только после перезапуска кластера. Проверьте описание измененных настроек и [перезапустите кластер](./cluster-stop.md) при необходимости.
