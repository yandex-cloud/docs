...
Если при создании кластера {{ mch-name }} не включить шардирование, будет создан один шард, содержащий все хосты кластера. Чтобы шардировать данные, [добавьте](#add-shard) еще один или несколько шардов и [создайте](../tutorials/sharding.md#example) распределенную таблицу.
...
В кластере с отключенным [сервисом координации](../concepts/coordination-system.md) или встроенным {{ CK }} невозможно создать шард из двух и более хостов. Сначала [включите сервис координации](update.md#coordination) {{ CK }} или {{ ZK }} на отдельных хостах.
