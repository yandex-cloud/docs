...
К кластеру можно подключиться как с использованием шифрования — через порты `9440` для [clickhouse-client]{% if lang == "ru" %}(https://clickhouse.tech/docs/ru/interfaces/cli/){% endif %}{% if lang == "en" %}(https://clickhouse.tech/docs/en/interfaces/cli/){% endif %} и `8443` для [HTTP-интерфейса]{% if lang == "ru" %}(https://clickhouse.tech/docs/ru/interfaces/http/){% endif %}{% if lang == "en" %}(https://clickhouse.tech/docs/en/interfaces/http/){% endif %}, так и без него — через порты `9000` и `8123` соответственно.
...
* `c-<идентификатор кластера>.rw.{{ dns-zone }}` для подключения к мастеру кластера.

* `<имя шарда>.c-<идентификатор кластера>.rw.{{ dns-zone }}` для подключения к мастеру [шарда](../concepts/sharding.md).
