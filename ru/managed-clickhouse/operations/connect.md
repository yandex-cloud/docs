...
При подключении из браузера SQL-запросы выполняется отдельно, без создания единой сессии с сервером {{ CH }}. Поэтому запросы, которые действуют в пределах сессии, не имеют эффекта. Например, `USE` или `SET`.
...
Подробнее см. раздел [{#T}](web-sql-query.md)
