...
`steady_cumulative` | Тест остановится, если если в течение определенного периода совокупные процентили не изменяются.</br>Пример: `steady_cumulative(1m)`.</br>Код завершения — `33`.
`limit` | Тест остановится через указанный период времени.</br>Пример: `limit(1m)`.
