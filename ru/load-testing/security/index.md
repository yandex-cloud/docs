...
* Сервисные роли:
    ...
    * `loadtesting.editor` — разрешает просмотр и удаление отчётов, создание, удаление или редактирование ВМ с агентами.
    * `loadtesting.admin` — на данный момент сервис не включает возможностей для администрирования, поэтому эта роль повторяет возможности роли `editor`.
    * `loadtesting.loadTester` — разрешает запуск нагрузочных тестов, остановку теста, загрузку тестовых данных, удаление отчётов.
    * `loadtesting.generatorClient` — разрешает запуск теста на агенте и загрузку результатов в хранилище (выдается сервисному аккаунту, от имени которого создаётся ВМ с агентом).
