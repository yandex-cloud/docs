---
title: Как обновить подключение виртуальной машины {{ compute-full-name }} к {{ backup-full-name }}
description: Следуя данной инструкции, вы сможете обновить подключение виртуальной машины {{ compute-full-name }} к {{ backup-name }}.
---
...
Если вы [удалили](delete-vm.md) ВМ {{ compute-full-name }} из {{ backup-name }} и хотите подключить ее к сервису заново, воспользуйтесь инструкциями:
...
1. Убедитесь, что неактуальная ВМ удалена из списка ВМ, подключенных к {{ backup-name }}:
    ...
    - Консоль управления {#console}
      ...
      1. На вкладке **{{ ui-key.yacloud.backup.label_instances }}** убедитесь, что в списке нет неактуальной ВМ с меткой ![irrelevant](../../_assets/console-icons/circle-info-fill.svg).
