...
* Реализовано подключение к {{ backup-name }} ВМ и серверов, расположенных за пределами {{ yandex-cloud }}. Подробнее в документации:
...
* Упрощено подключение существующей ВМ {{ compute-name }} к {{ backup-name }} в [консоли управления]({{ link-console-main }}) — теперь не нужно привязывать к ВМ сервисный аккаунт и вручную устанавливать агент {{ backup-name }}.
* Добавлены [политики авторизации](./security/index.md#access-policies) {{ iam-name }}, с помощью которых вы можете ограничить определенные действия с ресурсами {{ backup-name }} даже при наличии у пользователей разрешающих ролей.
