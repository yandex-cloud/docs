...
## Конфигурация Salt: хосты, {{ k8s }} и OCI Registry

В файл `pillar.sls` записывается конфигурация для Salt, используемая для:

* разворачивания кластера {{ k8s }};
...
k8s:
  ...
  block_size: 26
  # Хосты, которые будут выполнять роль мастеров {{ k8s }}
  master_hostnames:
    - onprem01.local
    - onprem02.local
    - onprem03.local
...
bootstrap:
  # Версия {{ objstorage-onprem-name }}, указана в поставке
  version: 26.3.0
  # Параметры инсталляции {{ objstorage-onprem-name }}
  values_object: |
    <конфигурация>
```

Где `values_object` — [конфигурация {{ objstorage-onprem-name }}](#objstorage-config) в формате YAML.

* Подсети `k8s.pod_subnet`, `k8s.service_subnet` используются исключительно внутри кластера {{ k8s }} и не должны пересекаться с другими подсетями, включая data, internal и management.
* `k8s.calico.node_address_autodetection` должен соответствовать internal-подсети.
* Количество мастеров {{ k8s }} зависит от требований надежности: три — для поддержки отказа одного хоста, пять — для поддержки отказа двух хостов.


## Конфигурация {{ objstorage-onprem-name }} {#objstorage-config}

Пример инсталляции {{ objstorage-name }} с комментариями:
