...
{{ objstorage-onprem-name }} предоставляет комплексную систему мониторинга для наблюдения за состоянием всех компонентов хранилища. Мониторинг реализован на базе Grafana с хранением метрик и логов непосредственно в S3.
...
Для мониторинга кластеров метаданных на базе {{ PG }} используется дашборд **CloudNativePG** в Grafana. На нем собраны метрики по всем кластерам:
...
Подробнее о замене дисков смотрите в документации по обслуживанию.
...
Текущий список метрик в Federation эндпоинт:
