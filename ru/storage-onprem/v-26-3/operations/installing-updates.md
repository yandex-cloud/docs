...
* Архив `yc-storage-<версия>.tar.gz` — ПО {{ objstorage-name }}, где `<версия>` — поставляемая версия.
* Два значения: новые версии `yc-storage-operator` и версия {{ objstorage-onprem-name }}.
...
1. [Обновите {{ objstorage-name }}](#update-objstorage).
...
При обновлении {{ objstorage-name }} возможна временная деградация производительности или недоступность S3 API строго по одному из адресов.
...
Выполняйте этот шаг на любом master-хосте {{ k8s }}. В большинстве случаев хост из первого шага также является master-хостом {{ k8s }}.
...
## Обновите {{ objstorage-name }} {#update-objstorage}
