...
description: "При создании {{ MY }}-кластера вы можете задать сеть для самого кластера, подсети для каждого из хостов кластера и запросить публичный доступ, чтобы подключаться к кластеру извне {{ yandex-cloud }}."
---
