...
1. В консоли управления [создайте виртуальную машину](../../compute/operations/vm-create/create-linux-vm.md) с публичным IP-адресом на базе [Ubuntu Linux 18.04]{% if lang == "ru" %}(https://cloud.yandex.ru/marketplace/products/f2e9qa7i4fmugh14tjnc){% endif %}{% if lang == "en" %}(https://cloud.yandex.com/en-ru/marketplace/products/f2e9qa7i4fmugh14tjnc){% endif %}. Минимальной конфигурации (1 ядро, 2 ГБ RAM, 10 ГБ дискового пространства) хватит для переноса базы до 1 ГБ. Чем больше переносимая база, тем больше должно быть оперативной памяти и дискового пространства (как минимум в два раза больше размера базы).

{% else %}

1. В консоли управления создайте виртуальную машину с публичным IP-адресом на базе [Ubuntu Linux 18.04]{% if lang == "ru" %}(https://cloud.yandex.ru/marketplace/products/f2e9qa7i4fmugh14tjnc){% endif %}{% if lang == "en" %}(https://cloud.yandex.com/en-ru/marketplace/products/f2e9qa7i4fmugh14tjnc){% endif %}. Минимальной конфигурации (1 ядро, 2 ГБ RAM, 10 ГБ дискового пространства) хватит для переноса базы до 1 ГБ. Чем больше переносимая база, тем больше должно быть оперативной памяти и дискового пространства (как минимум в два раза больше размера базы).
...
* Если вы восстанавливаете дамп с собственного сервера, [получите SSL-сертификат](../operations/connect.md#get-ssl-cert) и передайте параметры `--ssl-ca` и `--ssl-mode` в команде восстановления:
