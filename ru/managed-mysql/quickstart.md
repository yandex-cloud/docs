...
1. Подключаться к кластерам БД можно как изнутри, так и извне {{ yandex-cloud }}:
   * Чтобы подключиться изнутри {{ yandex-cloud }}, {% if product == "yandex-cloud" %}создайте виртуальную машину{% endif %}{% if product == "cloud-il" %}[создайте виртуальную машину на основе Linux](../compute/quickstart/quick-create-linux.md){% endif %} в той же облачной сети, что и кластер БД{% if product == "yandex-cloud" %} (на основе [Linux](../compute/quickstart/quick-create-linux.md) или [Windows](../compute/quickstart/quick-create-windows.md)){% endif %}.
