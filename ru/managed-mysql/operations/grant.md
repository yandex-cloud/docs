...
Пользователь, который создается вместе с кластером **{{ mmy-name }}**, автоматически получает роль владельца (`ALL_PRIVELEGES`) первой базы данных в кластере. После этого вы можете создавать других пользователей и настраивать их права по своему усмотрению.

## Назначить роли пользователю {#grant-role}
