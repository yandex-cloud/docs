...
Создать дамп базы данных следует с помощью утилиты `mysqldump`, которая подробно описана в [документации {{ MY }}](https://dev.mysql.com/doc/refman/5.7/en/mysqldump.html).
...
* Если вы восстанавливаете дамп с собственного сервера, необходимо [скачать сертификат](connect.md#configuring-an-ssl-certificate) и задать параметры SSL `--ssl-ca` и `--ssl-mode`.
