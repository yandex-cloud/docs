...
Подробнее об устройстве кластера {{ mmy-name }} смотрите в разделе [Взаимосвязь ресурсов сервиса](../concepts/index.md).
...
- Консоль управления {#console}
  ...
  1. В блоке **{{ ui-key.yacloud.mdb.forms.section_disk }}**:
     ...
     * Выберите объем, который будет использоваться для данных и резервных копий. Подробнее о том, как занимают пространство резервные копии, читайте в разделе [Резервные копии](../concepts/backup.md).
     ...
     * (Опционально) Выберите опцию **{{ ui-key.yacloud.compute.disk-form.label_disk-encryption }}**, чтобы зашифровать диск [пользовательским ключом KMS](../../kms/concepts/key.md).
       ...
       * Чтобы использовать созданный ранее ключ, выберите его в поле **{{ ui-key.yacloud.compute.disk-form.label_disk-kms-key }}**.
       ...
       Подробнее о шифровании дисков смотрите в разделе [Хранилище](../concepts/storage.md#disk-encryption).
  ...
  1. В блоке **{{ ui-key.yacloud.mdb.forms.section_network }}** выберите:
     ...
     * [Группы безопасности](../../vpc/concepts/security-groups.md) для сетевого трафика кластера. Может потребоваться дополнительная [настройка групп безопасности](./connect/index.md#configure-security-groups) для подключения к кластеру {{ mmy-name }}.
...
- CLI {#cli}
  ...
  1. Укажите параметры кластера {{ mmy-name }} в команде создания:
     ...
     * `--datalens-access` — разрешает доступ к кластеру из {{ datalens-full-name }}. Значение по умолчанию — `false`. Подробнее о настройке подключения смотрите в разделе [{#T}](datalens-connect.md).
...
- {{ TF }} {#tf}
  ...
  1. Опишите в конфигурационном файле параметры ресурсов, которые необходимо создать:
     ...
     * Чтобы активировать сбор статистики для [диагностики производительности кластера](performance-diagnostics.md), добавьте к описанию кластера {{ mmy-name }} блок `performance_diagnostics`:
     ...
     Более подробную информацию о ресурсах, которые вы можете создать с помощью {{ TF }}, читайте в [документации провайдера]({{ tf-provider-mmy }}).
  ...
  1. Создайте файл `body.json` и добавьте в него следующее содержимое:
      ...
      * `configSpec` — настройки кластера:
          ...
          * `access` — настройки доступа к кластеру из сервисов {{ yandex-cloud }}:
            ...
            * `dataLens` — доступ из {{ datalens-full-name }}. Подробнее о настройке подключения смотрите в разделе [{#T}](datalens-connect.md).
  ...
  1. Создайте файл `body.json` и добавьте в него следующее содержимое:
      ...
      * `config_spec` — настройки кластера:
          ...
          * `access` — настройки доступа к кластеру из сервисов {{ yandex-cloud }}:
            ...
            * `data_lens` — доступ из {{ datalens-full-name }}. Подробнее о настройке подключения смотрите в разделе [{#T}](datalens-connect.md).
