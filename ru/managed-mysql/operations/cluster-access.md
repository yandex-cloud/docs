---
title: Управление доступом к кластеру {{ mmy-full-name }}
description: Следуя данной инструкции, вы настроите права доступа к кластеру {{ mmy-name }}.
---

# Управление доступом к кластеру {{ mmy-name }}
...
- CLI {#cli}
  ...
  1. Получите список ролей, назначенных на кластер, выполнив команду:
      ...
      ```bash
      {{ yc-mdb-my }} cluster list-access-bindings <имя_или_идентификатор_кластера>
      ```
...
- REST API {#api}
  ...
  1. Воспользуйтесь методом [Cluster.ListAccessBindings](../api-ref/Cluster/listAccessBindings.md) и выполните запрос, например, с помощью {{ api-examples.rest.tool }}:
      ...
      curl \
        ...
        --url 'https://{{ api-host-mdb }}/managed-mysql/v1/clusters/<идентификатор_кластера>:listAccessBindings'
      ```
...
- CLI {#cli}
  ...
  1. Посмотрите описание команды CLI для назначения роли на кластер:
      ...
      ```bash
      {{ yc-mdb-my }} cluster add-access-binding --help
      ```

  1. Назначьте роль, выполнив команду:

      ```bash
      {{ yc-mdb-my }} cluster add-access-binding <имя_или_идентификатор_кластера> \
        --role <роль> \
...
- REST API {#api}
  ...
  1. Воспользуйтесь методом [Cluster.UpdateAccessBindings](../api-ref/Cluster/updateAccessBindings.md) и выполните запрос, например, с помощью {{ api-examples.rest.tool }}:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://{{ api-host-mdb }}/managed-mysql/v1/clusters/<идентификатор_кластера>:updateAccessBindings' \
        --data '{
...
- {{ TF }} {#tf}
   ...
   Для назначения ролей на кластер {{ mmy-name }} используйте ресурс `yandex_mdb_mysql_cluster_iam_binding` с параметром `members`.
   
   {% endnote %}

  1. Откройте актуальный конфигурационный файл с описанием кластера {{ mmy-name }}.
  ...
  1. Подтвердите изменение ресурсов.
     ...
     Подробнее о параметрах ресурса `yandex_mdb_mysql_cluster_iam_binding` смотрите в [документации провайдера]({{ tf-provider-resources-link }}/mdb_mysql_cluster_iam_binding).
...
- CLI {#cli}
  ...
  1. Посмотрите описание команды CLI для назначения ролей на кластер:
      ...
      ```bash
      {{ yc-mdb-my }} cluster set-access-bindings --help
      ```

  1. Назначьте роли, выполнив команду:

      ```bash
      {{ yc-mdb-my }} cluster set-access-bindings <имя_или_идентификатор_кластера> \
...
- REST API {#api}
  ...
  1. Воспользуйтесь методом [Cluster.SetAccessBindings](../api-ref/Cluster/setAccessBindings.md) и выполните запрос, например, с помощью {{ api-examples.rest.tool }}:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://{{ api-host-mdb }}/managed-mysql/v1/clusters/<идентификатор_кластера>:setAccessBindings' \
        --data '{
                  "accessBindings": [
                    {
                      "roleId": "<роль>",
      ...
      * `accessBindings.roleId` — назначаемая [роль](../security/index.md#roles-list), например `managed-mysql.editor`.
...
- CLI {#cli}
  ...
  1. Посмотрите описание команды CLI для отзыва роли на кластер:
      ...
      ```bash
      {{ yc-mdb-my }} cluster remove-access-binding --help
      ```
  1. Отзовите роль, выполнив команду:

      ```bash
      {{ yc-mdb-my }} cluster remove-access-binding <имя_или_идентификатор_кластера> \
        --role <роль> \
        --subject <тип_субъекта>:<идентификатор_субъекта>
      ```
...
- {{ TF }} {#tf}
  ...
  Для отзыва ролей на кластер {{ mmy-name }} используйте ресурс `yandex_mdb_mysql_cluster_iam_binding` с параметром `members`.
  ...
  1. Подтвердите изменение ресурсов.
     ...
     Подробнее о параметрах ресурса `yandex_mdb_mysql_cluster_iam_binding` посмотрите в [документации провайдера]({{ tf-provider-resources-link }}/mdb_mysql_cluster_iam_binding).
...
Чтобы сервисный аккаунт мог просматривать информацию обо всех кластерах {{ mmy-name }} в каталоге, но изменять ресурсы только конкретного кластера, выдайте ему роль `managed-mysql.viewer` на каталог и роль `managed-mysql.editor` на этот кластер:
...
- CLI {#cli}
  ...
  1. Назначьте роли на кластер:
      ...
      ```bash
      {{ yc-mdb-my }} cluster add-access-bindings <имя_или_идентификатор_кластера> \
        --access-binding role=managed-mysql.editor,subject=serviceAccount:<идентификатор_сервисного_аккаунта>
      ```
