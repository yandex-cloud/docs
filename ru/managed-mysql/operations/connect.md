...
Максимальное количество одновременных подключений к хосту кластера {{ mmy-name }} задается параметром `max_connections` и по умолчанию равно величине `<количество МБ RAM на одном хосте> / 32`, но не менее `100`.

Например, для хоста [класса s1.micro](../concepts/instance-types.md) с 8 ГБ RAM значение параметра `max_connections` по умолчанию равно `256`: `8 × 1024 / 32`.
