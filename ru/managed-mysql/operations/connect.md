...
Если к хосту нет публичного доступа, то к нему можно подключиться с виртуальной машины {{ compute-name }} без SSL-сертификата. 
...
Если вы подключаетесь к БД без публичного доступа с виртуальной машины {{ compute-name }}, то передайте флаг `--ssl-mode` со значением `DISABLED`:
