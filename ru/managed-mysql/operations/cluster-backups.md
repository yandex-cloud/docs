---
title: Управление резервными копиями {{ MY }}
description: Вы можете создавать резервные копии и восстанавливать кластеры из имеющихся резервных копий {{ MY }}. Технология Point-in-Time Recovery (PITR) позволяет восстановить состояние кластера на любой момент времени, начиная от создания резервной копии.
---

# Управление резервными копиями в {{ mmy-name }}

Вы можете создавать резервные копии и восстанавливать кластеры из имеющихся резервных копий, в том числе на указанный момент времени. Подробнее в разделе [Резервные копии](../concepts/backup.md).
...
- Консоль управления {#console}
  ...
  1. Нажмите на имя нужного кластера и выберите вкладку ![chevron-down](../../_assets/console-icons/chevron-down.svg) **{{ ui-key.yacloud.shared.layout.PageTabs.button_other_hnYwF }}** → **{{ ui-key.yacloud.mysql.cluster.switch_backups }}**.
...
- CLI {#cli}
  ...
  **Чтобы восстановить кластер из резервной копии:**
  ...
  1. Если вы планируете восстанавливать кластер из конкретной резервной копии, получите список доступных резервных копий кластеров {{ MY }}:
     ...
     Время завершения создания резервной копии указано в столбце `CREATED AT` списка доступных резервных копий в формате `yyyy-mm-dd hh:mm:ss` (`2020-08-10 12:00:00` в примере выше).

  1. Запросите создание кластера. Источник восстановления можно задать одним из способов:

     * Чтобы восстановить кластер из конкретной резервной копии, укажите ее идентификатор в параметре `--backup-id`:
       ...
       Параметр `time` можно не указывать, и тогда кластер будет восстановлен на момент создания выбранной резервной копии.

       {% endnote %}

     * Чтобы восстановить кластер на определенный момент времени по идентификатору исходного кластера, укажите параметры `--source-cluster-id` и `--time`:

       ```bash
       {{ yc-mdb-my }} cluster restore \
         --source-cluster-id=<идентификатор_исходного_кластера> \
         --time=<время> \
         ...
       ```

       В этом варианте параметр `--time` обязателен, а подходящая резервная копия будет выбрана автоматически.

     Полный пример команды для восстановления из конкретной резервной копии:
...
- {{ TF }} {#tf}
  ...
  * Кластера, созданного и удаленного через Консоль управления, CLI или API.
  ...
  Для восстановления может понадобиться идентификатор резервной копии. Получите список доступных резервных копий кластеров {{ MY }} [с помощью CLI](#list-backups):
  ...
  **Чтобы восстановить существующий кластер:**
  ...
  1. Добавьте в этот конфигурационный файл блок `restore`, используя один из вариантов:

      * Чтобы восстановить кластер из конкретной резервной копии, укажите ее идентификатор в параметре `backup_id`:
        ...
        В параметре `time` укажите момент времени в формате `yyyy-mm-ddThh:mm:ss`, на который нужно восстановить состояние кластера {{ MY }}.
      ...
      * Чтобы восстановить кластер на определенный момент времени по идентификатору исходного кластера, укажите параметры `source_cluster_id` и `time`:

        ```hcl
        resource "yandex_mdb_mysql_cluster" "<имя_кластера>" {
          ...
          restore {
            source_cluster_id = "<идентификатор_исходного_кластера>"
            time              = "<время>"
          }
        }
        ```
...
- REST API {#api}
  ...
  1. Создайте файл `body.json`.

      Для восстановления кластера в теле запроса нужно передать источник восстановления. Это можно сделать одним из способов:

      * Чтобы восстановить кластер из конкретной резервной копии, укажите ее идентификатор в параметре `backupId`:
      ...
      * Чтобы восстановить кластер на определенный момент времени по идентификатору исходного кластера, укажите параметры `sourceClusterId` и `time`:

        ```json
        {
            "sourceClusterId": "<идентификатор_исходного_кластера>",
            "time": "<время>",
            ...
        }
        ```
      ...
      Полный пример содержимого файла `body.json` для восстановления из конкретной резервной копии:
...
Минимальное значение приоритета использования хоста при создании резервных копий — `0`, максимальное — `100`, по умолчанию — `0`. Источником резервного копирования выбирается хост-реплика с наибольшим приоритетом. Подробнее в разделе [Создание резервной копии](../concepts/backup.md#size).
