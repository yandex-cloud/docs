...
Для каждого хоста в кластере {{ mmy-short-name }} можно указать [источник репликации](../concepts/replication.md#manual-source) и управлять [публичным доступом](../concepts/network.md#public-access-to-host) к хосту.
...
- CLI
  ...
  $ {{ yc-mdb-my }} host update <имя хоста>
       --cluster-name <имя кластера>
       --replication-source <имя хоста-источника репликации>
  ...
  В параметре `--host` для управления репликацией в кластере измените:
    - Источник репликации для хоста в опции `replication-source`, чтобы вручную управлять потоками репликации.
