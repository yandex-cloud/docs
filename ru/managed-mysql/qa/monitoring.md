...
Чтобы посмотреть графики в сервисе [{{ monitoring-full-name }}](https://monitoring.cloud.yandex.ru/), на главной странице выберите сервисный дашборд **{{ mmy-short-name }}**. Для просмотра отдельных метрик перейдите в раздел **Обзор метрик** и задайте параметру `service` значение **{{ mmy-short-name }}**.


#### Как настроить алерты {#alerts}

Настроить алерты можно можно в сервисе [{{ monitoring-full-name }}](https://monitoring.cloud.yandex.ru/): для этого воспользуйтесь инструкцией в разделе [{#T}](../../monitoring/operations/alert/create-alert.md).
