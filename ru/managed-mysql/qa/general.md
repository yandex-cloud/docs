...
Подробные инструкции см. в разделе [Как начать работать с {{ mmy-short-name }}<sup>®</sup>](../quickstart.md).
...
Для хранилища на локальных SSD-дисках количество хостов ограничено при создании: для {{ MY }}-кластеров можно создать не меньше 3 хостов.
