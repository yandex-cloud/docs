...
{{ mmy-short-name }} берет на себя трудоемкие задачи администрирования инфраструктуры {{ mmy-short-name }}:
...
{{ mmy-short-name }} доступен всем зарегистрированным пользователям Яндекс.Облака.
...
Для хранилища на NVMe-дисках количество хостов ограничено при создании: для {{ MY }}-кластеров можно создать не меньше 3 хостов.
...
{{ mmy-short-name }} поддерживает {{ MY }} 5.7.
