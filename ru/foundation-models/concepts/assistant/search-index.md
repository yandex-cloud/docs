...
При создании векторного индекса модель [эмбеддинга](../embeddings.md) создает из чанков векторы, которые хранят смысл текстового фрагмента. Как и в текстовом поиске, вы можете указать настроить размер чанка и размер перекрытия чанков в токенах. Учитывайте, что в векторном поиске токены, как правило, содержат больше одного символа.

Также вы можете переопределить модели эмбеддингов для запросов и ответов. По умолчанию используются модели эмбеддингов {{ foundation-models-name }}.
