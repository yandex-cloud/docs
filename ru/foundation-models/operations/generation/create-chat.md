...
Модели {{ gpt-lite }} и {{ gpt-pro }} не хранят контекст предшествующих сообщений, поэтому для реализации диалога с выбранной моделью историю сообщений нужно сохранять на стороне клиента и передавать в каждом обращении к модели. [Чат](../../concepts/generation/index.md), доступный в консоли управления, представляет собой серию промт-запросов, в которой в контекст каждого нового запроса добавляются ответы модели на предыдущие запросы. Модели генерации текста имеют [ограничения](../../concepts/limits.md) на длину контекста.
