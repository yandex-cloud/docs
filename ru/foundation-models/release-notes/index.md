...
1. С 1 декабря 2024 года модель {{ yagpt-name }} в версии для тестирования (ветка `RC`) станет основной версией (ветка `Latest`), а текущая версия модели останется доступной в ветке `Deprecated` в соответствии с [жизненным циклом](../concepts/yandexgpt/models.md#model-lifecycle) моделей.
