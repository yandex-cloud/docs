...
В течение 2024 года зона `{{ region-id }}-с` будет [выведена из эксплуатации](./ru-central1-c-deprecation.md). Ресурсы из нее можно переместить в новую зону доступности — `{{ region-id }}-d`.
...
Инструмент для самостоятельной миграции инсталляций {{ mgl-name }}, размещенных в зоне `{{ region-id }}-c`, планируется разработать до конца января 2024 года. Мы дополнительно оповестим пользователей, использующих {{ mgl-name }} в зоне `{{ region-id }}-c`, как только в интерфейсе появится возможность мигрировать их ресурсы из зоны `{{ region-id }}-c`.
