...
Нет, переключиться обратно на пробную версию нельзя, но грант не пропадет. Стартовый грант будет расходоваться в первую очередь. Подробнее о том, в какой последовательности расходуются средства, см. на странице [{#T}](../billing/payment/billing-cycle.md).
...
Платформа {{ yandex-cloud }} соответствует требованиям PCI DSS, поэтому использовать облачные сервисы для обработки платежей безопасно. Подробнее о сертификации PCI DSS см. в разделе [{#T}](../overview/security/conform.md#pci-dss). 
...
Да, организации и ИП могут заключить договор с двусторонним подписанием. Для этого необходимо отправить заявку через форму [Cообщение]({{ link-console-support }}). С вами свяжется менеджер {{ yandex-cloud }} для обсуждения условий подписания.
