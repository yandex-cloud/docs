# Меры безопасности на стороне Яндекс.Облака 
...
Служба безопасности Яндекс.Облака отвечает за мониторинг и соблюдение этих процессов. Специалисты службы постоянно совершенствуют СУИБ, проводят внутренние и внешние аудиты, ежегодно оценивают риски и находят способы их снизить. 

## Инвентаризация активов {#asset-inv}

В Яндекс.Облаке выстроен процесс инвентаризации активов, включая учет систем, обрабатывающих данные клиентов. Правила использования информации и активов, связанных с обработкой информации, записаны во внутренних документах и регулярно объясняются сотрудникам. При увольнении сотрудники возвращают все корпоративные активы. Доступы к системам отзываются автоматически. 
...
- Данные клиентов Яндекс.Облака, сохраненные на дисках, обязательно шифруются.
...
## Управление персоналом Яндекс.Облака {#employees}

Компания проводит мероприятия для сотрудников Яндекс.Облака, чтобы минимизировать риски в области ИБ, связанные с действиями сотрудников.
...
Изоляция административных и пользовательских ресурсов в Яндекс.Облаке происходит следующим образом: 
...
- Логическая изоляция с помощью сервиса {{ iam-name }} ({{ iam-short-name }}). Все административные операции проводятся через {{ iam-short-name }}. Для их выполнения нужны специальные права, недоступные пользователям Яндекс.Облака.  
...
- На уровне сети используются несколько типов межсетевых экранов: 
    ...
    - аппаратный межсетевой экран на границе инфраструктуры Яндекса и инфраструктуры Яндекс.Облака; 
...
Владельцем данных всегда является пользователь облачной платформы. Яндекс.Облако использует информацию клиента, размещенную на платформе, только для выполнения целей договора и уведомляет клиента об инцидентах, затрагивающих пользовательские данные. 
...
- Шифрование на уровне баз данных {{ ydb-full-name }} ({{ ydb-short-name }}) 
...
- При автоматической блокировке облака. По истечении 60 дней (при блокировке облака из-за нарушения условий использования — 7 дней) Яндекс.Облако может пометить ресурсы как удаляемые. 

- При удалении облака пользователя. Если облако не было восстановлено в течение 30 дней после удаления, ресурсы во всех сервисах помечаются как удаляемые. При разрыве контракта все облака пользователя и ресурсы в них немедленно помечаются как удаляемые. 
