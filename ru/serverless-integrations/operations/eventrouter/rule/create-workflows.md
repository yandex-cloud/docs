---
title: Как создать правило с приемником {{ sw-full-name }}
description: Следуя данной инструкции, вы сможете создать правило с приемником {{ sw-full-name }}.
---

# Создать правило с приемником {{ sw-full-name }}
...
- CLI {#cli}
  ...
  1. Создайте правило с приемником {{ sw-full-name }}:
      ...
      yc serverless eventrouter rule create \
        ...
        --filter '<jq-шаблон>' \
        --workflow-target \
      workflow-id=<идентификатор_рабочего_процесса>,\
      service-account-id=<идентификатор_сервисного_аккаунта_процесса>,\
...
- {{ TF }} {#tf}
  ...
  Чтобы создать [правило](../../../concepts/eventrouter/rule.md) с приемником {{ sw-full-name }}:
