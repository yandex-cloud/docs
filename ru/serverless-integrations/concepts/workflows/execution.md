...
* С помощью консоли управления, CLI, API. Подробнее в [{#T}](../../operations/workflows/execution/start.md).
* По таймеру. Расписание задается в формате [cron-выражения](../cron.md). Подробнее в [{#T}](../../operations/workflows/execution/start-by-timer.md).
* По ссылке через HTTPS. Подробнее в [{#T}](../../operations/workflows/execution/start-by-link.md).
* С помощью API-шлюза. Подробнее в [{#T}](../../../api-gateway/operations/spec-constructor/workflows.md).
* С помощью {{ er-full-name }}. Для этого рабочий процесс нужно указать в качестве приемника в [правиле](../eventrouter/rule.md). Подробнее о том, как создать правило, в [{#T}](../../operations/eventrouter/rule/create-workflows.md). Правило определяет события, при которых должен запуститься рабочий процесс. Тело события передается в запуск как входной параметр.

Если рабочий процесс:
* приватный, его может запустить только пользователь, у которого есть права на запуск рабочего процесса, например роль [serverless.workflows.executor](../../security/workflows.md#serverless-workflows-executor).
* публичный, его может запустить любой пользователь. Подробнее в [{#T}](../../operations/workflows/workflow/make-public.md).
...
`STEP_DATA_LIMIT_EXCEEDED` | Превышен лимит входных или выходных данных. Подробнее в [{#T}](../limits.md).
`STEP_NO_CHOICE_MATCHED` | Нет подходящего пути выполнения в `choices`. Подробнее в [{#T}](yawl/management/switch.md).
`STEP_PERMISSION_DENIED` | Нет доступа к ресурсу.
`STEP_TIMEOUT` | Превышен таймаут шага. Подробнее в [{#T}](../limits.md).
...
`STEP_FAIL` | Запуск завершился с ошибкой на шаге `Fail`. Подробнее в [{#T}](yawl/management/fail.md).
...
`FUNCTION_CALL_INVALID_RESPONSE` | Невалидный код функции или формат возвращаемого JSON-ответа. Подробнее в [{#T}](../../../functions/concepts/function-invoke.md#http-state).
