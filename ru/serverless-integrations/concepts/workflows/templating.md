...
* `not_var{{ .field }}` — поддерживается в интерфейсе {{ foundation-models-full-name }}.

Для составления jq-шаблонов можно использовать [JSON-схему](https://raw.githubusercontent.com/yandex-cloud/json-schema-store/refs/heads/master/serverless/workflows/yawl.json) рабочего процесса. В интерфейсе {{ foundation-models-full-name }} в полях, поддерживающих шаблонизацию, доступно отображение JSON-схемы в виде интерактивного дерева. Это позволяет не составлять jq-шаблон вручную.
