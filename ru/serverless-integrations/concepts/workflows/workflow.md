...
* указать сервисный аккаунт, который будет использоваться для получения доступа к закрытым ресурсам, например потокам данных {{ yds-full-name }}.
* включить опцию **{{ ui-key.yacloud.serverless-workflows.label_public-access }}**. Тогда рабочий процесс сможет запустить любой пользователь. Подробнее см. [{#T}](execution.md).
