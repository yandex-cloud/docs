...
{{ objstorage-name }} позволяет управлять [конфигурацией CORS](../../../concepts/lifecycles.md) бакета. Для загрузки конфигурации CORS в {{ objstorage-name }} необходимо сформировать XML-документ, описанный в этом разделе. Скачивая существующую конфигурацию, вы получаете документ такого же формата.
