...
При получении запроса [!KEYREF objstorage-name]:
...
При завершении загрузки клиент должен предоставить список частей, которые он отправлял. Описание каждой части должно содержать `ETag`, который клиент получает в ответ на каждую загруженную часть. Смотрите раздел [[!TITLE]](uploadpart.md).
...
`ETag` | Идентификатор, который клиент получил от [!KEYREF objstorage-name] в ответ на загрузку части.<br/><br/>Путь: `/CompleteMultipartUpload/Part/ETag`.
...
<CompleteMultipartUploadResult xmlns="http://s3.amazonaws.com/doc/2006-03-01/">
  <Location>http://Example-Bucket.storage.cloud.yandex.net/Example-Object</Location>
...
`Bucket` | Имя корзины, в которой находится объект.<br/><br/>Путь: `/CompleteMultipartUploadResult/Bucket`.
