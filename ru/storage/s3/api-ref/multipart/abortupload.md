...
Прерывает загрузку и удаляет все уже сохраненные части объекта из [!KEYREF objstorage-name]. Если запрос за на прерывание загрузки поступил в процессе загрузки какой-либо из частей, то результат не гарантирован.
