...
Фильтрует и возвращает содержимое объекта {{ objstorage-name }} на основе SQL-запроса.

{% note warning %}

Для выполнения запроса вы должны иметь разрешение на действия `s3:GetObject` и `s3:ObjectsSelect`. Подробнее читайте в разделе [{#T}](../../../concepts/policy.md).
