...
Если объекта не существует, то [!KEYREF objstorage-name] в ответе отметит его как удаленный.

Ответ можно настроить так, что [!KEYREF objstorage-name] выдаст одну из выборок:
...
`Quiet` | `<Quiet>true</Quiet>` устанавливает <q>тихий</q> режим.<br/><br/>[!KEYREF objstorage-name] запишет в ответ только ошибки удаления. Если ошибок нет, тела ответа не будет.<br/><br/>Если не указан, то значение по умолчанию — `false`.<br/><br/>Путь: `/Delete/Quiet`.
