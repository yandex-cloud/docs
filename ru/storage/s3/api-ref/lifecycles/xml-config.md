...
Дополнительно может содержать `ExpiredObjectDeleteMarker` — маркер удаления объекта с истекшим сроком действия, который указывает, удалит ли {{ objstorage-name }} маркер удаления при отсутствии неактивных версий.
