...
`s3:if-match` | Проверяет наличие заголовка `If-Match` в запросе для [условной записи](../../../concepts/object.md#conditional-writes) объекта. Используется для обязательной проверки ETag при перезаписи существующего объекта.
`s3:if-none-match` | Проверяет наличие заголовка `If-None-Match` в запросе для [условной записи](../../../concepts/object.md#conditional-writes) объекта. Используется для обязательной проверки отсутствия объекта при создании нового.
...
`yc:access-key-id` | Сравнивает идентификатор [статического ключа доступа](../../../../iam/concepts/authorization/access-key.md) с заданным в политике.<br/><br/>Например, таким образом можно разграничить доступ в разные бакеты или префиксы в одном бакете с помощью разных статических ключей, выпущенных на один [сервисный аккаунт](../../../../iam/concepts/users/service-accounts.md).
`yc:originip` | Сравнивает исходный IP-адрес, с которого пришел запрос, с заданным в политике.<br/><br/>В отличие от условия `aws:sourceip`, _не учитываются_ IP-адреса обратных прокси-серверов, например, переданных в заголовке `X-Forwarded-For`.<br/><br/>Используйте условие `yc:originip` для обеспечения более высокого уровня безопасности доступа.
