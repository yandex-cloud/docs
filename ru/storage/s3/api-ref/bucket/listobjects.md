...
`StorageClass` | [Класс хранилища](../../../concepts/storage-class.md) объекта: {% if product == "yandex-cloud" %}`STANDARD`, `COLD` или `ICE`{% endif %}{% if product == "cloud-il" %}`STANDARD` или `COLD`{% endif %}.<br/><br/>Путь: `/ListBucketResult/Contents/StorageClass`.
