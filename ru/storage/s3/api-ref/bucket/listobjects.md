...
Чтобы получить все элементы выдачи, если их больше `max-keys`, необходимо выполнить несколько последовательных запросов к {{ objstorage-name }} с параметром `continuation-token`, где для каждого запроса `continuation-token` равен значению элемента `NextContinuationToken` из предыдущего ответа. ||
...
|| `StorageClass` | [Класс хранилища](../../../concepts/storage-class.md) объекта: `STANDARD`, `COLD`, `ICE` или `INTELLIGENT_TIERING`.
...
В результирующей выдаче {{ objstorage-name }} оставит ключи, начиная со следующего за `marker`. ||
|| `max-keys` | Максимальное количество элементов в ответе.
...
Чтобы получить все элементы выдачи, если их больше `max-keys`, необходимо выполнить несколько последовательных запросов к {{ objstorage-name }} с параметром `marker`, где для каждого запроса `marker` равен значению элемента `NextMarker` из предыдущего ответа. ||
|| `prefix` | Строка, с которой должен начинаться ключ.
