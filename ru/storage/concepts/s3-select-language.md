...
Язык S3 Select использует только оператор `SELECT` из стандартного SQL. Поддерживаются следующие стандартные ANSI предложения:
...
S3 Select не поддерживает вложенные запросы или соединения.
...
Первая форма возвращает каждую строку, удовлетворяющую условию в предложении `WHERE`, как есть. Вторая форма возвращает строку с пользовательской проекцией выходных скалярных выражений для каждого столбца.
...
Как и в стандартном SQL, предложение `FROM` создает строки, которые фильтруются в предложении `WHERE` и возвращаются в виде списка данных, указанных в `SELECT`.
