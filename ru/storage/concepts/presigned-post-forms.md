---
title: Загрузка файла с помощью HTML формы
description: Из статьи вы узнаете про загрузку файла с помощью HTML формы.
__system:
  dislikeVariants:
...
Через форму нельзя загрузить объекты размером более 5 ГБ (подробнее на странице [{#T}](limits.md)).
...
Поле `expiration` содержит срок действия политики в формате [ISO8601](https://ru.wikipedia.org/wiki/ISO_8601), например, `2019-07-22T15:39:36Z`. По истечении срока действия политики {{ objstorage-name }} перестает принимать файлы, загружаемые с помощью формы.
