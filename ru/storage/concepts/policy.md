...
: Бакет (например, `samplebucket`), объект в бакете (`samplebucket/some/key`) или префикс (`samplebucket/some/path/*`), в том числе пустой префикс для обозначения всех объектов в бакете (`samplebucket/*`). В правиле можно указать несколько ресурсов.
  ...
  Если вы описываете политику в формате JSON, у ресурса должен быть префикс `arn:aws:s3:::`, например `arn:aws:s3:::samplebucket`.
...
* Правило, которое разрешает анонимному пользователю чтение объектов бакета `samplebucket` по зашифрованному подключению:
