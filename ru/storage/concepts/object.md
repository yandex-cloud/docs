...
Для бакета с ограниченным доступом сервисом генерируется подписанная ссылка, которая позволяет временно скачать объект даже из бакета с ограниченным доступом. [Подробнее про подписанные ссылки, их генерацию и использование](../../concepts/pre-signed-urls).
