...
Требования к ключу:
* кодировка UTF-8;
* размер меньше 1024 байт.

Используемые символы:

#|
|| **Категория** | **Перечень** | **Комментарий** ||
|| Оптимальные | `[a-zA-Z0-9]`, `!`, `-`, `_`, `.`, `*`, `'`, `(`, `)` | Эти символы, как правило, безопасно использовать. ||
|| Может потребоваться специальная обработка | `&`, `$`, `@`,`=`, `;`, `:`, `/`, `+`, `,`, `?`, пробелы и [управляющие ASCII-символы](https://ru.wikipedia.org/wiki/Управляющие_символы) с кодами `0`-`31` и `127` | Эти символы должны быть закодированы для URL или указаны в шестнадцатеричном виде (HEX). Некоторые из них являются непечатными, и браузер может их не поддерживать, поэтому они также требуют специальной обработки. ||
...
Подробнее в инструкции [{#T}](../operations/objects/object-meta.md).
...
Условия можно использовать при загрузке и составной загрузке объектов, копировании объектов целиком или по частям. Подробнее в инструкции [Условная загрузка объекта](../operations/objects/upload.md#conditional-writes).
