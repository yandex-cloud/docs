...
С помощью ACL можно выдать разрешения пользователю Яндекс.Облака, сервисному аккаунту и системной группе, при этом необходимо знать [идентификатор получателя разрешений](#accounts-ids). При выдаче разрешений вы можете использовать [предопределенные ACL](#predefined-acls), которые содержат популярные наборы доступов.
...
ACL | Описание
----|---------
`private`<br/>`bucket-owner-full-control` | Пользователи Яндекс.Облака получают разрешения в соответствии со своими ролями в IAM.
...
Включает в себя всех аутентифицированных пользователей Яндекс.Облака: как из вашего облака, так и из облаков других пользователей.
...
## Наследование прав доступа на бакет системными группами Яндекс.Облака {#inheritance}
...
- `allAuthenticatedUsers` — доступ к бакету получат все аутентифицированные пользователи Яндекс.Облака: как из вашего облака, так и из облаков других пользователей.
