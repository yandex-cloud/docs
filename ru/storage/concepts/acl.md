...
В Облаке используются два независимых друг от друга механизма управления доступами к ресурсам [!KEYREF objstorage-name]:

- [!KEYREF iam-name] ([Документация IAM](../../iam/concepts/index.md)).
- ACL [!KEYREF objstorage-name].

[!KEYREF objstorage-name] создает ACL для всех объектов и бакетов. По умолчанию, ACL пустой. Пользователь, обладающий соответствующими правами в IAM, может загрузить ACL для ресурсов [!KEYREF objstorage-name]. ACL содержит список пользователей, у которых есть доступ к объектам и бакетам, а также разрешения для этих пользователей. Разрешения и пользователи в IAM и ACL могут различаться. О том, как [!KEYREF objstorage-name] проверяет доступ к ресурсам, читайте в разделе [Проверка разрешений](#permissions-check).

Выдать разрешение можно для пользователя Облака, сервисного аккаунта и для предопределенной группы.
...
- В разделе [IAM Консоли Управления](https://console.cloud.yandex.ru/iam).
...
Чтобы выдать разрешение для предопределенной группы, необходимо знать ее URI. Подробнее смотрите в разделе [Предопределенные группы](#predefined_groups).

[!KEYREF objstorage-name] поддерживает [предопределенные ACL](#predefined_acls), которые содержат популярные наборы доступов.

Структуру ACL смотрите в разделе [XML-схема ACL](../s3/api-ref/acl/xmlscheme.md).

[!KEYREF objstorage-name] поддерживает операции загрузки и скачивания ACL, операции удаления ACL нет. Чтобы удалить все доступы, загрузите пустой ACL. По умолчанию, для каждого нового объекта или бакета [!KEYREF objstorage-name] создает пустой ACL.

> [!NOTE]
>
>ACL, загруженный для объектов, применяется мгновенно. ACL, загруженный для бакетов и доступы, измененные в IAM, применяются с задержкой. Продробнее про задержки читайте в [документации IAM](../../iam/concepts/access-control.md).


## Проверка разрешений {#permissions-check}

В Облаке используются два механизма управления доступами к ресурсам [!KEYREF objstorage-name]:

- [!KEYREF iam-name]
- ACL

При получении запроса к бакету или объекту, [!KEYREF objstorage-name] проверяет доступы с помощью обоих механизмов. Если необходимый доступ открыт хотя бы одним из способов, [!KEYREF objstorage-name] выполняет запрос.
...
Разрешения соответствуют ролям пользователей в [!KEYREF iam-name].
...
> [!NOTE]
> 
> Если при оформлении ACL указать доступ `WRITE`, но при этом не указать `READ`, то [!KEYREF objstorage-name] ответит с кодом `501 Not Implemented`.
...
## Предопределенные группы {#predefined_groups}
