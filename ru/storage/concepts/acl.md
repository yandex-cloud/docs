...
`WRITE` | `editor` | Для бакета: запись, перезапись и удаление объектов в бакете.<br/>Используется обязательно вместе с `READ`, отдельно указать разрешение `WRITE` нельзя.<br/><br/>Для объекта: разрешение не имеет смысла, при записи объекта проверяются разрешения для бакета.
