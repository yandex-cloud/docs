...
Чтобы выдать разрешение для системной группы, необходимо знать ее URI. Подробнее смотрите в разделе [Системные группы](#system-groups).

{{ objstorage-name }} поддерживает [предопределенные ACL](#predefined-acls), которые содержат популярные наборы доступов.
