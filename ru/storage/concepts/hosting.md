...
К бакету, имя которого содержит точку, нельзя обратиться по протоколу HTTPS. Для настройки HTTPS, [загрузите собственные сертификаты безопасности](../operations/hosting/certificate.md) в {{ objstorage-name }}.
