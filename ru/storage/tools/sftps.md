...
1. В папке `secrets`:
   ...
   * Если вы будете использовать SFTP, создайте файл `authorized_keys` с открытым {% if lang == "ru" and audience != internal %}[SSH-ключом](../../glossary/ssh-keygen.md){% else %}SSH-ключом{% endif %}:
