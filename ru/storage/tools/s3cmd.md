...
- По умолчанию S3cmd может отправлять дополнительный заголовок `x-amz-meta-s3cmd-attrs` с атрибутами вашего файла (mode, ownership, timestamps) при загрузке объекта. Отключить отправку атрибутов можно с помощью параметра `preserve_attrs` в конфигурационном файле `~/.s3cfg` или ключа `--no-preserve`.
