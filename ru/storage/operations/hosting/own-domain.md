...
1. Создайте на своем [DNS-сервере](../../../glossary/dns.md#dns-server), например в [{{ dns-full-name }}](../../../dns/operations/resource-record-create.md), ресурсную запись, которая привязывает ваше доменное имя к бакету. В {{ dns-name }} рекомендуется использовать [записи ANAME](../../../dns/concepts/resource-record.md#aname): 
