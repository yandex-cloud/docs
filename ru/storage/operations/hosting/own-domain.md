...
Чтобы поддержать для сайта протокол HTTPS, [загрузите сертификат безопасности](certificate.md) в {{ objstorage-name }}. Иначе сайт доступен только по протоколу HTTP. 
