---
title: Настроить и отключить шифрование по умолчанию в бакете {{ objstorage-full-name }}
description: Следуя данной инструкции, вы сможете настроить и отключить шифрование по умолчанию для новых объектов в бакете.
---

# Настроить и отключить шифрование по умолчанию в бакете


В {{ objstorage-short-name }} есть возможность шифровать объекты в бакете с помощью [ключей {{ kms-short-name }}](../../../kms/concepts/key.md):
* [Настройте шифрование по умолчанию](#add), чтобы все новые объекты шифровались указанным ключом.
* Указывайте ключ шифрования при [загрузке объекта через API](../../s3/api-ref/object/upload.md).

В этой инструкции описано, как настроить шифрование по умолчанию для новых объектов и [отключить его](#del). Изменение настройки не влияет на уже загруженные объекты.
...
## Настроить шифрование по умолчанию {#add}
...
## Отключить шифрование по умолчанию {#del}

После отключения новые объекты не будут автоматически шифроваться ключом из настроек бакета. Уже зашифрованные объекты останутся зашифрованными: сохраните ключ {{ kms-short-name }}, чтобы иметь возможность их расшифровать.

{% list tabs group=instructions %}

- Консоль управления {#console}

  Чтобы отключить шифрование по умолчанию, уберите выбор ключа {{ kms-short-name }} в настройках бакета:
  1. В [консоли управления]({{ link-console-main }}) выберите каталог.
  1. [Перейдите]({{ link-console-main }}/link/storage) в сервис **{{ ui-key.yacloud.iam.folder.dashboard.label_storage }}**.
  1. Выберите бакет, для которого хотите отключить шифрование по умолчанию.
  1. Перейдите на вкладку **{{ ui-key.yacloud.storage.bucket.switch_security }}**, затем на вкладку **{{ ui-key.yacloud.storage.bucket.switch_encryption }}**.
