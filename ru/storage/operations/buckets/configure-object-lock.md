---
title: Включить и выключить блокировку версий объектов (object lock)
description: Следуя данной инструкции, вы сможете включить и выключить возможность блокировок версий объектов, а также настроить блокировки по умолчанию в бакете {{ objstorage-name }}.
---
...
- {{ TF }} {#tf}
  ...
  1. Откройте файл конфигурации {{ TF }} и добавьте к описанию бакета блок `object_lock_configuration`:
      ...
      * `object_lock_configuration` — настройки блокировки версий объектов:
      ...
      Более подробная информация о параметрах бакета, которые вы можете задать с помощью {{ TF }}, приведена в [документации провайдера]({{ tf-provider-resources-link }}/storage_bucket).
...
- API {#api}
  ...
  Воспользуйтесь методом S3 API [putObjectLockConfiguration](../../s3/api-ref/bucket/putobjectlockconfiguration.md), методом REST API [update](../../api-ref/Bucket/update.md) для ресурса [Bucket](../../api-ref/Bucket/index.md) или вызовом gRPC API [BucketService/Update](../../api-ref/grpc/Bucket/update.md).
...
- AWS CLI {#cli}
  ...
  1. Опишите конфигурацию блокировок по умолчанию в формате JSON:
     ...
     * `ObjectLockEnabled` — статус механизма блокировок: `Enabled` — механизм включен.
       ...
       Это обязательное поле. Если не указать его со значением `Enabled`, появится сообщение об ошибке `InvalidRequest`, а механизм блокировок не включится. Смотрите также [Выключить возможность блокировок](#disable).
