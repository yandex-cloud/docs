...
- {{ yandex-cloud }} CLI {#cli}
  ...
  1. Опишите конфигурацию жизненных циклов объектов в формате JSON. Например:
      ...
      * `filter` — фильтр объектов. Необязательный параметр. Может содержать не более одного элемента каждого типа:
          ...
          * `objectSizeGreaterThan` — минимальный размер объекта в байтах. Под действие правила попадают объекты, размер которых больше указанного. Необязательный параметр.
          * `objectSizeLessThan` — максимальный размер объекта в байтах. Под действие правила попадают объекты, размер которых меньше указанного. Необязательный параметр.
      ...
      * `noncurrent_transitions` — параметр правила для изменения класса хранилища неактивных версий объектов со стандартного (`STANDARD`) на холодное (`COLD`, `STANDARD_IA`) , ледяное (`ICE`) или умное (`INTELLIGENT_TIERING`) и с холодного на ледяное. Необязательный параметр. Может содержать:
          * `noncurrent_days` — количество дней, через которое нетекущая версия будет перенесена в другой класс хранилища. Минимальное значение — `1`. Обязательный параметр.
          * `storage_class` — класс хранилища, в который будет перенесен объект. Может быть `COLD`, `STANDARD_IA`, `ICE` или `INTELLIGENT_TIERING`. Обязательный параметр.
          * `newer_noncurrent_versions` — количество последних нетекущих версий объекта, которые хранятся в исходном классе хранилища неограниченное время. Версии сверх этого количества переносятся в другой класс хранилища с учетом срока, заданного в `noncurrent_days`. Необязательный параметр.
      * `noncurrent_expiration` — параметр правила для удаления неактивных версий объектов. Необязательный параметр. Может содержать:
          * `noncurrent_days` — количество дней, через которое нетекущая версия будет удалена. Минимальное значение — `1`. Обязательный параметр.
          * `newer_noncurrent_versions` — количество последних нетекущих версий объекта, которые хранятся неограниченное время. Версии сверх этого количества удаляются с учетом срока, заданного в `noncurrent_days`. Необязательный параметр.
      ...
      * `noncurrent_delete_markers` — параметр правила для удаления [неактивных маркеров удаления](*noncurrent-delete-markers). Необязательный параметр. Может содержать:
          * `noncurrent_days` — действие правила вступает в силу спустя заданное в этом параметре количество дней с момента пометки версии объекта маркером удаления. Минимальное значение — `0`. При значении `0` будут удалены все неактивные маркеры удаления, существующие в бакете на данный момент. Обязательный параметр.
...
- AWS CLI {#aws-cli}
  ...
  1. Опишите конфигурацию жизненных циклов объектов в формате JSON. Например:
      ...
      * `NoncurrentVersionTransitions` — параметр правила для изменения класса хранилища неактивных версий объектов со стандартного (`STANDARD`) на холодное (`COLD`, `STANDARD_IA`) , ледяное (`ICE`) или умное (`INTELLIGENT_TIERING`) и с холодного на ледяное. Необязательный параметр. Может содержать:
          * `NoncurrentDays` — количество дней, через которое нетекущая версия будет перенесена в другой класс хранилища. Минимальное значение — `1`. Обязательный параметр.
          * `StorageClass` — класс хранилища, в который будет перенесен объект. Может быть `COLD`, `STANDARD_IA`, `ICE` или `INTELLIGENT_TIERING`. Обязательный параметр.
          * `NewerNoncurrentVersions` — количество последних нетекущих версий объекта, которые хранятся в исходном классе хранилища неограниченное время. Версии сверх этого количества переносятся в другой класс хранилища с учетом срока, заданного в `NoncurrentDays`. Необязательный параметр.
      ...
      * `NoncurrentVersionExpiration` — параметр правила для удаления неактивных версий объектов. Необязательный параметр. Может содержать:
          * `NoncurrentDays` — количество дней, через которое нетекущая версия будет удалена. Минимальное значение — `1`. Обязательный параметр.
          * `NewerNoncurrentVersions` — количество последних нетекущих версий объекта, которые хранятся неограниченное время. Версии сверх этого количества удаляются с учетом срока, заданного в `NoncurrentDays`. Необязательный параметр.
...
- {{ TF }} {#tf}
  ...
  1. Опишите в конфигурационном файле параметры ресурсов, которые необходимо создать:
     ...
     * `filter` — фильтр объектов. Необязательный параметр. Может содержать не более одного элемента каждого типа:
         ...
         * `object_size_greater_than` — минимальный размер объекта в байтах. Под действие правила попадают объекты, размер которых больше указанного. Необязательный параметр.
         * `object_size_less_than` — максимальный размер объекта в байтах. Под действие правила попадают объекты, размер которых меньше указанного. Необязательный параметр.
         * `tag` — [метка](../../concepts/tags.md#object-tags) объекта. Необязательный параметр. Под действие правила попадают объекты, которым присвоена указанная метка. Передается в виде записи, содержащей две пары значений, например:
     ...
     * `days` — количество дней, через которое нетекущая версия будет удалена. Минимальное значение — 1. Обязательный параметр.
     * `newer_noncurrent_versions` — количество последних нетекущих версий объекта, которые хранятся неограниченное время. Версии сверх этого количества удаляются с учетом срока, заданного в `days`. Необязательный параметр.

     Параметры `noncurrent_version_transition`:
     * `days` — количество дней, через которое нетекущая версия будет перенесена в другой класс хранилища. Минимальное значение — 1. Обязательный параметр.
     * `storage_class` — класс хранилища, в который будет перенесен объект. Может быть `COLD`, `STANDARD_IA`, `ICE` или `INTELLIGENT_TIERING`. Обязательный параметр.
     * `newer_noncurrent_versions` — количество последних нетекущих версий объекта, которые хранятся в исходном классе хранилища неограниченное время. Версии сверх этого количества переносятся в другой класс хранилища с учетом срока, заданного в `days`. Необязательный параметр.

     Более подробная информация о ресурсах, которые вы можете создать с помощью {{ TF }}, приведена в [документации провайдера]({{ tf-provider-link }}).
