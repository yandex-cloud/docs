...
- {{ yandex-cloud }} CLI
  ...
  Вы можете применить к бакету [предопределенный ACL](../../concepts/acl.md#predefined-acls) или настроить разрешения для отдельных пользователей, {% if audience == "external" %}[сервисных аккаунтов](../../../iam/concepts/users/service-accounts.md){% else %}сервисных аккаунтов{% endif %} и [системных групп](../../concepts/acl.md#system-groups) (группа всех пользователей интернета, группа всех аутентифицированных пользователей {{ yandex-cloud }}). Эти настройки несовместимы: у бакета должен быть либо предопределенный ACL, либо набор отдельных разрешений.
  ...
  : 1. Чтобы выдать разрешения с помощью ACL для пользователя {{ yandex-cloud }} или сервисного аккаунта, получите его идентификатор. {% if audience == "external" %}Подробнее см. в инструкциях [{#T}](../../../iam/operations/users/get.md) и [{#T}](../../../iam/operations/sa/get-id.md).{% endif %}
    1. Выполните команду:
