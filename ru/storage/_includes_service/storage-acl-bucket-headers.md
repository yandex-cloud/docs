...
`x-amz-grant-write` | Устанавливает получателю доступа разрешение на запись объектов. Используйте обязательно вместе с `x-amz-grant-read`, иначе [!KEYREF objstorage-name] ответит с кодом `501 Not Implemented`.
...
- `uri` — получатель доступа — предопределенная группа.
