...
API Object Storage частично совместим с API AWS S3 и вы можете использовать [инструменты](tools/index.md), предназначенные для работы с {% if lang == "ru" and audience != "internal" %}[S3](../glossary/s3.md){% else %}S3{% endif %}.
...
1. Выберите [класс хранилища](concepts/storage-class.md), который будет использоваться по умолчанию при загрузке объектов:
    ...
    * _Холодное хранилище_ предназначено для длительного хранения объектов с редкими запросами на чтение.
    {% if audience != "internal" %}* _Ледяное хранилище_ предназначено для хранения объектов от года с очень редкими запросами на чтение.{% endif %}
