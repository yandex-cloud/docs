...
При использовании умного хранилища стоимость хранения зависит не только от объема данных, но и от уровня доступа, на котором они находятся. Уровень доступа выбирается автоматически в зависимости от частоты обращений к объекту. Подробнее [об уровнях доступа](./concepts/storage-class.md#features-intelligent-tiering).
...
Оплачивается фактическое количество операций. При этом интерфейс выполнения операции не имеет значения. Например, стоимость будет одинаковой для скачивания объекта с помощью AWS S3 API, {{ yandex-cloud }} CLI или через консоль управления. Удаление данных (операция DELETE) не тарифицируется.
