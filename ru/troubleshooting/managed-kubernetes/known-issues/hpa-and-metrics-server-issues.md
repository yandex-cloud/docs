...
* В выводе информации о состоянии HPA (`kubectl describe hpa`) в кластере {{ managed-k8s-name }} отображаются сообщения следующего вида:
...
1. Вручную перенесите под с сервером метрик на другую, менее загруженную ноду кластера {{ managed-k8s-name }}.
2. Если это не поможет, измените конфигурацию пода с сервером метрик [по этой инструкции](../../../managed-kubernetes/tutorials/metrics-server.md).
...
2. Идентификатор проблемного пода кластера {{ managed-k8s-name }}.
3. Вывод команды `kubectl describe hpa` для проблемного кластера {{ managed-k8s-name }}.
