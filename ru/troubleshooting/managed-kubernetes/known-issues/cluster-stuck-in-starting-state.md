...
* При попытке создать или запустить существующий кластер {{ managed-k8s-name }} он переходит в состояние `STARTING`, но даже спустя длительное время не переходит в состояние `RUNNING`;
