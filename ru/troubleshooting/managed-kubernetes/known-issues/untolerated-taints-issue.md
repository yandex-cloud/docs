...
Taint – это пометка узла, которая указывает на то, что определенные поды не могут быть запущены на этом узле, если они не соответствуют определенным критериям. Если узлу назначена пометка, и некоторые поды не соответствуют этой пометке, то эти поды не будут запущены,
