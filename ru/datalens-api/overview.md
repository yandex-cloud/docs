...
1. {{ datalens-short-name }} возвращает идентификатор подключения `connection_id` и токен авторизации OAuth-токен. Подробнее об OAuth-токене в документации сервиса [{{ iam-name }}](../iam/concepts/authorization/oauth-token.md).
1. Пользователь передает полученную информацию партнеру и делает запрос на создание клиента для отправки данных.
1. Партнер создает клиента и получает IAM-токен в обмен на пользовательский OAuth-токен.
...
_Ключ партиционирования_ — это произвольное выражение из столбцов таблицы. Например, `["FieldName1","FieldName2","FieldName3"]`.
