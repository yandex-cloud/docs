...
Метод `List` позволяет фильтровать результат по определенным признакам. Например, можно получить список только тех ресурсов, у которых значение поля `name` равно
<q>test</q>.
