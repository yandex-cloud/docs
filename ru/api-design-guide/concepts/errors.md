...
400 | 9 | FAILED_PRECONDITION | Операция была отменена, так как не выполняются условия, необходимые для проведения операции. Например, попытка удалить непустой каталог или применение команды rmdir для объекта, который не является каталогом.
...
504 | 4 | DEADLINE_EXCEEDED | Превышено время ожидания ответа от сервера. Для операций, которые изменяют состояние системы, данная ошибка может быть возвращена, даже если операция была завершена успешно, но ответ 200 OK по каким-то причинам был задержан.
