...
- Топики реестра: 
    ...
    - `$registry/<ID реестра>/events` — топик для получения телеметрических данных.
    ...
    - `$registry/<ID реестра>/commands` — топик для отправки команд.
...
`$registry/<ID реестра>/events` | Отправляет телеметрические данные. | Получает телеметрические данные от всех устройств в реестре.<br/>Устройство неизвестно.
`$registry/<ID реестра>/commands` | Получает команды. | Отправляет команды всем устройствам в реестре. 
