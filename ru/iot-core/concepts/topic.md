...
_Перманентные топики_ — это топики, в которых сохраняется история действий устройств и реестров.
...
`$devices/<ID устройства>/state` | Отправляет телеметрические данные. | Получает телеметрические данные. <br/>Устройство известно.<br/>История действий сохранена.
`$devices/<ID устройства>/commands` | Получает команды. | Отправляет команды определенному устройству.
`$devices/<ID устройства>/config` | Получает команды. | Отправляет команды определенному устройству.<br/>История действий сохранена.
...
`$registries/<ID реестра>/state` | Отправляет телеметрические данные. | Получает телеметрические данные от всех устройств в реестре.<br/>Устройство неизвестно.<br/>История действий сохранена.
`$registries/<ID реестра>/commands` | Получает команды. | Отправляет команды всем устройствам в реестре. 
`$registries/<ID реестра>/config` | Получает команды. | Отправляет команды всем устройствам в реестре.<br/>История действий сохранена.
