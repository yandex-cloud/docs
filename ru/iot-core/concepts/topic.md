...
В сервисе реализованы два типа топиков: [топики устройства](#devices-topic) и [топики реестра](#registries-topic). Оба этих типа топиков поддерживают использование [сабтопиков](#subtopic). Вы можете использовать как обычные топики устройства и реестра, так и [перманентные топики](#permanent), в которых сохраняется информация о действиях пользователя.

## Перманентные топики {#permanent}

_Перманентные топики_ — это топики устройства и реестра, в которых сохраняется информация о действиях пользователя и история изменений.

В топик `state` устройство может писать, а реестр читать из него.  
В топик `config` реестр может писать, а устройство читать из него.  
...
- `$devices/<ID устройства>/state` — перманентный топик для отправки телеметрических данных c сохранением истории действий.
...
- `$devices/<ID устройства>/config` — перманентный топик для получения команд c сохранением истории действий.
...
- `$registries/<ID реестра>/events` — перманентный топик для получения телеметрических данных c сохранением истории действий.

    Устройство может писать в этот топик, а реестр — читать из него. Реестр, подписанный на эти топики, не будет знать, какое именно устройство отправило данные, так как в топике отсутствует уникальный идентификатор устройства.
- `$registries/<ID реестра>/commands` — топик для отправки команд.
- `$registries/<ID реестра>/commands` — перманентный топик для отправки команд c сохранением истории действий.
...
- `$devices/<ID устройства>/state/bedroom/temperature` — это сабтопик для получения данных о температуре в спальне c сохранением истории действий.
- `$registries/<ID реестра>/commands/bedroom` — это сабтопик для отправки команд всем устройствам в спальне.
- `$registries/<ID реестра>/config/bedroom` — это сабтопик для отправки команд всем устройствам в спальне c сохранением истории действий.
