...
- Символ `+` означает подстановку одной части топика или сабтопика, указанной между символами `/` либо в конце после символа `/`.
    ...
    Например, по подписке `$registries/<RID>/commands/+` будут получены сообщения топиков `$registries/<RID>/commands/1` и `$registries/<RID>/commands/2`, но проигнорированы `$registries/<RID>/commands/1/3`.
