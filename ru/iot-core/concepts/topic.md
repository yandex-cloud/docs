...
- `$registries/<ID реестра>/events` — перманентный топик для получения телеметрических данных.
...
- `$registries/<ID реестра>/commands` — перманентный топик для отправки команд.
...
Топикам `$<devices или registries>/<ID устройства или реестра>/commands` соответствуют перманентные топики `$<devices или registries>/<ID устройства или реестра>/config`.
