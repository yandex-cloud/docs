...
Чтобы разрешить доступ к ресурсам сервиса {{ iot-short-name }} (реестры и устройства),
...
Пользователь с ролью `{{ roles-viewer }}` может просматривать информацию о ресурсах, например посмотреть список устройств и их сертификаты.

#### {{ roles-editor }}

Пользователь с ролью `{{ roles-editor }}` может управлять устройствами, например создавать реестры и устройства, добавлять и удалять сертификаты.

Помимо этого роль `{{ roles-editor }}` включает в себя все разрешения роли `{{ roles-viewer }}`.

#### {{ roles-admin }}

Пользователь с ролью `{{ roles-admin }}` может управлять правами доступа к ресурсам, например разрешить другим пользователям просматривать устройства.
