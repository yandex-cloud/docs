---
title: Миграция с {{ iot-full-name }} на MQTT-решения с открытым исходным кодом
description: В практическом руководстве описано, как мигрировать с {{ iot-full-name }} на MQTT-решения с открытым исходным кодом.
---

# Миграция с {{ iot-full-name }} на MQTT-решения с открытым исходным кодом

{% include [iot-sunset-warning](../../_includes/iot-core/sunset-warning.md) %}

{% include [migration-note](../../_includes/iot-core/migration-note.md) %}

После закрытия {{ iot-full-name }} вы можете развернуть альтернативное MQTT-решение самостоятельно: на [виртуальной машине](../../compute/concepts/vm.md) {{ compute-full-name }} или в [кластере](../../managed-kubernetes/concepts/index.md#kubernetes-cluster) {{ managed-k8s-full-name }}.

Вместо {{ iot-full-name }} можно использовать следующие опенсорс-решения, которые можно запустить в {{ yandex-cloud }}:

* [Eclipse Mosquitto](https://mosquitto.org/) — рекомендуемый вариант для большинства сценариев, где {{ iot-full-name }} использовался как MQTT-брокер.
...
У {{ iot-full-name }} нет полностью совместимого опенсорс-аналога. После миграции вам нужно самостоятельно администрировать брокер или IoT-платформу: обновлять ПО, настраивать TLS, хранить учетные данные, делать резервные копии, мониторить доступность и планировать отказоустойчивость.
...
Когда подходит | Прямая замена MQTT-части {{ iot-full-name }} | Высокая нагрузка, много подключений | Нужны пользовательский интерфейс, дашборды, механизм rule engine, управление устройствами
Кластеризация | Нет (только active/passive) | Да, горизонтальное масштабирование | Да
Внешние зависимости | Нет | {{ KF }}, {{ RD }}, {{ PG }} | {{ PG }} (опционально другие хранилища)
...
* устройства используют MQTT и подключаются к {{ iot-full-name }} по TLS;
* основная логика работает вокруг публикации и подписки на топики;
* вы готовы заменить интеграции {{ iot-full-name }} с {{ sf-full-name }}, {{ serverless-containers-full-name }} или {{ yds-full-name }} собственным обработчиком сообщений;
...
* вы готовы поддерживать дополнительную инфраструктуру, например {{ KF }} и базу данных.

Выбирайте ThingsBoard Community Edition, если:

* {{ iot-full-name }} был частью более широкой IoT-системы;
...
При переносе с {{ iot-full-name }} на пользовательское MQTT-решение изменятся несколько важных частей архитектуры.

### Эндпоинт {#endpoint}

В {{ iot-full-name }} устройства подключались к управляемому эндпоинту сервиса. После миграции вам понадобится создать собственный эндпоинт, например:
...
* на `Service` типа `LoadBalancer` в {{ managed-k8s-full-name }}.
...
Если часть клиентов подключалась по MQTT поверх WebSocket (например, веб- или браузерные клиенты), настройте в Mosquitto отдельного слушателя с `protocol websockets` и TLS (обычно на порте `443`) и откройте этот порт в группе безопасности.

### Аутентификация {#authentication}

В {{ iot-full-name }} устройства могли использовать:
...
Пароли устройств и реестров не экспортируются из {{ iot-full-name }} в открытом виде. Получить их у {{ yandex-cloud }} нельзя. Если вы не сохранили пароли ранее, после миграции:
...
{{ iot-full-name }} также не экспортирует пароли реестров. Если ваш бэкенд использовал их для подключения к брокеру, выпустите для него новые учетные данные.

{% endnote %}

### Топики {#topics}

В {{ iot-full-name }} устройства и реестры используют следующие топики:
...
Если устройства подключались к {{ iot-full-name }} с использованием алиасов топиков (например, публиковали в `events` вместо `$devices/<device_id>/events`), после миграции переведите их на полные имена топиков: Mosquitto не разворачивает алиасы {{ iot-full-name }}.
...
В отличие от {{ iot-full-name }}, который маршрутизировал `events`, `$devices/<id>/events` и `devices/<id>/events` в одно логическое сообщение, в обычном MQTT-брокере `$devices/<id>/events` и `devices/<id>/events` — разные топики. Сообщение, опубликованное в один, не попадет в подписчика другого.
...
В {{ iot-full-name }} топики `state` и `config` являются постоянными: сервис хранит последнее сообщение в таком топике.
...
Если вы использовали триггеры для вызова функции {{ sf-name }} или контейнера {{ serverless-containers-name }}, после миграции их нужно заменить собственным подписчиком MQTT.

Такой подписчик должен:

* подписываться на MQTT-топики;
* преобразовывать сообщение в нужный формат;
* вызывать функцию, контейнер, HTTP API или отправлять сообщение в поток данных {{ yds-full-name }};
* логировать ошибки и повторять обработку при сбоях.

Пример такого обработчика приведен в разделе [{#T}](#replace-triggers).
...
5. Бэкенд-подписчик передает сообщения в ваши обработчики, базы данных, функции {{ sf-name }}, контейнеры {{ serverless-containers-name }} или потоки данных {{ yds-full-name }}.
...
## Выгрузите данные из {{ iot-full-name }} {#uploading-data}
...
Файлы экспорта могут содержать идентификаторы устройств, сертификаты, алиасы топиков и другие чувствительные данные. Не сохраняйте их в публичных репозиториях. Для хранения используйте закрытый [бакет](../../storage/concepts/bucket.md) {{ objstorage-full-name }}, ограниченные права доступа и шифрование.
...
Создайте ВМ {{ compute-full-name }} с Linux-образом и публичным IP-адресом или с доступом через сетевой балансировщик.
...
* храните TLS-ключи и пароли в {{ lockbox-full-name }} или в другом защищенном хранилище секретов;
* подключите {{ monitoring-full-name }} и {{ cloud-logging-full-name }}.
...
В {{ iot-full-name }} можно было настроить триггер, который вызывал функцию {{ sf-name }} или контейнер {{ serverless-containers-name }} при получении MQTT-сообщения. В пользовательском MQTT-брокере такую интеграцию нужно реализовать отдельным сервисом-подписчиком.
...
* на виртуальной машине {{ compute-name }};
* в кластере {{ managed-k8s-name }};
* в контейнере {{ serverless-containers-name }}, если приложение постоянно поддерживает MQTT-соединение;
...
## Замените экспорт в {{ yds-full-name }} {#replace-yds}

Если вы использовали экспорт сообщений из {{ iot-full-name }} в потоки данных {{ yds-full-name }}, используйте один из вариантов:

* MQTT-подписчик читает сообщения из Mosquitto и отправляет их в поток данных {{ yds-full-name }};
* Бэкенд-приложение одновременно публикует данные в MQTT и в поток данных {{ yds-full-name }};
...
1. После завершения миграции отключите публикацию в {{ iot-full-name }}.
...
Не откладывайте перенос до даты остановки {{ iot-full-name }}. После перевода сервиса в режим read-only вы не сможете изменять часть ресурсов, а после полной остановки устройства перестанут обмениваться сообщениями через сервис.
...
## Развертывание Mosquitto в {{ managed-k8s-full-name }} {#k8s}

Mosquitto можно запустить в кластере {{ managed-k8s-name }}. Для продакшен-окружения используйте:
...
* вы готовы сопровождать {{ KF }}, базу данных и другие компоненты.
...
Базу данных для TBMQ или ThingsBoard можно вынести в сервис {{ mpg-full-name }}, чтобы не администрировать СУБД самостоятельно, либо развернуть ее в кластере вместе с остальными компонентами.
...
ThingsBoard CE не является drop-in-заменой {{ iot-full-name }}. Устройства и бэкенд-приложения обычно нужно адаптировать под API и модель данных ThingsBoard.
...
* интеграции с {{ iam-short-name }};
* публикация сообщений через gRPC-интерфейс {{ iot-full-name }};
* триггеры;
* экспорт в {{ yds-full-name }};
* мониторинговые топики {{ iot-full-name }};
...
* публикация через gRPC-интерфейс — прямой совместимой замены нет, потому что в {{ iot-full-name }} использовалась {{ iam-short-name }}-авторизация. Если такой сценарий нужен, реализуйте отдельный бэкенд-адаптер с нужной схемой авторизации, который будет принимать gRPC/API-запросы от ваших систем и публиковать сообщения в Mosquitto;
* триггеры — MQTT-подписчик.
* экспорт — MQTT-подписчик, который пишет в потоки данных {{ yds-full-name }} или другое хранилище.
...
* Используйте MQTTS на порте `8883`.
...
* Собирайте логи Mosquitto в {{ cloud-logging-full-name }}.
...
* Реализована замена экспорта в {{ yds-full-name }}, если он использовался.
...
* Отключена зависимость продакшен-систем от {{ iot-full-name }}.
...
* [Документация {{ managed-k8s-full-name }}](../../managed-kubernetes/)
* [Документация {{ objstorage-full-name }}](../../storage/)
* [Документация {{ lockbox-full-name }}](../../lockbox/)
* [Документация {{ monitoring-full-name }}](../../monitoring/)
* [Документация {{ cloud-logging-full-name }}](../../logging/)
