...
1. **Распределение нагрузки (Active-Active)**:
   * Нагрузка распределяется между несколькими зонами (схема ниже);
...
Согласно [SLA](https://yandex.ru/legal/cloud_sla_mdb/ru/#klastera-servisa-upravlyaemogo-mysql-\/-yandex-managed-service-for-mysql,-popadayushie-pod-dejstvie-urovnya-obsluzhivaniya), высокодоступной считается конфигурация `Кластера БД, состоящего из двух или более Хостов БД, расположенных в различных зонах доступности`. Оптимальным является размещение узлов кластера БД в трех зонах доступности, так как для обеспечения высокой доступности используются системы на базе кворумных алгоритмов. При этом у разных сервисов могут быть специфические требования по обеспечению высокой доступности. Для некоторых сервисов MDB применяются особые SLA:
...
Рекомендации по обеспечению устойчивости к отказам зон:
   1. Используйте отдельные группы ВМ для каждой зоны доступности. Не рекомендуется использовать одну группу ВМ для создания виртуальных машин в разных зонах доступности, так как это может затруднить управление при отказе одной из зон.
   1. В {{ managed-k8s-name }} в качестве групп узлов c автоматическим масштабированием используются зональные группы ВМ. Для размещения узлов такого кластера в разных зонах используйте отдельные группы узлов для каждой зоны.
