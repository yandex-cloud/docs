---
title: Создание локального реестра в {{ cloud-registry-full-name }}
...
- Консоль управления {#console}
    ...
    1. В [консоли управления]({{ link-console-main }}) выберите [каталог](../../../resource-manager/concepts/resources-hierarchy.md#folder), в котором хотите создать локальный реестр.
    ...
    1. Если вы выбрали формат реестра:
        ...
        * `Docker`:
            ...
            1. Укажите политику неизменяемости артефактов:
            ...
            1. В блоке **Безопасность** включите опцию:
        ...
        * `Debian`, при необходимости настройте подпись метаданных реестра:

            Включите опцию **Подпись реестра** и укажите секрет [{{ lockbox-full-name }}](../../../lockbox/), содержащий GPG-ключ для подписи. Подробнее о подготовке GPG-ключа и создании секрета в разделе [{#T}](../debian/installation.md#prepare-gpg-key).
    ...
    1. В блоке **Информация о реестре**:
        ...
        * (Опционально) Введите описание и добавьте метки реестра в формате `ключ: значение`.
...
- CLI {#cli}
    ...
    * `--properties` — свойства реестра. Задаются строкой в формате `name1=value1,name2=value2`. Доступные свойства для локальных реестров:
        ...
        * `signingGpgKeySecretId` — идентификатор секрета {{ lockbox-full-name }}, содержащего GPG-ключ для подписи метаданных. Поддерживается только для формата реестра `debian`. Подробнее о подготовке GPG-ключа и создании секрета в разделе [{#T}](../debian/installation.md#prepare-gpg-key).
