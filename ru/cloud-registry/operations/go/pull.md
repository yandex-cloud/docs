---
title: Скачать Go-модуль из реестра {{ cloud-registry-name }}
description: Следуя данной инструкции, вы скачаете Go-модуль из реестра {{ cloud-registry-name }}.
---

# Скачать Go-модуль из реестра {{ cloud-registry-name }}
...
Загрузка Go-модулей выполняется штатной командой `go` — реестр {{ cloud-registry-name }} работает как Go-прокси и реализует [Module Proxy Protocol](https://go.dev/ref/mod#goproxy-protocol).
...
1. Укажите реестр как прокси Go-модулей и отключите проверку контрольных сумм через публичную [Go checksum database](https://sum.golang.org/) для Go-модулей из вашего реестра:
    ...
    * `GOPROXY` — список Go-прокси через запятую. Команда `go` ищет Go-модуль последовательно в каждом источнике:
        * `https://registry.yandexcloud.net/go/<идентификатор_реестра>` — ваш реестр {{ cloud-registry-name }};
