# Управление доступом в {{ yq-full-name }}
...
Пользователь с ролью `yq.editor` может просматривать, редактировать, удалять созданные им самим соединениями. Роль позволяет создавать, редактировать, удалять и запукать созданные пользователем запросы. Роль `yq.editor` включает в себя все разрешения роли `ydb.viewer`.
