...
1. При первом переходе из {{at-full-name}} в {{yq-full-name}} необходимо выполнить настройку интеграции:
   1. В интерфейсе **{{yq-full-name}}** в диалоге создания соединения выбрать сервисный аккаунт, от имени которого будет выполнять чтение данных из {{objstorage-full-name}} и нажать кнопку **Создать**.
