---
title: Чтение данных из {{ objstorage-full-name }} с помощью соединений с {{ yq-full-name }}
description: Из статьи вы узнаете, как настроить соединение и читать данные из {{ objstorage-full-name }} с помощью {{ yq-full-name }}.
---
...
* `<соединение>` — название [соединения](#create-connection) с хранилищем;
* `<путь>` — путь к файлу или файлам внутри бакета. Поддерживаются шаблоны `*`;
* `<формат_данных>` — [формат данных](formats.md#formats) в файлах;
* `<формат_сжатия>` — [алгоритм сжатия](formats.md#compression) файлов;
* `<описание_схемы>` — [описание схемы хранимых данных](#schema) в файлах;
