...
`ORDER BY` определяет сортировку входных данных, то есть перед выполнением всех операций поиска паттернов, данные будут предварительно отсортированы по указанным ключам или выражениям. Синтаксис аналогичен SQL-выражению `ORDER BY`. Сортировка допустима только по полям с типом [TimeStamp](https://ydb.tech/docs/ru/yql/reference/types/primitive#datetime), поддерживается только в направлении возрастания, `ASC`.
...
- [`ORDER_BY`](#order_by). В качестве столбцов сортировки можно задавать ровно один столбец(выражение) с типом [`Timestamp`](https://ydb.tech/docs/ru/yql/reference/types/primitive#datetime). Сортировка допустима только в направлении возрастания значений, `ASC`.
