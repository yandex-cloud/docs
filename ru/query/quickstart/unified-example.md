...
В примере выше использовался единый запрос для анализа данных из {{ objstorage-full-name }} и потоковых данных. Текст SQL-запросов анализа данных одинаков, запросы различаются подключениями к данным: одни данные хранятся в {{ objstorage-full-name }}, а другие в {{ yds-full-name }}. 