...
{{ yq-full-name }} позволяет обращаться к хранимым в {{ objstorage-full-name }} данным, выполняя запросы на диалекте SQL — [YQL](https://ydb.tech/ru/docs/yql/reference/syntax/).

В этом примере получим частотное распределение длительности поездок по числу поездок в виде гистограммы. Для этого подключимся к данным в бакете и выполним аналитический запрос.
