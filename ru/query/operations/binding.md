...
Привязка к данным в {{yq-full-name}} содержит информацию о [соединении](connection.md), формате и схеме данных и позволяет выполнять запросы к данным из разных источников.
...
1. В блоке **Параметры соединения**:
   1. Выберите тип соединения: `{{ yds-short-name }}` или `{{ objstorage-short-name }}`.
