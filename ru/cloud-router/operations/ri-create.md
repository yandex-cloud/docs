---
title: Как создать виртуальный маршрутизатор в {{ cr-name }}
description: Следуя этой инструкции, вы сможете создать виртуальный маршрутизатор в {{ cr-name }}.
---
...
- Консоль управления {#console}
  ...
  1. В [консоли управления]({{ link-console-main }}) выберите [каталог](../../resource-manager/concepts/resources-hierarchy.md#folder), в котором хотите создать [виртуальный маршрутизатор](../concepts/routing-instance.md).
  1. [Перейдите]({{ link-console-main }}/link/cloud-router) в сервис **{{ ui-key.yacloud.ui.constants.label_cloud-router_kBGNL }}**.
  1. Нажмите кнопку **{{ ui-key.yacloud.cloud-router.router.create-router_v4cPC }}**.
  ...
  1. (Опционально) Включите опцию **{{ ui-key.yacloud.common.deletion-protection }}**, чтобы защитить виртуальный маршрутизатор от непреднамеренного удаления.
  ...
  1. В поле **{{ ui-key.yacloud.cloud-router.router.networks_fbzKL }}** выберите необходимые [сети](../../vpc/concepts/network.md).
