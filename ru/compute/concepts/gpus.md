...
{{ compute-full-name }} предоставляет графические ускорители (GPU), которые можно добавить при создании виртуальной машины. Доступны графические карты [NVIDIA Tesla V100](https://www.nvidia.com/ru-ru/data-center/tesla-v100/) для PCle с 32 ГБ памяти HBM2. 

GPU обрабатывают некоторые типы данных эффективнее, чем vCPU, и могут использоваться для сложных вычислений. 
...
Кроме того, графические ускорители подходят в том числе для задач машинного обучения (Machine Learning, ML), искусственного интеллекта (Artificial Intelligence, AI) и обработки трехмерной графики (3D Rendering).

Контролировать GPU и соответствующую ему оперативную память можно непосредственно с виртуальной машины.
