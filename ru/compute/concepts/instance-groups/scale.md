...
{% if audience == "external" %} Если вы [записываете пользовательскую метрику](../../../monitoring/operations/metric/add.md) в сервисе {{ monitoring-full-name }}, вы можете использовать значения этой метрики для автоматического масштабирования в {{ ig-name }}. {% endif %}
