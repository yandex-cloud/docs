...
* Если [среднее значение метрики](#average-metric) поднимется выше целевого, {{ ig-name }} создаст новые ВМ в группе.
...
* При _зональном_ масштабировании для каждой зоны доступности независимо рассчитывается свое [среднее значение метрики](#average-metric) для масштабирования и нужное количество ВМ. По умолчанию автоматическое масштабирование — зональное. 
...
* *Период стабилизации* — после увеличения количества ВМ размер группы не уменьшается до окончания периода стабилизации, даже если [среднее значение метрики](#average-metric) стало достаточно низким.
...
Вы можете использовать до трех любых метрик из сервиса {{ monitoring-name }} для автоматического масштабирования в {{ ig-name }}. Для чтения метрик у [сервисного аккаунта](../../../iam/concepts/users/service-accounts.md), привязанного к группе ВМ, должна быть [роль](../../../monitoring/security/index.md#monitoring-viewer) `monitoring.viewer` или выше.
...
* _Тип метрики_, который влияет, как {{ ig-name }} будет рассчитывать [среднее значение метрики](#average-metric):
...
### Расчет среднего значения метрики {#average-metric}

Для расчета среднего значения метрики используется [_экспоненциально взвешенное скользящее среднее_](https://ru.wikipedia.org/wiki/Скользящая_средняя#Экспоненциально_взвешенное_скользящее_среднее). Это делает автоматическое масштабирование чувствительным к резкому росту метрики и при этом сглаживает пики. 

_Обычное среднее_ рассчитывается по формуле:
...
где $f(x)$ — функция метрики на временном отрезке $[ a, b ]$.

В значениях метрик могут быть резкие скачки, распределенные на всем временном отрезке $[ a, b ]$. При расчете обычного среднего не учитывается когда был скачок — ближе к началу временного отрезка или к концу. Это может привести к нерациональному масштабированию группы ВМ и лишним затратам на ресурсы.
...
где $w(x)$ — весовая функция $w(x) = k^{-(x - a)}$, $k \in (0, 1)$, позволяющая учитывать значения функции $f(x)$ ближе к концу отрезка $b$, то есть ближе к текущему моменту времени, с большими весами.

Коэффициент $k$ зависит от длительности измерения метрики и вычисляется по формуле:
