...
Если группа ВМ интегрирована с {{ network-load-balancer-name }} или {{ alb-name }}, то для проверки в {{ ig-name }} выставляйте более мягкие настройки, чем для проверки состояния в балансировщике (подробнее о проверках читайте в документации [{{ network-load-balancer-name }}](../../../network-load-balancer/concepts/health-check.md) или [{{ alb-name }}](../../../application-load-balancer/concepts/backend-group.md#health)). Балансировщик распределяет нагрузку на приложение, а {{ ig-name }} только следит за работоспособностью приложения.
