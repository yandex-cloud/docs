...
У [сервисного аккаунта](../../../iam/concepts/users/service-accounts.md), от имени которого выполняются операции в {{ ig-name }}, должна быть [роль](../../../vpc/security/index.md) для доступа к сети и подсетям, в которых размещается группа ВМ. Подробнее на странице [{#T}](access.md).
...
`metadata` | (опционально) Метаданные для шаблонной ВМ. Подробнее в разделе [{#T}](../vm-metadata.md).
