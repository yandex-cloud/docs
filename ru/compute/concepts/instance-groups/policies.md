...
`initial_size`* | Начальное количество виртуальных машин в группе. Максимальное значение — 100.
`max_size` | Максимальное количество виртуальных машин в группе. Максимальное значение — 100.
`min_zone_size` | Минимальное количество виртуальных машин в одной зоне доступности.
`measurement_duration`* | Время в секундах, за которое происходит усреднение метрик по нагрузке CPU. Если средняя нагрузка по окончании интервала оказывается выше значения `cpu_utilization_rule.utilization_target`, то {{ ig-name }} увеличит количество виртуальных машин в группе. Допустимые значения — от 60 секунд до 600 секунд включительно.
`warmup_duration`* | Время прогрева виртуальной машины в секундах. В течение этого времени на виртуальную машину подается трафик, но метрики по нагрузке CPU не учитываются. Максимальное значение — 600 секунд.
`stabilization_duration`* | Минимальный временной интервал, в секундах, для мониторинга нагрузки перед тем как {{ ig-name }} может уменьшить количество виртуальных машин в группе. В течение этого времени группа не будет уменьшаться даже если средняя нагрузка упадет ниже значения `cpu_utilization_rule.utilization_target`. Допустимые значения — от 60 секунд до 1800 секунд включительно.
`cpu_utilization_rule`* | Задает целевой уровень нагрузки CPU, который позволяет выполнять масштабирование, основываясь на средней нагрузки CPU для группы виртуальных машин.
