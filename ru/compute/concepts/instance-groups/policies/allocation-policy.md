...
При создании группы виртуальных машин можно выбрать, в каких регионах и [зонах доступности](../../../../overview/concepts/geo-scope.md) {{ yandex-cloud }} будут размещаться ВМ, а также задать список тегов для ВМ внутри каждой зоны доступности.

_Регион_ — это географическое расположение, в котором разворачиваются ВМ. Каждый регион имеет одну или несколько _зон_. На данный момент доступен только один регион.


Регион | Зоны | Географическая привязка
----- | ----- | -----
`{{ region-id }}` | `{{ region-id }}-a`<br/>`{{ region-id }}-b`<br/>`{{ region-id }}-d` | Владимирская, Рязанская и Калужская области.
