...
В групах c ручным масштабированием виртуальные машины распределяются между зонами равномерно. 

В соответствии с [политикой развертывания](../policies.md#allocation_policy), в каждую зону последовательно добавляется по одной виртуальной машине. Если список зон подошел к концу, а необходимое количество виртуальных машин еще не создано, то зоны используются повторно.  
 
{% note tip %}

Рекомендуем создавать группу таким образом, чтобы ее размер был кратен количеству зон. 
В этом случае количество машин в зонах будет одинаковым, и [балансировщик нагрузки](https://cloud.yandex.ru/docs/load-balancer/concepts/) (если он есть) будет равномерно распределять нагрузку между виртуальными машинами.
...
Количество виртуальных машин в зонах определяется алгоритмом [автоматического масштабирования](../scale#auto-scale), исходя из значений средней утилизации и средней нагрузки на зону. И оно может изменяться в зависимости от значений ограничений, заданных пользователем:
...
Пусть `min_zone_size` равно 2, а алгоритмом автоматического масштабирования расcчитывается, что нужно 0 виртуальных машин в зоне. Тогда в зоне будет 2 виртуальных машины, так как это минимальное допустимое количество машин в зоне.

**При распрделении учитываются ограничения `min_zone_size` и `max_size`**
...
1. Всего машин получается 30. Это на 10 больше, чем `max_size`. Поэтому из каждой зоны убираются виртуальные машины, пока не уберется 10. 
  Чем больше виртульных машин в зоне, тем больше машин может быть из нее удалено. Но при этом должно остаться не менее 5 виртуальных машин в зоне.
