...
**Платформа Ice Lake**<br>`intel-6338-c108-m958-n6400x6`<br>Intel<sup>®</sup> Xeon<sup>®</sup> Gold 6338 | 2 | 64 | 108 | 958 | 6 | 6397848715264&nbsp;Б <br>(~ 5,82 ТБ)
...
**Платформа Ice Lake Compute-Optimized**<br>`intel-6354-c56-m938-n6400x6`<br>Intel<sup>®</sup> Xeon<sup>®</sup> Gold 6354 | 2 | 32 | 56 | 938 | 6 | 6397848715264&nbsp;Б <br>(~ 5,82 ТБ)
...
Чтобы начать освобождение хоста от ВМ немедленно, не указывайте параметр `--replacement-deadline`.
...
ВМ с дополнительными дисками можно перенести на новый хост. Данные на сетевых дисках сохранятся, а данные на локальных дисках будут потеряны — они привязаны к физическому серверу. Чтобы не потерять информацию с локальных дисков, [настройте](../../backup/operations/backup-vm/create.md) резервное копирование в {{ backup-name }}.

{% cut "Пример команд для переноса ВМ с одного хоста на другой" %}
   ...
   1. Привязать ВМ к другому хосту:

      ```bash
      yc compute instance update \
        --id <идентификатор_ВМ> \
        --host-id <идентификатор_нового_хоста>
      ```
