...
**Платформа Ice Lake**<br>`intel-6338-c108-m958-n6400x6`<br>Intel<sup>®</sup> Xeon<sup>®</sup> Gold 6338 | 2 | 64 | 108 | 958 | 6 | 6398924554240&nbsp;Б <br>(~ 5,82 ТБ)
...
**Платформа Ice Lake Compute-Optimized**<br>`intel-6354-c56-m938-n6400x6`<br>Intel<sup>®</sup> Xeon<sup>®</sup> Gold 6354 | 2 | 32 | 56 | 938 | 6 | 6398924554240&nbsp;Б <br>(~ 5,82 ТБ)

{% cut "Архивные типы — на платформе Intel Cascade Lake" %}

{% note alert %}

Не создавайте выделенные хосты архивных типов. Используйте только действующие типы, указанные выше.

{% endnote %}

Тип и процессор<br>(платформа Cascade Lake) | Процессоры | Ядра | vCPU^1^ | RAM, ГБ | Диски | Размер диска
--- | --- | --- | --- | --- | --- | ---
`intel-6230-c66-m454`<br>Intel® Xeon® Gold 6230 | 2 | 40 | 66 | 454 | 4 | 1599246172160 Б <br>(~ 1,45 ТБ)
...
* с помощью команды CLI:
    ...
    Подробнее читайте в [инструкции по созданию группы](../operations/dedicated-host/create-host-group.md).

^1^ Указано количество vCPU, на которых вы можете запускать ВМ. Остальные vCPU хоста выделены для системного потребления (подробнее [ниже](#resource-fragmentation)): на процессорах Intel® Xeon® Gold 6338 — 20 vCPU, на процессорах Intel® Xeon® Gold 6354 — 8 vCPU.
...
На каждом выделенном хосте доступно несколько локальных дисков. Их количество и объем зависят от выбранной конфигурации хоста (смотрите [выше](#host-types)).
...
Высвобождаемый из-под нагрузки хост не тарифицируется, не учитывается в размере хост-группы и не используется для размещения вновь запускаемых ВМ.
...
1. [Остановить ВМ](../operations/vm-control/vm-stop-and-start.md).
