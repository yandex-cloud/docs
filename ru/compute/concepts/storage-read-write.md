...
Для [дисков](disk.md){% if product == "yandex-cloud" %} и [файловых хранилищ](filesystem.md){% endif %} действуют технические ограничения на операции чтения и записи. Ограничения относятся ко всему диску{% if product == "yandex-cloud" %} или хранилищу{% endif %} и к каждому блоку размещения (allocation unit) — единицы выделения дискового пространства. Величина блока размещения зависит от [типа диска{% if product == "yandex-cloud" %} или хранилища{% endif %}](../concepts/limits.md#compute-limits-disks).
...
Фактическое значение IOPS зависит как от характеристик диска{% if product == "yandex-cloud" %} или хранилища{% endif %}, так и от совокупного значения пропускной способности и размера запроса в байтах. Обеспечиваемый IOPS определяется по формуле:

![image](../../_assets/compute/iops.svg)

Где:
* _Макс. IOPS_ — [максимальное значение IOPS](../concepts/limits.md#limits-disks) для диска{% if product == "yandex-cloud" %} или хранилища{% endif %}.
* _Макс. bandwidth_ — [максимальное значение пропускной способности](../concepts/limits.md#limits-disks) для диска{% if product == "yandex-cloud" %} или хранилища{% endif %}.
...
## Производительность дисков{% if product == "yandex-cloud" %} и файловых хранилищ{% endif %} {#performance}

Максимальные значения IOPS достигаются на операциях чтения и записи размером 4 КБ. Сетевые SSD-диски{% if product == "yandex-cloud" %} и файловые хранилища{% endif %} предоставляют существенно большие значения IOPS на чтение, а также меньшие значения времени обработки запроса.

Чтобы получить максимальные значения пропускной способности, рекомендуется делать чтения и записи размером 4 МБ.

Производительность диска{% if product == "yandex-cloud" %} или хранилища{% endif %} зависит от его объема — чем выше количество блоков размещения, тем выше значения IOPS и пропускной способности.
