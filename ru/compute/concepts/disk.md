...
При подключении диска к виртуальной машине можно указать, должен ли он быть удален при удалении виртуальной машины. Также это можно настроить при создании и изменении виртуальной машины.
...
Вы можете создавать резервные копии дисков в виде [снимков](snapshot.md). Снимки реплицируются во всех зонах доступности, поэтому их также можно использовать для переноса дисков между зонами доступности.
...
Чтобы получить максимальные значения IOPS, рекомендуется делать чтения и записи, близкие к размеру блока диска (по умолчанию 4 КБ). Сетевые SSD-диски предоставляют существенно большие значения IOPS на чтение, а также меньшие значения времени обработки запроса.
