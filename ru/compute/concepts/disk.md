...
* Высокопроизводительный SSD-диск (`network-ssd-io-m3`) — обладает теми же характеристиками что и `network-ssd-nonreplicated` и обеспечивает избыточность. Высокопроизводительные SSD находятся на стадии [Preview](../../overview/concepts/launch-stages.md).
* [Локальные диски](dedicated-host.md#resource-disks) на выделенных хостах.  

Сетевые SSD, сетевые HDD-диски и высокопроизводительные SSD-диски обеспечивают достаточную избыточность для надежного хранения данных и позволяют непрерывно выполнять операции чтения и записи даже при одновременном выходе из строя нескольких физических дисков. Нереплицируемые диски не гарантируют сохранность данных.
...
* Из таких дисков нельзя создать [снимки](snapshot.md) и [образы](image.md). 
...
Если вам необходима повышенная производительность и обеспечение отказоустойчивости, рекомендуется использовать высокопроизводительные SSD-диски. Сейчас ограничения на создание образов и снимков высокопроизводительных SSD такие же, как и у нереплицируемых дисков. В скором времени появится возможность создавать снимки с заморозкой записи, а в будущем и и полноценные снимки без заморозки записи.
...
* [{{ backup-name }}](../../backup/) позволяет делать консистентные копии данных на ВМ с любыми типами дисков. Сервис находится на стадии [Preview](../../overview/concepts/launch-stages.md).
* [Снимки дисков](snapshot.md) — позволяют вручную или автоматически [по расписаниям](snapshot-schedule.md) создавать снимки сетевых SSD- и HDD-дисков. Нельзя делать снимки нереплицируемых и высокопроизводительных SSD-дисков.
