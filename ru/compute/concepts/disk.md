...
Диски не реплицируются{% if product == "yandex-cloud" %} на стороне {{ yandex-cloud }}{% endif %}, но пользователь может самостоятельно сделать диск реплицируемым с помощью [групп размещения](disk-placement-group.md).
