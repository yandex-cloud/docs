...
* Нереплицируемый SSD-диск (`nonrepl`) — сетевой диск с повышенной производительностью, реализованной за счет некоторых [ограничений](#nr-disks).

Стандартные сетевые SSD и HDD-диски обеспечивают достаточную избыточность для надежного хранения данных и позволяют непрерывно выполнять операции чтения и записи даже при одновременном выходе из строя нескольких физических дисков. Нереплицируемые диски не дублируют хранимую информацию. 

Если физический диск, на котором располагается сетевой SSD или HHD-диск, вышел из строя, виртуальная машина продолжит работу и быстро получит доступ к данным в полном объеме.
...
Несколько нереплицируемых дисков могут быть собраны в `placement group` для организации избыточности хранения данных на уровне приложения. При этом отдельные диски физически будут размещены в разных стойках дата-центра, что снизит вероятность одновременного выхода из строя всех дисков группы.
