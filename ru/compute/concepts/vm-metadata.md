...
При работе через CLI, значение еще можно передать в виде файла: `--metadata-from-file key=path/to/file`. Таким образом удобнее передать значение из множества строк.
...
## Обработка метаданных в публичных образах Яндекс.Облака

В публичных образах Linux Яндекс.Облака по умолчанию используется гостевой агент [cloud-init](https://cloud-init.io). В публичных образах с Windows - [Cloudbase-init](https://cloudbase.it/cloudbase-init/).
...
Например, вы можете задать в `user-data` пользователей и передать ключи SSH, с которыми они смогут подключаться к виртуальной машине:
