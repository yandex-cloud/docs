...
Агент для сброса паролей и программа для обновления агента входят в состав стандартных образов Windows с 12 июня 2022 года. Если ваша ВМ создана раньше или вы пользуетесь собственным образом, для сброса паролей установите агент и программу для его обновления по [инструкции](../operations/vm-guest-agent/install.md).
...
1. На стороне пользователя (в браузере, если сброс запрошен в консоли управления, или в PowerShell) по алгоритму {% if lang == "ru" %}[RSA](https://ru.wikipedia.org/wiki/RSA){% endif %}{% if lang == "en" %}[RSA](https://en.wikipedia.org/wiki/RSA_(cryptosystem)){% endif %} создается пара из открытого и закрытого ключей шифрования. Запрос с открытым ключом и именем пользователя ВМ, для которого нужно сбросить пароль, отправляется агенту через HTTPS-соединение.
1. Агент на ВМ:
