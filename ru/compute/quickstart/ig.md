...
1. В блоке **Интеграция с Load Balancer**:
    - (опционально) Нажмите переключатель **Создать целевую группу**. [Целевые группы](../../load-balancer/concepts/target-resources.md) нужны для распределения нагрузки [сетевым балансировщиком](../../load-balancer/concepts/index.md).
