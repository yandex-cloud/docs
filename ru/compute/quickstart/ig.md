...
1. В блоке **{{ ui-key.yacloud.compute.groups.create.section_instance }}** нажмите кнопку **{{ ui-key.yacloud.compute.groups.create.button_instance_empty-create }}**, чтобы задать конфигурацию базовой ВМ:
    ...
    * В блоке **{{ ui-key.yacloud.compute.instances.create.section_image }}** выберите публичный образ, на основе которого будут работать виртуальные машины группы.
    * В блоке **{{ ui-key.yacloud.compute.instances.create.section_storages }}**:
        ...
        * (Опционально) Нажмите кнопку **{{ ui-key.yacloud.common.add }}**, чтобы добавить дополнительные диски или файловые хранилища.
    * В блоке **{{ ui-key.yacloud.compute.instances.create.section_platform }}** выберите одну из готовых конфигураций ВМ или перейдите на вкладку **{{ ui-key.yacloud.component.compute.resources.label_tab-custom }}**, чтобы создать собственную конфигурацию.
    * В блоке **{{ ui-key.yacloud.compute.instances.create.section_network }}**:
        ...
        * Выберите [облачную сеть](../../compute/concepts/vm.md#network) и [подсеть](../../compute/concepts/network.md#subnet). Если подсети нет, создайте ее:
            ...
            * Разверните раскрывающийся список и нажмите кнопку ![image](../../_assets/console-icons/plus.svg) **{{ ui-key.yacloud.compute.instances.create.button_create-subnetwork-new }}**.
        ...
        * (Опционально) Выберите [группу безопасности](../../vpc/concepts/security-groups.md). Если группы безопасности нет, создайте ее:
            ...
            * Разверните раскрывающийся список и нажмите **{{ ui-key.yacloud.component.network-subnet-select.button_create-security-group }}**.
            * Введите имя и описание группы безопасности.
            * Нажмите кнопку ![image](../../_assets/console-icons/plus.svg) **{{ ui-key.yacloud.vpc.network.security-groups.content-section.button_add-rule }}**, чтобы создать в группе безопасности необходимые [правила](../../vpc/concepts/security-groups.md#security-groups-structure).
            * Нажмите кнопку **{{ ui-key.yacloud.common.create }}**.
    * В блоке **{{ ui-key.yacloud.compute.instances.create.section_access }}**:

        * Выберите **{{ ui-key.yacloud.compute.instance.access-method.field_os-login-access-method }}**, чтобы [подключаться](../../compute/operations/vm-connect/os-login.md) к виртуальным машинам и управлять доступом к ним с помощью [{{ oslogin }}](../../organization/concepts/os-login.md) в {{ org-full-name }}.
    ...
    * (Опционально) Разверните блок **{{ ui-key.yacloud.compute.instances.create.section_additional }}** и в появившихся полях:

        * Выберите [сервисный аккаунт](../../iam/concepts/users/service-accounts.md) из списка или создайте новый.
        * При необходимости разрешите доступ к [серийной консоли](../concepts/serial-console.md).
...
1. В блоке **{{ ui-key.yacloud.compute.groups.create.section_deploy }}** укажите:
    ...
    * **{{ ui-key.yacloud.compute.groups.create.field_deploy-max-creating }}** — сколько ВМ можно создавать одновременно.
    ...
    * **{{ ui-key.yacloud.compute.groups.create.field_deploy-max-deleting }}** — сколько ВМ можно останавливать одновременно.
...
1. (Опционально) В блоке **{{ ui-key.yacloud.compute.groups.create.section_ylb }}** включите опцию **{{ ui-key.yacloud.compute.groups.create.field_target-group-attached }}** и введите имя и описание целевой группы.

    [Целевые группы](../../network-load-balancer/concepts/target-resources.md) нужны для распределения нагрузки [сетевым балансировщиком](../../network-load-balancer/concepts/index.md).
