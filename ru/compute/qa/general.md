...
Для подключения к ВМ Linux [используйте SSH](../operations/vm-connect/ssh.md). {% if product == "yandex-cloud" %}К ВМ Windows подключайтесь по RDP.{% endif %}
...
1. Используйте сетевые SSD большого размера. Слабым местом системы могут оказаться сетевые диски. Они ограничены лимитами на IOPS и пропускную способность, которые зависят от типа диска и его размера. Подробнее в разделе документации [{#T}](../concepts/limits.md#compute-limits-disks).
...
Поддерживаются ОС на базе Linux{% if product == "yandex-cloud" %} и Windows{% endif %}.
