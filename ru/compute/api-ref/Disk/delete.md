...
Удаление диска - необратимая операция, его данные удаляются
навсегда. Однако при удалении диска не удаляются снимки или
образы, ранее созданные из этого диска. Снимки и образы
необходимо удалять отдельно.

Невозможно удалить диск, подключенный к виртуальной машине.
 
## HTTP-запрос
`DELETE /compute/v1/disks/{diskId}`
 
## Path-параметры {#path_params}
 
Name | Description
--- | ---
diskId | Обязательное поле. Идентификатор удаляемого диска. Чтобы получить идентификатор диска, используйте запрос [list](/docs/compute/api-ref/Disk/list).  Максимальная длина — 50 символов.
