...
## HTTP-запрос
`PATCH /compute/v1/images/{imageId}`
 
## Path-параметры {#path_params}
 
Name | Description
--- | ---
imageId | Обязательное поле. Идентификатор изменяемого ресурса Image. Чтобы получить идентификатор образа, используйте запрос [list](/docs/compute/api-ref/Image/list).  Максимальная длина — 50 символов.
...
minDiskSize | **string** (int64)<br>Минимальный размер диска, который может быть создан из этого образа. Указывается в байтах. Должно быть больше объема исходных данных и больше размера виртуального диска.  Допустимые значения — от 4194304 до 4398046511104 включительно.
