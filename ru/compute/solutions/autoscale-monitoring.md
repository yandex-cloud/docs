...
- AWS CLI
  ...
  1. Настройте AWS CLI для работы с {{ yandex-cloud }}:
     ...
     1. Создайте в домашнем каталоге файл `.aws/credentials` и укажите в нем статический ключ доступа, [полученный ранее](#create-service-account), и его идентификатор:
...
- Консоль управления
  ...
  1. В блоке **Шаблон виртуальной машины** нажмите кнопку **Задать** и настройте шаблон:
     ...
     1. В блоке **Сетевые настройки** выберите **Сеть** `queue-autoscale-network`. Подсеть `queue-autoscale-subnet-b` будет выбрана автоматически.
     1. В блоке **Доступ**:
