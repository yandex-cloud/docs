...
У пользователей Яндекс.Облака по умолчанию нет доступа ни к каким облачным ресурсам. Пользователь может выполнять только те операции с облачными ресурсами, на выполнение которых ему выдано разрешение. 

Вы можете управлять правами доступа к ресурсам и разрешениями на выполнение операций через назначение соответствующих ролей.

В сервисе [!KEYREF compute-full-name] вы можете управлять доступом к виртуальным машинам, дискам, образам и снимкам. Просматривать типы дисков и зоны доступности разрешено любому пользователю Яндекс.Облака
...
Роль может быть назначена на сам ресурс или на ресурс, от которого наследуются права доступа, например на каталог или облако.

На данный момент вы можете назначать роли на каталог или облако, но не на конкретные ресурсы сервиса [!KEYREF compute-name].
