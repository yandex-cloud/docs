...
Вы можете создать группу виртуальных машин фиксированного размера совместно с сетевым балансировщиком, который будет равномерно распределять нагрузку по облачным ресурсам. Подробнее читайте в разделе [#T](../../../load-balancer/concepts/index.md) документации сервиса Yandex Load Balancer.
...
- Консоль управления
  ...
  1. В блоке **Базовые параметры**:
      ...
      - Выберите сервисный аккаунт из списка или создайте новый. Cервисный аккаунт должен обладать ролью `editor`, чтобы создавать, обновлять и удалять виртуальные машины в группе.
  ...
  1. В блоке **Интеграция с Load Balancer** передвиньте переключатель вправо напротив поля **Создать целевую группу**.
...
- CLI
  ...
  1. Проверьте, есть ли в каталоге сети:
      ...
      Если ни одной сети в каталоге нет, [создайте нужные сети](../../../vpc/operations/subnet-create.md) в сервисе VPC.
  ...
  1. Опишите в созданном файле:
      ...
      - Целевую группу Load Balancer:
          ...
          Ключ | Значение
          ----- | -----
          `target_group_spec` | Спецификация целевой группы Load Balancer, связанной с группой виртуальных машин.
          `name` | Произвольное имя целевой группы Load Balancer. Имя должно быть уникальным в рамках каталога. Имя может содержать строчные буквы латинского алфавита, цифры и дефисы. Первый символ должен быть буквой. Последний символ не может быть дефисом. Максимальная длина имени — 63 символа.
