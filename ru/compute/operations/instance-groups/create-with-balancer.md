...
Вы можете создать группу виртуальных машин фиксированного размера совместно с сетевым балансировщиком, который будет равномерно распределять нагрузку по облачным ресурсам. Подробнее читайте в разделе [Сетевой балансировщик нагрузки](../../../network-load-balancer/concepts/index.md) документации сервиса {{ network-load-balancer-full-name }}.
...
- Консоль управления
  ...
  1. В блоке **Шаблон виртуальной машины** нажмите кнопку **Задать**, чтобы задать конфигурацию базовой виртуальной машины:
      ...
      - В блоке **Образы из Cloud Marketplace** выберите публичный [образ](../../operations/images-with-pre-installed-software/get-list.md).
      - В блоке **Диски**:
      ...
      - В блоке **Сетевые настройки**:
          - Выберите [облачную сеть](../../../vpc/concepts/network.md#network) из списка или укажите ее идентификатор. Если сети нет, нажмите кнопку **Создать новую сеть** и создайте ее:
              - В открывшемся окне укажите имя новой сети и выберите, к какой **Подсети в зоне** необходимо подключить виртуальную машину. У каждой сети должна быть как минимум одна [подсеть](../../../vpc/concepts/network.md#subnet) (если подсети нет, [создайте ее](../../../vpc/operations/subnet-create.md)).
  ...
  1. В блоке **Интеграция с {{ network-load-balancer-name }}** передвиньте переключатель вправо напротив поля **Создать целевую группу**.
  1. Введите произвольные **Имя целевой группы** и **Описание целевой группы**.
...
- CLI
  ...
  1. Выберите один из [публичных образов](../images-with-pre-installed-software/get-list.md) (например, [CentOS 7](https://cloud.yandex.ru/marketplace/products/f2esfplfav536pn90mdo)).
  ...
  1. Опишите в созданном файле:
      ...
      - Целевую группу {{ network-load-balancer-name }}:
          ...
          Ключ | Значение
          ----- | -----
          `target_group_spec` | Спецификация целевой группы {{ network-load-balancer-name }}, связанной с группой виртуальных машин.
  ...
  1. Создайте [балансировщик](../../../network-load-balancer/operations/load-balancer-create.md) и добавьте к нему целевую группу `first-target-group`.
