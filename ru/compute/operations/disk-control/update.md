...
- {{ TF }}
  ...
  Для увеличения с помощью {{ TF }}, диск необходимо создать как отдельный ресурс. Создание диска в блоке `initialize_params` для `yandex_compute_instance` не рекомендовано.
