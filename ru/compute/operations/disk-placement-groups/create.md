...
- {{ TF }}
  ...
  1. Опишите в конфигурационном файле параметры группы размещения дисков:
     ...
     * `zone` — [зона доступности](../../../overview/concepts/geo-scope.md). {% if product == "yandex-cloud" %}Рекомендуется создавать группы размещения дисков в зонах доступности `{{ region-id }}-a` или `{{ region-id }}-b`.{% endif %}
