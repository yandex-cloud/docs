...
Эта инструкция описывает, как загрузить файл с образом диска в [!KEYREF objstorage-name] и создать из него [образ](../../concepts/image.md) в сервисе [!KEYREF compute-name]. Поддерживаемые форматы образов: Qcow2, VMDK, VHD. На данный момент нельзя загрузить свой образ с операционной системой Windows.
