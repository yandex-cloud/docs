...
> [!IMPORTANT]
>
> Включенный доступ к серийной консоли небезопасен: злоумышленники могут получить доступ к вашей виртуальной машине. [Отключайте доступ](disable.md) после работы с серийной консолью.
...
> [!NOTE]
>
> Работа серийной консоли зависит от настроек используемой операционной системы. Сервис [!KEYREF compute-full-name] обеспечивает канал между пользователем и COM-портом виртуальной машины и не гарантирует стабильность работы консоли со стороны операционной системы.

Для подключения к виртуальной машине необходимо знать ее идентификатор. Как получить идентификатор виртуальной машины читайте в разделе [[!TITLE]](../vm-info/get-info.md).
...
Пользователь `yc-user` создается автоматически при создании виртуальной машины. Подробнее читайте в разделе [[!TITLE]](../vm-create/create-linux-vm.md).
