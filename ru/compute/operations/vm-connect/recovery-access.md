...
Если вы утратили закрытый ключ SSH для Linux{% if product == "yandex-cloud" %} или пароль пользователя Windows{% endif %}:
...
1. Подключитесь к ВМ по SSH{% if product == "yandex-cloud" %} или RDP{% endif %}, используя новые учетные данные.
...
1. [Создайте](../vm-create/create-from-snapshots.md) вспомогательную ВМ на базе Linux. В качестве диска с данными используйте диск из ранее созданного снимка.
...
Проблема может возникнуть из-за ошибки в настройках SSH,{% if product == "yandex-cloud" %} RDP,{% endif %} [групп безопасности](../../../vpc/concepts/security-groups.md) или сети. Для восстановления доступа подключитесь к ВМ с помощью [серийной консоли](../serial-console/index.md) и сделайте необходимые исправления.
