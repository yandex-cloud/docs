...
### Восстановление через создание ВМ {#recovery-new-vm}
...
### Восстановление с помощью метаданных {#recovery-metadata}

Задать новый пароль пользователю можно с помощью изменения [метаданных виртуальной машины](../../concepts/vm-metadata.md).

1. [Измените метаданные](../../operations/vm-metadata/update-vm-metadata.md) через консоль управления, CLI, Terraform или API. Используйте конфигурацию:

   ```
   #cloud-config
   bootcmd:
      - [sh, -xc, "sudo echo '<имя_пользователя>:<новый_пароль>':| chpasswd"]
   ```
   
1. [Перезапустите ВМ](../../operations/vm-control/vm-stop-and-start.md#restart).

1. Подключитесь к ВМ по [SSH](../../operations/vm-connect/ssh.md) или с помощью [серийной консоли](../../operations/serial-console/connect-ssh.md), используя новый пароль.

   {% note warning %}

   При использовании [серийной консоли](../../concepts/serial-console.md) оцените риск доступа к ВМ с ее помощью. Рекомендуем разрешать доступ к серийной консоли только в случае крайней необходимости и [запрещать его](../serial-console/index.md#disable), когда нужные действия выполнены.

   {% endnote %}

1. Задайте новый пароль для пользователя. Например, в дистрибутивах Linux с помощью команды:

   ```
   sudo passwd <имя_пользователя>
   ```

1. После успешного подключения [очистите метаданные](../../operations/vm-metadata/update-vm-metadata.md), чтобы после перезапуска ВМ пароль не сбрасывался.
