...
Для ВМ с включенным [доступом по OS Login](./os-login.md) рекомендуется при создании ВМ [указывать](../../concepts/vm-metadata.md#how-to-send-metadata) SSH-ключи: так вы сможете [подключиться к ВМ по SSH](#vm-connect), даже если отключите для нее доступ по OS Login.
