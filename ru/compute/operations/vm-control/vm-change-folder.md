...
* Перед переносом виртуальную машину нужно [остановить](./vm-stop-and-start.md).
* Метрики в [{{ monitoring-full-name }}](../../../monitoring/index.yaml) не переносятся. То, что было в предыдущем каталоге, останется в нем, новые метрики будут создаваться уже в новом каталоге.
