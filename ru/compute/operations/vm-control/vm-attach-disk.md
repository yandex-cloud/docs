...
- Linux
  ...
  1. Смонтируйте разделы диска с помощью [утилиты]{% if lang == "ru" %}(https://www.opennet.ru/man.shtml?topic=mount&category=8&russian=0){% endif %}{% if lang == "en" %}(https://manpages.ubuntu.com/manpages/xenial/en/man8/mount.8.html){% endif %} `mount`. Чтобы смонтировать раздел `vdc1` в папку `/mnt/vdc1`, выполните следующую команду:
  ...
  1. Настройте разрешения на чтение и запись на диске с помощью [утилиты]{% if lang == "ru" %}(https://www.opennet.ru/man.shtml?topic=chmod&russian=0&category=&submit=%F0%CF%CB%C1%DA%C1%D4%D8+man){% endif %}{% if lang == "en" %}(https://manpages.ubuntu.com/manpages/jammy/en/man1/chmod.1.html){% endif %} `chmod`. Например, чтобы разрешить запись на диск всем пользователям, выполните следующую команду:
