...
Вы можете подключиться к виртуальной машине по протоколу SSH, когда она будет запущена. Для этого можно использовать утилиту `ssh` в Linux и Mac OS и программу [PuTTy](https://www.chiark.greenend.org.uk/~sgtatham/putty/) для Windows. Не забудьте настроить использование созданной пары ключей: закрытый ключ должен соответствовать открытому ключу, переданному на виртуальную машину.
