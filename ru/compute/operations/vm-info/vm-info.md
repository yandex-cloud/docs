...
Вы можете получить всю основную информацию о виртуальной машине в консоли управления. Чтобы получить пользовательские [метаданные](../../concepts/vm-metadata.md), воспользуйтесь CLI или API.
...
Для получения более подробной информации о виртуальной машине нажмите на строку с ее именем:
...
- На вкладке **Мониторинг** приводится информация о нагрузках на виртуальной машине. Эту информацию можно получить только в консоли управления или изнутри виртуальной машины.
- На вкладке **Вывод последовательного порта** приводится информация, которую виртуальная машина выводит в последовательный порт (serial port). Чтобы получить вывод последовательного порта через API или CLI воспользуйтесь инструкцией [[!TITLE]](get-serial-port-output.md).
...
4. Чтобы получить информацию вместе с [метаданными](../../concepts/vm-metadata.md) используйте флаг `--full`:
...
`<элемент>` — это путь к элементу, который вы хотите получить. Если элемент не задан, в ответе вернется список доступных элементов.
