...
Сервис {{ yagpt-full-name }} находится стадии [Preview](../overview/concepts/launch-stages.md).
...
* режим работы модели;
* наличие опции логирования запроса.
...
Стоимость представления текста в виде векторов (получения эмбеддингов по тексту) зависит от объема текста, поданного на векторизацию, используемой модели и наличия опции логирования запроса.
...
* 1,0 — коэфициент стоимости использования модели {{ yagpt-name }} Lite в синхронном режиме.
...
* 2,5 — коэфициент стоимости использования модели {{ yagpt-name }} в асинхронном режиме.
...
* 1,0 — коэфициент использования векторизации текста с включенным логированием.
* 0,01&nbsp;₽ — стоимость обработки 1000 токенов. 
* 0,01&nbsp;₽ / 1000 — стоимость обработки 1 токена. 


> Стоимость использования {{ yagpt-name }} для векторизации текста со следующими параметрами:
> * число токенов в запросе — 500
> * логирование выключено
> 
> Расчет стоимости:
> 
> 500 × 2,0 × (0,01&nbsp;₽ / 1000) = 0,01&nbsp;₽
>
> Итого: 0,01&nbsp;₽.
 
Где:
* 500 — число токенов в запросе.
* 2,0 — коэфициент использования векторизации текста с выключенным логированием.
* 0,01&nbsp;₽ — стоимость обработки 1000 токенов. 
* 0,01&nbsp;₽ / 1000 — стоимость обработки 1 токена. 
