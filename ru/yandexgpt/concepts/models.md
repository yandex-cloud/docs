...
{{ yagpt-name }} предоставляет доступ к двум большим текстовым генеративным нейросетям: стандартной YandexGPT Lite, подходящей для решения задач в режиме реального времени, и большой YandexGPT, которая дает более точные ответы на сложные запросы.

Если стандартных моделей вам недостаточно, вы можете дообучить YandexGPT Lite в сервисе [{{ ml-platform-full-name }}]({{ link-datasphere-main }}), чтобы модель лучше разбиралась в вашей области знаний. 

Кроме того, {{ yagpt-name }} позволяет получать эмбеддинги текста с помощью специальных моделей  векторного представления текста.
...
|---|---|---|
| YandexGPT | `gpt://<идентификатор_каталога>/yandexgpt/latest` | Асинхронный, синхронный ^1^ |
| YandexGPT Lite | `gpt://<идентификатор_каталога>/yandexgpt-lite/latest` | Асинхронный, синхронный |
