...
|| 31 | В запросе передан идентификатор каталога (поле `folderid`), пользователь которого не прошел регистрацию в сервисе {{ search-api }}. ||
...
|| 33 | IP-адрес, с которого отправлен поисковый запрос, не совпадает с заданным(и) при регистрации. ||
