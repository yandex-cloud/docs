...
Кроме того, {{ search-api-name }} предоставляет инструмент для получения статистики поисковых запросов [{{ wordstat-name }}](./wordstat.md). {{ wordstat-name }} работает только через API в синхронном режиме. Веб-версия инструмента доступна на сайте [Вордстат](https://wordstat.yandex.ru/).
