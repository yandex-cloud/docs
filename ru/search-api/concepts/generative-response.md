...
В настоящий момент генеративный ответ доступен только для запросов, выполненных через интерфейс [API v1](./index.md#api-v1). По умолчанию можно отправить не более одного синхронного запроса в секунду для получения генеративного ответа. Подробнее об ограничения {{ search-api-name }} см. в разделе [{#T}](./limits.md).
