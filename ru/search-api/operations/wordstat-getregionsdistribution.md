...
{{ wordstat-name }} позволяет получить распределение числа запросов, содержащих заданную фразу, по регионам мира за последние 30 дней с помощью вызова [GetRegionsDistribution](../api-ref/grpc/Wordstat/getRegionsDistribution.md). 
...
- gRPC API {#grpc-api}
  ...
  1. Выполните gRPC-вызов, указав полученный ранее API-ключ или IAM-токен и путь к файлу с телом запроса:
      ...
      * `affinityIndex` — [индекс соответствия](https://ru.wikipedia.org/wiki/Индекс_соответствия) — отношение доли запросов в регионе к их доле по стране.
      ...
      {
        "results": [
          { "region": "213", "count": "3200", "share": 0.034, "affinityIndex": 1.8 },
          { "region": "2",   "count": "1500", "share": 0.016, "affinityIndex": 1.2 }
        ]
      }
      ```
