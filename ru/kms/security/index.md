...
Примитивные роли – общие для всех ресурсов Яндекс.Облака. Назначение любой из них дает некоторые разрешения как в {{ kms-short-name }}, так и в других облачных сервисах. Например, роль `admin`, назаченная на каталог, позволяет удалять любые ключи {{ kms-short-name }} в каталоге, а также дает возможность изменять сам каталог или ресурсы в нем. Управлять примитивными ролями можно с помощью консоли управления.
...
Хорошей практикой является хранение ключей {{ kms-short-name }} в выделенном каталоге, отдельно от других ресурсов Яндекс.Облака.
...
* [Безопасное использование Яндекс.Облака](../../iam/best-practices/using-iam-securely.md)
