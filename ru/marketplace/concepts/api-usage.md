...
Спецификация API выставления счетов по тарифу приведена в разделе [{#T}](../metering/api-ref/index.md).
...
* Перед публикацией продукта рекомендуем тестировать интеграцию с API с помощью ID и SKU демонстрационного продукта: `productId: {{ mp-test-product-id }}`, `skuId: {{ mp-test-sku-id }}`. Примеры работы с API приведены в разделе [{#T}](../api-ref/quickstart.md).
