...
* Образ должен поддерживать загрузку в режиме Legacy. Большинство дистрибутивов поддерживают этот режим по умолчанию.
* Образ должен поддерживать загрузку в режиме UEFI, например:

   * Для семейства Debian добавьте пакеты: `grub-efi-amd64-bin` и `dosfstools`.
   * Для семейства RedHat добавьте пакеты: `grub2-efi-x64` и `shim-x64`.

* Образ должен поддерживать агрегацию сетевых интерфейсов по протоколу LACP (IEEE 802.3ad).
