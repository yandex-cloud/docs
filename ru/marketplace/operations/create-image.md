...
Чтобы добавить в Marketplace продукт для {{ compute-full-name }}, нужно загрузить образ в {{ yandex-cloud }}. Продукты могут быть созданы на базе ОС Linux и Windows. Эта инструкция поможет создать образ на базе ОС Linux. Чтобы создать образ с ОС Windows, см. [{#T}](create-image-ms.md). 
