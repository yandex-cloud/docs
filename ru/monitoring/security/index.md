...
Чтобы разрешить доступ к ресурсам сервиса Yandex Monitoring,
...
Пользователь с ролью `viewer` может просматривать созданные дашборды и виджеты, а также загруженные метрики.

#### editor

Пользователь с ролью `editor` может создавать дашборды и виджеты, загружать метрики.

Помимо этого роль `editor` включает в себя все разрешения роли `viewer`.

#### admin

Пользователь с ролью `admin` может создавать дашборды и виджеты, загружать метрики.
