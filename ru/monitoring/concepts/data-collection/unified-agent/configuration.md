...
Для использования файла конфигурации необходимо передать путь к нему в параметр командной строки `--config` при запуске агента:
```bash
...
#### Вход prometheus_pull { #prometheus_pull_input }
