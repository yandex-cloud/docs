...
Для поставки пользовательских метрик в {{monitoring-full-name}} предоставляется {{unified-agent-full-name}} — агент для поставки метрик виртуальных машин и пользовательских приложений. Агент поддерживает сбор метрик в формате [Prometheus](https://prometheus.io) и отправку их в {{monitoring-full-name}}.
