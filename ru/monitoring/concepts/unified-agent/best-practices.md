...
Для надежной доставки метрик в {{ monitoring-full-name }} при помощи {{unified-agent-short-name}} рекомендуется использовать хранилище, которое позволяет буферизировать сообщения, проходящие по [цепочке преобразований](./concepts.md#pipes) перед отправкой их в выход канала.
