...
С 1 октября 2020 года в сервисе {{ monitoring-short-name }} будут действовать [новые тарифы](#prices).
...
----- | -----
Запись значений метрик ресурсов {{ yandex-cloud }} | 0,00 ₽
Запись значений пользовательских метрик через API<br/>(до 50 млн значений) | 7,00 ₽
Запись значений пользовательских через API<br/>(сверх 50 млн значений) | 4,00 ₽

C 1 октября 2020 года начнут действовать следующие тарифы:

Услуга | Цена за 1 млн значений <br>вкл. НДС
----- | -----
Чтение значений любых метрик через API<br/>(до 50 млн значений) | 5,00 ₽
Чтение значений любых метрик через API<br/>(сверх 50 млн значений) | 3,00 ₽
...
Например, вы настроили экспорт 100 метрик из {{ monitoring-short-name }} в собственную инсталляцию системы мониторинга Prometheus с периодом сбора **15 секунд**. Тогда за 30 дней будет прочитано:
