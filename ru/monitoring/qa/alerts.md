...
В [канале уведомлений](../concepts/alerting.md#channel-parameters) {% if product == "cloud-il" %}поддерживается только отправка уведомлений на электронную почту.{% endif %}{% if product == "yandex-cloud" %}поддерживаются следующие методы:
* email;
* SMS;
