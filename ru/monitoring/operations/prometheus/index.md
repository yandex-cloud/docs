...
Агрегация | Поддерживается агрегация с помощью [recording rules](https://prometheus.io/docs/prometheus/latest/configuration/recording_rules/). | Поддерживаются существующие файлы recoriding rules в формате YAML. Загрузка файлов и управление ими доступны через консоль или API.
...
* Не поддерживаются `alerting rules`.
