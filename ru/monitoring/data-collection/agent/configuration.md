...
В метаданные сессии добавляется ключ _metrics_format со значением spack.
В дальнейшем значение этого ключа будет использоваться для десериализации меток из тела сообщений.

{% note info %}

Фильтр не нужен для плагинов *system_metrics* и *agent_metrics*, только для пулла
