...
Дообучение доступно только для модели `premium` для всех способов распознавания речи.
...
Если новая версия модели после дообучения удовлетворяет требованиям метрик оценки качества, она будет подготовлена к релизу в статусе `premium:rc`. 

### Сроки готовности модели

Изменения поступают в модель `premium:rc` в течение 4 недель по стандартному циклу подготовки релиза.