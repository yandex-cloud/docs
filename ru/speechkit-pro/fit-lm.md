...
* `/raw_data/references/` — каталог с текстами аудиоразметки. Текст в каждом файле содержит расшифровку речи на соответствующей записи из каталога `/raw_data/audio/`.
    ...
    Аудиоразметка — это особый способ расшифровки аудиозаписей с оформлением результатов в текстовом виде. Воспользуйтесь [специальным сервисом](markup.md), предоставляемым SpeechKit Pro, или сформируйте их самостоятельно.
...
1. Выделите все ячейки с кодом в разделе **Подготовка текстов**, удерживая _Shift_ и нажимая слева от нужных ячеек:
    ...
    ...
    ```
1. Запустите выбранные ячейки, выбрав в меню **Run → Run Selected Cells** (также можно использовать сочетание клавиш _Shift+Enter_).
...
Подробнее об оценке качества моделей см. [{#T}](estimate-quality.md).
