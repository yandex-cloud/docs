---
title: Вычисление метрик по логам
description: Метрики по логам — непрерывное вычисление статистики по данным в логах с записью результатов в виде метрик в {{ monium-name }}.
---
...
_Метрика по логам_ — непрерывное вычисление статистики по данным из логов для создания метрик в {{ monium-name }}.
...
Адрес для записи логов через API: `{{ api-host-monium }}:443`.
...
- Интерфейс {{ monium-name }} {#console}
  ...
  1. На главной странице [{{ monium-name }}]({{ link-monium }}) слева выберите один из разделов:
     * ![alt](../../_assets/console-icons/compass.svg) **{{ ui-key.yacloud_monitoring.aside-navigation.all-panel.menu.category.explore }}** → **{{ ui-key.yacloud_monitoring.aside-navigation.menu-item.logs.title }}**. Нажмите кнопку **{{ ui-key.yacloud_monitoring.logs.logs-metrics.title }}**.
     * ![alt](../../_assets/console-icons/box.svg) **Поставка и хранение** → **{{ ui-key.yacloud_monitoring.aside-navigation.menu-item.logs-metrics.title }}**.
  1. Справа вверху нажмите **Создать**.
  1. (Опционально) Введите название метрики.
  1. Введите **ID** метрики — значение имени метрики. Обычно это `name`, но может использоваться и другая метка, например `sensor` или `signal`. Это имя задается в настройках шарда. Подробнее в разделе [{#T}](../concepts/querying.md).
  1. В блоке **{{ ui-key.yacloud_monitoring.logs-metrics.form.block.log-selector }}** введите запрос, выбирая метки из списка или в текстовом режиме. Запрос определяет, какие строки логов участвуют в вычислении метрики.
  1. В блоке **Правило вычисления** укажите:
     * **{{ ui-key.yacloud_monitoring.logs-metrics.form.row.aggregate-function-key-value }}** — `count`, `sum`, `min`, `max`, `avg`, `unique` или `uniqueNewfound`.
     * **{{ ui-key.yacloud_monitoring.logs-metrics.form.row.window }}** — `1 минута` или `5 минут`.
  1. Если нужно разбить метрику по значениям атрибута, в блоке **{{ ui-key.yacloud_monitoring.logs-metrics.form.block.group-by }}** введите имя атрибута.
  1. В блоке **{{ ui-key.yacloud_monitoring.logs-metrics.view.field.output-metric }}** проверьте автоматически сформированный запрос и при необходимости уточните значения меток `cluster` и `service`.
  1. Нажмите **Создать**.

  Чтобы создать метрику по уже построенному графику логов:

  1. На главной странице [{{ monium-name }}]({{ link-monium }}) слева выберите **Обзор** → **{{ ui-key.yacloud_monitoring.aside-navigation.menu-item.logs.title }}**.
  1. Введите запрос и нажмите **{{ ui-key.yacloud_monitoring.querystring.action.execute-query }}**.
  1. На вкладке **Статистика** рядом с графиком нажмите **{{ ui-key.yacloud_monitoring.logs.logs-metrics.create }}**.
...
* **{{ ui-key.yacloud_monitoring.logs-metrics.form.row.window }}**: `1 минута`
...
* **{{ ui-key.yacloud_monitoring.logs-metrics.form.block.group-by }}**: —
* **{{ ui-key.yacloud_monitoring.logs-metrics.form.row.window }}**: `5 минут`
...
* **{{ ui-key.yacloud_monitoring.logs-metrics.form.block.log-selector }}** — какие логи попадут в выборку.
* **{{ ui-key.yacloud_monitoring.logs-metrics.form.row.aggregate-function-key-value }}** — функция, с помощью которой вычисляется значение метрики: `count`, `min`, `max`, `avg`, `sum`, `unique`, `uniqueNewfound`.
...
* **{{ ui-key.yacloud_monitoring.logs-metrics.form.block.group-by }}** — по какому атрибуту создавать отдельные агрегаты для каждого значения.
* **{{ ui-key.yacloud_monitoring.logs-metrics.form.row.window }}** — временной диапазон для вычисления функции агрегации. Поддерживаются диапазоны: `1 минута` и `5 минут`.
* **{{ ui-key.yacloud_monitoring.logs-metrics.view.field.output-metric }}** — с каким селектором записать метрику в {{ monium-name }}.
...
В селекторе должны быть обязательные метки: `project`, `cluster`, `service` и `ID` метрики — метка, которая обозначает имя метрики в проекте или указанном шарде. Обычно это `name`, но может использоваться `sensor`, `signal` или другое произвольное имя метки. Подробнее в разделе [{#T}](../concepts/querying.md). Например:
...
Чтобы увеличить квоту, обратитесь в [службу поддержки]({{ link-console-support }}).
...
При поступлении в {{ monium-name }} двух строк лога выполняются действия:
...
* Все метрики записываются с типом [DGAUGE](../concepts/data-model#metric-types).
...
* Точки метрики за последние 30 минут могут быть пересчитаны и повторно отправлены в {{ monium-name }}. Если в шарде настроена агрегация по метрикам, это может привести к некорректному отображению значений.
...
Функции `avg`, `unique` и `uniqueNewfound` несовместимы с агрегацией в метриках {{ monium-name }}. Например, для `unique`:
