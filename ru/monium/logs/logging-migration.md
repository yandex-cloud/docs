...
[Чтение через API {{ cloud-logging-name }}](../../logging/api-ref/grpc/LogReading/read.md) будет поддерживаться до 1 марта 2027 года, если сохранена структура `folder__<идентификатор_каталога>` → `default` → `имя лог-группы`. Данные доступны в пределах срока хранения — 31 день. Переведите запросы на чтение на новый API до окончания поддержки старого API.