...
## Результаты поиска LLM-спанов {#search-results}

В режиме поиска спанов в результатах отображаются только спаны, связанные с работой AI-агентов. Такие спаны определяются по наличию атрибутов с префиксом `gen_ai.*` — в соответствии со стандартом OpenTelemetry. Для каждого такого спана в выдаче показывается:
...
## Страница LLM-трейса {#page-layout}

На странице просмотра отображаются только спаны тех сервисов, в которых есть атрибуты с маской `gen_ai.*`. Спаны остальных сервисов (бэкенды, базы данных и т. п.) на этой странице скрыты — так интерфейс фокусируется на диалоге с моделью. Чтобы увидеть полный трейс со всеми спанами, нажмите **Показать полный трейс** в верхней части страницы — откроется [стандартный просмотр трейса](../operations/trace-view.md).
...
* **Диалог** — история сообщений между пользователем и агентом: входные сообщения (промпты пользователя, результаты вызова инструментов) и ответ модели. Если часть ответа модели размечена как [рассуждения](manual_instrumentation.md#llm-reasoning-in-spans), она выделяется меткой «Размышления».
* **Доступные инструменты** — в разделе [Доступные инструменты](#tools).

Сообщения в блоке «Диалог» выводятся в хронологическом порядке. Роль (user, assistant, system, tool) и тип части сообщения (text, reasoning, tool_call, tool_call_response) отображаются явно, чтобы можно было быстро проследить цепочку: запрос пользователя → ответ модели с вызовом инструмента → результат инструмента → следующий запрос к модели → финальный ответ.
...
По этому блоку видно, какой набор инструментов видел агент в момент вызова. Если модель вызвала один из инструментов, соответствующий дочерний спан с типом операции `execute_tool` отображается ниже спана генерации; в нем видны параметры вызова и результат (атрибуты `gen_ai.tool.call.arguments` и `gen_ai.tool.call.result`). Необходимые атрибуты для отображения инструментов приведены в разделе [«Спаны вызова инструментов»](manual_instrumentation.md#tool-spans).
