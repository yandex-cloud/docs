...
По этому блоку видно, какой набор инструментов видел агент в момент вызова. Если модель вызвала один из инструментов, соответствующий дочерний спан с типом операции `execute_tool` отображается ниже спана генерации; в нем видны параметры вызова и результат (атрибуты `gen_ai.tool.call.arguments` и `gen_ai.tool.call.result`). Подробнее о том, какие атрибуты нужны для отображения инструментов, см. в разделе [«Спаны вызова инструментов»](manual_instrumentation.md#tool-spans).
