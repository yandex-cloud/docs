...
### Рассуждения модели в спанах {#llm-reasoning-in-spans}

В режиме рассуждений (reasoning) модель строит ответ на основе цепочки рассуждений. Чтобы просматривать эту цепочку отдельно от финального ответа в истории диалога, разметьте ее как часть сообщения с типом `reasoning`. В спане агента эта часть будет выделена меткой ![cpu](../../../_assets/console-icons/cpu.svg) **Размышления**.

#### Формат части сообщения с рассуждениями {#reasoning-part-format}

Чтобы разметить рассуждения, в атрибуте `gen_ai.output.messages` передайте [сообщение](#messages-format) с `role == "assistant"`. В массив `parts` добавьте отдельный элемент:

```json
{
  "type": "reasoning",
  "content": "<текст_рассуждений>"
}
```

Оба поля обязательны:

* `type` — тип части сообщения, всегда `reasoning`.
* `content` — текст рассуждений модели.

В одном сообщении можно передать рассуждения, текст ответа и вызов инструмента отдельными элементами массива `parts`. В интерфейсе все части сообщения отображаются в порядке, в котором они указаны в массиве `parts`.

#### Пример сообщения с рассуждениями {#reasoning-message-example}

Пример значения атрибута `gen_ai.output.messages` с рассуждениями и финальным ответом:
...
[
  {
    ...
    "parts": [
      {
        "type": "reasoning",
        "content": "**Проверка наличия таблицы**\n\nПроверка показала, что таблица `monthly_sales` отсутствует в схеме `reports` базы данных `analytics`. Перед загрузкой отчета нужно создать таблицу."
      },
      {
        "type": "text",
        "content": "Перед загрузкой отчета создайте таблицу `reports.monthly_sales` в базе данных `analytics`."
      }
    ],
    "finish_reason": "stop"
  }
...
Одно сообщение может содержать несколько частей разных типов, например текст и изображение. В интерфейсе {{ monium-name }} все части отобразятся в спане в той последовательности, в которой они указаны в массиве `parts`. При этом изображения автоматически отображаются внутри спана в виде превью в тот момент, когда соответствующий спан попадает в область видимости на экране. Чтобы открыть полноразмерную версию изображения, нажмите на превью.

#### Требования к ссылке на изображение {#image-link-format}

{{ monium-name }} не проксирует изображения, передаваемые в LLM-трейсах. Браузер пользователя загружает их напрямую из источника, указанного в ссылке. Поэтому URL изображений должны соответствовать следующим требованиям:

* **Схема URL**: допускается только `https://`.

    Ссылки со схемой `http://` браузер заблокирует как `mixed content`. Ссылки на изображения со схемой `data:` и кодированием [Base64]({{ link-base64 }}) не поддерживаются `{{ monium-name }}`.

    {% note tip %}

    Получить ссылку со схемой `https://` вы можете, загрузив изображение в [бакет](*buckets) {{ objstorage-full-name }} или на [ресурс](*cdn_resource) {{ cdn-full-name }}.

    {% endnote %}

* **Доступность из браузера пользователя**: если браузер не имеет доступа к изображению, изображение не отобразится в спане {{ monium-name }}. Например, ссылка на ресурс в интранете не откроется в браузере компьютера, не подключенного к этому интранету.
...
[*buckets]: Бакет — это часть хранилища {{ objstorage-full-name }}, выделенная для данных пользователя. Подробнее читайте в разделе [{#T}](../../../storage/concepts/bucket.md).

[*cdn_resource]: Ресурс — это основная логическая сущность сервиса {{ cdn-full-name }}, позволяющая настраивать и управлять распространением контента из источников через точки присутствия. Подробнее читайте в разделе [{#T}](../../../cdn/concepts/resource.md).

[*signed_url]: Подписанный URL — это URL, содержащий в своих параметрах данные для авторизации запроса. Подробнее о реализации подписанных ссылок в сервисе {{ objstorage-full-name }} читайте в разделе [{#T}](../../../storage/concepts/pre-signed-urls.md).

[*cors_cdn]: {{ cdn-full-name }} поддерживает кросс-доменные запросы по механизму CORS (cross-origin resource sharing). Подробнее читайте в разделе [{#T}](../../../cdn/concepts/cors.md).

[*cors]: CORS — это стандарт, позволяющий предоставлять веб-страницам доступ к объектам на сторонних интернет-ресурсах. Сторонним считается любой интернет-ресурс, который отличается от текущего схемой, доменом или портом. Подробнее читайте в разделе [{#T}](../../../glossary/cors.md).