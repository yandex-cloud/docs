...
{{ monium-name }} использует единый язык запросов для поиска и фильтрации данных телеметрии: метрик, логов и трейсов. С помощью языка запросов можно строить выражения-селекторы, применять функции для преобразования метрик и ссылаться на результаты других запросов как на переменные.

Описание модели данных, включая обязательные атрибуты, метки и метаинформацию, — в разделе [{#T}](data-model.md). Объекты конфигурации (проект, кластер, сервис), которые используются в запросах, — в разделе [{#T}](configuration-model.md).
...
Для поиска по меткам, хранящимся в метаинформации `meta`, указывайте полный квалификатор `meta.label_name`. Доступны те же операторы, что и для меток верхнего уровня, во всех типах телеметрии. Подробнее о метках и метаинформации — в разделе [{#T}](data-model.md).
...
Поиск трейсов описан в разделе [{#T}](../traces/operations/traces-explorer.md).
...
Высококардинальные метки, в том числе идентификаторы, рекомендуется хранить в метаинформации `meta`. Подробнее о различии между метками и метаинформацией — в разделе [{#T}](data-model.md#meta).
