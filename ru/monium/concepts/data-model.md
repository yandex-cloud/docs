...
В сервисе {{ monium-name }} действует единая модель данных для работы с телеметрией: метриками, логами и трейсами. Данные идентифицируются с помощью [объектов конфигурации](configuration-model.md) (проект, кластер, сервис) и разделяются на метки и метаинформацию.

В этом разделе описана организация данных и особенности каждого вида телеметрии. Поиск по данным — в разделе [{#T}](querying.md).
...
Обязательные атрибуты телеметрии (подробнее об объектах конфигурации — в разделе [{#T}](configuration-model.md)):
...
* метрика может иметь не более {{ labels_per_mtetric_limit }} меток, включая обязательные `project`, `cluster`, `service`;
