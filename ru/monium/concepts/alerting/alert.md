...
### Фиксация значений меток {#label-fixing}

Группировка не применяется к селекторам с условиями:
* Точное совпадение: `label=="value"`
* Отсутствие значения: `label="-"`

Это позволяет создать мультиалерт, где метрика каждого хоста делится на общую метрику для всех хостов (например, агрегат по кластеру).

**Пример использования фиксации метки `host`:**

```js
// Пример 1: фиксация через точное совпадение
let used = {name=quoteUsed, host='*'};
let limit = {name=quoteLimit, host=='cluster'};
let percent = last(used / limit);

// Пример 2: фиксация через отсутствие метки
let used = {name=quoteUsed, host='*'};
let limit = {name=quoteLimit, host='-'};
let percent = last(used / limit);
```

В переменную `used` подставляется каждое значение метки `host`, а значение переменной `limit` остается одинаковым для всех под-алертов.
