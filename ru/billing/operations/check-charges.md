...
- По меткам {#labels}
  ...
  1. Уточните критерии детализации: 
      ...
      * В списках **{{ ui-key.yacloud.billing.account.detail.value_folders }}** или **{{ ui-key.yacloud.billing.accounts.column_cloud }}** снимите отметки с элементов, данные по которым не нужно отображать.
...
- **{{ ui-key.yacloud.billing.account.detail.label_top-cost }}** — показывает стоимость потребленных услуг без учета скидки.
- **{{ ui-key.yacloud.billing.account.detail.label_top-total }}** — показывает стоимость потребленных услуг с учетом скидки.
