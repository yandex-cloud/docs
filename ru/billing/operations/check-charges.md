...
Скидка | {% if product == "yandex-cloud" %}Размер скидки в валюте договора (без учета НДС или иных налогов и сборов).{% endif %}{% if product == "cloud-il" %}Показатель не действует, поскольку на стадии Preview сервисы {{ yandex-cloud }} не тарифицируются. Из-за этого значения показателей **Общая стоимость** и **Итого** совпадают.{% endif %}
