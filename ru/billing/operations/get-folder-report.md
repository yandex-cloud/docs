...
- Консоль управления
  ...
  1. В открывшемся окне **Создание регулярного экспорта** укажите:
     ...
     * В поле **Папка** название папки для файла. Последний символ должен быть `/`.
     {% if product == "yandex-cloud" %}* Выберите язык, на котором будут отображаться названия продуктов – английский или русский.{% endif %}
...
* `currency` — валюта платежного аккаунта. {% if product == "yandex-cloud" %}Возможные значения: `RUB`, `USD` и `KZT`.{% endif %}{% if product == "cloud-il" %}Принимает значение `ILS`.{% endif %}
