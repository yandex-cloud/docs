---
title: "Инструкции по работе с сервисом Billing"
description: "Из статьи вы узнаете, как работать с сервисом {{ billing-name }} в {{ yandex-cloud }}, как активировать платную версию, зарезервировать потребление (CVoS), активировать промокод, пополнить лицевой счет, и многое другое."
---

# Все инструкции для сервиса {{ billing-name }}


## Работа с документами {#docs}

* [{#T}](spell-out-bill.md)
* [{#T}](download-reporting-docs.md)
* [{#T}](edo.md)


## Управление платежным аккаунтом {#account}

* [{#T}](create-new-account.md)
* [{#T}](change-data.md)
* [{#T}](get-data.md)
* [{#T}](pin-cloud.md)
* [{#T}](delete-account.md)
* [{#T}](activate-commercial.md)

## Управление способами оплаты {#payment-methods}

* [{#T}](pin-card.md)
* [{#T}](change-payment-method.md)

## Оптимизация расходов {#cost-optimization}

* [{#T}](budgets.md)
* [{#T}](purchase-cvos.md)

## Оплата услуг {#payment}

* [{#T}](activate-promocode.md)
* [{#T}](pay-the-bill.md)

## Анализ использования сервисов {{ yandex-cloud }} {#details}

* [{#T}](check-bill-history.md)
* [{#T}](check-charges.md)
* [{#T}](get-folder-report.md)
* [{#T}](dashboard.md)
* [{#T}](query-integration.md)