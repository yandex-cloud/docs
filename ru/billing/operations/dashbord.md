# Посмотреть детализацию использования сервисов в {{ datalens-full-name }}

Вы можете посмотреть сведения по использованию сервисов {{ yandex-cloud }}, собрав дашборд в {{ datalens-full-name }}.

## Cоздайте подключение {#create-connection}

{% list tabs %}

- Консоль управления

  1. Перейдите на [страницу подключений](https://datalens.yandex.ru/connections).
  1. Нажмите кнопку **Создать подключение**.
  1. Выберите коннектор **Yandex Cloud Billing**.
  1. Задайте **Название подключения**. Название может быть произвольным.
  1. В правом верхнем углу в выпадающем списке выберите, кому будет доступен дашборд — только вам или компании.
  1. Нажмите **Создать**.

{% endlist %}

## Откройте дашборд {#open-dashboard}

Перейдите на вкладку **Все объекты** и откройте дашборд **Yandex Cloud Billing Dashbord**. Если необходимо, вы можете изменить настройки дашборда. Подробнее о [работе с дашбордами](../../datalens/concepts/dashboard.md).

Дашборд регулярно обновляется новыми данными. Информация за любой день может быть изменена в течение месяца.
