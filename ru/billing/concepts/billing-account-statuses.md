...
На стадии [Preview](../../overview/concepts/launch-stages.md) сервисы {{ yandex-cloud }} не тарифицируются, поэтому статусы платежного аккаунта носят ознакомительный характер.
...
- `PAYMENT_NOT_CONFIRMED` —  требуется подтверждение аккаунта. {% if product == "yandex-cloud" %}На вашу почту, указанную в аккаунте Яндекса или Яндекс 360, отправлено письмо с описанием дальнейших действий.{% endif %}
