---
title: Управление доступом к кластеру {{ metastore-full-name }}
description: Следуя данной инструкции, вы настроите права доступа к кластеру {{ metastore-name }}.
---
...
- CLI {#cli}
  ...
  1. Назначьте роль, выполнив команду:
      ...
      * `--role` — назначаемая [роль](../../security/metastore-roles.md), например `managed-metastore.editor`.
...
- REST API {#api}
  ...
  1. Воспользуйтесь методом [Cluster.UpdateAccessBindings](../../api-ref/Cluster/updateAccessBindings.md) и выполните запрос, например, с помощью {{ api-examples.rest.tool }}:
      ...
      * `access_binding_deltas.roleId` — назначаемая [роль](../../security/metastore-roles.md), например `managed-metastore.editor`.
...
- REST API {#api}
  ...
  1. Воспользуйтесь методом [Cluster.SetAccessBindings](../../api-ref/Cluster/setAccessBindings.md) и выполните запрос, например, с помощью {{ api-examples.rest.tool }}:
      ...
      * `accessBindings.roleId` — назначаемая [роль](../../security/metastore-roles.md), например `managed-metastore.editor`.
...
- CLI {#cli}
  ...
  1. Отзовите роль, выполнив команду:
      ...
      * `--role` — отзываемая [роль](../../security/metastore-roles.md), например `managed-metastore.editor`.
...
- REST API {#api}
  ...
  1. Воспользуйтесь методом [Cluster.UpdateAccessBindings](../../api-ref/Cluster/updateAccessBindings.md) и выполните запрос, например, с помощью {{ api-examples.rest.tool }}:
      ...
      * `access_binding_deltas.roleId` — отзываемая [роль](../../security/metastore-roles.md), например `managed-metastore.editor`.
                  ...
                  "access_binding_deltas": [
                    {
                      "action": "ADD",
                      "access_binding": {
                        "role_id": "managed-metastore.viewer",
                        "subject": {
                  ...
                  "access_binding_deltas": [
                    {
                      "action": "ADD",
                      "access_binding": {
                        "role_id": "managed-metastore.editor",
                        "subject": {
                          "id": "<идентификатор_сервисного_аккаунта>",
                          "type": "serviceAccount"
                        }
