---
title: Подготовка бэкенда для источников данных
description: Следуя данной инструкции, вы сможете настроить бэкенд для источников метаданных в {{ data-catalog-full-name }}.
---

# Подготовка бэкенда для источников данных

{{ data-catalog-name }} использует подключения к [источнику](../../concepts/data-catalog-sources.md) для чтения технических метаданных — определений таблиц, представлений (VIEW), материализованных представлений, процедур, индексов и информации из системных таблиц источника. Чтобы получить расширенные статистические оценки данных, {{ data-catalog-name }} может выполнять запросы на чтение данных или срез данных.

Для корректного и полного чтения метаданных нужно настроить бэкенд источника {{ data-catalog-name }}, т. е. параметры и доступы на уровне БД или другого сервиса, из которого загружаются метаданные.
...
Чтобы {{ data-catalog-name }} выгружал максимально полные метаданные, настройте дополнительные права, доступы и параметры БД:
...
1. Выдайте созданному пользователю доступ ко всем объектам, которые нужно каталогизировать. Если у технического пользователя нет доступа к схеме, таблице, представлению или индексу, {{ data-catalog-name }} может не выгрузить метаданные этого объекта или выгрузить их не полностью.
...
Настройте бэкенд для загрузки [метаданных источника {{ PG }}](../../concepts/data-catalog-sources.md#source-pg):
...
1. В каждой базе, для которой выполняется профилирование данных:
   ...
   1. Примените настройки {{ PG }}:

      * `shared_preload_libraries = 'pg_stat_statements'` — после изменения перезапустите {{ PG }}.
...
Настройте бэкенд для загрузки [метаданных источника {{ MY }}](../../concepts/data-catalog-sources.md#source-my):
...
Для отображения информации о хранимых процедурах в каталоге пользователь должен иметь доступ к метаданным процедур. Обычно достаточно доступа к базе, но в {{ mmy-full-name }} права могут быть ограничены политиками сервиса. Проверьте, что пользователю видны процедуры:
...
Настройте бэкенд для загрузки [метаданных источника {{ CH }}](../../concepts/data-catalog-sources.md#source-ch):
...
1. Если используются системные словари {{ CH }}, также выдайте к ним доступ:

   ```sql
   GRANT SELECT ON system.dictionaries TO data_catalog_reader;
   ```

Если используется RBAC и профили {{ CH }}, можно дополнительно ограничить пользователя режимом «только чтение». При этом будут разрешены запросы на чтение, включая `SELECT`, `SHOW`, `DESCRIBE`, `EXISTS`. Такие команды считаются эквивалентными чтению из системных таблиц.


### {{ OS }}

Настройте бэкенд для загрузки [метаданных источника {{ OS }}](../../concepts/data-catalog-sources.md#source-os):
...
Настройте бэкенд для загрузки [метаданных источника {{ GP }}](../../concepts/data-catalog-sources.md#source-gp):
...
1. Для анализа наиболее частых запросов могут потребоваться дополнительные права. Например:
   ...
   Доступность `pg_stat_statements` и роли `pg_read_all_stats` зависит от версии {{ GP }} и настроек кластера.

   {% endnote %}

1. Чтобы {{ data-catalog-name }} автоматически отображал бизнес-описания, добавьте комментарии к таблицам и столбцам. Например:
...
Настройте бэкенд для загрузки [метаданных источников {{ SD }}/{{ MG }}](../../concepts/data-catalog-sources.md#source-mg):
...
|| {{ PG }}, {{ GP }} | `CONNECT`, `USAGE`, `SELECT` на схемы и таблицы | Доступ к определениям представлений
|
* Расширение `pg_stat_statements`.
* Роль `pg_read_all_stats`.
* Версия {{ PG }} 13 и выше | `SELECT` на таблицы ||
|| {{ MY }} | `SELECT` на БД | `SHOW VIEW` | Не поддерживается | `SELECT` на таблицы ||
|| {{ CH }} | `SELECT` на БД и системные таблицы
|
...
|| {{ OS }} | Чтение индексов и метаданных | Не применимо | Не поддерживается | Доступ на чтение индексов ||
|| {{ SD }}/{{ MG }} | Чтение БД | Настройка не требуется | Не поддерживается | Не поддерживается ||
|| {{ data-transfer-full-name }}, {{ websql-name }} | Настройка не требуется | Не поддерживается | Не поддерживается | Не поддерживается ||
|| {{ datalens-name }} | Для сервисного аккаунта источника данных необходим доступ к нужным дашбордам, датасетам, чартам и отчетам | Не поддерживается | Не поддерживается | Не поддерживается ||
|#
