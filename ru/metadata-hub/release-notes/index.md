...
* В [{{ connection-manager-name }}](../concepts/connection-manager.md) добавлена возможность управлять подключениями с пользовательскими инсталляциями {{ KF }} и кластерами {{ mkf-name }}, а также поддержаны [шардированные кластеры](../../managed-clickhouse/concepts/sharding.md) в {{ CH }}.
