...
Теперь вы можете загружать метаданные в {{ data-catalog-name }} из источников {{ TR }} и {{ AF }}. Из {{ TR }} собираются метаданные о структуре данных: каталоги, схемы, таблицы, колонки с типами, партиции, статистика таблиц, а также история запросов. Из {{ AF }} собираются метаданные об оркестрации: структура DAG-файлов и задач, их зависимости, расписание, статусы выполнения, а также lineage-события (через `OpenLineage`).
...
Поддержаны новые типы источников данных: {{ TR }} и {{ AF }}. Из {{ TR }} собираются метаданные о структуре данных: каталоги, схемы, таблицы, колонки с типами, партиции, статистика таблиц, а также история запросов. Из {{ AF }} собираются метаданные об оркестрации: структура DAG-файлов и задач, их зависимости, расписание, статусы выполнения, а также lineage-события (через `OpenLineage`).

### Май {#may26}

* Выпущена новая версия [{{ metastore-name }} 4.2.0](../concepts/metastore.md#versions).
* Добавлены параметры для хранения метаданных {{ metastore-name }}. Теперь при [создании](../operations/metastore/cluster-create.md) или [изменении кластера](../operations/metastore/cluster-update.md) можно задать имя бакета и путь к данным внутри него.
