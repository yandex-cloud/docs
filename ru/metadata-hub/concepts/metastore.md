...
## Версии {{ metastore-name }} {#versions}

В {{ yandex-cloud }} доступны следующие версии {{ metastore-name }}:

#|
|| **Версия {{ metastore-name }}** | **Особенности** ||
...
Начиная с версии 4.2.0 {{ metastore-name }} поддерживает два протокола подключения:

* **Thrift** — протокол на порту 9083. Доступен во всех версиях {{ metastore-name }}.
...
Вторая особенность касается сетевой безопасности. Для корректной работы {{ metastore-name }} необходима правильная настройка групп безопасности, разрешающих необходимый сетевой трафик. Без этого кластер может отображаться в состоянии `DEAD`, что затрудняет диагностику проблем (смотрите [инструкцию](../../metadata-hub/operations/metastore/configure-security-group.md) по настройке групп безопасности).
