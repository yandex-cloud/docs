...
Сеть с доступом в интернет, к которой физически подключены все серверы. Сетевой трафик между публичной сетью и интернетом на [некоторых](./network-restrictions.md#blocked-ports) TCP- и UDP-портах ограничен. Некоторые [конфигурации](./server-configurations.md) серверов {{ baremetal-name }} поддерживают резервирование каналов публичной сети [MC-LAG](./mc-lag.md).
