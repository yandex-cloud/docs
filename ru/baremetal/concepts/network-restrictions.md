...
Чтобы увеличить объем потребления трафика, [измените](../operations/servers/server-update.md) конфигурацию сервера, подключив к нему пакет потребления с ежедневной тарификацией:
...
## Защита от BPDU-пакетов {#bpdu-protection}

На сетевых коммутаторах {{ baremetal-name }} включен механизм защиты _BPDU Protection_. 

{% note warning %}

Отправка BPDU-пакетов с интерфейсов сервера {{ baremetal-name }} приводит к блокировке сетевого порта.

Например, это может быть вызвано созданием на сервере {{ baremetal-name }} виртуальных коммутаторов с включенным STP.

{% endnote %}

Если порт был заблокирован, отключите источник BPDU-пакетов и дождитесь автоматического восстановления порта. Обычно это происходит в течение 5 минут.
...
Исходящий SMTP-трафик заблокирован для предупреждения несанкционированных почтовых рассылок. В качестве альтернативы для почтовых рассылок рекомендуем использовать сервис [{{ postbox-full-name }}](../../postbox/concepts/index.md). Если вам требуется публичный адрес из диапазона, в котором разрешен исходящий SMTP-трафик, обратитесь в [техническую поддержку]({{ link-console-support }}). Используйте такие адреса в соответствии с [правилами допустимого использования сервисов]({{ link-cloud-aup }}). Чтобы получить PTR-запись для такого адреса, обратитесь в [техническую поддержку]({{ link-console-support }}).
