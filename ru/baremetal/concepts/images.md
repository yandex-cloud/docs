...
* В [консоли управления]({{ link-console-main }}) вкладка **KVM console** → кнопка **Mount Image** (рекомендуется). Такое монтирование выполняется быстрее, а образ не размонтируется при закрытии окна KVM-консоли и остается смонтированным до указанного времени. Если образ смонтирован, над KVM-консолью отображается надпись **Mounted Image** и время, до которого образ будет смонтирован.
...
Подробнее о монтировании ISO-образа через вкладку **KVM console** — в инструкции [{#T}](../operations/servers/reinstall-os-from-own-image.md#boot-from-image).
