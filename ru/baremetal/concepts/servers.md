...
Сервер может предоставляться пользователю как в одной из готовых [стандартных конфигураций](./server-configurations.md#standard), так и в [пользовательской конфигурации](./server-configurations.md#custom), собранной по индивидуальному [запросу](../operations/servers/custom-server-lease.md) пользователя.

{% include [bios-settings-warning](../../_includes/baremetal/bios-settings-warning.md) %}

## Аренда серверов {#server-lease}

Сервер в стандартной конфигурации можно арендовать на срок 1 день, 1, 3, 6 месяцев или 1 год. Конфигурации, доступные для аренды в каждом [пуле](#server-pools), можно посмотреть при заказе сервера в [консоли управления]({{ link-console-main }}). 

Сервер в пользовательской конфигурации можно арендовать на срок 1 месяц или 1 год.
