...
# Арендовать сервер в пользовательской конфигурации

В число [стандартных конфигураций](../../concepts/server-configurations.md#standard) серверов {{ baremetal-name }} входит большой перечень сборок с самыми разными аппаратными характеристиками, которые оптимально подойдут почти для любой задачи.

Если же ваша задача требует от сервера каких-то нетиповых аппаратных характеристик, вы можете сами [сконфигурировать](../../concepts/server-configurations.md#custom) ваш сервер. Для этого:
...
- Консоль управления {#console}
  ...
  1. Нажмите кнопку **{{ ui-key.yacloud.baremetal.label_create-server }}** и в открывшемся окне выберите вариант `{{ ui-key.yacloud.baremetal.servers.ServerConfigPage.custom-configs_b5Ab9 }}`. В открывшемся окне настройте конфигурацию сервера:
      ...
      1. В блоке **{{ ui-key.yacloud.baremetal.customConfig.CustomConfigForm.pool_75ogL }}** выберите [пул серверов](../../concepts/servers.md#server-pools), в котором будет расположен сервер.
      1. В блоке **{{ ui-key.yacloud.baremetal.customConfig.CustomConfigForm.powerReserve_8Ljpr }}** выберите, будет ли на сервере [резервирование питания](../../concepts/servers.md#server-power-supply).
