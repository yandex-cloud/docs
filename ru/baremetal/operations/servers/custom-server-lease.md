...
В результате сервер перейдет в [статус](../../concepts/servers.md#server-status) `ASSEMBLING`: его соберут, установят и подключат к сети в дата-центре. Затем сервер перейдет в статус `PROVISIONING` для подготовки к работе, а после ее завершения — в статус `RUNNING`. На сборку и подготовку сервера суммарно может потребоваться до четырех календарных дней.
