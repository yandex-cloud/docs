...
- Ubuntu/Debian (Netplan) {#ubuntu}
  ...
  1. Настройте конфигурацию Netplan, добавив в нее группы агрегирования (раздел `bonds`):
      ...
      * выключен (значение `dhcp4: false`) для индивидуальных интерфейсов (раздел `ethernets`).
      ...
      Если к серверу привязаны [дополнительные приватные подсети](../../concepts/private-network.md#tagged-vlan-subnet), настройте на сервере виртуальные сети VLAN. Подробнее читайте в разделе [{#T}](./set-up-tagged-vlan.md).
