...
Создание отказоустойчивой дисковой разметки требует опыта и понимания технологий [RAID](../../concepts/disks/raid.md) и/или [LVM](https://ru.wikipedia.org/wiki/LVM), поэтому рекомендуется использовать установку ОС из публичного образа {{ marketplace-short-name }}, а приведенный в этой инструкции способ установки использовать только при необходимости внесения существенных изменений, недоступных при установке из {{ marketplace-short-name }}.
...
1. [Подключитесь](./server-kvm.md) к KVM-консоли сервера.
    ...
    Дальнейшие действия по установке операционной системы будут выполняться в окне KVM-консоли. Монтирование ISO-образа можно выполнить как в окне KVM-консоли, так и через вкладку **KVM console** в [консоли управления]({{ link-console-main }}).
...
1. Смонтируйте ISO-образ на виртуальный CD-дисковод сервера. Для серверов на поддерживаемых платформах доступны два способа монтирования:
    ...
    - Через вкладку KVM console (рекомендуется)
      ...
      1. На странице сервера в [консоли управления]({{ link-console-main }}) перейдите на вкладку **KVM console**.
...
1. Отключите виртуальный CD-дисковод. Способ отключения зависит от того, каким способом был смонтирован ISO-образ:
    ...
    - Через вкладку KVM console
      ...
      На странице сервера в [консоли управления]({{ link-console-main }}) на вкладке **KVM console** нажмите кнопку отключения смонтированного образа.
