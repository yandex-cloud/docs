---
title: История изменений в {{ baremetal-full-name }}
description: В разделе представлена история изменений сервиса {{ baremetal-name }}.
---

# История изменений в {{ baremetal-full-name }}

## II квартал 2026 {#q2-2026}

* Опубликован новый [справочник REST API v2](./api-ref/authentication.md) сервиса {{ baremetal-name }}.
* [{{ yandex-cloud }} CLI](../cli/quickstart.md): добавлена ветка команд сервиса на новом синтаксисе `yc baremetal v2`.
* Описаны [сценарии совместного использования {{ baremetal-name }} с другими сервисами {{ yandex-cloud }}](./concepts/integrations.md).
...
* Описаны [типы дисков](./concepts/disks/disk-types.md) и [способы реализации RAID](./concepts/disks/raid.md), поддерживаемые в серверах {{ baremetal-name }}.
...
* Добавлены [события {{ at-name }}](./at-ref.md) уровня конфигурации сети ExternalFlow.

## I квартал 2026 {#q1-2026}

* Поддержаны [статические маршруты](./operations/create-static-route.md) в приватной сети {{ baremetal-name }}.
...
* Поддержано [подключение сервера {{ baremetal-name }} к кластеру {{ managed-k8s-name }}](./concepts/integrations.md) как внешнего узла.
...
* Опубликован [справочник REST API](./api-ref/authentication.md) сервиса {{ baremetal-name }}.
...
* Расширена документация по [образам ОС](./concepts/images.md) и [установке ОС из своего ISO-образа](./operations/servers/reinstall-os-from-own-image.md), в том числе с использованием {{ objstorage-name }}.
...
* Сервис {{ baremetal-full-name }} перешел в стадию [General Availability](../overview/concepts/launch-stages.md).
* Поддержано [подключение сервера к {{ backup-name }}](./operations/backup-baremetal.md) при его заказе.
