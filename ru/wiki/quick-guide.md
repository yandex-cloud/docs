...
Попробуйте возможности {{ wiki-name }}, создав свою первую страницу в старом редакторе. Страница будет создана в вашем личном разделе, так что не бойтесь экспериментировать.
...
1. В поле **{{ ui-key.beta-wiki.blocks_common_desktop.blocks_b-widget.name }}** введите заголовок страницы.

1. Поле с адресом заполняется автоматически. Не редактируйте его, если это ваша первая попытка создания страницы.

1. Выберите **Личный раздел**.

1. Нажмите кнопку **{{ ui-key.beta-wiki.blocks_common_desktop.blocks_b-head.create }}**.

1. В левую часть рабочей области страницы скопируйте этот пример или придумайте свой:
...
Список всех ваших страниц доступен на верхней панели {{ wiki-name }} в меню ![](../_assets/wiki/svg/structure-icon.svg) **{{ ui-key.beta-wiki.blocks_common_desktop.blocks_b-page-more-panel.tree }}**.
