...
В {{ wiki-full-name }} могут работать пользователи организаций, зарегистрированные в сервисе [{{ org-full-name }}](../organization/index.yaml). Для организации в {{ wiki-full-name }} создается своя база знаний, доступная только сотрудникам. О том, как подключить {{ wiki-full-name }}, читайте в разделе [{#T}](enable-wiki.md).

Чтобы получить доступ к {{ wiki-name }}, [войдите в аккаунт](login.md), который вы используете для работы в организации.

Если в компании есть система управления пользователями и доступом (например, Active Directory или Google Workspace), сотрудники могут использовать свои корпоративные аккаунты, чтобы войти в организацию {{ yandex-cloud }} и получить доступ к {{ wiki-name }} через систему единого входа (Single Sign-On, SSO). Для этого администратору организации нужно настроить федерацию удостоверений. Подробнее читайте в документации {{ org-full-name }} в разделе [Управление федерациями удостоверений](../organization/add-federation.md).
...
- Чтобы структурировать информацию на Вики, создайте дерево страниц и связывайте страницы перекрестными ссылками. Подробнее читайте в разделе [{#T}](structure.md).
...
- Настраивайте [права доступа к страницам](page-managment/access-setup.md): можно открыть страницу для всех или некоторых сотрудников, ограничить возможность редактирования или запретить просмотр страницы.

- Используйте интеграцию {{ wiki-name }} с сервисами {{ tracker-full-name }} и {{ forms-full-name }}:
  
  * [Как добавить на страницу список задач из {{ tracker-full-name }}](actions/objects.md#section_task_list).
  
  * [Как встроить форму на страницу](actions/objects.md#forms).
