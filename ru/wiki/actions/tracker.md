...
Внешний вид таких ссылок будет изменяться в зависимости от того, что происходит с задачей: кто ее исполнитель, какой у нее статус и так далее.

{% endif %}

## Ссылка на задачу {#ticket}

На [вики-страницах и в визуальном редакторе](../pages-types.md) вы можете вставлять _магические ссылки_ на отдельные задачи. Такие ссылки всегда содержат ключ, название, статус задачи и логин исполнителя. Чтобы вставить ссылку на задачу, скопируйте ее ключ и вставьте в текст страницы. 
...
С помощью блока `not_var{{tasks}}` вы можете разместить на вики-странице автоматически сформированный список задач. Например:
