---
title: Расследования в {{ yandex-siem-full-name }}
description: Расследования в {{ yandex-siem-full-name }} позволяют анализировать события безопасности с помощью KQL-запросов, шаблонов и датасетов.
---
...
_Расследование_ — это основная сущность для анализа событий безопасности в {{ yandex-siem-full-name }}. Расследование объединяет один или несколько запросов, которые позволяют искать и анализировать события за последние 90 дней.

{% include [queries-max-results](../../_includes/ycdr/queries-max-results.md) %}

## Связь с инстансом {{ yandex-siem-full-name }} {#instance-binding}

Расследования привязаны к [инстансу](index.md) {{ yandex-siem-full-name }}. Каждый инстанс содержит собственный набор расследований, шаблонов и датасетов. Расследования могут быть доступны:
...
Доступ к расследованиям определяется ролями. Подробнее в разделе [{#T}](../security/index.md).
