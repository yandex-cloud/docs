...
1. В сервисе {{ cns-short-name }} создайте каналы уведомлений. Для отправки уведомлений в очереди {{ message-queue-full-name }} каналы создавать не нужно.
...
1. Добавьте подписку  в топик и укажите в ней:

   * идентификатор (ARN) созданного ранее эндпоинта — для мобильных push-уведомлений и уведомлений в браузере;
   * номер телефона — для SMS;
   * URL очереди — для {{ message-queue-name }}.
