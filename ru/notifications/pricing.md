...
Отправка сообщений в очереди {{ message-queue-full-name }} тарифицируется согласно [правилам тарификации {{ message-queue-name }}](../message-queue/pricing.md). Дополнительная плата на стороне {{ cns-short-name }} не взимается.

{% note info %}

По умолчанию в сервисе действуют лимиты на отправку уведомлений. Подробнее в разделе [{#T}](concepts/limits.md). Если вы хотите отправлять больше уведомлений, обратитесь в [службу поддержки]({{ link-console-support }}).
