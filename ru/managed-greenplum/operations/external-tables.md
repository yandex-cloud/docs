...
Для подключения к внешним источникам необходимо [включить NAT в интернет](../../vpc/operations/enable-nat) для подсети, в которой расположен кластер {{ mgp-name }}.

{% else %}

Для подключения к внешним источникам необходимо включить NAT в интернет для подсети, в которой расположен кластер {{ mgp-name }}.
...
В этом SQL-запросе приведены не все доступные параметры. Подробнее см. в [документации {{ GP }}](https://docs.greenplum.org/6-4/pxf/intro_pxf.html) и примерах создания внешних таблиц.
