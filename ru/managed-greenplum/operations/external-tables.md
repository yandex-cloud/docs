...
Для подключения к внешним источникам необходимо настроить NAT-шлюз для подсети, в которой расположен кластер {{ mgp-name }}.
...
[{{ GP }} Platform Extension Framework (PXF)]({{ gp.docs.org }}/6-4/pxf/overview_pxf.html) — программная платформа, которая позволяет получить доступ к данным во внешних СУБД. Доступно подключение таблиц следующих внешних источников:
...
В этом SQL-запросе приведены не все доступные параметры. Подробнее см. в [документации {{ GP }}]({{ gp.docs.org }}/6-4/pxf/intro_pxf.html) и примерах создания внешних таблиц.
