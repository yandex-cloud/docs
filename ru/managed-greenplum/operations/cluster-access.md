---
title: Управление доступом к кластеру {{ mgp-full-name }}
description: Следуя данной инструкции, вы настроите права доступа к кластеру {{ mgp-name }}.
---
...
- {{ TF }} {#tf}
  ...
  1. Откройте актуальный конфигурационный файл с описанием кластера {{ mgp-name }}.
      ...
      Как создать такой файл, читайте в разделе [{#T}](cluster-create.md).
  ...
  1. Подтвердите изменение ресурсов.
      ...
      Подробнее читайте в [документации провайдера {{ TF }}]({{ tf-provider-resources-link }}/mdb_greenplum_cluster_iam_binding).
...
- REST API {#api}
  ...
  1. Воспользуйтесь методом [Cluster.SetAccessBindings](../api-ref/Cluster/setAccessBindings.md) и выполните запрос, например, с помощью {{ api-examples.rest.tool }}:
      ...
      * `accessBindings.roleId` — назначаемая [роль](../security/index.md#roles-list), например `managed-greenplum.editor`.
