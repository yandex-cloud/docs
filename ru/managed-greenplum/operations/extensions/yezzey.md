...
Вы можете [выгрузить](#offload-relation) редко используемые таблицы такого типа в гибридное хранилище и продолжить работать с ними, как с обычными таблицами. При необходимости таблицы можно [загрузить обратно](#load-relation) в хранилище кластера.

Подробнее о типах хранилищ и расширении {{ YZ }} читайте в разделе [Гибридное хранилище](../../concepts/hybrid-storage.md).

## Установить расширение {{ YZ }} в кластер {{ mgp-name }} {#install-extension}
...
1. [Подключитесь](../connect/index.md) к базе данных от имени владельца или пользователя, имеющего в базе данных разрешение `CREATE`, и выполните запрос:
...
Чтобы узнать, где размещена таблица и ее сегментные файлы — в хранилище кластера или в гибридном хранилище:

1. [Подключитесь](../connect/index.md) к базе данных от имени владельца или пользователя, имеющего в базе данных разрешение `SELECT`.
...
1. Выполните один из запросов, чтобы узнать, где размещена таблица:
    ...
    * С указанием схемы, где расположена таблица, и имени таблицы:
    ...
    Будет выведена информация о каждом сегменте кластера {{ mgp-name }}, в котором размещены данные таблицы.
    ...
    | `external_bytes` | Размер данных, выгруженных в гибридное хранилище. Если в столбце все значения нулевые, таблица размещена в хранилище кластера. Если в столбце есть ненулевые значения, таблица размещена в гибридном хранилище.|
    | `external_bloat_bytes` | Размер данных, выгруженных в гибридное хранилище, которые больше не используются, но еще не удалены. |
...
1. Выполните один из запросов, чтобы узнать, где размещены сегментные файлы таблицы:
    ...
    * Если в столбце есть ненулевые значения, то файлы размещены в гибридном хранилище.

## Определить, как размещена таблица в гибридном хранилище {#describe-relation-objstorage}

Если таблица [находится в гибридном хранилище](#describe-relation), то ее сегментные файлы хранятся в служебном бакете.
...
1. Выполните один из запросов:
    ...
    * `external_storage_filepath` — путь к сегментному файлу в бакете {{ objstorage-name }}.
        ...
        Путь к сегментному файлу формируется автоматически. Он зависит от структуры таблицы и количества сегментов в кластере {{ mgp-name }}.
...
## Выгрузить таблицу из хранилища кластера в гибридное хранилище {#offload-relation}

Если таблица находится в хранилище кластера, то ее можно выгрузить в гибридное хранилище. Чтобы понять, где она находится, [получите информацию о размещении таблицы](#describe-relation).
...
## Загрузить таблицу из гибридного хранилища в хранилище кластера {#load-relation}

Если таблица находится в гибридном хранилище, то ее можно загрузить в хранилище кластера. Чтобы понять, где она находится, [получите информацию о размещении таблицы](#describe-relation).
...
Пример использования расширения для работы с гибридным хранилищем описан в разделе [{#T}](../../tutorials/yezzey.md).
