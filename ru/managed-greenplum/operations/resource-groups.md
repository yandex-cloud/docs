...
Для создания ресурсной группы для компонента достаточно указать значения параметров:

* `MEMORY_AUDITOR=cgroup`;
* `CONCURRENCY=0`;
* `CPU_RATE_LIMIT` или `CPUSET`;
* `MEMORY_LIMIT`.
...
При удалении ресурсной группы, которая содержит запущенные экземпляры внешнего компонента, {{ GP }} обрывает работу этих экземпляров.
...
* Ресурсная группа для компонента

     Создайте ресурсную группу с тестовыми характеристиками:

    * Имя `testcomgroup`.
    * Зарезервированное ядро CPU под номером 2.
    * Доля оперативной памяти в сегменте — 10%.
    * Доля, выделенной в качестве общей квоты, — 70%.
    * Доля оперативной памяти в ресурсной группе, которую может занимать один оператор транзакции — 25%.
