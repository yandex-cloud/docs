---
title: Управление ролями и пользователями кластера {{ mgp-full-name }}
...
{{ mgp-name }} управляет правами доступа к базе данных с помощью двух типов ролей:
...
Пользователь-администратор создается вместе с кластером {{ mgp-name }} и автоматически получает роль администратора `mdb_admin`. [Подключитесь к базе данных](connect/index.md) от его имени, чтобы:
...
Подробнее о командах взаимодействия с ролями читайте в [документации {{ GP }}]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-sql_commands-sql_ref.html).
...
* SQL
    ...
    Доступные атрибуты читайте в разделе [{#T}](../concepts/cluster-users.md#attributes).
...
* SQL
    ...
    Доступные привилегии читайте в разделе [{#T}](../concepts/cluster-users.md#privileges).
