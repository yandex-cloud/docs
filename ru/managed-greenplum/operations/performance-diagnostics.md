...
Для диагностики производительности кластер {{ mgp-name }} использует выделенную базу данных `gpperfmon`, в которой сохраняется статистика запросов и состояний системы. Обращаться к базе данных `gpperfmon` может:
