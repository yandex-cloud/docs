...
* В Командном центре для кластеров на ядре {{ CB }} 2 теперь доступно управление ресурсными группами.
* В консоли управления добавлена вкладка **Рекомендации**. В ней доступны проверки загрузки CPU, учета памяти ZSTD, выделения памяти.
* Обновлен дашборд для [метрик PXF](metrics.md#managed-greenplum-pxf-metrics)
* В публичное API добавлен [метод листинга доступных версий СУБД](api-ref/Versions/list.md).
...
* Добавлены отдельные [роли](./security/index.md) для просмотра и редактирования (переноса) технического обслуживания без доступа к кластеру.
...
* Доступна новая версия {{ GP }} 6.25. В ней по умолчанию доступны инструменты [интеграции с гибридным хранилищем](tutorials/yezzey.md) и [командный центр](./concepts/command-center.md).
...
* Добавлена функция `mdb_toolkit.table_privileges()`, позволяющая пользователю с ролью `mdb_admin` просматривать историю выдачи и отзыва привилегий в кластере. Подробнее читайте в [документации {{ PG }}]({{ pg.docs.org }}/current/infoschema-table-privileges.html).
* Добавлена функция `mdb_toolkit.gp_workfile_usage_per_query()`, позволяющая пользователю с ролью `mdb_admin` просматривать информацию об активных запросах всех пользователей. Подробнее читайте в [документации {{ GP }}]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-gp_toolkit.html#topic34).
...
* Добавлена возможность редактирования кластера и его параметров через CLI, а также возможность разрешения доступа из {{ data-transfer-full-name }}. Подробнее читайте в разделе [Релизы YC CLI](../cli/release-notes.md#version0.94.0).
