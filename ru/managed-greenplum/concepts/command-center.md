---
title: Командный центр {{ mgp-name }}
description: Из статьи вы узнаете, что такое командный центр {{ mgp-name }}, текущее состояние, история состояний, история потребления, ключи запросов и идентификаторы запросов.
---

# Командный центр {{ mgp-name }}

_Командный центр_ в {{ mgp-name }} — это инструмент, с помощью которого можно:
...
* {{ GP }} — начиная c версии 6.25;
* {{ CB }} — начиная с версии 2.0.
...
Параметры сессий и запросов описаны в разделе [Параметры командного центра {{ mgp-name }}](command-center-parameters.md).
