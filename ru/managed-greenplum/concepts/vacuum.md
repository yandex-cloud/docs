...
По умолчанию в {{ mgp-name }} используются [heap-таблицы](tables.md) — стандартный тип таблиц {{ PG }}. Для них команды `VACUUM` и `VACUUM FULL` работают следующим образом:
...
[{{ YZ }}](../operations/extensions/yezzey.md) — это расширение для кластера {{ mgp-name }}, которое позволяет хранить таблицы AO и AOCO в [гибридном хранилище](hybrid-storage.md).
...
Системный каталог в {{ mgp-name }} — это набор системных таблиц в схеме `pg_catalog`. Системные таблицы хранят метаданные о таблицах, столбцах и других объектах базы данных. Со временем они раздуваются из-за создания и удаления DDL-объектов, особенно при массовой загрузке данных (например, с помощью [{{ maf-name }}](../../managed-airflow/index.yaml)). Поэтому для системного каталога также необходимо выполнять операции `VACUUM` и `VACUUM FULL`.
