...
Все хосты кластера {{ mgp-name }} размещаются в одной зоне доступности: `ru-central1-a` или `ru-central1-b`. Создавать хосты в зоне `ru-central1-c` нельзя. Подробнее см. в разделах [{#T}](../../overview/concepts/geo-scope.md) и [{#T}](../../overview/concepts/ru-central1-c-deprecation.md).
