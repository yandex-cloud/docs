...
Каждый кластер состоит из хостов баз данных — виртуальных машин с развернутыми серверами СУБД. Кластер {{ GP }} включает:
...
Хосты кластера {{ GP }} представляют собой [виртуальные машины](../../glossary/vm.md) в облаке. Такие виртуальные машины могут размещаться:
...
Все хосты кластера {{ GP }} размещаются в одной зоне доступности: `{{ region-id }}-a`, `{{ region-id }}-b` или `{{ region-id }}-d`.
