---
title: '{{ CB }} — современная, бурно развивающаяся альтернатива {{ GP }}'
description: Из статьи вы узнаете, почему СУБД {{ CB }} является самой достойной альтернативой {{ GP }}.
---

# Почему выбирают {{ CB }}

{{ CB }} — форк {{ GP }} под управлением Apache Software Foundation. {{ CB }} версии 2.x построен на актуальном PostgreSQL 14.x, в то время как {{ GP }} – на PostgreSQL 9.4, поддержка которого закончилась в феврале 2020. Далее {{ CB }} будет обновляться на PostgreSQL 16. Обновление выйдет в мажорном релизе Apache Cloudberry™ 3.x.

Переход с {{ GP }} на {{ CB }} не требует переучивания команды и переписывания приложений. {{ CB }} почти полностью совместим с {{ GP }}. Полный список несовместимых изменений (обычно малых и точечных) вынесен в [отдельный документ](./breaking-changes.md). Функционально эти изменения затрагивают небольшую часть сценариев.

## Преимущества {{ CB }}

### Быстрее в аналитике

Основной источник ускорений в аналитических движках — формат хранения данных. {{ CB }} использует гибридный формат хранения данных [PAX (Partition Attributes Across)](https://cloudberry.apache.org/docs/operate-with-data/pax-table-format/). Он объединяет удобство работы со строчными таблицами и колоночную скорость чтения, автоматически пропускает нерелевантные блоки и поддерживает современные кодеки сжатия.

### Оптимизация запросов

В {{ CB }} есть инструменты оптимизации запросов:
