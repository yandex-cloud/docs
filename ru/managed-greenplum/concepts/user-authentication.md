...
Подробнее об этих ограничениях читайте в разделе [Настройки правил аутентификации](#auth-settings).
...
* `md5` — по паролю. Подробнее читайте в [документации {{ PG }}]({{ pg-docs }}/auth-password.html).
* `iam` — [с помощью IAM](../operations/connect/clients.md#iam).
* `reject` — запрещает подключение пользователя.

Подробнее о настройках читайте в [документации {{ PG }}]({{ pg-docs }}/auth-pg-hba-conf.html).
