...
{{ GP }} поддерживает два типа ресурсных групп:

* _Ресурсные группы для ролей_ — определяют квоты, выделяемые для [пользователей](cluster-users.md), которым назначена определенная роль.
...
* _Ресурсные группы для компонентов_ — определяют количество ресурсов, выделяемых внешнему компоненту.
    ...
    В ресурсную группу может входить только один внешний компонент, квоты будут общими для всех его экземпляров.
...
| Название            | Описание |
| -----               | ----- |
| `MEMORY_AUDITOR`      | Средство контроля памяти, используемое ресурсной группой.{% if audience == "draft" %}Определяет ее тип. При создании ресурсной группы для компонента укажите значение `cgroup`. В ресурсной группе для ролей по умолчанию используется значение `vmtracker`.{% endif %} В {{ mgp-name }} по умолчанию используется значение `vmtracker`. |
| `CONCURRENCY`         | Максимальное количество транзакций, обрабатываемых в ресурсной группе одновременно. Включает в себя активные транзакции и транзакции в режиме ожидания. Значение по умолчанию — `20`.{% if audience == "draft" %} При создании ресурсной группы для компонента необходимо явно указать `0`.{% endif %} |
...
* Если сумма значений `MEMORY_LIMIT` всех ресурсных групп меньше `100`, то {{ GP }} выделяет незарезервированную оперативную память любым транзакциям любых ресурсных групп по принципу <q>первый вошел — первый обслужен</q>.
    ...
    * общая квота ресурсной группы закончилась.
    ...
    {% if audience == "draft" %}
    Незарезервированную оперативную память могут использовать только ресурсные группы для ролей.
    {% endif %}
