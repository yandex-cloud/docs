...
| `MEMORY_SHARED_QUOTA` | Доля зарезервированной памяти `MEMORY_LIMIT` (%), которая может использоваться всеми транзакциями ресурсной группы по мере необходимости. Минимальное значение — `1`, максимальное значение — `100`, по умолчанию — `80`. Подробнее в разделе [Особенности распределения оперативной памяти](#ram). |
...
Подробнее об управлении ролями читайте в разделе [Пользователи и роли](./cluster-users.md) и в [документации {{ GP }}]({{ gp.docs.broadcom }}/7/greenplum-database/admin_guide-roles_privs.html).
...
Подробнее о работе ресурсных групп читайте в [документации {{ GP }}]({{ gp.docs.broadcom }}/7/greenplum-database/admin_guide-workload_mgmt_resgroups.html).
