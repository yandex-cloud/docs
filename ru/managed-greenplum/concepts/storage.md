---
title: Типы дисков в {{ mgp-full-name }}
description: Из статьи вы узнаете о типах дисков в {{ mgp-name }} и особенностях хранилища на локальных SSD-дисках.
---
...
При заполнении хранилища любого хоста более чем на 95% {{ mgp-name }} переводит кластер в режим `read-only`. При этом для всех баз данных через запрос `ALTER DATABASE` выставляется настройка `DEFAULT_TRANSACTION_READ_ONLY = TRUE`.
...
* Вручную отключите режим read-only и освободите место в хранилище, удалив часть данных.
    ...
    Не допускайте, чтобы в процессе этих действий свободное дисковое пространство уменьшилось до нуля. Поскольку предохранительный механизм отключен, {{ mgp-name }} в этом случае аварийно завершит работу и кластер станет неработоспособным.
