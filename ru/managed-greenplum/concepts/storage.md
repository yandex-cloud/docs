...
Тип дисков у хостов-мастеров и хостов-сегментов может различаться. Исключение — кластеры с [шифрованием дисков](#disk-encryption): для обоих типов хостов должны использоваться диски `local-ssd`.
...
В {{ mgp-name }} шифрование доступно только для локальных SSD-дисков (`local-ssd`). Чтобы включить шифрование, выберите этот тип диска как для хостов-мастеров, так и для хостов-сегментов. Если хотя бы для одного из этих типов хостов выбраны сетевые диски, включить шифрование нельзя.

{% include [disk-encryption](../../_includes/mdb/disk-encryption.md) %}

{% note info %}

Включение шифрования может незначительно снизить производительность дисковой подсистемы в некоторых сценариях использования.
