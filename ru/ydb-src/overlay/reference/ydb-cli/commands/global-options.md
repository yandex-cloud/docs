...
С помощью глобальных параметров команд {{ ydb-short-name }} CLI вы можете указать эдпоинт, базу данных, подробность вывода, способ авторизации, профиль, а также получить справку {% if audience == "external" %}и информацию о лицензировании{% endif %}.
...
`--token-file` `PATH` | Использовать для авторизации [OAuth-токен YDB](../../../getting_started/start_auth.md#get_oauth_token).
{% endif %}
