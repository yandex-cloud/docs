...
description: Колоночные базы данных — базы данных с колоночным типом хранения. Информация в них упорядочивается и сохраняется по столбцам. Модель данных — отдельная характеристика. Она может быть как реляционной, так и нереляционной.
keywords:
...
{{ CH }} — пограничный случай: SQL, таблицы и JOIN поддерживаются, но многострочных транзакций с откатом нет, а `UPDATE`/`DELETE` выполняются не как обычные DML-операции, а как отложенные фоновые перезаписи данных. Поэтому {{ CH }} точнее называть SQL-ориентированной OLAP-СУБД с колоночным хранением.
...
| **Примеры**                           | PostgreSQL, MySQL, SQLite               | {{ CH }}, Redshift, DuckDB           |
...
* [{{ mch-full-name }}](/services/managed-clickhouse/) — сервис для работы с колоночными базами данных [{{ CH }}](https://clickhouse.com). С его помощью вы сможете анализировать и хранить данные, быстро обрабатывать большие объемы, а также сжимать информацию для экономии места. {{ CH }} легко масштабировать и интегрировать с другими сервисами. Подробнее в [документации](../managed-clickhouse/).
* [{{ dataproc-full-name }}](/services/data-proc/) — управляемый сервис, работающий на базе популярных инструментов обработки больших данных. Позволяет интегрировать колоночные базы данных с [Apache Hive](https://hive.apache.org/) и [Apache Spark](https://spark.apache.org/) для проведения более сложной аналитики и машинного обучения. Подробнее в [документации](../data-proc/).
* [{{ datalens-full-name }}](/services/datalens/) — сервис для визуализации и анализа данных, который может работать с данными из {{ CH }} и других источников. Подробнее в [документации](../datalens/).
...
Такие решения, как [{{ CH }}](https://clickhouse.com) и [Amazon Redshift](https://ru.wikipedia.org/wiki/Amazon_Redshift), уже зарекомендовали себя надежными и эффективными инструментами для работы с большими объемами данных. Они предлагают гибкость и возможность масштабирования, что делает их незаменимыми в современном мире.
