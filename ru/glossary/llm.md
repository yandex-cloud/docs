...
1. **Токенизация** — модель разбивает запрос на [токены](../foundation-models/concepts/generation/tokens.md), что позволяет ей лучше понимать естественный язык. Например, {{ gpt-pro }} разобьет наш запрос на пять токенов: «Какая», «погода», «в», «Москве», «?».
...
* Сервис [{{ foundation-models-full-name }}](/services/foundation-models/), включающий:
  * [Модели](../foundation-models/concepts/generation/models.md) генерации текста.
