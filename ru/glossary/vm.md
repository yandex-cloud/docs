...
Примером подобной хост-системы является сервис [{{ compute-full-name }}](/services/compute) — облачная платформа, использующая инфраструктуру и уникальные технологии Яндекса.
