...
* Виртуальные машины [{{ baremetal-full-name }}](/services/baremetal) предусматривают возможность создания подсетей с автоматическим конфигурированием параметров IP-адресации. Адреса назначаются на физический интерфейс сетевой карты с фиксацией MAC-адреса, что служит дополнительной защитой. Подробнее в [документации](../baremetal/concepts/dhcp.md).
* Подсети [{{ vpc-full-name }}](/services/vpc) позволяют управлять настройками DHCP и задавать пользователям подсети те же DNS- и NTP-серверы, что используются в вашей корпоративной сетевой инфраструктуре. Подробнее в [документации](../vpc/concepts/dhcp-options.md).