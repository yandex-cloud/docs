...
1. [{{ ydb-name }}](/services/ydb/) — распределенная отказоустойчивая СУБД, поддерживающая автоматическое шардирование. Подробнее в [документации](../ydb/).

1. Сервисы для работы с кластерами серверов разных СУБД со встроенным шардированием.

   * [{{ mmg-name }}](/services/storedoc/). Подробнее в [документации](../storedoc/).

   * [{{ mrd-name }}](/services/managed-redis/). Подробнее в [документации](../managed-valkey/).

   * [{{ mch-name }}](/services/managed-clickhouse/). Подробнее в [документации](../managed-clickhouse/).

   * [{{ mspqr-name }}](/services/managed-spqr/). Подробнее в [документации](../managed-spqr/).

   * [{{ mgp-name }}](/services/managed-greenplum/). Подробнее в [документации](../managed-greenplum/).
