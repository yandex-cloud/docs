...
* **Контроль затрат**
  ...
  Трафик больших языковых моделей стоит дорого, однако протокол не учитывает эти затраты и не ограничивает размер пакетов. Для решения этой проблемы разработчикам требуется задать максимальное количество [токенов](../foundation-models/concepts/generation/tokens.md).
