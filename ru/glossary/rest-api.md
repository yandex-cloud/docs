...
description: "REST API – это способ взаимодействия приложений. С помощью REST API выполняется обмен информацией через интернет между клиентской и серверной частью приложения, или между отдельными сервисами в системах с мультисервисной архитектурой."
---
...
Современная реализация этого подхода — gRPC, на ней реализованы [API сервисов {{ yandex-cloud }}](https://cloud.yandex.ru/docs/overview/api). С их помощью вы можете создавать приложения и сервисы, использующие ресурсы {{ yandex-cloud }}. 
