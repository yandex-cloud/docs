...
В сети интернета вещей представлено два основных вида участников: клиенты и брокеры.
...
Все клиенты подключаются к брокеру — центральному узлу агрегации, обработки, сортировки и пересылки сообщений MQTT. Таким образом, клиенты не зависят друг от друга и не знают о существовании друг друга.

![MQTT-брокер в сети с архитектурой «издатель — подписчик»](../_assets/glossary/mqtt-broker-scheme.svg)

Подобная архитектура сети обеспечивает плавную интеграцию новых устройств и безболезненное изъятие устройств, которые больше не нужны. Если какое-то устройство потеряет связь с брокером, сбоев в сети не возникнет: в большинстве случаев сообщение MQTT будет доставлено, как только связь восстановится.

В сети может быть несколько брокеров, а значит, один клиент может взаимодействовать со множеством брокеров, а один брокер — со множеством клиентов. Так в MQTT реализуются любые виды подписки: «один к одному», «один ко многим», «многие к одному».

## Особенности коммуникации MQTT {#mqtt-communication}

Когда издателю нужно сделать публикацию, он формирует сообщение MQTT, которое имеет заголовок с темой — топиком. Сообщение пересылается брокеру MQTT, и тот сортирует все поступающие данные, формируя своеобразную иерархию тем. Одновременно брокер распространяет сообщения среди подписчиков.
...
## Преимущества использования MQTT-брокера {#advantages}

MQTT-брокер выполняет в сети обмена данными все основные функции ядра коммуникации:
...
Применение MQTT-брокеров в структуре сети интернета вещей дает ее пользователям важные преимущества:
...
MQTT-брокеры реализуются в виде программ с проприетарным или открытым исходным кодом, которые могут работать как локально (на физическом компьютере), так и в облаке (на облачном сервере).
...
## Облачный MQTT-брокер от {{ yandex-cloud }} {#mqtt-broker-yc}
...
Сервис предлагает две модели управления ресурсами. В первой модели устройства объединяются в реестры и взаимодействуют через MQTT-сервер. Во второй — общение между отдельными устройствами и реестрами реализуется с помощью MQTT-брокера. Модели могут использоваться совместно или по отдельности, что обеспечивает гибкий подход к решению задач.
...
[Подробная документация](https://cloud.yandex.ru/docs/iot-core/) и [доступные тарифы](https://cloud.yandex.ru/docs/iot-core/pricing) помогают быстро приступить к работе. [Создайте свой первый реестр](https://console.cloud.yandex.ru/link/iot-core/) и протестируйте сервис {{ iot-short-name }} уже сейчас.
...
### Статьи и видео по теме {#see-also}
  ...
  * [Видео](https://www.youtube.com/watch?v=fLWTSQ0a8NM) об использовании MQTT-брокера {{ yandex-cloud }} для панелей Weintek
