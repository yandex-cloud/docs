...
description: Виртуальная локальная сеть (VLAN) — сеть, в которой сетевые каналы используют одну физическую структуру, но при этом логически изолированы друг от друга.
keywords:
...
* [{{ vpc-full-name }}](/services/vpc) — сервис для управления облачными сетями и связи облачных ресурсов между собой и с интернетом. Позволяет создавать изолированные подсети внутри одной сети. Подробнее в [документации](../vpc/).
* [{{ alb-full-name }}](/services/application-load-balancer) — сервис для распределения входящего трафика между разными компонентами ваших веб‑приложений. Подробнее в [документации](../application-load-balancer/).
* [{{ interconnect-full-name }}](/services/interconnect) — сервис для создания приватных выделенных сетевых соединений между локальной инфраструктурой и {{ yandex-cloud }}. Подробнее в [документации](../interconnect/).
