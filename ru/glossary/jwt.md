...
* **Подделка токена**. Если злоумышленник перехватит идентификационные данные пользователя, он сможет подделать токен и выдать себя за него.
  ...
  Для защиты можно нужно усложнять механизм проверки ключей, выпускаемых на основе токенов. Например, через использование refresh-токенов.
...
В {{ yandex-cloud }} доступен сервис [{{ iam-full-name }}](../iam/), который обеспечивает централизованное управление ресурсами с помощью разных методов. Рассмотрим основные методы аутентификации с использованием JWT:
...
Подробнее о методах аутентификации в {{ iam-name }} в [документации](../iam/concepts/authorization/index.md).
