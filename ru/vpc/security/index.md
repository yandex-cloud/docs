...
Чтобы разрешить доступ к ресурсам сервиса [!KEYREF vpc-full-name], облачным сетям и подсетям, назначьте пользователю нужные роли из приведенного ниже списка. На данный момент роль может быть назначена только на родительский ресурс (каталог или облако), роли которого наследуются вложенными ресурсами. 
...
#### [!KEYREF roles-viewer]
Пользователь с ролью `[!KEYREF roles-viewer]` может смотреть списки облачных сетей и подсетей в каталоге.

#### [!KEYREF roles-editor]
Пользователь с ролью `[!KEYREF roles-editor]` может выполнять любые операции с облачными сетями и подсетями в каталоге: создавать, удалять и изменять их.
