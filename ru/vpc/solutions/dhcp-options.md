...
Для выполнения данного сценария у вас уже должен быть развернут корпоративный DNS-сервер, доступный для виртуальных машин в облаке. Вы можете также выполнить [развертывание Active Directory](../../solutions/windows/active-directory.md), чтобы создать инфраструктуру с DNS-сервером:
...
Чтобы полностью пройти данный сценарий, необходимо [развернуть Active Directory](../../solutions/windows/active-directory.md). Если вы используете собственные DNS-серверы, доступные для виртуальных машин облака, указывайте в сценарии свои значения DNS-суффикса и IP-адресов.
