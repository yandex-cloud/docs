...
Чтобы создать [подсеть](../concepts/network.md):
...
1. Создайте облачную сеть в каталоге и облачной сети по умолчанию:
    ...
    При создании подсети укажите имя облачной сети, в которой создается подсеть, и CIDR. Флаг `--zone` указывает зону доступности, в которой создается подсеть. Если зона доступности не указана, подсеть будет создана в зоне доступности по умолчанию.
