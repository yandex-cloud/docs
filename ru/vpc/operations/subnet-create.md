...
- Terraform
  ...
  1. Опишите в конфигурационном файле параметры подсети:
	 ...
	 * `network_id` — идентификатор сети, в которой создается подсеть.
     ...
     Чтобы добавить, изменить или удалить подсеть, используйте ресурс `yandex_vpc_subnet` с указанием на сеть в поле `netword id` (см. [пример](#examples)).
...
- Terraform
  ...
  1. Опишите в конфигурационном файле параметры ресурса `yandex_vpc_subnet`:
     ...
     Более подробную информацию о параметрах ресурсов в Terraform см. в [документации провайдера]({{ tf-provider-link }}/vpc_subnet).
