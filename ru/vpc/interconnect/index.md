...
Для обмена маршрутной информацией и дальнейшей передачи трафика необходимо настроить протокол [BGP](https://ru.wikipedia.org/wiki/Border_Gateway_Protocol). Существует ограничение на количество принимаемых префиксов, максимально возможно анонсировать 500 маршрутов. В случае превышения порога BGP-сессия будет сброшена и заново переинициализирована.
...
Поверх магистрального канала квотируются два приватных соединения.
