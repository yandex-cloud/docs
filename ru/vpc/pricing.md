...
#### Сервисные подключения {#vpc-pe}

Все сервисные подключения [любых типов](concepts/private-endpoint.md#pe-services) всех клиентов разделяют общий пул ресурсов. В этом режиме ресурсы распределяются между подключениями, пропускная способность не гарантируется и зависит от текущей нагрузки на сервис.

Каждое сервисное подключение имеет номинальный лимит [Максимальная нагрузка на одно сервисное подключение](concepts/limits.md#vpc-limits). При превышении лимита и отсутствии технической возможности такой трафик не будет обрабатываться.
...
| Аутентификация через SSO | ![image](../_assets/common/no.svg) | ![image](../_assets/common/no.svg) | ![image](../_assets/common/yes.svg) | ![image](../_assets/common/yes.svg) |
