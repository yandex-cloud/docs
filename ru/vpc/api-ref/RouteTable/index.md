...
name | **string**<br><p>Name of the route table. The name must be unique within the folder. Value must match the regular expression ``\|[a-zA-Z]([-_a-zA-Z0-9]{0,61}[a-zA-Z0-9])?``.</p> 
description | **string**<br><p>Optional description of the route table. 0-256 characters long.</p> 
labels | **object**<br><p>Resource labels, ``key:value`` pairs. No more than 64 per resource. The string length in characters for each key must be 1-63. Each value must match the regular expression ``[-_0-9a-z]*``. Each key must match the regular expression ``[a-z][-_0-9a-z]*``.</p> 
