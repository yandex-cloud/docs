...
Свяжите вашу локальную сетевую инфраструктуру с облачной с помощью сервиса [{{ interconnect-full-name }}](../../interconnect/). Это позволит не использовать публичные IP-адресов как со стороны вашей сети, так и со стороны {{ yandex-cloud }}. Вместо этого вы сможете использовать внутренние IP-адреса из [подсетей](../../vpc/concepts/network.md#subnet), диапазоны которых определите сами.
...
Если вам нужен только [SSH](../../glossary/ssh-keygen.md)-доступ к ВМ — используйте [серийную консоль](../../compute/operations/index.md#serial-console) вместо подключения через публичный IP-адрес. Рекомендуем выключать возможность подключения через серийную консоль в то время, когда вы ей не пользуетесь.