...
Cоздайте [подсеть](concepts/network#subnet), в которой облачные ресурсы будут получать IP-адреса:
...
1. Задайте размер подсети, указав CIDR: IP-адрес и маску подсети, например, `10.10.0.0/20`. Подробнее про диапазоны IP-адресов в подсетях читайте в разделе [Облачные сети и подсети](../concepts/network.md).
1. Нажмите кнопку **Создать подсеть**. 
