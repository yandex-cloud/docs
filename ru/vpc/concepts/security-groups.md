...
Группа безопасности по умолчанию разрешает входящий трафик по `SSH` и `RDP` с любых IPv4-адресов. Для рабочих ресурсов, особенно с публичными IP-адресами, [создайте](../operations/security-group-create.md) пользовательскую группу безопасности и разрешите в ней только необходимый трафик.
