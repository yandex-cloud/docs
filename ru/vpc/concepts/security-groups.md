...
* Не предназначены для защиты от DDoS. {% if product == "yandex-cloud" %}Для фильтрации больших объемов нежелательного трафика используйте [сервис защиты от DDoS-атак](../ddos-protection/index.md).{% endif %}
...
Для работы [проверок состояния](../../network-load-balancer/concepts/health-check.md) ресурсов, подключенных к сетевому балансировщику, воспользуйтесь одним из способов:

* Рекомендованный способ: в поле `predefined_target` укажите значение `loadbalancer_healthchecks`.
* Вручную разрешите передачу трафика между диапазонами адресов `198.18.235.0/24`, `198.18.248.0/24` и целевыми ресурсами.
...
Пакеты, перенаправляемые с помощью статических маршрутов на адреса других сетей, не попадут под разрешающие правила. В таком случае необходимо создать правила с явным указанием CIDR источника или назначения.
