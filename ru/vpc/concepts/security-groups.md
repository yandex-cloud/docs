...
### Рекомендации для входящего трафика {#incoming-traffic-recommendations}

Для входящего трафика мы рекомендуем правила, разрешающие:
...
В новой сети автоматически создается группа безопасности по умолчанию. Такая группа:

* разрешает любой входящий и исходящий трафик;
* назначается виртуальным машинам при подключении к подсетям новой сети, если у них нет ни одной группы безопасности.
    
    Если группа безопасности по умолчанию назначена автоматически, то при запросе информации об интерфейсе виртуальной машины, кластере управляемой базы данных или Managed Service for Kubernetes в ответах API и CLI группа безопасности не отобразится. В консоли управления появится подсказка, что действует группа безопасности по умолчанию.
