...
* [Внутренний IP-адрес](*internal-ip) для каждого сетевого интерфейса ВМ.
* Маска подсети — определяет размер [подсети](*subnet), к которой подключается сетевой интерфейс ВМ.
* [Имя хоста и FQDN для ВМ](*hostname).
...
При передаче трафика формируется [сетевой поток](*network-flow). Пакеты прямого и обратного направлений относятся к одному потоку: порядок источника и получателя не учитывается. При этом для прямого и обратного трафика маршрут выбирается независимо по IP-адресу назначения. Если адресу соответствует несколько записей с перекрывающимися префиксами назначения, используется маршрут с наиболее длинным префиксом. Например, для адреса из диапазона `172.16.0.0/24` при наличии записей с префиксами `172.16.0.0/20` и `172.16.0.0/24` будет выбрана запись с префиксом `172.16.0.0/24`.
