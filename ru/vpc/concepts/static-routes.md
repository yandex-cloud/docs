...
С помощью статической маршрутизации можно направлять трафик из подсети на заданные диапазоны IP-адресов через виртуальные машины, выполняющие роль маршрутизаторов.

Маршрутизация осуществляется с помощью таблиц маршрутизации. Таблицы содержат статические маршруты, состоящие из префикса целевой подсети в нотации CIDR и внутреннего IP-адреса следующей виртуальной машины ([next hop](https://en.wikipedia.org/wiki/Hop_(networking)#Next_hop)). 

Таблица маршрутизации привязывается к подсети и не может содержать повторяющихся префиксов. После привязки таблицы весь трафик подсети будет проходить через указанную виртуальную машину.

При создании маршрута в качестве next hop можно указать свободный внутренний IP-адрес, который не привязан ни к одной виртуальной машине. В этом случае маршрут не будет работать, пока не будет создана виртуальная машина с соответствующим IP-адресом.
...
* вместо настройки маршрута в интернет, включить для подсети доступ в интернет через NAT. Возможность находится на [стадии preview](../../overview/concepts/launch-stages.md) и предоставляется по запросу в техническую поддержку.
