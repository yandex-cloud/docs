...
Продукты Microsoft в {{ yandex-cloud }} имеют несколько тарифных планов:

* _PAYG (Pay As You Go)_ — тарифицируется только фактическое потребление. {{ price-per-hour-count-per-second }}
* _Monthly_ — продукт тарифицируется ежемесячно, при этом в первый месяц будет начислена плата только за дни после покупки. Лицензии закрепляются за конкретной виртуальной машиной, и ее пересоздание повлечет за собой приобретение новой лицензии.
