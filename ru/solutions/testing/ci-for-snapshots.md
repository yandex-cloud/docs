...
1. Убедитесь, что в выбранном каталоге есть сеть с подсетью, к которой можно подключить виртуальную машину. Для этого на странице каталога выберите сервис **Yandex Virtual Private Cloud**. Если в списке есть сеть — нажмите на нее, чтобы увидеть список подсетей. Если ни одной подсети или сети нет, [создайте их](../../vpc/quickstart.md).
...
На созданную виртуальную машину нужно установить установить набор необходимых для работы тестового приложения компонентов и веб-сервер для обработки запросов. Само приложение будет написано на языке Python 2.
...
## 5. Создайте виртуальную машину с GitLab {#create-gitlab-vm}
...
1. Нажмите плитку **Create a project**.
...
1. [Подключитесь](../../compute/operations/vm-control/vm-connect-ssh.md) к виртуальной машине c GitLab по SSH: 
...
1. В поле `snapshot_name` укажите имя снимка первой виртуальной машины. 
   ...
   В поле `subnet_name` укажите имя подсети, к которой будут подключаться виртуальные машины — имя можно получить в консоли управления, открыв нужный каталог и перейдя на страницу сервиса Yandex Virtual Private Cloud.
1. Нажмите кнопку **Commit changes**.

## 9. Проверьте работу приложения на виртуальной машине, созданной с помощью CI {#test-new-vm}
...
1. В каталоге, где создавались виртуальные машины, откройте сервис **Yandex Compute Cloud**.
