...
С помощью {{ yandex-cloud }} вы можете автоматизировать рутинные действия, например, запускать определенный скрипт после каждого коммита в ветку `master` git-репозитория. В примере ниже после каждого коммита создается и тестируется виртуальная машина.
...
Чтобы выполнить шаги сценария, нужно зарегистрироваться в {{ yandex-cloud }} и создать платежный аккаунт:
...
1. Выберите публичный образ [Ubuntu 18.04](https://cloud.yandex.ru/marketplace/products/f2e9qa7i4fmugh14tjnc).
...
1. В консоли управления {{ yandex-cloud }} выберите каталог, в котором создана виртуальная машина.
...
1. Выберите образ [GitLab](https://cloud.yandex.ru/marketplace/products/f2eb5e8deq602ccak537).
1. В блоке **Вычислительные ресурсы** выберите следующую конфигурацию:
...
1. Чтобы скрипт CI мог создавать виртуальные машины, установите CLI {{ yandex-cloud }}:
