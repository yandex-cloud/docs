...
* плата за запись пользовательских метрик через API сервиса {{ monitoring-full-name }} (см. [тарифы](../../monitoring/pricing.md)).

## Создайте необходимые ресурсы {{ iot-short-name }} {#resources-step}

[Реестр](../../iot-core/concepts/index.md#registry) и [устройство](../../iot-core/concepts/index.md#device) — основные компоненты сервиса {{ iot-short-name }} для обмена данными и командами. Чтобы устройства могли обмениваться данными, их необходимо создавать в одном реестре. В этом сценарии  
...
Триггер вызовет функцию, когда в [топике устройства](../../iot-core/concepts/topic.md) появится сообщения.
