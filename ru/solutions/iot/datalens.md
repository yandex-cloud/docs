...
{% if audience != "internal" %} * плата за обмен данными с устройствами в сервисе {{ datalens-full-name }} (см. [тарифы](../../datalens/pricing.md)). {% endif %}
...
1. (опционально) Добавьте [алиасы](../../iot-core/concepts/topic.md#----aliases):
...
Подробнее об [MQTT-топиках в сервисе {{ iot-short-name }}](../../iot-core/concepts/topic.md).
...
1. В блоке **Класс хоста** в списке **Платформа** выберите `Intel Cascade Lake`.
...
1. В блоке **Хосты** настройте подключение к вашей базе данных через публичный IP-адрес (это необходимо для доступа к базе из {{ sf-full-name }}).
    ...
    1. В открывшемся окне выберите **Зону доступности**. Например `ru-central1-a`.
    1. Выберите **Подсеть**. Например `default-ru-central1-a`.
...
Триггер вызовет функцию, когда в [топике устройства](../../iot-core/concepts/topic.md) появятся сообщения.
...
1. Настройте связи:
    ...
    1. В верхней части открывшегося окна в списке выберите чарт `My-pg-dataset — Точечная карта`.
