...
* [Интерфейс командной строки Яндекс.Облака](../../cli/operations/install-cli.md).
...
Создание кластера {{ k8s }} может занять несколько минут. Когда созданный кластер перейдет в статус `RUNNING`, вы можете перейти к создание группы узлов в этом кластере.
