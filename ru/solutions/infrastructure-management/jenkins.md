...
Jenkins позволяет построить процесс непрерывной доставки изменений в конфигурациях ваших образов средству Packer для сборки образов виртуальных машин, используемых в Яндекс.Облаке. 
...
1. Установите интерфейс командной строки Яндекс.Облака.
1. Установите [Terraform](terraform-quickstart.md) и настройте его для работы с Яндекс.Облаком.
1. Настройте git-клиент. Если вы работаете под Windows, используйте [git-bash](https://gitforwindows.org).
1. Создайте ответвление [репозитория](https://github.com/yandex-cloud/examples) с примерами в своем аккаунте на GitHub.
1. Подготовьте SSH-ключ для доступа к виртуальным машинам.
1. [Создайте](../../vpc/operations/network-create.md) облачную сеть и подсеть в выбранной зоне доступности.
...
## 2. Создайте виртуальную машину с Jenkins {#create-jenkins-vm}
...
1. В блоке **Публичные образы** нажмите кнопку **Выбрать**. В открывшемся окне выберите образ **Jenkins**.
...
## 3. Установите Packer {#install-packer}
...
1. [Подключитесь](../../compute/operations/vm-control/vm-connect-ssh.md) к виртуальной машине по протоколу SSH. Для этого можно использовать утилиту `ssh` в Linux и macOS и программу `PuTTY` для Windows.
...
## 4. Настройте Jenkins {#configure-jenkins}
...
## 5. Настройте задачу для Jenkins {#jenkins-job}
...
## 6. Настройте GitHub-репозиторий {#configure-github-repo}
...
### 6.1. Настройте Webhook {#configure-webhook}
...
### 6.2. Добавьте на GitHub SSH-ключ {#add-ssh-key}
...
## 7. Создайте образ с помощью Jenkins {#create-image}
...
## 8. Разверните образы {#deploy-image}
