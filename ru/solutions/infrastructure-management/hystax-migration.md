...
1. Откройте в браузере публичный IP-адрес ВМ `hystax-acura-vm`. Через 8-20 минут откроется экран начальной настройки Hystax Acura.
...
1. В поле **Subnet ID** укажите идентификатор и CIDR подсети, к которой будут подключены ВМ после миграции.
1. Отредактируйте **Flavor name** (параметры создаваемой ВМ) следующим образом: `<platform>-<cpu>-<ram>-<core_fraction>`. Например, `2-8-16-100`.
