...
- CLI
   ...
   * `create-boot-disk` — создать для ВМ новый диск:
      ...
      * `size` — размер диска;
      * `image-id` — идентификатор образа диска. В данном случае используйте `image_id` из [описания продукта](https://cloud.yandex.ru/marketplace/products/hystax/hystax-acura-live-cloud-migration-3-7) в {{ marketplace-name }}.
