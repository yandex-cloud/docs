...
- CLI
   ...
   * `create-boot-disk` — создать для ВМ новый диск:
      ...
      * `size` — размер диска;
      * `image-id` — идентификатор образа диска. В данном случае используйте `image_id` из [описания продукта](https://cloud.yandex.ru/marketplace/products/hystax/hystax-acura-disaster-recovery-3-7) в {{ marketplace-name }}.
...
1. В блоке **Machines Groups** создайте группу защищаемых ВМ, например, `Srv-1`.
...
1. Скачайте и установите агент на ваши ВМ, которые предстоит защитить:
   ...
   - VMware
     ...
     1. В выпадающем списке выберите группу ВМ, для которой будут подготовлены агенты, например, `Srv-1`.
...
1. В блоке **Machines Groups** разверните группу ВМ, например, `Srv-1`.
...
Повторите шаги, указанные для [тестового восстановления](#run-tests).
