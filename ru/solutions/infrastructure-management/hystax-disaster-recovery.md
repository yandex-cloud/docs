# Аварийное восстановление в Яндекс.Облако с помощью Hystax Acura

Вы можете защитить ваши цифровые ресурсы с помощью решения Hystax Acura. Hystax Disaster Recovery обеспечивает резервирование ресурсов вне зависимости от их размещения. Ресурсы могут располагаться как на железных хостах  "bare metal" или виртуальных машинах, работающих в собственных ЦОД-ах, так и на виртуальных машинах различных облачных платформ. Поддерживаемые исходные платформы: Amazon Web Services, Google Cloud Platform, Microsoft Azure, Oracle Cloud, Alibaba Cloud, VMware, Hyper-V, OpenStack, физические серверы.

Для аварийного восстановления, в вашем облаке нужно создать ВМ с Hystax Acura, под управлением которой будет настраиваться и осуществляться сценарий аварийного восстановления. Сам процесс восстановления будет выполнять служебная ВМ с Hystax Acura Cloud Agent, которая восстановит ваши ресурсы, расположенные на физических или виртуальных машинах, в Яндекс.Облако. После запуска процедуры аварийного восстановления будут созданы реплики, которые будут для развертывания инфраструктуры. Агенты устанавливаются на виртуальные машины, резервирование которых предстоит осуществить.
...
1. [Подготовьте Hystax Acura Cloud Agent на исходной платформе](#prepare-agent).
...
## Добавьте платформу {#set-up-dr-flow}
...
Агенты устанавливаются на виртуальные машины, которые предстоит резервировать в Яндекс.Облаке. Чтобы получить и установить агент:
...
1. Войдите в раздел меню **DR Plans**
1. Нажмите кнопку **Add**.
1. Введите название плана аварийного восстановления: `YC Disaster Recovery`
1. Выберите один из режимов: 
    - ‘Basic’ режим помогает создать план со стандартными настройками
    - 'Expert’ использует скрипт JSON, позволяющий более гибко настраивать план аварийного восстановления (см. [подробное описание синтаксиса](https://docs.hystax.com/dr_overview.html#dr-plan-syntax)).
...
Убедитесь, что для каждой из ВМ указан правильный IP-адрес.

{% endnote %}

1. По окончании создания плана аварийного восстановления выбранные  ВМ перейдут в статус **Protected**
...
1. Убедитесь что план актуальный и корректный
1. Нажмите **Next**
1. В следующем окне укажите время точки восстановления, из которой будут созданы целевые узлы
1. Нажмите кнопку **Run Recover**

Дождитесь окончания процедуры аварийного восстановления и убедитесь, что все требуемые ресурсы успешно перенесены и ваши приложения готовы к работе.
