...
Сценарий описывает развертывание в Яндекс.Облаке группы доступности Always On. При выходе из строя одного или нескольких узлов группы система продолжит функционировать благодаря репликации и аварийному переключению базы данных, работающей на запись — таким образом обеспечивается высокая доступность СУБД. 
...
Перед тем, как разворачивать группу доступности, нужно зарегистрироваться в Облаке и создать платежный аккаунт:
...
Вы можете воспользоваться [мобильностью лицензий](../../compute/qa/licensing) и использовать собственную лицензию MSSQL Server в Яндекс.Облаке.
...
1. Создайте сеть с именем `ya-network`:
    ...
    - Bash 
      ...
      [Установите](../../cli/operations/install-cli.md) интерфейс командной строки Яндекс.Облака, чтобы использовать команды CLI в Bash. 
    ...
    - PowerShell 
      ...
      [Установите](../../cli/operations/install-cli.md) интерфейс командной строки Яндекс.Облака, чтобы использовать команды CLI в PowerShell. 
...
1. Подключитесь к ВМ `jump-server-vm` [с помощью RDP](../../compute/vm-connect/rdp.md). Используйте логин `Administrator` и ваш пароль. 
...
Шаги этого раздела следует выполнить на всех трех серверах баз данных: `ya-mssql1`, `ya-mssql2` и `ya-mssql3`.
...
1. Установите MSSQL. Смонтируйте образ, выполните установку и отсоедините образ:
   ...
   Повторите шаги 2-11 для ВМ `ya-mssql2` и `ya-mssql3`.
