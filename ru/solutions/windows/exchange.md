...
1. [Настройте балансировщик нагрузки](#set-up-load-balancer)
...
* плата за балансировку трафика (см. [тарифы {{ load-balancer-full-name }}](../../load-balancer/pricing.md));
...
1. Подключитесь к ВМ `fsw-vm` [с помощью RDP](../../compute/operations/vm-connect/rdp.md). Используйте логин `Administrator` и пароль `P@ssw0rd11`.
1. На ВМ `fsw-vm` запустите RDP и подключитесь к виртуальной машине `ad-vm-a`. Используйте логин `Administrator` и пароль `P@ssw0rd11`.
...
1. Создайте лес Active Directory:
   ...
   Windows перезапустится автоматически. Снова подключитесь к ВМ `ad-vm-a`. Используйте логин `yantoso\Administrator` и пароль `P@ssw0rd11` Снова откройте PowerShell.
...
1. Подключитесь к ВМ `fsw-vm` [с помощью RDP](../../compute/operations/vm-connect/rdp.md). Используйте логин `Administrator` и пароль `P@ssw0rd11`.
1. На ВМ `fsw-vm` запустите RDP и подключитесь к виртуальной машине `ad-vm-b`. Используйте логин `Administrator` и пароль `P@ssw0rd11`.
1. Создайте временную папку:
...
1. Добавьте контроллер в домен:
   ...
   Windows перезапустится автоматически. Снова подключитесь к ВМ `ad-vm-b`. Используйте логин `yantoso\Administrator` и пароль `P@ssw0rd11` Снова откройте PowerShell.
...
1. Добавьте сервер в домен:
   ...
   Windows перезапустится автоматически. Снова подключитесь к ВМ `fsw-vm`. Используйте логин `yantoso\Administrator` и пароль `P@ssw0rd11` Снова откройте PowerShell.
...
1. На ВМ `fsw-vm` запустите RDP и подключитесь к ВМ `vm-exchange-a` с помощью RDP. Используйте логин `Administrator` и пароль `P@ssw0rd11`. Запустите PowerShell.
1. Настройте DNS-клиент:
...
1. Добавьте ВМ `vm-exchange-a` в домен:
   ...
   После перезагрузки зайдите на ВМ, используя учетную `yantoso\Administrator` и пароль `P@ssw0rd11`. Снова откройте PowerShell.
...
1. Подключитесь к ВМ `fsw-vm` с помощью RDP. 
1. На ВМ `fsw-vm` запустите RDP и подключитесь к ВМ `vm-exchange-b` с помощью RDP. Используйте логин `Administrator` и пароль `P@ssw0rd11`. Запустите PowerShell.
1. Настройте DNS-клиент:
...
1. На ВМ `fsw-vm` запустите RDP и подключитесь к ВМ `vm-exchange-a` с помощью RDP. Используйте логин `yantoso\Administrator` и пароль `P@ssw0rd11`.
...
## Настройте балансировщик нагрузки {#set-up-load-balancer}
...
- Консоль управления
  ...
  Чтобы создать [сетевой балансировщик](../../load-balancer/concepts/index.md):
...
1. Подключитесь к ВМ `fsw-vm` с помощью RDP. 
1. Подключитесь к ВМ `vm-edge-a` с помощью RDP. Используйте логин `Administrator` и пароль `P@ssw0rd11`. Запустите PowerShell.
1. Создайте временную папку:
...
1. Настройте удаленный доступ к папке с дистрибутивами, находящейся на сервере `fsw-vm`:
   ...
   Введите логин `yantoso\Administrator` и пароль `P@ssw0rd11`.
...
1. Подключитесь к ВМ `fsw-vm` с помощью RDP. 
1. Подключитесь к ВМ `vm-edge-b` с помощью RDP. Используйте логин `Administrator` и пароль `P@ssw0rd11`. Запустите PowerShell.
1. Создайте временную папку:
...
Чтобы перестать платить за развернутые серверы, достаточно удалить все созданные [виртуальные машины](../../compute/operations/vm-control/vm-delete.md) и [балансировщик](../../load-balancer/operations/load-balancer-delete.md):
