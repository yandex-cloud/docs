...
1. Найдите публичный или внутренний IP адрес SFTP-клиента в консоли Яндекс.Облака в разделе настроек виртуальной машины. 
    ...
    Внутренние адреса SFTP-клиента и SFTP-сервера должны находиться в одной подсети или быть связаны через настройки марщрутизации.
...
1. Установите переменные окружения для корректной работы скрипта. Для этого откройте файл `~/.bash_profile`
    ...
    vi ~/.bash_profile
    ```
1. Добавьте следующие строчки в конец файла, подставив соответстующие значения:
    ```
...
1. Перешлите полученный архив на SFTP-сервер:
    ...
    * `backup_$(hostname)_$(date "+%Y%m%d_%H%M%S").tar.gz` – добавить к названию архива название компьютера, а также дату и время, когда был создан архив. 
        ...
        Название архива на сервере будет выглядеть примерно так: `backup_ftp-server.ru-central1.internal_20190803_180228.tar.gz`. Это позволит не потерятся в навигации по списку бэкапов на сервере.
