...
1. Выполните скрипт для настройки безопасности базы данных:
    ...
    ```
1. Войдите в командную строку базы данных и введите `root` пароль, который был создан на передыдущем шаге:
    ```bash
    mysql -u root -p
    ```
...
1. Настройте профиль резервного копирования, создав `label`:
    ...
    label
    ```
    Присвойте название новому `volume`:
    ```bash
...
1. Запустите резервное копирование:
    ...
    Выберите `1`, чтобы запустить `BackupFiles`:
    ```bash
...
1. Проверьте статус резервного копирования:
    ...
    status director
    ```    
    Убедитесь, что резервное копирование выполняется:
    ```
    Running Jobs:
