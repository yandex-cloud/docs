...
1. Убедитесь, что в выбранном каталоге есть сеть с подсетями в зонах доступности `ru-cental1-a`, `ru-central1-b` и `ru-cental1-c`. Для этого на странице каталога выберите сервис **{{ vpc-name }}**. Если нужной [сети](../../vpc/operations/network-create.md) или [подсетей](../../vpc/operations/subnet-create.md) нет, создайте их.
...
1. В блоке **Распределение** выберите три зоны доступности (`ru-cental1-a`, `ru-central1-b` и `ru-cental1-c`), чтобы обеспечить отказоустойчивость хостинга.
...
### Добавьте ресурсные записи {#add-records}
