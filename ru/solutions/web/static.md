...
1. В поле **Имя** введите имя бакета, например, `www.example.com`. Имя бакета будет использоваться в доменном имени сайта: `https://www.example.com.website.yandexcloud.net`.
...
1. Проверьте, что главная страница веб-сайта открывается. Для этого подключитесь к сайту через браузер по ссылке вида `https://{имя-бакета}.website.yandexcloud.net`.
1. Проверьте, что страница ошибки открывается. Для этого подключитесь к сайту через браузер по ссылке вида `https://{имя-бакета}.website.yandexcloud.net/error-check`.
...
В инструкции ниже описана настройка внешнего DNS-сервиса на примере [reg.ru](https://www.reg.ru/) для доменных имен `www.example.com.website.yandexcloud.net` и `www.example.com`:
...
После настройки и проверки доступности сайта загрузите в бакет остальные файлы, необходимые для работы сайта. Используйте для этого [консоль управления](https://console.cloud.yandex.ru/), [API](../../storage/s3/api-ref/object/upload.md) или один из доступных [инструментов для работы с Object Storage](../../storage/instruments/).