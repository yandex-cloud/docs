...
1. В поле **Имя** введите имя бакета, например, `www.example.com`. Имя бакета будет использоваться в доменном имени сайта: `https://www.example.com.[!KEYREF s3-web-host]`.
...
1. Проверьте, что главная страница веб-сайта открывается. Для этого подключитесь к сайту через браузер по ссылке вида `https://{имя-бакета}.[!KEYREF s3-web-host]`.
1. Проверьте, что страница ошибки открывается. Для этого подключитесь к сайту через браузер по ссылке вида `https://{имя-бакета}.[!KEYREF s3-web-host]/error-check`.
...
В инструкции ниже описана настройка внешнего DNS-сервиса на примере [reg.ru](https://www.reg.ru/) для доменных имен `www.example.com.[!KEYREF s3-web-host]` и `www.example.com`:

1. На вкладке **Веб-сайт** на странице бакета в [консоли управления](https://console.cloud.yandex.ru) найдите адрес сайта.
...
1. Создайте CNAME-запись со значениями полей:
   * **Subdomain** — `www`.
   * **Canonical name** — адрес сайта с точкой на конце, например, `www.example.com.[!KEYREF s3-web-host].`.
...
После настройки и проверки доступности сайта загрузите в бакет остальные файлы, необходимые для работы сайта. Используйте для этого [консоль управления](https://console.cloud.yandex.ru), [API](../../storage/s3/api-ref/object/upload.md) или один из доступных [инструментов для работы с [!KEYREF objstorage-name]](../../storage/instruments/).