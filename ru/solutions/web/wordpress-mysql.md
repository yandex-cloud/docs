...
2. Перейдите к блоку конфигурации подключения к кластеру `wp-mysql-tutorial-db-cluster`:
   ...
   - `<DB_HOST>` — хост вида `c-<идентификатор мастера>.rw.mdb.yandexcloud.net`. Чтобы узнать идентификатор мастера, [получите список хостов](../../managed-mysql/operations/hosts.md#list) через CLI и скопируйте `CLUSTER ID` с ролью `MASTER`. Также мастера можно узнать в консоли, на вкладке "База данных" нажмите "...", далее "Подключится" и найдите строчку "mysql --host=ХХХХ-ХХХХХХХХХХ.mdb.yandexcloud.net ", здесь ХХХХ-ХХХХХХХХХХ.mdb.yandexcloud.net это идентификатор мастера.
