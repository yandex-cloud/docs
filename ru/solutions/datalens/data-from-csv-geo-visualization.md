...
1. Введите название дашборда **ДТП** и нажмите **Создать**.

   ![image](../../_assets/datalens/solution-07/48-name-dashbord.png)
   
{% note info %}

Если это первый дашборд, созданный в каталоге, он откроется сразу после создания. Если в каталоге уже есть другие дашборды, то откроется их список. В этом случае нужно из списка открыть дашборд **ДТП**.
