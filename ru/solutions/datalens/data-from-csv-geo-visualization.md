...
1. [Добавьте селекторы на дашборд](data-from-csv-geo-visualization.md#step8).
...
1. Откройте главную страницу [{{ datalens-short-name }}](datalens.yandex.ru) и нажмите **Создать подключение**.
...
1. Дата и время ДТП показаны в следующих полях:
   ...
   В таком виде они не будут использовться в датасете. Скройте их, нажав значок видимости.
...
1. Для отображения сведений о дате и времени ДТП добавьте новое поле.
   ...
   В качестве временного решения проблемы с часовыми поясами от 24.02.2020 следует использовать формулу: 
   ```
...
1. В появившемся окне визарда нажмите на поле, в котором по умолчанию указано **Столбчатая диаграмма**, и выберите тип чарта **Тепловая карта**.
...
1. В диалоговом окне укажите название чарта: **ДТП — Тепловая карта** и нажмите **Сохранить**.
...
1. Проанализируйте количество ДТП по регионам. 
   ...
   1. Сохраните диаграмму по кнопке **Сохранить как**. 
      ...
      Укажите название чарта: **ДТП — Регионы кол-во** и сохраните его.
...
1. Проанализируйте смертность в ДТП по регионам.
   ...
   1. Замените показатель **Кол-во ДТП** на **Смертность в ДТП** с помощью перетаскивания.
      ...
      На этот раз в лидерах с большим отрывом Чечня, затем Кабардино-Балкария и Ингушетия.

   1. Сохраните чарт по кнопке **Сохранить как**

      ![image](../../_assets/datalens/solution-07/29-save-chart-as.png)

      с названием **ДТП — Смертность по регионам**.
...
1. Проанализируйте количество ДТП и смертность по неделям.
   ...
   1. Сохраните чарт по кнопке **Сохранить как**
      ...
      с названием **ДТП — ДТП и смертность по неделям**.
...
1. Теперь проанализируйте количество ДТП и смертность по дням недели.
   ...
   1. Сохраните чарт по кнопке **Сохранить как**
      ...
      с названием **ДТП — ДТП и смертность по дням недели**.
...
1. Проанализируйте количество ДТП и смертность по часам суток.
   ...
   1. Сохраните чарт по кнопке **Сохранить как**
      ...
      с названием **ДТП — ДТП и смертность по часам в течение дня**.
...
1. Введите название дашборда **ДТП 2018** и нажмите **Создать**.

   ![image](../../_assets/datalens/solution-07/48-name-dashbord.png)
   
{% note info %}

Если это первый дашборд, созданный в каталоге, он откроется сразу после создания. Если в каталоге уже есть другие дашборды, то откроется их список. В этом случае нужно из списка открыть дашборд **ДТП 2018**.
...
1. Добавьте первый чарт.
   ...
   1. В поле **Чарт** выберите из списка чартов созданный ранее чарт **ДТП — Тепловая Карта**.
...
1. Повторите предыдущие три шага для всех созданных чартов, добавив их на дашборд.

## Шаг 8. Добавьте селекторы на дашборд {#step8}
...
Необходимо обогатить созданный ранее датасет **ДТП** новыми поля.
...
1. Перетащите:
   ...
   - поле **Кол-во ДТП** — в раздел **Цвета**;
   - поле **Полигон** — в раздел **Геополигоны**.
