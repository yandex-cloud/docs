...
`ShardIterator` | Позиция в сегменте, с которой будет начато последовательное чтение сообщений. Значение интератора определяется порядковым номером сообщения в сегменте.<br/><br/>**Тип**: Строка.<br/>**Размер**: `1`-`512` символов.<br/>**Обязательный**: Да.
