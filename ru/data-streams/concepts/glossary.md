...
Поток данных (Stream) — это именованный набор сообщений. Запись и чтение сообщений в {{ yds-name }} ведутся через потоки. Потоки данных создаются на основе {{ ydb-full-name }} и хранятся в базах данных.
...
Возможна настройка потока данных таким образом, что количество сегментов будет увеличиваться при росте скорости записи в этот поток данных. Подробнее в разделе [автопартиционирование в документации {{ ydb-short-name }}]({{ ydb.docs }}{{ lang }}/concepts/datamodel/topic#autopartitioning).
...
Часть данных может потеряться при перегрузке отдельных [сегментов](#shard) или всего [потока](#stream-concepts). [Автопартиционирование]({{ ydb.docs }}{{ lang }}/concepts/datamodel/topic#autopartitioning) автоматически добавляет сегменты и распределяет нагрузку. Это помогает избежать потерь данных.
