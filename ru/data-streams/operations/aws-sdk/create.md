...
- Python
  ...
  * Идентификатор существующей [бессерверной]{% if audience == "external" %}(../../../ydb/pricing/serverless.md){% else %}(https://cloud.yandex.ru/docs/ydb/pricing/serverless){% endif %} базы данных {{ ydb-full-name }}, например `cc8028jgtuabcqutgtbv`. Как создать новую базу данных, читайте в [документации {{ ydb-short-name }}]{% if audience == "external" %}(../../../ydb/quickstart/create-db.md){% else %}(https://cloud.yandex.ru/docs/ydb/quickstart/create-db){% endif %}.
