...
- Python
  ...
  * Идентификатор существующей [бессерверной]{% if audience == "external" %}(../../../managed-ydb/pricing/serverless.md){% else %}(https://cloud.yandex.ru/docs/managed-ydb/pricing/serverless){% endif %} базы данных {{ ydb-full-name }}, например `cc8028jgtuabcqutgtbv`. Как создать новую базу данных, читайте в [документации {{ ydb-short-name }}]{% if audience == "external" %}[](../../../managed-ydb/quickstart.md#create-db){% else %}(https://cloud.yandex.ru/docs/managed-ydb/quickstart#create-db){% endif %}.
