# Управление потоками данных

Вы можете [посмотреть](#list-data-streams) список потоков, [изменить](#edit-data-stream) их настройки, [создать](#create-data-stream) новый поток или [удалить](#delete-data-stream) cуществующий.

## Создать поток данных {#create-data-stream}

{% list tabs %}

- Консоль управления

  {% include [create-stream-via-console](../../_includes/data-streams/create-stream-via-console.md) %}

{% endlist %}

## Посмотреть список потоков {#list-data-streams}

{% list tabs %}

- Консоль управления

  1. В [консоли управления]({{ link-console-main }}) выберите каталог, для которого вы хотите получить список потоков данных.
  1. В списке сервисов выберите **{{ yds-full-name }}**.
  1. Перейдите на вкладку **Потоки**.

{% endlist %}

## Изменить настройки потока {#edit-data-stream}

{% list tabs %}

- Консоль управления

  1. В [консоли управления]({{ link-console-main }}) выберите каталог, в котором вы хотите изменить настройки потока данных.
  1. В списке сервисов выберите **{{ yds-full-name }}**.
  1. Нажмите значок ![horizontal-ellipsis](../../_assets/horizontal-ellipsis.svg) в строке нужного потока данных и выберите **Изменить**.
  1. Настройте параметры потока данных:
     1. Выберите необходимое число сегментов.
     1. Задайте максимальную пропускную способность сегмента в секунду.
     1. Укажите максимальное время хранения данных.
  1. Нажмите кнопку **Изменить**.

{% endlist %}

## Удалить поток {#delete-data-stream}

{% list tabs %}

- Консоль управления

  1. В [консоли управления]({{ link-console-main }}) выберите каталог, в котором вы хотите удалить поток данных.
  1. В списке сервисов выберите **{{ yds-full-name }}**.
  1. Нажмите значок ![horizontal-ellipsis](../../_assets/horizontal-ellipsis.svg) в строке нужного потока данных и выберите пункт **Удалить**.
  1. В открывшемся окне нажмите кнопку **Удалить**.

{% endlist %}