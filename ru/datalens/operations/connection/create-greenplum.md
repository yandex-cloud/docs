...
1. Выберите тип подключения:
   ...
   - Выбрать в организации {#organization}
     ...
     * **Тип хоста**. Выберите тип хоста:
       ...
       * **Особые FQDN** — позволяет выбрать для подключения [особый FQDN]({{ link-docs }}/managed-greenplum/operations/connect/fqdn#fqdn-master), который всегда указывает на первичный хост-мастер {{ GP }}.
