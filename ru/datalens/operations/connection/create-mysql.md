...
1. Выберите тип подключения:
   ...
   - Выбрать в организации {#organization}
     ...
     * **Тип хоста**. Выберите тип хоста:
       ...
       * **Особые FQDN** — позволяет выбрать для подключения [особые FQDN]({{ link-docs }}/managed-mysql/operations/connect/#special-fqdns), которые всегда указывают на текущий хост-мастер или на наименее отстающую от мастера реплику в кластере {{ MY }}.
   ...
   - {{ connection-manager-name }} {#conn-man}
     ...
     Выберите [подключение]({{ link-docs }}/metadata-hub/concepts/connection-manager) к кластеру с управляемой БД {{ MY }}, созданное в сервисе {{ connection-manager-full-name }}:
