---
title: Особенности JOIN в {{ datalens-full-name }}
description: Из статьи вы узнаете об особенностях использования оператора JOIN в {{ datalens-full-name }}, а также ознакомитесь с видами запросов с JOIN в {{ CH }}.
---

# Особенности JOIN в {{ datalens-full-name }}

В интерфейсе создания датасета можно [объединять данные](../../../dataset/create-dataset.md#links), перетаскивая таблицы на рабочую область и настраивая связи между ними через оператор `JOIN`. Для таблиц {{ ytsaurus-name }} такое объединение вызывает ошибку. Ошибка возникает из-за структуры хранения таблиц и схемы выполнения запросов в {{ ytsaurus-name }} по умолчанию. Чтобы избежать ошибки, используйте [SQL-запрос к источнику](../../../dataset/create-dataset.md#add-data).
