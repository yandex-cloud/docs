# Рекомендации по созданию подключения к ClickHouse over YT

## Разделение датасетов по подключениям {#separation-by-connections}

* При работе с CHYT вместо публичной клики `*ch_datalens` используйте приватную. Если у вас еще нет приватной клики, вы можете [создать](https://yt.yandex-team.ru/docs/description/chyt/cliques/start) ее.
...
* Для создания быстрых дашбордов вместо публичной клики `*ch_datalens` используйте приватную. Если у вас еще нет приватной клики, вы можете 
[создать](https://yt.yandex-team.ru/docs/description/chyt/cliques/start) ее.
...
Чаще всего скорость обработки запросов снижается при чтении (не только при непосредственном чтении с диска, но и при разжатии данных, перекладывании из формата хранения YT в CH и т. д.). В этом случае не всегда помогает увеличение количества инстансов, поэтому рекомендуем изменить формат хранения данных:
...
* Старайтесь не использовать динамические таблицы. Любое чтение из динамической таблицы требует чтения версий и всего ключа. При этом ридеры менее паралельные, увеличиваются накладные расходы на конвертацию в формат ClickHouse и необходимо мержить строки с одинаковыми ключами. В итоге чтение из динамических таблиц гораздо медленнее. Точная цифра зависит от таблицы и паттерна работы с ней, но разница может доходить до 10 раз.
