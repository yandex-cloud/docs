...
1. Выберите тип подключения:
   ...
   - Выбрать в организации {#organization}
     ...
     * **Тип хоста**. Выберите тип хоста:
       ...
       * **Особые FQDN** — позволяет выбрать для подключения [особые FQDN](../../../managed-postgresql/operations/connect/fqdn.md#special-fqdns), которые всегда указывают на текущий хост-мастер или на наименее отстающую от мастера реплику в кластере {{ PG }}.
   ...
   - {{ connection-manager-name }} {#conn-man}
     ...
     Выберите [подключение](../../../metadata-hub/concepts/connection-manager.md) к кластеру с управляемой БД {{ PG }}, созданное в сервисе {{ connection-manager-full-name }}:
