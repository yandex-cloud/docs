...
1. В разделе **Базы данных** выберите подключение **{{ PG }}**.
...
1. Выберите тип подключения:
   ...
   - Выбрать в организации {#organization}
     ...
     * **Тип хоста**. Выберите тип хоста:
       ...
       * **Особые FQDN** — позволяет выбрать для подключения [особые FQDN]({{ link-docs }}/managed-postgresql/operations/connect/fqdn#special-fqdns), которые всегда указывают на текущий хост-мастер или на наименее отстающую от мастера реплику в кластере {{ PG }}.
   ...
   - {{ connection-manager-name }} {#conn-man}
     ...
     Выберите [подключение]({{ link-docs }}/metadata-hub/concepts/connection-manager) к кластеру с управляемой БД {{ PG }}, созданное в сервисе {{ connection-manager-full-name }}:
     ...
     * **Хост**. Выберите хост из списка доступных в кластере {{ PG }}.
