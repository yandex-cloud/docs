...
1. Выберите параметры загрузки файла:
    ...
    - **Заголовок**. Выберите опцию заголовка. Если выберете **Есть**, то {{ datalens-short-name }} использует первую строку данных в качестве
    заголовка.
1. Нажмите **Подключить**. Подключение появится в списке со статусом `materializing`. Это значит, что процесс загрузки данных
из CSV-файла в хранилище {{ datalens-short-name }} еще не завершен. Чтобы создать датасет на основе подключения к CSV-файлу,
дождитесь, когда статус подключения изменится на `materialized`.