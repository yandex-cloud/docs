...
* **Использовать предыдущее** — заменять пустые значения в чарте на значение предыдущей точки на оси. Доступно в настройках оси Y для [диаграммы с областями](../../visualization-ref/area-chart.md). ||
