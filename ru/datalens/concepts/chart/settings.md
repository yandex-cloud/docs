...
{% if audience == "internal" %}Фид комментариев | Id канала комментариев из другого чарта. Используется для автоматического дублирования общих для разных чартов комментариев. Настройка доступна только для чартов с временными рядами (у которых в секции **X** или **Y** используется измерение с типом **Дата** или **Дата и время**).<br/>Подробнее см. [инструкцию](../../operations/chart/use-comments.md).{% endif %}
