...
Примеры создания мультидатасетного чарта смотрите в [инструкции](../../../datalens/operations/chart/create-multidataset-chart.md).
...
* **{{ monitoring-short-name }}**. Позволяют построить график по данным {{ monitoring-full-name }}. Поверх этого источника не может быть построен датасет, работать с ним можно только с помощью QL-чартов. Также можно экспортировать в {{ datalens-short-name }} готовый график из {{ monitoring-short-name }}. Его можно отредактировать, сохранить и разместить на дашборде.
