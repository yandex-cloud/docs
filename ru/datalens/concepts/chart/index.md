...
В отличие от [обычных чартов](#dataset-based-charts), в QL-чартах логика использования визарда смещена в сторону SQL-запроса, то есть визард только отображает данные из запроса.
...
* не поддерживают [материализацию](../dataset/settings.md#materialization) {% if audience != "internal" %} и [публикацию](../datalens-public.md) {% endif %} данных ;
