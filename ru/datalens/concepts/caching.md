...
Кеш {{ datalens-short-name }} не сохраняет исходные данные из источника, а только временно хранит результаты отдельных запросов. Для хранения аналитических [витрин данных]({{ link-docs }}/glossary/datamart) вы можете использовать, например, [{{ mch-full-name }}](../../managed-clickhouse/).
