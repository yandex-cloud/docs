...
Вы также можете указать значения параметров в ссылке на чарт. Для этого нужно добавить знак `?` в конце URL, после него без пробела написать имя параметра, указать знак `=` и без пробела указать значение параметра. Можно перечислять несколько параметров в URL с помощью разделителя `&`. {% if audience == "internal" %}Пример ссылки: `https://datalens.yandex-team.ru/wizard/5fw75yaukoamx-parametrizirovannyy-pokazatel?measure_control=выручка&scale=year`, где `measure_control` и `scale` — названия параметров, а `выручка` и `year` — их значения.{% endif %}
