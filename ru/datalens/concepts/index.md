...
{{ datalens-full-name }} — это сервис для {% if lang == "ru" and audience != "internal" %}[бизнес-аналитики](../../glossary/business-analytics){% else %}бизнес-аналитики{% endif %}. Сервис позволяет подключаться к различным источникам данных, строить визуализации, собирать дашборды и делиться полученными результатами.
...
{% if product == "yandex-cloud" %}{% if audience != "internal" %} Вы можете создавать объекты сущностей самостоятельно, а также использовать готовые решения из [{{ datalens-short-name }} Marketplace](marketplace.md). {% endif %}{% endif %}

{% if product == "cloud-il" %}

## Воркбуки и коллекции {#workbooks-and-collections}
