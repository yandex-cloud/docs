...
_Чарт_ — это визуализация данных из источника данных, датасета в виде таблицы, диаграммы или картограммы.
 ...
 * **Географическая карта**:
   - [Карта](#map-chart)
     ...
     {% if audience == "internal" %} - [Полилинейная карта](#polyline-map-chart) {% endif %}
 * **Другое**:  
   - [Индикатор](#indicator)
...
* менять порядок внутри типа визуализации, при этом порядок типов визуализации остается неизменным (сверху вниз: точечная карта, фоновая карта, тепловая карта{% if audience == "internal" %}, полилинейная карта {% endif %}).
