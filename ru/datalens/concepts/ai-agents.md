---
title: AI-агенты в {{ datalens-full-name }}
description: Из этой статьи вы узнаете, как AI-агенты взаимодействуют с {{ datalens-name }}.
---

# AI-агенты в {{ datalens-full-name }}

AI-агенты [работают](../operations/ai-agents.md) с {{ datalens-name }} как полноценный инструмент автоматизации. Они могут создавать и изменять подключения, датасеты, чарты и дашборды, анализировать зависимости между объектами, клонировать и переносить контент между воркбуками.

Для этого {{ datalens-name }} предоставляет три уровня интеграции:
...
| Знания | [Навыки `datalens-skills`](https://github.com/datalens-tech/datalens-skills) | Доменные знания о {{ datalens-name }}: модель объектов, различия установок, правильные приемы работы |
...
Все три уровня построены поверх Public API и дополняют друг друга: навыки учат AI-агента, *как* работать с {{ datalens-name }}, а MCP-сервер и SDK дают ему *возможность* это делать.
...
Public API {{ datalens-name }} доступно по адресу `api.datalens.tech`, описывается спецификацией OpenAPI и построено в RPC-стиле: вы вызываете именованные команды с JSON-параметрами. Спецификация OpenAPI доступна по адресу `{{ api-host-datalens }}/json/`. Для аутентификации используются [IAM-токен]({{ link-docs }}/iam/concepts/authorization/iam-token) {{ yandex-cloud }} и идентификатор организации в заголовке `x-dl-org-id`. Подробнее в разделе [Работа с Public API](../operations/api-start.md).

Модель данных {{ datalens-name }}, с которой работает AI-агент:
...
[datalens-sdk](https://pypi.org/project/datalens-sdk/) — типизированный Python SDK для API {{ datalens-name }}. Он подходит для всего, что нужно сохранить в виде кода: скрипты миграции, генерация дашбордов, экспорт и клонирование объектов, CI-автоматизация.
...
* Подключает слои с деталями конкретной установки {{ datalens-name }}, если они доступны в окружении.
...
| Ответы на вопросы о {{ datalens-name }}, выбор подхода, знание модели объектов | Навык `datalens` |
| AI-отчеты в виде автономных HTML-страниц | Навык `datalens-html-pages` |
| Программный доступ из других языков и сред | Public API напрямую (спецификация — `{{ api-host-datalens }}/json/`) |
...
| `DATALENS_API_URL`            | —  | `{{ api-host-datalens }}` | Базовый URL API |
...
| Навык | Назначение |
| --- | --- |
| `datalens` | Стартовая точка: что такое {{ datalens-name }}, модель объектов, выбор интерфейса. Маршрутизирует к остальным навыкам |
| `datalens-sdk` | Работа с {{ datalens-name }} из Python: подключения, датасеты, чарты, дашборды. Сам подготавливает окружение и загружает инструкции, соответствующие установленной версии SDK |
| `datalens-html-pages` | Создание, валидация и публикация автономных HTML-страниц (AI-отчетов), которые {{ datalens-name }} строит в изолированном iframe |
...
* AI-агент работает с объектами {{ datalens-name }}, но не интерпретирует бизнес-значения метрик и не взаимодействует с веб-интерфейсом. Например, AI-агент не может сделать скриншот или нажать кнопку.
