...
{% if product == "yandex-cloud" %}В качестве источника данных для примеров будет использоваться прямое [подключение](../tutorials/data-from-ch-to-sql-chart#create-connection) к демонстрационной БД (датасет построен на основе таблицы `SampleSuperstore`).{% endif %}
...
Ключевое слово `FIXED` позволяет выполнить группировку по явно перечисленным измерениям вне зависимости от того, используются ли они в какой-либо секции чарта или нет.
