...
1. Оптимизируйте нагрузку на источник при организации дашборда:
   ...
   * Оптимизируйте структуру дашборда. Запросы к источнику выполняются для каждого чарта на текущей вкладке. При большом количестве чартов загрузка вкладки может занимать много времени. Расположите на одной вкладке минимальное количество чартов с учетом частоты их одновременного использования.
   {% if product == "yandex-cloud" %}* Сократите использование [автоматического обновления](dashboard.md#auto-update) данных дашборда. Не указывайте частоту обновления выше, чем частота обновления данных в источнике.{% endif %}
...
1. Используйте средства {{ datalens-short-name }}, помогающие оптимизировать работу с данными:
   ...
   * [Инспектор чартов](dashboard.md#diagnostics) — инструмент {{ datalens-short-name }}, позволяющий диагностировать проблемы в загрузке данных и их рендеринге в браузере.
   * [Кеширование](caching.md) часто запрашиваемых данных источника для быстрого доступа к ним.

   {% if audience == "internal" %}

   * [Трассировка](../operations/connection/chyt/chyt-datalens-tracing.md) запроса из {{ datalens-short-name }} в YT.
   * Посмотрите [рекомендации от YT](../operations/connection/chyt/chyt-recommendations.md).
