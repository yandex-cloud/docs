...
Если в источнике доступно несколько таблиц, их можно объединить с помощью оператора [JOIN]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/Join_(SQL)){% else %}(https://en.wikipedia.org/wiki/Join_(SQL)){% endif %}.
...
{% if product == "yandex-cloud" %}Подробнее про оптимизацию при использовании оператора `JOIN` в разделе [{#T}](../data-join.md#join-optimization).{% endif %}
