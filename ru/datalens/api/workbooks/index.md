...
Получить конфигурацию воркбука.
...
Создать воркбук. Будут созданы папка навигации и подключения, перечисленные в запросе. Сейчас можно создать только подключения.
...
`connection_secrets` | **list**  | Секреты, используемые в подключениях (пароли/токены). Соответствие секрета подключению определяется по имени. Поле не требуется для подключений без секретов, например, для CHYT с пользовательской аутентификацией.
`connection_secrets[]` | **ConnectionSecret**  | Секреты, используемые в подключениях (пароли/токены). Соответствие секрета подключению определяется по имени. Поле не требуется для подключений без секретов, например, для CHYT с пользовательской аутентификацией.
...
#### Example: Воркбук с CHYT-подключением
...
Применить конфигурацию воркбука. Выполняется сравнение текущего состояния каждого объекта в воркбуке и конфига из запроса. Объекты воркбука, отсутствующие в запросе, будут удалены.

Свойства объекта, измененные через UI и не поддержанные в API, не будут затронуты. Это поведение можно изменить установив флаг `force_rewrite`. Тогда все объекты будут переписаны вне зависимости от наличия изменений в UI.
...
Создать подключение в существующем воркбуке.
...
Составить конфигурацию датасета, исходя из схемы источника данных. В запросе передается датасет с пустым списком полей.

В ответе будет датасет со всеми полями источника данных. Для получения схемы используется подключение из поля `connection_ref`. ID полей будут сгенерированы из названия колонок. Предполагается, что пользователь их перепишет.
