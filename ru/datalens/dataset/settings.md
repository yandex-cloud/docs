---
title: Настройки датасета в {{ datalens-full-name }}
description: Настройки датасета в {{ datalens-full-name }} определяют, каким образом датасет будет работать с данными источника.
---

# Настройки датасета в {{ datalens-full-name }}
...
Включить или отключить использование подзапросов в качестве источника можно при [создании подключения](../concepts/connection/index.md) и при его редактировании.


## Параметризация источников датасетов {#parametrization}

Параметризация источников датасета позволяет динамически подставлять параметры из чарта в SQL-запрос или название таблицы или схемы. Подробнее на странице [{#T}](./parametrization.md).
