...
* [Отмена и восстановление изменений в датасете](#undo-redo)
...
1. Выберите [подключение](../concepts/connection/index.md) к источнику данных.
1. Добавьте данные из источника:
   ...
   - SQL-запрос
     ...
     * Чтобы использовать подзапросы в качестве источника, включите опцию **Уровень доступа SQL запросов** → **Разрешить подзапросы в датасетах** при создании или редактировании [подключения](../concepts/connection/index.md).
...
1. Выберите поля для связи между таблицами. При наведении указателя на поле отображается всплывающая подсказка с полным именем. Можете воспользоваться поиском по имени поля.
   Установить связь можно только по полям с одним и тем же [типом данных](./data-types.md). Информацию о типе данных поля {{ datalens-short-name }} получает непосредственно из подключения. Если вам нужно объединить таблицы по полям с разным типом данных, воспользуйтесь одним из вариантов:
...
1. Настроить отображение поля. Для измерений вы можете выбрать цвет и задать формат отображения для числовых типов, для показателей — только задать формат отображения чисел. Настройки применятся при использовании этих полей в чартах, построенных по данному датасету.
...
1. В окне **Добавление параметра** введите:
   ...
   * **Валидация значений**. Настройка будет доступна, если активировать опцию **Разрешить использовать в настройке источника**. Валидация значений по умолчанию: `[a-zA-Z0-9]*`. Вы можете настроить свою валидацию с помощью регулярного выражения Python: в поле **Валидация значений** выберите `Регулярное выражение` и в поле ниже введите свое регулярное выражение.

     {% note warning %}

     Использование регулярных выражений может быть небезопасным, поскольку открывает возможность добавления нежелательного кода в запрос со стороны чарта.
...
- Воркбук
  ...
  Датасет наследует права доступа, назначенные на воркбук. [Назначьте пользователю права](../workbooks-collections/workbooks-operations.md#wb-coll-grant) на воркбук, чтобы они распространились на датасет.
...
При редактировании датасета можно отменить или повторно выполнить внесенные изменения в пределах текущей [версии](./versioning.md):
