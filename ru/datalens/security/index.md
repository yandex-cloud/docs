...
Чтобы разрешить доступ к ресурсам сервиса [!KEYREF datalens-full-name], 
...
1. Выберите роль `editor` из списка. Подробнее о ролях в разделе [Роли](../../iam/concepts/access-control/roles.md) документации сервиса [!KEYREF iam-full-name].

## Роли

Ниже перечислены все роли, которые учитываются при проверке прав доступа в сервисе [!KEYREF datalens-full-name].
...
Пользователь с ролью `[!KEYREF roles-read]` может просматривать созданные дашборды и виджеты.

#### [!KEYREF roles-write]

Пользователь с ролью `[!KEYREF roles-write]` может создавать дашборды, виджеты.

Помимо этого роль `[!KEYREF roles-write]` включает в себя все разрешения роли `[!KEYREF roles-read]`.

#### [!KEYREF roles-admin]

Пользователь с ролью `[!KEYREF roles-admin]` может создавать дашборды и виджеты, утверждать права доступа.

Помимо этого роль `[!KEYREF roles-admin]` включает в себя все разрешения роли `[!KEYREF roles-write]`.
