...
* В исходные данные добавьте новое поле для хранения id пользователя. По этому полю будет происходить фильтрация всех запросов в источник.
   ...
   {% if audience == "internal" %} * Пользователи [datalens.yandex-team.ru](https://datalens.yandex-team.ru/) могут посмотреть свой id с помощью [API](https://staff-api.yandex-team.ru/v3/persons?_doc=1). В параметре `login` укажите логины пользователей через запятую, например `https://staff-api.yandex-team.ru/v3/persons?_fields=login,uid&login=user1,user2`. {% endif %}
