      ...
      - Для чарта {#chart}
        ...
        * Выберите **Режим работы параметров по умолчанию**:
          ...
          * **Всё запрещено** — все неподписанные параметры запрещены, кроме разрешенных.
          ...
          На [подписанные параметры](#signed-parameters) из токена эти ограничения не действуют.
...
По умолчанию значения подписанных параметров переопределяются внешними параметрами с таким же именем. Параметр селектора, параметр из настроек дашборда или параметр из ссылки будут приоритетнее, чем подписанные.

Подписанные параметры можно динамически обновить с помощью [обновления токена без потери состояния фильтров](#token-update).

{% note warning %}

С 26 августа 2025 года поведение подписанных параметров изменится.

{% endnote %}

Особенности нового поведения:

* Значения подписанных параметров станут приоритетными. Если в виджет прокидываются одновременно подписанный параметр и внешний параметр с тем же именем, применится подписанный.
* Селекторы по подписанным параметрам не будут влиять на чарты дашборда.

Подписанные параметры обеспечивают более безопасный доступ к данным — пользователи, получившие доступ к встроенным объектам, не смогут изменить эти параметры. Используя подписанные параметры для фильтрации чартов, можно безопасно предоставить конкретному пользователю лишь необходимый ему срез данных.
...
* Чтобы запретить изменение значения параметра:
  ...
  С 26 августа 2025 года поведение подписанных параметров изменится. Подписанные параметры станут неизменяемыми по умолчанию, и не нужно будет дополнительно настраивать списки разрешенных или запрещенных параметров.
