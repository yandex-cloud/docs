...
* `_reload_on_session_expire` — задает автоматическую перезагрузку страницы по истечении времени жизни сессии. Значение для включения параметра — `1`.
...
* `_bg_color` — задает фон чарта и дашборда в шестнадцатеричном формате. Например, `_bg_color=D7F7F7`.
* `_widgets_bg_color` — задает фон виджетов на дашборде в шестнадцатеричном формате. Например, `_widgets_bg_color=B7F7F7`.
...
* `_no_tabs ` — скрывает вкладки у дашборда. Значение для включения параметра — `1`.
* `_no_table_of_content` — скрывает содержание у дашборда. Значение для включения параметра — `1`.
* `_no_title` — скрывает заголовок у дашборда. Значение для включения параметра — `1`.
...
* Учитывайте, что приоритет значения подписанного параметра выше, чем значение внешних параметров с таким же названием (например, параметра в ссылке).
