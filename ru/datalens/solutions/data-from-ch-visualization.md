...
- [Шаг 1. Определите источник данных датасета](data-from-ch-visualization.md#step1)
- [Шаг 2. Настройте поля датасета](data-from-ch-visualization.md#step2)
- [Шаг 3. Создайте чарт — линейная диаграмма](data-from-ch-visualization.md#step3)
- [Шаг 4. Создайте чарт — столбчатая диаграмма](data-from-ch-visualization.md#step4)
- [Шаг 5. Создайте чарт — сводная таблица](data-from-ch-visualization.md#step5)
- [Шаг 6. Создайте чарт — тепловая карта](data-from-ch-visualization.md#step6)
- [Шаг 7. Создайте дашборд](data-from-ch-visualization.md#step7)
- [Шаг 8. Добавьте чарты на дашборд](data-from-ch-visualization.md#step8)
- [Шаг 9. Добавьте селекторы на дашборд](data-from-ch-visualization.md#step9)
...
Создайте [датасет](../concepts/dataset.md) на основе [подключения](../concepts/connection.md) **Sample ClickHouse** созданным на основе базы данных ClickHouse.
...
Для визуализации динамики продаж по месяцам создайте [чарт](../concepts/chart.md) — линейную диаграмму.

1. Выберите тип визуализации **Линейная диаграмма**.

    ![image](../../_assets/datalens/solution-02/19-choose-line-chart.png)

1. Добавьте на чарт дату продажи. Из раздела **Измерения** перетащите поле **OrderDate** в секцию **X**.
1. Добавьте на чарт показатель продаж. Для этого из раздела **Показатели** перетащите поле **Sales** в секцию **Y**.
1. Добавьте на чарт тип доставки. Для этого из раздела **Измерения** перетащите поле **PaymentType** в секцию **Цвет**.
...
1. Отобразите чарт по месяцам.
    ...
    1. В выпадающем списке типов группировок в разделе **Округление** выберите **Месяц**.
    1. Нажмите **Применить**.
...
Для визуализации продаж в разрезе брендов и продуктовых категорий создайте [чарт](../concepts/chart.md) — столбчатую диаграмму.
...
1. Поля **OrderDate**, **Sales** и **PaymentType** автоматически попадут в секции **X**, **Y** и **Цвет** соответственно.
...
1. Замените типы оплаты на категории товаров в цветах. Из раздела **Измерения** перетащите поле **ProductCategory** в секцию **X** и наведите его над полем **PaymentType**, пока то не станет красным.
...
Для визуализации продаж в разрезе продуктов и времени создайте [чарт](../concepts/chart.md) — сводную таблицу.

1. Скопируйте чарт, получившийся на предыдущем шаге.
    1. Нажмите значок галочки направленной вниз рядом с кнопкой **Сохранить** в верхнем правом углу.
...
1. Измените формат отображения **OrderDate** на годы.
    ...
    1. В выпадающем списке типов группировок в разделе **Часть даты** выберите **Год**.
    1. Нажмите **Применить**.
...
1. Измените цвет показателя продаж в таблице. Для этого из раздела **Показатели** перетащите поле **Sales** в секцию **Цвет**.
...
Для визуализации плотности заказов на карте Москвы создайте [чарт](../concepts/chart.md) — тепловую карту.
...
1. Добавьте на карту координаты точек доставки. Для этого из раздела **Измерения** перетащите поле **DeliveryAddressCoord** в раздел **Геоточки (тепловая карта)**.
...
Добавьте [селекторы](../concepts/dashboard.md#selector), чтобы иметь возможность фильтровать чарты по дате, районам Москвы, продуктам и статусам клиентов.
