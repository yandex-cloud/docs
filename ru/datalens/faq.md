...
1. Если в исходных данных range, и добавлены колонки только в последних, то стоит отметить, что chyt сам берёт пересечение множеств колонок по всему range (уточнение спросить у Антона Васильева). Обсуждение было тут: https://t.me/c/1068660824/45777

### У меня есть 2 таблички (2 датасета). В них есть общие поля. На эти общие поля хочется повесить один селектор. Как это правильно сделать?

пример: https://datalens.yandex-team.ru/vpcr34sjm4wmp-primer-svyazi-dvuh-datasetov-cherez-selektor?state=7ae8de37186
Лена, на дашборде с примером описано, как сделать, можно взять оттуда описание
Для внешней доки пример давать нельзя. Надо или отдельный придумать или как-то про это написать, чтобы не использовать ссылку на внутренний пример
...
Функцию отключили, потому что она работала не так, как ожидало большинство пользователей: копировались не только объекты, но и связи между ними.

### Можно ли откатить версию объекта DL в прошлом?

Пользователь самостоятельно сделать это не может
Только для внутренней доки: [Тикет](https://st.yandex-team.ru/DLFR-458) на эту возможность.
Если заведете тикет в очередь DLHELP с указанием объектов DL и версии (времени), к которому надо вернуть объект - мы постараемся все вернуть
