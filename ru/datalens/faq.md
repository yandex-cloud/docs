...
### В каких случаях {{ datalens-short-name }} предоставляет доступ на объекты? {#object-access}

Доступ предоставляется, если уволился единственный администратор и нет возможности редактировать нужные объекты {{ datalens-short-name }}.

Чтобы запросить права на объект:

1. Перейдите на [страницу сервиса {{ datalens-short-name }}](https://datalens.yandex-team.ru).
1. В левом нижнем углу окна нажмите значок жучка.
1. В открывшемся окне напротив **Проблема была решена?** нажмите кнопку **Нет**.
1. Выберите тип проблемы **Проблема с доступами**.
1. В поле **Тема** укажите тему вашего обращения.
1. В поле **Сообщение** укажите ссылки на объекты или папку, в которой они находятся. Вы можете добавить их после создания тикета.
1. Нажмите **Отправить**.
1. В созданном тикете укажите ABC-сервис (или скоуп), которому предоставляются права доступа.
   Права выдаются только группе, чтобы другие сотрудники в этой группе имели доступ к объектам в ваше отсутствие.
1. Если вы не являетесь руководителем отсутствующего администратора, призовите в тикет его руководителя и попросите дать разрешение на выдачу прав.

### Почему нет доступа к {{ datalens-short-name }}? {#access-denied}

Можно выделить два вида проблем с доступом:

* Отсутствует сетевой доступ. См. [изображение](https://jing.yandex-team.ru/files/elenbaskakova/access_denied.png). Проверьте, что:
   * Подключение к VPN активно (для штатных и внештатных сотрудников). 
   * Доступ был предоставлен по инструкции в разделе [{#T}](security/external-access.md).
* Отсутствует доступ к объектам {{ datalens-short-name }}. См. [изображение](https://jing.yandex-team.ru/files/elenbaskakova/denied_dashboard.png). Проверьте, что:
   * Заявка на получение доступа есть в [Puncher](https://puncher.yandex-team.ru/), сотрудник входит в ABC-сервис, и срок действия у заявки не окончен (только для внештатных сотрудников). 
   * Staff- или ABC-группа указана верно в заявке (только для внештатных сотрудников).
...
- Для {{ datalens-short-name }} можно [настроить федерацию учетных записей](../organization/add-federation.md#federation-usage).
