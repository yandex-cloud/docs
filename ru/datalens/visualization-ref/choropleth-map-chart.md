...
{% if product == "yandex-cloud" %}* Если у вас в данных есть поле с названием городов, регионов или стран, используйте {% if audience == "external" %}[функцию](../function-ref/TOPONYM_TO_GEOPOLYGON.md) `TOPONYM_TO_GEOPOLYGON([city])`{% else %}[справочник геоточек/геополигонов](https://geointellect.com/files/geo_for_datalens.zip) нашего партнера Геоинтеллект{% endif %}.{% endif %}
