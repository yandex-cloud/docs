...
Чтобы показать полигоны на карте, добавьте в датасет поле с типом данных `Геополигон`:

* Создайте текстовое поле, в котором лежат данные вида `[[[55.60807, 37.5698], [55.60847, 37.56992], [55.60851, 37.57095]]]` — координаты точек полигона в квадратных скобках. Вы можете предварительно подготовить данные в БД и определить тип данных `Геополигон` в интерфейсе описания датасета.
* Используйте [функцию](../function-ref/GEOPOLYGON.md) `GEOPOLYGON(polygon_string)`, чтобы создать вычисляемое поле с типом данных `Геополигон`.
