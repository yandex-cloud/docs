...
Добавлена интеграция с сервисом [{{ connection-manager-name }}]({{ link-docs }}/metadata-hub/concepts/connection-manager) для подключений:
...
1. В открывшемся окне настройте:
   ...
   * (опционально) Если у вас настроена [федерация удостоверений]({{ link-docs }}/organization/concepts/add-federation), сохраните федерацию, чтобы поделиться ссылкой с пользователями федерации.
