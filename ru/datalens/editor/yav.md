...
[В коде](#dostup-k-sekretam-iz-koda) секреты будут доступны через алиасы, поэтому важно, чтобы алиас имел уникальное название.

Все добавленные секреты будут забираться из самой последней версии секрета в [Секретнице](https://yav.yandex-team.ru/).

## Права доступа на чарт с секретами и безопасность
...
## Доступ к секретам из кода {#dostup-k-sekretam-iz-koda}
