...
* [Независимая перерисовка контролов](#pererisovka-kontrolov)
* [Изменение нескольких параметров](#izmenenie-neskolkih-parametrov)
* [Переходы на новую строку](#perehody-na-novuyu-stroku)
...
* **`ChartEditor.updateParams(params)`** - доопределяет параметры чарта аргументом `params: object`.
  ...
  Вызов данного метода имеет смысл только при изменении значения у контрола с [`postUpdateOnChange`](#izmenenie-neskolkih-parametrov).
  В противном случае вызов этого метода может привести к некорректной работе компоненты и связанных элементов.

## Доступные контролы {#available-controls}

* [Поле ввода](elements.md#pole-vvoda)
* [Календарь](elements.md#kalendar)
* [Календарь на интервал](elements.md#kalendar-na-interval)
* [Селект](elements.md#selekt)
* [Чекбокс](elements.md#chekboks)
* [Кнопка](elements.md#knopka)
* [Переход на новую строку](elements.md#perehod-na-novuyu-stroku)

## Изменение параметров и перерисовка чарта {#changing-chart-parametrs}

Все контролы, за исключением [кнопки](elements.md#knopka) и [перехода на новую строку](elements.md#perehod-na-novuyu-stroku),
...
Изменение значения такого контрола инициирует [независимую перерисовку контролов](#nezavisimaya-pererisovka-kontrolov),
...
Другими словами, при изменении значения контрола происходит [независимая перерисовка контролов](#nezavisimaya-pererisovka-kontrolov),
...
Также можно использовать [кнопку](elements.md#knopka) с `onClick`.
...
Также можно воспользоваться контролом [перехода на новую строку](elements.md#perehod-na-novuyu-stroku).
