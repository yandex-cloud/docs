...
* `custom` — конфигурация параметров ячейки для кросс-фильтрации. Подробнее в разделе [{#T}](../cross-filtration.md).
...
* `type` — переопределяет значение, заданное в head, для конкретной ячейки. Подробнее о поле читайте выше.
* `align` — задает значение выравнивания для индикаторов, если не задано в `head`. Подробнее о поле читайте выше.
* `min` — задает минимальное пороговое значение для индикатора, если не задано в `head`. Подробнее о поле читайте выше.
* `max` — задает максимальное пороговое значение для индикатора, если не задано в head. Подробнее о поле читайте выше.
* `showLabel` — задает отображение подписи для индикатора, если не задано в head. Подробнее о поле читайте выше.
* `barHeight` — задает CSS-стиль для высоты индикатора, если не задано в head. Подробнее о поле читайте выше.
...
* `onClick` — действие по клику на ячейку. Поле необязательное, тип значения объект вида:
  ...
  * `args` — статические аргументы для действия (например, для действия `setParams` аргументами является объект с ключом и новым значением параметра для установки).
...
* `size` — размер таблицы (включает в себя размер шрифта, межстрочного интервала и отступов внутри ячеек). Тип значения строка из возможных значений: `l`, `m`, `s`.
* `events` — позволяет задать конфигурацию для кросс-фильтрации для таблицы, которую можно переопределить на уровне конкретной ячейки с помощью поля `custom`. Подробнее в разделе [{#T}](../cross-filtration.md).
