...
* `paramFrom` — название связанного параметра начала интервала (вместо поля `param`). Обязательное поле, тип значения строка в ISO формате или [относительная дата](../tabs.md#relativedate).
* `paramTo` — название связанного параметра конца интервала (вместо поля `param`). Обязательное поле, тип значения — строка в ISO-формате или [относительная дата](../tabs.md#relativedate).
...
Этот вид чарта обладает теми же возможностями, что и [таблица](../../../visualization-ref/table-chart.md) в визарде. Вы можете кастомизировать стили, настраивать кросс-фильтрацию, пагинацию и т.п.
