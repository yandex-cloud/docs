...
Чарт парсит параметр и применяет соответствующую фильтрацию по полю, с которым связан параметр (подробнее в примере). Если селектору передаются числовые значения параметров, приведите их к строке или массиву строк. Это позволит избежать несогласованности отображения данных в разных состояниях виджета, например когда он просто отображается на дашборде и когда открыт на редактирование.
...
В результате выполнения вкладки [Controls](../tabs.md#controls) экспортируются данные для отрисовки элементов управления чарта — контролов. Вкладка доступна для всех [типов визуализаций](./index.md).

Основное назначение контролов — изменение параметров и перерисовка чарта. Подробнее о параметрах чарта читайте в описании вкладки [Params](../tabs.md#params).
...
Контрол с `type: "range-datepicker"`. Можно использовать либо `param`, либо `paramTo` и/или `paramFrom`. При наличии `paramTo` и/или `paramFrom` поле `param` не будет работать. Значением связанного параметра должна быть строка с ISO датой или относительная дата. Если используется поле `param` вместо `paramFrom` и `paramTo`, то его значением должен быть [интервал](../tabs.md#interval).

Кроме общих полей контролов доступно:

* `paramFrom` — название связанного параметра начала интервала (вместо поля `param`). Обязательное поле, тип значения строка в ISO формате или [относительная дата](../tabs.md#relative-date).
* `paramTo` — название связанного параметра конца интервала (вместо поля `param`). Обязательное поле, тип значения — строка в ISO-формате или [относительная дата](../tabs.md#relative-date).
...
* `onClick` — действие по клику на кнопку. Обязательное поле, тип объект:
  ...
  * `mode` — определяет стратегию обновления параметров, возможные значения:
    * `replace` (по умолчанию) — перезаписывает все существующие параметры новыми `args`;
    * `merge` — объединяет новые `args` с текущими существующими параметрами.
...
Чтобы в настройках параметров внешнего селектора указать [относительную дату](../tabs.md#relative-date), используйте формат вида `__relative_<знак><количество><единица измерения>`.
