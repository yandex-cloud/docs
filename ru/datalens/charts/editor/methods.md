...
Возвращает идентификатор субъекта {{ iam-short-name }} текущего пользователя.
...
Строка с идентификатором субъекта {{ iam-short-name }}. Для публичного чарта, встраивания без пользовательской авторизации или запроса, в котором пользователь не определен, возвращается пустая строка.
...
Возвращает имя пользователя из атрибута `preferredUsername` текущей сессии {{ iam-short-name }}.
...
Задает структурированные данные чарта. Используется для двух задач:

* Экспорт данных Advanced-чарта в форматы CSV, XLSX и Markdown. Без вызова метода экспорт для Advanced-чарта недоступен.
* Работа Нейроаналитика: если данные заданы, Нейроаналитик будет использовать их вместо парсинга HTML-разметки чарта.

#### Поддерживаемые виды чартов {#set-raw-data-charts}

[Advanced-чарт](./widgets/advanced.md).

#### Аргументы {#set-raw-data-args}

`data` (_массив_) — массив с данными. Поддерживаемые форматы:

* Массив объектов: `[{x: 1, y: 2}, ...]`
* Массив массивов (матрица): `[["header1", "header2"], [1, 2], ...]`
* Массив примитивов: `[1, 2, 3]`

#### Возвращаемый результат {#set-raw-data-result}

Нет.

#### Пример {#set-raw-data-example}
