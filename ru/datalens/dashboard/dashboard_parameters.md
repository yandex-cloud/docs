...
* `{{ link-datalens-main }}/test-dashboard?price=__lte_1000` — значение параметра `price` должно быть меньше или равно `1000`.
* `{{ link-datalens-main }}/test-dashboard?category=__istartswith_Бытов` — значение параметра `category` должно начинаться на `Бытов` или на `бытов`, например: `Бытовой`, `Бытовые`, `бытовые`.
* `{{ link-datalens-main }}/test-dashboard?category=__startswith_Бытов` — значение параметра `category` должно начинаться на `Бытов`, например: `Бытовой`, `Бытовые`.
* `{{ link-datalens-main }}/test-dashboard?category=__leneq_15` — длина значения параметра `category` должна быть равна `15`, например: `Моющие средства`, `Кухонные товары`.
* `{{ link-datalens-main }}/test-dashboard?category=__in_Моющие%20средства&__in_Кухонные%20товары` — значение параметра `category` должно принадлежать множеству `Моющие средства`, `Кухонные товары`.
