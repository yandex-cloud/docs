...
Допускается добавление изображений с доменов `*.yandex.ru`, `*.yandex.net`, `yastat.net`, `yastatic.net` или `{{ s3-storage-host }}`. Самый простой способ — загрузить и опубликовать изображение в [{{ objstorage-full-name }}](../../storage/quickstart.md). Сервис бесплатен, если у вас не больше 1 ГБ хранения в стандартном хранилище в месяц.
