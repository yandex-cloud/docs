---
title: Загрузка аудиоданных через консоль управления
description: Следуя данной инструкции, вы сможете загрузить аудиоданные в {{ speechsense-name }} с помощью консоли управления.
---

# Загрузка аудиоданных через консоль управления {{ speechsense-name }}

Эта инструкция поможет загрузить в {{ speechsense-name }} аудиоданные через консоль управления. Используйте этот вариант, чтобы быстро загрузить данные и ознакомиться с функциональностью {{ speechsense-name }}. Для полноценной загрузки данных используйте [API](upload-data.md).
...
1. В открывшемся окне выберите файлы для загрузки одним из способов:
    ...
    * Нажмите кнопку **Добавить файлы** и выберите один или несколько файлов.
    ...
    Через консоль управления можно загрузить только аудиофайлы [формата](../../concepts/formats.md) MP3. Чтобы загрузить аудиофайлы других поддерживаемых {{ speechsense-name }} форматов, используйте загрузку через API.
