...
1. Запустите скрипт `search_data.py` с нужными параметрами:
    ...
    * `--organization-id`, `--space-id`, `--connection-id`, `--project-id` — идентификаторы соответственно [организации](../../../organization/operations/organization-get-id.md), пространства, подключения и проекта, в которых выполняется поиск данных.
