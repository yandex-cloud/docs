...
* Анализ от [{{ yagpt-name }}]({{ link-docs-ai }}ai-studio/concepts/generation/models).
...
* для аудио — текстовая расшифровка диалога, автоматически выполненная с помощью [{{ speechkit-full-name }}]({{ link-docs-ai }}speechkit/overview);
...
Подробнее о техниках работы с промптами см. в [{#T}](../training.md).