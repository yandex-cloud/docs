...
1. Перечислите через запятую почтовые адреса сотрудников на Яндексе (например, `{{ example-account }}`) и нажмите кнопку **Добавить**. {% if audience == "draft" %}Каждый пользователь получит письмо с предложением вступить в организацию.{% endif %}
