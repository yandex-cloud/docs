...
Чтобы проверить работу триггера, переведите задачу типа **Релиз** в статус **Можно тестировать**. Триггер отправит в чат Telegram сообщение c текстом: <q>Пора тестировать задачу TEST-123: Настроить триггеры ({{ link-tracker-example-1 }})</q>.
