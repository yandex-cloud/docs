...
1. На верхней панели {{ tracker-name }} нажмите {% if audience == "external" %}![](../_assets/tracker/tracker-settings.png) → **Управление шаблонами**.{% else %}ваш портрет и выберите **Персональные настройки**. Затем нажмите кнопку **Управление шаблонами**.{% endif %}
...
{% if audience == "internal" %}Если вам нужен шаблон задачи, который по умолчанию смогут использовать все пользователи, [настройте форму для создания задачи](manager/forms-integration.md) в сервисе Яндекс.Формы. Созданную [форму можно использовать вместо стандартной страницы создания задачи](manager/attach-form.md) в вашей очереди.{% endif %}
