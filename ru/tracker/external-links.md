...
Также доступна расширенная функциональность связей, которая требует [поддержки на стороне внешнего приложения](ext-app-setup.md): отображение подробной информации о связанных объектах, создание и удаление связей во внешнем приложении по запросу из {{ tracker-name }}{% if audience == "internal" %}, саджест (подсказка) при добавлении связи через пользовательский интерфейс {{ tracker-name }}{% endif %}.
