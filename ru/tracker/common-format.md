...
В запросах к {{ api-short-name }} указывайте заголовки:
 ...
 - `Accept-Language: <тег языка>` — по умолчанию HTTP-запрос возвращает локализованные поля на русском языке, чтобы получить в результате запроса значения локализованных полей на английском языке укажите заголовок с тегом **en**.
