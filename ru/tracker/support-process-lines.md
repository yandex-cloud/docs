...
1. В настройках интеграции формы настройте [создание задачи в {{ tracker-name }}]{% if audience == "external" %}({{ support-forms-tracker }}){% else %}({{ support-forms-tracker-ya }}){% endif %} для первой линии поддержки:

    1. Настройте [условие отправки уведомления]{% if audience == "external" %}({{ support-forms-notify }}){% else %}({{ support-forms-notify-ya }}){% endif %}, в списке <q>Тема обращения</q> выбраны темы для первой линии поддержки.
