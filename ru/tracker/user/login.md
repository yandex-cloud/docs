...
Для работы в {{ tracker-name }} вы можете использовать аккаунт на Яндексе или домене вашей организации, а также [федеративный аккаунт](../../organization/add-federation.md).

Создать организацию и [подключить {{ tracker-name }}](../enable-tracker.md) могут только пользователи, у которых есть аккаунт на Яндексе.
...
- Веб-интерфейс
	...
	1. Войдите в аккаунт, который вы используете для работы в {{ tracker-name }}:
		...
		* Чтобы войти с помощью почтового адреса на Яндексе или на домене вашей организации, нажмите кнопку **Войти в аккаунт на Яндексе** и укажите ваш логин вида `{{ example-account }}` или `login@example.com`. Если у вас еще нет организации, она будет создана при первом входе в {{ tracker-name }}.
...
- Веб-интерфейс
	...
	1. В правом верхнем углу нажмите на фото профиля.

	1. Нажмите на название организации. Выберите организацию из списка доступных или нажмите **+ Добавить организацию**, чтобы [создать новую](../../organization/enable-org.md).

		{% note info %}

		Если вы авторизованы через федерацию удостоверений, для переключения необходимо выйти из текущего аккаунта. Для этого в правом верхнем углу нажмите на фото профиля и кнопку ![](../../_assets/tracker/logout-fed.png).
...
- Веб-интерфейс
	...
	1. Выберите другого пользователя из списка или нажмите **Добавить аккаунт**. 
	
	1. Если вы выбрали **Добавить аккаунт**, выполните [авторизацию](#auth).
