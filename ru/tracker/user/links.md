...
Дублирующая задача
:   Используйте этот тип связи, чтобы сгруппировать одинаковые по смыслу задачи. Например, если создано несколько задач типа <q>{{ ui-key.startrek.blocks-desktop_queue-new-presets.preset--task-type--bug }}</q>, которые описывают одну и ту же проблему.
