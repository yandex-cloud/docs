...
Цветной текст.<br/><br/>Поддерживаются цвета:<br/><br/>`red` {% if locale == "ru" %}/ `крас` / `красный`{% endif %};<br/>`green` {% if locale == "ru" %}/ `зел` / `зеленый`{% endif %};<br/>`blue` {% if locale == "ru" %}/ `син` / `синий`{% endif %};<br/>`grey` / `gray` {% if locale == "ru" %}/ `сер`{% endif %};<br/>`yellow` {% if locale == "ru" %}/ `жел`{% endif %}. | ```!!(син)Текст синего цвета!!``` | ![](../../_assets/tracker/format/colored.png)
