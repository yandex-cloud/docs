...
1. Настройте разрешения **Чтение**, **Запись** и **Настройка прав** для категорий:
    ...
    - **Роли** — настройки для пользователей, обладающих определенными ролями по отношению к шаблону:
        - **{{ ui-key.startrek-backend.roles.templateRole.name.OWNER }}** — владелец шаблона. По умолчанию владелец — это создатель шаблона и только у него есть разрешения **Запись** и **Настройка прав**.
