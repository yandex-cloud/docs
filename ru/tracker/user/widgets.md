...
- **Результаты** — количество задач которые отображаются на одной странице виджета. Если задач окажется больше, вы сможете воспользоваться переключателем страниц в нижней части виджета.
- **Фильтр** — фильтр, на основе которого формируется список задач. Вы можете задать фильтр с помощью [прямой ссылки](filters.md), [имени](create-filter.md) или [языка запросов](query-filter.md).
...
Простая сводная таблица, которая позволяет просматривать статистику задач по двум любым параметрам. На пересечениях таблицы приводится количество задач, удовлетворяющих параметрам. С помощью двумерной таблицы вы можете, например, отслеживать количество и статусы задач определенной группы исполнителей.

Чтобы настроить виджет, задайте его параметры:

- **Название** — название, которое отображается в заголовке виджета.

- **Фильтр**. Статистика собирается по задачам, удовлетворяющим определенному фильтру. Вы можете задать фильтр с помощью [прямой ссылки](filters.md), [имени](create-filter.md) или [языка запросов](query-filter.md).

- **Ось x** — параметр, по которому строятся столбцы таблицы.

- **Ось y** — параметр, по которому строятся строки таблицы.

- **Число строк таблицы** — максимальное количество строк таблицы, которые отображаются в виджете. Если строк окажется больше, вы сможете воспользоваться кнопкой **Показать все** в нижней части виджета.
...
Особый вид сводной таблицы по двум параметрам. В отличие от простой двумерной таблицы, на пересечениях приводится не число задач, а сумма значений третьего параметра. С помощью такой таблицы вы можете, например, отслеживать время, которое исполнители затратили на работу над различными компонентами очереди.
...
- **Сумма по** — параметр, сумма значений которого приводится на пересечениях.

- **Число строк таблицы** — максимальное количество строк таблицы, которые отображаются в виджете. Если строк окажется больше, вы сможете воспользоваться кнопкой **Показать все** в нижней части виджета.

- **Сортировка в строке** — порядок сортировки строк таблицы.

- **Сортировка в колонке** — порядок сортировки столбцов таблицы.
...
- **Число строк таблицы** — максимальное количество строк диаграммы, которые отображаются в виджете. Если строк окажется больше, вы сможете воспользоваться кнопкой **Показать все** в нижней части виджета.

- **Статистика по полю** — параметр задач, по которому собирается статистика.

- **Сортировка** — порядок сортировки столбцов диаграммы.

- **Автообновление** — периодичность, с которой обновляются данные в виджете.

#### График событий за период
...
- **График по полю** — выберите событие, которое отслеживается в статистике.
...
Записывайте свои идеи и делайте пометки прямо на дашборде. Вы можете делать записи простым текстом или использовать [Вики-разметку](wiki-markup.md).
