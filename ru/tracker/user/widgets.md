...
{% if audience == "draft" %}- **Автообновление** — периодичность, с которой обновляются данные в виджете.{% endif %}
...
Виджет безопасности отображает список задач, у которых для [глобального поля](create-param.md) **Безопасность** установлено значение **Yes**.
