...
1. Задайте одно или несколько действий над задачей:
    ...
    - **Изменить статус задачи**
         ...
         Автоматические действия выполняет служебный робот {% if audience == "internal" %}`spock@`{% else %}`yndx-tracker-cnt-robot@`{% endif %}. Чтобы робот смог изменить статус задачи:

         - Если для перехода в статус [настроены условия](../manager/workflow-action-edit.md#section_jrk_hmb_wbb), ограничивающие права пользователей на изменение статуса, добавьте в список пользователей робота {% if audience == "internal" %}`spock@`{% else %}`yndx-tracker-cnt-robot@`{% endif %}.
...
1. Настройте действие — призыв в комментарии исполнителя задачи:
    ...
    1. В поле **Призвать пользователей из поля** введите **Исполнитель**.
