...
1. Выберите одно из условий:
    ...
    Тип комментария | {% if audience == "internal" %}**Письмо на почту** — создан [комментарий с помощью письма](comments.md#section_zmr_mxt_52b).</br></br>{% endif %}**Комментарий в {{ tracker-name }}** — создан комментарий от внутреннего пользователя.
...
- Для некоторых операций сравнения можно задать дополнительные опции: 
  ...
  - **Не соответствовал до этого** — триггер сработает, если выполнена операция сравнения и изменилось значения поля.
