...
1. Выберите **Дашборды** → **Новый дашборд** в верхней панели {{ tracker-name }}.
...
1. Добавьте и настройте виджеты. Существует несколько типов виджетов:
    ...
    Большинство виджетов содержат информацию об определенном наборе задач. Перечень задач, информация о которых попадает в виджет, задается с помощью [фильтра](filters.md). Вы можете задать фильтр с помощью с помощью [прямой ссылки](filters.md), [имени](create-filter.md) или [языка запросов](query-filter.md).
