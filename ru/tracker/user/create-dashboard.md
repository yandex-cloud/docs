...
1. Добавьте и настройте виджеты. Существует несколько типов виджетов:
    ...
    {% if audience == "external" %}- [Форма приглашения](widgets.md#invite_form) — виджет, который позволяет пригласить коллег в {{ tracker-name }}.{% endif %}

    Большинство виджетов содержат информацию об определенном наборе задач. Перечень задач, информация о которых попадает в виджет, задается с помощью [фильтра](filters.md). Вы можете задать фильтр с помощью [прямой ссылки](filters.md), [имени](create-filter.md) или [языка запросов](query-filter.md).
