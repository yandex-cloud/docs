...
Если нужного вам параметра среди них нет, вы можете [создать новое поле в вашей очереди](#section_local_field). Если нужно добавить поле, которое будет доступно во всех очередях {{ tracker-name }}, {% if audience == "external" %}администратор может [создать глобальное поле](#section_global_field){% else %}отправьте заявку на создание глобального поля с помощью [формы](#section_global_field){% endif %}.
