...
На доске отображаются задачи только из тех очередей, к которым у пользователя есть доступ. Чтобы ограничить доступ к отдельным задачам, [добавьте компоненты с ограниченным доступом](../manager/queue-access.md#section_tbh_cs5_qbb).  
