...
1. Введите запрос и нажмите кнопку **{{ ui-key.startrek.blocks-desktop_issues-filter.action--apply-query }}**.
...
- `AND` — логический оператор **{{ ui-key.startrek-backend.messages.trigger.condition.type.and }}**. Условия, объединенные таким оператором, должны выполняться одновременно.
    ...
    ```
    Такой запрос выведет задачи, у которых `параметр1` имеет значение `значение1` **{{ ui-key.startrek-backend.messages.trigger.condition.type.and }}** `параметр2` имеет значение `значение2`.
...
- `OR` — логический оператор **{{ ui-key.startrek-backend.messages.trigger.condition.type.or }}**. Должно выполняться хотя бы одно из условий, объединенных таким оператором.
    ...
    ```
    Такой запрос выведет задачи, у которых выполнено хотя бы одно из условий: `параметр1` имеет значение `значение1` **{{ ui-key.startrek-backend.messages.trigger.condition.type.or }}** `параметр2` имеет значение `значение2`.
...
Кроме поиска по параметрам, вы можете воспользоваться привычным текстовым поиском. Для этого в поле ввода запросов введите текст, который нужно найти, и нажмите **{{ ui-key.startrek.blocks-desktop_issues-filter.action--apply-query }}**.
...
* Имя и фамилия в кавычках: `"имя фамилия"` — для поиска по совпадению с именем и фамилией пользователя.

    Пример:
    ```no-highlight
    Author: "Алиса Литл"
    ```
    Такой запрос выведет задачи, автор которых — пользователь с именем Алиса Литл. При этом в выдачу могут попасть задачи других авторов, у которых логин или имя похожи на `Алиса Литл`. Чтобы избежать этого, используйте поиск по логину с символом `@`.
...
----- | ----- | -----
```"Access" ``` | Логины или имена пользователей | Поиск задач, у которых в поле **{{ ui-key.startrek-backend.fields.issue.access }}** указаны заданные пользователи.<br/>Например:<br/>```"Access": user3370@, "Алиса Литл" ```
...
```"History" ``` | Текстовая строка | Поиск задач, в истории изменений которых есть слова и словоформы заданной фразы.<br/>Поиск осуществляется только по значениям полей **{{ ui-key.startrek.blocks-desktop_b-page-agile-admin-tab_type_card-settings.issue-summary }}** и **{{ ui-key.startrek.blocks-desktop_b-create-ticket-form.fill-desc }}**.<br/>Например:<br/>```"History": "проще простого" ```
...
```"Related To Queue" ``` | Названия или ключи очередей | Поиск задач, которые связаны с задачами определенных очередей (связь типа <q>{{ ui-key.startrek.blocks_touch_b-related-issues.type-relates }}</q>).<br/>Например:<br/>``` "Related To Queue": Тестирование ```
```"Relates" ``` | Ключи задач | Поиск задач, связанных с определенными задачами (связь типа <q>{{ ui-key.startrek.blocks_touch_b-related-issues.type-relates }}</q>).<br/>Например:<br/>```"Relates": "TASK-123", "TASK-321" ```
