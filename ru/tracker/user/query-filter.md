...
## Несколько значений параметра {#section_pf4_slk_lz}
...
## Поиск по тексту {#section_qsz_glk_lz}
...
- Чтобы найти только те задачи, в которых отсутствует заданный текст, воспользуйтесь оператором `~`:

    ```no-highlight
    Comment: ~"Consectetur adipiscing elit"
    ```

    Такой фильтр выведет только те задачи, в комментариях к которым отсутствует совпадение с текстом запроса.
...
При поиске по полям, содержащим дату, вы можете пользоваться операциями сложения и вычитания, временными интервалами и [функциями](#section_pmb_kmk_lz).
...
## Операции сравнения {#section_kws_ylk_lz}
...
## Параметры фильтров {#section_d5p_d4n_jdb}
...
```"Favorited by" ``` | В качестве значения параметра может использоваться только [функция](#section_pmb_kmk_lz) `me()` | Поиск ваших избранных задач.<br/>Например:<br/>```"Favorited by": me() ```
...
```"Pending Reply From" ``` | Логины или имена пользователей | Поиск задач, в которых требуется ответ от заданного пользователя (пользователя [призвали в комментарии](comments.md#section_nl2_1qv_tz)).<br/>Например, найти задачи, в которых нужен ответ от пользователя с логином `user3370` или от пользователя по имени Алиса Литл:<br/>```"Pending Reply From": user3370@, "Алиса Литл" ```
