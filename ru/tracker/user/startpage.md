...
Стартовая страница открывается при нажатии на логотип {{ tracker-name }}. Вы можете [выбрать](#change-start-page), какую страницу использовать в качестве стартовой. По умолчанию выбрана [**Моя страница**](https://tracker.yandex.ru/pages/my) — персональная для каждого пользователя страница с несколькими виджетами:
...
1. Включите опцию **Виджет опубликован**.
...
Чтобы перейти на начальную страницу, на верхней панели {{ tracker-name }} нажмите **Моя страница**. По умолчанию на странице отображается список задач из фильтра [<q>Все задачи</q>](my-tickets.md): задачи, для которых вы являетесь автором, исполнителем или наблюдателем.

{% endif %}

В качестве стартовой страницы вы можете использовать любой [фильтр](create-filter.md), [дашборд](dashboard.md) или [доску задач](../manager/create-agile-board.md). {% if audience == "external" %}При этом **Моя страница** останется доступной по прямой ссылке: [https://tracker.yandex.ru/pages/my](https://tracker.yandex.ru/pages/my) или по нажатию на домик в шапке {{ tracker-name }}.{% endif %}

{% if audience == "external" %}Пока сменить стартовую страницу можно только в старом интерфейсе:{% endif %}
