...
В новом интерфейсе на страницах задач появилась вкладка **Коммиты**. Она находится внизу задачи рядом с комментариями и доступна, если к вашему {{ tracker-name }} [привязан репозиторий](../manager/add-repository.md) и для задачи сделан хотя бы один коммит.
