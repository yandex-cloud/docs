...
* Глобальный уровень доступа к {{ tracker-name }}:
    ...
    * [Сотрудник с правом «Чтение»](access.md#readonly).
...
#### Сотрудники с правом «Чтение» {#read-only}

Сотрудники, которые работают в режиме «Чтение», могут использовать возможности {{ tracker-name }}, связанные с просмотром задач. Для таких сотрудников плата за использование сервиса не взимается.

Если [администратор](#admin) имеет доступ к {{ tracker-name }} в режиме «Чтение», он может изменить параметры доступа для своей учетной записи самостоятельно.

Действия, доступные в режиме «Только чтение»:
