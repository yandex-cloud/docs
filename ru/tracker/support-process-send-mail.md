...
1. Перейдите в настройки очереди и в разделе **{{ ui-key.startrek.ui_components_page-queue-admin_QueueAdminPageContent.menu-item-aliases }}** [добавьте адрес очереди](manager/queue-mail.md).
...
1. Над полем комментария включите опцию **{{ ui-key.startrek.blocks-desktop_b-ticket-workflow.new-comment-type--external }}** и укажите адрес получателя.
    ...
    Если вы [принимаете обращения пользователей через форму](#form), в форме можно запросить почту пользователя и сохранить ответ в поле задачи <q>{{ ui-key.startrek-backend.fields.issue.emailFrom }}</q>. В этом случае почта пользователя также подставится в поле **{{ ui-key.startrek-backend.fields.issue.emailTo }}** автоматически.
