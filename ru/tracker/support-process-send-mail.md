...
1. {% if audience == "external" %}Перейдите в настройки очереди и в разделе **Почтовые ящики** [добавьте адрес очереди](manager/queue-mail.md){% else %}[Настройте почтовый адрес](manager/queue-mail.md#sec-mail-yandex) для очереди службы поддержки{% endif %}.
