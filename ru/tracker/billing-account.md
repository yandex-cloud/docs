...
## Подключить платежный аккаунт {{ yandex-cloud }} {#bind}

Если вы пользуетесь сервисами {{ yandex-cloud }} и хотите использовать тот же платежный аккаунт для оплаты услуг {{ tracker-name }}, подключите этот аккаунт в {{ tracker-name }}.

Подключение не доступно для платежных аккаунтов с задолженностью или со [статусом](../billing/concepts/billing-account-statuses.md) `Приостановлен`.

Если вы подключите платежный аккаунт с действующим [пробным периодом](../getting-started/free-trial/concepts/quickstart.md), для него автоматически будет активирована платная версия. Средства начнут списываться после израсходования стартового гранта или завершения пробного периода. 

Чтобы подключить платежный аккаунт:
...
1. Рядом с нужным аккаунтом нажмите **Подключить**.
