...
1. Задайте параметры:
    - **Тип задачи** — выберите один из доступных типов.
        {% if audience == "external" %}По умолчанию в {{ tracker-name }} доступны наиболее популярные типы задач. Если нужного вам типа среди них нет, администратор может [создать его](#create-ticket-type).{% endif %}
    ...
    - **Резолюции** — выберите возможные резолюции.
        {% if audience == "external" %}По умолчанию в {{ tracker-name }} доступны наиболее популярные резолюции. Если нужной вам резолюции среди них нет, администратор может [создать ее](create-resolution.md).{% endif %}
