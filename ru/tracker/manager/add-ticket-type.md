...
1. Задайте параметры:
    - **Тип задачи** — выберите один из доступных типов.
        {% if audience == "external" %}По умолчанию в {{ tracker-name }} доступны наиболее популярные типы задач. Если нужного вам типа среди них нет, администратор организации может [создать его](create-ticket-type.md).{% endif %}
    ...
    - **Резолюции** — выберите возможные резолюции.
        {% if audience == "external" %}По умолчанию в {{ tracker-name }} доступны наиболее популярные резолюции. Если нужной вам резолюции среди них нет, администратор организации может [создать ее](create-resolution.md).{% endif %}
    {% if audience == "internal" %}Если в {{ tracker-name }} нет нужного вам типа задач или резолюции, отправьте заявку на их создание с помощью [формы](../user/create-param.md#section_bjz_3p4_xgb).{% endif %}
