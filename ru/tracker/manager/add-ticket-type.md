...
1. Задайте параметры:
    * **Тип задачи** — выберите один из доступных типов.
        {% if audience == "external" %}В {{ tracker-name }} доступны наиболее популярные типы задач. Если нужного вам типа среди них нет, администратор может [создать его](#create-ticket-type).{% endif %}
