...
Чтобы настроить интеграцию с почтой{% if audience == "external" %}:{% else %}, [добавьте для очереди почтовый адрес](#sec-mail-yandex). Вы можете добавить несколько адресов, чтобы создавать задачи разных типов или с разными [компонентами](edit-versions-and-components.md).{% endif %}
...
Настроить почтовый адрес очереди можно, только если к вашей организации в Яндекс.Коннекте [подключен почтовый домен]{% if locale == "ru" %}(https://yandex.ru/support/connect/add-domain.html){% else %}(https://yandex.com/support/connect/add-domain.html){% endif %}.
...
1. Нажмите кнопку **Добавить адрес**.
    Если добавление адреса очереди недоступно, значит у вашей организации нет домена. Домен нужен для создания почтовых ящиков и рассылок, в том числе чтобы создать адрес очереди. Домен можно бесплатно [подключить в Яндекс.Коннекте]{% if locale == "ru" %}(https://yandex.ru/support/connect/add-domain.html){% else %}(https://yandex.com/support/connect/add-domain.html){% endif %}.
...
1. Чтобы привязать созданную рассылку к очереди, обратитесь в поддержку {{ tracker-name }} с помощью формы в конце этого раздела:

    1. В пункте **Область вопроса \ проблемы** выберите **Очереди \ Queues**.

    1. В пункте **Тип обращения** выберите **Привязка рассылки к очереди**.
