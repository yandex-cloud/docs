...
{% if lang == "ru" %}[Гибкие методологии разработки]({{ link-wiki-agile }}){% else %}[Гибкие методологии разработки]({{ link-wiki-agile-en }}){% endif %} были созданы для управления разработкой программного обеспечения, теперь их успешно применяют в самых разных областях. Существует несколько гибких методологий, например [Скрам]({{ link-wiki-scrum }}) и [Канбан]({{ link-wiki-kanban }}).
