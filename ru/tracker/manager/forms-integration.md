...
1. Настройте [интеграцию с {{ tracker-name }}](../../forms/create-task.md):
    ...
    1. Чтобы сохранить в параметрах задачи ответ на определенный вопрос формы, в настройках интеграции добавьте поле задачи, нажмите **{{ ui-key.startrek.blocks-desktop_trigger-action.webhook-variables }}** → **Ответ на вопрос** и выберите вопрос. 
...
1. [Опубликуйте](../../forms/publish.md) форму. Форма будет доступна по ссылке, указанной в поле **{{ ui-key.startrek.blocks-desktop_page-admin-tab_type_fields.field-type--uri }}**.
