...
В [бета-версии нового интерфейса {{ tracker-full-name }}](../user/personal.md#sec_beta) расширены возможности для работы с проектами: добавлен список задач проекта и улучшена диаграмма Ганта.
...
* Вкладка **Доска задач**. Виртуальная доска с карточками, на которой удобно отслеживать статусы задач проекта. О работе с доской задач читайте в разделе [{#T}](agile-new-use.md).

  {% note info %}
  
  Также доска проекта отображается в разделе ![](../../_assets/tracker/svg/boards.svg)&nbsp;**Доски задач** на панели слева.
