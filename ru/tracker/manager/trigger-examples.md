...
- Как [автоматически отправлять пользователю уведомление](#notify_form) после создания задачи из обращения в службу поддержки через Яндекс.Формы.
...
1. Убедитесь, что у всех сотрудников, которые могут быть назначены исполнителями задач, есть [полный доступ к {{ tracker-name }}](../enable-tracker.md#access).
...
1. Задайте условия, чтобы триггер срабатывал при закрытии задачи в случае, если поле <q>Затраченное время</q> не заполнено:
    ...
    1. Добавьте условие **Системные** → **Статус** → **стало равно** → **Закрыт**.
...
2. [Настройте псевдонимы и подписи](queue-mail.md#send_outside), если нужно.
...
Предположим, что сотрудники службы поддержки регистрируют обращения в {{ tracker-name }}. Пользователи обращаются в службу поддержки через форму обратной связи, которая создана в сервисе [Яндекс.Формы]{% if audience == "external" %}({{ link-forms }}){% else %}({{ link-forms-ya }}){% endif %}. На основе заполненной формы создаются задачи в {{ tracker-name }}.
...
3. Если пользователи не являются сотрудниками организации, [разрешите отправку писем из задач на внешние адреса](queue-mail.md#send_outside).
...
С помощью триггера в комментарии задачи можно добавлять форму с автоматическим предзаполнением полей. Для этого в текст комментария нужно добавить специальный код со ссылкой на форму. Значения в поля формы можно передавать через [GET-параметры]({{ support-forms-get-params-ya }}). Например, передавать параметры задачи с помощью переменных, которые доступны в триггере.
...
1. Перейдите в сервис [Яндекс.Формы]{% if audience == "external" %}({{ link-forms }}){% else %}({{ link-forms-ya }}){% endif %} и создайте форму. {% if audience == "internal" %}В какой админке создавать форму, [читайте в документации]({{ support-forms-instance-ya }}).{% endif %}
...
1. Задайте действие триггера: 
   ...
   {{=<% %>=}}{{iframe src=//forms.yandex-team.ru/surveys/<id_формы>/?iframe=1&<id_вопроса>=<значение> frameborder=0 width=100% height=660px scrolling=no}}
    ...
    `<id_вопроса>` — [идентификатор вопроса]({{ support-forms-question-id }});
...
#### Шаг 3. Добавить в очередь robot-forms@

Для корректной вставки формы дайте доступ к очереди роботу `robot-forms@`. Подробнее о настройке доступа читайте в разделе [Настроить доступ к очереди](queue-access.md).
