...
Предположим, что сотрудники службы поддержки регистрируют обращения в {{ tracker-name }}. Пользователи обращаются в службу поддержки через форму обратной связи, которая создана в сервисе [Яндекс.Формы]({{ link-forms }}). На основе заполненной формы создаются задачи в {{ tracker-name }}.
...
1. Перейдите в сервис [Яндекс.Формы]({{ link-forms }}) и создайте форму. {% if audience == "internal" %}В какой админке создавать форму, [читайте в документации]({{ support-forms-instance }}).{% endif %}
