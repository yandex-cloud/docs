...
- Как настроить [Динамические чеклисты](#checklist).
...
1. Перейдите в сервис [{{ forms-full-name }}]({{ link-forms }}) и создайте форму. {% if audience == "internal" %}В какой админке создавать форму, [читайте в документации](../../forms/go-to-forms.md).{% endif %}
...
#### Шаг 3. Добавить в очередь {% if audience == "internal" %}robot-forms@{% else%}yndx-forms-cnt-robot@{% endif %}

Для корректной вставки формы дайте доступ к очереди роботу {% if audience == "internal" %}`robot-forms@`{% else%}yndx-forms-cnt-robot@{% endif %}. Подробнее о настройке доступа читайте в разделе [Настроить доступ к очереди](queue-access.md).
...
## Динамические чеклисты {#checklist} 

Чтобы выделить этапы выполнения задачи, используйте [чеклисты](../user/checklist.md). С помощью триггера и [{{ api-name }}](../about-api.md) вы можете настроить автоматическое создание и обновление чеклиста:
...
1. Укажите параметры запроса. В поле **Тело запроса** укажите параметры пунктов чеклиста:

    #|
    || **Поле** | **Содержание** ||  
    || Метод | POST ||
    || Адрес | `{{ link-tracker }}v2/issues/not_var{{issue.key}}/checklistItems` || 
    ...
    >Пример: добавить три пункта в чеклист; исполнитель пункта — автор задачи.
    >```json
    >[
    >    {
    >        "checked": false,
    >        "text": "Пункт чеклиста 1",
    >        "assignee": "not_var{{issue.author.login}}"
    >    },
    >    {
    >        "checked": false,
    >        "text": "Пункт чеклиста 2",
    >        "assignee": "not_var{{issue.author.login}}"
    >    },
    >    {
    >        "checked": false,
    >        "text": "Пункт чеклиста 3",
    >        "assignee": "not_var{{issue.author.login}}"
    >    }
    >]
    >```
    Подробнее о запросе: [{#T}](../concepts/issues/add-checklist-item.md). Чтобы уточнить параметры запроса, используйте [переменные](../user/vars.md). ||
    |#

1. Чтобы сохранить триггер, нажмите кнопку **Создать**. 

После срабатывания триггера в задаче будет создан чеклист. Если чеклист в задаче уже есть, в него будут добавлены новые пункты.
...
1. Укажите параметры запроса. В поле **Тело запроса** укажите параметры создаваемой подзадачи. Для подстановки значений из исходной задачи используйте [переменные](../user/vars.md):
    ...
    {% if audience == "external" %}|| Заголовки | Заголовок: `X-Org-ID`.
    Значение: Идентификатор организации. Идентификатор указан в поле **ID организации для API** на странице [настроек {{ tracker-name }}]({{ link-settings }}). ||{% endif %}
    |#
