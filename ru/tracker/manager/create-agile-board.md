...
1. Чтобы на доску автоматически попадали задачи с определенными параметрами, включите опцию **Добавлять задачи автоматически** и задайте условия фильтра для отбора задач, которые должны отображаться на доске. Позже условия можно будет изменить.
    ...
    Фильтр с локальными полями задать при создании доски нельзя. Это можно будет сделать позже, на [странице редактирования](edit-agile-board.md#section_ft1_3l3_3z) с помощью [языка запросов](../user/query-filter.md).
    
    {% endnote %}

    Если не включить опцию **Добавлять задачи автоматически**, задачи на доску нужно будет [добавлять вручную](../user/agile.md#section_psd_wl4_vgb).
