...
1. Придумайте уникальный [ключ очереди](#key). Ключ может состоять только из латиницы и не должен содержать пробелов и специальных символов. Также ключ удаленной очереди нельзя использовать повторно.
