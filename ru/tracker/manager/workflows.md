...
## Общая поддержка {#section_nsm_jrm_qz}
...
## Поддержка по линиям {#section_sup_lines}

 Этот шаблон очереди поможет организовать службу поддержки пользователей с двумя линиями. 
Воркфлоу включает типы задач:
