...
* На новой доске доступны инструменты для {% if lang == "ru" %}[гибкой разработки]({{ link-wiki-agile }}){% else %}[гибкой разработки]({{ link-wiki-agile-en }}){% endif %}: [бэклог задач](backlog.md) и [спринты](create-agile-sprint.md). Покер планирования и диаграмма сгорания появятся чуть позже.
