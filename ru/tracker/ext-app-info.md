...
Передайте [дежурному в команде {{ tracker-name }}](https://abc.yandex-team.ru/services/STARTREK/duty/) информацию, которая нужна для регистрации вашего приложения в {{ tracker-name }}.
...
Обязательных параметров достаточно, чтобы обеспечить минимальную функциональность связей между задачами в {{ tracker-name }} и объектами в вашем приложении: отображение иконки приложения, ключа объекта и ссылки на объект.
 ...
 `responsible` | Строка | Ответственные за приложение, к которым можно обратиться, если с приложением будут проблемы. {% if audience == "internal" %}Можно указать ссылку на ABC-сервис.{% endif %}
...
Если ваше приложение поддерживает расширенную функциональность связей, например отображение подробной информации об объекте или автоматическое создание связей во внешнем приложении по запросу из {{ tracker-name }}, передайте соответствующие дополнительные параметры приложения.
 ...
 {% if audience == "internal" %}`suggestUrl` | Строка | URL, который {{ tracker-name }} может вызвать для получения саджеста (подсказки) по объектам во внешнем приложении при создании связи через пользовательский интерфейс {{ tracker-name }}. Подробнее читайте в подразделе [Получение саджеста по объектам в приложении](ext-app-setup.md#suggest).<br/>Формат: `scheme://host:port*not_var{{suggestInput}}*`<br/>где<br/><ul><li>`suggestInput` — текст, введенный пользователем;</li><li>`*` — любая последовательность символов.</li></ul>{% endif %}