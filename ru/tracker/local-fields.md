...
Также вы можете оставить заявку на изменение параметров локального поля через [форму](https://forms.yandex-team.ru/surveys/66769/). В комментарии к заявке напишите, к какой очереди привязано поле.
...
- При создании доски задач не получится настроить автоматическое добавление задач по значению локального поля. Это условие можно будет добавить позже, на [странице редактирования](manager/edit-agile-board.md#section_ft1_3l3_3z) доски с помощью [языка запросов](user/query-filter.md).
...
- Изменить параметр локального поля, например название или описание, или добавить множество значений для параметра. Подробнее в [справочнике API {{ tracker-name }}](concepts/queues/edit-local-field.md).