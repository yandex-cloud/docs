...
Редактирование параметров локального поля через веб-интерфейс {{ tracker-name }} не поддерживается. Чтобы изменить параметры локального поля, отправьте заявку через [форму](https://forms.yandex-team.ru/surveys/14549/). В комментарии к заявке напишите, к какой очереди привязано поле.
