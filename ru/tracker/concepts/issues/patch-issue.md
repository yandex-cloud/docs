...
Статус задачи может быть изменен только с помощью запроса [«Выполнить переход в статус»](new-transition.md).
...
```json
PATCH /{{ ver }}/issues/<ключ_или_идентификатор_задачи>
Host: {{ host }}
...
attachmentIds | Массив идентификаторов временных файлов, которые были [предварительно загружены](./temp-attachment.md) в {{ tracker-name }}. Перечисленные файлы прикрепятся к задаче и будут отображаться на вкладке **Вложения**. | Массив строк
descriptionAttachmentIds | Массив идентификаторов временных файлов, которые были [предварительно загружены](./temp-attachment.md) в {{ tracker-name }}. Перечисленные файлы прикрепятся к задаче и будут отображаться в ее описании. | Массив строк
...
> Пример 3: Прикрепить файлы к описанию задачи.
> 
> - Используется HTTP-метод PATCH.
> - Редактируется задача TEST-3.
> - Предварительно следует [загрузить временные файлы](./temp-attachment.md) и указать их идентификаторы.
> - Временные файлы прикрепляются к описанию задачи.
> 
>
> ```
