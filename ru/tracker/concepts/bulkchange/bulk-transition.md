...
Чтобы узнать, какие переходы доступны для задачи, выполните [запрос списка переходов](../issues/get-transitions.md). Для некоторых статусов (например, «Закрыт») в параметре `value` тела запроса должна быть указана [резолюция]({{ link-admin-resolutions }}).
