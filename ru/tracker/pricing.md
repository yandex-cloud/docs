...
Стоимость использования {{ tracker-name }} зависит от общего числа пользователей, у которых есть [полный доступ к сервису](access.md). Ежемесячный платеж списывается с [платежного аккаунта](billing-account.md), привязанного к {{ tracker-name }}. 
