---
title: "Как мигрировать в {{ tracker-full-name }} из других сервисов"
description: "Следуя данной инструкции, вы сможете мигрировать в {{ tracker-full-name }} из других сервисов." 
---
...
* Если вы используете Jira, наши специалисты помогут перенести данные в {{ tracker-name }}. Для этого подготовьте данные для миграции [по инструкции ниже](#jira) и обратитесь в техническую поддержку через [форму обратной связи]({{ link-tracker-feedback }}).
...
1. Передайте запрос на миграцию из Jira в {{ tracker-name }} в техническую поддержку через [форму обратной связи]({{ link-tracker-feedback }}). В запросе укажите:
...
1. Подготовьте Python клиент:
   ...
   1. Инициализируйте клиент.
      ...
      ```
      Здесь `<OAuth-токен>` — ваш OAuth-токен, а `<идентификатор_организации>` — идентификатор организации. Чтобы узнать идентификатор организации, перейдите на [страницу настроек {{ tracker-name }}]({{ link-settings }}). Идентификатор указан в поле **ID организации для API**.
...
1. Выполните скрипт для загрузки данных из таблицы. 
   ...
   1. Создайте задачи, при необходимости обновите имя автора или измените другие атрибуты задачи. 
      ...
      for i, row in df.iterrows():
            ...
            issue.update(<ключ_поля>='<значение>')
