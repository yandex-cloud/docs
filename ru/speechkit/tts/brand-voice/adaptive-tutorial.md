# Использование {{ brand-voice-name }} Adaptive

Чтобы синтезировать фразу с помощью подготовленной модели адаптивного синтеза речи [{{ brand-voice-name }} Adaptive](index.md#adaptive):

1. [Подготовьте шаблон аудиозаписи](#prepare-fragment).
1. [Отправьте данные в API {{ brand-voice-name }}](#send-to-api).
...
## Подготовьте шаблон аудиозаписи {#prepare-fragment}
...
* Запишите аудио шаблонной фразы. Голос диктора должен быть тем же, который использовался для записей в обучении.
* Разметьте текст шаблонной фразы согласно [требованиям](./income-data-format.md#text-requirements) и оберните переменные части в скобки `{}`.
...
Длительность фразы для синтеза не должна превышать 24 секунды (ограничение API) вместе с переменной частью. Оптимальное восприятие фразы достигается при длительности не более 16 секунд — как в разговорной речи.
              ...
              request = tts_pb2.UtteranceSynthesisRequest(
                  ...
                  text_template = tts_pb2.TextTemplate(
                                  ...
                                  variables = [
                                      ...
                                      tts_pb2.TextVariable(
                                          variable_name = '<имя переменной в template>',
                                          # Пример: '{time}'
                                          variable_value ='<текст для синтеза>'
                  ...
                  hints = [
                     tts_pb2.Hints(
                      ...
                      tts_pb2.Hints(
                          voice = '{идентификатор модели {{ brand-voice-name }} Adaptive}'
                      )
                  ]
              )
