...
* ZIP-архив аудиозаписей в формате [WAV]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/WAV){% endif %}{% if lang == "en" %}(https://en.wikipedia.org/wiki/WAV){% endif %}, в котором содержатся:
...
1. Длина фразы целиком не должна превышать 160 символов, включая переменную часть.
