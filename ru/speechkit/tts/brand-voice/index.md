...
Технология {{ brand-voice-name }} позволяет создавать уникальные голоса для модели синтеза речи. 

{% endif %}

{% if product == "yandex-cloud" %}

Технология {{ brand-voice-name }} позволяет создавать уникальные голоса для модели синтеза речи. В зависимости от целей использования обученной модели {{ yandex-cloud }} предлагает три направления {{ brand-voice-name }}.
...
Технология {{ brand-voice-cc-name }} позволяет озвучивать текст по _шаблонам_ — специально заготовленным фразам, в которых изменяются отдельные ключевые части — _переменные_. {{ brand-voice-cc }} не требует подготовки специальной модели, обучающейся на голосе вашего диктора: голос будет скопирован прямо из шаблонов, которые вы передаете для генерации фраз. При этом речь генерируется целиком, а не склеивается из заранее записанного шаблона и сгенерированной переменной части. 

Применение {{ brand-voice-cc-name }} ограничено телефонными звонками. Синтезируемые тексты должны быть достаточно короткими: длительность синтезированной фразы не может быть больше {{ tts-v3-time }}, а длина фразы не должна превышать {{ tts-v3-count }} вместе с переменной частью.

С помощью {{ brand-voice-cc-name }} вы легко сможете автоматизировать шаблонные диалоги. 

> Например, аудио с фразой `Здравствуйте, Михаил! Это компания Облака и тучи. Меня зовут Анастасия. Вам удобно разговаривать?` можно легко преобразовать в аудио `Здравствуйте, Анна! Это компания Новые окна и двери. Меня зовут Матвей. Вам удобно разговаривать?` без записей дополнительных фраз.

### Требования и ограничения {{ brand-voice-cc-name }} {#restrictions-cc}

Для работы {{ brand-voice-cc-name }} не требуются обучающие данные, поскольку модель копирует голос из шаблона. Поэтому для синтеза необходимы только аудиофайл с шаблоном фразы и текст с размеченными переменными. Подробнее о требованиях к текстам см. [{#T}](../templates.md#requirements-text).
...
Синтезируемые тексты должны быть достаточно короткими: длительность синтезированной фразы не может быть больше {{ tts-v3-time }}, а длина фразы не должна превышать {{ tts-v3-count }} вместе с переменной частью.

По умолчанию {{ brand-voice-cc-name }} логирует передаваемые шаблоны (текст и аудио). При этом синтезированные аудиозаписи и переменные части, а значит, и ваши данные, не попадают в логирование. Вы можете отключить логирование с помощью [заголовка](../../concepts/support-headers#request-headers) `x-data-logging-enabled: false`, однако это может отразиться на качестве работы сервиса.
