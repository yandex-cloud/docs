...
Все голоса совместимы с {{ realtime-api }} для создания голосовых агентов в сервисе {{ ai-studio-full-name }}.
...
| `ru-RU` | русский | `zamira_ru` | Ж | (по умолчанию) нейтральный — `neutral`</br>строгая — `strict`</br>дружелюбная — `friendly`| v3 |
...
| `uz-UZ` | узбекский | `zamira` | Ж | (по умолчанию) нейтральный — `neutral`</br>строгая — `strict`</br>дружелюбная — `friendly` | v3 |
