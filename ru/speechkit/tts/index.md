...
## Потоковый синтез {#streaming}

{{ speechkit-name }} позволяет [поочередно отправлять тексты](api/tts-streaming.md) для синтеза в рамках одной сессии с помощью API v3. Это удобно, когда текст неизвестен заранее, но вам важна скорость синтеза. Например, синтез ответов [LLM](../../glossary/llm.md) по мере их генерации.

Потоковый синтез пока не поддерживает голоса [{{ brand-voice-lite-name }}](./brand-voice/index.md#lite).
