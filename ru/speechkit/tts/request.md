...
`format` | Формат синтезируемого аудио.<br/>Допустимые значения:<ul><li>`lpcm` — аудиофайл синтезируется с помощью метода LPCM без WAV-заголовка. Характеристики аудио:<ul><li>Дискретизация — 8, 16 или 48 kHz в зависимости от значения параметра `quality`.</li><li>Битовая глубина — 16-bit.</li><li>Порядок байтов — обратный (little-endian).</li><li>Аудиоданные хранятся как подписанные числа (signed integer).</li></ul></li><li>`oggopus` — аудиофайл синтезируется с помощью аудиокодека OPUS и контейнера OGG ([OggOpus](https://wiki.xiph.org/OggOpus)).</li></ul>Значение параметра по умолчанию: `oggopus`.
...
Если синтез прошел успешно, в ответе будет бинарное содержимое аудиофайла. Формат выходных даннх зависит от значения параметра `format`.
