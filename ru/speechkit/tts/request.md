...
emotion | **string**<br>Эмоциональная окраска голоса. Рекомендуем использовать только для [голосов с поддержкой эмоций](voices.md).<br/>Допустимые значения:<ul><li>`good` — радостный, доброжелательный;</li><li>`evil` — раздраженный;</li><li>`neutral` (по умолчанию) — нейтральный.</li></ul>
