...
- Русский язык
    ...
    * По тегу `general:deprecated` доступна предыдущая версия модели.
    ...
    #### Релиз 27.10.2020 {#2710}

    Обновление в синтезе — по тегу `alena:rc` доступна новая версия премиум-голоса `alena`.

    Основные изменения в этом релизе:
    * Изменен механизм нормализации. Теперь синтез правильнее произносит имена и названия. Исправлено склонение числительных.
    * Много исправлений в произношении отдельных слов.

    Приглашаем к тестированию! В течение ближайшего месяца эта версия заменит текущую версию голоса `alena`.
