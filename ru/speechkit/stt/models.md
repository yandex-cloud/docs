...
- Русский язык
    ...
    * По тегу `deferred-general` доступна модель отложенной транскрибации по [специальным тарифам](../pricing#prices-stt). Данная модель предполагает, что аудио попадает в очередь с более низким приоритетом и обрабатывается в наименее нагруженное время, но не более 24 часов.
    ...
    Представляем новую версию _Амати_ модели `hqa`, доступную теперь в транскрибации. В новой модели исправлены случаи, когда вместо речи распознавалась тишина. Кроме того, улучшено распознавание текстов из тематических доменов новостей и медицины.
