...
- Русский язык
    ...
    * `general` — основная поддерживаемая модель для русского языка, которая распознает речь на любую тему на русском языке: короткие и длинные фразы, а также имена, адреса, даты и числа.
    ...
    ### Выход новых версий {#new-versions}
    ...
    * `general:deprecated` — версия _Диоген_.
    ...
    Версии для распознавания коротких аудио и для распознавания длинных аудио, доступные по тегу `general`, остались без изменений.
