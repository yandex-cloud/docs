# Распознавание коротких аудиофайлов

В этом разделе описано, как распознавать короткие аудиофайлы.

## Требования к аудиофайлу

Передаваемый аудиофайл должен соответствовать требованиям:

1. Максимальный размер файла — [!KEYREF stt-short—fileSize].
1. Максимальная длительность аудио — [!KEYREF stt-short-audioLength].
1. Максимальное количество аудиоканалов — [!KEYREF stt-short-channelsCount].

Если ваш файл больше, длиннее или у него больше аудиоканалов, используйте [распознавание длинных аудиофайлов](transcribation.md).
...
`topic` | Языковая модель, которую следует использовать при распознавании.<br/>Чем точнее выбрана модель, тем лучше результат распознавания. В одном запросе можно указать только одну модель.<br/>[Допустимые значения](../stt/models.md) зависят от выбранного языка. Значение параметра по умолчанию: `general`.
...
`format` | [Формат](formats.md) передаваемого аудио.<br/>Допустимые значения:<ul><li>`lpcm` — [LPCM без WAV-заголовка](formats.md#lpcm).</li><li>`oggopus` (по умолчанию) — [OggOpus](formats.md#oggopus).</li></ul>
