# Сервис потокового распознавания речи

Потоковый режим позволяет использовать SpeechKit Cloud API для отправки аудиоданных частями. В отличие от [POST HTTP API](request.md), в потоковом режиме можно получать промежуточные результаты распознавания. Паузы, после которых отправляются финальные результаты распознавания, определяются автоматически. Этот режим подходит для распознавания звука, передаваемого в потоке.

> Например, как только пользователь начинает говорить, ваше приложение сразу же начинает передавать его речь на сервер для распознавания. Сервер обрабатывает данные и отправляет вашему приложению промежуточные и финальные результаты распознавания каждой фразы. Промежуточные результаты используются для того, чтобы показать пользователю ход распознавания.
...
Вы можете указать, чтобы сервис возвращал промежуточные результаты распознавания. Для этого в [сообщении с настройками распознавания](#specification-msg) укажите `specification.partial_results=true`. В ответе для промежуточных результатов `final=false`, а для финальных `final=true`.
...
Параметр | Описание
----- | -----
`specification.language_code` | Язык, для которого будет выполнено распознавание.<br/>Допустимые значения:<ul><li>`ru-RU` (по умолчанию) — русский язык,</li><li>`en-US` — английский язык.</li><li>`tr-TR` — турецкий язык.</li></ul>
`specification.model` | Языковая модель, которую следует использовать при распознавании.<br/>Чем точнее выбрана модель, тем лучше результат распознавания. В одном запросе можно указать только одну модель.<br/>[Допустимые значения](../stt/index.md#model) зависят от выбранного языка. Значение параметра по умолчанию: `general`.
`specification.profanity_filter` | Фильтр ненормативной лексики.<br/>Допустимые значения:<ul><li>`true` — исключать ненормативную лексику из результатов распознавания.</li><li>`false` (по умолчанию) — не исключать ненормативную лексику.</li></ul>
`specification.partial_results` | Фильтр промежуточных результатов.<br/>Допустимые значения:<ul><li>`true` — возвращать промежуточные результаты (часть распознанного высказывания). Для промежуточных результатов `final` равно `false`.</li><li>`false` (по умолчанию) — возвращать только финальные результаты (распознанное высказывание целиком).
`specification.format` | Формат передаваемого аудио.<br/>Допустимые значения:<ul><li>`LINEAR16_PCM` — аудиофайл в формате [LPCM](https://en.wikipedia.org/wiki/Pulse-code_modulation) без WAV-заголовка. Характеристики аудио:<ul><li>Дискретизация — 8, 16 или 48 kHz в зависимости от значения параметра `sampleRateHertz`.</li><li>Разрядность квантования — 16-bit.</li><li>Порядок байтов — обратный (little-endian).</li><li>Аудиоданные хранятся как знаковые числа (signed integer).</li></ul></li><li>`OGG_OPUS` (по умолчанию) — данные закодированы с помощью аудиокодека OPUS и упакованы в контейнер OGG ([OggOpus](https://wiki.xiph.org/OggOpus)).</li></ul>
`specification.sampleRateHertz` | Частота дискретизации передаваемого аудио.<br/>Этот параметр обязателен, если значение `format` равно `LINEAR16_PCM`. Допустимые значения:<ul><li>`48000` (по умолчанию) — частота дискретизации 48 кГц;</li><li>`16000` — частота дискретизации 16 кГц;</li><li>`8000` — частота дискретизации 8 кГц.</li></ul>
`folder_id` | Обязательный параметр. Идентификатор каталога, к которому есть доступ у вашего аккаунта.

### Сообщение с аудиофрагментом {#audio-msg}

Параметр | Описание
----- | -----
`audio_content` | Фрагмент аудио в виде массива байт. Аудио должно быть в формате [OggOpus](https://wiki.xiph.org/OggOpus) или [LPCM](https://en.wikipedia.org/wiki/Pulse-code_modulation).
