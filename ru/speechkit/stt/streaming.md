...
Чтобы использовать сервис, создайте приложение, которое будет осуществлять потоковое распознавание речи: отправлять аудиофрагменты и обрабатывать ответ с результатами распознавания.
...
Не дожидаясь окончания потока сообщений с аудиофрагментами, {{ speechkit-name }} будет возвращать [промежуточные результаты](api/streaming-api.md#response) распознавания. 

Чтобы приложение смогло обращаться к сервису, необходимо сгенерировать код интерфейса клиента для используемого языка программирования из файла [stt_service.proto](https://github.com/yandex-cloud/cloudapi/blob/master/yandex/cloud/ai/stt/v2/stt_service.proto) репозитория [{{ yandex-cloud }} API](https://github.com/yandex-cloud/cloudapi/).
...
В каждом запросе приложение должно передавать [идентификатор каталога](../../resource-manager/operations/folder/get-id.md), на который у вас есть роль `editor` или выше. Подробнее о необходимых разрешениях см. в разделе [Управление доступом](../security/index.md).
