...
Узнайте как получить IAM-токен для вашего аккаунта в соответствующей инструкцию
...
Если в течении 5 секунд в сервис не отправлялись сообщения или достигнут лимит по длительности или размеру данных, сессия обрывается. Чтобы продолжить распознавание речи необходимо снова отправить сообщение с настройками распознавания и сервис начнет новую сессию.
...
При успешном распознавании фрагмента речи придет сообщение содержащие список результатов распознавания `chunks[]`. Каждый результат содержит поля:

* `alternatives[]` - список вариантов распознанного текста. Каждый вариант содержит поля:
    * `text` - распознанный текст.
    * `confidence` - достоверность определения. Сейчас сервис всегда возвращает значение 100%.
...
1. Выберите аудиофайл для распознавания. В примерах используется файл `speech.pcm` в формате аудиофайл в формате [LPCM](https://en.wikipedia.org/wiki/Pulse-code_modulation) с частотой дискретизации 8000.
    > [!NOTE]
    >
