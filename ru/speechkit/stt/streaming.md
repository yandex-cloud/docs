...
* `alternatives[]` - список вариантов распознанного текста. Каждый вариант содержит поля:
    ...
    * `confidence` - достоверность определения в процентах. Это поле сейчас не поддерживается и всегда возвращает значение `1`.
...
* `endOfUtterance` — флаг, указывающий что этот результат содержит конец фразы. Если значение `true`, то следующего результата полученного результата начнется новая фраза.
