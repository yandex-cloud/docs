Роль `connection-manager.admin` позволяет управлять подключениями и доступом к ним, а также просматривать информацию о подключениях, в том числе секретные данные, такие как пароли доступа к БД.
