Роль `monium.viewer` позволяет просматривать информацию о ресурсах платформы {{ monium-name }} с возможностью чтения всех видов телеметрии.
...
* читать все виды телеметрии платформы {{ monium-name }}: [метрики](../../monium/metrics/quickstart.md), [логи](../../monium/logs/quickstart.md) и [распределенные трассировки](../../monium/traces/index.md);
