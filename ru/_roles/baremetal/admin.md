...
Роль `baremetal.admin` позволяет управлять серверами {{ baremetal-name }}, приватными подсетями, виртуальными сегментами сети (VRF) и образами операционных систем серверов, а также резервным копированием серверов с помощью сервиса {{ backup-name }}.
