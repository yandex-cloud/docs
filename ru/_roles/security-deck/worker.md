Роль `security-deck.worker` позволяет просматривать информацию об области сканирования модуля {{ dspm-name }} и контролируемых ресурсах модулей {{ kspm-name }}, {{ cspm-name }} и {{ td-name }} в {{ sd-name }}.
...
* просматривать логи, регистрируемые в инфраструктуре клиента, с помощью сервиса [{{ at-full-name }}](../../audit-trails/index.yaml) для [модуля {{ td-name }}](../../security-deck/concepts/threat-detector.md).

Роль выдается [сервисному аккаунту](../../iam/concepts/users/service-accounts.md), от имени которого будет выполняться [сканирование](../../security-deck/concepts/dspm.md#scanning) {{ dspm-name }}, проверка {{ kspm-name }}, {{ cspm-name }} или {{ td-name }}. Роль назначается на организацию, облако, каталог или (при использовании [модуля {{ dspm-name }}](../../security-deck/concepts/dspm.md)) бакет.
