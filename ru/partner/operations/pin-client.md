...
Если клиент не перешел по ссылке в письме и не может найти письмо, вы можете сформировать ссылку для подтверждения партнерства самостоятельно и передать ее клиенту. Для этого в ссылке `{{ link-cloud-partners }}invite/accept/ID_сабаккаунта` замените `ID_сабаккаунта` на идентификатор партнера вида `dn2...`.
