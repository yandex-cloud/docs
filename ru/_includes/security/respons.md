...
Безопасность систем, использующих облачные сервисы, требует разделения ответственности между клиентом (владельцем конечной системы) и провайдером (владельцем облачной инфраструктуры). Это разделение зависит от модели облачных сервисов: {% if lang == "ru" and product == "yandex-cloud" %}[IaaS (Infrastructure as a Service)](https://cloud.yandex.ru/blog/posts/2022/01/iaas){% else %}IaaS (Infrastructure as a Service){% endif %} или PaaS (Platform as a Service). 
