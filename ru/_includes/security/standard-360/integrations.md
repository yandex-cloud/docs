...
Доступ к организации реализован через единый вход (SSO) с использованием внешнего SAML-совместимого поставщика удостоверений (IdP). Доменные пользователи не должны существовать параллельно с SSO-аккаунтами. Если у вас нет своего IdP, можно использовать {{ org-full-name }}. Для этого создайте SAML-приложение в {{ org-full-name }} и настройте его как на стороне {{ org-full-name }}, так и на стороне поставщика услуг.

Подробнее смотрите в [Создать SAML-приложение в {{ org-full-name }}](../../../organization/operations/applications/saml-create.md).
