...
Физическую безопасность дата-центров обеспечивает Платформа {{ yandex-cloud }}, смотрите [подробное описание мер физической безопасности](../../../security/standarts.md). Если критичные данные передаются за пределы {{ yandex-cloud }}, то клиент отвечает за управление физическим доступом для всех мест обработки данных.
...
Вы можете автоматизировать аудит выполнения всех рекомендаций с помощью [{{ sd-full-name }}](../../../security-deck/) — это комплексная CNAPP-платформа, включающая в себя модули:
