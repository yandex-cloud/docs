...
* **Обновлены пункты:**
  ...
  * В пункте **1.4 Только необходимые администраторы управляют членством в {{ iam-short-name }}-группах** уточнены роли для управления составом групп. Указано, что роль `{{ roles-editor }}` не позволяет управлять членством в группе или назначать роли. Для управления составом конкретной группы рекомендована роль `organization-manager.groups.memberAdmin`.
  * В пункте **1.5 Используются сервисные роли вместо примитивных: {{ roles-admin }}, {{ roles-editor }}, {{ roles-viewer }}, {{ roles-auditor }}** рекомендованы сервисные роли для просмотра конфигурации и метаданных. В проверки примитивных ролей в консоли управления и Bash добавлена роль `{{ roles-auditor }}`.
  * В пункте **1.6 Используется роль {{ roles-auditor }} для исключения доступа к данным пользователей** добавлено предупреждение: безопаснее использовать сервисные роли уровня `auditor`, а при назначении примитивной роли `{{ roles-auditor }}` необходимо ограничивать набор ресурсов и учитывать наследование прав.
  ...
  * В пункте **1.19 Доступ для подрядных организаций и третьих лиц контролируется** рекомендованы сервисные роли без доступа к данным вместо примитивной роли `{{ roles-auditor }}`.

### Изменения в версии 1.4.3 {#version-1-4-3}

Дата публикации 08.09.2026.

* Для требований, где есть [проверка в CSPM](../../../security-deck/concepts/standard-compliance/yc-cloud-security-standard), добавлена сноска.
...
* **Добавлены пункты:**
  ...
  * 4.2.1 В Managed Services for Databases используется шифрование данных at rest с ключом {{ kms-short-name }}.

* **Обновлены пункты:**

  * В пункте **2.1 Для объектов облака используется межсетевой экран или группы безопасности**:
      * Изменена логика скрипта: поиск осуществляется по ВМ без привязанных групп безопасности вместо идентификаторов продуктов в {{ marketplace-name }}.
      * Добавлено пояснение, что {{ yandex-cloud }} не может автоматически отслеживать конфигурацию BYOI.
  * В пункте **2.2 В {{ vpc-name }} создана группа безопасности и не используется группа безопасности по умолчанию** вместо проверки на использование группы безопасности по умолчанию скрипт ищет наличие хотя бы одной пользовательской группы безопасности.
  ...
  * В пункте **2.6 Используется защищенный удаленный доступ** интегрировано требование использовать {{ oslogin }} для доступа к ВМ и кластерам {{ k8s }} и добавлена проверка через CLI.
  ...
  * В пункте **3.8 Отсутствует публичный доступ к бакету {{ objstorage-name }}** проверка переписана на `yc storage bucket get`, удалена зависимость от AWS CLI и статических ключей.
  * В пункте **3.9 В {{ objstorage-name }} используются политики доступа (Bucket Policy)** проверка переписана на `yc storage bucket get`, удалена зависимость от AWS CLI и статических ключей.
  * В пункте **3.10 В {{ objstorage-name }} включена функция «Блокировка версии объекта» (Object Lock)**:
  ...
  * В пункте **3.10 В {{ objstorage-name }} включена функция «Блокировка версии объекта» (objectlock)** проверка переписана на `yc storage bucket get`, удалена зависимость от AWS CLI и статических ключей.
  * В пункте **3.20 {{ serverless-containers-short-name }}/{{ sf-name }} использует внутреннюю сеть {{ vpc-short-name }}**:
  ...
  * В пункте **3.26 Публичный доступ отсутствует для {{ ydb-short-name }}**:
      * Переписан текст проверки.
      * Добавлено требование назначать группы безопасности на кластеры Dedicated {{ ydb-name }}.
  ...
  * В пункте **3.35 Для подключения к виртуальной машине или узлу {{ k8s }} используется {{ oslogin }}** в скрипт добавлены три уровня проверки: глобальный уровень, уровень ВМ, уровень {{ k8s }}.
  * В пункте **3.39 Используется {{ backup-short-name }} или механизм snapshot по расписанию** добавлена проверка через CLI и технические метрики проверки.
  * В пункте **4.2 В {{ objstorage-full-name }} включено шифрование данных at rest с ключом {{ kms-short-name }}** проверка переписана на `yc storage bucket get`, удалена зависимость от AWS CLI и статических ключей.
  * В пункте **4.11 Для {{ kms-short-name }} ключей включена ротация**:
      ...
      * Добавлена ссылка на инструкцию по вращению ключей {{ kms-short-name }}.
  * В пункте **7.5 В {{ managed-k8s-name }} используется безопасная конфигурация** изменена логика проверки, внедрен механизм прерывания `short circuit`.
...
* **Добавлены пункты:**
    ...
    * 1.26 Регулярно проводится аудит прав доступа пользователей и сервисных аккаунтов с использованием {{ sd-full-name }} {{ ciem-name }}.
...
* **Обновлены пункты:**
    ...
    * В пункте **5.4 Выполнен hardening бакета {{ objstorage-name }}, где хранятся аудитные логи {{ at-full-name }}** добавлены ссылки на лучшие практики безопасности.
