...
* [Инструкция по настройке SAML федерации удостоверений](../../../organization/add-federation.md#federation-usage).
...
- Проверка в консоли управления
  ...
  1. Откройте консоль {{ yandex-cloud }} в вашем браузере.
  1. Перейдите в нужный каталог и откройте вкладку **Сервисные аккаунты**.
...
* [Двухфакторная аутентификация — Яндекс ID]{% if lang == "ru" %}(https://yandex.ru/support/id/authorization/twofa.html){% endif %}{% if lang == "en" %}(https://yandex.com/support/id/authorization/twofa.html){% endif %}.
...
* `{{ roles-cloud-owner }}`;
* `{{ roles-admin }}`, назначенный на организацию;
...
Для просмотра данных биллинга назначьте роль `{{ roles-viewer }}` на платежный аккаунт выделенному сотруднику организации с федеративным аккаунтом.
...
- Проверка в консоли управления
  ...
  1. Проверьте кому назначены роли: `{{ roles-admin }}`, `{{ roles-cloud-owner }}`.
...
В настройках [федерации удостоверений](../../../organization/add-federation.md) необходимо убедиться, что значение параметра **Время жизни cookie** меньше либо равно 6 часов. Это необходимо, чтобы минимизировать риск компрометации рабочих станций пользователей облака.
