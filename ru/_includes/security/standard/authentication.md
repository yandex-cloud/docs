...
* `organization-manager.groups.memberAdmin`, `organization-manager.groups.admin` или `{{ roles-admin }}`, назначенные на конкретную группу или на организацию.
...
- Проверка в консоли управления {#console}
  ...
  1. Откройте консоль {{ yandex-cloud }} в вашем браузере.
  1. Перейдите во вкладку **Все сервисы** → **{{ org-full-name }}** → **Пользователи**.
  1. Если у всех учетных записей в колонке **Права доступа** отсутствуют примитивные роли `{{ roles-admin }}`, `{{ roles-editor }}`, `{{ roles-viewer }}` и `{{ roles-auditor }}`, то рекомендация выполняется. Если нет, то перейдите к п. <q>Инструкции и решения по выполнению</q>.
  1. Далее перейдите в общее меню облака (нажать на облако в исходном меню облака). Выберите вкладку **Права доступа**.
  1. Если у всех учетных записей в колонке **Роли** отсутствуют примитивные роли `{{ roles-admin }}`, `{{ roles-editor }}`, `{{ roles-viewer }}` и `{{ roles-auditor }}`, то рекомендация выполняется. Если нет, то перейдите к п. <q>Инструкции и решения по выполнению</q>.
...
Вместо примитивной роли `{{ roles-auditor }}` рекомендуется назначать сервисные роли уровня `auditor`, например [compute.auditor](../../../iam/roles-reference.md#compute-auditor). Это более безопасный подход: он позволяет ограничить доступ ресурсами нужного сервиса.

Если вы используете примитивную роль `{{ roles-auditor }}`, внимательно выбирайте ресурсы, на которые ее назначаете. Эта роль действует во всех сервисах, а ее разрешения наследуются вложенными ресурсами. Назначайте ее только на минимально необходимый набор ресурсов.
...
* `billing.accounts.admin`, `billing.accounts.editor` и `{{ roles-admin }}`, назначенные на платежный аккаунт;
...
- Проверка в консоли управления {#console}
  ...
  1. Проверьте, кому назначены роли: `billing.accounts.owner`, `billing.accounts.admin`, `billing.accounts.editor`, `{{ roles-admin }}`.
