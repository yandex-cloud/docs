...
Кроме того, {{ monitoring-name }} можно применять для мониторинга работоспособности самого сервиса {{ at-name }} и мониторинга событий безопасности. Выгрузка метрик в SIEM-систему возможна через {% if lang == "ru" %}[API](../../../glossary/rest-api.md){% else %}API{% endif %}, см. [инструкцию](../../../monitoring/operations/metric/get.md).
