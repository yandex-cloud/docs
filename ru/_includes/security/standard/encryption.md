...
* Контролировать шифрование и жизненный цикл ключей {{ kms-short-name }}, а также управлять ими. Подробнее в разделе [{#T}](../../../kms/operations/key.md).
* Повысить уровень контроля доступа к данным на диске за счет необходимости прав на ключ {{ kms-short-name }}. Подробнее в разделе [{#T}](../../../kms/operations/key-access.md).
* Отслеживать операции шифрования и расшифрования вашим ключом {{ kms-short-name }} с помощью сервиса {{ at-full-name }}. Подробнее в разделе [{#T}](../../../kms/concepts/index.md#keys-audit).
...
Рекомендуется выдавать пользователям и сервисным аккаунтам гранулярные доступы на конкретные ключи сервиса {{ kms-short-name }}. Подробнее в статье [Управление доступом в {{ kms-name }}](../../../kms/security/) документации {{ kms-short-name }}.
...
Подробнее о ротации ключей смотрите в разделе [Версия ключа](../../../kms/concepts/version.md) документации {{ kms-short-name }}.
...
Инструкции по работе с сервисом смотрите в [документации](../../../lockbox/) {{ lockbox-short-name }}.
...
Другие рекомендации по безопасному использованию {{ TF }} смотрите в статье [Безопасная конфигурация: {{ TF }}](../../../security/standard/virtualenv-safe-config.md#tf-using).
