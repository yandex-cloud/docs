...
#### 4.6 Используется шифрование данных на уровне приложения {#self-data-app}
...
#### 4.7 Используется шифрование дисков и снимков виртуальных машин {#managed-vm-kms}
...
#### 4.8 Ключи {{ kms-name }} хранятся в аппаратном модуле безопасности (HSM) {#keys-hsm}
...
#### 4.9 Права на управление ключами в {{ kms-short-name }} выданы контролируемым пользователям {#keys-controlled-users}
...
#### 4.10 Для {{ kms-short-name }} ключей включена ротация {#keys-rotation}
...
#### 4.11 Для ключей {{ kms-short-name }} включена защита от удаления {#keys-deletion-protection}
...
#### 4.12 В организации используется {{ lockbox-full-name }} для безопасного хранения секретов {#secrets-lockbox}
...
#### 4.13 Для {{ serverless-containers-name }} и {{ sf-name }} используются секреты {{ lockbox-short-name }} {#secrets-serverless-functions}
...
#### 4.14 При работе {{ coi }} используется шифрование секретов {#secrets-coi}
...
#### 4.15 Администратор облака имеет инструкцию по действиям в случае компрометации секретов его облака {#secrets-scanning}
