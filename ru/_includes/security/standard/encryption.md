...
- Проверка в консоли управления {#console}
  ...
  1. [Перейдите]({{ link-console-main }}/link/application-load-balancer) в сервис **{{ alb-name }}**.
...
- Проверка в консоли управления {#console}
  ...
  1. [Перейдите]({{ link-console-main }}/link/cdn) в сервис **{{ cdn-name }}**.
...
* Контролировать шифрование и жизненный цикл ключей {{ kms-short-name }}, а также управлять ими. Подробнее читайте в разделе [{#T}](../../../kms/operations/key.md).
* Повысить уровень контроля доступа к данным на диске за счет необходимости прав на ключ {{ kms-short-name }}. Подробнее читайте в разделе [{#T}](../../../kms/operations/key-access.md).
* Отслеживать операции шифрования и расшифрования вашим ключом {{ kms-short-name }} с помощью сервиса {{ at-full-name }}. Подробнее читайте в разделе [{#T}](../../../kms/concepts/index.md#keys-audit).
...
- Проверка в консоли управления {#console}
  ...
  1. [Перейдите]({{ link-console-main }}/link/kms) в сервис **{{ kms-name }}**.
...
Рекомендуется выдавать пользователям и сервисным аккаунтам гранулярные доступы на конкретные ключи сервиса {{ kms-short-name }}. Подробнее читайте в статье [Управление доступом в {{ kms-name }}](../../../kms/security/) документации {{ kms-short-name }}.
...
Подробнее о ротации ключей читайте в разделе [Версия ключа](../../../kms/concepts/version.md) документации {{ kms-short-name }}.
...
Инструкции по работе с сервисом в [документации](../../../lockbox/) {{ lockbox-short-name }}.
...
- Проверка в консоли управления {#console}
  ...
  1. [Перейдите]({{ link-console-main }}/link/lockbox) в сервис **{{ lockbox-short-name }}**.
...
- Проверка в консоли управления {#console}
  ...
  1. [Перейдите]({{ link-console-main }}/link/functions) в сервис **{{ sf-name }}**.
...
{{ kms-short-name }} предоставляет возможность шифрования секретов, используемых в конфигурации {{ TF }}, в частности, для передачи секретов на виртуальную машину в зашифрованном виде. Смотрите инструкцию в разделе [Шифрование секретов в {{ TF-full }}](../../../kms/tutorials/terraform-secret.md) документации {{ kms-short-name }}. Передача секретов через переменные окружения в открытом виде небезопасна, поскольку они отображаются в свойствах ВМ.
...
Другие рекомендации по безопасному использованию {{ TF }} читайте в статье [Безопасная конфигурация: {{ TF }}](../../../security/standard/virtualenv-safe-config.md#tf-using).
