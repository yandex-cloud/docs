...
* В поле **{{ ui-key.yacloud.cdn.field_secure-key-type }}** задайте ограничение на доступ к контенту по IP-адресу:
   ...
   * `{{ ui-key.yacloud.cdn.value_secure-key-type-enable }}` — доступ к файлам будет разрешен только с определенного IP-адреса получателя контента. IP-адрес задается вне CDN-ресурса и указывается в качестве параметра при формировании [MD5](https://ru.wikipedia.org/wiki/MD5)-хеша для подписанной ссылки.
