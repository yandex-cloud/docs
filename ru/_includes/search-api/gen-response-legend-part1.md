...
* В полях `site`, `host` и `url` задается область поиска — обязательный параметр запроса. При этом поля `site`, `host` и `url` — взаимоисключающие, задать можно только одно из них:
