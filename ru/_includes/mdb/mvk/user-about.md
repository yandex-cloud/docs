...
Вместе с кластером создается пользователь по умолчанию `default`. Он имеет разрешения на все ключи и каналы Pub/Sub кластера, а также может выполнять все команды, кроме административных. Подробные разрешения пользователя можно получить с [информацией о пользователе](../../../managed-valkey/operations/user-list.md#get). 