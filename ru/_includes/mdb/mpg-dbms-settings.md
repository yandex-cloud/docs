...
- **Autovacuum max workers**{#setting-autovacuum-max-workers} {{ tag-con }} {{ tag-api }} {{ tag-tf }} {{ tag-cli }}
  ...
  Минимальное значение — `1`, максимальное значение — `32`. Для данной настройки значение по умолчанию зависит от выбранного [класса хостов](../../managed-postgresql/concepts/instance-types.md).
...
- **Autovacuum vacuum cost delay**{#setting-autovacuum-vacuum-cost-delay} {{ tag-all }}
  ...
  Время (в миллисекундах) «заморозки» процесса автоочистки.

  Минимальное значение — `-1` (настройка не применяется), максимальное значение — `100`. Для данной настройки значение по умолчанию зависит от выбранного [класса хостов](../../managed-postgresql/concepts/instance-types.md).
...
- **Autovacuum vacuum cost limit**{#setting-autovacuum-vacuum-cost-limit} {{ tag-all }}
  ...
  Целое число, максимальный размер «штрафа» для [процесса автоочистки](https://www.postgresql.org/docs/current/routine-vacuuming.html#AUTOVACUUM). Используется при вычислении истинного времени «заморозки» процесса: на эту величину делится значение параметра [Autovacuum vacuum cost delay](#setting-autovacuum-vacuum-cost-delay).

  Минимальное значение — `-1` (настройка не применяется), максимальное значение — `10000`. Для данной настройки значение по умолчанию зависит от выбранного [класса хостов](../../managed-postgresql/concepts/instance-types.md).
...
- **Max connections**{#setting-max-connections} {{ tag-con }} {{ tag-api }} {{ tag-cli }} {{ tag-tf }}
  ...
  Минимальное значение — `1`, максимальное значение — `262143`. Значение по умолчанию зависит от выбранного [класса хостов](../../managed-postgresql/concepts/instance-types.md) и рассчитывается так: `200 × <количество vCPU на одном хосте>`. Например, для кластера [класса s1.micro](../../managed-postgresql/concepts/instance-types.md) значение настройки по умолчанию равно `400`, и не может быть увеличено.
...
- **Max wal size**{#setting-max-wal-size} {{ tag-all }}
  ...
  Минимальное значение — `2`, максимальное значение — `2251799812636672` (2 ПБ), по умолчанию — `5368709120` (5 ГБ).
...
- **Min wal size**{#setting-min-wal-size} {{ tag-all }}
  ...
  Объем дискового пространства (в байтах), занимаемый [WAL](https://www.postgresql.org/docs/current/wal-intro), при превышении которого старые файлы WAL в контрольных точках удаляются.

  Минимальное значение — `2`, максимальное значение — `2251799812636672` (2 ПБ), по умолчанию — `1073741824` (1 ГБ).
...
- **Shared buffers**{#setting-shared-buffers} {{ tag-con }} {{ tag-api }} {{ tag-cli }} {{ tag-tf }}
  ...
  Минимальное значение — `16`, максимальное значение — `8796093014016` (8 ТБ), по умолчанию — 25% от общего размера RAM хоста кластера {{ mpg-name }}. Настройка зависит от выбранного [класса хостов](../../managed-postgresql/concepts/instance-types.md).
