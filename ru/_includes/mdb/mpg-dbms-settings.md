...
- **Synchronous commit**{#setting-synchronous-commit} {{ tag-all }}
  ...
  - `on` (`SYNCHRONOUS_COMMIT_ON` для Terraform, CLI и API) (по умолчанию) — транзакция подтверждается, если WAL записан на диск мастера и на диск реплики.
  - `local` (`SYNCHRONOUS_COMMIT_LOCAL` для Terraform, CLI и API) — транзакция подтверждается, если WAL записан на диск мастера.
  - `remote_apply` (`SYNCHRONOUS_COMMIT_REMOTE_APPLY` для Terraform, CLI и API) — транзакция подтверждается, если WAL записан на диск мастера, реплика приняла WAL и применила изменения из него.
  - `remote_write` (`SYNCHRONOUS_COMMIT_REMOTE_WRITE` для Terraform, CLI и API) — транзакция подтверждается, если WAL записан на диск мастера, реплика приняла WAL и передала его операционной системе для записи на диск. В случае потери дисковой системы мастера и сбоя операционной системы на реплике данные транзакции с таким уровнем синхронизации могут быть утеряны.
