...
* **{{ ui-key.yacloud.mdb.forms.backup-retain-period }}** — время, в течение которого нужно хранить созданные автоматически резервные копии (в днях). Если для такой копии истекает срок хранения, то она удаляется. Значение по умолчанию — {{ mch-backup-retention }} дней. Подробнее в разделе [Резервные копии](../../../managed-clickhouse/concepts/backup.md#storage).
...
* **{{ ui-key.yacloud.mdb.forms.base_field_service-account }}** — аккаунт, от имени которого пользовательские программы могут управлять кластером. Подробнее в документации [{#T}](../../../managed-clickhouse/security.md).
...
* **{{ ui-key.yacloud.mdb.forms.label_disk-encryption }}** — опция включает шифрование диска [пользовательским ключом KMS](../../../kms/concepts/key.md). Задайте ключ одним из способов:
   ...
   * Чтобы использовать созданный ранее ключ, выберите его в поле **{{ ui-key.yacloud.compute.disk-form.label_disk-kms-key }}**.
   ...
   Подробнее о шифровании дисков в разделе [Хранилище](../../../managed-clickhouse/concepts/storage.md#disk-encryption).
...
* **{{ ui-key.yacloud.mdb.forms.additional-field-serverless }}** — включите эту опцию, чтобы разрешить доступ к кластеру из сервиса [{{ sf-full-name }}](../../../functions/concepts/index.md). Подробнее о настройке доступа в документации [{{ sf-name }}](../../../functions/operations/database-connection.md).
