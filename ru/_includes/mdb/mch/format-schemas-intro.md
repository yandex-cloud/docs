...
Чтобы использовать подключенные схемы формата, вставку данных в {{ mch-name }} следует производить с помощью [HTTP-интерфейса]({{ ch.docs }}{{ lang }}/interfaces/http), т. к. в этом случае сериализация и десериализация данных производятся на стороне сервера с использованием подключенных схем.
