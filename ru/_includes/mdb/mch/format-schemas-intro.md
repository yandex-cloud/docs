...
Чтобы использовать подключенные схемы формата, вставку данных в {{ mch-name }} следует производить с помощью [HTTP-интерфейса]({{ ch.docs }}/interfaces/http/), т. к. в этом случае сериализация и десериализация данных производится на стороне сервера с использованием подключенных схем.

{% endnote %}

Подробнее о форматах данных см. [в документации {{ CH }}]({{ ch.docs }}/interfaces/formats/).
