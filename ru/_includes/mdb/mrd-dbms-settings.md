...
* **Allow data loss**{#settings-allow-data-loss} {{ tag-all }}
    ...
    Если флаг включен, при выходе из строя текущего мастера кластер может переключиться на реплику, которая не содержит последних записей. Это может привести к потере данных. Если большинство хостов находится в одной [зоне доступности](../../overview/concepts/geo-scope.md), то {{ VLK }} продолжит работать, несмотря на отсутствие последних записей.
