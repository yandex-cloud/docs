...
- **Storage**{#setting-storage} {{ tag-con }} {{ tag-cli }} {{ tag-api }}
  ...
  - **Wired tiger**{#setting-wired-tiger}
    ...
    - **Engine config → Cache size gb**{#setting-engine-cache-size}
      ...
      Минимальное значение — `0.25`. Максимальное значение и значение по умолчанию [зависят от выбранного класса хостов](#settings-instance-dependent) и задаются формулами:

      - Максимальное значение — `0,9 × <объем RAM на хосте>`.
      - Значение по умолчанию — `0,5 × <объем RAM на хосте>`.
