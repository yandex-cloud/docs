...
- **Shared buffers**{#setting-shared-buffers} {{ tag-con }} {{ tag-api }} {{ tag-cli }} {{ tag-tf }}
  ...
  Объем памяти (в блоках по 8 КБ), который {{ PG }} может использовать для буферов в разделяемой памяти (shared memory).

  Минимальное значение — `16` (128 КБ), максимальное значение [зависит от выбранного класса хостов](#settings-instance-dependent) и равно 80% от общего размера RAM хоста кластера {{ mpg-name }}. По умолчанию установлено значение 25% от общего размера RAM, но не более 8 ГБ.
