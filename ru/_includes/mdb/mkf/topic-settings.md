...
* **Minimum number of in-sync replicas** {{ tag-all }} {#settings-topic-min-insync-replicas}
    ...
    Минимальное количество синхронизированных реплик, от которых необходимо дождаться подтверждения записи, чтобы запись сообщения в топик считалась успешной.

    Совместное использование параметров `min.insync.replicas` и `acks` (`acks=all` или `acks=-1`) позволяет обеспечить более высокие гарантии сохранности данных.

    Рекомендуемое значение зависит от количества [хостов-брокеров](../../../managed-kafka/concepts/brokers.md):

    * для кластеров с одним хостом-брокером — `1`;
    * для кластеров с тремя и более хостами-брокерами — `2`. Такое значение используется для обеспечения высокой доступности кластера. Подробнее в разделе [{#T}](../../../managed-kafka/concepts/ha-cluster.md#three-or-more-hosts).
...
* **Segment bytes** {{ tag-all }} {#settings-topic-segment-bytes}
    ...
    Минимальное значение для {{ KF }} 3.x — `14` байт, для {{ KF }} 4.x (начиная с версии 4.0) — `1048576` байт (1 МиБ).

    Значение по умолчанию — `1073741824` (1 ГиБ).

    Ограничение проверяется при создании и изменении топика. Если значение меньше минимального, API возвращает ошибку `INVALID_ARGUMENT`.
