...
Порт в system.clusters показывает какой порт используется для межсерверного взаимодействия при выполнении распределенных запросов. В Облаке для этой цели используется только порт 9440 (порт с TLS)
