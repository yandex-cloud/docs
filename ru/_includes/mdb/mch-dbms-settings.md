...
* **Merge tree**{#setting-merge-tree} {{ tag-con }} {{ tag-api }} {{ tag-tf }}
    ...
    * **Max bytes to merge at min space in pool** — максимальный общий размер [кусков данных](https://clickhouse.tech/docs/ru/operations/settings/merge-tree-settings/#max-bytes-to-merge-at-min-space-in-pool) для слияния, когда в фоновом пуле минимум свободных потоков.
