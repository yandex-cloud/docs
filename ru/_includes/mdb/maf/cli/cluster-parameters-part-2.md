...
* `--webserver`, `--scheduler`, `--worker`, `--triggerer`, `--dag-processor` — конфигурация [компонентов](../../../../managed-airflow/concepts/index.md#components) {{ maf-name }}:
    ...
    * `resource-preset-id` — [идентификатор вычислительных ресурсов](../../../../managed-airflow/concepts/instance-types.md) веб-сервера, планировщика, DAG-процессора, воркера и службы Triggerer. Возможные значения:
