- **Conn limit**{#setting-conn-limit} {{ tag-con }} {{ tag-api }} {{ tag-cli }}
  ...
  - Суммарное количество подключений, зарезервированных для пользователей, не должно превышать значение параметра [Max connections](../../managed-postgresql/concepts/settings-list.md#setting-max-connections), который был указан [при создании кластера](../../managed-postgresql/operations/cluster-create.md). При этом нужно учитывать, что {{ mpg-name }} резервирует 15 служебных подключений на каждом хосте. Например, если для кластера задан параметр `"max_connections": 100`, то вы можете зарезервировать не больше 85 подключений на каждый хост кластера для пользователей.
...
- **Login**{#setting-user-login} {{ tag-api }} {{ tag-cli }}
   ...
   Управляет возможностью входа пользователя на кластер {{ PG }}.
