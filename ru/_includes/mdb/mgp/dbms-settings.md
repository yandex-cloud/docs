...
При добавлении столбца в оптимизированную для добавления данных колоночную таблицу (`append-optimized`) с помощью команды `ALTER TABLE`, этот параметр определяет, будут ли параметры сжатия данных таблицы для столбца (`compresstype`, `compresslevel` и `blocksize`) унаследованы от значений таблицы.
...
#|
|| **Доступен в версии** | **Тип** | **Допустимые значения** | **Значение по умолчанию** | **Контекст** ||
|| 6.28 и выше | Boolean | `true` (`on`), `false` (`off`) | Зависит от версии:
* в 6.28 — `false` (`off`);
* в 6.29 и выше — `true` (`on`) | `user` ||
|#
...
Задает объем памяти, который экземпляр мастера использует для буферов общей памяти. Значение этого параметра должно быть не меньше 1 МБ и не более значения следующего выражения (в частности, зависит от [max_connections](#setting-max-connections)):
...
#### track_activity_query_size {#setting-track-activity-query-size}

| Доступен в версии | Тип | Допустимые значения | Значение по умолчанию | Контекст |
|-------------------|---------|---------------------|-----------------------|--------------|
| 6.25 и выше | Integer | от 100 до 102400 | 1024 | `postmaster` |

Определяет объем памяти в байтах, выделяемый для хранения текста SQL-запроса, выполняемого каждой активной сессией. Текст запроса отображается в поле `query` системного представления `pg_stat_activity`. Если текст запроса превышает заданный размер, он обрезается.

Увеличение значения позволяет просматривать более длинные запросы в `pg_stat_activity`, но увеличивает объем общей памяти, резервируемой сервером. Память выделяется отдельно для каждой активной сессии, поэтому при выборе значения учитывайте максимальное количество подключений к кластеру.
