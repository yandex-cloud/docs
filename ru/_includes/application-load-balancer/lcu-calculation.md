...
Для каждой зоны доступности количество ресурсных единиц вычисляется отдельно. Минимальное количество единиц в час для одной зоны — 2. {% if product == "yandex-cloud" %}Работа балансировщика в зонах, где прием трафика отключен, не тарифицируется.{% endif %}
