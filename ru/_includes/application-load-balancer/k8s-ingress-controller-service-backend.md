...
* `port` (`ServiceBackendPort`, обязательное)
  ...
  * `name` (`string`)
    ...
    Имя должно совпадать с одним из имен портов, указанных в полях `spec.ports.name` ресурса `Service`. Подробнее в [спецификации ресурса]({{ configuration-local-link2 }}/service-for-ingress.md).
  ...
  * `number` (`int32`)
    ...
    Номер должен совпадать с одним из номеров портов, указанных в полях `spec.ports.port` ресурса `Service`. Подробнее в [спецификации ресурса]({{ configuration-local-link2 }}/service-for-ingress.md).
