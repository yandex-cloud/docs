...
* `port` (`ServiceBackendPort`, обязательное)
  ...
  * `name` (`string`)
    ...
    Имя должно совпадать с одним из имен портов, указанных в полях `spec.ports.name` ресурса `Service`. Подробнее см. в [спецификации ресурса](../../application-load-balancer/k8s-ref/service.md).
