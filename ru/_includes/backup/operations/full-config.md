...
| `quiesceSnapshottingEnabled` | Использовать режим `quiescing` при создании снапшотов. | <ul><li>`true` — включено;</li><li>`false` — выключено.</li></ul> |
...
- reattempts
  ...
  | `silentModeEnabled` | Режим «Не беспокоить». Сокращает количество взаимодейтсвий с пользователем, когда это возможно. По умолчанию выключено. | <ul><li>`true` — включено;</li><li>`false` — выключено.</li></ul> |
...
- vmSnapshotReattempts
  ...
  Настройка повторения операций создания снапшотов в случае сбоев.
  
  | Атрибут | Описание | Возможные значения |
  |---|---|---|
  | `vmSnapshotReattempts.enabled` | Повторять попытки создать снапшот ВМ при возникновении ошибок. | <ul><li>`true` — включено;</li><li>`false` — выключено.</li></ul> |
...
- retention
  ...
  | `retention.afterBackup` | Применять правила хранения резервных копий после завершения операции резервного копирования. | <ul><li>`true` — включено;</li><li>`false` — выключено.</li></ul> |
