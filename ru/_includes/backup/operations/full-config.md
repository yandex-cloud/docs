...
- prePostCommands
  ...
  | `type` | Момент выполнения команды (запуска файла). | <ul><li>`COMMAND_TYPE_UNSPECIFIED` — не задано;</li><li>`PRE_COMMAND` — выполнять команду (запускать файл) перед созданием резервной копии;</li><li>`POST_COMMAND` — выполнять команду (запускать файл) после завершения создания резервной копии.</li></ul>В каждую политику вы можете добавить не более одной команды типа `PRE_COMMAND` и не более одной — типа `POST_COMMAND`. |
