...
1. Подключитесь к ВМ:
    ...
    * `-p` — порт подключения, по умолчанию `22`. Если используете другой порт, убедитесь, что он указан в `/etc/ssh/sshd_config` на ВМ и группа безопасности разрешает входящий трафик. Необязательный параметр.
