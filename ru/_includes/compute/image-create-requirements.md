...
* Установлены драйверы {% if product == "yandex-cloud" %}`virtio-net`, `virtio-blk` и `virtio-pci`. Если вы собираетесь подключать к ВМ [файловые хранилища](../../compute/concepts/filesystem.md), также должен быть установлен драйвер `virtiofs`.{% endif %}{% if product == "cloud-il" %}`virtio-net`, `virtio-blk`, `virtio-pci` и `virtiofs`.{% endif %} См. [инструкцию](../../compute/operations/image-create/custom-image.md#virtio).
