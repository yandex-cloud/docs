...
Количество оптимизированных для развертывания образов в одном облаке^1^ | 0
Количество групп виртуальных машин в одном облаке | 10
Суммарное количество GPU для всех виртуальных машин в одном облаке^1^ | 0
Количество одновременно выполняемых [операций](../api-design-guide/concepts/operation.md) в одном каталоге | 15
Максимальное количество [групп размещения](../compute/concepts/placement-groups.md) в одном облаке | 2

{% endif %}

^1^ Чтобы увеличить [квоты]({{ link-console-quotas }}) на {% if product == "yandex-cloud" %}файловые хранилища, {% endif %}виртуальные машины с GPU, оптимизированные для развертывания образы{% if product == "yandex-cloud" %} или выделенные хосты{% endif %}, обратитесь в [техническую поддержку]({{ link-console-support }}).
...
- Intel Ice Lake
  ...
  Максимальное количество дисков{% if product == "yandex-cloud" %} и файловых хранилищ{% endif %}, подключенных к одной виртуальной машине^2^ | Меньше или равно 32 vCPU — 8<br>Больше> 32 vCPU — 16^3^
  Максимальное количество групп безопасности на один интерфейс | 5
  Максимальное количество ВМ в одной [группе размещения](../compute/concepts/placement-groups.md) | 5
...
#### Лимиты виртуальных машин на работу с сетевыми дисками {#compute-limits-vm-disks}

Лимиты на работу с сетевыми дисками зависят от [платформы](../compute/concepts/vm-platforms.md) виртуальной машины:
...
- Intel Broadwell
  ...
  Вид ограничения | Значение
  --- | ---
  Максимальный^4^ [IOPS](../compute/concepts/storage-read-write.md) для 1 vCPU | 10000
  Максимальная^5^ [пропускная способность](../compute/concepts/storage-read-write.md) для 1 vCPU | 125 МБ/с

- Intel Cascade Lake

  Вид ограничения | Значение
  --- | ---
  Максимальный^4^ [IOPS](../compute/concepts/storage-read-write.md) для 1 vCPU | 6000
  Максимальная^5^ [пропускная способность](../compute/concepts/storage-read-write.md) для 1 vCPU | 75 МБ/с
...
#### Лимиты дисков{% if product == "yandex-cloud" %} и файловых хранилищ{% endif %} {#compute-limits-disks}
