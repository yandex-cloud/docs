...
Например, очереди сообщений используют [поисковые роботы]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/Поисковый_робот){% endif %}{% if lang == "en" %}(https://en.wikipedia.org/wiki/Web_crawler){% endif %}. Обходчик интернета находит URL страниц и складывает их в очередь. Процессы-парсеры получают эти URL, скачивают страницы и выполняют обработку. Очередь сообщений выступает как очередь задач, повышая отказоустойчивость и масштабируемость такой системы.
