...
1. Если у вас еще нет OAuth-токена для работы с {{ tracker-name }} API, [получите его](https://yandex.ru/support/tracker/{{ lang }}/api/access#about_OAuth).

    {% note warning %}

    Для работы с {{ tracker-name }} API у пользователей должны быть соответствующие права в Трекере. Чтобы настроить интеграцию, убедитесь, что у вас есть права на изменение настроек очереди. Подробнее о правах доступа читайте в разделе [Роли и права доступа](https://yandex.ru/support/tracker/{{ lang }}/role-model).

    {% endnote %}

1. Проверьте наличие доступа к API с помощью [запроса информации о текущем пользователе](https://yandex.ru/support/tracker/{{ lang }}/api/users/get-user-info).
