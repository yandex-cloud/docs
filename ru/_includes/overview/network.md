...
* [Виртуальная сеть](#overlay) — работает поверх сетевой физической инфраструктуры. Сервисы виртуальной сети [Virtual Private Cloud (VPC)](../../vpc/concepts/) предоставляют пользователю:
  * IP-связность между облачными ресурсами.
  * Доступ облачных ресурсов к интернету.
  * Набор [сетевых сервисов](../../vpc/concepts/) для использования в составе развертываемых решений.
...
* [Точки присутствия](../../interconnect/concepts/pops.md) (Point of Presence или PoP). На точках присутствия размещается сетевое оборудование транспортной сети. На отдельных точках присутствия организуются подключения к внешним сетям и интернету (*Internet Peering*). На отдельных точках присутствия клиентам {{ yandex-cloud }} предоставляется возможность организовать подключение к [своим облачным ресурсам](../../interconnect/concepts/priv-con.md) и [публичным сервисам](../../interconnect/concepts/pub-con#svc-list) {{ yandex-cloud }} с помощью услуги [{{ interconnect-full-name }}](../../interconnect/concepts/).

Транспортная сеть {{ yandex-cloud }} состоит из двух уровней:

* Уровень оптической сети DWDM. Оборудование оптического спектрального уплотнения ([DWDM](https://en.wikipedia.org/wiki/Wavelength-division_multiplexing#DWDM_systems)) обеспечивает организацию логических каналов связи (L2) для подключения оборудования уровня пакетной сети IP/MPLS. С помощью DWDM-оборудования транспортная сеть может легко наращивать свою емкость (полосу пропускания) до десятков терабит.
* Уровень пакетной сети IP/MPLS. Обеспечивает IPv6- и IPv4-связность между зонами доступности и точками присутствия (Underlay и Overlay), а также обеспечивает сетевой транспорт для работы всех сервисов {{ yandex-cloud }}.
...
#### Трафик между ВМ в одном ЦОД {#between-vm-in-one-az}

Трафик от ВМ `VM-A1` до ВМ `VM-A2` в ЦОД-А будет передаваться по пути:

1. `VM-A1` → `VRouter на Server-A1`.
1. `Server-A1` → `Server-A2` (по внутренней сети ЦОД).
1. `VRouter на Server-A2` → `VM-A2`.

#### Трафик между ВМ в разных ЦОД {#between-vm-in-different-az}

Трафик от ВМ `VM-A2` в ЦОД-А до ВМ `VM-B1` в ЦОД-B будет передаваться по пути:

1. `VM-A2` → `VRouter на Server-A2`.
1. `Server-A2` → Граничное сетевое оборудование транспортной сети ЦОД-А.
1. Граничное сетевое оборудование транспортной сети ЦОД-А → граничное сетевое оборудование транспортной сети ЦОД-B.
1. Граничное сетевое оборудование ЦОД-B → `Server-B1`.
...
1. `VM-A1` → `VRouter на Server-A1`.
1. `Server-A1` → функция `NAT-GW` компонента CloudGate (по внутренней сети внутри ЦОД-A).
1. `NAT-GW` → граничное сетевое оборудование в ЦОД-A.
1. Граничное сетевое оборудование в ЦОД-A → сетевое оборудования на точке присутствия, где организовано подключение к внешним сетям и интернету.
