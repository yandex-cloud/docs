...
При создании {{ datalens-short-name }} было решено в операциях над полями `Дата и время` приводить их к [UTC]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/Всемирное_координированное_время){% else %}(https://en.wikipedia.org/wiki/Coordinated_Universal_Time){% endif %}).
