...
Поскольку проекты запускаются на разных платформах {{ yandex-cloud }} с разными вычислительными мощностями, {{ ml-platform-name }} не гарантирует доступность 100% объема оперативной памяти. Если вам нужна конфигурация с фиксированным объемом RAM, выбирайте платформу, для которой указано точное значение.
