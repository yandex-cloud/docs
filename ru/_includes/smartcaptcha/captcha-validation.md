...
* Токен можно использовать только один раз для запроса результата проверки. При повторной попытке валидировать тот же токен вы получите ответ `"status": "failed"` с сообщением `Token invalid or expired`.
