...
На сервере OpenVPN заранее создан пользователь `openvpn` с административными правами. Пароль генерируется автоматически. Чтобы получить доступ к административной панели:
...
1. Выполните команду: 
   ...
   ```
   На экране отобразится пароль, который находится в текстовом файле.

   Для входа в административную панель используйте логин `openvpn` и полученный пароль.
...
1. Введите имя пользователя `openvpn` и полученный на предыдущем шаге пароль.
