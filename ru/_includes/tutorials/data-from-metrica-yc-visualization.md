...
1. [Подключите {{ datalens-short-name }} и создайте чарты](#datalens-connection-chart-creation)
    ...
    1. [Создайте чарт — столбчатая диаграмма](#creating-column-chart)
...
1. [Воронки](#funnels)
    ...
    1. [{{ datalens-short-name }}. Воронки по браузерам. Добавьте чарт на дашборд](#settiting-browser-funnels-chart-on-dashboard)
    1. [{{ datalens-short-name }}. Воронки по браузерам. Настройте дашборд](#settiting-browser-funnels-chart-on-dashboard)
1. [Когорты](#cohorts)
    1. [{{ ml-platform-short-name }}. Проведите Когортный анализ](#cohort-analysis)
    1. [{{ datalens-short-name }}. Создайте датасет и чарта с визуализацией когорт](#creating-dataset-chart-with-cohort)
...
### 3.4 Создайте чарт — столбчатая диаграмма {#creating-column-chart}
...
1. Добавьте или скорректируйте следующие поля в области чарта:
    * Удалите **EventDate** из области **X**, перетащив его обратно в список полей.
...
###  5.5 {{ datalens-short-name }}. Воронки по браузерам. Добавьте чарт на дашборд {#settiting-browser-funnels-chart-on-dashboard}
...
### 5.6 {{ datalens-short-name }}. Воронки по браузерам. Настройте дашборд {#settiting-browser-funnels-chart-on-dashboard}
...
### 6.2 {{ datalens-short-name }}. Создайте датасет и чарта с визуализацией когорт {#creating-dataset-chart-with-cohort}
...
1. Для этого в области **Столбцы** в поле week_num нажмите значок решетки. Отформатируйте числа в значениях поля **week_num** – уберите знаки после запятой. В открывшемся окне выполните настройку:
...
1. Чтобы сделать таблицу цветной, добавьте поле **Визиты** в область **Цвета** и нажмите значок шестиренки. В открывшемся окне выполните настройку цвета:
...
### 6.4 {{ datalens-short-name }}. Добавьте чарты на новую вкладку дашборда {#adding-charts-to-dashboard-tab}
...
### 6.5 {{ datalens-short-name }}. Создайте чарты {#creating-chart}
...
### 6.6 {{ datalens-short-name }}. Добавьте чарты на дашборд {#adding-chart-to-dashboard}
