...
[1С-Битрикс: Управление сайтом](https://ru.wikipedia.org/wiki/1%D0%A1-%D0%91%D0%B8%D1%82%D1%80%D0%B8%D0%BA%D1%81:_%D0%A3%D0%BF%D1%80%D0%B0%D0%B2%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5_%D1%81%D0%B0%D0%B9%D1%82%D0%BE%D0%BC) это система управления контентом (CMS), с помощью которой вы можете создать интернет-магазин, корпоративный сайт или новостной портал и достаточно просто управлять структурой и содержимым вашего сайта.

В этой инструкции вы развернете и настроите интернет-магазин на платформе 1С-Битрикс. В процессе настройки вы создадите виртуальную машину в {{ yandex-cloud }}, развернете на ней 1С-Битрикс и запустите необходимые сервисы. В качестве базы данных вы будете использовать отказоустойчивый кластер {{ mmy-name }}.
...
1. В блоке **Образы из {{ marketplace-name }}** выберите образ операционной системы [Ubuntu 18.04 lts](https://cloud.yandex.ru/marketplace/products/f2e9qa7i4fmugh14tjnc).
...
1. В блоке **Доступ** укажите данные для доступа к виртуальной машине:
   ...
   - В поле **SSH-ключ** скопируйте значение вашего открытого SSH-ключа. Пару ключей для подключения по SSH необходимо [создать самостоятельно](../../../compute/operations/vm-connect/ssh.md).
...
## Настройте виртуальную машину для работы с 1C-Битрикс {#configure-server}

Чтобы настроить ВМ для работы с 1С-Битрикс:

1. Войдите на виртуальную машину по SSH (`ubuntu` — имя пользователя, которое вы задали при [создании ВМ](#create-vm)):
   ...
   1. Нажмите на имя вашей виртуальной машины (в примере — `bitrix`).
...
1. Скачайте дистрибутив <q>1С-Битрикс: Управление сайтом</q>:
...
1. Откройте файл конфигурации `/etc/php/7.4/apache2/php.ini` и укажите необходимые параметры PHP.
   ...
   В зависимости от версии <q>1С-Битрикс</q> путь к файлу конфигурации может отличаться. Укажите путь до актуальной версии PHP на виртуальной машине.
...
1. Откройте веб-интерфейс <q>1С-Битрикс: Управление сайтом</q>. Для этого в браузере перейдите по адресу `http://<публичный-IP-адрес-ВМ>/`. Откроется страница с предложением установить 1С-Битрикс.
...
1. Настройка базы данных.
   ...
   1. В полях **Имя пользователя** и **Пароль** укажите данные, которые вы указывали при создании базы данных.
   1. В поле **Имя базы данных** укажите имя базы данных (в примере `db1`).
...
Чтобы получать резервные копии системы, периодически [создавайте снимки диска](../../../compute/operations/disk-control/create-snapshot.md) виртуальной машины.
