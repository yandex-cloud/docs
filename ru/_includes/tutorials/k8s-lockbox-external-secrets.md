[External Secrets Operator](https://external-secrets.io/provider-yandex-lockbox/) позволяет настроить синхронизацию секретов [{{ lockbox-full-name }}](../../lockbox/) с секретами кластера {{ managed-k8s-name }}.
...
1. Создайте хранилище секретов [SecretStore](https://external-secrets.io/api-secretstore/), содержащее секрет `yc-auth`:
...
1. Создайте объект [ExternalSecret](https://external-secrets.io/api-externalsecret/), указывающий на секрет `lockbox-secret` в хранилище `secret-store`:
