В инструкции ниже описана настройка внешнего DNS-сервиса на примере [reg.ru]{% if region == "int" %}(https://www.reg.com/){% else %}(https://www.reg.ru/){% endif %} для доменных имен `www.example.com` и `example.com`:
...
1. Войдите в панель управления внешнего DNS-сервиса. Перейдите в [список ваших доменов]{% if region == "int" %}(https://www.reg.com/user/domain_list){% else %}(https://www.reg.ru/user/domain_list){% endif %} и нажмите на имя нужного домена.
