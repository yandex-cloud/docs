В инструкции ниже описана настройка внешнего DNS-сервиса на примере [reg.ru]{% if lang == "ru" %}(https://www.reg.ru/){% endif %}{% if lang == "en" %}(https://www.reg.com/){% endif %} для доменных имен `www.example.com` и `example.com`:
...
1. Войдите в панель управления внешнего DNS-сервиса. Перейдите в [список ваших доменов]{% if lang == "ru" %}(https://www.reg.ru/user/domain_list){% endif %}{% if lang == "en" %}(https://www.reg.com/user/domain_list){% endif %} и нажмите на имя нужного домена.
