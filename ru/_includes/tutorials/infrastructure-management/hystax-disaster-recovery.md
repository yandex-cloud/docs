...
* плата за каждую защищаемую ВМ (см. [описание продукта]{% if lang == "ru" %}(https://cloud.yandex.ru/marketplace/products/hystax/hystax-acura-disaster-recovery-3-7){% endif %}{% if lang == "en" %}(https://cloud.yandex.com/en/marketplace/products/hystax/hystax-acura-disaster-recovery-3-7){% endif %} в {{ marketplace-name }}).
...
- Консоль управления
   ...
   1. В блоке **Сетевые настройки**:
      ...
      * Если доступен список **Группы безопасности**, выберите [группу безопасности](../../../vpc/concepts/security-groups.md#default-security-group), для которой ранее настраивали разрешения сетевого трафика. Если такого списка нет, для виртуальной машины будет разрешен любой входящий и исходящий трафик.
...
- CLI
   ...
   * `name` — имя виртуальной машины, например, `hystax-acura-vm`.

   * `zone` — [зона доступности](../../../overview/concepts/geo-scope.md), например, `ru-central1-b`.

   * `cores` — [количество vCPU](../../../compute/concepts/vm.md) виртуальной машины.

   * `memory` — [размер оперативной памяти](../../../compute/concepts/vm.md) виртуальной машины.
   ...
   * `create-boot-disk` — создать для ВМ новый диск:
      ...
      * `image-id` — идентификатор образа диска.
        ...
        В данном случае используйте `image_id` из [описания продукта]{% if lang == "ru" %}(https://cloud.yandex.ru/marketplace/products/hystax/hystax-acura-disaster-recovery-3-7){% endif %}{% if lang == "en" %}(https://cloud.yandex.com/en/marketplace/products/hystax/hystax-acura-disaster-recovery-3-7){% endif %} в {{ marketplace-name }}.
...
Виртуальные машины создаются с динамическим публичным IP-адресом. Так как ВМ с Hystax Acura может быть перезагружена, сделайте IP-адрес статическим.
...
Подробнее см. в разделе [Пошаговые инструкции](../../../vpc/operations/subnet-create) документации {{ vpc-short-name}}.


## Создайте план аварийного восстановления {#disaster-recovery-plan}

План аварийного восстановления (DR plan) содержит описание виртуальных машин и сетевые настройки. План можно сгенерировать автоматически или создать вручную.
...
- Вручную
   ...
   1. Добавьте виртуальные машины с помощью кнопки ![image](../../../_assets/options.svg). При необходимости укажите порядок их инициализации с помощью опции **Move to another Rank**.
