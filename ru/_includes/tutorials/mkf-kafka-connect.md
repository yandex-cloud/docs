Примечание!
С недавнего времени в {{ mkf-name }} поддерживаются Managed-коннекторы.
Если нужный коннектор на данный момент есть в списке поддерживаемых, 
его можно настроить используя различные интерфейсы(Консоль, CLI, Terraform).
См. в разделе ["коннекторы"](https://cloud.yandex.ru/docs/managed-kafka/concepts/connectors) 
