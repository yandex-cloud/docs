...
1. [Подключитесь к виртуальной машине](#connect-vm).
...
Сделайте бэкап файлов сайта и базы данных одним из способов:

* С помощью различных плагинов для WordPress (например, [BackWPup]({% if lang == "ru" %} https://ru.wordpress.org/plugins/backwpup {% endif %} {% if lang == "en" %} https://wordpress.org/plugins/backwpup {% endif %}) или [Updraft Plus]({% if lang == "ru" %} https://ru.wordpress.org/plugins/updraftplus {% endif %} {% if lang == "en" %} https://wordpress.org/plugins/updraftplus {% endif %})).
* С помощью встроенных средств в панели управления хостингом, на котором располагается ваш сайт.
* Скопировать все файлы на свой жесткий диск через FTP-клиент, а базу данных экспортировать через панель [phpMyAdmin](https://www.phpmyadmin.net/). 
...
* плата за публичные DNS-запросы и зоны (см. [тарифы {{ dns-full-name }}](https://cloud.yandex.ru/docs/dns/pricing)).
...
1. В поле **Имя** введите имя ВМ — `wordpress-vm`.
...
1. В блоке **Выбор образа/загрузочного диска** перейдите на вкладку **Cloud Marketplace** и нажмите **Посмотреть больше**. Выберите [LAMP]({% if lang == "ru" %} https://cloud.yandex.ru/marketplace/products/yc/lamp {% endif %} {% if lang == "en" %}https://cloud.yandex.com/en-ru/marketplace/products/yc/lamp{% endif %})-образ, который содержит необходимый набор компонентов: операционную систему семейства Linux, веб-сервер Apache, СУБД MySQL и интерпретатор PHP.

1. В блоке **Диски** выберите тип диска и установите нужный размер.
...
## Подключитесь к виртуальной машине {#connect-vm}
...
Для подключения к виртуальной машине необходимо указать ее публичный адрес. 

Чтобы скопировать публичный адрес ВМ:
...
1. Найдите созданную виртуальную машину и нажмите на ее имя.

1. В разделе **Сеть** скопируйте адрес из поля **Публичный IPv4**.

Выполните [подлючение к виртуальной машине](../../../compute/operations/vm-connect/ssh.md).
...
* Установить и настроить `phpMyAdmin` для работы с базой данных сайта.
...
### Установите phpMyAdmin {#install-phpmyadmin}
   ...
   - Ubuntu
	 ...
	 1. Настройте доступ `phpMyAdmin` к базе данных. Для этого на вопрос `Configure database for phpmyadmin with dbconfig-common?` выберите `<yes>`. Ввод пароля можно пропустить, система автоматически сгенерирует его. Чтобы скопировать сгенерированный пароль, выполните команду:
	 ...
	 1. Откройте `phpMyAdmin` через браузер. В адресной строке введите `http://<публичный адрес виртуальной машины>/phpmyadmin`.
...
## Импортируйте базу данных {#import-db}
   ...
   - Ubuntu
     ...
     1. Зайдите в phpMyAdmin — введите в адресной строке браузера:
        ...
        ```
        http://<публичный адрес виртуальной машины>/phpmyadmin`
        ```  
     ...
     1. Импортируйте базу данных из бэкапа:

        * в левой колонке выберите созданную базу данных;
        * нажмите **Импорт** и выберите сохраненный бэкап базы данных. Если бэкап базы данных превышает установленный по умолчанию лимит 2 Мб, отредактируйте файл конфигурации `php.ini`, где задано данное ограничение.
	    ...
	    1. Повторите операцию импорта базы данных.	 
	
   {% endlist %}   

### Отключите phpMyAdmin {#disable-phpmyadmin}

Все основные действия по переносу базы данных выполнены. Чтобы не подвергать `phpMyAdmin` атакам извне, отключите утилиту:
...
Для работы с базой данных вы можете использовать терминал.

## Перенесите файлы сайта {#transfer-files}

Чтобы перенести файлы бэкапа на виртуальную машину, воспользуйтесь FTP-клиентом [FileZilla](https://filezilla-project.org/):

1. Откройте FileZilla.

1. Перейдите в раздел **Файл** → **Менеджер сайтов** и добавьте новый сайт. 

1. Выберите протокол SFTP, введите публичный IP-адрес виртуальной машины. В разделе **Тип входа** выберите **Файл с ключом**. 

1. Укажите пользователя, созданного при настройке виртуальной машины, и выберите файл ключа — ключ находится в папке `/Users/<имя_пользователя>/.ssh/`.

   FTP-клиент не видит скрытую папку по умолчанию. Нажмите сочетание клавиш `CMD+Shift+G` и выберите файл `id_rsa` без расширения `pub`. Будет выведено сообщение, что файл не поддерживается. FTP-клиент предложит переконвертировать его в нужный формат. Нажмите **Да**. Сохраните переконвертированный файл в удобном вам месте. 

1. Нажмите **Подключиться** и введите кодовую фразу, которую сформировали в начале работы. Вы подключитесь к виртуальной машине.

1. Найдите папку `/var/www/html` на виртуальной машине и скопируйте в нее файл бэкапа. Файл index.html копировать не нужно.   
...
У сайтов на WordPress иногда возникает проблема открытия внутренних ссылок при переходе от одного хостинга к другому.

В этом случае, проверьте существует ли файл `.htaccess` в корневой папке вашего сайта  — `var/www/html/.htaccess`.
