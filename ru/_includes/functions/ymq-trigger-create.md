...
О том, как создать триггер для {{ message-queue-short-name}}, который будет вызывать контейнер, читайте в [документации {{ serverless-containers-full-name }}](../../serverless-containers/operations/ymq-trigger-create.md).
