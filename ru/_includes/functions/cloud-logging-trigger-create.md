Создайте триггер для [{{ cloud-logging-name }}](../../logging/), который будет вызывать [функцию](../../functions/concepts/function.md) {{ sf-name }} при добавлении записей в [лог-группу](../../logging/concepts/log-group.md).

О том, как создать триггер для {{ cloud-logging-name }}, который будет вызывать контейнер, читайте в [документации {{ serverless-containers-full-name }}](../../serverless-containers/operations/cloud-logging-trigger-create.md).
