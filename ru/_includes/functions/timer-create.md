Создайте [таймер](../../functions/concepts/trigger/timer.md) — триггер, который будет вызывать [функцию](../../functions/concepts/function.md) {{ sf-name }} по расписанию.

О том, как создать таймер, который будет вызывать контейнер, читайте в [документации {{ serverless-containers-full-name }}](../../serverless-containers/operations/timer-create.md).
