{% if audience == "external" %} Создайте триггер для [{{ yds-name }}](../../data-streams/), который будет вызывать [функцию](../../functions/concepts/function.md) {{ sf-name }} при отправке данных в [поток](../../data-streams/concepts/glossary.md#stream-concepts).

О том, как создать триггер для {{ yds-name }}, который будет вызывать контейнер, читайте в [документации {{ serverless-containers-full-name }}](../../serverless-containers/operations/data-streams-trigger-create.md). {% endif %}
