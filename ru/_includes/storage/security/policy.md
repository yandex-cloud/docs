...
#### Правило, которое разрешает скачивать объекты только из указанного диапазона IP-адресов: {#download-specific-ip-range}
  ...
  [Условие](../../../storage/s3/api-ref/policy/conditions.md#keys) `yc:OriginIp` проверяет только исходный IP-адрес без учета прокси. Если нужно проверять также IP-адреса обратных прокси-серверов, например, переданных в заголовке `X-Forwarded-For`, используйте условие `aws:SourceIp`.
