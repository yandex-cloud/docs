## Организация приватного соединения {#prc-create}
...
### Обращение в поддержку для организации нового приватного соединения {#prc-ticket}
...
Текст обращения:
Прошу добавить новое приватное соединение в уже существующий транк. Параметры соединения следующие:
...
* `yandex_cloud_ip` — IP адрес из стыковой (пиринговой) подсети на оборудовании Yandex Cloud. Выбирается клиентом.
* `peer_bgp_asn` — номер [BGP ASN](../../interconnect/concepts/priv-con.md#bgp-asn) на оборудовании клиента. Выбирается клиентом.
* `cloud_id`, `folder_id`, `vpc_net_id` - расположение VPC с ресурсами для подключения в рамках данного приватного соединения.
...
### Контроль состояния приватного соединения {#prc-check}
