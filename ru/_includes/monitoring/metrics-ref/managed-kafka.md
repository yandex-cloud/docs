## Сервис {{ managed-kafka-full-name }} {#managed-kafka}

Общие метки для всех метрик сервиса {{ managed-kafka-full-name }}: 
...
Подробнее о сервисе в документации [{{ managed-kafka-full-name }}](../../managed-kafka/).
