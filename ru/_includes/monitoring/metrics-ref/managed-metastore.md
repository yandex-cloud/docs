...
Общие метки для всех метрик сервиса {{ metastore-name }}:
...
Дополнительная метка: `method` — название метода API {{ metastore-name }}. Может принимать следующие значения:
* `total` — все методы.
...
* `method` — название метода API {{ metastore-name }}. Может принимать следующие значения:
  * `total` — все методы.
  * `init` — данные не экспортируются.
...
`COUNTER`, штуки | Количество ошибок при выполнении DirectSQL-запросов, после которых {{ metastore-name }} перешел к выполнению запросов через JDO. ||
...
`COUNTER`, штуки | Количество операций создания и удаления объектов {{ metastore-name }}.
Дополнительные метки:
...
`DGAUGE`, штуки | Количество активных подключений к кластеру {{ metastore-name }} по протоколу Thrift. ||
...
`DGAUGE`, штуки | Количество баз данных, таблиц и партиций в хранилище кластера {{ metastore-name }}.
...
Метрики пула JDBC-подключений доступны в {{ metastore-name }} начиная с версии `4.2`.
