## Сервис {{ managed-redis-full-name }} {#managed-redis}

Общие метки для всех метрик сервиса {{ managed-redis-full-name }}: 
...
| `redis_hit_rate`<br/>`DGAUGE`, % | Доля запросов, данные для которых {{ managed-redis-full-name }} находит в кеше. | 
