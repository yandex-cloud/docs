Подготовьте ключи для использования с вашими виртуальными машинами. Для этого:
...
1. Создайте новый ключ с помощью команды `ssh-keygen`. Выполните команду:
   ...
   Публичная часть ключа будет сохранена в файле с названием `<имя_ключа>.pub`. Откройте файл с помощью Блокнота или другого текстового редактора и скопируйте строку ключа в поле для публичного ключа при создании новой виртуальной машины через консоль управления.
...
1. В качестве типа генерируемой пары выберите **RSA** и укажите длину `2048`. Нажмите **Generate** и поводите мышкой в поле выше до тех пор, пока не закончится создание ключа.

   ![ssh_generate_key](../compute/_assets/ssh-putty/ssh_generate_key.png)

1. В поле **Key passphrase** введите надёжный пароль. Повторно введите его в поле ниже.
...
1. Сохраните открытый ключ из текстового поля в текстовый файл с названием `id_rsa.pub`. Ключ в этом файле будет сохранен одной строкой. Откройте файл с помощью Notepad или другого текстового редактора и скопируйте строку ключа в поле для публичного ключа при создании новой виртуальной машины через консоль управления.
