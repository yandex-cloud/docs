...
- Linux/MacOS
  ...
  1. Создайте новый ключ с помощью команды `ssh-keygen`:
     ...
     Открытый ключ будет сохранен в файле `<имя_ключа>.pub`. Вставьте содержимое файла в поле **SSH-ключ** при создании новой виртуальной машины через консоль управления.
...
- Windows 10
  ...
  1. Создайте новый ключ с помощью команды `ssh-keygen`. Выполните команду:
   ...
   1. При создании виртуальной машины через консоль управления укажите открытый ключ. Для этого откройте файл `<имя_ключа>.pub` с помощью Notepad и скопируйте значение ключа в поле **SSH-ключ**. Обратите внимание, что ключ должен быть записан **одной строкой** (не должно быть переносов и разрывов строки).
