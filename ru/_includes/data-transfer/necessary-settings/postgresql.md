...
* Пользовательская инсталляция
    ...
    * **IP или FQDN хоста** — укажите IP-адрес или FQDN хоста-мастера.
    * **Порт базы данных** — укажите номер порта, который сервис {{ data-transfer-name }} будет использовать для подключения.
    * **Сертификат CA** — если требуется шифрование передаваемых данных, например для соответствия требованиям [PCI DSS]({% if lang == "ru" %}{{ link-pci-dss-ru }}{% endif %}{% if lang =="en" %}{{ link-pci-dss-en }}{% endif %}), загрузите файл [сертификата](../../../managed-postgresql/operations/connect.md#get-ssl-cert) или добавьте его содержимое в текстовом виде.
