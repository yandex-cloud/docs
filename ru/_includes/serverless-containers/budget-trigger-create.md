Создайте триггер для [бюджетов](../../billing/concepts/budget.md), который будет вызывать [контейнер](../../serverless-containers/concepts/container.md) {{ serverless-containers-name }} при превышении пороговых значений.

О том, как создать триггер для бюджетов, который будет вызывать функцию, читайте в [документации {{ sf-name }}](../../functions/operations/trigger/cr-trigger-create.md).
