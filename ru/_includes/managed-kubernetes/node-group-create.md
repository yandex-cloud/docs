...
1. В блоке **{{ ui-key.yacloud.k8s.node-groups.create.section_disk }}**:
   * Укажите **{{ ui-key.yacloud.k8s.node-groups.create.field_disk-type }}** узла {{ managed-k8s-name }}:
     ...
     * `{{ ui-key.yacloud.k8s.node-groups.create.value_network-ssd-io-m3 }}` — сетевой диск с повышенной производительностью, обеспечивает избыточность.
     * `{{ ui-key.yacloud.k8s.node-groups.create.value_network-ssd-nonreplicated }}` — сетевой диск с повышенной производительностью. Размер такого диска можно менять только с шагом 93 ГБ.
