...
* `metadata` (`ObjectMeta`, обязательное)
  ...
  * `name` (`string`, обязательное)
    ...
    Это имя нужно указать в поле `spec.rules.http.paths.backend.resource.name` [ресурса `Ingress`]({{ configuration-local-link }}/ingress.md).
...
* `spec` (`HttpBackendGroupSpec`)
  ...
  * `backends` (`[]HttpBackend`)
    ...
    * `storageBucket` (`StorageBucketBackend`)
      ...
      Указание на [бакет {{ objstorage-full-name }}](../../../storage/concepts/bucket.md), который должен обрабатывать запросы в качестве бэкенда. Использование бакета в качестве бэкенда описано в разделе [{#T}](../../../application-load-balancer/concepts/backend-group.md#types).
