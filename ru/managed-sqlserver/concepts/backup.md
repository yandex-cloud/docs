...
description: "{{ mms-short-name }} обеспечивает автоматическое и ручное резервное копирование баз данных {{ MS }} . Резервная копия автоматически создается раз в день. Время начала резервного копирования задается при создании или изменении кластера СУБД MS {{ MS }}."
keywords:
  - бекап
  - backup
...
Сервис поддерживает экспорт резервных копий баз данных в бакеты [{{ objstorage-name }}](../../storage/), а также восстановление баз данных из них. В обоих случаях к кластеру должен быть привязан [сервисный аккаунт](../../iam/concepts/users/service-accounts) с правами на доступ к бакету.
