...
{{ mms-short-name }} доступен всем зарегистрированным пользователям Яндекс.Облака.
...
Для хранилища на локальных SSD-дисках количество хостов ограничено при создании: для кластеров {{ MS }} можно создать не меньше 3 хостов.
...
{{ mms-short-name }} поддерживает {{ MS }} TODO: какие версии?.
