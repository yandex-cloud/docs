...
Пользователь, который создается вместе с кластером **{{ mms-name }}**, автоматически получает роль владельца (`DB_OWNER`) первой базы данных в кластере. После этого вы можете создавать других пользователей и настраивать их права по своему усмотрению.
...
Чтобы назначить **привилегию** определенному пользователю, [создайте](cluster-users.md#adduser) его с помощью консоли управления или API Яндекс.Облака, затем выполните команду `GRANT` от имени владельца базы, например:
...
Чтобы назначить **роль** пользователю, используйте консоль управления или API Яндекс.Облака: назначение роли запросом `GRANT` отменяется при следующей операции с базой.
