...
Чтобы назначить **привилегию** определенному пользователю, [создайте](cluster-users.md#adduser) его с помощью консоли управления или API {{ yandex-cloud }}, затем выполните команду `GRANT` от имени владельца базы, например:
...
Чтобы назначить **роль** пользователю, используйте консоль управления или API {{ yandex-cloud }}: назначение роли запросом `GRANT` отменяется при следующей операции с базой.
