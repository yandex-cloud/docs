...
* **Transactions/sec on primary** — среднее количество транзакций, выполненных на хосте-мастере за одну секунду.

* **Disk capacity on primary, [bytes]** — емкость диска на хосте-мастере (в байтах):

    * **Used Space** — использованное дисковое пространство.
    * **Available Space** — свободное дисковое пространство.

* **Is Primary [bool]** — какой хост является мастером и как долго.

* **Lazy Writes/sec on primary** — скорость физической записи на хосте-мастере. {{ MS }} разделяет запись данных для повышения эффективности работы с хранилищем:
