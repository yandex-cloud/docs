...
{% endif %}
  ...
  Чтобы получить список резервных копий всех кластеров **{{ mms-name }}** в каталоге:
  1. Перейдите на [страницу каталога]({{ link-console-main }}) и выберите сервис **{{ mms-name }}**.
...
1. Подготовьте инфраструктуру:
    ...
    1. Выдайте сервисному аккаунту права на чтение и запись в бакет одним из способов:
        ...
        * [Настройте ACL бакета](../../storage/operations/buckets/edit-acl.md), добавив в список сервисный аккаунт и выдав ему разрешения `READ и WRITE` (рекомендуется).
...
1. Выполните экспорт:
    ...
    - Консоль управления
        ...
        1. Укажите параметры экспорта:
            ...
            * **Имя бакета** — имя бакета в {{ objstorage-name }}, куда будут сохранены файлы базы данных.
    ...
    - CLI
        ...
        * `--cluster-name` — имя кластера {{ mms-name }}.
        * `--bucket` — имя бакета в {{ objstorage-name }}, куда будут сохранены файлы базы данных.
        ...
        * `<имя базы данных>` — имя базы данных, резервная копия которой будет экспортирована. Чтобы узнать имя, [получите список баз данных в кластере](databases.md#list-db).

    - API

        Воспользуйтесь методом API [exportBackup](../api-ref/Database/exportBackup.md) и передайте в запросе:

        * Идентификатор кластера в параметре `clusterId`. Чтобы узнать идентификатор, [получите список кластеров в каталоге](cluster-list.md#list-clusters).
        * Имя экспортируемой базы данных в параметре `databaseName`. Чтобы узнать имя, [получите список баз данных в кластере](databases.md#list-db).
...
1. Подготовьте инфраструктуру:
    ...
    1. Выдайте сервисному аккаунту права на чтение из бакета одним из способов:
        ...
        * [Настройте ACL бакета](../../storage/operations/buckets/edit-acl.md), добавив в список сервисный аккаунт и выдав ему разрешение `READ` (рекомендуется).
...
1. Выполните восстановление:
    ...
    - API
        ...
        Воспользуйтесь методом API [importBackup](../api-ref/Database/importBackup.md) и передайте в запросе:
