...
- Консоль управления
  ...
  1. Выберите одну или несколько БД, к которым должен иметь доступ пользователь:
     1. Нажмите кнопку **Добавить базу данных**.
     1. Выберите БД из выпадающего списка.
     1. Повторите два предыдущих шага, пока не будут выбраны все требуемые БД.
     1. Чтобы удалить БД, добавленную по ошибке, нажмите значок ![image](../../_assets/cross.svg) справа от имени БД.
  1. [Задайте роли пользователя](grant.md#grant-role) для каждой из выбранных БД.
...
- CLI
  ...
  1. Укажите параметры пользователя в команде создания (в примере приведены не все доступные параметры):
     ...
     {{ yc-mdb-ms }} user create <имя пользователя> \
       ...
       --databases=<список БД, к которым пользователь должен иметь доступ>
     ```
     ...
     * `--databases` — [список имен БД](databases.md#list-db), к которым пользователь должен иметь доступ.
...
- API
  ...
  * Одну или несколько БД, к которым должен иметь доступ пользователь, в одном или нескольких параметрах `userSpec.permissions.databaseName`.
  * [Роли пользователя](grant.md#predefined-db-roles) для каждой из выбранных БД в одном или нескольких параметрах `userSpec.permissions.roles`.
...
- Консоль управления
  ...
  1. Настройте права пользователя на доступ к определенным БД:
     1. Чтобы предоставить доступ к требуемым БД:
        1. Нажмите кнопку **Добавить базу данных**.
        1. Выберите БД из выпадающего списка.
        1. Повторите два предыдущих шага, пока не будут выбраны все требуемые БД.
     1. Чтобы отозвать доступ к определенной БД, удалите ее из перечня, нажав значок ![image](../../_assets/cross.svg) справа от имени БД.
  1. Задайте нужные роли пользователя для каждой из выбранных БД.
...
- CLI
  ...
  Чтобы настроить права пользователя на доступ к определенным БД, выполните команду:
  ...
  Где `--databases` – список имен БД.

  Имя кластера можно запросить со [списком кластеров в каталоге](cluster-list.md#list-clusters), имена БД — со [списком БД в кластере](databases.md#list-db).

  Эта команда предоставляет пользователю доступ к БД, указанным в списке.

  Чтобы отозвать доступ к БД, передайте в аргументе `--databases` список, из которого эта БД исключена.

- {{ TF }}

  Чтобы настроить права пользователя на доступ к определенным БД:
  1. Откройте актуальный конфигурационный файл {{ TF }} с планом инфраструктуры.

     О том, как создать такой файл, см. в разделе [{#T}](cluster-create.md).
  1. Добавьте необходимое количество блоков `permission` к описанию пользователя кластера — по одному на каждую БД:
