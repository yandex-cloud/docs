...
```
PATCH https://load-balancer.api.cloud.yandex.net/network-load-balancer/v1/targetGroups/{targetGroupId}
```
...
name | **string**<br><p>Имя целевой группы. Имя должно быть уникальным в каталоге.</p> <p>Значение должно соответствовать регулярному выражению `` \|[a-z][-a-z0-9]{1,61}[a-z0-9] ``.</p> 
