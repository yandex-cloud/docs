...
Вычисляет `measure` для даты/времени с указанным сдвигом.
Аргумент `date_dimension` задает измерение, вдоль которого делается сдвиг.
...
Возможен вариант использования `AGO(measure, date_dimension, number)`. В этом случае третий аргумент является количеством дней.

Обратите внимание, что это не оконная функция, и она не поддерживает такие параметры как `BEFORE FILTER BY`. В качестве альтернативы среди оконных функций можно воспользоваться [LAG](LAG.md).
