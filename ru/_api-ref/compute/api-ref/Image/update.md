...
Параметр | Описание
--- | ---
imageId | Обязательное поле. Идентификатор изменяемого ресурса Image. Чтобы получить идентификатор образа, используйте [list](/docs/compute/api-ref/Image/list) запрос.  Максимальная длина строки в символах — 50.
...
minDiskSize | **string** (int64)<br><p>Минимальный размер диска, который может быть создан из этого образа. Указывается в байтах. Должно быть больше объема исходных данных и больше размера виртуального диска.</p> <p>Допустимые значения — от 4194304 до 4398046511104 включительно.</p> 
