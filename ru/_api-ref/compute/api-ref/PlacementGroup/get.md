...
labels | **object**<br><p>Метки групп размещения в формате <code>key:value</code>.</p> 
spreadPlacementStrategy | **object**<br>Стратегия распределенного размещения (`spread`). Каждая из виртуальных машин в группе расположена на отдельной стойке.<br><p>Стратегия распределенного размещения (<code>spread</code>). Каждая из виртуальных машин в группе расположена на отдельной стойке.</p> <p>Это пустая структура, которую необходимо передать для явного указания стратегии размещения.</p> 