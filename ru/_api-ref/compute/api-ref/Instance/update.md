...
platformId | **string**<br><p>Идентификатор аппаратной платформы виртуальной машины. Это поле влияет на допустимые значения в поле <a href="/docs/compute/api-ref/Instance/update#body_params">resourcesSpec</a>.</p> <p>В настоящее время доступна только одна платформа - <code>standard-v1</code>. Эта платформа подходит для большинства задач. Эта платформа позволяет создавать машины различных типов: с большим объемом памяти, с большим количеством ядер, с высокой производительностью.</p> <p>Значение должно соответствовать регулярному выражению <code>standard-v1</code>.</p> 
resourcesSpec | **object**<br><p>Вычислительные ресурсы виртуальной машины, такие как объем памяти и количество ядер. Чтобы узнать список допустимых значений, см. <a href="/docs/compute/concepts/vm-types">Типы виртуальных машин</a>.</p> 
