...
Удаление снимка — необратимая операция, восстановить
удаленный снимок невозможно. Однако при удалении диска не
удаляются снимки или образы, ранее созданные из этого диска.
Снимки и образы необходимо удалять отдельно.
...
Параметр | Описание
--- | ---
diskId | Обязательное поле. Идентификатор удаляемого диска. Чтобы получить идентификатор диска, используйте запрос [list](/docs/compute/api-ref/Disk/list).  Максимальная длина строки в символах — 50.
