...
## Включите NAT в интернет для подсети с промежуточной ВМ {#enable-nat}

В подсети будет развернута промежуточная ВМ.
...
- {{ TF }}
  ...
  1. Создайте конфигурационный файл с вызовом модуля `yc-splunk-trail`:
     ...
     * `coi_subnet_id` — идентификатор подсети с включенным NAT в интернет.
...
2. [Удалите](../../storage/operations/buckets/delete) бакет {{ objstorage-name }}.

3. [Удалите](../../kms/operations/key#delete) ключ {{ kms-name }}.