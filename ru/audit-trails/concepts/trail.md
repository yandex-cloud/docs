...
* Блок **Назначение**:
    * **Назначение** — {% if product == "yandex-cloud" %}значения{% endif %}{% if product == "cloud-il" %}значение{% endif %} `{{ objstorage-name }}`{% if product == "yandex-cloud" %} или `{{ cloud-logging-name }}`{% endif %}.
...
{% endif %}
* Блок **Сервисный аккаунт** — сервисный аккаунт, от имени которого будет выполняться загрузка аудитных логов в бакет {% if product == "yandex-cloud" %} или лог-группу{% endif %}. Если аккаунту нужны дополнительные роли, появится предупреждение с перечислением ролей.
