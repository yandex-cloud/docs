...
Для публичного соединения необходимо использовать технологию трансляции сетевых адресов [NAT]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/NAT){% endif %}{% if lang == "en" %}(https://en.wikipedia.org/wiki/Network_address_translation){% endif %} на стороне вашего оборудования. Допускаются следующие варианты реализации NAT:
