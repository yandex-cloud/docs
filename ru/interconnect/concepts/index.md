...
Для обмена маршрутной информацией и дальнейшей передачи трафика необходимо настроить протокол [BGP]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/Border_Gateway_Protocol){% endif %}{% if lang == "en" %}(https://en.wikipedia.org/wiki/Border_Gateway_Protocol){% endif %}:
