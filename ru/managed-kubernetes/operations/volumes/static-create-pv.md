...
После этого рядом с используемым диском в консоли управления в **{{ ui-key.yacloud.iam.folder.dashboard.label_compute }}** в разделе **{{ ui-key.yacloud.compute.storage_uisyT }}** появится надпись **{{ ui-key.yacloud.compute.disks.label_disk-used }}**.
