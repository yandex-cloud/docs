...
Вы можете использовать [бакет](../../../storage/concepts/bucket.md) [{{ objstorage-full-name }}](../../../storage/) в качестве хранилища для пода. Подробнее в разделе [{#T}](s3-csi-integration.md).
...
1. Посмотрите информацию о созданном поде:
   ...
   После создания пода:
   * В [консоли управления]({{ link-console-main }}) в **{{ ui-key.yacloud.iam.folder.dashboard.label_compute }}** в разделе **{{ ui-key.yacloud.compute.storage_uisyT }}** появится новый [диск](../../../compute/concepts/disk.md) с префиксом `k8s-csi` в имени диска.
