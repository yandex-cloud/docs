...
Для [групп безопасности](../../vpc/concepts/security-groups.md) действует принцип <q>весь трафик, который не разрешен, запрещен</q>. Чтобы кластер работал, необходимо [создать правила](../../vpc/operations/security-group-update.md#add-rule) в его группах безопасности, разрешающие:
...
1. Добавьте правило для исходящего трафика, разрешающее хостам кластера подключаться к внешним ресурсам, например, для скачивания образов с Docker Hub или работы с [{{ objstorage-full-name }}](../solutions/backup.md):
   * Диапазон портов — `{{ port-any }}`.
   * Протокол — любой (`Any`).
   * Тип источника — `CIDR`.
