...
- Консоль управления {#console}
  ...
  1. В поле **{{ ui-key.yacloud.k8s.clusters.create.field_master-version }}** блока **{{ ui-key.yacloud.k8s.clusters.create.section_main-cluster }}** получите список доступных версий.
...
- Консоль управления {#console}
  ...
  * `{{ ui-key.yacloud.k8s.clusters.create.value_maintenance-weekly }}` — укажите день, начало и длительность интервала времени в UTC, в течение которого запустится обновление. Параметр не связан с длительностью самого обновления и временем его окончания. При необходимости выберите несколько вариантов с помощью кнопки **{{ ui-key.yacloud.k8s.clusters.create.button_add-day-of-week }}**.
...
- CLI {#cli}
  ...
  * `--daily-maintenance-window` — задать начало и длительность интервала времени в UTC, в течение которого запустится обновление.
    ...
    Пример настройки ежедневного окна запуска обновлений кластера {{ managed-k8s-name }}: начало интервала — 22:00 UTC, длительность интервала — не более десяти часов:
  ...
  * `--weekly-maintenance-window` — задать дни, начало и длительность интервала времени в UTC, в течение которого запустится обновление.
    ...
    Пример настройки окна запуска обновлений кластера {{ managed-k8s-name }} по понедельникам и вторникам: начало интервала — 22:00 UTC, длительность интервала — не более десяти часов:
...
- CLI {#cli}
  ...
  * `--daily-maintenance-window` — задать начало и длительность интервала времени в UTC, в течение которого запустится обновление.
    ...
    Пример настройки ежедневного окна запуска обновлений группы узлов {{ managed-k8s-name }}: начало интервала — 22:00 UTC, длительность интервала — не более десяти часов:
  ...
  * `--weekly-maintenance-window` — задать дни, начало и длительность интервала времени в UTC, в течение которого запустится обновление.
    ...
    Пример настройки окна запуска обновлений группы узлов {{ managed-k8s-name }} по понедельникам и вторникам: начало интервала — 22:00 UTC, длительность интервала — не более десяти часов:
