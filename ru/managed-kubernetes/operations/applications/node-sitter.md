...
1. Задайте настройки приложения:
   ...
   * (опционально) **Доверенные сертификаты** — скопируйте содержимое файла с сертификатами x.509 в формате PEM для установки в хранилище доверенных сертификатов на узлах группы с прерываемыми ВМ. При пересоздании узлов сертификаты будут устанавливаться заново.
   ...
   * (опционально) **Исключения для прокси** — укажите список сайтов-исключений, для доступа к которым не будет использоваться прокси. Значение поля передается через переменную окружения `NO_PROXY`.
