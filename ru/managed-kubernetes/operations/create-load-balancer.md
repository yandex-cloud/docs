...
Доступ к приложению может быть предоставлен из внутренних сетей, но не из кластера {{ k8s }}: из [подсетей](../../vpc/concepts/network.md#subnet) {{ vpc-full-name }} или внутренних подсетей организации, подключенных к {{ yandex-cloud }} с помощью сервиса [{{ interconnect-full-name }}](../../vpc/interconnect/index.md) или через VPN. Для этого используйте сервис типа `LoadBalancer` на основе [внутреннего сетевого балансировщика нагрузки](../../load-balancer/concepts/internal-load-balancer.md).
