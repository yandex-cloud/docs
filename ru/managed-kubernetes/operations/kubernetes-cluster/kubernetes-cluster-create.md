...
- Terraform
  ...
  1. Опишите в конфигурационном файле параметры ресурсов, которые необходимо создать. Вы можете добавить несколько записей одновременно.
     ...
     * `service_account_id` — идентификатор сервисного аккаунта для ресурсов. От его имени будут создаваться ресурсы, необходимые кластеру {{ k8s }}. Указанный аккаунт службы должна иметь роль редактирования в каталоге, в которой будет расположен кластер.
