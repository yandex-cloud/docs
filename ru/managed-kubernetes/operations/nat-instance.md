# Настройка виртуальной машины в качестве NAT-инстанса

В Яндекс.Облаке можно настроить связь нескольких виртуальных машин с интернетом через NAT-инстанс. При этом будет использован только один публичный IP-адрес — тот, который будет присвоен NAT-инстансу.

{% note important %}

Для корректной работы NAT-инстанса необходимо поместить его в подсеть без таблиц маршрутизации.

{% endnote %}

## Подготовьтесь к работе {#before-begin}

Создайте подсеть для NAT-инстанса.

1. Создайте [облачную сеть](../../vpc/operations/network-create.md) с любым именем, например `nat-instance-network`.
1. Создайте в облачной сети [подсеть](../../vpc/operations/subnet-create.md) для NAT-инстанса с любым именем, например `nat-subnet`. Не привязывайте к ней таблицы маршрутизации.

## Создайте NAT-инстанс {#create-nat-instance}

Вы можете автоматически создать NAT-инстанс c помощью YC CLI или создать ВМ и настроить ее вручную.

### Создание NAT-инстанс с помощью YC CLI из готового образа {#yc-create}
...
1. Посмотрите внутренний IP-адрес NAT-инстанса.

    1. В консоли управления в {{ compute-name }} в разделе **Виртуальные машины** из списка ВМ выберите NAT-инстанс. В примере это ВМ с именем `nat-instance`. 
...
### Ручное создание и настройка NAT-инстанса {#create}
