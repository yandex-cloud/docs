...
1. Создайте группу узлов с {{ k8s }}-метками:
   ...
   - Консоль управления {#console}
     ...
     1. Выберите кластер {{ managed-k8s-name }}, для которого нужно создать группу узлов.
     1. Перейдите на вкладку **{{ ui-key.yacloud.k8s.cluster.switch_cluster-overview }}** и выберите раздел **{{ ui-key.yacloud.k8s.nodes.label_node-groups }}**. Нажмите кнопку **{{ ui-key.yacloud.k8s.cluster.node-groups.button_create }}**. Если открылось меню выбора типа группы, выберите ![cloud](../../../_assets/console-icons/cloud.svg) **{{ ui-key.yacloud.k8s.nodes.button_create-ng }}**.
...
1. Посмотрите информацию о созданной группе узлов с {{ k8s }}-метками:
   ...
   - Консоль управления {#console}
     ...
     1. Выберите один из узлов {{ managed-k8s-name }}.
     1. На вкладке **{{ ui-key.yacloud.k8s.node.overview.label_labels }}** перечислены системные и пользовательские {{ k8s }}-метки узла.
