...
yc managed-kubernetes node-group create \
  ...
  --location zone=<зона_доступности>,subnet-name=<имя_подсети> \
  --public-ip \
...
- Консоль управления
  ...
  1. Откройте раздел **Compute Cloud** в каталоге, где создан ваш кластер {{ k8s }}.
...
- Linux/macOS/Windows 10
  ...
  ```bash
  ssh <имя пользователя>@<публичный_IP-адрес_узла>
  ```
