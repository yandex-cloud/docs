...
- Консоль управления {#console}
  ...
  1. Выберите кластер {{ managed-k8s-name }} и перейдите на вкладку **{{ ui-key.yacloud.k8s.cluster.switch_network }}**.
  1. Перейдите на вкладку с типом ресурса, который нужно создать: **{{ ui-key.yacloud.k8s.network.label_services }}** или **{{ ui-key.yacloud.k8s.network.label_ingress }}**.
...
- Консоль управления {#console}
  ...
  1. [Перейдите]({{ link-console-main }}/link/managed-kubernetes) в сервис **{{ ui-key.yacloud.iam.folder.dashboard.label_managed-kubernetes }}**.
  1. Выберите кластер {{ managed-k8s-name }} и перейдите на вкладку **{{ ui-key.yacloud.k8s.cluster.switch_network }}**.
  1. Перейдите на вкладку с типом ресурса, который нужно отредактировать: **{{ ui-key.yacloud.k8s.network.label_services }}** или **{{ ui-key.yacloud.k8s.network.label_ingress }}**.
...
- Консоль управления {#console}
  ...
  1. [Перейдите]({{ link-console-main }}/link/managed-kubernetes) в сервис **{{ ui-key.yacloud.iam.folder.dashboard.label_managed-kubernetes }}**.
  1. Выберите кластер {{ managed-k8s-name }} и перейдите на вкладку **{{ ui-key.yacloud.k8s.cluster.switch_network }}**.
  1. Перейдите на вкладку с типом ресурса, который нужно удалить: **{{ ui-key.yacloud.k8s.network.label_services }}** или **{{ ui-key.yacloud.k8s.network.label_ingress }}**.
