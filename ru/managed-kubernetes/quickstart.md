...
Создайте [кластер [!KEYREF k8s]](concepts/index.md#kubernetes-cluster) и [группу узлов](concepts/index.md#node-group) и настройте kubectl для работы с ними.
...
1. [Подсети](../vpc/operations/subnet-create.md) в зонах доступности, где будут созданы кластер [!KEYREF k8s] и группа узлов.
...
## Создание кластера [!KEYREF k8s] {#create-kubernetes-cluster}

1. Посмотрите описание команды CLI для создания кластера [!KEYREF k8s]:

    ```
    $ yc managed-kubernetes cluster create --help
    ```
    
1. Создайте кластер [!KEYREF k8s]:
...
1. Посмотрите описание команды CLI для создания группы узлов [!KEYREF k8s]:
