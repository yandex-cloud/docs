...
* `updateMode: "Initial"`, чтобы {{ k8s-vpa }} применял рекомендации только при создании подов. Для уже запущенных подов ресурсы не изменяются, даже если рекомендации со временем обновляются.
* `updateMode: "Recreate"`, чтобы {{ k8s-vpa }} автоматически управлял ресурсами подов, пересоздавая их при значительном расхождении между текущими запросами и рекомендациями. После удаления пода контроллер (Deployment, StatefulSet и т. д.) создаёт новый под с обновлёнными значениями ресурсов.
* `updateMode: "InPlaceOrRecreate"`, чтобы {{ k8s-vpa }} пытался обновлять запросы и лимиты ресурсов без перезапуска пода, если это возможно. Если обновление «in place» недоступно, под будет пересоздан аналогично режиму Recreate.
В этом режиме используется механизм Resize Container Resources In-Place.
