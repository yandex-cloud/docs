...
1. Выберите диапазон, который соответствует требованиям и вмещает нужное количество узлов.
   ...
   ```text
   Количество блоков = 2^(маска подсети узлов − маска диапазона)
   ```

   Например, диапазон `10.1.0.0/16` при маске подсети узлов `/24` содержит `2^(24 − 16) = 256` блоков: от `10.1.0.0/24` до `10.1.255.0/24`. Расчет не учитывает возможное резервирование блоков сервисом {{ managed-k8s-name }}.
...
1. [Расширьте группу узлов](../operations/node-group/node-group-update.md) и [проверьте блоки адресов, назначенные новым узлам](../operations/network/manage-ip-masq-agent.md#verify).
