...
* При динамической подготовке тома: после удаления объекта `PersistentVolumeClaim` на основе {% if product == "yandex-cloud" %}классов хранилищ `yc-network-hdd` или{% endif %}{% if product == "cloud-il" %}класса хранилища{% endif %} `yc-network-ssd`, соответствующие объект `PersistentVolume` и диск {{ compute-name }} **будут удалены**.
