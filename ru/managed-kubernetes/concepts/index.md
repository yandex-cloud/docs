...
_Группа узлов_ объединяет вычислительные ресурсы кластера {{ k8s }}, на которых запускаются пользовательские контейнеры. Группы узлов бывают следующих типов:

* _Облачная группа узлов_ — [группа виртуальных машин](../../compute/concepts/instance-groups/index.md) {{ compute-full-name }} с одинаковой конфигурацией. Сервис создает и обслуживает ВМ, поддерживает заданное количество узлов и позволяет настроить автоматическое масштабирование. Это основной сценарий [создания группы узлов](../operations/node-group/node-group-create.md).

* _Внешняя группа узлов_ объединяет серверы, которые вы самостоятельно настраиваете и [подключаете к кластеру](../operations/external-nodes-connect.md). Это серверы, расположенные вне {{ yandex-cloud }}. За [подготовку и обслуживание внешних узлов](external-nodes.md#responsibility) отвечает пользователь.

* _Группа узлов на серверах {{ baremetal-name }}_ состоит из выделенных физических серверов одной конфигурации. При [создании группы](../operations/baremetal-group/create-baremetal.md) сервис арендует серверы, настраивает их и подключает к кластеру. Предварительно арендовать и подключать каждый сервер вручную не нужно. Серверы отображаются в {{ baremetal-name }}, а группа управляется через консоль управления в {{ managed-k8s-name }}.

Приведенные ниже настройки ВМ, автоматического масштабирования и шаблона узла относятся к облачным группам.
...
Для облачной группы узлов вы можете задать следующие параметры:
* Имя и описание.
...
Для реализации механизма маскарадинга на каждом узле кластера развернут под `ip-masq-agent`. Настройки этого пода хранятся в объекте ConfigMap с именем `ip-masq-agent` в пространстве имен `kube-system`. В параметре `data.config.nonMasqueradeCIDRs` задаются диапазоны назначения, для которых маскарадинг не требуется, например, для доступа к подам через [VPN](../../glossary/vpn.md) или [{{ interconnect-full-name }}](../../interconnect/index.yaml).

{% note warning %}

Если вы добавляете собственные диапазоны в `nonMasqueradeCIDRs` или изменяете другие параметры маскарадинга, требуется режим ручного управления `disabled`. В режиме `enforced` контроллер при очередной синхронизации перезаписывает весь `data.config` системным шаблоном, поэтому пользовательские диапазоны и настройки не сохраняются.

{% endnote %}

Порядок переключения режима и изменения ConfigMap описан в разделе [Настройка маскарадинга](../operations/network/manage-ip-masq-agent.md#manual-management). В режиме `disabled` после каждого добавления диапазона подов необходимо самостоятельно обновлять `nonMasqueradeCIDRs`.

Пример параметра `nonMasqueradeCIDRs`:
