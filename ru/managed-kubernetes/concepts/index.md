...
Сервис полностью управляет мастером, а также следит за состоянием и работоспособностью группы узлов. Пользователь может управлять узлами напрямую, а также настраивать кластер {{ k8s }} с помощью консоли управления Облака, CLI и API Managed Service for Kubernetes.
...
- [Настроить виртуальную машину в качестве NAT-инстанса](../operations/nat-instance.md). В таком случае будет использован только один публичный IP-адрес — тот, который присвоен шлюзу.

{% endnote %}

При работе с кластером {{ k8s }} на инфраструктуре Яндекс.Облака задействуются следующие ресурсы:
...
- _Региональный_ — создается распределенно в трех подсетях в каждой зоне доступности. При недоступности одной зоны региональный мастер остается работоспособным. 
    ...
    - Региональный мастер поддерживается только при [создании в подсетях](../../vpc/operations/subnet-create.md) с диапазонами `172.16.0.0/12` и `192.168.0.0/16`.
