...
[{{ k8s }}]{% if region == "int" %}(https://https://kubernetes.io/){% else %}(https://kubernetes.io/ru/){% endif %} — система для управления контейнерными приложениями. {{ k8s }} предоставляет механизмы взаимодействия с кластером, с помощью которых осуществляется автоматизации развертывания, масштабирования и управления приложениями в контейнерах.
...
Не путайте [облачные метки группы узлов](../../overview/concepts/services.md#labels) (`labels`) и [{{ k8s }}-метки узлов]{% if region == "int" %}(https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/){% else %}(https://kubernetes.io/ru/docs/concepts/overview/working-with-objects/labels/){% endif %} (`node_labels`), которыми управляет {{ managed-k8s-name }}.
