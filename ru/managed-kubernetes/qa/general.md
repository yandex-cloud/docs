...
Мы рекомендуем использовать последнюю доступную официальную версию [kubectl]{% if lang == "ru" %}(https://kubernetes.io/ru/docs/tasks/tools/install-kubectl/){% endif %}{% if lang == "en" %}(https://kubernetes.io/docs/tasks/tools/#kubectl){% endif %}, чтобы избежать проблем совместимости.
