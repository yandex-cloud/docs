...
Вы можете изменять ресурсы только для группы узлов. В одном кластере {{ k8s }} можно создавать группы с разными конфигурациями{% if product == "yandex-cloud" %} и размещать их в разных [зонах доступности](../../overview/concepts/geo-scope.md){% endif %}. Подробнее читайте в разделе [{#T}](../operations/node-group/node-group-update.md).
