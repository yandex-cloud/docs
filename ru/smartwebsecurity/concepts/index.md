...
{{ sws-name }} проверяет HTTP-запросы к защищаемому ресурсу на соответствие [правилам](rules.md), заданным в [профиле безопасности](profiles.md). Профиль безопасности можно подключить к [виртуальному хосту L7-балансировщика](../operations/host-connect.md#host), [домену](../operations/host-connect.md#domain) или [API-шлюзу](../operations/host-connect.md#gateway). В зависимости от результатов проверки запросы направляются к защищаемому ресурсу, блокируются или отправляются в сервис [{{ captcha-full-name }}](../../smartcaptcha/) для дополнительной верификации.
