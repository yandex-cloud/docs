...
* _Полная защита_ — трафик проверяется на основе ML-моделей и поведенческого анализа. Подозрительные запросы отправляются в {{ captcha-name }}.
     ...
     Для корректной работы вашего приложения обрабатывайте HTTP-запросы с динамической подгрузкой контента в режиме _Защита API_.
...
* _Защита API_ используется для эндпоинтов:
     ...
     * на которых обрабатываются запросы с динамической подгрузкой контента (например, `ajax`, `xhr`, `iframe` и так далее).
