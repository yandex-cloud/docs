...
* JA3 — пассивный метод для идентификации клиента. Собирает идентификаторы значений из полей пакета приветствия клиента в формате `TLSVersion,Ciphers,Extensions,EllipticCurves,EllipticCurvePointFormats` и хеширует MD5 для создания 32-символьного отпечатка.

    Примеры отпечатков JA3:

    * Стандартный клиент Tor — `e7d705a3286e19ea42f587b344ee6865`.
    * Вредоносное ПО Trickbot — `6734f37431670b3ab4292b8f60f29984`.
    * Вредоносное ПО Emotet — `4d7a28d6f2263ed61de88ca66eb011e3`.

* JA4 — набор методов сетевой идентификации с более глубоким анализом. {{ sws-name }} поддерживает один метод JA4 — идентификация TLS-клиента.


    Все отпечатки JA4 имеют формат `a_b_c`, где разделители обозначают разные части идентификатора. Например, `t13d3812h2_8a4b5c6d_7e8f9a0b`. Это позволяет выполнять гибкий поиск и обнаружение, используя только части отпечатка.
