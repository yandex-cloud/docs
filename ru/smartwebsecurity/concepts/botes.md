---
title: Управление роботизированным трафиком
description: '{{ sws-name }} определяет легитимных и подозрительных ботов, использует категории ботов, Bot Score и отпечатки TLS-соединений.'
---
...
Категории ботов и список легитимных ботов доступны как [предустановленные списки {{ sws-name }}](lists.md#yc-rules). Их не нужно создавать и обновлять вручную. Используйте их в условиях `Bot category` и `Bot name`, чтобы пропускать полезных роботов или применять к ним отдельные правила.
...
Результат фильтрации трафика и уровень оценки бота доступны в логах для более тонкой настройки правил безопасности.
...
1. `FacebookExternalHit`^1^ — бот собирает, кеширует и показывает информацию о сайте или приложении, в том числе название, описание и иконку (миниатюрное изображение).
1. `MetaExternalAgent`^2^ — бот просматривает веб‑контент для обучения моделей искусственного интеллекта и улучшения продуктов путем прямого индексирования контента.
1. `Meta‑ExternalFetcher`^2^ — бот поддерживает способность Meta AI предоставлять пользователям актуальную информацию, выходящую за рамки обучающих данных.
...
1. `Meta‑WebIndexerBot`^2^ — бот просматривает веб‑ресурсы, чтобы повысить качество результатов поиска Meta AI для пользователей.
1. `Meta‑ExternalAdsBot`^2^ — бот просматривает веб‑контент в поисках сценариев использования, таких как улучшение рекламы и других бизнес‑продуктов и услуг.


^1^ Facebook — сервис предоставляется организацией Meta, признанной экстремистской.
^2^ Meta признана экстремистской организацией, ее деятельность в России запрещена.

{% endcut %}

## Оценка бота {#configuration}

Чтобы настроить индивидуальные правила для вашего трафика, можно задать правила фильтрации по уровню роботизированности запроса — Bot Score.

{{ sws-name }} присваивает трафику уровень от `0` (человек) до `100` (бот).
...
## Отпечатки (FingerPrints) {#fingerprint}

[FingerPrint](https://developers.cloudflare.com/bots/additional-configurations/ja3-ja4-fingerprint/) — это отпечаток SSL/TLS‑соединения по методу [JA3](https://github.com/salesforce/ja3) или [JA4](https://github.com/FoxIO-LLC/ja4), который собирает балансировщик или прокси-сервер. Формируется на основе версии TLS, наборов шифров, расширений, алгоритмов подписи и других параметров

Отпечатки могут использоваться в следующих сценариях:

* Блокировка трафика с известными отпечатками вредоносного ПО и ботов.
* Идентификация приложений для разрешения легитимного трафика.
* Выявление DGA-ботов при постоянной смене доменов и IP-адресов.
* Снижение доли ложных блокировок.

Технология FingerPrint позволяет идентифицировать конкретные клиенты путем анализа параметров пакета приветствия. Примеры таких клиентов — браузеры, приложения и вредоносное ПО. В процессе анализируются параметры, которые клиент отправляет в открытом виде на этапе установления TLS‑соединения: версия TLS, версия записи TLS, наборы шифров, параметры сжатия, список расширений, алгоритмы подписи, алгоритм шифрования данных, хеш-функция.

Для идентификации клиентов {{ sws-name }} использует методы JA3 и JA4:

* JA3 — пассивный метод для идентификации клиента. JA3 формируется по параметрам первого сообщения TLS‑рукопожатия (ClientHello), которое клиент отправляет серверу в формате: `TLSVersion,Ciphers,Extensions,EllipticCurves,EllipticCurvePointFormats`. Используется исходная JA3‑строка без MD5‑преобразования.

    Примеры отпечатков JA3 (строк):
...
* JA4 — набор методов сетевой идентификации с более глубоким анализом. {{ sws-name }} поддерживает один вариант JA4 — отпечаток TLS-клиента.

    Отпечаток JA4 имеет формат `a_b_c`, где разделители обозначают разные части идентификатора. Например, `t13d3812h2_8a4b5c6d_7e8f9a0b`. Такой формат позволяет выполнять поиск и корреляцию как по полному отпечатку, так и по отдельным частям.

Отпечатки отсутствуют в незашифрованном HTTP-трафике и во внутреннем трафике сервисов {{ yandex-cloud }}, а обновление клиентского приложения может изменить его отпечаток. Поэтому рекомендуется предусматривать альтернативные условия фильтрации и регулярно обновлять правила безопасности.