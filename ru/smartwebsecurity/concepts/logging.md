---
title: Логирование в {{ sws-name }}
description: Какие данные о запросах, правилах и вердиктах {{ sws-name }} доступны в логах и как использовать их для анализа трафика.
---

# Логирование

{{ sws-name }} позволяет записывать и анализировать данные о запросах и правилах, которые сработали для этих запросов. Эти данные помогают настраивать защиту и разбирать инциденты.

Для записи логов доступны два варианта:

* логирование через {{ sws-name }};
* логирование через L7-балансировщик {{ alb-name }}, к которому подключен профиль безопасности.

Информация в этом разделе относится к логированию через {{ sws-name }}. Этот вариант предоставляет больше возможностей для анализа, чем логирование через {{ alb-name }}. Запись запросов через L7-балансировщик описана в разделе [{#T}](../operations/configure-logging-alb.md).

{% note info %}

Сейчас логи {{ sws-name }} записываются в {{ cloud-logging-name }}. Этот сервис будет закрыт во II квартале 2027 года, работа с логами перейдет в единую Observability-платформу [{{ monium-name }}](../../monium/). Для новых профилей безопасности {{ cloud-logging-name }} станет недоступен, при этом появится возможность настроить запись логов в {{ monium-name }}.

{% endnote %}

В логах содержится информация об HTTP-запросах, которые обработал сервис {{ sws-name }}. Можно записывать только запросы, которые были заблокированы с вердиктом `DENY` или отправлены на капчу с вердиктом `CAPTCHA`. Дополнительно можно записывать часть легитимных запросов с вердиктом `ALLOW`. Чтобы уменьшить объем логов, укажите долю таких запросов — от 1 до 100 процентов.
...
В поле `action` вердикт `JS_CHALLENGE` означает, что запрос направлен на [проверку JS Challenge](rules.md#client-checks), а `COOKIE_CHALLENGE` — на проверку Cookie Challenge.
...
|| `action` | Действие, которое было применено к запросу: `ALLOW`, `DENY`, `CAPTCHA`, `JS_CHALLENGE` или `COOKIE_CHALLENGE`. ||
|| `matched_rule_name` | Имя сработавшего правила.
Имя `sws_service_rule` соответствует [сервисному правилу](../../smartwebsecurity/concepts/rules.md#service-rule), которое может блокировать трафик при атаках. ||
...
|| `matched_rule_verdict` | Действие (вердикт), которое было применено правилом к запросу. Возможные значения: `ALLOW`, `DENY` или `CAPTCHA`. ||
...
|| `arl_verdict` | Действие (вердикт), которое применено к запросу профилем ARL: `ALLOW`, `DENY` или `CAPTCHA`. ||
...
Чтобы анализировать логи {{ sws-name }}, составьте запросы с нужными полями и их комбинациями. Примеры готовых запросов приведены в разделе [Примеры готовых фильтров для логов](../operations/configure-logging.md#filtration).
