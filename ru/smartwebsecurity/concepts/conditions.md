...
|
Список легитимных ботов, принадлежащих различным компаниям и сервисам
|
...
| | ||
|| `Вероятность бота` (Bot score) | |
При проверке трафика система присваивает ему рейтинг от `0` до `100`. Для фильтрации трафика можно указать значение или диапазон значений этого рейтинга с помощью операторов `>=`, `<=`, `=`, `!=`.
