...
* Логирование через {{ sws-name }} перешло на стадию [General Availability](../overview/concepts/launch-stages.md). Теперь собственный интерфейс предоставляет больше возможностей для анализа, чем логирование через L7-балансировщик {{ alb-name }}.
...
* Добавлено [сервисное правило](concepts/rules.md#service-rule) для усиления защиты при атаках. Теперь {{ sws-name }} может блокировать вредоносные запросы, даже если профиль безопасности пользователя их разрешает. При срабатывании сервисного правила в логах появляется отметка `sws_service_rule`.

* 9 июня 2026 года сервис перешел на новый формат условий и имен полей в API, CLI и {{ TF }}. Несколько значений в одном условии теперь объединяются в одно текстовое поле с разделителем `|`. Изменились имена полей `http_methods`, `authorities`, `ja3_ranges`, `ja4_ranges`. Существующие правила автоматически перенесены в новый формат. Подробнее в разделе [Миграция на новый формат условий](operations/api-migration.md).
