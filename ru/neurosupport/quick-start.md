...
1. При необходимости создать [каталог](../resource-manager/operations/folder/create.md). По умолчанию уже создан каталог с названием `default`.
...
При возникновении вопросов добавляйтесь в [чат технической поддержки](https://t.me/yandex_neurosupport_bot).
...
- `index_name` — Строковое имя индекса (строка, обязательное). Для формирования index_name используйте полученный ранее префикс, значение должно начинаться с него. Подробнее в разделе [Загрузка базы знаний](#knowledge-base-upload).
...
- `documents` — Список документов для индексации (обязательное). Обратите внимание, что большие документы при загрузке разбиваются на части меньшего размера. Идентификатор каждой такой части будет `(идентификатор документа)_(номер части)`. Например, объемный документ с `"doc_id": "instructions"` будет загружен частями с идентификаторами `"doc_id": "instructions_0", "instructions_1"` и т.д. (обязательное):
  ...
  - `typed_meta` — значения метаданных для гибкой фильтрации (опциональное, подробнее в разделе [Гибкая фильтрация по метаданным](#flexible-metadata-filtering)):
...
- `meta_features` — Дополнительные метаданные. Подробнее в разделе [Гибкая фильтрация по метаданным](#flexible-metadata-filtering) (опциональное).
...
- `candidates_top_k` — Количество чанков, которые передаются в модель. Значение по умолчанию 10 (опциональное).
...
##### Формирование промпта {#transfer-custom-promt}
