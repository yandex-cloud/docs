...
Назначьте сервисному аккаунту необходимые роли по инструкции из статьи [{#T}](../../iam/operations/sa/assign-role-for-sa.md).
...
Назначьте этой учетной записи роль `iam.serviceAccounts.user` **на используемый сервисный аккаунт** по [инструкции](../../iam/operations/sa/set-access-bindings.md#assign-role-to-sa). Также роль можно назначить на родительский каталог или облако.

Если запрос выполняется от имени того же сервисного аккаунта, который вы привязываете к ресурсу, ему тоже нужны права на использование самого себя. Например, такие права проверяются при создании ревизии контейнера в {{ serverless-containers-name }}.
...
Роль `{{ roles-cloud-owner }}` можно назначить только на облако. В разделе прав доступа выберите облако и назначьте роль по [инструкции](../../resource-manager/operations/cloud/set-access-bindings.md#access-to-user). Назначить эту роль может [владелец облака](../../resource-manager/concepts/resources-hierarchy.md#owner).
...
Если проблема сохраняется, [обратитесь в поддержку]({{ link-console-support }}). Приложите полный текст ошибки и [HAR-файл](../../support/create-har.md), записанный при ее воспроизведении.
...
Возможно, исходная учетная запись удалена на стороне {{ yandex-360 }} или поставщика удостоверений. Проверьте это у администратора и, если учетная запись больше не используется, [удалите пользователя из организации](../../organization/operations/edit-account.md).
