...
* через [HTTPS-интерфейс]({{ ch.docs }}{{ lang }}/interfaces/http):
    * по шифрованному соединению с использованием [SSL](../../glossary/ssl-certificate) — порт 8443;
    * без шифрования — порт 8123;

* через [клиент командной строки]({{ ch.docs }}{{ lang }}/interfaces/cli):
...
{{ mch-short-name }} использует Native / TLS протокол (порт 9440) для межсерверного взаимодействия при выполнении распределенных запросов. Возможность менять протокол или порт не предусмотрена.
