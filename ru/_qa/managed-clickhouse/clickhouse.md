...
{{ CH }} поддерживает только добавление и чтение данных, так как предназначен прежде всего для {% if lang == "ru" and audience != "internal" %}[аналитики](../../glossary/data-analytics.md){% else %}аналитики{% endif %} (OLAP). В остальных случаях, скорее всего, удобнее использовать {{ PG }}.
