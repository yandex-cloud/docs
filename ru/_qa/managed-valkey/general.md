...
- [Класс хостов](../../managed-valkey/concepts/instance-types.md) (характеристики производительности — процессоры, память и т. п.).
- [Тип диска](../../managed-valkey/concepts/storage.md) и его размер (резервируется в полном объеме при создании кластера).
...
Подробные инструкции см. в разделе [Начало работы](../../managed-valkey/quickstart.md).

#### Сколько хостов БД может содержать кластер? {#how-many-hosts}

Минимальное количество хостов в кластере зависит от:
* выбранных [платформы и класса хостов](../../managed-valkey/concepts/instance-types.md);
* выбранного [типа диска](../../managed-valkey/concepts/storage.md);
* включенного или выключенного при [создании](../../managed-valkey/operations/cluster-create.md) кластера [шардирования](../../managed-valkey/concepts/sharding.md).
...
Подробнее см. в разделе [Квоты и лимиты](../../managed-valkey/concepts/limits.md).
...
Технические и организационные ограничения MDB приведены в разделе [Квоты и лимиты](../../managed-valkey/concepts/limits.md).
...
Подробнее см. в разделе [Техническое обслуживание](../../managed-valkey/concepts/maintenance.md).

#### Какую версию {{ VLK }} использует {{ mrd-short-name }}? {#dbms-version}

{{ mrd-short-name }} поддерживает {{ VLK }} версии {{ versions.console.str }}.
...
Такая ошибка возникает, когда хост добавляется в кластер {{ mrd-name }} с [классом хостов](../../managed-valkey/concepts/instance-types.md#available-flavors) b2.medium или b3-c1-m4. В этом случае [измените класс хостов](../../managed-valkey/operations/update.md#change-resource-preset) на другой. После этого кластер будет недоступен около пяти — семи минут. Затем добавьте хост в кластер.

Кластеры с классами хостов b2.medium или b3-c1-m4 могут содержать только один хост на кластер или [шард](../../managed-valkey/concepts/sharding.md). Если хотите сохранить класс хостов, после смены класса можно добавить хост с нужной конфигурацией и удалить прежний хост. Затем верните первоначальный класс хостов.
...
[Создайте алерт](../../managed-valkey/operations/monitoring.md#monitoring-integration) с метрикой `disk.used_bytes` в сервисе {{ monitoring-full-name }}. Метрика показывает размер использованного дискового пространства в кластере {{ mrd-name }}.
...
Чтобы увеличить максимальные значения IOPS и bandwidth и снизить вероятность троттлинга, расширьте размер хранилища при [изменении кластера](../../managed-valkey/operations/update.md#change-disk-size).

Рассмотрите возможность перехода на более быстрый тип дисков путем [восстановления кластера](../../managed-valkey/operations/cluster-backups.md#restore) из резервной копии.
