...
* [**Max connections**](../../managed-postgresql/concepts/settings-list.md#setting-max-connections) — максимальное количество подключений, зарезервированных для всех пользователей. По умолчанию равно `200 × <количество vCPU на каждом хосте>`. При этом нужно учитывать, что в это число входит 15 служебных подключений: например, если для кластера задан параметр `"max_connections": 100`, то вы можете зарезервировать не более 85 подключений на каждый хост кластера для пользователей.
...
#### Можно ли изменить тип диска в существующем кластере с {% if audience != "internal" %}network-hdd на network-ssd или local-ssd{% else %}local-hdd на local-ssd{% endif %}? {#disk-type}
