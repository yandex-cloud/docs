...
Сообщение означает, что сервер принудительно завершил соединение. Одна из возможных причин — длительность активной сессии или транзакции превысила значение настройки [Session duration timeout](../../managed-postgresql/concepts/settings-list.md#setting-session-duration-timeout).
...
Ошибки возникают, если сумма значений настройки [Conn limit](../../managed-postgresql/concepts/settings-list.md#setting-conn-limit) для всех пользователей превышает значение настройки [Max connections](../../managed-postgresql/concepts/settings-list.md#setting-max-connections) за вычетом 15 служебных подключений.
...
Ошибка может возникнуть при попытке создать расширение с помощью SQL-команды или при восстановлении дампа, содержащего операции с расширениями. В {{ mpg-name }} права суперпользователя не предоставляются, а [управлять расширениями](../../managed-postgresql/operations/extensions/cluster-extensions.md#update-extensions) можно только с помощью интерфейсов {{ yandex-cloud }}.

Если ошибка возникла при восстановлении дампа:

1. Перед восстановлением [установите](../../managed-postgresql/operations/extensions/cluster-extensions.md#update-extensions) в базе-приемнике все необходимые поддерживаемые расширения.
...
Если команда `pg_restore` запущена без параметра `--exit-on-error`, после ошибки она продолжит восстановление остальных объектов. Проверьте результат восстановления и наличие нужных расширений в базе-приемнике. Подробнее в руководстве [{#T}](../../managed-postgresql/tutorials/data-migration.md#restore).
...
Подробнее о репликации читайте в [соответствующем разделе](../../managed-postgresql/concepts/replication.md).
...
Подробнее о способах подключения к хосту-мастеру читайте в разделе [Подключение к базе данных](../../managed-postgresql/operations/connect/fqdn.md#automatic-master-host-selection).
...
Ошибка возникает из-за того, что в {{ PG }} `18` изменились правила сортировки (collation), а база данных использует старые правила.
