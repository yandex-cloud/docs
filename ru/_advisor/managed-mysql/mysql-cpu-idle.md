...
Хост постоянно использует весь ресурс CPU, что может приводить к замедлению обработки запросов. Проверьте [нагрузку](../../managed-mysql/operations/performance-diagnostics.md) и оптимизируйте запросы (в том числе с [AI-ассистентом](../../websql/operations/ai-assistant.md) в {{ websql-name }}) или увеличьте вычислительные ресурсы кластера.
...
1. В разделе **{{ ui-key.yacloud.mdb.forms.section_host-resources }}** выберите класс хоста с нужным количеством vCPU.
