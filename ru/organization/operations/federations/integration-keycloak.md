...
1. Платформа {% if lang == "ru" %}[Docker](/blog/posts/2022/03/docker-containers){% else %}Docker{% endif %}. Если у вас не установлен Docker, [установите его](https://docs.docker.com/get-docker/). Убедитесь, что Docker Engine запущен.
...
Телефон | Используется для отправки уведомлений из сервисов {{yandex-cloud}}.<br>Пример: {% if product == "yandex-cloud" %}+71234567890{% endif %}{% if product == "cloud-il" %}+972571234567{% endif %}.<br> Ограничение значения по длине: {{saml-limit-phone}}. | `phone`
...
Значение аттрибута `thumbnailPhoto` превышающее ограничение будет игнорироваться, остальные аттрибуты будут сохраняться обрезанными до величины ограничения.
