...
С помощью [федерации удостоверений](../../add-federation.md) вы можете использовать [Azure Active Directory](https://azure.microsoft.com/ru-ru/services/active-directory/) (далее — Azure AD) для аутентификации в облаке.

Чтобы настроить аутентификацию:

1. [Начните создавать SAML-приложение](#configure-sso-azure-start).
...
1. [Завершите создание SAML-приложения](#configure-sso-azure-finish).

1. [Добавьте пользователей в SAML-приложение](#add-users-azure).

1. [Добавьте пользователей в организацию](#add-users-organization).
...
## Начните создавать SAML-приложение {#configure-sso-azure-start}

Получите сведения о поставщике удостоверений (IdP) — SAML-приложении в Azure AD:
...
На странице **Вход на основе SAML** отобразятся данные IdP-сервера. Не закрывайте страницу: информация понадобится при [создании федерации](#create-federation) и [добавлении сертификата](#add-certificate).
...
На странице **Вход на основе SAML** в Azure AD в разделе **3. Сертификат подписи SAML** скачайте сертификат (Base64). Добавьте сертификат в созданную федерацию:
...
Когда вы настроите аутентификацию с помощью федерации, пользователи смогут войти в консоль управления по ссылке, в которой содержится идентификатор федерации. Эту же ссылку необходимо будет указать при настройке IdP-сервера.
...
## Завершите создание SAML-приложения {#configure-sso-azure-finish}

Когда вы создали федерацию и получили ссылку для входа в консоль, завершите создание SAML-приложения в Azure AD:

1. Cнова откройте страницу настроек SAML-приложения **Вход на основе SAML**.
...
## Добавьте пользователей в SAML-приложение {#add-users-azure}

Добавьте пользователей на IdP-сервер в Azure AD:
...
## Добавьте пользователей в организацию {#add-users-organization}

Если при [создании федерации](#create-federation) вы не включили опцию **Автоматически создавать пользователей**, федеративных пользователей нужно добавить в организацию вручную.
...
Когда вы закончили настройку IdP-сервера, протестируйте, что все работает:

1. Откройте браузер в гостевом режиме или режиме инкогнито.

1. Перейдите по [ссылке для входа в консоль](#get-link), полученной ранее. Браузер должен перенаправить вас на страницу аутентификации Microsoft.
