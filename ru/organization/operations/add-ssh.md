...
Загрузите SSH-ключи в профиль пользователя организации в {{ org-name }}, чтобы с их помощью он мог подключаться к виртуальным машинам и узлам кластеров {{ managed-k8s-full-name }} с включенным доступом по [OS Login](../concepts/os-login.md).

Для добавления SSH-ключа в профиль пользователя убедитесь, что на уровне организации [включена](./os-login-access.md) опция **Доступ по OS Login при помощи SSH-ключей**. При необходимости [создайте](../../compute/operations/vm-connect/ssh.md#creating-ssh-keys) новую пару SSH-ключей.

Чтобы добавить SSH-ключ в профиль пользователя организации:
...
- {{ TF }} {#tf}
  ...
  1. Опишите в конфигурационном файле параметры ресурсов, которые необходимо создать:
      ...
      * `subject_id` — [идентификатор](./users-get.md) пользователя, в профиль которого добавляется SSH-ключ.
      * `data` — содержимое публичного SSH-ключа.
      * `name` — имя загружаемого ключа.
...
Чтобы пользователи могли самостоятельно загружать SSH-ключи в свои профили, [включите](./os-login-access.md) опцию **Разрешить пользователям загружать собственные SSH-ключи**.