...
Если в вашей компании есть система управления пользователями и доступом (например, {{ microsoft-idp.ad-short }} или Google Workspace), вы можете использовать ее для аутентификации сотрудников в {{ org-full-name }}. Тогда не нужно будет создавать новый аккаунт в Яндексе для каждого сотрудника компании, сотрудники смогут получить доступ к сервисам {{ yandex-cloud }} с помощью своих корпоративных аккаунтов.

## Настройка федераций в {{ org-full-name }} {#federation-usage}

С помощью федераций удостоверений вы можете настроить систему единого входа (Single Sign-On, SSO) и использовать корпоративные аккаунты для аутентификации в {{ org-full-name }}. В этом случае ваша корпоративная система управления учетными записями пользователей выступает в роли поставщика удостоверений (IdP — identity provider).

В {{ org-full-name }} можно создать федерацию удостоверений с любым сервисом управления учетными данными (поставщиком удостоверений), который поддерживает протокол [SAML]({{ link-saml }}).

Информация о логинах и паролях пользователей хранится у поставщика удостоверений. При входе в {{ org-full-name }} система направляет пользователя для аутентификации на сервер поставщика удостоверений (IdP). В [профиль](../operations/users-get.md#yandex-and-federated) федеративного пользователя в организации {{ org-full-name }} загружаются лишь те [атрибуты](../operations/setup-federation.md#claims-mapping), которые были заданы при настройке федерации удостоверений, и только после первой аутентификации этого пользователя в {{ yandex-cloud }}.
...
Для обеспечения дополнительной безопасности, вы можете включить подписывание запросов аутентификации электронной подписью. При этом вам потребуется дополнительно настроить отношения доверия между вашей федерацией удостоверений {{ org-full-name }} и вашим поставщиком удостоверений (IdP).

#### Настройка отношений доверия между федерацией и IdP {#build-trust}

Настройка отношений доверия между федерацией удостоверений в {{ org-full-name }} и поставщиком удостоверений включает два этапа:

* Настройка отношений доверия на стороне федерации удостоверений.

    Когда поставщик удостоверений сообщает {{ org-full-name }}, что пользователь прошел аутентификацию, он подписывает сообщение своим сертификатом. Чтобы сервис {{ org-full-name }} мог проверить подпись, скачайте и [добавьте](../operations/setup-federation.md#add-certificate-fed) этот сертификат в вашу федерацию удостоверений.
...
* Настройка отношений доверия на стороне IdP-сервера.
    ...
    Когда федерация удостоверений {{ org-full-name }} отправляет запрос поставщику удостоверений, она подписывает такие запросы SAML-сертификатом {{ yandex-cloud }}. Чтобы поставщик удостоверений мог проверить подпись, скачайте SAML-сертификат {{ yandex-cloud }} и добавьте его на ваш IdP-сервер.
...
Если вы используете группы пользователей в вашем поставщике удостоверений или собираетесь это сделать, [настройте сопоставление групп](../operations/federation-group-mapping.md) пользователей между поставщиком удостоверений и {{ org-full-name }}. Пользователи в группах поставщика удостоверений будут иметь права доступа к ресурсам {{ yandex-cloud }} из сопоставленных групп в {{ org-full-name }}.
