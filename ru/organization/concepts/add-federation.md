...
Информация о логинах и паролях пользователей хранится у поставщика удостоверений. При входе в {{ org-name }} система направляет пользователя для аутентификации на сервер поставщика удостоверений (IdP). В [профиль](../operations/users-get.md#yandex-and-federated) федеративного пользователя в организации {{ org-name }} загружаются лишь те [атрибуты](../operations/setup-federation.md#claims-mapping), которые были заданы при настройке федерации удостоверений, и только после первой аутентификации этого пользователя в {{ yandex-cloud }}.
