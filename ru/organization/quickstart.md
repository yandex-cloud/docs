...
Пользователь будет подключен к организации, как только примет приглашение по ссылке из письма и выберет подходящий аккаунт для входа.

Подробнее о пользователях читайте в разделе [Членство в организации](../organization/concepts/membership.md).
...
Чтобы назначить роли для управления отдельными сервисами {{ yandex-cloud }}, используйте сервис [{{ iam-full-name }}](../iam/concepts/index.md).

Чтобы назначить роль пользователю после отправки приглашения:
...
Если в вашей компании есть система управления пользователями и доступом (например, Active Directory или Google Workspace), вы можете использовать ее для аутентификации сотрудников в {{ org-full-name }}. Тогда сотрудники будут получать доступ к сервисам {{ yandex-cloud }} с помощью своих корпоративных аккаунтов.
...
Более того, {{ org-full-name }} может выступать в качестве поставщика удостоверений, которого вы можете использовать для аутентификации ваших пользователей в сторонних системах и сервисах с помощью [приложений](./concepts/applications/index.md).
...
Подробнее читайте в разделе [{{ oslogin }}](./concepts/os-login.md).
