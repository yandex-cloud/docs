...
1. Введите почтовые адреса {% if product == "yandex-cloud" %}пользователей Яндекса{% endif %}{% if product == "cloud-il" %}пользователей Google{% endif %}, которых вы хотите пригласить в организацию{% if product == "yandex-cloud" %} (например, `{{login-example}}`){% endif %}.

1. Нажмите кнопку **Добавить**. Пользователи будут подключены к организации.

Чтобы получить доступ к сервисам, которые подключены к организации, приглашенным пользователям будет достаточно войти в свой аккаунт{% if product == "yandex-cloud" %} на Яндексе{% endif %}{% if product == "cloud-il" %} в Google{% endif %}.
