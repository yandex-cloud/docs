...
Для выполнения запроса к API Yandex Message Queue необходимо выполнить POST запрос по пути  `/` на адрес `https://message-queue.cloud.yandex.net`.
...
Параметры передаются в теле запроса, в формате [form-urlencoded](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods/POST).

### Передача массивов параметров {#array-parameters}

Некоторые параметры сообщения, например, атрибуты сообщения или очереди, передаются в виде массивов. Каждый элемент массива передаётся в виде двух или более параметров запроса. Например, для атрибутов это имя и значение элемента массива.
