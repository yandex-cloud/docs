...
1. Выполните в командной строке:
   ...
   ```
   Скрипт установит программное обеспечение YC CLI и добавит путь до исполняемого файла в переменную окружения `PATH`. 
   > [!NOTE]
   >
...
1. Выполните в командной строке:
   ...
   curl [!KEYREF yc-install-path] | bash
   ```
   Скрипт автоматически установит программное обеспечение YC CLI и добавит путь до исполняемого файла в переменную окружения `PATH`. 

1. Установите пакет `bash-completion`:
   ```
   brew install bash-completion
   ```
   После завершения установки добавьте в файл ` ~/.bash_profile` строки:
   ```
