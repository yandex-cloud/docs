...
1. Скачайте исполняемый файл YC CLI для [Windows 64-bit]([!KEYREF yc-windows-path-amd64]) или [Winodws 32-bit]([!KEYREF yc-windows-path-386]) и сохраните на своем компьютере, например, в директории `C:\Yandex.Cloud`. 
...
Запустите команду `yc init`, чтобы выполнить базовую настройку YC CLI:
  ...
  2. Выберите одно из предложенных облаков, в которых у вас есть права доступа:
     ...
     ```
     Вы сможете изменить выбор облака позднее.
  3. Выберите каталог по умолчанию:
     ...
     ```
     Вы сможете изменить зону доступности по умолчанию позднее.
