...
1. Выполните в командной строке:
   ...
   ```
   Скрипт установит YC CLI и добавит путь до исполняемого файла в переменную окружения `PATH`.
    
   > [!NOTE]
   >
   > Скрипт дополнит переменную `PATH` только, если его запустить в командных оболочках `bash` или `zsh`.
   > 
   > Если вы запустили скрипт в другой оболочке, добавьте путь до YC CLI в переменную `PATH` самостоятельно.
...
YC CLI поддерживает автодополнение команд в командной оболочке `bash`. Чтобы автодополнение заработало:
...
1. Сформируйте ссылку, чтобы скачать актуальную версию YC CLI:
    * для Windows 64-bit — `https://[!KEYREF s3-storage-host][!KEYREF yc-windows-path-amd64]`
    * для Windows 32-bit — `https://[!KEYREF s3-storage-host][!KEYREF yc-windows-path-386]`
      
   Вместо `<current-version>` укажите версию YC CLI, опубликованную по [ссылке](https://[!KEYREF s3-storage-host]/yandexcloud-yc/release/stable).

1. Скачайте исполняемый файл YC CLI и сохраните на своем компьютере, например, в директории `C:\Yandex-Cloud`.
1. Добавить в переменную окружения **PATH** путь до директории с исполняемым файлом YC CLI:
...
## Инициализация {#initialize}
  ...
  1. Запустите команду `yc init`, чтобы выполнить настройку вашего профиля YC CLI.
