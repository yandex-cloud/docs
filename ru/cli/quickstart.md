---
title: Начало работы с интерфейсом командной строки
description: Следуя данной инструкции, вы научитесь работать с интерфейсом командной строки.
---

# Начало работы с интерфейсом командной строки {{ yandex-cloud }} CLI
...
Установите CLI на компьютер, с которого хотите управлять ресурсами {{ yandex-cloud }}. Для этого в терминале компьютера выполните команды, представленные ниже.

## Установка {#install}

{% include [interactive-install-cli-intro](../_includes/cli/interactive-install-cli-intro.md) %}

Чтобы _вручную_ настроить директорию установки CLI, автодополнение, скрипт автозапуска оболочки и добавить CLI в переменную окружения `PATH`, воспользуйтесь [неинтерактивной установкой CLI с помощью скрипта](./operations/install-cli.md#non-interactive).
...
1. Подключитесь к ВМ по SSH:
   ...
   1. Подключитесь к ВМ по SSH от имени пользователя `yc-user`, используя закрытый ключ и полученный публичный IP-адрес ВМ:
