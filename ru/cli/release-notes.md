...
Сервис {{ kms-full-name }} находится на стадии Preview. Подробнее про сервис читайте в [документации](https://cloud.yandex.ru/services/kms).

**Улучшено**
- Теперь при запуске YC CLI с флагом --debug, строка лога ответа API вызова содержит не только gRPC-код статуса, но и его описание.
