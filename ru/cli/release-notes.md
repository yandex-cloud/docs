...
* Добавлен флаг --impersonate-service-account-id string для имперсонации под сервисный акаунт.

### Изменения в сервисах {{ yandex-cloud }} {#services}

#### {{ cloud-logging-name }} {#cloud-logging}

* Исправлена работа `read` при указании флага `--follow`.

#### {{ managed-k8s-name }} {#k8s}

* Команда `yc k8s node-group create`.
  Добавлен флаг `--container-network-settings` для конфигурации контейнерной сети узлов кластера. Содержит единственный параметр:
  * `pod-mtu` — MTU для подов.
