...
editable: false
noIndex: true
sourcePath: en/_cli-ref-beta/cli-ref-beta/managed-ytsaurus/cli-ref-beta/cluster/update.md
---
...
|`--cluster-id`|<b>`string`</b><br/>ID of the cluster to update.|
|`--description`|<b>`string`</b><br/>New description for the cluster.|
|`--labels`|<b>`stringToString`</b><br/>Cluster labels as 'key:value' pairs.<br/><br/>Existing set of labels is completely replaced by the provided set, so if you just want<br/>to add or remove a label:<br/>1. Get the current set of labels with a [ClusterService.Get] request.<br/>2. Add or remove a label in this set.<br/>3. Send the new set in this field.|
|`--name`|<b>`string`</b><br/>New name for the cluster.<br/>The name must be unique within the folder.|
|`--security-group-ids`|<b>`strings`</b><br/>New security groups for the cluster.|
|`--spec`|<b>`shorthand/json`</b><br/>New cluster specification.<br/><br/>Example:|
...
|`--subnet-id`|<b>`string`</b><br/>New subnet for the cluster.|
