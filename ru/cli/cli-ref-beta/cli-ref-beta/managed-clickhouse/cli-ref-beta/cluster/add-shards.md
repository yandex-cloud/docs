...
ID of the ClickHouse cluster to add shards to. To get the ClickHouse cluster ID, use a [ClusterService.List] request. ||
...
Properties of each shard to be created. If [ShardSpec.config_spec] is not specified for some shard, for this shard it defaults to the configuration of the first shard in the cluster.
