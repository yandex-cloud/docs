...
| `--cloud-id` | `cloud-id` | {% if audience != "internal" %}[Идентификатор облака](../../resource-manager/operations/cloud/get-id.md){% else %}Идентификатор облака{% endif %}, который будет использован при выполнении команды. Применяется только в команде [yc resource-manager folder list](../cli-ref/managed-services/resource-manager/folder/list.md) или совместно с `folder-name`. В остальных случаях параметр игнорируется. |
...
| — | `service-account-key`| Авторизованный ключ {% if audience != "internal" %}[сервисного аккаунта](../../iam/concepts/users/service-accounts.md){% else %}сервисного аккаунта{% endif %} в {{ yandex-cloud }}. Устанавливается из файла, в котором хранится ключ. При установке данного параметра профиля обнуляется параметр `token`. |
...
| `--debug` | — | Вывести журнал отладки при выполнении команды. |
| `--debug-grpc` | — | Вывести журнал отладки gRPC при выполнении команды. Очень подробный, полезен для отладки проблем с подключением. |
| `--cloud-id` | `cloud-id` | [Идентификатор облака](../../resource-manager/operations/cloud/get-id.md), который будет использован при выполнении команды. Применяется только в команде [yc resource-manager folder list](../cli-ref/managed-services/resource-manager/folder/list.md) или совместно с `folder-name`. В остальных случаях параметр игнорируется. |
| `--folder-id` | `folder-id` | Идентификатор каталога, который будет использован при выполнении команды. При установке данного параметра профиля обнуляется параметр `folder-name`.|
| `--folder-name` | `folder-name` | Имя каталога, который будет использован при выполнении команды. При установке данного параметра профиля обнуляется параметр `folder-id`.|
| `--format` | `format` | Формат, в котором будет представлен вывод: `text` (по умолчанию), `yaml`, `json`, `json-rest`. |
| `-h`, `--help` | — | Вывести справку о команде. |
| `--no-user-output` | — | Исключить вывод, предназначенный пользователю, из потока вывода ошибок `stderr`. |
| `--profile` | — | Имя профиля, параметры которого будут использованы при выполнении команды. |
| `--endpoint` | `endpoint` | Доменное имя и порт, к которым CLI будет обращаться с запросами. Для корректной работы CLI с {{ yandex-cloud }} используйте значение `{{ api-host }}:443` либо в параметре профиля (см. [инструкцию](../operations/profile/profile-create.md#create)), либо в глобальном флаге при каждом запуске. |
| — | `service-account-key`| Авторизованный ключ [сервисного аккаунта](../../iam/concepts/users/service-accounts.md) в {{ yandex-cloud }}. Устанавливается из файла, в котором хранится ключ. |
