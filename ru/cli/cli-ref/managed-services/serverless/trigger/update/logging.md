---
editable: false
sourcePath: en/_cli-ref/cli-ref/managed-services/serverless/trigger/update/logging.md
---

# yc serverless trigger update logging
...
`yc serverless trigger update logging <TRIGGER-NAME> [Flags...] [Global Flags...]`
...
|`--new-log-group-id`|<b>`string`</b><br/>Log group identifier.|
|`--new-log-group-name`|<b>`string`</b><br/>Log group name.|
|`--new-resource-ids`|<b>`value[,value]`</b><br/>List of resources to subscribe.|
|`--new-resource-types`|<b>`value[,value]`</b><br/>List of resource types to subscribe.|
|`--new-stream-names`|<b>`value[,value]`</b><br/>List of stream names to subscribe.|
|`--new-log-levels`|<b>`value[,value]`</b><br/>List of log levels to subscribe.<br/>Available levels are: 'trace', 'debug', 'info', 'warn', 'error', 'fatal'.|
|`--new-batch-size`|<b>`int`</b><br/>Batch size, 1 - 100. Default: 1|
|`--new-batch-cutoff`|<b>`duration`</b><br/>Batch cutoff. 1 - 60s. Default: 1s.|
|`--new-invoke-function-id`|<b>`string`</b><br/>Function to be invoked by worker on the data from Logging.|
|`--new-invoke-function-name`|<b>`string`</b><br/>Function to be invoked by worker on the data from Logging.|
...
|`--new-invoke-container-id`|<b>`string`</b><br/>Container to be invoked by worker on the data from Logging.|
|`--new-invoke-container-name`|<b>`string`</b><br/>Container to be invoked by worker on the data from Logging.|
...
|`--new-gateway-id`|<b>`string`</b><br/>Gateway to which worker will broadcast the data from Logging|
|`--new-gateway-name`|<b>`string`</b><br/>Gateway to which worker will broadcast the data from Logging|
