...
* создать кластер в {{ ml-platform-name }}, используя специальный ресурс — шаблон {{ dataproc-name }};
* самостоятельно создать кластер в сервисе {{ dataproc-full-name }} и интегрировать его в проект {{ ml-platform-name }}.
