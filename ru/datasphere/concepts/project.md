...
_Проект_ — это основное рабочее место пользователя, которое запускается на виртуальной машине {{ yandex-cloud }}. Проект хранит в себе состояние интерпретатора, переменные, установленные пакеты и многое другое. Работа с проектом происходит в среде разработки [{{ jlab }}Lab](https://jupyter.org/). 
