...
_Проект_ — это основное рабочее место пользователя, которое запускается на {% if lang == "ru" and audience != "internal" %}[виртуальной машине](../../glossary/vm.md){% else %}виртуальной машине{% endif %} {{ yandex-cloud }}. Проект хранит в себе состояние интерпретатора, переменные, установленные пакеты и многое другое. Работа с проектом происходит в среде разработки [{{ jlab }}Lab](https://jupyter.org/). 
