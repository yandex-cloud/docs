...
{{ ml-platform-full-name }} — среда для ML-разработки полного цикла. {{ ml-platform-full-name }} является частью платформы данных и предоставляет широкие возможности для простого взаимодействия с сервисами {{ yandex-cloud }}.
...
Выбирайте необходимые вычислительные ресурсы под конкретные части кода в [режиме {{ ds }}](project.md#serverless). При [изменении конфигурации](configurations.md) все переменные и состояние интерпретатора будут перенесены на новую виртуальную машину. Сервис автоматически управляет выделением ресурсов. Если вы не будете производить вычисления, сервис не будет выделять ресурсы. 

Если же вы хотите использовать выделенную виртуальную машину, в [режиме {{ dd }}](project.md#dedicated) вы можете выбрать конфигурацию и получить управляемый сервис без необходимости настраивать ВМ. Выделенная конфигурация сохранится за вами до тех пор, пока вы специально не освободите вычислительные ресурсы.

### Сохранение состояния при завершении работы {#save-state}

Работая в режиме {{ ds }}, вам не нужно все время держать открытой вкладку с ноутбуком: запущенные вычисления продолжатся, даже если вы закроете проект, а [состояние интерпретатора](save-state.md), все переменные и результаты вычислений сохранятся. Вы сможете продолжить работу, когда снова откроете проект.
...
### Различные типы ресурсов {#resources}

Данные для подключения к любому объектному хранилищу больше не нужно каждый раз вводить заново. Для хранения всех сведений появился ресурс **{{ ui-key.yc-ui-datasphere.resources.s3 }}**, который позволяет [подключать и отключать](../operations/data/connect-to-s3.md) бакет прямо в интерфейсе {{ ml-platform-name }}.
