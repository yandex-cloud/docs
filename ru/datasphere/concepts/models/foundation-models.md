...
{{ ml-platform-full-name }} предоставляет возможность работать с фундаментальными моделями, чтобы вы могли использовать их для решения своих задач и при необходимости дообучать на своих данных. Дообучение производится по методу P-tuning, результаты дообучения хранятся в {{ ml-platform-name }}.
...
Чтобы дообучить модель {{ yagpt-name }} по методу P-tuning, вам нужно подготовить файл с формате JSON, содержащий примеры запросов и эталонных ответов:
