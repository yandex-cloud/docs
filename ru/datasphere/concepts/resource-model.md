...
* [Коннекторы к хранилищам S3](s3-connector.md) — сохраненные конфигурации для подключения [бакетов {{ objstorage-name }}](../../storage/concepts/bucket.md). Бакеты можно монтировать в файловую систему проекта, чтобы облегчить доступ к данным из кода. Как создать коннектор S3, смотрите [{#T}](../operations/data/s3-connectors.md).
