...
1. Секция `working-storage` определяет параметры рабочей директории. По умолчанию рабочая директория создается на системном диске, и ее размер составляет около 20 ГБ. Если для выполнения задания нужно больше места, вы можете создать вторую рабочую директорию размером от 100 ГБ до 10 ТБ. Тогда {{ ml-platform-name }} подключит ее к ВМ как дополнительный диск и удалит после завершения задачи.

   Дополнительное хранилище оплачивается согласно [правилам тарификации хранения данных](https://cloud.yandex.ru/ru/docs/datasphere/pricing#prices-jobs).
