...
{{ taas-name }} дает возможность быстро обучить модель, используя распределенное обучение <q>под ключ</q>. Чтобы использовать {{ taas }} в {{ ml-platform-name }}, не нужны специальные навыки: платформа поможет оптимально масштабировать обучение, написанное для одного GPU, на несколько ускорителей одной или нескольких [виртуальных машин](../../glossary/vm.md).
