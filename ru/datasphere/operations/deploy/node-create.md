...
Вы можете развернуть отдельную ячейку ноутбука или Docker-образ в виде самостоятельного сервиса, используя [ноды](../../concepts/resource-model.md#resources).
...
1. Закрепите контрольную точку:
    ...
    1. Выберите в списке последнюю добавленную контрольная точку с именем `Cell run` и убедитесь, что в ней содержится нужный код.
...
1. Создайте ноду:
    ...
    1. В зависимости от типа ноды задайте прочие параметры:
        ...
        - Ячейка
          ...
          * В блоках **Входные переменные** и **Выходные переменные** определите имена и типы выходных и входных переменных, на основе которых будет автоматически сгенерирован API. Вы можете добавить переменные, нажимая кнопку ![Add](../../../_assets/plus.svg) **Добавить**.
