# Настроить проект для вычислений на кластерах Apache Spark™

Чтобы начать вычисления на кластерах Apache Spark™, подготовьте проект {{ ml-platform-name }}. Для вычислений создайте новый кластер {{ dataproc-full-name }} или используйте уже существующий.
...
1. В верхней панели нажмите **File** и выберите **{{ dataproc-name }} Clusters**.
...
- Консоль управления
  ...
  1. Вставьте в поле **Публичный ключ** публичную часть вашего {% if lang == "ru" and audience != "internal" %}[SSH-ключа](../../glossary/ssh-keygen.md){% else %}SSH-ключа{% endif %}. Как сгенерировать и использовать SSH-ключи, читайте в [документации {{ compute-full-name }}](../../compute/operations/vm-connect/ssh.md).
...
1. Откройте проект, для которого вы создали кластер {{ dataproc-name }}.
1. В ячейку вставьте код для вычисления, например:
   ...
   Где `#!spark --cluster <имя кластера>` — обязательная системная команда для запуска вычислений на кластере. Посмотреть имя кластера можно в окне **{{ dataproc-name }} Clusters** в меню **File**.
