...
* Задайте сложный пароль аккаунту {% if product == "yandex-cloud" %}Яндекс ID{% endif %}{% if product == "cloud-il" %} Google{% endif %}, от имени которого создано облако. Используйте аккаунт {% if product == "yandex-cloud" %}Яндекс ID{% endif %}{% if product == "cloud-il" %} Google{% endif %} только в случае крайней необходимости, например, если федерация недоступна.
