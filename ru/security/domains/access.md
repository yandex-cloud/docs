...
{% endif %}
{% if product == "cloud-il" %}
* аккаунты Google — учетные записи в системе аутентификации Google;
{% endif %}
* [федеративные аккаунты](../../iam/concepts/#saml-federation) — учетные записи в корпоративной [SAML-совместимой федерации удостоверений](../../organization/add-federation.md), например Active Directory;
* [сервисные аккаунты](../../iam/concepts/#sa) — учетные записи, от имени которых программы могут управлять ресурсами.

{% if product == "yandex-cloud" %}Аккаунты на Яндексе{% endif %}{% if product == "cloud-il" %}Аккаунты Google{% endif %} и федеративные аккаунты аутентифицируются в собственных системах. {{ yandex-cloud }} не имеет доступа к паролям этих пользователей и аутентифицирует только сервисные аккаунты с помощью сервиса {{ iam-short-name }}.
...
[Инструкция по настройке SAML федерации удостоверений](../../organization/add-federation#federation-usage).
{% if product == "yandex-cloud" %}
...
Используйте федеративные аккаунты вместо аккаунтов {% if product == "yandex-cloud" %}Яндекс ID{% endif %}{% if product == "cloud-il" %}Google{% endif %}, где это возможно.
...
{% if product == "yandex-cloud" %}
Для аккаунта Яндекс ID настройте 2FA согласно [инструкции]{% if lang == "ru" %}(https://yandex.ru/support/id/authorization/twofa.html){% endif %}{% if lang == "en" %}(https://yandex.com/support/id/authorization/twofa.html){% endif %}.
{% endif %}
{% if product == "cloud-il" %}
Для аккаунта Google настройте 2FA согласно [инструкции](https://support.google.com/accounts/answer/185839?ref_topic=2954345l).
{% endif %}
...
* На время активного использования данного аккаунта включите двухфакторную аутентификацию (2FA) в {% if product == "yandex-cloud" %}Яндекс ID{% endif %}{% if product == "cloud-il" %}аккаунте Google{% endif %}.
...
Для управления платежным аккаунтом назначьте роль `{{ roles-admin }}` или `{{ roles-editor }}` на платежный аккаунт выделенному сотруднику организации с федеративным аккаунтом. Для просмотра данных биллинга назначьте роль `{{ roles-viewer }}` на платежный аккаунт выделенному сотруднику организации с федеративным аккаунтом.
...
Назначьте роль `{{ roles-cloud-owner }}` одному или нескольким сотрудникам организации с федеративным аккаунтом. Аккаунту {% if product == "yandex-cloud" %}Яндекс ID{% endif %}{% if product == "cloud-il" %} Google{% endif %}, с которым создано облако, назначьте сложный пароль и используйте только в случае крайней необходимости (например, при поломке федерации).

Роль `{{ roles-cloud-owner }}` автоматически выдается при создании облака. Пользователь с этой ролью может выполнять любые операции с облаком и ресурсами в нем, а также выдавать доступ к облаку другим пользователям: назначать роли и отзывать их.
Назначьте роль `{{ roles-cloud-owner }}` одному или нескольким сотрудникам организации с федеративным аккаунтом. Аккаунту Google, с которым создано облако, назначьте сложный пароль и используйте только в случае крайней необходимости (например, при поломке федерации).
