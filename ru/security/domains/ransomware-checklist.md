...
## Включено логирование действий с бакетами {{ objstorage-name }} {#enable-logging}


Помимо {{ at-name }}, который фиксирует управляющие события, рекомендуется включить [логирование](../../storage/operations/buckets/enable-logging) доступа к объектам в бакетах {{ objstorage-name }}. Это позволяет отслеживать операции чтения и записи объектов (`GET`, `PUT`, `DELETE`), что критично для расследования инцидентов с шифровальщиками, атакующими хранилища данных. Логи доступа следует направлять в отдельный защищенный бакет.
