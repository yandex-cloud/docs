...
* **Резервное копирование**: настройте резервное копирование кластера {{ k8s }} в {{ objstorage-name }}, см. [руководство](../../managed-kubernetes/tutorials/backup.md). Следуйте рекомендациям раздела [Безопасная конфигурация {{ objstorage-name }}](../standard/virtualenv-safe-config.md#objstorage).
