...
В рамках {% if lang == "ru" %}[IaaS-сервисов](https://cloud.yandex.ru/blog/posts/2022/01/iaas){% else %}IaaS-сервисов{% endif %} клиент несет ответственность за конфигурацию своих ресурсов.
...
* При включенном доступе к серийной консоли по {% if lang == "ru" and audience != "internal" %}[SSH](../../glossary/ssh-keygen.md){% else %}SSH{% endif %} следует убедиться, что работа с учетными данными и пароль для локального входа в операционную систему соответствуют стандартам регуляторов. Например, в инфраструктуре для хранения данных платежных карт пароль должен соответствовать требованиям стандарта {% if product == "yandex-cloud" %}[PCI DSS](/security/standards/pci){% else %}PCI DSS{% endif %}: содержать как буквы, так и цифры, иметь длину не менее 7 символов и меняться не реже чем каждые 90 дней.
...
* Устанавливать обновления операционной системы и ядра, которые обеспечивают защиту от атак с использованием побочных каналов (например, [Kernel page-table isolation для Linux](https://en.wikipedia.org/wiki/Kernel_page-table_isolation), приложения, собранные с [Retpoline]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/Spectre_(уязвимость)){% endif %}{% if lang == "en" %}(https://en.wikipedia.org/wiki/Spectre_%28security_vulnerability%29){% endif %}).
...
Дополнительно возможен {% if lang == "ru" and audience != "internal" %}[анализ](../../glossary/data-analytics.md){% else %}анализ{% endif %} логов {{ objstorage-name }} при помощи {{ datalens-full-name }}.
...
Не следует без необходимости включать доступ к базам данных с критичными данными из консоли управления{% if product == "yandex-cloud" %}, {{ datalens-name }}{% endif %} и других сервисов. Доступ к БД из консоли управления может потребоваться для отправки SQL-запросов в БД и визуализации структуры данных{% if product == "yandex-cloud" %}; доступ из {{ datalens-name }} может потребоваться для анализа и визуализации данных. Эти доступы осуществляются {% endif %}{% if product == "cloud-il" %}Этот доступ осуществляется{% endif %} через служебную сеть {{ yandex-cloud }}, с аутентификацией и использованием шифрования TLS. Включить и отключить {% if product == "yandex-cloud" %}доступы из консоли управления, {{ datalens-name }}{% endif %}{% if product == "cloud-il" %}доступ из консоли управления{% endif %} или других сервисов можно в настройках кластера или при его создании, в блоке дополнительных настроек.
...
## {{ container-registry-full-name }}{% if product == "yandex-cloud" %} и {{ cos-full-name }} {% endif %}{#container-registry-solution}
...
{% if product == "yandex-cloud" %}Рекомендуется использовать [сканер уязвимостей](../../container-registry/concepts/vulnerability-scanner.md) в образах, встроенный в {{ container-registry-full-name }}.{% endif %}

## {{ certificate-manager-full-name }} {#cert-manager}

Сервис {{ certificate-manager-name }} позволяет управлять TLS-сертификатами{% if product == "yandex-cloud" %} для API-шлюзов сервиса {{ api-gw-name }}, а также{% endif %} для сайтов и бакетов в {{ objstorage-name }}. Сервис {{ alb-full-name }} интегрирован с {{ certificate-manager-name }} для хранения и установки сертификатов. Рекомендуется использовать {{ certificate-manager-name }} для получения и автоматической ротации сертификатов.
