...
&#x2713; **Защитите аккаунты на Яндексе**:
   ...
   * Включите [двухфакторную аутентификацию](https://yandex.ru/support/passport/authorization/twofa.html) для своего [Яндекс ID](../../iam/concepts/users/accounts.md#passport) и аккаунтов пользователей организации.
   * Храните [OAuth-токен](../../iam/concepts/authorization/oauth-token.md) в секрете. Если токен скомпрометирован, [отзовите его](https://yandex.ru/dev/oauth/doc/dg/reference/token-invalidate-docpage/) и выпустите новый. По возможности используйте IAM-токен: он действует {{ iam-token-lifetime }}, а OAuth-токен — один год.
...
&#x2713; **Настройте SSO для внешних систем**: используйте [приложения в {{ org-full-name }}](../../organization/concepts/applications.md), если {{ yandex-cloud }} должен выступать как поставщик удостоверений (IdP) для внешних сервисов.
...
&#x2713; **Назначайте `{{ roles-auditor }}` там, где не нужен доступ к данным**: эта роль подходит, например, внешним подрядчикам и аудиторам и помогает соблюдать принцип минимальных привилегий.
...
&#x2713; **Регулярно проводите аудит доступов**: проверяйте права пользователей и сервисных аккаунтов с помощью [модуля {{ ciem-name }}](../../security-deck/concepts/ciem.md) или [{{ yandex-cloud }} CLI](../../cli/), отзывайте избыточные роли и блокируйте либо удаляйте неиспользуемые учетные записи старше 30 дней.
...
&#x2713; **Используйте IAM-токены**: [IAM-токен](../../iam/concepts/authorization/iam-token.md) действует {{ iam-token-lifetime }}, поэтому безопаснее долгоживущих ключей. Скомпрометированный или ненужный токен [отзовите](../../iam/operations/iam-token/revoke-iam-token.md). О выпуске токена читайте в [инструкции](../../iam/operations/iam-token/create-for-sa.md).

&#x2713; **Контролируйте использование ключей**: на странице сервисного аккаунта в [консоли управления]({{ link-console-main }}) отслеживайте дату и время последнего использования ключей и удаляйте неиспользуемые.
...
&#x2713; **Отслеживайте секреты {{ yandex-cloud }} в открытых источниках**: сервис позволяет обнаруживать API-ключи, {{ iam-short-name }} Cookies, IAM-токены, статические ключи доступа, OAuth-токены и серверные ключи {{ captcha-full-name }}. [Подробнее](../operations/search-secrets.md).

&#x2713; **Отзывайте секреты, попавшие в открытый доступ**: отзывайте и перевыпускайте их, проверяйте несанкционированные действия, удаляйте лишние ресурсы и сообщайте об инциденте в [службу технической поддержки]({{ link-console-support }}). [Подробнее](../../iam/operations/compromised-credentials.md).

&#x2713; **Храните ключи и токены в {{ lockbox-name }}**: сохраняйте их в [секретах {{ lockbox-name }}](../../lockbox/tutorials/static-key-in-lockbox/index.md) и используйте полезную нагрузку секретов при обращении к ним.

&#x2713; **Используйте {{ oslogin }} для централизованного SSH-доступа**: [{{ oslogin }}](../../organization/concepts/os-login.md) позволяет управлять доступом к ВМ через {{ iam-full-name }} и хранить SSH-ключи в профилях {{ org-full-name }}. Чтобы использовать сервис, [включите доступ](../../organization/operations/os-login-access.md) на уровне организации.
