...
Все действия внутри узла Kubernetes являются ответственностью пользователя. Пользователь несет ответственность за безопасность узлов и их корректную настройку в соответствии с требованиями PCI DSS и другихстандартов безопасности.
...
При работе с сервисом {{ managed-k8s-short-name }} для выполнения требований PCI DSS и другихстандартов безопасности запрещается:
...
Интеграция {{ lockbox-name }} с Kubernetes выполнена с помощью открытого проекта [External Secrets](https://external-secrets.io/latest/). Решение доступно в [{{ marketplace-name }} для Kubernetes](/marketplace/products/yc/external-secrets) в базовом упрощенном сценарии.
...
[Описано](https://external-secrets.io/latest/guides-multi-tenancy/#eso-as-a-service) множество способ разделения доступа к секретам в рамках данного инструмента.
