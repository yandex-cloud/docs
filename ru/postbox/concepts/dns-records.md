...
* Простая настройка (Easy DKIM) — {{ postbox-name }} автоматически генерирует ключи DKIM и управляет их ротацией. После создания [адреса](glossary.md#address) в консоли управления отобразятся две CNAME-записи, которые нужно добавить в DNS-зону домена. Эти записи указывают на публичные ключи, которыми управляет сервис. Подробнее в разделе [{#T}](#key-rotation).
...
При простой настройке DKIM-подписи {{ postbox-name }} периодически заменяет пары DKIM-ключей на новые. Ротация выполняется автоматически: управлять расписанием или запускать ее вручную нельзя.

Чтобы {{ postbox-name }} мог заменять ключи, пользователь должен добавить две CNAME-записи в свой DNS-провайдер. Каждая из них делегирует сервису право публиковать публичный ключ. Благодаря этому {{ postbox-name }} заменяет ключи на своей стороне, и пользователю не нужно править CNAME-записи в DNS-провайдере.
