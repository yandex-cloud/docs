...
* Простая настройка (Easy DKIM) — {{ postbox-name }} автоматически генерирует ключи DKIM и управляет их ротацией. После создания [адреса](glossary.md#adress) в консоли управления отобразятся две CNAME-записи, которые нужно добавить в DNS-зону домена. Эти записи указывают на публичные ключи, которыми управляет сервис. Подробнее в разделе [{#T}](#key-rotation).
