...
Для отслеживания открытия писем {{ postbox-name }} использует технологию отслеживающего пикселя (`tracking pixel`). При включенном отслеживании сервис автоматически добавляет в первую HTML-часть письма невидимый элемент изображения размером 1×1 пиксель.
...
Если отслеживание открытия писем выключено, {{ postbox-name }} не обрабатывает маркеры `{{ses:openTracker}}` и оставляет их в HTML-части письма без изменений.
...
Описание формата уведомлений об открытии письма приведено в разделе [{#T}](notification.md#open).
