---
title: Управление пользовательским стоп-листом в {{ postbox-full-name }}
description: Следуя данной инструкции, вы сможете добавить адрес в пользовательский стоп-лист, посмотреть список адресов и удалить адрес из него.
---

# Управление пользовательским стоп-листом

В [пользовательском стоп-листе](../concepts/suppression-list.md#user) находятся адреса получателей, которым {{ postbox-name }} не отправляет письма, если причина, по которой адрес добавлен в список, [блокирует отправку](../concepts/suppression-list.md#reasons). Список создается для облака и является общим для всех каталогов, адресов и конфигураций внутри него.
...
Чтобы {{ postbox-name }} проверял пользовательский стоп-лист при отправке писем, [задайте](create-configuration.md) настройки стоп-листа в конфигурации.
...
- Консоль управления {#console}
    ...
    1. На панели слева выберите **{{ ui-key.yacloud.postbox.label_suppressed-destinations }}**.
    1. Нажмите **{{ ui-key.yacloud.common.add }}**.
    1. В поле **{{ ui-key.yacloud.postbox.Postbox.SuppressedDestinations.email-address_h3moC }}** укажите адрес получателя, который необходимо добавить в пользовательский стоп-лист.
    1. В поле **{{ ui-key.yacloud.postbox.Postbox.SuppressedDestinations.reason_wp4hJ }}** выберите [причину](../concepts/suppression-list.md#reasons), по которой адрес получателя необходимо добавить в пользовательский стоп-лист.
