...
В этом сценарии описано, как настроить автоматическое [сканирование](../concepts/vulnerability-scanner.md) Docker-образа на наличие уязвимостей при загрузке.

{% note warning %}

Сервис сканирования находится на стадии [Preview](../../overview/concepts/launch-stages.md). Для получения доступа к нему обратитесь в [техническую поддержку](https://support.cloud.yandex.ru/).
...
1. Создайте [сервисный аккаунт](../../iam/concepts/users/service-accounts.md) с именем `editor` и назначьте ему роль `editor` на каталог, в котором создали реестр.
    ...
    - CLI 
      ...
      1. Назначьте роль сервисному аккаунту:
          ...
          * `<идентификатор каталога>` — идентификатор каталога, на который назначается роль.
          * `<идентификатор сервисного аккаунта>` — идентификатор сервисного аккаунта, которому назначается роль.
...
Создайте [функцию](../../functions/concepts/function.md) с именем `scan-on-push`, которая будет запускать сканирование образа:
