...
Чтобы разрешить доступ к ресурсам сервиса Container Registry (реестры и Docker-образы),
...
Ниже перечислены все роли, которые учитываются при проверке прав доступа в сервисе Container Registry.
...
Пользователь с ролью `viewer` может просматривать информацию о ресурсах, например посмотреть список реестров или скачать Docker-образ.

#### editor

Пользователь с ролью `editor` может управлять Docker-образами, например, загрузить или удалить.

Помимо этого роль `editor` включает в себя все разрешения роли `viewer`.

#### admin

Пользователь с ролью `admin` может управлять правами доступа к ресурсам, например разрешить другим пользователям просматривать реестры или работать с Docker-образами.
