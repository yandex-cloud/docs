...
* `Error response from daemon: unauthorized: Authentication problem ; requestId = <идентификатор_запроса>`
  ...
  **Почему возникла ошибка**:
  ...
  - также ошибка может быть связана с безуспешными попытками Docker вызвать [docker-credential-yc](../operations/authentication.md#cred-helper). Например, из-за отсутствия пути к исполняемому файлу в переменной окружения PATH.
