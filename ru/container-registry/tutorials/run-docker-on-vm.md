...
В этом руководстве описаны шаги, необходимые для запуска {% if lang == "ru" %}[Docker-образа](https://cloud.yandex.ru/blog/posts/2022/03/docker-containers){% else %}Docker-образа{% endif %} на ВМ с использованием реестра {{ cos-full-name }}.
...
1. Аутентифицируйтесь от своего имени:
   ...
   - С помощью IAM-токена
     ...
     1. [Получите]{% if product == "yandex-cloud" %}(../../iam/operations/iam-token/create.md){% endif %}{% if product == "cloud-il" %}(../../iam/operations/iam-token/create-for-federation.md){% endif %} IAM-токен.
     1. Выполните команду:
