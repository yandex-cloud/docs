...
Репозиторий определяется комбинацией `<ID реестра>/<имя Docker-образа>`.
...
- В командах Docker CLI необходимо использовать полное имя, включающее в себя адрес {{ container-registry-short-name }}: 
    ...
    ```
- В командах  YC CLI необходимо использовать имя репозитория без адреса {{ container-registry-short-name }}: 
