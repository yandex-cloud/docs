...
Digest'ы Docker-образов (идентификаторы манифестов вида `sha256:...`) отображаются в столбце **TAGS** как значения вида `sha256:...`. Чтобы удалить такие объекты, укажите в правиле одновременно параметры `untagged: true` и `tag_regexp: sha256.*`: параметр `tag_regexp` отбирает digest'ы по регулярному выражению, а `untagged: true` гарантирует, что удаляется именно образ без «настоящего» тега.
...
Правило `Digests` удаляет digest'ы образов (значения вида `sha256:...`), которые соответствуют следующим условиям:
* Без «настоящего» тега.
* Старше 48 часов.
...
* `tag_regexp` — тег Docker-образа для фильтрации. Регулярное выражение `test.*` для `tag_regexp` позволяет получить все образы с тегами, начинающимися на `test`. Регулярное выражение `sha256.*` позволяет получить digest'ы образов, которые отображаются в столбце **TAGS** как значения вида `sha256:...`.
