...
master | **[Master](../cluster.proto#Master)**<br>Свойства мастера для кластера Kubernetes. 
ip_allocation_policy | **[IPAllocationPolicy](../cluster.proto#IPAllocationPolicy)**<br>Политика распределения IP-адресов для служб и модулей внутри кластера Kubernetes в разных зонах доступности. 
...
&nbsp;&nbsp;zonal_master | **[ZonalMaster](../cluster.proto#ZonalMaster)**<br>Параметры зоны доступности мастера. 
&nbsp;&nbsp;regional_master | **[RegionalMaster](../cluster.proto#RegionalMaster)**<br>Параметры региона для мастера. 
version | **string**<br>Версия компонентов Kubernetes, которая запущена на мастере. 
endpoints | **[MasterEndpoints](../cluster.proto#MasterEndpoints)**<br>Эндпойнт мастера. Эндпойнты состоят из схемы и порта (т. е. `https://ip-address:port`) и могут использоваться клиентами для связи с API Kubernetes кластера Kubernetes. 
master_auth | **[MasterAuth](../cluster.proto#MasterAuth)**<br>Параметры, используемые для аутентификации мастера. 
...
maintenance_policy | **[MasterMaintenancePolicy](../cluster.proto#MasterMaintenancePolicy)**<br>Политика обновления мастера. 
...
health | enum **Health**<br>Состояние кластера Kubernetes. <ul><li>`HEALTHY`: Кластер Kubernetes работает нормально.</li><li>`UNHEALTHY`: Кластер Kubernetes не работает и не может выполнять свои основные функции.</li><ul/>
network_id | **string**<br>Идентификатор облачной сети, к которой принадлежит кластер Kubernetes. 
master | **[Master](../cluster.proto#Master1)**<br>Свойства мастера для кластера Kubernetes. 
ip_allocation_policy | **[IPAllocationPolicy](../cluster.proto#IPAllocationPolicy1)**<br>Политика распределения IP-адресов для служб и модулей внутри кластера Kubernetes в разных зонах доступности. 
internet_gateway | **oneof:** `gateway_ipv4_address`<br>
&nbsp;&nbsp;gateway_ipv4_address | **string**<br>Адрес шлюза IPv4. Максимальная длина строки в символах — 15.
service_account_id | **string**<br>Сервисный аккаунт, используемый для выделения Compute Cloud и VPC ресурсов для кластера Kubernetes. 
node_service_account_id | **string**<br>Сервисный аккаунт, используемый узлами кластера Kubernetes для доступа к Container Registry или для загрузки логов и метрик узла. 
...
--- | ---
master_type | **oneof:** `zonal_master` или `regional_master`<br>
&nbsp;&nbsp;zonal_master | **[ZonalMaster](../cluster.proto#ZonalMaster1)**<br>Параметры зоны доступности мастера. 
&nbsp;&nbsp;regional_master | **[RegionalMaster](../cluster.proto#RegionalMaster1)**<br>Параметры региона для мастера. 
version | **string**<br>Версия компонентов Kubernetes, которая запущена на мастере. 
endpoints | **[MasterEndpoints](../cluster.proto#MasterEndpoints1)**<br>Эндпойнт мастера. Эндпойнты состоят из схемы и порта (т. е. `https://ip-address:port`) и могут использоваться клиентами для связи с API Kubernetes кластера Kubernetes. 
master_auth | **[MasterAuth](../cluster.proto#MasterAuth1)**<br>Параметры, используемые для аутентификации мастера. 
...
version | **string**<br>Версия компонентов Kubernetes, которая запущена на мастере. 
maintenance_policy | **[MasterMaintenancePolicy](../cluster.proto#MasterMaintenancePolicy2)**<br>Политика обновления мастера. 
...
health | enum **Health**<br>Состояние кластера Kubernetes. <ul><li>`HEALTHY`: Кластер Kubernetes работает нормально.</li><li>`UNHEALTHY`: Кластер Kubernetes не работает и не может выполнять свои основные функции.</li><ul/>
network_id | **string**<br>Идентификатор облачной сети, к которой принадлежит кластер Kubernetes. 
master | **[Master](../cluster.proto#Master2)**<br>Свойства мастера для кластера Kubernetes. 
ip_allocation_policy | **[IPAllocationPolicy](../cluster.proto#IPAllocationPolicy3)**<br>Политика распределения IP-адресов для служб и модулей внутри кластера Kubernetes в разных зонах доступности. 
internet_gateway | **oneof:** `gateway_ipv4_address`<br>
&nbsp;&nbsp;gateway_ipv4_address | **string**<br>Адрес шлюза IPv4. Максимальная длина строки в символах — 15.
service_account_id | **string**<br>Сервисный аккаунт, используемый для выделения Compute Cloud и VPC ресурсов для кластера Kubernetes. 
node_service_account_id | **string**<br>Сервисный аккаунт, используемый узлами кластера Kubernetes для доступа к Container Registry или для загрузки логов и метрик узла. 
...
Поле | Описание
--- | ---
version | **[UpdateVersionSpec](../version.proto#UpdateVersionSpec)**<br>Спецификация обновления мастера. 
maintenance_policy | **[MasterMaintenancePolicy](../cluster.proto#MasterMaintenancePolicy3)**<br>Политика обновления мастера. 
...
--- | ---
policy | **oneof:** `anytime`, `daily_maintenance_window` или `weekly_maintenance_window`<br>Политика обновления.
&nbsp;&nbsp;anytime | **[AnytimeMaintenanceWindow](../maintenance.proto#AnytimeMaintenanceWindow4)**<br>Обновление мастера в любое время. 
&nbsp;&nbsp;daily_maintenance_window | **[DailyMaintenanceWindow](../maintenance.proto#DailyMaintenanceWindow4)**<br>Обновление мастера в любой день в течение указанного временного окна. 
