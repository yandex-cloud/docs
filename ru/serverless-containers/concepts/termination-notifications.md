...
В некоторых случаях сервис может принудительно остановить [экземпляр контейнера](./container.md#container-instance). При этом в пользовательское приложение поступает _уведомление о предстоящем принудительном завершении работы_. 
...
Этот тип уведомления направляется в пользовательское приложение при соблюдении следующих условий:

* контейнер является [долгоживущим](./long-lived-containers.md);
* до истечения таймаута, заданного в настройках ревизии контейнера, остается более десяти минут;
* экземпляр контейнера в текущий момент активен, то есть обрабатывает как минимум один вызов.

Уведомление о предстоящей принудительной остановке экземпляра контейнера отправляется в пользовательское приложение за десять минут до момента принудительного завершения работы экземпляра. В качестве уведомления отправляется [POSIX-сигнал](https://man7.org/linux/man-pages/man7/signal.7.html) `Termination signal` — [SIGTERM](https://ru.wikipedia.org/wiki/SIGTERM).
...
Этот тип уведомления направляется в пользовательское приложение при условии, что экземпляр контейнера неактивен, то есть в текущий момент не обрабатывает ни одного вызова.

При принятии решения о принудительном завершении работы неактивного экземпляра контейнера в пользовательское приложение сначала направляется POSIX-сигнал `SIGTERM`, а затем (спустя 50 миллисекунд) — POSIX-сигнал `SIGKILL`, который принудительно остановит процесс.
...
Отправка уведомлений в пользовательское приложение неактивного экземпляра контейнера — безусловная. Она не зависит от заданного в ревизии контейнера значения таймаута. Отключить отправку таких уведомлений или изменить время ожидания нельзя.