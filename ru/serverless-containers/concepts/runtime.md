...
Приложение должно принимать HTTP-запросы на порт, переданный в переменной окружения `PORT`. После получения запроса экземпляр контейнера должен отправить ответ в течение времени, которое указано в поле **Таймаут** в настройках ревизии контейнера. Оно включает в себя время запуска экземпляра контейнера. В противном случае запрос завершается и возвращается ошибка 504.
