...
_Долгоживущий контейнер_ — это контейнер, в настройках [ревизии](./container.md#revision) которого задан таймаут, превышающий десять минут. Максимально возможное значение таймаута указано в разделе [{#T}](./limits.md#serverless-containers-limits).
...
В некоторых случаях сервис может принудительно остановить [экземпляр](./container.md#container-instance) долгоживущего контейнера, обрабатывающий вызов, до истечения таймаута, заданного в настройках ревизии контейнера.

В таких случаях в пользовательское приложение поступит [уведомление](./termination-notifications.md#notify-when-active) о предстоящей принудительной остановке экземпляра контейнера. Уведомление направляется за десять минут до фактического принудительного завершения работы экземпляра.
...
Если до истечения таймаута, заданного в настройках ревизии контейнера, остается менее десяти минут, уведомление о принудительной остановке экземпляра контейнера в пользовательское приложение не направляется.
