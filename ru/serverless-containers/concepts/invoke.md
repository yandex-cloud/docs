...
При вызове контейнера c помощью триггера {{ sf-full-name }} JSON-описание события триггера передается в теле HTTP-запроса к контейнеру. Подробнее о [триггерах](trigger/index.md).
