...
Отзывает разрешения у указанного пользователя ClickHouse.
 

 
## HTTP-запрос
`POST /managed-clickhouse/v1/clusters/{clusterId}/users/{userName}:revokePermission`
...
Поле | Описание
--- | ---
databaseName | **string**<br>Имя базы данных, к которой пользователь должен потерять доступ.  Длина строки в символах должна быть от 1 до 63. Значение должно соответствовать регулярному выражению `` [a-zA-Z0-9_]+ ``.
