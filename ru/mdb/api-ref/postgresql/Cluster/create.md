...
Поле | Описание
--- | ---
folderId | **string**<br>Обязательное поле. Идентификатор каталога, в котором нужно создать кластер PostgreSQL.  Максимальная длина — 50 символов.
...
description | **string**<br>Описание кластера PostgreSQL. Длина описания должна быть от 0 до 256 символов.  Максимальная длина — 256 символов.
...
databaseSpecs | **object**<br>Обязательное поле. Описания баз данных, которые нужно создать в кластере PostgreSQL.
databaseSpecs.<br>name | **string**<br>Обязательное поле. Имя базы данных PostgreSQL. Длина имени должна быть от 1 до 63 символов.  Длина строки в символах должна быть от 1 до 63. Значение должно соответствовать регулярному выражению `` [a-zA-Z0-9_]+ ``.
databaseSpecs.<br>owner | **string**<br>Обязательное поле. Имя пользователя, которого нужно назначить владельцем базы данных. Чтобы получить список доступных пользователей PostgreSQL, сделайте запрос [list](/docs/mdb/api-ref/postgresql/User/list).  Длина строки в символах должна быть от 1 до 63. Значение должно соответствовать регулярному выражению `` [a-zA-Z0-9_]+ ``.
databaseSpecs.<br>lcCollate | **string**<br>POSIX-локаль для порядка сортировки строк. Может быть установлена только при создании.  Значение должно соответствовать регулярному выражению `[a-zA-Z_]+.UTF-8|C`.
databaseSpecs.<br>lcCtype | **string**<br>POSIX-локаль для классификации символов. Может быть установлена только при создании.  Значение должно соответствовать регулярному выражению `[a-zA-Z_]+.UTF-8|C`.
databaseSpecs.<br>extensions | **object**<br>Расширения PostgreSQL, которые следует подключить для базы данных.
databaseSpecs.<br>extensions.<br>name | **string**<br>Имя расширения, например `pg_trgm` или `pg_btree`. Расширения, поддерживаемые MDB, приведены в [документации сервиса](/docs/mdb/concepts).
databaseSpecs.<br>extensions.<br>version | **string**<br>Версия расширения.
userSpecs | **object**<br>Обязательное поле. Описания пользователей базы данных, которых нужно создать в кластере PostgreSQL.
userSpecs.<br>name | **string**<br>Обязательное поле. Имя пользователя PostgreSQL.  Длина строки в символах должна быть от 1 до 63. Значение должно соответствовать регулярному выражению `` [a-zA-Z0-9_]+ ``.
userSpecs.<br>password | **string**<br>Обязательное поле. Пароль пользователя PostgreSQL.  Длина строки в символах должна быть от 8 до 128.
...
userSpecs.<br>connLimit | **integer** (int64)<br>Количество подключений к базе данных, которые должны быть доступны пользователю.  Минимальная значение — 10.
hostSpecs | **object**<br>Обязательное поле. Конфигурации для отдельных хостов, которые должны быть созданы для кластера PostgreSQL.
