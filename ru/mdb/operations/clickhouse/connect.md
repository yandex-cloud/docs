...
Чтобы подключиться к кластеру с помощью клиента командной строки, укажите путь к SSL-сертификату в [конфигурационном файле](https://clickhouse.yandex/docs/en/interfaces/cli/#interfaces_cli_configuration), в элементе `<caConfig>`:
```xml
<config>
  <openSSL>
    <client>
