...
| 4 | 504 | `DEADLINE_EXCEEDED` | Превышен срок выполнения запроса | Проблемы в сети между клиентом и сервером либо превышен таймаут ожидания ответа. Попробуйте отправить запрос повторно. При использовании {{ ml-sdk-name }} увеличьте таймаут с помощью [метода](../sdk-ref/sync/chat/completions.md#yandex_ai_studio_sdk._chat.completions.model.ChatModel) `config.run(timeout)`. Если проблема сохраняется, обратитесь в [техническую поддержку](../../support/overview.md) |
