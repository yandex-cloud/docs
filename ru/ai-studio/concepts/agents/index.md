...
* _{{ retrieval-tool-name }} Tool_ – реализует сценарии RAG и позволяет AI-агенту искать информацию для ответа в ваших файлах (базе знаний). Вы можете загрузить документы базы знаний в консоли управления, через {{ vector-store-name }} API или через [Files API](../assistant/files.md) и создать поисковый индекс. [Поисковые индексы](../search/vectorstore.md) хранят информацию из документов в векторном виде и позволяют агентам использовать ее при ответе.
...
* _File Search Tool_ — позволяет агенту искать информацию в пользовательских файлах. Подробнее про использование [инструмента поиска в файлах](./tools/filesearch.md).
