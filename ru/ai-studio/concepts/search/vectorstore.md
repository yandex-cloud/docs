...
Вы можете управлять стратегией разбиения на фрагменты при создании поискового индекса {{ vector-store-name }}. Для этого задайте параметр `chunking_strategy`:
...
### Метаданные на уровне {{ vector-store-name }} {#search-metadata}
...
Поисковый индекс {{ vector-store-name }} можно подключить к голосовому или текстовому [агенту](../agents/index.md) при помощи инструмента File Search.

По умолчанию поиск в {{ vector-store-name }} работает в гибридном режиме:
