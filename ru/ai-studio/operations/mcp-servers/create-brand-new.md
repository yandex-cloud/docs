...
- Консоль управления {#console}
  ...
  1. На панели слева выберите ![logo-mcp](../../../_assets/console-icons/logo-mcp.svg) **MCP-серверы** и нажмите кнопку **Создать MCP-сервер**. В открывшемся окне:
      ...
      1. В блоке **Инструменты** выберите [тип](../../concepts/mcp-hub/index.md#brand-new) добавляемого в MCP-сервер инструмента — `HTTPS-запрос`, `{{ sf-name }}` или `{{ sw-name }}`:
          ...
          - HTTPS-запрос
            ...
            1. В поле **Метод** выберите HTTP-метод запроса: `GET`, `POST`, `DELETE`, `PATCH`, `OPTIONS` или `HEAD`.
            ...
            1. В блоке **Параметры HTTPS-запроса** настройте параметры HTTPS-запросов:
                ...
                * В секции **Тело запроса** посмотрите пример получившегося запроса и при необходимости скорректируйте его.
                ...
                Все параметры HTTPS-запроса поддерживают шаблонизацию — их значения могут генерироваться динамически. Язык шаблонизации — `jq`. Подробнее см. в [документации jq](https://jqlang.github.io/jq/manual/).
