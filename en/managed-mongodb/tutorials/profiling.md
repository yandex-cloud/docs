...
1. On an external host that has network access to a {{ MG }} host (see [{#T}](../operations/connect.md)), install the `mongostat` and `mongotop` [utilities](../operations/tools.md#monitoring-tools) that provide {{ MG }} performance data.
