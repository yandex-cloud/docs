...
#### What is [!KEYREF mmg-short-name]? {#what-is}

[!KEYREF mmg-short-name] is a service that helps you create, operate, and scale [!KEYREF MG] databases in the cloud infrastructure.

With [!KEYREF mmg-short-name], you can:

- Create a database with the required performance characteristics.
- Scale processing power and storage dedicated for your databases as needed.
- Get database logs.

[!KEYREF mmg-short-name] takes on labor-intensive [!KEYREF mmg-short-name] infrastructure administration tasks:
...
You interact with a database cluster in [!KEYREF mmg-short-name] in the same way as with a regular database in your local infrastructure. This allows you to manage internal database settings to meet your app's requirements.

#### What part of DB management and maintenance is [!KEYREF mmg-short-name] responsible for? {#services}

When creating clusters, [!KEYREF mmg-short-name] allocates resources, installs the DBMS, and creates databases.

For the created and running databases, [!KEYREF mmg-short-name] automatically creates backups and applies fixes and updates to the DBMS.

[!KEYREF mmg-short-name] also provides data replication between database hosts (both inside and between availability zones) and automatically switches the load over to a backup replica in the event of a failure.

#### For which tasks should I use [!KEYREF mmg-short-name] and for which VMs with databases? {#mdb-advantage}

Yandex.Cloud offers two ways to work with databases:

- [!KEYREF mmg-short-name] allows you to operate template databases with no need to worry about administration.
...
#### How do I get started with [!KEYREF mmg-short-name]? {#quickstart}

[!KEYREF mmg-short-name] is available to all registered Yandex.Cloud users.

To create a database cluster in [!KEYREF mmg-short-name], decide what the characteristics will be:
...
You can connect to [!KEYREF mmg-short-name] databases using standard DBMS methods.
...
Maintenance in [!KEYREF mmg-short-name] implies:
...
One month after the database version becomes deprecated, [!KEYREF mmg-short-name] automatically sends email notifications to the owners of DB clusters created with this version.
...
In [!KEYREF mmg-short-name], the usage cost is calculated based on the following parameters:
...
Yes, backup is enabled by default. For [!KEYREF MG], a full backup is performed once a day with the possibility to restore it to any saved backup.
