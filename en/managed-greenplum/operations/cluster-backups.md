...
- Management console {#console}
    ...
    1. Click the cluster name and select ![chevron-down](../../_assets/console-icons/chevron-down.svg) **{{ ui-key.yacloud.shared.layout.PageTabs.button_other_hnYwF }}** → **{{ ui-key.yacloud.mdb.cluster.switch_backups }}**.

    To get a list of all backups in your folder:
    1. Open the [folder dashboard]({{ link-console-main }}).
    1. [Navigate]({{ link-console-main }}/link/managed-greenplum) to **{{ ui-key.yacloud.iam.folder.dashboard.label_managed-greenplum }}**.
    1. In the left-hand panel, select ![chevron-down](../../_assets/console-icons/chevron-down.svg) **{{ ui-key.yacloud.shared.layout.PageTabs.button_other_hnYwF }}** → **{{ ui-key.yacloud.mdb.cluster.switch_backups }}**.
...
- CLI {#cli}
    ...
    To get a list of {{ mgp-name }} cluster backups, run this command:
...
- Management console {#console}
    ...
    1. In the **{{ ui-key.yacloud.forms.label_service-account-select }}** field, select the service account or create a new one. User applications will use this account to manage the cluster.
    ...
    1. If there were external data sources created in the cluster, select **{{ ui-key.yacloud.greenplum.label_pxf-sources }}** to restore them. You cannot select individual sources.

    1. If there were user authentication rules created in the cluster, select **{{ ui-key.yacloud.greenplum.label_hba-rules }}** to restore them. You cannot select individual rules.
    ...
    {{ mgp-name }} will start creating the cluster from the backup.
...
- CLI {#cli}
    ...
    1. View the description of the CLI command for restoring a {{ mgp-name }} cluster:
    ...
    1. Make the following request to create a cluster from a backup:
        ...
        * `--time`: Time point to restore the {{ mgp-name }} cluster to, in `yyyy-mm-ddThh:mm:ssZ` format. By default, the cluster will be restored to the backup state.
        * `--name`: Cluster name.
        * `--environment`: Environment:
