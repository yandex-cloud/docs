...
- {{ CH }}
    ...
    1. Create a PXF table called `pxf_ch` to reference an external table called `test` in the {{ CH }} cluster:
        ...
        LOCATION ('pxf://test?PROFILE=JDBC&JDBC_DRIVER=ru.yandex.clickhouse.ClickHouseDriver&DB_URL=jdbc:clickhouse://c-<cluster ID>.rw.mdb.yandexcloud.net:8123/db1&USER=chuser&PASS=chpassword') FORMAT 'CUSTOM' (FORMATTER='pxfwritable_import');
        ```
