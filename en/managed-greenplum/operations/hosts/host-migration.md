...
All {{ mgp-name }} cluster hosts are located in the same {{ yandex-cloud }} [availability zone](../../../overview/concepts/geo-scope.md). You can move hosts from one availability zone to another. To do this, [recover the cluster from a backup](../cluster-backups.md#restore). During cluster recovery from a backup, specify a new availability zone.
