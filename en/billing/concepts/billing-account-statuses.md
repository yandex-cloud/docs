...
- `PAYMENT_NOT_CONFIRMED`: account confirmation required. An email with further instructions is sent to the {% if product == "cloud-il" %}Google {% endif %}email address specified in your {% if product == "yandex-cloud" %}Yandex or Yandex 360{% endif %} account.
...
{% if product == "yandex-cloud" %}- `FIRST_PAYMENT_REQUIRED`: account creation must be confirmed by topping up your personal account.{% endif %}
...
{% if product == "yandex-cloud" %}| `FIRST_PAYMENT_REQUIRED` | None | The use of {{ yandex-cloud }} services is not allowed. <br/>It's most likely that you previously created a billing account that activated the trial period. <br/>The status persists until you top up your personal account. |{% endif %}
