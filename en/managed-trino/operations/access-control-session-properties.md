---
title: Assigning access rules to session system properties in {{ mtr-name }}
description: Rules define whether users can set certain session system properties.
---

# Assigning access rules to session system properties in {{ mtr-name }}

Access rules for session system properties define whether users can set certain session system properties in a {{ mtr-name }} cluster.

{% note warning %}

Set rules for session system properties to limit user consumption of system resources.
...
* If none of the rules match the user-property pair, the user is not allowed to set this particular session system property.
* If no rules are specified for system properties of a session, any user can set any system properties.
* Access rules for session system properties apply together with the top-level [rules for objects in catalogs](./access-control-catalogs.md).

## Setting rules when creating a cluster {set-at-create}

You can set access rules for session system properties when creating a {{ mtr-name }} cluster.
...
- CLI {#cli}
  ...
  1. Create a file named `access_control.yaml` and paste the following code into it:
     ...
     * `system_session_properties`: List of rules for session system properties. Each rule contains the required `allow` parameter, as well as the optional `property`, `groups`, `users`, and `description` parameters.
...
- {{ TF }} {#tf}
  ...
  1. Add the `yandex_trino_access_control` resource with the `system_session_properties` rule list to the configuration file.
     ...
     * `system_session_properties`: List of rule sections for session system properties. Each rule contains the required `allow` parameter, as well as the optional `property`, `groups`, `users`, and `description` parameters.
...
You can set or update access rules for session system properties in an existing {{ mtr-name }} cluster.
...
## Example of setting access rules for session system properties {#example}

Let's configure access rules for session system properties:
1. Allow users in the group with the `admins_group_id` to set any properties.
1. Allow users in the group with the `analytics_group_id` to set properties with names such as `query_.*`.
