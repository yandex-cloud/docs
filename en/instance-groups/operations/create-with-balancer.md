...
1. In the created file, specify:
    ...
    - Target group [!KEYREF load-balancer-name]:
        ...
        | Key | Value |
        | ----- | ----- |
        | `target_group_spec` | Specification of the [!KEYREF load-balancer-name] target group associated with the instance group. |
        | `name` | A name for the [!KEYREF load-balancer-name] target group. The name must be unique within the folder. The name may contain lowercase Latin letters, numbers, and hyphens. The first character must be a letter. The last character can't be a hyphen. The maximum length of the name is 63 characters. |
