...
- Management console {#console}
  ...
  1. Under **{{ ui-key.yacloud.cdn.label_section-additional }}**, enable **{{ ui-key.yacloud.cdn.label_access }}**.
...
- CLI {#cli}
  ...
  1. Enable end-user access to the content by setting the `--active` flag to `true`:
...
- {{ TF }} {#tf}
  ...
  1. Open the {{ TF }} configuration file and add or edit the `yandex_cdn_resource` section by setting `active = true`.
