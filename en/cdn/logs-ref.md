...
|| `request_uri` | String | Full original resource URI received by the edge server. ||
...
|| `upstream_addr` | String | IP address of the source that has responded to the request. This field will be empty if the response originated from the edge server cache. ||
...
|| `upstream_response_time` | String | Source response time. This field will be empty if the response originated from the edge server cache. ||
|#
...
|| `$vp_enabled` | `1` | Flag for the Streaming platform feature set.
* `1`: Enabled.
* `0`: Disabled. ||
