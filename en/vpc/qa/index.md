...
{{ yandex-cloud }} only [blocks](../concepts/limits.md#vpc-egress-traffic-filter) traffic sent from pubic IPs on port 25. {% if product == "yandex-cloud" %}Traffic sent to Yandex email servers is the exception. {% endif %}

{% if product == "yandex-cloud" %}
To prevent email traffic from being blocked, reconfigure your email sever to use ports 465 and 587. To do this, use these ports as an email gateway and [delegate your domain]{% if lang == "ru" %}(https://yandex.ru/support/business/domains/delegate-domain.html){% endif %}{% if lang == "en" %}(https://yandex.com/support/business/delegate-domain.html){% endif %} to Yandex servers with [Yandex Mail for Domains](https://habr.com/company/plesk/blog/304204/).
