...
- {{ TF }} {#tf}
  ...
  1. Add the `data` and `output` sections to the {{ TF }} configuration file:
     ...
     * `output "gateway"`: Output variable containing information about the NAT gateway creation time:
     ...
     You can replace `created_at` with another variable to get the information you need. For more on the properties of the `yandex_vpc_gateway` data source, see [this provider guide]({{ tf-provider-datasources-link }}/vpc_gateway).
