...
The standard MTU is 1500 bytes, but MTUs may vary in different networks that packets are transmitted through. For example, when [{{ ddos-protection-full-name }}](../ddos-protection/index.md) or VPN tunnels are used, the MTU value should be lower to make sure that packets are not lost. Packet loss due to MTU issues may look like an unresponsive TCP session, such as during a TLS handshake or SSH access.
...
If {{ ddos-protection-name }} is enabled, we recommend setting the MTU to 1450, if you need to limit the size of all packets, and the TCP MSS to 1410, if you only need to limit the size of TCP packets.
