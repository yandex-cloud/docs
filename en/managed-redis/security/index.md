...
To allow access to the [!KEYREF mrd-name] service resources (DB clusters and hosts, cluster backups, databases, and their users), assign the user the appropriate roles from the list below. At this time, a role can only be assigned to a parent resource (folder or cloud), and the roles are inherited by nested resources.
...
The list below shows all roles that are considered when verifying access rights in the [!KEYREF mrd-name] service.
