...
The number of hosts in [!KEYREF mrd-short-name] clusters is limited by the quotas on the number of CPUs and the amount of RAM available to the DB clusters in your cloud. To check the resources in use, open the [Quotas](https://console.cloud.yandex.ru/?section=quotas) page and find the **[!KEYREF mrd-full-name]** service.
...
1. See the description of the CLI command for adding a host:
   ...
   ```
   $ [!KEYREF yc-mdb-rd] host add --help
   ```

1. Run the add host command:

   ```
   $ [!KEYREF yc-mdb-rd] host add
        --cluster-name <cluster name>
        --host zone-id=<availability zone>,subnet-id=<subnet ID>
   ```

   [!KEYREF mrd-short-name] will run the add host operation.

   The subnet ID should be specified if the availability zone contains multiple subnets, otherwise [!KEYREF mrd-short-name] automatically selects a single subnet. The cluster name can be requested with a [list of clusters in the folder](cluster-list.md#list-clusters).
...
You can remove a host from a [!KEYREF RD] cluster if it is not the only host in it. To replace a single host, first create a new host and then delete the old one.

If the host is the master at the time of deletion, [!KEYREF mrd-short-name] automatically assigns another replica as the master.
...
```
$ [!KEYREF yc-mdb-rd] host delete <host name>
     --cluster-name=<cluster name>
```
