...
- CLI
   ...
   To {% if audience != "internal" %}increase{% else %}modify{% endif %} storage size of {{ RD }} hosts:
   ...
   1. Specify the desired storage size in the update cluster command.{% if audience != "internal" %}It must be at least as large as the current `disk_size` value in the cluster properties.{% endif %}
