...
#### What is [!KEYREF mrd-short-name]? {#what-is}

[!KEYREF mrd-short-name] is a service that helps you create, operate, and scale [!KEYREF RD] databases in the cloud infrastructure.

With [!KEYREF mrd-short-name], you can:

- Create a database with the required performance characteristics.
- Scale processing power and storage dedicated for your databases as needed.
- Get database logs.

[!KEYREF mrd-short-name] takes on labor-intensive [!KEYREF mrd-short-name] infrastructure administration tasks:
...
You interact with a database cluster in [!KEYREF mrd-short-name] in the same way as with a regular database in your local infrastructure. This allows you to manage internal database settings to meet your app's requirements.

#### What part of DB management and maintenance is [!KEYREF mrd-short-name] responsible for? {#services}

When creating clusters, [!KEYREF mrd-short-name] allocates resources, installs the DBMS, and creates databases.

For the created and running databases, [!KEYREF mrd-short-name] automatically creates backups and applies fixes and updates to the DBMS.

[!KEYREF mrd-short-name] also provides data replication between database hosts (both inside and between availability zones) and automatically switches the load over to a backup replica in the event of a failure.

#### For which tasks should I use [!KEYREF mrd-short-name] and for which VMs with databases? {#mdb-advantage}

Yandex.Cloud offers two ways to work with databases:

- [!KEYREF mrd-short-name] allows you to operate template databases with no need to worry about administration.
...
#### How do I get started with [!KEYREF mrd-short-name]? {#quickstart}

[!KEYREF mrd-short-name] is available to all registered Yandex.Cloud users.

To create a database cluster in [!KEYREF mrd-short-name], decide what the characteristics will be:
...
A [!KEYREF mrd-short-name] cluster can include from 1 to 7 DB hosts.

#### How can I access a running DB host? {#db-access}

You can connect to [!KEYREF mrd-short-name] databases using standard DBMS methods. However, there is one exception: [!KEYREF mrd-short-name] hosts can be accessed only from Yandex.Cloud VMs connected to the same network.
...
Maintenance in [!KEYREF mrd-short-name] implies:

- Automatic installation of DBMS updates and fixes for your database hosts.
- Changes to the host class and storage size.
- Other [!KEYREF mrd-short-name] maintenance activities.

#### Which [!KEYREF RD] version does [!KEYREF mrd-short-name] use? {#dbms-version}

[!KEYREF mrd-short-name] supports [!KEYREF RD] 5.
...
One month after the database version becomes deprecated, [!KEYREF mrd-short-name] automatically sends email notifications to the owners of DB clusters created with this version.
...
In [!KEYREF mrd-short-name], the usage cost is calculated based on the following parameters:
...
Yes, backup is enabled by default. For [!KEYREF RD], a full backup is performed once a day with all DB cluster transaction logs saved. This allows you to restore the cluster state to any point in time during the backup storage period, except for the last 30 seconds.
...
For any DB hosts, you can track metrics specific to the type of the corresponding DBMS. For example, for [!KEYREF RD], you can track:
