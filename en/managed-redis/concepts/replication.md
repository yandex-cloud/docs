...
For more information about how replication works in [!KEYREF RD], read the [DBMS documentation](https://redis.io/topics/replication).
...
In order to make decisions about cluster performance, the majority of Sentinel services need to be healthy. As a result, it is more cost-efficient to use clusters with an odd number of hosts when working with [!KEYREF mrd-name]. For example, a cluster with three hosts can lose one host and continue working, while a cluster with four hosts can also lose no more than one host: if a second host is lost, the remaining Sentinel instances will not be enough to select a new master.
...
The owners of [!KEYREF mrd-name] clusters cannot configure the Sentinel services, but they can read information that the services provide. Read more about Sentinel in the [DBMS documentation](https://redis.io/topics/sentinel).
