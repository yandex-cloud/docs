...
- Management console {#console}
  ...
  1. In the left-hand panel, select ![square-article](../../_assets/console-icons/square-article.svg) **{{ ui-key.yacloud.smart-web-security.customPages_rriaz }}**.
  1. In the row with the template, click ![options](../../_assets/console-icons/ellipsis.svg) → ![pencil](../../_assets/console-icons/pencil.svg) **{{ ui-key.yacloud.common.edit }}**.
  1. In the window that opens, edit the following parameters:
      ...
      * **{{ ui-key.yacloud.smart-web-security.custom-pages.label_status_code }}**: One of the standard [HTTP response codes](https://www.iana.org/assignments/http-status-codes/http-status-codes.xhtml) for which the client will get a page based on your template.
      * **{{ ui-key.yacloud.smart-web-security.custom-pages.label_format }}**: `HTML`, `JSON`, `XML`, or `Custom`.
          ...
          To set a custom header, select `Custom`. You can change the **{{ ui-key.yacloud.smart-web-security.custom-pages.label_content_type }}** for `Custom` headers.

      * To add an HTTP header, click **{{ ui-key.yacloud.smart-web-security.CustomPageForm.add_header_8Zgm9 }}** and specify its **{{ ui-key.yacloud.common.label_key }}** (header name) and **{{ ui-key.yacloud.common.value }}**. You can add up to 10 headers per template.

          To delete a header, click ![options](../../_assets/console-icons/xmark.svg) next to its row.

      * **{{ ui-key.yacloud.smart-web-security.custom-pages.label_body }}**: Edit the page content the client will get as a response. The response must not exceed `2 KB` (about 2,000 UTF-8 characters) in size. Add the content as per the response format you selected while preserving the markup.
...
- Management console {#console}
  ...
  1. [Navigate]({{ link-console-main }}/link/smartwebsecurity) to **{{ ui-key.yacloud.iam.folder.dashboard.label_smartwebsecurity }}**.
  1. In the left-hand panel, select ![square-article](../../_assets/console-icons/square-article.svg) **{{ ui-key.yacloud.smart-web-security.customPages_rriaz }}**.
  1. In the row with the template, click ![options](../../_assets/console-icons/ellipsis.svg) → ![trash-bin](../../_assets/console-icons/trash-bin.svg) **{{ ui-key.yacloud.common.delete }}**.
