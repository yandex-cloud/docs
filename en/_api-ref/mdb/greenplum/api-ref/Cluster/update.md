...
masterConfig.<br>config.<br>pool | **object**<br><p>Odyssey connection pooler settings.</p> <p>Configuration of the connection pooler.</p> 
masterConfig.<br>config.<br>pool.<br>mode | **string**<br><p>Odyssey route server pool mode.</p> <ul> <li>SESSION: Assign server connection to a client until it disconnects. Default value.</li> <li>TRANSACTION: Assign server connection to a client for a transaction processing.</li> </ul> 
masterConfig.<br>config.<br>pool.<br>size | **integer** (int64)<br><p>The number of servers in the pool. Clients are put in a wait queue, when all servers are busy. Set to zero to disable the limit.</p> 
masterConfig.<br>config.<br>pool.<br>clientIdleTimeout | **integer** (int64)<br><p>Server pool idle timeout, in seconds. A server connection closes after it has been idle for the specified duration. Set to zero to disable.</p> 
...
masterConfig.<br>config.<br>logDuration | **boolean** (boolean)<br><p>Causes the duration of every completed statement which satisfies log_statement to be logged. For details, see Greenplum® <a href="https://docs.greenplum.org/6-5/ref_guide/config_params/guc-list.html#log_duration">documentation</a>.</p> 
