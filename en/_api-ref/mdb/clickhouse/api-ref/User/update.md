...
settings.<br>insertQuorumTimeout | **integer** (int64)<br><p>Quorum write timeout in milliseconds.</p> <p>If the write quorum is enabled in the cluster, this timeout expires and some data is not written to the <code>insertQuorum</code> replicas, then ClickHouse will abort the execution of <strong>INSERT</strong> query and return an error. In this case, the client must send the query again to write the data block into the same or another replica.</p> <p>Minimum value: <strong>1000</strong>, 1 second (default: <strong>60000</strong>, 1 minute).</p> <p>The minimum value is 1000.</p> 
...
settings.<br>maxBytesBeforeExternalSort | **integer** (int64)<br><p>This setting is equivalent of the <code>maxBytesBeforeExternalGroupBy</code> setting, except for it is for sort operation (<strong>ORDER BY</strong>), not aggregation.</p> 
