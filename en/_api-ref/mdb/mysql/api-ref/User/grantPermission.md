...
--- | ---
clusterId | Required. ID of the cluster to grant permission to the user in.  To get this ID, make a [list](/docs/managed-mysql/api-ref/Cluster/list) request.  The maximum string length in characters is 50.
userName | Required. Name of the user to grant permission to.  To get this name, make a [list](/docs/managed-mysql/api-ref/User/list) request.  The maximum string length in characters is 63. Value must match the regular expression `` [a-zA-Z0-9_]* ``.
