...
Field | Description
--- | ---
backupId | **string**<br><p>Required. ID of the backup to create a cluster from. To get the backup ID, use a <a href="/docs/managed-sqlserver/api-ref/Cluster/listBackups">listBackups</a> request.</p> 
time | **string** (date-time)<br><p>Required. Timestamp of the moment to which the sqlserver cluster should be restored.</p> <p>String in <a href="https://www.ietf.org/rfc/rfc3339.txt">RFC3339</a> text format.</p> 
name | **string**<br><p>Required. Name of the new sqlserver cluster. The name must be unique within the folder.</p> <p>Value must match the regular expression <code>[a-zA-Z0-9_-]*</code>.</p> 
description | **string**<br><p>Description of the new sqlserver cluster.</p> <p>The maximum string length in characters is 256.</p> 
...
environment | **string**<br><p>Deployment environment of the new sqlserver cluster.</p> <ul> <li>PRODUCTION: Stable environment with a conservative update policy: only hotfixes are applied during regular maintenance.</li> <li>PRESTABLE: Environment with more aggressive update policy: new versions are rolled out irrespective of backward compatibility.</li> </ul> 
configSpec | **object**<br><p>Configuration for the sqlserver cluster to be created.</p> 
...
hostSpecs[] | **object**<br><p>Configurations for sqlserver hosts that should be added to the cluster that is being created from the backup.</p> 
...
networkId | **string**<br><p>ID of the network to create the sqlserver cluster in.</p> <p>The maximum string length in characters is 50.</p> 
