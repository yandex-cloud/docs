...
- Google Compute Engine (some fields are not supported). - Amazon EC2 (some fields are not supported).
...
```bash yc compute instance update <VM_instance_ID_or_name> \ --metadata-options aws-v1-http-token=DISABLED ```
