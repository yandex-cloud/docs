...
| `authentication.federation_id` ^1^ | **string**<br>ID of the federation the federated user belongs to |
| `authentication.federation_name` ^1^ | **string**<br>Name of the federation the federated user belongs to |
| `authentication.federation_type` ^1^ | **string**<br>Federation type. The possible value is:<ul><li>`PRIVATE_FEDERATION`: Federation managed by {{ yandex-cloud }} clients</li></ul> |
| `authentication.token_info` ^2^ | **object**<br>Authentication data of the event subject |
...
| `authentication.token_info.impersonator_federation_id` ^1^ | **string**<br>ID of the federation the impersonated federated user belongs to |
| `authentication.token_info.impersonator_federation_name`&nbsp;^1^ | **string**<br>Name of the federation the impersonated federated user belongs to |
| `authentication.token_info.impersonator_federation_type`&nbsp;^1^ | **string**<br>Federation type. The possible value is:<ul><li>`PRIVATE_FEDERATION`: Federation managed by {{ yandex-cloud }} clients</li></ul> |
...
<small>^1^ This field is available when `subject_type` = `FEDERATED_USER_ACCOUNT`.</small>
<small>^2^ The field block is not used for every data event.</small>
