...
Follow these instructions to create a new trail that will upload audit logs of a single folder's resources to an {{ objstorage-name }} bucket{% if product == "yandex-cloud" %} with encryption enabled{% endif %}.
...
1. Assign roles to the service account:
   ...
   - CLI
      ...
      * Assign the [`storage.uploader`](../../storage/security/index.md#storage-uploader) role for the folder to host the trail:
      ...
      {% if product == "yandex-cloud" %}* Assign the role [`kms.keys.encrypterDecrypter`](../../kms/security/index.md#service) for the encryption key:
