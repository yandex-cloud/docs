...
- Management console
  ...
  1. Create a security group for the load balancer:
     ...
     1. Select the **Network** `ddos-network`.
...
Backends of your application are deployed on the VM instance of the [target group](../concepts/target-group.md). The target group will be connected to the load balancer so that requests can be sent to the backend endpoints of your application. In this scenario, creating an instance group with the minimum configuration is enough.
...
- Management console
  ...
  1. Under **Instance template**, click **Define** and set up the configuration for a basic instance:
     ...
     * Under **Image/boot disk selection**, go to the **Cloud Marketplace** tab and click **Show more**. Select **LEMP** and click **Use**.
  ...
  1. Under **Integration with Application Load Balancer**, select **Create target group** and specify `tg-ddos` as the group name. [Read more about target groups](../concepts/target-group.md).
...
- CLI
  ...
  1. Get the resource IDs required to create an instance group using the commands:
     * [yc iam service-account get &lt;service account name&gt;](../../cli/cli-ref/managed-services/iam/service-account/get.md): For your service account.
     ...
     instance_template:
         ...
         network_interface_specs:
             - network_id: <cloud network ID>
               subnet_ids:
                 - <subnet ID in the ru-central1-a zone>
                 - <subnet ID in the ru-central1-b zone>
                 - <subnet ID in the ru-central1-c zone>
...
You must link the target group created with the instance group to the [backend group](../concepts/backend-group.md) that defines the traffic allocation settings.
...
- Management console
  ...
  1. Click **Add listener** under **Listeners**. Set the listener settings:
     ...
     1. Select the **List** type and specify the IP address with DDoS protection you [reserved earlier](#reserve-ip).
...
## Delete the resources created {#delete-resources}
...
1. Delete the non-billable resources that block the deletion of billable resources:
   ...
   1. [Delete](../operations/http-router-delete.md) the `ddos-router` HTTP router.
   1. [Delete](../operations/backend-group-delete.md) the `ddos-backend-group` backend group.
