...
To log load balancer operations, a [log group](../../functions/concepts/log-group.md) is created automatically in {{ cloud-logs-full-name }}. Under the use case, log delivery from this log group to the database will be set up using {{ sf-full-name }} resources: a [trigger](../../functions/concepts/trigger/cloudlogs-trigger.md) and called [function](../../functions/concepts/function.md).

To create a database, in this use case we'll use {{ mpg-full-name }}.

{% note info %}

Some use case steps can't be performed in the [management console]({{ link-console-main }}). You can [get the log group ID](#get-log-group-id) and [create a trigger](#set-up-sf-create-trigger) with this ID only in the Command Line Interface (CLI) or API.
...
1. [Create necessary resources in {{ alb-full-name }}](#create-alb-resources).
1. [Get the ID of the log group for the load balancer](#get-log-group-id).
1. [Create necessary resources in {{ sf-full-name }}](#set-up-sf).
...
* A payment for calling the log-processing function and allocating computing resources to the function (see [{{ sf-full-name }} pricing](../../functions/pricing.md)).
...
Logs received from the load balancer will be written to the database that is part of the [cluster](../../managed-postgresql/concepts/index.md) in {{ mpg-name }}.
...
- Management console
  ...
  After you [create a cluster](#set-up-db-create-cluster), you are automatically redirected to the **Clusters** page.
...
As web servers for your site, you'll use an [instance group](../../compute/concepts/instance-groups/index.md) from {{ compute-name }}. In this use case, the servers will be deployed on the LEMP stack (Linux, nginx, MySQL, PHP; for more information, see the tutorial [Website on a LAMP or LEMP stack](../../tutorials/web/lamp-lemp.md)).
...
- Management console
  ...
  1. Under **Network settings**, select the **Network** `alb-logging-network`, [which you created previously](#create-network), and its subnets.

  1. In the **Public address** field, select **Auto**.

  1. Select the `alb-logging-vms` security group [you created previously](#create-security-groups).
  ...
  1. Under **Integration with Application Load Balancer**, select **Create target group** and specify `alb-logging-tg` as the group name. [Read more about target groups](../concepts/target-group.md).

  1. Click **Create**.

{% endlist %}

It may take several minutes to create an instance group. After creation, the group is assigned the [status](../../compute/concepts/instance-groups/statuses.md#group-statuses) `RUNNING`, and all its VMs have the [status](../../compute/concepts/instance-groups/statuses.md#vm-statuses) `RUNNING_ACTUAL`.
...
The backend group should be linked to an [HTTP router](../concepts/http-router.md) that defines HTTP routing rules.
...
Together with a load balancer, this creates a [log group](../../functions/concepts/log-group.md) for writing logs. You'll need the log group ID when creating a trigger for {{ cloud-logs-name }} in {{ sf-name }}.
...
[Functions](../../functions/concepts/function.md) will receive messages from the log group and write them to the table you [previously created](#set-up-db-create-table).
...
- CLI
  ...
  yc serverless trigger create cloud-logs alb-logging-trigger \
    --log-groups <ID of the log group> \
    --invoke-function-name alb-logging-function \
    --invoke-function-service-account-name alb-logging-function-service-account \
    --batch-size 10 \
    --batch-cutoff 15s
  ```
  * `--log-groups`: ID of the log group for the load balancer that you [received earlier](#get-log-group-id).
  * `--invoke-function-name`: The name of the function you [previously created](#set-up-sf-create-function).
...
- API
  ...
  Use the gRPC API call [TriggerService/Create](../../functions/triggers/api-ref/grpc/trigger_service.md#Create) or the REST API method [create](../../functions/triggers/api-ref/Trigger/create.md) API method.
...
1. Delete the non-billable resources that block the deletion of billable resources:
   ...
   1. [Delete](../operations/backend-group-delete.md) the `alb-logging-router` backend group.
