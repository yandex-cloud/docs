...
1. [Upload the website files to the VM](#upload-site-files).
...
If you have an active billing account, you can create or select a folder to run your VM in from the [Yandex Cloud page](https://console.cloud.yandex.com/cloud). Make sure that you do all the actions under the use case in this folder.
...
If your site does not have a certificate, you can [use {{ certificate-manager-name }} to get a certificate from Let's Encrypt<sup>®</sup>](../../certificate-manager/operations/managed/cert-create.md). This does not require additional steps after creating a certificate. It is imported automatically.
...
- Management console
  ...
  1. Under **Network settings**, select **Network** `mysite-network`, [which you created previously](#create-network), and its subnets.
  1. Select the `mysite-sg-vms` security group [you created previously](#create-security-groups).
...
You must link the target group created with the VM group to the [backend group](../concepts/backend-group.md) that defines the traffic allocation settings.

For the backends, groups will implement [health checks](../concepts/backend-group.md#health-checks): the load balancer will periodically send health check requests to the VMs and expect a response after a certain delay.
...
The backend group should be bound to an [HTTP router](../concepts/http-router.md) that defines routing rules.
...
1. Delete the non-billable resources that block the deletion of billable resources:
   ...
   1. [Delete](../operations/application-load-balancer-delete.md) the `mysite-alb` L7 load balancer.
   1. [Delete](../operations/http-router-delete.md) the `mysite-router` HTTP router.
   1. [Delete](../operations/backend-group-delete.md) the `my-site-bg` backend group.

1. [Delete](../../compute/operations/instance-groups/delete.md) the `mysite-ig` virtual machine group.
