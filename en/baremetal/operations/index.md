---
title: Working with {{ baremetal-full-name }}. Step-by-step guides
description: In this article, you will learn how to work with {{ baremetal-full-name }} in {{ yandex-cloud }}.
---

# Step-by-step guides for {{ baremetal-full-name }}

## Servers {#servers}

* [{#T}](./servers/server-lease.md)
* [{#T}](./servers/get-info.md)
* [{#T}](./servers/server-update.md)
* [{#T}](./servers/server-kvm.md)
* [{#T}](./servers/server-stop-and-start.md)
* [{#T}](./servers/server-lease-cancel.md)
* [{#T}](./image-upload.md)
* [{#T}](./backup-baremetal.md)
* [{#T}](./servers/rescue-boot.md)
* [{#T}](./servers/reinstall-os-from-marketplace.md)
* [{#T}](./servers/reinstall-os-from-own-image.md)
* [{#T}](./servers/reset-password.md)
* [{#T}](./servers/switch-raid-member.md)
* [{#T}](./servers/add-new-ssh-key.md)
* [{#T}](./servers/restore-grub.md)
* [{#T}](./servers/use-hwatcher.md)

## Network {#network}

* [{#T}](./network-create.md)
* [{#T}](./subnet-create.md)
* [{#T}](./reserve-public-subnet.md)
* [{#T}](./create-vpc-connection.md)