---
title: Managing access to a {{ mkf-full-name }} cluster
description: Follow this guide to configure {{ mkf-name }} cluster access permissions.
---

# Managing access to a {{ mkf-name }} cluster
...
- CLI {#cli}
  ...
  1. To get a list of roles assigned for the cluster, run this command:
      ...
      ```bash
      {{ yc-mdb-kf }} cluster list-access-bindings <cluster_name_or_ID>
      ```
...
- REST API {#api}
  ...
  1. Call the [Cluster.ListAccessBindings](../api-ref/Cluster/listAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --url 'https://{{ api-host-mdb }}/managed-kafka/v1/clusters/<cluster_ID>:listAccessBindings'
      ```
...
- gRPC API {#grpc-api}
  ...
  1. Call the [ClusterService.ListAccessBindings](../api-ref/grpc/Cluster/listAccessBindings.md) method, e.g., via the following {{ api-examples.grpc.tool }} request:
      ...
      grpcurl \
        ...
        yandex.cloud.mdb.kafka.v1.ClusterService.ListAccessBindings
      ```
...
- CLI {#cli}
  ...
  1. View the description of the CLI command for assigning a role for a cluster:
      ...
      ```bash
      {{ yc-mdb-kf }} cluster add-access-binding --help
      ```

  1. Use this command to assign a role:

      ```bash
      {{ yc-mdb-kf }} cluster add-access-binding <cluster_name_or_ID> \
        --role <role> \
      ...
      * `--role`: [Role](../security/index.md#roles-list), e.g., `managed-kafka.editor`.
...
- REST API {#api}
  ...
  1. Call the [Cluster.UpdateAccessBindings](../api-ref/Cluster/updateAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://{{ api-host-mdb }}/managed-kafka/v1/clusters/<cluster_ID>:updateAccessBindings' \
        --data '{
      ...
      * `access_binding_deltas.roleId`: [Role](../security/index.md#roles-list), e.g., `managed-kafka.editor`.
...
- gRPC API {#grpc-api}
  ...
  1. Call the [ClusterService.UpdateAccessBindings](../api-ref/grpc/Cluster/updateAccessBindings.md) method, e.g., via the following {{ api-examples.grpc.tool }} request:
      ...
      grpcurl \
        ...
        {{ api-host-mdb }}:{{ port-https }} \
        yandex.cloud.mdb.kafka.v1.ClusterService.UpdateAccessBindings
      ```
...
- CLI {#cli}
  ...
  1. View the description of the CLI command for assigning roles for a cluster:
      ...
      ```bash
      {{ yc-mdb-kf }} cluster set-access-bindings --help
      ```

  1. To assign roles, run this command:

      ```bash
      {{ yc-mdb-kf }} cluster set-access-bindings <cluster_name_or_ID> \
...
- REST API {#api}
  ...
  1. Call the [Cluster.SetAccessBindings](../api-ref/Cluster/setAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://{{ api-host-mdb }}/managed-kafka/v1/clusters/<cluster_ID>:setAccessBindings' \
        --data '{
                  "accessBindings": [
                    {
                      "roleId": "<role>",
      ...
      * `accessBindings.roleId`: [Role](../security/index.md#roles-list), e.g., `managed-kafka.editor`.
...
- gRPC API {#grpc-api}
  ...
  1. Call the [ClusterService.SetAccessBindings](../api-ref/grpc/Cluster/setAccessBindings.md) method, e.g., via the following {{ api-examples.grpc.tool }} request:
      ...
      grpcurl \
        ...
        {{ api-host-mdb }}:{{ port-https }} \
        yandex.cloud.mdb.kafka.v1.ClusterService.SetAccessBindings
      ```
...
- CLI {#cli}
  ...
  1. View the description of the CLI command for revoking a role for a cluster:
      ...
      ```bash
      {{ yc-mdb-kf }} cluster remove-access-binding --help
      ```
  1. To revoke a role, run this command:

      ```bash
      {{ yc-mdb-kf }} cluster remove-access-binding <cluster_name_or_ID> \
        --role <role> \
        --subject <subject_type>:<subject_ID>
      ```
      ...
      * `--role`: [Role](../security/index.md#roles-list) being revoked, e.g., `managed-kafka.editor`.
...
- REST API {#api}
  ...
  1. Call the [Cluster.UpdateAccessBindings](../api-ref/Cluster/updateAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      * `access_binding_deltas.roleId`: [Role](../security/index.md#roles-list) being revoked, e.g., `managed-kafka.editor`.
...
For a service account to be able to view the info of all {{ mkf-name }} clusters in the folder but modify the resources in one particular cluster alone, give it the `managed-kafka.viewer` role for the folder and the `managed-kafka.editor` role for that cluster:
...
- CLI {#cli}
  ...
  1. Assign the roles for the cluster:
      ...
      ```bash
      {{ yc-mdb-kf }} cluster add-access-bindings <cluster_name_or_ID> \
        --access-binding role=managed-kafka.editor,subject=serviceAccount:<service_account_ID>
      ```
