...
| `gp.is_alive`<br/>`DGAUGE` | Host health indicator.<br/>`1` if DB host is alive, `0` if it is not. |
| `gp.is_alive_segment`<br/>`DGAUGE` | Segment host health indicator.<br/>`1` if DB host is alive, `0` if not. |
| `gp.is_dead`<br/>`DGAUGE` | Host health indicator.<br/>`0` if DB host is alive, `1` if not. |
...
| `system.n_cpus`<br/>`DGAUGE`, number | Limit on CPUs in use. |
