...
| `mem.limit_bytes`<br/>`DGAUGE`, bytes | Maximum allowed instance memory (usage limit) |
...
| `cmdstat_<command>_calls`<br/>`DGAUGE`, count | Number of the specified completed commands, e.g., EVAL, GET, HGETALL.<br/>The `command` special label is the command ID. |
| `cmdstat_<command>_usec`<br/>`DGAUGE`, microseconds | Total running time of the specified commands, e.g., EVAL, GET, HGETALL.<br/>The `command` special label is the command ID. |
...
| `hosts.subcluster.<subcluster_name>.total`<br/>`DGAUGE`, count | Number of instances per subcluster |
...
| `latency_percentiles_usec_<command>_p<percentile>`<br/>`DGAUGE`, microseconds | Command execution time in the given percentile. For example, `latency_percentiles_usec_get_p50` is the median execution time of the GET command, and `latency_percentiles_usec_hmset_p99.9` is the execution time of the HMSET command in the 99.9th percentile.<br/>The `command` special label is the command ID. |
| `latest_backup_sec`<br/>`DGAUGE`, seconds | Unix timestamp of the most recent automatic backup |
...
| `redis_acl_access_denied_auth`<br/>`DGAUGE`, count | Number of {{ RD }} connection attempts failed during authorization with an `access denied` error |
...
| `redis_instantaneous_output_kbps`<br/>`DGAUGE`, Kbps | Current outgoing traffic from the {{ RD }} shard in kilobits per second |
| `redis_instantaneous_output_repl_kbps`<br/>`DGAUGE`, Kbps | Current outgoing replication traffic from the {{ RD }} shard in kilobits per second |
...
| `redis_total_net_output_bytes`<br/>`DGAUGE`, bytes | Total bytes of outgoing network traffic from a given {{ RD }} shard since its startup |
| `redis_total_net_repl_input_bytes`<br/>`DGAUGE`, bytes | Total bytes of incoming replication network traffic to a given {{ RD }} shard since its startup |
| `redis_total_net_repl_output_bytes`<br/>`DGAUGE`, bytes | Total bytes of outgoing replication network traffic from a given {{ RD }} shard since its startup |
...
| `redis_used_cpu_sys_main_thread`<br/>`DGAUGE`, seconds | CPU (system) time spent by the main thread of a given {{ RD }} shard since its startup |
| `redis_used_cpu_user_main_thread`<br/>`DGAUGE`, seconds | CPU time (in user space) spent by the main thread of a given {{ RD }} shard since its startup |
...
| `slowlog_new`<br/>`DGAUGE`, count | New entries in the {{ RD }} shard slow log over the most recent monitoring window |
| `slowlog_top`<br/>`DGAUGE`, microseconds | Maximum command execution time among the latest slow log entries on the {{ RD }} shard during the most recent monitoring window.<br/>The `cmd` special label is the command ID. |
