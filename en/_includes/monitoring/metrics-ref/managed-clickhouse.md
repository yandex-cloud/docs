...
| Name<br/>Type, units | Description |
| ----- | ----- |
| `mem.active_bytes`<br/>`DGAUGE`, bytes | Active resident memory (frequently accessed and released when absolutely necessary). Usage type: `active` |
...
| `mem.free_bytes`<br/>`DGAUGE`, bytes | Amount of free RAM available, excluding `mem.buffers_bytes` and `mem.cached_bytes`. Usage type: `free` |
...
| `mem.used_bytes`<br/>`DGAUGE`, bytes | Amount of RAM currently used by running processes. Usage type: `used` |
...
## {{ ZK }} metrics {#managed-clickhouse-zookeeper}

| Name<br/>Type, units | Description |
| ----- | ----- |
| `zk_avg_latency`<br/>`DGAUGE`, milliseconds | {{ ZK }} host average response delay |
| `zk_ephemerals_count`<br/>`DGAUGE`, count | Number of ephemeral nodes |
| `zk_num_alive_connections`<br/>`DGAUGE`, count | Number of connections |
| `zk_outstanding_requests`<br/>`DGAUGE`, count | Number of requests being processed |
| `zk_server_state_follower`<br/>`DGAUGE`, 0/1 | Either `1` if the {{ ZK }} host role is `Follower` or `0` if not |
| `zk_server_state_leader`<br/>`DGAUGE`, 0/1 | Either `1` if the {{ ZK }} host role is `Leader` or `0` if not |
| `zk_watch_count`<br/>`DGAUGE`, count | Number of watches |
| `zk_znode_count`<br/>`DGAUGE`, count | Number of znodes |
...
| `ch_local_disk_max_parts_per_partition`<br/>`DGAUGE`, count | Maximum number of parts per partition on local disks |
...
| `ch_system_async_metrics_MemoryCode`<br/>`DGAUGE`, bytes | Memory utilized by the process code |
| `ch_system_async_metrics_MemoryDataAndStack`<br/>`DGAUGE`, bytes | Memory utilized by the process data and stack |
| `ch_system_async_metrics_MemoryResident`<br/>`DGAUGE`, bytes | Actual memory usage by the server |
| `ch_system_async_metrics_MemoryResidentMax`<br/>`DGAUGE`, bytes | Maximum actual memory usage by the process |
| `ch_system_async_metrics_MemoryResidentWithoutPageCache`<br/>`DGAUGE`, bytes | Memory utilized by the process less page cache |
| `ch_system_async_metrics_MemoryShared`<br/>`DGAUGE`, bytes | Shared process memory size |
| `ch_system_async_metrics_MemoryVirtual`<br/>`DGAUGE`, bytes | Total virtual memory of the process |
| `ch_system_async_metrics_NumberOfDatabases`<br/>`DGAUGE`, count | Total databases on the server |
| `ch_system_async_metrics_NumberOfDetachedByUserParts`<br/>`DGAUGE`, count | Number of parts detached by the user |
| `ch_system_async_metrics_NumberOfDetachedParts`<br/>`DGAUGE`, count | Total number of detached parts |
| `ch_system_async_metrics_NumberOfPendingMutations`<br/>`DGAUGE`, count | Number of pending mutations |
| `ch_system_async_metrics_NumberOfPendingMutationsOverExecutionTime`<br/>`DGAUGE`, count | Number of pending mutations exceeding their execution time threshold |
...
| `ch_system_async_metrics_TotalPrimaryKeyBytesInMemory`<br/>`DGAUGE`, bytes | Total memory utilized to store primary key values in RAM |
| `ch_system_async_metrics_TotalPrimaryKeyBytesInMemoryAllocated`<br/>`DGAUGE`, bytes | Total memory allocated for primary key values in RAM |
