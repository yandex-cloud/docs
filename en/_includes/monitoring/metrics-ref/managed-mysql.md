...
| Name<br/>Type, units | Description |
| ----- | ----- |
| `dmesg_events.mysql.oom`<br/>`DGAUGE`, events | Number of {{ MY }} out-of-memory events |
| `dmesg_events.system.oom`<br/>`DGAUGE`, events | Number of system out-of-memory events |
...
| `net.ip_outtransmits`<br/>`DGAUGE`, packets | Number of successfully sent IP packets |
...
| `dbaas_conf.hosts.subcluster.source.total`<br/>`DGAUGE`, hosts | Number of hosts in the source subcluster |
| `dbaas_conf.hosts.subcluster.target.total`<br/>`DGAUGE`, hosts | Number of hosts in the target subcluster |
| `dbaas_conf.hosts.total`<br/>`DGAUGE`, hosts | Total hosts used by the service |
| `disk.mysql.binlogs_bytes`<br/>`DGAUGE`, bytes | Amount of space used by {{ MY }} service logs | 
| `disk.mysql.data_bytes`<br/>`DGAUGE`, bytes | Amount of space used by data | 
| `disk.mysql.default_tablespace_bytes`<br/>`DGAUGE`, bytes | Amount of space used by data in the default tablespace | 
| `disk.mysql.innodb_logs_bytes`<br/>`DGAUGE`, bytes | Amount of space used by InnoDB logs | 
| `disk.mysql.relaylogs_bytes`<br/>`DGAUGE`, bytes | Amount of space used by {{ MY }} service logs | 
| `disk.mysql.temp_tablespace_bytes`<br/>`DGAUGE`, bytes | Amount of space used by data in the temporary tablespace | 
...
| `mysql.fileio.read_bytes`<br/>`DGAUGE`, bytes per second | Data read rate.<br/>Additional label: `filetype`. | 
...
| `mysql_connections.system`<br/>`DGAUGE`, connections | Number of system connections |
| `mysql_connections.user`<br/>`DGAUGE`, connections | Number of user connections |
...
| `mysql_Innodb_buffer_pool_pages_data`<br/>`DGAUGE`, count | Number of pages with data in the InnoDB buffer, including `dirty` and `clean` | 
...
| `mysql_Innodb_data_reads_rate`<br/>`DGAUGE`, operations per second | InnoDB disk read rate | 
| `mysql_Innodb_data_writes_rate`<br/>`DGAUGE`, operations per second | InnoDB write rate | 
...
| `mysql_Innodb_rows_deleted_rate`<br/>`DGAUGE`, rows per second | InnoDB row deletion rate | 
| `mysql_Innodb_rows_inserted_rate`<br/>`DGAUGE`, rows per second | InnoDB row insertion rate | 
| `mysql_Innodb_rows_read_rate`<br/>`DGAUGE`, rows per second | Rate of row reads from InnoDB tables | 
| `mysql_Innodb_rows_updated_rate`<br/>`DGAUGE`, rows per second | InnoDB row update rate | 
...
| `mysql_Queries_rate`<br/>`DGAUGE`, queries per second | Query execution rate, including user queries and stored procedures | 
| `mysql_Questions_rate`<br/>`DGAUGE`, queries per second | User query execution rate | 
...
| `mysql_Select_full_join_rate`<br/>`DGAUGE`, queries per second | Rate of joins using table scans rather than indexes | 
| `mysql_Select_full_range_join_rate`<br/>`DGAUGE`, queries per second | Number of joins searching by range in the reference table | 
| `mysql_Slow_queries_rate`<br/>`DGAUGE`, queries per second | Number of slow queries running longer than the `long_query_time` per unit of time | 
| `mysql_Sort_range_rate`<br/>`DGAUGE`, queries per second | Number of range sorts per unit of time | 
...
| `mysql_Table_open_cache_hits_rate`<br/>`DGAUGE`, operations per second | Open table cache hit rate | 
...
| `mysql_Threads_created_rate`<br/>`DGAUGE`, threads per second | Thread creation rate | 
...
| `mysql_role`<br/>`DGAUGE`, 1/2 | Current {{ MY }} server role in the replication.<br/>It can be either `2` if the server is a replica (secondary server) or `1` if it is a master (primary server). | 
