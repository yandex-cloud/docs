...
| `kafka_controller_KafkaController_PreferredReplicaImbalanceCount`<br/>`DGAUGE`, pcs | Imbalance indicator in the required allocation of replicas, normally shows `0`. |
...
| `kafka_is_alive`<br/>`DGAUGE`, 0/1 | Broker health indicator.<br/>`1` if a broker is alive, `0` if it is not. |
...
| `kafka_server_BrokerTopicMetrics_MessagesIn`<br/>`DGAUGE`, pcs | Number of written messages |
