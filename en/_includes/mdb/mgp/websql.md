...
{{ websql-name }} is a {{ yandex-cloud }} service that allows you to connect to managed database clusters and work with their databases, tables, schemas, and queries. It is a web-based tool that requires no additional authorization and simplifies working with {{ VLK }} commands by prompting the user.
...
1. Click **{{ ui-key.yacloud.mdb.cluster.websql-connections.action_create-connection }}** and specify the following connection settings:
    ...
    * **Cluster**: Defaults to your current {{ mgp-name }} cluster.
