* **gp_add_column_inherits_table_setting**{#setting-gp-add-column-inherits-table-setting} {{ tag-con }} {{ tag-tf }} {{ tag-api }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/6/greenplum-database/ref_guide-config_params-guc-list.html#gp_add_column_inherits_table_setting).
...
* **gp_enable_global_deadlock_detector**{#setting-gp-enable-global-deadlock-detector} {{ tag-con }} {{ tag-tf }} {{ tag-api }}
    ...
    This setting is disabled by default. If it detects one, `UPDATE` and `DELETE` operations will be performed one after another rather than in parallel.

    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#gp_enable_global_deadlock_detector).

* **gp_enable_zstd_memory_accounting**{#setting-gp-enable-zstd-memory-accounting} {{ tag-con }} {{ tag-tf }} {{ tag-api }}

    Enables {{ GP }} memory accounting for workfiles compressed using the Zstandard (`zstd`) algorithm. If this setting is on, {{ GP }} will use its native memory allocator for `zstd` instead of the `zstd` library’s built-in allocator, which allows detecting available memory overruns using standard tools without server failures.

    This setting is enabled by default ({{ GP }} uses its native allocator).
...
* **gp_global_deadlock_detector_period**{#setting-gp-global-deadlock-detector-period} {{ tag-con }} {{ tag-tf }} {{ tag-api }}
    ...
    The minimum value is `5` and the default is `120`.

    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#gp_global_deadlock_detector_period).

* **gp_max_slices**{#setting-gp-gp-max-slices} {{ tag-con }} {{ tag-tf }} {{ tag-api }}

    Sets the maximum number of slices (parts of the query plan which run on the segment instances) an SQL query can create. If a query generates more slices than the specified number, {{ GP }} will return an error and terminate the SQL query. The default value is `0`, unlimited.

    Running an SQL query which generates a large number of slices reduces the system’s speed. An example of such a query may be one containing `UNION` or `UNION ALL` operators for multiple complex views. You can run the `EXPLAIN` command to view slice statistics for an SQL query.
...
* **gp_workfile_compression**{#setting-gp-workfile-compression} {{ tag-con }} {{ tag-tf }} {{ tag-api }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#gp_workfile_compression).
...
* **gp_workfile_limit_per_query**{#setting-gp-workfile-limits} {{ tag-con }} {{ tag-tf }} {{ tag-api }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#gp_workfile_limit_per_query).
...
* **gp_workfile_limit_files_per_query**{#setting-gp-workfile-limit-files} {{ tag-con }} {{ tag-tf }} {{ tag-api }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#gp_workfile_limit_files_per_query).
...
* **gp_workfile_limit_per_segment**{#setting-gp-workfile-limit-per-segment} {{ tag-con }} {{ tag-tf }} {{ tag-api }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#gp_workfile_limit_per_segment).
...
* **log_connections**{#setting-log-connections} {{ tag-con }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#log_connections).
...
* **log_disconnections**{#setting-log-disconnections} {{ tag-con }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#log_disconnections).

* **log_error_verbosity**{#setting-log-error-verbosity} {{ tag-con }}

    Defines the verbosity level for {{ GP }} log entries. Log detail levels, from least to most verbose:

    * `terse`.
    * `default` (default value).
    * `verbose`.

    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#log_error_verbosity).
...
* **log_hostname**{#setting-log-hostname} {{ tag-con }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#log_hostname).
...
* **log_min_duration_statement**{#setting-log-min-duration-statement} {{ tag-con }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#log_min_duration_statement).

* **log_min_messages**{#setting-log-min-messages} {{ tag-con }}

    Defines the {{ GP }} logging level. The system logs all messages with the selected severity level or higher. Possible values (in ascending order of severity): `DEBUG5`, `DEBUG4`, `DEBUG3`, `DEBUG2`, `DEBUG1`, `INFO`, `NOTICE`, `WARNING`, `ERROR`, `LOG`, `FATAL`, and `PANIC`.
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#log_min_messages).
...
* **log_statement**{#setting-log-statement} {{ tag-con }} {{ tag-tf }} {{ tag-api }}
    ...
    * `ALL`: Logs all SQL commands.
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#log_statement).
...
* **log_statement_stats**{#setting-log-statement-stat} {{ tag-con }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#log_statement_stats).
...
* **master_shared_buffers**{#setting-master-shared-buffers} {{ tag-con }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#shared_buffers).
...
* **max_connections**{#setting-max-connections} {{ tag-con }} {{ tag-tf }} {{ tag-api }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#max_connections).
...
* **max_prepared_transactions**{#setting-max-prepared-transactions} {{ tag-con }} {{ tag-tf }} {{ tag-api }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#max_prepared_transactions).
...
* **max_slot_wal_keep_size**{#setting-max-slot-wal-keep-size} {{ tag-con }} {{ tag-tf }} {{ tag-api }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#max_slot_wal_keep_size).
...
* **max_statement_mem**{#setting-max-statement-mem} {{ tag-con }} {{ tag-tf }} {{ tag-api }}
    ...
    For more information, see [this {{ GP }} article]({{ gp.docs.broadcom }}/7/greenplum-database/ref_guide-config_params-guc-list.html#max_statement_mem).
