...
* **gp_max_slices**{#setting-gp-gp-max-slices} {{ tag-con }} {{ tag-tf }} {{ tag-api }}
    ...
    Running a query that generates a large number of slices might affect Greenplum Database performance. For example, a query that contains `UNION` or `UNION ALL` operators over several complex views can generate a large number of slices. You can run `EXPLAIN` on the query to view slice statistics for the query.
