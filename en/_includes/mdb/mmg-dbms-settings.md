- **Net → Max incoming connections**{#setting-max-incoming-connections} {{ tag-con }} {{ tag-cli }} {{ tag-api }}
  ...
  The minimum value is `10`. The maximum value [depends on the selected host class](#settings-instance-dependent) and is equal to the total RAM size on the host in MB, with a maximum of `16384`. Default value: `1024`.
...
- **Operation profiling**{#setting-operation-profiling} {{ tag-con }} {{ tag-cli }} {{ tag-api }}
  ...
  - **Mode**{#setting-operation-profiling-mode}
    ...
    - `slowOp` (default): Information is only collected about slow operations (that take longer than the threshold value set by the [Slow op threshold](#setting-slow-op-threshold) setting).
