...
1. On the [cloud page]{% if lang == "ru" %}(https://console.cloud.yandex.ru/cloud){% endif %}{% if lang == "en" %}(https://console.cloud.yandex.com/cloud){% endif %}, create or select a folder for a new service.
...
1. [Create a {{ ydb-short-name }} database](../ydb/quickstart/create-db#control-plane) in Serverless mode. Save the **Endpoint** from the **Document API endpoint** section.
1. {% if audience == "external" %} [Create a table](../ydb/operations/schema.md) {% endif %} in the database:
...
* `convert`: transfer a video to convert. The function writes the task to the `tasks` table {% if audience == "external" %} using the [Document API](../ydb/quickstart/document-api/aws-http.md). {% endif %}
