By default, users with the `{{ roles-datalens-admin }}` role can export workbooks. You can disable this option at the {{ datalens-short-name }} instance level. After you prohibit export:
...
Only the {{ datalens-short-name }} instance [admin](../../../datalens/security/roles.md#datalens-admin), i.e., a user with the `{{ roles-datalens-admin }}` role, can disable export.
