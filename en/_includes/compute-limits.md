...
- Intel Ice Lake
   ...
   | Maximum number of disks{% if product == "yandex-cloud" %} and file stores{% endif %} attached to a single VM^2^ | Less than or equal to 32 vCPUs: 8<br>More than 32 vCPUs: 16^3^ |
   | Maximum number of security groups per interface | 5 |
   | Maximum number of VMs in a single [placement group](../compute/concepts/placement-groups.md) | 5 |
