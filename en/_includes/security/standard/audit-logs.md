...
You can also use {{ monitoring-name }} to monitor the health of the {{ at-name }} service itself and track security events. You can export metrics to a SIEM system via the {% if lang == "ru" %}[API](../../../glossary/rest-api.md){% else %}API{% endif %}, see the [instructions](../../../monitoring/operations/metric/get.md).
...
* MaxPatrol SIEM: [Collecting, monitoring, and analyzing audit logs in MaxPatrol SIEM](https://cloud.yandex.ru/docs/audit-trails/tutorials/maxpatrol)
