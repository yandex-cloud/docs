...
1. If the user already has at least one role, click ![icon-context-menu](../../_assets/console-icons/ellipsis.svg) and select **{{ ui-key.yacloud.common.resource-acl.button_new-bindings }}** in the row with this user.

    If the user is not on the list, click **{{ ui-key.yacloud.common.resource-acl.button_new-bindings }}** in the top-right corner. In the window that opens, select a user from the list or use the search bar.
