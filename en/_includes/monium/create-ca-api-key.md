...
1. Specify the service account name, e.g., `monium-ca`.
...
1. Select **{{ ui-key.yacloud.iam.folder.service-account.overview.field_key-scope }}**: `yc.monium.telemetry.write`.
   If you plan to transfer only particular data types, instead of `yc.monium.telemetry.write`, select one or more of the following roles: `yc.monium.metrics.write`, `yc.monium.logs.write`, and `yc.monium.traces.write`.
