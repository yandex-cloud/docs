...
* `sort_mode`: Rule defining the search results sorting mode. This is an optional parameter. The possible values are:
...
* `groups_on_page`: Maximum number of groups that can be returned per search results page. This is an optional parameter. The values range from `1` to `100`. Default value: `20`.

* `docs_in_group`: Maximum number of documents that can be returned per group. This is an optional parameter. The values range from `1` to `3`. Default value: `1`.
