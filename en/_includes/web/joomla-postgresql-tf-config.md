...
```hcl
# Declaring variables for secret parameters
...
# Creating a subnet in the {{ region-id }}-a
 availability zone
resource "yandex_vpc_subnet" "joomla-pg-network-subnet-a" {
  name           = local.subnet_name1
  zone           = "{{ region-id }}-a"
...
# Creating a database cluster {{ PG }}
...
# Creating a CNAME-type DNS record
