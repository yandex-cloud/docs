...
{{ managed-k8s-name }} {{ alb-name }} tools, e.g., [ingress controller]({{ ingress-local-link }}/index.md) and [Gateway API]({{ gateway-local-link }}/index.md), use a [service account](../../../iam/concepts/users/service-accounts.md) to deploy infrastructure. This account requires the following permissions:
...
- Ingress controller
  ...
  | ----- | ----- | ----- |
  | {{ alb-name }}<br/>([roles](../../../application-load-balancer/security/index.md)) | Service resource management | `alb.editor` |
