...
To ensure proper operation of the [ingress controller]({{ ingress-local-link }}/index.md) and [Gateway API]({{ gateway-local-link }}/index.md), you must configure appropriate [security groups](../../../vpc/concepts/security-groups.md) for your [cluster](../../../managed-kubernetes/concepts/index.md#kubernetes-cluster), [{{ managed-k8s-full-name }} node groups](../../../managed-kubernetes/concepts/index.md#node-group), and [{{ alb-name }}](../../../application-load-balancer/concepts/application-load-balancer.md).
...
* All required security group rules documented in the relevant guides:
  ...
  * For the load balancer, see [{#T}](../../../application-load-balancer/concepts/application-load-balancer.md#security-groups). The final outbound rule for [backend](../../../application-load-balancer/concepts/backend-group.md) VMs must allow connections to either the CIDR range of your cluster’s [node group](../../../managed-kubernetes/concepts/index.md#node-group) [subnet](../../../vpc/concepts/network.md#subnet) or the security group associated with your node groups.
...
* Cluster and node group’s security group for control plane traffic:
  ...
  - Incoming traffic {#incoming}
    ...
    `{{ port-any }}` | `{{ ui-key.yacloud.vpc.network.security-groups.forms.value_any }}`| `{{ ui-key.yacloud.vpc.network.security-groups.forms.value_sg-rule-destination-sg }}` | `{{ ui-key.yacloud.vpc.network.security-groups.forms.value_sg-rule-sg-type-self }}` | For traffic between the control plane components and worker nodes
...
* Node group’s security group for control plane traffic:
  ...
  - Outgoing traffic {#outgoing}
    ...
    {{ ui-key.yacloud.vpc.network.security-groups.forms.field_sg-rule-port-range }} | {{ ui-key.yacloud.vpc.network.security-groups.forms.field_sg-rule-protocol }} | {{ ui-key.yacloud.vpc.network.security-groups.forms.field_sg-rule-destination }} | {{ ui-key.yacloud.vpc.network.security-groups.forms.field_sg-rule-cidr-blocks }} | {{ ui-key.yacloud.vpc.network.security-groups.forms.field_sg-rule-description }}
    --- | --- | --- | --- | ---
    `{{ port-any }}` | `{{ ui-key.yacloud.vpc.network.security-groups.forms.value_any }}` | `{{ ui-key.yacloud.vpc.network.security-groups.forms.value_sg-rule-destination-cidr }}` | `0.0.0.0/0` | For external resource access
...
{% include [terraform-security-groups-example](../../application-load-balancer/tf-security-groups-example.md) %}