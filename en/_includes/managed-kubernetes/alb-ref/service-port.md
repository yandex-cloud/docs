...
* In `HttpBackendGroup`, in the `spec.backends.service.port.number` field (see the [configuration]({{ configuration-local-link }}/http-backend-group.md)).
||
...
* In `HttpBackendGroup`, in the `spec.backends.service.port.name` field (see the [configuration]({{ configuration-local-link }}/http-backend-group.md)).
||
