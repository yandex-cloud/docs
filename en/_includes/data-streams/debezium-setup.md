...
1. Go to the `conf` folder and create a file named `application.properties` with the following contents:
    ...
    * `<endpoint>`: Endpoint for a {{ yds-short-name }} data stream, e.g., `https://yds.serverless.yandexcloud.net/{{ region-id }}/b1g89ae43m6he********/etn01eg4rn1********`. You can see the endpoint on the stream page (see [{#T}](../../data-streams/operations/manage-streams.md#list-data-streams)).
