...
1. Enter the notification channel **{{ ui-key.yacloud.cns.field_channel-name }}**. The channel names are unique within the entire {{ cns-short-name }}. It may contain lowercase and uppercase Latin letters, numbers, underscores, hyphens, and periods. It may be from 1 to 256 characters long. For APNs channels, we recommend specifying the bundle ID in the name; for FCM and HMS, the full package name; for RuStore, `packageName`.
