* `--opensearch-node-group`: `{{ OS }}` host group configuration, where:
   ...
   * `resource-preset-id`: Host class that defines the specifications of virtual machines the {{ OS }} nodes will be deployed on. All available options are listed under [Host classes](../../managed-opensearch/concepts/instance-types.md).
