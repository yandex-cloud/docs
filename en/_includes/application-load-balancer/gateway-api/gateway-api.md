...
* The `Gateway` resource is managed by the cluster operator. The resource describes the incoming traffic reception and routing (`HTTPRoute` and `GRPCRoute`) rules. The system creates an [L7 load balancer](../../../application-load-balancer/concepts/application-load-balancer.md) to handle traffic for the `Gateway`. The load balancer uses [HTTP routers](../../../application-load-balancer/concepts/http-router.md) to route incoming traffic.
...
* [Gateway]({{ configuration-local-link }}/gateway.md): Incoming traffic reception and routing (`HTTPRoute`) rules.
* [GatewayPolicy]({{ configuration-local-link }}/gateway-policy.md): `Gateway` resource policy, incoming traffic processing configuration.
* [YCCertificate]({{ configuration-local-link }}/yc-certificate.md): {{ certificate-manager-name }} certificate parameters for setting up TLS connections in the `Gateway` resource.
* [HTTPRoute]({{ configuration-local-link }}/http-route.md) and [GRPCRoute]({{ configuration-local-link }}/grpc-route.md): Rules for traffic routing between backends or redirection.
* [RoutePolicy]({{ configuration-local-link }}/route-policy.md): `HTTPRoute` resource policy and backend access rule configuration.
* [YCStorageBucket]({{ configuration-local-link }}/yc-storage-bucket.md): {{ objstorage-name }} bucket parameters for setting up a backend in the `HTTPRoute` resource.
* [Service]({{ configuration-local-link }}/service-for-gateway.md): Description of {{ k8s }} services used as backends.
