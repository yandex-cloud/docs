...
* Do not transfer tables without primary keys. You can add them to the list of excluded tables in [source endpoint settings](../../data-transfer/operations/endpoint/source/postgresql.md#additional-settings).
* Add the replica ID to tables without `primary key`:
...
If there are no primary keys in a table, logical replication [will not capture row changes]({{ pg-docs }}/logical-replication-publication.html) (`UPDATE`, `DELETE`).
