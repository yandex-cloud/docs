...
You can manage the hierarchy of cloud resources and route DNS requests. For example, you can create subzones for the production and test environments and, within them, subzones for applications, DB clusters, caching servers, and so on.

![image](../../_assets/dns/zones-example-2.svg "DNS hierarchy example")
...
For security reasons, public subzones may only be located in a single folder. Remember this when organizing the structure of your domain names. For more complex scenarios, contact [support](../../support/overview.md).
...
Domain names from internal zones are only available for use in [{{ vpc-name }}](../../vpc) (VPC) networks specified when creating a zone. Within internal zones, you can use the entire namespace in the subnets of the selected network, including `internal.` and `.`.
...
Internal zones can be created in {{ vpc-short-name }} networks automatically. A list of these zones depends on the address range of the subnets used. For example:

{% cut "{{ yandex-cloud }} internal zones" %}
