---
title: Managing access to a {{ msp-full-name }} cluster
description: Follow this guide to configure {{ msp-name }} cluster access permissions.
---

# Managing access to a {{ msp-name }} cluster
...
- CLI {#cli}
  ...
  1. To get a list of roles assigned for the cluster, run this command:
      ...
      ```bash
      {{ yc-mdb-sp }} cluster list-access-bindings <cluster_name_or_ID>
      ```
...
- CLI {#cli}
  ...
  1. Use this command to assign a role:
      ...
      ```bash
      {{ yc-mdb-sp }} cluster add-access-binding <cluster_name_or_ID> \
        --role <role> \
      ...
      * `--role`: [Role](../security.md#roles-list), e.g., `managed-spark.editor`.
...
- gRPC API {#grpc-api}
  ...
  1. Call the [ClusterService.UpdateAccessBindings](../api-ref/grpc/Cluster/updateAccessBindings.md) method, e.g., via the following {{ api-examples.grpc.tool }} request:
      ...
      * `access_binding_deltas.roleId`: [Role](../security.md#roles-list), e.g., `managed-spark.editor`.
...
- CLI {#cli}
  ...
  1. To assign roles, run this command:
      ...
      ```bash
      {{ yc-mdb-sp }} cluster set-access-bindings <cluster_name_or_ID> \
...
- gRPC API {#grpc-api}
  ...
  1. Call the [ClusterService.SetAccessBindings](../api-ref/grpc/Cluster/setAccessBindings.md) method, e.g., via the following {{ api-examples.grpc.tool }} request:
      ...
      * `resource_id`: Cluster ID.
      * `accessBindings.roleId`: [Role](../security.md#roles-list), e.g., `managed-spark.editor`.
...
- CLI {#cli}
  ...
  1. To revoke a role, run this command:
      ...
      ```bash
      {{ yc-mdb-sp }} cluster remove-access-binding <cluster_name_or_ID> \
        --role <role> \
        --subject <subject_type>:<subject_ID>
      ```
      ...
      * `--role`: [Role](../security.md#roles-list) being revoked, e.g., `managed-spark.editor`.
...
- gRPC API {#grpc-api}
  ...
  1. Call the [ClusterService.UpdateAccessBindings](../api-ref/grpc/Cluster/updateAccessBindings.md) method, e.g., via the following {{ api-examples.grpc.tool }} request:
      ...
      * `resource_id`: Cluster ID.
      * `access_binding_deltas.roleId`: [Role](../security.md#roles-list) being revoked, e.g., `managed-spark.editor`.
...
For a service account to be able to view the info of all {{ msp-name }} clusters in the folder but modify the resources in one particular cluster alone, give it the `managed-spark.viewer` role for the folder and the `managed-spark.editor` role for that cluster:
...
- CLI {#cli}
  ...
  1. Assign the roles for the cluster:
      ...
      ```bash
      {{ yc-mdb-sp }} cluster add-access-bindings <cluster_name_or_ID> \
        --access-binding role=managed-spark.editor,subject=serviceAccount:<service_account_ID>
      ```
