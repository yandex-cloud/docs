...
[Draw.io](https://drawio.yandex-team.ru/) is an online service for creating diagrams.

To add a diagram to a Wiki page:

1. Draw a diagram in draw.io editor.
1. In the **File** menu, select **Embed** → **Yandex Wiki**.
1. Configure your diagram parameters and click **Create**.
1. Copy the code.

Embed the resulting `iframe` block on your page:
...
You can learn more about the `iframe` block parameters in [{#T}](iframe.md).

{% note info %}

To edit your diagram, go back to draw.io, generate a new embed code, and paste it into the page. Otherwise, the diagram hosted on {{ wiki-name }} won't update.

If you edit the diagram on the Wiki page, the edited copy will appear on draw.io. However, the changes will not be applied to the original diagram.
