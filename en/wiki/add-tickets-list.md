...
You can add {{ wiki-name }} links to individual issues in [{{ tracker-name }}]({{ link-tracker }}) or automatically generated issue lists to pages. The appearance of these links changes depending on what happens to the issue: who is its assignee, what is its status, and so on.
