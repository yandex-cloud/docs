...
1. Configure the application:
   ...
   * **Public IP address**: Enable this optional setting if you need to assign a public IP address to the service. For Thumbor to run, a {{ k8s }} service of the `LoadBalancer` type and the appropriate [{{ network-load-balancer-full-name }}](../../../network-load-balancer) load balancer will be created automatically.
...
1. To install a [Helm chart](https://helm.sh/docs/topics/charts/) with Thumbor, run the following command, specifying the parameters of the resources you created [earlier](#before-you-begin):
   ...
   {% endlist %}
    ...
    If `public_address` is set to `true`, a {{ k8s }} service of the `LoadBalancer` type and a corresponding [{{ network-load-balancer-name }}](../../../network-load-balancer) will be created automatically to ensure Thumbor runs correctly.
...
* [{{ network-load-balancer-name }} guides](../../../network-load-balancer/)
