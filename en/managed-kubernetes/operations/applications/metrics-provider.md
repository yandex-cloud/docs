...
{{ MP }} streams metrics of [{{ managed-k8s-name }} cluster](../../concepts/index.md#kubernetes-cluster) objects to monitoring systems and [auto scaling systems](../../concepts/autoscale.md). You can also broadcast metrics the other way round: for example, cluster objects can receive metrics from [{{ monitoring-full-name }}](../../../monitoring/concepts/index.md).
...
1. Set up and install {{ MP }}:
   ...
   helm install \
     ...
     --set-file yandexMetrics.token.serviceAccountJson=<path_to_file_with_authorized_service_account_key> \
