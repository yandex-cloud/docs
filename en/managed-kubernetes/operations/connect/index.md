...
You can connect to a [{{ managed-k8s-name }} cluster](../../concepts/index.md#kubernetes-cluster) in the following ways:
...
To connect to cluster nodes, follow the steps in [{#T}](../node-connect-ssh.md).
...
When connecting to a {{ k8s }} cluster, the user [logs in to {{ iam-full-name }}](../../../iam/concepts/authorization/index.md) and gets access permissions based on the [assigned role](../../security/index.md#yc-api). To log in, install the [{{ yandex-cloud }} command-line interface (CLI)](../../../cli/quickstart.md).
