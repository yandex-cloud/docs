...
[Kubernetes](https://Kubernetes.io) is an environment for managing containerized applications. Kubernetes offers ways to run clusters that can automate tasks like deploying, scaling, and managing applications in containers.

The main entity used in the service is a _cluster Kubernetes_.
...
_A master_ is a node that manages a Kubernetes cluster.

The master runs Kubernetes control processes that include the Kubernetes API server, scheduler, and main resource controllers. The master's lifecycle is managed by the service when creating or deleting a Kubernetes cluster. The master is responsible for global solutions that are run on all Kubernetes cluster nodes. These include scheduling workloads (such as containerized applications), managing the lifecycle of workloads, and scaling.

### Node group {#node-group}

_A node group_ is a group of VMs with the same configuration in a Kubernetes cluster that is running the user's containers.
