...
* When [creating a cluster](../operations/kubernetes-cluster/kubernetes-cluster-create.md), select the [Regional](../concepts/index.md#master) master type. {{ k8s }} [services](service.md) will be available in the event of a failure at the [availability zone](../../overview/concepts/geo-scope.md) level. [The Service Level Agreement]{% if product == "yandex-cloud" %}{% if lang == "ru" %}(https://yandex.ru/legal/cloud_sla_kb/){% endif %}{% if region == "en" %}(https://yandex.com/legal/cloud_sla_kb/){% endif %}{% endif %}{% if product == "cloud-il" %}(https://nebius.com/il/docs/legal/sla-levels/managed-kubernetes){% endif %} {{ managed-k8s-name }} applies to the configuration with the regional master.
