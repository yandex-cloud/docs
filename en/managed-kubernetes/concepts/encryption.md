...
A _secret_ is confidential information used by a {{ k8s }} cluster when managing pods, such as{% if product == "yandex-cloud" %} OAuth keys,{% endif %} passwords, SSH keys, and so on. By default, {{ k8s }} keeps secrets as plaintext. To learn more, see the [{{ k8s }} documentation](https://kubernetes.io/docs/concepts/configuration/secret/).
