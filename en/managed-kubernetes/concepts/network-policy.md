...
* Create richer network policies, for example, by filtering pod-to-pod traffic at the [L7 application layer]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/Сетевая_модель_OSI#Уровни_модели_OSI){% endif %}{% if lang == "en" %}(https://en.wikipedia.org/wiki/OSI_model#Layer_architecture){% endif %} or using the DNS name of an external resource.
