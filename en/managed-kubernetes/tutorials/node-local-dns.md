...
To reduce the number of DNS queries to a [{{ k8s }} cluster](../concepts/index.md#kubernetes-cluster), enable NodeLocal DNS Cache. The feature is available in {{ k8s }} clusters version {% if product == "cloud-il" %}1.18{% endif %}{% if product == "yandex-cloud" %}1.20{% endif %} and higher.
...
1. Create a {{ k8s }} cluster and a group of nodes.
   ...
   You can use an existing cluster and a group of {{ k8s }} nodes or create new ones. Make sure that the **{{ k8s }} version** field is set to version {% if product == "cloud-il" %}1.18{% endif %}{% if product == "yandex-cloud" %}1.20{% endif %} or higher.
