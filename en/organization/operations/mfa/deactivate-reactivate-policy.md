...
- {{ TF }} {#tf}
  ...
  1. Open the {{ TF }} configuration file and update the `status` parameter in the `yandex_organizationmanager_mfa_enforcement` resource:
     ...
     For more on `yandex_organizationmanager_mfa_enforcement` properties, see [this provider guide]({{ tf-provider-resources-link }}/organizationmanager_mfa_enforcement).
