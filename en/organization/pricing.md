...
description: This article covers the {{ org-full-name }} pricing policy.
---
...
## What goes into the cost of using {{ org-full-name }} {#rules}
...
You can [manage](#pricing-management) {{ org-full-name }} billing through the [{{ cloud-center }} interface]({{ link-org-cloud-center }}). For more information, see [{#T}](./operations/manage-billing.md).
...
On their first login to an external application, an {{ org-full-name }} user starts consuming their [organization’s](./concepts/organization.md) monthly _connection limit_. Then, until the month ends, this user can log in to any external applications without limitations.
...
By default, the organization has a non-billable limit of 15 users per month who can authenticate to external applications. This means you will not be charged for using {{ org-full-name }}.
