---
title: Managing access to a {{ mrd-full-name }} cluster
...
- REST API {#api}
  ...
  1. Call the [Cluster.ListAccessBindings](../api-ref/Cluster/listAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --url 'https://{{ api-host-mdb }}/managed-redis/v1/clusters/<cluster_ID>:listAccessBindings'
      ```
...
- CLI {#cli}
  ...
  1. Use this command to assign a role:
      ...
      ```bash
      {{ yc-mdb-rd }} cluster add-access-binding <cluster_name_or_ID> \
        --role <role> \
...
- REST API {#api}
  ...
  1. Call the [Cluster.UpdateAccessBindings](../api-ref/Cluster/updateAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://{{ api-host-mdb }}/managed-redis/v1/clusters/<cluster_ID>:updateAccessBindings' \
        --data '{
...
- CLI {#cli}
  ...
  1. To assign roles, run this command:
      ...
      ```bash
      {{ yc-mdb-rd }} cluster set-access-bindings <cluster_name_or_ID> \
...
- {{ TF }} {#tf}
  ...
  1. Add resource descriptions:
      ...
      resource "yandex_mdb_redis_cluster_iam_binding" "<resource_2_local_name>" {
        cluster_id = "<cluster_ID>"
        role       = "<role_2>"
        members    = ["<subject_type>:<subject_ID>"]
      }
...
- REST API {#api}
  ...
  1. Call the [Cluster.SetAccessBindings](../api-ref/Cluster/setAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://{{ api-host-mdb }}/managed-redis/v1/clusters/<cluster_ID>:setAccessBindings' \
        --data '{
                  "accessBindings": [
                    {
                      "roleId": "<role>",
...
- CLI {#cli}
  ...
  1. To revoke a role, run this command:
      ...
      ```bash
      {{ yc-mdb-rd }} cluster remove-access-binding <cluster_name_or_ID> \
        --role <role> \
        --subject <subject_type>:<subject_ID>
      ```
      ...
      * `--role`: [Role](../security/index.md#roles-list) being revoked, e.g., `managed-redis.editor`.
...
- REST API {#api}
  ...
  1. Call the [Cluster.UpdateAccessBindings](../api-ref/Cluster/updateAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      * `access_binding_deltas.roleId`: [Role](../security/index.md#roles-list) being revoked, e.g., `managed-redis.editor`.
...
For a service account to be able to view the info of all {{ mrd-name }} clusters in the folder but modify the resources in one particular cluster alone, give it the `managed-redis.viewer` role for the folder and the `managed-redis.editor` role for that cluster:
...
- CLI {#cli}
  ...
  1. Assign the roles for the cluster:
      ...
      ```bash
      {{ yc-mdb-rd }} cluster add-access-bindings <cluster_name_or_ID> \
        --access-binding role=managed-redis.editor,subject=serviceAccount:<service_account_ID>
      ```
