...
1. Wait for the cluster state to change to **Alive**. In the [management console]({{ link-console-main }}), select the folder and [navigate]({{ link-console-main }}/link/managed-valkey) to **{{ ui-key.yacloud.iam.folder.dashboard.label_managed-redis }}**. You can check the cluster state in the **{{ ui-key.yacloud.mdb.clusters.column_availability }}** column.
