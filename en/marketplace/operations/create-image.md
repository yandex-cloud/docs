...
To add a product for {{ compute-full-name }} to the Marketplace, you need to upload the image to {{ yandex-cloud }}. The products can be based on Linux{% if product == "cloud-il" %} and Windows Server{% endif %}. This section will help you create an image based on Linux OS.{% if product == "cloud-il" %} To create an image with Windows, see [{#T}](create-image-ms.md).{% endif %}
