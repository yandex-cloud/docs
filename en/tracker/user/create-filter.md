...
On the top panel in {{ tracker-name }}, choose **Issues** → **Find issues**.
...
- **Query language**
    ...
    You can use the query language to set special conditions that are not accessible via normal parameters. For instance, you can find issues that have been opened within the last week. To learn more about the query language, see [{#T}](query-filter.md).
...
You can save your filter so you don't have to set it up each time you want to use it. Your saved filters are available on the {{ tracker-name }} top panel in **Issues** → **My filters**.
...
1. On the {{ tracker-name }} top panel in **Issues** → **My filters**, choose your filter.

1. [Change the issue search conditions](#set-conditions) using quick filters, parameters, or the query language.

1. Click **Save filter** above the issue list.

    To save a filter under a different name, click ![](../../_assets/tracker/icon-save-as.png) and give it a name.
