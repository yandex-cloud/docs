...
{% if audience == "internal" %} ## Converting a comment into an email message {#create-comment-from-mail}

Respond to any notification about an issue's status change to create a comment via email:

1. Open an email with a notification about an issue change.

1. Write the comment text in the message body.

1. Click **Submit**.

The comment is created automatically after the message is sent.{% endif %}
