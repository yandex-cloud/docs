...
| Colored text.<br/><br/>Supported colors:<br/><br/>`red` {% if locale == "ru" %}/ `red` / `red`{% endif %}.<br/>`green` {% if locale == "ru" %}/ `green` / `green`{% endif %}.<br/>`blue` {% if locale == "ru" %}/ `blue` / `blue`{% endif %}.<br/>`grey` / `gray` {% if locale == "ru" %}/ `grey`{% endif %}.<br/>`yellow` {% if locale == "ru" %}/ `yellow`{% endif %}. | ```!!(blue)Blue text!!``` | ![](../../_assets/tracker/format/colored.png) |
