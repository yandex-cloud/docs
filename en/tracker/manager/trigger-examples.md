...
- How to configure [Dynamic checklists](#checklist).
...
1. [Set up an email address for the queue]{% if audience == "external" %}(queue-mail.md#section_gwv_hqb_hgb){% else %}(queue-mail.md#sec-mail-yandex){% endif %} to store issues created based on user requests.
...
1. If the users aren't Yandex employees, [allow sending emails from issue pages to external addresses](queue-mail.md#send_outside).
...
1. Go to [{{ forms-full-name }}]({{ link-forms }}) and create a form. {% if audience == "internal" %}For information about which admin panel to create a form in, [see the documentation](../../forms/go-to-forms.md).{% endif %}
...
1. Use the following code as the comment's text:
   ...
   ```
   {{=<% %>=}}/iframe/(src="https://forms.yandex-team.ru/surveys/<form_id>/?iframe=1&<question_id>=<value>" frameborder=0 width=500)
   ```
...
#### Step 3. Add {% if audience == "internal" %}robot-forms@{% else%}yndx-forms-cnt-robot@{% endif %} to the queue

To ensure the form is inserted correctly, provide access to the queue for the {% if audience == "internal" %}`robot-forms@`{% else%}yndx-forms-cnt-robot@{% endif %} robot. To learn more about setting up access, see [Setting access rights for queues](queue-access.md).
...
## Dynamic checklists {#checklist}

To single out issue completion stages, use [checklists](../user/checklist.md). Using a trigger and [{{ api-name }}](../about-api.md), you can set up auto creation and update of a checklist:
...
1. Specify the request parameters. In the **Request body** field, set the checklist item parameters:
   ...
   >        "assignee": "not_var{{issue.author.login}}"
   >    },
   >    {
   >        "checked": false,
   ...
   > ]
   > ```
   For more information about the request, see [{#T}](../concepts/issues/add-checklist-item.md). To specify the request parameters, use [variables](../user/vars.md). ||
   |#

1. To save the trigger, click **Create**.

Once the trigger is fired, a checklist is created in the issue. If the issue already contains a checklist, new items are added to it.
...
1. Specify the request parameters. In the **Request body** field, set the parameters of a new sub-issue.
   ...
   {% if audience == "external" %}|| Headers | Header: `X-Org-ID`.
   Value: Organization ID. The ID is shown in the **Organization ID for API** field on the [{{ tracker-name }} settings]({{ link-settings }}) page. ||{% endif %}
   |#
