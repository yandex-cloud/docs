...
To set up email integration{% if audience == "external" %}:{% else %}, [add an email address to the queue](#sec-mail-yandex). You can add multiple addresses for creating issues with different types or different [components](components.md).{% endif %}
...
## Adding an email address {#sec-mail-yandex}

To add an email address to your queue, follow these steps:

1. [Create](https://ml.yandex-team.ru/request/) a mailing list with the appropriate email address.

1. To link the created mailing list to your queue, contact {{ tracker-name }} support using this [form](https://forms.yandex-team.ru/surveys/14549/):

    1. Under **Issue area \ problems**, select **Separator (ticket creation based on email)**.

    1. Under **What's your question**, select **Link a mailing list to a queue**.

    1. Specify the parameters for linking the mailing list to the queue.

1. To change the mailing list settings, for example, create sender names or link new components, use the same form.
