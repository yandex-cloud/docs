...
Teams that follow the agile software development approach use the [Planning poker]{% if locale == "ru" %}(https://ru.wikipedia.org/wiki/Poker_планирования){% else %}(https://en.wikipedia.org/wiki/Planning_poker){% endif %} technique for estimating their workload. Each team member individually rates an issue's complexity by using Story Points as a scoring system. The team members then announce and discuss their scores and decide on a final score. Planning poker usually requires special cards with numbers that represent the issue's score.
