...
1. In the form integration settings, configure [issue creation in {{ tracker-name }}]{% if audience == "external" %}({{ support-forms-tracker }}){% else %}({{ support-forms-tracker-ya }}){% endif %} for the first support line:

    1. Set up a [notification conditions]{% if audience == "external" %}({{ support-forms-notify }}){% else %}({{ support-forms-notify-ya }}){% endif %}, the <q>Request subject</q> list contains the subjects selected for the first support line.
