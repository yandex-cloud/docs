...
Apart from workflows, you can configure the following in the queue: [access rights to issues](manager/queue-access.md), [automated issue processing ](./automation.md), integration with {% if audience == "internal" %}[Mail](manager/queue-mail.md) and{% endif %} [{{ forms-full-name }}](manager/forms-integration.md), and so on. These features let you flexibly configure queues to suit your team's tasks.
