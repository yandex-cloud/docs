...
User requests are converted to {{ tracker-name }} issues before they are processed. Most users tend to send their requests via email or a website. In any case, {% if audience == "external" %}users outside your company{% else %}external users{% endif %} don't have access to {{ tracker-name }}.
...
To accept user requests by email, {% if audience == "external" %}set up the email address of the queue{% else %}[set up the email address for the queue](manager/queue-mail.md#sec-mail-yandex) where issues will be created based on user requests.{% endif %}. All emails sent to this address will automatically be converted to {{ tracker-name }} issues.
