For large organizations (100 users and up), the cost of using {{ tracker-name }} is [calculated on a progressive scale](../../tracker/pricing.md#sec_calculate). For the first 100 users, full access to {{ tracker-name }} costs {{ sku|RUB|b2b.tracker.license.v1|pricingRate.6|int|string }} per user per month. If 100 employees are already subscribed to {{ tracker-name }}, the subscription fee for the next 150 users will be lower: {{ sku|RUB|b2b.tracker.license.v1|pricingRate.100|int|string }} per user per month. If more than 250 employees are subscribed, the monthly fee for new users is {{ sku|RUB|b2b.tracker.license.v1|pricingRate.250|int|string }}.
...
| 6–100 | {{ sku|RUB|b2b.tracker.license.v1|pricingRate.6|int|string }} (including first 5) |
