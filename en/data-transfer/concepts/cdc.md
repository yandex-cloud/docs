...
{% if audience == "external" %}The implementation maturity may vary among different {{ data-transfer-full-name }} installation types. Transfers between {{ MG }} sources and {{ KF }} targets are not supported yet. See the list of available transfers [here](../transfer-matrix.md).{% endif %}
