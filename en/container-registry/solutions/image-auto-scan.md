...
This scenario describes how to set up automatic vulnerability [scanning](../concepts/vulnerability-scanner.md) of [Docker images](../concepts/docker-image.md) at push.
...
1. Create a [registry](../concepts/registry.md) to push a Docker image to.
   ...
   - Management console
     ...
     1. In the [management console]({{ link-console-main }}), select a [folder](../../resource-manager/concepts/resources-hierarchy.md#folder) where you want to create a registry.
...
1. Create a [service account](../../iam/concepts/users/service-accounts.md) named `editor` and assign to it the `editor` role for the folder where you created the registry.
   ...
   - API
     ...
     Use the [create](../../iam/api-ref/ServiceAccount/create.md) method for the [ServiceAccount](../../iam/api-ref/ServiceAccount/) resource and [updateAccessBindings](../../resource-manager/api-ref/Folder/updateAccessBindings.md) for the [Folder](../../resource-manager/api-ref/Folder/) resource.
...
Create a [function](../../functions/concepts/function.md) in {{ sf-full-name }} named `scan-on-push` that will run the image scan:
...
- Management console
  ...
  1. Go to **Editor** and create a version of the function:
     1. Under **Function code**:
        ...
        * Select how you want to edit the **Code editor** function.
...
- API
  ...
  Use the [create](../../functions/functions/api-ref/Function/create) and [createVersion](../../functions/functions/api-ref/Function/createVersion) for the [Function](../../functions/functions/api-ref/Function/) resource.
...
- Management console
  ...
  1. Under **Container Registry settings**:
     ...
     * In the **Event types** field, select the [event](../../functions/concepts/trigger/cr-trigger.md#event) **Creating a Docker image tag**.
...
1. Log in to the registry under your username:
   ...
   - Using a Docker Credential helper
     ...
     1. Configure Docker to use `docker-credential-yc`:
        ...
        The Credential helper only works when using Docker without `sudo`. For information about how to configure Docker to run as the current user without `sudo`, see the [official Docker documentation](https://docs.docker.com/engine/install/linux-postinstall/#manage-docker-as-a-non-root-user).
...
1. Assign a tag to the Docker image:
   ...
   ```bash
   docker tag ubuntu:20.04 cr.yandex/<registry ID>/ubuntu:20.04
   ```

1. Push the Docker image to {{ container-registry-name }}:

   ```bash
   docker push cr.yandex/<registry ID>/ubuntu:20.04
   ```
