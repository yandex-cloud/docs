...
* As a user [using a {{ iam-full-name }} token](#iam) (maximum lifetime is **{{ iam-token-lifetime }}**).
...
1. [Get an {{ iam-name }} token]{% if product == "yandex-cloud" %}(../../iam/operations/iam-token/create.md){% endif %}{% if product == "cloud-il" %}(../../iam/operations/iam-token/create-for-federation.md){% endif %}.
1. Run the command:
...
These instructions only apply to external {{ k8s }} clusters{% if product == "yandex-cloud" %} and [external nodes](../../managed-kubernetes/concepts/external-nodes.md){% endif %} managed by {{ managed-k8s-name }}.
