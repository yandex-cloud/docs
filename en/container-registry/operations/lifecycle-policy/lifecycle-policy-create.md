...
To create a lifecycle policy, specify the [repository ID](../repository/repository-list.md#repository-get-by-name).
...
- CLI
    ...
    1. Create a lifecycle policy by running the command with the following parameters:
        ...
        * `rules`: Path to the file with the policy description.
        ...
        The default policy is created with the `DISABLED` status. You can create an active policy at once by specifying the `--active` flag, but we don't recommend this.
