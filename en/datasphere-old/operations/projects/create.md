# Creating an empty project

{% include [link-on-new-ui-datasphere-in-old-ui](../../../_includes/datasphere/datasphere-old-note.md) %}

A project in {{ ml-platform-name }} is a [{{ jlab }}Lab](https://jupyter.org/) development environment running on a {{ yandex-cloud }} {% if lang == "ru" and audience != "internal" %}[VM](../../../glossary/vm.md){% else %}VM{% endif %}.

These instructions describe how to create an empty project. If you already have an `.ipynb` file, you can create a project from it.

{% include [include](../../../_includes/datasphere/create-project.md) %}

#### What's next {#what-is-next}

* [{#T}](install-dependencies.md)
* [{#T}](control-compute-resources.md)
* [{#T}](export.md)
