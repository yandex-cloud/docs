...
1. In the [manageent console]({{ link-console-main }}), select **{{ mch-name }}** from the list on the left.
...
1. Create an application:
   ...
   1. Go to **Platforms** → **Web services**. In the **Callback URI #1** field, insert `https://oauth.yandex.ru/verification_code`.
...
1. Click `https://oauth.yandex.ru/authorize?response_type=token&client_id=<app ID>`. For the `client_id` parameter, add the copied ID of your app.
1. Click **Log in as …**.
