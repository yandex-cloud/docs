...
1. Open an SSH connection to the {{ dataproc-name }} host by specifying its FQDN or IP address if public access is enabled for the host. {% if product == "yandex-cloud" %} For image 2.0, specify the `ubuntu` user, for image 1.4 — `root`. For example: {% endif %} {% if product == "cloud-il" %} Specify `ubuntu` as the username. For example: {% endif %}
