---
title: Managing access to a {{ dataproc-full-name }} cluster
description: Follow this guide to configure {{ dataproc-name }} cluster access permissions.
---

# Managing access to a {{ dataproc-name }} cluster
...
- CLI {#cli}
  ...
  1. To get a list of roles assigned for the cluster, run this command:
      ...
      ```bash
      {{ yc-dp }} cluster list-access-bindings <cluster_name_or_ID>
      ```
...
- REST API {#api}
  ...
  1. Call the [Cluster.ListAccessBindings](../api-ref/Cluster/listAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --url 'https://dataproc.{{ api-host }}/dataproc/v1/clusters/<cluster_ID>:listAccessBindings'
      ```
...
- CLI {#cli}
  ...
  1. Use this command to assign a role:
      ...
      ```bash
      {{ yc-dp }} cluster add-access-binding <cluster_name_or_ID> \
        --role <role> \
      ...
      * `--role`: [Role](../security/index.md#roles-list), e.g., `dataproc.editor`.
...
- REST API {#api}
  ...
  1. Call the [Cluster.UpdateAccessBindings](../api-ref/Cluster/updateAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://dataproc.{{ api-host }}/dataproc/v1/clusters/<cluster_ID>:updateAccessBindings' \
        --data '{
      ...
      * `access_binding_deltas.roleId`: [Role](../security/index.md#roles-list), e.g., `dataproc.editor`.
...
- CLI {#cli}
  ...
  1. To assign roles, run this command:
      ...
      ```bash
      {{ yc-dp }} cluster set-access-bindings <cluster_name_or_ID> \
...
- REST API {#api}
  ...
  1. Call the [Cluster.SetAccessBindings](../api-ref/Cluster/setAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://dataproc.{{ api-host }}/dataproc/v1/clusters/<cluster_ID>:setAccessBindings' \
        --data '{
                  "accessBindings": [
                    {
                      "roleId": "<role>",
      ...
      * `accessBindings.roleId`: [Role](../security/index.md#roles-list), e.g., `dataproc.editor`.
...
- CLI {#cli}
  ...
  1. To revoke a role, run this command:
      ...
      ```bash
      {{ yc-dp }} cluster remove-access-binding <cluster_name_or_ID> \
        --role <role> \
        --subject <subject_type>:<subject_ID>
      ```
      ...
      * `--role`: [Role](../security/index.md#roles-list) being revoked, e.g., `dataproc.editor`.
...
- REST API {#api}
  ...
  1. Call the [Cluster.UpdateAccessBindings](../api-ref/Cluster/updateAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      * `access_binding_deltas.roleId`: [Role](../security/index.md#roles-list) being revoked, e.g., `dataproc.editor`.
...
For a service account to be able to view the info of all {{ dataproc-name }} clusters in the folder but modify the resources in one particular cluster alone, give it the `dataproc.viewer` role for the folder and the `dataproc.editor` role for that cluster:
...
- CLI {#cli}
  ...
  1. Assign the roles for the cluster:
      ...
      ```bash
      {{ yc-dp }} cluster add-access-bindings <cluster_name_or_ID> \
        --access-binding role=dataproc.editor,subject=serviceAccount:<service_account_ID>
      ```
