...
description: 'You can delete a {{ dataproc-name }} cluster together with its stored data. Note that data backups are not created automatically: if you want to save data for cluster recovery or future use, you can do that, for example, using {{ objstorage-full-name }}.'
---
...
To delete a {{ dataproc-name }} cluster, the user must have the `dataproc.editor` role [assigned](../../iam/operations/roles/grant.md). For more information, see the [role description](../security/index.md#roles-list).
