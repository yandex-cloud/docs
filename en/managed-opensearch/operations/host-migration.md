---
title: Migrating {{ mos-full-name }} cluster host groups to a different availability zone
description: Follow this guide to move host groups in a {{ mos-full-name }} cluster to a different availability zone.
---
...
If your cluster is used as an [endpoint](../../data-transfer/concepts/index.md#endpoint) when transferring data using {{ data-transfer-name }}:

1. Make sure not all the cluster host groups are located in the `{{ region-id }}-d` availability zone; otherwise, the transfer will not run the way it should. If you are migrating host groups to the `{{ region-id }}-d` availability zone, place at least one group in the `{{ region-id }}-a` or `{{ region-id }}-b` zone. The transfer will select the appropriate zone automatically. If there is a single host group in your cluster, place it in the `{{ region-id }}-a` or `{{ region-id }}-b` zone.
