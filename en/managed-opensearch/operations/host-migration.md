...
1. Make sure that not all cluster host groups are located in the `{{ region-id }}-d` availability zone; otherwise, the transfer will not operate correctly. If you are migrating host groups to the `{{ region-id }}-d` availability zone, place at least one group in the `{{ region-id }}-a` or `{{ region-id }}-b` zone. The transfer will select the appropriate zone automatically. If there is a single host group in your cluster, place it in the `{{ region-id }}-a` or `{{ region-id }}-b` zone.
