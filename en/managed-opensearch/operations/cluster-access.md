---
title: Managing access to a {{ mos-full-name }} cluster
description: Follow this guide to configure {{ mos-name }} cluster access permissions.
---

# Managing access to a {{ mos-name }} cluster
...
- CLI {#cli}
  ...
  1. To get a list of roles assigned for the cluster, run this command:
      ...
      ```bash
      {{ yc-mdb-os }} cluster list-access-bindings <cluster_name_or_ID>
      ```
...
- REST API {#api}
  ...
  1. Call the [Cluster.ListAccessBindings](../api-ref/Cluster/listAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --url 'https://{{ api-host-mdb }}/managed-opensearch/v1/clusters/<cluster_ID>:listAccessBindings'
      ```
...
- CLI {#cli}
  ...
  1. Use this command to assign a role:
      ...
      ```bash
      {{ yc-mdb-os }} cluster add-access-binding <cluster_name_or_ID> \
        --role <role> \
...
- {{ TF }} {#tf}
  ...
  1. Add a resource description:
      ...
      ```hcl
      resource "yandex_mdb_opensearch_cluster_iam_binding" "<local_resource_name>" {
  ...
  1. Confirm updating the resources.
      ...
      For more information, see [this {{ TF }} provider guide]({{ tf-provider-resources-link }}/mdb_opensearch_cluster_iam_binding).
...
- REST API {#api}
  ...
  1. Call the [Cluster.UpdateAccessBindings](../api-ref/Cluster/updateAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://{{ api-host-mdb }}/managed-opensearch/v1/clusters/<cluster_ID>:updateAccessBindings' \
        --data '{
...
- CLI {#cli}
  ...
  1. To assign roles, run this command:
      ...
      ```bash
      {{ yc-mdb-os }} cluster set-access-bindings <cluster_name_or_ID> \
...
- {{ TF }} {#tf}
  ...
  1. Add resource descriptions:
      ...
      ```hcl
      resource "yandex_mdb_opensearch_cluster_iam_binding" "<resource_1_local_name>" {
        cluster_id = "<cluster_ID>"
        role       = "<role_1>"
        members    = ["<subject_type>:<subject_ID>"]
      }

      resource "yandex_mdb_opensearch_cluster_iam_binding" "<resource_2_local_name>" {
        cluster_id = "<cluster_ID>"
        role       = "<role_2>"
        members    = ["<subject_type>:<subject_ID>"]
      }
...
- REST API {#api}
  ...
  1. Call the [Cluster.SetAccessBindings](../api-ref/Cluster/setAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://{{ api-host-mdb }}/managed-opensearch/v1/clusters/<cluster_ID>:setAccessBindings' \
        --data '{
                  "accessBindings": [
                    {
                      "roleId": "<role>",
      ...
      * `accessBindings.roleId`: [Role](../security/index.md#roles-list) being assigned, e.g., `managed-opensearch.editor`.
...
- CLI {#cli}
  ...
  1. To revoke a role, run this command:
      ...
      ```bash
      {{ yc-mdb-os }} cluster remove-access-binding <cluster_name_or_ID> \
        --role <role> \
        --subject <subject_type>:<subject_ID>
      ```
...
For a service account to be able to view the info of all {{ mos-name }} clusters in the folder but modify the resources in one particular cluster alone, give it the `managed-opensearch.viewer` role for the folder and the `managed-opensearch.editor` role for that cluster:
...
- CLI {#cli}
  ...
  1. Assign the roles for the cluster:
      ...
      ```bash
      {{ yc-mdb-os }} cluster add-access-binding <cluster_name_or_ID> \
        --access-binding role=managed-opensearch.editor,subject=serviceAccount:<service_account_ID>
      ```
