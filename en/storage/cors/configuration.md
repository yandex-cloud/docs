...
| `AllowedHeader` | Header allowed in a request to an object. If multiple headers are allowed, specify each one in a separate `AllowedHeader` element. You can use a single `*` character in the header name to define a template. For example, `<AllowedHeader>*</AllowedHeader>` means that all headers are allowed.<br/><br/>An [OPTIONS](../s3/api-ref/object/options.md) request contains the `Access-Control-Request-Headers` header. Object Storage maps the headers passed in `Access-Control-Request-Headers` to the `AllowedHeader` set and returns a list of allowed headers in response to the `OPTIONS` request.<br/><br/>Path: `/CORSConfiguration/CORSRule/AllowedHeader`. |
