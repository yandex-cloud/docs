...
The response may contain no more than 1,000 elements. If there are more uploads, [!KEYREF objstorage-name] returns the `IsTruncated` element as well as the `NextKeyMarker` and `NextUploadIdMarker` elements to be used for the `key-marker` and `upload-id-​marker` parameters of a subsequent request.
...
| Parameter | Description |
| ----- | ----- |
| `delimiter` | Delimiter character.<br/><br/>If this parameter is specified, [!KEYREF objstorage-name] interprets the key as the path to the file with folder names separated by the `delimiter` character. The user will get a list of <q>files</q> and <q>folders</q> in the <q>root</q> of the bucket. <q>The files</q> will be output in the `Uploads` elements, and the <q>folders</q> will be in the `CommonPrefixes` elements.<br/><br/>If the request also specifies the `prefix` parameter, [!KEYREF objstorage-name] will return the list of <q>files</q> and <q>folders</q> in the <q>prefix</q>`folder`. |
| `max-uploads` | Maximum number of uploads in a response.<br/><br/>By default, [!KEYREF objstorage-name] outputs a maximum of 1,000 keys. This parameter should be used if you need to get less than 1,000 keys in a single response.<br/><br/>If the number of keys meeting the selection criteria is greater than the number that could fit in the output, the response contains `<IsTruncated>true</IsTruncated>`.<br/><br/>To get all output objects if their number exceeds the `max-keys` value, make several consecutive requests to [!KEYREF objstorage-name] with the `key-marker` parameter, where each request's `key-marker` value is equal to the value of the `NextKeyMarker` element in the previous response. |
...
| `prefix` | String to start the key from.<br/><br/>[!KEYREF objstorage-name] selects only those keys which start with `prefix`. |
