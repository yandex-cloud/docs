...
The response may contain no more than 1,000 elements. If there are more parts in the multipart upload, [!KEYREF objstorage-name] returns the `IsTruncated` marker and the `NextPartNumberMarker` element. The remaining elements can be obtained via consecutive requests with the `part-number-marker` parameter set to `NextPartNumberMarker` from the previous request.
...
You can change a response from [!KEYREF objstorage-name] using the parameters described in the table below.

| Parameter | Description |
| ----- | ----- |
| `encoding-type` | Encoding of server responses.<br/><br/>[!KEYREF objstorage-name] can encode responses in the format requested by the client. |
| `max-parts` | The maximum number of elements in a response per request.<br/><br/>By default, 1000. |
| `part-number-marker` | Number of the part to start a response from.<br/><br/>[!KEYREF objstorage-name] will only include in the response the parts whose numbers are greater than the one specified. |
...
| `Encoding-Type` | Encoding in which [!KEYREF objstorage-name] provides a key in the XML response.<br/><br/>Appears if the client passed the `encoding-type` parameter in the request.<br/><br/>Path: `/ListPartsResult/Encoding-Type`. |
...
| `IsTruncated` | Flag indicating that a list is incomplete.<br/><br/>If `IsTruncated` is `true`, this means that [!KEYREF objstorage-name] returned an incomplete list of parts.<br/><br/>Path: `/ListPartsResult/IsTruncated`. |
