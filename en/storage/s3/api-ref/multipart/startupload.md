...
`X-Amz-Storage-Class` | [Object storage class](../../../concepts/storage-class.md).<br/><br/>Possible values:<ul><li>`STANDARD`: Standard storage</li><li>`COLD`, `STANDARD_IA`, or `NEARLINE`: Cold storage</li><li>`ICE` or `GLACIER`: Ice storage</li><li>`INTELLIGENT_TIERING`: Intelligent-tiering storage</li></ul>If the header is not specified, the object storage is defined in the bucket settings.
