...
| `Owner` | User information.<br/><br/>Users can specify this element for `objectPutAcl` and `bucketPutAcl` requests. If the element is specified, then, when uploading an ACL, [!KEYREF objstorage-name] checks whether the passed ID matches the actual ID. If not, code 403 is returned.<br/><br/>Path: `/AccessControlPolicy/Owner`. |
