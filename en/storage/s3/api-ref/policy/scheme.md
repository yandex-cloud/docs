...
`Statement[].Principal` | **string**<br>(Optional) ID of the recipient of the requested permission.<br>Possible values: `*`, `o9dc3v17ajehpriseeio`.
`Statement[].NotPrincipal` | **string**<br>(Optional) ID of the entity that won't receive the requested permission.<br>Possible values: `*`, `"AWS": "arn:aws:iam::AWS-account-ID:user/user-name"`.
...
`Statement[].NotAction` | **string**<br>Determines the [action](actions.md) that won't be executed when the policy is triggered.<br>Possible values: `s3:GetObject`, `s3:PutObject`.
...
`Statement[].NotResource` | **string**<br>Specifies the resource that won't be affected by the action.<br>Possible values: `arn:aws:s3:::sample_bucket/*`, `arn:aws:s3:::sample_bucket/object`.
