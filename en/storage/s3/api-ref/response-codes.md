...
If there are no errors, [!KEYREF objstorage-name] returns HTTP code 2xx. The response code and body depend on the request and are given in the request descriptions.
...
| 400 | `BadDigest` | The hash passed in the `Content-MD5` header does not match the one calculated on the [!KEYREF objstorage-name] side. |
...
| 403 | `NotSignedUp` | Use of [!KEYREF objstorage-name] is not allowed for this account. |
| 403 | `RequestTimeTooSkewed` | The request sending time is too different from the server time. |
| 403 | `SignatureDoesNotMatch` | The provided request signature does not match the one calculated by [!KEYREF objstorage-name]. |
...
| 503 | `ServiceUnavailable` | [!KEYREF objstorage-name] is not available.<br/>Reduce your request rate. |
