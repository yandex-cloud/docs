...
If a cross-domain ([CORS]{% if lang == "ru" and audience != "internal" %}(../../../glossary/cors.md){% else %}(https://en.wikipedia.org/wiki/Cross-origin_resource_sharing){% endif %}) request is sent, it may contain [headers](object/options.md#request-headers) of the `options` preflight request.
