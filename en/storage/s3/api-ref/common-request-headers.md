...
| `Content-MD5` | 128-bit MD5 hash value of the request body, `base64` encoded.<br/><br/>Compliant with [RFC 1864](http://www.ietf.org/rfc/rfc1864.txt).<br/><br/>{{ objstorage-name }} uses a header to make sure that the sent data matches the received data.<br/><br/>If the bucket uses [default locks of object versions](../../concepts/object-lock.md#default), the `Content-MD5` header is required for uploading or copying an object version. |
