...
|| **Header** | **Description** ||
|| `ETag` |
[MD5 hash](https://en.wikipedia.org/wiki/MD5) of an object if the object is uploaded as a single file. If the object is uploaded as a [multipart upload](../../concepts/multipart.md), then to calculate the `ETag` value, you need to get the MD5 hash sum of the uploaded parts (`MD5_sum`) and add the number of parts (`N`) to the resulting string: `"MD5_sum-N"`.

It does not include object metadata. ||
|| `X-Amz-Request-Id` |
Unique request ID.

You may need it when contacting the {{ yandex-cloud }} support team if any problems occur. ||
|#
