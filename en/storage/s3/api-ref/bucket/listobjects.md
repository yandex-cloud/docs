...
| `StorageClass` | Object [storage class](../../../concepts/storage-class.md): {% if product == "yandex-cloud" %}`STANDARD`, `COLD`, or `ICE`{% endif %}{% if product == "cloud-il" %}`STANDARD` or `COLD`{% endif %}.<br/><br/>Path: `/ListBucketResult/Contents/StorageClass`. |
