...
`LastModified` | Date and time of the object’s last update.<br/><br/>Path: `/ListBucketResult/Contents/LastModified`.
...
`NextContinuationToken` | Value to insert in the `continuation-token` query parameter to get the next part of the list if the whole list does not fit in the current response.<br/>Returned only if `IsTruncated = true`.<br/><br/>Path: `/ListBucketResult/NextContinuationToken`.
