...
| `marker` | Key to start the output from.<br/><br/>In the resulting output, Object Storage leaves the keys starting from the one following the `marker`. |
| `max-keys` | Maximum number of elements in a response.<br/><br/>By default, Object Storage outputs no more than 1,000 `Contents` and `CommonPrefixes` elements. This parameter should be used if you need to get less than 1,000 elements in a single response.<br/><br/>If the number of keys meeting the selection criteria is greater than the number that could fit in the output, the response contains `<IsTruncated>true</IsTruncated>`.<br/><br/>To get all output elements if there are more than the `max-keys` value, make several consecutive requests to Object Storage with the `marker` parameter, where for each request, the `marker` is equal to the value of the `MaxKeys` element from the previous response. |
| `prefix` | String to start the key from.<br/><br/>Object Storage selects only those keys which start with `prefix`.<br/><br/>Can be used simultaneously with the `delimiter` parameter. In this case, the output logic is the same as that specified in the `delimiter` parameter description. |
