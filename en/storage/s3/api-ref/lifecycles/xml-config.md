...
{{ objstorage-name }} allows you to manage [object lifecycles](../../../concepts/lifecycles.md) for objects in buckets. To upload the lifecycle configuration to {{ objstorage-name }}, you need to create an XML document as described in this section. You can obtain a document in this format by downloading an existing configuration.
...
| Element | Description |
| ----- | ----- |
| `Date` | Date of rule execution.<br/><br/>Format: [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601), for example, `YYYY-MM-DD`. Time: always 00:00 UTC.<br/><br/>Path: `LifecycleConfiguration\Rule\Expiration\Date`. |
...
| `ID` | Unique rule ID.<br/><br/>The maximum length is 255 characters. An optional parameter that you can use to search for a rule in a configuration.<br/><br/>Path: `LifecycleConfiguration\Rule\ID`. |
| `LifecycleConfiguration` | Root element of an XML document.<br/><br/>It can contain up to 1000 `Rule` elements.<br/><br/>Path: `LifecycleConfiguration`. |
