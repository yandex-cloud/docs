...
1. Fill out the fields as follows:
   ...
   * **{{ ui-key.yacloud.storage.bucket.cors.field_allowed-headers }}**: Comma-separated list of headers allowed in the object request. Each header is written in a separate `AllowedHeader` field in the CORS configuration, e.g., `X-Request-Id, X-Request-With`.
   ...
   * **{{ ui-key.yacloud.storage.bucket.cors.field_max-age-seconds }}**: Time, in seconds, for the browser to cache the result of an object request using the `OPTIONS` method.
