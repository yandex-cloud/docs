...
| `302 Found` | If [!KEYREF objstorage-name] receives a request to the resource `http://[!KEYREF s3-web-host]/bucket/x` without `/` at the end, [!KEYREF objstorage-name] will first try to find an `x` object. If this object does not exist but `x/index.html` is set as the website's home page, the user will get a 302 response and be redirected to `http://[!KEYREF s3-web-host]/bucket/x/`. |
...
| `500 Service Error` | Internal error in [!KEYREF objstorage-name]. |
