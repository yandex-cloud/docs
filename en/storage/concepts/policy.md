...
* A policy that gives each user and service account full access to a folder named the same as the {% if audience != "internal" %}[user ID](../../iam/operations/users/get.md) or [service account ID](../../iam/operations/sa/get-id.md){% else %}user or service account ID{% endif %}:
