...
To compose a pre-signed URL, you must have {% if audience != "internal" %}[static access keys](../../iam/operations/sa/create-access-key.md){% else %}static access keys{% endif %}.
...
- Signing key:
   ...
   The `sign` function was introduced to indicate the method of key calculation that uses the [HMAC](https://en.wikipedia.org/wiki/HMAC) algorithm with the [SHA256](https://en.wikipedia.org/wiki/SHA-2).
...
- AWS CLI
   ...
   For a link to be generated properly, the `--endpoint-url` parameter pointing to the {{ objstorage-name }} hostname is required. For detailed information, see this [section about AWS CLI specifics](../tools/aws-cli.md#specifics).
