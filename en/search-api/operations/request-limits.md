...
* `user`: Username. It must be the same as the username on Yandex specified during registration.
* `key`: API key issued during registration.


### Query request

The following request returns information about hourly limits on the number of search queries that can be sent by the `xml-search-user` user in the next 24 hours:
...
In response to the request for hourly limits, {{ search-api }} returns a UTF-8 encoded XML file with search results.

{% note info %}

If the allowed number of queries during one of the hours is exceeded, queries in excess of the limit are deducted from the value for the respective hour in the next 24 hours. The excess data is factored in when generating a response.
