...
* A fee for load balancing (see [pricing{{ alb-full-name }}](../../application-load-balancer/pricing.md)).
...
1. Make sure that the selected folder contains a network with subnets in the `ru-cental1-a`, `ru-central1-b`, and `ru-cental1-c` availability zones. To do this, select **{{ vpc-name }}** on the folder page. If you don't have the right [network](../../vpc/operations/network-create.md) or [subnets](../../vpc/operations/subnet-create.md), create them.

## Create security groups {#create-security-groups}

[Security groups](../../application-load-balancer/concepts/application-load-balancer.md#security-groups) include rules that let the load balancer receive incoming traffic and redirect it to the VMs so that they can receive the traffic.
...
VMs from the [target group](../../application-load-balancer/concepts/target-group.md) will be used to deploy your application's backends. The target group will be attached to the load balancer so that requests can be sent to the backend endpoints of your application.
...
1. Under **Allocation**, select three availability zones (`ru-cental1-a`, `ru-central1-b`, and `ru-cental1-c`) to ensure fault tolerance of your hosting.
...
1. Upload the website files to the VM via [SCP protocol](https://en.wikipedia.org/wiki/Secure_copy_protocol).
...
Link the target group created together with the instance group to a [backend group](../../application-load-balancer/concepts/backend-group.md) that defines the traffic allocation settings.

For group backends, [health checks](../../application-load-balancer/concepts/backend-group.md#health-checks) will be created: the load balancer will periodically send health check requests to the VM and expect a response within a set timeout.
...
### Add resource records {#add-records}
