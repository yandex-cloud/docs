...
* A fee for using a network load balancer (see [pricing {{ network-load-balancer-full-name }}](../../load-balancer/pricing.md)).
...
- Bash
  ...
  Get the subnet identifier using the following command:
...
The password must meet the [complexity requirements](https://docs.microsoft.com/en-us/windows/security/threat-protection/security-policy-settings/password-must-meet-complexity-requirements).
...
### Create the virtual machines {#create-group-vms}
...
1. Specify DNS server addresses:
...
1. Create groups to access the backups and DB servers:
...
1. Run the RDP client and connect to the `ya-mssql1`  VM with the `Administrator` account  and your password. Use the public IP address of the VM to connect.
...
1. Create folders for backups, storage of databases, logs, and temporary files:
...
1. After the restart, reconnect to the VM with the `yantoso\Administrator` username and open PowerShell.
...
1. Install MSSQL. Mount an image, perform installation, and dismount the image:
...
1. Repeat steps 2-14 for VMs `ya-mssql2` and `ya-mssql3`.

1. Disable the VM's internet access:
