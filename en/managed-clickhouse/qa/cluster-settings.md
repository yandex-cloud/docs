...
Go to the cluster settings, enable the [{#T}](../operations/cluster-users.md#sql-user-management) option, and grant a user the appropriate permissions [using a statement]{% if lang == "ru" %}(https://clickhouse.com/docs/ru/sql-reference/statements/grant/){% endif %}{% if lang == "en" %}(https://clickhouse.com/docs/en/sql-reference/statements/grant/){% endif %} named `GRANT`.
