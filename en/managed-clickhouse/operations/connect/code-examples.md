...
1. [Install the clickhouse-odbc driver](https://github.com/ClickHouse/clickhouse-odbc#installation) matching your system's architecture. For example, if you are using a 32-bit application for ODBC connections, you need a 32-bit driver.
...
1. Click **OK**.
1. Connect to the {{ CH }} cluster via ODBC, e.g., using Microsoft Excel.
...
1. Make sure the `allow_url_fopen` parameter is set to `On` in the PHP settings:
