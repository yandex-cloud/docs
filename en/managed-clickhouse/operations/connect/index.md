---
title: Pre-configuring {{ CH }} cluster connection in {{ mch-full-name }}
description: Follow this guide to pre-configure a connection to a database in a {{ CH }} cluster.
---

# Pre-configuring a connection to a {{ CH }} cluster
...
You can connect to a cluster either using encryption (via ports `{{ port-mch-cli }}` for [clickhouse-client]({{ ch.docs }}/interfaces/cli/) and `{{ port-mch-http }}` for the [HTTP interface]({{ ch.docs }}/interfaces/http/)) or without encryption (via ports `9000` and `8123`, respectively).
...
* [{{ CH }} connections FAQ](../../qa/connection.md)
