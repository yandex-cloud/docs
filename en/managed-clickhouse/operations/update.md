...
1. Specify the class in the update cluster command:
   ...
   You can change the ZooKeeper host class using the same parameter, `--zookeeper-resource-preset`.
...
1. Make sure the cloud's quota is sufficient to increase the storage size: open the [Quotas](https://console.cloud.yandex.ru/?section=quotas) for your cloud and check that the [!KEYREF mch-full-name] section still has space remaining in the **space** line.
...
You can change the cluster's storage size using the API [update](../api-ref/Cluster/update.md) method: pass the appropriate values in the request parameter `configSpec.clickhouse.resources.diskSize` (for ZooKeeper — `configSpec.zookeeper.resources.diskSize`).

Make sure the cloud's quota is sufficient to increase the storage size: open the [Quotas](https://console.cloud.yandex.ru/?section=quotas) for your cloud and check that the [!KEYREF mch-full-name] section still has space remaining in the **space** line.
