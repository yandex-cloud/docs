...
To add data to the database as part of a normal routine, use the `INSERT` query described in the [documentation for {{ CH }}](https://{{ ch-domain }}/docs/en/sql-reference/statements/insert-into/). The `INSERT` queries should be sent no more than once per second.

To upload large data, use data compression during transmission: for example, you can enable it for HTTP or `clickhouse-client`. To learn more, see the [documentation for {{ CH }}](https://{{ ch-domain }}/docs/en/interfaces/http/).
