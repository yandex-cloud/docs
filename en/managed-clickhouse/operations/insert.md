...
To add data to the database as part of a normal routine, use the `INSERT` query described in the [documentation for {{ CH }}](https://clickhouse.tech/docs/en/sql-reference/statements/insert-into/). The `INSERT` queries should be sent no more than once per second.

To upload large data, use data compression during transmission: for example, you can enable it for HTTP or `clickhouse-client`. To learn more, see the [documentation for {{ CH }}](https://clickhouse.tech/docs/en/interfaces/http/).
...
1. [Prepare format schemas](#prepare-format-schemas).
...
1. Examine the data format that will be used for insertion so that the correct format schemas are [prepared](#prepare-format-schemas).
   ...
   To learn more about supported data types, see the documentation for [Cap'n Proto](https://capnproto.org/language.html), [Protobuf](https://developers.google.com/protocol-buffers/docs/proto3), and [{{ CH }}](https://clickhouse.tech/docs/en/sql-reference/data-types/).
