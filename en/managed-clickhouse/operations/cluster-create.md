...
description: Follow this guide to create a {{ CH }} cluster with one or multiple database hosts.
---
...
A {{ CH }} cluster consists of one or more database hosts. If the cluster consists of multiple hosts, you can set up [replication](../concepts/replication.md) between them. Use one of the [coordination services](../concepts/coordination-system.md) for replication: {{ CK }} or {{ ZK }}.
...
* [{{ connection-manager-name }}](../../metadata-hub/concepts/connection-manager.md) connection with information about the database connection.

* [{{ lockbox-name }} secret](../../metadata-hub/concepts/secret.md) that stores the DB owner's user password.

The connection and secret will be created for each new database user. To view all connections, open the **{{ ui-key.yacloud.connection-manager.label_connections }}** tab on the cluster page. You need the `connection-manager.viewer` role to view the connection details.
...
- Management console {#console}
  ...
  1. Select the [{{ CH }} version](../concepts/update-policy.md) the {{ mch-name }} cluster will use from the **{{ ui-key.yacloud.mdb.forms.base_field_version }}** drop-down list. For most clusters, we recommend selecting the latest LTS version.
  ...
  1. Under **{{ ui-key.yacloud.mdb.forms.section_settings }}**:
      ...
      * [Configure the DBMS](../concepts/settings-list.md#server-level-settings), if required. You can do it later.
        ...
        Using the {{ yandex-cloud }} interfaces, you can manage a limited number of settings. Using SQL queries, you can [specify the {{ CH }} settings at the query level](change-query-level-settings.md).
...
- CLI {#cli}
  ...
  1. Specify the cluster properties in this command (the example does not show all that are available):
      ...
      * {% include [Deletion protection](../../_includes/mdb/cli/deletion-protection.md) %}
      ...
      1. To allow access to the cluster from [{{ sf-full-name }}](../../functions/concepts/index.md), provide `--serverless-access`. For more information about access setup, see [this {{ sf-name }} guide](../../functions/operations/database-connection.md).
  ...
  To create a {{ mch-name }} cluster with the built-in {{ CK }} coordination service:
    ...
    1. Create a configuration file describing the [cloud network](../../vpc/concepts/network.md#network) and [subnets](../../vpc/concepts/network.md#subnet).
        ...
        * Subnets: Description of the [subnets](../../vpc/concepts/network.md#network) to connect the cluster hosts to. If you already have suitable subnets, you do not need to describe them again.
        ...
        ```hcl
        resource "yandex_vpc_network" "<network_name_in_{{ TF }}>" {
          name = "<network_name>"
        }
...
## Creating a cluster with {{ ZK }} {#create-cluster-zk}
...
- CLI {#cli}
  ...
  1. Specify the cluster properties in this command (the example does not show all that are available):
      ...
      * `--zookeeper-disk-size`: {{ ZK }} storage size in GB.
      * `--user`: Contains the {{ CH }} user `name` and `password`.
...
You can create a {{ CH }} cluster with the settings of another one created earlier. Do this by importing the original {{ CH }} cluster configuration into {{ TF }}. This way, you can either create an identical copy or use the imported configuration as the baseline and modify it as needed. The import feature is handy when the original {{ CH }} cluster has a lot of settings and you need to create a similar one.
...
- CLI {#cli}
  ...
  * One {{ CH }} `{{ host-class }}` host in the `{{ subnet-id }}` subnet, in the `{{ region-id }}-a` availability zone.
