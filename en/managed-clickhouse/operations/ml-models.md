...
{{ mch-short-name }} lets you analyze data by applying [CatBoost](https://catboost.ai/) machine learning models without additional tools. To apply a model, add it to your cluster and call it in an SQL query using the built-in [`modelEvaluate()`](https://clickhouse.yandex/docs/ru/query_language/functions/other_functions/#function-modelevaluate) function. After running this query, you get model predictions for each row of input data. For more information about machine learning, {{ CH }} see the [documentation](https://clickhouse.yandex/docs/ru/guides/apply_catboost_model/).
...
In this example, we'll use public data from the [Amazon Employee Access Challenge](https://www.kaggle.com/c/amazon-employee-access-challenge). The model is trained to predict values in the `ACTION` column. The same data and model are used in the examples provided in the [{{ CH }} documentation](https://clickhouse.yandex/docs/ru/guides/apply_catboost_model/) and on [GitHub](https://github.com/ClickHouse/clickhouse-presentations/blob/master/tutorials/catboost_with_clickhouse_ru.md).

{% endnote %}

To upload data to {{ CH }} and test the model:

1. Install the [{{ CH }} CLI](https://clickhouse.yandex/docs/ru/interfaces/cli/) and set up a cluster connection as described in the [documentation](../../managed-clickhouse/operations/connect.md#cli).
...
1. Upload the data to the table:
   ...
   $ clickhouse-client --host <host FQDN> \
                     ...
                     < train.csv
   ```
1. In the [management console](https://console.cloud.yandex.ru/), add the test model:
