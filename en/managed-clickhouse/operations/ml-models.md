...
1. Upload the trained model file to {{ objstorage-full-name }}.

1. Set up public read access to the model file.

1. Get a link to the model file.
...
1. Upload the file with the current model to {{ objstorage-full-name }}.
...
After disabling a model, the corresponding object is kept in the {{ objstorage-full-name }} bucket. If you no longer need this model object, you can delete it.
