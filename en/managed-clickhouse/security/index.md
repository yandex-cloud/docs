---
title: Access management in Yandex Managed Service for ClickHouse
...
The table below lists the roles needed to perform a given action. You can always assign a role granting more permissions than the role specified. For example, you can assign `{{ roles-editor }}` instead of `{{ roles-viewer }}`.
...
Creating clusters with hosts that have public access enabled | `create` | Or `{{ roles-vpc-public-admin }}` together with `{{ roles-mdb-admin }}`, or `{{ roles-editor }}` for the folder
...
[Assign](../../iam/operations/roles/grant.md), [revoke](../../iam/operations/roles/revoke.md), and view roles granted for the resource or cloud | `setAccessBindings`, `updateAccessBindings`, `listAccessBindings` | `{{ roles-admin }}` for this folder or cloud
