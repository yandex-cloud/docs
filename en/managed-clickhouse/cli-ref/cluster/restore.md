...
|`--shard`|<b>`PROPERTY=VALUE[,PROPERTY=VALUE...]`</b><br/>Configurations of the shards that should be created in the cluster. Their number, names and order should match those in the backups whose ids you provide in the flags.<br/><br/>Possible property names:<br/><ul> <li><code>name</code>:     Shard name.</li> <li><code>disk-size-limit</code>:     Disk size limit of the shard in the cluster (GB).</li> <li><code>planned-usage-threshold</code>:     Planned usage threshold of the shard in the cluster.</li> <li><code>emergency-usage-threshold</code>:     Emergency usage threshold of the shard in the cluster.</li> <li><code>weight</code>:     Weight of the shard in the cluster; an integer.</li> </ul>|
