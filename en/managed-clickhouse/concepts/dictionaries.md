...
As their data sources, dictionaries can use [{{ CH }} internal dictionaries](https://{{ ch-domain }}/docs/en/query_language/dicts/internal_dicts/) or [external sources](https://{{ ch-domain }}/docs/en/query_language/dicts/external_dicts/), including HTTP(s) resources or other DBMS ({{ MY }}, {{ CH }}, {{ MG }}, {{ PG }}).
