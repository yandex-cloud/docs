...
{{ mch-short-name }} provides automatic and manual database backups. Backups take up space in the storage allocated to the cluster. {% if audience != "internal" %} If the total amount of data and backups exceed the storage size, the price [increases](../pricing.md). {% endif %}
