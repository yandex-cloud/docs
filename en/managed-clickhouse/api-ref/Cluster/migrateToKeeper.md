...
editable: false
apiPlayground:
  - url: https://{{ api-host-mdb }}/managed-clickhouse/v1/clusters/{clusterId}:migrateToKeeper
    method: post
    path:
      type: object
      properties:
...
# Managed Service for ClickHouse API, REST: Cluster.MigrateToKeeper
...
```
POST https://{{ api-host-mdb }}/managed-clickhouse/v1/clusters/{clusterId}:migrateToKeeper
```
...
## Body parameters {#yandex.cloud.mdb.clickhouse.v1.MigrateClusterToKeeperRequest}
...
```json
{
  ...
  "allowDegradationToReadOnly": "boolean"
}
```
