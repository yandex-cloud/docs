...
- `label="<glob>"`: Returns all the metrics that have labels that match a [glob expression]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/Шаблон_поиска){% endif %}{% if lang == "en" %}(https://en.wikipedia.org/wiki/Glob_(programming)){% endif %}.
