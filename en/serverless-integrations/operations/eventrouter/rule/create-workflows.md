---
title: How to create a rule with a {{ sw-full-name }} target
description: Follow this guide to create a rule with a {{ sw-full-name }} target.
---
...
- Management console {#console}
  ...
  1. Under **{{ ui-key.yacloud.serverless-event-router.label_targets }}**, click **{{ ui-key.yacloud.common.add }}** and configure the target:
      ...
      1. Specify `{{ sw-full-name }}` as the target type.
      1. Select the workflow to receive the messages matching the rule at its input.
      1. Select the [service account](../../../../iam/concepts/users/service-accounts.md) which has the `serverless.workflows.executor` role or higher for the selected workflow or [create](../../../../iam/operations/sa/create.md) a new account.
...
- CLI {#cli}
  ...
  1. Create a rule with a {{ sw-full-name }} target:
      ...
      service-account-id=<workflow_service_account_ID>,\
      ...
      * `--workflow-target`: A flag to configure the `{{ sw-full-name }}` type target and its parameters:
...
- {{ TF }} {#tf}
  ...
  1. In the configuration file, define the parameters of the resources you want to create:
      ...
      * `workflow`: A section to configure the `{{ sw-full-name }}` type target and its parameters:
...
- API {#api}
  ...
  To create a [rule](../../../concepts/eventrouter/rule.md) with a {{ sw-full-name }} target, use the [Create](../../../../serverless-integrations/eventrouter/api-ref/Rule/create.md) REST API method for the [rule](../../../../serverless-integrations/eventrouter/api-ref/Rule/index.md) resource or the [rule/Create](../../../../serverless-integrations/eventrouter/api-ref/grpc/Rule/create.md) gRPC API call.
