...
--- | --- | --- | --- | --- | ---
`input` | `string` | None | [Overall state of the workflow](../../workflow.md#state) | Yes | A jq template to filter the workflow state fed into the step.
`choices` | [Choice](#Choice)[] | Yes | None | None | List of possible further execution paths.
`default` | `string` | None | None | None | ID of the step to execute if none of the conditions specified in `choices` return `true`.
