...
* Via the management console, CLI, or API. Read more in [{#T}](../../operations/workflows/execution/start.md).
* On a timer. The schedule is set in [cron expression](../cron.md) format. Read more in [{#T}](../../operations/workflows/execution/start-by-timer.md).
* Via a link over HTTPS. Read more in [{#T}](../../operations/workflows/execution/start-by-link.md).
* Using an API gateway. Read more in [{#T}](../../../api-gateway/operations/spec-constructor/workflows.md).
* Using {{ er-full-name }}. To do this, you need to specify the workflow as a target in a [rule](../eventrouter/rule.md). For more on how to create a rule, see [{#T}](../../operations/eventrouter/rule/create-workflows.md). The rule defines the events that should trigger a workflow. The event body is provided to the execution as an input parameter.

If a workflow is:
* Private, to run it, the user needs workflow execution permissions, e.g., the [serverless.workflows.executor](../../security/workflows.md#serverless-workflows-executor) role.
* Public, any user can run it. Read more in [{#T}](../../operations/workflows/workflow/make-public.md).
...
`STEP_DATA_LIMIT_EXCEEDED` | Input or output data limit exceeded. Read more in [{#T}](../limits.md).
`STEP_NO_CHOICE_MATCHED` | There is no suitable execution path in `choices`. Read more in [{#T}](yawl/management/switch.md).
`STEP_PERMISSION_DENIED` | No access to resource.
`STEP_TIMEOUT` | Step timeout exceeded. Read more in [{#T}](../limits.md).
...
`STEP_FAIL` | The execution ended with an error at the `Fail` step. Read more in [{#T}](yawl/management/fail.md).
...
`GRPC_CALL_CANCELLED`<br/>`GRPC_CALL_DEADLINE_EXCEEDED`<br/>`GRPC_CALL_UNIMPLEMENTED`<br/>`GRPC_CALL_UNAVAILABLE`<br/>`GRPC_CALL_UNKNOWN`<br/>`GRPC_CALL_INTERNAL`<br/>`GRPC_CALL_RESOURCE_EXHAUSTED`<br/>`GRPC_CALL_UNAUTHENTICATED` | gRPC statuses. For more information, see [this gRPC guide](https://grpc.github.io/grpc/core/md_doc_statuscodes.html).
...
`FUNCTION_CALL_INVALID_RESPONSE` | Invalid function code or format of returned JSON response. Read more in [{#T}](../../../functions/concepts/function-invoke.md#http-state).
