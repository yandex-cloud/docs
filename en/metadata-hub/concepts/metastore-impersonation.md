...
When creating a service account for {{ metastore-name }}, we recommend using the `{{ roles.metastore.integrationProvider }}` role as it already includes the `{{ roles-logging-writer }}` and `{{ roles-monitoring-editor }}` roles. To learn more about the role features, see this [reference](../security/metastore-roles.md#managed-metastore-integrationProvider).
