---
title: Managing access to a {{ metastore-full-name }} cluster
description: Follow this guide to configure {{ metastore-name }} cluster access permissions.
---

# Managing access to a {{ metastore-name }} cluster

You can assign a user or service account a [role](../../security/metastore-roles.md) that grants access to a specific [cluster](../../concepts/metastore.md).
...
- CLI {#cli}
  ...
  1. To get a list of roles assigned for the cluster, run this command:
      ...
      ```bash
      {{ yc-metastore }} cluster list-access-bindings <cluster_name_or_ID>
      ```
...
- REST API {#api}
  ...
  1. Call the [Cluster.ListAccessBindings](../../api-ref/Cluster/listAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --url 'https://{{ api-host-metastore }}/managed-metastore/v1/clusters/<cluster_ID>:listAccessBindings'
      ```
...
- CLI {#cli}
  ...
  1. Use this command to assign a role:
      ...
      ```bash
      {{ yc-metastore }} cluster add-access-binding <cluster_name_or_ID> \
        --role <role> \
      ...
      * `--role`: [Role](../../security/metastore-roles.md) being assigned, e.g., `managed-metastore.editor`.
...
- REST API {#api}
  ...
  1. Call the [Cluster.UpdateAccessBindings](../../api-ref/Cluster/updateAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://{{ api-host-metastore }}/managed-metastore/v1/clusters/<cluster_ID>:updateAccessBindings' \
        --data '{
      ...
      * `access_binding_deltas.roleId`: [Role](../../security/metastore-roles.md) being assigned, e.g., `managed-metastore.editor`.
...
- CLI {#cli}
  ...
  1. To assign roles, run this command:
      ...
      ```bash
      {{ yc-metastore }} cluster set-access-bindings <cluster_name_or_ID> \
...
- REST API {#api}
  ...
  1. Call the [Cluster.SetAccessBindings](../../api-ref/Cluster/setAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://{{ api-host-metastore }}/managed-metastore/v1/clusters/<cluster_ID>:setAccessBindings' \
        --data '{
                  "accessBindings": [
                    {
                      "roleId": "<role>",
      ...
      * `accessBindings.roleId`: [Role](../../security/metastore-roles.md) being assigned, e.g., `managed-metastore.editor`.
...
- CLI {#cli}
  ...
  1. To revoke a role, run this command:
      ...
      ```bash
      {{ yc-metastore }} cluster remove-access-binding <cluster_name_or_ID> \
        --role <role> \
        --subject <subject_type>:<subject_ID>
      ```
      ...
      * `--role`: [Role](../../security/metastore-roles.md) being revoked, e.g., `managed-metastore.editor`.
...
For a service account to be able to view the info of all {{ metastore-name }} clusters in the folder but modify the resources in one particular cluster alone, give it the `managed-metastore.viewer` role for the folder and the `managed-metastore.editor` role for that cluster:
...
- CLI {#cli}
  ...
  1. Assign the roles for the cluster:
      ...
      ```bash
      {{ yc-metastore }} cluster add-access-bindings <cluster_name_or_ID> \
        --access-binding role=managed-metastore.editor,subject=serviceAccount:<service_account_ID>
      ```
