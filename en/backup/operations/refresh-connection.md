---
title: How to update a {{ compute-full-name }} VM connection to {{ backup-full-name }}
description: Follow this guide to update a {{ compute-full-name }} VM connection to {{ backup-name }}.
---
...
1. Make sure the outdated VM is no longer on the list of VMs connected to {{ backup-name }}:
    ...
    - Management console {#console}
      ...
      1. On the **{{ ui-key.yacloud.backup.label_instances }}** tab, check that the outdated VM is not listed with the ![irrelevant](../../_assets/console-icons/circle-info-fill.svg) label.
