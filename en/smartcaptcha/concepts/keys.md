...
The client key (`client_key`) is used to [add a {{ captcha-name }} widget](./widget-methods.md) to a website. This is a public key that is specified on the page hosting the widget.

The server key (`server_key`) is used by website backend to get user request [verification results](./validation.md#validation-result). The server key is private and should be stored in a secure location, e.g., using [{{ lockbox-full-name }}](../../lockbox/).
...
Learn how to get the client and server keys in [{#T}](../operations/get-keys.md).
