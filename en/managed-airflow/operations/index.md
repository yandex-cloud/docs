---
title: How to work with {{ maf-full-name }}
description: In this article, you will learn how to work with {{ maf-full-name }} in {{ yandex-cloud }}.
---

# Step-by-step guides for {{ maf-name }}

## Clusters {#clusters}

* [Information about existing clusters](cluster-list.md)
* [Creating a cluster](cluster-create.md)
* [Updating a cluster](cluster-update.md)
* [Stopping and starting a cluster](cluster-stop.md)
* [Cluster maintenance](cluster-maintenance.md)
* [Deleting a cluster](cluster-delete.md)

## Connections {#connections}

* [{#T}](update-connections.md)
* [{#T}](clickhouse.md)
* [{#T}](postgres-operator.md)

## Storing artifacts in {{ lockbox-full-name }} {#lockbox}

* [{#T}](load-variable-from-lockbox.md)
* [{#T}](lockbox-secrets-in-maf-cluster.md)

## Working with DAG files {#dags}

* [Uploading DAG files to a cluster](upload-dags.md)
* [{#T}](get-iam-token.md)

## Working with {{ objstorage-full-name }}

* [{#T}](upload-file-to-s3.md)
* [Connecting to a {{ objstorage-full-name }} bucket with a bucket policy](s3-policy-connect.md)
* [{#T}](read-file-from-dags-bucket.md)

## {{ AF }} interfaces {#interfaces}

* [Working with {{ AF }} interfaces](af-interfaces.md)

## Logs {#logs}

* [Transferring cluster logs to {{ cloud-logging-full-name }}](af-logging.md)
