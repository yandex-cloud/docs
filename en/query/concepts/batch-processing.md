...
{{ yq-full-name }} lets you access data stored in {{ objstorage-full-name }} in the same way as data stored in a DBMS by making queries in an SQL-like language called [YQL]{% if lang == "en" %}(https://ydb.tech/en/docs/yql/reference/syntax/){% endif %}{% if lang == "ru" %}(https://ydb.tech/ru/docs/yql/reference/syntax/){% endif %}.
