...
description: You can connect to {{ ES }} cluster hosts with the _Data node_ role from the internet, if you have set up public access to the host of interest, and from {{ yandex-cloud }} VMs located in the same virtual network.
keywords:
...
You can connect to the hosts in the {{ mes-name }} cluster with the [_Data node_](../concepts/hosts-roles.md#data-node) role:
...
To connect to a host, you need its fully qualified domain name ([FQDN](../concepts/network.md#hostname)). You can use:

* FQDN of a specific host with the _Data node_ role, e.g., `{{ host-name }}.{{ dns-zone }}`.
  ...
  * [Request a list of cluster hosts](cluster-hosts.md#list-hosts) using the CLI or API.
