...
Hosts in {{ mes-name }} clusters are deployed on {{ compute-full-name }} VMs. You can create these virtual machines on any standard platform supported by {{ compute-name }}.{% if audience != "internal" %} For a detailed description of the platforms, see [{#T}](../../compute/concepts/vm-platforms.md).{% endif %}
