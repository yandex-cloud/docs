...
- Manually
   ...
   1. Create a {{ objstorage-name }} bucket with restricted access. This bucket will be used as a snapshot repository.
   1. Create a service account and assign it the `storage.editor` role. A service account is required to access the bucket from the source and target clusters.
...
- Using {{ TF }}
   ...
   1. Download the [mes-migration.tf](https://github.com/yandex-cloud/examples/tree/master/tutorials/terraform/mes-migration.tf) configuration file to a separate working directory. The file describes:
   ...
   1. Create a static access key for the `sa-bucket` service account.
...
- Manually
   ...
   * {% if audience != "internal" %}[Delete snapshots](../../storage/operations/objects/delete.md) from the bucket and then delete [the entire bucket](../../storage/operations/buckets/delete.md){% else %}Delete snapshots from the bucket and then delete the entire bucket{% endif %}.
