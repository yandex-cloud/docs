...
1. [Select points of presence](#points-of-presence).
1. [Set trunk connection settings](#trunk-connection-config).
1. [Set private connection settings](#private-connection-config).
1. [Prepare private connection parameters](#interconnect-params).
1. [Prepare cloud network parameters](#vpc-networks) for private connection.
...
Select the [type of a transceiver](concepts/index.md#trunk-link) for linking with the {{ yandex-cloud }} equipment. You'll need to specify it in the request for enabling the service. Consider the capabilities of your equipment, as well as the distance between your equipment and the point of presence.

## Points of presence {#points-of-presence}

Select one or more *points of presence* (POPs) where you want to organize a [trunk connection](concepts/index.md). We recommend choosing at least two POPs: this will ensure geographical fault tolerance and service redundancy. Each of the POPs allows you to access your cloud resources in any of the {{ yandex-cloud }} [availability zones](../overview/concepts/geo-scope.md).

![interconnect](../_assets/cloud-interconnect/interconnect.svg)

Points of presence (POPs) and their designations:

* MMTS-9, ul. Butlerova, 7, Moscow, Russia: `M9`.
* Dataline OST, ul. Borovaya, 7, str. 10, Moscow, Russia: `OST`.
* Dataline NORD, Korovinskoe shosse, 41, Moscow, Russia: `NORD`.
* StoreData, ul. Nizhegorodskaya, 32, str. A, Moscow, Russia: `STD`.
...
* The port must be in trunk mode according to the IEEE 802.1Q standard with a VLAN identifier (VID) from `1` to `4060`. Native VLAN is not supported.
* To transfer jumbo frames, you can set the MTU size to 8910 bytes.

Within a single POP, you can use:

* Channel aggregation methods using [LACP](https://en.wikipedia.org/wiki/Link_aggregation#Link_Aggregation_Control_Protocol) in the `Active` mode.
...
* IPv4 subnet for BGP peering. Select an IPv4 subnet from the ranges defined in [RFC 1918](https://datatracker.ietf.org/doc/html/rfc1918) to organize BGP interaction between the client device and the {{ yandex-cloud }} edge router. The subnet size can be `/30` or `/31`. One of the addresses of this subnet will be configured on the {{ yandex-cloud }} equipment and the other address on the client device. These addresses will be used to establish a BGP session between the sides.
...
1. Fill out the [agreement letter form](https://storage.yandexcloud.net/doc-files/interconnect-agreement-M9-example.docx). Specify details about your equipment rack and connection parameters. Notify [technical support](https://concole.cloud.yandex.ru/support) or your manager that you wish to activate {{ interconnect-name }} on your own.
...
1. Fill out the [agreement letter form](https://storage.yandexcloud.net/doc-files/interconnect-agreement-Ost-example.docx). Specify details about your equipment rack and connection parameters. Notify [technical support](https://concole.cloud.yandex.ru/support) or your manager that you wish to activate {{ interconnect-name }} on your own.
1. We will respond with a signed agreement letter with the port number allocated for your connection on the {{ yandex-cloud }} side.
1. Request a cross connection from your telecom operator or technical support by attaching the agreement letter.
1. The operator or site staff will then be able to start installing a cross connection and connecting it to a dedicated port in the {{ yandex-cloud }} equipment.
...
1. Fill out the [agreement letter form](https://storage.yandexcloud.net/doc-files/interconnect-agreement-Nord-example.docx). Specify details about your equipment rack and connection parameters. Notify [technical support](https://concole.cloud.yandex.ru/support) or your manager that you wish to activate {{ interconnect-name }} on your own.
1. We will respond with a signed agreement letter with the port number allocated for your connection on the {{ yandex-cloud }} side.
1. Request a cross connection from your telecom operator or technical support by attaching the agreement letter.
1. The operator or site staff will then be able to start installing a cross connection and connecting it to a dedicated port in the {{ yandex-cloud }} equipment.
