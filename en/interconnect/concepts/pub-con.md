...
Each {{ yandex-cloud }} service has its own entry point: `API Endpoint`. You can view the list of entry points for {{ yandex-cloud }} services [here](https://{{ api-host }}/endpoints). Each entry point has its own ID and address that consists of `FQDN API Endpoint` and the number of the port to which this service gets requests.
