...
In the {{ yandex-cloud }} infrastructure, [{{ PG }}](https://www.postgresql.org/) server clusters are deployed and supported using the {{ mpg-short-name }} service.

To utilize a {{ mpg-short-name }} cluster host as a data source for {{ ml-platform-name }}:

1. [Create](../../../managed-postgresql/operations/cluster-create.md) a new {{ mpg-short-name }} cluster with public access for the host. You can also use an existing cluster with publicly available hosts.
