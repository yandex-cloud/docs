# Configuring the environment for deploying a microservice
...
1. [Upload](../../container-registry/operations/docker-image/docker-image-push.md) the Docker image to the container registry. To do this, run the cell with the code:
   ...
   * `<cloud_registry_path>`: {{ container-registry-name }} registry address.
...
1. In the node creation form, specify the path to the image in the format `cr.yandex/<cloud_registry_path>:<tag>`. For authorization in {{ container-registry-name }}, enter the token type `json_key` as the username, and enter the secret as the password.
