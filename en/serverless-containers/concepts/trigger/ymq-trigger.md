...
Triggers for {{ message-queue-short-name }} require [service accounts](../../../iam/concepts/users/service-accounts.md) to read messages from a message queue and call a container. You can use the same service account for both operations.
...
* To create a trigger, you need the following roles:
   ...
   * `{{ roles-viewer }}` for the folder with the container that the trigger calls.
...
* To run a trigger, service accounts need the following roles:
   ...
   * `serverless.containers.invoker` for the folder with the container that calls the trigger.
...
* [Trigger for {{ message-queue-short-name }} that passes messages to the {{ sf-name }} function](../../../functions/concepts/trigger/ymq-trigger.md).