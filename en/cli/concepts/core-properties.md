...
| `--folder-id` | `folder-id` | [ID of the folder](../../resource-manager/operations/folder/get-id.md) to use when executing the command. This ID is used to access a resource by name in a folder different from the default folder. When accessing a resource by its ID, you do not have to specify the folder: the search is global based on access permissions. When this profile parameter is set, `folder-name` gets cleared. |
| `--folder-name` | `folder-name` | Name of the folder to use when executing the command. This ID is used to access a resource by name in a folder different from the default folder. When accessing a resource by its ID, you do not have to specify the folder: the search is global based on access permissions. When this profile parameter is set, `folder-id` gets cleared. |
...
| — | `service-account-key`| The authorized key of a [service account](../../iam/concepts/users/service-accounts.md) in {{ yandex-cloud }}. It is set from the file where the key is stored. When this profile parameter is set, `token` gets cleared. |
| — | `services.<service_name>.version` | The default version of the service command tree. For more information, see [{#T}](../operations/command-versioning.md). |
| `--token` | `token` | Token used for authentication in {{ yandex-cloud }}. When this profile parameter is set, `service-account-key` gets cleared. |