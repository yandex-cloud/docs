...
| `--cloud-id` | `cloud-id` | [ID of the cloud](../../resource-manager/operations/cloud/get-id.md) to use when executing the command. It is only used in the [yc resource-manager folder list](../cli-ref/resource-manager/cli-ref/folder/list.md) command or along with the `folder-name` parameter. In other cases, the parameter is ignored. |
