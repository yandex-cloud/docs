...
* Added the `yc dataproc job cancel <JOB-ID>|<JOB-NAME>` command.
  ...
  Now you can cancel the unfinished task.
...
* Added the `--data-stream` parameter to support {{ yds-full-name }} to the `yc logging group {create,update}` commands.
...
* The `yc managed-kafka cluster update` command.

  Added the `--version` flag that lets you update the {{ KF }} cluster version.

##### {{ managed-k8s-name }} {#k8s}

* The `yc k8s cluster update` command.
...
* The `yc completion <SHELL>` command.
...
{% if audience != "internal" %}{{ certificate-manager-name }} is at the [Preview](../overview/concepts/launch-stages.md) stage. {% endif %}Read more about the service in the [documentation](../certificate-manager/).
...
{% if audience != "internal" %}{{ api-gw-name }} is at the [Preview](../overview/concepts/launch-stages.md) stage. {% endif %}Read more about the service in the [documentation](../api-gateway/).
...
* Commands `yc <service> <resource> set-access-binding`.
...
* {% if audience != "internal" %}For [resources that you can assign roles for](../iam/concepts/access-control/resources-with-access-control.md). {% else %}For resources that roles can be assigned for.{% endif %}
...
{% if audience != "internal" %}{{ kms-name }} is at the [Preview](../overview/concepts/launch-stages.md) stage. {% endif %}Read more about the service in the [documentation](../kms/).
...
* [Federated authentication](operations/authentication/federated-user.md) is now supported for non-interactive `yc` calls. This mode is used when the CLI is not started directly by the user, for example via a script or in `kubectl` when working with {{ k8s }} clusters in {{ managed-k8s-name }}.
...
* Commands `yc load-balancer network-load-balancer create` and `yc load-balancer network-load-balancer update`.
  ...
  For the `--listener` flag, you can now set the `target-port` parameter that lets you configure NAT so that target resources receive traffic on a port other than `listener`.
