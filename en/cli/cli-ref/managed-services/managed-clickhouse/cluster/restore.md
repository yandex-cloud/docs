...
|`--enable-sql-user-management`| Whether user management through SQL commands is enabled. --labels key=value[,key=value...] A list of labels as key-value pairs to add for the cluster being restored.|
|`--service-account`|<b>`string`</b><br/> Service account for the ClickHouse cluster being restored. --security-group-ids value[,value] A list of security groups for the ClickHouse cluster.|
