...
ID of the backup to restore from. To get this ID, make a [BackupService.List] request (lists all backups in a folder) or a [ClusterService.ListBackups] request (lists all backups for an existing cluster). ||
...
Deployment environment of the new cluster. Possible Values: 'production', 'prestable' ||
|| `--folder-id` | `string`
...
Timestamp of the moment to which the MySQL cluster should be restored. (RFC3339) ||
