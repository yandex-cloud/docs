...
# yc beta managed-clickhouse ml-model create
...
`yc beta managed-clickhouse ml-model create <CLUSTER-ID>`
...
ID of the cluster to create a model in. To get a cluster ID make a [ClusterService.List] request. ||
...
Type of the model. Possible Values: 'ml-model-type-catboost' ||
|| `--uri` | `string`
