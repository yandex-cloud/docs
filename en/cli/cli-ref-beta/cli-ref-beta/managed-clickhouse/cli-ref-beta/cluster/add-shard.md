...
# yc beta managed-clickhouse cluster add-shard
...
`yc beta managed-clickhouse cluster add-shard <CLUSTER-ID>`
...
``` ||
|| `--copy-schema` | Whether to copy schema to hosts of the shard to be created. The schema is copied from hosts of an existing shard. ||
