...
# yc beta managed-greenplum cluster update
...
`yc beta managed-greenplum cluster update <CLUSTER-ID>`
...
Custom labels for the Greenplum® cluster as 'key:value' pairs. For example, '"project":"mvp"' or '"source":"dictionary"'. The new set of labels completely replaces the old one. To add a label, request the current set with the [ClusterService.Get] method, then send an [ClusterService.Update] request with the new label added to the set. ||
