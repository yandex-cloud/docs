...
editable: false
noIndex: true
sourcePath: en/_cli-ref-beta/cli-ref-beta/managed-kafka/cli-ref-beta/topic/update.md
---

# yc beta managed-kafka topic update
...
`yc beta managed-kafka topic update <CLUSTER-ID>`
...
ID of the Apache Kafka® cluster to update a topic in. To get the cluster ID, make a [ClusterService.List] request. ||
|| `--topic-name` | `string`

Name of the topic to update. To get the name of the topic, make a [TopicService.List] request. ||
|| `--topic-spec` | `shorthand/json`

New configuration of the topic. Use [update_mask] to prevent reverting all topic settings that are not listed in [topic_spec] to their default values.
