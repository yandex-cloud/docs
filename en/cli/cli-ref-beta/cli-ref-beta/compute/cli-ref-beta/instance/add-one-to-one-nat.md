...
|`--one-to-one-nat-spec`|<b>`shorthand/json`</b><br/>An external IP address configuration.<br/>If not specified, then this instance will have no external internet access.<br/><br/>Example:|
|`--one-to-one-nat-spec`|<b>`address=value,`</b><br/>dns-record-specs=[{dns-zone-id=value, fqdn=value, ptr=true, ttl=1}], ip-version=IPV4<br/><br/>Shorthand Syntax:<br/>{address=str, dns-record-specs=[{dns-zone-id=str, fqdn=str, ptr=bool, ttl=int},...], ip-version=IPV4\|IPV6}<br/>Fields:<br/>address           string           —<br/>dns-record-specs  []struct         — External DNS configuration<br/>dns-zone-id  string            — DNS zone id (optional, if not set, private zone used)<br/>fqdn         string  required  — FQDN (required)<br/>ptr          bool              — When set to true, also create PTR DNS record (optional)<br/>ttl          int               — DNS record ttl, values in 0-86400 (optional)<br/>ip-version        enum<IPV4\|IPV6>  — External IP address version.<br/>|
