...
description: Access management in {{ kms-full-name }}. This section describes the resources you can assign a role for, the roles this service has, and the roles required for specific actions.
---
...
To assign a role for a resource, you need the `kms.admin` role or one of the following roles for that resource:
...
You can assign a role for individual resources within {{ kms-name }} in the [management console]({{ link-console-main }}), using the {{ yandex-cloud }} [CLI](../../cli/cli-ref/kms/cli-ref/index.md), [API](../api-ref/authentication.md), or [{{ TF }}]({{ tf-provider-link }}):
...
Service roles provide more granular control over {{ kms-short-name }} keys tailored to {{ kms-short-name }} specifics, maintaining a strict separation between key administrators (the `kms.admin` role) and users (the `kms.keys.encrypterDecrypter` role).
...
Getting a list of keys in a folder | `list` | `kms.auditor` for the folder
...
[Deleting keys](../operations/key.md#delete) and [destroying versions](../operations/version.md#delete)| `delete`, `scheduleVersionDestruction`, `cancelVersionDestruction` | `kms.admin` for the key
[Assigning](../../iam/operations/roles/grant.md) and [revoking a role](../../iam/operations/roles/revoke.md) | `setAccessBindings`, `updateAccessBindings` | `kms.admin` for the key
Viewing roles assigned for a key | `listAccessBindings` | `kms.auditor` for the key
