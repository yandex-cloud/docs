...
{{ kms-name }} supports the following {{ TF }} provider resources:
...
| [yandex_kms_symmetric_key_iam_binding]({{ tf-provider-resources-link }}/kms_symmetric_key_iam_binding) | [Assigning](../iam/concepts/access-control/index.md#access-bindings) access permissions for a symmetric encryption key. <br>We recommend using `kms_symmetric_key_iam_member` rather than `kms_symmetric_key_iam_binding`. |
...
{{ kms-name }} supports the following {{ TF }} provider data sources:
