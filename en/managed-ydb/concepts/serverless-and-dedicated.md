...
In Yandex Cloud, a serverless database supports data operations using both the YDB API and [Document API]{% if lang == "ru" %}(https://cloud.yandex.ru/docs/ydb/docapi/api-ref/){% endif %}{% if lang == "en" %}(https://cloud.yandex.com/en/docs/ydb/docapi/api-ref/){% endif %}, an Amazon DynamoDB-compatible HTTP API. You can use this API to perform operations on document tables.
