...
Field | Description
--- | ---
backupId | **string**<br><p>Required. ID of the backup to create a cluster from. To get the backup ID, use a <a href="/docs/mdb/api-ref/clickhouse/Cluster/listBackups">listBackups</a> request.</p> 
name | **string**<br><p>Required. Name of the new ClickHouse cluster. The name must be unique within the folder.</p> <p>Value must match the regular expression <code>|[a-z][-a-z0-9]{1,61}[a-z0-9]</code>.</p> 
description | **string**<br><p>Description of the new ClickHouse cluster. 0-256 characters long.</p> <p>The maximum string length in characters is 256.</p> 
...
environment | **string**<br><p>Required. Deployment environment of the new ClickHouse cluster.</p> <p>Deployment environment.</p> <ul> <li>PRODUCTION: Stable environment with a conservative update policy: only hotfixes are applied during regular maintenance.</li> <li>PRESTABLE: Environment with more aggressive update policy: new versions are rolled out irrespective of backward compatibility.</li> </ul> 
...
hostSpecs.<br>subnetId | **string**<br><p>ID of the subnet that the host should belong to. This subnet should be a part of the network that the cluster belongs to. The ID of the network is set in the <a href="/docs/mdb/api-ref/clickhouse/Cluster#representation">Cluster.networkId</a> field.</p> <p>The maximum string length in characters is 50.</p> 
