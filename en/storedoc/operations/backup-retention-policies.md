# Managing backup policies in {{ mmg-name }}
...
- Management console {#console}
    ...
    1. Click the name of the cluster and select ![chevron-down](../../_assets/console-icons/chevron-down.svg) **{{ ui-key.yacloud.shared.layout.PageTabs.button_other_hnYwF }}** → **{{ ui-key.yacloud.mongodb.cluster.switch_backups }}**.
    1. On the **{{ ui-key.yacloud.mdb.cluster.backups.label_title }}** page, select the **{{ ui-key.yacloud.mdb.BackupPolicy.label_backup-policies_s4xXC }}** tab.
...
- CLI {#cli}
    ...
    1. See the description of the CLI command for creating a policy:
        ...
        ```bash
        {{ yc-mdb-mg }} backup-retention-policy create --help
        ```

    1. Create a policy:

        ```bash
        {{ yc-mdb-mg }} backup-retention-policy create \
...
- {{ TF }} {#tf}
    ...
    1. Add a resource description:
        ...
        ```hcl
        resource "yandex_mdb_mongodb_backup_retention_policy" "<policy_name>" {
    ...
    1. Confirm updating the resources.
        ...
        For more information, see [this {{ TF }} provider guide]({{ tf-provider-resources-link }}/mdb_mongodb_backup_retention_policy).
...
- REST API {#api}
    ...
    1. Call the [BackupRetentionPolicy.Create](../api-ref/BackupRetentionPolicy/create.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
        ...
        curl \
          ...
          --url 'https://{{ api-host-mdb }}/managed-mongodb/v1/clusters/<cluster_ID>/retention_policies' \
          --data '{
                     "cron": {
...
- gRPC API {#grpc-api}
    ...
    1. To create a policy:
        ...
        1. Call the [BackupRetentionPolicyService.Create](../api-ref/grpc/BackupRetentionPolicy/create.md) method, e.g., via the following {{ api-examples.grpc.tool }} request:
...
- CLI {#cli}
    ...
    1. See the description of the CLI command for getting a list of policies:
        ...
        ```bash
        {{ yc-mdb-mg }} backup-retention-policy list --help
        ```

    1. Get a list of policies for the {{ mmg-name }} cluster:

        ```bash
        {{ yc-mdb-mg }} backup-retention-policy list \
           --cluster-id <cluster_ID>
        ```
...
- REST API {#api}
    ...
    1. Call the [BackupRetentionPolicy.List](../api-ref/BackupRetentionPolicy/list.md) method, e.g., via the following {{ api-examples.rest.tool }} request:

        ```bash
        curl \
          --request GET \
          --header "Authorization: Bearer $IAM_TOKEN" \
          --url 'https://{{ api-host-mdb }}/managed-mongodb/v1/clusters/<cluster_ID>/retention_policies?pageSize=<number_of_results>'
        ```
    ...
    1. Check the [server response](../api-ref/BackupRetentionPolicy/list.md#yandex.cloud.mdb.mongodb.v1.ListBackupRetentionPoliciesResponse) to make sure your request was successful.
...
- gRPC API {#grpc-api}
    ...
    1. To get a list of policies for a cluster:
        ...
        1. Call the [BackupRetentionPolicyService.List](../api-ref/grpc/BackupRetentionPolicy/list.md) method, e.g., via the following {{ api-examples.grpc.tool }} request:
        ...
        1. Check the [server response](../api-ref/grpc/BackupRetentionPolicy/list.md#yandex.cloud.mdb.mongodb.v1.ListBackupRetentionPoliciesResponse) to make sure your request was successful.
...
- CLI {#cli}
    ...
    1. See the description of the CLI command for deleting a policy:
        ...
        ```bash
        {{ yc-mdb-mg }} backup-retention-policy delete --help
        ```

    1. Delete a policy:

        ```bash
        {{ yc-mdb-mg }} backup-retention-policy delete <policy_ID> \
           --cluster-id <cluster_ID>
        ```
...
- {{ TF }} {#tf}
    ...
    1. Delete the `yandex_mdb_mongodb_backup_retention_policy` resource with the name of the policy you want to delete.
...
- REST API {#api}
    ...
    1. Call the [BackupRetentionPolicy.Delete](../api-ref/BackupRetentionPolicy/delete.md) method, e.g., via the following {{ api-examples.rest.tool }} request:

        ```bash
        curl \
          --request DELETE \
          --header "Authorization: Bearer $IAM_TOKEN" \
          --url 'https://{{ api-host-mdb }}/managed-mongodb/v1/clusters/<cluster_ID>/retention_policies/<policy_ID>'
        ```
...
- gRPC API {#grpc-api}
    ...
    1. To delete a policy:
        ...
        1. Call the [BackupRetentionPolicyService.Delete](../api-ref/grpc/BackupRetentionPolicy/delete.md) method, e.g., via the following {{ api-examples.grpc.tool }} request:
