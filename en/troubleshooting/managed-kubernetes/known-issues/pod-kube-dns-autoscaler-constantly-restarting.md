...
Updating {{ k8s }} and node groups from version 1.21 to version 1.22 triggered `kube-dns-autoscaler` cyclical restarts.
...
1. [Add the official `cluster-proportional-autoscaler`](https://github.com/kubernetes-sigs/cluster-proportional-autoscaler/tree/master) repository to your deployment:
   ```
...
1. Using our adapted `values`, install `cluster-proportional-autoscaler`:
   ...
   {% cut "Installing `cluster-proportional-autoscaler`" %}
