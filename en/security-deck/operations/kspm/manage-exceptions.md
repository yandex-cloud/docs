...
- {{ sd-name }} UI {#cloud-sd}
  ...
  1. In the top-right corner, click **{{ ui-key.yacloud_org.security.controls.ControlsExceptionsPage.action_create }}** ![chevron-down](../../../_assets/console-icons/chevron-down.svg) and select `{{ kspm-full-name }}`. On the page that opens:
      ...
      When creating exceptions for `Admission` rules, you can additionally manage the operating modes of these rules. For more information, see [{#T}](./manage-rule-modes.md#apply-exception).
      ...
      1. Optionally, under **{{ ui-key.yacloud_org.security.workspaces.section-title_9BLTm }}**, update the objects subject to the exception. You can specify multiple values separated by a comma, space, or `Enter`.
