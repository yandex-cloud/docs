---
title: Managing access to a {{ mmy-full-name }} cluster
description: Follow this guide to configure {{ mmy-name }} cluster access permissions.
---

# Managing access to a {{ mmy-name }} cluster
...
- REST API {#api}
  ...
  1. Call the [Cluster.ListAccessBindings](../api-ref/Cluster/listAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --url 'https://{{ api-host-mdb }}/managed-mysql/v1/clusters/<cluster_ID>:listAccessBindings'
      ```
...
- CLI {#cli}
  ...
  1. Use this command to assign a role:
      ...
      ```bash
      {{ yc-mdb-my }} cluster add-access-binding <cluster_name_or_ID> \
        --role <role> \
      ...
      * `--role`: [Role](../security/index.md#roles-list), e.g., `managed-mysql.editor`.
...
- {{ TF }} {#tf}
  ...
  1. Add a resource description:
     ...
     * `role`: [Role](../security/index.md#roles-list), e.g., `managed-mysql.editor`.
     * `members`: Array of designations of [subjects](../../iam/concepts/access-control/index.md#subject) the role is assigned to.
...
- REST API {#api}
  ...
  1. Call the [Cluster.UpdateAccessBindings](../api-ref/Cluster/updateAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://{{ api-host-mdb }}/managed-mysql/v1/clusters/<cluster_ID>:updateAccessBindings' \
        --data '{
      ...
      * `access_binding_deltas.roleId`: [Role](../security/index.md#roles-list) being assigned, e.g., `managed-mysql.editor`.
...
- CLI {#cli}
  ...
  1. To assign roles, run this command:
      ...
      ```bash
      {{ yc-mdb-my }} cluster set-access-bindings <cluster_name_or_ID> \
...
- {{ TF }} {#tf}
  ...
  1. Add resource descriptions:
     ...
     resource "yandex_mdb_mysql_cluster_iam_binding" "<resource_2_local_name>" {
       cluster_id = "<cluster_ID>"
       role       = "<role_2>"
       members    = ["<subject_type>:<subject_ID>"]
     }
...
- REST API {#api}
  ...
  1. Call the [Cluster.SetAccessBindings](../api-ref/Cluster/setAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://{{ api-host-mdb }}/managed-mysql/v1/clusters/<cluster_ID>:setAccessBindings' \
        --data '{
                  "accessBindings": [
                    {
                      "roleId": "<role>",
      ...
      * `accessBindings.roleId`: [Role](../security/index.md#roles-list) being assigned, e.g., `managed-mysql.editor`.
...
- CLI {#cli}
  ...
  1. To revoke a role, run this command:
      ...
      ```bash
      {{ yc-mdb-my }} cluster remove-access-binding <cluster_name_or_ID> \
        --role <role> \
        --subject <subject_type>:<subject_ID>
      ```
      ...
      * `--role`: [Role](../security/index.md#roles-list) being revoked, e.g., `managed-mysql.editor`.
...
- REST API {#api}
  ...
  1. Call the [Cluster.UpdateAccessBindings](../api-ref/Cluster/updateAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      * `access_binding_deltas.roleId`: [Role](../security/index.md#roles-list) being revoked, e.g., `managed-mysql.editor`.
...
For a service account to be able to view the info of all {{ mmy-name }} clusters in the folder but modify the resources in one particular cluster alone, give it the `managed-mysql.viewer` role for the folder and the `managed-mysql.editor` role for that cluster:
...
- CLI {#cli}
  ...
  1. Assign the roles for the cluster:
      ...
      ```bash
      {{ yc-mdb-my }} cluster add-access-bindings <cluster_name_or_ID> \
        --access-binding role=managed-mysql.editor,subject=serviceAccount:<service_account_ID>
      ```
