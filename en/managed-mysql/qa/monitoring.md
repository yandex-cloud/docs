...
To view charts in [{{ monitoring-full-name }}]{% if lang == "ru" %}(https://monitoring.cloud.yandex.ru/){% endif %}{% if lang == "en" %}(https://monitoring.cloud.yandex.com/){% endif %}, on the home page, select the **{{ mmy-short-name }}** service dashboard.
...
You can configure alerts using [{{ monitoring-full-name }}]{% if lang == "ru" %}(https://monitoring.cloud.yandex.ru/){% endif %}{% if lang == "en" %}(https://monitoring.cloud.yandex.com/){% endif %}. To do this, use the instructions under [{#T}](../../monitoring/operations/alert/create-alert.md). When selecting a metric, specify the `service` parameter as **{{ mmy-short-name }}**.
