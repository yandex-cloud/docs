...
* The entire server (administrative privileges).
...
The permissions that apply at the level of the entire DB server are set in the [user settings](./settings-list.md#setting-global-permissions).

To differentiate the user permissions at the level of individual databases, each user is [granted](../operations/grant.md) one or more _roles_. Each role allows the user to perform the actions specified in the [role description](#db-roles). A user is created along with a cluster and this user is granted the `ALL_PRIVILEGES` role in the first database of the cluster.

To manage user permissions at the level of the entire server or an individual database, [use](../operations/grant.md) the {{ yandex-cloud }} interfaces. Changes made by SQL commands at these levels are not saved: the cluster returns to the state set through the {{ yandex-cloud }} interfaces.

To manage user permissions at the level of an individual DB object, use the `GRANT` and `REVOKE` SQL commands.

## User roles in {{ mmy-name }} clusters {#db-roles}
