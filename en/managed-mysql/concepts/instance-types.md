...
Hosts in the cluster {{ mmy-name }} are deployed on virtual machines in {{ compute-full-name }}. These virtual machines can be created on any of the platforms that are supported by {{ compute-name }}. Platforms are described in detail in the section [#T](../../compute/concepts/vm-platforms.md).
