...
* [IAM token](iam-token.md) is the recommended and most secure type. It is suitable for most operations, such as [creating a VM](../../../compute/operations/vm-create/create-linux-vm.md). It does not work with certain services or APIs that require other types of credentials.
    ...
    You can set up automatic IAM token renewal using [refresh tokens](./refresh-token.md) for all types of user [accounts](../users/accounts.md). This allows them to access the [{{ yandex-cloud }} CLI](../../../cli/index.yaml) without re-authenticating in the browser after the IAM token expires.
