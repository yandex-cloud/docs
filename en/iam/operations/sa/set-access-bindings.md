...
- API
   ...
   1. Find out the user ID from the login using the [getByLogin](../../api-ref/YandexPassportUserAccount/getByLogin.md):
    ...
    1. Assign the user the `editor` role for the `my-robot` service account. In the `action` property, specify `ADD`, and in the `subject` property, specify the `userAccount` type and the user ID:
...
- {{ TF }}
   ...
   1. Add the resource parameters to the configuration file and specify the users' role to access the service account:
      ...
      * `members`: List of users or service account the role is being assigned to. Specified in the form `userAccount:<user ID>` or `serviceAccount:<service account ID>`. Required parameter.
