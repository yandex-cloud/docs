...
**How to fix**: [Assign the `viewer` or `container-registry.images.puller` role](../../iam/operations/sa/set-access-bindings.md) to the service account for a repository, registry, or folder. For more information about the roles available in the service, see our [documentation](../../container-registry/security/index.md).
...
**How to fix**: Check if there is access to {{ container-registry-name }} by running the `nc -vz {{ registry }} 443` command. If there is no access, [configure a NAT instance](../../tutorials/routing/nat-instance/index.md) or assign a public IP address to the VMs with {{ coi }}. You can also [set up a NAT gateway](../../vpc/operations/create-nat-gateway.md) for the subnet you are creating the VMs in.
...
**How to fix**: Stop the VM and [increase disk size](../../compute/operations/disk-control/update.md#change-disk-size).

## Disk partition is not specified {#disk-partition}
...
**How to fix**: Run the `lsblk` command to learn the number of the partition for mounting. Add the `partition: x` parameter to the [Docker Compose specification](../concepts/coi-specifications.md#compose-spec), where `x` is the number of the partition for mounting, and [update](../tutorials/vm-update.md) the VM using the edited specification.

## The disk cannot be mounted {#disk-mount}
...
**How to fix**: Set the `device-name` parameter equal to `device_name` in the [Docker Compose specification](../concepts/coi-specifications.md#compose-spec-example). You can set the `device-name` parameter in flags and CLI commands used to add disks to a VM or in the management console when [attaching](../../compute/operations/vm-control/vm-attach-disk#attach) a disk to a VM. In the example above, set `device-name` to `coi-data`.
