...
{{ datalens-short-name }} can connect to Yandex Metrica directly (for details, see [Web analytics with a connection to Yandex Metrica](../../datalens/tutorials/data-from-metrica-visualization.md)). In this case, you will not have access to some analytical functions for data as the data is accessed over an API with limited functionality. The API may slowly respond to queries involving large amounts of data. If using the API a lot, you may exceed the API limits.
