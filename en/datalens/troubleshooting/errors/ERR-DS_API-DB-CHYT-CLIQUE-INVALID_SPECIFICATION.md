---
title: '{{ datalens-full-name }} error ERR.DS_API.DB.CHYT.CLIQUE.INVALID_SPECIFICATION'
description: This page describes the Invalid clique specification {{ datalens-full-name }} error. Probably, clique does not exists.
---

# [{{ datalens-full-name }}] Invalid clique specification. Probably, clique does not exists

`ERR.DS_API.DB.CHYT.CLIQUE.INVALID_SPECIFICATION`

The clique specified in the connection settings does not exist. The possible causes might include the following:

* Clique name incorrectly specified in the connection. Make sure that you have correctly specified the name of the clique in the connection settings.
* The clique has been deleted. Replace the clique in the connection settings with another clique.
