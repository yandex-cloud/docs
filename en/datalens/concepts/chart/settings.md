...
| {% if audience == "internal" %}Comments feed | ID of the comments chart from another chart. It's used for automatic duplication of comments that are shared by different charts. The setting is only available for charts that use time series (those that have, in their **X** or **Y** section, a dimension with the type of **Date** or **Date and time**).<br/>For more, see the [instructions](../../operations/chart/use-comments.md).{% endif %} |
