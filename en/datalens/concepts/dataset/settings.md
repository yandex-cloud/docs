...
{{ datalens-short-name }} works with the source in direct access mode:

* {{ datalens-short-name }} doesn't store data on its side. CSV files are uploaded to S3 storage and stored there.
* All the calculations for previewing datasets, charts, and dashboards are run on the data source resources.
...
You can configure permissions for an entire dataset. {% if audience != "internal" %} For more information, see [{#T}](../../operations/dataset/manage-access.md). {% endif %}
