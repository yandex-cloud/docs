...
* Importing a bundle that includes `lego-on-react`, `@yandex-data-ui/dt100`, `@yandex-data-ui/react-components`,
...
* Importing the source files to avoid repeat inclusion of `lego-on-react`, `@yandex-data-ui/dt100`,
   `@yandex-data-ui/react-components`,  `@yandex-data-ui/common` into the bundle.
...
<ChartKit
    ...
    // Don't show a loader during updates
    silentLoading={true}
    // Or show a loader after 500 ms
    silentLoading={500}
    // Show a small loader in the upper-right corner instead of a standard centered loader
    compactLoader={true}
    // Theme modifier
    theme="light"
    // Array of menu items
...
ChartKit.setSettings({
    ...
    // Request decorator (presently, only headers can be decorated)
