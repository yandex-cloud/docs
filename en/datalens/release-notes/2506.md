...
description: Check out the {{ datalens-full-name }} release notes for June 2025.
---
...
Added the **Retry** button to the [workbook export and import](../workbooks-collections/export-and-import.md) error window.

### Linking a card to a billing account {#ba-card-link}

You can now optionally link a card when [creating a billing account]({{ link-docs }}/billing/operations/create-new-account).

### Changes in notifications {#notifications-changes}
...
Fixed the issue with replacing tables in a connection to [Yandex Documents](../operations/connection/create-yadocs.md), where only the last element would be replaced correctly.
...
* Fixed the issue with copying links when pasting a selector into another widget using the **Paste** button. Now selectors are copied together with their links, same as when added to the dashboard via the **Insert** button on the bottom panel of the dashboard page.
* Fixed copying in the dashboard access link settings window: now you can just click ![icon](../../_assets/console-icons/copy.svg) in the **Link** field. We also added the **Open current tab by default** option.
* Fixed the following issues:
  ...
  * Switching to edit mode would change the indent between the widget pinning area and the dashboard's main content.
  * On dashboards without tabs or widget pinning areas, the main content area was pinning itself to the dashboard header.

### Fixes in charts {#chart-fixes}

Fixed the bug with opening a chart preview via the **Open in new tab** menu item. The preview page would be loading indefinitely unless you had relevant data viewing permissions. Now you get an access restriction message instead.

Also fixed the bug causing the address bar parameters to disappear when redirected to a new address.
