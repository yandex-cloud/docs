...
* New dashboards available in [{{ datalens-gallery }}]({{ link-datalens-main }}/gallery). For more information, see the [{{ datalens-short-name }}](https://t.me/YandexDataLens/28631/145412) Telegram chat.
...
* Implemented the option to [save a chart as a `PNG` image](../operations/chart/save-as-image.md). Proceed as follows:
...
* Fixed the issue of incorrectly saved original background when [copying](../operations/dashboard/copying-widgets.md) a dashboard widget. Earlier, if set to `Basic chart background`, the original widget background would reset to transparent when copying.
