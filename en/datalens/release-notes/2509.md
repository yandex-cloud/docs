...
* Now only a user with the `{{ roles-datalens-instances-admin }}` or `{{ roles-datalens-admin }}` [{{ datalens-short-name }} role](../security/roles.md#service-roles) can create a [Usage Analytics](../operations/connection/create-usage-tracking.md) connection. To allow other users to view a connection and query it, the connection administrator can configure [object access permissions](../security/manage-access.md).
...
Read more about the limitations [here](../concepts/chart/data-export.md#restrictions).
