...
## LLM trace page {#page-layout}

The view page displays only spans of the services that have attributes with the `gen_ai.*` mask. Spans of other services (backends, databases, etc.) will be hidden on this page, thus focusing the interface on the conversation with the model. To see the full trace with all spans, click **Show full trace** at the top of the page. This will open the [standard trace view](../operations/trace-view.md).
...
## Search and filtering {#filtering}
