...
You can assign a role to a {% if audience != "internal" %}[cloud](../../resource-manager/concepts/resources-hierarchy.md#cloud) or [folder](../../resource-manager/concepts/resources-hierarchy.md#folder){% else %}cloud or folder{% endif %}. Cloud roles also apply to nested folders.
...
To use the service, you need the `editor` {% if audience != "internal" %}[role](../../iam/concepts/access-control/roles.md){% else %}role{% endif %} or higher to the folder where projects are being created. With the `viewer` role, you can only view the list of projects and the contents of files that were downloaded.
