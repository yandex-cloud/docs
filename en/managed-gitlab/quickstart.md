# Getting started with {{ mgl-short-name }}
...
1. In the **Administrator Data** block specify:
    * **Email** — the email address of the administrator of your GitLab instance.
    * **Login** — the username for the administrative account.

1. Click **Create**.

1. Wait until the cluster is ready: its status on the {{ mgl-short-name }} dashboard will change to **Running** and its state to **Alive**. This may take some time.

## Set up a working environment {#configure-mgl}

In this section, you will learn how to prepare an instance {{ mgl-short-name }} to work.

1. Activate your instance {{ mgl-short-name }}:
...
1. [Create a group](https://docs.gitlab.com/ee/ssh/#add-an-ssh-key-to-your-gitlab-account), in which your project will be hosted.

1. [Create a user](https://docs.gitlab.com/ee/user/profile/account/create_accounts.html) and [add it to the group](https://docs.gitlab.com/ee/user/group/#add-users-to-a-group).

1. Create an SSH key and [assign it to the user account](https://docs.gitlab.com/ee/ssh/#add-an-ssh-key-to-your-gitlab-account).

1. [Create an empty project](https://docs.gitlab.com/ee/user/project/working_with_projects.html#blank-projects), where your repository will be hosted.
...
1. [Transmit the changes](https://docs.gitlab.com/ee/gitlab-basics/start-using-git.html#add-and-commit-local-changes) in {{ mgl-short-name }}:
