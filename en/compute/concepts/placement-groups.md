...
_VM placement groups_ are groups of VMs, each hosted on physical hardware as per a strategy. {{ compute-full-name }} uses [spread placement](#spread) and [partition placement](#partition) strategies.
...
_Spread placement_ is a VM placement strategy that guarantees that each VM instance is hosted on a separate server rack. If one rack fails, the others will continue to run normally. Virtual machines within the same group can reside in different [availability zones](../../overview/concepts/geo-scope.md).
...
To learn about the limits on the number of VMs in a spread placement group, see [{#T}](../concepts/limits.md#compute-limits-vm).
...
_Partition placement_ is a VM placement strategy that guarantees that VMs from different partitions reside on different server racks in a data center of one of the availability zones. If one of the server racks with the group's partitions fails, only one partition will be affected.
