...
* Service account to [work with cloud resources](../operations/vm-connect/auth-inside-vm) from inside the VM, e.g., to deliver metrics to {{ monitoring-full-name }}, send logs to {{ cloud-logging-full-name }}, or connect to {{ backup-full-name }}. This service account is specified in the access parameter group.
* Service account to access encrypted disks. This service account is specified in the disk parameter group.

To use encryption in {{ compute-name }}, a user must have the following roles:
* `iam.serviceAccounts.user` and higher on the service account that is used for encryption. For more information, see [Roles of {{ iam-full-name }}](../../iam/concepts/access-control/roles.md#sa-user).
* `kms.viewer` and higher on the key that is used for encryption. For more information, see [Roles of {{ kms-full-name }}](../../iam/concepts/access-control/roles.md#kms-viewer).
