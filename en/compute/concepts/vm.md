...
A VM is created in one of the [folders](../../resource-manager/concepts/resources-hierarchy.md#folder) in your [cloud](../../resource-manager/concepts/resources-hierarchy.md#cloud) and inherits access rights from them. [More information about the {{ yandex-cloud }}](../../resource-manager/concepts/resources-hierarchy.md).
...
You can also attach additional disks to the VM. You can either attach a previously created disk or create a disk along with the VM. The new disk can be empty, or you can restore it from a [snapshot](../concepts/snapshot.md) or [image](../concepts/image.md).
...
You can set your own metadata when creating or updating VMs. For example, to connect to a Linux VM, you need to provide an SSH key to it. This is done using the metadata service. For more information, see [{#T}](vm-metadata.md).

## Network {#network}

When creating a VM, you need to set the settings of the network interface connected to it: select the [subnet](../../vpc/concepts/network.md#subnet) to connect the VM to, configure [internal and public IP addresses](../../vpc/concepts/address.md), and add the necessary [security groups](../../vpc/concepts/security-groups.md). This allows the VM to interact with other services on the intranet and internet.
