...
You can allow public access to your image by assigning the role `{{ roles-image-user }}` to the system group `{{ subjects-allAuthenticatedUsers }}`. Then other Yandex.Cloud users will be able to use your image to create their own disks and VMs. In this case, the role `{{ roles-image-user }}` does not grant permission to delete or change your image.
