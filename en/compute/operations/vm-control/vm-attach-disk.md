...
- Management console {#console}
  ...
  1. Optionally, if you do not have a suitable disk, [create](../disk-create/empty.md) a new one.
  1. In the row of the disk you want to attach, click ![image](../../../_assets/console-icons/ellipsis.svg) and select **{{ ui-key.yacloud.compute.disks.button_action-attach }}**.
...
- {{ TF }} {#tf}
  ...
  1. In the configuration file, add the new `secondary_disk` section to the `yandex_compute_instance` resource description:
      ...
      For more information about `yandex_compute_disk` properties, see [this provider guide]({{ tf-provider-datasources-link }}/compute_disk).
