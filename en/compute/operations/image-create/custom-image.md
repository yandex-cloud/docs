...
To successfully upload your image, make sure to install the {% if product == "yandex-cloud" %}`virtio-blk`, `virtio-net`, and `virtio-pci` drivers. To use {{ compute-name }} file storage, install the `virtiofs`{% endif %}{% if product == "cloud-il" %}`virtio-blk`, `virtio-net`, `virtio-pci`, and `virtiofs`{% endif %} drivers.
