...
- {{ TF }}
   ...
   1. In the configuration file, describe the disk placement group parameters:
      ...
      * `zone`: [Availability zone](../../../overview/concepts/geo-scope.md). {% if product == "yandex-cloud" %}We recommend creating disk placement groups in the `{{ region-id }}-a` or `{{ region-id }}-b` availability zone.{% endif %}
