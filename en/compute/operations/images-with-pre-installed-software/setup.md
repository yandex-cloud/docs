...
For VMs with content management systems to be deployed, such as Drupal, GitLab, {% if product == "yandex-cloud" %}Joomla, {% endif %}or WordPress, use a [web setup wizard](#web-setup) to perform the initial configuration.
...
1. Type `http://<IP address>` in the browser address bar. The authorization window opens.
   ...
   {% endnote %}
1. Complete the steps of the web setup wizard and follow the recommendations from the developers: [Drupal](https://drupal.org/), [GitLab](https://about.gitlab.com), {% if product == "yandex-cloud" %}[Joomla](https://joomla.org/), {% endif %}or [WordPress](https://wordpress.org/).
