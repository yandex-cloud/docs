...
deviceName | **string** <br> includes only one of the fields `diskId`, `deviceName`<br><br><p>Serial number of the disk that should be detached. This value is reflected into the /dev/disk/by-id/ tree of a Linux operating system running within the instance.</p> <p>Value must match the regular expression <code>[a-z][a-z0-9-_]{,19}</code>.</p> 
