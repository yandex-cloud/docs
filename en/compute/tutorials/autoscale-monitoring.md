...
- Management console
  ...
  1. Create a subnet named `queue-autoscale-subnet-b` in the `ru-central1-b` zone:
...
- CLI
  ...
  1. Create a subnet named `queue-autoscale-subnet-b` in the `ru-central1-b` zone:
     ...
     * `--zone`: The availability zone where the subnet is created (`ru-central1-b`).
...
- API
  ...
  1. Create a subnet named `queue-autoscale-subnet-b` in `ru-central1-b` with the `192.168.1.0/24` CIDR using the [SubnetService/Create](../../vpc/api-ref/grpc/subnet_service.md#Create) gRPC API call or the [create](../../vpc/api-ref/Subnet/create.md) REST API method. In the request body, specify the network ID.
...
- AWS CLI
  ...
  1. Create a queue named `queue-autoscale-queue` and save its URL to the `queue` file:
     ...
     | tee queue
     ```
     * `--endpoint`: The root URL of the {{ message-queue-name }} API: `https://message-queue.api.cloud.yandex.net`.
...
1. In the `server-packer.json` file, change the object in the `builders` array:
   ...
   * In the `service_account_id` field, specify the ID of the `queue-autoscale-sa` service account.
     ...
     {% cut "How to get the service account ID" %}
   ...
   * In the `folder_id` field, specify the ID of the folder in {{ yandex-cloud }}.
     ...
     {% cut "How to get the folder ID" %}

     {% list tabs %}

     - Management console

       In the [management console]({{ link-console-main }}), find the `example-folder` folder in the list and copy its **ID**.
...
- Management console
  ...
  1. Under **Allocation**, select the `ru-central1-b` availability zone.
...
1. Open dashboards in {{ monitoring-full-name }}:
   ...
   - Management console
     ...
     1. Open the dashboard with information about the instance group:
        1. In [{{ monitoring-name }}]{% if lang == "ru" %}(https://monitoring.cloud.yandex.ru){% endif %}{% if lang == "en" %}(https://monitoring.cloud.yandex.com){% endif %}, select the **Compute — Instance Groups** service dashboard.
        1. In the **Instance Group ID** field, select `queue-autoscale-ig`.

     1. Open the dashboard with queue information:
        1. In [{{ monitoring-name }}]{% if lang == "ru" %}(https://monitoring.cloud.yandex.ru){% endif %}{% if lang == "en" %}(https://monitoring.cloud.yandex.com){% endif %}, select the **Message Queue** service dashboard.
...
1. [Delete](../../vpc/operations/subnet-delete.md) `queue-autoscale-subnet-b`.
1. [Delete](../../vpc/operations/network-delete.md) `queue-autoscale-network`.
1. [Delete](../../iam/operations/sa/delete.md) `queue-autoscale-sa`.
