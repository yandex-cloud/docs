...
The virtual router is a core resource in the [{{ cr-name }}](./index.md) resource model. It is designed to create network topologies enabling network connectivity between cloud resources in different [networks](../../vpc/concepts/network.md).
...
1. No direct exchange of routing info between any [private connections](../../interconnect/concepts/priv-con.md) (PRCs) within a virtual router is allowed.

1. The IP prefixes announced by the customer’s network hardware also propagate to the virtual router and from there to the virtual networks connected to that virtual router. Currently, you cannot view these IP prefixes at the virtual router level. We recommend getting this information from the customer's network hardware. In some cases, if routing issues arise, you may want to contact [support]({{ link-console-support }}) for this information.

1. You can only add the [VPC network ID](../../vpc/operations/network-get-info.md) (`vpc_network_id`) to one virtual router at a time. An attempt to add the same ID to another virtual router will fail.

1. You can only add the [private connection ID](../../interconnect/operations/priv-con-get-info.md) (`cic_private_connection_id`) to one virtual router at a time. An attempt to add the same ID to another virtual router will fail.
