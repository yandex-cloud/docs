...
hosts[].<br>role | **string**<br><p>Role of the host in the cluster. If the field has default value, it is not returned in the response.</p> <ul> <li>ROLE_UNKNOWN: Role of the host in the cluster is unknown. Default value.</li> <li>MASTER: Host is the master PostgreSQL server in the cluster.</li> <li>REPLICA: Host is a replica (standby) PostgreSQL server in the cluster.</li> </ul> 
