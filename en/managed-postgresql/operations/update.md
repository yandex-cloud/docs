...
1. Make sure the cloud's quota is sufficient to increase the storage size: open the [Quotas](https://console.cloud.yandex.ru/?section=quotas) for your cloud and check that the [!KEYREF mpg-full-name] section still has space remaining in the **space** line.
...
Make sure the cloud's quota is sufficient to increase the storage size: open the [Quotas](https://console.cloud.yandex.ru/?section=quotas) for your cloud and check that the [!KEYREF mpg-full-name] section still has space remaining in the **space** line.
...
You can change the connection pooler's operation mode for a cluster using the API [update](../api-ref/Cluster/update.md) method: pass the appropriate value in the request parameter `configSpec.poolerConfig.poolingMode`.
