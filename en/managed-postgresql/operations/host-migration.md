...
1. To connect to the database after migration, specify the new host’s FQDN in your backend or client, e.g., in your application code or graphical IDE. Delete the original host's FQDN in the source availability zone.
   ...
   You will see the FQDN under `NAME` in the command output. Alternatively, you can connect using a [special FQDN](connect/fqdn.md#special-fqdns).
...
1. Wait for the cluster state to change to **Alive**. In the management console, [navigate]({{ link-console-main }}/link/managed-postgresql) to **{{ ui-key.yacloud.iam.folder.dashboard.label_managed-postgresql }}**. You can check the cluster state in the **{{ ui-key.yacloud.mdb.clusters.column_availability }}** column.
