---
title: Managing access to a {{ mpg-full-name }} cluster
description: Follow this guide to configure {{ mpg-name }} cluster access permissions.
---

# Managing access to a {{ mpg-name }} cluster
...
- REST API {#api}
  ...
  1. Call the [Cluster.ListAccessBindings](../api-ref/Cluster/listAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --url 'https://{{ api-host-mdb }}/managed-postgresql/v1/clusters/<cluster_ID>:listAccessBindings'
      ```
...
- CLI {#cli}
  ...
  1. View the description of the CLI command for assigning a role for a cluster:
      ...
      ```bash
      {{ yc-mdb-pg }} cluster add-access-binding --help
      ```

  1. Use this command to assign a role:

      ```bash
      {{ yc-mdb-pg }} cluster add-access-binding <cluster_name_or_ID> \
        --role <role> \
...
- CLI {#cli}
  ...
  1. View the description of the CLI command for assigning roles for a cluster:
      ...
      ```bash
      {{ yc-mdb-pg }} cluster set-access-bindings --help
      ```

  1. To assign roles, run this command:

      ```bash
      {{ yc-mdb-pg }} cluster set-access-bindings <cluster_name_or_ID> \
...
- REST API {#api}
  ...
  1. Call the [Cluster.SetAccessBindings](../api-ref/Cluster/setAccessBindings.md) method, e.g., via the following {{ api-examples.rest.tool }} request:
      ...
      curl \
        ...
        --header "Authorization: Bearer $IAM_TOKEN" \
        --header "Content-Type: application/json" \
        --url 'https://{{ api-host-mdb }}/managed-postgresql/v1/clusters/<cluster_ID>:setAccessBindings' \
        --data '{
                  "accessBindings": [
                    {
                      "roleId": "<role>",
...
- CLI {#cli}
  ...
  1. View the description of the CLI command for revoking a role for a cluster:
      ...
      ```bash
      {{ yc-mdb-pg }} cluster remove-access-binding --help
      ```
  1. To revoke a role, run this command:

      ```bash
      {{ yc-mdb-pg }} cluster remove-access-binding <cluster_name_or_ID> \
        --role <role> \
        --subject <subject_type>:<subject_ID>
      ```
...
For a service account to be able to view the info of all {{ mpg-name }} clusters in the folder but modify the resources in one particular cluster alone, give it the `managed-postgresql.viewer` role for the folder and the `managed-postgresql.editor` role for that cluster:
...
- CLI {#cli}
  ...
  1. Assign the roles for the cluster:
      ...
      ```bash
      {{ yc-mdb-pg }} cluster add-access-bindings <cluster_name_or_ID> \
