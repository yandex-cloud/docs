...
- Java
  ...
  1. Build a project:
      ...
      As a result of running the command, the file `SeriesItemOps06.jar` is generated in the folder `target/release/`.
...
- Python
  ...
  1. Create the file `SeriesItemOps06.py`, for example, using the nano editor:
...
- PHP
  ...
  1. Create the file `SeriesItemOps06.php`, for example, using the nano editor:
  ...
  1. Run the program:
      ...
      Error executing "DeleteItem" on "<Document API endpoint>"; AWS HTTP error: Client error: `POST <Document API endpoint>` resulted in a `400 Bad Request` response:
