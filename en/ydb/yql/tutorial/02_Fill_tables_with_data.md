...
We assume that you already created the tables on step [{#T}](01_Create_demo_tables.md).
...
REPLACE and UPSERT are used to insert a new record or update an existing one by primary key value. INSERT is used to insert a new record. REPLACE and UPSERT are more efficient than INSERT. While executing INSERT, {{ ydb-short-name }} makes read operation first to ensure that primary key value doesn't exist, and then write operation. Contrary, while executing REPLACE or UPSERT, {{ ydb-short-name }} makes only blind write (writing data without previously reading it in the same transaction). It is recommended to use REPLACE on initial data write operations, because columns that aren't participating in the query will be filled with the default values.
