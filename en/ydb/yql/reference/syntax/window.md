...
You can set an optional *frame definition* (`frame_definition`) in one of two ways:
...
### Calculation algorithm {#partition}
...
If `PARTITION BY` isn't set, all rows in the source table get in the same partition. If `ORDER BY` is set, it determines the order of rows in a partition.
