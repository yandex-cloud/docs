...
* When you need to output the statistics on cells from a container column (for example, by using [`GROUP BY`](group_by.md)).
* When the cells in a container column store IDs from another table that you want to join using [`JOIN`](join.md).
...
`FLATTEN BY` interprets [optional data types](../types/optional.md) as lists of length 0 or 1. The table rows with `NULL` are skipped, and the column type changes to a similar non-nullable one.<br><br>`FLATTEN BY` makes only one conversion at a time, so use `FLATTEN LIST BY` or `FLATTEN OPTIONAL BY` on optional containers, for example, `Optional<List<String>>`.
