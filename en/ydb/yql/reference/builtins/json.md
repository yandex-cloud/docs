...
JsonPath considers any values received from a JSON document to be non-Boolean. For example, a query like `! $.is_valid_user` (logical negation applied to the `is_valid_user` field) is syntactically incorrect as the `is_valid_user` field isn't a Boolean value (even if it actually stores `true` or `false`). The correct way to write such a query is to explicitly use a comparison with a Boolean value, such as `$.is_valid_user == false`.
...
3. If the comparison result is true, the `FOUND flag is set.`
...
1. Setting the `ERROR` flag, the operator returns `null`
2. Setting the `FOUND` flag, the operator returns `true`
...
The `like_regex` predicate lets you check if a string matches a regular expression. The syntax of regular expressions is the same as in [HyperScan UDF](../udf/list/hyperscan.md) and [REGEXP](../syntax/expressions.md#_2).
...
1. `(1.3).ceiling()` returns `1`
2. `(1.8).ceiling()` returns `1`
3. `(1.5).ceiling()` returns `1`
4. `(1.0).ceiling()` returns `1`
...
Let the `planet` variable be qual to
...
3. If the result of JsonPath execution is one or more values, the returned value is `True.`
4. Otherwise, `False.`
...
SELECT
    -- JsonPath error, False is returned, as the default section used is FALSE ON ERROR.
...
1. If the `ON EMPTY` section isn't specified, the used section is `NULL ON EMPTY`
2. If the `ON ERROR` section isn't specified, the used section is `NULL ON ERROR`
3. If the `RETURNING` section isn't specified, then `Utf8 is used as `<type>` `
...
SELECT
    ...
    -- Hence, the value from ON ERROR is used.
    JSON_VALUE(
        $json,
        "$.friends[50].age"
        RETURNING Uint64
...
1. If the `ON EMPTY` section isn't specified, the used section is `NULL ON EMPTY`
2. If the `ON ERROR` section isn't specified, the used section is `NULL ON ERROR`
3. If the `WRAPPER` section isn't specified, the used section is `WITHOUT WRAPPER`
4. If the `WITH WRAPPER` section is specified but `CONDITIONAL` or `UNCONDITIONAL` is omitted, then `UNCONDITIONAL is used.`
