...
- Dedicated
   ...
   **Examples**
   ...
   Creating a single-node Dedicated {{ ydb-short-name }} DB with the minimum configuration, named `dedb` and accessible from the internet:
   ...
   Asynchronously creating a three-node Dedicated {{ ydb-short-name }} DB with geographic redundancy, 300 GB of storage, and computing nodes with 64 GB RAM each, named `dedb3` and accessible from the internet:
