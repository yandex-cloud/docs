...
Before you start, install the [{{ ydb-short-name }} CLI]{% if lang == "ru" %}(https://ydb.tech/ru/docs/reference/ydb-cli/install){% endif %}{% if lang == "en" %}(https://ydb.tech/en/docs/reference/ydb-cli/install){% endif %}. To connect to your {{ ydb-full-name }} database from the {{ ydb-short-name }} CLI, specify the [endpoint and path](#endpoint-and-path) and [select and set up](#auth) authentication mode.
