...
{{ ydb-short-name }} uses {% if audience != "external" %} [YQL](https://yql.yandex-team.ru/docs/ydb/) {% else %} [YQL](../yql/reference/overview.md) {% endif %} data types. Some YQL types have limited support: they are used only in calculations, but can't function as column types or parts of the primary key. All columns, including key columns, can contain a special `NULL`  value.
...
Thanks to the indexed view, `JsonDocument` lets you bypass the document model using `JsonPath` without the need to parse the full content. This improves efficiency for {% if audience != "external" %}[JSON API](https://yql.yandex-team.ru/docs/yt/builtins/json/){% else %}[JSON API](../yql/reference/builtins/json.md){% endif %} operations, reducing delays and user query costs. Execution of `JsonDocument` queries can be up to several times more efficient depending on the type of load.
...
To store numbers (JSON Number) in `JsonDocument` and perform arithmetic operations on them in the {% if audience != "external" %}[JSON API](https://yql.yandex-team.ru/docs/yt/builtins/json/){% else %}[JSON API](../yql/reference/builtins/json.md){% endif %}, the [Double](https://en.wikipedia.org/wiki/Double-precision_floating-point_format) type is used. Precision might be lost when non-standard representations of numbers are used in the source JSON document.
