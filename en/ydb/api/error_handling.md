...
* **Temporary failures** (retryable, hereinafter — R): Include a short-term loss of network connection, temporary unavailability or overload of a {{ ydb-short-name }} subsystem, or a failure of {{ ydb-short-name }} to respond to a query within the set timeout. If one of these errors occurs, a retry of the failed query is likely to be successful after a certain period of time.
...
{{ ydb-short-name }} The SDK provides a built-in mechanism for handling temporary failures. By default, the SDK uses the recommended retry policy that can be changed to meet the requirements of the client app. {{ ydb-short-name }} returns termination codes that let you determine whether a retry is appropriate and which interval to select.
...
| TRANSPORT_UNAVAILABLE | A transport error, the endpoint is unavailable, or the connection is broken and can't be reestablished | Check the endpoint or other network settings | R |
