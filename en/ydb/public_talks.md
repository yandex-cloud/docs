...
### 2021: about:cloud on Serverless {#about-cloud-20210715}

* Subject: Migrating an application from {{ PG }} to {{ ydb-full-name }} Serverless. Performance, cost, risks.
  * [About the event](https://cloud.yandex.ru/events/408#about).
  ...
  * Description: A study conducted by the {{ ydb-full-name }} team examines aspects of migrating an E-commerce application originally developed for {{ PG }} to {{ ydb-full-name }}. The primary objective of the study is to compare the performance and the cost of the {{ PG }} and {{ ydb-short-name }} solutions.
...
* Subject: Immersion in Serverless. {{ ydb-name }} is born.
...
* Subject: Cooking with serverless. Voice-based service for doctor's appointments and registration at a clinic
  ...
  * Description: What kind of a serverless stack do you need, what does a script consist of, and how can a CRM be built on {{ yandex-cloud }}. The Voximplant communication platform and {{ yandex-cloud }} have cooked up a recipe for a voice service for registering at a clinic and making doctors' appointments. You can also use it for other similar serverless tasks.

### 2021: Open systems. DBMS {#ospdatabases2021}

* Subject: Serverless alternative to conventional databases.
  * [Article](https://www.osp.ru/os/2021/01/13055826).
  * Description: Serverless computing has become the next step towards reducing the infrastructure "tax" that users of cloud services have to pay; however, there are few databases for this ecosystem out there, and the objective of {{ ydb-name }} is to make up this shortage.

### 2021: {{ ydb-short-name }} Webinar 2021-01-21 {#webinar20210121}
...
* Subject: Serverless case study: Integration of {{ yandex-cloud }} services with Voximplant voice technologies.
  ...
  * Description:
    ...
    1. CRM system built around a serverless {{ api-gw-name }}, {{ sf-full-name }}, and {{ ydb-name }} (serverless).
...
### 2020: {{ ydb-short-name }} Webinar 2020-04-23 {#webinar20200423}
...
* Topic: {{ ydb-name }}: How to make HDD perform 10K IOPS and insert 50K records into a table on a single core.
  ...
  * Description:
    1. A story about successfully using {{ ydb-name }} as a backend for distributed Jaeger tracing from [Auto.ru](https://auto.ru) and [Yandex.Realty](https://realty.yandex.ru/) representatives.
...
* Topic: {{ ydb-name }}: An effective alternative to traditional noSQL solutions.
  ...
  * Description: How and why {{ ydb-name }} was created, how it differs from other databases, and what tasks it is best suited for.
...
### 2019: {{ ydb-short-name }} Webinar {#webinar}

* Topic: {{ ydb-name }} use experience in Yandex products.
  * [Presentation](https://storage.yandexcloud.net/ydb-public-talks/2019-webinar.pptx).
...
* Topic: {{ ydb-name }}: Network Block Store.
...
## Comprehensive {{ ydb-short-name }} materials {#internals}

### 2019: HighLoad++ Moscow {#highloadmoscow2019}

* Topic: Distributed transactions in {{ ydb-short-name }}.
...
* Topic: {{ ydb-name }} Structure.
  ...
  * Description: A detailed talk about the {{ ydb-short-name }} structure and answers to questions.

### 2019: YaTalks Yekaterinburg {#yatalks}

* Subject: {{ ydb-name }}: Yandex' distributed SQL database.
  ...
  * Description: Basics of distributed query execution in {{ ydb-short-name }}:
    ...
    1. Specifics of the {{ yql-full-name }} ({{ yql-short-name }}) SQL dialect.
...
* Topic: {{ ydb-name }}: Distributed queries in the cloud.
  ...
  * Description: How {{ ydb-short-name }} runs {{ yql-short-name }} queries.
...
* Topic: {{ ydb-name }}: How we ensure fault-tolerance.
  ...
  * Description: How {{ ydb-short-name }} enables redundant data storage and distributed consensus.

{% if audience != "external" %}

### 2019: HydraConf {#hydraconf}

* Topic: {{ ydb-name }}: how we guarantee fault tolerance.
...
* Topic: Distributed transactions in {{ ydb-short-name }}.
  ...
  * Description: Mechanism to execute distributed transactions in {{ ydb-short-name }}.
{% endif %}
