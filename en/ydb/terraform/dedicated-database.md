---
title: Creating and reconfiguring a dedicated {{ ydb-short-name }} database in {{ yandex-cloud }}
description: This guide describes the properties of the `yandex_ydb_database_dedicated` resource for creating a dedicated {{ ydb-short-name }} database in {{ yandex-cloud }}.
---
...
resource "yandex_ydb_database_dedicated" "my_database_2" {
  ...
  network_id  = "enpv06v9b6og********" # ID of the network to host the new database
  subnet_ids = [
...
* `folder_id`: ID of the {{ yandex-cloud }} folder to host the new database. This is a required parameter.
* `resource_preset_id`: Name of the VM configuration preset. You can use the following presets: `medium`, `medium-m64`, `medium-m96`, `large`, `xlarge`. They differ in the processor platform, number of vCPUs, and amount of RAM. This [article](../concepts/resources.md#resource-presets) gives a detailed description of the VM configuration presets.
...
The time it takes to create a dedicated database depends on the number of allocated VM instances and storage groups. To delete a dedicated database, remove the `yandex_ydb_database_dedicated` resource from the configuration file and run this command sequence: `terraform validate`, `terraform plan`, and `terraform apply`. 