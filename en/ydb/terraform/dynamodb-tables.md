...
You can only create document tables when using a database in {{ ydb-full-name }} in [serverless mode](../concepts/serverless-and-dedicated.md#serverless).
...
* `skip_credentials_validation`: Skipping validation of credentials through the STS (Amazon Web Services Security Token Service) API. Token validation will be skipped.
...
The specified `skip_` parameters are required as they ensure compatibility of the AWS provider's API requests to {{ yandex-cloud }}.
...
* `billing_mode`: Billing model for data reads and writes. Only the `PAY_PER_REQUEST` mode is supported.
* `hash_key`: Attribute to use as the hash key. It is defined in the `attribute` section.
* `range_key`: Attribute to use as the range (sort) key. It is defined in the `attribute` section.
...
The first time you run the script, you need to run `terraform init` to initialize the AWS provider. Then you can run `terraform plan` and `terraform apply`. This will create a document table named as specified in the `name` argument.