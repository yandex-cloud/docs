# Getting authentication credentials {#get-auth-credos}

To authenticate in {{ yandex-cloud }} and manage YDB databases via Terraform, you can use a [service account](../../iam/concepts/users/service-accounts.md) [Yandex account](../../iam/concepts/index.md#passport), or [federated account](../../iam/concepts/index.md#saml-federation). For Terraform authentication and access to YDB, you will also need the Yandex Cloud CLI. If you do not have yc yet, follow the [guide](../../cli/quickstart.md#install) to install it.
...
Terraform will use the defined environment variables for authentication, so keep in mind that the `IAM token` lifetime cannot exceed 12 hours. After the token expires, Terraform will issue an authentication error. In which case you should update the environment variable: re-run the command: `export YC_TOKEN=$(yc iam create-token)`.

You can automate the process of obtaining a new token using `crontab`: `enter crontab -e` and enter `0 * * * * export YC_TOKEN=$(yc iam create-token)`. Now, `crontab` will independently update the token every hour within the current session. To update the token when opening a new session, run one of the following commands:
```bash
