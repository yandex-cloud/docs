# Rules for estimating the cost of YQL queries to {{ ydb-name }}
...
To evaluate the {{ ydb-name }} API request cost, the CPU cost and the I/O cost are calculated. A maximum from the calculated values is selected.
* CPU cost.
