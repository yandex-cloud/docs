...
* YQL is an SQL-like language that allows you to work with relational tables and is supported by the [SDK](../reference/ydb-sdk/index.md), [CLI](../quickstart/yql-api/ydb-cli.md), and the [management console {{ ydb-short-name }}](../quickstart/yql-api/ydb-ui.md).
...
* [Special APIs](request_units_special.md).
...
You can force a database backup to [{{ objstorage-full-name }}](../../storage/index.yaml). The cost of this operation depends on the amount of data copied. When calculating the cost, the actual amount is rounded up to a multiple of 1 GB.
