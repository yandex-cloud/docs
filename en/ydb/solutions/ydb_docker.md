...
{% note info "YDB recipe" %}

For testing in Arcadia, we recommend using the [YDB recipe](../getting_started/ydb_recipe.md)
...
## Using a Docker image {#docker-usage}

Container images are published by Registry at ``registry.yandex.net/yandex-docker-local-ydb:stable``. To download the most up-to-date image version, 
run the command:
...
You can then launch a new container:
...
The image contains a pre-built version of the [YDB CLI](../getting_started/ydb_cli.md), which you can use to run various commands, such as:
...
You can also use ``ya ydb`` to access data in a container, for example:
