...
* Permission to accept incoming traffic through port `2135`. Make sure that the assigned [security group](../../vpc/concepts/security-groups.md) contains the corresponding rule. If not, add it.
...
- IAM token
    ...
    To avoid entering it every time you run `{{ ydb-cli }}` commands, save the **value of your** [IAM token](../../iam/concepts/authorization/iam-token.md) to the `IAM_TOKEN` environment variable.
