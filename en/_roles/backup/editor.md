
The `backup.editor` role enables managing the connection of target resources to {{ backup-name }}, managing backup policies, taking backups, and restoring resources from the existing backups.

Users with this role can:
* View info on connected backup [providers](../../backup/concepts/index.md#providers), as well as connect providers available in {{ backup-name }}.
* View info on [backup policies](../../backup/concepts/policy.md) as well as on [resources](../../backup/concepts/index.md#protected-resources) linked to such policies.
* Create, modify, and delete backup policies, as well as link, unlink, and run them on target resources.
* View info on [access permissions](../../iam/concepts/access-control/index.md) granted for backup policies.
* View info on resources [connected](../../backup/concepts/vm-connection/index.md) to {{ backup-name }}, as well as connect and disconnect resources to and from it.
* View info on [backups](../../backup/concepts/backup.md), as well as delete them and use them to restore resources.
* View info on {{ backup-name }} [quotas](../../backup/concepts/limits.md#backup-quotas).
* View info on the relevant [cloud](../../resource-manager/concepts/resources-hierarchy.md#cloud).
* View info on the relevant [folder](../../resource-manager/concepts/resources-hierarchy.md#folder) and its statistics.

This role includes the `backup.viewer` and `backup.user` permissions.

To assign the `backup.editor` role, you need the `admin` role for the cloud or `backup.admin` for the folder.