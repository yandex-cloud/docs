You can assign the `datalens.collections.editor` role to a collection. It allows you to edit the relevant collection and all its nested collections, workbooks, and all objects within such workbooks. In the {{ datalens-name }} UI, this role is referred to as `Editor`. You may want to only assign this role through the {{ datalens-name }} UI.
