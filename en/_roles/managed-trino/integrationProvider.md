The `managed-trino.integrationProvider` role enables a {{ TR }} cluster to work with user resources required for its operation on behalf of a service account. You need to assign this role to a service account linked to a {{ TR }} cluster.
...
* View details on [{{ monitoring-name }} quotas](../../monitoring/concepts/limits.md#monitoring-quotas).
