...
The `compute.editor` role allows you to manage instances, instance groups, disks, images, GPU clusters, and other {{ compute-name }} resources, as well as manage instance backups using {{ backup-name }}.
...
This role includes the `compute.operator`, `compute.osLogin`, `compute.snapshotSchedules.editor`, `compute.disks.user`, `vpc.user`, and `backup.user` permissions.
