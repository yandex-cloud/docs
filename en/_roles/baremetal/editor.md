...
The `baremetal.editor` role enables managing {{ baremetal-name }} servers, private subnets, virtual routing and forwarding (VRF) segments, and OS server images, as well as managing server backups using {{ backup-name }}.
...
This role includes the `baremetal.operator` and `backup.user` permissions.