The `security-deck.admin` role enables managing subscriptions to events of access to organization resources by {{ yandex-cloud }} employees, managing workspaces, alerts, and alert sinks, as well as {{ dspm-name }}, {{ cspm-name }}, {{ kspm-name }}, {{ vuln-man-short-name }}, and {{ td-name }} resources. This role enables viewing masked and unprocessed data in scan results.
...
* View info on [alerts](../../security-deck/concepts/workspace.md#connectors), as well as create, modify, and delete them.
...
* View info on access permissions granted for {{ td-name }} and modify such permissions.
