...
The `managed-airflow.integrationProvider` role allows the {{ AF }} cluster to work with user resources required for its operation on behalf of the service account. You can assign this role to a service account linked to the {{ AF }} cluster.
...
* View bucket, object, and object version [labels](../../storage/concepts/tags.md), as well as {{ objstorage-name }} statistics.
* View info on [{{ lockbox-name }} secrets](../../lockbox/concepts/secret.md#secret) and granted access permissions for them.
* View details on [{{ objstorage-name }}](../../storage/concepts/limits.md#storage-quotas), [{{ monitoring-name }}](../../monitoring/concepts/limits.md#monitoring-quotas), and [{{ lockbox-name }}](../../lockbox/concepts/limits.md#quotas) quotas.
...
The role does not provide access to {{ lockbox-name }} secret contents. To grant the {{ AF }} cluster access to {{ lockbox-name }} secret contents, additionally assign the `lockbox.payloadViewer` [role ](../../lockbox/security/index.md#lockbox-payloadViewer)to the [service account](../../iam/concepts/users/service-accounts.md) either for the relevant folder or for specific secrets.