...
Logging is used for app diagnostics. However, just having logs is not sufficient to perform analysis, they need to be stored and handled in a convenient way. To accomplish this, logs are sent to storage systems: [Hadoop](https://cloud.yandex.com/services/data-proc), [{{ CH }}](https://cloud.yandex.com/services/managed-clickhouse), [{{ ES }}](https://cloud.yandex.com/services/managed-elasticsearch), or to specialized cloud systems, such as [{{ cloud-logging-name }}](../../logging/).
