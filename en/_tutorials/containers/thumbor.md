...
[Thumbor](https://thumbor.readthedocs.io/en/latest/) is an [open-source](https://github.com/thumbor/thumbor) project for on-demand image processing. Thumbor provides basic settings for editing images. For example, you can use it to resize the original image, enhance the contrast, or remove the red-eye effect.

Thumbor is a convenient tool for website image processing, such as creating thumbnails for video previews. Thumbor supports image caching. This helps reduce website maintenance effort.

The example below demonstrates images posted to a website and edited with Thumbor, including resizing and watermarking. To upload images faster, you need to configure a CDN for the website using [{{ cdn-full-name }}](../../cdn/concepts/index.md).

To edit images using Thumbor and set up a CDN:
...
* {{ cdn-name }}: Outgoing traffic volume (see [{{ cdn-name }} pricing](../../cdn/pricing.md)).
...
1. Upload the images to the bucket:
   ...
   - {{ TF }} {#tf}
      ...
      You can only upload objects to a bucket after you create it, so use a separate configuration file to upload images.
      ...
      1. In the `images-for-thumbor.tf` file, specify relative or absolute paths to the images. For example, if your images are stored in the same directory as configuration files, specify:
...
1. Get the Thumbor's external IP address:
...
1. Create an origin group in {{ cdn-name }}:
   ...
   The response returns the origin group ID in the `origin_group_id` field. You will need this ID in the next step.

1. Create a CDN resource and associate it with the origin group:
   ...
   It takes 15 to 30 minutes to set up a CDN resource.

1. In the [management console]({{ link-console-main }}), open the CDN resource page and get the CDN provider's domain name, such as `{{ cname-example-yc }}`.

1. Set up a CNAME record for your domain:

   1. Navigate to your domain’s DNS settings on your DNS hosting provider’s website.
   1. Set up a CNAME record to point to the address you copied earlier in the `.yccdn.cloud.yandex.net` domain. For example, if the website domain name is `{{ domain-name-example }}`, create a CNAME record or replace an existing one for `cdn`:
...
Open your website at these URLs:
...
You will see the prepared images of different sizes. Each image contains a [Creative Commons](https://en.wikipedia.org/wiki/Creative_Commons) watermark.
...
1. Delete the other resources depending on how you created them:
    ...
    - Manually {#manual}
        ...
        1. [Public static IP address](../../vpc/operations/address-delete.md) if you reserved one for the cluster.
