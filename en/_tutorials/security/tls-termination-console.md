...
- Management console {#console}
  ...
  1. Create an HTTPS request listener:
      ...
      1. Under **{{ ui-key.yacloud.alb.section_default-sni-match }}**, select `mysite-cert` for certificate and `mysite-router` for HTTP router.
      1. Add an SNI match for `my-site.com`:
        ...
        1. Select `mysite-cert` for certificate and `mysite-router` for HTTP router.
  1. Click **{{ ui-key.yacloud.common.create }}**.
...
- Management console {#console}
  ...
  1. Create or edit the [A record](../../dns/concepts/resource-record.md#a) for `my-site.com` so that it points to the copied IP address:
      ...
      1. If you do not have a public [DNS zone](../../dns/concepts/dns-zone.md), create one:
          ...
          1. In the **{{ ui-key.yacloud.dns.label_zone }}** field, enter the website's domain name with a trailing dot: `my-site.com.`.
