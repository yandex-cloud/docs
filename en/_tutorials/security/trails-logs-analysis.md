# Uploading audit logs to {{ mch-name }} and visualizing data in {{ datalens-name }}
...
- CLI {#cli}
   ...
   1. [Create a service account](../../iam/operations/sa/create.md) named `sa-trail-logs`:
...
- Management console {#console}
   ...
   1. Specify the settings for a {{ CH }} cluster:
      ...
      1. Under **{{ ui-key.yacloud.mdb.forms.section_settings }}**, enter `trail_data` for the DB name, `user` for the username, and the password. Remember the database name.
...
After activating the transfer, go to {{ mch-name }} and check that the `trail_data` database now contains the `trail_logs_stream` table with {{ at-full-name }} events.
