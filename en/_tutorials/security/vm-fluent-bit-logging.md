...
1. `systemd` will call `logtest.sh` to run the service; therefore, make the file executable:
...
1. Recheck `logtest.service` status; the service must now be active:
...
1. Add the `yc-logging.so` library path to the `/etc/fluent-bit/plugins.conf` plugin settings file:
