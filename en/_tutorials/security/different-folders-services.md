...
* **Team folders**: Folders containing team targets, e.g., virtual machines (VMs), databases, [{{ network-load-balancer-name }}](../../network-load-balancer/)-enabled L3-L4 load balancers (NLBs), and more.
...
* If the target’s internal IP address changes, manually update the L7 load balancer's target group configuration.
...
The system will deploy your application backends on the [target group](../../application-load-balancer/concepts/target-group.md) VM. The load balancer will distribute requests to your application’s backend endpoints via the target group.
...
- Management console {#console}
  ...
  1. Configure the other targets. To do this, click **{{ ui-key.yacloud.alb.button_add-target }}** and specify addresses and subnets.
...
[Backend groups](../../application-load-balancer/concepts/backend-group.md) contain settings for traffic balancing and target health checks. The wizard will automatically create one backend and one health check group. It will also use the group you created earlier as the target group.
...
- Management console {#console}
  ...
  1. For **{{ ui-key.yacloud.alb.label_security-groups }}**, select **{{ ui-key.yacloud.component.security-group-field.label_sg-from-list }}** and then the security group you created earlier.
  ...
  1. Configure the listener:
     ...
     * Under**{{ ui-key.yacloud.alb.section_external-address-specs }}**, enable a public IP address and specify the following:
        ...
        * **Type**: Set it to `List` and select the IP address you reserved previously.
