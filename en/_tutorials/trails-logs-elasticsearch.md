...
1. Download a directory with [{{ TF }} modules](https://github.com/yandex-cloud/yc-solution-library-for-security/tree/master/auditlogs/export-auditlogs-to-ELK_main/terraform/modules):
   ...
   * `yc-elastic-trail`:
      ...
      * Creates an {% if lang == "ru" and audience != "internal" %}[SSH key](../glossary/ssh-keygen.md){% else %}SSH key{% endif %} pair and saves the private part to the disk and the public part to the VM.
