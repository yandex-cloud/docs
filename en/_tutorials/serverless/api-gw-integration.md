...
The integration will be applied to implement the [CRUD]{% if lang == "ru" %}(https://ru.wikipedia.org/wiki/CRUD){% endif %}{% if lang == "en" %}(https://en.wikipedia.org/wiki/Create,_read,_update_and_delete){% endif %} API for a movie database deployed in {{ ydb-name }}.
