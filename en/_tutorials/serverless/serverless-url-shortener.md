...
The service accepts user requests via a public [API gateway](../../api-gateway/concepts/index.md). The [hosting service](../../storage/concepts/hosting.md) sends the user an HTML page with a field for entering the URL. A [function](../../functions/concepts/function.md) sends the entered URL for storage in a [serverless database](../../ydb/concepts/serverless_and_dedicated.md#ydb-serverless-operation-mode), shortens it, and returns it to the user. When the user enters the shortened URL, the function finds the full URL in the database and redirects the user's request to it.
