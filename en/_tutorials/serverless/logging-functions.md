...
1. A trail uploads [audit logs to a log group](../../audit-trails/operations/create-trail.md).
1. A [log group](../../logging/concepts/log-group.md) acts as an interface between a trail and a {{ sf-name }} trigger.
1. A [{{ cloud-logging-full-name }}](../../functions/concepts/trigger/cloud-logging-trigger.md) trigger calls the function when adding an entry to a log group.
1. A [{{ sf-name }}](../../functions/concepts/function.md) function to execute the response logic.
