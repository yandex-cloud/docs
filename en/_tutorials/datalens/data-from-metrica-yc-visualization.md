...
1. Specify the settings for a {{ CH }} cluster:
   ...
   1. Under **Resources**, select the `Intel Cascade Lake` platform, the `burstable` type, and the `b2.medium` host type.
   ...
   1. Under **Service settings**, enable the following options:
...
1. Open the [({{ link-datasphere-main }}) homepage]{{ ml-platform-name }}.
1. In the window that opens, click **Create** → **Create project**.
1. Give your project a name. Naming conventions:

   {% include [name-format](../../_includes/name-format.md) %}

1. Specify a [community](../../datasphere/concepts/community.md) for the project and click **Сreate**.
1. Click **Open project in {{ jlab }}Lab**.
...
1. Create an application:
   ...
   1. Go to **Which platform is the app required for?**. → **Web services**. In the **Callback URL** field, insert `https://oauth.yandex.com/verification_code`.
   1. Go to **What data do you need?** → **Yandex Metrica (metrika)**. Enable **Access to statistics and ability to view all counter settings (metrika:read)**.
   1. Click **Create app**.
...
1. Select **Hosts** from the ![hosts](../../_assets/datalens/hosts.svg) list on the left.
...
1. In the list on the left, select ![datalens](../../_assets/datalens/datalens.svg) **{{ datalens-short-name }}**.
...
1. Fill in the connection settings:
   ...
   1. Enter [password](#ch-connection) and click **Check connection**.
...
1. To calculate the number of hits, create a calculated field: in **Field**, enter `Hits`. In the workspace, enter `1` and click **Create**.
...
1. In the top right-hand corner, click ![save-button](../../_assets/datalens/save-button.svg) → **Save as**.
...
1. In the panel on the left, select ![dashboards](../../_assets/datalens/dashboard.svg) **Dashboards** and click **Create issue**.
1. Enter `ch_metrica_data` as the name of the dashboard and click **Create**.
1. Add the first chart to the dashboard. To do this, in the top right corner, click **Add** → **Chart**:
...
1. Add filtering to select a specific browser. To do this, in the top right corner, click **Edit** → **Add** → **Selector**.
...
1. In the **Default value** field, select browsers:
   ...
   * yandex_browser
   * yandexsearch
1. In the **Name** field, enter a name for the selector and enable **Show**.
1. Click **Add**.
...
1. Open the [{{ datalens-short-name }}](https://datalens.yandex.ru/) homepage (or click ![datasets](../../_assets/datalens/datasets.svg) **DataLens** in the left panel) and click **Create dataset**.
...
1. Format the numbers in the values of the **week_num** field by removing the decimal places. To do this, in the **Columns** section, in the **week_num** field, click the ![image](../../_assets/datalens/mesh.svg) icon. In the window that opens, set the following configuration:
...
1. Select the format for **Visits from the first week**. To do this, click the ![image](../../_assets/datalens/mesh.svg) icon under **Measures** in the **Visits from the first week** field. In the window that opens, set the following configuration:
   ...
   1. Click **Apply**.
1. Edit the threshold values for the measure's colors. Under **Colors**, click the ![gear](../../_assets/datalens/gear.svg) icon. In the window that opens, enable **Set threshold values**, then specify the threshold values of `0.01`, `0.025`, and `0.1` and click **Apply**.
1. Click **Save**.

### 6.5. {{ datalens-short-name }}. Add charts to a new dashboard tab {#adding-charts-to-dashboard-tab}

1. In the panel on the left, click ![dashboards](../../_assets/datalens/dashboard.svg) **Dashboards** and open the dashboard.
1. Click **Edit** → **Tabs**.
...
1. In the top right corner, click ![image](../../_assets/datalens/save-button.svg) **Save**.
