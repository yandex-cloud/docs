...
Visualizing data from [{{ tracker-full-name }}]({{ link-tracker-cloudless }}) with {{ datalens-full-name }} allows you to build more advanced analytics than with the {{ tracker-short-name }} tools.

To visualize data from {{ tracker-short-name }} in {{ datalens-short-name }}:
...
1. [Create a database for storing your {{ tracker-short-name }} data](#database-create).
1. [Create an OAuth token to access {{ tracker-short-name }}](#oauth-token).
1. [Create a function in {{ sf-name }} for importing data](#function-import).
...
* Using the function in {{ sf-name }} (see [{{ sf-name }} pricing](../../functions/pricing.md)).
...
## Create an OAuth token to access {{ tracker-short-name }} {#oauth-token}
...
1. Log in under the {{ tracker-short-name }} account you will use for data visualization.
...
1. Create a [trigger](../../functions/concepts/trigger/index.md) to regularly export new data to the database:
    ...
    1. Enter the account name. By default, the account has the `{{ roles-functions-invoker }}` role to work with the trigger.
...
1. On the top panel, select the [visualization type](../../datalens/visualization-ref/index.md). By default, the **Column chart** type is selected.
