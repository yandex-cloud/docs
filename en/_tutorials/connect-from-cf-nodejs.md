...
You'll create a [function](../functions/concepts/function.md) with an application in [Node.js](https://nodejs.org/en/), which runs a simple query against a {{ ydb-short-name }} database. You deploy the application using Bash scripts and use the `tcs` command to compile.
