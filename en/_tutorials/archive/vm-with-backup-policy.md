...
The minimum [folder](../../resource-manager/concepts/resources-hierarchy.md#folder) role required for [activating the service](../../backup/concepts/index.md#providers) is `backup.editor` (see [its description](../../backup/security/index.md#backup-editor) for details).
...
- Management console {#console}
   ...
   1. Enter the [service account](../../iam/concepts/users/service-accounts.md) name: `backup-sa`.
...
- API {#api}
   ...
   1. Create a `cloud-network` using the [create](../../vpc/api-ref/Network/create.md) REST API method for the [Network](../../vpc/api-ref/Network/index.md) resource or the [NetworkService/Create](../../vpc/api-ref/grpc/network_service.md#Create) gRPC API call.
...
- Management console {#console}
   ...
   1. Add the following outgoing traffic rules in succession:
...
- CLI {#cli}
   ...
   1. Describe the configuration of the backup policy being created in the `backup-policy-schema.json` file.
...
- Management console {#console}
   ...
   1. Enter the VM name: `backup-instance`.
   1. Select the `{{ region-id }}-d` [availability zone](../../overview/concepts/geo-scope.md).
   1. In the **{{ ui-key.yacloud.compute.instances.create.section_image }}** section on the **{{ ui-key.yacloud.compute.instances.create.image_value_os-products }}** tab, select `Ubuntu 20.04`.
   ...
   1. Under **{{ ui-key.yacloud.compute.instances.create.section_access }}**:
      ...
      1. In the **{{ ui-key.yacloud.compute.instances.create.field_user }}** field, enter the username: `yc-user`.
   ...
   1. Under **{{ ui-key.yacloud.common.metadata }}**, add a field with the `cloudbackup` key and the `{"initialPolicies": ["<daily_backup_policy_ID>"]}` value.
...
- {{ yandex-cloud }} CLI {#cli}
   ...
   1. Save the `daily-backup` policy ID in the `cloudbackup.json` file:

      ```json
      {"initialPolicies": ["<daily-backup_policy_ID>"]}
      ```
...
- API {#api}
   ...
   * In the `metadata` field, the `user-data` object containing custom metadata configuration with a script to install a backup agent.
...
Policy is linked asynchronously after you create and initialize a VM, as well as install and configure a backup agent. This may take up to 10-15 minutes. As a result, the virtual machine will appear in the list of {{ backup-name }} VMs and the list of VMs linked to the `daily-backup` policy.
