...
1. Create a [security group](../../../vpc/concepts/security-groups.md) that allows the traffic required for your Nextcloud infrastructure:
    ...
    - Management console {#console}
      ...
      1. Under **{{ ui-key.yacloud.vpc.network.security-groups.label_section-rules }}**, [create](../../../vpc/operations/security-group-add-rule.md) the following traffic management rules:
          ...
          | Ingress | `healthchecks`            | `All`   | `{{ ui-key.yacloud.vpc.network.security-groups.forms.value_any }}`  | `{{ ui-key.yacloud.vpc.network.security-groups.forms.value_sg-rule-sg-type-balancer }}` | `–` |
...
- Management console {#console}
  ...
  1. In the [management console]({{ link-console-main }}), select the folder where you are deploying your infrastructure.
  1. Navigate to **{{ ui-key.yacloud.iam.folder.dashboard.label_managed-mysql }}** and click **{{ ui-key.yacloud.mdb.clusters.button_create }}**.
...
- Management console {#console}
  ...
  1. In the [management console]({{ link-console-main }}), select the folder where you are deploying your infrastructure.
  1. Navigate to **{{ ui-key.yacloud.iam.folder.dashboard.label_managed-mysql }}** and select the `nextcloud-db-cluster` cluster.
...
1. For the certificate to be issued successfully, pass domain ownership verification:
    ...
    - Management console {#console}
      ...
      1. Add a [CNAME record](../../../dns/concepts/resource-record.md#cname) required for domain ownership verification to your domain’s public DNS zone. This step's further actions will depend on whether your domain is managed by {{ dns-full-name }} or a third-party DNS provider:
...
- Management console {#console}
  ...
  1. In the **{{ ui-key.yacloud.compute.groups.create.field_service-account }}** field, select `nextcloud-sa`.
