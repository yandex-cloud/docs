...
1. [Create the required resources{{ iot-full-name }}](#resources-step).
...
1. [Create a function for processing data in {{ sf-full-name }}](#processing-function-step).
    ...
    1. [Create a trigger to invoke the data processing function](#processing-function-trigger).
...
1. [Configure monitoring in {{ datalens-full-name }}](#configure-datalens).
    ...
    1. [Create a chart for temperature and mains voltage](#create-chart).
...
If you do not have the command line interface {{ yandex-cloud }} yet, [install and initialize it](../../cli/quickstart.md#install).
...
1. Save the password locally or remember it. The service does not show passwords after the registry is created.
...
1. Click **Add device**.
1. In the **Name** field, enter the device name, e.g., `my-device-1`.
...
The emulator will send data from the devices to the {{ mpg-full-name }} cluster.
...
1. Set **Environment variables** for each sensor in the server room:
   ...
   1. Fill in the **Key** and **Value** fields for the environment variables:
      ...
      | `REGISTRY_ID` | ID of the registry you created. | For details, see the service <br>management console **{{ iot-short-name }}** |
...
1. Under **Basic parameters**, give your cluster a name in the **Cluster name** field, e.g., `my-pg-database`.
...
1. Select the VM type: on the **burstable** tab, select **b2.medium**.
...
1. Do not change the values in the **Locale for sorting** and **Charset locale** fields. The default value is `C`.
1. Under **Network**, select `default` in the list.
1. Under **Hosts**, set up a connection to your database via a public IP address, which is required to access the database from {{ sf-full-name }}.
    1. On the right side of the availability zone line, click ![image](../../_assets/pencil.svg).
    1. In the window that opens, select an **Availability zone**, e.g., `{{ region-id }}-a`.
    1. Select a **Subnet**, e.g., `default-{{ region-id }}-a`.
    1. Enable **Public access**.
    1. Click **Save**.
1. Enable **DataLens access**.
1. Click **Create cluster**.

It may take a few minutes to create a cluster. As a result, a window with cluster details will open.
...
1. In the left part of the window, select **Functions**.
1. Click **Create function**.
1. In the **Name** field, enter the name of the function, e.g., `my-db-function`.
1. (optional) In the **Description** field, add additional information about the function.
1. Click **Create**.
...
1. Create a service account to be used for the function to process data from the device:
    1. Click **Create an account**.
    1. In the **Create service account** window that opens, enter the account name in the **Name** field, e.g., `my-db-function-service-account`.
...
1. Configure **Environment variables**. You can view database connection parameters in {{ mpg-full-name }}.
    ...
    1. Fill in the **Key** and **Value** fields for the environment variables:
        ...
        | `DB_HOSTNAME` | Hostname in {{ mpg-full-name }}. | For details, see the<br>management console for {{ mpg-full-name }} |
...
The trigger will call the function when messages appear in the [device topic](../../iot-core/concepts/topic/devices-topic.md).
...
1. Select the **PostgreSQL** connector.
   ...
   When connecting to an external data source (which is not a {{ yandex-cloud }} resource), grant access to the source [for the IP address ranges{{ datalens-name }}](../../datalens/concepts/connection.md#changing-connection-ranges).
...
1. Click **Save**.
1. In the window that opens, set the chart name or use the generated `My-pg-dataset — Line chart` name and click **Save**.
...
1. From the **Dimensions** section in the left part of the window, drag the measurements to the **Map** section:
   ...
   * `item1_fullness`, `item2_fullness`, `item3_fullness`, `item4_fullness`, `cash_drawer`, and `coin_drawer`, to the **Tooltips section**.

      In the right part of the window, you will see a scalable map with vending machines marked with points on the map and tooltips marked with legend lines.
1. Click **Save**.
1. In the window that opens, set the chart name or use the generated `My-pg-dataset — Map` name and click **Save**.
...
1. Add the following charts to the dashboard: `My-pg-dataset — Map` and `My-pg-dataset — Line chart`, which you created at the previous steps:
...
Learn more [about dashboards {{ datalens-full-name }}](../../datalens/concepts/dashboard.md).
