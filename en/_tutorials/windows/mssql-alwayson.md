...
1. Connect to the VM `jump-server-vm` [using RDP](../../compute/operations/vm-connect/rdp.md). Enter `Administrator` as the username and then your password.
...
1. Run the RDP client and connect to the `ya-mssql1`  VM with the `Administrator` account  and your password. Use the VM's public IP address to connect.
...
### Add the bastion host to the domain {#jump-server-domain}
