...
1. To have a certificate issued, pass domain ownership verification:
    ...
    - {{ yandex-cloud }} CLI {#cli}
        ...
        1. Create a CNAME [resource record](../../dns/concepts/resource-record.md) to pass domain ownership verification:
            ...
            Where `<dns_challenge_value>` is the value you saved in the previous step required for domain ownership verification via a CNAME record.
    ...
    If your domain is managed from outside {{ yandex-cloud }}, create a resource record for domain ownership verification using tools provided by your domain administrator. For more information, see [{#T}](../../certificate-manager/concepts/challenges.md).
...
- Management console {#console}
  ...
  1. In the [management console]({{ link-console-main }}), select `example-folder`.
  1. [Navigate]({{ link-console-main }}/link/dns) to **{{ ui-key.yacloud.iam.folder.dashboard.label_dns }}** and select the DNS zone you created [earlier](#configure-dns).
