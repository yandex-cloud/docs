...
To run this scenario, make sure you have a corporate DNS server deployed that is available for the VMs in the cloud. You can also {% if product == "cloud-il" %}[deploy Active Directory](../../tutorials/windows/active-directory.md){% else %}deploy Active Directory{% endif %} to create an infrastructure with a DNS server:
...
To fully complete this tutorial, {% if product == "cloud-il" %}[deploy Active Directory](../../tutorials/windows/active-directory.md){% else %}deploy Active Directory{% endif %}. If you use your own DNS servers that are available for cloud VMs, specify your own DNS suffix and IP address values in the scenario.
...
* A fee for VM disk storage (see [pricing {{ compute-full-name }}](../../compute/pricing.md)).
