...
| `s3-nlb` | {{ objstorage-name }} internal NLB accepting TCP traffic with destination port 443 and distributing it across VM instances in a target group |
...
Only the cloud resources that use this solution can access the registry. The [registry access policy](../../container-registry/operations/registry/registry-access) only allows access from NAT instance public IP addresses, so you will not be able to access it from any other IP address. If you need, you can remove this limitation in {{ TF }} settings.
