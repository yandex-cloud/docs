With {{ data-transfer-name }}, you can move data from a {% if audience != "internal" and product == "yandex-cloud" %}[{{ yds-name }} data stream](../../data-streams/concepts/glossary.md#stream-concepts){% else %}{{ yds-name }} data stream{% endif %} to an {{ objstorage-name }} bucket.
...
* Using {{ TF }}
   ...
   1. In the `data-transfer-yds-obj.tf` file, specify:
      ...
      * `folder_id`: {% if audience != "internal" %}[ID of the folder](../../resource-manager/operations/folder/get-id.md){% else %}ID of the folder{% endif %} to create resources in.
