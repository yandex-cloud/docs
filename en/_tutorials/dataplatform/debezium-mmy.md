...
1. {% if audience != "internal" %}[Connect to a virtual machine over SSH](../../compute/operations/vm-connect/ssh.md#vm-connect){% else %}Connect to a virtual machine over SSH{% endif %} and perform preliminary setup:
...
1. {% if audience != "internal" %}Delete the [virtual machine](../../compute/operations/vm-control/vm-delete.md).{% else %}Delete the virtual machine.{% endif %}

   If you reserved a public static IP address for the virtual machine, release and {% if audience != "internal" %}[delete it](../../vpc/operations/address-delete.md){% else %}delete it{% endif %}.
