...
1. A fee for a continuously running VM (see [{{ compute-full-name }} pricing](../../compute/pricing.md)).
1. A fee for using a dynamic or static external IP address (see [{{ vpc-full-name }} pricing](../../vpc/pricing.md)).
...
1. Connect to the online store via the [link](http://ya-sample-store.local).
...
1. Create an order in the online store via the [link](http://ya-sample-store.local).
