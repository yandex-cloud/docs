...
1. Set up your data delivery infrastructure:
    ...
    - {{ TF }} {#tf}
        ...
        1. Download the [data-transfer-mkf-mmy.tf](https://github.com/yandex-cloud-examples/yc-data-transfer-from-kafka-to-mysql/blob/main/data-transfer-mkf-mmy.tf) configuration file to the same working directory.
            ...
            * [Security group](../../vpc/concepts/security-groups.md) and the rules allowing connections to the {{ mkf-name }} and {{ mmy-name }} clusters.
...
Check that data from the {{ mkf-name }} source cluster’s topic is transferred to the {{ mmy-name }} cluster:
...
1. Check that the {{ mmy-name }} cluster's `sensors` table contains the data that was sent:
    ...
    1. [Connect to the {{ mmy-name }} cluster](../../managed-mysql/operations/connect/index.md).
