...
- Using {{ TF }}
   ...
   1. Specify in `clickhouse-cluster-and-vm-for-rabbitmq.tf`:
      ...
      * The ID of the public Ubuntu {% if audience != "internal" %}[image](../../compute/operations/images-with-pre-installed-software/get-list.md){% else %}image{% endif %} without {% if lang == "ru" and audience != "internal" %}[GPU](../../glossary/gpu.md){% else %}GPU{% endif %} for the VM. For example, `fd879gb88170to70d38a` for Ubuntu 20.04 LTS.
...
1. {% if audience != "internal" %}[Connect](../../compute/operations/vm-connect/ssh.md) to a virtual machine over {% if lang == "ru" %}[SSH](../../glossary/ssh-keygen.md){% else %}SSH{% endif %}{% else %}Connect to a virtual machine over SSH{% endif %}.
...
1. Install the `clickhouse-client` utility to connect to the database in the {{ mch-name }} cluster.
   ...
   1. Connect the {% if audience != "internal" %}[DEB repository]({{ ch.docs }}/getting-started/install/#install-from-deb-packages){% else %}DEB repository{% endif %} {{ CH }}:
