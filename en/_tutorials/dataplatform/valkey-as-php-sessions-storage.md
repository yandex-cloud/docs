...
- Manually {#manual}
    ...
    1. [Create a {{ mrd-name }} cluster](../../managed-valkey/operations/cluster-create.md) with any suitable configuration. When creating a {{ mrd-name }} cluster, specify the same network and security groups as those of the VM hosting the web server.
    ...
    * `/etc/php/7.2/fpm/` for NGINX
    ...
    {% list tabs group=cluster %}
      ...
      - Sharded cluster {#sharded}
        ...
        Here, `<FQDN1>`, `<FQDN2>`, and `<FQDN3>` are fully qualified domain names of [cluster master hosts](../../managed-valkey/operations/hosts.md#list). For example, for a cluster with three shards and `password` for password, the `session.save_path` parameter value will look like this:
