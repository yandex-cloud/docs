...
* Using {{ TF }}
   ...
   1. Download the [data-transfer-mpg-ydb.tf](https://github.com/yandex-cloud/examples/tree/master/tutorials/terraform/data-transfer/data-transfer-mpg-ydb.tf) configuration file to the same working directory.
      ...
      * {% if audience != "internal" %}[Security groups](../../vpc/concepts/security-groups.md){% else %}Security groups{% endif %} and the rule required to connect to a {{ mpg-name }} cluster.
...
1. Make sure that the data from the source {{ mpg-name }} cluster has been moved to the {{ ydb-name }} database:
   ...
   * {{ ydb-short-name }} CLI
      ...
      1. {% if audience != "internal" %}[Connect to the {{ ydb-name }} database](../../ydb/operations/connection.md){% else %}Connect to the {{ ydb-name }} database{% endif %}.
