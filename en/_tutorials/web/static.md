...
Delegation is the transfer of authority from the registrar's servers to yours. For a domain, [NS](../../dns/concepts/resource-record.md#ns) ({% if product == "yandex-cloud" %}`ns1.yandexcloud.net` and `ns2.yandexcloud.net`{% endif %}{% if product == "cloud-il" %}`ns1.cloudil.com` and `ns2.cloudil.com`{% endif %}) resource records are created.
...
You can verify domain delegation using the [Whois]{% if lang == "ru" %}(https://www.reg.ru/whois/check_site){% endif %}{% if lang == "en" %}(https://www.reg.com/whois/check_site){% endif %} service or the `dig` utiltiy:
