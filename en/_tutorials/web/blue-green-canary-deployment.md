...
- API
   ...
   1. Create the `canary-network` network using the gRPC API {% if audience != "internal" %}[NetworkService/Create](../../vpc/api-ref/grpc/network_service.md#Create){% else %}NetworkService/Create{% endif %} or REST API {% if audience != "internal" %}[create](../../vpc/api-ref/Network/create.md){% else %}create{% endif %} method.
   1. Create the `canary-subnet-{{ region-id }}-a`, `canary-subnet-{{ region-id }}-b` and `canary-subnet-{{ region-id }}-c` in the three availability zones by calling the gRPC API {% if audience != "internal" %}[SubnetService/Create](../../vpc/api-ref/grpc/subnet_service.md#Create){% else %}SubnetService/Create{% endif %} or the REST API {% if audience != "internal" %}[create](../../vpc/api-ref/Subnet/create.md){% else %}create{% endif %} method.
...
1. Create or edit CNAME records for `cdn.yandexcloud.example` and `cdn-staging.yandexcloud.example` to link them to the copied domain name:
   ...
   - API
      ...
      1. Create a DNS zone named `canary-dns-zone` using the gRPC API {% if audience != "internal" %}[DnsZoneService/Create](../../dns/api-ref/grpc/dns_zone_service.md#Create){% else %}DnsZoneService/Create{% endif %} call or the {% if audience != "internal" %}[create](../../dns/api-ref/DnsZone/create.md){% else %}create{% endif %} REST API method.
      1. Add the `cdn` and `cdn-staging` CNAME records to the zone, copying the `cl-....edgecdn.ru` value with thegRPC API {% if audience != "internal" %}[DnsZoneService/UpdateRecordSets](../../dns/api-ref/grpc/dns_zone_service.md#UpdateRecordSets){% else %}DnsZoneService/UpdateRecordSets{% endif %} or the REST API {% if audience != "internal" %}[updateRecordSets](../../dns/api-ref/DnsZone/updateRecordSets.md){% else %}updateRecordSets{% endif %}method.
...
1. Delete `index.html` from the CDN resource cache:
   ...
   - API
      ...
      1. Get the ID of the CDN resource that you created using the gRPC API {% if audience != "internal" %}[ResourceService/List](../../cdn/api-ref/grpc/resource_service.md#List){% else %}ResourceService/List{% endif %} call or the REST API {% if audience != "internal" %}[list](../../cdn/api-ref/Resource/list.md){% else %}list{% endif %} method.
      1. Delete the `index.html` file from the cache using the gRPC API {% if audience != "internal" %}[CacheService/Purge](../../cdn/api-ref/grpc/cache_service#Purge){% else %}CacheService/Purge{% endif %} call or the {% if audience != "internal" %}[purge](../../cdn/api-ref/Cache/purge.md){% else %}purge{% endif %} method.
...
1. Disable caching of the CDN resource and delete the `index.html` file from the cache:
   ...
   - API
      ...
      1. Get the ID of the CDN resource that you created using the gRPC API {% if audience != "internal" %}[ResourceService/List](../../cdn/api-ref/grpc/resource_service.md#List){% else %}ResourceService/List{% endif %} call or the REST API {% if audience != "internal" %}[list](../../cdn/api-ref/Resource/list.md){% else %}list{% endif %} method.
      1. Disable caching using the gRPC API {% if audience != "internal" %}[ResourceService/Update](../../cdn/api-ref/grpc/resource_service.md#Update){% else %}ResourceService/Update{% endif %} call or the REST API {% if audience != "internal" %}[list](../../cdn/api-ref/Resource/update.md){% else %}list{% endif %} method.
...
1. Configure the `canary-bg-production` so that 20% of the traffic coming to the `cdn.yandexcloud.example` domain name is processed by the `canary-backend-green` backend that runs version 2:
   ...
   - API
      ...
      Use the gRPC API {% if audience != "internal" %}[BackendGroupService/UpdateBackend](../../application-load-balancer/api-ref/grpc/backend_group_service.md#UpdateBackend){% else %}BackendGroupService/UpdateBackend{% endif %} call of the REST API {% if audience != "internal" %}[updateBackend](../../application-load-balancer/api-ref/BackendGroup/updateBackend.md){% else %}updateBackend{% endif %} method.
...
1. Re-enable caching:
   ...
   - API
      ...
      1. Get the ID of the CDN resource that you created using the gRPC API {% if audience != "internal" %}[ResourceService/List](../../cdn/api-ref/grpc/resource_service.md#List){% else %}ResourceService/List{% endif %} call or the REST API {% if audience != "internal" %}[list](../../cdn/api-ref/Resource/list.md){% else %}list{% endif %} method.
      1. Enable caching using the gRPC API {% if audience != "internal" %}[ResourceService/Update](../../cdn/api-ref/grpc/resource_service.md#Update){% else %}ResourceService/Update{% endif %} call or the REST API {% if audience != "internal" %}[list](../../cdn/api-ref/Resource/update.md){% else %}list{% endif %} method.
...
1. If you set up CNAME records in {{ dns-name }}, {% if audience != "internal" %}[delete](../../dns/operations/zone-delete.md){% else %}delete{% endif %} the `canary-dns-zone` DNS zone.
...
1. {% if audience != "internal" %}[Delete](../../application-load-balancer/operations/application-load-balancer-delete.md){% else %}Delete{% endif %} the `canary-balancer` L7 load balancer.
...
1. {% if audience != "internal" %}[Delete](../../vpc/operations/subnet-delete.md){% else %}Delete{% endif %} the `canary-subnet-{{ region-id }}-a`, `canary-subnet-{{ region-id }}-b`, and `canary-subnet-{{ region-id }}-c` subnets.
1. {% if audience != "internal" %}[Delete](../../vpc/operations/network-delete.md){% else %}Delete{% endif %} `canary-network` network.
