...
* [Create](../../iam/operations/sa/create.md) a service account named `my-emulator-function-service-account` to send data. [Assign](../../iam/operations/sa/assign-role-for-sa.md) it the `{{ roles-functions-invoker }}`, `iot.devices.writer`, and `{{ roles-viewer }}` [roles](../../iam/concepts/access-control/roles.md).
* Create a service account named `my-db-function-service-account` to process data. Assign it the `{{ roles-functions-invoker }}` and `{{ roles-editor }}` roles.
...
- Management console {#console}
  ...
  1. Click **{{ ui-key.yacloud.common.create }}**.
  1. Create `my-device-2` and `my-device-3` in the same way.
  ...
  1. Create a function version:
     ...
     1. Under **{{ ui-key.yacloud.serverless-functions.item.editor.label_title-params }}**, specify:
        ...
        * **{{ ui-key.yacloud.serverless-functions.item.editor.field_environment-variables }}**:
           ...
           | `REGISTRY_ID` | ID of the registry you created | See the {{ iot-name }}<br>[management console]({{ link-console-main }}) |
...
- Management console {#console}
  ...
  1. (Optional) In the **{{ ui-key.yacloud.serverless-functions.triggers.form.section_function-retry }}** and **{{ ui-key.yacloud.serverless-functions.triggers.form.section_dlq }}** sections, set parameters to prevent data loss in case of failure:
     ...
     * **{{ ui-key.yacloud.serverless-functions.triggers.form.section_dlq }}** allow you to redirect messages that the trigger sends to the function.
        ...
        You can use a [standard](../../message-queue/concepts/queue.md#standard-queues) message queue as a [dead-letter queue](../../message-queue/concepts/dlq.md). If you do not have a message queue, [create one in {{ message-queue-full-name }}](../../message-queue/operations/message-queue-new-queue.md).
  ...
  1. Create a function version:
     ...
     1. Under **{{ ui-key.yacloud.serverless-functions.item.editor.label_title-params }}**, specify:
        ...
        * **{{ ui-key.yacloud.serverless-functions.item.editor.field_environment-variables }}**:
           ...
           | `DB_HOSTNAME` | Host FQDN in {{ mpg-name }} | See the {{ mpg-name }}<br>[management console]({{ link-console-main }}) |
...
- Management console {#console}
  ...
  1. Select the `public` directory.
  1. View the results of data processing in the `iot_events` and `iot_position` tables.
...
To monitor the state of your devices, configure a [connection](../../datalens/concepts/connection.md), create a [dataset](../../datalens/concepts/dataset/index.md), [charts](../../datalens/concepts/chart/index.md), and [dashboard](../../datalens/concepts/dashboard.md).
...
- {{ datalens-short-name }} interface {#console}
  ...
  1. Select the `db1` database and `user1`.
...
- {{ datalens-short-name }} interface {#console}
  ...
  1. From the menu on the left, select the `public.iot_events` and `public.iot_position` tables and drag them to the right.
  1. Click **Save**.
...
- {{ datalens-short-name }} interface {#console}
  ...
  1. Create a field to define device coordinates:
     ...
     1. On the right side of the window, click ![image](../../_assets/console-icons/plus.svg) **Add field**.
...
- {{ datalens-short-name }} interface {#console}
  ...
  1. Click **Save**.
  1. In the window that opens, enter `MyDash`.
  1. Click **Create**.
