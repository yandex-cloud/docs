...
1. Add the new certificate to {{ certificate-manager-name }}:
    ...
    - Management console {#console}
      ...
      1. Select `{{ ui-key.yacloud.certificate-manager.request.challenge-type_label_dns }}` as the [domain rights check type](../../certificate-manager/concepts/challenges.md).
...
1. To successfully issue the certificate, pass the domain rights check:
    ...
    - Management console {#console}
        ...
        1. In the [management console]({{ link-console-main }}), select the folder to create your infrastructure in.
        1. From the list of services, select **{{ ui-key.yacloud.iam.folder.dashboard.label_certificate-manager }}**.
        1. In the list of certificates, select `joomla-cert`.
