...
- Management console {#console}
   ...
   1. Optionally, add an alias:
      ...
      1. Fill in the fields: enter an alias, e.g., `events`, and [topic](../../iot-core/concepts/topic/index.md) type after `$devices/{id}`, e.g., `events`.

         You can use the `events` alias instead of the `$devices/{id}/events` topic.
...
- Management console {#console}
   ...
   1. Create a function version:
      ...
      1. In a similar way, create a `package.json` file with the following contents:
      ...
      1. Under **{{ ui-key.yacloud.serverless-functions.item.editor.label_title-params }}**, specify:
         ...
         * **{{ ui-key.yacloud.serverless-functions.item.editor.field_environment-variables }}**:
            ...
            `IOT_CORE_DEVICE_ID`       | ID of the device you created.                              | See the {{ iot-name }}<br>[management console]({{ link-console-main }}).
...
- Management console {#console}
   ...
   1. In the **{{ ui-key.yacloud.serverless-functions.item.testing.field_tag }}** list, select `$latest`, which is the latest created function.
...
- Management console {#console}
   ...
   1. In the [management console]({{ link-console-main }}), select the folder you are using to complete this tutorial.
   1. In the list of services, select **{{ ui-key.yacloud.iam.folder.dashboard.label_serverless-functions }}**.
   1. In the panel on the left, select the **{{ ui-key.yacloud.serverless-functions.switch_list }}** section.
   1. Create a function:
      1. Click **{{ ui-key.yacloud.serverless-functions.list.button_create }}**.
...
- {{ monitoring-short-name }} interface {#console}
   ...
   1. Go to the **{{ ui-key.yacloud_monitoring.aside-navigation.menu-item.dashboards.title }}** tab.
...
- {{ monitoring-short-name }} interface {#console}
   ...
   1. Create a query for the chart:
      ...
      1. In the list of chart types (**name=**), select `Temperature`: Temperature in the room.
   ...
   * `Water sensor`: Water on the floor (there is water/no water).
   * `Smoke sensor`: Smoke in the room (there is smoke/no smoke).
...
- {{ monitoring-short-name }} interface {#console}
   ...
   1. In the [management console]({{ link-console-main }}), select the folder you are using to complete this tutorial.
   1. In the list of services, select **{{ ui-key.yacloud.iam.folder.dashboard.label_monitoring }}**.
   1. Go to the **{{ ui-key.yacloud_monitoring.aside-navigation.menu-item.channels.title }}** tab.
