...
In this scenario, you will set up monitoring and notifications about changes in sensor readings connected to [{{ iot-name }}](../../iot-core/). Sensors are emulated using the [{{ sf-full-name }}](../../functions/) service. If your sensors are connected, use them.

You do not need to create or configure any [VMs](../../compute/concepts/vm.md) to work with the scenario, as all operations are based on serverless computing in {{ sf-name }}. The source code used in this scenario is available on [GitHub](https://github.com/yandex-cloud/examples/tree/master/iot/Scenarios/ServerRoomMonitoring).
...
1. [Create a device emulator based on {{ sf-name }}](#emulator-step).
...
The cost includes:
* Fee for the number of {{ iot-name }} messages (see [pricing](../../iot-core/pricing.md)).
* Fee for {{ sf-name }} [function](../../functions/concepts/function.md) calls (see [pricing](../../functions/pricing.md)).
* Fee for logging user [metrics](../../monitoring/concepts/data-model.md#metric) via the [{{ monitoring-full-name }}](../../monitoring/) service API.


## Create the required {{ iot-name }} resources {#resources-step}

[Registry](../../iot-core/concepts/index.md#registry) and [device](../../iot-core/concepts/index.md#device) are the main components of the {{ iot-name }} service used for exchanging data and commands. Devices can only exchange data if they were created in the same registry.

### Create a registry and configure authorization using your login and password {#registry-step}

To create a registry:
1. In the [management console]({{ link-console-main }}), select the [folder](../../resource-manager/concepts/resources-hierarchy.md#folder) where you are running the scenario.
1. Select **{{ iot-name }}**.
...
1. In the **Password** field, set the password to access the registry.
   ...
   To create a password, you can use the [password generator](https://passwordsgenerator.net/). Make sure to save your password, as you will need it later.
...
1. Click **Add device**.
1. In the **Name** field, enter the device name, e.g., `my-device`.
...
1. (Optional) Add an alias:
   1. Click **Add alias**.
   1. Fill out the fields by providing an alias, e.g., `events`, and the [topic](../../iot-core/concepts/topic/index.md) type after `$devices/<deviceID>`, e.g., `events`.
...
* Create a trigger to invoke the data processing function by a signal.
...
1. In the **New file** window that opens, enter `device-emulator.js`.
1. Click **Create**.
1. Select the created file in the left part of the **Code editor** window.
1. In the right part of the **Code editor** window, paste the function code from [GitHub](https://github.com/yandex-cloud/examples/blob/master/iot/Scenarios/ServerRoomMonitoring/device-emulator.js).
1. Repeat steps 10-14 and, in a similar way, create the `package.json` file containing the following:
...
1. In the **Memory** field, keep the value of `128 MB`.
1. Create a [service account](../../iam/concepts/users/service-accounts.md) that the function will use to send data to {{ iot-name }}:
   ...
   1. Add the `{{ roles-functions-invoker }}` and `iot.devices.writer` [roles](../../iam/concepts/access-control/roles.md) for invoking the function and writing data to resources:
...
1. Set **Environment variables** for each sensor in the server room:
   ...
   1. Fill in the **Key** and **Value** fields for the environment variables:
      ...
      | `IOT_CORE_DEVICE_ID` | ID of the device you created | See in **{{ iot-name }}** <br>management console |
...
1. (Optional) To get detailed information from the sensors, subscribe the registry to the {{ iot-name }} device topic.

   Where:
   * `$devices/<deviceID>/events`: Device topic
   * `<deviceID>`: ID of the device in the service
   ...
   Learn more about the [subscription to device topics in {{ iot-name }}](../../iot-core/operations/subscribe#one-device).
1. In the [management console]({{ link-console-main }}), select the folder where you are running the scenario.
1. Select **{{ sf-name }}**.
...
If the function is successful, the **Function status** field will show **Done** and the **Function output** field will show the following:
...
If you subscribe to the {{ iot-name }} device topic, you will get the following JSON:
...
Learn more about [MQTT topics in {{ iot-name }}](../../iot-core/concepts/topic/index.md)
...
1. (Optional) Set parameters under **Repeat request settings** and **Dead Letter Queue settings**. They provide data security.
   ...
   * **Dead Letter Queue settings** are required to forward messages that consumers could not process in standard [queues](../../message-queue/concepts/queue.md).
...
1. In the left part of the window, select **Functions**.
1. Click **Create function**.
1. In the **Name** field, enter a name for the function, e.g., `my-monitoring-func`.
1. (Optional) In the **Description** field, add additional information about the function.
1. Click **Create**.
1. In the **Editor** window that opens, select `python37` from the **Runtime environment** list.
1. Select **Method**: go to the **Code editor** tab.
1. In the left part of the **Code editor** window, click **Create file**.
1. In the **New file** window that opens, enter `monitoring.py`.
1. Click **Create**.
1. In the left part of the **Code editor** window, select the created file.
1. In the right part of the window, paste the function code from [GitHub](https://github.com/yandex-cloud/examples/blob/master/iot/Scenarios/ServerRoomMonitoring/monitoring.py).
...
1. Create a service account to be used for the function to process data from the device:
   1. Click **Create account**.
   1. In the **Create service account** window that opens, enter the account name in the **Name** field, e.g., `my-metrics-function-service-account`.
   1. Add the `{{ roles-functions-invoker }}` and `editor` roles for invoking functions and editing resources:
      1. Click the ![image](../../_assets/plus-sign.svg) icon.
      1. Select a role from the list.
      1. Click **Create**.
   1. Configure **Environment variables**:
      ...
      1. Fill in the **Key** and **Value** fields for the environment variables:
         ...
         | `METRICS_FOLDER_ID` | ID of the folder where the services are deployed and for which you will create a dashboard in {{ monitoring-name }}. | See the management console for details. |
...
1. Select **Triggers**.
1. Click **Create trigger**.
1. In the **Name** field, enter a name for the trigger, e.g., `my-monitoring-func-trigger`.
1. (Optional) In the **Description** field, add additional information about the trigger.
...
1. Under **{{ iot-name }} message settings**, enter the parameters previously set for the registry and device:
   ...
   * **Device**: `my-device`.
   * **MQTT topic**: `$devices/<deviceID>/events`, where `<deviceID>` is the device ID in **{{ iot-name }}**.
1. Under **Function settings**, enter the parameters previously set for the function:
...
1. (Optional) Set parameters under **Repeat request settings** and **Dead Letter Queue settings**. They provide data security.
   ...
   * **Dead Letter Queue settings** are required to forward messages that consumers could not process in standard queues.
      ...
      You can configure a standard message queue as a DLQ. If you do not have a message queue yet, create one in {{ message-queue-name }}.
1. Click **Create trigger**.
...
1. Repeat the steps for each chart in the list:
   ...
   * `Smoke sensor`: Whether there is any smoke.
...
* `70 degrees`: `Alarm` (critical value)
...
1. In the **Name** field, enter a name for the channel, e.g., `my-message-channel`.
...
1. (Optional) Select the notification channel from the table and set up additional notification parameters:
   ...
   * To set up sending repeated notifications, select in the **Notify again** list when you want to receive them:
      ...
      * In `30 minutes`
      * In `1 hour`
   * To edit the notification channel, click **...** in the right part of the row.
...
1. If the sensor readings reach the set values, you will receive notifications.

## Delete the created cloud resources {#cleanup}

If you no longer need the cloud resources created in this scenario:
* [Delete a registry in the {{ iot-name }} service](../../iot-core/operations/registry/registry-delete.md).
* [Delete a device in the {{ iot-name }} service](../../iot-core/operations/device/device-delete.md).
* [Delete functions in the {{ sf-name }} service](../../functions/operations/function/function-delete.md).
* [Delete function triggers in the {{ sf-name }} service](../../functions/operations/trigger/trigger-delete.md).
* [Delete charts in the {{ monitoring-name }} service](../../monitoring/operations/).
* [Delete dashboards in the {{ monitoring-name }} service](../../monitoring/operations/).
* [Delete alerts and notification channels in the {{ monitoring-name }} service](../../monitoring/operations/).