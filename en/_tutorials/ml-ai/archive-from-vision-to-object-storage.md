...
- Management console {#console}
  ...
  1. Under **{{ ui-key.yacloud.compute.instances.create.section_storages }}**, select the [disk](../../compute/concepts/disk.md#disks-types) type (`{{ ui-key.yacloud.compute.value_disk-type-network-ssd_4Mmub }}`) and specify its size (`19 {{ ui-key.yacloud.common.units.label_gigabyte }}`).
