...
* Manually
   ...
   1. In the cluster subnet, set up an NAT gateway and create a security group allowing all incoming and outgoing traffic from all addresses.
   ...
   1. Create a virtual machine running Linux on the same cloud network as the {{ mgp-name }} cluster.
...
* Using Terraform
   ...
   1. If you don't have {{ TF }}, install and configure it by following the instructions.
   1. Download the file with the provider settings. Place it in a separate working directory and specify the parameter values.
   ...
   1. In a file named `greenplum-s3-vm.tf`, specify:
      ...
      * Username and path to the {% if lang == "ru" and audience != "internal" %}[SSH key](../../glossary/ssh-keygen.md){% else %}SSH key{% endif %} for virtual machine access.
   ...
   1. Go to the [management console]({{ link-console-main }}) and set up an NAT gateway for the subnet hosting your cluster.
...
* Manually
   ...
   1. [Delete the bucket in {{ objstorage-name }}](../../storage/operations/buckets/delete.md).
   1. [Delete a {{ mgp-name }} cluster](../../managed-greenplum/operations/cluster-delete.md).
