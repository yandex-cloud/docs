...
- Using Terraform
   ...
   1. In the file, specify a password for the user `user`, which will be used for accessing {{ mkf-name }} clusters as well as a username and the public SSH key for the virtual machine. If the virtual machine has Ubuntu 20.04 installed from the recommended {% if audience != "internal" %}[image list](../compute/operations/images-with-pre-installed-software/get-list.md){% else %}image list{% endif %}, the username specified here will be ignored. If this is the case, use `ubuntu` as your username for the [connection](#prepare-vm).
