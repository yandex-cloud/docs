...
Next, we'll configure {{ KFC }} to interact with a {{ mkf-name }} cluster. The tool will be deployed on a {{ yandex-cloud }} VM as a separate installation. SSL encryption will be used to protect the connection.
...
- Using Terraform
   ...
   1. In the file, specify a password for the `user` user, which will be used for accessing {{ mkf-name }} clusters as well as a username and the public {% if lang == "ru" and audience != "internal" %}[SSH key](../glossary/ssh-keygen.md){% else %}SSH key{% endif %} for the virtual machine. If the virtual machine has Ubuntu 20.04 installed from the recommended {% if audience != "internal" %}[image list](../compute/operations/images-with-pre-installed-software/get-list.md){% else %}image list{% endif %}, the username specified here will be ignored. If this is the case, use `ubuntu` as your username for the [connection](#prepare-vm).
...
- Manually
   ...
   1. Delete the VM.
   1. If you reserved a public static IP address for the virtual machine, release and delete it.
   1. Delete the {{ mkf-name }} cluster.
