...
In this scenario, you will set up {{ GL }} on a [virtual machine](../../compute/concepts/vm.md), create a simple C++ project, set up a test scenario, and test its execution.

To create and test a project in the {{ GL }} environment:
1. [Get your cloud ready](#before-you-begin).
1. [Required paid resources](#paid-resources).
1. [Create a {{ GL }} VM](#create-vm).
...
Wait about five minutes until the VM is created and all its services are up and running. After all of the services are up and running, {{ GL }} becomes accessible via the web interface in a browser.
...
1. Register your runner in {{ GL }}:
   ...
   1. Enter your {{ GL }} server address. When you see this prompt:
   ...
   1. Enter the registration token for the runner. To get it, go to the project page in {{ GL }}, select **Settings** in the left-hand panel, and open the **CI/CD** tab. Then click **Expand** under **Runners**. Under **Set up a specific Runner manually**, copy the token from _Step 3_ and enter it when prompted as follows:
...
After committing, the system will automatically start testing the latest commit. To check its results, select **Build** on the left-hand panel in the {{ GL }} project, and then **Pipelines** from the drop-down menu. You should see a line with the first test showing `passed`. By clicking the cloud icon, you can download the build artifacts.
