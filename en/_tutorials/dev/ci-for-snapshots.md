...
1. In the `/srv/test-app` folder, create a file named `api.py`:
...
1. In the `/srv/test-app` folder, create a file named `wsgi.py`:
...
1. In the `/srv/test-app` folder, create a file named `test-app.ini`:
...
1. Create a symbolic link that points to the nginx configuration file, i.e., `test-app.conf`:
...
1. In the browser, open the link formatted as follows: `http://<VM_public_IP_address>/root`.
...
1. [Connect](../../compute/operations/vm-connect/ssh.md#vm-connect) to the VM with {{ GL }} over SSH:
   ...
   ```bash
   ssh <login>@<public_IP_address_of_your_VM_with_{{ GL }}>
   ```
...
1. Register the Runner:
   ...
   * For `Please enter the gitlab-ci token for this runner`, enter your Runner token.
...
1. Create a file named `test.py` with the functional testing script:
   ...
   1. In the window that opens, name the file as follows: `test.py`.
...
1. Open the home page of the `gitlab-test` repository.
   ...
   ```http
   http://<public_IP_address_of_your_VM_with_{{ GL }}>/root/gitlab-test
   ```
...
1. In the `snapshot_name` field, enter the name of your snapshot for the first VM.
   In the `folder_id` field, specify the ID of the folder where VMs are created.
   In the `subnet_name` field, specify the name of the subnet the VMs will connect to. You can retrieve the name in the management console by opening the appropriate folder and going to the {{ vpc-name }} service page.
...
1. The application created in the previous steps should also function on the created VM and return the reversed words from the `text` parameter.
