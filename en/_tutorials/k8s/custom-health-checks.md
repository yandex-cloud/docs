# Health checking applications in a {{ managed-k8s-full-name }} cluster using a {{ alb-full-name }} L7 load balancer
...
By default, the {{ alb-name }} Ingress controller receives application health check requests from the L7 load balancer on TCP port `10501` and health checks the [kube-proxy](https://kubernetes.io/docs/reference/command-line-tools-reference/kube-proxy/) pods on each cluster node. Given that kube-proxy is healthy, the process is as follows: if an application does not respond in a particular pod, {{ k8s }} redirects traffic to a different pod or node.
...
* `8080`: Return an `OK` message in the response body. This functionality will be used for application health checks.
