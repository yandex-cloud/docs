...
This tutorial describes how to set up integration between a [{{ mgl-full-name }} instance](../../managed-gitlab/concepts/index.md#instance) and a [{{ managed-k8s-name }} cluster](../../managed-kubernetes/concepts/index.md#kubernetes-cluster) running [Argo CD](/marketplace/products/yc/argo-cd) and [{{ GLR }}](/marketplace/products/yc/gitlab-runner) to build Docker containers with [Kaniko](https://github.com/GoogleContainerTools/kaniko).
...
- {{ TF }} {#tf}
  ...
  1. Download the [k8s-argocd.tf](https://github.com/yandex-cloud-examples/yc-mk8s-argo-cd/blob/main/k8s-argocd.tf) {{ managed-k8s-name }} cluster configuration file to the same working directory. This file describes:
     ...
     * {% include [configure-sg-terraform](../../_includes/managed-kubernetes/security-groups/configure-sg-tf-lvl3.md) %}
        ...
        These security groups also include the rules required for the {{ mgl-name }} instance and [{{ container-registry-name }}](../../container-registry/concepts/registry.md).
...
1. This will run the build script. To view its progress, in the left-hand panel in {{ GL }}, select **Build** → **Pipelines**. Wait for both build steps to complete.
...
### Install Argo CD in the {{ managed-k8s-name }} cluster {#install}
...
1. Get the admin password from the {{ k8s }} secret:
...
### Add the {{ GL }} repository to Argo CD {#create}

1. In {{ GL }}, navigate to **Settings** in the left-hand panel and select **Access Tokens** from the drop-down list.
...
1. Click **Connect Repo** and select **VIA HTTPS** from the list.
   ...
   To enable access, in the left-hand panel in {{ GL }}, select **Admin → Settings → General**. Under **Visibility and access controls**, find the **Enabled Git access protocols** setting and select the option that allows HTTP(S) access from the list.
...
1. In the form that opens, specify the following settings:
   * **Repository URL**: Repository URL in `https://<{{ GL }}_instance_name>.gitlab.yandexcloud.net/<admin_username>/gitlab-test.git` format.
...
1. In the form that opens, specify the following settings:
   ...
   * **Repository URL**: Specify the repository URL in `https://<{{ GL }}_instance_name>.gitlab.yandexcloud.net/<admin_username>/gitlab-test.git` format.
   * **Path**: `.helm`.
...
1. To check that the application is running, use the following command in the {{ managed-k8s-name }} cluster:
