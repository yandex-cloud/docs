...
With automated installation, the {{ managed-k8s-name }} cluster connects to the {{ baremetal-name }} server over SSH by itself and installs all required system components.
...
1. Use `kubectl` to get a secret containing `kubeconfig` for the {{ baremetal-name }} server you are connecting to and save it to a file:
...
1. Transfer the `kube.config` file to the {{ baremetal-name }} server:
