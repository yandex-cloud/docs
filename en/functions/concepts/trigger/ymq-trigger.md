...
The [ trigger](../trigger/) for {{ message-queue-short-name }} is used to process the message queue. It receives messages from the queue and passes them to a {{ sf-name }} [function](../function.md) or {{ serverless-containers-name }} [container](../../../serverless-containers/concepts/container.md) for processing. Once successfully processed, a trigger deletes messages from the queue, and if an error occurs, it places messages back in the queue and sets a [visibility timeout](../../../message-queue/concepts/visibility-timeout.md). If the queue does not have a [Dead Letter Queue](../../../message-queue/concepts/dlq.md) configured, a message will continue to be returned to a function or container until it is successfully processed or until its storage timeout expires.
...
{{ message-queue-short-name }} triggers require [service accounts](../../../iam/concepts/users/service-accounts.md) to read messages from a message queue and to call a function or container. You can use the same service account for both operations.
