...
After receiving and verifying the SAML token signature, {{ iam-short-name }} sets a cookie for the {% if product == "yandex-cloud" %}cloud.yandex.TLD{% endif %}{% if product == "cloud-il" %}cloudil.TLD{% endif %} domain for the user. {{ iam-short-name }} stores and manages the cryptographic keys used to control cookie integrity.
