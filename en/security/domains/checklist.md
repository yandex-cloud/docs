...
* **Bastion host**: Create a bastion VM to access the infrastructure using control protocols (for example, {% if lang == "ru" and audience != "internal" %}[SSH](../../glossary/ssh-keygen.md){% else %}SSH{% endif %} or RDP).
...
* **Secret management**: Use secret management services, such as [{{ lockbox-name }}](../../lockbox/){% if product == "yandex-cloud" %} or [HashiCorp Vault with {{ kms-name }} support](/marketplace/products/yc/vault-yckms) from {{ marketplace-name }}{% endif %}.<!-- или контейнер cr.yandex/yc/vault -->
...
* **Secret management**: Use [{{ lockbox-name }}](../../lockbox/){% if product == "yandex-cloud" %} or [HashiCorp Vault with {{ kms-name }} support](/marketplace/products/yc/vault-yckms) from {{ marketplace-name }}{% endif %}.<!-- или контейнер `cr.yandex/yc/vault` -->
{% if product == "yandex-cloud" %}
