...
You can also {% if lang == "ru" and audience != "internal" %}[analyze](../../glossary/data-analytics.md){% else %}analyze{% endif %} {{ objstorage-name }} logs in {{ datalens-full-name }}.
...
When accessing the database in Dedicated mode, we recommend that you use it inside {{ vpc-full-name }}, disabling public access to it from the internet. In Serverless mode, the database can be accessed from the internet. You must therefore take this into account when modeling threats to your PCI DSS infrastructure. {% if audience == "external" %}For more information about the operating modes, see the {{ ydb-name }} documentation, [Serverless and Dedicated modes](../../ydb/concepts/serverless-and-dedicated.md).{% endif %}
