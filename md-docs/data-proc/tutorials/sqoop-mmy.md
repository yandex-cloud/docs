...
Утилита [Sqoop](../operations/sqoop-usage.md) позволяет импортировать базы данных в кластер {{ dataproc-name }}. В зависимости от конфигурации кластера {{ dataproc-name }} вы можете выполнить импорт в:
...
1. [Создайте кластер {{ dataproc-name }}](../operations/cluster-create.md) любой подходящей вам [конфигурации](../concepts/instance-types.md).
...
1. Если вы используете группы безопасности для кластеров и виртуальной машины, настройте их так, чтобы разрешить подключение:
    ...
    * [к виртуальной машине и кластеру {{ dataproc-name }}](../operations/security-groups.md);
...
1. Укажите параметры инфраструктуры в файле конфигурации `clusters-mysql-data-proc-and-vm.tf` в блоке `locals`:
    ...
    * `dp_public_key` — абсолютный путь к [публичному SSH-ключу](../operations/connect-ssh.md) для кластера {{ dataproc-name }}.
...
- Директория HDFS {#hdfs}
    ...
    1. [Подключитесь](../operations/connect-ssh.md) по SSH к хосту подкластера {{ dataproc-name }} для хранения данных.
    1. Выполните команду:
...
* Вручную {#manual}
    ...
    1. Удалите кластеры:
        ...
        * [{{ dataproc-name }}](../operations/cluster-delete.md).
