[Документация Yandex Cloud](../../index.md) > [Yandex Data Processing](../index.md) > [Практические руководства](index.md) > Работа с заданиями > Работа с заданиями Spark
...
1. Скачайте и загрузите в бакет для исходных данных jar-файл `spark-app_2.11-0.1.0-SNAPSHOT.jar`, собранный из исходного текста программы анализа [word_count.scala](https://storage.yandexcloud.net/examples/scala-spark/word_count.scala) на языке Scala:
    ...
    Подробнее о том, как собрать приложение для Spark, написанное на языке Scala, смотрите в разделе [Использование Spark Submit](run-spark-job.md#spark-submit).
