...
* Плата за кластер {{ dataproc-name }}: использование вычислительных ресурсов ВМ и сетевых дисков {{ compute-name }}, а также сервиса {{ cloud-logging-name }} для работы с логами (см. [тарифы {{ dataproc-name }}](../pricing.md)).
...
- Вручную {#manual}
    ...
    1. [Создайте кластер {{ dataproc-name }}](../operations/cluster-create.md) с любой [подходящей конфигурацией хостов](../concepts/instance-types.md) и следующими настройками:
...
- {{ TF }} {#tf}
    ...
    1. Укажите в файле `s3-dataproc-ch.tf`:
        ...
        * `dp_ssh_key` — абсолютный путь к публичному ключу для кластера {{ dataproc-name }}. [Подробнее о подключении к хосту {{ dataproc-name }} по SSH](../operations/connect-ssh.md).
...
1. [Создайте задание PySpark](../operations/jobs-pyspark.md#create), указав в поле **{{ ui-key.yacloud.dataproc.jobs.field_main-python-file }}** путь к файлу скрипта: `s3a://<имя_входного_бакета>/scripts/join-tables.py`.
...
1. [Создайте задание PySpark](../operations/jobs-pyspark.md#create), указав в поле **{{ ui-key.yacloud.dataproc.jobs.field_main-python-file }}** путь к файлу скрипта: `s3a://<имя_входного_бакета>/scripts/parquet-to-ch.py`.
...
1. Остальные ресурсы удалите в зависимости от способа их создания:
    ...
    - Вручную {#manual}
        ...
        1. [Кластер {{ dataproc-name }}](../operations/cluster-delete.md).
