...
Вы можете перенести метаданные между [кластерами {{ dataproc-full-name }}](../concepts/index.md) с СУБД Hive. Сначала метаданные экспортируются из одного кластера, затем импортируются в другой с помощью [{{ metastore-full-name }}](../../metadata-hub/concepts/metastore.md).
...
- Вручную {#manual}
    ...
    1. [Создайте два кластера {{ dataproc-name }}](../operations/cluster-create.md) с именами `dataproc-source` и `dataproc-target` и следующими настройками:
...
1. [Добавьте в настройки кластера](../operations/cluster-update.md) `dataproc-target` свойство `spark:spark.hive.metastore.uris` со значением `thrift://<IP-адрес_кластера_{{ metastore-name }}>:{{ port-metastore }}`.
