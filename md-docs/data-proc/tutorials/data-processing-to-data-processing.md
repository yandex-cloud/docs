...
Вы можете сохранять данные из [кластера {{ dataproc-full-name }}](../concepts/index.md) в [бакет {{ objstorage-full-name }}](../../storage/concepts/bucket.md), используя отдельный [кластер {{ metastore-full-name }}](../../metadata-hub/concepts/metastore.md) для хранения метаданных таблиц. Это позволит затем работать с сохраненными данными другому кластеру {{ dataproc-name }}, имеющему доступ к бакету и подключенному к тому же кластеру {{ metastore-name }}.
...
Если в кластере {{ dataproc-name }} есть таблицы, которые должны быть доступны в другом кластере {{ dataproc-name }}, [перенесите таблицы](metastore-import.md) в нужный кластер с помощью {{ metastore-name }}.
...
- Вручную {#manual}
    ...
    1. [Создайте два кластера {{ dataproc-name }}](../operations/cluster-create.md) с именами `dataproc-source` и `dataproc-target`, с любой [подходящей конфигурацией хостов](../concepts/instance-types.md) и следующими настройками:
...
- {{ TF }} {#tf}
    ...
    1. Укажите в файле `dataproc-to-dataproc.tf`:
        ...
        * `dp_ssh_key` — абсолютный путь к публичному ключу для кластеров {{ dataproc-name }}. [Подробнее о подключении к хосту {{ dataproc-name }} по SSH](../operations/connect-ssh.md).
...
1. [Добавьте в настройки кластеров](../operations/cluster-update.md) {{ dataproc-name }} свойство `spark:spark.hive.metastore.uris` со значением `thrift://<IP-адрес_кластера_{{ metastore-name }}>:{{ port-metastore }}`.
...
- Spark
    ...
    1. [Создайте задание PySpark](../operations/jobs-pyspark.md#create), указав в поле **{{ ui-key.yacloud.dataproc.jobs.field_main-python-file }}** путь к файлу скрипта: `s3a://<имя_входного_бакета>/scripts/create-table.py`.

    1. Дождитесь завершения задания и проверьте, что в выходном бакете в папке `countries` появился файл `part-00000-...`.

{% endlist %}

Теперь данные из созданной таблицы хранятся в бакете {{ objstorage-name }}, а метаинформация о ней — в кластере {{ metastore-name }}. Кластер `dataproc-source` можно [удалить](../operations/cluster-delete.md).
...
- Spark
    ...
    1. [Создайте задание PySpark](../operations/jobs-pyspark.md#create), указав в поле **{{ ui-key.yacloud.dataproc.jobs.field_main-python-file }}** путь к файлу скрипта: `s3a://<имя_входного_бакета>/scripts/obtain-table.py`.
...
1. Удалите остальные ресурсы в зависимости от способа их создания:
    ...
    - Вручную {#manual}
        ...
        1. [Кластеры {{ dataproc-name }}](../operations/cluster-delete.md).
