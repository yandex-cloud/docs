...
При работе в мультикластерном режиме {{ dataproc-name }} использует базу данных [{{ ydb-full-name }}](../../../ydb/index.md) для координации доступа к таблицам Delta Lake из разных кластеров и заданий {{ SPRK }}.
...
1. Установите следующие [свойства](../../concepts/settings-list.md) на уровне кластеров или на уровне отдельных заданий {{ SPRK }}, которые должны иметь доступ к таблицам Delta Lake:
    ...
    * `spark.io.delta.storage.S3DynamoDBLogStore.ddb.lockbox` в значение идентификатора секрета {{ lockbox-short-name }}, который доступен на вкладке **{{ ui-key.yacloud.common.overview }}** вашего {{ lockbox-short-name }} в [консоли управления]({{ link-console-cloud }}).
