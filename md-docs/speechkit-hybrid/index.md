# Yandex SpeechKit Hybrid

SpeechKit Hybrid — это технологии Yandex SpeechKit, работающие внутри вашей инфраструктуры. В основе SpeechKit Hybrid лежат контейнеры Docker, которые отлично подходят для обеспечения требований к безопасности и управлению данными.

SpeechKit Hybrid основан на тех же моделях синтеза и распознавания речи, что и SpeechKit.

SpeechKit Hybrid <a href="https://reestr.digital.gov.ru/reestr/2356994/?sphrase_id=5385972">включен</a> в реестр программного обеспечения, созданный в соответствии со статьей 12.1 Федерального закона «Об информации, информационных технологиях и о защите информации», по основному классу 07.04 «Средства распознавания и синтеза речи».

# Yandex SpeechKit Hybrid

 - [Системные требования](system-requirements.md)

 - [Системные требования CPU](system-requirements-cpu.md)

 - [Архитектура сервиса](architecture.md)

## Создание демонстрационного стенда

 - [Создание стенда SpeechKit Hybrid](quickstart.md)

 - [Поиск и устранение ошибок](quickstart-debugging.md)

## Распознавание речи

 - [О технологии](stt/index.md)

 - [Тестирование производительности](stt/testing.md)

## Синтез речи

 - [О технологии](tts/index.md)

 - [Тестирование производительности](tts/testing.md)

## Администрирование SpeechKit Hybrid

 - [Настройки сервиса](settings.md)

 - [Мониторинг сервисов SpeechKit Hybrid](monitoring.md)

 - [Мониторинг Envoy](monitoring-envoy.md)

 - [Уровни логирования](operations/logging.md)

 - [Терминирование TLS](operations/tls-proxy.md)

 - [Аутентификация в API](auth.md)

 - [Правила тарификации](pricing.md)

 - [Релизы SpeechKit Hybrid](release-notes.md)

 - [Документация Яндекс Спичкит Гибрид (Yandex SpeechKit Hybrid)](docs.md)