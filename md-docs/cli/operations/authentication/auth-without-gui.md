[Документация Yandex Cloud](../../../index.md) > [Интерфейс командной строки](../../index.md) > [Пошаговые инструкции](../index.md) > Аутентификация > Без графического интерфейса и браузера

# Аутентификация в Yandex Cloud CLI на сервере без графического интерфейса
...
1. Запустите аутентификацию:
    ...
    - Аккаунт на Яндексе {#yandex-account}
      ...
      Подробнее на странице [Аутентификация в Yandex Cloud CLI с аккаунтом на Яндексе](user.md).
    ...
    - Федеративный аккаунт {#federated-account}
      ...
      Подробнее на странице [Аутентификация в Yandex Cloud CLI от имени федеративного пользователя](federated-user.md).
    ...
    - Аккаунт локального пользователя {#local-account}
      ...
      Подробнее на странице [Аутентификация в Yandex Cloud CLI от имени локального пользователя](local-user.md).
...
1. На локальном компьютере откройте новое окно терминала и выполните команду для создания SSH-туннеля, указав порт, полученный в предыдущем шаге, имя пользователя и IP-адрес ВМ:
    ...
    ```bash
    ssh -N -L <порт>:127.0.0.1:<порт> <имя_пользователя>@<IP-адрес_ВМ>
    ```

    После подключения команда не выводит результат и не завершается, пока работает SSH-туннель. Не закрывайте это окно терминала до завершения настройки CLI.

1. В браузере на локальном компьютере откройте URL для аутентификации, полученный ранее.
1. Вернитесь в окно терминала с SSH-сессией ВМ, в котором запущена команда `yc init`. Если аутентификация прошла успешно, CLI перейдет к следующему шагу настройки профиля.
1. Завершите настройку CLI.
1. В окне локального терминала завершите работу SSH-туннеля с помощью сочетания клавиш **Ctrl** + **C**.
