...
`total_http` | Аналогично `http`, но суммарно за весь период времени. Описание совпадает с `total_time`.</br>Примеры: `total_http(5xx,10%,10s)`, `total_http(3xx,40%,10s)`.</br>Код завершения — `26`.
`total_net` | Аналогично `net`, но суммарно за весь период времени. Описание совпадает с `total_time`.</br>Примеры: `total_net(79,10%,10s)`, `total_net(11x,50%,15s)`.</br>Код завершения — `27`.
