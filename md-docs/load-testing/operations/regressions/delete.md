# Удалить дашборд

{% note warning %}

С 1 июля 2026 года сервис Load Testing прекращает работу. Подробнее на странице [Закрытие сервиса Yandex Load Testing](../../sunset.md).

{% endnote %}


Если дашборд вам больше не нужен, удалите его.

{% list tabs group=instructions %}

- Консоль управления {#console}

  1. В [консоли управления](https://console.yandex.cloud) откройте каталог, в котором вы хотите удалить дашборд.
  1. Перейдите в сервис **Load Testing**.
  1. На панели слева выберите ![image](../../../_assets/load-testing/regressions.svg) **Регрессии**.
  1. Справа от имени нужного дашборда нажмите кнопку ![image](../../../_assets/options.svg) и выберите **Удалить**.

{% endlist %}