...
_Внешний агент_ — физический или виртуальный сервер с инструментом для тестирования и [генераторами нагрузки](../concepts/load-generator.md), который размещен вне сервиса Load Testing.

Запуск и просмотр результатов нагрузочного тестирования с внешнего агента идентичны работе с [агентом](../concepts/agent.md) Load Testing. Подробнее в разделе [Как начать работать с Yandex Load Testing](../quickstart.md).

Нагрузочное тестирование с внешнего агента применяется в следующих случаях:
* цель тестирования состоит из нескольких виртуальных машин, а суммарный входящий и исходящий трафик с них превышает [возможности агента Load Testing](../concepts/agent.md#benchmark);
...
1. [Назначьте](../../iam/operations/roles/grant.md) сервисному аккаунту [роли](../security/index.md#roles-list) `loadtesting.editor` и `loadtesting.generatorClient`.
...
1. Аутентифицируйтесь в Container Registry от [имени пользователя](../../container-registry/operations/authentication.md#user) или с помощью [Docker Credential Helper](../../container-registry/operations/authentication.md#cred-helper).
...
[Выполните](../quickstart.md#run-test) нагрузочное тестирование. В поле **Агенты** выберите внешний агент, например `external-agent`.
