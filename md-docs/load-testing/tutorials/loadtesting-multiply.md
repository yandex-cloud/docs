...
{{ load-testing-name }} можно использовать для тестирования сервиса с помощью нескольких [агентов тестирования](../concepts/agent.md).

Нагрузочное тестирование с нескольких агентов применяется если:
* Цель тестирования состоит из нескольких [виртуальных машин](../../compute/concepts/vm.md), а суммарный входящий и исходящий трафик с них превышает [возможности одного агента](../concepts/agent.md#benchmark).
* Запросы, которые направляются в цель тестирования, требуют много вычислительных мощностей.
* Цель тестирования намного эффективнее [генератора нагрузки](../concepts/load-generator.md).

Агенты запускаются и подают нагрузку на цель тестирования синхронно. [Результаты тестирования](../concepts/load-test-results.md) доступны в обобщенных и независимых [отчетах](../concepts/reports.md) с каждого агента.
...
* [{#T}](../operations/security-groups-agent.md).
* [{#T}](../operations/security-groups-target.md).
...
1. Создайте первый агент:
   ...
   - Консоль управления {#console}
     ...
     1. В блоке **{{ ui-key.yacloud.load-testing.section_agent }}**:
        * Выберите подходящий [тип агента](../concepts/agent.md#benchmark).
     ...
     1. Дождитесь завершения процесса создания ВМ. Статус агента должен смениться на `Ready for test`.
        ...
        * У агента есть [доступ](../operations/security-groups-agent.md) к `loadtesting.{{ api-host }}:443` и [публичный IP-адрес](../../vpc/concepts/address.md).
  ...
  1. В блоке **Конфигурация 1** укажите параметры тестирования для первого агента:
     ...
     1. В блоке **{{ ui-key.yacloud.load-testing.label_test-settings }}**:
        ...
        * **Тестирующие потоки** — `1000`.
        ...
        Это будет означать, что генератор нагрузки сможет параллельно обрабатывать 1000 операций (создать 1000 соединений или ждать 1000 ответов от сервиса одновременно). [Подробнее о тестирующих потоках](../concepts/testing-stream.md).
        ...
        * В меню **Тип нагрузки**:
          ...
          * Добавьте еще один **Профиль нагрузки**:
          ...
          Эти настройки дадут генератору нагрузки указание наращивать нагрузку от 1 до 100 запросов в секунду первые 60 секунд, а потом 5 минут поддерживать нагрузку 100 запросов в секунду. [Подробнее о профиле нагрузки](../concepts/load-profile.md).
        * **Тип запросов** — укажите [тип](../concepts/payloads/uri.md) `URI`.
