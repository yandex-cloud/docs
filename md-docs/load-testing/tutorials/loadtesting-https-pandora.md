...
{{ load-testing-name }} можно использовать для тестирования сервиса со ступенчатой нагрузкой по протоколу [HTTPS](../../glossary/ssl-certificate.md) с помощью [генератора нагрузки](../concepts/load-generator.md) [Pandora](../concepts/load-generator.md#pandora).
...
Если [агент](../concepts/agent.md) размещается на платформе {{ yandex-cloud }}, взимается плата за вычислительные ресурсы (см. [тарифы {{ compute-full-name }}](../../compute/pricing.md)).
...
1. Создайте агент:
   ...
   - Консоль управления {#console}
     ...
     1. Дождитесь завершения процесса создания [виртуальной машины](../../compute/concepts/vm.md). Статус агента должен смениться на `Ready for test`.
        ...
        * У агента есть [доступ](../operations/security-groups-agent.md) к `loadtesting.{{ api-host }}:443` и [публичный IP-адрес](../../vpc/concepts/address.md#public-addresses).
...
1. Сформируйте тестовые данные в формате [URI](../concepts/payloads/uri.md):
   ...
   Также указаны два запроса, отмеченные тегами `index` и `get_test`. Генератор будет повторять их по очереди в пределах заданного [профиля нагрузки](../concepts/load-profile.md).
...
1. В зависимости от выбранного способа задайте параметры теста:
   ...
   - {{ ui-key.yacloud.load-testing.label_settings-type-form }}
     ...
     1. В поле **Тестирующие потоки** укажите значение `5000`.
        ...
        Для большинства тестов хватит 1000–10 000 [потоков](../concepts/testing-stream.md).
     ...
     1. Укажите еще один [автостоп](../concepts/auto-stop.md):
   ...
   - {{ ui-key.yacloud.load-testing.label_settings-type-config }}
     ...
     Посмотрите [пример файла конфигурации](../concepts/testing-stream.md#config_example). Также пример файла конфигурации можно посмотреть в уже имеющихся тестах.
