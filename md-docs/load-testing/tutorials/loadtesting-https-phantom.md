...
{{ load-testing-name }} можно использовать для тестирования сервиса с постоянной нагрузкой по протоколу [HTTPS](../../glossary/ssl-certificate.md) с помощью [генератора нагрузки](../concepts/load-generator.md) [Phantom](../concepts/load-generator.md#phantom).
...
1. Сформируйте тестовые данные в формате [URI](../concepts/payloads/uri.md):
   ...
   Также указаны два запроса, отмеченные тегами `index` и `get_test`. Генератор нагрузки будет повторять их по очереди в пределах заданного [профиля нагрузки](../concepts/load-profile.md).
...
1. В зависимости от выбранного способа задайте параметры теста:
   ...
   - Форма
     ...
     1. В меню **Автостоп** нажмите ![image](../../_assets/plus-sign.svg) **Автостоп** и введите описание:
        ...
        * **Размер окна** — `60s`.
        ...
        Этот критерий остановит тест, если в течение 60 секунд будет занято 90% тестирующих потоков, что свидетельствует о возникновении проблем тестирования. [Подробнее об автостопе](../concepts/auto-stop.md).
   ...
   - Конфигурационный файл
     ...
     В поле для ввода конфигурации введите настройки тестирующих потоков в формате `yaml`:
        ...
        [Подробнее об автостопе](../concepts/auto-stop.md).
