...
* Минимальный размер системного диска — 150 GB. Для серверов с ролями `control-plane` или `combined` (подробнее ниже) в качестве системного диска должен использоваться SSD или NVMe. На серверах с ролью `worker` может использоваться системный диск любого типа (HDD, SSD, NVMe).
...
DNS-зона `$cluster.$baseDomain` должна быть делегирована на хосты с ролью `control-plane` или `combined` (блок `hosts:` конфигурации).
...
Здесь под `ca.crt` и `ca.key` подразумеваются подписанный сертификат и ключ промежуточного CA. Подробнее про управление сертификатами в [разделе "Certificate Manager"](concepts/components/certificate-manager.md).
...
1. Загрузите образы контейнеров:
   ...
   * `--image-bundle` — тип пакета образов. Используйте `full` для полного набора всех необходимых образов.
   ...
   Пакет `full` содержит только базовые компоненты Stackland. Для загрузки образов отдельно лицензируемых компонентов, таких как SpeechSense, используйте отдельную команду с `--image-bundle speechsense`. Подробнее в разделе [Загрузить образы SpeechSense](operations/speechsense/install-images.md).
...
* Убедитесь, что в графе **STAGE** на дашборде, отображаемом на локальной консоли сервера (на иллюстрации ниже), значится `Maintenance`.

![talos-dashboard](_assets/talos-dashboard.png)

Данный метод сброса сохраняет сетевые настройки, в частности IP-адрес сервера. Если необходимо изменить их, это можно сделать вручную через меню, доступное по клавише **F3** (на иллюстрации ниже). Введите требуемую конфигурацию целиком (отредактировать текущие параметры нельзя), после чего перезагрузите сервер.
