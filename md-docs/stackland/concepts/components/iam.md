...
Данные Identity and Access Management хранятся в YDB. Начиная с Stackland 26.3 администратор кластера может сохранить резервную копию этой базы данных в локальный архив `tar.gz` и восстановить ее из архива с помощью скриптов.

При восстановлении текущая YDB и ее PVC удаляются, а кластер временно становится недоступен. Подробные требования и порядок действий приведены в инструкциях [Создать резервную копию YDB](../../operations/iam/ydb-backup.md) и [Восстановить YDB из резервной копии](../../operations/iam/ydb-restore.md).
