...
* `simpleScalable` — масштабируемая конфигурация с компонентами `writer`, `reader` и `backend`, которая хранит данные во внутреннем S3. Для этого режима должен быть включен компонент [Object Storage](storage.md).
