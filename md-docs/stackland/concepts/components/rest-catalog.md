...
Каталог можно [подключить к Managed Service for Trino](../../operations/trino/create-catalog.md), указав его имя. Параметры подключения и доступа к хранилищу настраиваются автоматически.

## Каталог {#catalog}

Каталог описывается ресурсом `StacklandRestCatalog`. Для хранения данных таблиц автоматически создаются отдельный бакет в [Object Storage](storage.md) и ключ доступа к нему. Выбирать бакет и настраивать доступ к нему не требуется.
...
При выполнении запросов через Trino используются права пользователя или сервисного аккаунта, от имени которого выполнен вход в Trino. Также учитываются [правила доступа Trino](trino.md#access-control).
