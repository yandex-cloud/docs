...
Для настройки окружения в данном руководстве используется [Консоль управления Yandex Cloud](http://console.yandex.cloud). Чтобы воспользоваться другим интерфейсом Yandex BareMetal, обратитесь к [документации сервиса](https://yandex.cloud/ru/services/baremetal).
...
1. Откройте форму создания приватной сети в сервисе Yandex BareMetal и активируйте переключатель **IP-адресация и маршрутизация**. Открыть форму можно по инструкции [Создать приватную подсеть](../../baremetal/operations/subnet-create.md).
...
1. Укажите диапазон для IP-адресов, например `192.168.22.2` — `192.168.22.120`. Диапазон должен быть достаточно широким, чтобы вместить все узлы будущего кластера Stackland и еще один хост.
...
1. Арендуйте сервер для бастиона, как описано в [документации](../../baremetal/operations/servers/server-lease.md).
    ...
    В качестве бастиона можно использовать виртуальную машину, запущенную в Yandex Cloud. Настроить сетевую связность между виртуальной машиной и серверами BareMetal можно с помощью [инструкции в официальной документации](../../baremetal/tutorials/bm-vrf-and-vpc-interconnect.md).
...
1. Арендуйте не менее трех серверов для будущего кластера Stackland.
...
Перед началом настройки зайдите на бастион через SSH с помощью публичного IP-адреса, указанного на странице сервера. Если сервер подключен к эфемерной публичной подсети, проверьте фактический адрес для подключения в параметрах публичного сетевого интерфейса сервера.

### 3.1 Настройка сетевых доступов {#net-settings}

Для обеспечения внешнего доступа к кластеру Stackland рекомендуется установить VPN-сервер, например, [OpenVPN](http://www.openvpn.net) или [Wireguard](https://www.wireguard.com). Чтобы подключиться к виртуальной сети, можно адаптировать и применить инструкцию из документации [Yandex Cloud VPC](../../vpc/tutorials/openvpn.md).
...
1. Обеспечьте анонс маршрутов до приватной подсети, созданной на шаге 1, клиентам, подключающимся по VPN. Это необходимо, чтобы сервисы в кластере Stackland были доступны через VPN-подключение. За подробностями обращайтесь к документации выбранного VPN-сервера.
...
- [Интерфейс командной строки Yandex Cloud](../../cli/index.md);
...
1. Перезагрузите зоны BIND командой:
    ...
    Перед перезагрузкой зон можно проверить конфигурацию командами:
    ...
    Если для зоны `stackland.internal` настроено делегирование на узлы будущего кластера, `named-checkzone` может вывести предупреждения о NS-записях без A/AAAA-записей в родительской зоне. Убедитесь, что имена узлов разрешаются в зоне `baremetal.internal`.
...
Конфигурационный файл для инсталлятора имеет формат, описанный в [Руководстве по установке](../quickstart.md#configuration). При развертывании на BareMetal следует обратить особое внимание на секцию сетевых настроек, поскольку сервера BareMetal имеют несколько сетевых адаптеров.
...
На этом процесс установки Stackland на Yandex BareMetal завершен. Описание дальнейших шагов по настройке кластера вы можете найти в соответствующих разделах [документации](../index.md).