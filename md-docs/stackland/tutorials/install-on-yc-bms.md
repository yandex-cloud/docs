...
1. Откройте форму создания приватной сети в сервисе {{ baremetal-full-name }} и активируйте переключатель **IP-адресация и маршрутизация**. Открыть форму можно по инструкции [Создать приватную подсеть](../../baremetal/operations/subnet-create.md).
...
IP-адреса, назначенные серверам по DHCP, могут изменяться. Описание работы DHCP см. в [документации сервиса {{ baremetal-full-name }}](../../baremetal/concepts/dhcp.md#dhcp-private).
...
1. Арендуйте сервер для бастиона, как описано в [документации](../../baremetal/operations/servers/server-lease.md).
    ...
    В качестве бастиона можно использовать виртуальную машину, запущенную в Yandex Cloud. Настроить сетевую связность между виртуальной машиной и серверами {{ baremetal-name }} можно с помощью [инструкции в официальной документации](../../baremetal/tutorials/bm-vrf-and-vpc-interconnect.md).
...
На этом процесс установки {{ stackland-name }} на {{ baremetal-full-name }} завершен. Описание дальнейших шагов по настройке кластера вы можете найти в соответствующих разделах [документации](../index.md).