...
### Инициировать синхронизацию License Server с облаком {#force-sync}

В штатном режиме синхронизация с облаком выполняется автоматически по расписанию, никаких действий от администратора не требуется. Если требуется не дожидаясь очередного цикла синхронизации получить обновленные лимиты или отправить накопленные данные о потреблении, можно использовать метод `ForceSync`. Чтобы вызвать метод, выполните запрос к административному API License Server:
...
curl -X POST \
  ...
  -d '{}' \
  'https://<адрес_license_server>:<порт>/api/v1/admin/sync'
```

Где:

* `<admin_token>` — Bearer-токен административного API. Задается в конфигурации License Server в параметре `security.admin_token`.
* `<адрес_license_server>:<порт>` — адрес и HTTP-порт License Server. По умолчанию порт `8080`.

В офлайн-режиме `ForceSync` перечитывает локальный файл лицензий с диска. В онлайн-режиме License Server дополнительно обращается к API Yandex Cloud: отправляет накопленные данные о потреблении и забирает обновленные лимиты.

Ответ содержит:

* `sync_status` — итоговый статус операции (`CLOUD_SYNC_STATUS_COMPLETED` или `CLOUD_SYNC_STATUS_FAILED`);
* `licenses_updated` — признак того, что лицензии были перечитаны;
* `usage_uploaded` — признак того, что данные о потреблении отправлены в облако (только в онлайн-режиме);
* `last_cloud_sync` — время завершения операции;
* `message` — пустая строка при успехе или текст ошибки.

После успешного завершения операции обновленные лимиты появятся в `PlatformConfig.status.licensing` при ближайшей синхронизации агента лицензирования с License Server.