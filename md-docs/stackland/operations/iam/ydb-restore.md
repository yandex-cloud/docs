[Документация Yandex Cloud](../../../index.md) > [Yandex Cloud Stackland](../../index.md) > [Пошаговые инструкции](../index.md) > Пользователи и группы > Восстановить YDB из резервной копии

# Восстановить YDB из резервной копии

Скрипт `ydb-restore.sh` восстанавливает базу данных [Identity and Access Management](../../concepts/components/iam.md) в YDB из архива, созданного с помощью [скрипта резервного копирования](ydb-backup.md).

Скрипт поддерживается в Stackland версии **26.3 и выше**.

{% note alert %}

Восстановление необратимо удаляет существующую YDB и все ее PVC. Изменения данных Identity and Access Management, сделанные после создания выбранной резервной копии, будут потеряны.
...
1. Подготовьте kubeconfig с административным доступом к восстанавливаемому кластеру. Доступ должен сохраняться при недоступности Identity and Access Management. Например, можно использовать kubeconfig с ролью `cluster-admin`, созданный [при установке кластера](../../quickstart.md#installation-common).
...
1. Проверьте состояние YDB и Identity and Access Management:
...
1. Войдите в консоль управления и убедитесь, что пользователи и настройки Identity and Access Management восстановлены из выбранной копии.
...
Если после ошибки в пространстве имен `stackland-iam` осталась политика `CiliumNetworkPolicy/ydb-restore-block-iam-uber`, не удаляйте ее вручную. Она блокирует обращения Identity and Access Management к восстанавливаемой базе данных. При повторном запуске скрипт переиспользует политику и удалит ее после восстановления данных.
