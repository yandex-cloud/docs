[Документация Yandex Cloud](../../../index.md) > [Yandex Cloud Stackland](../../index.md) > [Пошаговые инструкции](../index.md) > Пользователи и группы > Назначить права доступа
...
Для пользователя один ресурс `*AccessBinding` закрепляется за первым успешно
разрешённым IAM-пользователем. После появления `status.boundSubjectID` изменение
`spec.subject` не переносит роль другому пользователю. Чтобы сменить получателя,
удалите ресурс привязки и создайте новый. Неоднозначное имя можно исправить через
`subject.id` или `federationId` только до первого успешного назначения.
