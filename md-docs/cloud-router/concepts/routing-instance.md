[Документация Yandex Cloud](../../index.md) > [Yandex Cloud Router](../index.md) > [Концепции](index.md) > Виртуальный маршрутизатор

# Виртуальный маршрутизатор

Виртуальный маршрутизатор — это основной ресурс в ресурсной модели сервиса [Cloud Router](index.md), предназначенный для создания сетевых топологий, в рамках которых облачные ресурсы в разных [сетях](../../vpc/concepts/network.md) будут иметь сетевую связность между собой.

## Общие правила и рекомендации {#cr-general}

При использовании сервисов [Cloud Router](index.md) и [Cloud Interconnect](../../interconnect/concepts/index.md) настоятельно рекомендуется следовать следующим правилам и рекомендациям:

1. Во всех сегментах сети (как со стороны облака, так и со стороны On-Prem), которые будут объединяться с помощью виртуального маршрутизатора, должна использоваться согласованная и непересекающаяся IP-адресация. IP-префиксы [подсетей](../../vpc/concepts/network.md#subnet), которые добавляются в виртуальный маршрутизатор, должны быть согласованы между собой. Нельзя добавить два одинаковых IP-префикса в один виртуальный маршрутизатор. IP-адресация на стороне `On-Prem` должна быть согласована с IP-адресацией в виртуальных облачных сетях. Пересечение IP-адресации всегда будет приводить к нежелательным потерям IP-связности. **Тщательно планируйте IP-адресацию**.

1. Сетевое взаимодействие между [виртуальными сетями](../../vpc/concepts/network.md) организуется только в пределах одной облачной [организации](../../organization/concepts/organization.md). Все ресурсы сервисов Cloud Router и Cloud Interconnect, а также виртуальные облачные сети, которые будут объединяться в сетевую топологию с помощью виртуального маршрутизатора, должны размещаться в [облачных каталогах](../../resource-manager/concepts/resources-hierarchy.md#folder) в пределах одной облачной организации. При необходимости обеспечить сетевую связность между виртуальными облачными сетями, которые принадлежат разным облачным организациям, рекомендуется мигрировать облака из нескольких организаций в одну.
...
1. Прямой обмен маршрутной информацией между любыми [приватными соединениями](../../interconnect/concepts/priv-con.md) (PRC) в пределах виртуального маршрутизатора не допускается.

1. IP-префиксы, которые анонсирует сетевое оборудование клиента, также попадают в виртуальный маршрутизатор и далее в виртуальные сети, которые подключены к этому виртуальному маршрутизатору. В настоящее время посмотреть эти IP-префиксы на уровне виртуального маршрутизатора нельзя. Рекомендуется получать эту информацию от сетевого оборудования клиента. В отдельных случаях при решении проблем с маршрутизацией можно обратиться с запросом в [техническую поддержку](https://center.yandex.cloud/support) для получения такой информации.

1. [Идентификатор сети VPC](../../vpc/operations/network-get-info.md) — `vpc_network_id` можно одновременно добавить только в один виртуальный маршрутизатор. Добавить этот же идентификатор в другой виртуальный маршрутизатор не получится.

1. [Идентификатор приватного соединения](../../interconnect/operations/priv-con-get-info.md) — `cic_private_connection_id` можно одновременно добавить только в один виртуальный маршрутизатор. Добавить этот же идентификатор в другой виртуальный маршрутизатор не получится.


## Рекомендации по настройке приватных соединений {#prc} 

1. Всегда организовывайте два канала связи Cloud Interconnect через две [точки присутствия](../../interconnect/concepts/pops.md). Это поможет избежать отказов в работе ваших сервисов в случае отключения одного из каналов.
...
1. Если у клиента два канала связи Cloud Interconnect подключаются к разным маршрутизаторам, то между этими клиентскими маршрутизаторами необходимо дополнительно установить взаимодействие (связность) по протоколу **iBGP**. В случае отсутствия такой связности при выходе из строя одного из каналов связи на одном из маршрутизаторов соседний маршрутизатор ничего не узнает об этом отказе, и трафик из отказавшего канала не будет перемаршрутизирован на оставшийся в работе канал связи средствами протокола BGP.

1. При необходимости распределения исходящего трафика из облачных сетей по нескольким каналам связи в Cloud Interconnect используйте на клиентских маршрутизаторах префиксы разной длины для BGP-анонсов:
