...
[Проверьте](../operations/ri-get-info.md), есть ли в каталоге виртуальный маршрутизатор, который можно использовать для этого сценария.

Если подходящего маршрутизатора нет, [создайте его](../operations/ri-create.md). При создании:
...
Если вы используете существующий виртуальный маршрутизатор, [добавьте](../operations/ri-prefixes-upsert.md#add-network) в него сеть `sample-network` и IP-префикс `192.168.11.0/24` для зоны доступности `ru-central1-b`.
...
1. Если вы добавляли сеть `sample-network` в существующий виртуальный маршрутизатор, [удалите ее](../operations/ri-prefixes-upsert.md#remove-network).
1. Если вы создавали виртуальный маршрутизатор специально для этого руководства, [удалите его](../operations/ri-delete.md).
