...
Обработчик логов [Fluent Bit](https://fluentbit.io/) позволяет транслировать логи с [виртуальных машин](../../compute/concepts/vm.md), созданных из образов {{ coi }}, в сервис [{{ cloud-logging-full-name }}](../index.md). Для передачи логов используется модуль [Fluent Bit plugin for {{ cloud-logging-full-name }}](https://github.com/yandex-cloud/fluent-bit-plugin-yandex).
...
1. Создайте файл `spec.yaml`. Он описывает спецификацию двух контейнеров: с приложением, генерирующим логи, и агентом Fluent Bit.
   ...
   * `YC_GROUP_ID` — идентификатор [лог-группы по умолчанию](../concepts/log-group.md) `default`.
...
- API {#api}
  ...
  Посмотреть записи в лог-группе можно с помощью вызова gRPC API [LogReadingService/Read](../api-ref/grpc/LogReading/read.md).
