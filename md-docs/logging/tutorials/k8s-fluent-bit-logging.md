...
- Вручную {#manual}
  ...
  1. [Назначьте](../../iam/operations/sa/assign-role-for-sa.md#binding-role-resource) сервисному аккаунту для ресурсов роль [logging.writer](../security/index.md#logging-writer). Она необходима для отправки логов кластером Managed Service for Kubernetes в Cloud Logging.
...
- Terraform {#tf}
  ...
  1. Скачайте в ту же рабочую директорию файл конфигурации кластера Managed Service for Kubernetes [k8s-cluster-with-master-logging.tf](https://github.com/yandex-cloud-examples/yc-mk8s-cloud-logging/blob/main/k8s-cluster-with-master-logging.tf).
     ...
     * [Лог-группа](../concepts/log-group.md) Cloud Logging.
...
[Проверьте передачу логов](../operations/read-logs.md) мастера Managed Service for Kubernetes в лог-группу Cloud Logging.
...
* Сервис Cloud Logging: объем записываемых данных и время их хранения ([тарифы Cloud Logging](../pricing.md)).
...
- Вручную {#manual}
  ...
  1. Создайте сервисный аккаунт для Cloud Logging с ролями [logging.writer](../security/index.md#roles-list) и [monitoring.editor](../../monitoring/security/index.md#monitoring-editor). Он будет использоваться для работы Fluent Bit.
...
[Проверьте передачу логов](../operations/read-logs.md) подов и сервисов Managed Service for Kubernetes в лог-группу Cloud Logging.
