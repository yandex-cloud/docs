...
- Вручную {#manual}
  ...
  1. [Назначьте](../../iam/operations/sa/assign-role-for-sa.md#binding-role-resource) сервисному аккаунту для ресурсов роль [{{ roles-logging-writer }}](../security/index.md#logging-writer). Она необходима для отправки логов кластером {{ managed-k8s-name }} в {{ cloud-logging-name }}.
...
Чтобы настроить передачу логов [подов](../../managed-kubernetes/concepts/index.md#pod), [сервисов](../../managed-kubernetes/concepts/index.md#service) и системных логов [узлов](../../managed-kubernetes/concepts/index.md#node-group) [{{ managed-k8s-full-name }}](../../managed-kubernetes/index.md) в [{{ cloud-logging-full-name }}](../index.md):
...
* Плата за сервис {{ cloud-logging-name }}: запись и хранение данных (см. [тарифы {{ cloud-logging-name }}](../pricing.md)).
...
- Вручную {#manual}
  ...
  1. Создайте сервисный аккаунт для {{ cloud-logging-name }} с ролями [logging.writer](../security/index.md#roles-list) и [monitoring.editor](../../monitoring/security/index.md#monitoring-editor). Он будет использоваться для работы Fluent Bit.
...
- С помощью {{ marketplace-full-name }} {#marketplace}
  ...
  Установите Fluent Bit согласно [инструкции](../../managed-kubernetes/operations/applications/fluentbit.md#marketplace-install). В настройках приложения укажите идентификатор [созданной ранее](#before-you-begin) лог-группы. Идентификатор лог-группы можно получить со [списком лог-групп в каталоге](../operations/list.md).
...
- Вручную {#manual}
  ...
  1. Укажите идентификатор [созданной ранее](#before-you-begin) лог-группы и опционально идентификатор кластера в секции `[OUTPUT]` файла `config.yaml`:
     ...
     Идентификатор [лог-группы](../concepts/log-group.md) можно получить со [списком лог-групп в каталоге](../operations/list.md), идентификатор кластера — [со списком кластеров в каталоге](../../managed-kubernetes/operations/kubernetes-cluster/kubernetes-cluster-list.md#list).
...
- Вручную {#manual}
    ...
    1. Укажите идентификатор [созданной ранее](#before-you-begin) лог-группы в секциях `[OUTPUT]` файла `config.yaml`:
        ...
        Идентификатор [лог-группы](../concepts/log-group.md) можно получить со [списком лог-групп в каталоге](../operations/list.md).
...
[Проверьте передачу логов](../operations/read-logs.md) подов и сервисов {{ managed-k8s-name }} в лог-группу {{ cloud-logging-name }}.
