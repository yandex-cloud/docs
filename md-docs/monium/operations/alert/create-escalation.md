[Документация Yandex Cloud](../../../index.md) > [Monium](../../index.md) > Алерты > Пошаговые инструкции > Создание политики эскалаций
...
- Интерфейс Monium {#console}
  ...
  1. Выберите расписание эскалации:
     ...
     * **Пользовательская настройка** — отправка уведомлений по индивидуальному расписанию:
        ...
        * **Дни отправки** — дни недели, в которые эскалация будет отправлять уведомления. Чтобы отправлять уведомления каждый день, отметьте все дни или удалите эту настройку.
          ...
          У настройки **Не уведомлять в выходные и праздники** приоритет выше, чем у **Дни отправки**. Поэтому в выходные и праздничные дни уведомления не будут отправляться, независимо от выбранных дней отправки.
  ...
  1. Настройте шаги эскалации — последовательность каналов и параметры отправки уведомлений:
      ...
      * Укажите параметры первого шага эскалации:
        * **Метод** — [метод отправки уведомлений](../../concepts/alerting/notification-channel.md#channel-parameters):
        ...
        * **Игнорировать при повторе** — опция, позволяющая отправить уведомление на этом шаге эскалации только один раз, а при последующих повторах игнорировать этот шаг. Например, если вы хотите отправить только одно письмо о случившейся проблеме.
...
- Интерфейс Monium {#console}
  ...
  1. Добавьте метод отправки уведомлений и выберите в списке политику эскалаций.
...
- Интерфейс Monium {#console}
  ...
  * Перейдите в раздел ![alt](../../../_assets/console-icons/shield-exclamation.svg) **Алерты и SLO** → ![alt](../../../_assets/console-icons/triangle-thunderbolt.svg) **Лента уведомлений**.
