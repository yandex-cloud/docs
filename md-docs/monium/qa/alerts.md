[Документация Yandex Cloud](../../index.md) > [Monium](../index.md) > Вопросы и ответы > Алерты / уведомления
...
1. Проверьте получателей, метод отправки и статусы алерта, для которых включены уведомления, в [параметрах канала](../concepts/alerting/notification-channel.md#channel-parameters). Там же описаны требования к правам получателя и настройкам его аккаунта.
...
1. Проверьте [статус отправки](../concepts/alerting/notification-channel.md#dispatch-statuses). Статус `Ресурс исчерпан` означает, что достигнуто ограничение метода отправки. В Monium действует [лимит](../concepts/limits.md#limits-metrics-alerts) — 100 SMS на один номер в сутки.
