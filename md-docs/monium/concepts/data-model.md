...
* метрика может иметь не более 64 меток, включая обязательные `project`, `cluster`, `service`;
...
* значение метки должно удовлетворять регулярному выражению `^[ -!#-&(-)+->@-_a-{}-~]{1,1024}$`, от 1 до 1024 печатных ASCII-символов, кроме символов `` |*?"'\` ``.
