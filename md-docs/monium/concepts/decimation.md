[Документация Yandex Cloud](../../index.md) > [Monium](../index.md) > Метрики > Прореживание данных

# Прореживание данных

В Monium Metrics понятие _прореживание данных_ используется в двух случаях:
...
Все метрики в хранилище Monium Metrics старше недели агрегируются с интервалом 5 минут для уменьшения объема хранимых исторических данных. Параметры прореживания изменить нельзя.
...
Когда на графике или в запросе к Monium Metrics API запрошен достаточно широкий временной диапазон, из хранилища Monium Metrics потребуется прочитать большой объем данных. Это может привести к увеличению времени на выполнение запроса или ошибкам в отображении результатов на экране.

Для решения этих проблем используется прореживание данных.

Прореживание уменьшает количество точек, которые вернутся из хранилища Monium Metrics при чтении данных. А также выравнивает временные метки возвращаемых точек по определенной сетке. Дополнительно это помогает снизить нагрузку на диски и сеть.
