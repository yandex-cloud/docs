[Документация Yandex Cloud](../../../index.md) > [Monium](../../index.md) > Визуализация данных > Виджеты

# Виджет

_Виджет_ — это элемент дашборда, отображающий информацию в виде графика или текста.

В сервисе Monium Metrics можно добавить виджеты:
...
Monium Metrics позволяет отображать на графиках метрики сервисов Yandex Cloud и пользовательские метрики, загруженные с помощью API.
...
## Список алертов {#alert-list}

Виджет показывает на дашборде актуальное состояние алертов или под-алертов мультиалертов, найденных по селектору, в виде цветной плитки. На дашборд можно добавить алерты из любого проекта, к которому у вас есть доступ.

## SLO {#slo}

Виджет для отображения списка [SLO](../../slo/index.md) с актуальными значениями SLI и остатка Error Budget. При нажатии на название SLO откроется страница просмотра.
