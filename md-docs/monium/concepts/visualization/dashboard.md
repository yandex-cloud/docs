[Документация Yandex Cloud](../../../index.md) > [Monium](../../index.md) > Визуализация данных > Дашборд

# Дашборд в Monium

*Дашборд* — набор виджетов, которые вы можете группировать в удобном порядке, настраивать размер и добавлять пояснения. Дашборд позволяет следить за изменением метрик в реальном времени или анализировать накопленные метрики в динамике. Метрики на графиках дашборда отображаются за определенный временной интервал, общий для всех графиков.
...
Для более точной настройки интервала времени можно использовать временную шкалу над дашбордом.
...
Подробнее в разделе [Как добавить параметр дашборда](../../operations/dashboard/add-parameters.md).
...
Для всех типов параметров доступно указание *Значение по умолчанию* — это значение параметра будет использоваться при загрузке дашборда в веб-интерфейсе Monium Metrics.
...
* Подстановка значения метки в запросе.
    > В запросе `"cpu.iowait"{project="folder__aoe6mk1r3b47********", cluster="prod", service="{{myparm}}", host="*"}` в значение метки `service` будет подставлено значение параметра `myparm`.
...
* Фильтр `cluster=*prod*` ограничит значения параметра списком `prod`, `preprod-1` и `preprod-2`.
...
Функция повторения графиков по определенному параметру позволяет строить несколько аналогичных графиков, но с разными значениями этого параметра. Это помогает при исследовании аномалий и проблем, поскольку можно разделить одну метрику на несколько и изучить каждую в отдельности.

Например, чтобы посмотреть графики загрузки процессора для каждой виртуальной машины, вы можете настроить функцию повторения по параметру `host`. В результате вы получите отдельный график для каждой ВМ, что поможет локализовать проблему. Добавив параметр `cpu`, вы сможете построить графики загрузки каждого из процессоров на виртуальной машине.

Для виртуальных машин рекомендуется регулярно следить за такими параметрами, как использование процессора и памяти, ошибки при операциях с диском.

Настройка повторения графиков описана в разделе [Повторение графиков по параметрам](../../operations/dashboard/add-parameters.md#repeated-graphs).