...
Подробнее см. [Как добавить параметр дашборда](../../operations/dashboard/add-parameters.md).
...
* Подстановка значения метки в запросе.
    > В запросе `"cpu.iowait"{project="folder__aoe6mk1r3b47********", cluster="prod", service="not_var{{myparm}}", host="*"}` в значение метки `service` будет подставлено значение параметра `myparm`.
...
Настройку повторения графиков см. в разделе [Повторение графиков по параметрам](../../operations/dashboard/add-parameters.md#repeated-graphs).