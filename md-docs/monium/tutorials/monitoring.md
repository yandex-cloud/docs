[Документация Yandex Cloud](../../index.md) > [Monium](../index.md) > Метрики > [Практические руководства](index.md) > Мониторинг показаний датчиков и уведомления о событиях
...
* плата за запись пользовательских [метрик](../../monitoring/concepts/data-model.md#metric) через API сервиса [Monium Metrics](../../monitoring/index.md) ([тарифы Monium Metrics](../../monitoring/pricing.md)).
...
- Консоль управления {#console}
  ...
  1. Создайте версию функции:
      ...
      1. В блоке **Переменные окружения** укажите:
          ...
          `METRICS_FOLDER_ID` | Идентификатор каталога, в котором развернуты сервисы и для которого вы создадите дашборд в сервисе Monium Metrics. | Идентификатор можно узнать в [консоли управления](https://console.yandex.cloud).
...
- Консоль управления {#console}
  ...
  Все данные от устройства автоматически попадут в сервис **Monium Metrics**.

{% endlist %}

## Настройте мониторинг показаний датчиков {#configure-monitoring}

Для наблюдения за показаниями датчиков используется дашборд в сервисе [Monium Metrics](../../monitoring/index.md). Показания датчиков поступают с контроллера на сервер раз в минуту по [MQTT-протоколу](../../glossary/mqtt-server.md#mqtt-protocol). Когда показания датчиков достигают заданных значений, Monium Metrics отправляет уведомления пользователям.
...
- Интерфейс Monium Metrics {#console}
  ...
  1. В [консоли управления](https://console.yandex.cloud) выберите каталог, в котором вы выполняете руководство.
  1. [Перейдите](https://monium.yandex.cloud) в сервис **Monitoring**.
...
Вы можете создавать и настраивать алерты на любую метрику в сервисе Monium Metrics.
...
* [Удалите дашборд в сервисе Monium Metrics](../../monitoring/operations/dashboard/delete-dashboard.md)
* [Удалите алерт в сервисе Monium Metrics](../../monitoring/operations/alert/delete-alert.md)
* Удалите [канал уведомлений](../../monitoring/concepts/alerting/notification-channel.md) в сервисе Monium Metrics.