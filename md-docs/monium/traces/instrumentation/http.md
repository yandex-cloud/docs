[Документация Yandex Cloud](../../../index.md) > [Monium](../../index.md) > [Трейсы](../index.md) > Инструментация приложений > HTTP-взаимодействия
...
Если клиент не использует шаблонизированные URL, имя спана содержит только метод. Если шаблон известен — он включается в имя через атрибут `url.template`. Серверный спан включает шаблонизированный маршрут (`/users/:user_id`), а не конкретный путь (`/users/321`). Подробнее в [спецификации именования HTTP-спанов](https://opentelemetry.io/docs/specs/semconv/http/http-spans/#name).
