[Документация Yandex Cloud](../../../index.md) > [Monium](../../index.md) > [Трейсы](../index.md) > Инструментация приложений > Повторные попытки

# Повторные попытки

При повторных попытках (retry) каждая попытка оформляется отдельным дочерним спаном. Это позволяет видеть в трейсе, сколько попыток потребовалось, какие из них завершились ошибкой и какая — успехом.

## Паттерн разметки {#pattern}

Родительский спан охватывает всю операцию. Каждая попытка — отдельный дочерний спан с тем же именем. Неудачные попытки получают статус `ERROR`, успешная — `OK`. Атрибут `http.resend_count` указывает порядковый номер повторной попытки (начиная с `1` для первого ретрая).

## Атрибуты {#attributes}

#|
|| Атрибут | Описание ||
|| `http.resend_count` | Порядковый номер повторной попытки. Отсутствует у первого запроса, `1` — у первого ретрая, `2` — у второго и т. д. ||
|#

Атрибуты самих запросов (HTTP, RPC и др.) заполняются по конвенциям соответствующего типа операции — подробнее в разделах [HTTP-взаимодействия](http.md) и [RPC-вызовы](rpc.md).

## Пример {#example}

Сервис `payment-service` обрабатывает платеж и запрашивает курс валюты у внешнего сервиса. Первый запрос завершается ошибкой, повторная попытка — успехом:

#|
|| Атрибут | `process_payment` | `GET` (попытка 1) | `GET` (попытка 2) ||
|| Kind | `SERVER` | `CLIENT` | `CLIENT` ||
|| Status | `OK` | `ERROR` | `OK` ||
|| `http.resend_count` | — | — | `1` ||
|#