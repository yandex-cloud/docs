...
* _Метрики_ — числовые показатели, измеряемые во времени (например, RPS, загрузка CPU). Используются для графиков и оповещений (алертов).
    ...
    [Метрики ресурсов](metrics-ref/index.md) {{ yandex-cloud }} передаются в {{ monium-name }} автоматически. Также многие сервисы предоставляют [сервисные дашборды](visualization/index.md#service-dashboards) с набором готовых виджетов, которые отображают состояние ваших облачных ресурсов.
...
Для сбора метрик {{ prometheus-name }} поддерживается интеграция через [{{ managed-prometheus-name }}](operations/prometheus/index.md).
