...
Все метрики сервиса [{{ monitoring-name }}](../../monitoring/index.md) имеют общую метку `service=monitoring`. Метрики могут поступать от {{ monitoring-short-name }} API и {{ managed-prometheus-name }} API.
