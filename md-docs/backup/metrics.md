...
В этом разделе описаны метрики сервиса {{ backup-name }}, поставляемые в [{{ monitoring-name }}](../monitoring/index.md).
...
|| resource_id | Идентификатор [виртуальной машины](../compute/concepts/vm.md) в [{{ compute-name }}](../compute/index.md) или [сервера {{ baremetal-name }}](../baremetal/concepts/servers.md) в [{{ baremetal-full-name }}](../baremetal/index.md). ||
...
|| `agent_alive`
`DGAUGE` | Индикатор активности [агента {{ backup-name }}](concepts/agent.md). Возможные значения:
* `1` — агент работает.
