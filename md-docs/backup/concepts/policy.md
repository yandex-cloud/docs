[Документация Yandex Cloud](../../index.md) > [Yandex Cloud Backup](../index.md) > [Концепции](index.md) > Политики резервного копирования

# Политики резервного копирования


Резервные копии [защищаемых ресурсов](index.md#protected-resources) создаются в Cloud Backup автоматически в соответствии с _политиками резервного копирования_.

[Создание](../operations/policy-vm/create.md) политик доступно после [активации](index.md#providers) сервиса Cloud Backup.

{% note info %}

Создать или [изменить](../operations/policy-vm/update.md) политику можно с помощью консоли управления Yandex Cloud, Terraform, а также задать по [спецификации](#specification) в формате [JSON](https://ru.wikipedia.org/wiki/JSON), используя [CLI](../../cli/quickstart.md) Yandex Cloud или API.
...
По умолчанию защищаемые ресурсы Cloud Backup не связаны ни с одной политикой резервного копирования. Чтобы начать создавать резервные копии, [привяжите](vm-connection/index.md) ваш ресурс к одной или нескольким политикам.

В политике вы указываете:

* Тип создаваемых копий: полные или инкрементальные. Подробнее в разделе [Типы копий](backup.md#types).
...
* Тип и настройки расписания:

    * `Фиксированное расписание` — периодичность создания копий в часах, днях, неделях или месяцах. Время указывается в часовом поясе [UTC±00:00](https://ru.wikipedia.org/wiki/UTC±00:00).
    * `Интервал между копированием` — интервал в часах или днях между завершением предыдущего копирования и началом нового.

* Настройки [хранения резервных копий](#retention).

{% note info %}

Резервные копии создаются по локальному времени, установленному на [защищаемом ресурсе](index.md#protected-resources). Возможно небольшое отставание от расписания в зависимости от текущей нагрузки на сервис.

{% endnote %}

## Хранение копий {#retention}

Для политики можно настроить хранение копий. Для каждого ресурса, привязанного к политике, могут храниться:
...
Настройки действуют на все защищаемые ресурсы, привязанные к политике.
...
Если вы вносите изменения в правила хранения резервных копий, то по умолчанию они вступят в силу после создания очередной резервной копии. Подробное описание параметров настройки правил хранения резервных копий читайте в следующем разделе.

## Спецификация политики резервного копирования {#specification}

Политику резервного копирования в Cloud Backup можно [создать](../operations/policy-vm/create.md) или [изменить](../operations/policy-vm/update.md) по спецификации в формате [JSON](https://ru.wikipedia.org/wiki/JSON) с помощью [командной строки](../../cli/quickstart.md) Yandex Cloud или API-запроса.

Полная спецификация политики резервного копирования в Cloud Backup представлена ниже. Также актуальная версия спецификации доступна на странице [справочника REST API](../backup/api-ref/Policy/create.md).
...
| `preserveFileSecuritySettings` | Сохранять настройки безопасности файлов. **Параметр больше не поддерживается.** | <ul><li>`true` — включено;</li><li>`false` — выключено.</li></ul> |
| `runLater` | Если во время запланированного резервного копирования защищаемый ресурс был в статусе `Stopped`, все пропущенные задания резервного копирования выполнятся после запуска этого ресурса. | <ul><li>`true` — включено;</li><li>`false` — выключено.</li></ul> |
| `sectorBySector` | Посекторное резервное копирование. Создает резервную копию всех секторов диска или тома, включая пустые области и нераспределенное пространство. Для дисков с неподдерживаемыми файловыми системами режим применяется автоматически. Данные приложений из такой копии восстановить нельзя. | <ul><li>`true` — включено;</li><li>`false` — выключено.</li></ul> |
| `validationEnabled` | Проверка возможности восстановления данных из созданной резервной копии. При проверке для каждого доступного для восстановления блока вычисляется контрольная сумма, поэтому она может занимать длительное время. | <ul><li>`true` — включено;</li><li>`false` — выключено.</li></ul> |
| `lvmSnapshottingEnabled` | Для создания моментального [снимка](../../glossary/snapshot.md) тома будет использоваться LVM. Если создать снимок с помощью LVM не удастся, он будет создан с помощью агента Cloud Backup. | <ul><li>`true` — включено;</li><li>`false` — выключено.</li></ul> |
...
| `quiesceSnapshottingEnabled` | Использовать режим `quiescing` при создании резервных копий. **Параметр больше не поддерживается.** | <ul><li>`true` — включено;</li><li>`false` — выключено.</li></ul> |
...
- prePostCommands
  ...
  Настройка действий, которые Cloud Backup будет выполнять перед началом резервного копирования и после его завершения.

  | Атрибут | Описание | Пример значения |
  |---|---|---|
  | `cmd` | Команда или путь к исполняемому файлу, которые требуется выполнить (запустить). | `/usr/bin/myapp` |
  | `args` | Параметры командной строки, которые будут применяться при выполнении команды (запуске файла). | `-d -rw` |
  | `enabled` | Включить или выключить выполнение команды (запуск файла). | <ul><li>`true` — включено;</li><li>`false` — выключено.</li></ul> |
  | `stopOnError` | Остановить создание резервной копии при возникновении ошибки выполнения команды (запуска файла). | <ul><li>`true` — останавливать резервное копирование;</li><li>`false` — не останавливать резервное копирование.</li></ul> |
  | `type` | Момент выполнения команды (запуска файла). | <ul><li>`COMMAND_TYPE_UNSPECIFIED` — не задано;</li><li>`PRE_COMMAND` — выполнять команду (запускать файл) перед началом резервного копирования;</li><li>`POST_COMMAND` — выполнять команду (запускать файл) после завершения резервного копирования;</li><li>`PRE_DATA_COMMAND` — выполнять команду (запускать файл) непосредственно перед захватом данных (созданием снимка);</li><li>`POST_DATA_COMMAND` — выполнять команду (запускать файл) непосредственно после захвата данных (создания снимка).</li></ul> В каждую политику вы можете добавить не более одной команды каждого типа: `PRE_COMMAND`, `POST_COMMAND`, `PRE_DATA_COMMAND` и `POST_DATA_COMMAND`.<br><br>Команды `PRE_DATA_COMMAND` и `POST_DATA_COMMAND` выполняются максимально близко к моменту создания снимка, в отличие от `PRE_COMMAND` и `POST_COMMAND`, которые выполняются в начале и в конце всего процесса резервного копирования. |
  | `wait` | Дожидаться завершения выполнения команды (запуска файла), прежде чем запускать резервное копирование. | <ul><li>`true` — дожидаться завершения выполнения;</li><li>`false` — не дожидаться завершения выполнения.</li></ul> |
  | `workdir` | Рабочая директория выполнения команды (запуска файла). | `/etc/myapp/` |
...
* [Создать политику резервного копирования](../operations/policy-vm/create.md)
* [Привязать защищаемый ресурс к политике резервного копирования](../operations/policy-vm/attach-and-detach-vm.md)
* [Отвязать защищаемый ресурс от политики резервного копирования](../operations/policy-vm/detach-vm.md)
* [Изменить политику резервного копирования](../operations/policy-vm/update.md)
* [Получить информацию о политике резервного копирования](../operations/policy-vm/get-info.md)