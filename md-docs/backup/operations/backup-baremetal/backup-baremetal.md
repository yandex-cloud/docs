...
Информацию о том, как подключить сервер {{ baremetal-name }} к {{ backup-name }} при его заказе, см. в инструкции [{#T}](lease-server-with-backup.md).
...
- Консоль управления {#console}
  ...
  1. Выберите политику, к которой вы хотите привязать сервер.
      ...
      При необходимости [создайте](../policy-vm/create.md) новую политику резервного копирования.
...
Чтобы избежать ошибок при восстановлении из резервной копии, перед восстановлением рекомендуется сопоставить параметры дисков и разделов резервной копии с параметрами дисков и разделов [ВМ](../../../compute/concepts/vm.md) или [сервера](../../../baremetal/concepts/servers.md) {{ baremetal-full-name }}. Подробнее см. в разделе [{#T}](../backup-vm/view-disk-layout.md).
...
1. [Удалите](../backup-vm/delete.md) резервную копию в {{ backup-name }} с помощью CLI.

#### См. также {#see-also}

* [{#T}](lease-server-with-backup.md)