...
Создание [политики](../../concepts/policy.md) доступно после [активации](../../concepts/index.md#providers) сервиса Yandex Cloud Backup.
...
Резервные копии создаются по локальному времени, установленному на [защищаемом ресурсе](../../concepts/index.md#protected-resources). Возможно небольшое отставание от расписания в зависимости от текущей нагрузки на сервис.
...
- Консоль управления {#console}
  ...
  1. На странице создания политики резервного копирования:
      ...
      * Выберите [тип резервных копий](../../concepts/backup.md#types):
          ...
          * `Всегда полные` — сохраняются все данные [защищаемого ресурса](../../concepts/index.md#protected-resources).
      ...
      * В блоке **Настройки хранения** задайте политику [хранения копий](../../concepts/policy.md#retention):
        ...
        * `Только последнюю` — укажите количество последних копий, которые нужно хранить, или количество дней, недель или месяцев, копии за которые нужно хранить. Остальные копии будут удаляться автоматически.
            ...
            При создании политики с помощью консоли управления последующие изменения настроек хранения резервных копий применяются только после создания очередной резервной копии. Если изменения не вступили в силу, то [отвяжите](detach-vm.md) политику от защищаемого ресурса и затем [привяжите](attach-and-detach-vm.md) ее заново.
      
            {% endnote %}

      * Разверните блок **Дополнительно** и в открывшейся форме:

          * (Опционально) Включите опцию **Запуск пропущенных заданий после загрузки**, чтобы выполнять все пропущенные задания резервного копирования после запуска ресурса, если во время запланированного резервного копирования защищаемый ресурс был выключен или к нему не было сетевого доступа. Доступно только для инкрементальных копий.
