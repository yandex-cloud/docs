...
После установки агента ВМ будет добавлена в сервис **Cloud Backup** на вкладку **Виртуальные машины** раздела **Подключённые ресурсы**, и ее можно будет [привязать](../operations/policy-vm/attach-and-detach-vm.md) к [политике резервного копирования](../concepts/policy.md) Cloud Backup.
...
1. Убедитесь, что агент Cloud Backup установлен:
   ...
   - Консоль управления {#console}
     ...
     1. Убедитесь, что в блоке **Резервное копирование** в поле **Статус подключения** отображается значение `Нет политики`.
