[Документация Yandex Cloud](../../index.md) > [Yandex Cloud Billing](../index.md) > [Концепции](index.md) > Порог оплаты

# Порог оплаты

Порог оплаты — это отрицательный [баланс лицевого счета](personal-account.md#balance). Порог оплаты представляет собой реализацию постоплатной модели использования Yandex Cloud. Другими словами — это граница, после которой Yandex Cloud имеет право:
...
- [типа платежного аккаунта](billing-account.md#ba-types);
- объема потребляемых ресурсов;
- вашей финансовой надежности.

Информация о размере порога оплаты представлена в [сервисе Yandex Cloud Billing](https://center.yandex.cloud/billing/accounts) на странице ![image](../../_assets/console-icons/flag.svg) **Аккаунт** в правом верхнем блоке вместе с информацией о текущем балансе.
...
Порог оплаты не может быть отключен и будет действовать до [удаления платежного аккаунта](../operations/delete-account.md). Для изменения порога оплаты создайте [новое обращение в поддержку](https://center.yandex.cloud/support).

{% endnote %}

## Использование порога оплаты {#using}

Порог оплаты используется только при отсутствии [грантов](bonus-account.md) и только если [баланс лицевого счета (ЛС)](personal-account.md#balance) равен нулю. При использовании порога оплаты у вас появляется задолженность — отрицательный баланс ЛС. 
...
- Физические лица {#individuals}
  ...
  Отслеживать состояние [баланса лицевого счета](personal-account.md#balance) можно в [сервисе Yandex Cloud Billing](https://center.yandex.cloud/billing/accounts) на странице ![image](../../_assets/console-icons/flag.svg) **Аккаунт**:
