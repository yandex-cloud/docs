...
1. Создайте эндпоинты и трансфер:
    ...
    Избегайте любых изменений в схеме данных на кластере-источнике и кластере-приемнике во время работы трансфера. Подробнее см. в разделе [Работа с базами данных во время трансфера](../../data-transfer/operations/db-actions.md).
...
1. После успешного запуска репликации однократно выполните команду:
   ...
   Это необходимо, чтобы при смене мастера в кластере-источнике репликация корректно перенастроилась на новый хост-мастер. Подробнее о настройке см. в [документации MySQL®](https://dev.mysql.com/doc/refman/8.0/en/change-master-to.html).
...
Подробнее о статусе репликации см. в [документации MySQL®](https://dev.mysql.com/doc/refman/8.0/en/replication-administration-status.html).
