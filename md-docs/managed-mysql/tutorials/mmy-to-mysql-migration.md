...
Миграция данных из стороннего кластера {{ MY }} описана в статье [{#T}](data-migration/index.md).
...
* Установите на хосты кластера-приемника [серверные SSL-сертификаты {{ mmy-name }}](../operations/connect/index.md#get-ssl-cert). Они требуются для подключения к публично доступному кластеру-источнику.
...
1. Создайте дамп базы кластера-источника:
   ...
   Используйте [особый FQDN](../concepts/network.md#fqdn-master), который всегда указывает на текущий хост-мастер в кластерах {{ mmy-name }}.
...
1. После успешного запуска репликации однократно выполните команду:
   ...
   Это необходимо, чтобы при смене мастера в кластере-источнике репликация корректно перенастроилась на новый хост-мастер. Подробнее о настройке см. в [документации {{ MY }}](https://dev.mysql.com/doc/refman/8.0/en/change-master-to.html).
