...
Подробнее об устройстве кластера Managed Service for MySQL® смотрите в разделе [Взаимосвязь ресурсов сервиса](../concepts/index.md).
...
- Консоль управления {#console}
  ...
  1. В блоке **Сетевые настройки** выберите:
     ...
     * [Группы безопасности](../../vpc/concepts/security-groups.md) для сетевого трафика кластера. Может потребоваться дополнительная [настройка групп безопасности](connect/index.md#configure-security-groups) для подключения к кластеру Managed Service for MySQL®.
...
- CLI {#cli}
  ...
  1. Укажите параметры кластера Managed Service for MySQL® в команде создания:
     ...
     * `--datalens-access` — разрешает доступ к кластеру из Yandex DataLens. Значение по умолчанию — `false`. Подробнее о настройке подключения смотрите в разделе [Подключение к кластеру MySQL® из DataLens](datalens-connect.md).
...
- Terraform {#tf}
  ...
  1. Опишите в конфигурационном файле параметры ресурсов, которые необходимо создать:
     ...
     * Чтобы настроить [автоматическое увеличение размера хранилища](../concepts/storage.md#disk-size-autoscaling), добавьте к описанию кластера блок `disk_size_autoscaling`:
       ...
       resource "yandex_mdb_mysql_cluster" "<имя_кластера>" {
         ...
         disk_size_autoscaling {
           disk_size_limit           = <максимальный_размер_хранилища_в_ГБ>
     ...
     * Чтобы активировать сбор статистики для [диагностики производительности кластера](performance-diagnostics.md), добавьте к описанию кластера Managed Service for MySQL® блок `performance_diagnostics`:
     ...
     Более подробную информацию о ресурсах, которые вы можете создать с помощью Terraform, читайте в [документации провайдера](../../terraform/resources/mdb_mysql_cluster.md).
  ...
  1. Создайте файл `body.json` и добавьте в него следующее содержимое:
      ...
      * `configSpec` — настройки кластера:
          ...
          * `access` — настройки доступа к кластеру из сервисов Yandex Cloud:
            ...
            * `dataLens` — доступ из Yandex DataLens. Подробнее о настройке подключения смотрите в разделе [Подключение к кластеру MySQL® из DataLens](datalens-connect.md).
  ...
  1. Создайте файл `body.json` и добавьте в него следующее содержимое:
      ...
      * `config_spec` — настройки кластера:
          ...
          * `access` — настройки доступа к кластеру из сервисов Yandex Cloud:
            ...
            * `data_lens` — доступ из Yandex DataLens. Подробнее о настройке подключения смотрите в разделе [Подключение к кластеру MySQL® из DataLens](datalens-connect.md).
