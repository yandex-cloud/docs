...
- CLI {#cli}
  ...
  **Чтобы восстановить кластер из резервной копии:**
  ...
  1. Если вы планируете восстанавливать кластер из конкретной резервной копии, получите список доступных резервных копий кластеров MySQL®:
...
- Terraform {#tf}
  ...
  * Кластера, созданного и удаленного через Консоль управления, CLI или API.
  ...
  Для восстановления может понадобиться идентификатор резервной копии. Получите список доступных резервных копий кластеров MySQL® [с помощью CLI](#list-backups):
  ...
  **Чтобы восстановить существующий кластер:**
  ...
  1. Добавьте в этот конфигурационный файл блок `restore`, используя один из вариантов:
      ...
      * Чтобы восстановить кластер из конкретной резервной копии, укажите ее идентификатор в параметре `backup_id`:
        ...
        В параметре `time` укажите момент времени в формате `yyyy-mm-ddThh:mm:ss`, на который нужно восстановить состояние кластера MySQL®.
