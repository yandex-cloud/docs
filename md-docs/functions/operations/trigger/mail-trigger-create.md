[Документация Yandex Cloud](../../../index.md) > [Yandex Cloud Functions](../../index.md) > [Пошаговые инструкции](../index.md) > Создание триггера > Создать триггер для почты

# Создать триггер для почты, который вызывает функцию Cloud Functions

Создайте [триггер для почты](../../concepts/trigger/mail-trigger.md), который будет вызывать [функцию](../../concepts/function.md) Cloud Functions, когда на электронную почту приходит письмо. Адрес электронной почты генерируется сервисом автоматически во время создания триггера.
...
- Terraform {#tf}
    ...
    1. Опишите в конфигурационном файле параметры триггера:
       ...
       * `source` — параметры источника событий:
         ...
         * `mail` — параметры почтового триггера:
           ...
           * `attachments_bucket` — параметры бакета, в который будут сохраняться вложения из писем. Необязательный блок:
               ...
               * `service_account_id` — идентификатор сервисного аккаунта, у которого есть права на загрузку объектов в бакет Object Storage.
       ...
       * `mail` — параметры триггера:
           ...
           * `service_account_id` — идентификатор сервисного аккаунта, у которого есть права на загрузку объектов в бакет Object Storage. Необязательный параметр.
...
Cloud Functions автоматически сгенерирует адрес электронной почты, при отправке писем на который будет запускаться триггер. Чтобы посмотреть его, [получите подробную информацию о триггере](trigger-list.md#trigger-get).
...
* [Создать триггер для почты, который вызывает контейнер Serverless Containers](../../../serverless-containers/operations/mail-trigger-create.md)
* [Создать триггер для почты, который отправляет сообщения в WebSocket-соединения](../../../api-gateway/operations/trigger/mail-trigger-create.md)