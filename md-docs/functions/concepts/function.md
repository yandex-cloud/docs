...
Файл больше 3,5 МБ необходимо [загружать через Object Storage](../../storage/operations/objects/upload.md). Подробнее об ограничениях читайте в разделе [Квоты и лимиты в Cloud Functions](limits.md).

{% endnote %}

Подробнее о том, как создать версию, читайте в разделе [Создать версию функции](../operations/function/version-manage.md).
...
_Контекст_ обеспечивает взаимодействие кода вашей функции с Cloud Functions. Например, с его помощью функция может узнать, сколько времени осталось до момента, когда Cloud Functions завершит ее выполнение.

Поступающие запросы обрабатываются функцией по одному. Среда выполнения доступна для функции, пока функция обрабатывает запросы. После того как функция вернула ответ, продолжение работы этого экземпляра функции не гарантируется. Прекращение соединения со стороны клиента всегда приводит к завершению обработки вызова функцией.
...
Чтобы уменьшить количество холодных стартов, Cloud Functions создает экземпляры функции в фоновом режиме, не для обработки пользовательских запросов. При этом количество экземпляров, созданных таким образом, не превышает [квоты](limits.md#functions-quotas).

Кроме этого, в Cloud Functions можно задать следующие настройки масштабирования:
...
* `provisioned_instances_count` — количество [подготовленных экземпляров](function.md#provisioned-instances);
...
Чтобы один экземпляр функции в один момент времени мог обрабатывать несколько ее вызовов, можно задать параметр `concurrency` при создании версии функции.
...
Когда экземпляр функции обрабатывает одновременно несколько вызовов, в логи записывается идентификатор только последнего из них. Чтобы в логи записывались идентификаторы всех вызовов, которые обрабатывала функция, используйте [структурировананные логи](logs.md#structured-logs).

Если хотя бы один вызов достигнет таймаута, он и все остальные вызовы, которые обрабатываются тем же экземпляром функции, завершатся. Подробнее о таймауте в [Лимиты](limits.md#functions-limits).
...
* [Запуск вычислений в Yandex DataSphere с помощью API](../tutorials/batch-code-execution.md)
