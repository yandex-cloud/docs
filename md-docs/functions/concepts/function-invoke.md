...
Результат, который возвращает функция, также должен представлять собой JSON-документ, содержащий код ответа HTTP, заголовки ответа и содержимое ответа. Cloud Functions автоматически обработает этот JSON, и пользователь получит данные в виде стандартного HTTPS-ответа.
...
* `requestContext` содержит следующую структуру: 
    ...
    ```json
    {
        ...
        "requestId": "<Идентификатор запроса, генерируется в роутере>",
...
* `body` — содержимое запроса в виде строки. Данные могут быть закодированы в формате Base64 (в этом случае Cloud Functions установит параметр `isBase64Encoded: true`).
...
curl \
  --request POST \
  --data "hello, world!" \
  "https://functions.yandexcloud.net/<идентификатор_функции>?a=1&a=2&b=1"
```
...
* `413 Payload Too Large` — превышение [лимита](limits.md#limits) на размер JSON-структуры запроса превышает 3,5 МБ.
* `429 TooManyRequests` — слишком высокая интенсивность вызова функции: 
    * Превышение [квоты](limits.md#functions-quotas) на количество выполняемых запросов.
...
### Вызов без специальных заголовков в ответе {#without-x-headers}

HTTP-ответ по умолчанию содержит специальные заголовки, которые начинаются с `X-`, например `X-Request-Id`. Чтобы удалить эти заголовки из ответа, используйте параметр `customHeaderPolicy`.

Возможные значения параметра:

* `stripAll` — удалить специальные заголовки при любом ответе.
* `stripOnSuccess` — удалить специальные заголовки только в случае успешного ответа функции.
...
curl -i \
  --request POST \
  --data "hello, world!" \
  "https://functions.yandexcloud.net/<идентификатор_функции>?customHeaderPolicy=stripAll"
```
...
* `-d, --data` — данные передаются как аргумент.
    ...
    ```bash
    yc serverless function invoke <идентификатор_функции> -d '{"queryStringParameters": {"parameter_name": "parameter_value"}}'
    ```

* `--data-file` — данные читаются из файла.

    ```bash
    yc serverless function invoke <идентификатор_функции> --data-file <путь_к_файлу>
    ```

    Аналогично команде с аргументом `-d`, имеющим значение `@<имя файла>`: `yc serverless function invoke <идентификатор_функции> -d @<путь_к_файлу>`

* `--data-stdin` — данные читаются из потока ввода. 

     ```bash
     echo '{"queryStringParameters": {"parameter_name": "parameter_value"}}' | yc serverless function invoke <идентификатор_функции> --data-stdin
     ```

    Аналогично команде с аргументом `-d`, имеющим значение `@-`: 
    
    ```bash
    echo '{"queryStringParameters": {"parameter_name": "parameter_value"}}' | yc serverless function invoke <идентификатор_функции> -d @-`.
    ```
...
Пример функции для вызова внешнего API при срабатывании алерта приведен в разделе [Создание канала уведомлений с вызовом функции](../../monitoring/operations/alert/create-channel-function.md).