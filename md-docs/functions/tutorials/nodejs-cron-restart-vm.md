...
В этом руководстве вы создадите [функцию](../concepts/function.md) [{{ sf-full-name }}](../index.md) на языке [Node.js](../lang/nodejs/index.md), которая будет вызываться по расписанию и запускать [прерываемую ВМ](../../compute/concepts/preemptible-vm.md) {{ compute-full-name }}, если ВМ остановлена.
...
- Консоль управления {#console}
  ...
  1. В [консоли управления]({{ link-console-main }}) перейдите в каталог, в котором хотите создать [триггер](../concepts/trigger/index.md).
  1. Перейдите в сервис **{{ ui-key.yacloud.iam.folder.dashboard.label_serverless-functions }}**.
