...
Пользовательская интеграция представляет собой [функцию](../concepts/function.md) {{ sf-full-name }} или [контейнер](../../serverless-containers/concepts/container.md) {{ serverless-containers-full-name }}, которые предназначены для решения типовой задачи.
