...
В этом руководстве вы создадите агент со стримингом ответа через [веб-сокеты](https://{{ lang }}.wikipedia.org/wiki/WebSocket) на [{{ sf-full-name }}](../index.md) и [{{ api-gw-full-name }}](../../api-gateway/index.md). Функция будет использовать [OpenAI Agents SDK](https://openai.github.io/openai-agents-python/) для работы с моделями [{{ ai-studio-full-name }}]({{ link-docs-ai }}ai-studio/concepts/generation/index#yandex).
