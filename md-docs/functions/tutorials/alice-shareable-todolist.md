...
Данные проекта хранятся в бакете [{{ objstorage-full-name }}](../../storage/index.md) и базе данных [{{ ydb-full-name }}](../../ydb/index.md). Функции [{{ sf-full-name }}](../index.md) обрабатывают запросы, а [{{ api-gw-full-name }}](../../api-gateway/index.md) обеспечивает взаимодействие сервисов.
...
* плата за количество вызовов функций, вычислительные ресурсы, выделенные для выполнения функций, и исходящий трафик (см. [тарифы {{ sf-full-name }}](../pricing.md));
