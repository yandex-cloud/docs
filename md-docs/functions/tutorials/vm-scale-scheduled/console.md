[Документация Yandex Cloud](../../../index.md) > [Yandex Cloud Functions](../../index.md) > [Практические руководства](../index.md) > Рабочие процессы и автоматизация на Serverless > [Масштабирование группы виртуальных машин по расписанию](index.md) > Консоль управления, CLI и API
...
- Консоль управления {#console}
  ...
  1. В блоке **Шаблон виртуальной машины** нажмите **Задать** и в открывшемся окне:
     ...
     * В блоке **Доступ**:
       ...
       * В поле **SSH-ключ** вставьте содержимое файла открытого ключа [SSH](../../../glossary/ssh-keygen.md). Создать пару ключей можно по [инструкции](../../../compute/operations/vm-connect/ssh.md#creating-ssh-keys).
     * Нажмите **Сохранить**.
...
- API {#api}
  ...
  1. Создайте функцию `vm-scale-scheduled-function` с помощью метода REST API [create](../../functions/api-ref/Function/create.md) для ресурса [Function](../../functions/api-ref/Function/index.md) или вызова gRPC API [FunctionService/Create](../../functions/api-ref/grpc/Function/create.md). Из ответных данных получите идентификатор созданной функции.
...
[Триггер](../../concepts/trigger/index.md) определяет условия запуска функции. Для запуска по расписанию используется [триггер-таймер](../../concepts/trigger/timer.md).
...
- API {#api}
  ...
  Используйте метод REST API [create](../../triggers/api-ref/Trigger/create.md) для ресурса [Trigger](../../triggers/api-ref/Trigger/index.md) или вызов gRPC API [TriggerService/Create](../../triggers/api-ref/grpc/Trigger/create.md), чтобы создать триггер типа `timer` с cron-выражением `*/2 * * * ? *`, связанный с функцией `vm-scale-scheduled-function` версии `$latest` и сервисным аккаунтом `vm-scale-scheduled-sa`.
...
- Консоль управления {#console}
  ...
  1. Убедитесь, что в блоке **Виртуальные машины** каждые две минуты изменяется количество ВМ: увеличивается с 2 до 3, затем уменьшается с 3 до 2 и т. д. Также вы можете проверить обновление группы на вкладке **Операции**.
...
1. [Удалите](../../operations/trigger/trigger-delete.md) триггер.
