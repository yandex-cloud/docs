...
Организовать сценарии регулярных запусков в [{{ ml-platform-full-name }}]({{ link-datasphere-main }}) можно с помощью API, вызывая исполнение ячеек ноутбука в [{{ sf-full-name }}](../index.md).
...
* плата за количество вызовов функции [{{ sf-name }}](../pricing.md).
