...
В этом руководстве вы настроите передачу событий [{{ postbox-full-name }}](../../postbox/index.md) в [{{ yds-full-name }}](../../data-streams/index.md) и вызов [вебхуков](https://ru.wikipedia.org/wiki/Webhook) во внешних системах. Для обработки и хранения данных будут использоваться сервисы [{{ sf-full-name }}](../index.md) и [{{ ydb-full-name }}](../../ydb/index.md).
...
* `yds-functions` — от его имени будет вызываться [функция](../concepts/function.md) {{ sf-name }} и будут записываться события в [поток данных](../../data-streams/concepts/glossary.md#stream-concepts) {{ yds-short-name }};
...
Создайте [функцию](../concepts/function.md), которая будет отправлять вебхук, а также [триггер](../concepts/trigger/index.md), который будет вызывать эту функцию при регистрации событий в потоке данных.
...
1. Убедитесь, что данные об отправке писем поступают в сервисы:
    ...
    * Посмотрите [логи функции](../operations/function/function-logs.md) и [графики мониторинга триггера](../operations/trigger/trigger-monitoring.md).
