...
Реализовать предлагаемое решение можно с помощью [консоли управления]({{ link-console-main }}) {{ yandex-cloud }}, даже не имея навыков разработки. Для обработки и хранения данных будут использоваться сервисы [{{ sf-full-name }}](../index.md) и [{{ ydb-full-name }}](../../ydb/index.md).
...
* `yds-functions` — от его имени будет вызываться [функция](../concepts/function.md) {{ sf-name }} и будут записываться данные в [базу данных](../../ydb/concepts/resources.md#database) {{ ydb-short-name }};
...
1. [Удалите](../operations/trigger/trigger-delete.md) триггер для вызова функции {{ sf-name }}.
1. [Удалите](../operations/function/function-delete.md) функцию {{ sf-name }}.
