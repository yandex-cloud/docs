...
Для логирования работы балансировщика будет создана [лог-группа](../../logging/concepts/log-group.md) в {{ cloud-logging-name }}. Поставка логов из этой лог-группы в БД будет настроена с помощью ресурсов сервиса {{ sf-full-name }} — [триггера](../concepts/trigger/cloud-logging-trigger.md) и вызываемой им [функции](../concepts/function.md).
...
* Функции {{ sf-full-name }}: количество вызовов функции, время простоя подготовленных экземпляров и выделенные для выполнения функции вычислительные ресурсы (см. [тарифы {{ sf-full-name }}](../pricing.md)).
