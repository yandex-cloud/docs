...
[Триггер для бюджетов](../concepts/trigger/budget-trigger.md) будет вызывать [функцию](../concepts/function.md) {{ sf-name }}, а функция — останавливать [виртуальные машины](../../compute/concepts/vm.md#project) {{ compute-name }} и отправлять сообщение в [очередь](../../message-queue/concepts/queue.md) {{ message-queue-full-name }}. 

[Триггер для {{ message-queue-name }}](../concepts/trigger/ymq-trigger.md) будет передавать поступившие в очередь сообщения во вторую функцию {{ sf-name }}, которая будет отправлять уведомления в Telegram с помощью специально созданного бота.
...
- API {#api}
  ...
  Чтобы создать триггер для {{ message-queue-name }}, воспользуйтесь методом [create](../triggers/api-ref/Trigger/create.md) для ресурса [Trigger](../triggers/api-ref/Trigger/index.md) или вызовом gRPC API [TriggerService/Create](../triggers/api-ref/grpc/Trigger/create.md).
...
1. [Удалите](../operations/trigger/trigger-delete.md) триггеры {{ sf-name }}.
1. [Удалите](../operations/function/function-delete.md) функции {{ sf-name }}.
