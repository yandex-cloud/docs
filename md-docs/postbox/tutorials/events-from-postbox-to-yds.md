...
В этом руководстве вы настроите передачу событий [{{ postbox-full-name }}](../index.md) в [{{ yds-full-name }}](../../data-streams/index.md) и визуализацию этих событий с помощью [{{ datalens-full-name }}](../../datalens/index.md) для последующего анализа. Источник событий — отправка электронных писем.
...
В сервисе {{ postbox-name }} настройте [конфигурацию](../concepts/glossary.md#subscription) для регистрации событий в {{ yds-name }}, создайте [адрес](../concepts/glossary.md#adress) для отправки писем и [пройдите](../operations/check-domain.md) проверку прав на домен.
...
- Консоль управления {#console}
  ...
  1. В блоке **{{ ui-key.yacloud.postbox.label_event-destinations }}** нажмите кнопку **{{ ui-key.yacloud.common.add }}** и в появившейся форме:
      ...
      1. В поле **{{ ui-key.yacloud.postbox.label_name }}** укажите название [подписки](../concepts/glossary.md#subscription) `postbox-events-subscribe`.
...
- Расширенная настройка {#advanced}
    ...
    При расширенной настройке вы самостоятельно [генерируете ключ](../operations/create-address.md) для создания DKIM-подписи. На странице адреса в блоке **{{ ui-key.yacloud.postbox.section_dkim }}** отображается одна TXT-запись, которую нужно добавить в DNS-провайдер.
...
1. Удобным вам способом [отправьте](../operations/send-email.md#send-email) несколько тестовых писем с использованием адреса {{ postbox-name }} и статического ключа доступа, созданных ранее.
    ...
    - Консоль управления {#console}
      ...
      * Проверьте БД:
          ...
          1. Выберите таблицу `postbox_events`.
              ...
              В таблице должны появиться записи. В некоторых столбцах может быть значение `NULL` — это зависит от [типа уведомления](../concepts/notification.md#types), которое было получено от сервиса {{ postbox-name }}.
...
1. [Удалите](../operations/index.md) адрес и конфигурацию {{ postbox-name }}.
