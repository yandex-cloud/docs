...
* Простая настройка (Easy DKIM) — Yandex Cloud Postbox автоматически генерирует ключи DKIM и управляет их ротацией. После создания [адреса](glossary.md#address) в консоли управления отобразятся две CNAME-записи, которые нужно добавить в DNS-зону домена. Эти записи указывают на публичные ключи, которыми управляет сервис. Подробнее в разделе [Ротация ключей DKIM](#key-rotation).
...
#### Ротация ключей DKIM {#key-rotation}

При простой настройке DKIM-подписи Yandex Cloud Postbox периодически заменяет пары DKIM-ключей на новые. Ротация выполняется автоматически: управлять расписанием или запускать ее вручную нельзя.

Чтобы Yandex Cloud Postbox мог заменять ключи, пользователь должен добавить две CNAME-записи в свой DNS-провайдер. Каждая из них делегирует сервису право публиковать публичный ключ. Благодаря этому Yandex Cloud Postbox заменяет ключи на своей стороне, и пользователю не нужно править CNAME-записи в DNS-провайдере.

TXT-записи с публичными ключами доступны одновременно по обеим CNAME-записям только в момент ротации ключей: новый ключ уже опубликован, а старый еще нужен, чтобы получатели могли проверить подписи ранее отправленных писем. В остальное время доступна одна TXT-запись.
