...
Приходит, когда почтовый сервер получателя на попытку доставки отвечает ошибкой, которую Yandex Cloud Postbox считает не требующей повторной попытки доставки, или адрес получателя находится в одном из [стоп-листов](suppression-list.md).
