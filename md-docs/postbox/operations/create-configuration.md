[Документация Yandex Cloud](../../index.md) > [Yandex Cloud Postbox](../index.md) > [Пошаговые инструкции](index.md) > Создание конфигурации

# Создание конфигурации

Создайте [конфигурацию](../concepts/configuration.md), чтобы настроить правила обработки писем:
...
- Консоль управления {#console}
    ...
    1. Чтобы получать уведомления об операциях с письмами, в блоке **Подписки** нажмите **Добавить**:
    ...
    1. Чтобы отправлять письма от Yandex Cloud Postbox до серверов получателя только с использованием TLS-шифрования, в блоке **Настройки доставки** включите опцию **Доставлять письмо только по защищённому соединению** и укажите минимальную версию TLS.
    ...
    1. Чтобы проверять [пользовательский стоп-лист](../concepts/suppression-list.md#user) при отправке писем получателям, в блоке **Настройки стоп-листа** включите опцию **Проверять список** и в поле **Тип исключения** выберите причины. На адреса, добавленные в стоп-лист по указанным причинам, не будут отправляться письма. Если настройки стоп-листа не задать, отправку будет блокировать только причина `COMPLAINT`. Подробнее в разделе [Причины добавления адреса в пользовательский стоп-лист](../concepts/suppression-list.md#reasons).
...
- AWS CLI {#aws-cli}
    ...
    1. Чтобы получать уведомления об операциях с письмами, добавьте [подписку](../concepts/glossary.md#subscription) в конфигурацию:
...
Чтобы применять конфигурацию ко всем письмам, которые отправляются с адреса, [привяжите](bind-configuration.md) конфигурацию к адресу.