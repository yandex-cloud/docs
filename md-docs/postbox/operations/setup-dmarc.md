...
Чтобы защитить домен от подделки отправителей и повысить безопасность почтовых сообщений, необходимо настроить [DMARC-политику](https://{{ lang }}.wikipedia.org/wiki/DMARC). Настройки вашей доменной зоны должны содержать TXT-запись `_dmarc.<домен>.`. Вы можете добавить запись у вашего регистратора или в сервисе [{{ dns-full-name }}](../../dns/index.md), если вы делегировали ваш домен.
