...
Для управления репозиториями с кодом {{ yandex-cloud }} предоставляет сервис [{{ mgl-full-name }}](https://yandex.cloud/ru/services/managed-gitlab/).
{{ mgl-name }} позволяет настроить развертывание приложений на [виртуальных машинах](../compute/concepts/vm.md) в [{{ compute-full-name }}](https://yandex.cloud/ru/services/compute/), а также [поддерживает](../managed-gitlab/concepts/index.md) интеграцию с [{{ container-registry-full-name }}](https://yandex.cloud/ru/services/container-registry/) и [{{ managed-k8s-full-name }}](https://yandex.cloud/ru/services/managed-kubernetes/).
