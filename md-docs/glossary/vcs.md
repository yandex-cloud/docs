[Документация Yandex Cloud](../index.md) > [Облачная терминология](index.md) > Управление проектами и задачами > Система контроля версий

# Система контроля версий

_Система контроля версий (СКВ, VCS)_ — это один из ключевых инструментов разработки, позволяющий наладить непрерывную интеграцию и развертывание ПО ([CI/CD](ci-cd.md)) в процессе работы над проектом. СКВ фиксирует все изменения в проекте, позволяет сохранять различные его версии и при необходимости вернуться к нужному состоянию. Это защищает исходный код от непреднамеренного удаления или повреждения. Также система контроля версий помогает организовать удобную командную работу над проектом, делая процесс более гибким и безопасным.
...
1. Создание _пул-реквеста_ (_Pull Request_) — запроса на сохранение в родительской ветке изменений, которые были внесены в рабочую ветку. В пул-реквесте другие разработчики могут обсудить правки, добавить свои коммиты при необходимости, принять или отклонить предложенные изменения.
1. _Мерж_ (_Merge_) веток — рабочую ветку вливают в родительскую. Теперь изменения внесены и доступны всем пользователям.
...
Для управления репозиториями с кодом Yandex Cloud предоставляет сервис [Yandex Managed Service for GitLab](https://yandex.cloud/ru/services/managed-gitlab/).
Managed Service for GitLab позволяет настроить развертывание приложений на [виртуальных машинах](../compute/concepts/vm.md) в [Yandex Compute Cloud](https://yandex.cloud/ru/services/compute/), а также [поддерживает](../managed-gitlab/concepts/index.md) интеграцию с [Yandex Container Registry](https://yandex.cloud/ru/services/container-registry/) и [Yandex Managed Service for Kubernetes](https://yandex.cloud/ru/services/managed-kubernetes/).
