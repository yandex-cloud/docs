...
Внедрить спринт в работу очень просто с помощью [{{ tracker-full-name }}]({{ link-tracker }}). Благодаря этому сервису использовать спринт будет просто и удобно. Воспользуйтесь [пошаговой инструкцией]({{ link-tracker-cloudless }}manager/create-agile-sprint), чтобы создать и настроить спринт.