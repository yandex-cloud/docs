...
| **Пример использования** | Получение списка ВМ [{{ compute-full-name }}](https://yandex.cloud/ru/services/compute/) | Платежная система уведомляет интернет-магазин об оплате заказа |
...
* Message Queue (очередь сообщений) — механизм, который встраивается в систему как посредник, хранящий и передающий события от сервиса к клиенту. Например, [{{ message-queue-full-name }}](https://yandex.cloud/ru/services/message-queue/).
...
* [{{ mgl-full-name }}](https://yandex.cloud/ru/services/managed-gitlab/) — сервис для управления платформой разработки GitLab в инфраструктуре {{ yandex-cloud }}. Подробнее в [документации](../managed-gitlab/index.md).
...
* [{{ sf-full-name }}](https://yandex.cloud/ru/services/functions/) — сервис для создания и управления функциями, которые можно использовать в качестве вебхуков. Подробнее в [документации](../functions/index.md).
* [{{ postbox-full-name }}](https://yandex.cloud/ru/services/postbox/) — сервис для отправки почтовых сообщений, которые можно отслеживать с помощью вебхуков. Подробнее в [документации](../tutorials/serverless/postbox-webhook.md).
* [{{ monitoring-full-name }}](https://yandex.cloud/ru/services/monitoring/) — сервис для сбора метрик и отслеживания работы приложений, события из которого можно получать с помощью вебхуков. Подробнее в [документации](../monitoring/operations/alert/alert-call-function.md).
* [{{ api-gw-full-name }}](https://yandex.cloud/ru/services/api-gateway/) — сервис для создания и управления API-шлюзами, которые можно использовать в качестве вебхуков. Подробнее в [документации](../serverless-integrations/tutorials/telegram-ai-bot-workflows.md).
