[Документация Yandex Cloud](../index.md) > [Облачная терминология](index.md) > Аналитика > Бизнес-аналитика
...
Одним из таких инструментов, которым активно пользуется бизнес, является сервис [Yandex DataLens](https://datalens.ru/promo). С его помощью можно подключаться к различным источникам данных, строить графики и диаграммы, создавать дашборды и делиться полученными результатами.

В DataLens доступны разные типы подключений — CSV-файлы, MySQL, Oracle Database, ClickHouse®, PostgreSQL. На базе  данных из подключения Yandex DataLens формирует датасет. Датасет описывает набор данных и его структуру. Данные визуализируются на основе датасетов в чартах.
...
[Зарегистрируйтесь](https://console.yandex.cloud/) в Yandex Cloud и начните пользоваться DataLens уже сегодня.

Подробнее в [документации](../datalens/index.md).
