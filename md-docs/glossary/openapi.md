...
* `paths` — доступные эндпоинты и методы, которые через них вызываются. Каждой HTTP-операции должен быть присвоен путь. Подробнее см. раздел [HTTP-методы](#http-methods).
* `components` — многократно используемые компоненты, которые встречаются в разных частях спецификации. Подробнее см. раздел [Описание переиспользуемых объектов](#components).
...
* `security` — требования авторизации для операций. Подробнее см. раздел [Безопасность и аутентификация](#security).
...
Полноценная API-платформа | <ul><li>[Yandex API Gateway](https://yandex.cloud/ru/services/api-gateway)</li><li>[Stoplight](https://stoplight.io/)</li></ul>
...
Yandex Cloud предлагает [Yandex API Gateway](https://yandex.cloud/ru/services/api-gateway) — полностью управляемый сервис для разработки, развертывания, управления и обеспечения безопасности API-шлюзов. Среди основных возможностей сервиса:
...
Подробнее о сервисе см. в [документации](../api-gateway/index.md).

#### Полезные материалы {#see-also}

* [Создание интерактивного приложения с использованием Yandex API Gateway и протокола WebSocket](https://yandex.cloud/ru/blog/posts/2023/04/yandex-api-gateway-and-websocket)
* [Распределение нагрузки с помощью Yandex API Gateway и других инструментов балансировки](https://yandex.cloud/ru/blog/posts/2024/04/api-gateway-or-yandex-alb)
* [Разграничение доступа к API с помощью Yandex API Gateway](https://yandex.cloud/ru/blog/posts/2023/07/yandex-api-gateway-and-jwt-authorizer)