...
С помощью gRPC вы можете обращаться к большинству ресурсов {{ yandex-cloud }}. Например, создание бакета с заданными параметрами (имя, каталог размещения, класс хранения, макисмальный размер и др.) в [{{ objstorage-full-name }}](../storage/index.md) можно инициировать вызовом метода `BucketService/Create`.
