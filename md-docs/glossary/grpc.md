...
С помощью gRPC вы можете обращаться к большинству ресурсов Yandex Cloud. Например, создание бакета с заданными параметрами (имя, каталог размещения, класс хранения, макисмальный размер и др.) в [Yandex Object Storage](../storage/index.md) можно инициировать вызовом метода `BucketService/Create`.
...
Инфраструктурой для таких решений является облачная платформа Yandex Cloud. Ознакомьтесь с [возможностями](../overview/index.md) платформы и [основными принципами](../api-design-guide/concepts/general.md) работы API.
