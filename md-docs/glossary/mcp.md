[Документация Yandex Cloud](../index.md) > [Облачная терминология](index.md) > Искусственный интеллект > MCP-сервер
...
1. **Клиент** — компонент хоста, который отправляет запросы серверу по протоколу MCP.
...
* **Интеграция с базами данных**
  ...
  * Кешировать результаты запросов, чтобы модель могла получить их без повторного обращения к базе данных.
...
| Аспект | MCP-сервер | Бессерверные API |
|--------|------------|------------------|
