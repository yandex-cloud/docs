[Документация Yandex Cloud](../index.md) > [Облачная терминология](index.md) > Базы данных > Data Lake
...
Yandex Cloud предлагает следующие инструменты для работы с озерами данных:
   * Сервис [Managed Service for Apache Kafka®](https://yandex.cloud/ru/services/managed-kafka/) — работа с кластерами в режиме реального времени. Преимущество платформы — высокая пропускная способность и низкая задержка. Подробнее в [документации](../managed-kafka/index.md).
   * Сервис [Yandex Data Processing](../data-proc/index.md), который автоматически создает кластеры, настраивает сеть, устанавливает и обновляет ПО:
