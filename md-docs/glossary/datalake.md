...
{{ yandex-cloud }} предлагает следующие инструменты для работы с озерами данных:
   * Сервис [{{ mkf-name }}](https://yandex.cloud/ru/services/managed-kafka/) — работа с кластерами в режиме реального времени. Преимущество платформы — высокая пропускная способность и низкая задержка. Подробнее в [документации](../managed-kafka/index.md).
   * Сервис [{{ dataproc-name }}](../data-proc/index.md), который автоматически создает кластеры, настраивает сеть, устанавливает и обновляет ПО:
