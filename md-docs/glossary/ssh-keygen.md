[Документация Yandex Cloud](../index.md) > [Облачная терминология](index.md) > Безопасность > Протокол SSH
...
Например, если вы работаете с сервисом [Yandex Compute Cloud](https://yandex.cloud/ru/services/compute/) с помощью консоли управления, при [создании виртуальной машины Linux](../compute/operations/vm-create/create-linux-vm.md) содержимое открытого ключа нужно скопировать в поле **SSH-ключ**. Если вы создаете виртуальную машину с помощью команды (например, через интерфейс командной строки CLI), то в качестве одного из параметров указывается расположение файла с публичным ключом, и в процессе выполнения команды этот ключ копируется в виртуальную машину.
