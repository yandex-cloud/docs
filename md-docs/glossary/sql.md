...
В инфраструктуре {{ yandex-cloud }} существует множество сервисов, при работе с которыми используется SQL:
  ...
  * [{{ websql-full-name }}](https://yandex.cloud/ru/services/websql/) — сервис для работы с публично доступными БД.
  
    Подробнее в [документации {{ websql-name }}](../websql/index.md).

  * [{{ dataproc-full-name }}](https://yandex.cloud/ru/services/data-proc/) — платформа для обработки больших данных (например, Apache Spark, Hadoop).

    Подробнее в [документации {{ dataproc-name }}](../data-proc/index.md).

  * [{{ datalens-full-name }}](https://yandex.cloud/ru/services/datalens/) — инструмент для визуализации данных и бизнес-аналитики.
    
    Подробнее в [документации {{ datalens-name }}](../datalens/index.md).

  * [{{ yds-full-name }}](https://yandex.cloud/ru/services/data-streams/) — сервис для обработки потоковых данных.

    Подробнее в [документации {{ yds-name }}](../data-streams/index.md).

  * [{{ data-transfer-full-name }}](https://yandex.cloud/ru/services/data-transfer/) — инструмент для переноса данных между системами и сервисами. SQL-запросы могут применяться для настройки.

    Подробнее в [документации {{ data-transfer-name }}](../data-transfer/index.md).

  * [{{ mch-full-name }}](https://yandex.cloud/ru/services/managed-clickhouse/) — fully managed аналитическая база данных для быстрой обработки больших объемов данных.

    Подробнее в [документации {{ mch-short-name }}](../managed-clickhouse/index.md).

  * [{{ mmy-name }}](https://yandex.cloud/ru/services/managed-mysql/) — управляемая база данных {{ MY }}.

    Подробнее в [документации {{ mmy-short-name }}](../managed-mysql/index.md).

  * [{{ mpg-name }}](https://yandex.cloud/ru/services/managed-postgresql/) — управляемая база данных {{ PG }}.

    Подробнее в [документации {{ mpg-short-name }}](../managed-postgresql/index.md).

  * [{{ ydb-full-name }}](https://yandex.cloud/ru/services/ydb/) — распределенная современная база данных, предназначенная для масштабируемых приложений и хранения данных с высокой доступностью.

    Подробнее в [документации {{ ydb-name }}](../ydb/index.md).
  
  * [{{ mspqr-full-name }}](https://yandex.cloud/ru/services/managed-spqr/) — сервис для работы с кластерами серверов разных СУБД со встроенным шардированием.
    
    Подробнее в [документации {{ mspqr-name }}](../managed-spqr/index.md).
