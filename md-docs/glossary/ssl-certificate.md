[Документация Yandex Cloud](../index.md) > [Облачная терминология](index.md) > Безопасность > SSL-сертификат
...
* {#wildcard-certificate}**Wildcard Certificate** — действует также на поддомены ресурса.
