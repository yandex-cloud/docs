...
Например, [{{ objstorage-full-name }}](https://yandex.cloud/ru/services/storage/) — универсальный облачный сервис для хранения данных, который поддерживает кросс-доменные запросы к ресурсам в каждом бакете. Подробнее в [документации](../storage/index.md).
