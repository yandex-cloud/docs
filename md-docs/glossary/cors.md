[Документация Yandex Cloud](../index.md) > [Облачная терминология](index.md) > Сети и доставка контента > CORS
...
При создании перекрестного HTTP-запроса браузер клиента добавляет в него объявление домена веб-страницы, инициирующей запрос. Домен объявляется в заголовке Origin.
...
В случае, если сервер по адресу `www.server-site.ru` разрешает странице-источнику запроса доступ к данным с домена, в его ответе на запрос появится CORS-заголовок `Access-Control-Allow-Origin` с именем объявленного домена:
...
Если заголовок Origin отсутствует в запросе браузера клиента, сервер не возвращает CORS-заголовки в ответе.
...
Например, [Yandex Object Storage](https://yandex.cloud/ru/services/storage/) — универсальный облачный сервис для хранения данных, который поддерживает кросс-доменные запросы к ресурсам в каждом бакете. Подробнее в [документации](../storage/index.md).
