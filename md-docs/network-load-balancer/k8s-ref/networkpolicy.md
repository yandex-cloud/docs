[Документация Yandex Cloud](../../index.md) > [Yandex Network Load Balancer](../index.md) > [Справочник инструментов для Managed Service for Kubernetes](index.md) > NetworkPolicy
...
`NetworkPolicy` — стандартный ресурс Kubernetes. В этом справочнике описаны поля и аннотации ресурса, которые поддерживают Network Load Balancer для Managed Service for Kubernetes. Полный справочник ресурса приведен в [документации Kubernetes](https://kubernetes.io/docs/reference/kubernetes-api/policy-resources/network-policy-v1/).

Подробнее о сетевых политиках в разделе [Сетевые политики кластера Kubernetes](../../managed-kubernetes/concepts/network-policy.md).
...
Читайте [документацию Kubernetes](https://kubernetes.io/docs/reference/kubernetes-api/common-definitions/label-selector/#LabelSelector)
||
...
Диапазоны `198.18.235.0/24` и `198.18.248.0/24` указываются всегда, так как они [зарезервированы Network Load Balancer](../concepts/health-check.md#target-statuses) для проверки состояния узлов
||
|#