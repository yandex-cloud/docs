...
* Нагрузка генерируется приложением [Load Testing Tool](https://yandex.cloud/ru/marketplace/products/yc/load-testing) из Yandex Cloud Marketplace и подается на [Yandex Network Load Balancer](../index.md). Сетевой балансировщик нагрузки распределяет трафик по ВМ.
