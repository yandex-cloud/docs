...
* Нагрузка генерируется приложением [{{ load-testing-name }} Tool](https://yandex.cloud/ru/marketplace/products/yc/load-testing) из {{ marketplace-full-name }} и подается на [{{ network-load-balancer-full-name }}](../index.md). Сетевой балансировщик нагрузки распределяет трафик по ВМ.
