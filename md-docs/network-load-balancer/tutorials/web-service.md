...
- Консоль управления {#console}
    ...
    1. В блоке **Правила** [создайте](../../vpc/operations/security-group-add-rule.md) правила в соответствии с таблицей:
       ...
       || Входящий | `udp500` | `500` | `UDP` | `Диапазон адресов` | `<публичный_IP-адрес_удаленной_ВМ>/32` ||
       || Входящий | `udp4500` | `4500` | `UDP` | `Диапазон адресов` | `<публичный_IP-адрес_удаленной_ВМ>/32` ||
       || Входящий | `internal` | `0-65535` | `Любой` | `Диапазон адресов` | 
         * `192.168.5.0/24`
         * `192.168.15.0/24`
         * `192.168.25.0/24`
         * `10.129.0.0/24` ^1^ ||
       || Исходящий | `udp500` | `500` | `UDP` | `Диапазон адресов` | `<публичный_IP-адрес_удаленной_ВМ>/32` ||
       || Исходящий | `udp4500` | `4500` | `UDP` | `Диапазон адресов` | `<публичный_IP-адрес_удаленной_ВМ>/32` ||
       || Исходящий | `intersubnet` | `0-65535` | `Любой` | `Диапазон адресов` | 
         * `192.168.5.0/24`
         * `192.168.15.0/24`
         * `192.168.25.0/24`
         * `10.129.0.0/24` ^1^ ||
...
1. [Добавьте](../../vpc/operations/security-group-add-rule.md) в группу безопасности `web-service-sg` два новых правила для входящего трафика:
   ...
   #|
   || **Описание** | **Диапазон портов** | **Протокол** | **Источник /<br/>Назначение** | **IPv4 CIDR** ||
