...
* С помощью консоли управления, CLI, API. Подробнее см. [Запустить рабочий процесс](../../operations/workflows/execution/start.md).
* По таймеру. Расписание задается в формате [cron-выражения](../cron.md). Подробнее см. [Запустить рабочий процесс по таймеру](../../operations/workflows/execution/start-by-timer.md).
* По ссылке через HTTPS. Подробнее см. [Запустить рабочий процесс по ссылке](../../operations/workflows/execution/start-by-link.md).
* С помощью API-шлюза. Подробнее см. [Добавить расширение x-yc-apigateway-integration:http для интеграции с Yandex Workflows](../../../api-gateway/operations/spec-constructor/workflows.md).
* С помощью Yandex EventRouter. Для этого рабочий процесс нужно указать в качестве приемника в [правиле](../eventrouter/rule.md). Подробнее о том, как создать правило, см. [Создать правило с приемником Yandex Workflows](../../operations/eventrouter/rule/create-workflows.md). Правило определяет события, при которых должен запуститься рабочий процесс. Тело события передается в запуск как входной параметр.
...
* публичный, его может запустить любой пользователь. Подробнее см. [Сделать рабочий процесс публичным](../../operations/workflows/workflow/make-public.md).
...
`STEP_DATA_LIMIT_EXCEEDED` | Превышен лимит входных или выходных данных. Подробнее см. [Квоты и лимиты в Serverless Integrations](../limits.md).
`STEP_NO_CHOICE_MATCHED` | Нет подходящего пути выполнения в `choices`. Подробнее см. [Switch](yawl/management/switch.md).
`STEP_PERMISSION_DENIED` | Нет доступа к ресурсу.
`STEP_TIMEOUT` | Превышен таймаут шага. Подробнее см. [Квоты и лимиты в Serverless Integrations](../limits.md).
...
`STEP_FAIL` | Запуск завершился с ошибкой на шаге `Fail`. Подробнее см. [Fail](yawl/management/fail.md).
...
`FUNCTION_CALL_INVALID_RESPONSE` | Невалидный код функции или формат возвращаемого JSON-ответа. Подробнее см. [Возможные коды ответа Cloud Functions](../../../functions/concepts/function-invoke.md#http-state).
