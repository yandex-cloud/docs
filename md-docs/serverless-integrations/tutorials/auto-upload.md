[Документация Yandex Cloud](../../index.md) > [Yandex Serverless Integrations](../index.md) > [Практические руководства](index.md) > Автоматическая загрузка данных в Yandex SpeechSense с помощью Workflows
...
1. Когда в папке появляются новые файлы, триггер вызывает [функцию](../../functions/concepts/function.md) `workflow-call`, которая запускает [рабочий процесс Workflows](../concepts/workflows/workflow.md).
