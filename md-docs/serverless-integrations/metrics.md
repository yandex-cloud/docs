...
В этом разделе описаны метрики сервиса {{ si-name }}, поставляемые в [{{ monitoring-name }}](../monitoring/index.md).
...
Все метрики {{ er-name }} имеют общую метку `bus` — идентификатор [шины](concepts/eventrouter/bus.md).
...
Имя метрики<br>Тип, единицы измерения | Описание<br>Метки
--- | ---
`serverless_eventrouter_bus_put_event_per_second`<br>`DGAUGE`, сообщения/с | Частота отправки сообщений с помощью вызова gRPC API [EventService/Put](eventrouter/api-ref/grpc/Event/put.md).
...
`serverless_eventrouter_put_event_latency_milliseconds`<br>`DGAUGE`, миллисекунды | Время от получения сообщения, отправленного с помощью вызова gRPC API [EventService/Put](eventrouter/api-ref/grpc/Event/put.md), до записи в шину.


### Метрики коннекторов {#er-connectors}

Все метрики [коннекторов](concepts/eventrouter/connector.md) имеют общую метку `connector` — идентификатор коннектора.
...
Все метрики [правил](concepts/eventrouter/rule.md) имеют общие метки:

Метка | Значение
--- | ---
matched | Соответствие условиям, заданным в [фильтре](concepts/eventrouter/rule.md#filter).
...
Все метрики [приемников](concepts/eventrouter/rule.md#target) имеют общие метки:
...
`serverless_eventrouter_target_throttled_per_second`<br>`DGAUGE`, сообщения/с | Частота возникновения сообщений, не обработанных из-за превышения [квоты или лимита](concepts/limits.md#event-router).