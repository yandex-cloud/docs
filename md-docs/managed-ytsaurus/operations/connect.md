...
Если для кластера заданы ограничения входящего трафика, подключайтесь из интернета с IP-адреса, который входит в список разрешенных диапазонов. Ограничения не влияют на доступ из Virtual Private Cloud. Чтобы изменить список разрешенных диапазонов, [измените настройки кластера](cluster-update.md).
