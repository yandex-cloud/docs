[Документация Yandex Cloud](../../../index.md) > [Yandex Identity and Access Management](../../index.md) > [Концепции](../index.md) > [Аутентификация](index.md) > API-ключ
...
При [просмотре списка](../../operations/authentication/manage-api-keys.md#list-api-keys) API-ключей отображаются последние шесть символов секретной части ключа. В Yandex Cloud CLI они показаны в поле `MASKED KEY`, в API — в поле `maskedSecret`, а в консоли управления — в столбце **API-ключ**.

Это позволяет сопоставить API-ключ с его идентификатором. Например, для проверки наличия ключа в базах утечек или при сопоставлении ключей в других системах с ключами в Yandex Cloud.

Демонстрация шести символов безопасна, она не раскрывает ключ полностью.
