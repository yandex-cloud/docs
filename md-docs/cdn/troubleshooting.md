...
Чтобы этого избежать, [включите сжатие файлов](operations/resources/enable-compression.md). Тогда {{ cdn-short-name }} будет всегда запрашивать из источника контент без сжатия, а при запросе от клиента с заголовком `Accept-Encoding: gzip` сожмет файлы самостоятельно, без отправки заголовка источнику.

## Как включить поддержку протокола WebSocket {#websocket-support}

Чтобы включить протокол [WebSocket](https://{{ lang }}.wikipedia.org/wiki/WebSocket), обратитесь в [техническую поддержку]({{ link-console-support }}). В обращении укажите:
