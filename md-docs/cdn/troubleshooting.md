[Документация Yandex Cloud](../index.md) > [Yandex Cloud CDN](index.md) > Вопросы и ответы
...
* [При загрузке объектов из CDN возникают ошибки `no such bucket` и `no such key`](#no-such-bucket-no-such-key)
...
* [Как изменить настройки политики доступа по определенным IP-адресам в созданном CDN-ресурсе?](#access-policy-update)
* [Почему CDN возвращает один и тот же IP-адрес в разных регионах?](#returning-only-one-ip-address)
...
Если объекты из бакета Object Storage загружаются через CDN не с первого раза или только после обновления страницы, проверьте адрес источника и значение HTTP-заголовка `Host` в настройках CDN-ресурса. Для источника-бакета они должны совпадать. Выбор адреса для обычного бакета и хостинга статического сайта описан в разделе [Как правильно настроить заголовок](concepts/servers-to-origins-host.md#best-practices).

Например, если адрес источника — `<имя_бакета>.website.yandexcloud.net`, а заголовок `Host` — `<имя_бакета>.storage.yandexcloud.net`, запросы могут завершаться ошибками `no such bucket` или `no such key`.

Если значения различаются, [измените заголовок `Host`](operations/resources/configure-basics.md), чтобы он совпадал с адресом источника. Дождитесь применения настроек — это может занять до 15 минут — и повторите загрузку объектов через CDN.
...
* если источником служит бакет Object Storage, для него [настроен CORS](../storage/operations/buckets/cors.md).

Настройка CORS для CDN-ресурса добавляет только заголовок `Access-Control-Allow-Origin` в ответ CDN-сервера и не заменяет настройку источника. Подробнее о распределении настроек между CDN-сервером и источником читайте в разделе [CORS в Cloud CDN](concepts/cors.md).
...
Один и тот же IP-адрес при обращении к CDN-ресурсу из разных регионов сам по себе не означает, что все запросы обрабатывает один сервер. Общий принцип выбора сервера описан в [обзоре сервиса](concepts/index.md), а особенности балансировки ресурсов с выделенным IP-адресом — в разделе [Выделенная IP-адресация](concepts/dedicated-ip-addressing.md).

Чтобы проверить, какие серверы обрабатывают запросы, сравните значения HTTP-заголовка `X-Node` в ответах на запросы из разных регионов. Разные значения указывают на разные серверы CDN.