...
- Консоль управления {#console}
  ...
  1. (Опционально) В блоке **Дополнительно**:
      ...
      * В поле **Кеширование query-параметров** выберите режим:
        * `Не кешировать` — не учитывать query-параметры в ключе кеша.
        * `Кешировать всё` — учитывать все query-параметры.
        * `Кешировать всё, кроме` — учитывать все query-параметры, кроме указанных.
        * `Кешировать только` — учитывать только указанные query-параметры.

        Для режимов `Кешировать всё, кроме` и `Кешировать только` укажите query-параметры. Чтобы добавить поле, нажмите **Добавить query-параметр**. Можно указать не более десяти параметров. Допустимая длина имени — до 30 символов. Используйте латинские буквы, цифры и символ `_`.
...
- Terraform {#tf}
  ...
  1. Опишите в конфигурационном файле параметры создаваемого CDN-ресурса `yandex_cdn_resource`:
      ...
      * Блок `options` содержит дополнительные параметры CDN-ресурсов:
      ...
      С помощью Terraform можно настроить учет всех query-параметров или игнорирование всех query-параметров. Чтобы учитывать только указанные параметры или все параметры, кроме указанных, используйте консоль управления или API.
