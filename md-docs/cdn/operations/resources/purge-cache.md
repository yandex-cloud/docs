...
- CLI {#cli}
  ...
  1. Очистите кеш файлов:
      ...
      * `--path` — список путей к файлам CDN-ресурса, кеш для которых необходимо очистить.
          ...
          Если не указать параметр `--path` с путями к файлам, кеш будет очищен для всех файлов ресурса. Подробнее см. в разделе [Очистка кеша](../../concepts/caching.md#purge).
