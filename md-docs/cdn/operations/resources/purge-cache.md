[Документация Yandex Cloud](../../../index.md) > [Yandex Cloud CDN](../../index.md) > [Пошаговые инструкции](../index.md) > Ресурсы > Очистить кеш ресурса
...
- Консоль управления {#console}
  ...
  1. В правом верхнем углу нажмите кнопку ![image](../../../_assets/console-icons/trash-bin.svg) **Очистить кеш**.

  1. Выберите тип очистки кеша:

      * `Полная` — для очистки кеша всех файлов.
      * `Выборочная` — для очистки кеша выбранных файлов. Пути к нужным файлам укажите через запятую в поле **Пути к файлам**.

          Для частичной очистки кеша можно указывать пути к конкретным файлам и каталогам. Каждый путь должен начинаться с символа `/`.
          
          {% note info %}
          
          * Количество путей к файлам в одном запросе на очистку кеша [ограничено](../../concepts/limits.md#cdn-limits).
          ...
          * Если для ресурса включена [сегментация файлов](enable-segmentation.md), при выборочной очистке кеша конкретного файла нужно учитывать, что файл хранится на CDN-серверах по частям. Используйте один из вариантов:
  ...
  1. Очистите кеш файлов:
      ...
      * `--path` — список путей к файлам CDN-ресурса, кеш для которых необходимо очистить.
          ...
          * Если для ресурса включена [сегментация файлов](enable-segmentation.md), при выборочной очистке кеша конкретного файла нужно учитывать, что файл хранится на CDN-серверах по частям. Используйте один из вариантов:
          ...
          Если не указать параметр `--path` с путями к файлам, кеш будет очищен для всех файлов ресурса. Подробнее в разделе [Очистка кеша](../../concepts/caching.md#purge).
