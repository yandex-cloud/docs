[Документация Yandex Cloud](../../../index.md) > [Yandex Cloud CDN](../../index.md) > [Пошаговые инструкции](../index.md) > Ресурсы > Создать ресурс
...
- Консоль управления {#console}
  ...
  1. Задайте основные настройки CDN-ресурса:
      ...
      * Если в существующем ресурсе блок **Разрешенные методы** содержит методы `POST`, `PUT`, `PATCH`, `DELETE`, они будут недоступны для пользователей нового ресурса. Чтобы узнать о возможности использования этих методов, обратитесь в [техническую поддержку](https://center.yandex.cloud/support). При запросе опишите сценарий использования.
      ...
      * В блоке **Контент**:
        ...
        * В поле **Запрос контента** выберите `Из одного источника` или `Из группы источников`:
          ...
          * При запросе контента `Из группы источников` выберите [группу источников](../../concepts/origins.md#groups) или создайте новую:
          ...
          Подробнее в разделе [Источники и их группы](../../concepts/origins.md).
      ...
      * В блоке **Безопасность**:
        ...
        * В поле **Профиль безопасности TLS** выберите [профиль безопасности TLS](../../concepts/clients-to-servers-tls.md#tls-profiles).
        * (Опционально) Чтобы настроить [политику доступа по странам](../../concepts/geo-acl.md):
          ...
          Подробнее в разделе [Настройка политики доступа по странам](configure-geo-acl.md).
...
- Terraform {#tf}
  ...
  1. Опишите в конфигурационном файле параметры создаваемого CDN-ресурса:
     ...
     * `options` — дополнительные параметры CDN-ресурса. Необязательный параметр. Возможные значения:
         ...
         * `enable_ip_url_signing` — опциональный параметр, который включает ограничение доступа к CDN-ресурсу по IP-адресу с помощью [защищенных токенов](../../concepts/secure-tokens.md). Сам доверенный IP-адрес задается вне CDN-ресурса и указывается в качестве параметра при формировании [MD5](https://ru.wikipedia.org/wiki/MD5)-хеша для [подписанной ссылки](../../concepts/secure-tokens.md#protected-link). Если параметр не задан, доступ к файлам будет разрешен с любых IP-адресов.
...
После того как CDN-ресурс был создан, [узнайте](get-resources-info.md#get-cname) доменное имя CDN-провайдера и создайте для указанного имени [ресурсную запись CNAME](../../../dns/concepts/resource-record.md#cname) в настройках вашего DNS-хостинга, например в [Yandex Cloud DNS](../../../dns/operations/resource-record-create.md). Подробнее в разделе [Доменные имена для раздачи контента](../../concepts/resource.md#hostnames).
...
Новый ресурс начнет корректно работать после того, как запись CNAME, которую вы создали в своем DNS-хостинге (смотрите [раздел о доменных именах для раздачи контента](../../concepts/resource.md#hostnames)), распространится по серверам DNS. На это может потребоваться несколько часов.
