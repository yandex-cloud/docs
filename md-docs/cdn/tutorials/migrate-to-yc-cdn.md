...
В стоимость поддержки создаваемой CDN-инфраструктуры входит плата за исходящий трафик с CDN-серверов (см. [тарифы {{ cdn-name }}](../pricing.md)).
...
В данном разделе приведена инструкция по созданию CDN-ресурса с типом источника `{{ ui-key.yacloud.cdn.value_source-type-url }}`. Если в качестве источника вы используете [бакет](../../storage/concepts/bucket.md) {{ objstorage-full-name }} или [балансировщик](../../application-load-balancer/concepts/application-load-balancer.md) {{ alb-full-name }}, при создании CDN-ресурса воспользуйтесь инструкцией [{#T}](../operations/resources/create-resource.md).
