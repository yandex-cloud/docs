...
Учитывайте этот трафик при оценке допустимой нагрузки на источник и расходов на его работу. Подробнее см. в разделе [Почему растут нагрузка на источник и расходы на его работу](../troubleshooting.md#origin-load-growth).
...
|| `Не кешировать` | Все query-параметры игнорируются. Для запросов с одним путем сохраняется одна копия файла независимо от значений параметров. ||
|| `Кешировать всё` | Учитываются все query-параметры. Для каждого уникального набора параметров сохраняется отдельная копия файла. ||
|| `Кешировать всё, кроме` | Указанные query-параметры игнорируются, остальные учитываются. ||
|| `Кешировать только` | Учитываются только указанные query-параметры, остальные игнорируются. ||
|#

Например, если выбрать режим `Кешировать только` и указать параметр `segment`, запросам `/file.m3u8?expires=1&segment=1` и `/file.m3u8?expires=2&segment=1` будет соответствовать один объект в кеше. Запросу `/file.m3u8?expires=1&segment=2` будет соответствовать другой объект.

Кеширование ответов на запросы с файлами cookie настраивается отдельно с помощью опции **Игнорировать**. Если опция выключена, ответы на запросы с заголовком `Cookie` не кешируются, и файлы запрашиваются из источника заново. Если опция включена, ответы кешируются, и CDN-сервер может возвращать файлы из кеша.
