[Документация Yandex Cloud](../../index.md) > [Yandex Cloud CDN](../index.md) > Концепции > Обзор сервиса
...
Yandex Cloud CDN предоставляет разработчикам веб-сервисов _сеть распространения контента_ (Content Delivery Network, CDN). _CDN-серверы_, расположенные в [точках присутствия](points-of-presence.md), получают контент из ваших _источников_, кешируют его и доставляют конечным клиентам по их запросам. Это позволяет снизить нагрузку на источники и сократить время доставки контента конечным пользователям.

### Как работает CDN

1. Вы размещаете контент в [источниках](origins.md). Источником может быть ваш сервер с доменным именем, [бакет Object Storage](../../storage/concepts/bucket.md) или [L7-балансировщик Application Load Balancer](../../application-load-balancer/concepts/application-load-balancer.md).
1. В Cloud CDN вы создаете [_ресурс_](resource.md) и указываете в нем источники, доменные имена для раздачи контента и другие настройки CDN. 
1. Cloud CDN разворачивает CDN-серверы во множестве регионов мира и предоставляет вам доменное имя _CDN-балансировщика_, который принимает запросы и передает их серверам. Вы должны связать это имя с доменными именами для раздачи контента с помощью DNS-записей типа CNAME.
...
|| [Точки присутствия](points-of-presence.md) | Физическое расположение CDN-серверов (POP — point of presence), которые кешируют и доставляют контент конечным пользователям. ||
...
|| [Следование перенаправлениям запросов](follow-redirects.md) | Настройка следования перенаправлениям запросов, полученным от источников контента. ||
|| [Скрытие заголовков от источника](hiding-headers.md) | Настройка белого списка заголовков, которые передаются от источника клиенту. ||
...
|| [Политика доступа по странам](geo-acl.md) | Настройка доступа к CDN-ресурсам для пользователей из указанных стран. ||
|| [Выделенная IP-адресация CDN-ресурсов](dedicated-ip-addressing.md) | Услуга предоставления выделенного IP-адреса для CDN-ресурсов ||
|| [Локационные правила](location-rules.md) | Механизм точечного переопределения настроек CDN-ресурса для запросов, пути которых соответствуют заданным шаблонам. ||
