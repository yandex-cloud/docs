[Документация Yandex Cloud](../../index.md) > [Yandex Cloud CDN](../index.md) > [Концепции](index.md) > Ресурс
...
* HTTP-заголовки и методы с помощью [инструкции](../operations/resources/configure-http.md);
...
* Для контента, распространяемого через CDN. Обратите внимание, что Cloud CDN следует использовать только с ресурсными записями [CNAME](../../dns/concepts/resource-record.md#cname), поскольку при использовании записи [ANAME](../../dns/concepts/resource-record.md#aname) конечный пользователь получит ответ, не зависящий от его геолокации. Подробнее о CNAME-записях и их [ограничениях](https://www.rfc-editor.org/rfc/rfc1912#section-2.4) в [RFC-1035](https://www.ietf.org/rfc/rfc1035.html#section-3.3.1).
...
* Новый ресурс начнет корректно работать после того, как запись CNAME, которую вы создали в своем DNS-хостинге (смотрите [раздел о доменных именах для раздачи контента](resource.md#hostnames)), распространится по серверам DNS. На это может потребоваться несколько часов.
