# История изменений {{ websql-full-name }} в 2026 году
...
В {{ websql-name }} появились вкладки: теперь страницы подключений и редактора запросов можно открывать на вкладках и переключаться между ними, сохраняя рабочий контекст.

### Улучшения {#improvements-december-2025}

Добавлено отображение партиции {{ PG }} в дереве подключений.
...
1. Исправлен порядок отображения полей для индексов в {{ PG }}.
1. Исправлено выполнение запросов в одной сессии в {{ CH }}.
...
1. Поддержана [ленивая загрузка](https://en.wikipedia.org/wiki/Lazy_loading) структуры данных для подключений {{ PG }} и {{ GP }}.
1. Улучшен детальный обзор значения колонки в формате JSON.
1. Добавлено отображение типов значений в результате запроса {{ MG }}/{{ SD }}.
1. Добавлено отображение уведомлений от сервера {{ PG }} при получении результата запроса.

### Решенные проблемы {#fixes-october-2025}

1. Исправлена ошибка, при которой отображались результаты с одинаковыми колонками.
1. Исправлено дублирование индексов в подсказках редактора для {{ PG }}.
...
1. [{{ websql-name }}](https://yandex.cloud/ru/services/websql) — первый сервис, для которого настроена интеграция с [{{ metadata-hub-full-name }}](../../metadata-hub/index.md). Все [подключения](../concepts/index.md#connection) хранятся в [{{ connection-manager-name }}](../../metadata-hub/concepts/connection-manager.md), но управлять ими ([создавать](../operations/create-connection.md), [удалять](../operations/delete-connection.md), [изменять](../operations/change-connection-settings.md)) можно и из интерфейса {{ websql-name }}. Все изменения из {{ websql-name }} отображаются в {{ connection-manager-name }} и наоборот.
