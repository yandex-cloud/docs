...
>
> | Направление<br/>трафика | Диапазон портов | Протокол | Источник | IPv4 CIDR |
> | --- | --- | --- | --- | --- |
> | Входящий | `8080` | `TCP` | `Диапазон адресов` | `10.0.1.0/24` |
> | Входящий | `8080` | `TCP` | `Диапазон адресов` | `10.0.2.0/24` |
...
### Взаимная TLS-аутентификация (mTLS) {#mtls}

Application Load Balancer поддерживает [взаимную TLS-аутентификацию](https://en.wikipedia.org/wiki/Mutual_authentication#mTLS) (mTLS) для HTTPS-обработчиков. При использовании mTLS клиент проверяет сертификат балансировщика, а балансировщик — сертификат клиента, прежде чем принять трафик.

Чтобы включить проверку клиентских сертификатов, укажите в настройках обработчика PEM-файл с корневым сертификатом удостоверяющего центра, выпустившего эти сертификаты. Балансировщик установит TLS-соединение, только если клиент предоставит сертификат, подписанный доверенным удостоверяющим центром.

Данные проверенного клиентского сертификата (сертификат, сведения об издателе и субъекте) можно передавать на бэкенд в HTTP-заголовках. Бэкенд может использовать эти данные для идентификации клиента и сохранять в аудитных логах.

Подробнее о настройке проверки клиентских сертификатов — в руководстве [Терминирование TLS-соединений](../tutorials/tls-termination/console.md).
