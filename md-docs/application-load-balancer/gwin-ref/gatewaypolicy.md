[Документация Yandex Cloud](../../index.md) > [Yandex Application Load Balancer](../index.md) > Инструменты для Managed Service for Kubernetes > Gwin > Конфигурация ресурсов (англ.) > GatewayPolicy
...
| autoScale | **[AutoScalePolicy](#autoscalepolicy)** <br> [Autoscaling](../concepts/application-load-balancer.md#lcu-scaling) settings of the application load balancer. |
...
| Field | Description |
|-------|-------------|
| minZoneSize | **int** <br> Lower limit for the number of [resource units](../concepts/application-load-balancer.md#lcu-scaling) in each [availability zone](../../overview/concepts/geo-scope.md). The minimum value is 2. <br> Example: `2` |
| maxSize | **int** <br> Upper limit for the total number of [resource units](../concepts/application-load-balancer.md#lcu-scaling) across all availability zones. If the value is 0, there is no upper limit. <br> Example: `10` |
...
| Field | Description |
|-------|-------------|
| logGroupID | **string** <br> [Cloud Logging](../../logging/index.md) log group ID to store [access logs](../logs-ref.md). If not set then logs will be stored in default log group for the folder where load balancer located. <br> Example: `log-group-id-1` |
