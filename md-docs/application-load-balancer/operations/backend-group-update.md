[Документация Yandex Cloud](../../index.md) > [Yandex Application Load Balancer](../index.md) > [Пошаговые инструкции](index.md) > Группы бэкендов > Изменить группу бэкендов
  ...
  1. В открывшемся окне задайте настройки бэкенда:
     ...
     * Следующие настройки доступны только для типа бэкенда **Целевая группа**:
       ...
       * В блоке **Настройки балансировки**:
         ...
         * **Локализация трафика** — доля входящего трафика, которую узел балансировщика передает бэкендам из своей зоны доступности. Остальной трафик поровну делится между другими зонами. Подробнее в разделе [Локализация трафика](../concepts/backend-group.md#locality).
       ...
       * В блоке **Настройки протокола**:
         ...
         * Для группы бэкендов типа `gRPC`:
           ...
           * **Протокол** — протокол соединений с бэкендом: `Открытый` или `Зашифрованный`. Для зашифрованного протокола укажите **SNI** и **Доверенный корневой сертификат** (смотрите выше).
