...
Разверните контроллер Gwin в вашем кластере по [инструкции](quickstart.md). Контроллер не будет реагировать на ресурсы контроллера ALB Ingress, поскольку в его спецификации используется другой `ingressClassName`.
...
1. Удалите приложение ALB Ingress Controller.
