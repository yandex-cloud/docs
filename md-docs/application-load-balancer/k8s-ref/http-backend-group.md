...
* `spec` (`HttpBackendGroupSpec`)
  ...
  * `backends` (`[]HttpBackend`)
    ...
    * `storageBucket` (`StorageBucketBackend`)
      ...
      Указание на [бакет Yandex Object Storage](../../storage/concepts/bucket.md), который должен обрабатывать запросы в качестве бэкенда. Подробнее об использовании бакета как бэкенда в разделе [Типы бэкендов](../concepts/backend-group.md#types).
