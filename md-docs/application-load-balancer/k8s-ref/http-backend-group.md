...
Ресурс `HttpBackendGroup` позволяет объединять бэкенды — сервисы {{ k8s }}, между которыми распределяется трафик, — в группу. По таким ресурсам [Ingress-контроллер {{ alb-name }}]({{ ingress-local-link }}/index.md) создает [группы бэкендов](../concepts/backend-group.md).
...
* `spec` (`HttpBackendGroupSpec`)
  ...
  * `backends` (`[]HttpBackend`)
    ...
    * `healthChecks` (`[]HealthChecks`)
      ...
      Настройки пользовательских [проверок состояния](../concepts/backend-group.md#health-checks) приложений в кластере {{ managed-k8s-name }}.
