...
* [HTTP-роутеры](../concepts/http-router.md) (если используются ресурсы [HTTPRoute]({{ configuration-local-link }}/http-route.md) или [GRPCRoute]({{ configuration-local-link }}/grpc-route.md)).
...
* `metadata` (`ObjectMeta`, обязательное)
  ...
  * `annotations` (`map[string]string`, обязательное)
    ...
    * `gateway.alb.yc.io/logs.logGroupId` (`string`)
      ...
      Идентификатор [лог-группы](../../logging/concepts/log-group.md) для записи [логов балансировщика](../logs-ref.md) в {{ cloud-logging-full-name }}.
