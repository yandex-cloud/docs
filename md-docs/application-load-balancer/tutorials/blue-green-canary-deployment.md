[Документация Yandex Cloud](../../index.md) > [Yandex Application Load Balancer](../index.md) > [Практические руководства](index.md) > Сине-зеленое и канареечное развертывание версий сервиса
...
- API {#api}
  ...
  Используйте вызов gRPC API [BackendGroupService/Create](../api-ref/grpc/BackendGroup/create.md) или метод REST API [create](../api-ref/BackendGroup/create.md).
...
- API {#api}
  ...
  1. Создайте HTTP-роутер `canary-router` с помощью вызова gRPC API [HttpRouterService/Create](../api-ref/grpc/HttpRouter/create.md) или метода REST API [create](../api-ref/HttpRouter/create.md).
  1. Создайте виртуальные хосты `canary-vh-production` и `canary-vh-staging`, привязанные к роутеру, и их маршруты с помощью вызова gRPC API [VirtualHostService/Create](../api-ref/grpc/VirtualHost/create.md) или метода REST API [create](../api-ref/VirtualHost/create.md).
...
- API {#api}
  ...
  Используйте вызов gRPC API [LoadBalancerService/Create](../api-ref/grpc/LoadBalancer/create.md) или метод REST API [create](../api-ref/LoadBalancer/create.md).
...
1. Настройте группу бэкендов `canary-bg-production` так, чтобы 20% трафика доменного имени `cdn.yandexcloud.example` приходилось на бэкенд `canary-backend-green` с версией 2:
   ...
   - API {#api}
     ...
     Используйте вызов gRPC API [BackendGroupService/UpdateBackend](../api-ref/grpc/BackendGroup/updateBackend.md) или метод REST API [updateBackend](../api-ref/BackendGroup/updateBackend.md).
...
1. [Удалите](../operations/application-load-balancer-delete.md) L7-балансировщик `canary-balancer`.
