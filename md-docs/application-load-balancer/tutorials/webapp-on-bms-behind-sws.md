...
* плата за использование вычислительных ресурсов L7-балансировщика (см. [тарифы Yandex Application Load Balancer](../pricing.md));
...
- Консоль управления {#console}
  ...
  1. В открывшемся окне с настройками конфигурации сервера:
      ...
      1. В блоке **Приватная сеть** в поле **Приватная подсеть** выберите созданную ранее подсеть `subnet-m4`.
      1. В блоке **Публичная сеть** в поле **Публичный адрес** выберите `Из эфемерной подсети`.
      1. В блоке **Доступ**:
...
1. [Подключитесь по SSH](../../compute/operations/vm-connect/ssh.md) к серверу BareMetal `alb-back-0`:
    ...
    Публичный IP-адрес сервера можно узнать в [консоли управления](https://console.yandex.cloud) на странице **Обзор** с информацией о сервере BareMetal. Нужный адрес указан в блоке **Публичная сеть** в поле **Публичный адрес**.
...
1. Выполните проверку сетевого доступа к серверу `alb-back-0`, указав его приватный IP-адрес:
    ...
    Приватный IP-адрес сервера можно узнать в [консоли управления](https://console.yandex.cloud) на странице **Обзор** с информацией о сервере BareMetal. Нужный адрес указан в блоке **Приватная сеть** в поле **Приватный IP-адрес**.
