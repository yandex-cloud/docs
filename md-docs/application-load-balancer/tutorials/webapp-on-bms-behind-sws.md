...
В этом руководстве вы развернете веб-приложение на [серверах](../../baremetal/concepts/servers.md) Yandex BareMetal. Для обеспечения равномерного распределения нагрузки на хосты с приложением вы настроите [L7-балансировщик](../concepts/application-load-balancer.md) Yandex Application Load Balancer. Создаваемое веб-приложение будет защищено от ботов, [DDoS](../../glossary/ddos.md)- и веб-атак в помощью [профиля безопасности](../../smartwebsecurity/concepts/profiles.md) Yandex Smart Web Security. [Приватное соединение](../../interconnect/concepts/priv-con.md) между [облачной сетью](../../vpc/concepts/network.md#network) Yandex Virtual Private Cloud и [приватной сетью](../../baremetal/concepts/private-network.md) Yandex BareMetal обеспечивается с помощью [Routing Instance](../../cloud-router/concepts/routing-instance.md) Yandex Cloud Interconnect.
...
- API {#api}
  ...
  Чтобы создать приватное соединение с виртуальным маршрутизатором, воспользуйтесь методом REST API [PrivateCloudConnection.Create](../../baremetal/api-ref/PrivateCloudConnection/create.md) или вызовом gRPC API [PrivateCloudConnectionService/Create](../../baremetal/api-ref/grpc/PrivateCloudConnection/create.md).
...
- Консоль управления {#console}
  ...
  1. В [консоли управления](https://console.yandex.cloud) выберите каталог, в котором вы создаете инфраструктуру.
  1. [Перейдите](https://console.yandex.cloud/link/baremetal) в сервис **BareMetal**.
  1. Нажмите кнопку **Заказать сервер** и в открывшемся окне выберите вариант `Выбрать из готовых` и подходящую [конфигурацию](../../baremetal/concepts/server-configurations.md) сервера BareMetal в пуле серверов `ru-central1-m4`.
...
- Консоль управления {#console}
  ...
  1. В поле **Имя** укажите имя [целевой группы](../concepts/target-group.md) `bms-target-group`.
...
- Консоль управления {#console}
  ...
  1. В поле **Имя** укажите имя [группы бэкендов](../concepts/backend-group.md) `bms-backend-group`.
  1. В поле **Тип** выберите `HTTP`.
  1. В блоке **Бэкенды** нажмите кнопку **Добавить** и в появившейся форме **Новый бэкенд**:

      1. В поле **Имя** задайте имя [бэкенда](../concepts/backend-group.md#types) `bms-backend`.
...
- Консоль управления {#console}
  ...
  1. В поле **Имя** укажите имя [HTTP-роутера](../concepts/http-router.md) `http-80`.
  1. В блоке **Виртуальные хосты** нажмите кнопку **Добавить виртуальный хост** и в открывшейся форме **Новый виртуальный хост**:

      1. В поле **Имя** укажите имя [виртуального хоста](../concepts/http-router.md#virtual-host) `http-vh` и нажмите кнопку **Добавить маршрут**.
      1. В открывшейся форме **Новый маршрут** в поле **Имя** задайте имя [маршрута](../concepts/http-router.md#routes) `main-route-80`.
...
- Консоль управления {#console}
  ...
  1. В секции **Обработчики** нажмите кнопку **Добавить обработчик** и в открывшейся форме:
      ...
      1. В поле **Имя** укажите имя [обработчика](../concepts/application-load-balancer.md#listener) `alb-bms-listener`.
...
- Консоль управления {#console}
  ...
  1. На панели слева выберите ![arl](../../_assets/smartwebsecurity/arl.svg) **Профили ARL** и нажмите **Создать профиль ARL**.
  1. В поле **Имя** задайте имя [профиля ARL](../../smartwebsecurity/concepts/arl.md) `arl-profile-demo`.
  1. Нажмите ![plus](../../_assets/console-icons/plus.svg) **Добавить правило** и в открывшемся окне:
      ...
      1. Значения остальных параметров оставьте без изменений и нажмите **Сохранить правило**.
  1. Нажмите **Создать**.
...
- Консоль управления {#console}
  ...
  1. В [консоли управления](https://console.yandex.cloud) выберите каталог, в котором вы создаете инфраструктуру.
  1. [Перейдите](https://console.yandex.cloud/link/smartwebsecurity) в сервис **Smart Web Security**.
  1. На панели слева выберите ![waf](../../_assets/smartwebsecurity/waf.svg) **Профили WAF** и нажмите **Создать профиль WAF**.
...
- Консоль управления {#console}
  ...
  1. Нажмите **Создать профиль** и выберите вариант `С чистого листа`.
  ...
  1. Создайте новое [правило](../../smartwebsecurity/concepts/rules.md) `Web Application Firewall`:
      ...
      1. Нажмите ![plus](../../_assets/console-icons/plus.svg) **Добавить правило**.
  ...
  1. Создайте новое правило `Smart Protection`:
      ...
      1. Повторно нажмите ![plus](../../_assets/console-icons/plus.svg) **Добавить правило**.
...
- Консоль управления {#console}
  ...
  1. На панели сверху нажмите ![plug](../../_assets/console-icons/plug-connection.svg) **Подключить к хосту**.
...
1. [Удалите](../operations/application-load-balancer-delete.md) L7-балансировщик, затем последовательно удалите [HTTP-роутер](../operations/http-router-delete.md), [группу бэкендов](../operations/backend-group-delete.md) и [целевую группу](../operations/target-group-delete.md) балансировщика.
1. [Удалите](../../smartwebsecurity/operations/profile-delete.md) профиль безопасности Smart Web Security, затем удалите [профиль WAF](../../smartwebsecurity/operations/waf-profile-delete.md) и [профиль ARL](../../smartwebsecurity/operations/arl-profile-delete.md).
