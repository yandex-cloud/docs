[Документация Yandex Cloud](../../index.md) > [Практические руководства](../index.md) > Прикладные решения > Интернет вещей (IoT) > Мониторинг показаний датчиков и уведомления о событиях
...
В этом руководстве вы настроите мониторинг и уведомления об изменениях показаний датчиков, подключенных к сервису [Yandex IoT Core](../../iot-core/index.md). Датчики будут эмулированы с помощью сервиса [Yandex Cloud Functions](../../functions/index.md). Если у вас есть подключенные датчики, используйте их.
...
- Консоль управления {#console}
  ...
  1. Создайте версию функции:
      ...
      1. Нажмите кнопку **Создать файл**:
          ...
          1. Нажмите **Создать**.
          1. Добавьте содержимое файла — код функции с [GitHub](https://github.com/yandex-cloud-examples/yc-server-room-iot-monitoring/blob/master/device-emulator.js).
      ...
      1. В блоке **Переменные окружения** укажите:
          ...
          `IOT_CORE_DEVICE_ID`       | Идентификатор устройства, которое вы создали.         | Идентификатор можно узнать в [консоли управления](https://console.yandex.cloud)<br>сервиса Yandex IoT Core.
...
(Опционально) Для получения подробной информации с датчиков, [подпишите](../../iot-core/operations/subscribe.md#one-device) реестр на [топик](../../iot-core/concepts/topic/index.md) устройства Yandex IoT Core.
...
- Консоль управления {#console}
  ...
  1. Создайте версию функции:
      ...
      1. В поле **Источник кода** выберите `Редактор кода`.
      1. Нажмите кнопку **Создать файл**:
          1. Задайте имя файла — `monitoring.py`.
          1. Нажмите **Создать**.
          1. Добавьте содержимое файла — код функции с [GitHub](https://github.com/yandex-cloud-examples/yc-server-room-iot-monitoring/blob/master/monitoring.py).
      ...
      1. В блоке **Переменные окружения** укажите:
          ...
          `METRICS_FOLDER_ID` | Идентификатор каталога, в котором развернуты сервисы и для которого вы создадите дашборд в сервисе Monitoring. | Идентификатор можно узнать в [консоли управления](https://console.yandex.cloud).
...
- Интерфейс Monitoring {#console}
  ...
  1. Справа вверху нажмите ![image](../../_assets/console-icons/plus.svg) → ![image](../../_assets/console-icons/chart-column.svg) **График**.
  ...
  1. Создайте запрос для графика:
      ...
      1. В редакторе запроса в строке ![image](../../_assets/monitoring/chart.svg) **Введите значение метки service** выберите:
          * `service` → `custom`;
          * `name` → `temperature` — температура в помещении;
          * `device_id` → идентификатор устройства, по которому вы хотите создать график.
  1. Справа вверху нажмите кнопку ![image](../../_assets/console-icons/floppy-disk.svg).
...
- Интерфейс Monitoring {#console}
  ...
  1. В [консоли управления](https://console.yandex.cloud) выберите каталог, в котором вы выполняете руководство.
  1. [Перейдите](https://console.yandex.cloud/link/monitoring) в сервис **Monitoring** и посмотрите, как изменились показатели графиков.
...
- Интерфейс Monitoring {#console}
  ...
  1. В [консоли управления](https://console.yandex.cloud) выберите каталог, в котором вы выполняете руководство.
  1. [Перейдите](https://monitoring.yandex.cloud) в сервис **Monitoring**.
  1. На панели слева выберите ![image](../../_assets/console-icons/antenna-signal.svg) **Каналы уведомлений**.
  1. Нажмите кнопку **Создать**.
  1. В открывшемся окне:
      1. В поле **Имя** укажите название канала. Например, `my-message-channel`.
      1. В списке **Метод** выберите ![image](../../_assets/console-icons/envelope.svg) `Email`.
...
- Интерфейс Monitoring {#console}
  ...
  1. В [консоли управления](https://console.yandex.cloud) выберите каталог, в котором вы выполняете руководство.
  1. [Перейдите](https://monitoring.yandex.cloud) в сервис **Monitoring**.
  1. Внизу страницы в блоке ![image](../../_assets/console-icons/megaphone.svg) **Алерты** нажмите **Создать алерт**.
  1. В поле **Название** введите имя алерта.
  1. В блоке **Настройки алерта**:
      
      1. В редакторе запроса в строке ![image](../../_assets/monitoring/chart.svg) **Введите значение метки service** выберите:
          * `service` → `custom`;
          * `name` → `temperature` — температура в помещении;
          * `device_id` → идентификатор устройства, по которому вы хотите создать алерт.
  ...
  1. В блоке **Условия срабатывания** задайте условия срабатывания алерта:
      ...
      1. В списке **Окно вычисления** выберите `5m`.
...
* [Удалите реестр в сервисе Yandex IoT Core](../../iot-core/operations/registry/registry-delete.md)
* [Удалите устройство в сервисе Yandex IoT Core](../../iot-core/operations/device/device-delete.md)
