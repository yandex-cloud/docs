[Документация Yandex Cloud](../../index.md) > [Практические руководства](../index.md) > Хранение и восстановление данных > Все руководства

# Хранение и восстановление данных

## Резервное копирование в Yandex Object Storage {#backup}

* [Обзор](storage-backup-overview.md)
* [Резервное копирование в Yandex Object Storage с помощью AWS S3 Sync](storage-backup-aws-s3-sync.md)
* [Резервное копирование в Yandex Object Storage с помощью rclone](storage-backup-rclone.md)
* [Резервное копирование в Yandex Object Storage с помощью GeeseFS](storage-backup-geesefs.md)
* [Резервное копирование в Yandex Object Storage с помощью MSP360 Backup (CloudBerry Desktop Backup)](object-storage-cloudberry.md)
* [Резервное копирование в Yandex Object Storage с помощью Duplicati](object-storage-duplicati.md)
* [Резервное копирование в Yandex Object Storage с помощью Bacula](backup-with-bacula.md)
* [Резервное копирование в Yandex Object Storage с помощью Veritas Backup Exec](object-storage-veritas.md)
* [Резервное копирование ВМ с помощью «Хайстекс Акура — Система резервного копирования»](../infrastructure-management/hystax-backup.md)
* [Резервное копирование кластера Managed Service for Kubernetes в Yandex Object Storage](kubernetes-backup.md)

## Другие руководства {#other}

* [Миграция в Yandex Cloud с помощью Хайстекс Акура](../infrastructure-management/hystax-migration.md)
* [Защита от сбоев с помощью Хайстекс Акура](../infrastructure-management/hystax-disaster-recovery.md)
* [Однонодовый файловый сервер](single-node-file-server/index.md)
* [Настройка SFTP-сервера на Centos 7](backup-and-archive-to-sftp.md)
* [Развертывание параллельной файловой системы GlusterFS в высокодоступном режиме](ha-regional-glusterfs.md)
* [Развертывание параллельной файловой системы GlusterFS в высокопроизводительном режиме](ha-regional-glusterfs-high-performance.md)
* [Автоматическая привязка политики резервного копирования Yandex Cloud Backup к ВМ](vm-with-backup-policy/index.md)
* [Репликация логов в Yandex Object Storage с помощью Yandex Data Streams](replicate-logs-to-storage-via-data-streams.md)
* [Репликация логов в Yandex Object Storage с помощью Fluent Bit](replicate-logs-to-storage.md)
* [Использование Yandex Object Storage в Yandex Data Processing](copy-files-from-object-storage.md)
* [Подключить существующий сервер Yandex BareMetal к Yandex Cloud Backup](backup-baremetal.md)
* [Развертывание Nextcloud в Yandex Cloud](integrate-nextcloud/index.md)