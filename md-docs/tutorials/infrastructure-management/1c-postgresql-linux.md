...
Создаваемая инфраструктура «1С:Предприятие» будет состоять из рабочего сервера 1С, сервера лицензий 1С и [кластера {{ mpg-short-name }}](../../managed-postgresql/concepts/index.md).  Серверы 1С будут работать под управлением ОС [CentOS 7](https://yandex.cloud/ru/marketplace/products/yc/centos-7) и не будут иметь выхода в интернет. Доступ к кластеру будет осуществляться через шифрованное [соединение с сервером OpenVPN](../routing/openvpn.md).
