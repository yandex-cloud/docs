[Документация Yandex Cloud](../../index.md) > [Практические руководства](../index.md) > [Базовая инфраструктура](../infrastructure/index.md) > Cloud DNS > Настройка локального кеширующего DNS-резолвера
...
Если на виртуальной машине установлена операционная система Ubuntu версии 18.04 LTS или выше, то в ней по умолчанию уже используется кеширующий резолвер `systemd-resolved`, и дополнительных действий по установке и настройке не требуется. Подробнее в разделе [Проверьте работоспособность systemd-resolved](#test-resolver).

Если на виртуальной машине используется более старая версия Ubuntu, например, [16.04 LTS](https://yandex.cloud/ru/marketplace/products/yc/ubuntu-16-04-lts) — кеширующий DNS-резолвер нужно установить самостоятельно. Мы рекомендуем использовать `dnsmasq` или `unbound`. Подробнее в разделе [Установите альтернативный резолвер](#alternate-resolvers).
...
1. [Создайте](../../compute/operations/index.md#vm-create) виртуальную машину с ОС Ubuntu. Например, [Ubuntu 18.04 LTS](https://yandex.cloud/ru/marketplace/products/yc/ubuntu-18-04-lts).
...
Параметры конфигурации приведены для примера. Как настроить кеширующий резолвер с учетом нагрузки на ВМ и ваших потребностей смотрите в документации выбранного резолвера.
...
- dnsmasq
   ...
   1. Отредактируйте конфигурационный файл `/etc/dnsmasq.conf`, увеличив размер кеша до 1000 и ограничив локальной машиной адреса, которые слушает кеширующий резолвер.
