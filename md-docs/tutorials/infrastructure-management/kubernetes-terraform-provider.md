...
Использовать Terraform для создания Kubernetes-ресурсов удобно, если через Terraform вы поддерживаете инфраструктуру для [кластера Yandex Managed Service for Kubernetes](../../managed-kubernetes/concepts/index.md#kubernetes-cluster). Так вы можете описывать все ресурсы на одном языке разметки.
...
Подготовьте для кластера Managed Service for Kubernetes [постоянный том](../../managed-kubernetes/concepts/volume.md#persistent-volume). Используйте для этого конфигурационный файл:
