[Документация Yandex Cloud](../../index.md) > [Практические руководства](../index.md) > [Базовая инфраструктура](../infrastructure/index.md) > Инструменты > Источники данных Terraform
...
1. В файле конфигурации после настроек провайдера добавьте:
    ...
    * `output "resource_active"` — выходная переменная, которая содержит информацию о текущем идентификаторе образа для указанного семейства:
    ...
    Подробнее о параметрах источника данных `yandex_compute_image` смотрите в [документации провайдера](../../terraform/data-sources/compute_image.md).
