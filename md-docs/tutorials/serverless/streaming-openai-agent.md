...
В этом руководстве вы создадите агент со стримингом ответа через [веб-сокеты](https://ru.wikipedia.org/wiki/WebSocket) на [Yandex Cloud Functions](../../functions/index.md) и [Yandex API Gateway](../../api-gateway/index.md). Функция будет использовать [OpenAI Agents SDK](https://openai.github.io/openai-agents-python/) для работы с моделями [Yandex AI Studio](https://aistudio.yandex.ru/docs/ru/ai-studio/concepts/generation/index#yandex).
...
1. AI-агент расширяет запрос пользователя дополнительным контекстом и отправляет его в [модель генерации текста](https://aistudio.yandex.ru/docs/ru/ai-studio/concepts/generation/index).
1. [Сервисный аккаунт](../../iam/concepts/users/service-accounts.md) с помощью [API-ключа](../../iam/concepts/authorization/api-key.md) предоставляет AI-агенту доступ к [Text Generation API](https://aistudio.yandex.ru/docs/ru/ai-studio/text-generation/api-ref/index).
...
В секрете [Yandex Lockbox](../../lockbox/index.md) будет храниться секретный ключ.
...
- Консоль управления {#console}
  ...
  1. Создайте [версию функции](../../functions/concepts/function.md#version):
     ...
     1. Выберите среду выполнения `Python 3.14`, отключите опцию **Добавить файлы с примерами кода** и нажмите **Продолжить**.
     1. В поле **Источник кода** выберите `ZIP-архив` и прикрепите созданный ранее архив `function.zip`.
     ...
     1. В блоке **Параметры** укажите:
        ...
        * Если вы не хотите сохранять логи и [платить](../../logging/pricing.md) за использование сервиса [Cloud Logging](../../logging/index.md), отключите опцию **Запись логов**.
...
1. Создайте API-шлюз:
    ...
    - Консоль управления {#console}
      ...
      1. Если вы не хотите сохранять логи и платить за использование сервиса Cloud Logging, отключите опцию **Запись логов**.
      1. Нажмите **Создать**.
      1. Выберите созданный API-шлюз. Сохраните значение поля **Служебный домен для WebSocket** — оно потребуется на следующем шаге.
