...
Фреймворк Functions Framework позволяет создавать функции и запускать их в [контейнерах](../../serverless-containers/concepts/container.md) {{ serverless-containers-full-name }} без использования сервиса [{{ sf-full-name }}](../../functions/index.md). Написанные этим способом функции можно переносить между различными платформами, такими как [Cloud Run](https://cloud.google.com/run), [Cloud Run functions](https://cloud.google.com/functions), [Knative](https://knative.dev/docs/) и [{{ serverless-containers-full-name }}](../../serverless-containers/index.md), а также между этими платформами и вашей локальной машиной разработки.
