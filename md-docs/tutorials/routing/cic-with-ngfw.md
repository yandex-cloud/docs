...
Для организации сетевой IP-связности между ресурсами в собственной инфраструктуре клиента и облачными ресурсами в {{ yandex-cloud }} может использоваться услуга [{{ interconnect-full-name }}](../../interconnect/concepts/index.md).

В данном документе рассмотрены настройки [маршрутизации](../../vpc/concepts/routing.md) в облачной сети и настройки [приватного соединения](../../interconnect/concepts/priv-con.md) {{ interconnect-name }}, которые позволяют обеспечить сетевую связность инфраструктуры клиента с сегментами, размещенными за NGFW.
...
1. Используйте [модуль `route-switcher`](https://github.com/yandex-cloud-examples/yc-route-switcher) для переключения трафика в {{ vpc-short-name }} `interconnect`, `dmz`, `app` в случае отказа основного NGFW на резервный. Модуль `route-switcher` используется в практических руководствах на основе [UserGate NGFW](high-accessible-dmz-usergate.md) или [Check Point NGFW](high-accessible-dmz.md).
...
Познакомьтесь с вариантами организации услуги {{ interconnect-name }} в [документации](../../interconnect/concepts/index.md). Для обеспечения отказоустойчивости подключения к услуге рекомендуется организация нескольких транков — по одному в каждой [точке присутствия](../../interconnect/concepts/pops.md).

Воспользуйтесь [инструкцией](../../interconnect/operations/index.md) для организации приватного соединения в зависимости от Вашего сценария подключения к услуге {{ interconnect-name }}.
