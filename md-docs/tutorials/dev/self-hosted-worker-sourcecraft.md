...
* плата за диск и запущенную ВМ (см. [тарифы Yandex Compute Cloud](../../compute/pricing.md));
...
[Создайте](https://sourcecraft.dev/portal/docs/ru/sourcecraft/operations/service-connections#create-service-connection) сервисное подключение с названием `default-service-connection`, используйте для него [созданный ранее](#infra) сервисный аккаунт с [ролью](../../compute/security/index.md#compute-editor) `compute.editor` на каталог.
...
Чтобы перестать платить за созданные ресурсы, [запустите вручную](https://sourcecraft.dev/portal/docs/ru/sourcecraft/operations/run-workflow-manually) рабочий процесс `delete-vm` или [удалите](../../compute/operations/vm-control/vm-delete.md) ВМ с пользовательским воркером в интерфейсе Yandex Cloud.
