...
Данные в {{ yds-short-name }} передаются по [HTTP-протоколу](../../data-streams/kinesisapi/api-ref.md). С помощью сервиса [{{ api-gw-full-name }}](https://yandex.cloud/ru/services/api-gateway) можно реализовать любой протокол приема данных. Принятые в {{ api-gw-name }} данные можно уже отправить в {{ yds-short-name }}.
