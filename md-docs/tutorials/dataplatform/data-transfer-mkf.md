...
* Между разными [зонами доступности](../../overview/concepts/geo-scope.md) — можно [перенести кластер с одним хостом](../../managed-kafka/operations/host-migration.md#one-host) из одной зоны в другую.
...
* Кластеры Managed Service for Apache Kafka®: выделенные хостам вычислительные ресурсы, объем хранилища и резервных копий ([тарифы Managed Service for Apache Kafka®](../../managed-kafka/pricing.md)).
...
1. Подготовьте инфраструктуру поставки данных:
   ...
   - Terraform {#tf}
       ...
       1. Скачайте в ту же рабочую директорию файл конфигурации [data-transfer-mkf-mkf.tf](https://github.com/yandex-cloud-examples/yc-data-transfer-mirror-managed-kafka/blob/main/data-transfer-mkf-mkf.tf).
           ...
           * кластер-приемник Managed Service for Apache Kafka® с публичным доступом из интернета;
...
- Вручную {#manual}
  ...
  1. [Удалите кластеры Managed Service for Apache Kafka®](../../managed-kafka/operations/cluster-delete.md).
