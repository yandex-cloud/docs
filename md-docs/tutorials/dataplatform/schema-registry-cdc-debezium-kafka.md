...
Вы можете создать реестр схем, чтобы отслеживать эволюцию схем для источника {{ KF }} или {{ mkf-full-name }} с помощью сервиса [{{ data-transfer-full-name }}](../../data-transfer/index.md). В этом руководстве отслеживается эволюция схем для источника {{ mkf-name }}. 
