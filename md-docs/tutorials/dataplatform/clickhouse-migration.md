...
Вы также можете перенести базу данных из кластера {{ CH }} в кластер {{ mch-name }} при помощи {{ data-transfer-name }}. О том, как это сделать, см. в [практическом руководстве](ch-to-mch-migration.md).
