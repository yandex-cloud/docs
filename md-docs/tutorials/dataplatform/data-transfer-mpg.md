[Документация Yandex Cloud](../../index.md) > [Практические руководства](../index.md) > [Построение Data Platform](index.md) > Apache Kafka® > Поставка данных с помощью Data Transfer > Поставка данных из Managed Service for PostgreSQL в Managed Service for Apache Kafka®
...
- Вручную {#manual}
  ...
  1. [Создайте кластер-источник Managed Service for PostgreSQL](../../managed-postgresql/operations/cluster-create.md) любой подходящей конфигурации со следующими настройками:
  ...
  1. Если вы используете группы безопасности, настройте их так, чтобы к кластерам можно было подключаться из интернета:
      ...
      * [Инструкция для Managed Service for PostgreSQL](../../managed-postgresql/operations/connect/index.md#configuring-security-groups).
...
- Terraform {#tf}
  ...
  1. Скачайте в ту же рабочую директорию файл конфигурации [cdc-mpg-mkf.tf](https://github.com/yandex-cloud-examples/yc-data-transfer-postgresql-to-kafka/blob/main/cdc-mpg-mkf.tf).
      ...
      * база данных и пользователь с ролью [mdb_replication](../../managed-postgresql/concepts/roles.md#mdb-replication) в кластере Managed Service for PostgreSQL;
...
1. Если вы создавали инфраструктуру вручную, [назначьте](../../managed-postgresql/operations/grant.md#grant-role) роль [mdb_replication](../../managed-postgresql/concepts/roles.md#mdb-replication) пользователю `pg-user`. Это необходимо, чтобы пользователь мог создать публикацию, через которую Data Transfer получает информацию об изменениях в данных в кластере-источнике.
