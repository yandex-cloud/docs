...
1. Внесите изменения в конфигурационный файл `php.ini` для вашего веб-сервера.
    ...
    * `/etc/php/7.2/fpm/` для NGINX.
    ...
    Подробнее про подключение к кластеру смотрите в разделе [Предварительная настройка для подключения к кластеру Valkey™](../../managed-valkey/operations/connect/index.md).
