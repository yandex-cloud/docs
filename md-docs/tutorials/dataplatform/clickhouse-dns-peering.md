[Документация Yandex Cloud](../../index.md) > [Практические руководства](../index.md) > Базовая инфраструктура > Cloud DNS > Настройка Cloud DNS для доступа к кластеру Managed Service for ClickHouse® из других облачных сетей
...
1. Настройте сетевую связность между облачными сетями `mch-net` и `another-net`, например, с помощью [IPSec-шлюза](../routing/ipsec/index.md). Другие способы настроить сетевую связность смотрите в разделе [Руководства по работе с сетевой инфраструктурой в Yandex Cloud](../routing/index.md).
