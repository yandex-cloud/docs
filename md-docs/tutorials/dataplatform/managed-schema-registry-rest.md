[Документация Yandex Cloud](../../index.md) > [Практические руководства](../index.md) > [Построение Data Platform](index.md) > Apache Kafka® > Использование схем формата данных с Managed Service for Apache Kafka® > Использование Managed Schema Registry с Managed Service for Apache Kafka® с помощью REST API
...
Также Managed Service for Apache Kafka® предоставляет [REST API для Apache Kafka®](../../managed-kafka/concepts/available-apis.md#managed-kafka-api). В том числе, с помощью этого API можно отправлять и получать сообщения без использования сторонних производителей и потребителей. Эти возможности также будут продемонстрированы в этом практическом руководстве.
...
- Вручную {#manual}
    ...
    1. [Создайте пользователя](../../managed-kafka/operations/cluster-accounts.md#create-account) с именем `user1` и [выдайте ему права](../../managed-kafka/operations/cluster-accounts.md#grant-permission) на топик `messages`:
        ...
        * `ACCESS_ROLE_PRODUCER`.
        ...
        Этот пользователь сможет отправлять и получать сообщения в рамках топика, а также [выполнять любые операции над субъектами в Managed Schema Registry](../../managed-kafka/concepts/managed-schema-registry.md#msr-auth), которые связаны с топиком.
...
1. Создайте схему формата данных для ключа сообщения Apache Kafka®.
    ...
    [Имя субъекта для схемы](../../managed-kafka/concepts/managed-schema-registry.md#subjects) должно состоять из имени топика, в котором будет использоваться эта схема (`messages`) и суффикса `-key`.
...
1. Создайте схему формата данных для значения сообщения Apache Kafka®.
    ...
    [Имя субъекта для схемы](../../managed-kafka/concepts/managed-schema-registry.md#subjects) должно состоять из имени топика, в котором будет использоваться эта схема (`messages`) и суффикса `-value`.
