...
Подробнее о запросе `INSERT INTO` смотрите в [документации ClickHouse®](https://clickhouse.com/docs/ru/sql-reference/statements/insert-into#insert).
...
Список поддерживаемых форматов данных приведен в [документации ClickHouse®](https://clickhouse.com/docs/ru/interfaces/formats). Настройка схем форматов данных Cap'n Proto и Protobuf описана в разделе [Управление схемами формата данных](../../managed-clickhouse/operations/format-schemas.md).
...
Если для пользователя установлена настройка [Async insert](../../managed-clickhouse/concepts/settings-list.md#setting-async-insert), то все запросы на вставку от этого пользователя сначала попадают в буфер в оперативной памяти. Данные из буфера сбрасываются в таблицу при выполнении одного из условий:

* Размер буфера достиг значения настройки [Async insert max data size](../../managed-clickhouse/concepts/settings-list.md#setting-async-insert-max-data-size).
* С момента первого после сброса данных `INSERT`-запроса прошло время, указанное в настройке [Async insert busy timeout](../../managed-clickhouse/concepts/settings-list.md#setting-async-insert-busy-timeout).
...
При использовании асинхронных вставок недоступна [дедупликация строк](https://clickhouse.com/docs/ru/guides/developer/deduplication).

Подробную информацию об асинхронной вставке данных смотрите в [документации ClickHouse®](https://clickhouse.com/docs/ru/best-practices/use-materialized-views).
...
Информацию о дополнительных параметрах движка и ограничениях таблиц на движке `Buffer` смотрите в [документации ClickHouse®](https://clickhouse.com/docs/ru/engines/table-engines/special/buffer).
...
1. Изучите формат данных, который будет использоваться при вставке, чтобы [подготовить](#prepare-format-schemas) корректные схемы формата.
   ...
   Подробнее о поддерживаемых типах данных смотрите в документации [Cap'n Proto](https://capnproto.org/language.html), [Protobuf](https://developers.google.com/protocol-buffers/docs/proto3) и [ClickHouse®](https://clickhouse.com/docs/ru/sql-reference/data-types).
...
1. Создайте файл с описанием схемы:
   ...
   - Cap'n Proto {#capn}
     ...
     Подробнее о формате файла смотрите в [документации Cap'n Proto](https://capnproto.org/language.html).
   ...
   - Protobuf {#protobuf}
     ...
     Подробнее о формате файла смотрите в [документации Protobuf](https://developers.google.com/protocol-buffers/docs/overview).
...
1. [Подключите схему формата данных](../../managed-clickhouse/operations/format-schemas.md#add-format-schema) к кластеру `chcluster`:
...
1. [Подключитесь к кластеру](../../managed-clickhouse/operations/connect/clients.md) и проверьте, что данные были успешно вставлены, выполнив запрос `SELECT`:
