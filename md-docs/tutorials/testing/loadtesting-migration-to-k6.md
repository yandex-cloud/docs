[Документация Yandex Cloud](../../index.md) > [Практические руководства](../index.md) > Разработка и тестирование > Миграция нагрузочного тестирования на k6

# Миграция нагрузочного тестирования с Load Testing на k6

{% note warning %}

С 1 июля 2026 года сервис Load Testing прекратил работу. Подробнее на странице [Закрытие сервиса Yandex Load Testing](../../load-testing/sunset.md).
...
## Соответствие возможностей Load Testing и k6 {#overview}

#|
|| **Возможность в Load Testing** | **Аналог в k6** | **Подробнее** ||
...
| [Профили нагрузки](#load-profiles) ||
|| **Тестовые данные** — форматы URI, HTTP_JSON, GRPC_JSON, файлы из Object Storage
...
|| **Мониторинг агента** — CPU, память, диск, сеть через Telegraf/Monitoring
| Telegraf, node_exporter и Prometheus или Monitoring для ВМ в облаке
...
В Load Testing автостоп прерывал тест при нарушении заданных условий. В k6 аналогичную функцию выполняют [пороговые значения (thresholds)](https://grafana.com/docs/k6/latest/using-k6/thresholds/) с параметром `abortOnFail`.
