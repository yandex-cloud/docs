...
С реализацией такой схемы помогут сервисы {{ yandex-cloud }} — [{{ alb-full-name }}](../../application-load-balancer/index.md) (ALB) и [{{ sws-full-name }}](../../smartwebsecurity/concepts/index.md) (SWS).
...
* **Каталоги команд** — каталоги, в которых размещены целевые ресурсы команд. Например, виртуальные машины (ВМ), базы данных, балансировщики L3-L4 [{{ network-load-balancer-name }}](../../network-load-balancer/index.md) (NLB) и другие.
...
1. При использовании HTTPS [добавьте TLS-сертификат](../../certificate-manager/quickstart/index.md) в {{ certificate-manager-name }}.
