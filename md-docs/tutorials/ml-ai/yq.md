...
* плата за данные, считанные [Yandex Query при исполнении запросов](../../query/pricing.md).
...
* `--raw-results` — (опционально) параметр, чтобы возвращать необработанные результаты выполнения запроса в Query. Спецификация формата доступна в разделе [Соответствие YQL и Json-типов](../../query/api/yql-json-conversion-rules.md).
...
Переменная `output2` будет выглядеть следующим образом:
