[Документация Yandex Cloud](../../../index.md) > [Yandex Cloud Registry](../../index.md) > [Пошаговые инструкции](../index.md) > Управление реестром > Создание реестра > Локальный
...
- CLI {#cli}
    ...
    * `--properties` — свойства реестра. Задаются строкой в формате `name1=value1,name2=value2`. Доступные свойства для локальных реестров:
        ...
        * `signingGpgKeySecretId` — идентификатор секрета Yandex Lockbox, содержащего GPG-ключ для подписи метаданных. Поддерживается только для формата реестра `debian`. Подробнее о подготовке GPG-ключа и создании секрета в разделе [Подготовка GPG-ключа](../debian/installation.md#prepare-gpg-key).
