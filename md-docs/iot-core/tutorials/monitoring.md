...
В этом руководстве вы настроите мониторинг и уведомления об изменениях показаний датчиков, подключенных к сервису [{{ iot-name }}](../index.md). Датчики будут эмулированы с помощью сервиса [{{ sf-full-name }}](../../functions/index.md). Если у вас есть подключенные датчики, используйте их.
...
* плата за количество сообщений {{ iot-name }} (см. [тарифы {{ iot-full-name }}](../pricing.md));
* плата за количество вызовов [функции](../../functions/concepts/function.md) {{ sf-name }} (см. [тарифы {{ sf-full-name }}](../../functions/pricing.md));
* плата за запись пользовательских [метрик](../../monitoring/concepts/data-model.md#metric) через API сервиса [{{ monitoring-name }}](../../monitoring/index.md) (см. [тарифы {{ monitoring-full-name }}](../../monitoring/pricing.md)).
...
(Опционально) Для получения подробной информации с датчиков, [подпишите](../operations/subscribe.md#one-device) реестр на [топик](../concepts/topic/index.md) устройства {{ iot-name }}.
...
Для наблюдения за показаниями датчиков используется дашборд в сервисе [{{ monitoring-full-name }}](../../monitoring/index.md). Показания датчиков поступают с контроллера на сервер раз в минуту по [MQTT-протоколу](../../glossary/mqtt-server.md#mqtt-protocol). Когда показания датчиков достигают заданных значений, {{ monitoring-name }} отправляет уведомления пользователям.
...
* [Удалите реестр в сервисе {{ iot-name }}](../operations/registry/registry-delete.md)
* [Удалите устройство в сервисе {{ iot-name }}](../operations/device/device-delete.md)
