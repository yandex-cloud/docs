...
В этом руководстве вы настроите мониторинг состояния устройств (например, вендинговых автоматов), подключенных к сервису [{{ iot-full-name }}](../index.md) и расположенных в разных точках города. Вы сможете наблюдать за состоянием автоматов на карте и графиках сервиса [{{ datalens-full-name }}](../../datalens/index.md). Датчики будут эмулированы с помощью сервиса [{{ sf-full-name }}](../../functions/index.md). Если у вас есть подключенные датчики, используйте их. Для хранения данных будет использован сервис [{{ mpg-full-name }}](../../managed-postgresql/index.md).
...
[Реестр](../concepts/index.md#registry) и [устройство](../concepts/index.md#device) — основные компоненты сервиса {{ iot-name }} для обмена данными и командами. Чтобы устройства могли обмениваться данными, их необходимо создавать в одном реестре.
...
Вы также можете использовать авторизацию с помощью сертификатов. Подробнее [об авторизации в {{ iot-name }}](../concepts/authorization.md).
...
1. (Опционально) Чтобы получать подробную информацию с датчиков, [подпишите](../operations/subscribe.md#one-device) реестр на [топик](../concepts/topic/index.md) устройства {{ iot-name }}.
...
Триггер вызовет функцию, когда в [топике устройства](../concepts/topic/devices-topic.md) появятся сообщения.
...
1. [Удалите](../operations/device/device-delete.md) устройства в сервисе {{ iot-name }}.
1. [Удалите](../operations/registry/registry-delete.md) реестр в сервисе {{ iot-name }}.
