[Документация Yandex Cloud](../../index.md) > [Yandex BareMetal](../index.md) > [Практические руководства](index.md) > Организация сетевой связности между приватными подсетями BareMetal и Virtual Private Cloud

# Организация сетевой связности между приватными подсетями BareMetal и Yandex Virtual Private Cloud с помощью Yandex Cloud Interconnect
...
В этом руководстве вы организуете сетевую связность между [сервером](../concepts/servers.md) BareMetal, расположенным в [приватной подсети](../concepts/private-network.md) Yandex BareMetal, и [виртуальной машиной](../../compute/concepts/vm.md) Yandex Compute Cloud, расположенной в подсети [облачной сети](../../vpc/concepts/network.md#network) Yandex Virtual Private Cloud.
...
Для настройки Cloud Interconnect в сервисе BareMetal понадобятся приватная маршрутизируемая [подсеть](../concepts/private-network.md#private-subnet) и [VRF](../concepts/private-network.md#vrf-segment) в BareMetal, [облачная сеть](../../vpc/concepts/network.md#network) с одной или более [подсетями](../../vpc/concepts/network.md#subnet) Virtual Private Cloud, а также виртуальный маршрутизатор, в составе которого будут [анонсированы](../../interconnect/concepts/priv-con.md#prc-announce) один или несколько префиксов приватных подсетей VPC.
...
После того как в вашем каталоге будет создан необходимый виртуальный маршрутизатор, создайте [приватное соединение](../concepts/private-network.md#private-connection-to-vpc) Cloud Interconnect в сервисе BareMetal:
...
Чтобы сервер BareMetal мог обращаться к облачным ресурсам по доменным именам из внутренних DNS-зон Yandex Cloud, [настройте разрешение DNS-имен](bm-vpc-dns-forwarder.md) с помощью входящего DNS-подключения.
