...
- Консоль управления {#console}
  ...
  1. В строке с сервером `server-m3-1` нажмите значок ![image](../../_assets/console-icons/ellipsis.svg) и выберите **KVM-консоль**.
      ...
      1. Посмотрите список сетевых интерфейсов сервера и определите интерфейс, подключенный к приватной подсети:
          ...
          Убедиться в том, что выбран верный сетевой интерфейс, можно с помощью MAC-адреса. MAC-адрес нужного сетевого интерфейса сервера BareMetal вы можете [посмотреть](../operations/servers/get-info.md) на странице с информацией о сервере в блоке **Приватная сеть**.
