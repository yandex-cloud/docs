...
1. [Создайте VRF и приватную подсеть BareMetal](bm-vrf-and-vpc-interconnect.md#setup-vrf).

1. [Арендуйте сервер BareMetal](bm-vrf-and-vpc-interconnect.md#rent-bms) с образом ОС `Ubuntu 24.04` и доступом в интернет.

   Вы также можете использовать BareMetal-серверы с другими операционными системами.

1. [Создайте виртуальную машину](bm-vrf-and-vpc-interconnect.md#create-vm).

1. [Создайте Routing Instance](bm-vrf-and-vpc-interconnect.md#request-routing-instance).

1. [Создайте приватное соединение](bm-vrf-and-vpc-interconnect.md#create-private-connection).
...
1. Создайте в кластере Managed Service for Kubernetes секрет, содержащий приватный [SSH-ключ](../../glossary/ssh-keygen.md) для подключения к серверу BareMetal:
    ...
    Если вы утратили закрытую часть SSH-ключа, указанного при аренде сервера, вы можете [добавить](../operations/servers/add-new-ssh-key.md) в профиль root-пользователя сервера новый публичный SSH-ключ.
