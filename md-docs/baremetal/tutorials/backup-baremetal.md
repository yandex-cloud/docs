[Документация Yandex Cloud](../../index.md) > [Yandex BareMetal](../index.md) > [Практические руководства](index.md) > Подключение существующего сервера BareMetal к Cloud Backup

# Подключение сервера BareMetal к Yandex Cloud Backup
...
Информацию о том, как подключить сервер BareMetal к Cloud Backup при его заказе, смотрите в инструкции [Арендовать сервер Yandex BareMetal с подключением к Cloud Backup](../../backup/operations/backup-baremetal/lease-server-with-backup.md).

Подробнее о работе с серверами BareMetal смотрите в разделе [Пошаговые инструкции для сервиса Yandex BareMetal](../operations/index.md).
...
* плата за аренду сервера ([тарифы Yandex BareMetal](../pricing.md));
...
- Консоль управления {#console}
  ...
  1. На панели сверху нажмите кнопку ![plus-sign](../../_assets/console-icons/plus.svg) **Создать новый ключ** и выберите `Создать авторизованный ключ`.
...
Активация сервиса запускает работу провайдера резервного копирования. Подробнее о провайдере резервного копирования и передаваемых ему данных в разделе [Активация сервиса и провайдер резервного копирования](../../backup/concepts/index.md#providers).
...
- Консоль управления {#console}
  ...
  1. Нажмите кнопку **Заказать сервер** и в открывшемся окне выберите вариант `Выбрать из готовых` и подходящую [конфигурацию](../concepts/server-configurations.md) сервера BareMetal.
  ...
  1. (Опционально) В блоке **Диск** настройте разметку [дисков](../concepts/disks/disk-types.md):
     ...
     1. Нажмите кнопку **Сохранить**.
        ...
        Параметры разметки диска важны для дальнейшего восстановления сервера из резервной копии. Подробнее в инструкции [Восстановить виртуальную машину Yandex Compute Cloud или сервер Yandex BareMetal из резервной копии](../../backup/operations/backup-vm/recover.md).
  ...
  1. В блоке **Сетевые интерфейсы** в разделе **Интерфейс 1**:
     ...
     * В поле **Публичный адрес** выберите способ назначения публичного адреса:
         ...
         * `Из выделенной подсети` — чтобы назначить IP-адрес из диапазона адресов [выделенной публичной подсети](../concepts/public-network.md#public-subnet).
             ...
             В появившемся поле выберите публичную подсеть или нажмите кнопку **Заказать**, чтобы [заказать](../operations/reserve-public-subnet.md) новую.
...
- CLI {#cli}
  ...
  1. Арендуйте сервер:
      ...
      * `--network-interfaces` — сетевые интерфейсы сервера:
        ...
        * `private-interface` — приватный интерфейс. В параметре `native-subnet-id` укажите идентификатор [приватной подсети](../concepts/private-network.md#private-subnet).
        * `public-interface` — публичный интерфейс. В настройке `native-subnet` укажите идентификатор [выделенной публичной подсети](../concepts/public-network.md#public-subnet) в параметре `subnet-id`. Необязательная настройка.
...
Подробнее об аренде сервера смотрите в [документации BareMetal](../operations/servers/server-lease.md).
...
- Консоль управления {#console}
  ...
  1. В открывшемся окне в поле **Тип ресурса** выберите `Серверы BareMetal` и в списке серверов выберите нужный сервер.
...
- Консоль управления {#console}
  ...
  1. [Перейдите](https://console.yandex.cloud/link/backup) в сервис **Cloud Backup**.
  1. На панели слева выберите ![server](../../_assets/console-icons/server.svg) **Подключённые ресурсы** и перейдите на вкладку **Серверы BareMetal**.
  1. В строке с нужным сервером нажмите значок ![options](../../_assets/console-icons/ellipsis.svg) и выберите **Создать резервную копию**.
  ...
  Запустится процесс создания резервной копии сервера BareMetal. Прогресс создания копии будет отображаться в строке соответствующего сервера в поле **Статус операции**.
...
Восстановить резервную копию виртуальной машины на сервер или резервную копию сервера на виртуальную машину невозможно. При этом вы можете восстановить резервную копию [внешнего](../../backup/concepts/vm-connection/external-resources.md) сервера на [сервер Yandex BareMetal](../../backup/concepts/vm-connection/baremetal.md).
...
Чтобы избежать ошибок при восстановлении из резервной копии, перед восстановлением рекомендуется сопоставить параметры дисков и разделов резервной копии с параметрами дисков и разделов ВМ или сервера, на которые будет выполняться восстановление. Подробнее в разделе [Посмотреть параметры дисков и разделов в резервной копии](../../backup/operations/backup-vm/view-disk-layout.md).
...
- Консоль управления {#console}
  ...
  1. [Перейдите](https://console.yandex.cloud/link/backup) в сервис **Cloud Backup**.
  1. На панели слева выберите ![backups](../../_assets/console-icons/archive.svg) **Резервные копии** и перейдите на вкладку **Серверы BareMetal**.
  1. В строке с резервной копией, из которой нужно восстановить сервер BareMetal, нажмите значок ![image](../../_assets/console-icons/ellipsis.svg) и выберите **Восстановить сервер BareMetal**.
...
1. [Откажитесь](../operations/servers/server-lease-cancel.md) от аренды сервера BareMetal.
