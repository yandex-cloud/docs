...
В [публичной сети](public-network.md) для всех серверов {{ baremetal-name }}, в том числе для серверов с сетевыми картами со скоростью соединения более 1 Гбит/с и для серверов с группами агрегирования сетевых интерфейсов [MC-LAG](mc-lag.md), по умолчанию включен пакет потребления трафика:
