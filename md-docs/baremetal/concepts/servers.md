...
Любая [конфигурация](server-configurations.md) сервера {{ baremetal-name }} включает в себя как минимум два [сетевых интерфейса](https://ru.wikipedia.org/wiki/Сетевая_плата), которые могут иметь следующие варианты подключения:
...
При этом у каждого сервера {{ baremetal-name }}:
...
Если сетевой интерфейс сервера поддерживает подключение как к приватной, так и к публичной сети, то для такого интерфейса вы можете изменять тип сети с публичной на приватную или наоборот. Подробнее об изменении типа сети для сетевого интерфейса читайте в разделе [{#T}](../operations/servers/switch-network-type.md).
...
К сетевому интерфейсу (или группе агрегирования MC-LAG) сервера, которые подключены к [приватной сети](private-network.md), можно [привязать](../operations/servers/set-up-tagged-vlan.md) одновременно несколько [приватных подсетей](private-network.md#private-subnet). Подробнее о добавлении дополнительных приватных подсетей читайте в разделе [{#T}](../operations/servers/set-up-tagged-vlan.md).
...
В течение всего срока карантина можно восстановить сервер для дальнейшего использования. Подробнее на странице [{#T}](../operations/servers/restore-from-quarantine.md).
