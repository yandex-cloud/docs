...
- Linux/macOS {#linux-macos}
  ...
  Для подключения к серверу необходимо указать его публичный IP-адрес, который можно узнать в консоли управления, в поле **Публичный IP-адрес** блока **Публичная сеть** на странице сервера.
