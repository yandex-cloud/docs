[Документация Yandex Cloud](../../../index.md) > [Yandex BareMetal](../../index.md) > [Пошаговые инструкции](../index.md) > Серверы > Настроить группу агрегирования MC-LAG

# Настроить группу агрегирования MC-LAG

Серверы с поддержкой [MC-LAG](../../concepts/mc-lag.md) подключены к каждой из сетей ([публичной](../../concepts/public-network.md) и [приватной](../../concepts/private-network.md)) одновременно с помощью двух сетевых карт. При этом для обеспечения отказоустойчивости каждая пара сетевых интерфейсов, подключенных к сети, на стороне сервера должна быть объединена в _группу агрегирования_. Подробнее в разделе [Особенности настройки групп агрегирования и сетевых интерфейсов](../../concepts/mc-lag.md#additional-setup).
...
- Ubuntu/Debian (Netplan) {#ubuntu}
  ...
  1. Узнайте, какие из интерфейсов относятся к публичной сети, а какие — к приватной:
        ...
        1. Перейдите в сервис **BareMetal** и в открывшемся списке серверов выберите нужный сервер.

            На открывшейся странице в разделе **Сетевые интерфейсы** в поле `MAC-адрес` указаны MAC-адреса интерфейсов, подключенных соответственно к публичной и приватной сетям.
  ...
  1. Настройте конфигурацию Netplan, добавив в нее группы агрегирования (раздел `bonds`):
      ...
      * выключен (значение `dhcp4: false`) для индивидуальных интерфейсов (раздел `ethernets`).
      ...
      Если к серверу привязаны [дополнительные приватные подсети](../../concepts/private-network.md#tagged-vlan-subnet), настройте на сервере виртуальные сети VLAN. Подробнее читайте в разделе [Настроить на сервере дополнительную приватную подсеть с тегированным VLAN](set-up-tagged-vlan.md).
