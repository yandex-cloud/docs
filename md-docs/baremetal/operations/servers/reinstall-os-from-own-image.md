[Документация Yandex Cloud](../../../index.md) > [Yandex BareMetal](../../index.md) > [Пошаговые инструкции](../index.md) > Серверы > Переустановить ОС сервера > Переустановить ОС из своего ISO-образа

# Переустановить операционную систему из своего ISO-образа

Yandex BareMetal позволяет пользователям устанавливать и переустанавливать операционную систему сервера из собственных ISO-образов. Таким способом вы можете установить на сервер операционные системы семейств [Linux](https://ru.wikipedia.org/wiki/Linux) или [Windows](https://ru.wikipedia.org/wiki/Windows). При этом, если устанавливаемая ОС требует лицензии, пользователь должен использовать собственную лицензию.

При установке или переустановке ОС из [своего ISO-образа](../../concepts/images.md#user-images) вы можете произвольным образом [перераспределять](../../concepts/server-advanced-settings.md#storage-management) [дисковое пространство](../../concepts/disks/disk-types.md), доступное на сервере.

Создание отказоустойчивой дисковой разметки требует опыта и понимания технологий [RAID](../../concepts/disks/raid.md) и/или [LVM](https://ru.wikipedia.org/wiki/LVM), поэтому рекомендуется использовать установку ОС из публичного образа Marketplace, а приведенный в этой инструкции способ установки использовать только при необходимости внесения существенных изменений, недоступных при установке из Marketplace.

Например, установка из своего ISO-образа может потребоваться, если нужная ОС недоступна в Marketplace, либо если требуется нестандартная конфигурация разметки дисков, режим [UEFI](../../concepts/server-advanced-settings.md#install-os-uefi)/[SecureBoot](https://ru.wikipedia.org/wiki/Secure_Boot), шифрование корневой системы или другие настройки, недоступные при установке из образа Marketplace.
...
1. [Загрузите](../image-upload.md#upload-file) ISO-образ в [Yandex Object Storage](../../../storage/index.md) и получите ссылку на загруженный объект:
   ...
   1. Создайте бакет Object Storage:
       ...
       - Консоль управления {#console}
         ...
         1. На странице создания бакета:
             ...
             1. Включите опцию **Установить максимальный размер бакета**, чтобы задать максимальный размер бакета.
   ...
   1. Загрузите скачанный файл с образом в созданный бакет:
       ...
       - Консоль управления {#console}
         ...
         1. В [консоли управления](https://console.yandex.cloud) выберите каталог, в котором находится созданный бакет.
         1. [Перейдите](https://console.yandex.cloud/link/storage) в сервис **Object Storage** и перейдите в бакет, в который нужно загрузить образ.
   ...
   1. Получите ссылку на загруженный в бакет образ:
       ...
       - Консоль управления {#console}
         ...
         1. [Перейдите](https://console.yandex.cloud/link/storage) в сервис **Object Storage** и выберите нужный бакет.
       ...
       {% endlist %}
1. [Создайте](../image-upload.md#create-image) из загруженного ISO-образа образ BareMetal:
    ...
    - Консоль управления {#console}
      ...
      1. Вставьте ссылку на образ, полученную в Object Storage на предыдущем шаге.
...
1. [Подключитесь](server-kvm.md) к KVM-консоли сервера.
    ...
    Дальнейшие действия по установке операционной системы будут выполняться в окне KVM-консоли. Монтирование ISO-образа можно выполнить как в окне KVM-консоли, так и через вкладку **KVM console** в [консоли управления](https://console.yandex.cloud).

    {% endnote %}

1. Смонтируйте ISO-образ на виртуальный CD-дисковод сервера. Для серверов на поддерживаемых платформах доступны два способа монтирования:

    {% note info %}
    
    В Yandex BareMetal поддерживаются серверы различных платформ. Доступность кнопки **Mount Image** на вкладке **KVM console** в [консоли управления](https://console.yandex.cloud) и пунктов меню **Media** → **Virtual Media Wizard...** в окне KVM-консоли зависит от платформы сервера. Если в KVM-консоли вы не видите нужную кнопку или пункт меню, обратитесь в [техническую поддержку](https://center.yandex.cloud/support).
    
    {% endnote %}

    {% list tabs %}

    - Через вкладку KVM console (рекомендуется)

      Способ доступен для серверов на поддерживаемых платформах. Смонтированный таким образом ISO-образ не размонтируется при закрытии окна KVM-консоли и остается смонтированным до указанного времени.

      1. На странице сервера в [консоли управления](https://console.yandex.cloud) перейдите на вкладку **KVM console**.
      1. Нажмите кнопку **Mount Image**.
      1. В открывшемся диалоге выберите [загруженный ранее](../image-upload.md) ISO-образ операционной системы и подтвердите монтирование.

      Если образ смонтирован, над KVM-консолью отображается надпись **Mounted Image** и время, до которого образ будет смонтирован.

    - Через окно KVM-консоли

      Способ доступен для всех серверов. При закрытии окна KVM-консоли образ размонтируется.
...
1. Чтобы запустить сервер с выбранного ISO-образа, выберите один из способов:

    {% list tabs %}

    - С помощью кнопки Reboot to cdrom

      В правом верхнем углу KVM-консоли нажмите кнопку **Reboot to cdrom**.

      Сервер перезапустится и будет загружен с виртуального CD-дисковода в режиме `UEFI`.

    - Через BIOS

      Этот способ позволяет принудительно выбрать загрузочный носитель и режим `UEFI` в [BIOS](https://ru.wikipedia.org/wiki/BIOS). В частности, на сервере с конфигурацией `BA-i201-H`, чтобы установить операционную систему в режиме UEFI, необходимо загрузиться с CD-дисковода в режиме UEFI.

      1. [Перезапустите сервер](server-stop-and-start.md#restart).
...
1. Чтобы продолжить установку, переведите курсор на **Done** и нажмите **Enter**.
    ...
    В появившейся форме **Confirm destructive action** нажмите **Continue**, чтобы подтвердить свое согласие на форматирование дисков.
...
1. Отключите виртуальный CD-дисковод. Способ отключения зависит от того, каким способом был смонтирован ISO-образ:

    {% list tabs %}

    - Через вкладку KVM console

      На странице сервера в [консоли управления](https://console.yandex.cloud) на вкладке **KVM console** нажмите кнопку отключения смонтированного образа.
