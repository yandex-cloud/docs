...
- Консоль управления {#console}
  ...
  1. Укажите параметры хоста:
     ...
     * (Опционально) Выберите опцию **Публичный доступ**, если хост должен быть доступен извне Yandex Cloud.
...
Если после изменения хоста к нему невозможно [подключиться](connect/index.md), убедитесь, что [группа безопасности](../concepts/network.md#security-groups) кластера Yandex StoreDoc настроена корректно для подсети, в которую помещен хост.
...
Если хост является первичным в момент удаления, Yandex StoreDoc автоматически выберет новый мастер.
...
1. Хост перестает принимать запросы на запись. Если хост был мастером (`PRIMARY`), Yandex StoreDoc попытается сделать его вторичной репликой (`SECONDARY`). В случае неудачи операция прерывается.
...
При перезагрузке мастера не происходит его автоматического переключения. Чтобы избежать недоступности кластера Yandex StoreDoc, [переключите мастер](stepdown.md) кластера до его перезагрузки.
