[Документация Yandex Cloud](../../index.md) > [Yandex StoreDoc](../index.md) > [Пошаговые инструкции](index.md) > Кластеры > Изменение настроек кластера
...
- Консоль управления {#console}
  ...
  1. Измените дополнительные настройки кластера:
     ...
     - **Перепаковка** — включите опцию, чтобы автоматически выполнять перепаковку для освобождения дискового пространства.
       
       Перепаковка выполняется во время [окна обслуживания](../concepts/maintenance.md#maintenance-window) не чаще одного раза в неделю. В кластерах с одним хостом перепаковка не выполняется.
       
       Для перепаковки отбираются коллекции, у которых процент раздувания не меньше, чем **Мин. процент раздувания (bloating) в коллекции**. Если предварительная оценка показывает, что перепаковка отобранных коллекций позволит освободить не менее, чем **Мин. суммарный объём освобождаемого дискового пространства**, то перепаковка выполняется.
       
       Задайте параметры перепаковки:
       
       * **Мин. суммарный объём освобождаемого дискового пространства** — минимальный суммарный объем дискового пространства в мегабайтах, который должен освободиться при перепаковке коллекций. Значение по умолчанию — `10` МБ.
       * **Мин. процент раздувания (bloating) в коллекции** — минимальный процент раздувания коллекции, при котором коллекция отбирается для перепаковки. Значение по умолчанию — `10%`.
       
       Чтобы выполнять перепаковку на хосте-мастере, включите опцию **Перепаковка мастер-реплики**. Перед перепаковкой хоста-мастера роль мастера переключается на другой хост. Если опция отключена, перепаковка хоста-мастера не выполняется.
     ...
     - **Защита от удаления** — защита кластера от непреднамеренного удаления.
       ...
       Настроить защиту от удаления для базы данных можно при [создании базы данных](databases.md#add-db) или [изменении ее настроек](databases.md#update-db), а для пользователя — при [создании пользователя](cluster-users.md#adduser) или [изменении его настроек](cluster-users.md#updateuser).
  ...
  Чтобы изменить дополнительные настройки кластера:
    ...
    1. Выполните команду, передав список настроек, которые хотите изменить:
        ...
        * `--backup-retain-period` — срок хранения автоматических резервных копий (в днях).
          ...
          Значение параметра `<срок_хранения>` задается в диапазоне от 7 до 60 (по умолчанию — 7). Эта функциональность находится на стадии [Preview](../../overview/concepts/launch-stages.md). Подробнее читайте в разделе [Резервные копии](../concepts/backup.md).
  ...
  1. Создайте файл `body.json` и добавьте в него следующее содержимое:
      ...
      * `config_spec` — настройки кластера:
        ...
        * `autocompact_config` — настройки автоматической перепаковки:
          ...
          * `enabled` — автоматическая перепаковка для освобождения дискового пространства: `true` или `false`.
          * `bloat_percent` — минимальный процент раздувания коллекции, при котором коллекция отбирается для перепаковки. Значение по умолчанию — `10%`.
          * `target_free_space` — минимальный суммарный объем дискового пространства в мегабайтах, который должен освободиться при перепаковке коллекций. Значение по умолчанию — `10` МБ.
          * `compaction_type` — перепаковка хоста-мастера. Возможные значения:
          ...
          Для перепаковки отбираются коллекции, у которых процент раздувания не меньше `bloat_percent`. Если предварительная оценка показывает, что их перепаковка позволит освободить не меньше `target_free_space`, то перепаковка выполняется.
