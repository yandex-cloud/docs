[Документация Yandex Cloud](../../index.md) > [Yandex StoreDoc](../index.md) > [Пошаговые инструкции](index.md) > Кластеры > Управление политиками резервного копирования

# Управление политиками резервного копирования в Yandex StoreDoc
...
- Консоль управления {#console}
    ...
    1. Укажите параметры новой политики:
        ...
        1. Выберите один из вариантов запуска резервного копирования:
            ...
            * **Каждый день**.
            * **По дням недели**. Для этого варианта дополнительно выберите один или несколько дней недели.
...
- CLI {#cli}
    ...
    1. Посмотрите описание команды CLI для создания политики:
        ...
        ```bash
        yc managed-mongodb backup-retention-policy create --help
        ```
...
- Terraform {#tf}
    ...
    1. Добавьте описание ресурса:
        ...
        ```hcl
        resource "yandex_mdb_mongodb_backup_retention_policy" "<имя_политики>" {
        ...
        * `cron` — настройки расписания, по которому создаются резервные копии:
        
            * `day_of_month` — день месяца. Возможные значения: `1–31`.
            * `day_of_week` — день недели. Возможные значения: `0–6` или `SUN–SAT`.
            * `month` — месяц. Возможные значения: `1–12` или `JAN–DEC`.
        
            Расписание задается в виде cron-выражения, но часы и минуты не указываются.
    ...
    1. Подтвердите изменение ресурсов.
        ...
        Подробнее в [документации провайдера Terraform](../../terraform/resources/mdb_mongodb_backup_retention_policy.md).
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [BackupRetentionPolicy.Create](../api-ref/BackupRetentionPolicy/create.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
        ...
        curl \
          ...
          --url 'https://mdb.api.cloud.yandex.net/managed-mongodb/v1/clusters/<идентификатор_кластера>/retention_policies' \
          --data '{
                     "cron": {
    ...
    1. Чтобы создать политику:
        ...
        1. Воспользуйтесь вызовом [BackupRetentionPolicyService.Create](../api-ref/grpc/BackupRetentionPolicy/create.md) и выполните запрос, например, с помощью [gRPCurl](https://github.com/fullstorydev/grpcurl):
            ...
            grpcurl \
                ...
                -proto ~/cloudapi/yandex/cloud/mdb/mongodb/v1/backup_retention_policy_service.proto \
...
- CLI {#cli}
    ...
    1. Посмотрите описание команды CLI для получения списка политик:
        ...
        ```bash
        yc managed-mongodb backup-retention-policy list --help
        ```

    1. Получите список политик для кластера Yandex StoreDoc:
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [BackupRetentionPolicy.List](../api-ref/BackupRetentionPolicy/list.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
        ...
        curl \
          --request GET \
          --header "Authorization: Bearer $IAM_TOKEN" \
          --url 'https://mdb.api.cloud.yandex.net/managed-mongodb/v1/clusters/<идентификатор_кластера>/retention_policies?pageSize=<количество_результатов>'
        ```
    ...
    1. Убедитесь, что запрос был выполнен успешно, изучив [ответ сервера](../api-ref/BackupRetentionPolicy/list.md#yandex.cloud.mdb.mongodb.v1.ListBackupRetentionPoliciesResponse).
    ...
    1. Чтобы получить список политик для кластера:
        ...
        1. Воспользуйтесь вызовом [BackupRetentionPolicyService.List](../api-ref/grpc/BackupRetentionPolicy/list.md) и выполните запрос, например, с помощью [gRPCurl](https://github.com/fullstorydev/grpcurl):
            ...
            grpcurl \
              ...
              mdb.api.cloud.yandex.net:443 \
              yandex.cloud.mdb.mongodb.v1.BackupRetentionPolicyService.List
            ```
        ...
        1. Убедитесь, что запрос был выполнен успешно, изучив [ответ сервера](../api-ref/grpc/BackupRetentionPolicy/list.md#yandex.cloud.mdb.mongodb.v1.ListBackupRetentionPoliciesResponse).
...
- CLI {#cli}
    ...
    1. Посмотрите описание команды CLI для удаления политики:
        ...
        ```bash
        yc managed-mongodb backup-retention-policy delete --help
        ```

    1. Удалите политику:

        ```bash
        yc managed-mongodb backup-retention-policy delete <идентификатор_политики> \
           --cluster-id <идентификатор_кластера>
        ```
...
- Terraform {#tf}
    ...
    1. Удалите ресурс `yandex_mdb_mongodb_backup_retention_policy` с именем удаляемой политики.
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [BackupRetentionPolicy.Delete](../api-ref/BackupRetentionPolicy/delete.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
        ...
        curl \
          --request DELETE \
          --header "Authorization: Bearer $IAM_TOKEN" \
          --url 'https://mdb.api.cloud.yandex.net/managed-mongodb/v1/clusters/<идентификатор_кластера>/retention_policies/<идентификатор_политики>'
        ```
