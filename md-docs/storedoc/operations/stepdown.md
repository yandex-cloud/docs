[Документация Yandex Cloud](../../index.md) > [Yandex StoreDoc](../index.md) > [Пошаговые инструкции](index.md) > Шардирование и репликация > Переключение мастера

# Переключение мастера

Если в кластере есть [несколько реплик](../concepts/replication.md), то при необходимости можно переключить мастер.
...
- CLI {#cli}
    ...
    Чтобы переключить мастер в кластере, выполните команду:

    ```bash
    yc managed-mongodb hosts stepdown <имя_текущего_мастера> \
       --name=<имя_кластера>
    ```

    Имя мастера для нужного шарда можно получить со [списком хостов в кластере](hosts.md#list), имя кластера — со [списком кластеров в каталоге](cluster-list.md#list-clusters).
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [Cluster.StepdownHosts](../api-ref/Cluster/stepdownHosts.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
        ...
        Идентификатор кластера можно получить со [списком кластеров в каталоге](cluster-list.md#list-clusters), а имя мастера для нужного шарда — со [списком хостов в кластере](hosts.md#list).
