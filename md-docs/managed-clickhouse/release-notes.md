...
Встречайте {{ CK }}! Он поможет координировать и распределять запросы между хостами ваших кластеров {{ CH }}. Клиент-серверный протокол {{ CK }} совместим с {{ ZK }}, поэтому для взаимодействия с ним можно использовать любой стандартный клиент {{ ZK }}.
...
* Добавлена возможность включения [диагностики производительности](operations/performance-diagnostics.md) в версии {{ TF }} 0.202.0 и в версии CLI 1.8.0. {{ tag-tf }} {{ tag-cli }}
* Добавлена возможность [восстанавливать кластер](operations/cluster-backups.md) с помощью {{ TF }}. {{ tag-tf }}
...
* Доступны новые версии {{ CH }}: [26.3 LTS](https://clickhouse.com/blog/clickhouse-release-26-03) и [26.4](https://clickhouse.com/blog/clickhouse-release-26-04).
...
* Добавлен инструмент **Диагностика производительности** для анализа производительности кластера через GUI в реальном времени. Функциональность находится на стадии [Preview](../overview/concepts/launch-stages.md). Для участия в тестировании обратитесь в [техническую поддержку]({{ link-console-support }}).
* Добавлена возможность включить `mysql_protocol` через {{ TF }}. {{ tag-tf }}
* Доступна новая версия {{ CH }} 26.1. Добавлена поддержка операций `DELETE` для Delta Lake.
* В консоли управления теперь отображается полная версия СУБД, что позволяет отслеживать минорные обновления. {{ tag-con }}
...
* Доступна новая версия {{ CH }} [25.12](https://clickhouse.com/blog/clickhouse-release-25-12). В ней, например, стали быстрее алгоритмы `INNER JOIN`.
...
* Улучшена реализация методов `stats` и `Persistent Watches` в [{{ CK }}](concepts/replication.md#ck) для поддержки протокола совместимого с {{ ZK }}.
...
* Сервис координации {{ CK }} при использовании на отдельных хостах [тарифицируется](pricing.md#prices-keeper). Он работает быстрее {{ ZK }} и использует алгоритм консенсуса [Raft](https://ru.wikipedia.org/wiki/Raft_(алгоритм)). Такая конфигурация рекомендуется для production-среды.
* Доступна новая версия {{ CH }} [25.11](https://clickhouse.com/blog/clickhouse-release-25-11).
...
* Доступны новые версии {{ CH }}: [25.3](https://clickhouse.com/blog/clickhouse-release-25-03), [25.4](https://clickhouse.com/blog/clickhouse-release-25-04), [25.5](https://clickhouse.com/blog/clickhouse-release-25-05) и [25.6](https://clickhouse.com/blog/clickhouse-release-25-06).
