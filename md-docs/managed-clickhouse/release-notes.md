[Документация Yandex Cloud](../index.md) > [Yandex Managed Service for ClickHouse®](index.md) > История изменений
...
* Для CLI и API добавлена поддержка аутентификации через IAM.
* Добавлена поддержка самоподписных сертификатов.
* Улучшена логика восстановления из резервной копии. Теперь прогресс восстановления сохраняется чаще — не после обработки каждой таблицы целиком, а с более мелкой периодичностью. Это позволяет значительно сократить время восстановления кластера.
* Добавлена поддержка [Replicated](https://clickhouse.com/docs/reference/engines/database-engines/replicated) баз данных.
* В консоли управления добавлена возможность запланировать синхронизацию реплик.
* При добавлении новых реплик теперь по умолчанию включено копирование схемы данных. Это позволяет обеспечить консистентность структур таблиц между репликами.
* Добавлена возможность создания кластера из резервной копии в Terraform.
...
* Доступна новая версия ClickHouse®: [26.5](https://clickhouse.com/blog/clickhouse-release-26-05)
...
* Добавлены настройки гибридного хранилища: `data_cache_enabled`, `data_cache_max_size` и `move_factor`.
