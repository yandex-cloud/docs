...
- Консоль управления {#console}
  ...
  * Настройки хранения данных на SSD для способов `ssd_cache` и `complex_key_ssd_cache`:
    ...
    * **Размер блока** — размер блока чтения в байтах. Значение по умолчанию — `4096` (4 КБ).
    * **Размер файла кеша** — максимальный размер файла кеша в байтах. Значение по умолчанию — `4294967296` (4 ГБ).
    * **Размер буфера чтения** — размер буфера в оперативной памяти для чтения данных с SSD в байтах. Значение по умолчанию — `65536` (64 КБ).
    * **Размер буфера записи** — размер буфера в оперативной памяти для записи данных на SSD в байтах. Значение по умолчанию — `4096` (4 КБ).

    Подробнее о настройках для способов `*_ssd_cache` читайте в [документации ClickHouse®](https://clickhouse.com/docs/ru/reference/statements/create/dictionary/layouts/ssd-cache).

  * **Столбец начала диапазона:** — столбец с данными, который соответствует началу диапазона. Используется для способов `range_hashed` и `complex_key_range_hashed`. Укажите:

    * **Имя** — имя столбца.
    * **Тип** — тип данных столбца.

  * **Столбец конца диапазона:** — столбец с данными, который соответствует концу диапазона. Используется для способов `range_hashed` и `complex_key_range_hashed`. Укажите:

    * **Имя** — имя столбца.
    * **Тип** — тип данных столбца.

  * **Составной ключ** — столбцы, образующие составной ключ словаря. Используется для способов `complex_key_*` и `ip_trie`:
  ...
  * **Столбцы данных** — описание столбцов в таблице-источнике, которые содержат данные для словаря. Используется для способов `flat`, `hashed`, `range_hashed`, `cache`, `sparse_hashed`, `direct`, `complex_key_*`, `ip_trie` и `ssd_cache`. Для каждого элемента укажите:
  ...
  * **Частота обновления** — периодичность обновления словаря. Выберите тип периода обновления и его настройки:
