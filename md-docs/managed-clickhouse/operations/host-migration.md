...
1. Дождитесь, когда кластер перейдет в состояние **Alive**. В консоли управления [перейдите](../../console/operations/select-service.md#select-service) в сервис **Managed Service for&nbsp;ClickHouse**. Состояние кластера отображается в столбце **Доступность**.
