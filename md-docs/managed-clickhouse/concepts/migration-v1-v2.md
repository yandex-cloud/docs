[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for ClickHouse®](../index.md) > Справочник Terraform > Изменения в кластере v2

# Изменения в кластере v2

[Ресурс v1](../../terraform/resources/mdb_clickhouse_cluster.md) (`yandex_mdb_clickhouse_cluster`) устарел и будет удален в одной из следующих версий провайдера.

[Ресурс v2](../../terraform/resources/mdb_clickhouse_cluster_v2.md) (`yandex_mdb_clickhouse_cluster_v2`) реализован на базе [Terraform Plugin Framework](https://developer.hashicorp.com/terraform/plugin/framework) и решает ряд проблем ресурса v1:
...
* Изменения, которые Terraform отображает в `plan`, не всегда соответствовали результату `apply`.
...
| `clickhouse.config.kafka_topic { }` | В v2 не поддерживается конфигурация Apache Kafka® на уровне отдельных топиков. |
...
| `extension` | `extension { name = "..." version = "..." }` | Расширение кластера ClickHouse®. Задается в виде блока. Поддерживается только в v2, отсутствовало в v1. |
...
| `mysql_protocol` | Поле типа `bool`, которое позволяет использовать протокол совместимости с MySQL®. |
...
* **Не поддерживается в данной версии ClickHouse®.**
    ...
    Некоторые поля доступны только с определенной версии. Требования к версии описаны в [документации провайдера](../../terraform/index.md).
