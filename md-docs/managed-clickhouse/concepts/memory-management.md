...
В отличие от других [настроек СУБД](settings-list.md) значение настройки **Max server memory usage** нельзя изменять напрямую. Managed Service for ClickHouse® выставляет для нее значение автоматически в зависимости от объема оперативной памяти хостов ClickHouse® (RAM):
