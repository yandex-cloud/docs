[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for ClickHouse®](../index.md) > [Концепции](index.md) > Репликация
...
Кластер Managed Service for ClickHouse®, в котором функционирует репликация с использованием сервиса координации ClickHouse® Keeper или ZooKeeper на отдельных хостах, считается [высокодоступным](high-availability.md). В таком кластере доступно создание [реплицируемых таблиц](#replicated-tables) и [реплицируемых баз данных](#replicated-db).
