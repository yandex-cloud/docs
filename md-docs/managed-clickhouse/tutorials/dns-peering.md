[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for ClickHouse®](../index.md) > [Практические руководства](index.md) > Настройка Yandex Cloud DNS для доступа к кластеру из других облачных сетей
...
1. Подготовьте инфраструктуру:
    ...
    - Вручную
        ...
        1. В сети `mch-net` [создайте кластер](../operations/cluster-create.md) Managed Service for ClickHouse® любой подходящей конфигурации с хостами без публичного доступа.
        ...
        1. Настройте правила групп безопасности кластера и ВМ [по инструкции](../operations/connect/index.md#configuring-security-groups)
...
1. (Опционально) [Подключитесь по SSH](../../compute/operations/vm-connect/ssh.md#vm-connect) к виртуальной машине `mch-net-vm` и [настройте подключение к кластеру](../operations/connect/clients.md) через `clickhouse-client`, чтобы убедиться, что группы безопасности настроены верно и подключение к кластеру по FQDN из той же облачной сети доступно.
...
1. [Настройте подключение к кластеру](../operations/connect/clients.md) через `clickhouse-client` и убедитесь, что подключение к кластеру из другой облачной сети по FQDN работает.
