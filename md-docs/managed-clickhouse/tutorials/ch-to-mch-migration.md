...
Вы также можете использовать этот способ для переноса данных между двумя кластерами {{ mch-name }}. Для успешного переноса в эндпоинте-источнике укажите порты `8443` и `9440`, а также добавьте [SSL-сертификат](../operations/connect/index.md#get-ssl-cert).
...
1. Подготовьте инфраструктуру:
    ...
    - Вручную {#manual}
        ...
        1. [Создайте кластер-приемник {{ mch-name }}](../operations/cluster-create.md), вычислительная мощность и размер хранилища которого соответствуют среде, в которой развернута копируемая база данных.
...
1. Убедитесь, что в базу данных {{ mch-name }} перенеслись данные из кластера-источника {{ CH }}:
    ...
    - CLI {#cli}
        ...
        1. [Подключитесь к БД](../operations/connect/clients.md#clickhouse-client) в кластере {{ mch-name }} с помощью `clickhouse-client`.
