...
1. [Создайте кластер {{ mch-name }}](../operations/cluster-create.md) любой подходящей конфигурации. При создании кластера:
    ...
    * [Добавьте хосты {{ ZK }}](../operations/zk-hosts.md), чтобы обеспечить высокую доступность кластера.
...
1. Чтобы создать представление импортированных данных, [подключитесь к базе данных кластера {{ mch-name }}](../operations/connect/clients.md) и выполните SQL-запрос:
    ...
    * `<идентификатор_кластера>` — идентификатор кластера {{ mch-name }}. Его можно получить вместе со [списком кластеров в каталоге](../operations/cluster-list.md).
