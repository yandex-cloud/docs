[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for ClickHouse®](../index.md) > [Практические руководства](index.md) > Копирование данных из Managed Service for OpenSearch в Managed Service for ClickHouse® с помощью Yandex Data Transfer
...
- Вручную {#manual}
    ...
    1. В той же [зоне доступности](../../overview/concepts/geo-scope.md) [создайте кластер-приемник Managed Service for ClickHouse®](../operations/cluster-create.md#create-cluster) любой подходящей конфигурации с хостами в публичном доступе.
    ...
    1. Убедитесь, что группы безопасности кластеров [Managed Service for OpenSearch](../../managed-opensearch/operations/connect/index.md#security-groups) и [Managed Service for ClickHouse®](../operations/connect/index.md#configuring-security-groups) разрешают подключение через интернет.
...
1. Убедитесь, что в базу данных Managed Service for ClickHouse® перенеслись данные из кластера-источника Managed Service for OpenSearch:
    ...
    - CLI {#cli}
      ...
      1. Если у вас нет `clickhouse-client`, [установите его](../operations/connect/clients.md#clickhouse-client).
      1. [Подключитесь к БД](../operations/connect/clients.md#clickhouse-client) в кластере Managed Service for ClickHouse®.
