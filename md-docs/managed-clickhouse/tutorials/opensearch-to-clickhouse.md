...
- Вручную {#manual}
    ...
    1. В той же [зоне доступности](../../overview/concepts/geo-scope.md) [создайте кластер-приемник {{ mch-name }}](../operations/cluster-create.md#create-cluster) любой подходящей конфигурации с хостами в публичном доступе.
    ...
    1. Убедитесь, что группы безопасности кластеров [{{ mos-name }}](../../managed-opensearch/operations/connect/index.md#security-groups) и [{{ mch-name }}](../operations/connect/index.md#configuring-security-groups) разрешают подключение через интернет.
...
1. Убедитесь, что в базу данных {{ mch-name }} перенеслись данные из кластера-источника {{ mos-name }}:
    ...
    - CLI {#cli}
      ...
      1. [Получите SSL-сертификат](../operations/connect/index.md#get-ssl-cert) для подключения к кластеру {{ mch-name }}.
      1. Если у вас нет `clickhouse-client`, [установите его](../operations/connect/clients.md#clickhouse-client).
      1. [Подключитесь к БД](../operations/connect/clients.md#clickhouse-client) в кластере {{ mch-name }}.
