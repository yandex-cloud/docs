...
* Постоянно запущенный кластер {{ mch-name }} (см. [тарифы {{ mch-name }}](../pricing.md));
...
1. Укажите параметры кластера:
    ...
    * {{ ui-key.yacloud.mdb.forms.section_service-settings }}:
    ...
    Полный список настроек см. в разделе [Настройки {{ mch-name }}](../concepts/settings-list.md).
...
* [Удалите {{ CH }}-кластер](../operations/cluster-delete.md);
