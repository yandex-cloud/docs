[Документация Yandex Cloud](../../../../index.md) > [Yandex Monitoring](../../../index.md) > [Yandex Managed Service for Prometheus®](../index.md) > [Чтение метрик](index.md) > Remote Read
...
1. [Создайте сервисный аккаунт](../../../../iam/operations/sa/create.md) с ролью `monitoring.viewer` на выбранный каталог.
...
1. В [конфигурацию Prometheus](https://prometheus.io/docs/prometheus/latest/configuration/configuration/#remote_read), в секцию `remote_read`, добавьте эндпоинт (`url`):
   ...
   remote_read:
     ...
     - url: 'https://monitoring.api.cloud.yandex.net/prometheus/workspaces/<Идентификатор_воркспейса>/api/v1/read'
   ...
   ```
1. Перезапустите Prometheus или перезагрузите конфигурацию. 

## Примеры ошибок {#errors}

| Код ошибки | Текст ошибки | Описание |
|----|----|----|
| `404` | Эндпоинт не найден. Убедитесь, что в конфигурации указан правильный `url`. | `remote_read: remote server https://monitoring.api.cloud.yandex.net/prometheus/workspaces/monb1piptmdo********/api/v1/read returned`<br/>`HTTP status 404 Not Found: {"status":"error","errorType":"not_found","error":"workspace not found: monb1piptmdo********"}` |
