[Документация Yandex Cloud](../../../index.md) > [Yandex Monitoring](../../index.md) > [Концепции](../index.md) > [Визуализация](index.md) > Виджеты

# Виджет

_Виджет_ — это элемент дашборда, отображающий информацию в виде графика или текста.

В сервисе Yandex Monitoring можно добавить виджеты:
...
Yandex Monitoring позволяет отображать на графиках метрики сервисов Yandex Cloud и пользовательские метрики, загруженные с помощью API.
...
* **Авто** — назначается одна из цветовых схем:
  * **Градиентная** — для графика типа **Область**, если все линии имеют хотя бы одно числовое значение.
  * **Стандартная** — во всех остальных случаях.
* **Стандартная** — цвет рассчитывается по установленному в системе алгоритму.
* **Градиентная** — плавный цветовой переход в зависимости от значения метрик: от красного (высокие значения) к зеленому (низкие).
* **По имени линии** — в этом случае для каждой линии будет рассчитано уникальное значение (хэш), на основе которого будет сгенерирован цвет. Линии с одинаковым названием почти всегда будут отображаться одинаково.
* **По порогам** — цвет линий определяется на основе заданных пользователем [пороговых значений](../../operations/dashboard/add-widget.md#thresholds) и выбранного метода агрегации данных. Позволяет визуально выделить линии, соответствующие критическим или целевым значениям.
