[Документация Yandex Cloud](../../../../index.md) > [Yandex Monitoring](../../../index.md) > [Агент сбора метрик](index.md) > Конфигурирование > Входы
...
Читайте также [настройка инфраструктуры работы с сессиями](services.md#flow_control).
...
```yaml
  - input:
    ...
    # Описание выходного канала для передачи метрик в Monitoring.
...
* В качестве источника данных можно указать конкретный файл или маску с параметром `multi_file`.
  ...
  В метаданные сессии добавляется ключ `file` — имя файла без пути. Чтобы записывать в метаданные полный путь к файлу, используется параметр `multi_add_full_path_to_meta`.
...
- input:
    ...
    config:
      ...
      # Для каждого эндпоинта (из параметра endpoints ниже) создается отдельная сессия.
