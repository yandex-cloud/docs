[Документация Yandex Cloud](../../index.md) > [Yandex Monitoring](../index.md) > [Справочник метрик](index.md) > Yandex Managed Service for Valkey™

# Метрики Yandex Managed Service for Valkey™

В этом разделе описаны метрики сервиса [Yandex Managed Service for Valkey™](../../managed-valkey/index.md), поставляемые в Monitoring.

Имя метрики пишется в метку `name`.

Общие метки для всех метрик сервиса Yandex Managed Service for Valkey™:
...
## Метрики CPU {#managed-valkey-cpu-metrics}
...
## Метрики диска {#managed-valkey-disk-metrics}
...
## Метрики дисковых операций {#managed-valkey-diskio-metrics}
...
## Метрики RAM {#managed-valkey-ram-metrics}
...
## Метрики сети {#managed-valkey-net-metrics}
...
## Метрики сервиса {#managed-valkey-metrics}
...
| `redis_acl_access_denied_auth`<br/>`DGAUGE`, штуки | Количество попыток подключения к Redis, завершившихся ошибкой авторизации из-за отказа в доступе. |
| `redis_acl_access_denied_channel`<br/>`DGAUGE`, штуки | Количество отказов в доступе при попытке операции с каналом из-за несоответствия ACL в Redis. |
| `redis_acl_access_denied_cmd`<br/>`DGAUGE`, штуки | Количество отказов в доступе при попытках выполнения команд, запрещенных политиками ACL в Redis. |
| `redis_acl_access_denied_key`<br/>`DGAUGE`, штуки | Количество отказов в доступе к ключам, защищенным политиками ACL в Redis. |
...
| `redis_child_oom_count`<br/>`DGAUGE`, штуки | Количество случаев завершения дочерних процессов Redis (например, во время создания снапшотов или форков) из-за нехватки памяти (Out of Memory, OOM). |
...
| `redis_expired_keys`<br/>`DGAUGE`, штуки | Общее количество ключей Redis, которые были автоматически удалены по истечении срока их действия (TTL). |
| `redis_hit_rate`<br/>`DGAUGE`, % | Доля запросов, данные для которых Yandex Managed Service for Valkey™ находит в кеше. |
| `redis_instantaneous_input_kbps`<br/>`DGAUGE`, Кбит/с | Текущий входящий трафик на шард Redis в килобитах в секунду. |
| `redis_instantaneous_input_repl_kbps`<br/>`DGAUGE`, Кбит/с | Текущий входящий трафик репликации на шард Redis в килобитах в секунду. |
| `redis_instantaneous_ops_per_sec`<br/>`DGAUGE`, операции/с | Количество запросов в секунду. |
| `redis_instantaneous_output_kbps`<br/>`DGAUGE`, Кбит/с | Текущий исходящий трафик с шарда Redis в килобитах в секунду. |
| `redis_instantaneous_output_repl_kbps`<br/>`DGAUGE`, Кбит/с | Текущий исходящий трафик репликации с шарда Redis в килобитах в секунду. |
...
| `redis_keys_db`<br/>`DGAUGE`, штуки | Общее количество ключей, хранящихся в базе данных Redis на конкретном шарде.<br/>Специальная метка `db` — номер БД. |
...
| `redis_mem_fragmentation_ratio`<br/>`DGAUGE`, коэффициент | Соотношение выделенной памяти к фактически используемой на конкретном шарде Redis (показатель фрагментации памяти). |
...
| `redis_result_code`<br/>`DGAUGE`, код | Код результата последней операции Redis на конкретном шарде. |
| `redis_total_commands_processed`<br/>`DGAUGE`, штуки | Общее количество команд, обработанных на конкретном шарде Redis с момента его запуска. |
| `redis_total_net_input_bytes`<br/>`DGAUGE`, байты | Общее количество байт входящего сетевого трафика на данный шард Redis с момента его запуска. |
| `redis_total_net_output_bytes`<br/>`DGAUGE`, байты | Общее количество байт исходящего сетевого трафика с данного шарда Redis с момента его запуска. |
| `redis_total_net_repl_input_bytes`<br/>`DGAUGE`, байты | Общее количество байт входящего сетевого трафика репликации на данный шард Redis с момента его запуска. |
| `redis_total_net_repl_output_bytes`<br/>`DGAUGE`, байты | Общее количество байт исходящего сетевого трафика репликации с данного шарда Redis с момента его запуска. |
...
| `redis_used_cpu_sys_main_thread`<br/>`DGAUGE`, секунды | Процессорное (системное) время, затраченное основным потоком данного шарда Redis с момента его запуска. |
| `redis_used_cpu_user_main_thread`<br/>`DGAUGE`, секунды | Процессорное время (в пространстве пользователя), затраченное основным потоком данного шарда Redis с момента его запуска. |
...
| `redis_used_memory`<br/>`DGAUGE`, байты | Фактическое использование памяти на хосте. Если значение параметра `redis_used_memory` достигнет значения параметра `redis_maxmemory`, при попытке вставить новые записи Yandex Managed Service for Valkey™ применит режим управления памятью, установленный настройкой [Maxmemory policy](../../managed-valkey/concepts/settings-list.md#settings-maxmemory-policy). |
| `sentinel_can_read`<br/>`DGAUGE`, 0/1 | Показатель доступности шарда Redis для чтения согласно данным Sentinel.<br/>Принимает значение `1`, если шард доступен для чтения, `0` — если нет. |
| `sentinel_can_write`<br/>`DGAUGE`, 0/1 | Показатель доступности шарда Redis для записи согласно данным Sentinel.<br/>Принимает значение `1`, если шард доступен для записи, `0` — если нет. |
| `sentinel_connected_clients`<br/>`DGAUGE`, штуки | Количество клиентов, в данный момент подключенных к данному инстансу Redis Sentinel. |
| `sentinel_is_alive`<br/>`DGAUGE`, 0/1 | Показатель доступности Redis Sentinel.<br/>Принимает значение `1`, если Sentinel доступен, `0` — если нет. |
| `sentinel_is_master`<br/>`DGAUGE`, 0/1 | Признак того, что наблюдаемый шард Redis является мастером согласно данным Sentinel.<br/>Принимает значение `1`, если шард — мастер, `0` — если нет. |
| `sentinel_total_error_replies`<br/>`DGAUGE`, штуки | Общее количество ошибочных ответов, возвращенных Redis Sentinel клиентам. Считается с момента запуска Sentinel. |
...
| `slowlog_new`<br/>`DGAUGE`, штук | Количество новых записей в slowlog на шарде Redis за последний интервал мониторинга. |
| `slowlog_top`<br/>`DGAUGE`, микросекунды | Максимальное время выполнения команды среди последних записей в slowlog на шарде Redis за последний интервал мониторинга.<br/>Специальная метка `cmd` — идентификатор команды. |
