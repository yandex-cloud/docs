...
В этом руководстве вы настроите мониторинг и уведомления об изменениях показаний датчиков, подключенных к сервису [{{ iot-name }}](../../iot-core/index.md). Датчики будут эмулированы с помощью сервиса [{{ sf-full-name }}](../../functions/index.md). Если у вас есть подключенные датчики, используйте их.
...
* плата за запись пользовательских [метрик](../concepts/data-model.md#metric) через API сервиса [{{ monitoring-name }}](../index.md) (см. [тарифы {{ monitoring-full-name }}](../pricing.md)).
...
Для наблюдения за показаниями датчиков используется дашборд в сервисе [{{ monitoring-full-name }}](../index.md). Показания датчиков поступают с контроллера на сервер раз в минуту по [MQTT-протоколу](../../glossary/mqtt-server.md#mqtt-protocol). Когда показания датчиков достигают заданных значений, {{ monitoring-name }} отправляет уведомления пользователям.
...
* [Удалите дашборд в сервисе {{ monitoring-name }}](../operations/dashboard/delete-dashboard.md)
* [Удалите алерт в сервисе {{ monitoring-name }}](../operations/alert/delete-alert.md)
* Удалите [канал уведомлений](../concepts/alerting/notification-channel.md) в сервисе {{ monitoring-name }}.