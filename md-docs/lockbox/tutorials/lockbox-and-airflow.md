...
* Плата за использование секрета {{ lockbox-name }}: (см. [тарифы {{ lockbox-name }}](../pricing.md)).
...
1. [Создайте сервисный аккаунт](../../iam/operations/sa/create.md#create-sa) `airflow-sa` с ролями:
   ...
   * `lockbox.payloadViewer`.
   ...
   Роль `lockbox.payloadViewer` не обязательно выдавать на весь каталог. Достаточно [назначить ее на конкретный секрет {{ lockbox-name }}](../operations/secret-access.md) после [его создания](#create-lockbox-secret).
...
[Создайте секрет {{ lockbox-name }}](../operations/secret-create.md) с параметрами:
...
1. [Секрет {{ lockbox-name }}](../operations/secret-delete.md).
