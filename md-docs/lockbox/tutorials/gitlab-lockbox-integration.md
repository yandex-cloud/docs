...
Вы можете безопасно хранить пароли в виде [секретов {{ lockbox-full-name }}](../concepts/secret.md) и использовать их в своих сценариях CI в {{ mgl-full-name }}. Так вы можете защитить и скрыть пароли в CI-скриптах от пользователей {{ GL }}, в том числе администраторов.
...
* Плата за хранение секрета и запросы к нему (см. [тарифы {{ lockbox-name }}](../pricing.md)).
...
1. [Создайте секрет](../operations/secret-create.md) {{ lockbox-name }} с названием `MY_SECRET`.
...
* [секрет {{ lockbox-name }}](../operations/secret-delete.md);
