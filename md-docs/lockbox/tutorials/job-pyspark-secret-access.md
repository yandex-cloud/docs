...
Вы можете использовать секрет [{{ lockbox-name }}](../concepts/secret.md) для подключения к кластеру [{{ mpg-full-name }}](../../managed-postgresql/index.md) из PySpark-задания в [{{ msp-full-name }}](../../managed-spark/index.md). Для этого [сервисному аккаунту](../../iam/concepts/users/service-accounts.md) кластера {{ msp-full-name }} необходимо предоставить доступ к секрету. Секрет создается сервисом {{ connection-manager-full-name }} автоматически при создании пользователя {{ mpg-name }}.
