[Документация Yandex Cloud](../../index.md) > [Yandex Lockbox](../index.md) > [Пошаговые инструкции](index.md) > Получить информацию о секрете
...
- Terraform {#tf}
  ...
  1. Добавьте в конфигурационный файл Terraform блоки `data` и `output`:
     ...
     * `output "current_version"` — выходная переменная, которая содержит информацию о текущей версии секрета:
     ...
     Вместо `current_version` вы можете выбрать любой другой параметр для получения информации. Подробнее о параметрах источника данных `yandex_lockbox_secret` в [документации провайдера](../../terraform/data-sources/lockbox_secret.md).
  1. Создайте ресурсы:
...
- Terraform {#tf}
  ...
  1. Добавьте в конфигурационный файл Terraform блоки `data` и `output`:
     ...
     * `output "my_secret_entries"` — выходная переменная, которая хранит содержимое секрета:
     ...
     Подробнее о параметрах источника данных `yandex_lockbox_secret_version` в [документации провайдера](../../terraform/data-sources/lockbox_secret_version.md).
