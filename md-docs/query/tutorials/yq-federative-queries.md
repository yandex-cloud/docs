...
Для работы с данными {{ objstorage-name }} вам понадобится бакет с таблицей, [подключение](../concepts/glossary.md#connection) {{ yq-name }} и [привязка](../concepts/glossary.md#binding) к данным.
...
- Консоль управления {#console}
  ...
  1. В блоке **{{ ui-key.yql.yq-binding-form.binding-parameters.title }}**:
     ...
     1. В поле **{{ ui-key.yql.yq-binding-form.binding-path-pattern.title }}** укажите [путь к таблице](../sources-and-sinks/object-storage.md#path_format) в бакете — `/visits.csv`.
