...
* плата за данные, считанные [{{ yq-full-name }} при исполнении запросов](../pricing.md).
...
* `--raw-results` — (опционально) параметр, чтобы возвращать необработанные результаты выполнения запроса в {{ yq-name }}. Спецификация формата доступна в разделе [{#T}](../api/yql-json-conversion-rules.md).
