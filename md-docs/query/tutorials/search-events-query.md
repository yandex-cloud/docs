...
В [{{ yq-full-name }}](../index.md) интегрирована поддержка {{ at-name }}. Вы можете анализировать события ресурсов {{ yandex-cloud }}, выполняя [аналитические](../concepts/batch-processing.md) и [потоковые](../concepts/stream-processing.md) запросы на языке {{ yql-short-name }}.
...
Чтобы подключить бакет с [аудитными логами](../../audit-trails/concepts/events.md) к {{ yq-full-name }} и выполнить запросы на языке [{{ yql-short-name }}](../yql-tutorials/index.md):
