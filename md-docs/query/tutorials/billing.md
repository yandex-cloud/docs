...
[{{ billing-name }}](../../billing/index.md) позволяет получать информацию об объеме использованных ресурсов, проверять расходы денежных средств и оплачивать потребление ресурсов. Детализацию расходов можно отправлять в [{{ objstorage-full-name }}](../../storage/index.md) и далее обрабатывать с помощью {{ yq-full-name }}.
...
1. В разделе **{{ ui-key.yql.yq-billing.billing.title }}** в интерфейсе {{ yq-name }} выберите в списке [привязок к данным](../concepts/glossary.md#binding) нужную.
