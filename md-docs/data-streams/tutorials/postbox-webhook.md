...
В этом руководстве вы настроите передачу событий [{{ postbox-full-name }}](../../postbox/index.md) в [{{ yds-full-name }}](../index.md) и вызов [вебхуков](https://ru.wikipedia.org/wiki/Webhook) во внешних системах. Для обработки и хранения данных будут использоваться сервисы [{{ sf-full-name }}](../../functions/index.md) и [{{ ydb-full-name }}](../../ydb/index.md).
...
* `yds-functions` — от его имени будет вызываться [функция](../../functions/concepts/function.md) {{ sf-name }} и будут записываться события в [поток данных](../concepts/glossary.md#stream-concepts) {{ yds-short-name }};
