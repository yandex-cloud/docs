...
С помощью сервиса {{ data-transfer-name }} вы можете перенести данные из [потока {{ yds-name }}](../concepts/glossary.md#stream-concepts) в бакет {{ objstorage-full-name }}:
...
1. [Создайте поток данных {{ yds-name }}](../operations/aws-cli/create.md).
...
1. [Отправьте в поток {{ yds-name }}](../operations/aws-cli/send.md) новое сообщение:
