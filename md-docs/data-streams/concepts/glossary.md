...
Возможна настройка потока данных таким образом, что количество сегментов будет увеличиваться при росте скорости записи в этот поток данных. Подробнее в разделе [автопартиционирование в документации YDB](https://ydb.tech/docs/ru//concepts/topic#partitioning).
...
Часть данных может потеряться при перегрузке отдельных [сегментов](#shard) или всего [потока](#stream-concepts). [Автопартиционирование](https://ydb.tech/docs/ru//concepts/topic#autopartitioning) автоматически добавляет сегменты и распределяет нагрузку. Это помогает избежать потерь данных.
