...
Поток данных (Stream) — это именованный набор сообщений. Запись и чтение сообщений в Data Streams ведутся через потоки. Потоки данных создаются на основе Yandex Managed Service for YDB и хранятся в базах данных.
...
Возможна настройка потока данных таким образом, что количество сегментов будет увеличиваться при росте скорости записи в этот поток данных. Подробнее в разделе [автопартиционирование в документации YDB](https://ydb.tech/docs/ru/ru/concepts/datamodel/topic#autopartitioning).
...
Потребители данных — это приложения, которые получают и обрабатывают данные из Data Streams. Все потребители данных разделяют [общую квоту](limits.md) на чтение.

## Выделенные потребители данных {#extended-consumers}

Для некоторых случаев модель общей [квоты](https://console.yandex.cloud/cloud?section=quotas) для всех потребителей данных является слишком ограничительной, поэтому каждому потребителю выдают гарантированную скорость чтения. Потребители данных с гарантированной скоростью чтения называются _выделенными потребителями данных_.

## Автопартиционирование {#autopartitioning}

Часть данных может потеряться при перегрузке отдельных [сегментов](#shard) или всего [потока](#stream-concepts). [Автопартиционирование](https://ydb.tech/docs/ru/ru/concepts/datamodel/topic#autopartitioning) автоматически добавляет сегменты и распределяет нагрузку. Это помогает избежать потерь данных.
