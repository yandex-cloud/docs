...
* `ExtractAfter` — возвращает часть значения, которая находится после указанной подстроки. Например, вместо `user@site.com` при указании в качестве подстроки `@` возвращает `site.com`.
* `ExtractBefore` — возвращает часть значения, которая находится до указанной подстроки. Например, вместо `user@example.com` при указании в качестве подстроки `@` возвращает `user`.
...
* `Constant` — заменяет текущее значение на указанную константу. Например, вместо любого значения атрибута возвращает `User`.
* `Replace` — заменяет указанную подстроку в значении атрибута на другую строку. Например, вместо `user@example.com` при указании подстроки `example.com` и строки замены `company.com` возвращает `user@company.com`.
