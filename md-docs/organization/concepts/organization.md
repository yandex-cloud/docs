...
* Сервис [{{ iam-full-name }}](../../iam/concepts/index.md) — позволяет запретить выполнение определенных операций с помощью [политик авторизации](../../iam/concepts/access-control/access-policies.md), а также назначить пользователям роли для выполнения определенных операций с конкретным ресурсом {{ yandex-cloud }}.
