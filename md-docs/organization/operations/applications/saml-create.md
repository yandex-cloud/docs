[Документация Yandex Cloud](../../../index.md) > [Yandex Identity Hub](../../index.md) > [Пошаговые инструкции](../index.md) > Управление приложениями (SSO) > SAML-приложения > Создать приложение
...
- Интерфейс Cloud Center {#cloud-center}
  ...
  1. Чтобы добавить дополнительные атрибуты пользователей, в правом верхнем углу страницы нажмите кнопку ![plus](../../../_assets/console-icons/plus.svg) **Добавить атрибут** и в открывшемся окне:
      ...
      1. (Опционально) Нажмите **Добавить трансформацию**, чтобы добавить [трансформацию атрибута](*transform). Выберите один из типов трансформации:

         * `ExtractAfter` — возвращает часть значения, которая находится после указанной подстроки. Подстрока указывается в поле **Подстрока**. Например, `user@site.com` + `@` → `site.com`.
         * `ExtractBefore` — возвращает часть значения, которая находится до указанной подстроки. Подстрока указывается в поле **Подстрока**. Например, `user@example.com` + `@` → `user`.
         * `ToLowercase` — приводит все символы в значении атрибута к нижнему регистру.
         * `ToUppercase` — приводит все символы в значении атрибута к верхнему регистру.
         * `Trim` — удаляет пробелы в начале и в конце значения атрибута.
         * `IfEmpty` — заменяет текущее значение атрибута, если оно пустое. Можно выбрать значение для замены из списка значений атрибутов или указать свое. Например, если значение `username` пустое, использовать `email`.
         * `Constant` — заменяет текущее значение на указанную константу. Константа указывается в поле **Значение**. Например, `<значение_атрибута>` → `User`.
         * `Replace` — заменяет указанную подстроку в значении атрибута на другую строку. Подстрока указывается в поле **Найти**, строка замены — в поле **Заменить на**. Например, `user@example.com` + `example.com` + `company.com` → `user@company.com`.
      ...
      1. Измените имя и/или значение атрибута.
          ...
          * `urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress` — идентификатор пользователя передается в формате адреса электронной почты. Доступные значения:
              * `SubjectClaims.preferred_username` — значение по умолчанию при переключении на этот формат.
                  ...
                  Уникальность и неизменяемость передаваемого идентификатора не гарантируются: в одной организации могут быть два пользователя с одинаковым идентификатором `preferred_username`. Например: [федеративный](../../../iam/concepts/users/accounts.md#saml-federation) пользователь и [локальный](../../../iam/concepts/users/accounts.md#local) пользователь могут иметь одинаковое значение этого атрибута.
