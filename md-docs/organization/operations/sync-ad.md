...
- Linux {#linux}
  ...
  1. В открывшемся файле задайте конфигурацию агента синхронизации. Конфигурация зависит от [способа аутентификации](../concepts/ad-sync/sync-agent.md#agent-ad-auth), используемого агентом на стороне Active Directory, и задается в [YAML](https://yaml.org/)-файле в следующем формате:
      ...
      - По логину и паролю {#password_linux}
        ...
        * `check_config_permissions` — настройки [проверки прав доступа](../concepts/ad-sync/sync-agent.md#auth-data-security) к файлам с аутентификационными данными, выполняемой при запуске агента. Необязательный параметр. Возможные значения:
        ...
        * `enable_password_writeback` — параметр, управляющий [обратной записью паролей](../concepts/ad-sync/index.md#password-writeback) пользователей в Active Directory.
        ...
        * `dry_run` — настройки [тестового запуска](../concepts/ad-sync/sync-agent.md#dry-run) агента (dry run):
        ...
        * `drsr` — настройки протокола [DRSR](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-drsr/) для аутентификации на стороне Active Directory [пользователя](../concepts/ad-sync/index.md#dc-setup) с назначенными правами на выполнение репликации данных в каталоге:
        ...
        * `logger` — настройки [логирования](../concepts/ad-sync/sync-agent.md#logging) процесса синхронизации:
...
- Windows {#windows}
  ...
  1. В открывшемся файле задайте конфигурацию агента синхронизации. Конфигурация зависит от [способа аутентификации](../concepts/ad-sync/sync-agent.md#agent-ad-auth), используемого агентом на стороне Active Directory, и задается в [YAML](https://yaml.org/)-файле в следующем формате:
      ...
      - От имени аккаунта gMSA {#gmsa-windows}
        ...
        * `drsr` — настройки протокола [DRSR](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-drsr/) для аутентификации на стороне Active Directory [аккаунта gMSA](*gmsa_account) с [назначенными правами](../concepts/ad-sync/index.md#dc-setup) на выполнение репликации данных в каталоге:
