[Документация Yandex Cloud](../../../../index.md) > [Yandex Identity Hub](../../../index.md) > [Практические руководства](../../index.md) > [Настройка единого входа в приложения (SSO)](../index.md) > Loop > OpenID Connect

# Создать OIDC-приложение в Yandex Identity Hub для интеграции с Loop

[Loop](https://loop.ru/) — корпоративный мессенджер с поддержкой единого входа по стандарту [OpenID Connect](https://ru.wikipedia.org/wiki/OpenID#OpenID_Connect) (OIDC).

Чтобы пользователи вашей [организации](../../../concepts/organization.md) могли аутентифицироваться в Loop с помощью технологии единого входа по стандарту OpenID Connect, создайте [OIDC-приложение](../../../concepts/applications/oidc.md) в Yandex Identity Hub и настройте его на стороне Yandex Identity Hub и на стороне Loop.
...
- Интерфейс Cloud Center {#cloud-center}
    ...
    1. В правом верхнем углу страницы нажмите ![Circles3Plus](../../../../_assets/console-icons/circles-3-plus.svg) **Создать приложение** и в открывшемся окне:
        ...
        1. В поле **Имя** задайте имя создаваемого приложения: `loop-oidc-app`.
...
Чтобы настроить интеграцию Loop с созданным OIDC-приложением в Yandex Identity Hub, выполните настройки на стороне Yandex Identity Hub и на стороне Loop.
...
- Интерфейс Cloud Center {#cloud-center}
    ...
    1. Войдите в сервис [Yandex Identity Hub](https://center.yandex.cloud/organization).
    1. На панели слева нажмите ![shapes-4](../../../../_assets/console-icons/shapes-4.svg) **Приложения** и выберите приложение `loop-oidc-app`.
...
### Настройте OIDC-аутентификацию на стороне Loop {#setup-sp}

1. Войдите в системную консоль Loop по адресу `https://<имя_инстанса>.loop.ru/admin_console/authentication/openid`, где `<имя_инстанса>` — имя вашего инстанса Loop.
...
- Интерфейс Cloud Center {#cloud-center}
    ...
    1. Справа сверху нажмите ![pencil](../../../../_assets/console-icons/pencil.svg) **Редактировать** и в открывшемся окне:
        1. В поле **Redirect URI** укажите адрес обратного вызова `https://<имя_инстанса>.loop.ru/signup/openid/complete`, где `<имя_инстанса>` — имя вашего инстанса Loop (поддомена на `loop.ru`).
...
Для аутентификации в Loop, добавьте в OIDC-приложение Yandex Identity Hub нужных пользователей и/или [группы пользователей](../../../concepts/groups.md).
...
Чтобы проверить корректность работы OIDC-приложения и интеграции с Loop, выполните вход в Loop под учетной записью одного из добавленных пользователей. Для этого:
