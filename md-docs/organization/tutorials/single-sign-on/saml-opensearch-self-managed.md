...
Чтобы пользователи вашей [организации](../../concepts/organization.md) могли аутентифицироваться в {{ OS }} с помощью технологии единого входа по стандарту [SAML](https://ru.wikipedia.org/wiki/SAML), создайте [SAML-приложение](../../concepts/applications.md#saml) в {{ org-full-name }}, настройте его на стороне {{ org-full-name }} и на стороне {{ OS }}.
...
1. Настройте SAML-аутентификацию для кластера {{ OS }}.
    ...
    1. Чтобы сопоставить группы пользователей {{ org-full-name }} с внутренними ролями {{ OS }} при SAML-аутентификации, отредактируйте файл `roles_mapping.yml` плагина **{{ OS }} Security Plugin**. Добавьте требуемые [группы](../../concepts/groups.md) из {{ org-full-name }} в секцию `backend_roles` соответствующих ролей в {{ OS }}, как показано ниже.
        ...
        1. Настройте сопоставления ролей:
            ...
            1. В поле **Backend roles** укажите [группу пользователей](../../concepts/groups.md) {{ org-full-name }}, которую требуется сопоставить с ролью {{ OS }}, например `opensearch-users`.
...
1. После настройки сопоставления ролей на стороне {{ OS }} [создайте](../../operations/create-group.md) требуемые [группы](../../concepts/groups.md):
