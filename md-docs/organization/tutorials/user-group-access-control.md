[Документация Yandex Cloud](../../index.md) > [Yandex Identity Hub](../index.md) > [Практические руководства](index.md) > Разграничение прав доступа для групп пользователей

# Разграничение прав доступа для групп пользователей с различными ролями в Yandex Identity Hub

В настоящем руководстве приведен пример решения по использованию [групп пользователей](../concepts/groups.md) для разграничения [прав доступа](../../iam/concepts/access-control/index.md) к ресурсам в [организации Yandex Identity Hub](../../overview/roles-and-resources.md).
...
* в продуктовой среде на скачивание Docker-образов из реестров Container Registry, просмотр информации о кластерах Kubernetes, подключение к виртуальным машинам Compute Cloud через [OS Login](../concepts/os-login.md), а также на просмотр информации о ресурсах и [метриках](../../monitoring/concepts/data-model.md#metric) сервиса Monitoring;
...
После создания организации вы станете ее [владельцем](../security/index.md#organization-manager-organizations-owner) и сможете управлять ее настройками.
...
Создайте три [группы пользователей](../concepts/groups.md): `security` для инженеров по информационной безопасности, `devops` для инженеров DevOps и `developers` для разработчиков:
...
1. Назначьте роли на организацию:
    ...
    - Интерфейс Cloud Center {#cloud-center}
      ...
      1. Перейдите на вкладку **Группы** и выберите [группу](../concepts/groups.md) `security`.
...
* подключаться к виртуальным машинам Compute Cloud через [OS Login](../concepts/os-login.md) в продуктовой среде ([роль](../../compute/security/index.md#compute-oslogin) `compute.osLogin`, [роль](../../resource-manager/security/index.md#resource-manager-auditor) `resource-manager.auditor` или выше на облако `production`);
...
- Интерфейс Cloud Center {#cloud-center}
    ...
    1. Для добавления пользователей в группу:
       ...
       1. Выберите [группу](../concepts/groups.md) из выпадающего списка.

    1. Повторите действия для всех ранее созданных групп.

{% endlist %}

Сразу после того, как пользователи примут приглашения по ссылке из письма, они станут [участниками организации](../concepts/membership.md) и будут отображаться в разделе ![icon-users](../../_assets/console-icons/person.svg) **Пользователи** сервиса [Yandex Identity Hub](https://center.yandex.cloud/organization/users).
...
Если созданная тестовая организация вам больше не нужна, [удалите](../operations/delete-org.md) ее.
...
* Yandex Identity Hub:
    * [Инструкции по работе с Yandex Identity Hub](../operations/index.md)
