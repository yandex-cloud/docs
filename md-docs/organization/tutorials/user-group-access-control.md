...
В настоящем руководстве приведен пример решения по использованию [групп пользователей](../concepts/groups.md) для разграничения [прав доступа](../../iam/concepts/access-control/index.md) к ресурсам в [организации {{ org-full-name }}](../../overview/roles-and-resources.md).
...
1. Назначьте роли на организацию:
    ...
    - Интерфейс {{ cloud-center }} {#cloud-center}
      ...
      1. Перейдите на вкладку **{{ ui-key.yacloud_org.pages.groups }}** и выберите [группу](../concepts/groups.md) `security`.
...
* подключаться к виртуальным машинам {{ compute-name }} через [{{ oslogin }}](../concepts/os-login.md) в продуктовой среде ([роль](../../compute/security/index.md#compute-oslogin) `compute.osLogin`, [роль](../../resource-manager/security/index.md#resource-manager-auditor) `resource-manager.auditor` или выше на облако `production`);
...
1. Пригласите сотрудников в организацию:
    ...
    Сразу после того, как пользователи примут приглашения по ссылке из письма, они станут [участниками организации](../concepts/membership.md) и будут отображаться в списке в [разделе **{{ ui-key.yacloud_org.pages.users }}**]({{ link-org-cloud-center }}/users) вашей организации.
