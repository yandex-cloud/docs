...
Старые версии данных не удаляются автоматически. Если они больше не нужны, то можно удалить их, чтобы освободить занятое ими хранилище и устранить раздувание (bloat). Для этого обычно используются команды [VACUUM FULL](https://www.postgresql.org/docs/current/sql-vacuum.html) или [CLUSTER](https://www.postgresql.org/docs/current/sql-cluster.html). Однако эти команды требуют эксклюзивной блокировки таблиц на все время выполнения, что не всегда удобно или допустимо.

Расширение [pg_repack](https://github.com/reorg/pg_repack) позволяет перепаковывать (repack) таблицы и индексы, чтобы устранить их раздувание. В отличие от других методов, `pg_repack` устанавливает эксклюзивную блокировку таблицы на короткий промежуток времени. Подробнее на [странице расширения](https://reorg.github.io/pg_repack/).

## Установить расширение pg_repack в кластер PostgreSQL {#install-extension}
...
1. [Задайте нулевые значения](../update.md#change-postgresql-config) для следующих настроек кластера PostgreSQL:
...
Для работы с расширением используется одноименный клиент. Клиент устанавливается на хост, с которого есть возможность [подключения к кластеру PostgreSQL](../connect/index.md).

Чтобы установить клиент:

1. [Определите версию расширения](cluster-extensions.md#postgresql) `pg_repack`, которая установлена в кластер PostgreSQL.
1. [Установите клиент](https://reorg.github.io/pg_repack/#installation) `pg_repack`.
    ...
    1. Подключите [Apt-репозиторий PostgreSQL](https://wiki.postgresql.org/wiki/Apt), который содержит часть зависимостей, необходимых для сборки:
...
1. Запустите клиент `pg_repack` с нужными параметрами.
    ...
    Ниже приведены команды для запуска клиента с распространенными комбинациями параметров. Описание всех поддерживаемых параметров на [странице расширения](https://reorg.github.io/pg_repack/).
...
* Кластер PostgreSQL версии `16`, в котором:
    ...
    * В базу данных `db1` [установлено расширение](#install-extension) `pg_repack` версии `1.5.2`.

* Виртуальная машина Yandex Cloud с Ubuntu 22.04 LTS, в которой:

    * [Установлен собранный из исходных файлов клиент](#install-client) `pg_repack` версии `1.5.2`.
    * [Есть возможность подключения к кластеру](../connect/index.md) по [SSL](../connect/index.md#get-ssl-cert).

{% endnote %}

В этом примере используется инструмент [pgbench](https://www.postgresql.org/docs/current/pgbench.html), который предназначен для нагрузочного тестирования PostgreSQL и позволяет в автоматическом режиме выполнять следующие операции:
...
1. [Получите идентификатор кластера PostgreSQL](../cluster-list.md#list-clusters).
...
1. Посмотрите статистику для таблиц и индексов `pgbench_*`:
    ...
    1. [Подключитесь к базе данных](../connect/clients.md#bash) `db1` от имени владельца `user1`. Для подключения используйте утилиту `psql`.
