[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for PostgreSQL](../index.md) > Вопросы и ответы > Все вопросы на одной странице
...
* [Почему при попытке подключиться к базе данных из Data Studio возникает ошибка?](#google-looker)
...
Подробнее читайте в [документации сервиса](../operations/connect/index.md).

#### Можно ли подключиться к кластеру из Docker-контейнера? {#conn-from-docker}

Да, для этого [настройте Dockerfile](../operations/connect/clients.md#connection-docker).

Пример подключения из контейнера в сервисе Yandex Serverless Containers описан в [практическом руководстве](../../serverless-containers/tutorials/pg-connect.md).
...
Подробнее о получении сертификата и подключении к базе данных читайте в [документации сервиса](../operations/connect/index.md).
...
Для подключения из Data Studio необходимо сгенерировать и указать в настройках подключения файл сертификата клиента и приватный ключ. Инструкцию читайте в разделе [Подключение из Data Studio](../operations/connect/clients.md#connection-google-looker).
...
Подробнее о миграции с помощью Data Transfer читайте в руководстве [Миграция базы данных](../tutorials/outbound-replication.md).
