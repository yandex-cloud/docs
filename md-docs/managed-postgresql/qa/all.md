...
Нет, права суперпользователя недоступны пользователям {{ mpg-name }}. Все виды действий с кластером доступны пользователям с ролью [mdb_admin](../concepts/roles.md#mdb-admin).

#### Могу ли я скопировать данные из таблицы {{ mpg-name }} в локальный файл? А заполнить таблицу данными из локального файла? {#copy-write-data}

Да, вы можете как скопировать данные из таблицы в локальный файл, так и заполнить таблицу данными из локального файла. Подробнее см. в разделе [{#T}](../operations/copy-write-data.md).
...
Воспользуйтесь инструкциями в разделах [{#T}](../concepts/storage.md#read-only-solutions) и [{#T}](../operations/storage-space.md#read-only-solutions).
...
После создания БД изменить ее владельца невозможно. Если [создать БД через {{ TF }}](../operations/databases.md#add-db) и затем поменять ее владельца в параметре `owner`, это приведет к пересозданию БД и потере данных.
...
* в сервисе [{{ monitoring-full-name }}]({{ link-monitoring }}) с возможностью [настроить алерты](../operations/monitoring.md#monitoring-integration) по заданным метрикам.
...
Воспользуйтесь сервисом [{{ monitoring-full-name }}]({{ link-monitoring }}) и [настройте алерты](../operations/monitoring.md#monitoring-integration) по критичным для вас параметрам.


#### Как настроить алерт, который срабатывает при заполнении определенного процента дискового пространства? {#disk-space-percentage}

[Создайте алерт](../operations/monitoring.md#monitoring-integration) с метрикой `disk.used_bytes` в сервисе {{ monitoring-full-name }}. Метрика показывает размер использованного дискового пространства в кластере {{ mpg-name }}.
