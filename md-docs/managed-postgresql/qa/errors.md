# Сообщения об ошибках в {{ mpg-name }}
...
Ошибка возникает из-за того, что [остановить](../operations/cluster-stop.md#stop-cluster) кластер {{ mpg-name }} можно только при наличии хотя бы одной резервной копии кластера.
...
Ошибка может возникнуть при миграции БД в {{ mpg-short-name }} с помощью скрипта, который пытается установить и использовать [расширение {{ PG }}](../operations/extensions/cluster-extensions.md). Причина ошибки в том, что в кластерах {{ mpg-short-name }} нельзя управлять расширениями {{ PG }} с помощью команд SQL.
...
Ошибка вызвана тем, что в логическом дампе присутствуют операции установки или обновления [расширений {{ PG }}](../operations/extensions/cluster-extensions.md). В кластерах {{ mpg-short-name }} нельзя управлять расширениями с помощью команд SQL.
