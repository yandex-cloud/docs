...
1. [Создайте кластер {{ mpg-name }}](../operations/cluster-create.md) любой подходящей вам [конфигурации](../concepts/instance-types.md) со следующими настройками:
...
1. Если вы используете группы безопасности для кластеров и виртуальной машины, настройте их так, чтобы разрешить подключение:
    ...
    * [к кластеру {{ mpg-name }}](../operations/connect/index.md#configuring-security-groups).
...
1. [Подключитесь к базе данных](../operations/connect/index.md) `db1` кластера {{ mpg-name }} от имени пользователя `user1`.
