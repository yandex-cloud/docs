...
Миграция данных в сторонний кластер {{ PG }} описана в разделе [{#T}](outbound-replication.md).
...
Пользователи автоматически не переносятся в кластер {{ mpg-name }}. Их нужно [создать](../operations/cluster-users.md#adduser) в новом кластере заново.
...
1. Подготовьте инфраструктуру:
    ...
    - Вручную {#manual}
        ...
        1. [Создайте кластер-приемник {{ mpg-name }}](../operations/cluster-create.md) любой подходящей конфигурации. Включите те же [расширения {{ PG }}](../operations/extensions/cluster-extensions.md), что и в кластере-источнике.
    ...
    - {{ TF }} {#tf}
        ...
        1. Укажите в файле `data-transfer-pgsql-mpg.tf`:
            ...
            * `pg-extensions` – список [расширений {{ PG }}](../operations/extensions/cluster-extensions.md) в кластере-источнике;
...
- Вручную {#manual}
    ...
    [Создайте кластер-приемник {{ mpg-name }}](../operations/cluster-create.md) любой подходящей конфигурации. При этом:
...
- Вручную {#manual}
    ...
    [Удалите кластер {{ mpg-name }}](../operations/cluster-delete.md).
...
- Вручную {#manual}
    ...
    1. [Создайте кластер-приемник {{ mpg-name }}](../operations/cluster-create.md) любой подходящей конфигурации. При этом следующие параметры должны быть такими же, как и в кластере-источнике:
        ...
        * Имя пользователя.
            ...
            Вы можете использовать различные имена пользователей для источника и приемника, но это может привести к ошибке при восстановлении дампа. Подробнее см. в разделе [Перемещение и восстановление кластера {{ PG }}](../qa/backup.md#backup-error)
            
            {% endnote %}

        * [Расширения {{ PG }}](../operations/extensions/cluster-extensions.md).
    ...
    1. Если вы используете группы безопасности для промежуточной виртуальной машины и кластера {{ mpg-name }}, [настройте их](../operations/connect/index.md#configure-security-groups).
...
- Вручную {#manual}
    ...
    * [Удалите кластер {{ mpg-full-name }}](../operations/cluster-delete.md).
