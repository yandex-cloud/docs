# Загрузка данных из {{ ydb-full-name }} в {{ mpg-full-name }}
...
- Вручную {#manual}
    ...
    1. [Создайте кластер {{ mpg-name }}](../operations/cluster-create.md) любой подходящей конфигурации с хостами в публичном доступе и следующими настройками:

        * **{{ ui-key.yacloud.mdb.forms.database_field_name }}** — `db1`.
        * **{{ ui-key.yacloud.mdb.forms.database_field_user-login }}** — `user1`.

    1. Если вы используете группы безопасности в кластере, убедитесь, что они настроены правильно и допускают подключение к кластеру [{{ mpg-name }}](../operations/connect/index.md#configuring-security-groups).
...
1. [Подключитесь к базе данных](../operations/connect/clients.md) `db1` в кластере-приемнике {{ mpg-name }}.
