[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for PostgreSQL](../index.md) > [Практические руководства](index.md) > Запись логов балансировщика в PostgreSQL
...
БД будет создана с помощью сервиса [Yandex Managed Service for PostgreSQL](../index.md).
...
Логи, полученные от балансировщика, будут записываться в базу данных, которая входит в состав [кластера](../concepts/index.md) Managed Service for PostgreSQL.
...
- Консоль управления {#console}
  ...
  1. В блоке **База данных** укажите атрибуты БД:
     ...
     * **Пароль** пользователя. Длина — от 8 до 128 символов.
     ...
     Для базы данных, которая создается вместе с кластером, устанавливаются настройки набора символов (кодировки) `LC_CTYPE=C` и `LC_COLLATE=C`. После создания эти настройки изменить нельзя, но вы можете [создать новую базу](../operations/databases.md#add-db) с нужными настройками.
...
Подробнее о создании кластера — в разделе [Как создать кластер PostgreSQL](../operations/cluster-create.md#create-cluster).

В дальнейшем настройки кластера можно будет [изменить](../operations/update.md).
...
1. [Удалите](../operations/cluster-delete.md) кластер `alb-logging-cluster`.
