...
В данном руководстве вы организуете запись данных с устройства в базу данных. В качестве примера будет использоваться [кластер {{ mpg-name }}](../concepts/index.md). Чтобы подключить устройство к [{{ iot-full-name }}](../../iot-core/index.md), вам понадобится MQTT-брокер. Все шаги выполняются в [консоли управления]({{ link-console-main }}).
...
1. [Удалите](../operations/cluster-delete.md) кластер {{ PG }}.
