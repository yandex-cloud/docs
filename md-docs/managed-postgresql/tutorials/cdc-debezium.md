[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for PostgreSQL](../index.md) > [Практические руководства](index.md) > Поставка данных в Yandex Managed Service for Apache Kafka® с помощью Debezium
...
Вы можете отслеживать изменения данных в Yandex Managed Service for PostgreSQL и отправлять их в Yandex Managed Service for Apache Kafka® с помощью технологии Change Data Capture (CDC).
...
1. [Создайте _кластер-источник_](../operations/cluster-create.md) со следующими настройками:
...
1. Если вы используете группы безопасности, настройте их так, чтобы к кластерам можно было подключаться из интернета и созданной виртуальной машины, а к ней — из интернета по [SSH](../../glossary/ssh-keygen.md):
    ...
    * [Настройка групп безопасности кластера Managed Service for PostgreSQL](../operations/connect/index.md#configuring-security-groups).
...
1. [Подключитесь к виртуальной машине по SSH](../../compute/operations/vm-connect/ssh.md#vm-connect) и выполните ее предварительную настройку:
    ...
    1. Установите на виртуальную машину сертификаты и убедитесь в доступности кластеров:
        ...
        * [Managed Service for PostgreSQL](../operations/connect/index.md#get-ssl-cert) (используйте утилиту `psql`).
...
1. [Назначьте пользователю](../operations/grant.md) `user1` роль `mdb_replication`.
...
1. [Подключитесь к базе данных](../operations/connect/index.md) `db1` от имени пользователя `user1`.
...
1. Создайте файл `/etc/debezium/mdb-connector.conf` с настройками коннектора Debezium для подключения к кластеру-источнику:
    ...
    * `database.hostname` — [особый FQDN](../operations/connect/fqdn.md#fqdn-master) для подключения к хосту-мастеру кластера-источника.

        Идентификатор кластера можно получить со [списком кластеров в каталоге](../operations/cluster-list.md#list).
...
`kafkacat` — это устаревший инструмент. Он работает только для кластеров с версией Apache Kafka® ниже 4.0. Если ваш кластер с версией Apache Kafka® 4.0 или выше, используйте утилиту [kafkactl](../../managed-kafka/operations/connect/clients.md#kafkactl).
...
1. [Подключитесь к кластеру-источнику](../operations/connect/index.md).
