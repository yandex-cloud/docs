[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for PostgreSQL](../index.md) > [Практические руководства](index.md) > Захват изменений PostgreSQL и поставка в YDS
...
- Вручную {#manual}
    ...
    1. [Создайте кластер-источник Managed Service for PostgreSQL](../operations/cluster-create.md) любой подходящей [конфигурации](../concepts/instance-types.md) с хостами в публичном доступе и следующими настройками:
        * **Имя БД** — `db1`.
    ...
    1. Настройте [группы безопасности](../operations/connect/index.md#configuring-security-groups) и убедитесь, что они допускают подключение к кластеру.


    1. [Выдайте роль](../operations/grant.md#grant-privilege) `mdb_replication` пользователю `pg-user`.
...
1. [Подключитесь к кластеру Managed Service for PostgreSQL](../operations/connect/index.md), создайте в базе данных `db1` таблицу `measurements` и заполните ее данными:
