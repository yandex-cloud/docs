...
Вы можете получить пароль пользователя [{{ mpg-full-name }}](../index.md) из [секрета {{ lockbox-name }}](../../lockbox/concepts/secret.md). Это можно сделать через [{{ yandex-cloud }} CLI](../../cli/index.md). Для этого [сервисному аккаунту](../../iam/concepts/users/service-accounts.md), под которым вы авторизованы в {{ yandex-cloud }} CLI, нужно настроить права доступа к секрету пользователя. Информацию о секрете, необходимую для настройки прав, можно получить из подключения [{{ connection-manager-name }}](../../metadata-hub/concepts/connection-manager.md).
