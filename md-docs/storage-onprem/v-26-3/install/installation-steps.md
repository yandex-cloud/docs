...
В примерах установки используется IPv4-адресация. On-premises Yandex Object Storage также поддерживает IPv6.

Для использования IPv6 укажите IPv6-адреса и подсети в [конфигурации стенда](setup-install-params.md). В файле `group_vars/<имя_стенда>/main.yaml` задайте `ip_type: IPv6`, а в блоке `bootstrap.values_object` — `ipType: ipv6`. Для параметра `k8s.block_size` рекомендуется значение `122`.
...
* Размещает `/opt/kubernetes/Makefile`, который используется для создания кластера Kubernetes.
...
## Шаги 7–10. Установка Kubernetes {#step-7}
...
## Шаг 13. Установка On-premises Yandex Object Storage {#step-13}

Плейбук `init-storage.yaml` применяет манифесты `shared-secrets.yaml` и `instance.yaml`. Создается ресурс `Instance`, после чего `yc-storage-operator` начинает установку компонентов Object Storage внутри кластера.
...
|| `step7` | Инициализация кластера Kubernetes. ||
...
|| `step13` или `bootstrap` | Запуск установки Object Storage внутри кластера. ||
...
Если секреты созданы, хосты и диски подготовлены, а архив распакован, начните с установки Kubernetes:
...
Если кластер Kubernetes уже готов, установите ArgoCD, оператор и компоненты Object Storage:
...
При выборе отдельных шагов сохраняйте зависимости: секреты нужны для последующих этапов; настройка хостов и распаковка архива — для создания кластера Kubernetes; готовый кластер и `topolvm` — для установки ArgoCD. Перед запуском `bootstrap` должны быть установлены ArgoCD и `yc-storage-operator`, а приложение оператора должно иметь статус `Healthy`.
