[Документация Yandex Cloud](../../index.md) > [On-premises Yandex Object Storage](../index.md) > Версия 26.3 > Справочник CLI (англ.) > yarl > delete > handler

# cli yarl delete handler

Delete a per-handler operation quota

```
cli yarl delete handler <entity> [flags]
```

## Options

```
  -h, --help                  help for handler
  -o, --operation Operation   handler name
  -s, --scope Scope           scope: bucket, tenant (default bucket)
```

## Options inherited from parent commands

```
  -c, --config-dir string   path to configuration directory
      --debug               enable debug mode
      --insecure            use if console has self-signed certificate
  -p, --profile string      configuration profile
```

## See also

* [cli yarl delete](cli_yarl_delete.md)	 — Delete a yarl quota