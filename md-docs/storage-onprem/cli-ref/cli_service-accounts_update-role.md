[Документация Yandex Cloud](../../index.md) > [On-premises Yandex Object Storage](../index.md) > Версия 26.3 > Справочник CLI (англ.) > service-accounts > update-role

# cli service-accounts update-role

Update service account role

```
cli service-accounts update-role <service_account_id> [flags]
```

## Options

```
  -h, --help            help for update-role
  -r, --role string     Service account role (viewer, editor, admin)
  -t, --tenant string   Tenant ID
```

## Options inherited from parent commands

```
  -c, --config-dir string   path to configuration directory
      --debug               enable debug mode
      --insecure            use if console has self-signed certificate
  -p, --profile string      configuration profile
```

## See also

* [cli service-accounts](cli_service-accounts.md)	 — Manage service accounts