[Документация Yandex Cloud](../../index.md) > [On-premises Yandex Object Storage](../index.md) > Версия 26.3 > Справочник CLI (англ.) > tenants > reactivate

# cli tenants reactivate

Reactivate suspended tenant

```
cli tenants reactivate <tenant_id> [flags]
```

## Options

```
  -h, --help   help for reactivate
```

## Options inherited from parent commands

```
  -c, --config-dir string   path to configuration directory
      --debug               enable debug mode
      --insecure            use if console has self-signed certificate
  -p, --profile string      configuration profile
```

## See also

* [cli tenants](cli_tenants.md)	 — Tenants management