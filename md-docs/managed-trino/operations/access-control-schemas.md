[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for Trino](../index.md) > [Пошаговые инструкции](index.md) > Правила доступа к объектам > Назначение правил для схем
...
- Консоль управления {#console}
  ...
  1. В поле **Схемы** нажмите кнопку **Добавить правило**.
  1. В открывшемся окне задайте параметры правила:
     ...
     1. В поле **Владелец схемы** выберите, является ли пользователь владельцем схемы:
        * `YES` — является.
        * `NO` — не является.
  ...
  1. Создайте файл `access_control.yaml` и добавьте в него следующее содержимое:
     ...
     * `catalog` и `schema` — совместно определяют схемы, к которым будет применяться правило. Каждый из параметров является опциональным.
       ...
       * `schema` — схемы, задаваемые одним из параметров:
       ...
       В Managed Service for Trino полное имя схемы формируется по шаблону `<имя_каталога>.<имя_схемы>`. Правило применяется к схеме, только если ее полное имя соответствует всем заданным параметрам.
...
- Консоль управления {#console}
  ...
  1. Перейдите в блок **Настройки доступа** → **Схемы**.
