...
Общая информация о правилах доступа приведена в разделе [{#T}](../concepts/access-control.md).
...
- Консоль управления {#console}
  ...
  1. В поле **{{ ui-key.yacloud.trino.label_rbac-table }}** нажмите кнопку **{{ ui-key.yacloud.trino.label_rbac-add-rule }}**.
  1. В открывшемся окне задайте параметры правила:
     ...
     1. (Опционально) В поле **{{ ui-key.yacloud.trino.ClusterForm.label_table-privileges_2ikK4 }}** выберите разрешенные действия над таблицами:
     ...
     1. (Опционально) В поле **{{ ui-key.yacloud.trino.ClusterForm.label_filter_ewYvF }}** укажите булево SQL-выражение, определяющее доступ пользователей к строкам таблицы.
     ...
     1. (Опционально) Задайте полные имена таблиц, к которым будет применяться правило, в формате `<имя_каталога>.<имя_схемы>.<имя_таблицы>`. Каждая часть имени задается с помощью отдельного поля.
        ...
        1. Выберите, в каком формате указать таблицы:
           * **{{ ui-key.yacloud.trino.rbac-catalog-match-by-name }}** — выберите имена таблиц.
           * **{{ ui-key.yacloud.trino.rbac-catalog-match-by-name-regexp }}** — введите регулярное выражение для имен таблиц.
           * **{{ ui-key.yacloud.trino.rbac-catalog-match-by-empty }}** — в поле `<имя_таблицы>` может быть любое значение.

     1. (Опционально) В поле **{{ ui-key.yacloud.trino.ClusterForm.label_table-columns_68mj2 }}** добавьте список правил, ограничивающих доступ пользователей к столбцам таблицы:
        1. Нажмите кнопку **{{ ui-key.yacloud.trino.ClusterForm.label_table-column-add_241Co }}**.
  ...
  1. Создайте файл `access_control.yaml` и добавьте в него следующее содержимое:
     ...
     * `catalog`, `schema` и `table` — совместно определяют таблицы, к которым будет применяться правило. Каждый из параметров является опциональным.
       ...
       * `table` — таблицы, задаваемые одним из параметров:
       ...
       В {{ mtr-name }} полное имя таблицы формируется по шаблону `<имя_каталога>.<имя_схемы>.<имя_таблицы>`. Правило применяется к таблице, только если ее полное имя соответствует всем заданным параметрам.
...
- Консоль управления {#console}
  ...
  1. Перейдите в блок **{{ ui-key.yacloud.trino.ClusterView.RBACView.label_rbac-settings_o2F64 }}** → **{{ ui-key.yacloud.trino.label_rbac-table }}**.
