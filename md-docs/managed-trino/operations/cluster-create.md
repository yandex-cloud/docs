...
- Консоль управления {#console}
    ...
    1. В блоке **Базовые параметры**:
        ...
        1. Выберите [версию Trino](../concepts/versions.md#available-versions).
    ...
    1. (Опционально) В блоке **Политика перезапросов** задайте параметры [отказоустойчивого выполнения запросов](../concepts/retry-policy.md):
        ...
        1. Выберите **Тип хранилища** для Exchange Manager:
    ...
    1. Задайте конфигурацию [координатора](../concepts/index.md#coordinator) и [воркеров](../concepts/index.md#workers).
        ...
        В классах хостов `standard` и `memory-optimized` часть ресурсов виртуальной машины резервируется для служебных компонентов Managed Service for Trino. Чтобы определить ресурсы, доступные координатору или воркеру Trino, используйте таблицы в разделе [Классы хостов](../concepts/instance-types.md). Для классов `legacy` число vCPU и объем RAM в имени класса соответствуют ресурсам, доступным компоненту Trino.
    ...
    1. В блоке **Дополнительные настройки**:
        ...
        1. (Опционально) Задайте параметры [TLS](../../glossary/tls.md).
           ...
           Чтобы настроить TLS для коннекторов ClickHouse®, PostgreSQL или MySQL®, [создайте подключение](../../metadata-hub/operations/create-connection.md#on-premise-connection) к пользовательской инсталляции в Connection Manager. В настройках подключения включите использование TLS.
...
- gRPC API {#grpc-api}
    ...
    1. Выполните запрос [ClusterService/Create](../api-ref/grpc/Cluster/create.md), например с помощью [gRPCurl](https://github.com/fullstorydev/grpcurl):
