...
Каждый кластер {{ mtr-name }} запускается в отдельной группе узлов {{ k8s }}, которая включает в себя необходимую сетевую инфраструктуру: [виртуальную сеть](../../vpc/concepts/network.md#network), [группу безопасности](../../vpc/concepts/security-groups.md) и [сервисный аккаунт](../../iam/concepts/users/service-accounts.md). Группы узлов изолированы друг от друга как средствами виртуальных сетей, так и средствами самого {{ k8s }}. Группы узлов управляются общим {{ k8s }}-мастером.
...
* {{ CH }}
* Delta Lake
* [{{ GP }}/Cloudberry](greenplum-connector.md) {{ preview-stage }}
* Hive
* Hudi {{ preview-stage }}
* Iceberg
