...
Чтобы обеспечить отказоустойчивое выполнение запросов при включенном Fault tolerant execution, Exchange Manager периодически выгружает промежуточные результаты в служебный бакет Object Storage. Если запрос не был выполнен, то [координатор](index.md#coordinator) назначит повторное выполнение запроса другому воркеру, который сможет использовать ранее обработанные данные.
