...
Кластер {{ mtr-name }} выделяет специальные IP-адреса в своей подсети. С них кластер подключается к другим ресурсам {{ yandex-cloud }}. Вы можете добавить [каталог](index.md#catalog) и настроить его на подключение к ресурсам в вашем облаке, которые поддерживаются [коннекторами](index.md#connector) {{ mtr-name }}.
...
Чтобы использовать сервисное подключение, при [создании](../operations/cluster-create.md) кластера {{ mtr-name }} включите приватный доступ к кластеру.