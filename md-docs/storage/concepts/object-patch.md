[Документация Yandex Cloud](../../index.md) > [Yandex Object Storage](../index.md) > [Концепции](index.md) > Частичное изменение объекта

# Частичное изменение объекта

В Object Storage реализован механизм _частичного изменения и дозаписи объектов_ в бакете.

{% note info %}

Механизм частичного изменения объекта не входит в стандартную функциональность [S3 API](../s3/index.md) и доступен для бакетов с выключенным [версионированием](versioning.md).
...
Таким образом, частичная перезапись позволяет увеличить скорость работы с Object Storage и уменьшить стоимость.
...
Также вы можете напрямую [отправлять запросы](../s3/index.md) patch к Object Storage API, либо расширить набор методов [AWS SDK](../tools/index.md), воспользовавшись [спецификацией](#specification) метода patch.

## Одновременное изменение объектов {#concurrent-patch}

В Object Storage поддерживается одновременное частичное изменение объектов.
