[Документация Yandex Cloud](../../index.md) > [Yandex Object Storage](../index.md) > [Управление доступом](overview.md) > Политика доступа (Bucket Policy)
...
[Условная запись объектов](../concepts/object.md#conditional-writes) позволяет избежать случайной перезаписи и конфликтов при одновременных загрузках. По умолчанию использование условий записи является необязательным — клиент может передавать заголовки `If-Match` и `If-None-Match` по своему усмотрению.

С помощью [политики доступа](../concepts/policy.md) можно сделать проверку условий обязательной. В этом случае все запросы на запись объектов без соответствующих заголовков будут отклоняться.
