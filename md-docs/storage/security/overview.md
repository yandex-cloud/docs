...
Также в Object Storage на доступ к бакетам и объектам может влиять включенное шифрование. Подробнее о работе с зашифрованными бакетами см. на странице [Шифрование в Object Storage](../concepts/encryption.md).
...
[Identity and Access Management](index.md) — основной способ управления доступом в Yandex Cloud с помощью назначения ролей и [политик авторизации](../../iam/concepts/access-control/access-policies.md). Позволяет базово разграничить доступы, подробнее см. [Какие роли действуют в сервисе](index.md#roles-list).
...
[Список управления доступом (ACL)](acl.md) — перечень разрешений на выполнение действий, хранящийся непосредственно в Object Storage. Позволяет базово разграничить доступы. Разрешения ACL для бакетов и объектов отличаются, подробнее см. [Виды разрешений](acl.md#permissions-types).
...
* **Ограничение доступа в бакет на уровне сервиса** — доступ в бакет разрешается только из сервисных подключений VPC. Нужно указать конкретные названия подключений. При этом доступ в бакет из публичной сети будет невозможен. Доступ в бакет с помощью [консоли управления](https://console.yandex.cloud) можно включить отдельной опцией. Подробнее см. на странице [Настройте ограничение для доступа только из сервисных подключений](../operations/buckets/access-via-vpc.md#setup-service-restriction).
