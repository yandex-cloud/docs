...
[{{ iam-name }}](index.md) — основной способ управления доступом в {{ yandex-cloud }} с помощью назначения ролей и [политик авторизации](../../iam/concepts/access-control/access-policies.md). Позволяет базово разграничить доступы, подробнее см. [{#T}](index.md#roles-list).
...
* **Ограничение доступа в бакет на уровне сервиса** — доступ в бакет разрешается только из сервисных подключений {{ vpc-short-name }}. Нужно указать конкретные названия подключений. При этом доступ в бакет из публичной сети будет невозможен. Доступ в бакет с помощью [консоли управления]({{ link-console-main }}) можно включить отдельной опцией. Подробнее см. на странице [Настройте ограничение для доступа только из сервисных подключений](../operations/buckets/access-via-vpc.md#setup-service-restriction).

### Копирование объектов на стороне сервера (Server Side Copy) {#server-side-copy}

Особенности [копирования объектов на стороне сервера (Server Side Copy)](../operations/objects/copy.md) в бакетах с включенным доступом только из сервисных подключений {{ vpc-short-name }} приведены в таблице:
