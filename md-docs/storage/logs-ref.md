[Документация Yandex Cloud](../index.md) > [Yandex Object Storage](index.md) > Логи бакета

# Справочник логов Object Storage


В этом разделе описаны поля логов, поставляемых бакетом.

Логи Object Storage сохраняются в формате JSON. Одна запись в логах соответствует одному запросу к бакету.

Подробнее в разделе [Механизм логирования действий с бакетом](concepts/server-logs.md).
...
`vhost` | String | Виртуальный хост запроса.<br>Возможные значения:<br>– `storage.yandexcloud.net`.<br>– `<имя_бакета>.storage.yandexcloud.net`.<br>– `website.yandexcloud.net`.<br> – `<имя_бакета>.website.yandexcloud.net`.
...
| `POST` | `OBJECT_RESTORE` | Восстановление объекта в бакете из архива. Не поддерживается Object Storage. |
