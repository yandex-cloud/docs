...
* Поддержана [аутентификация в {{ objstorage-name }}](api-ref/authentication.md) с помощью [IAM-токена](../iam/concepts/authorization/iam-token.md) и [эфемерных ключей доступа](operations/buckets/manage-ephemeral-keys.md).
...
* Появилась возможность при [настройке доступа в бакет только из сервисных подключений {{ vpc-short-name }} на уровне сервиса](operations/buckets/access-via-vpc.md#enable-service-restriction) оставить бакет доступным через [консоль управления]({{ link-console-main }}).
* Реализована функциональность настройки сервисных подключений {{ vpc-short-name }} в [консоли управления]({{ link-console-main }}).
...
* Улучшена работа с [жизненными циклами](concepts/lifecycles.md) объектов в бакете:
  ...
  * работа с новыми фильтрами реализована в консоли управления, CLI, {{ TF }} и с помощью [инструментов](tools/index.md) с поддержкой S3 API.
