[Документация Yandex Cloud](../../../index.md) > [Yandex Object Storage](../../index.md) > [Пошаговые инструкции](../index.md) > Бакеты > Настроить доступ в бакет с помощью сервисного подключения из VPC

# Настроить доступ в бакет с помощью сервисного подключения из Yandex Virtual Private Cloud
...
Также при такой настройке остается возможность [копировать объекты на стороне сервера (Server Side Copy)](../objects/copy.md) в такой бакет и из него.
...
- Консоль управления {#console}
  ...
  1. Перейдите на вкладку **Безопасность**, затем на вкладку **Политика доступа**.
...
После включения данной опции доступ в бакет из публичной сети будет невозможен. Доступ в бакет с помощью [консоли управления](https://console.yandex.cloud) можно включить отдельной опцией. Подробнее об особенностях копирования в разделе [Копирование объектов на стороне сервера (Server Side Copy)](#server-side-copy).
...
- API {#api}
  ...
  Чтобы восстановить доступ к бакету из публичной сети, воспользуйтесь методом REST API [update](../../api-ref/Bucket/update.md) для ресурса [Bucket](../../api-ref/grpc/Bucket/index.md) или вызовом gRPC API [Bucket/Update](../../api-ref/grpc/Bucket/update.md).
