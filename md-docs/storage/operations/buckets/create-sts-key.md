...
1. Опишите конфигурацию [политики доступа](../../concepts/policy.md) в виде [схемы данных](../../s3/api-ref/policy/scheme.md) формата JSON.
    ...
    * `Statement` — правила политики доступа:
    ...
    Проверка доступа по [ACL](../../security/acl.md) объекта происходит после проверки политики Security Token Service. Поэтому если для сервисного аккаунта, через который вы получаете временные ключи доступа, настроены разрешения ACL на объекты в бакете, эти объекты станут доступны для запросов с временным ключом доступа, независимо от указанной политики. Подробнее см. на [схеме взаимодействия механизмов управления доступом](../../security/overview.md) в Object Storage.
