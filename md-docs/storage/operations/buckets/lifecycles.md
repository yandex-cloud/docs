[Документация Yandex Cloud](../../../index.md) > [Yandex Object Storage](../../index.md) > [Пошаговые инструкции](../index.md) > Бакеты > Управление жизненными циклами объектов
...
- Консоль управления {#console}
  ...
  1. Перейдите на вкладку **Настройки**, затем на вкладку **Жизненный цикл**.
  1. Нажмите **Настроить**.
  1. Заполните открывшуюся форму. Вы можете добавлять, удалять и редактировать правила конфигурации.
     ...
     1. Заполните поля:
        ...
        * **Максимальный размер в байтах** — правило срабатывает для всех объектов, размер которых меньше указанного.
        * **Минимальный размер в байтах** — правило срабатывает для всех объектов, размер которых больше указанного.
     1. Выберите и настройте нужные типы действий, выполняемые с объектами при срабатывании правила:
        ...
        * `NoncurrentVersionExpiration` — удаление нетекущих версий объектов из бакета:
           * **Срок срабатывания** — количество дней, через которое нетекущая версия будет удалена.
           * **Хранить версий** — количество последних нетекущих версий, которые хранятся неограниченное время. Версии сверх этого количества удаляются с учетом срока срабатывания.
        * `NoncurrentVersionTransition` — перемещение нетекущих версий объектов из стандартного (`STANDARD`) в холодное (`COLD`) , ледяное (`ICE`) или умное (`INTELLIGENT_TIERING`) хранилище, а также из холодного в ледяное хранилище:
           * **Срок срабатывания** — количество дней, через которое нетекущая версия будет перенесена в другой класс хранилища.
           * **Класс хранилища** — тип хранилища, в которое будут перемещены объекты.
           * **Хранить версий** — количество последних нетекущих версий, которые хранятся в исходном классе хранилища неограниченное время. Версии сверх этого количества переносятся в другой класс хранилища с учетом срока срабатывания.
