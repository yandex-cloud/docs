...
Если у бакета включено [шифрование](concepts/encryption.md), назначьте сервисному аккаунту [роль](../kms/security/index.md#service) `kms.keys.encrypterDecrypter` на [ключ {{ kms-short-name }}](../kms/concepts/key.md), которым зашифрован бакет. Это можно сделать, например, с помощью команды CLI:
