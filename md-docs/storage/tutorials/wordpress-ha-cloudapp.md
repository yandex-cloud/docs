...
* Плата за использование бакета для хранения медиафайлов (см. [тарифы {{ objstorage-full-name }}](../pricing.md)).
...
1. [Удалите](../operations/objects/delete-all.md) все объекты из бакетов {{ objstorage-name }}, созданных при установке приложения, так как удалять можно только пустые бакеты.
