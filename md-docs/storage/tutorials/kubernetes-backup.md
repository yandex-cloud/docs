...
- Вручную {#manual}
  ...
  1. [Создайте бакет в {{ objstorage-name }}](../operations/buckets/create.md).
  ...
  1. Выдайте [сервисному аккаунту](../../iam/concepts/users/service-accounts.md) права **READ и WRITE** к [бакету](../concepts/bucket.md) в {{ objstorage-name }}. Для этого [выполните настройки ACL бакета](../operations/buckets/edit-acl.md).
...
- {{ TF }} {#tf}
  ...
  1. Скачайте в ту же рабочую директорию файл конфигурации [velero-backup.tf](https://github.com/yandex-cloud-examples/yc-mk8s-velero-backup/blob/main/velero-backup.tf).
     ...
     * [Бакет](../concepts/bucket.md) в {{ objstorage-name }}.
  1. Укажите в файле `velero-backup.tf`:
     ...
     * `storage_sa_id` — идентификатор сервисного аккаунта с ролью `storage.admin`. С помощью него будет создан бакет в {{ objstorage-name }} с разрешением `READ и WRITE` в [ACL](../concepts/acl.md) для сервисного аккаунта `sa_name_velero`.
...
1. Установите приложение Velero:
   ...
   - Вручную {#manual}
       ...
       1. [Назначьте ему роль](../../iam/concepts/access-control/roles.md) `storage.editor` для доступа к [{{ objstorage-full-name }}](../index.md).
...
- Вручную {#manual}
  ...
  * [Удалите бакет {{ objstorage-name }}](../operations/buckets/delete.md).
