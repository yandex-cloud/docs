...
В практическом руководстве показано, как развернуть [MLFlow Tracking Server](https://mlflow.org/docs/latest/tracking.html) для логирования экспериментов и артефактов на отдельной виртуальной машине [{{ compute-full-name }}](../../compute/index.md). Эксперименты будут проводиться в {{ jlab }}Lab Notebook. Для хранения внутренних объектов будет использоваться база данных [{{ mpg-full-name }}](../../managed-postgresql/index.md), а для хранения артефактов — бакет [{{ objstorage-full-name }}](../index.md).
