...
Помимо решения, описанного в статье, доступ в Object Storage от ресурсов облачной сети без доступа в интернет также можно реализовать с помощью [сервисного подключения VPC](../../vpc/concepts/private-endpoint.md). Подробнее см. на странице [Доступ в бакет с помощью сервисного подключения из VPC](../operations/buckets/access-via-vpc.md).

{% endnote %}

Подключиться к [Yandex Object Storage](../index.md) в Yandex Cloud можно через соответствующий [API Endpoint](../../api-design-guide/concepts/endpoints.md), FQDN которого потом преобразуется в публичный IP-адрес с помощью службы DNS.
...
| `s3-bucket-<...>` | [Бакет](../concepts/bucket.md) в Object Storage. |
...
* плата за использование Object Storage (см. [тарифы Yandex Object Storage](../pricing.md)).
...
1. Получите объект из бакета в Object Storage с помощью инструмента [AWS CLI](../tools/aws-cli.md). Имя бакета будет получено из переменной среды на тестовой ВМ.
