...
Предполагается, что патч состоит из одного файла с именем `ycgame-update-v1.1.exe`. Он будет загружен в [бакет](../concepts/bucket.md) [{{ objstorage-full-name }}](../index.md).
...
* Плата за хранение данных в {{ objstorage-name }}, операции с ними и исходящий трафик (см. [тарифы {{ objstorage-name }}](../pricing.md)).
...
- API {#api}
  ...
  Используйте метод API [create](../s3/api-ref/bucket/create.md).
...
- API {#api}
  ...
  Используйте метод API [putBucketLogging](../s3/api-ref/bucket/putBucketLogging.md) для бакета с файлами. Тело HTTP-запроса:
...
- {{ TF }} {#tf}
  ...
  1. Добавьте к конфигурационному файлу из [шага с созданием бакетов](#create-buckets) параметры [объекта](../concepts/object.md), который необходимо загрузить:
...
- API {#api}
  ...
  Используйте метод API [upload](../s3/api-ref/object/upload.md).
...
1. Получите логи запросов к бакету с файлами:
   ...
   - API {#api}
     ...
     1. Получите список объектов в бакете с логами с помощью метода API [listObjects](../s3/api-ref/bucket/listobjects.md).
     1. Найдите в полученном списке объект с логом, сохраненным после скачивания файла `ycgame-update-v1.1.exe`, и скачайте его с помощью метода API [get](../s3/api-ref/object/get.md).

   {% endlist %}

1. По логам запросов к бакету-источнику убедитесь, что CDN-серверы не скачивали файл из источника после вашего запроса. Подробнее о содержимом логов см. в разделе [{#T}](../concepts/server-logs.md#object-format) документации {{ objstorage-name }}.
...
1. [Удалите](../operations/objects/delete.md) объект `ycgame-update-v1.1.exe` из бакета с файлами.
1. [Удалите](../operations/buckets/delete.md) бакет с файлами.
1. [Удалите](../operations/objects/delete.md) все объекты из бакета с логами.
1. [Удалите](../operations/buckets/delete.md) бакет с логами.
