...
С помощью этого руководства вы разместите свой статический сайт, созданный на фреймворке [Gatsby](https://www.gatsbyjs.com/docs), в сервисе [{{ objstorage-name }}](../index.md) и настроите доступ к нему по протоколу HTTPS с помощью сервиса [{{ certificate-manager-full-name }}](../../certificate-manager/index.md).
...
Установите и настройте AWS CLI по [инструкции](../tools/aws-cli.md#before-you-begin).
...
Чтобы разместить статический сайт в облаке, [создайте](../operations/buckets/create.md) бакет и [настройте](../operations/hosting/setup.md#hosting) его.

{% list tabs group=instructions %}

- Консоль управления {#console}

  1. В [консоли управления]({{ link-console-main }}) выберите каталог, в котором хотите создать [бакет](../concepts/bucket.md).
  ...
  1. Выберите тип [доступа](../concepts/bucket.md#bucket-access) **Публичный** для всех операций.
...
[Добавьте](../operations/hosting/certificate.md#cert-manager) сертификат в настройках бакета:
...
- API {#api}
  ...
  Чтобы привязать к бакету TLS-сертификат {{ certificate-manager-name }}, воспользуйтесь методом REST API [setHTTPSConfig](../api-ref/Bucket/setHTTPSConfig.md) для ресурса [Bucket](../api-ref/Bucket/index.md) или вызовом gRPC API [BucketService/SetHTTPSConfig](../api-ref/grpc/Bucket/setHTTPSConfig.md).
