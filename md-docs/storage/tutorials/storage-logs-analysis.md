[Документация Yandex Cloud](../../index.md) > [Yandex Object Storage](../index.md) > [Практические руководства](index.md) > Сбор и анализ логов > Анализ логов с использованием DataLens

# Анализ логов Object Storage при помощи DataLens


Для бакета Yandex Object Storage можно включить [логирование действий](../concepts/server-logs.md). В логах хранится информация по операциям с [бакетом](../concepts/bucket.md) и [объектами](../concepts/object.md), которые в нем находятся. Анализ логов бакета может быть полезен, например, если вам нужно выявить причины резкого повышения нагрузки или понять общую картину распределения трафика.
...
- API {#api}
  ...
  Используйте метод REST API [create](../s3/api-ref/bucket/create.md).
...
- Terraform {#tf}
  ...
  Чтобы включить механизм логирования в бакете, который вы хотите отслеживать:
     ...
     1. Откройте файл конфигурации Terraform и добавьте блок `logging` во фрагмент с описанием бакета.
        ...
        * `target_prefix` — [префикс ключа](../concepts/server-logs.md#key-prefix) для объектов с логами.
...
- API {#api}
  ...
  Используйте метод REST API [putBucketLogging](../s3/api-ref/bucket/putBucketLogging.md).
