[Документация Yandex Cloud](../../index.md) > [Yandex Object Storage](../index.md) > [Практические руководства](index.md) > Перенос данных и интеграция с базами данных > Выгрузка данных Greenplum® в гибридное хранилище Yandex Object Storage

# Выгрузка данных Greenplum® в гибридное хранилище Yandex Object Storage

# Выгрузка данных Greenplum® в гибридное хранилище


В кластере Greenplum® можно включить [гибридное хранилище](../../managed-greenplum/concepts/hybrid-storage.md) для [таблиц типа AO и AOCO](../../managed-greenplum/concepts/tables.md) (append-optimized storage). Тогда с помощью [расширения Yezzey](../../managed-greenplum/operations/extensions/yezzey.md) данные этих таблиц можно перенести из хранилища кластера в гибридное хранилище.
...
* Бакет Yandex Object Storage: использование хранилища и выполнение операций с данными ([тарифы Object Storage](../pricing.md)).
...
1. [Создайте кластер](../../managed-greenplum/operations/cluster-create.md) Greenplum®. При создании убедитесь, что включена опция **Гибридное хранилище**.
...
1. Получите SSL-сертификат для подключения к базе данных Greenplum®:
   ...
   - Windows (PowerShell) {#windows}
      ...
      Корпоративные политики и антивирус могут блокировать скачивание сертификата. Подробнее в разделе [Вопросы и ответы](../../managed-greenplum/qa/connection.md#get-ssl-error).
...
## Выгрузите таблицу Greenplum® в гибридное хранилище {#transfer}
...
1. Перенесите данные таблицы `ao_table` в гибридное хранилище:
...
1. Проверьте, сколько места в локальном кеше кластера и в гибридном хранилище занимают:
   ...
   * каждый сегментный файл таблицы `ao_table`:
   ...
   Если столбец `external_bytes` в выводе команд содержит ненулевые значения, значит, таблица перенесена в гибридное хранилище.

1. Посмотрите, какие сегментные файлы таблицы перенесены в гибридное хранилище:
...
## Перенесите таблицу из гибридного хранилища в хранилище кластера {#offload-to-local-storage}

Если нужно перенести таблицу `ao_table` из гибридного хранилища обратно в хранилище кластера, выполните команду:
