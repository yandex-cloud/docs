...
* плата за хранение данных в бакете и операции с ними (см. [тарифы {{ objstorage-full-name }}](../pricing.md));
...
- Консоль управления {#console}
  ...
  1. Задайте параметры [публичного доступа](../security/public-access.md) на чтение [объектов](../concepts/object.md) в бакете, получение списка объектов и чтение настроек бакета — `{{ ui-key.yacloud.storage.bucket.settings.access_value_private }}`.
...
1. [Удалите](../operations/objects/delete-all.md) все объекты из бакета {{ objstorage-name }}.
1. [Удалите](../operations/buckets/delete.md) бакет {{ objstorage-name }}.
