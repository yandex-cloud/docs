...
Вы создадите [бакет](../concepts/bucket.md) в [{{ objstorage-name }}](../index.md), смонтируете его к файловой системе Windows с помощью [rclone](https://rclone.org) и загрузите туда данные для обучения [модели](../../datasphere/concepts/models/index.md) в [{{ ml-platform-full-name }}]({{ link-datasphere-main }}).
