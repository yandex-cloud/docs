...
В стоимость поддержки инфраструктуры входит плата за использование бакета ([тарифы Object Storage](../../pricing.md)).
...
- Консоль управления {#console}
    ...
    1. В [консоли управления](https://console.yandex.cloud) выберите каталог, в котором хотите создать [бакет](../../concepts/bucket.md), например `example-folder`.
    1. [Перейдите](https://console.yandex.cloud/link/storage) в сервис **Object Storage**.
    1. Нажмите **Создать бакет**.
    1. На странице создания бакета:
        * укажите имя бакета в соответствии с [правилами именования](../../concepts/bucket.md#naming);
...
- Консоль управления {#console}
  ...
  1. В блоке **Назначение** задайте параметры объекта назначения:
      ...
      1. **Бакет** — выберите [созданный ранее](#create-backet) бакет.

  1. В блоке **Сбор логов с управляющего слоя**:

      1. Включите сбор логов.
      1. **Ресурс** — выберите `Организация`.
      1. **Организация** — не требует заполнения, содержит имя текущей организации.
      1. **Облако** — оставьте значение по умолчанию `Все`.
...
Если для выполнения руководства вы создали отдельный бакет, вы можете [удалить его](../../operations/buckets/delete.md), чтобы перестать платить за [использование бакета](../../pricing.md).