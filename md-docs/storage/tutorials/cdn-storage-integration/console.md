...
- Консоль управления {#console}
  ...
  1. В поле **{{ ui-key.yacloud.storage.bucket.settings.field_name }}** укажите уникальное [имя](../../concepts/bucket.md#naming) бакета.
...
- {{ yandex-cloud }} CLI {#cli}
  ...
  Выполните команду, указав уникальное [имя](../../concepts/bucket.md#naming) бакета:
...
1. Загрузите файл в бакет:
    ...
    - {{ yandex-cloud }} CLI {#cli}
      ...
      * `--body` — локальный путь, по которому находится созданный ранее HTML-файл. Например: `./index.html`.
      ...
      Подробнее о команде `yc storage s3api put-object` читайте в [справочнике CLI](../../cli-ref/s3api/put-object.md).

   - API {#api}
   
     Воспользуйтесь методом REST API [upload](../../s3/api-ref/object/upload.md).
