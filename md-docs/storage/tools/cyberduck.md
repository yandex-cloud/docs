...
- macOS {#macos}
  ...
  1. Укажите параметры подключения:
      * **Server** — `storage.yandexcloud.net`.
        ...
        Чтобы подключиться к определенному бакету, в поле **Server** укажите `<имя_бакета>.storage.yandexcloud.net`. Таким образом можно подключиться только к бакету, в имени которого не содержится точка. Для бакетов с точкой в имени воспользуйтесь [альтернативной инструкцией](#alternative-connection).
      * **Port** — `443`.
      * **Access Key ID** — идентификатор статического ключа, [полученный ранее](#before-you-begin).
      * **Secret Access Key** — содержимое статического ключа, [полученное ранее](#before-you-begin).
  1. Нажмите кнопку **Connect**.
