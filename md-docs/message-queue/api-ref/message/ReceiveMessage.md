...
* `Body` — тело сообщения.
* `MD5OfBody` — MD5-хеш тела сообщения.
...
`ReceiveRequestAttemptId` | **string** | Нет | Идентификатор для повтора попытки получения сообщений из FIFO-очереди. Подробнее в разделе [Дедупликация](../../concepts/deduplication.md#request-attempts).
...
`MessageGroupId` | Идентификатор группы сообщений, используется в очередях FIFO. Подробнее в разделе [Дедупликация](../../concepts/deduplication.md).
