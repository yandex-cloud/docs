[Документация Yandex Cloud](../../index.md) > [Yandex Message Queue](../index.md) > [Практические руководства](index.md) > Автомасштабирование группы ВМ для обработки сообщений из очереди

# Автоматическое масштабирование группы виртуальных машин для обработки сообщений из очереди Message Queue
...
* плата за запросы к очереди ([тарифы Message Queue](../pricing.md)).
...
- API {#api}
  ...
  1. Создайте очередь `queue-autoscale-queue` с помощью метода API [CreateQueue](../api-ref/queue/CreateQueue.md). В ответных данных будет указан URL очереди.
...
1. [Удалите](../operations/message-queue-delete-queue.md) очередь `queue-autoscale-queue`.
