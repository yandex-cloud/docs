...
Yandex Neurosupport — сервис для подсказок и автоответов на основе базы знаний, специализированный под домен технической поддержки. Выдает рекомендации в реальном времени и естественной языковой манере. Может использоваться для формирования подсказок оператору поддержки, для автоматических ответов клиентам и в качестве внутреннего помощника для сотрудников.

Neurosupport генерирует рекомендацию, как ответить на диалог или запрос, используя:
...
1. Проверить доступ к API. Адрес: [https://supportgpt.api.cloud.yandex.net](https://supportgpt.api.cloud.yandex.net).
...
Neurosupport работает только по API. Логику вызовов API, конкретный состав передаваемых данных, логику использования результата вам требуется определить на своей стороне, в частности:
...
`https://supportgpt.api.cloud.yandex.net/indexer/v1/indexes/{index_name}`
...
`https://supportgpt.api.cloud.yandex.net/indexer/v1/indexes`
...
`https://supportgpt.api.cloud.yandex.net/indexer/v1/indexes/{index_name}/switch_version`
...
`https://supportgpt.api.cloud.yandex.net/indexer/v1/indexes/{index_name}/rename`
...
`https://supportgpt.api.cloud.yandex.net/indexer/v1/indexes/{index_name}/delete`
...
При использовании Neurosupport есть 2 ключевых шага:
...
В Neurosupport реализовано версионирование индексов документов. Каждое изменение в индексе сохраняется в новую версию, между которыми при необходимости можно переключаться.
...
#|
|| **Название типа** | **Соответствие в PostgreSQL** | **Диапазоны значений / Формат** | **Примеры** ||
...
- [Рекомендации по использованию YandexGPT](https://aistudio.yandex.ru/docs/ru/ai-studio/gpt-prompting-guide/popular-problems-solving)
- [Параметры LLM](https://aistudio.yandex.ru/docs/ru/ai-studio/gpt-prompting-guide/introduction/llm-settings)
- [Основы работы с промптами](https://aistudio.yandex.ru/docs/ru/ai-studio/gpt-prompting-guide/introduction/basics)
- [Элементы промпта](https://aistudio.yandex.ru/docs/ru/ai-studio/gpt-prompting-guide/introduction/elements)
- [Общие советы для создания промптов](https://aistudio.yandex.ru/docs/ru/ai-studio/gpt-prompting-guide/introduction/general-tips)
- [Примеры промптов](https://aistudio.yandex.ru/docs/ru/ai-studio/gpt-prompting-guide/introduction/examples)
...
Для оценки качества ответов рекомендуется применять сравнение side-by-side, то есть сравнивать ответы Neurosupport с ответами оператора, который был дан для аналогичного диалога.
...
1. Корректно будет сравнивать Neurosupport и оператора только на тех диалогах, где нет какой-то дополнительной информации, доступной оператору (но недоступной для модели) и где не требуется дополнительных действий (т.&nbsp;к. оператор, как правило, отвечает на основании результата данных действий).

1. Сравните долю полученных оценок 3, 4, 5 для оператора и для Neurosupport — это покажет, какую долю потока возможно автоматизировать с помощью сервиса.