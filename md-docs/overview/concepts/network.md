[Документация Yandex Cloud](../../index.md) > [Обзор платформы](../index.md) > Платформа Yandex Cloud > Устройство сети

# Устройство сети в Yandex Cloud

Сеть в Yandex Cloud можно условно разделить на две большие части: 

* [Физическая сеть](#underlay) — это аппаратная сеть внутри [центров обработки данных (ЦОД)](geo-scope.md), транспортная сеть между ЦОД и в [местах подключения](../../interconnect/concepts/pops.md) к внешним сетям и интернету. Физическую сеть часто называют термином *Underlay*.
* [Виртуальная сеть](#overlay) — работает поверх физической сетевой инфраструктуры. Сервисы виртуальной сети [Virtual Private Cloud (VPC)](../../vpc/concepts/index.md) предоставляют пользователю:
...
* [ЦОД (зоны доступности)](geo-scope.md) Yandex Cloud.
...
Виртуальная сеть в Yandex Cloud состоит из набора сетевых функций сервиса [Virtual Private Cloud](../../vpc/concepts/index.md) и предоставляет пользователям следующие возможности:
...
CloudGate — группы сервисных ВМ в каждой [зоне доступности](geo-scope.md), которые обеспечивают IP-взаимодействие между физической и виртуальной сетями, а также предоставляют дополнительные сетевые функции:
...
Группы сервисных виртуальных машин для всех сетевых функций CloudGate развернуты в каждой [зоне доступности](geo-scope.md). На схеме выше для упрощения понимания представлено расположение элементов CloudGate в рамках одной зоны доступности. 
...
ЦОД напрямую соотносятся с [зонами доступности](geo-scope.md) Yandex Cloud.
...
* [Архитектура и защита базового интернет-сервиса](../../vpc/tutorials/web-service.md)
* [Маршрутизация через NAT-инстанс](../../vpc/tutorials/nat-instance/index.md)
* [Настройки DHCP для работы с корпоративным DNS-сервером](../../vpc/tutorials/dhcp-options.md)
