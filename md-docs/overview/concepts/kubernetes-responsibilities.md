[Документация Yandex Cloud](../../index.md) > [Обзор платформы](../index.md) > Сервисы Yandex Cloud > Разграничение зон контроля > Разграничение зон контроля в Managed Service for Kubernetes
...
* **Системные компоненты на узлах**. Yandex Cloud устанавливает на [узлы](../../managed-kubernetes/concepts/index.md#node-group) следующие компоненты:
   ...
   * Сетевые плагины CNI (Calico / Cilium), если при создании кластера выбран предустановленный CNI.
...
В [кластере без предустановленного CNI](../../managed-kubernetes/concepts/cluster-without-cni.md) Yandex Cloud управляет мастером и группами узлов. За сетевой плагин отвечает [пользователь](#responsibility).

### Системные плагины {#system-plugins}

{% note info %}

Пользователь не может изменять или удалять системные плагины под управлением Yandex Cloud.
...
* [Собственный сетевой плагин CNI](#responsibility).
...
В кластере без предустановленного CNI пользователь отвечает за:

* Выбор сетевого плагина.
* Установку и настройку плагина перед запуском приложений.
* Обновление плагина.
* Работоспособность плагина и его совместимость с конфигурацией кластера, в том числе при изменении сетевых настроек и обновлении кластера.
