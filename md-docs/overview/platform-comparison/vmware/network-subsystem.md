...
В NSX-T используются различные варианты балансировки, смотрите схему:
...
Внутри — свой собственный виртуальный коммутатор с uplink и внутренними портами, смотрите схему:
...
Для реализации выделенного подключения on-premise-инфраструктуры заказчика к облаку подключается сервис для создания приватных выделенных сетевых соединений - [Cloud Interconnect](../../../interconnect/concepts/index.md), который использует eBGP для обмена маршрутной информацией. 
...
| В Yandex Cloud есть возможность использовать отдельно балансировщик уровня L4 — [Network Load Balancer](https://yandex.cloud/ru/services/network-load-balancer) (NLB) и L7 — [Application Load Balancer](https://yandex.cloud/ru/services/application-load-balancer) (ALB).
...
Для поиска проблем в оверлейных (облачных) сетях Yandex Cloud есть инструмент под названием [Карта инфраструктуры](../../../console/concepts/infrastructure-map.md). Карта позволяет быстро проверить корректность настройки виртуальной сетевой связности между объектами облака (cloud wiring).
