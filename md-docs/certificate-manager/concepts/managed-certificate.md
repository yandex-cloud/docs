...
Сертификат, созданный с помощью Certificate Manager, можно использовать только в [указанных](services.md) сервисах Yandex Cloud.
...
1. Для выпуска сертификата пройдите процедуру проверки прав на домены, которые вы указали ранее.
    ...
    В зависимости от выбранного типа проверки, самостоятельно разместите файл на веб-сервере либо добавьте ресурсную запись `TXT` или `CNAME` с нужным значением в DNS-сервисе. Подробнее о типах проверки и способах их прохождения читайте в разделе [Проверка прав на домен](challenges.md).
...
1. После успешного прохождения проверки прав на домены, сертификат будет выпущен, и перейдет в статус `Issued`. Вы можете использовать сертификат в сервисах, интегрированных с Certificate Manager.
...
1. Завершение выпуска сертификата. На этом этапе автоматически выполняются следующие действия:
    ...
    1. Завершение выпуска и сохранение сертификата в Certificate Manager.
...
1. Пройдите процедуру проверки прав на домены.
    ...
    Если сертификат используется в [статическом сайте в Object Storage](../../tutorials/web/static/index.md) и не содержит [доменов с маской](https://en.wikipedia.org/wiki/Wildcard_certificate),
