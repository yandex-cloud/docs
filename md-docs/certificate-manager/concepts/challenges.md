[Документация Yandex Cloud](../../index.md) > [Yandex Certificate Manager](../index.md) > [Концепции](index.md) > Проверка прав на домен
...
Certificate Manager ожидает успешного завершения процедуры проверки по каждому домену из сертификата (статус всех проверок — `Valid`). После этого Let's Encrypt выпустит сертификат. Статус сертификата изменится на `Issued`, и вы сможете использовать его в сервисах, интегрированных с Certificate Manager.

Если процедура проверки не будет выполнена в течение одной недели, сертификат перейдет в статус `Invalid` при первоначальном получении или `Renewal_failed` при обновлении. Чтобы после этого получить сертификат, создайте новый запрос на выпуск от Let's Encrypt.
...
1. [Перейдите](https://console.yandex.cloud/link/certificate-manager) в сервис **Certificate Manager** и нажмите на имя нужного сертификата.
...
При использовании CNAME-записи достаточно пройти проверку один раз. Для этого необходимо делегировать Certificate Manager право ответа в DNS-зоне домена, используемой при проверке. В этом случае проверки будут проходить автоматически.
...
* Для каждого домена сертификата в веб-сервере настроено перенаправление с
    ...
    Этот эндпоинт доступен только по IPv6.
