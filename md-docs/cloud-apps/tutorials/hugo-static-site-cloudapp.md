[Документация Yandex Cloud](../../index.md) > [Cloud Apps](../index.md) > [Практические руководства](index.md) > Создание сайта на Hugo с помощью Cloud Apps

# Установка Hugo с помощью Cloud Apps


С помощью этого руководства вы установите и настроите статический сайт на [Hugo](https://gohugo.io/) — генераторе статических сайтов — используя сервис [Cloud Apps](../index.md). Приложение развернет сайт в [Object Storage](../../storage/index.md) и настроит доступ по домену с автоматическим выпуском SSL-сертификатов.
...
1. [Установите Hugo с помощью Cloud Apps](#install-hugo).
...
- Консоль управления {#console}
  ...
  1. Задайте параметры приложения:
      ...
      || **Поддомен** | — | Пустой | Домен, на котором будет доступен статический сайт на Hugo. Формируется добавлением поддомена к домену DNS-зоны: `site.example.com` или `example.com`, если поддомен не указан. ||
      || **Конфигурация веб-сайта** | Да | — | В выпадающем списке выберите **Hugo**. ||
...
1. [Создайте](https://sourcecraft.dev/portal/docs/ru/sourcecraft/operations/repo-create#create-from-template) репозиторий из [шаблона `cloudapp-hugo`](https://sourcecraft.dev/yandex-cloud-examples/cloudapp-hugo).
...
1. Обновите конфигурационные файлы в редакторе, например в Visual Studio Code:
   ...
   1. Файл `hugo.toml`:
      ...
      [[deployment.targets]]
      name = 'production'
      url = 's3://<домен>?region=ru-central-1&endpoint=https://storage.yandexcloud.net'
      ...
      ```
...
1. Установите локально [Hugo](https://gohugo.io/installation/) нужной версии для локальной разработки.
...
1. Для сборки продакшен-версии выполните команду:
   ...
   Результат сборки будет сохранен в папке `public/`.
...
1. Проверьте, что открывается главная страница вашего Hugo-сайта и работает его навигация.
...
1. Удалите сервисное подключение SourceCraft:
   ...
   1. Выберите репозиторий, созданный из шаблона `cloudapp-hugo`.
