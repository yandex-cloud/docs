...
- Консоль управления {#console}
  ...
  1. [Режим защиты](../concepts/profiles.md#protection-mode) определяет, какие правила профиля применяются к трафику. Для каждого правила вы [выбираете режимы](rule-add.md), в которых оно действует. По умолчанию правило действует во всех режимах.
     
     Режимы в порядке усиления защиты:
     
     * **Нормальный** — правила для штатной нагрузки.
     * **Повышенная защита** — правила для подозрительного роста нагрузки.
     * **Под атакой** — правила для атаки или критической нагрузки.
     
     В разделе **Адаптивная защита** выберите способ переключения режимов защиты:
     
     * **Автоматически** — сервис будет переключать режим в зависимости от входящей нагрузки.
     
       По умолчанию действует режим **Нормальный**. Для режимов **Повышенная защита** и **Под атакой** настройте:
     
       1. Порог включения: в поле **Включать, если входящая нагрузка превышает** укажите количество входящих запросов и выберите интервал подсчета. Режим включится, если число запросов за этот интервал превысит порог.
       1. Время сохранения режима: в поле **Сохранять текущий режим после снижения нагрузки** задайте задержку перед возвратом к нормальному режиму после снижения нагрузки ниже порога. Это поможет избежать частых переключений при колебаниях нагрузки.
     
       Чтобы подобрать пороги:
     
       * Оцените обычные пики трафика по графику [Inbound total RPS](monitoring.md#monitoring-dashboards) — он показывает число входящих запросов в секунду. Выберите период без атак, включающий будни, выходные и часы пик. Учтите ожидаемый рост трафика, например во время рекламных кампаний.
       * Для режима **Повышенная защита** задайте порог выше обычных пиков с запасом на допустимые всплески.
       * Для режима **Под атакой** задайте более высокий порог, но ниже уровня нагрузки, при котором приложение начинает замедляться или возвращать ошибки. Определите этот уровень по метрикам приложения или результатам нагрузочного тестирования.
     
     * **Вручную** — выберите нужный режим. Он будет действовать независимо от нагрузки, пока вы не выберете другой или не включите автоматическое управление.
