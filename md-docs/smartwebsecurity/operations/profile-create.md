...
- Консоль управления {#console}
  ...
  1. Выберите или создайте капчу [{{ captcha-full-name }}](../../smartcaptcha/index.md) для проверки подозрительных запросов:
  ...
  1. (опционально) Настройте опцию **{{ ui-key.yacloud.smart-web-security.waf.field_analyze-request-body }}**, выбрав действие при превышении максимального размера:
...
- CLI {#cli}
  ...
  1. Чтобы создать профиль безопасности, выполните команду:
     ...
     * `--captcha-id` — идентификатор капчи [{{ captcha-full-name }}](../../smartcaptcha/index.md) для проверки подозрительных запросов. Необязательный параметр.
...
- {{ TF }} {#tf}
  ...
  1. Опишите в конфигурационном файле {{ TF }} параметры ресурсов, которые необходимо создать:
      ...
      * `captcha_id` — идентификатор капчи [{{ captcha-full-name }}](../../smartcaptcha/index.md) для проверки подозрительных запросов. Необязательный параметр.
