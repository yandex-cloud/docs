...
С реализацией такой схемы помогут сервисы {{ yandex-cloud }} — [{{ alb-full-name }}](../../application-load-balancer/index.md) (ALB) и [{{ sws-full-name }}](../concepts/index.md) (SWS).
...
Сервис {{ sws-name }} позволяет защитить ваши ресурсы от [DDoS-атак](../../glossary/ddos.md) и ботов на уровне L7. Дополнительно можно подключить [WAF](../concepts/waf.md) и ограничить нагрузку на ваш ресурс с помощью модуля [Advanced Rate Limiter](../concepts/arl.md) (ARL). Параметры защиты ваших ресурсов настраиваются в профиле безопасности {{ sws-name }}. Профиль безопасности подключается к L7-балансировщику.
...
Другие способы создания профиля безопасности см. в разделе [{#T}](../operations/profile-create.md).
