...
[{{ sws-name }}](../concepts/index.md) (SWS) защищает веб-ресурсы от интернет-угроз, фильтруя вредоносный трафик.
...
* [Плата за количество запросов](../pricing.md) в сервис {{ sws-name }}.
...
* Плата за ресурсы [прокси-сервера](../pricing.md) при использовании [доменов](../concepts/domain-protect.md).
...
- L7-балансировщик {#balancer}
  ...
  [{{ alb-name }}](balancer-with-sws-profile/index.md) равномерно распределяет входящий трафик между узлами, что позволяет избежать перегрузки и повысить отказоустойчивость.
...
- Консоль управления {#console}
  ...
  1. Включите тестовый режим для правила Smart Protection `sp-rule-1`:
     ...
     1. Нажмите **{{ ui-key.yacloud.common.save-changes }}**.
      ...
      Трафик, обработанный правилами в режиме **{{ ui-key.yacloud.smart-web-security.overview.column_dry-run-rule }}**, тарифицируется. Подробнее см. раздел [{#T}](../pricing.md).
...
[Списки адресов](../concepts/lists.md) помогают оптимизировать проверку трафика, разрешая запросы для заведомо надежных (белых) адресов. Небезопасные или сомнительные адреса можно сразу блокировать или отправлять в {{ captcha-name }}.
...
Подробное описание работы с логами см. в разделе [{#T}](../operations/configure-logging.md).
...
Подробное описание графиков см. в разделе [{#T}](../operations/monitoring.md).
