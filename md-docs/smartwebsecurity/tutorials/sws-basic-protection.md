...
Для выполнения нагрузочного тестирования можно воспользоваться сервисом [Load Testing](../../load-testing/index.md). Подробнее о настройке нагрузки по протоколу HTTP смотрите в разделе [HTTPS-тест с постоянной нагрузкой с помощью Phantom](../../load-testing/tutorials/loadtesting-https-phantom.md).
