...
* Значение [ASN](https://wikipedia.org/wiki/Autonomous_system_(Internet)) в виде целого числа без префикса `as`
|
* `1.2.33.44`
...
| | ||
|| `Bot score` | |
При проверке трафика система присваивает клитенту рейтинг от `0` до `100`. Для фильтрации трафика можно указать значение или диапазон значений этого рейтинга с помощью операторов `>=`, `<=`, `=`, `!=`.
Рамки значений: `не более 20` — человек, `20–40` — вероятно человек, `40–60` — не определено, `60–80` — вероятно бот, `более 80` — бот.
|
* `=10`
* `>=20`
* `<=40`
