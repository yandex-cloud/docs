[Документация Yandex Cloud](../../index.md) > [Yandex Smart Web Security](../index.md) > [Концепции](index.md) > ARL (лимит на запросы)
...
Для настройки ARL используются профили. Профиль состоит из набора правил, которые устанавливают лимиты на запросы определенного типа и действия при превышении лимитов.
...
Подключение к профилю безопасности описано в разделе [Создайте и подключите профиль ARL](../quickstart.md#arl).
...
Для вычисления лимитов и тестирования правил ARL предусмотрен режим **Только логирование (Dry Run)**. В этом режиме действия правил к запросам не применяются. Это позволяет оценить возможности бэкендов и подобрать оптимальные значения лимитов.

## Действия при превышении лимита {#over-limit-actions}

Запросы сверх лимита можно блокировать или направлять на капчу. При блокировке запроса клиенту будет возвращена ошибка `429` — `Too Many Requests`.
