...
* Отправлять на капчу запросы свыше лимита — запросы, поступившие после достижения лимита, будут направлены в [{{ captcha-name }}](../../smartcaptcha/index.md). Капча настраивается через профиль безопасности, к которому подключен профиль ARL. Это действие помогает отделять легитимных пользователей от ботов. При этом запросы не будут заблокированы полностью, доступность приложения сохранится.
