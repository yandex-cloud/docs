[Документация Yandex Cloud](../../index.md) > [Yandex Audit Trails](../index.md) > [Практические руководства](index.md) > Экспорт аудитных логов в SIEM > Загрузка аудитных логов в SIEM ArcSight
...
1. [Трейл](../concepts/trail.md) загружает логи в бакет Object Storage.
...
1. [Создайте ВМ](../../compute/operations/vm-create/create-linux-vm.md) из образа Linux с операционной системой [Ubuntu 20.04](https://yandex.cloud/ru/marketplace/products/yc/ubuntu-20-04-lts).
...
- Консоль управления {#console}
  ...
  1. На панели сверху нажмите **Создать новый ключ** и выберите **Создать статический ключ доступа**.
...
- CLI {#cli}
  ...
  1. [Роль](../security/index.md#roles) `audit-trails.viewer` на каталог:
...
- Консоль управления {#console}
  ...
  1. В блоке **Назначение** задайте параметры объекта назначения:
     ...
     1. **Назначение** — `Object Storage`.
     1. **Бакет** — имя бакета.
     1. **Префикс объекта** — необязательный параметр, участвует в [полном имени](../concepts/format.md#log-file-name) файла аудитного лога.
     ...
     {% endnote %}
      ...
      1. **Ключ шифрования** — укажите ключ шифрования `arcsight-kms`, которым [зашифрован](../../storage/concepts/encryption.md) бакет.

  1. В блоке **Сбор логов с управляющего слоя**:

     1. Включите сбор логов.
     1. **Ресурс** — выберите `Каталог`.
     1. **Каталог** — не требует заполнения, содержит имя текущего каталога.

  1. В блоке **Сбор логов со слоя данных** отключите сбор логов.
  1. В блоке **Сервисный аккаунт** выберите `sa-arcsight`.
  1. В поле **Имя** укажите имя создаваемого трейла, например, `arcsight-trail`.
  1. Нажмите **Создать**.
