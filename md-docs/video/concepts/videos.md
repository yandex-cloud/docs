...
Вы можете загрузить видео с несколькими аудиодорожками или добавить их после загрузки с помощью [нейроперевода](videos.md#translation). Дорожки выбираются в меню [плеера](player.md), подробнее см. в разделе [{#T}](../operations/player-control.md#audio).
...
[Нейроперевод](../operations/video/translation.md) позволяет автоматически переводить видео на несколько языков и озвучивать его [разными голосами](https://habr.com/ru/news/905118/). Список поддерживаемых языков см. в [интерфейсе {{ video-name }}]({{ link-video-main }}).

### Субтитры {#subtitles}

Нейросетевые [субтитры](../operations/video/subtitles.md) помимо текста, который произносится в видео, содержат разделение на спикеров. Речь нового спикера будет начинаться с новой строки и тире, что повышает удобство восприятия речи. Список поддерживаемых языков см. в [интерфейсе {{ video-name }}]({{ link-video-main }}).
