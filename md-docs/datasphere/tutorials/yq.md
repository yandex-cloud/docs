[Документация Yandex Cloud](../../index.md) > [Yandex DataSphere](../index.md) > [Практические руководства](index.md) > Аналитика данных > Анализ данных с помощью Query
...
1. Настройте пакет `yandex_query_magic`. Это можно сделать, задав параметры с помощью line команды `yq_settings`:
  ...
  * `--env-auth <переменная_окружения_environment_variable>` — устанавливает режим аутентификации авторизованным ключом, содержимое которого находится в [секрете Yandex DataSphere](../concepts/secrets.md). [Создайте](../operations/data/secrets.md) секрет DataSphere, а имя созданного секрета укажите в параметре `--env-auth`.
