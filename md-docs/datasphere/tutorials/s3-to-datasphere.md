...
Вы создадите [бакет](../../storage/concepts/bucket.md) в [{{ objstorage-name }}](../../storage/index.md), смонтируете его к файловой системе Windows с помощью [rclone](https://rclone.org) и загрузите туда данные для обучения [модели](../concepts/models/index.md) в [{{ ml-platform-full-name }}]({{ link-datasphere-main }}).
...
Чтобы подключиться к бакету из {{ ml-platform-name }}, вам понадобится [коннектор S3](../concepts/s3-connector.md):
