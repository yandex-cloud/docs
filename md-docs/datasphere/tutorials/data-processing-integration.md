...
После этого файл появится в папке `test` бакета. Чтобы просматривать содержимое бакета в интерфейсе {{ jlab }}Lab, создайте и активируйте в проекте [коннектор S3](../operations/data/s3-connectors.md).
...
Подробнее о запуске вычислений на кластерах {{ dataproc-name }} в {{ ml-platform-name }} см. в [концепции](../concepts/data-processing.md#existing-clusters).
...
* [проект {{ ml-platform-name }}](../operations/projects/delete.md);
