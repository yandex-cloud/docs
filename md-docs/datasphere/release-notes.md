...
Для дообучения моделей используйте [инструменты]({{ link-docs-ai }}ai-studio/concepts/tuning/index) сервиса {{ ai-studio-full-name }}.
...
* При работе с [{{ dataproc-full-name }}](../data-proc/index.md) с помощью [коннектора Spark](concepts/spark-connector.md) добавлена возможность [синхронизации окружения](concepts/data-processing-operations.md#synchronization).
...
* У проектов {{ ml-platform-name }} появился новый тип ресурсов — [коннекторы Spark](concepts/spark-connector.md) для интеграции с [{{ dataproc-full-name }}](../data-proc/index.md).
