...
{{ ml-platform-name }} позволяет обрабатывать большие объемы данных на кластерах [{{ dataproc-full-name }}](../../data-proc/index.md). С помощью коннектора Spark вы можете как [использовать уже имеющиеся кластеры {{ dataproc-name }}](data-processing.md#spark-with-existing-cluster), так и [создавать временные кластеры](data-processing.md#spark-with-temporary-cluster).
...
Работа с коннекторами Spark также доступна в [{{ ds-cli }}](jobs/work-with-spark.md).
