...
Работа с заданиями также доступна в [Visual Studio Code](https://code.visualstudio.com/) с помощью расширения [{{ ds-jobs }} Toolkit](../../operations/projects/jobs-toolkit.md).
...
По умолчанию время жизни данных задания составляет 14 дней. По его истечении удалится вся хранимая информация, включая логи и графики мониторинга. Вы можете изменить это значение с помощью {{ ds-cli }}:

```bash
datasphere project job set-data-ttl --id <идентификатор_задания> --days <время_жизни_в_днях>
```

Также вы можете очистить хранимые данные, нажав **{{ ui-key.yc-ui-datasphere.common.clear-cache }}** на вкладке заданий проекта.
...
1. Ресурсы {{ ml-platform-name }}, задействованные в заданиях, определяются в секциях `s3-mounts` ([коннекторы S3](../s3-connector.md)) и `datasets` ([датасеты](../dataset.md)). Чтобы использовать коннектор s3 или датасет в задании, укажите идентификатор доступного в проекте ресурса и (опционально) определите переменную для него. Если переменная не задана, обращение к ресурсу в секции `cmd` возможно по его идентификатору.
   ...
   Для доступа к файлам в бакете {{ objstorage-name }}, подключенном через коннектор S3, используйте путь `/job/s3/<идентификатор_коннектора>/<объект_в_бакете>`.
...
1. Секция `spark` описывает конфигурацию для [работы](work-with-spark.md) с кластерами [{{ dataproc-full-name }}](../../../data-proc/index.md), используя [Spark Connector](../spark-connector.md). При указании коннектора Spark убедитесь, что он [доступен](../../operations/data/spark-connectors.md#share) в проекте.
