...
Атрибут | Описание
--- | ---
`IsDefault` | Указывает, является ли данный канал каналом по умолчанию при отправке SMS в этом облаке. Установить `IsDefault` можно только для канала в состоянии `Sandbox` или `Active`. Из-за особенностей Cloud Notification Service этот атрибут нельзя передавать вместе с другими атрибутами.
