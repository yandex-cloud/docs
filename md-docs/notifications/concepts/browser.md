...
В сервисе действуют ограничения. Подробнее см. в разделе [Квоты и лимиты Yandex Cloud Notification Service](limits.md).

{% endnote %}

Порядок настройки Push-уведомлений:

1. В сервисе CNS создайте канал **Push-уведомления в браузере**.
...
1. Чтобы отправить уведомление, создайте сообщение в CNS и укажите в качестве получателя идентификатор (ARN) эндпоинта.
...
1. Команда на отправку поступает в CNS.
1. CNS отправляет уведомления на серверы, заданные в эндпоинтах.
...
Создать канал уведомлений можно с помощью метода HTTP API [create](../api-ref/create-platform-application.md) для ресурса PlatformApplications сервиса Yandex Cloud Notification Service или аналога метода для CLI или SDK.
...
Создать эндпоинт можно с помощью метода HTTP API [create](../api-ref/create-platform-endpoint.md) для ресурса Endpoint сервиса Yandex Cloud Notification Service или аналога метода для CLI или SDK.
...
Отправить уведомление можно с помощью метода HTTP API [publish](../api-ref/publish.md) для ресурса Endpoint сервиса Yandex Cloud Notification Service или аналога метода для CLI или SDK.
