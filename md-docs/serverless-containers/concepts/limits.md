...
Максимальное количество приемников у триггера ^3^ | 5
Максимальный размер сообщения для триггера^4^ | 230 КБ
...
^3^ Лимит `Максимальное количество приемников у триггера` — общий для сервисов API Gateway, Cloud Functions и Serverless Containers.
^4^ В сообщение может быть добавлена служебная метаинформация — в таком случае максимальный размер сообщения уменьшится.