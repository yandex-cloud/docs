[Документация Yandex Cloud](../../../index.md) > [Yandex Serverless Containers](../../index.md) > [Концепции](../index.md) > Триггер > Обзор

# Триггеры в Serverless Containers. Обзор

_Триггер_ — условие, при выполнении которого автоматически вызывается [контейнер](../container.md) Serverless Containers. Один триггер может одновременно вызывать несколько контейнеров Serverless Containers и функций Cloud Functions, а также отправлять сообщения в WebSocket-соединения одного или нескольких API-шлюзов Yandex API Gateway — типы получателей можно комбинировать.

Триггеры позволяют автоматизировать работу с другими сервисами Yandex Cloud, например: Yandex Object Storage, Yandex Message Queue и Yandex Container Registry.

{% note info %}

Триггер начинает работать в течение 5 минут после создания.

{% endnote %}

В Serverless Containers доступны следующие типы триггеров: 
* [таймер](timer.md);
* [триггер для Message Queue](ymq-trigger.md);
* [триггер для Object Storage](os-trigger.md);
* [триггер для Container Registry](cr-trigger.md);
* [триггер для Cloud Logging](cloud-logging-trigger.md);
* [триггер для Yandex IoT Core](iot-core-trigger.md);
* [триггер для бюджетов](budget-trigger.md);
* [триггер для Data Streams](data-streams-trigger.md);
* [триггер для почты](mail-trigger.md);
* [триггер для Telegram](telegram-trigger.md).

У каждого типа триггера есть индивидуальные условия срабатывания. Например, у триггера для Object Storage это может быть [создание](../../../storage/operations/objects/upload.md) или [изменение](../../../storage/operations/objects/edit-acl.md) объекта. Некоторые условия задаются в настройках объекта сервиса, например у триггера для бюджетов это пороги для уведомлений в настройках [бюджета](../../../billing/concepts/budget.md).

## Особенности вызова контейнера триггером {#invoke}

Триггеры вызывают контейнер с учетом установленных [квот и лимитов](../limits.md).

При вызове контейнера триггером существуют следующие особенности:

* Контейнер необходимо настроить таким образом, чтобы при успешном вызове он возвращал код состояния `2xx`. Другие коды состояния будут интерпретированы как ошибка вызова, что приведет к повторному вызову контейнера.
* Перед передачей сообщений в контейнер триггер изменяет их формат. У каждого типа триггера свой формат сообщений. Подробнее читайте в описании соответствующего триггера.
* Сервисному аккаунту, от имени которого будет вызываться контейнер, необходима роль `serverless-containers.containerInvoker`. Другие роли, необходимые для корректной работы триггера, зависят от его типа. Подробнее читайте в описании соответствующего триггера.
* Если работа триггера была приостановлена пользователем, а затем снова запущена, то все события, произошедшие за время простоя, не будут обработаны.

## Фильтрация событий {#filter-messages}

Для каждого вызываемого ресурса в триггере можно задать фильтр — выражение в [формате jq](https://jqlang.github.io/jq/manual/), которое применяется к событию перед отправкой. Фильтр возвращает булевый ответ. Если результат:
* `true`, событие отправляется в приемник;
* `false` или событие не удалось распарсить, оно не отправляется.

Фильтр задается отдельно для каждого приемника, поэтому один триггер может доставлять разные события разным приемникам. Если фильтр не задан, в приемник отправляются все события.

## Преобразование событий {#transform-messages}

Для каждого приемника в триггере можно задать шаблон преобразования в [формате jq](https://jqlang.github.io/jq/manual/), по которому событие преобразуется перед отправкой. Преобразование позволяет, например:

* привести событие к формату, ожидаемому приемником;
* оставить в событии только нужные поля;
* добавить в событие собственные поля и значения.

Шаблон задается отдельно для каждого приемника, поэтому один триггер может отправлять разным приемникам разные представления одного и того же события. Если шаблон не задан, событие отправляется в приемник в исходном виде.

## Группирование сообщений {#batch-messages}

Вы можете настроить группирование сообщений перед вызовом контейнера у следующих триггеров:

* триггер для Message Queue;
* триггер для Cloud Logging;
* триггер для Object Storage;
* триггер для Container Registry;
* триггер для Yandex IoT Core;
* триггер для Data Streams;
* триггер для почты.

Это позволит вызывать контейнер не для каждого отдельного сообщения, а для целой группы сообщений. Для этого в параметрах триггера задайте:

* **Время ожидания, с** — время, в течение которого происходит группирование сообщений. Число сообщений при этом не превышает указанный размер группы.
* **Размер группы** — количество сообщений для группирования.

Подробнее о группировании сообщений читайте в инструкциях по созданию соответствующего триггера.

## Повторные вызовы контейнера {#invoke-retry}

Вы можете настроить повторный вызов контейнера, если текущий завершился неуспешно. Для этого в параметрах триггера задайте:

* **Интервал** — время, через которое будет сделан повторный вызов контейнера.
* **Количество попыток** — количество повторных вызовов контейнера, которые будут сделаны, прежде чем триггер отправит сообщение в [Dead Letter Queue](../dlq.md).

Настройка доступна для всех типов триггеров, кроме триггера для Message Queue.

Подробнее о повторных запросах читайте в инструкции по созданию соответствующего триггера.

## Полезные ссылки {#see-also_}

* [Триггеры для запуска функции Cloud Functions](../../../functions/concepts/trigger/index.md)
* [Триггеры для отправки сообщений в WebSocket-соединения](../../../api-gateway/concepts/trigger/index.md)