...
* загрузка новой версии Docker-образа в [{{ container-registry-full-name }}](../../container-registry/index.md);
...
Если хотя бы один вызов достигнет таймаута, он и все остальные вызовы, которые обрабатываются тем же экземпляром контейнера, завершатся. Подробнее о таймауте см. [{#T}](limits.md#serverless-containers-limits).
