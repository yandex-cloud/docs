[Документация Yandex Cloud](../../index.md) > [Yandex Serverless Containers](../index.md) > [Практические руководства](index.md) > Бэкенд на Serverless > Разработка CRUD API для сервиса фильмов
...
Взаимодействие с базой данных через [Document API](../../ydb/docapi/api-ref/index.md) реализовано с помощью библиотеки [AWS SDK for JavaScript v3](https://docs.aws.amazon.com/AWSJavaScriptSDK/v3/latest/index.html):
...
При выполнении операций с данными для авторизации используются [IAM-токены](../../iam/concepts/authorization/iam-token.md). Для получения IAM-токена перед выполнением операции вызывается [сервис метаданных](../operations/sa.md).
...
1. Создайте [ревизию](../concepts/container.md) контейнера из Docker-образа с версией `0.0.1`:
