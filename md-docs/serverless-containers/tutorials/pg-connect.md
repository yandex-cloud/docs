...
* Контейнер {{ serverless-containers-name }}: количество вызовов контейнера, время простоя подготовленных экземпляров и выделенные для выполнения приложения вычислительные ресурсы (см. [тарифы {{ container-registry-name }}](../pricing.md)).
...
1. [Создайте контейнер {{ serverless-containers-name }}](../operations/create.md) с именем `demo-pg-connect`.
...
1. [Удалите контейнер {{ serverless-containers-name }}](../operations/delete.md).
