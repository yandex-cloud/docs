...
Фреймворк Functions Framework позволяет создавать функции и запускать их в [контейнерах](../concepts/container.md) Yandex Serverless Containers без использования сервиса [Yandex Cloud Functions](../../functions/index.md). Написанные этим способом функции можно переносить между различными платформами, такими как [Cloud Run](https://cloud.google.com/run), [Cloud Run functions](https://cloud.google.com/functions), [Knative](https://knative.dev/docs/) и [Yandex Serverless Containers](../index.md), а также между этими платформами и вашей локальной машиной разработки.

В данном руководстве рассматривается сценарий, при котором вы локально создадите функцию с помощью фреймворка Functions Framework. Затем из этой функции вы соберете [Docker-образ](../../container-registry/concepts/docker-image.md), который загрузите в [реестр](../../container-registry/concepts/registry.md) Yandex Container Registry. Из сохраненного в реестре Docker-образа вы создадите [контейнер](../concepts/container.md) Serverless Containers, при вызове которого будет выполняться код вашей функции.
...
- Node.js {#node}
  ...
  1. Установите менеджер пакетов [npm](https://nodejs.org/en/learn/getting-started/an-introduction-to-the-npm-package-manager):
      ...
      ```
  1. Создайте директорию для нового проекта:
  ...
  1. Локально запустите вашу функцию, чтобы убедиться, что все работает:
      ...
      1. Запустите функцию:
          ...
          ```
      1. Откройте дополнительное окно терминала и в этом окне выполните команду:
...
Используйте загруженный в Container Registry Docker-образ, чтобы создать [ревизию](../concepts/container.md#revision) контейнера Serverless Containers.
...
- Консоль управления {#console}
  ...
  1. В блоке **Логирование** отключите опцию **Запись логов**, чтобы отключить запись логов в [лог-группу](../../logging/concepts/log-group.md).
      ...
      Вы можете оставить опцию включенной, чтобы [записывать](../operations/logs-write.md) логи в журнал выполнения контейнера. Запись и хранение логов [тарифицируются](../../logging/pricing.md).
...
- CLI {#cli}
  ...
  1. Создайте ревизию созданного ранее контейнера:
      ...
      * `--no-logging` — параметр отключает запись логов в [лог-группу](../../logging/concepts/log-group.md). Уберите этот параметр из команды, чтобы [записывать](../operations/logs-write.md) логи в журнал выполнения контейнера. Запись и хранение логов [тарифицируются](../../logging/pricing.md).
...
- API {#api}
  ...
  Чтобы создать ревизию контейнера, воспользуйтесь методом REST API [deployRevision](../containers/api-ref/Container/deployRevision.md) для ресурса [Container](../containers/api-ref/Container/index.md) или вызовом gRPC API [ContainerService/DeployRevision](../containers/api-ref/grpc/Container/deployRevision.md).
...
1. [Удалите](../operations/delete.md) контейнер Serverless Containers.
