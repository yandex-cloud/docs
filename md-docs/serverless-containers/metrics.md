[Документация Yandex Cloud](../index.md) > [Yandex Serverless Containers](index.md) > Метрики Monitoring

# Справочник метрик Yandex Monitoring

В этом разделе описаны метрики сервиса Serverless Containers, поставляемые в [Monitoring](../monitoring/index.md).

Имя метрики пишется в метку `name`.

## Метрики контейнеров {#containers-metrics}

Общие метки для всех метрик [контейнеров](concepts/container.md):

Метка | Значение
----|----
container | Идентификатор контейнера
revision | Идентификатор [ревизии контейнера](concepts/container.md#revision)
...
* [Посмотреть графики мониторинга триггера в Serverless Containers](operations/trigger-monitoring.md)