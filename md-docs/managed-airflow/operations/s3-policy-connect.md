...
Чтобы проверить подключение к бакету {{ objstorage-name }}, [загрузите DAG-файл в бакет](upload-dags.md#dag-bucket-import). DAG должен отобразиться в веб-интерфейсе {{ AF }}.