...
- CLI {#cli}
    ...
    Чтобы запросить список кластеров Apache Airflow™ в каталоге по умолчанию, выполните команду:
...
- CLI {#cli}
    ...
    Чтобы получить информацию о кластере Apache Airflow™, выполните команду:
...
Все действия с кластерами Managed Service for Apache Airflow™ сохраняются в виде списка операций. Каждой операции присваивается уникальный идентификатор.
...
- CLI {#cli}
    ...
    ```bash
    yc managed-airflow cluster list-operations <имя_или_идентификатор_кластера>
    ```
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [Cluster.ListOperations](../api-ref/Cluster/listOperations.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
        ...
        curl \
            --request GET \
            --header "Authorization: Bearer $IAM_TOKEN" \
            --url 'https://airflow.api.cloud.yandex.net/managed-airflow/v1/clusters/<идентификатор_кластера>/operations'
        ```
