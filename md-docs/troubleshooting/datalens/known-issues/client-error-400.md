...
Сейчас у {{ datalens-short-name }} нет возможности передать этот и подобные параметры, поэтому построить чарт таким образом не получится, но можно [выгрузить данные в {{ CH }}](../../../datalens/qa/index.md#uploading-data-logs-api).