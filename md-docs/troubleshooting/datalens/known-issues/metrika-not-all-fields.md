...
Если среди этих полей нет тех, которые нужны для вашего сценария, то  для полноценной работы с данными рекомендуем делать экспорт данных из [Logs API {{ metrika }}](https://yandex.ru/dev/metrika/doc/api2/logs/intro.html) в [облачный {{ CH }}](../../../managed-clickhouse/index.md) и подключать {{ datalens-name }} уже к этой базе данных.