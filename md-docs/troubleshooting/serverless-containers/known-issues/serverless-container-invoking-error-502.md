...
1. Описание процедуры вызова контейнера: вызывается ли контейнер из кода стороннего приложения через API сервиса, напрямую из интерфейса [Консоли управления](https://console.yandex.cloud) или средствами утилиты YC CLI.
1. Полный текст ошибки из журнала контейнера Serverless Containers.