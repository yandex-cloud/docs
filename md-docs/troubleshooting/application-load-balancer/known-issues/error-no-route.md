[Документация Yandex Cloud](../../../index.md) > [Все решения](../../index.md) > [Application Load Balancer](../index.md) > Устранение ошибки `no_route` при открытии адреса
...
В балансировщике Application Load Balancer настроено несколько обработчиков по SNI. Если открыть в браузере первый адрес, а в новой вкладке второй, то возвращается ошибка `"error_details""no_route"`. Через 30 секунд ошибка пропадает и ресурс становится доступен. 
