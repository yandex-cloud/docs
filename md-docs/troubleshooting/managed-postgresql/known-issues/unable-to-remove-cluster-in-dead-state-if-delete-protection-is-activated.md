...
* Кластер Managed Service for PostgreSQL перешел в состояние `UNKNOWN` сразу же после создания, и его не получается удалить из [Консоли управления](https://console.yandex.cloud).
...
Если вышеописанные действия не помогли решить проблему, [создайте запрос в техническую поддержку](https://center.yandex.cloud/support). При создании запроса укажите идентификатор проблемного кластера и кратко опишите проблему.