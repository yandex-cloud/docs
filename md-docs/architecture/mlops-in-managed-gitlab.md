...
В качестве среды разработки и тестирования используется виртуальная машина DSVM. Масштабируемый управляемый сервис [{{ mgl-full-name }}](../managed-gitlab/index.md) обеспечивает работу инстанса {{ GL }} с предустановленным пакетом [ML Registry]({{ gl.docs }}/user/project/ml/model_registry/). Благодаря этому вы можете пользоваться следующими инструментами MLOps:
...
[Инстанс {{ GL }}](../managed-gitlab/concepts/index.md) развертывается в ВМ под управлением [сервиса {{ mgl-name }}](../managed-gitlab/index.md). Доступ к инстансу осуществляется по его адресу через стандартный веб-интерфейс {{ GL }}.
