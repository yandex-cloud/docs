...
В данном руководстве приведен пример решения задачи по развертыванию веб-приложения в отказоустойчивой конфигурации в инфраструктуре {{ yandex-cloud }}. В основе инфраструктуры, обеспечивающей работу веб-приложения, лежит группа масштабируемых управляемых сервисов экосистемы {{ yandex-cloud }}: [{{ managed-k8s-full-name }}](../managed-kubernetes/index.md), [{{ mpg-full-name }}](../managed-postgresql/index.md) и [{{ alb-full-name }}](../application-load-balancer/index.md).

Кроме указанных основных сервисов, для запуска и проверки веб-приложения используются вспомогательные сервисы: [{{ container-registry-full-name }}](../container-registry/index.md), [{{ certificate-manager-full-name }}](../certificate-manager/index.md), [{{ dns-full-name }}](../dns/index.md), [{{ compute-full-name }}](../compute/index.md), [{{ iam-full-name }}](../iam/index.md), [{{ kms-full-name }}](../kms/index.md) и [{{ vpc-full-name }}](../vpc/index.md).
...
{{ alb-name }} интегрирован с сервисом [{{ certificate-manager-full-name }}](../certificate-manager/index.md), который выполняет автоматическое получение сертификатов [Let's Encrypt](https://letsencrypt.org).
...
* Инфраструктуру можно быстро расширить за счет использования других управляемых сервисов, таких как [{{ mrd-full-name }}](../managed-valkey/index.md), [{{ mkf-full-name }}](../managed-kafka/index.md), [{{ objstorage-full-name }}](../storage/index.md) и др. 
...
1. Подготовьте файлы с описанием инфраструктуры:
    ...
    1. В файле `terraform.tfvars` задайте пользовательские параметры:
        ...
        * `target_host` — имя вашего домена. Домен должен быть [делегирован](../dns/concepts/dns-zone.md#public-zones) сервису [{{ dns-full-name }}](../dns/index.md).
