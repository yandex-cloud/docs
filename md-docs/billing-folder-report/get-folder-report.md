...
## Получить детализацию расходов {#get-details}

Чтобы получать детализацию расходов по каталогам:

1. [Создайте бакет](../storage/operations/buckets/create.md) в сервисе Object Storage. В этом бакете Yandex Cloud будет сохранять файлы с детализацией. Вы можете сохранять детализацию в нескольких бакетах.
...
1. Откройте бакет в консоли управления и скопируйте ссылку на него из адресной строки. Ссылка на бакет имеет формат:
    ...
    ```bash
    https://console.yandex.cloud/folders/<FOLDER-ID/storage/bucket/<BUCKET-NAME>
    ```
1. Отправьте на почту вашему менеджеру или [службе поддержки](https://center.yandex.cloud/support) заявку на включение детализации расходов по каталогам и укажите:
...
* `resource_id` — идентификатор ресурса, имя ресурса или идентификатор подписки. Значение поля может быть пустым, если использование сервиса относится ко всему каталогу или не содержит ресурсов. Соответствие сервисов и значения столбца `resource_id`:
   ...
   * Monitoring, DataLens, Yandex IoT Core, SpeechKit, Translate и Vision OCR — пустое значение.
...
* `label.user_labels.<имя метки>` — метки, проставленные ресурсам. Как управлять метками описано в разделе [Управление метками](label-management.md).
...
* [Документация сервиса Yandex Cloud Billing в Yandex Cloud](../billing/index.md)
