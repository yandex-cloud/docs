[Документация Yandex Cloud](../../../index.md) > [Yandex API Gateway](../../index.md) > [Концепции](../index.md) > [Расширения спецификации](index.md) > Ограничение скорости обработки запросов

# Ограничение скорости обработки запросов

{% note warning %}

Расширения `x-yc-apigateway-rate-limits` и `x-yc-apigateway-rate-limit` устарели и больше не поддерживаются. Чтобы ограничить скорость обработки запросов, используйте [интеграцию с Yandex Smart Web Security](sws.md).

{% endnote %}

{% note warning %}

Расширения предназначены для равномерного распределения нагрузки на приложение при легитимном трафике. При DoS-атаках корректная работа не гарантируется. Для защиты приложений используйте [профиль безопасности Smart Web Security](sws.md).
