...
Подробнее о подключении собственного домена к API-шлюзу см. раздел [Подключить домен](../operations/api-gw-domains.md).

Подробнее о добавлении сертификатов см. раздел [Добавить сертификат от Let's Encrypt®](../../certificate-manager/operations/managed/cert-create.md).
