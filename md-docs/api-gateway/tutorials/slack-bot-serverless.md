...
* плата за использование API-шлюза (см. тарифы [Yandex API Gateway](../pricing.md));
...
1. [Создайте](../operations/api-gw-create.md) API-шлюз:
...
Чтобы бот начал отвечать на сообщения пользователей, созданные функции необходимо связать с приложением. Для этого [отредактируйте](../operations/api-gw-update.md) спецификацию API-шлюза `for-slack-bot` и добавьте в параметр `paths` конфигурации метода `POST`:
