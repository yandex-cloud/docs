...
* [API-шлюз](../concepts/index.md) {{ api-gw-name }} для приема и перенаправления запросов от пользователей в функции {{ sf-name }}.
...
* плата за запросы к созданным API-шлюзам и исходящий трафик (см. [тарифы {{ api-gw-full-name }}](../pricing.md));
...
1. [Удалите](../operations/api-gw-delete.md) API-шлюз {{ api-gw-name }}.
