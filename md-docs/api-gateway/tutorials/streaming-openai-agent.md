[Документация Yandex Cloud](../../index.md) > [Yandex API Gateway](../index.md) > [Практические руководства](index.md) > Искусственный интеллект > Создание AI-агента со стримингом ответа через веб-сокеты
...
В этом руководстве вы создадите агент со стримингом ответа через [веб-сокеты](https://ru.wikipedia.org/wiki/WebSocket) на [Yandex Cloud Functions](../../functions/index.md) и [Yandex API Gateway](../index.md). Функция будет использовать [OpenAI Agents SDK](https://openai.github.io/openai-agents-python/) для работы с моделями [Yandex AI Studio](https://aistudio.yandex.ru/docs/ru/ai-studio/concepts/generation/index#yandex).
...
1. Пользователь устанавливает веб-сокетное соединение с [API-шлюзом](../concepts/index.md) и отправляет по нему запрос к AI-агенту.
...
1. AI-агент передает данные от модели по мере их поступления. Данные сразу перенаправляются в открытое пользователем веб-сокетное соединение с помощью [API Gateway WebSocket Connection Service](../apigateway/websocket/api-ref/Connection/send.md). Завершается выполнение функции.
...
Если вы не хотите привязывать AI-агент к вендору, разверните функцию в Yandex Serverless Containers, как описано в руководстве [Разработка функций в Functions Framework и их развертывание в Yandex Serverless Containers](../../tutorials/serverless/functions-framework-to-container.md).
