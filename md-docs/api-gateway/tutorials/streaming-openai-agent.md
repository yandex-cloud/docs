...
В этом руководстве вы создадите агент со стримингом ответа через [веб-сокеты](https://{{ lang }}.wikipedia.org/wiki/WebSocket) на [{{ sf-full-name }}](../../functions/index.md) и [{{ api-gw-full-name }}](../index.md). Функция будет использовать [OpenAI Agents SDK](https://openai.github.io/openai-agents-python/) для работы с моделями [{{ ai-studio-full-name }}]({{ link-docs-ai }}ai-studio/concepts/generation/index#yandex).
...
1. AI-агент передает данные от модели по мере их поступления. Данные сразу перенаправляются в открытое пользователем веб-сокетное соединение с помощью [{{ api-gw-name }} WebSocket Connection Service](../apigateway/websocket/api-ref/Connection/send.md). Завершается выполнение функции.
