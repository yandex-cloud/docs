...
- Консоль управления {#console}
    ...
    * **Настройки шардирования** — задайте настройки для [шардирования](../../../../managed-clickhouse/concepts/sharding.md):
      ...
      * **Равномерное случайное шардирование** — данные по шардам будут распределяться случайным образом (количество данных на каждом шарде будет примерно одинаковым).
        ...
        Для трансферов из ClickHouse® в ClickHouse® шардирование не поддерживается. Возможное обходное решение: создайте на кластере-приемнике распределенную таблицу и выполните трансфер в нее, выбрав политику очистки `Truncate` или `Не очищать`.
      
        Настройки шардирования, включая **Равномерное случайное шардирование** и **Шардирование по значению колонки**, игнорируются при репликации из PostgreSQL. Это связано с архитектурными ограничениями: для обеспечения целостности данных при операциях `UPDATE` и `DELETE` репликационный поток направляется строго в один шард.
