...
Подсеть может быть указана в настройках эндпоинта вручную (для эндпоинтов типа **Пользовательская инсталляция**), либо выбрана автоматически в случае [эндпоинтов MDB](#managed-cluster-subnets). Такая подсеть называется __выбранной подсетью__. Сеть, которой принадлежит выбранная подсеть, называется __выбранной сетью__.

Если хосты в настройках эндпоинта указаны в виде доменных имен, то для их разрешения в IP-адреса будут использоваться [DNS-серверы](../../glossary/dns.md#dns-server), указанные в настройках DHCP выбранной подсети. Все DNS-серверы подсети должны разрешать доменное имя хоста в IP-адрес, иначе трансфер может не запуститься, т. к. для разрешения имен службы трансфера используют произвольный DNS-сервер подсети. Подробнее читайте в разделе [IP-адреса и доменные имена в настройках эндпоинтов](#ip-addresses-and-domain-names).

Подсети, выбранные для обоих эндпоинтов одного трансфера, должны принадлежать одной и той же [зоне доступности](../../overview/concepts/geo-scope.md).

## Подсети кластеров MDB {#managed-cluster-subnets}

Подсеть можно указать только для эндпоинтов типа **Пользовательская инсталляция**. Если в настройках эндпоинта указан не хост, а ID кластера MDB, для доступа к эндпоинту будет автоматически выбрана одна из подсетей, к которым подключен кластер базы данных.
...
* выбрать для одного из эндпоинтов тип **Пользовательская инсталляция** и указать для него любую подсеть с зоной доступности, соответствующей одной из зон другого эндпоинта. Если подходящей подсети нет, создайте новую подсеть в нужной зоне и укажите ее в настройках эндпоинта типа **Пользовательская инсталляция**.
...
Если в настройках эндпоинта хост указан в виде IP-адреса, выбранная для эндпоинта подсеть будет использована для доступа к кластеру, даже если указанный IP-адрес [не принадлежит](#ip-address-availability) выбранной для эндпоинта сети.

Если используется эндпоинт типа **Пользовательская инсталляция** с хостом, указанным в виде доменного имени, либо используется эндпоинт MDB, то для разрешения имени хоста в IP-адрес будет использован DNS-сервер, указанный в настройках DHCP выбранной подсети, либо DNS-сервер по умолчанию (второй адрес в диапазоне подсети). Для успешной работы трансфера адрес, в который разрешается доменное имя хоста, должен принадлежать выбранной для эндпоинта сети, а адрес DNS-сервера должен быть [доступен](#ip-address-availability) через выбранную подсеть.

Для подключения к пользовательским инсталляциям Data Transfer может использовать только те FQDN, которые являются А-записями, CNAME-записи не поддерживаются. Это значит, что к пользовательским инсталляциям нельзя подключиться через особые FQDN, например `…rw.mdb.yandexcloud.net` или `…ro.mdb.yandexcloud.net`. Поэтому при настройке эндпоинтов типа **Пользовательская инсталляция** указывайте только прямые FQDN, соответствующие A-записям.
...
* с помощью [Yandex Cloud Interconnect](../../interconnect/index.md);
* с помощью [VPN](../../glossary/vpn.md).

Если вам нужно перенести данные между вашим облаком Yandex Cloud и другим облаком (в том числе другим облаком Yandex Cloud) или между вашим облаком Yandex Cloud и кластером пользовательской инсталляции на вашей площадке, разрешите подключения к базе данных в стороннем облаке или на вашей площадке из интернета с [IP-адресов, используемых сервисом Data Transfer](../../overview/concepts/public-ips.md#virtual-private-cloud).

Если вы используете для подключения [сервис Cloud Interconnect](../../interconnect/concepts/index.md) или [VPN](../../glossary/vpn.md), отдельные настройки стороннего облака не требуются. Чтобы обеспечить сетевую связность, укажите в настройках эндпоинта подсеть, из которой можно подключиться к внешнему ресурсу.

Для запуска трансферов, для работы которых необходим доступ в интернет, нужно обладать ролью [`data-transfer.admin`](../security/index.md). Для создания эндпоинтов, в настройках которых указана подсеть, надо назначить пользователю роль [`vpc.user`](../../vpc/security/index.md) на каталог, в котором находится подсеть.