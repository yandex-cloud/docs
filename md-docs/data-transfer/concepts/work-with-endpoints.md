...
* **Для таблиц, партиционированных методом наследования**:
    ...
    * Если после того, как трансфер перешел на этап репликации, на источнике будут созданы новые таблицы-наследники, необходимо будет перенести их на приемник вручную.
    ...
    При переносе базы данных из PostgreSQL в другую СУБД пользователь может включить в эндпоинте-источнике настройку [Объединить наследуемые таблицы](../operations/endpoint/source/postgresql.md#additional-settings). Поведение трансфера зависит от значения настройки и таблиц, указанных в параметре **Список включённых таблиц**:
