...
- Terraform {#tf}
  ...
  1. Добавьте описание ресурса:
      ...
      resource "yandex_mdb_sharded_postgresql_shard" "<имя_шарда>" {
        ...
        name       = "<имя_шарда>"
        shard_spec = {
          mdb_postgresql = "<идентификатор_кластера_PostgreSQL>"
        }
      }
      ```
      ...
      * `shard_spec.mdb_postgresql` — идентификатор кластера Managed Service for PostgreSQL в составе шарда.

      Подробнее в [документации провайдера Terraform](../../terraform/resources/mdb_sharded_postgresql_shard.md).
