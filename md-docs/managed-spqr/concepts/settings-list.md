...
* **Show Notice Messages**
    ...
    Подробнее о настройке `show_notice_messages` читайте в [документации Sharded PostgreSQL](https://docs.pg-sharding.tech/configuration/router#debug-settings).
...
* **Time Quantiles**
    ...
    Подробнее о настройке `time_quantiles` читайте в [документации Sharded PostgreSQL](https://docs.pg-sharding.tech/configuration/router#statistics-settings).
...
* **Default Route Behavior**
    ...
    * `allow` — разрешает выполнение запросов на нескольких шардах (полезно для DDL и административных запросов).
    ...
    Подробнее о настройке `query_routing.default_route_behaviour` читайте в [документации Sharded PostgreSQL](https://docs.pg-sharding.tech/configuration/router#query-routing-settings).
...
* **Enhanced Multishard Processing**
    ...
    Подробнее о настройке `query_routing.enhanced_multishard_processing` читайте в [документации Sharded PostgreSQL](https://docs.pg-sharding.tech/configuration/router#query-routing-settings).
...
* **Default Target Session Attrs**
    ...
    * `smart read write` — подключение к хосту-мастеру. Если он недоступен, подключение не выполняется. Если в файле конфигурации для настройки `query_routing.auto_route_ro_on_standby` установлено значение `true`, то запросы только на чтение данных автоматически перенаправляются к хостам-репликам (с семантикой `prefer standby`).
    ...
    Подробное описание настроек `query_routing.default_target_session_attrs` и `query_routing.auto_route_ro_on_standby` вы найдете в [документации Sharded PostgreSQL](https://docs.pg-sharding.tech/configuration/router#query-routing-settings).
...
* **Default Commit Strategy**
    ...
    * `two pc` — двухэтапная фиксация с гарантиями атомарности между шардами.
    ...
    Подробнее о настройке `default_commit_strategy` читайте в [документации Sharded PostgreSQL](https://docs.pg-sharding.tech/configuration/router#transaction-settings).