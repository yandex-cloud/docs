...
* _Действия_, которые нужно выполнить при переходе индекса в определенное состояние. Действия выполняются в том порядке, в котором они указаны.
    ...
    В Managed Service for OpenSearch в дополнение к [стандартным действиям OpenSearch](https://opensearch.org/docs/latest/im-plugin/ism/policies/#ism-supported-operations) возможно также [изменение кодека индекса](indexing.md#codec-change).

* _Переходы_ — условия, при которых индекс будет переведен в другое состояние. Переходы проверяются в том порядке, в котором они указаны.
...
Подробнее о политиках см. [в документации OpenSearch](https://opensearch.org/docs/latest/im-plugin/ism/policies/).
