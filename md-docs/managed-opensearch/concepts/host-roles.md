...
* Группа хостов `OpenSearch` может содержать хосты с ролями [DATA](#data), [MANAGER](#manager), [WARM](#warm) или [INGEST](#ingest).
  
  В кластере должны обязательно присутствовать хосты с ролями `DATA` и `MANAGER`. Если в кластере есть только одна группа `OpenSearch`, ее хосты совмещают обе эти роли.
...
Функциональность Warm Storage находится на [стадии Preview](../../overview/concepts/launch-stages.md). Чтобы её активировать, обратитесь в [техническую поддержку](https://center.yandex.cloud/support).

Подробнее о снапшотах с возможностью поиска в [документации OpenSearch](https://opensearch.org/docs/latest/tuning-your-cluster/availability-and-recovery/snapshots/searchable_snapshot/).

## Хосты с ролью INGEST {#ingest}

Хосты с этой ролью предварительно обрабатывают данные перед их сохранением в индекс. Отдельные хосты с ролью `INGEST` могут быть полезны, если вы используете ресурсоемкие конвейеры приема данных перед загрузкой в OpenSearch. Если в кластере нет хостов с ролью `INGEST`, эту роль выполняют хосты с ролью `DATA`.

Подробнее о конвейерах приема данных в [документации OpenSearch](https://opensearch.org/docs/latest/ingest-pipelines/).
