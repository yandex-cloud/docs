[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for OpenSearch](../index.md) > [Пошаговые инструкции](index.md) > Кластеры > Техническое обслуживание

# Техническое обслуживание кластера OpenSearch

Вы можете управлять [техническим обслуживанием](../concepts/maintenance.md) кластера Yandex Managed Service for OpenSearch.

## Получить список обслуживаний {#list-maintenance}

Для сервиса Managed Service for OpenSearch можно получить список обслуживаний в [облаке](#list-cloud-maintenance), [каталоге](#list-folder-maintenance) или [кластере](#list-cluster-maintenance).
...
- gRPC API {#grpc-api}
    ...
    1. Воспользуйтесь вызовом [MaintenanceService.List](../api-ref/grpc/Maintenance/list.md) и выполните запрос, например с помощью [gRPCurl](https://github.com/fullstorydev/grpcurl):
        ...
        grpcurl \
          ...
          -proto ~/cloudapi/yandex/cloud/mdb/opensearch/v1/maintenance_service.proto \
          -rpc-header "Authorization: Bearer $IAM_TOKEN" \
          -d '{
                "cloud_id": "<идентификатор_облака>"
              }' \
          mdb.api.cloud.yandex.net:443 \
          yandex.cloud.mdb.opensearch.v1.MaintenanceService.List
        ```
...
- gRPC API {#grpc-api}
    ...
    1. Воспользуйтесь вызовом [MaintenanceService.Get](../api-ref/grpc/Maintenance/get.md) и выполните запрос, например с помощью [gRPCurl](https://github.com/fullstorydev/grpcurl):
        ...
        grpcurl \
          ...
          mdb.api.cloud.yandex.net:443 \
          yandex.cloud.mdb.opensearch.v1.MaintenanceService.Get
        ```
...
- CLI {#cli}
    ...
    1. Посмотрите описание команды CLI для переноса обслуживания:
        ...
        ```bash
        yc managed-opensearch cluster reschedule-maintenance --help
        ```
    ...
    1. Чтобы перенести обслуживание на ближайшее окно, выполните команду, указав тип переноса `next-available-window`. Чтобы перенести на определенную дату и время, укажите тип переноса `specific-time`:
        ...
        ```bash
        yc managed-opensearch cluster reschedule-maintenance <имя_или_идентификатор_кластера> \
...
- REST API {#api}
    ...
    1. Перенесите обслуживание одним из двух способов:
        ...
        * Воспользуйтесь методом [Cluster.RescheduleMaintenance](../api-ref/Cluster/rescheduleMaintenance.md) и выполните запрос, например с помощью [cURL](https://curl.se/):
            ...
            --header "Authorization: Bearer $IAM_TOKEN" \
            --header "Content-Type: application/json" \
            --url 'https://mdb.api.cloud.yandex.net/managed-opensearch/v1/clusters/<идентификатор_кластера>:rescheduleMaintenance' \
            --data '{
        ...
        * Воспользуйтесь методом [Maintenance.Reschedule](../api-ref/Maintenance/reschedule.md) и выполните запрос, например с помощью [cURL](https://curl.se/):
            ...
            --header "Authorization: Bearer $IAM_TOKEN" \
            --header "Content-Type: application/json" \
            --url 'https://mdb.api.cloud.yandex.net/managed-opensearch/v1/maintenances/<идентификатор_обслуживания>:reschedule' \
            --data '{
                        "rescheduleType": <тип_переноса>,
    ...
    1. Перенесите обслуживание одним из двух способов:
        ...
        * Воспользуйтесь вызовом [MaintenanceService.Reschedule](../api-ref/grpc/Maintenance/reschedule.md) и выполните запрос, например с помощью [gRPCurl](https://github.com/fullstorydev/grpcurl):
            ...
            grpcurl \
              ...
              mdb.api.cloud.yandex.net:443 \
              yandex.cloud.mdb.opensearch.v1.MaintenanceService.Reschedule
            ```
...
- CLI {#cli}
    ...
    1. Посмотрите описание команды CLI для изменения окна обслуживания:
        ...
        ```bash
        yc managed-opensearch cluster update --help
        ```
