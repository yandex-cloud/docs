...
В этом разделе описаны метрики сервиса {{ mos-name }}, поставляемые в [{{ monitoring-name }}]({{ monitoring-doc-links }}). Чтобы посмотреть детальную информацию о состоянии кластера и его хостов, воспользуйтесь [инструкцией](operations/monitoring.md).
