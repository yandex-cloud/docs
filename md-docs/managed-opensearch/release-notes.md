...
* При создании кластера появились шаблоны конфигурации: **Начальная**, **Стандартная**, **Гибридная**, **AI/RAG**, **Тестовая**.
* В Terraform добавлена возможность задать размер хранилища в гигабайтах через параметр `disk_size_gb`.
* В консоли управления появилась более подробная и прозрачная информация о техническом обслуживании кластера: длительности операции обслуживания, влияние на пользователя, расчетное время начала операции, что делать пользователю, если задача на техническое обслуживание завершилась неуспешно.
* Добавлена метрика `opensearch_breakers_parent_tripped`, которая сигнализирует о троттлинге.
* В публичный API добавлен [метод листинга доступных версий СУБД](api-ref/Versions/list.md).
...
* Доступны новые версии OpenSearch: [3.6.0 и 3.7.0](https://docs.opensearch.org/latest/version-history/).
