# Подключение MCP-клиента к кластеру {{ OS }} в сервисе {{ mos-short-name }}
...
1. Подготовьте инфраструктуру:
    ...
    - Вручную {#manual}
        ...
        1. [Создайте кластер {{ mos-name }}](../operations/cluster-create.md#create-cluster) нужной вам конфигурации с публичным доступом к любой группе хостов.
...
1. Проверьте подключение к кластеру с помощью утилиты [cURL](https://curl.haxx.se/):
    ...
    FQDN хоста можно получить со [списком хостов в кластере](../operations/host-groups.md#list-hosts).
...
1. Создайте отдельного пользователя для MCP-клиента.
    ...
    - OpenSearch Dashboards {#dashboards}
        ...
        1. [Подключитесь](../operations/connect/clients.md#dashboards) к {{ OS }} Dashboards от имени `admin`.
...
Если клиент не позволяет отдельно указать путь к CA-сертификату, заранее установите [SSL-сертификат](../operations/connect/index.md#ssl-certificate) в системное хранилище сертификатов.
