...
Вы можете использовать [{{ keycloak }}](https://www.keycloak.org/) для [аутентификации пользователей](../operations/saml-authentication.md), которые работают с {{ OS }} Dashboards в кластере {{ mos-full-name }}.
...
1. Убедитесь, что вы можете [подключиться к {{ OS }} Dashboards](../operations/connect/clients.md#dashboards) с использованием реквизитов пользователя `admin`.
...
1. Сопоставьте роли кластера {{ OS }} с ролями на стороне {{ keycloak }}. Это позволит получить доступ к кластеру через SSO.
    ...
    1. [Подключитесь](../operations/connect/clients.md#dashboards) к {{ OS }} Dashboards от имени пользователя `admin`.
...
1. [Подключитесь](../operations/connect/clients.md#dashboards) к {{ OS }} Dashboards.
