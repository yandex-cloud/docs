...
Вы можете использовать [Keycloak](https://www.keycloak.org/) для [аутентификации пользователей](../operations/saml-authentication.md), которые работают с OpenSearch Dashboards в кластере Managed Service for OpenSearch.
...
1. Убедитесь, что вы можете [подключиться к OpenSearch Dashboards](../operations/connect/clients.md#dashboards) с использованием реквизитов пользователя `admin`.
...
1. Настройте SSO для кластера.
    ...
    Далее приведены инструкции для консоли управления, но можно [использовать и другие доступные интерфейсы Yandex Cloud](../operations/saml-authentication.md#configuration-sso).
...
1. Сопоставьте роли кластера OpenSearch с ролями на стороне Keycloak. Это позволит получить доступ к кластеру через SSO.
    ...
    1. [Подключитесь](../operations/connect/clients.md#dashboards) к OpenSearch Dashboards от имени пользователя `admin`.
...
1. [Подключитесь](../operations/connect/clients.md#dashboards) к OpenSearch Dashboards.
