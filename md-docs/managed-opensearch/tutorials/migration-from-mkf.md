[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for OpenSearch](../index.md) > [Практические руководства](index.md) > Миграция и копирование данных > Поставка данных из Yandex Managed Service for Apache Kafka®
...
1. Подготовьте инфраструктуру поставки данных:
    ...
    - Вручную {#manual}
        ...
        1. [Создайте кластер-приемник Managed Service for OpenSearch](../operations/cluster-create.md#create-cluster) любой подходящей конфигурации со следующими настройками:
        ...
        1. Для подключения к кластерам с локальной машины пользователя, настройте группы безопасности:
            ...
            * [Managed Service for OpenSearch](../operations/connect/index.md#security-groups).
...
Вы можете поставлять данные в кластер Managed Service for OpenSearch от имени пользователя `admin`, имеющего роль `superuser`, но безопаснее для каждой задачи создавать отдельных пользователей с ограниченными привилегиями. Подробнее в разделе [Управление пользователями OpenSearch](../operations/cluster-users.md).
