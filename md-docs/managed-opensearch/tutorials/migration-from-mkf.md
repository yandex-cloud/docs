...
* Кластер Managed Service for Apache Kafka®: использование выделенных хостам вычислительных ресурсов и объем хранилища (см. [тарифы Managed Service for Apache Kafka®](../../managed-kafka/pricing.md)).
...
Вы можете поставлять данные в кластер Managed Service for OpenSearch от имени пользователя `admin`, имеющего роль `superuser`, но безопаснее для каждой задачи создавать отдельных пользователей с ограниченными привилегиями. Подробнее см. в разделе [Управление пользователями OpenSearch](../operations/cluster-users.md).
