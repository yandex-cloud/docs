...
```
POST https://mdb.api.cloud.yandex.net/managed-opensearch/v1/clusters/{clusterId}:rescheduleMaintenance
```
...
## Body parameters {#yandex.cloud.mdb.opensearch.v1.RescheduleMaintenanceRequest}
...
The value can also point to a moment in the past if [rescheduleType.IMMEDIATE](rescheduleMaintenance.md#yandex.cloud.mdb.opensearch.v1.RescheduleMaintenanceRequest.RescheduleType) reschedule type is selected.
...
- `HEALTH_UNKNOWN`: Health of the cluster is unknown ([Host.health](listHosts.md#yandex.cloud.mdb.opensearch.v1.Host) for every host in the cluster is UNKNOWN).
- `ALIVE`: Cluster is working normally ([Host.health](listHosts.md#yandex.cloud.mdb.opensearch.v1.Host) for every host in the cluster is ALIVE).
- `DEAD`: Cluster is inoperable ([Host.health](listHosts.md#yandex.cloud.mdb.opensearch.v1.Host) for every host in the cluster is DEAD).
- `DEGRADED`: Cluster is working below capacity ([Host.health](listHosts.md#yandex.cloud.mdb.opensearch.v1.Host) for at least one host in the cluster is not ALIVE). ||
|| status | **enum** (Status)
