...
Ошибка вызвана тем, что сегменты логов ваших топиков занимают больше места, чем есть в хранилище брокеров. Подробнее о расчете необходимого места смотрите в разделе [Минимальный размер хранилища](../concepts/storage.md#minimal-storage-size).
...
Подробнее о том, какие виды API доступны в кластере и какие для них есть ограничения, смотрите в разделе [API в Managed Service for Apache Kafka®](../concepts/available-apis.md).

Пример использования скриптов смотрите в разделе [Подключение к кластеру Apache Kafka® из приложений](../operations/connect/clients.md).
...
* Создайте в Yandex Monitoring график [метрики](../metrics.md) `kafka_group_topic_partition_lag`.
* Используйте инструмент командной строки `kafka-consumer-groups.sh`, который поставляется вместе с дистрибутивом Apache Kafka®. Предварительно [настройте подключение](../operations/connect/clients.md#with-ssl) к кластеру.
...
* Используйте встроенный веб-интерфейс Kafka UI, который позволяет отслеживать состояние групп потребителей. Подробнее о его возможностях в разделе [Веб-интерфейс Kafka UI для Apache Kafka®](../concepts/kafka-ui.md).
...
Подробнее в разделе [Отслеживание потери сообщений в топике Apache Kafka®](../tutorials/retention-policy.md).
