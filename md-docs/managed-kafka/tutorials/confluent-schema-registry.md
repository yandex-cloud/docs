...
В {{ mkf-name }} вы можете использовать интегрированный реестр схем формата данных [{{ mkf-msr }}](../concepts/managed-schema-registry.md#msr). Подробнее см. в разделе [{#T}](managed-schema-registry.md). Если вам необходим реестр [Confluent Schema Registry](https://docs.confluent.io/platform/current/schema-registry/index.html), используйте информацию из этого руководства.
...
1. [Создайте служебный топик](../operations/cluster-topics.md#create-topic) с именем `_schemas` со следующими настройками:
...
1. [Создайте пользователя](../operations/cluster-accounts.md#create-account) с именем `registry` и [выдайте ему права](../operations/cluster-accounts.md#grant-permission) на топик `_schemas`:
    * `ACCESS_ROLE_CONSUMER`,
    * `ACCESS_ROLE_PRODUCER`.
...
1. Измените файл `/etc/schema-registry/schema-registry.properties`, отвечающий за настройки Confluent Schema Registry:
    ...
    1. Добавьте в конец файла строки:
        ...
        Список хостов-брокеров можно получить [со списком хостов кластера](../operations/cluster-hosts.md).
