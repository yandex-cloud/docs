...
* Плата за кластеры {{ mkf-name }}: использование вычислительных ресурсов, выделенных хостам (в том числе хостам {{ ZK }}), и дискового пространства (см. [тарифы {{ KF }}](../pricing.md)).
...
- Вручную {#manual}
    ...
    1. [Создайте необходимое количество кластеров {{ mkf-name }}](../operations/cluster-create.md) любой подходящей вам [конфигурации](../concepts/instance-types.md). Для подключения к кластерам с локальной машины пользователя, а не из облачной сети {{ yandex-cloud }}, включите публичный доступ к кластерам при их создании.
    ...
    1. [Создайте необходимое количество топиков](../operations/cluster-topics.md#create-topic) в кластерах {{ mkf-name }}. Имена топиков не должны повторяться.

    1. Чтобы [производитель и потребитель](../concepts/producers-consumers.md) могли работать с топиками, [создайте](../operations/cluster-accounts.md#create-account) в каждом кластере {{ mkf-name }} по два пользователя:
...
Данные отправляются от имени пользователей [с ролью `ACCESS_ROLE_PRODUCER`](#before-you-begin). Подробнее о настройке SSL-сертификата и работе с `kafkacat` см. в разделе [{#T}](../operations/connect/clients.md).
...
- Вручную {#manual}
    ...
    - Удалите кластеры:
        ...
        - [{{ mkf-full-name }}](../operations/cluster-delete.md).
