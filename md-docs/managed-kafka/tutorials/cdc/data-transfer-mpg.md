[Документация Yandex Cloud](../../../index.md) > [Yandex Managed Service for Apache Kafka®](../../index.md) > [Практические руководства](../index.md) > Поставка данных с помощью Data Transfer > Поставка из PostgreSQL в Apache Kafka®
...
- Terraform {#tf}
  ...
  1. Скачайте в ту же рабочую директорию файл конфигурации [cdc-mpg-mkf.tf](https://github.com/yandex-cloud-examples/yc-data-transfer-postgresql-to-kafka/blob/main/cdc-mpg-mkf.tf).
      ...
      * топик и пользователь с ролями [ACCESS_ROLE_CONSUMER](../../concepts/account-roles.md#access-role-consumer) и [ACCESS_ROLE_PRODUCER](../../concepts/account-roles.md#access-role-producer) на этот топик в кластере Managed Service for Apache Kafka®;
...
Настройка кластера-приемника различается в зависимости от используемого [способа управления топиками](../../concepts/topics.md#management). При этом имена топиков для данных конструируются по тому же принципу, что и в [Debezium](https://debezium.io/documentation/reference/connectors/postgresql.html#postgresql-topic-names) — `<префикс_топика>.<имя_схемы>.<имя_таблицы>`. В этом руководстве в качестве примера будет использоваться префикс `cdc`.
...
- Интерфейсы Yandex Cloud {#yc}
    ...
    1. [Создайте топик](../../operations/cluster-topics.md#create-topic) с именем `cdc.public.measurements`.

        Если необходимо отслеживать изменения в нескольких таблицах, создайте для каждой из них отдельный топик.

    1. [Создайте пользователя](../../operations/cluster-accounts.md#create-account) с именем `kafka-user` и ролями `ACCESS_ROLE_CONSUMER` и `ACCESS_ROLE_PRODUCER`, действующими на созданный топик.
