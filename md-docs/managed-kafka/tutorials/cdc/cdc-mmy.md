[Документация Yandex Cloud](../../../index.md) > [Yandex Managed Service for Apache Kafka®](../../index.md) > [Практические руководства](../index.md) > Поставка данных с помощью Data Transfer > Поставка из MySQL® в Apache Kafka®
...
1. [Создайте кластер-приемник Managed Service for Apache Kafka®](../../operations/cluster-create.md) любой подходящей конфигурации с хостами в публичном доступе.


1. Если вы используете группы безопасности, настройте их так, чтобы к кластерам можно было подключаться из интернета:

    * [Инструкция для Managed Service for MySQL®](../../../managed-mysql/operations/connect/index.md#configuring-security-groups).
    * [Инструкция для Managed Service for Apache Kafka®](../../operations/connect/index.md#configuring-security-groups).
...
Настройки различаются в зависимости от используемого [способа управления топиками](../../concepts/topics.md#management). При этом имена топиков для данных формируются по следующему принципу — `<префикс_топика>.<имя_схемы>.<имя_таблицы>`. В этом руководстве в качестве примера будет использоваться префикс `cdc`.
...
- Интерфейсы Yandex Cloud {#yc}
    ...
    1. [Создайте топик](../../operations/cluster-topics.md#create-topic) с именем `cdc.db1.measurements`.

        Для отслеживания изменений в нескольких таблицах создайте для каждой из них отдельный топик.

    1. [Создайте пользователя](../../operations/cluster-accounts.md#create-account) с именем `kafka-user` и ролями `ACCESS_ROLE_CONSUMER` и `ACCESS_ROLE_PRODUCER`, действующими для созданных топиков. Чтобы включить все такие топики, укажите в имени топика `cdc.*`.

- Admin API {#api}

    Если для управления топиками используется Kafka Admin API:

    1. Создайте [пользователя-администратора](../../operations/cluster-accounts.md) с именем `kafka-user`.
...
1. В отдельном терминале запустите утилиту `kafkacat` в режиме потребителя:
    ...
    FQDN хостов-брокеров можно получить со [списком хостов в кластере Managed Service for Apache Kafka®](../../operations/cluster-hosts.md).
