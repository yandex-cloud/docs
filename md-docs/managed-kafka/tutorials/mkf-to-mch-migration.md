[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for Apache Kafka®](../index.md) > [Практические руководства](index.md) > Поставка данных с помощью Data Transfer > Поставка из Apache Kafka® в ClickHouse®
...
- Вручную {#manual}
    ...
    1. [Создайте кластер-источник Managed Service for Apache Kafka®](../operations/cluster-create.md) любой подходящей [конфигурации](../concepts/instance-types.md). Для подключения к кластеру с локальной машины пользователя, а не из облачной сети Yandex Cloud, включите публичный доступ к кластеру при его создании.

    1. [Создайте топик](../operations/cluster-topics.md#create-topic) в кластере Managed Service for Apache Kafka®.

    1. Чтобы [производитель и потребитель](../concepts/producers-consumers.md) могли работать с топиком в кластере Managed Service for Apache Kafka®, [создайте пользователей](../operations/cluster-accounts.md#create-account):
...
- Terraform {#tf}
    ...
    1. Укажите в файле `data-transfer-mkf-mch.tf`:
        ...
        * Параметр `transfer_enabled = 0` — отключает создание эндпоинтов и трансфера. Они будут созданы при [подготовке трансфера](#prepare-transfer).
...
1. Установите утилиты:
    ...
    * [kafkacat](https://github.com/edenhill/kcat) — для чтения и записи данных в топик Apache Kafka®.
        ...
        Убедитесь, что можете с ее помощью [подключиться к кластерам Managed Service for Apache Kafka® через SSL](../operations/connect/clients.md#bash-zsh).
...
- Terraform {#tf}
  ...
  1. Укажите в файле `data-transfer-mkf-mch.tf` значение параметра `transfer_enabled = 1`.
  1. Проверьте корректность файлов конфигурации Terraform с помощью команды:
