[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for Apache Kafka®](../index.md) > [Практические руководства](index.md) > Перемещение данных из Apache Kafka® > Миграция базы данных из стороннего кластера Apache Kafka®
...
Оба способа подойдут в том числе для [миграции кластера](../operations/host-migration.md#one-host) Managed Service for Apache Kafka® с одним хостом в другую [зону доступности](../../overview/concepts/geo-scope.md).
...
- Вручную {#manual}
    ...
    1. [Создайте кластер-приемник](../operations/cluster-create.md).
    1. Подготовьте кластер-приемник:
        1. Создайте [пользователя](../operations/cluster-accounts.md#create-account) с именем `admin-cloud`.
        1. [Создайте топик](../operations/cluster-topics.md#create-topic) с произвольными настройками. Он понадобится только для настройки доступа пользователя к топикам.
        1. [Назначьте пользователю роль](../operations/cluster-accounts.md#grant-permission) `ACCESS_ROLE_ADMIN` на все топики (`*`).
        1. Включите настройку [Auto create topics enable](../concepts/settings-list.md#settings-auto-create-topics).
        1. Настройте [группы безопасности](../operations/connect/index.md#configuring-security-groups) для подключения к кластеру-приемнику.

    1. [Создайте для кластера-приемника коннектор](../operations/cluster-connector.md#create-connector) с типом `MirrorMaker` и настройками:
...
- Terraform {#tf}
    ...
    1. Скачайте в ту же рабочую директорию файл конфигурации [kafka-mirrormaker-connector.tf](https://github.com/yandex-cloud-examples/yc-kafka-data-migration-from-on-premise/blob/main/kafka-mirrormaker-connector.tf).
        ...
        * кластер-приемник Managed Service for Apache Kafka® с включенной настройкой [Auto create topics enable](../concepts/settings-list.md#settings-auto-create-topics);
        * [пользователь-администратор](../operations/cluster-accounts.md#create-account) `admin-cloud` для кластера-приемника;
...
1. [Подключитесь](../operations/connect/clients.md#bash-zsh) к топику кластера-приемника с помощью утилиты kafkacat. К имени топика кластера-источника добавьте префикс.
...
- Вручную {#manual}
    ...
    [Удалите кластер Yandex Managed Service for Apache Kafka®](../operations/cluster-delete.md). Коннектор удалится вместе с кластером.
...
- Вручную {#manual}
  ...
  1. [Создайте кластер-приемник Managed Service for Apache Kafka®](../operations/cluster-create.md).
  1. Подготовьте кластер-приемник:
      * [Создайте пользователя](../operations/cluster-accounts.md#create-account) с именем `admin-cloud`.
      * [Создайте топик](../operations/cluster-topics.md#create-topic) с произвольными настройками. Он понадобится только для настройки доступа пользователя к топикам.
      * [Назначьте пользователю роль](../operations/cluster-accounts.md#grant-permission) `ACCESS_ROLE_ADMIN` на все топики (`*`).
      * Включите настройку [Auto create topics enable](../concepts/settings-list.md#settings-auto-create-topics).
      * Настройте [группы безопасности](../operations/connect/index.md#configuring-security-groups) для подключения к кластеру-приемнику.
...
- Terraform {#tf}
  ...
  1. Скачайте в ту же рабочую директорию файл конфигурации [kafka-mirror-maker.tf](https://github.com/yandex-cloud-examples/yc-kafka-data-migration-from-on-premise/blob/main/kafka-mirror-maker.tf).
      ...
      * кластер Managed Service for Apache Kafka® с включенной настройкой [Auto create topics enable](../concepts/settings-list.md#settings-auto-create-topics);
      * [пользователь-администратор](../operations/cluster-accounts.md#create-account) Apache Kafka® `admin-cloud`, которому назначена роль `ACCESS_ROLE_ADMIN` на все топики кластера;
...
1. [Подключитесь](../../compute/operations/vm-connect/ssh.md) к виртуальной машине по [SSH](../../glossary/ssh-keygen.md).
    ...
    1. Скачайте [SSL-сертификат](../operations/connect/index.md#get-ssl-cert) для подключения к кластеру Managed Service for Apache Kafka®:
...
1. Настройте межсетевой экран (firewall) и [группы безопасности](../operations/connect/index.md#configuring-security-groups), если это требуется для подключения MirrorMaker к кластеру-приемнику и кластеру-источнику.
...
1. Создайте в каталоге `mirror-maker` файл конфигурации MirrorMaker `mm2.properties`:
   ...
   * Параметр `<T>` — количество одновременно работающих процессов MirrorMaker. Рекомендуется указывать не менее `2` для равномерного распределения нагрузки репликации. Подробнее [в документации Apache Kafka®](https://kafka.apache.org/42/operations/geo-replication-cross-cluster-data-mirroring/#configuration-file-syntax).
   ...
   FQDN брокеров Managed Service for Apache Kafka® можно запросить со [списком хостов в кластере](../operations/cluster-hosts.md).
...
- Вручную {#manual}
    ...
    * [Удалите кластер Yandex Managed Service for Apache Kafka®](../operations/cluster-delete.md).
