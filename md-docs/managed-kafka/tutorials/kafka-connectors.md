...
Оба способа подойдут в том числе для [миграции кластера](../operations/host-migration.md#one-host) {{ mkf-name }} с одним хостом в другую зону доступности.
...
- {{ TF }} {#tf}
    ...
    1. Скачайте в ту же рабочую директорию файл конфигурации [kafka-mirrormaker-connector.tf](https://github.com/yandex-cloud-examples/yc-kafka-data-migration-from-on-premise/blob/main/kafka-mirrormaker-connector.tf).
        ...
        * кластер-приемник {{ mkf-name }} с включенной настройкой [Auto create topics enable](../concepts/settings-list.md#settings-auto-create-topics);
...
- Вручную {#manual}
    ...
    [Удалите кластер {{ mkf-full-name }}](../operations/cluster-delete.md). Коннектор удалится вместе с кластером.
...
- Вручную {#manual}
  ...
  1. [Создайте кластер-приемник {{ mkf-name }}](../operations/cluster-create.md).
  1. Подготовьте кластер-приемник:
...
- {{ TF }} {#tf}
  ...
  1. Скачайте в ту же рабочую директорию файл конфигурации [kafka-mirror-maker.tf](https://github.com/yandex-cloud-examples/yc-kafka-data-migration-from-on-premise/blob/main/kafka-mirror-maker.tf).
      ...
      * кластер {{ mkf-name }} с включенной настройкой [Auto create topics enable](../concepts/settings-list.md#settings-auto-create-topics);
      * [пользователь-администратор](../operations/cluster-accounts.md#create-account) {{ KF }} `admin-cloud`, которому назначена роль `ACCESS_ROLE_ADMIN` на все топики кластера;
...
1. [Подключитесь](../../compute/operations/vm-connect/ssh.md) к виртуальной машине по [SSH](../../glossary/ssh-keygen.md).
    ...
    1. Скачайте [SSL-сертификат](../operations/connect/index.md#get-ssl-cert) для подключения к кластеру {{ mkf-name }}:
...
1. Создайте в каталоге `mirror-maker` файл конфигурации MirrorMaker `mm2.properties`:
   ...
   * Параметр `<T>` — количество одновременно работающих процессов MirrorMaker. Рекомендуется указывать не менее `2` для равномерного распределения нагрузки репликации. Подробнее [в документации {{ KF }}](https://kafka.apache.org/42/operations/geo-replication-cross-cluster-data-mirroring/#configuration-file-syntax).
   ...
   FQDN брокеров {{ mkf-name }} можно запросить со [списком хостов в кластере](../operations/cluster-hosts.md).
...
- Вручную {#manual}
    ...
    * [Удалите кластер {{ mkf-full-name }}](../operations/cluster-delete.md).
