...
* [Управляют топиками](cluster-topics.md#admin-api). Подробнее в разделе [Топики и разделы](../concepts/topics.md).

После [создания кластера](cluster-create.md) Apache Kafka® вы можете:
...
* [импортировать пользователя в Terraform](#import-account);
...
- REST API {#api}
  ...
  1. Воспользуйтесь методом [User.list](../api-ref/User/list.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
     ...
     curl \
       ...
       --url 'https://mdb.api.cloud.yandex.net/managed-kafka/v1/clusters/<идентификатор_кластера>/users'
     ```
...
Для создания пользователя-администратора воспользуйтесь интерфейсами командной строки, [API](../../glossary/rest-api.md) или Terraform.
...
- Консоль управления {#console}
  ...
  1. Нажмите значок ![image](../../_assets/console-icons/ellipsis.svg) для нужного пользователя и выберите пункт:
     ...
     * **Изменить пароль**, чтобы [указать другой пароль](#update-password) для пользователя.
     * **Настроить**, чтобы [выдать](#grant-permission) или [отозвать](#revoke-permission) права доступа к топикам.
...
- Terraform {#tf}
  ...
  1. Найдите в этом файле ресурс `yandex_mdb_kafka_user` для нужного пользователя и внесите правки.
     ...
     С помощью Terraform можно [изменить пароль](#update-password) пользователя, а также [выдать](#grant-permission) или [отозвать](#revoke-permission) права доступа к топикам.
...
- REST API {#api}
  ...
  1. Воспользуйтесь методом [User.update](../api-ref/User/update.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
     ...
     curl \
       ...
       --header "Authorization: Bearer $IAM_TOKEN" \
       --header "Content-Type: application/json" \
       --url 'https://mdb.api.cloud.yandex.net/managed-kafka/v1/clusters/<идентификатор_кластера>/users/<имя_пользователя>' \
       --data '{
...
- Консоль управления {#console}
  ...
  1. Нажмите кнопку ![image](../../_assets/console-icons/plus.svg) **Добавить топик**. Если такой кнопки нет — значит, для этого пользователя добавлены права на все существующие топики кластера.
  ...
  1. Нажмите на значок ![image](../../_assets/console-icons/plus.svg) в столбце **Роли** для нужного топика и выберите роль:
     ...
     * `ACCESS_ROLE_TOPIC_ADMIN`: роль с максимальными правами на управление топиками через [Admin API Apache Kafka®](https://kafka.apache.org/42/apis/#admin-api).
     ...
     Пользователю также предоставляется доступ к субъектам схем данных. То, какие субъекты доступны, зависит от указанных ролей и топиков. Подробнее читайте в разделе [Субъекты в Managed Schema Registry](../concepts/managed-schema-registry.md#subjects).
...
- CLI {#cli}
  ...
  1. Выдайте права на доступ к нужным топикам, передав параметры `--permission`:
     ...
     * `allow_host` — (опционально) IP-адрес, с которого разрешен доступ этому пользователю. Чтобы задать несколько адресов, добавьте нужное количество опций `allow_host` через запятую.
     ...
     Вместе с доступом к топику пользователю также предоставляется доступ к субъектам схем данных. То, какие субъекты доступны, зависит от указанных ролей и топиков. Подробнее читайте в разделе [Субъекты в Managed Schema Registry](../concepts/managed-schema-registry.md#subjects).
...
- Terraform {#tf}
  ...
  1. Добавьте блок `permission`:
     ...
     * `allow_hosts` — список IP-адресов, с которых пользователю разрешен доступ к топику.
     ...
     Вместе с доступом к топику пользователю также предоставляется доступ к субъектам схем данных. Список доступных субъектов зависит от указанных ролей и топиков. Подробнее в разделе [Субъекты в Managed Schema Registry](../concepts/managed-schema-registry.md#subjects).
...
- REST API {#api}
  ...
  1. Воспользуйтесь методом [User.grantPermission](../api-ref/User/grantPermission.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
     ...
     curl \
       ...
       --header "Authorization: Bearer $IAM_TOKEN" \
       --header "Content-Type: application/json" \
       --url 'https://mdb.api.cloud.yandex.net/managed-kafka/v1/clusters/<идентификатор_кластера>/users/<имя_пользователя>:grantPermission' \
       --data '{
                 "permission": [
                   {
                    "topicName": "<имя_топика>",
...
- gRPC API {#grpc-api}
  ...
  1. Клонируйте репозиторий [cloudapi](https://github.com/yandex-cloud/cloudapi):
     ...
     Далее предполагается, что содержимое репозитория находится в директории `~/cloudapi/`.
  1. Воспользуйтесь вызовом [UserService/GrantPermission](../api-ref/grpc/User/grantPermission.md) и выполните запрос, например, с помощью [gRPCurl](https://github.com/fullstorydev/grpcurl):
...
- REST API {#api}
  ...
  1. Воспользуйтесь методом [User.revokePermission](../api-ref/User/revokePermission.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
     ...
     curl \
       ...
       --header "Authorization: Bearer $IAM_TOKEN" \
       --header "Content-Type: application/json" \
       --url 'https://mdb.api.cloud.yandex.net/managed-kafka/v1/clusters/<идентификатор_кластера>/users/<имя_пользователя>:revokePermission' \
       --data '{
                 "permission": [
                   {
                    "topicName": "<имя_топика>",
...
- gRPC API {#grpc-api}
  ...
  1. Клонируйте репозиторий [cloudapi](https://github.com/yandex-cloud/cloudapi):
     ...
     Далее предполагается, что содержимое репозитория находится в директории `~/cloudapi/`.
  1. Воспользуйтесь вызовом [UserService/RevokePermission](../api-ref/grpc/User/revokePermission.md) и выполните запрос, например, с помощью [gRPCurl](https://github.com/fullstorydev/grpcurl):
...
- Terraform {#tf}
    ...
    1. Выполните команду для импорта пользователя:
        ...
        Подробнее об импорте пользователей в [документации провайдера Terraform](../../terraform/resources/mdb_kafka_user.md#import).
