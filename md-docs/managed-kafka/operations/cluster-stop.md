[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for Apache Kafka®](../index.md) > [Пошаговые инструкции](index.md) > Кластеры > Остановка и запуск кластера

# Остановка и запуск кластера Apache Kafka®

При необходимости вы можете остановить кластер Apache Kafka® и запустить его заново. При остановке кластера все данные в нем сохранятся: они снова станут доступны, когда вы запустите кластер.
...
- CLI {#cli}
  ...
  ```bash
  yc managed-kafka cluster stop <имя_или_идентификатор_кластера>
  ```
...
- REST API {#api}
  ...
  1. Воспользуйтесь методом [Cluster.stop](../api-ref/Cluster/stop.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
     ...
     curl \
       ...
       --url 'https://mdb.api.cloud.yandex.net/managed-kafka/v1/clusters/<идентификатор_кластера>:stop'
     ```
...
- CLI {#cli}
  ...
  ```bash
  yc managed-kafka cluster start <имя_или_идентификатор_кластера>
  ```
...
- REST API {#api}
  ...
  1. Воспользуйтесь методом [Cluster.start](../api-ref/Cluster/start.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
     ...
     curl \
       --request POST \
       --header "Authorization: Bearer $IAM_TOKEN" \
       --url 'https://mdb.api.cloud.yandex.net/managed-kafka/v1/clusters/<идентификатор_кластера>:start'
     ```
