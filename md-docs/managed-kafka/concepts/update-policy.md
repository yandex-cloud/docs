[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for Apache Kafka®](../index.md) > [Концепции](index.md) > Политика работы с версиями Apache Kafka®

# Политика работы с версиями Apache Kafka®

{% note warning %}

Политика работы с версиями Apache Kafka® действует начиная с версии 3.4.

{% endnote %}

Apache Kafka® выпускает новые версии три раза в год согласно [официальной документации](https://cwiki.apache.org/confluence/display/KAFKA/Time+Based+Release+Plan#TimeBasedReleasePlan-WhatIsOurEOLPolicy?).

У каждой версии в инфраструктуре Yandex Cloud есть [этапы жизненного цикла](#version-lifecycle), которые присваиваются в зависимости от даты релиза в Open Source. При устаревании используемая в кластере Managed Service for Apache Kafka® версия Apache Kafka® обновляется.
...
| Доступность в Managed Service for Apache Kafka® | На этой версии можно создавать новые кластеры и работать с ними. | 0 месяцев |
...
Срок поддержки каждой версии — минимум 18 месяцев с даты добавления в Managed Service for Apache Kafka®.

| Версия / Статус | Доступность в Managed Service for Apache Kafka® | Недоступность для новых кластеров | Поддержка прекращена | Принудительное обновление |
|:---|:---|:---|:---|:---|
...
**Версия 3.9**: с данной версии доступна миграция кластеров с ZooKeeper на KRaft.

**Версия 4.0+**: полностью прекращена поддержка ZooKeeper (доступен только Apache Kafka® Raft).

{% endnote %}

## Политика обновления {#update-policy}

Кластер Managed Service for Apache Kafka® на неподдерживаемой версии будет обновлен до ближайшей поддерживаемой версии.
