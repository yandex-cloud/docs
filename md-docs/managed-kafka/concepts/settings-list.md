...
* **Minimum number of in-sync replicas** <code><b><small>Все интерфейсы</small></b></code> {#settings-topic-min-insync-replicas}
    ...
    * для кластеров с двумя и более хостами-брокерами — `2`. Такое значение используется для обеспечения высокой доступности кластера. Подробнее в разделе [Кластер с тремя и более хостами](ha-cluster.md#three-or-more-hosts).
