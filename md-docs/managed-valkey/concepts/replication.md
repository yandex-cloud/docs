...
Yandex Managed Service for Valkey™ использует стандартную репликацию Valkey™ и реализует высокую доступность данных в кластере с помощью агента управления состоянием хоста [rdsync](https://github.com/yandex/rdsync).
...
Подробнее о том, как организована репликация в Valkey™, читайте в [документации СУБД](https://valkey.io/topics/replication/).

## Отказоустойчивость {#availability}

Для обеспечения отказоустойчивости в архитектуру Yandex Managed Service for Valkey™ интегрирован агент управления состоянием хоста [rdsync](https://github.com/yandex/rdsync), разработанный Яндексом.
...
Отключайте персистентность только если для работы вашего приложения не важна сохранность данных, например при использовании Yandex Managed Service for Valkey™ в качестве кеша. В таком случае последние записанные в Valkey™ данные будут храниться только в оперативной памяти и могут быть утеряны при аварийном завершении работы сервера.
...
* **no-appendfsync-on-rewrite yes**{#setting-no-appendfsync}
  ...
  Когда выполняется `BGREWRITEAOF`, работает `fsync()`. Valkey™ записывает самую короткую последовательность команд, необходимую для восстановления текущего набора данных в памяти. Размер данных регулируется настройкой [aof-rewrite-incremental-fsync](#setting-rewrite-incremental).
...
Подробнее о механизмах обеспечения персистентности Valkey™ читайте в [документации СУБД](https://valkey.io/topics/persistence) и в описании конфигурационного файла [valkey.conf](https://github.com/valkey-io/valkey/blob/unstable/valkey.conf).
...
С персистентностью, включенной только на репликах, действуют следующие настройки Valkey™:
