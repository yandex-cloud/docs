[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for Valkey™](../index.md) > [Концепции](index.md) > Шардирование

# Шардирование в Yandex Managed Service for Valkey™

[Шардирование](../../glossary/sharding.md) в Yandex Managed Service for Valkey™ реализовано с помощью [Valkey™ Cluster](https://valkey.io/topics/cluster-tutorial).

## Преимущества шардирования {#advantages}

Шардирование предлагает распределить нагрузку по хостам базы данных — это позволяет преодолеть ограничения ресурсов одного сервера, что особенно актуально при больших объемах данных или необходимости в интенсивных вычислениях.

Горизонтальное масштабирование включает в себя распределение набора данных и нагрузки по нескольким узлам. Емкость одной машины или ее скорость могут быть невысокими, но в горизонтально масштабируемом кластере каждая машина обрабатывает лишь часть общей нагрузки и хранит лишь часть общих данных. Это делает систему потенциально эффективнее, чем единственный сервер с большой емкостью.

Более подробно шардирование баз Valkey™ рассмотрено в [документации Valkey™](https://valkey.io/topics/cluster-spec/).

## Структура Valkey™ Cluster {#valkey-cluster-structure}

Valkey™ Cluster позволяет создать инсталляцию Valkey™ с автоматическим шардированием данных между хостами. Valkey™ Cluster включает в себя набор хостов, на которых хранятся данные. Valkey™ Cluster делится на шарды, каждый из которых состоит из мастера и набора реплик. На хосты-мастеры записываются данные от клиентов, которые затем [реплицируются](replication.md).

В каждом кластере находятся 16348 *хеш-слота*, которые равномерно распределяются между шардами. Слоты определяют набор данных, который хранится в том или ином шарде.

## Масштабирование {#scaling}

При необходимости горизонтального масштабирования кластера, в него можно [добавлять новые шарды](../operations/shards.md#add).

Новый шард создается без хеш-слотов. Чтобы перераспределить данные, необходимо [выполнить ребалансировку кластера](../operations/shards.md#rebalance-cluster). После этого на новый шард переместится часть уже существующих слотов.

Перемещение слотов между шардами не требует остановки работы кластера. Если клиент отправил запрос к мастеру на данные, которые были перемещены на другой шард, этот запрос будет переадресован на новый шард, куда переместились данные. Сами хосты не проксируют запросы, а только перенаправляют клиента на правильный шард.

{% note info %}

Также вы можете настроить в кластере [автомасштабирование шардов](autoscaling.md) и задать пороги потребления вычислительных ресурсов. Например, при нагрузке на ресурсы выше заданного порога в кластер автоматически будет добавлен новый шард и запустится ребалансировка кластера.

{% endnote %}

Yandex Managed Service for Valkey™ может содержать от 1 до 10 шардов, каждый из которых, в свою очередь, может включать в себя разное количество хостов. Минимальное количество хостов в шарде зависит от [выбранного типа диска](storage.md#storage-type-selection).

Допустимо [создать кластер](../operations/cluster-create.md) либо с одним шардом, либо с тремя и более шардами. Кластер с двумя шардами нельзя создать, но можно [изменить количество шардов](../operations/shards.md) в уже существующем кластере.

Информация об ограничениях на количество хостов в шарде приведена в разделе [Квоты и лимиты](limits.md).

## Примеры использования {#examples}

* [Использование кластера Yandex Managed Service for Valkey™ в качестве хранилища сессий PHP](../tutorials/valkey-as-php-sessions-storage.md)
* [Миграция базы данных из стороннего кластера Valkey™](../tutorials/data-migration.md)