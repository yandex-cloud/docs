...
* **Allow data loss**{#settings-allow-data-loss} <code><b><small>Все интерфейсы</small></b></code>
    ...
    Если флаг включен, при выходе из строя текущего мастера кластер может переключиться на реплику, которая не содержит последних записей. Это может привести к потере данных. Если большинство хостов находится в одной [зоне доступности](../../overview/concepts/geo-scope.md), то Valkey™ продолжит работать, несмотря на отсутствие последних записей.
