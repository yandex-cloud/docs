# Общие вопросы про Yandex Managed Service for Valkey™


* [Что такое Yandex Managed Service for Valkey™?](#what-is)

* [Какую часть работы по управлению и сопровождению баз данных берет на себя Yandex Managed Service for Valkey™?](#services)

* [Для каких задач стоит использовать Yandex Managed Service for Valkey™, а для каких — виртуальные машины с базами данных?](#mdb-advantage)

* [Что такое хост базы данных и кластер базы данных?](#what-is-cluster)

* [Как начать работу с Yandex Managed Service for Valkey™?](#quickstart)
...
* [Какую версию Valkey™ использует Yandex Managed Service for Valkey™?](#dbms-version)
...
Yandex Managed Service for Valkey™ — это сервис, который помогает вам создавать, эксплуатировать и масштабировать базы данных Valkey™ в облачной инфраструктуре.

С Yandex Managed Service for Valkey™ вы можете:
...
Yandex Managed Service for Valkey™ берет на себя трудоемкие задачи администрирования инфраструктуры Valkey™:
...
Вы взаимодействуете с кластером БД в Yandex Managed Service for Valkey™ как с обычной базой данных в вашей локальной инфраструктуре. Благодаря этому вы можете управлять внутренними настройками БД в соответствии с требованиями вашего приложения.


#### Какую часть работы по управлению и сопровождению баз данных берет на себя Yandex Managed Service for Valkey™? {#services}

При создании кластеров Yandex Managed Service for Valkey™ выделяет ресурсы, устанавливает СУБД и создает базы данных.

Для созданных и запущенных баз данных Yandex Managed Service for Valkey™ автоматически создает резервные копии, а также устанавливает исправления и обновления СУБД.

Также Yandex Managed Service for Valkey™ обеспечивает репликацию данных между хостами БД (как внутри, так и между [зонами доступности](../../overview/concepts/geo-scope.md)) и автоматически переключает нагрузку на резервную реплику в случае аварии.
...
- Yandex Managed Service for Valkey™ позволяет вам эксплуатировать шаблонные базы данных, не заботясь об администрировании.
...
Yandex Managed Service for Valkey™ доступен всем зарегистрированным пользователям Yandex Cloud.

Чтобы создать кластер базы данных в Yandex Managed Service for Valkey™, необходимо определиться с его характеристиками:
...
Вы можете подключаться к базам данных Yandex Managed Service for Valkey™ способами, стандартными для СУБД, с одним ограничением: хосты Yandex Managed Service for Valkey™ доступны только с виртуальных машин Yandex Cloud, подключенных к той же сети.
...
Под обслуживанием в Yandex Managed Service for Valkey™ понимается:
...
- другие сервисные работы Yandex Managed Service for Valkey™.

Подробнее в разделе [Техническое обслуживание](../concepts/maintenance.md).

#### Какую версию Valkey™ использует Yandex Managed Service for Valkey™? {#dbms-version}

Yandex Managed Service for Valkey™ поддерживает Valkey™ версии 7.2, 8.0, 8.1 или 9.0.
...
Через месяц после того, как версия СУБД становится неподдерживаемой, Yandex Managed Service for Valkey™ автоматически оповещает владельцев кластеров БД, созданных с этой версией, по электронной почте.
...
В Yandex Managed Service for Valkey™ стоимость потребления рассчитывается исходя из следующих параметров:

- Выбранный класс хостов.
- Размер хранилища, зарезервированного для хоста БД.
- Объем резервных копий кластера БД. Объем резервных копий, равный объему хранилища, не тарифицируется. Хранение резервных копий сверх этого объема оплачивается по [тарифам](../../managed-valkey/pricing.md).
- Количество часов работы хоста БД. Неполные часы округляются до целого значения. Стоимость часа работы для каждого класса хостов приведена в разделе [Правила тарификации](../../managed-valkey/pricing.md).
...
Такая ошибка возникает, когда хост добавляется в кластер Yandex Managed Service for Valkey™ с [классом хостов](../concepts/instance-types.md#available-flavors) b2.medium или b3-c1-m4. В этом случае [измените класс хостов](../operations/update.md#change-resource-preset) на другой. После этого кластер будет недоступен около пяти — семи минут. Затем добавьте хост в кластер.

Кластеры с классами хостов b2.medium или b3-c1-m4 могут содержать только один хост на кластер или [шард](../concepts/sharding.md). Если хотите сохранить класс хостов, после смены класса можно добавить хост с нужной конфигурацией и удалить прежний хост. Затем верните первоначальный класс хостов.

#### Включено ли резервное копирование хостов БД по умолчанию? {#default-backup}

Да, по умолчанию резервное копирование включено. Для Valkey™ выполняется полное резервное копирование один раз в сутки, и сохраняются все журналы транзакций кластера БД. Это позволяет восстановить состояние кластера на любой момент времени в пределах периода хранения резервных копий, за исключением последних 30 секунд.
...
Для всех хостов БД можно отслеживать метрики, специфические для типа соответствующей СУБД. Например для Valkey™ можно отслеживать:
...
[Создайте алерт](../operations/monitoring.md#monitoring-integration) с метрикой `disk.used_bytes` в сервисе Yandex Monitoring. Метрика показывает размер использованного дискового пространства в кластере Yandex Managed Service for Valkey™.
...
Рассмотрите возможность перехода на более быстрый тип дисков путем [восстановления кластера](../operations/cluster-backups.md#restore) из резервной копии.
