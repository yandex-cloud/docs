...
- Консоль управления {#console}
  ...
  1. В [консоли управления](https://console.yandex.cloud) перейдите в каталог, список кластеров которого нужно получить.
...
- CLI {#cli}
  ...
  Чтобы запросить список кластеров Valkey™ в каталоге по умолчанию, выполните команду:
...
- Консоль управления {#console}
  ...
  1. В [консоли управления](https://console.yandex.cloud) перейдите в каталог, информацию о кластере которого нужно получить.
  1. Перейдите в сервис **Yandex Managed Service for&nbsp;Valkey™**.
...
- CLI {#cli}
  ...
  Чтобы получить информацию о кластере Valkey™, выполните команду:
...
Все действия с кластерами Yandex Managed Service for Valkey™ сохраняются в виде списка операций. Каждой операции присваивается уникальный идентификатор.
...
- Консоль управления {#console}
  ...
  Чтобы посмотреть операции со всеми кластерами Yandex Managed Service for Valkey™, на панели слева выберите ![image](../../_assets/console-icons/list-check.svg) **Операции**. В открывшемся списке также отображаются операции для ресурсов, которые были удалены.
...
- CLI {#cli}
  ...
  Чтобы получить список операций для кластера Yandex Managed Service for Valkey™, воспользуйтесь командой:
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [Cluster.ListOperations](../api-ref/Cluster/listOperations.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
        ...
        curl \
            --request GET \
            --header "Authorization: Bearer $IAM_TOKEN" \
            --url 'https://mdb.api.cloud.yandex.net/managed-redis/v1/clusters/<идентификатор_кластера>/operations'
        ```
