...
- Консоль управления {#console}
  ...
  * **Connected Clients** — количество открытых подключений к сервисам на хостах кластера.
      ...
      Если при подключении к кластеру возникают ошибки, причиной могут быть неактивные приложения, которые слишком долго держат подключения открытыми. В этом случае [измените в настройках {{ VLK }}](update.md#change-valkey-config) значение параметра [Timeout](../concepts/settings-list.md#settings-timeout).
