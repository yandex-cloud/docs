[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for Valkey™](../index.md) > [Пошаговые инструкции](index.md) > Логи и мониторинг > Просмотр логов кластера
...
- Консоль управления {#console}
    ...
    1. Выберите тип отображаемых событий: **Логи Valkey** или **Авторизация и подключения** (события аудита).

        Чтобы включить запись логов авторизации и подключений, включите опцию **Логировать события аудита** в [настройках кластера](update.md#change-additional-settings).
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [Cluster.ListLogs](../api-ref/Cluster/listLogs.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
        ...
        * `serviceType` — тип сервиса, для которого нужно запросить логи. Возможное значение — `REDIS`.
    ...
    1. Убедитесь, что запрос был выполнен успешно, изучив [ответ сервера](../api-ref/Cluster/listLogs.md#yandex.cloud.mdb.redis.v1.ListClusterLogsResponse).
...
- gRPC API {#grpc-api}
    ...
    1. Воспользуйтесь вызовом [ClusterService.ListLogs](../api-ref/grpc/Cluster/listLogs.md) и выполните запрос, например, с помощью [gRPCurl](https://github.com/fullstorydev/grpcurl):
        ...
        * `service_type` — тип сервиса, для которого нужно запросить логи. Возможное значение — `REDIS`.
    ...
    1. Убедитесь, что запрос был выполнен успешно, изучив [ответ сервера](../api-ref/grpc/Cluster/listLogs.md#yandex.cloud.mdb.redis.v1.ListClusterLogsResponse).
...
- REST API {#api}
    ...
    1. Убедитесь, что запрос был выполнен успешно, изучив [ответ сервера](../api-ref/Cluster/streamLogs.md#yandex.cloud.mdb.redis.v1.StreamLogRecord).
...
- gRPC API {#grpc-api}
    ...
    1. Убедитесь, что запрос был выполнен успешно, изучив [ответ сервера](../api-ref/grpc/Cluster/streamLogs.md#yandex.cloud.mdb.redis.v1.StreamLogRecord).
