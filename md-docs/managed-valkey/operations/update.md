[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for Valkey™](../index.md) > [Пошаговые инструкции](index.md) > Кластеры > Изменение настроек кластера
...
Если соответствующая настройка отключена (по умолчанию), то Valkey™ использует IP-адреса в качестве адресов хостов. Если эта настройка включена, то IP-адрес хоста будет подменяться на его FQDN. Информация об этой настройке и сферах ее применения приведена в разделе [Использование FQDN вместо IP-адресов](../concepts/network.md#fqdn-ip-setting).
...
Подробнее о персистентности данных и ее настройках читайте в разделе [Персистентность](../concepts/replication.md#persistence).
...
* Хост класера с хранилищем на локальных SSD-дисках может быть недоступен длительное время, если потребуется миграция данных на другой физический сервер.
...
- CLI {#cli}
  ...
  1. Запросите список доступных классов хостов (в колонке `ZONE IDS` указаны [зоны доступности](../../overview/concepts/geo-scope.md), в которых можно выбрать соответствующий класс):
...
Подробнее о хранилище и автоматическом увеличении читайте в разделе [Хранилище в Yandex Managed Service for Valkey™](../concepts/storage.md).
...
- Консоль управления {#console}
  ...
  1. Настройте доступные параметры в соответствии с [документацией Valkey™](https://valkey.io/topics/valkey.conf/).
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [Cluster.Update](../api-ref/Cluster/update.md) и выполните запрос, например с помощью [cURL](https://curl.se/):
        ...
        * `configSpec.redisConfig_<версия_Valkey™>` — набор настроек Valkey™. Укажите каждую настройку на отдельной строке через запятую.
            ...
            Список версий Valkey™, доступных для параметра, приведен в [описании метода](../api-ref/Cluster/update.md#yandex.cloud.mdb.redis.v1.UpdateClusterRequest). Описание и возможные значения настроек читайте в разделе [Настройки Valkey™](../concepts/settings-list.md).
...
- Консоль управления {#console}
  ...
  1. Измените дополнительные настройки кластера:
     ...
     - **Логировать события аудита** — опция включает запись логов о подключении и авторизации. Логирование событий аудита может влиять на производительность кластера.
     
         Эта настройка не влияет на запись стандартных логов Valkey™.
