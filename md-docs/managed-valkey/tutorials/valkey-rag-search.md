[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for Valkey™](../index.md) > [Практические руководства](index.md) > Использование Yandex Managed Service for Valkey™ с модулем Search и моделей Yandex AI Studio для векторного поиска в RAG-системе
...
* Кластер Yandex Managed Service for Valkey™: использование выделенных хостам вычислительных ресурсов, объем хранилища и резервных копий ([тарифы Yandex Managed Service for Valkey™](../pricing.md)).
...
- Вручную {#manual}
  ...
  1. [Создайте кластер Yandex Managed Service for Valkey™](../operations/cluster-create.md) любой подходящей конфигурации со следующими настройками:
...
1. Подключитесь к базе данных кластера Yandex Managed Service for Valkey™:
    ...
    - С помощью утилиты valkey-cli {#with-valkey-cli}
      ...
      1. Подключитесь к кластеру:
          ...
          * Для подключения к шардированному кластеру выполните команду:
            ...
            О том, как получить FQDN хоста, читайте в разделе [FQDN хостов Valkey™](../operations/connect/fqdn.md).
    ...
    - С помощью Yandex WebSQL {#with-websql}
      ...
      Перед подключением [включите](../operations/update.md#change-additional-settings) доступ к сервису [Yandex WebSQL](../../websql/index.md) в настройках кластера.
