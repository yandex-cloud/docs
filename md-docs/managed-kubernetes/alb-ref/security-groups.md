...
Для корректной работы [Ingress-контроллера](ingress-controller/index.md) или [Gateway API](gateway-api/index.md) нужно настроить [группы безопасности](../../vpc/concepts/security-groups.md) [кластера](../concepts/index.md#kubernetes-cluster) и [групп узлов Yandex Managed Service for Kubernetes](../concepts/index.md#node-group) и [балансировщика нагрузки](../../application-load-balancer/concepts/application-load-balancer.md) Application Load Balancer.
...
* Все стандартные правила, описанные в соответствующих разделах документации:
  * Для кластера и групп узлов — в разделе [Настройка групп безопасности](../operations/connect/security-groups.md) документации Managed Service for Kubernetes.
  * Для балансировщика — в разделе [Группы безопасности](../../application-load-balancer/concepts/application-load-balancer.md#security-groups). Последнее правило, для исходящего трафика на виртуальные машины [бэкендов](../../application-load-balancer/concepts/backend-group.md), должно разрешать соединения в [подсети](../../vpc/concepts/network.md#subnet) [групп узлов](../concepts/index.md#node-group) кластера или его группу безопасности.
...
* Группа безопасности кластера и группы узлов для служебного трафика:
  ...
  - Исходящий трафик {#outgoing}
    ...
    Диапазон портов | Протокол | Назначение | IPv4 CIDR | Описание
    --- | --- | --- | --- | ---
    `0-65535` | `Любой` (`Any`) | `Группа безопасности` | `Текущая` (`Self`) | Для трафика между [мастером](../concepts/index.md#master) и узлами
...
* Группа безопасности группы узлов для служебного трафика:
  ...
  - Входящий трафик {#incoming}
    ...
    Диапазон портов | Протокол | Источник | IPv4 CIDR | Описание
    --- | --- | --- | --- | ---
    `0-65535` | `Любой` (`Any`) | `Диапазон адресов` | `10.96.0.0/16`[^\[К\]^](#example)<br>`10.112.0.0/16`[^\[С\]^](#example) | Для трафика между [подами](../concepts/index.md#pod) и [сервисами](../concepts/index.md#service)
...
* Группа безопасности кластера для служебного трафика:
  ...
  - Исходящий трафик {#outgoing}
    ...
    Диапазон портов | Протокол | Назначение | IPv4 CIDR | Описание
    --- | --- | --- | --- | ---
    `4443` | `TCP` | `Диапазон адресов` | `10.96.0.0/16`[^\[К\]^](#example) | Для трафика между мастером и [подами](../concepts/index.md#pod) `metric-server`
...
Группы безопасности для кластера и групп узлов описаны в разделе [Настройка групп безопасности](../operations/connect/security-groups.md).
