...
Чтобы управлять ресурсами Application Load Balancer с помощью [Ingress-контроллера](ingress-controller/index.md) и [Gateway API](gateway-api/index.md) для Yandex Managed Service for Kubernetes, нужно создать конфигурации — файлы в формате YAML с описаниями ресурсов Kubernetes: `Ingress`, `HttpBackendGroup`, `Gateway`, `HTTPRoute` и т. д.
...
1. Создайте ресурс. Конфигурации доступных ресурсов описаны в разделе [Справочник инструментов Application Load Balancer для Managed Service for Kubernetes](index.md).
...
1. Если вы работаете с Ingress-контроллером и добавили в конфигурацию новый ресурс `Ingress`, проверьте, что он создан и получил публичный IP-адрес:
   ...
   В столбце `ADDRESS` должен появиться IP-адрес. В противном случае балансировщик не создался или создался некорректно — проверьте логи [пода](../concepts/index.md#pod) `yc-alb-ingress-controller-*` (не `yc-alb-ingress-controller-hc-*`).