...
[Container Storage Interface для S3](https://yandex.cloud/ru/marketplace/products/yc/csi-s3) (_CSI_) позволяет динамически резервировать [бакеты](../../../storage/concepts/bucket.md) S3-совместимых хранилищ и монтировать их к [подам](../../concepts/index.md#pod) [кластера {{ managed-k8s-name }}](../../concepts/index.md#kubernetes-cluster) в виде [постоянных томов](../../concepts/volume.md#persistent-volume) (_PersistentVolume_). Подключение выполняется при помощи [FUSE](https://ru.wikipedia.org/wiki/FUSE_(модуль_ядра))-реализации файловой системы [GeeseFS](https://github.com/yandex-cloud/geesefs).

{% note info %}

Перед публикацией в {{ marketplace-full-name }} новые версии приложения тестируются на работоспособность в инфраструктуре {{ yandex-cloud }}, поэтому могут обновляться с задержкой. Чтобы использовать самую последнюю версию, [установите ее c помощью Helm-чарта из репозитория на GitHub](csi-s3.md#helm-github-install).
...
1. В разделе **{{ ui-key.yacloud.marketplace-v2.label_available-products }}** выберите [Container Storage Interface для S3](https://yandex.cloud/ru/marketplace/products/yc/csi-s3) и нажмите кнопку **{{ ui-key.yacloud.marketplace-v2.button_k8s-product-use }}**.
