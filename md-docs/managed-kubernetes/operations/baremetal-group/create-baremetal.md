...
В кластере Managed Service for Kubernetes можно создать [группу узлов](../../concepts/index.md#node-group) на выделенных серверах BareMetal. Сервис арендует серверы, автоматически настроит их и подключит к кластеру. По умолчанию срок аренды серверов — один день.
