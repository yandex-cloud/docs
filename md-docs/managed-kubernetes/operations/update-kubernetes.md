...
- Консоль управления {#console}
  ...
  1. В поле **Версия Kubernetes** блока **Конфигурация мастера** получите список доступных версий.
...
- Консоль управления {#console}
  ...
  * `В выбранные дни` — укажите день, начало и длительность интервала времени в UTC, в течение которого запустится обновление. Параметр не связан с длительностью самого обновления и временем его окончания. При необходимости выберите несколько вариантов с помощью кнопки **Добавить день и время**.
...
- CLI {#cli}
  ...
  * `--daily-maintenance-window` — задать начало и длительность интервала времени в UTC, в течение которого запустится обновление.
    ...
    Пример настройки ежедневного окна запуска обновлений кластера Managed Service for Kubernetes: начало интервала — 22:00 UTC, длительность интервала — не более десяти часов:

    ```bash
    --daily-maintenance-window 'start=22:00,duration=10h'
    ```

  * `--weekly-maintenance-window` — задать дни, начало и длительность интервала времени в UTC, в течение которого запустится обновление.

    Пример настройки окна запуска обновлений кластера Managed Service for Kubernetes по понедельникам и вторникам: начало интервала — 22:00 UTC, длительность интервала — не более десяти часов:
     ...
     * Чтобы включить режим ежедневного обновления:
       ...
       resource "yandex_kubernetes_cluster" "<имя_кластера>" {
         ...
         maintenance_policy {
           auto_upgrade = true
           maintenance_window {
             start_time = "<время_начала_интервала_запуска_обновления>"
             duration   = "<длительность_интервала_запуска_обновления>"
           }
         }
       }
       ```
       ...
       * `start_time` — время начала интервала, в течение которого запустится обновление, в формате [UTC](https://ru.wikipedia.org/wiki/Всемирное_координированное_время).
       * `duration` — длительность интервала, в течение которого запустится обновление (например `4h30m`). Параметр не связан с длительностью самого обновления и временем его окончания.
     ...
     * Чтобы включить режим обновления в выбранные дни (можно указать несколько периодов):
       ...
       resource "yandex_kubernetes_cluster" "<имя_кластера>" {
         ...
         maintenance_policy {
           auto_upgrade = true
           maintenance_window {
             day        = "<день_начала_интервала_запуска_обновления>"
             start_time = "<время_начала_интервала_запуска_обновления>"
             duration   = "<длительность_интервала_запуска_обновления>"
           }
           maintenance_window {
...
- API {#api}
  ...
  * Чтобы настроить ежедневные обновления, добавьте блок `dailyMaintenanceWindow`:
    ...
    "dailyMaintenanceWindow": {
      "startTime": {
        "hours": "<час_начала_интервала_запуска_обновления>",
        "minutes": "<минута_начала_интервала_запуска_обновления>",
        "seconds": "<секунда_начала_интервала_запуска_обновления>",
        "nanos": "<доля_секунды_начала_интервала_запуска_обновления>"
      },
      "duration": "<длительность_интервала_запуска_обновления>"
    }
    ```
    ...
    * `hours` — час начала интервала, в течение которого запустится обновление, в формате [UTC](https://ru.wikipedia.org/wiki/Всемирное_координированное_время).
    * `nanos` — доля секунды начала интервала, в течение которого запустится обновление, в наносекундах.
    * `duration` — длительность интервала, в течение которого запустится обновление, в часах. Параметр не связан с длительностью самого обновления и временем его окончания.
      ...
      "daysOfWeek": [
        {
          ...
          "duration": "<длительность_интервала>"
        }
      ]
    }
    ```
...
- CLI {#cli}
  ...
  * `--daily-maintenance-window` — задать начало и длительность интервала времени в UTC, в течение которого запустится обновление.
    ...
    Пример настройки ежедневного окна запуска обновлений группы узлов Managed Service for Kubernetes: начало интервала — 22:00 UTC, длительность интервала — не более десяти часов:
  ...
  * `--weekly-maintenance-window` — задать дни, начало и длительность интервала времени в UTC, в течение которого запустится обновление.
    ...
    Пример настройки окна запуска обновлений группы узлов Managed Service for Kubernetes по понедельникам и вторникам: начало интервала — 22:00 UTC, длительность интервала — не более десяти часов:
