...
* Плата за вычислительные ресурсы и диски [узлов](../concepts/index.md#node-group) кластера Managed Service for Kubernetes (см. [тарифы Yandex Compute Cloud](../../compute/pricing.md)).
* Плата за использование [мастера](../concepts/index.md#master) Managed Service for Kubernetes и исходящий трафик (см. [тарифы Managed Service for Kubernetes](../pricing.md)).
...
1. Если у вас еще нет кластера Managed Service for Kubernetes:
    1. [Создайте](../operations/kubernetes-cluster/kubernetes-cluster-create.md) кластер.
        ...
        Для остальных релизных каналов доступен [ручной обмен токенов](wlif-managed-k8s-integration.md#manual-token-exchange) сервисного аккаунта Kubernetes на IAM-токен.

        {% endnote %}

    1. [Создайте](../operations/node-group/node-group-create.md) группу узлов.
    1. [Настройте](../operations/connect/security-groups.md) группы безопасности для кластера и группы узлов.
...
1. Настройте интеграцию с федерацией сервисных аккаунтов для кластера и группы узлов:
    ...
    - Terraform {#tf}
      ...
      1. Получите URL эмитента (`issuer`) и URL набора ключей JWKS (`jwks_uri`) для настройки федерации сервисных аккаунтов с помощью источника данных Terraform [yandex_kubernetes_cluster](../../terraform/data-sources/kubernetes_cluster.md).
          ...
          Также вы можете [узнать](../operations/kubernetes-cluster/kubernetes-cluster-list.md#get) эти параметры в [консоли управления](https://console.yandex.cloud), с помощью CLI или API.
...
1. [Подключитесь](../operations/connect/index.md#kubectl-connect) к кластеру Managed Service for Kubernetes с помощью `kubectl`.
...
Ниже представлен сценарий c использованием контроллера DaemonSet `yc-metadata-server` на узлах для автоматического обмена токенов сервисного аккаунта Kubernetes на IAM-токен, доступный для кластеров в [релизном канале](../concepts/release-channels-and-updates.md) `RAPID`. Для кластеров в других релизных каналах см. подраздел [Ручной обмен токена сервисного аккаунта Kubernetes на IAM-токен](#manual-token-exchange).
...
* [Удалите](../operations/node-group/node-group-delete.md) группу узлов Managed Service for Kubernetes.
* [Удалите](../operations/kubernetes-cluster/kubernetes-cluster-delete.md) кластер Managed Service for Kubernetes.
