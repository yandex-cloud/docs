...
Если в кластере {{ managed-k8s-name }} используется контроллер сетевых политик Cilium, то настройка имеет свои особенности. Воспользуйтесь следующей [инструкцией](../operations/cilium-node-local-dns.md).
...
- Вручную {#manual}
  ...
  1. [Создайте кластер {{ managed-k8s-name }}](../operations/kubernetes-cluster/kubernetes-cluster-create.md) и [группу узлов](../operations/node-group/node-group-create.md) с [публичным доступом в интернет](../operations/node-group/node-group-update.md#node-internet-access) и с группами безопасности, подготовленными ранее.
...
- {{ marketplace-full-name }} {#marketplace}
  ...
  Установите [NodeLocal DNS](https://yandex.cloud/ru/marketplace/products/yc/node-local-dns) с помощью {{ marketplace-name }}, как описано в [инструкции](../operations/applications/node-local-dns.md#marketplace-install).
