...
Ingress-контроллер, установленный в кластер, разворачивает [L7-балансировщик](../../application-load-balancer/concepts/application-load-balancer.md) со всеми необходимыми ресурсами Application Load Balancer, основываясь на конфигурации созданных вами ресурсов [Ingress](../alb-ref/ingress.md) и [HttpBackendGroup](../alb-ref/http-backend-group.md).

L7-балансировщик автоматически проверяет работоспособность приложения в кластере. В зависимости от результатов проверки L7-балансировщик открывает или закрывает внешний трафик к бэкенду (ресурсу [Service](../alb-ref/service-for-ingress.md)). Подробнее в разделе [Проверки состояния](../../application-load-balancer/concepts/backend-group.md#health-checks).
...
С помощью этого руководства вы настроите собственные проверки состояния приложения в параметрах ресурса [HttpBackendGroup](../alb-ref/http-backend-group.md) и откроете для этих проверок отдельный порт на узлах кластера в параметрах ресурса [Service](../alb-ref/service-for-ingress.md) типа `NodePort`.
...
- Terraform {#tf}
   ...
   1. Укажите в файле `k8s-custom-health-checks.tf`:
      ...
      * `k8s_version` — версия Kubernetes. Доступные версии перечислены в разделе [Релизные каналы](../concepts/release-channels-and-updates.md).
...
Установите приложение [ALB Ingress Controller](https://yandex.cloud/ru/marketplace/products/yc/alb-ingress-controller) согласно [инструкции](../operations/applications/alb-ingress-controller.md) в отдельном пространстве имен `yc-alb`. Далее в этом пространстве имен будут созданы все нужные ресурсы Kubernetes.
...
На основании ресурсов [Ingress](../alb-ref/ingress.md) и [HttpBackendGroup](../alb-ref/http-backend-group.md) Ingress-контроллер развернет [L7-балансировщик](../../application-load-balancer/concepts/application-load-balancer.md) со всеми необходимыми ресурсами Application Load Balancer.
...
1. Проверьте, что работают проверки состояния приложения:
   ...
   - Консоль управления {#console}
      ...
      1. Выберите нужный L7-балансировщик и перейдите на вкладку **Проверки состояния**.
