...
* Плата за каждый кластер {{ managed-k8s-name }}: использование мастера и исходящий трафик (см. [тарифы {{ managed-k8s-name }}](../pricing.md)).
...
- Вручную {#manual}
  ...
  1. [Создайте два кластера](../operations/kubernetes-cluster/kubernetes-cluster-create.md) {{ managed-k8s-name }}. При создании укажите группы безопасности, подготовленные ранее.
...
- {{ TF }} {#tf}
  ...
  1. Укажите в файле `velero-backup.tf`:
     ...
     * `k8s_version` — [версию {{ k8s }}](../concepts/release-channels-and-updates.md) 1.22 или выше.
...
1. [Установите kubectl]({{ k8s-docs }}/tasks/tools/install-kubectl) и [настройте его на работу с первым кластером {{ managed-k8s-name }}](../operations/connect/index.md#kubectl-connect).
...
1. [Настройте kubectl](../operations/connect/index.md#kubectl-connect) на работу со вторым кластером {{ managed-k8s-name }}.
...
- Вручную {#manual}
  ...
  * [Удалите кластеры {{ managed-k8s-name }}](../operations/kubernetes-cluster/kubernetes-cluster-delete.md).
