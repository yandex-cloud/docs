[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for Kubernetes](../index.md) > [Практические руководства](index.md) > Мониторинг > Передача логов кластера Managed Service for Kubernetes в Cloud Logging
...
Вы можете отправлять логи [кластера Managed Service for Kubernetes](../concepts/index.md#kubernetes-cluster) в Cloud Logging:
* Для отправки логов [мастера Managed Service for Kubernetes](../concepts/index.md#master) используйте настройку `master logging` при [создании](../operations/kubernetes-cluster/kubernetes-cluster-create.md) или [изменении](../operations/kubernetes-cluster/kubernetes-cluster-update.md) кластера. Настройка доступна только через API, CLI и Terraform.
* Для отправки логов [подов](../concepts/index.md#pod) и [сервисов](../concepts/index.md#service) используйте приложение [Fluent Bit с плагином для Cloud Logging](https://yandex.cloud/ru/marketplace/products/yc/fluent-bit) в кластере Managed Service for Kubernetes.
...
- Вручную {#manual}
  ...
  1. [Создайте кластер Managed Service for Kubernetes](../operations/kubernetes-cluster/kubernetes-cluster-create.md#kubernetes-cluster-create) и [группу узлов](../operations/node-group/node-group-create.md). При создании кластера Managed Service for Kubernetes укажите ранее созданные сервисные аккаунты для ресурсов и узлов.
  1. [Настройте группы безопасности](../operations/connect/security-groups.md) для работы кластера Managed Service for Kubernetes.
...
- Terraform {#tf}
  ...
  1. Скачайте в ту же рабочую директорию файл конфигурации кластера Managed Service for Kubernetes [k8s-cluster-with-master-logging.tf](https://github.com/yandex-cloud-examples/yc-mk8s-cloud-logging/blob/main/k8s-cluster-with-master-logging.tf).
     ...
     * [Группы безопасности](../operations/connect/security-groups.md) и правила, необходимые для работы кластера Managed Service for Kubernetes и [группы узлов](../concepts/index.md#node-group):
...
Чтобы настроить передачу логов [подов](../concepts/index.md#pod), [сервисов](../concepts/index.md#service) и системных логов [узлов](../concepts/index.md#node-group) [Yandex Managed Service for Kubernetes](../index.md) в [Yandex Cloud Logging](../../logging/index.md):
...
- Вручную {#manual}
  ...
  1. Если у вас еще нет [сети](../../vpc/concepts/network.md#network), [создайте ее](../../vpc/operations/network-create.md).
  1. Если у вас еще нет [подсетей](../../vpc/concepts/network.md#subnet), [создайте их](../../vpc/operations/subnet-create.md) в [зонах доступности](../../overview/concepts/geo-scope.md), где будут созданы [кластер Managed Service for Kubernetes](../concepts/index.md#kubernetes-cluster) и [группа узлов](../concepts/index.md#node-group).
...
- С помощью Yandex Cloud Marketplace {#marketplace}
  ...
  Установите Fluent Bit согласно [инструкции](../operations/applications/fluentbit.md#marketplace-install). В настройках приложения укажите идентификатор [созданной ранее](#before-you-begin) лог-группы. Идентификатор лог-группы можно получить со [списком лог-групп в каталоге](../../logging/operations/list.md).
...
- Вручную {#manual}
  ...
  1. Укажите идентификатор [созданной ранее](#before-you-begin) лог-группы и опционально идентификатор кластера в секции `[OUTPUT]` файла `config.yaml`:
     ...
     Идентификатор [лог-группы](../../logging/concepts/log-group.md) можно получить со [списком лог-групп в каталоге](../../logging/operations/list.md), идентификатор кластера — [со списком кластеров в каталоге](../operations/kubernetes-cluster/kubernetes-cluster-list.md#list).
