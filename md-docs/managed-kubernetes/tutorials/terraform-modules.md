...
На этой странице рассказано, как подключить модули и использовать их для создания тестовой инфраструктуры: [облачной сети](../../vpc/concepts/network.md#network) с тремя [подсетями](../../vpc/concepts/network.md#subnet) [{{ vpc-full-name }}](../../vpc/index.md) и [кластера {{ managed-k8s-full-name }}](../concepts/index.md#kubernetes-cluster).
...
* Плата за [высокодоступный мастер {{ managed-k8s-name }}](../concepts/index.md#master) (см. [тарифы {{ managed-k8s-name }}](../pricing.md)).
* Плата за постоянно запущенные [виртуальные машины](../../compute/concepts/vm.md) в [группе узлов {{ managed-k8s-name }}](../concepts/index.md#node-group) (см. [тарифы {{ compute-full-name }}](../../compute/pricing.md)).
