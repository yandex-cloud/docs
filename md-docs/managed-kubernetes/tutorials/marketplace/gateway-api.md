...
[Gateway API](https://github.com/kubernetes-sigs/gateway-api) — набор ресурсов [API](../../../glossary/rest-api.md), моделирующих сетевое взаимодействие в [кластере {{ k8s }}](../../concepts/index.md#kubernetes-cluster).
...
* `dev-gw.yaml` и `prod-gw.yaml` — настройки Gateway. В этих манифестах нужно указать:
  * [Группы безопасности](../../operations/connect/security-groups.md), в которых развернут кластер {{ k8s }}, в параметре `metadata.annotations.gateway.alb.yc.io/security-groups`.
