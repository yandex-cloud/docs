...
В {{ managed-k8s-name }} поддерживается «горячее» увеличение размера томов с помощью объекта `PersistentVolumeClaim` без необходимости перезапуска подов, которые эти тома используют.

Подробнее см. в разделе [{#T}](../operations/volumes/volume-expansion.md).
...
Чтобы использовать зашифрованные диски, у [облачного сервисного аккаунта](index.md#service-accounts), назначенного кластеру {{ managed-k8s-name }}, должна быть [роль](../../kms/security/index.md#kms-keys-encrypterDecrypter) `kms.keys.encrypterDecrypter` на ключ или каталог.

{% endnote %}

Для [статической подготовки тома](#static-provisioning) вы можете использовать существующий зашифрованный диск, аналогично обычному диску из {{ compute-name }}.
...
* `parameters:type` — [тип диска](volume.md#disks-types) в {{ compute-name }}. Возможные значения:
...
Подробнее об использовании зашифрованных дисков см. на странице [{#T}](../operations/volumes/encrypted-disks.md).
