[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for Kubernetes](../index.md) > Концепции > Взаимосвязь ресурсов сервиса
...
Подробнее о настройках мастера смотрите на странице [Создание кластера Managed Service for Kubernetes](../operations/kubernetes-cluster/kubernetes-cluster-create.md).
...
Подробнее на странице [Получение списка доступных конфигураций мастера в кластере Kubernetes](../operations/kubernetes-cluster/kubernetes-cluster-configuration-list.md).

Изменение конфигурации мастера не требует остановки кластера Managed Service for Kubernetes.

## Группа узлов {#node-group}

_Группа узлов_ объединяет вычислительные ресурсы кластера Kubernetes, на которых запускаются пользовательские контейнеры. Группы узлов бывают следующих типов:

* _Облачная группа узлов_ — [группа виртуальных машин](../../compute/concepts/instance-groups/index.md) Yandex Compute Cloud с одинаковой конфигурацией. Сервис создает и обслуживает ВМ, поддерживает заданное количество узлов и позволяет настроить автоматическое масштабирование. Это основной сценарий [создания группы узлов](../operations/node-group/node-group-create.md).
...
* _Группа узлов на серверах BareMetal_ состоит из выделенных физических серверов одной конфигурации. При [создании группы](../operations/baremetal-group/create-baremetal.md) сервис арендует серверы, настраивает их и подключает к кластеру. Предварительно арендовать и подключать каждый сервер вручную не нужно. Серверы отображаются в BareMetal, а группа управляется через консоль управления в Managed Service for Kubernetes.
...
* Параметры ядра.
  ...
  * _Небезопасные_ (unsafe) параметры влияют на работу не только подов, но и узла в целом. В Managed Service for Kubernetes нельзя изменять небезопасные параметры ядра, имена которых не были явно указаны при [создании группы узлов](../operations/node-group/node-group-create.md).
  ...
  Подробнее о параметрах ядра в [документации Kubernetes](https://kubernetes.io/docs/tasks/administer-cluster/sysctl-cluster/).
...
Подробнее о taint-политиках и исключениях читайте в [документации Kubernetes](https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/).
...
* [Kubernetes-метки](https://kubernetes.io/ru/docs/concepts/overview/working-with-objects/labels/) — используются для группировки объектов Kubernetes и [распределения подов по узлам кластера](https://kubernetes.io/docs/tasks/configure-pod-container/assign-pods-nodes). Обозначаются в CLI как `node-labels` и в Terraform — как `node_labels`.
  ...
  При назначении Kubernetes-меток указывайте характеристики узлов, по которым вы будете группировать объекты. Примеры Kubernetes-меток приведены в [документации Kubernetes](https://kubernetes.io/ru/docs/concepts/overview/working-with-objects/labels/#причины-использования).
...
* через SSH-клиент с помощью стандартной пары SSH-ключей, смотрите [Подключение к узлу по SSH](../operations/node-connect-ssh.md);
* через SSH-клиент и CLI с помощью OS Login, смотрите [Подключение к узлу через OS Login](../operations/node-connect-oslogin.md).
...
Для реализации механизма маскарадинга на каждом узле кластера развернут под `ip-masq-agent`. Настройки этого пода хранятся в объекте ConfigMap с именем `ip-masq-agent` в пространстве имен `kube-system`. В параметре `data.config.nonMasqueradeCIDRs` задаются диапазоны назначения, для которых маскарадинг не требуется, например, для доступа к подам через [VPN](../../glossary/vpn.md) или [Yandex Cloud Interconnect](../../interconnect/index.md).
...
* **Облачные сервисные аккаунты**
  ...
  Подробнее в разделе [Управление доступом в Managed Service for Kubernetes](../security/index.md) и [Сервисные аккаунты](../../iam/concepts/users/service-accounts.md).
* **Сервисные аккаунты Kubernetes**
  ...
  * Настройки прав доступа для этих приложений.
  ...
  Подробнее в [Создание статического файла конфигурации](../operations/connect/create-static-conf.md), а также в [документации Kubernetes](https://kubernetes.io/docs/reference/access-authn-authz/service-accounts-admin/).
