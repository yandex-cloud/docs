...
* Диапазон IP-адресов для подов: `10.1.0.0/16`.
  ...
  Для кластеров в [релизном канале](release-channels-and-updates.md) `RAPID` можно использовать [несколько диапазонов IP-адресов подов](cluster-multi-cidr.md). Добавьте CIDR в соответствии с [требованиями](cluster-multi-cidr.md#requirements), а после [настройте маскарадинг IP-адресов](../operations/network/manage-ip-masq-agent.md).
...
Диапазон подов содержит `2^(24 − 16) = 256` блоков `/24`: от `10.1.0.0/24` до `10.1.255.0/24`. Расчет не учитывает возможное резервирование блоков сервисом Managed Service for Kubernetes. При планировании также учитывайте [квоты и лимиты](limits.md).
