...
* **Сеть**. Yandex Cloud обеспечивает создание, настройку и удаление сетевых балансировщиков [Yandex Network Load Balancer](../nlb-ref/index.md) с помощью объекта `Service` с типом `LoadBalancer`.
...
* **metadata-server** — ресурс `DaemonSet`, который устанавливается на все узлы кластера и обеспечивает автоматический обмен JWT-токена [сервисного аккаунта](index.md#service-accounts) Kubernetes на IAM-токен облачного сервисного аккаунта.
...
* Версия Kubernetes для мастера и [релизный канал](release-channels-and-updates.md).
...
* Включение логирования Control Plane: [аудитные логи](../at-ref.md), логи [Cluster Autoscaler](autoscale.md), логи событий, логи API-сервера Kubernetes.
