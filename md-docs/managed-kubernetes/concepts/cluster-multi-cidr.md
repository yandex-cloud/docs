...
1. Выберите диапазон, который соответствует требованиям и вмещает нужное количество узлов.
   ...
   Например, диапазон `10.1.0.0/16` при маске подсети узлов `/24` содержит `2^(24 − 16) = 256` блоков: от `10.1.0.0/24` до `10.1.255.0/24`. Расчет не учитывает возможное резервирование блоков сервисом Managed Service for Kubernetes.
