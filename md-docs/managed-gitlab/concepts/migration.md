...
Миграция доступна только для _пользовательской инсталляции_ — GitLab, установленного на сервер или локальный компьютер (self-managed GitLab). Перенести данные из GitLab.com нельзя.
...
На каждом этапе миграции подразумевается, что данные переносятся с помощью резервной копии пользовательской инсталляции GitLab. Вы передаете эту копию технической поддержке Yandex Cloud, и поддержка разворачивает инстанс Managed Service for GitLab из резервной копии.

Подробнее о том, как провести миграцию, читайте в [инструкции](../operations/instance/migration.md).

Версия GitLab в пользовательской инсталляции должна совпадать с версией GitLab в инстансе Managed Service for GitLab. Точная версия для облака согласуется с технической поддержкой во время миграции, так как GitLab регулярно выпускает новые версии.

## Пакеты услуг по миграции {#packages}

Для миграции в Managed Service for GitLab действуют два пакета услуг:

* **Базовый пакет** — во время миграции вы самостоятельно настраиваете инстанс Managed Service for GitLab, в случае проблем можно завести отдельное обращение в поддержку. На каждую проблему заводится отдельное обращение.

   Чтобы воспользоваться базовым пакетом, должно выполняться хотя бы одно из условий:

   * Тип инстанса GitLab, который определяет [количество ядер (vCPU) и объем памяти (RAM)](../../compute/concepts/vm-platforms.md), — `s2.small` или выше.
