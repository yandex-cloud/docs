...
## Рекомендации по настройке безопасности инстанса GitLab {#secure-instance}

{% note tip %}

Перед настройкой инстанса изучите [общие рекомендации по безопасности GitLab](https://docs.gitlab.com/ee/security/).
...
### Работа с Docker изнутри GitLab {#using-docker}
...
* Не используйте `Shell executor`, `Docker-in-Docker` и `Docker socket binding` — это дает доступ к `Docker socket` и `privileged mode`. Подробнее в [статье Securing GitLab CI pipelines with Sysbox](https://blog.nestybox.com/2020/10/21/gitlab-dind.html).
...
### Интеграция с Yandex Managed Service for Kubernetes {#using-kubernetes}

* Для безопасной интеграции используйте [GitLab Agent for Kubernetes](https://docs.gitlab.com/ee/user/clusters/agent/).
* Чтобы исключить использование [сервисных аккаунтов](../../iam/concepts/users/service-accounts.md) с ролью `cluster-admin` и открытия API Kubernetes в интернет, не используйте [certificate-based интеграцию](https://docs.gitlab.com/ee/user/infrastructure/clusters/).
* Чтобы исключить связанность агента GitLab Runner и Kubernetes, используйте развертывание с помощью [CI/CD tunnel](https://docs.gitlab.com/ee/user/clusters/agent/ci_cd_tunnel.html).
...
* Настраивайте доступ к проектам с помощью [механизма групп GitLab](https://docs.gitlab.com/ee/user/group/).
...
### Безопасная конфигурация агента GitLab Runner {#configuring-runners}

* Используйте наиболее изолированные и безопасные [управляющие программы `Docker` и `Kubernetes`](https://docs.gitlab.com/runner/executors/). Не рекомендуется использовать устаревший `Shell executor`.
* Чтобы ограничить сетевой доступ к агенту GitLab Runner, используйте [группы безопасности](../../vpc/concepts/security-groups.md).
