...
* [Почему возникает ошибка о минимальном объеме памяти для процессов {{ mgp-name }}?](#memory-limit)
...
Кластер {{ mgp-name }} состоит минимум из 4 хостов:
...
Ошибка возникает, если при количестве соединений, достигающем значения настройки [max_connections](../concepts/settings-list.md#setting-max-connections), объем памяти на каждый процесс {{ mgp-name }} составляет меньше 20 МБ. Минимальный объем памяти на каждый процесс кластера рассчитывается по формуле:
...
Подробнее в разделе [{#T}](../concepts/hybrid-storage.md#storage-vacuum).
