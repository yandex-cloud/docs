...
| `net.udp_noports`<br/>`DGAUGE`, штуки | Общее количество полученных UDP-пакетов, для которых на порте назначения не было приложения.  |
...
## Метрики расширения кластера {#managed-greenplum-expand-metrics}

Эти метрики позволяют наблюдать за ходом [расширения кластера](concepts/expand.md).

| Имя<br/>Тип, единицы измерения | Описание |
| ------------------------------ | -------- |
| `gp_expand.bytes_done`<br/>`DGAUGE`, %                | Процент перераспределенных данных. |
| `gp_expand.bytes_in_progress`<br/>`DGAUGE`, байты     | Объем данных, который перераспределяется в настоящий момент времени. |
| `gp_expand.bytes_left`<br/>`DGAUGE`, байты            | Объем данных, который осталось перераспределить. |
| `gp_expand.bytes_total`<br/>`DGAUGE`, байты           | Общий объем данных, который нужно перераспределить. |
| `gp_expand.in_progress`<br/>`DGAUGE`                  | Показатель активности процесса перераспределения данных.<br/>Принимает значение `1`, если процесс запущен, и `0`, если нет. |
| `gp_expand.tables_done`<br/>`DGAUGE`, %               | Процент перераспределенных таблиц. |
...
| `snmp6.udp6noports`<br/>`DGAUGE`, штуки | Общее количество полученных UDPv6-пакетов, для которых на порте назначения не было приложения (в сетевом протоколе SNMP). |
...
В этом разделе описаны метрики компонентов СУБД, Odyssey и Yezzey, поставляемые в [Monitoring](../monitoring/concepts/index.md).
