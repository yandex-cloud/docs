...
[Кластер](../../glossary/cluster.md) Greenplum® состоит из хостов-мастеров, которые принимают запросы от клиента, и хостов-сегментов, обеспечивающих обработку и хранение данных.
...
Для создания кластера Greenplum® нужна роль [vpc.user](../../vpc/security/index.md#vpc-user) и роль [managed-greenplum.editor или выше](../security/index.md#roles-list). О том, как назначить роль, см. [документацию Identity and Access Management](../../iam/operations/roles/grant.md).
...
- Консоль управления {#console}
    ...
    1. Выберите версию Greenplum®.

    
    1. (Опционально) Чтобы разместить хосты-мастеры или хосты-сегменты на выделенных хостах, выберите группы [выделенных хостов](../../compute/concepts/dedicated-host.md). Можно назначить группы на один из видов хостов Greenplum® либо сразу на оба.
    ...
    1. При необходимости задайте дополнительные настройки кластера:
        ...
        * **Запись логов** — опция включает логирование операций в кластере. За хранение логов взимается плата в соответствии с тарифами Yandex Cloud Logging. Для записи логов [назначьте](../../iam/operations/sa/assign-role-for-sa.md) выбранному сервисному аккаунту роль `logging.writer`.
          ...
          * Выберите, какие логи записывать:
             ...
             * **Логи Greenplum** — опция включает запись логов Greenplum®. Уровень логирования можно задать с помощью параметра [Log min messages](../concepts/settings-list.md#setting-log-min-messages) в блоке **Настройки СУБД**.
...
- Terraform {#tf}
    ...
    1. Запишите идентификатор первоначального кластера Greenplum® в переменную окружения:
    ...
    1. Импортируйте настройки первоначального кластера Greenplum® в конфигурацию Terraform:
