...
Чтобы распределить данные по нескольким СУБД и иметь доступ к ним, с помощью PXF создаются [внешние таблицы](../concepts/external-tables.md) — специальные объекты в Greenplum®, которые ссылаются на таблицы, бакеты или файлы внешних источников. В этом разделе приведены [инструкции](#work-with-pxf), как создать внешние таблицы, ссылающиеся на внешние СУБД.
