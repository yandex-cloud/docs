...
Вы можете настроить регулярный сбор логов о работе кластера Yandex MPP Analytics for PostgreSQL. Логи поставляются в [лог-группу](../../logging/concepts/log-group.md) в сервисе Cloud Logging. Можно выбрать лог-группу одного из двух типов:
...
1. Создайте кластер Yandex MPP Analytics for PostgreSQL с включенными настройками логирования и [созданным ранее](#before-you-begin) сервисным аккаунтом:
   ...
   * Консоль управления {#console}
      ...
      1. В блоке **Дополнительные настройки**:
         ...
         * Чтобы логи записывались в лог-группу по умолчанию, в поле **Назначение** выберите значение **Каталог**.
         * Выберите **Каталог**, лог-группу которого нужно использовать.
         * Включите опции **Логи командного центра** и **Логи Yandex MPP Analytics for PostgreSQL**. Уровень логирования можно задать с помощью параметра [Log min messages](../concepts/settings-list.md#setting-log-min-messages) в блоке **Настройки СУБД**.
   ...
   * CLI {#cli}
      ...
      * `--greenplum-version` — версия СУБД (Greenplum® или Apache Cloudberry™). Подробнее о [доступных версиях СУБД](../concepts/overview.md).
   ...
   * Terraform {#tf}
      ...
      1. В конфигурационном файле с описанием кластера укажите следующие параметры:
          ...
          * `version` — версия СУБД (Greenplum® или Apache Cloudberry™). Подробнее о [доступных версиях СУБД](../concepts/overview.md).
   ...
   * API {#api}
      ...
      В теле запроса на создание кластера ([Cluster.Create](../api-ref/Cluster/create.md) в REST API или [ClusterService.Create](../api-ref/grpc/Cluster/create.md) в gRPC API) укажите следующие параметры:
...
1. Проверьте передачу логов кластера в лог-группу:
   ...
   * API {#api}
      ...
      Чтобы посмотреть записи в лог-группе, воспользуйтесь вызовом REST API [Cluster.ListLogs](../api-ref/Cluster/listLogs.md) или gRPC API [LogReadingService/Read](../../logging/api-ref/grpc/LogReading/read.md).
...
1. Создайте кластер Yandex MPP Analytics for PostgreSQL с включенными настройками логирования и [созданным ранее](#before-you-begin) сервисным аккаунтом:
   ...
   * Консоль управления {#console}
      ...
      1. В блоке **Дополнительные настройки**:
         ...
         * Включите опции **Логи Yandex MPP Analytics for PostgreSQL** и **Логи командного центра**. Уровень логирования можно задать с помощью параметра [Log min messages](../concepts/settings-list.md#setting-log-min-messages) в блоке **Настройки СУБД**.
