...
Подключение к кластеру Greenplum® производится только через [первичный хост-мастер](../../concepts/index.md). Чтобы определить роли хостов, получите [список хостов в кластере](../cluster-list.md#get-hosts).
...
Чтобы обеспечить безопасное подключение к Managed Service for Trino, рекомендуется настроить группы безопасности [на стороне Greenplum®](#configuring-sg-greenplum) и (опционально) [на стороне Managed Service for Trino](#configuring-sg-trino).
...
#### Настройка на стороне Greenplum® {#configuring-sg-greenplum}
...
#### Настройка на стороне Managed Service for Trino {#configuring-sg-trino}
