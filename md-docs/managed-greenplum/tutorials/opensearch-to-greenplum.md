[Документация Yandex Cloud](../../index.md) > [Yandex MPP Analytics for PostgreSQL](../index.md) > [Практические руководства](index.md) > Копирование данных из Managed Service for OpenSearch в Yandex MPP Analytics for PostgreSQL с помощью Yandex Data Transfer
...
- Вручную {#manual}
    ...
    1. В той же [зоне доступности](../../overview/concepts/geo-scope.md) [создайте кластер-приемник Greenplum®](../operations/cluster-create.md#create-cluster) любой подходящей конфигурации. При создании кластера:
    ...
    1. Убедитесь, что группы безопасности кластеров [Managed Service for OpenSearch](../../managed-opensearch/operations/connect/index.md#security-groups) и [Greenplum®](../operations/connect/index.md#configuring-security-groups) разрешают подключение через интернет.
...
1. Убедитесь, что в Yandex MPP Analytics for PostgreSQL перенеслись данные из кластера-источника Managed Service for OpenSearch:
   ...
   1. [Получите SSL-сертификат](../operations/connect/index.md#get-ssl-cert) для подключения к кластеру Greenplum®.
