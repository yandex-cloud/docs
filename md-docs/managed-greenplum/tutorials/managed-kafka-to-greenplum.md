[Документация Yandex Cloud](../../index.md) > [Yandex MPP Analytics for PostgreSQL](../index.md) > [Практические руководства](index.md) > Поставка данных с помощью Data Transfer > Поставка из Apache Kafka® в Greenplum®
...
1. Подготовьте инфраструктуру:
    ...
    - Вручную {#manual}
        ...
        1. [Создайте кластер-приемник Greenplum®](../operations/cluster-create.md#create-cluster) любой подходящей конфигурации с именем пользователя-администратора `user` и хостами в публичном доступе.
...
1. Отправьте данные из файла `sample.json` в топик `sensors` Managed Service for Apache Kafka® с помощью утилит `jq` и `kafkacat`:
    ...
    Подробнее о настройке SSL-сертификата и работе с `kafkacat` в разделе [Подключение к кластеру Apache Kafka® из приложений](../../managed-kafka/operations/connect/clients.md).

1. Убедитесь, что в базу данных Greenplum® перенеслись данные из кластера-источника Managed Service for Apache Kafka®:

    1. [Подключитесь к базе данных Greenplum®](../operations/connect/index.md).
