# Параметры командного центра {{ mgp-name }}

В этом разделе описаны параметры сессий и запросов, доступные в [командном центре {{ mgp-name }}](command-center.md).
...
* **{{ ui-key.yacloud.greenplum.cluster.perf-diag.label_field-session-rsgid }}** — идентификатор объекта [ресурсной группы](resource-groups.md).
...
* **{{ ui-key.yacloud.greenplum.cluster.perf-diag.label_field-running-query-key }}** — [ключ запроса](command-center.md#statement-key).
...
* **{{ ui-key.yacloud.greenplum.cluster.perf-diag.label_query-field-query-id }}** — [неуникальный хеш](command-center.md#statement-id), полученный из текста запроса.
