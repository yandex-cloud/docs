...
С момента создания кластера Yandex MPP Analytics for PostgreSQL в нем присутствует три ресурсные группы:
...
Подробнее об управлении ролями читайте в разделе [Пользователи и роли](cluster-users.md) и в [документации Greenplum®](https://techdocs.broadcom.com/us/en/vmware-tanzu/data-solutions/tanzu-greenplum/7/greenplum-database/admin_guide-roles_privs.html).
...
* (Рекомендованный) Указать долю ресурсов CPU сегмента, которая будет доступна для ресурсной группы (параметр `CPU_RATE_LIMIT`).
    ...
    В Yandex MPP Analytics for PostgreSQL используется режим управления, при котором вычислительные ресурсы CPU простаивающих ресурсных групп перераспределяются в пользу ресурсных групп с высокой загрузкой. Если ресурсная группа выходит из простоя, то перераспределенные ресурсы CPU возвращаются к ней.
...
Подробнее о работе ресурсных групп читайте в [документации Greenplum®](https://techdocs.broadcom.com/us/en/vmware-tanzu/data-solutions/tanzu-greenplum/7/greenplum-database/admin_guide-workload_mgmt_resgroups.html).
