[Документация Yandex Cloud](../../index.md) > [Yandex MPP Analytics for PostgreSQL](../index.md) > [Концепции](overview.md) > Расчет конфигурации кластера

# Расчет конфигурации кластера

В этом разделе приведены общие рекомендации для расчета конфигурации кластера при использовании Yandex MPP Analytics for PostgreSQL в качестве корпоративного [хранилища данных](https://ru.wikipedia.org/wiki/Хранилище_данных) небольших или средних размеров — до 100 ТБ несжатых данных.
...
Рекомендуется проводить нагрузочное тестирование продуктивного кластера перед вводом в эксплуатацию. При необходимости Yandex MPP Analytics for PostgreSQL позволяет [изменить конфигурацию кластера](../operations/cluster-config.md) с минимальным временем простоя.
...
Формула для расчета размера хранилища учитывает следующие факторы:

* Зеркалирование — Yandex MPP Analytics for PostgreSQL хранит две копии данных.
* Сжатие — в Yandex MPP Analytics for PostgreSQL объем, занимаемый данными на диске, меньше объема самих данных. Степень сжатия зависит от типа данных и выбранного алгоритма сжатия. Для расчета показатель сжатия принимается равным 3.
* Запас свободного места — для транзакционных логов, spill-файлов, системных файлов. Рекомендуемое заполнение хранилища при эксплуатации Yandex MPP Analytics for PostgreSQL — не более 70%.

С учетом этих факторов суммарный требуемый размер хранилища для всех хостов-сегментов приблизительно равен объему данных без сжатия:

```text
Размер хранилища = <объем_данных_без_сжатия> × 2 ÷ 3 ÷ 0,7 = <объем_данных_без_сжатия> × 0,95
```

Объем данных, который предполагается хранить в гибридном хранилище, используя модуль [Yezzey](../operations/extensions/yezzey.md), не следует учитывать при расчете размера хранилища Yandex MPP Analytics for PostgreSQL.

О типах дисков, которые используются для организации хранилища, читайте в разделе [Типы дисков](storage.md).

### Расчет количества хостов-сегментов и количества сегментов на хост {#number-segments}

Архитектура сервиса Yandex MPP Analytics for PostgreSQL реализует параллелизацию обработки данных. Единицей параллелизации является сегмент. Чем больше сегментов в кластере, тем больше ресурсов выделяется на один запрос.
...
Подробнее о классах хостов читайте в разделе [Доступные классы хостов](instance-types.md#available-flavors).
