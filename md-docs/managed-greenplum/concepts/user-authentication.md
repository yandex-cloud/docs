...
* `md5` — по паролю. Подробнее читайте в [документации PostgreSQL](https://www.postgresql.org/docs/current/auth-password.html).
* `iam` — [с помощью IAM](../operations/connect/clients.md#iam).
* `reject` — запрещает подключение пользователя.

Подробнее о настройках читайте в [документации PostgreSQL](https://www.postgresql.org/docs/current/auth-pg-hba-conf.html).