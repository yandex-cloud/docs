...
По умолчанию в Yandex MPP Analytics for PostgreSQL используются [heap-таблицы](tables.md) — стандартный тип таблиц PostgreSQL. Для них команды `VACUUM` и `VACUUM FULL` работают следующим образом:
...
[Подробнее об операции очистки в PostgreSQL](https://www.postgresql.org/docs/18/routine-vacuuming.html).
...
[Подробнее о wraparound в PostgreSQL](https://www.postgresql.org/docs/18/routine-vacuuming.html#VACUUM-FOR-WRAPAROUND).
...
### Очистка таблиц Yezzey {#yezzey}

[Yezzey](../operations/extensions/yezzey.md) — это расширение для кластера Yandex MPP Analytics for PostgreSQL, которое позволяет хранить таблицы AO и AOCO в [гибридном хранилище](hybrid-storage.md).
...
Системный каталог в Yandex MPP Analytics for PostgreSQL — это набор системных таблиц в схеме `pg_catalog`. Системные таблицы хранят метаданные о таблицах, столбцах и других объектах базы данных. Со временем они раздуваются из-за создания и удаления DDL-объектов, особенно при массовой загрузке данных (например, с помощью [Managed Service for Apache Airflow™](../../managed-airflow/index.md)). Поэтому для системного каталога также необходимо выполнять операции `VACUUM` и `VACUUM FULL`.
...
Получить информацию о статистике bloat системного каталога и статистике очистки пользовательских таблиц можно в [консоли управления](https://console.yandex.cloud) на вкладке **Мониторинг** страницы управления кластером или в сервисе [Yandex Monitoring](../../monitoring/concepts/index.md), на графиках в блоке **Background activities**.

[Подробнее о мониторинге состояния кластера Yandex MPP Analytics for PostgreSQL](../operations/monitoring.md#monitoring-cluster).

## Автоматическая очистка {#autovacuum}

В Yandex MPP Analytics for PostgreSQL очистка (`VACUUM`) пользовательских таблиц и системного каталога выполняется автоматически в фоновом режиме. Однако рекомендуется отслеживать состояние таблиц и индексов с помощью [графиков bloat](../operations/monitoring.md#monitoring-cluster) и выполнять очистку [самостоятельно](#vacuum-recomendation), если автоматическая очистка оказывается недостаточной и таблицы раздуваются.
