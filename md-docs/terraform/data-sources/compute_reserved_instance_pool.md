[Документация Yandex Cloud](../../index.md) > [Terraform в Yandex Cloud](../index.md) > Справочник Terraform > Ресурсы (англ.) > Compute Cloud > Data Sources > compute_reserved_instance_pool
...
- `allow_oversubscription` (*Read-Only*) (Bool). Allows the pool to contain more linked instances than the number of available slots (size without pending or unavailable slots).
...
- `gpu_settings` [Block]. GPU settings.
  - `gpu_cluster_id` (*Read-Only*) (String). Attach instance to specified GPU cluster.
...
- `platform_id` (*Read-Only*) (String). ID of the hardware platform configuration for pool instances.
...
- `resources_spec` [Block]. Computing resources of pool instances, such as the amount of memory and number of cores.
  - `core_fraction` (*Read-Only*) (Number). Baseline level of CPU performance with the ability to burst performance above that baseline level.
