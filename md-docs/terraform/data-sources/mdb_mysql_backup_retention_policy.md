[Документация Yandex Cloud](../../index.md) > [Terraform в Yandex Cloud](../index.md) > Справочник Terraform > Ресурсы (англ.) > Managed Service for MySQL > Data Sources > mdb_mysql_backup_retention_policy

# yandex_mdb_mysql_backup_retention_policy (DataSource)
...
```terraform
//
// Get information about existing MDB MySQL Backup Retention Policy.
//
data "yandex_mdb_mysql_backup_retention_policy" "my_policy" {
  cluster_id = "some_cluster_id"
  policy_id  = "some_policy_id"
}

output "policy_name" {
  value = data.yandex_mdb_mysql_backup_retention_policy.my_policy.policy_name
}
```

## Arguments & Attributes Reference

- `cluster_id` (**Required**)(String). ID of the MySQL cluster.
