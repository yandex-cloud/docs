[Документация Yandex Cloud](../../index.md) > [Terraform в Yandex Cloud](../index.md) > Справочник Terraform > Ресурсы (англ.) > Smart Web Security > Data Sources > sws_load_balancer

# yandex_sws_load_balancer (DataSource)
...
- `deletion_protection` (*Read-Only*) (Bool). Forbids Delete() calls.
...
- `log_options` [Block]. Cloud logging settings.
  ...
  - `discard_rule` [Block]. Ordered list of rules, first matching rule applies.
    - `discard_percent` (*Read-Only*) (Number). Percent of logs to be discarded: 0 - keep all, 100 or unset - discard all.
    - `http_code_intervals` (*Read-Only*) (List Of String). Groups of HTTP codes like 4xx that should be discarded.
    - `http_codes` (*Read-Only*) (List Of Number). HTTP codes that should be discarded.
  - `log_group_id` (*Read-Only*) (String). ID of Cloud Logging log group.
 Keep empty to use default log group.
- `name` (**Required**)(String). Name of the load balancer.
- `region_id` (*Read-Only*) (String). "ru-central1", "kz1", etc.
- `resource_units` (*Read-Only*) (Number). Number of initially allocated resource units.
