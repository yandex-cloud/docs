[Документация Yandex Cloud](../../index.md) > [Terraform в Yandex Cloud](../index.md) > Справочник Terraform > Ресурсы (англ.) > Managed Service for PostgreSQL > Resources > mdb_postgresql_cluster

# yandex_mdb_postgresql_cluster (Resource)

Manages a PostgreSQL cluster within the Yandex Cloud. For more information, see [the official documentation](../../managed-postgresql/index.md). [How to connect to the DB](../../managed-postgresql/quickstart.md#connect). To connect, use port 6432. The port number is not configurable.
...
- `config` [Block]. Configuration of the PostgreSQL cluster.
  ...
  - `access` [Block]. Access policy to the PostgreSQL cluster.
    ...
    - `serverless` (Bool). Allow access for [connection to managed databases from functions](../../functions/operations/database-connection.md).
    - `web_sql` (Bool). Allow access for [SQL queries in the management console](../../managed-postgresql/operations/web-sql-query.md).
...
- `database` [Block]. 
  ...
  - `extension` [Block]. Set of database extensions.
    - `name` (**Required**)(String). Name of the database extension. For more information on available extensions see [the official documentation](../../managed-postgresql/operations/cluster-extensions).
