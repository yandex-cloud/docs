[Документация Yandex Cloud](../../index.md) > [Terraform в Yandex Cloud](../index.md) > Справочник Terraform > Ресурсы (англ.) > Certificate Manager > Resources > cm_certificate_iam_binding
...
**Warning:** This resource is authoritative for the given `role` on the target `certificate` and manages the complete set of its members. When you change or delete `yandex_cm_certificate_iam_binding`, the `role` may be removed from other subjects on the `certificate` as well — including subjects granted outside of this resource (via the corresponding `*_iam_member` resource, the management console, CLI or API). Those subjects are not tracked in the Terraform state, so a plain `terraform plan` does not list them. Be careful.
