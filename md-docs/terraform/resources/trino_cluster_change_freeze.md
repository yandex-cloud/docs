[Документация Yandex Cloud](../../index.md) > [Terraform в Yandex Cloud](../index.md) > Справочник Terraform > Ресурсы (англ.) > Managed Service for Trino > Resources > trino_cluster_change_freeze

# yandex_trino_cluster_change_freeze (Resource)
...
```terraform
resource "yandex_trino_cluster_change_freeze" "example" {
  resource_id = yandex_trino_cluster.example.id
...
terraform import yandex_trino_cluster_change_freeze.example c9q-resource-id:change-freeze-id
```