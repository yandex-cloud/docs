[Документация Yandex Cloud](../../index.md) > [Terraform в Yandex Cloud](../index.md) > Справочник Terraform > Ресурсы (англ.) > Cloud DNS > Resources > dns_zone_iam_binding
...
**Warning:** This resource is authoritative for the given `role` on the target `dns_zone` and manages the complete set of its members. When you change or delete `yandex_dns_zone_iam_binding`, the `role` may be removed from other subjects on the `dns_zone` as well — including subjects granted outside of this resource (via the corresponding `*_iam_member` resource, the management console, CLI or API). Those subjects are not tracked in the Terraform state, so a plain `terraform plan` does not list them. Be careful.
...
- `dns_zone_id` (**Required**)(String). The ID of the compute `dns_zone` to attach the policy to.
...
- `role` (**Required**)(String). The role that should be assigned. Only one yandex_dns_zone_iam_binding can be used per role.
