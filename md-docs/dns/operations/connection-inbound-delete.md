...
Если входящее DNS-подключение используется в DNS-настройках приватных подсетей BareMetal, сначала [исключите его из настроек](../../baremetal/operations/configure-dns.md#disable-dns) всех таких подсетей. Пока подключение используется хотя бы одной подсетью, удалить его нельзя.
...
- CLI {#cli}
  ...
  1. Удалите входящее DNS-подключение, указав его имя или идентификатор:
      ...
      Если для удаляемого входящего DNS-подключения активирована опция защиты от удаления, перед удалением отключите эту опцию. Подробнее об изменении DNS-подключения в разделе [Изменить входящее DNS-подключение](connection-inbound-update.md).
