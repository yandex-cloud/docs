...
1. Корпоративная сеть:
    ...
    * В подсети `subnet1` размещен DNS-сервер (DNS-форвардер) с IP-адресом `172.16.1.200`.
        ...
        Этот сервер обслуживает DNS-зону в подсети `subnet1` и перенаправляет DNS-запросы вашего компьютера с IP-адресом `172.16.1.10` в облачную сеть на IP-адрес [входящего DNS-подключения](../concepts/dns-connection.md#dns-inbound), созданного на стороне Yandex Cloud.
