[Документация Yandex Cloud](../../index.md) > [Yandex Cloud DNS](../index.md) > [Концепции](index.md) > Ресурсные записи
...
Подробнее об A-записях в [RFC-1035](https://www.ietf.org/rfc/rfc1035.html#section-3.4.1).
...
Подробнее об AAAA-записях в [RFC-3596](https://tools.ietf.org/html/rfc3596).
...
Подробнее о CAA-записях в [RFC-8659](https://tools.ietf.org/html/rfc8659).
...
Обратите внимание, что CNAME нельзя совмещать с другими записями в одном домене. Подробнее о CNAME-записях и их [ограничениях](https://www.rfc-editor.org/rfc/rfc1912#section-2.4) в [RFC-1035](https://www.ietf.org/rfc/rfc1035.html#section-3.3.1).
...
Подробнее о MX-записях в [RFC-1035](https://www.ietf.org/rfc/rfc1035.html#section-3.3.9).
...
Подробнее о NS-записях в [RFC-1035](https://www.ietf.org/rfc/rfc1035.html#section-3.3.11).
...
Подробнее о PTR-записях в [RFC-1035](https://www.ietf.org/rfc/rfc1035.html#section-3.3.12).
...
Подробнее о SOA-записях в [RFC-1035](https://www.ietf.org/rfc/rfc1035.html#section-3.3.13).
...
Подробнее о SRV-записях в [RFC-2782](https://www.ietf.org/rfc/rfc2782.html).
...
Подробнее об SVCB-записях в [RFC-9460](https://www.ietf.org/rfc/rfc9460.html), [RFC-9461](https://www.ietf.org/rfc/rfc9461.html).
...
* Сервис использует формат [MASTER FILES](https://www.ietf.org/rfc/rfc1035.html#section-5). Согласно спецификации этого формата, с символа `;` вне двойных кавычек начинается комментарий, т. е. все содержимое после него игнорируется. Если вы хотите использовать символ `;` в значении TXT-записи, заключите содержащую его строку в двойные кавычки `""`.
...
Подробнее о TXT-записях в [RFC-1035](https://www.ietf.org/rfc/rfc1035.html#section-3.3.14) и [RFC-1464](https://tools.ietf.org/html/rfc1464).

### Автоматическое разбиение длинных TXT-записей {#txt-splitting}

При создании TXT-записи API Cloud DNS автоматически разбивает длинные значения на текстовые блоки, учитывая уже заданное разбиение. Непрерывная последовательность символов без пробелов и кавычек считается отдельным блоком. Текст в двойных кавычках также считается одним блоком, даже если содержит пробелы.

Каждый блок обрабатывается отдельно:

* Если содержимое блока не превышает 255 символов, блок сохраняется без изменений.
* Если блок длиннее 255 символов, он разбивается слева направо на части по 255 символов. Последняя часть содержит остаток и может быть короче.
* Если длина исходного блока кратна 255, дополнительный пустой блок не создается.

Порядок данных сохраняется, а полученные части остаются блоками одной TXT-записи. Отдельные TXT-записи для этих частей не создаются. Например, блок из 300 символов разбивается на части длиной 255 и 45 символов, а блок из 510 символов — на две части по 255 символов.

Внешние двойные кавычки относятся к текстовому представлению записи и не учитываются при подсчете длины. Если исходный блок был заключен в кавычки, каждая полученная часть также заключается в отдельную пару кавычек. Пробелы между блоками используются как разделители и не входят в их содержимое, а пробелы внутри кавычек считаются частью значения.

Если каждый из существующих блоков содержит не более 255 символов, значение не изменяется, даже если общая длина TXT-записи превышает 255 символов. При этом сохраняется ограничение на общую длину значения TXT-записи — 1024 знака.

Автоматическое разбиение применяется только к данным перед символом `;` вне двойных кавычек. Комментарий после этого символа не входит в данные DNS-записи и не возвращается DNS-резолвером. Символ `;` внутри двойных кавычек считается обычной частью значения.

Ограничение на длину блока связано с форматом DNS. Согласно [разделу 3.3 RFC-1035](https://www.rfc-editor.org/rfc/rfc1035.html#section-3.3), длина одного блока (`character-string`) кодируется одним октетом, поэтому блок может содержать не более 255 октетов данных. Для поддерживаемых сервисом ASCII-символов это соответствует 255 символам. При этом [раздел 3.3.14 RFC-1035](https://www.rfc-editor.org/rfc/rfc1035.html#section-3.3.14) разрешает включать в одну TXT-запись несколько таких блоков. Правила обработки комментариев описаны в [разделе 5.1 RFC-1035](https://www.rfc-editor.org/rfc/rfc1035.html#section-5.1).
