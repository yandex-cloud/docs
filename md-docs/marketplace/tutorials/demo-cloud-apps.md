# Создание демонстрационного продукта {{ cloud-apps-name }}
...
1. Заполните анкету на [странице {{ marketplace-full-name }}](https://yandex.cloud/ru/marketplace).
...
1. Нажимая кнопку ![image](../../_assets/console-icons/plus.svg) **Добавить ресурс**, последовательно добавьте следующие ресурсы:
    ...
    * `Ресурс пользователя` → `Версия секрета {{ lockbox-short-name }}`:<br>

        Параметр | Значение | Описание
        --- | --- | ---
        Имя | `lockbox-secret-with-db-password` | Чтобы изменить имя, нажмите ![image](../../_assets/console-icons/pencil.svg) рядом с именем версии секрета {{ lockbox-name }}, которое было сгенерировано автоматически. Имя ресурса должно быть уникальным в рамках конфигурации. Пользователь увидит его при установке приложения.
...
1. Убедитесь, что все ресурсы созданы. [Подключитесь](../../compute/operations/vm-connect/ssh.md) к ВМ и [проверьте](../../managed-postgresql/operations/connect/index.md), что с нее есть доступ к кластеру {{ mpg-full-name }}.