# Практические руководства Cloud Marketplace

* [Развертывание S2S VPN с помощью strongSwan в Yandex Cloud](s2s-vpn.md)
* [Установка Ghost CMS High Availability с помощью Cloud Apps](ghost-cms-cloudapp.md)
* [Установка WordPress High Availability с помощью Cloud Apps](wordpress-ha-cloudapp.md)
* [Установка WordPress с помощью Cloud Apps](wordpress-cloudapp.md)

## Использование продуктов Cloud Marketplace в Yandex Managed Service for Kubernetes {#marketplace-tutorials}

* [Интеграция с Argo CD](argo-cd.md)
* [Интеграция с Crossplane](crossplane.md)
* [Синхронизация с секретами Yandex Lockbox](kubernetes-lockbox-secrets.md)
* [Настройка Fluent Bit для работы с Yandex Cloud Logging](fluent-bit-logging.md)
* [Настройка Gateway API](gateway-api.md)
* [Настройка L7-балансировщика Yandex Application Load Balancer с помощью Ingress-контроллера](alb-ingress-controller.md)
* [Настройка логирования для L7-балансировщика Yandex Application Load Balancer с помощью Ingress-контроллера](alb-ingress-controller-log-options.md)
* [Проверка состояния приложений в кластере Yandex Managed Service for Kubernetes с помощью L7-балансировщика Yandex Application Load Balancer](custom-health-checks.md)
* [Создание L7-балансировщика с профилем безопасности Smart Web Security через Ingress-контроллер Application Load Balancer](alb-ingress-with-sws-profile.md)
* [Настройка Kyverno & Kyverno Policies](kyverno.md)
* [Использование Metrics Provider для трансляции метрик](metrics-provider.md)
* [Редактирование изображений для сайтов с помощью приложения Thumbor](thumbor.md)
* [Использование Istio](istio.md)
* [Использование HashiCorp Vault для хранения секретов](hashicorp-vault.md)