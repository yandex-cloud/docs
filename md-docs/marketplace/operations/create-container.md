...
Вы также можете создать продукты для Yandex Compute Cloud на базе ОС [Linux](create-image.md), воспользовавшись соответствующими рекомендациями.
...
По умолчанию во время публикации все Docker-образы, входящие в продукт, из реестра издателя перемещаются в публично доступный реестр `yc-marketplace`. При этом вся иерархия продукта, определенная издателем, сохраняется. Чтобы не перемещать Docker-образы в реестр `yc-marketplace`, используйте параметр `private_artifacts` в [спецификации продукта](#manifest).
...
Helm-чарт должен содержать файл `values.yaml`, в котором в виде параметров будут перечислены все используемые Docker-образы. Имена Docker-образов в файле `values.yaml` должны начинаться с префикса `.Values` и указывать на Docker-образы, размещенные в реестре издателя, чтобы публикация и дальнейшая установка продукта в кластере пользователя прошли без ошибок.

{% note info %}

Marketplace устанавливает продукты для Managed Service for Kubernetes в кластеры, используя опцию `atomic` в Helm.

{% endnote %}

Все используемые Docker-образы должны быть заданы как `values` и перечислены в спецификации продукта. Это необходимо, так как в процессе публикации они будут перемещаться. Docker-образы, не указанные в спецификации продукта или описанные только в шаблонах, могут оказаться недоступны для пользователей при установке продукта — например, в кластер с особенностями конфигурации или без доступа в интернет.

Вы можете собирать в шаблоне указание на Docker-образ, например, меняя тег в зависимости от переменной. Чтобы это работало, в явном виде опишите в `values` и спецификации продукта все Docker-образы, которые могут понадобиться, а в шаблоне используйте ссылки на значения из `values`.

{% note warning %}

Для пространства имен `default` применяются более строгие политики безопасности, которые могут повлиять на работоспособность продукта. Если работа вашего продукта в `default` не гарантируется, необходимо указать в инструкции по развертыванию, что пользователь должен устанавливать его в другое пространство имен.

{% endnote %}


### Спецификация пода {#pod-spec}
...
Спецификация пода, в котором имя Docker-образа заменено на переменную YAML path, описанную в файле `values.yaml`:
...
### Зависимые Helm-чарты {#dependent-charts}

Включайте зависимые Helm-чарты в состав родительского, используя пути относительно корня родительского Helm-чарта, например:
...
dependencies:
  - name: redis
    version: 23.1.3
    repository: "file://charts/redis"  ## "oci://..." — нельзя.
  - name: common
    version: 2.0.0
    repository: "file://charts/common"
```

Docker-образы, используемые зависимыми Helm-чартами, должны быть параметризованы в `values.yaml` родительского Helm-чарта и описаны в спецификации продукта. Подробнее в статье [Scope, Dependencies, and Values](https://helm.sh/docs/topics/charts/#scope-dependencies-and-values).

Для удобства можно хранить Docker-образы в неплоской структуре реестра. `<chart>` и `<component-name>` могут содержать `/`, то есть иметь вложенные директории, поэтому должны выполняться условия:

* Helm-чарт находится в `cr.yandex/<registry-id>/<vendor-name>/<product-name>/<chart>`.
* Docker-образы находятся в `cr.yandex/<registry-id>/<vendor-name>/<product-name>/<component-name>:<tag>`.


### Пустое значение ключа {#empty-key}

При описании Helm-чартов в формате YAML вы можете использовать пустые значения для ключей в `values` и в то же время позволить задавать значение того же ключа в спецификации продукта.

По умолчанию пустое значение приравнивается к пустой строке. Если требуется не строка, а объект, то задавайте пустое значение таким образом:
...
## Внесение изменений в Helm-чарт {#update-helm-chart}

Helm-чарты кешируются на стороне бэкенда. Чтобы внести изменения в Helm-чарт, загрузите его заново с новым тегом и обновите тег в спецификации продукта.

Helm, в отличие от Docker, корректно поддерживает SemVer. Поэтому: `1.2.3-alpha` < `1.2.3` < `1.2.3+yandex`.


## Особенности среды выполнения {#runtime-features}

На кластерах Managed Service for Kubernetes, кроме устаревших кластеров, в качестве Container Runtime используется containerd. Сокет доступен по пути `/var/run/containerd/containerd.sock`.


## Спецификация продукта {#manifest}

Для публикации продукта необходима спецификация — документ, в котором будут описаны параметры развертывания продукта. Подготовьте спецификацию и [загрузите](../../storage/operations/objects/upload.md) ее в Yandex Object Storage.
...
1. `helm_charts` — обязательное поле. Содержит список имен и тегов Helm-чарта продукта. Можно указать только один Helm-чарт.
   ...
   helm_charts:
     - name: cr.yandex/b1gq90dgh25********/Vendor/Product/chart
       ...
       # Указывает на то, что все заданные values относятся к этому Helm-чарту.
       reuse_values: true
   ```

   В Helm-чарте должно быть указано поле `images`. Содержит список метаданных Docker-образов, входящих в продукт. Значения переменных параметров метаданных Docker-образа — это ссылки в формате YAML Path на переменные из файла `values.yaml`. Записи могут быть оформлены в одном из форматов:
   
   * Имя Docker-образа, адрес реестра и тег описаны отдельными полями:
   ...
   * Имя Docker-образа и адрес реестра описаны в одном поле, тег — в другом:
   ...
   * Описан полный путь до Docker-образа:
...
1. `requirements` — обязательное поле. Обязательные параметры кластера, в котором продукт будет разворачиваться. Раздел должен содержать параметр `min_k8s_version`, определяющий диапазон поддерживаемых версий Kubernetes.
...
1. `user_values` — необязательный параметр. Список переменных продукта, которые пользователь может переопределить во время установки или редактирования уже установленного продукта через консоль управления Yandex Cloud.

   {% note tip %}

   При перечислении в спецификации продукта значений, для которых будет запрошен пользовательский ввод, рекомендуется использовать правильные типы. Например, технически можно использовать `string` вместо `networkid`, но во втором случае пользователю будет предложено окно с выбором, а после ввода произойдет валидация. Также настоятельно рекомендуется указывать `secret: true` для чувствительных значений, которые, например, не должны попасть в логи.

   {% endnote %}

   Каждая переменная описывается обязательными полями:
   ...
   * Тип переменной. Доступны значения:
      ...
      * `clusterid` — идентификатор [кластера Kubernetes](../../managed-kubernetes/concepts/index.md#kubernetes-cluster).
      ...
      * `networkid` — идентификатор [сети](../../vpc/concepts/network.md#network) Yandex Virtual Private Cloud.
      ...
      * `subnetid` — идентификатор [подсети](../../vpc/concepts/network.md#subnet) Virtual Private Cloud.
      ...
      * `service_account_aws_key_value` — [статический ключ](../../iam/concepts/authorization/access-key.md) сервисного аккаунта для доступа к Object Storage. Передается в формате JSON.
      ...
      * `loggroupidvalue` — идентификатор [лог-группы](../../logging/concepts/log-group.md) Yandex Cloud Logging.
      ...
      * `kmskeyidvalue` — содержимое [ключа Yandex Key Management Service](../../kms/concepts/key.md).
      ...
      * `storagebucketvalue` — [бакет](../../storage/concepts/bucket.md) Object Storage.
...
1. `private_artifacts` — необязательный параметр. Позволяет не перемещать все Docker-образы, входящие в продукт, из реестра издателя в публично доступный реестр `yc-marketplace`. Доступны значения `true` и `false`, значение по умолчанию — `false`.

   Параметр задается для версии продукта. Версию, для которой задан параметр `private_artifacts = true`, пользователь сможет установить только с помощью [интерфейсов](../../overview/concepts/interfaces.md) Yandex Cloud. Установка с помощью команды `helm install` будет недоступна.

   Доступ на скачивание компонентов, опубликованных на карточке продукта в разделе **Состав продукта**, будет выдаваться только сервисному аккаунту, от имени которого пользователь устанавливает продукт. Если пользователь удалит продукт, Marketplace отзовет доступ у сервисного аккаунта.
