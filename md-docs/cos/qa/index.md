...
Обновление [Docker-контейнера](https://yandex.cloud/ru/blog/posts/2022/03/docker-containers) не будет работать с [тегами](../../container-registry/concepts/docker-image.md#version), которые переходят от версии к версии. Мы рекомендуем использовать новый тег для каждого [Docker-образа](../concepts/docker-image.md).
...
В [Docker Compose спецификации](../concepts/coi-specifications.md#compose-spec) не поддерживается инструкция `build`: Docker-образ должен быть собран заранее. Кроме того, в Docker Compose спецификации не должно быть ссылок на другие файлы: они не доставляются на ВМ.
...
* Вы также можете посмотреть [ошибки в логах](../error/index.md), выполнив на ВМ команду `sudo journalctl -eu yc-container-daemon`.
...
* Если нужны другие настройки [сети](../../vpc/concepts/network.md#network) для Docker-контейнеров, можно сделать это, используя Container Optimized Image с Docker Compose спецификацией. 
