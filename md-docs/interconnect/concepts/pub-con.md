[Документация Yandex Cloud](../../index.md) > [Yandex Cloud Interconnect](../index.md) > [Концепции](index.md) > Публичное соединение
...
Для повышения уровня защиты BGP-соединения можно использовать BGP-аутентификацию с использованием механизма `BGP MD5 password`. При включении данной функциональности рекомендуется использовать в качестве пароля строку длиной более 20 символов, состоящую из латинских букв, цифр и специальных символов: `_`, `.`, `<`, `>`, `{`, `}`, `(`, `)`, `-`.
...
Протокол BFD всегда включен со стороны оборудования Yandex Cloud со следующими значениями параметров:
...
Значение таймеров в секундах, которое настроено на оборудовании Yandex Cloud по умолчанию: `minimum-hold-time` = `90`.
