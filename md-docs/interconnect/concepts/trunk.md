[Документация Yandex Cloud](../../index.md) > [Yandex Cloud Interconnect](../index.md) > [Концепции](index.md) > Транковое подключение
...
В [точке присутствия](pops.md) для клиента может быть организовано одно или несколько транковых подключений. При этом, организация нескольких [приватных соединений](priv-con.md) в одну облачную сеть через разные транки на одной точке присутствия не допускается. 


Для обеспечения отказоустойчивости рекомендуем организовать несколько транковых подключений (не менее двух) — по одному в каждой [точке присутствия](pops.md).
...
## Агрегирование физических портов на оборудовании Yandex Cloud {#lag}
...
* [Подключение через партнера](#partner-link) — у вас нет собственного оборудования в ТП, и вы используете услуги компании-партнера Yandex Cloud, которая организует для вас подключение к оборудованию Yandex Cloud.
...
### Транковое подключение через партнера {#partner-link}

Использование **транкового подключения через партнера** упрощает подключение к Yandex Cloud за счет:
* Сокращения времени подключения услуги CIC.
* Снижения эксплуатационных затрат на организацию кроссировок.

Через такие транковые подключения можно организовывать как [приватные](priv-con.md), так и [публичные](pub-con.md) соединения.
...
Для транкового подключения через партнера существует ограничение скорости передачи пакетов в секунду (Packets Per Second), которое рассчитывается по формуле:

`PpsRate (Pps) = Capacity (Mbps) × 101.166667`

Где:

* `PpsRate` — максимальная скорость передачи пакетов в секунду (Pps);
* `Capacity` — пропускная способность транкового подключения в мегабитах в секунду (Mbps).
...
* [Подключение через партнера](#mux-partner).
...
Транковое подключение через партнера организуется с помощью технологии **802.1ad (QinQ)**, которая использует стек из двух VLAN-тегов:

* **верхний VLAN-тег** (`C-VLAN`) — идентификатор клиента в рамках [транкового подключения через партнера](#partner-link). Определяется значением VLAN-ID из пула, заданного партнером при организации транка. Тег `C-VLAN` позволяет сетевому оборудованию Yandex Cloud отделять трафик одного клиента от другого в рамках отдельного транкового подключения через партнера. Данный тег назначается транковому подключению через партнера в момент создания этого подключения.
* **нижний VLAN-тег** (`PRC-VLAN`) — определяется значением VLAN-ID [приватного соединения](priv-con.md) в транковом подключении. Задается при создании приватного соединения.

Теги `C-VLAN` и `PRC-VLAN` в стеке могут совпадать по значению. Схема связи между партнером и клиентом согласуется ими напрямую, без участия Yandex Cloud.

Сетевое оборудование партнера должно:

* снимать `C-VLAN`-тег при получении QinQ-Ethernet-кадров от оборудования Yandex Cloud и отдавать в направлении сетевого оборудования клиента обычный 802.1q-транк;
* добавлять соответствующий `C-VLAN`-тег при получении 802.1q-Ethernet-кадра от клиента, после чего QinQ-Ethernet-кадры могут передаваться в направлении оборудования Yandex Cloud.

Как в случае прямого клиентского подключения, так и в случае транкового подключения через партнера, клиент должен всегда получать «чистый» 802.1q-транк (без `C-VLAN`-тега) с набором VLAN-ID, который соответствует набору сконфигурированных клиентом приватных соединений.
