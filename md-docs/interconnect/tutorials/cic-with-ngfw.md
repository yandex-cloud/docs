...
Для организации сетевой IP-связности между ресурсами в собственной инфраструктуре клиента и облачными ресурсами в {{ yandex-cloud }} может использоваться услуга [{{ interconnect-full-name }}](../concepts/index.md).

В данном документе рассмотрены настройки [маршрутизации](../../vpc/concepts/routing.md) в облачной сети и настройки [приватного соединения](../concepts/priv-con.md) {{ interconnect-name }}, которые позволяют обеспечить сетевую связность инфраструктуры клиента с сегментами, размещенными за NGFW.
...
Познакомьтесь с вариантами организации услуги {{ interconnect-name }} в [документации](../concepts/index.md). Для обеспечения отказоустойчивости подключения к услуге рекомендуется организация нескольких транков — по одному в каждой [точке присутствия](../concepts/pops.md).

Воспользуйтесь [инструкцией](../operations/index.md) для организации приватного соединения в зависимости от Вашего сценария подключения к услуге {{ interconnect-name }}.
