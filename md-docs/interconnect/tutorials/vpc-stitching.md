[Документация Yandex Cloud](../../index.md) > [Yandex Cloud Interconnect](../index.md) > [Практические руководства](index.md) > Организация сетевой связности между несколькими виртуальными сетями с помощью VPC Stitching
...
Функция доступна только при подключенной услуге [Cloud Interconnect](../index.md) с объемом трафика не менее [300 ТБ](../pricing.md).
...
Для VPC Stitching не предоставляется SLA. Объем переданного через VPC Stitching трафика нельзя отслеживать средствами мониторинга. Передача данных сверх [емкости](../concepts/capacity.md) транкового подключения возможна, но не гарантируется.
...
* у вас есть роли [cic.editor](../security/index.md#cic-editor) и [cloud-router.editor](../../cloud-router/security/index.md#cloudrouter-editor) в каталогах с ресурсами Cloud Interconnect и Cloud Router соответственно;
* если вы будете использовать Yandex Cloud CLI, он [установлен и настроен](../../cli/quickstart.md).

Для VPC Stitching необходимы:

* [транковое подключение](../concepts/trunk.md) с объемом данных не менее 300 ТБ, что соответствует емкости 1 Гбит/с;
* [приватное соединение](../concepts/priv-con.md), созданное в этом транковом подключении со специальными параметрами VPC Stitching;
...
Если у вас уже есть транковое подключение с объемом данных не менее 300 ТБ, перейдите к [созданию приватного соединения](#create-prc). Если объем данных меньше 300 ТБ, [измените емкость подключения](../operations/trunk-update.md#capacity) и дождитесь подтверждения поддержки.
...
Дождитесь подтверждения поддержки. После создания подключения [получите его идентификатор](../operations/trunk-get-info.md) — он понадобится при создании приватного соединения.
...
- Консоль управления {#console}
  ...
  1. В открывшемся окне:
      ...
      1. Нажмите **Создать**.
  1. [Получите идентификатор](../operations/priv-con-get-info.md) созданного приватного соединения.
