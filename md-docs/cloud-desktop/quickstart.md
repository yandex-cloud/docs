...
В {{ cloud-desktop-name }} управление доступом реализовано с помощью [разграничения ролей {{ iam-full-name }}](security/index.md) и [списков управления доступом (ACL)](concepts/acl.md). См. [пример использования механизмов доступа](concepts/acl.md#example).
