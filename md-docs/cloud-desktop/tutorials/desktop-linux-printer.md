...
Это руководство поможет настроить печать файлов из [{{ cloud-desktop-full-name }}](../index.md) на принтер, подключенный к локальной машине под управлением Linux. Решение основано на использовании общего каталога: задания на печать из облачной среды сохраняются в общий каталог, а локальная система перехватывает их и направляет на физический принтер.
...
* Плата за использование вычислительных ресурсов, дисков и исходящий трафик рабочего стола (см. [тарифы {{ cloud-desktop-full-name }}](../pricing.md)).
...
1. [Создайте](../operations/desktop-groups/create.md) группу рабочих столов. При создании выберите образ с ОС на базе Linux, например Ubuntu 20.04.
...
1. [Создайте](../operations/desktops/create.md) рабочий стол.
...
- Linux {#linux}
  ...
  1. Создайте файл `install-print-watcher.sh` и вставьте в него код скрипта:
      ...
      При запсуке скрипт `install-print-watcher.sh` создаст автоматическую систему для перехвата PDF-файлов из удаленного рабочего стола и печати на локальном принтере. Он установит фоновую службу, которая следит за появлением файлов в специальной папке, и будет автоматически отправлять их на печать через систему CUPS. Скрипт требует права администратора для установки системной службы и использует стандартные инструменты Linux.

  1. Сделайте файл исполняемым и запустите скрипт:
      ...
      * `--user` — имя пользователя локальной машины. Обязательный параметр.
      * `--dir` — папка, за которой нужно следить. По умолчанию `/srv/printdrop`.
      * `--printer` — имя принтера CUPS. Если не указано, будет использоваться принтер, который установлен в системе по умолчанию.
      * `--fwdir` — имя папки на удаленном рабочем столе для пересылки. По умолчанию `yc-print`.
...
- Linux {#linux}
  ...
  1. [Подключитесь](../operations/desktops/connect.md) к рабочему столу.
  1. Создайте файл `install-printdrop-backend.sh` и вставьте в него код скрипта:
  ...
  1. Сделайте файл исполняемым и запустите скрипт:
      ...
      * `--user` — имя пользователя рабочего стола, у которого есть права на папку, в которую будут помещаться задания на печать. Утилита будет переключаться на него при выполнении. Обязательный параметр.
      * `--dir` — папка, в которую будут помещаться задания на печать. По умолчанию `yc-print`.
      * `--printer` — имя принтера CUPS, который нужно создать. По умолчанию `DropPrinter`.

  1. Убедитесь, что принтер `DropPrinter` создан:
...
- Linux {#linux}
  ...
  1. На рабочем столе отправьте на печать PDF-файл, выбрав созданный виртуальный принтер `DropPrinter`.

      В результате отобразится сообщение о поступлении файла в очередь печати.

  1. На локальной машине проверьте файлы, поступившие на печать с рабочего стола:
...
1. [Удалите](../operations/desktops/delete.md) рабочий стол.

При необходимости удалите [подсети](../../vpc/operations/subnet-delete.md), [сеть](../../vpc/operations/network-delete.md) и [группу рабочих столов](../operations/desktop-groups/delete.md).