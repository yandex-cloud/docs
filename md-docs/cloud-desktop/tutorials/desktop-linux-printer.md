...
- Linux {#linux}
  ...
  1. Создайте файл `install-print-watcher.sh` и вставьте в него код скрипта:
      ...
      При запуске скрипт `install-print-watcher.sh` создаст автоматическую систему для перехвата PDF-файлов из удаленного рабочего стола и печати на локальном принтере. Он установит фоновую службу, которая следит за появлением файлов в специальной папке, и будет автоматически отправлять их на печать через систему CUPS. Скрипт требует права администратора для установки системной службы и использует стандартные инструменты Linux.
