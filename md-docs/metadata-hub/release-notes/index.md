[Документация Yandex Cloud](../../index.md) > [Yandex MetaData Hub](../index.md) > История изменений
...
### Метаданные Trino и Apache Airflow™



Теперь вы можете загружать метаданные в Data Catalog из источников Trino и Apache Airflow™. Из Trino собираются метаданные о структуре данных: каталоги, схемы, таблицы, колонки с типами, партиции, статистика таблиц, а также история запросов. Из Apache Airflow™ собираются метаданные об оркестрации: структура DAG-файлов и задач, их зависимости, расписание, статусы выполнения, а также lineage-события (через `OpenLineage`).
...
Теперь пользователям Data Catalog доступно удобное пространство для управления разметкой данных с режимом массового редактирования и отображением статистики по разметке.
...
Настраивайте MCP-сервер для Data Catalog и подключайте AI-агентов для новых возможностей:
...
Поддержаны новые типы источников данных: Trino и Apache Airflow™. Из Trino собираются метаданные о структуре данных: каталоги, схемы, таблицы, колонки с типами, партиции, статистика таблиц, а также история запросов. Из Apache Airflow™ собираются метаданные об оркестрации: структура DAG-файлов и задач, их зависимости, расписание, статусы выполнения, а также lineage-события (через `OpenLineage`).
...
* Добавлена возможность просматривать текущий статус источника данных на связях. Например, можно увидеть ошибки в Data Transfer и быстрее обнаружить проблемы с поставкой данных.
