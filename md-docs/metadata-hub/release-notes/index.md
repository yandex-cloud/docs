...
Теперь пользователям {{ data-catalog-name }} доступно удобное пространство для управления разметкой данных с режимом массового редактирования и отображением статистики по разметке.
...
В разделе **Хранилище данных** вы можете просматривать наборы данных, загруженные из нескольких источников, просматривать информацию об этих источниках и связях между данными.
...
### MCP-сервер метаданных

![image](../../_assets/metadata-hub/changelogs/data-catalog-mcp.png)

Настраивайте MCP-сервер для {{ data-catalog-name }} и подключайте AI-агентов для новых возможностей:
* поиска по метаданным;
* просмотра каталогов метаданных;
* получения метаданных по их идентификаторам;
* получения связей между метаданными.
...
Добавлен новый тип источника данных — [{{ datalens-name }}](../../datalens/index.md) — сервис бизнес-аналитики для визуализации данных. Теперь вы можете [настраивать загрузку](../operations/data-catalog/create-ingestion.md) данных из {{ datalens-name }} и [анализировать связи](../quickstart/data-analyst-quickstart.md) между датасетами.


### Январь {#jan26}

Параметры подключения к [{{ schema-registry-name }}](../concepts/schema-registry.md) теперь можно указывать в настройках источников данных [{{ KF }}](../../data-transfer/operations/endpoint/source/kafka.md) и [{{ yds-name }}](../../data-transfer/operations/endpoint/source/data-streams.md) в сервисе {{ data-transfer-name }}.
...
Пользователи [{{ data-catalog-name }}](../concepts/data-catalog.md) получили дополнительные возможности для анализа и управления метаданными:
...
Исправлена ошибка при использовании подключений к пользовательской инсталляции {{ SD }}.
...
* Добавлен MCP-сервер для {{ data-catalog-name }}, который позволяет подключать AI-агентов для поиска по метаданным, просмотра каталогов, получения метаданных по их идентификаторам и получения связей между метаданными.

* Появилась возможность добавлять запросы в {{ websql-name }}.
...
* В [{{ data-catalog-name }}](../concepts/data-catalog.md) добавлены новые источники данных: [{{ websql-name }}](../../websql/index.md), [{{ SD }}/{{ MG }}](../../storedoc/index.md), [{{ OS }}](../../managed-opensearch/index.md) и [{{ GP }}](../../managed-greenplum/index.md). Также пользователи могут [исполнять SQL-запросы](../operations/data-catalog/web-sql-query.md) с помощью сервиса {{ websql-name }}.
...
Поддержаны соединения для пользовательских инсталляций БД {{ RD }} и кластеров [{{ mrd-full-name }}](../../managed-valkey/concepts/index.md).

### Июль {#july}

#### Новые возможности {#new-features-july}

Добавлена интеграция [{{ connection-manager-name }}](../concepts/connection-manager.md) с [{{ mch-short-name }}](../../managed-clickhouse/index.md). Подключения для кластеров {{ mch-short-name }} [будут создаваться автоматически](../quickstart/connection-manager.md).
...
1. Добавлена интеграция [{{ connection-manager-name }}](../concepts/connection-manager.md) с [{{ mmy-short-name }}](../../managed-mysql/index.md). Подключения для кластеров {{ mmy-short-name }} [будут создаваться автоматически](../quickstart/connection-manager.md).
