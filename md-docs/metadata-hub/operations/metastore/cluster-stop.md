[Документация Yandex Cloud](../../../index.md) > [Yandex MetaData Hub](../../index.md) > Apache Hive™ Metastore > Пошаговые инструкции > Остановка и запуск кластера

# Остановка и запуск кластера Apache Hive™ Metastore

При необходимости вы можете остановить кластер Apache Hive™ Metastore и запустить его заново. При остановке кластера все данные в нем сохранятся: они снова станут доступны, когда вы запустите кластер.
...
- CLI {#cli}
  ...
  Чтобы остановить кластер Apache Hive™ Metastore, выполните команду:

  ```bash
  yc managed-metastore cluster stop <имя_или_идентификатор_кластера>
  ```
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [Cluster.Stop](../../api-ref/Cluster/stop.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
        ...
        curl \
            ...
            --url 'https://metastore.api.cloud.yandex.net/managed-metastore/v1/clusters/<идентификатор_кластера>:stop'
        ```
...
- CLI {#cli}
  ...
  Чтобы запустить кластер Apache Hive™ Metastore, выполните команду:

  ```bash
  yc managed-metastore cluster start <имя_или_идентификатор_кластера>
  ```
...
- REST API {#api}
    ...
    1. Воспользуйтесь методом [Cluster.Start](../../api-ref/Cluster/start.md) и выполните запрос, например, с помощью [cURL](https://curl.se/):
        ...
        curl \
            --request POST \
            --header "Authorization: Bearer $IAM_TOKEN" \
            --url 'https://metastore.api.cloud.yandex.net/managed-metastore/v1/clusters/<идентификатор_кластера>:start'
        ```
