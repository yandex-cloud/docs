...
Раздел содержит рекомендации пользователям {{ yandex-cloud }} по настройкам безопасности в [{{ managed-k8s-full-name }}](../../managed-kubernetes/index.md).
...
* интеграция с облачным сервисом [{{ certificate-manager-full-name }}](../../certificate-manager/index.md);
* отсутствие необходимости установки контроллера в кластер, так как все разворачивается на стороне [{{ alb-name }}](../../application-load-balancer/index.md).
...
* [Сервисные роли {{ managed-k8s-name }}](../../managed-kubernetes/security/index.md#yc-api) (доступ к {{ yandex-cloud }} API): позволяют управлять кластерами и группами узлов (например, создать кластер, создать/редактировать/удалить группу узлов и т.д.).
...
Работу с {{ k8s }} secrets рекомендуется выполнять с помощью решений класса SecretManager. В {{ yandex-cloud }} такое решением является сервис [{{ lockbox-name }}](../../lockbox/index.md).

Интеграция {{ lockbox-name }} с {{ k8s }} выполнена с помощью открытого проекта [External Secrets](https://external-secrets.io/latest/). Решение доступно в {{ marketplace-name }} в базовом упрощенном сценарии — [External Secrets Operator с поддержкой Yandex Lockbox](https://yandex.cloud/ru/marketplace/products/yc/external-secrets).
...
В {{ marketplace-name }} доступны плагин [Filebeat](https://yandex.cloud/ru/marketplace/products/yc/filebeat) для передачи логов в Elastic и [Fluent Bit с плагином {{ cloud-logging-name }}](https://yandex.cloud/ru/marketplace/products/yc/fluent-bit).
