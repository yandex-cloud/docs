...
&#x2713; **Регулярно проводите аудит доступов**: проверяйте права пользователей и сервисных аккаунтов с помощью [модуля {{ ciem-name }}](../../security-deck/concepts/ciem.md) или [{{ yandex-cloud }} CLI](../../cli/index.md), отзывайте избыточные роли и блокируйте либо удаляйте неиспользуемые учетные записи старше 30 дней.
...
&#x2713; **Отслеживайте секреты {{ yandex-cloud }} в открытых источниках**: сервис позволяет обнаруживать API-ключи, {{ iam-short-name }} Cookies, IAM-токены, статические ключи доступа и серверные ключи {{ captcha-full-name }}. [Подробнее](../operations/search-secrets.md).
