...
&#x2713; **Защитите аккаунты на Яндексе**:
   ...
   * Храните [OAuth-токен](../../iam/concepts/authorization/oauth-token.md) в секрете. Если токен скомпрометирован, [отзовите его](https://yandex.ru/dev/oauth/doc/dg/reference/token-invalidate-docpage/) и выпустите новый. По возможности используйте IAM-токен: он действует 12 часов, а OAuth-токен — один год.
...
&#x2713; **Отслеживайте секреты Yandex Cloud в открытых источниках**: сервис позволяет обнаруживать API-ключи, IAM Cookies, IAM-токены, статические ключи доступа, OAuth-токены и серверные ключи Yandex SmartCaptcha. [Подробнее](../operations/search-secrets.md).
