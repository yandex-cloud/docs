[Документация Yandex Cloud](../../index.md) > [Безопасность в Yandex Cloud](../index.md) > [Стандарт по защите облачной инфраструктуры, версия 1.4.4](index.md) > Версии

# Версии стандарта по защите облачной инфраструктуры Yandex Cloud

### Изменения в версии 1.4.4 {#current-version}

Дата публикации 16.09.2026.

* **Обновлены пункты:**

  * В пункте **1.4 Только необходимые администраторы управляют членством в IAM-группах** уточнены роли для управления составом групп. Указано, что роль `editor` не позволяет управлять членством в группе или назначать роли. Для управления составом конкретной группы рекомендована роль `organization-manager.groups.memberAdmin`.
  * В пункте **1.5 Используются сервисные роли вместо примитивных: admin, editor, viewer, auditor** рекомендованы сервисные роли для просмотра конфигурации и метаданных. В проверки примитивных ролей в консоли управления и Bash добавлена роль `auditor`.
  * В пункте **1.6 Используется роль auditor для исключения доступа к данным пользователей** добавлено предупреждение: безопаснее использовать сервисные роли уровня `auditor`, а при назначении примитивной роли `auditor` необходимо ограничивать набор ресурсов и учитывать наследование прав.
  * В пункте **1.17 Привилегированные роли назначены только доверенным администраторам** вместо примитивных ролей рекомендованы сервисные роли с минимально необходимыми разрешениями. Для работы с платежным аккаунтом указаны роли `billing.accounts.editor`, `billing.accounts.admin` и `billing.accounts.viewer` в зависимости от задач сотрудника; обновлена проверка привилегированных назначений.
  * В пункте **1.19 Доступ для подрядных организаций и третьих лиц контролируется** рекомендованы сервисные роли без доступа к данным вместо примитивной роли `auditor`.

### Изменения в версии 1.4.3 {#version-1-4-3}

Дата публикации 08.09.2026.

* Для требований, где есть [проверка в CSPM](../../security-deck/concepts/standard-compliance/yc-cloud-security-standard.md), добавлена сноска.

* **Добавлены пункты:**

  * 1.1.2 Доступ пользователей к ресурсам назначается через группы, а не напрямую.
  * 1.27 Настроена парольная политика для локальных учетных записей и административного доступа.
  * 4.2.1 В Managed Services for Databases используется шифрование данных at rest с ключом KMS.

* **Обновлены пункты:**

  * В пункте **2.1 Для объектов облака используется межсетевой экран или группы безопасности**:
      * Изменена логика скрипта: поиск осуществляется по ВМ без привязанных групп безопасности вместо идентификаторов продуктов в Cloud Marketplace.
      * Добавлено пояснение, что Yandex Cloud не может автоматически отслеживать конфигурацию BYOI.
  * В пункте **2.2 В Virtual Private Cloud создана группа безопасности и не используется группа безопасности по умолчанию** вместо проверки на использование группы безопасности по умолчанию скрипт ищет наличие хотя бы одной пользовательской группы безопасности.
  * В пунктах **2.3 В группах безопасности отсутствует слишком широкое правило доступа** и **2.4 Доступ по управляющим портам открыт только для доверенных IP-адресов** в скрипты добавлен блок `safe navigation` и логический оператор «ИЛИ», из результатов исключены правила со значением `null`.
  * В пункте **2.5 Включена защита от DDoS-атак**:
      * Разделено требование на базовую защиту (L3/L4) и прикладную защиту (L7).
      * Критичность понижена до «Информационная».
      * Добавлено предупреждение об асимметричном роутинге.
      * Исправлена логика jq-запроса для отсеивания внутренних адресов.
  * В пункте **2.6 Используется защищенный удаленный доступ** интегрировано требование использовать OS Login для доступа к ВМ и кластерам Kubernetes и добавлена проверка через CLI.
  * В пункте **2.9 Исходящий доступ в интернет контролируется** проверка преобразована в информационный радар исходящего доступа, а скрипт теперь находит все ВМ с публичными адресами и конфигурации управляемого NAT-шлюза.
  * В пункте **3.8 Отсутствует публичный доступ к бакету Object Storage** проверка переписана на `yc storage bucket get`, удалена зависимость от AWS CLI и статических ключей.
  * В пункте **3.9 В Object Storage используются политики доступа (Bucket Policy)** проверка переписана на `yc storage bucket get`, удалена зависимость от AWS CLI и статических ключей.
  * В пункте **3.10 В Object Storage включена функция «Блокировка версии объекта» (Object Lock)**:
      * Добавлено пояснение, что блокировка версии объекта не работает при выключенном версионировании.
      * Оба вывода проверки теперь должны содержать `Status: Enabled`, чтобы она завершилась успешно.
  * В пункте **3.10 В Object Storage включена функция «Блокировка версии объекта» (objectlock)** проверка переписана на `yc storage bucket get`, удалена зависимость от AWS CLI и статических ключей.
  * В пункте **3.20 Serverless Containers/Cloud Functions использует внутреннюю сеть VPC**:
      * В результате выполнения скрипта функции без заданного поля сети теперь собираются в массив.
      * Если массив пустой, пользователь получит сообщение `SUCCESS: Все функции привязаны к сети VPC`, иначе — `FAILURE`.
  * В пункте **3.26 Публичный доступ отсутствует для YDB**:
      * Переписан текст проверки.
      * Добавлено требование назначать группы безопасности на кластеры Dedicated Managed Service for YDB.
      * Описан риск компрометации данных через системный порт 8765.
      * CLI-скрипт проверки исправлен на использование `yc ydb database dedicated get`.
  * В пункте **3.35 Для подключения к виртуальной машине или узлу Kubernetes используется OS Login** в скрипт добавлены три уровня проверки: глобальный уровень, уровень ВМ, уровень Kubernetes.
  * В пункте **3.39 Используется Cloud Backup или механизм snapshot по расписанию** добавлена проверка через CLI и технические метрики проверки.
  * В пункте **4.2 В Yandex Object Storage включено шифрование данных at rest с ключом KMS** проверка переписана на `yc storage bucket get`, удалена зависимость от AWS CLI и статических ключей.
  * В пункте **4.11 Для KMS ключей включена ротация**:
      * Добавлено пояснение, что шифрование задается только при создании диска.
      * Для существующих ВМ добавлена инструкция по созданию диска из снимка.
      * Добавлена ссылка на инструкцию по вращению ключей KMS.
  * В пункте **7.5 В Managed Service for Kubernetes используется безопасная конфигурация** изменена логика проверки, внедрен механизм прерывания `short circuit`.

### Изменения в версии 1.4.2 {#version-1-4-2}
