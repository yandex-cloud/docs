...
В Object Storage реализовано несколько механизмов для управления доступом к ресурсам. Алгоритм взаимодействия этих механизмов смотрите в разделе [Обзор способов управления доступом в Object Storage](../../storage/security/overview.md).
...
Дополнительную информацию об отслеживании версий функций смотрите в разделе [Резервное копирование в Cloud Functions](../../functions/concepts/backup.md).
...
Для целей отладки функции, можно использовать специальные запросы, которые возвращают JSON-структуру запроса и необходимый для отладки результат. Подробнее в разделе [отладка функции](../../functions/concepts/function-invoke.md#example).
...
Подробнее о том какие роли действуют в сервисе API Gateway смотрите в разделе [Какие роли действуют в сервисе](../../api-gateway/security/index.md#roles-list).
...
Если API-шлюзу не требуется доступ к ресурсам из указанной облачной сети, удалите ее из настроек шлюза. Подробнее в статье [Изменить API-шлюз](../../api-gateway/operations/api-gw-update.md).
...
Подробнее в статье [Работа с API-шлюзом по протоколу WebSocket](../../api-gateway/tutorials/api-gw-websocket.md).
...
Рекомендуется включать механизм логирования при создании API-шлюза. Подробнее в статье [Записать логи в журнал выполнения в API Gateway](../../api-gateway/operations/api-gw-logs-write.md).
