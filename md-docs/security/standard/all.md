[Документация Yandex Cloud](../../index.md) > [Безопасность в Yandex Cloud](../index.md) > [Стандарт по защите облачной инфраструктуры, версия 1.4.4](index.md) > Все разделы на одной странице
...
Для управления доступом к ресурсам удобно использовать [группы пользователей](../../organization/operations/create-group.md). Контролируйте, кто может изменять состав группы: добавление пользователя в группу дает ему права, назначенные этой группе.

Управлять членством в группе позволяют следующие роли:

* `organization-manager.groups.memberAdmin`, `organization-manager.groups.admin` или `admin`, назначенные на конкретную группу или на организацию.
* `organization-manager.admin` или `organization-manager.organizations.owner`, назначенные на организацию.
...
Для управления составом группы [назначьте](../../organization/operations/access-manage-group.md) роль `organization-manager.groups.memberAdmin` на группу только необходимым администраторам. Удалите у остальных учетных записей роли, позволяющие управлять членством в группе. Унаследованные роли отзывайте на том ресурсе, на котором они назначены.
...
Для просмотра конфигурации и метаданных используйте сервисные роли без доступа к данным, например [compute.auditor](../../iam/roles-reference.md#compute-auditor). Выбирайте роли только для тех сервисов и ресурсов, с которыми пользователь работает.
...
- Проверка в консоли управления {#console}
  ...
  1. Откройте консоль Yandex Cloud в вашем браузере.
  1. Перейдите во вкладку **Все сервисы** → **Yandex Identity Hub** → **Пользователи**.
  1. Если у всех учетных записей в колонке **Права доступа** отсутствуют примитивные роли `admin`, `editor`, `viewer` и `auditor`, то рекомендация выполняется. Если нет, то перейдите к п. <q>Инструкции и решения по выполнению</q>.
  1. Далее перейдите в общее меню облака (нажать на облако в исходном меню облака). Выберите вкладку **Права доступа**.
  1. Если у всех учетных записей в колонке **Роли** отсутствуют примитивные роли `admin`, `editor`, `viewer` и `auditor`, то рекомендация выполняется. Если нет, то перейдите к п. <q>Инструкции и решения по выполнению</q>.
...
- Проверка через CLI {#cli}
  ...
  1. Выполните команду для поиска учетных записей с назначенными примитивными ролями на уровне организации:
      ...
      yc organization-manager organization list-access-bindings \
        --id=${ORG_ID} \
        --format=json | jq -r '.[] | select(.role_id=="admin" or .role_id=="editor" or .role_id=="viewer" or .role_id=="auditor")'
      ```
  ...
  1. Выполните команду для поиска учетных записей с назначенными примитивными ролями на уровне облаков:
      ...
      do yc resource-manager cloud list-access-bindings --id=$CLOUD_ID --format=json | jq -r '.[] | select(.role_id=="admin" or .role_id=="editor" or .role_id=="viewer" or .role_id=="auditor")'
      done
      ```
  ...
  1. Выполните команду для поиска учетных записей с назначенными примитивными ролями на уровне всех каталогов в ваших облаках:
      ...
      do yc resource-manager folder list-access-bindings --id=$FOLDER_ID --format=json | jq -r '.[] | select(.role_id=="admin" or .role_id=="editor" or .role_id=="viewer" or .role_id=="auditor")'
      done;
      done
      ```
...
Вместо примитивной роли `auditor` рекомендуется назначать сервисные роли уровня `auditor`, например [compute.auditor](../../iam/roles-reference.md#compute-auditor). Это более безопасный подход: он позволяет ограничить доступ ресурсами нужного сервиса.

Если вы используете примитивную роль `auditor`, внимательно выбирайте ресурсы, на которые ее назначаете. Эта роль действует во всех сервисах, а ее разрешения наследуются вложенными ресурсами. Назначайте ее только на минимально необходимый набор ресурсов.
...
* `billing.accounts.admin`, `billing.accounts.editor` и `admin`, назначенные на платежный аккаунт;
...
Для управления платежным аккаунтом назначьте роль [billing.accounts.editor](../../billing/security/index.md#billing-accounts-editor) на платежный аккаунт выделенному сотруднику организации с федеративным аккаунтом. Если сотруднику нужно управлять доступом к платежному аккаунту, назначьте роль [billing.accounts.admin](../../billing/security/index.md#billing-accounts-admin).

Для просмотра платежных данных назначьте роль [billing.accounts.viewer](../../billing/security/index.md#billing-accounts-viewer) на платежный аккаунт выделенному сотруднику организации с федеративным аккаунтом.
...
Для административных задач назначайте федеративным аккаунтам сервисные роли с минимально необходимыми разрешениями. Например, для управления доступом к платежному аккаунту используйте `billing.accounts.admin`. Выбирайте область назначения роли по задаче: отдельный ресурс, каталог или облако. Минимизируйте количество аккаунтов с этими ролями и регулярно перепроверяйте потребность в этих ролях для тех аккаунтов, которым они назначены.
...
- Проверка в консоли управления {#console}
  ...
  1. Проверьте, кому назначены роли: `billing.accounts.owner`, `billing.accounts.admin`, `billing.accounts.editor`, `admin`.
...
Подробнее читайте в разделе [Ключи сервисного аккаунта](../../iam/concepts/users/service-accounts.md#sa-key).
...
Подробнее о настройке сбора событий аудита на разных уровнях читайте в разделе [Сбор, мониторинг и анализ аудитных логов Managed Service for Kubernetes](../domains/kubernetes.md#collection-monitoring-analysis-audit-logs).
