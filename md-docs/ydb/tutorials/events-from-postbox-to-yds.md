[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for YDB](../index.md) > [Практические руководства](index.md) > Передача событий Yandex Cloud Postbox в Yandex Data Streams и их анализ с помощью Yandex DataLens
...
Реализовать предлагаемое решение можно с помощью [консоли управления](https://console.yandex.cloud) Yandex Cloud, даже не имея навыков разработки. Для обработки и хранения данных будут использоваться сервисы [Yandex Cloud Functions](../../functions/index.md) и [Yandex Managed Service for YDB](../index.md).
...
* `yds-functions` — от его имени будет вызываться [функция](../../functions/concepts/function.md) Cloud Functions и будут записываться данные в [базу данных](../concepts/resources.md#database) YDB;
...
В качестве хранилища событий может использоваться любая подходящая для задач аналитики СУБД. В данном руководстве используется [бессерверная база данных](../concepts/resources.md#serverless) Managed Service for YDB. Для других СУБД SQL-запрос создания таблицы и код функции загрузки событий в нее могут отличаться.
...
1. [Удалите](../operations/manage-databases.md#delete-db) базу данных Managed Service for YDB.
