...
Данные проекта хранятся в бакете [{{ objstorage-full-name }}](../../storage/index.md) и базе данных [{{ ydb-full-name }}](../index.md). Функции [{{ sf-full-name }}](../../functions/index.md) обрабатывают запросы, а [{{ api-gw-full-name }}](../../api-gateway/index.md) обеспечивает взаимодействие сервисов.
...
* плата за операции с {{ ydb-short-name }} и хранение данных (см. [тарифы {{ ydb-full-name }}](../pricing/serverless.md));
...
Создайте базу данных {{ ydb-short-name }} в [режиме Serverless](../concepts/serverless-and-dedicated.md#serverless):
