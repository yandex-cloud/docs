[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for YDB](../index.md) > [Практические руководства](index.md) > Построение пайплайна CI/CD в GitLab с использованием serverless-продуктов
...
1. Добавьте переменные окружения с выключенной опцией защиты. Все необходимые переменные и их значения были выведены в конце выполнения скрипта `bootstrap.sh`:
   ...
   * `DOCAPI_ENDPOINT_prod` — Document API эндпоинт БД [Yandex Managed Service for YDB](../index.md) в каталоге `prod`.
