...
Чтобы создать поток данных, в который будут отправляться события {{ postbox-name }}, потребуется база данных {{ ydb-name }}. Создайте [бессерверную базу данных](../concepts/resources.md#serverless) {{ ydb-name }}:
