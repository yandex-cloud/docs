...
1. Оплата за использование {{ ydb-short-name }} — операции с базой и хранение данных (см. [тарифы {{ ydb-full-name }}](../pricing/serverless.md)).
...
1. Перейдите в сервис **{{ ui-key.yacloud.iam.folder.dashboard.label_ydb }}** и откройте свою базу.
...
Чтобы освободить ресурсы, удалите тестовую базу {{ ydb-short-name }} по [инструкции](manage-databases.md#delete-db).