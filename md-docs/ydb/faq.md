...
## Ошибки при работе с базой данных {#errors}

### Что означает ошибка `RESOURCE_EXHAUSTED`? {#resource-exhausted}

Ошибка означает, что один из показателей потребления ресурсов базы данных достиг установленного ограничения.

1. На странице базы данных проверьте ограничения на вкладке **Обзор**.
1. В мониторинге сопоставьте показатели `Requests` и `Units consumed` с периодами высокой задержки `Latency`.
1. Если значение показателя `Units consumed` достигло квоты на потребление RU в секунду, [запросите увеличение квоты](https://console.yandex.cloud/cloud?section=quotas).
1. Если значение показателя `Units consumed` достигло ограничения пропускной способности Serverless БД, [увеличьте это ограничение](operations/manage-databases.md#update-db-serverless).
...
Квоту для каждой базы можно изменить в ее [настройках](operations/manage-databases.md#update-db-serverless). Если вам не хватает квоты на облако, обратитесь в [техническую поддержку](https://center.yandex.cloud/support).
