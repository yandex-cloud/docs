[Документация Yandex Cloud](../../index.md) > [Yandex Container Registry](../index.md) > [Практические руководства](index.md) > Развертывание gRPC-сервиса на основе Docker-образа
...
DataSphere может создать ноду из Docker-образа, который хранится в [Yandex Container Registry](../index.md). Чтобы загрузить образ, создайте реестр:
...
Чтобы сервисный аккаунт мог [аутентифицироваться в Yandex Container Registry](../operations/authentication.md), создайте авторизованный ключ.
...
- CLI {#cli}
   ...
   1. [Аутентифицируйтесь в Container Registry](../index.md).
   ...
   1. Загрузите Docker-образ в Container Registry, подставив идентификатор вашего реестра `datasphere-registry`:
      ...
      ```bash
      docker tag grpc-docker cr.yandex/<идентификатор_реестра>/grpc:v1
      docker push cr.yandex/<идентификатор_реестра>/grpc:v1
      ```
...
1. Создайте ноду. Для этого на странице проекта в правом верхнем углу нажмите кнопку **Создать ресурс**. Во всплывающем окне выберите **Нода**. Укажите настройки ноды:
   1. В поле **Имя** введите имя ноды — `grpc`.
...
1. [Удалите](../operations/registry/registry-delete.md) реестр Container Registry.