...
Вы можете [сканировать уязвимости](../concepts/vulnerability-scanner.md) [Docker-образов](../concepts/docker-image.md) в [{{ container-registry-full-name }}](../index.md) при непрерывном развертывании приложений [{{ managed-k8s-full-name }}](../../managed-kubernetes/index.md) через {{ GL }}.
...
* Плата за хранение созданных Docker-образов и сканер уязвимостей (см. [тарифы {{ container-registry-name }}](../pricing.md)).
...
- Вручную {#manual}
  ...
  1. [Создайте сервисные аккаунты](../../iam/operations/sa/create.md):
     ...
     * Для узлов с ролями [{{ roles-cr-puller }}](../security/index.md#container-registry-images-puller) и [{{ roles-cr-pusher }}](../security/index.md#container-registry-images-pusher) на каталог с [реестром](../concepts/registry.md) [Docker-образов](../concepts/docker-image.md). От его имени узлы {{ managed-k8s-name }} будут загружать в реестр собранные в {{ GL }} Docker-образы, а также скачивать их для запуска [подов](../../managed-kubernetes/concepts/index.md#pod).
  ...
  1. [Создайте реестр {{ container-registry-full-name }}](../operations/registry/registry-create.md).
...
- {{ TF }} {#tf}
  ...
  1. Скачайте в ту же рабочую директорию файл конфигурации [k8s-and-registry-for-gitlab.tf](https://github.com/yandex-cloud-examples/yc-mk8s-cr-gitlab/blob/main/k8s-and-registry-for-gitlab.tf).
     ...
     * [Реестр](../concepts/registry.md) [{{ container-registry-full-name }}](../index.md).
...
1. Создайте [переменные окружения {{ GL }}]({{ gl.docs }}/ee/ci/variables/):
   ...
   1. Добавьте переменные окружения в зависимости от способа аутентификации {{ managed-k8s-name }} в {{ GL }}:
      ...
      * `CI_REGISTRY` — адрес созданного ранее [реестра](../concepts/registry.md) в формате `{{ registry }}/<идентификатор_реестра>`.
...
1. [Удалите все Docker-образы](../operations/docker-image/docker-image-delete.md) из [реестра](../concepts/registry.md) {{ container-registry-name }}.
