...
В этом сценарии описано, как подписать [Docker-образы](../concepts/docker-image.md) с помощью [Cosign](https://docs.sigstore.dev/cosign/overview/) в [{{ container-registry-full-name }}](../index.md), а затем настроить проверку подписей в [{{ managed-k8s-full-name }}](../../managed-kubernetes/index.md) с помощью ключей {{ kms-full-name }}.
...
* Плата за [использование хранилища](../pricing.md) {{ container-registry-name }}.
...
- Вручную {#manual}
  ...
  1. [Создайте сервисные аккаунты](../../iam/operations/sa/create.md):
     ...
     * Сервисный аккаунт для узлов с ролью [{{ roles-cr-puller }}](../security/index.md#required-roles) на каталог с [реестром](../concepts/registry.md) Docker-образов. От его имени узлы будут скачивать из реестра необходимые Docker-образы.
  ...
  1. [Создайте реестр {{ container-registry-name }}](../operations/registry/registry-create.md).
...
1. [Настройте](../operations/configure-docker.md) Docker и [аутентифицируйтесь в {{ container-registry-name }}](../operations/authentication.md).
1. [Создайте несколько Docker-образов](../operations/docker-image/docker-image-create.md). Один из образов в дальнейшем будет подписан с помощью Cosign, остальные образы будут неподписанными.
1. [Загрузите Docker-образы](../operations/docker-image/docker-image-push.md) в реестр {{ container-registry-name }}.
...
- Подпись образа на асимметричных ключах {{ kms-name }}
  ...
  1. Подпишите образ в {{ container-registry-name }}:
     ...
     * `<идентификатор_реестра>` — [идентификатор реестра](../operations/registry/registry-list.md#registry-list) {{ container-registry-name }}, в котором находится подписываемый образ.
     * `<имя_Docker-образа>` — имя подписываемого [Docker-образа](../operations/docker-image/docker-image-list.md#docker-image-list) в реестре {{ container-registry-name }}.
  ...
  1. Вручную проверьте, что подпись Docker-образа корректна:
     ...
     * `<идентификатор_реестра>` — [идентификатор реестра](../operations/registry/registry-list.md#registry-list) {{ container-registry-name }}, в котором находится образ.
     * `<имя_Docker-образа>` — [имя Docker-образа](../operations/docker-image/docker-image-list.md#docker-image-list) в реестре {{ container-registry-name }}.
...
1. Создайте [авторизованный ключ](../../iam/concepts/authorization/key.md) для сервисного аккаунта с ролью [{{ roles-cr-puller }}](../security/index.md#required-roles) и запишите его в файл:
...
- Вручную {#manual}
  ...
  1. [Удалите все Docker-образы](../operations/docker-image/docker-image-delete.md) из реестра {{ container-registry-name }}.
  1. [Удалите реестр {{ container-registry-name }}](../operations/registry/registry-delete.md).
