[Документация Yandex Cloud](../../index.md) > [Yandex Container Registry](../index.md) > [Практические руководства](index.md) > Подпись и проверка Docker-образов в Managed Service for Kubernetes
...
* Сервис Container Registry: хранение созданных Docker-образов и использование сканера уязвимостей ([тарифы Container Registry](../pricing.md)).
...
- Вручную {#manual}
  ...
  1. [Создайте сервисные аккаунты](../../iam/operations/sa/create.md):
     ...
     * Сервисный аккаунт для узлов с ролью [container-registry.images.puller](../security/index.md#required-roles) на каталог с [реестром](../concepts/registry.md) Docker-образов. От его имени узлы будут скачивать из реестра необходимые Docker-образы.
...
1. [Настройте](../operations/configure-docker.md) Docker и [аутентифицируйтесь в Container Registry](../operations/authentication.md).
...
1. [Загрузите Docker-образы](../operations/docker-image/docker-image-push.md) в реестр Container Registry.
...
- Подпись образа на асимметричных ключах Key Management Service
  ...
  1. Подпишите образ в Container Registry:
     ...
     * `<идентификатор_реестра>` — [идентификатор реестра](../operations/registry/registry-list.md#registry-list) Container Registry, в котором находится подписываемый образ.
     * `<имя_Docker-образа>` — имя подписываемого [Docker-образа](../operations/docker-image/docker-image-list.md#docker-image-list) в реестре Container Registry.
     * `<тег>` — тег версии образа, которую требуется подписать.
     ...
     В реестре Container Registry должен появиться второй объект с тегом `sha256-....sig` и хешем `cr.yandex/<идентификатор_реестра>/<имя_Docker-образа>@sha256:...`.
  1. Вручную проверьте, что подпись Docker-образа корректна:
     ...
     * `<идентификатор_реестра>` — [идентификатор реестра](../operations/registry/registry-list.md#registry-list) Container Registry, в котором находится образ.
     * `<имя_Docker-образа>` — [имя Docker-образа](../operations/docker-image/docker-image-list.md#docker-image-list) в реестре Container Registry.
...
1. Создайте [авторизованный ключ](../../iam/concepts/authorization/key.md) для сервисного аккаунта с ролью [container-registry.images.puller](../security/index.md#required-roles) и запишите его в файл:
...
- Вручную {#manual}
  ...
  1. [Удалите все Docker-образы](../operations/docker-image/docker-image-delete.md) из реестра Container Registry.
