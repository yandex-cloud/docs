[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for Apache Spark™](../index.md) > [Практические руководства](index.md) > Интеграция с DataSphere

# Интеграция Managed Service for Apache Spark™ и Yandex DataSphere
...
1. [Создайте кластер Managed Service for Apache Spark™](../operations/cluster-create.md) любой подходящей конфигурации с параметрами:
...
1. Нажмите **Выбрать**.
   ...
   В ноутбуке через переменную `spark` станет доступна [локальная сессия PySpark](https://spark.apache.org/docs/3.5.6/api/python/reference/pyspark.sql/api/pyspark.sql.SparkSession.html). В кластере Apache Spark™ будет создано и запущено [задание](../concepts/index.md#job) SparkConnect для выполнения кода ячеек ноутбука.
