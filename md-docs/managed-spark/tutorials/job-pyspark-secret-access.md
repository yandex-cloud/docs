...
Вы можете использовать секрет [{{ lockbox-name }}](../../lockbox/concepts/secret.md) для подключения к кластеру [{{ mpg-full-name }}](../../managed-postgresql/index.md) из PySpark-задания в [{{ msp-full-name }}](../index.md). Для этого [сервисному аккаунту](../../iam/concepts/users/service-accounts.md) кластера {{ msp-full-name }} необходимо предоставить доступ к секрету. Секрет создается сервисом {{ connection-manager-full-name }} автоматически при создании пользователя {{ mpg-name }}.
...
* Кластер {{ msp-name }}: выделенные хостам вычислительные ресурсы (см. [тарифы {{ msp-name }}](../pricing.md)).
...
1. [Создайте сервисный аккаунт](../../iam/operations/sa/create.md#create-sa) `spark-agent` и назначьте ему роли:
    ...
    * [managed-spark.integrationProvider](../security.md#managed-spark-integrationProvider) — для взаимодействия {{ msp-full-name }} с другими сервисами;
...
1. [Создайте кластер {{ msp-full-name }}](../operations/cluster-create.md) со следующими настройками:
...
- Консоль управления {#console}
  ...
  1. В кластере {{ msp-full-name }} [создайте задание](../operations/jobs-pyspark.md) со следующими параметрами:
