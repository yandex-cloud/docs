...
- Консоль управления {#console}
    ...
    1. При необходимости задайте дополнительные настройки кластера:
       ...
       1. Выберите **Способ настройки ПО**:
...
- CLI {#cli}
    ...
    1. Укажите параметры кластера в команде создания (в примере приведены не все доступные параметры):
        ...
        * `--spark-version` — версия Apache Spark™.

           {% note info  %}
                      
           После создания кластера вы можете [изменить версию Apache Spark™](cluster-version-update.md).
...
- Terraform {#tf}
    ...
    * Сервисный аккаунт — `ajev56jp96ji********`.
    * Сеть — `msp-network`.
    * Подсеть — `msp-subnet`. [Зона доступности](../../overview/concepts/geo-scope.md) подсети — `ru-central1-a`, диапазон — `10.1.0.0/16`.
    * 2 драйвера с [классом вычислительных ресурсов](../concepts/instance-types.md) — `c2-m16`.
    * 4 исполнителя с классом вычислительных ресурсов — `c2-m16`.
    * Включенный сервер истории.
    * Защита от непреднамеренного удаления.
