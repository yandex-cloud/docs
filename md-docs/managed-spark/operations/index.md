[Документация Yandex Cloud](../../index.md) > [Yandex Managed Service for Apache Spark™](../index.md) > Пошаговые инструкции > Все инструкции

# Пошаговые инструкции для Yandex Managed Service for Apache Spark™

## Кластеры {#clusters}

* [Информация об имеющихся кластерах](cluster-list.md)
* [Создание кластера](cluster-create.md)
* [Изменение кластера](cluster-update.md)
* [Обновление версии Apache Spark™](cluster-version-update.md)
* [Остановка и запуск кластера](cluster-stop.md)
* [Подключение к кластеру](connect.md)
* [Управление доступом к кластеру](cluster-access.md)
* [Техническое обслуживание](cluster-maintenance.md)
* [Удаление кластера](cluster-delete.md)

## Задания (jobs) {#jobs}

* [Задания Spark](jobs-spark.md)
* [Задания PySpark](jobs-pyspark.md)
* [Задания SparkConnect](jobs-sparkconnect.md)

## Логи и мониторинг {#monitoring-and-logs}

* [Мониторинг состояния кластера](monitoring.md)
* [Просмотр логов кластера](cluster-logs.md)
* [Мониторинг состояния Spark-приложений](spark-monitoring.md)
* [Диагностика и примеры устранения проблем производительности Spark-приложений](spark-diagnostics.md)