...
1. В блоке **Шаблон виртуальной машины** нажмите кнопку **Задать**, чтобы задать конфигурацию базовой ВМ:
    ...
    * В блоке **Образ загрузочного диска** выберите публичный образ, на основе которого будут работать виртуальные машины группы.
    * В блоке **Диски и файловые хранилища**:

        * Выберите [тип диска](../concepts/disk.md#disks-types).
        * Укажите размер диска.
        * (Опционально) Нажмите кнопку **Добавить**, чтобы добавить дополнительные диски или файловые хранилища.
    * В блоке **Вычислительные ресурсы** выберите одну из готовых конфигураций ВМ или перейдите на вкладку **Своя конфигурация**, чтобы создать собственную конфигурацию.
    * В блоке **Сетевые настройки**:

        * Выберите [облачную сеть](../concepts/vm.md#network) и [подсеть](../concepts/network.md#subnet). Если подсети нет, создайте ее:

            * Разверните раскрывающийся список и нажмите кнопку ![image](../../_assets/console-icons/plus.svg) **Создать подсеть**.
        ...
        * (Опционально) Выберите [группу безопасности](../../vpc/concepts/security-groups.md). Если группы безопасности нет, создайте ее:
            ...
            * Разверните раскрывающийся список и нажмите **Создать группу**.
            * Введите имя и описание группы безопасности.
            * Нажмите кнопку ![image](../../_assets/console-icons/plus.svg) **Добавить правило**, чтобы создать в группе безопасности необходимые [правила](../../vpc/concepts/security-groups.md#security-groups-structure).
            * Нажмите кнопку **Создать**.
    * В блоке **Доступ**:

        * Выберите **Доступ по OS Login**, чтобы [подключаться](../operations/vm-connect/os-login.md) к виртуальным машинам и управлять доступом к ним с помощью [OS Login](../../organization/concepts/os-login.md) в Yandex Identity Hub.
    ...
    * (Опционально) Разверните блок **Дополнительно** и в появившихся полях:
...
1. В блоке **В процессе создания и обновления разрешено** укажите:
    ...
    * **Одновременно создавать** — сколько ВМ можно создавать одновременно.
    * **Время запуска** — время запуска, после которого ВМ начнет получать нагрузку.
    * **Одновременно останавливать** — сколько ВМ можно останавливать одновременно.
...
1. (Опционально) В блоке **Интеграция с Network Load Balancer** включите опцию **Создать целевую группу** и введите имя и описание целевой группы.
