...
По умолчанию в облаке установлена [нулевая квота](limits.md#compute-quotas) на использование выделенных хостов. Чтобы изменить [квоту](https://console.yandex.cloud/cloud?section=quotas), обратитесь в [техническую поддержку](https://center.yandex.cloud/support).
...
* в [консоли управления](https://console.yandex.cloud) на странице создания группы выделенных хостов в Compute Cloud;
...
При выходе из строя выделенного хоста данные на локальных дисках будут потеряны. Для сохранности данных самостоятельно настройте репликацию, например, с помощью [Yandex Cloud Backup](../../backup/concepts/index.md).
...
Идентификаторы `host-id` и `server-id` нового хоста и дату автоматического освобождения можно узнать в [консоли управления](https://console.yandex.cloud) на странице **Список хостов**, а также с помощью команды [CLI](../../cli/quickstart.md):
...
Дата и время автоматического освобождения устанавливаются на седьмые сутки с момента выделения подменного хоста. Compute Cloud начнет автоматическое освобождение хоста в период с `REPLACEMENT DEADLINE` до `REPLACEMENT DEADLINE + 1 час`. В зависимости от типа ВМ:
...
* [Динамической миграцией](live-migration.md) — для этого обратитесь в [техническую поддержку](https://center.yandex.cloud/support).
* [Остановкой и последующим запуском](../operations/vm-control/vm-stop-and-start.md) в [консоли управления](https://console.yandex.cloud) или с помощью команд CLI:
...
ВМ с дополнительными дисками можно перенести на новый хост. Данные на сетевых дисках сохранятся, а данные на локальных дисках будут потеряны — они привязаны к физическому серверу. Чтобы не потерять информацию с локальных дисков, [настройте](../../backup/operations/backup-vm/create.md) резервное копирование в Cloud Backup.
...
* [Создание виртуальной машины на выделенном хосте](../operations/dedicated-host/running-host-vms.md)
