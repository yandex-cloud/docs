...
На ВМ с Linux из [{{ marketplace-name }}]({{ link-cloud-marketplace }}) или из пользовательских образов с [установленным набором драйверов](../operations/image-create/custom-image.md#requirements) подключенный дополнительный диск будет доступен в качестве нового устройства без перезагрузки ВМ. 
...
* [{{ backup-name }}](../../backup/index.md) позволяет делать консистентные копии данных на ВМ с любыми типами дисков.
...
Для защиты критичных данных в {{ compute-name }} рекомендуется использовать шифрование дисков с помощью ключей [{{ kms-full-name }}](../../kms/index.md).
