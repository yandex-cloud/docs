[Документация Yandex Cloud](../../index.md) > [Yandex Compute Cloud](../index.md) > [Концепции](index.md) > Виртуальные машины > Сетевые интерфейсы виртуальных машин
...
Публичный IP-адрес можно сделать статическим. Подробнее читайте в разделе [Сделать публичный IP-адрес виртуальной машины статическим](../operations/vm-control/vm-set-static-ip.md).

Подробнее о правилах тарификации IP-адресов читайте в разделе [Публичные IP-адреса](../../vpc/pricing.md#prices-public-ip) документации сервиса Virtual Private Cloud.
...
1. В консоли управления невозможно ввести параметр `hostname` для новой ВМ. В качестве имени хоста используется имя ВМ, указанное пользователем:
	...
	* Если поле **Имя** при создании ВМ оставить пустым — будет присвоен внутренний FQDN `<идентификатор_ВМ>.auto.internal`.
	* Если в поле **Имя** ввести имя ВМ — будет присвоен внутренний FQDN `<имя_ВМ>.<регион>.internal`.

1. При работе с CLI, [API](../../glossary/rest-api.md) и Terraform внутренний FQDN создается по следующим правилам:
