...
В [зонах доступности](../../overview/concepts/geo-scope.md) `{{ region-id }}-d` и `{{ region-id }}-e` не поддерживаются ВМ на некоторых платформах {{ compute-name }}. Подробнее читайте в разделе [Ограничения](#restrictions).
...
Чтобы перенести ВМ с неподдерживаемой платформой в зоны `{{ region-id }}-d` или `{{ region-id }}-e`, воспользуйтесь одним из вариантов:
* [Сделайте](../operations/disk-control/create-snapshot.md) снимок диска и [создайте](../operations/vm-create/create-from-snapshots.md) из него новую ВМ в зоне `{{ region-id }}-d` или `{{ region-id }}-e` на другой платформе.
