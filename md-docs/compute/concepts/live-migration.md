[Документация Yandex Cloud](../../index.md) > [Yandex Compute Cloud](../index.md) > [Концепции](index.md) > Виртуальные машины > Динамическая миграция

# Динамическая миграция

В Compute Cloud используется механизм динамической миграции, который позволяет перемещать запущенные [виртуальные машины](vm.md) между физическими серверами без остановки и перезагрузки. Пауза при этом обычно не превышает 10 секунд.
...
* ВМ с [GPU](gpus.md) ^1^.
* ВМ с [политикой обслуживания](maintenance-policies.md) `RESTART` ^1^.
* [Прерываемых](preemptible-vm.md) ВМ ^2^.
* ВМ управляемых СУБД с локальными SSD-дисками.
* ВМ на выделенных хостах с [локальными SSD-дисками](dedicated-host.md#resource-disks).
* ВМ на выделенных хостах с [привязкой к конкретному хосту](dedicated-host.md#bind-vm).
