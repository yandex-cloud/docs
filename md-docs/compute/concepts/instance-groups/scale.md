[Документация Yandex Cloud](../../../index.md) > [Yandex Compute Cloud](../../index.md) > [Концепции](../index.md) > [Группы виртуальных машин](index.md) > Типы масштабирования
...
* Если [среднее значение метрики](#average-metric) поднимется выше целевого, Instance Groups создаст новые ВМ в группе.
...
* [Нагрузка CPU](#cpu-utilization).
* [Любые метрики](#monitoring-metrics) из сервиса [Yandex Monitoring](../../../monitoring/index.md).

#### Нагрузка CPU {#cpu-utilization}

Instance Groups может управлять размером группы так, чтобы стараться поддерживать среднюю нагрузку CPU не выше целевого уровня. Средняя нагрузка CPU рассчитывается для ВМ отдельно из каждой зоны доступности или из всей группы (при [зональном или региональном масштабировании](#auto-scale-type) соответственно).
...
Вы можете использовать до трех любых метрик из сервиса Monitoring для автоматического масштабирования в Instance Groups. Для чтения метрик у [сервисного аккаунта](../../../iam/concepts/users/service-accounts.md), привязанного к группе ВМ, должна быть [роль](../../../monitoring/security/index.md#monitoring-viewer) `monitoring.viewer` или выше.

В Instance Groups при использовании метрик мониторинга необходимо указать:
* _Имя метрики_, которое вы задали в Monitoring.
 * _[Метки](../../../monitoring/concepts/data-model.md#label)_, которые вы задали в Monitoring:
...
* _Тип метрики_, который влияет, как Instance Groups будет рассчитывать [среднее значение метрики](#average-metric):
...
* _Целевое значение метрики_, по которому Instance Groups рассчитывает нужное количество ВМ. Для `UTILIZATION`-метрик целевое значение — это желаемый уровень потребления ресурсов каждой ВМ, для `WORKLOAD`-метрик — максимальная допустимая нагрузка на каждую ВМ.
...
* [Создать автоматически масштабируемую группу виртуальных машин](../../operations/instance-groups/create-autoscaled-group.md).