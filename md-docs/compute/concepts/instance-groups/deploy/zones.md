[Документация Yandex Cloud](../../../../index.md) > [Yandex Compute Cloud](../../../index.md) > [Концепции](../../index.md) > [Группы виртуальных машин](../index.md) > [Обновление](index.md) > Распределение виртуальных машин по зонам
...
* `min_zone_size` (минимальный размер зоны) — в зоне не может быть меньше виртуальных машин, чем указано в `min_zone_size`.
  ...
  Допускается указывать `min_zone_size=0`, если у группы в [политике масштабирования](../policies/scale-policy.md#auto-scale-policy) задана хотя бы одна метрика, характеризующая суммарную нагрузку на все ВМ (`custom_rules: rule_type: WORKLOAD`). В этом случае зона может быть пустой.
...
* `initial_size` (начальное количество ВМ в группе):
  * для групп с региональным масштабированием, у которых в политике масштабирования не задана ни одна метрика, характеризующая суммарную нагрузку на все ВМ (`custom_rules: rule_type: WORKLOAD`), — не менее 1.
  * для групп с зональным масштабированием, у которых в политике масштабирования не задана ни одна метрика, характеризующая суммарную нагрузку на все ВМ, — не меньше количества зон, в которых развернута группа.
  * для групп, у которых в политике масштабирования задана хотя бы одна метрика, характеризующая суммарную нагрузку на все ВМ, — 0 и более.
  
  Ограничение нужно, чтобы процесс автоматического масштабирования не завис в ожидании метрик от ВМ, которых нет в зоне или регионе.
