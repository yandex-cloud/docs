[Документация Yandex Cloud](../../../index.md) > [Yandex Compute Cloud](../../index.md) > [Концепции](../index.md) > [Группы виртуальных машин](index.md) > Переменные в шаблоне виртуальной машины

# Переменные в шаблоне виртуальной машины

Instance Groups позволяет создать группу однотипных виртуальных машин из [шаблона](instance-template.md). Чтобы такие ВМ имели разные характеристики, используйте механизм подстановок системных и пользовательских переменных в шаблоне. Например, с помощью переменных и [тегов](policies/allocation-policy.md#tags) вы можете [создать группу ВМ с фиксированными IP-адресами](../../operations/instance-groups/create-with-fixed-ip.md).

_Системная переменная_ — значение, которое Instance Groups вычисляет во время создания ВМ и подставляет его в шаблон для дальнейшего использования.

_Пользовательская переменная_ — значение, которое Instance Groups берет из списка. Он заранее сформирован пользователем, переменные в нем описаны в формате `ключ:значение`.
...
--- | --- | ---
`{instance_group.id}` | Идентификатор группы ВМ. | Строковое значение, например `cl1cd2vd4ohr********`. Подробнее на странице [Получить информацию о группе ВМ](../../operations/instance-groups/get-info.md).
`{instance_group.labels.label_key}` | Значение [метки](../../../resource-manager/concepts/labels.md) группы ВМ с ключом `label_key`. | Строковое значение, например `label_value`. Подробнее на странице [Получить информацию о группе ВМ](../../operations/instance-groups/get-info.md).
...
`{instance.labels.another_label_key}` | Значение метки конкретной ВМ с ключом `another_label_key`. | Строковое значение, например `another_label_value`. Подробнее на странице [Получить информацию о ВМ](../../operations/vm-info/get-info.md).
...
`{instance.tag}` | Тег ВМ в группе внутри зоны доступности. | Значение, указанное в поле `allocation_policy.zones.instance_tags_pool`. Подробнее на странице [Политика распределения](policies/allocation-policy.md).
`{instance.zone_id}` | Идентификатор зоны доступности. | Значение, указанное в поле `allocation_policy.zones.zone_id`. Подробнее на странице [Политика распределения](policies/allocation-policy.md).
...
1. На первом этапе Instance Groups заменит [системные переменные](#first-stage) на вычисленные значения.
...
1. На втором этапе Instance Groups преобразует полученные переменные в значения из списка в блоке `variables`:
...
* [Переменные в шаблоне узла Yandex Managed Service for Kubernetes](../../../managed-kubernetes/concepts/node-group/variables-in-the-template.md)