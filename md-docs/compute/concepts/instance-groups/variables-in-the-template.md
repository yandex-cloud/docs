...
--- | --- | ---
`{instance_group.id}` | Идентификатор группы ВМ. | Строковое значение, например `cl1cd2vd4ohr********`. Подробнее см. [Получить информацию о группе ВМ](../../operations/instance-groups/get-info.md).
`{instance_group.labels.label_key}` | Значение [метки](../../../resource-manager/concepts/labels.md) группы ВМ с ключом `label_key`. | Строковое значение, например `label_value`. Подробнее см. [Получить информацию о группе ВМ](../../operations/instance-groups/get-info.md).
...
`{instance.labels.another_label_key}` | Значение метки конкретной ВМ с ключом `another_label_key`. | Строковое значение, например `another_label_value`. Подробнее см. [Получить информацию о ВМ](../../operations/vm-info/get-info.md).
...
`{instance.tag}` | Тег ВМ в группе внутри зоны доступности. | Значение, указанное в поле `allocation_policy.zones.instance_tags_pool`. Подробнее см. [Политика распределения](policies/allocation-policy.md).
`{instance.zone_id}` | Идентификатор зоны доступности. | Значение, указанное в поле `allocation_policy.zones.zone_id`. Подробнее см. [Политика распределения](policies/allocation-policy.md).
