[Документация Yandex Cloud](../../../../index.md) > [Yandex Compute Cloud](../../../index.md) > [Концепции](../../index.md) > [Группы виртуальных машин](../index.md) > Группы ВМ при зональном инциденте > Обзор

# Группы виртуальных машин Yandex Compute Cloud при зональном инциденте
...
> Например, если инцидент произошел в зоне `ru-central1-a`, а ВМ группы размещены только в зонах `ru-central1-b` и `ru-central1-d`.
...
Зональный инцидент влияет на функциональность группы ВМ в проблемной зоне. Большинство функций в других зонах продолжает работать штатно. Подробнее на странице [Мультизональная группа ВМ Yandex Compute Cloud с ВМ в зоне инцидента](multi-zonal.md).
...
* в Yandex Application Load Balancer группы ВМ используются в качестве [ресурсных единиц](../../../../application-load-balancer/concepts/application-load-balancer.md#lcu-scaling) L7-балансировщика;
* в Yandex Managed Service for Kubernetes группы ВМ используются в качестве [групп узлов](../../../../managed-kubernetes/concepts/index.md#node-group) кластера;
* в Yandex Managed Service for YDB группы ВМ используются в качестве [узлов кластера в Dedicated-режиме](../../../../ydb/concepts/serverless-and-dedicated.md#dedicated).
