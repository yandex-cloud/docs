...
Большинство дистрибутивов Linux распространяется по открытым лицензиям, которые не ограничивают использование в облачных средах. Вы можете [загрузить собственный образ](../operations/image-create/upload.md) и создать из него ВМ.

Для продуктов Microsoft возможность использования собственных лицензий зависит от условий лицензирования конкретного продукта и требует [выделенных хостов](../concepts/dedicated-host.md). Подробнее в разделе [Использование своей лицензии для продуктов Microsoft](../../microsoft/byol.md).
...
Для [зашифрованных дисков](../concepts/encryption.md#encryption-options) этот способ не работает. Записанные нули шифруются ключом диска и не сжимаются. После заполнения свободного пространства нулями снимок увеличится до полного размера диска.
...
- Windows {#windows}
  ...
  Остановите дисковые операции и используйте утилиту `SDelete`. Узнать, как она работает, и скачать ее можно в [документации Microsoft](https://docs.microsoft.com/ru-ru/sysinternals/downloads/sdelete).
...
1. Создайте образ из вашего снимка в разделе **Снимки дисков** или из самого диска в разделе **Диски и хранилища**.
