...
Для повышения прав до `root` используйте `sudo`. Пользователь, указанный при создании ВМ, входит в группу `sudo`. У `root` нет пароля, поэтому достаточно подключиться к ВМ по SSH (см. инструкцию [Подключиться к виртуальной машине Linux по SSH](../operations/vm-connect/ssh.md)) и ввести команду:
...
Вероятно, вашими данными завладели, и к ВМ получили доступ третьи лица. Сделайте [снимок диска](../concepts/snapshot.md) ВМ (см. инструкцию [Создать снимок диска](../operations/disk-control/create-snapshot.md)) и создайте новую ВМ из него (см. [Создать виртуальную машину с дисками из снимков](../operations/vm-create/create-from-snapshots.md)). Если к новой ВМ удалось подключиться, используйте более сложные пароли и не распространяйте ваши секретные данные.
...
При возникновении проблем с подключением к ВМ по сети вы можете использовать серийную консоль. Подробнее о ней в разделе [Начало работы с серийной консолью](../operations/serial-console/index.md).
...
Если вы устанавливали пароль для пользователя, [настройте](../operations/serial-console/index.md) серийную консоль ВМ и подключитесь к ней. Мы рекомендуем [подключаться с помощью CLI](../operations/serial-console/connect-cli.md).
...
1. Сделайте снимок проблемного диска: см. инструкцию [Создать снимок диска](../operations/disk-control/create-snapshot.md).
1. Создайте новую ВМ с дополнительным (не загрузочным) диском, восстановленным из снимка: см. [Создать виртуальную машину с дисками из снимков](../operations/vm-create/create-from-snapshots.md).
1. Подключитесь к ВМ: см. [Работа на виртуальной машине](../operations/index.md#vm-use).
1. Смонтируйте диск: см. [Смонтировать диск, созданный из снимка или образа](../operations/vm-control/vm-attach-disk.md#mount-disk-and-fix-uuid).
