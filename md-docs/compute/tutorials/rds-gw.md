[Документация Yandex Cloud](../../index.md) > [Yandex Compute Cloud](../index.md) > [Практические руководства](index.md) > Развертывание Remote Desktop Gateway
...
1. Создайте подсеть в сети `rdgw-network`.
   ...
   {% list tabs group=instructions %}
    ...
    - Консоль управления {#console}
      ...
      1. [Перейдите](https://console.yandex.cloud/link/vpc) в сервис **Virtual Private Cloud** в каталоге, где требуется создать подсеть.
...
- Консоль управления {#console}
   ...
   1. [Перейдите](https://console.yandex.cloud/link/vpc) в сервис **Virtual Private Cloud** в каталоге, где требуется создать группу безопасности.
   1. На панели слева выберите **Группы безопасности**.
   ...
   1. В блоке **Правила** создайте следующие правила для управления трафиком по инструкции под таблицей:
        ...
        1. В открывшемся окне в поле **Порты** укажите один порт или диапазон портов, куда или откуда будет поступать трафик.
...
- Консоль управления {#console}
     ...
     1. В блоке **Образ загрузочного диска**:
         ...
         * В поле **Наполнение** выберите `Образ` и в списке ниже выберите образ **Windows Server 2022 Datacenter**. Как загрузить свой образ для продуктов Microsoft смотрите в разделе [Импортировать нужный образ](../../microsoft/byol.md#how-to-import).
     ...
     1. В блоке **Диски и файловые хранилища** задайте размер загрузочного [диска](../concepts/disk.md) `60 ГБ`.
...
- CLI {#cli}
    ...
    1. В терминале PowerShell создайте скрипт `setpass` для настройки пароля учетной записи `Administrator` через поле `user-data` в [метаданных ВМ](../concepts/vm-metadata.md). Утилита `cloudbase-init` выполнит его при первом запуске.
...
1. [Подключитесь](../operations/vm-connect/rdp.md) к созданной ВМ по протоколу RDP.
