...
В качестве примера проекта будет использовано веб-приложение, реализованное на [Django](https://www.djangoproject.com/), которое имитирует корзину товаров интернет-магазина. В базе данных хранятся описания товаров, а состояние корзины товаров сервис хранит в сессии пользователя. Django-приложение разворачивается в [контейнере Yandex Serverless Containers](../../serverless-containers/concepts/container.md), при этом секреты безопасно доставляются в приложение с помощью сервиса [Yandex Lockbox](../../lockbox/index.md). [Yandex API Gateway](../../api-gateway/index.md) принимает запросы от пользователей и перенаправляет их в [контейнер приложения](../../glossary/containerization.md#containers-apps).
...
* Узлы кластера Managed Service for Kubernetes: использование вычислительных ресурсов и хранилища ([тарифы Yandex Compute Cloud](../pricing.md)).
...
* Секреты Yandex Lockbox: количество хранимых версий секретов и запросы к ним ([тарифы Yandex Lockbox](../../lockbox/pricing.md)).
...
* API-шлюз: количество запросов к шлюзу ([тарифы API Gateway](../../api-gateway/pricing.md)).
* Инстанс GitLab. Стоимость зависит от способа создания инстанса:
   ...
   * ВМ с образом GitLab — оплачиваются вычислительные ресурсы ВМ и образ GitLab ([тарифы Compute Cloud](../pricing.md)).
...
Создайте инстанс Managed Service for GitLab или [виртуальную машину](../concepts/vm.md) с [образом](../concepts/image.md) GitLab в той же [облачной сети](../../vpc/concepts/network.md#network), где расположен кластер Managed Service for Kubernetes.
...
- ВМ с образом GitLab {#gl-image-vm}
  ...
  1. На странице сервиса Yandex Compute Cloud выберите созданную [ВМ](../concepts/vm.md) и скопируйте ее [публичный IP-адрес](../../vpc/concepts/address.md#public-addresses).
...
1. [Удалите инстанс Managed Service for GitLab](../../managed-gitlab/operations/instance/instance-delete.md) или [созданную ВМ с образом GitLab](../operations/vm-control/vm-delete.md).
