[Документация Yandex Cloud](../../../index.md) > [Yandex Compute Cloud](../../index.md) > [Практические руководства](../index.md) > [Привязка доменного имени к ВМ с веб-сервером](index.md) > Terraform
...
1. В файле `bind-domain-to-vm.auto.tfvars` задайте пользовательские параметры:
    ...
    * `ssh_key_path` — путь к файлу с открытым SSH-ключом для аутентификации пользователя на ВМ, например `~/.ssh/id_ed25519.pub`. Подробнее в разделе [Создание пары ключей SSH](../../operations/vm-connect/ssh.md#creating-ssh-keys).
...
1. [Получите публичный IP-адрес ВМ](../../operations/vm-info/get-info.md#outside-instance) — он потребуется далее, чтобы [проверить работу хостинга](#test).
