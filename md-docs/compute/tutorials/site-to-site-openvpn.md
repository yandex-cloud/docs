[Документация Yandex Cloud](../../index.md) > [Yandex Compute Cloud](../index.md) > [Практические руководства](index.md) > Создание туннеля между двумя подсетями при помощи OpenVPN Access Server
...
* плата за диски и постоянно запущенные виртуальные машины ([тарифы Yandex Compute Cloud](../pricing.md));
...
- Консоль управления {#console}
  ...
  1. В [консоли управления](https://console.yandex.cloud) перейдите в каталог, в котором хотите создать облачную сеть.
...
- Консоль управления {#console}
  ...
  1. Нажмите ![image](../../_assets/console-icons/nodes-right.svg) **Создать подсеть**.
...
- Консоль управления {#console}
  ...
  1. В блоке **Образ загрузочного диска** в поле **Поиск продукта** выберите образ для ВМ.
  1. В блоке **Расположение** выберите [зону доступности](../../overview/concepts/geo-scope.md), в которой находится подсеть `ovpn-left`.
  ...
  1. В блоке **Общая информация** задайте имя ВМ: `ao-openvpn-test`.
...
- Консоль управления {#console}
  ...
  1. На странице [каталога](../../resource-manager/concepts/resources-hierarchy.md#folder) в [консоли управления](https://console.yandex.cloud) нажмите кнопку ![image](../../_assets/console-icons/plus.svg) **Создать ресурс** и выберите ![image](../../_assets/console-icons/cpu.svg) **Виртуальная машина**.
  1. В блоке **Образ загрузочного диска** в поле **Поиск продукта** выберите образ для ВМ.
  1. В блоке **Расположение** выберите [зону доступности](../../overview/concepts/geo-scope.md), в которой находится подсеть `ovpn-right`.
  1. В блоке **Сетевые настройки**:
  ...
  1. В блоке **Общая информация** задайте имя ВМ: `vm-ovpn-gw`.
  1. Нажмите кнопку **Создать ВМ**.
...
- Консоль управления {#console}
  ...
  1. В блоке **Расположение** выберите [зону доступности](../../overview/concepts/geo-scope.md), в которой находится подсеть `ovpn-left`.
  1. В блоке **Диски и файловые хранилища** задайте размер загрузочного [диска](../concepts/disk.md) `10 ГБ`.
  ...
  1. В блоке **Сетевые настройки**:
      ...
      * В поле **Группы безопасности** выберите [группу безопасности](../../vpc/concepts/security-groups.md). Если оставить поле пустым, будет назначена [группа безопасности по умолчанию](../../vpc/concepts/security-groups.md#default-security-group).
      * Разверните блок **Дополнительно** и в поле **Внутренний IPv4 адрес** выберите `Вручную`.
...
- Консоль управления {#console}
  ...
  1. Для работы OpenVPN Access Server [добавьте](../../vpc/operations/security-group-add-rule.md) следующие правила в группу безопасности по умолчанию:
     ...
     --- | --- | --- | --- | --- | ---
     Входящий | `VPN Server` | `443` | `TCP` | `Диапазон адресов` | `0.0.0.0/0`
     Входящий | `VPN Server` | `1194` | `UDP` | `Диапазон адресов` | `0.0.0.0/0`
