...
* плата за постоянно запущенные [ВМ](../../concepts/vm.md) (см. [тарифы {{ compute-full-name }}](../../pricing.md));
...
Создавая группы ВМ, учитывайте [лимиты](../../concepts/limits.md). Чтобы не нарушить работу компонента {{ ig-name }}, не изменяйте и не удаляйте вручную созданные им ресурсы: [целевую группу](../../../application-load-balancer/concepts/target-group.md) {{ alb-name }}, ВМ и диски. Вместо этого измените или удалите группу полностью.
  ...
  1. В блоке **{{ ui-key.yacloud.compute.groups.create.section_instance }}** нажмите кнопку **{{ ui-key.yacloud.compute.groups.create.button_instance_empty-create }}** и укажите конфигурацию базовой ВМ:
     ...
     * В блоке **{{ ui-key.yacloud.compute.instances.create.section_access }}** укажите данные для доступа на ВМ:
       ...
       * В поле **{{ ui-key.yacloud.compute.instances.create.field_key }}** вставьте содержимое файла открытого ключа.
        ...
        Для подключения по [SSH](../../../glossary/ssh-keygen.md) необходимо создать пару ключей. Подробнее в разделе [{#T}](../../operations/vm-connect/ssh.md#creating-ssh-keys).
