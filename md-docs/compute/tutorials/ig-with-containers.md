...
Вы можете создать [группу виртуальных машин](../concepts/instance-groups/index.md) на базе [образа](../concepts/image.md) [{{ coi }}](../../cos/concepts/index.md) с несколькими [Docker-контейнерами](https://yandex.cloud/ru/blog/posts/2022/03/docker-containers) внутри.
...
1. Узнайте идентификатор последней версии [публичного образа](../operations/images-with-pre-installed-software/get-list.md) {{ coi }}.
   
   Образ {{ coi }} в реестре [{{ container-registry-name }}](../../container-registry/index.md) может обновляться и меняться в соответствии с релизами. При этом образ на [виртуальной машине](../concepts/vm.md) не обновится автоматически до последней версии. Чтобы создать [группу ВМ](../concepts/instance-groups/index.md) с последней версией {{ coi }}, необходимо самостоятельно проверить ее наличие:
...
1. Сохраните спецификацию группы ВМ с {{ coi }} и несколькими Docker-контейнерами в файл `specification.yaml`:
   ...
   Передать [SSH-ключ](../../glossary/ssh-keygen.md) в [метаданных ВМ](../concepts/vm-metadata.md) можно с помощью параметра `ssh-keys` или в ключе с пользовательскими метаданными `user-data`. В этом практическом руководстве используется первый вариант.
...
1. Создайте группу ВМ с именем `group-coi-containers` с помощью спецификации `specification.yaml`:
   ...
   - API {#api}
     ...
     Воспользуйтесь методом [CreateFromYaml](../instancegroup/api-ref/InstanceGroup/createFromYaml.md) для ресурса `InstanceGroup`.
...
1. Убедитесь, что группа ВМ с {{ coi }} и несколькими Docker-контейнерами создана:
   ...
   - API {#api}
     ...
     Посмотрите список созданных ВМ с помощью метода [List](../instancegroup/api-ref/InstanceGroup/list.md) для ресурса `InstanceGroup`.

   {% endlist %}

## Проверьте группу ВМ с {{ coi }} и несколькими Docker-контейнерами {#check}

1. [Подключитесь](../operations/vm-connect/ssh.md#vm-connect) к одной из созданных ВМ по SSH:
