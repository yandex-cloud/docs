[Документация Yandex Cloud](../../index.md) > [Yandex Compute Cloud](../index.md) > [Практические руководства](index.md) > Развертывание параллельной файловой системы GlusterFS в высокопроизводительном режиме
...
* плата за постоянно работающие ВМ и диски ([тарифы Yandex Compute Cloud](../pricing.md));
...
1. Отредактируйте файл `variables.tf`, указав параметры разворачиваемых ресурсов:
   ...
   1. В секции `storage_node_per_zone` значение `default` измените на `30`.
      ...
      Максимальная агрегированная пропускная способность всей системы рассчитывается как произведение пропускной способности каждого сегмента (450 МБ/с для [сетевых SSD-дисков](../concepts/disk.md#disks-types)) на количество сегментов (30), что составляет около 13,5 ГБ/с.
   ...
   1. Если вам необходима повышенная производительность и не важна гарантия сохранности данных, вы можете использовать [нереплицируемые SSD-диски](../concepts/disk.md#nr-disks). Для этого в секции `disk_type` значение `default` измените на `network-ssd-nonreplicated`. Кроме этого, в секции `disk_size` значение `default` должно быть кратно 93.
