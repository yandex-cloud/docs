...
* Плата за постоянно запущенные ВМ (см. [тарифы {{ compute-name }}](../pricing.md)).
* Плата за хранение созданных образов (см. [тарифы {{ compute-name }}](../pricing.md#prices-storage)).
...
* [Подготовьте](../operations/vm-connect/ssh.md) [SSH-ключ](../../glossary/ssh-keygen.md) для доступа к ВМ.
...
1. В блоке **{{ ui-key.yacloud.compute.instances.create.section_storages }}** задайте размер загрузочного [диска](../concepts/disk.md) `15 {{ ui-key.yacloud.common.units.label_gigabyte }}`.
...
1. [Подключитесь](../operations/vm-connect/ssh.md) к ВМ по протоколу SSH. Для этого можно использовать утилиту `ssh` в Linux и macOS и программу `PuTTY` для Windows.
...
1. Внесите изменения в шаблоны Packer, находящиеся в директории `packer`. Документацию шаблонов Packer можно найти на [сайте](http://packer.io/docs/templates/index.html) разработчика. В параметрах `image_family` и `source_image_family` указываются [семейства образов](../concepts/image.md#family), которые будет собирать Jenkins.
...
* [Удалите созданные ВМ](../operations/vm-control/vm-delete.md).
* [Удалите созданные образы](../operations/image-control/delete.md).
