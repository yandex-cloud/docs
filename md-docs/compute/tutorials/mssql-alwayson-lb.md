...
ВМ нужно создавать на [выделенных хостах](../concepts/dedicated-host.md). Получить идентификатор выделенного хоста можно с помощью Yandex Cloud CLI, выполнив команду `yc compute host-group list-hosts` (подробнее о команде смотрите в [справочнике](../../cli/cli-ref/compute/cli-ref/host-group/list-hosts.md)).
