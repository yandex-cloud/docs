...
Аналогичным образом USB-устройства можно доставить на [виртуальную машину](../concepts/vm.md) {{ compute-full-name }}.
...
1. [Подключитесь по SSH](../operations/vm-connect/ssh.md) к арендованному ранее серверу {{ baremetal-name }} `my-usbip-client`.
1. Установите WireGuard и необходимые зависимости:
...
1. [Подключитесь по SSH](../operations/vm-connect/ssh.md) к серверу {{ baremetal-name }} `my-usbip-client` и выполните команду:
...
1. [Подключитесь по SSH](../operations/vm-connect/ssh.md) к серверу {{ baremetal-name }} `my-usbip-client`.
1. Установите дополнительные пакеты, необходимые для работы `usbip`:
