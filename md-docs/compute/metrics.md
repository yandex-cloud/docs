...
`DGAUGE`, %
|
Утилизация процессорных ядер ВМ (vCPU) в процентах. Принимает значения от 0% до [уровня производительности vCPU](concepts/performance-levels.md).
Метки:
...
|| `maintenance_event`
|
`1`, если на ВМ активно [событие обслуживания](concepts/maintenance-policies.md).
Метки:
- *resource_id* — имя или идентификатор ВМ.
- *resource_type* — тип ресурса. Единственное значение — `vm`.
...
Метка | Значение
----|----
filestore | Идентификатор [файлового хранилища](concepts/filesystem.md)
instance | Имя [виртуальной машины](concepts/vm.md)
...
`RATE`, миллисекунды
|
Совокупное время выполнения операций индексации в файловом хранилище. Специальная метка *request* — тип операции. ||
|| `filestore.index_errors`
`RATE`, операции/с
|
Количество операций индексации в файловом хранилище, завершившихся ошибкой. ||
|| `filestore.index_latency`
`RATE`, миллисекунды
|
Задержка выполнения операций индексации в файловом хранилище.
Специальные метки:
* *request* — тип операции.
* *bin* — бакеты гистограммы. ||
|| `filestore.index_ops`
`RATE`, операции/с
|
Среднее количество операций индексации в файловом хранилище. ||
|| `filestore.read_bytes`
`RATE`, байты/с
|
Среднее количество байт, прочитанных из файлового хранилища. ||
|| `filestore.read_bytes_burst`
`DGAUGE`, байты/с
|
Максимальное количество байт, прочитанных из файлового хранилища. ||
|| `filestore.read_errors`
`RATE`, операции/с
|
Количество операций чтения из файлового хранилища, завершившихся ошибкой. Специальная метка *request* — тип операции. ||
|| `filestore.read_latency`
`RATE`, миллисекунды
|
Гистограмма распределения времени обработки запросов на чтение из файлового хранилища. Специальная метка *bin* — бакеты гистограммы. ||
|| `filestore.read_ops`
`RATE`, операции/с
|
Среднее количество операций чтения из файлового хранилища. ||
|| `filestore.read_ops_burst`
`DGAUGE`, операции/с
|
Максимальное количество операций чтения из файлового хранилища. ||
|| `filestore.write_bytes`
`RATE`, байты/с
|
Среднее количество байт, записанных в файловое хранилище. ||
|| `filestore.write_bytes_burst`
`DGAUGE`, байты/с
|
Максимальное количество байт, записанных в файловое хранилище. ||
|| `filestore.write_errors`
`RATE`, операции/с
|
Количество операций записи в файловое хранилище, завершившихся ошибкой. ||
|| `filestore.write_latency`
`RATE`, миллисекунды
|
Гистограмма распределения времени обработки запросов на запись в файловое хранилище. Специальная метка *bin* — бакеты гистограммы. ||
|| `filestore.write_ops`
`RATE`, операции/с
|
Среднее количество операций записи в файловое хранилище. ||
|| `filestore.write_ops_burst`
`DGAUGE`, операции/с
|
Максимальное количество операций записи в файловое хранилище. ||
|#

## Метрики групп виртуальных машин {#ig-metrics}

Доступные метрики зависят от [типа масштабирования](concepts/instance-groups/scale.md) группы ВМ.
