[Документация Yandex Cloud](../../../../index.md) > [Yandex Compute Cloud](../../../index.md) > Справочник API > [gRPC (англ.)](../index.md) > [Instance](index.md) > AttachNetworkInterface

# Compute Cloud API, gRPC: InstanceService.AttachNetworkInterface

Attaches the network-interface to the instance.
To attach a network-interface, the instance must have a `STOPPED` status ([Instance.status](../GpuCluster/listInstances.md#yandex.cloud.compute.v1.Instance)).
To check the instance status, make a [InstanceService.Get](get.md#Get) request.
To stop the running instance, make a [InstanceService.Stop](stop.md#Stop) request.
