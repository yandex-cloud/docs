[Документация Yandex Cloud](../../../index.md) > [Yandex Compute Cloud](../../index.md) > [Пошаговые инструкции](../index.md) > Виртуальные машины > Управление > Перенести ВМ в другую зону доступности
...
Также вы можете перенести ВМ из зон `ru-central1-a` и `ru-central1-b` в зону `ru-central1-d` с помощью специальной команды [CLI](../../../cli/cli-ref/compute/cli-ref/instance/relocate.md).
...
В зоне `ru-central1-d` не поддерживаются ВМ на [платформе](../../concepts/vm-platforms.md) Intel Broadwell. Чтобы перенести ВМ с такой платформы в зону `ru-central1-d`, вы можете:

* Сделать снимок диска и создать из него новую ВМ в зоне `ru-central1-d` на другой платформе.
...
- Другие ОС {#other}
  ...
  1. Остановите виртуальную машину (смотрите раздел [Остановить](vm-stop-and-start.md#stop)).
...
- Консоль управления {#console}
  ...
  1. В блоке **Дополнительно**:
     ...
     * (Опционально) Настройте [политику обслуживания](../../concepts/maintenance-policies.md). В консоли управления можно выбрать только тип политики. Чтобы настроить время ожидания перед обслуживанием, используйте CLI, Terraform или API.
...
- CLI {#cli}
  ...
  1. Подготовьте снимки нужных дисков, смотрите раздел [Создать снимок диска](../disk-control/create-snapshot.md).
...
## Перенести ВМ в зону ru-central1-d с помощью специальной команды {#relocate-command}

С помощью специальной команды CLI можно перенести виртуальную машину только в зону доступности `ru-central1-d`. В таком случае у ВМ сохраняются идентификатор и метаданные. Вместе с ВМ в новую зону доступности будут перенесены и все подключенные к ней диски.
...
- CLI {#cli}
  ...
  1. Перенесите нужную виртуальную машину в другую зону доступности:
      ...
      * `--destination-zone-id` — идентификатор [зоны доступности](../../../overview/concepts/geo-scope.md). Поддерживается только `ru-central1-d`.
...
В этом примере ВМ `my-vm-1` переносится из зоны доступности `ru-central1-b` в зону доступности `ru-central1-d`.
...
В этом примере ВМ `my-vm-1` с двумя дисками в группе размещения переносится из зоны доступности `ru-central1-b` в зону доступности `ru-central1-d`.
