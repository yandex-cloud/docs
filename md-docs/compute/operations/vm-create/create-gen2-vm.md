[Документация Yandex Cloud](../../../index.md) > [Yandex Compute Cloud](../../index.md) > [Пошаговые инструкции](../index.md) > Виртуальные машины > Создание > Создать ВМ с оборудованием поколения Gen 2
...
* Установлены драйверы `virtio-net`, `virtio-blk` и `virtio-pci`. Если вы собираетесь подключать к [виртуальной машине](../../concepts/vm.md) [файловые хранилища](../../concepts/filesystem.md), также должен быть установлен драйвер `virtiofs`. Смотрите [инструкцию](../image-create/custom-image.md#virtio).
* Терминал `ttyS0` (порт COM1) настроен в качестве [серийной консоли](../../concepts/serial-console.md). Смотрите [инструкцию](../image-create/custom-image.md#serial-console).
...
* Если образ создан на основе Amazon Machine Image (AMI), в настройках `cloud-init` отключена проверка облачной платформы, на которой запускается ВМ. Смотрите [инструкцию](../image-create/custom-image.md#ec2).
  ...
  1. В блоке **Сетевые настройки**:
     ...
     * Выберите [подходящие группы безопасности](../../../vpc/concepts/security-groups.md):
         ...
         * Чтобы подключаться к виртуальной машине по `RDP`, группа безопасности должна разрешать входящий сетевой трафик по протоколам `TCP` и `UDP` на порт `3389`.
         ...
         Если оставить поле пустым, виртуальной машине будет автоматически назначена [группа безопасности по умолчанию](../../../vpc/concepts/security-groups.md#default-security-group), которая разрешает подключение к ВМ по `SSH`, `RDP`, `ICMP` и любой исходящий трафик. Для рабочих ВМ, особенно с публичным IP-адресом, выберите пользовательскую группу безопасности с минимально необходимыми правилами.
