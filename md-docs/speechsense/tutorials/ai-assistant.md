...
Вы можете получить из диалога информацию, извлеченную с помощью [AI-ассистента](../concepts/assistants.md). Его можно подключать к аудиодиалогам и к чатам. В этом руководстве рассмотрено подключение к чатам. В качестве примера приведена переписка клиента с турагентством.
...
* Сервис SpeechSense: количество символов в каждом текстовом диалоге (см. [тарифы SpeechSense](../pricing.md)).
...
- Интерфейс SpeechSense {#speechsense}
    ...
    1. Введите название [пространства](../concepts/resources-hierarchy.md#space).
    1. Нажмите кнопку **Создать**.
    1. [Привяжите платежный аккаунт](../operations/space/link-ba.md) к пространству для оплаты SpeechSense.
...
Создайте сервисный аккаунт с [ролью](../security/index.md#speechsense-data-editor) `speech-sense.data.editor` для доступа amoCRM к проекту SpeechSense.
...
- Интерфейс SpeechSense {#speechsense}
  ...
  1. Выберите сервисный аккаунт `speechsense` и укажите роль [Data editor](../security/index.md#speechsense-data-editor). Эта роль позволит сервисному аккаунту загружать данные в SpeechSense.
...
- REST API {#api}
    ...
    Воспользуйтесь методом [Assistants.List](../api-ref/Assistants/list.md) и выполните запрос, например с помощью [cURL](https://curl.se/):
...
- gRPC API {#grpc-api}
    ...
    Воспользуйтесь вызовом [AssistantsService.List](../api-ref/grpc/Assistants/list.md) и выполните запрос, например с помощью [gRPCurl](https://github.com/fullstorydev/grpcurl):
...
- REST API {#api}
    ...
    Воспользуйтесь методом [Talk.Get](../api-ref/Talk/get.md) и выполните запрос, например с помощью [cURL](https://curl.se/):
...
Некоторые ресурсы платные. Чтобы за них не списывалась плата, [удалите проект SpeechSense](../operations/project/delete.md), если вы больше не будете его использовать. AI-ассистент будет удален при удалении проекта.