...
[Подключение](concepts/connection/index.md) содержит информацию о параметрах доступа к источнику данных. Например, IP-адрес хоста БД, порт. На базе подключения вы можете создавать датасеты.

Чтобы создать подключение:


1. Перейдите на [главную страницу](https://datalens.ru/?skipPromo=true) DataLens.
1. На панели слева нажмите ![image](../_assets/console-icons/thunderbolt.svg) **Подключения**.
...
1. [Добавьте](security/add-new-user.md) в DataLens пользователей с аккаунтами на Яндексе:
   ...
   Приняв приглашение и выбрав аккаунт для входа, пользователь сможет войти в вашу организацию и начать пользоваться DataLens. Пользователь получит роль `datalens.creator` в организации с текущим экземпляром DataLens. Для входа в DataLens пользователю потребуется [рабочее место](settings/seats.md).
