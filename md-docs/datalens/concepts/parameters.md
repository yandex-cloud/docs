...
Вы можете изменять значение параметра на дашбордах как с помощью селекторов с ручным вводом, так и с помощью селекторов на основе датасета. При этом существуют следующие ограничения:

* Не используйте опцию **Множественный выбор**. Это может привести к непредсказуемым результатам и вызвать ошибку.
* Не используйте выбор операции в селекторе (оставьте прочерк в поле выбора из выпадающего списка **Операция**). Это не изменит значение параметра и не повлияет на вычисление выражений.
* При использовании селектора по параметру из датасета разорвите связь этого селектора с другими селекторами на вкладке дашборда (установите тип [связи](../dashboard/link.md) **Не связаны**).
...
1. [Добавьте селектор](../operations/dashboard/add-selector.md) на дашборд для управления параметром чарта:
   ...
   - На основе датасета
     ...
     * выберите **На основе датасета**;
     * в поле **Датасет** выберите датасет с данными для селектора;
     * в поле **Поле** выберите имя параметра из чарта;
     * в поле выбора из выпадающего списка **Операция** оставьте прочерк;
     * в поле **Значение по умолчанию** укажите значение, которое будет подставлено в параметр чарта;
     * укажите **Заголовок** селектора для отображения на дашборде.
...
1. Создайте на основе созданного датасета чарт с типом [Столбчатая диаграмма](../visualization-ref/column-chart.md#create-diagram):
   ...
   * В чарт [добавьте поле](calculations/index.md#how-to-create-calculated-field) `period` с формулой `DATETRUNC([OrderDatetime], [scale])` и перетащите его в секцию **X**.
...
1. Проверьте, что связь между селектором и чартом на дашборде установлена правильно:
   ...
   1. В режиме редактирования дашборда в верхней части экрана нажмите значок ![image](../../_assets/console-icons/gear.svg). Справа откроется окно настроек.
...
1. Создайте чарт с типом [Столбчатая диаграмма](../visualization-ref/column-chart.md#create-diagram):
   ...
   * В чарт [добавьте поле](calculations/index.md#how-to-create-calculated-field) `shop_product_cat` с формулой `CASE([dimension_control], "по магазинам", [ShopName], "по категориям товаров",[ProductCategory], "по подкатегориям товаров", [ProductSubcategory], '')` и перетащите его в секцию **X**
   * Перетащите измерение `Sales` в секцию **Y**. Автоматически измерение `Sales` будет преобразовано в показатель `SUM[Sales]`.
...
1. Создайте чарт с типом [Круговая диаграмма](../visualization-ref/pie-chart.md#create-diagram):
   ...
   * В чарт [добавьте поле](calculations/index.md#how-to-create-calculated-field) `dimension_colors` с формулой `IF([colors]="категории", [ProductCategory], [ProductSubcategory])`и перетащите его в секцию **Цвет**.
