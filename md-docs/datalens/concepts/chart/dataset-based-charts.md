[Документация Yandex Cloud](../../../index.md) > [Yandex DataLens](../../index.md) > [Чарты](index.md) > Чарты в визарде > Обзор

# Чарты в визарде Yandex DataLens


Визард — это конструктор, в котором можно создавать чарты на основе данных из одного или нескольких датасетов (подробнее в [Мультидатасетные чарты](multidataset-chart.md)). Вы можете создать неограниченное количество чартов на базе одного датасета.



{% note info %}

​Чтобы посмотреть текст SQL-запроса из чарта в источник, в правом верхнем углу чарта нажмите значок ![image](../../../_assets/console-icons/ellipsis.svg) и выберите **Инспектор**. Текст запроса отображается в разделе **Источники данных** → **Запрос в источник данных**. Подробнее читайте в разделе [Инспектор чартов в Yandex DataLens](inspector.md).
...
В DataLens существуют ограничения на количество строк данных, которое могут отобразить чарты. Ознакомьтесь с ними в разделе [Квоты и лимиты в Yandex DataLens](../limits.md).
...
1. Область предпросмотра, где отображается визуализация. Для просмотра можно выбрать несколько элементов на графиках и диаграммах с помощью зажатой клавиши `Ctrl`.
