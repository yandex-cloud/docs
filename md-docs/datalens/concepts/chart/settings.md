[Документация Yandex Cloud](../../../index.md) > [Yandex DataLens](../../index.md) > [Чарты](index.md) > Чарты в визарде > Настройки чарта

# Настройки чарта в Yandex DataLens
...
Подробнее в [инструкции](../../operations/chart/config-chart-navigator.md). ||
...
|| Сортировка по строкам | Позволяет сортировать данные по строкам [сводной таблицы](../../visualization-ref/pivot-table-chart.md#rows-sorting).<br/><br/>Доступные значения:

* **Вкл** — разрешить сортировку по строкам (по умолчанию).
* **Выкл** — запретить сортировку по строкам. ||
...
|| Агрегация | Задает тип агрегации. Функции агрегации доступны в соответствии с таблицей [Агрегация данных](../../dataset/data-model.md#aggregation). ||
...
* `Markdown` — разметка [Markdown в Yandex DataLens](../../dashboard/markdown.md). Доступно только для полей с типом `Строка` в секциях в зависимости от типа визуализации.
...
|| Агрегация | Задает тип агрегации. Функции агрегации доступны в соответствии с таблицей [Агрегация данных](../../dataset/data-model.md#aggregation). Недоступно для [QL-чартов](ql-charts.md). ||
|| Формат | Задает формат отображения значения. ||
...
* **Вкл** — использовать название поля. Если секция содержит несколько полей, то DataLens использует имя поля, которое указано первым в списке.
...
* **Авто** — использовать автоматический масштаб. Вы можете указать, каким образом DataLens задает масштаб: от 0 до максимального значения поля (**Автомасштаб от 0 до max**) или от минимального до максимального значений полей (**Автомасштаб от min до max**).
* **Вручную** — указать масштаб оси вручную. Вы можете задать максимальное и минимальное значения по оси — DataLens обрежет линии чарта по этому значению. ||
|#

### Цвета {#color-settings}

В [общем случае](#color-general-settings) вы можете задать определенный цвет для какого-либо значения на графике.
...
|| Цвета | Задает цвет для значения. Доступные цвета зависят от типа градиента. ||
...
#### Общие настройки цвета {#color-general-settings}
...
1. Укажите параметры настройки:

   {% note info %}

   Для измерения используется тип заливки **Палитра**, для показателя — **Градиент**.
   ...
   - Для измерения {#measure}
     ...
     1. Задайте цвет одним из способов:

        * Выберите цветовую палитру из предустановленных или [созданную](../../operations/chart/create-palette.md) пользователем. Вы можете установить цвет для каждого измерения из предустановленных вариантов текущей цветовой палитры. Чтобы вернуть цвета по умолчанию для текущей палитры, нажмите **AUTO**.
...
### Формы {#forms-settings}

Настройка доступна только для [линейной](../../visualization-ref/line-chart.md) (в том числе в составе [комбинированной](../../visualization-ref/combined-chart.md)) и [точечной](../../visualization-ref/scatter-chart.md) диаграмм в [визарде](dataset-based-charts.md) и в [QL-чартах](ql-charts.md).

В линейной диаграмме настройка позволяет задать форму, размер, а также форму окончания и соединения линий для измерения или поля [Measure Names](measure-values.md). Чтобы настроить формы и размер линий:

1. В правом верхнем углу секции **Формы** нажмите значок ![image](../../../_assets/console-icons/gear.svg) (значок появляется при наведении указателя на секцию).
1. На вкладке **Линии** для каждого значения задайте:
  
   * Выберите из списка или введите вручную толщину линии. Доступные значения: от `1` до `12` пикселей. При выборе `Авто` будет использован размер с вкладки **Общие настройки** или размер по умолчанию.
   * Выберите форму линии из предустановленных значений. При выборе `Авто` будет использована форма с вкладки **Общие настройки** или значение по умолчанию.
...
1. На вкладке **Общие настройки** задайте толщину, форму соединения и окончания линий для всех значений с вкладки **Линии**.

   {% cut "Общие настройки линий" %}
...
|| Тултипы | Позволяют сделать подсказку, которая отобразится при наведении указателя на точку. Подсказка будет содержать значения измерений и показателей. Для полей с типом `Строка` можно настроить использование базового синтаксиса [Markdown в Yandex DataLens](../../dashboard/markdown.md): нажмите на значок перед названием поля и включите опцию **Markdown**. ||
...
При редактировании чарта в визарде или [QL-чарта](ql-charts.md) можно отменить или повторно выполнить внесенные изменения в пределах текущей [версии](versioning.md):
