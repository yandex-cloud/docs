...
Работы для Галереи готовит не только команда Yandex DataLens, но и пользователи сервиса. Просматривая работы, вы сможете изучить разнообразный опыт других аналитиков и попробовать применить его у себя.

Доступные действия с работами в Галерее:

* [поиск](#search);
* [просмотр](#review);
* [использование](#deploy) (не для всех работ) — разворачивание в своем экземпляре или экспорт в другие окружения DataLens;
* [добавление](#suggest).

## Поиск работ {#search}

На [главной странице](https://datalens.ru/gallery) галереи вы можете найти **Работу месяца**, которая отмечается значком ![image](../../_assets/datalens/award.svg), подборку лучших работ и работы популярных тематик.
...
На [странице со списком всех работ](https://datalens.ru/gallery/all) доступны:

* Поиск по названию или автору работы. Введите текст в поле поиска — появятся работы, в названии или имени автора которых есть введенная строка.
* Фильтр по категориям (учитывает фильтр по другим параметрам). В выпадающем списке отметьте категории, к которым относится работа. Фильтруются работы, в описании которых присутствуют все отмеченные категории.
* Сортировка. Выберите, какие работы показывать сначала: новые или старые.
* Фильтр по другим параметрам (учитывает фильтр по категориям). Нажмите значок ![image](../../_assets/console-icons/funnel.svg), включите нужные фильтры и нажмите кнопку **Применить**. Доступные фильтры:
  
  * **Работы партнёров**
  * **Работа месяца: история наград**
  * **Можно использовать**
...
1. В правом верхнем углу нажмите **Открыть**. Будет открыт функционирующий дашборд DataLens: вы сможете применять селекторы, переключаться между вкладками и т.д., в зависимости от возможностей, которые настроил автор дашборда.

В некоторых работах вы можете:

* обратиться напрямую к автору с помощью кнопки **Связаться с автором** вверху карточки.
* запросить помощь эксперта (описав свою задачу: предметную область проекта, источники данных, количество пользователей) с помощью кнопки **Обсудить проект** вверху карточки.
...
Работы из Галереи можно использовать двумя способами:
* Развернуть в своем экземпляре DataLens — будет создана копия воркбука со всеми объектами.
* Скачать JSON-файл для экспорта и импортировать его в стороннем окружении DataLens — в DataLens Open Source или DataLens On-premises.

{% note info %}

Вы можете развернуть работу в своем экземпляре DataLens при условии [включенных воркбуков и коллекций](../workbooks-collections/index.md#enable-workbooks).

{% endnote %}

Чтобы развернуть работу в своем экземпляре DataLens:
1. Нажмите на карточку работы.
...
* Контактная информация. Если вы участник [Партнерской программы Yandex Cloud](https://yandex.cloud/ru/partners/) и укажете почту, которую используете для авторизации в ней, то в вашей работе появится кнопка **Связаться с автором** — так пользователи смогут обратиться к вам напрямую.
