[Документация Yandex Cloud](../../index.md) > [Yandex DataLens](../index.md) > DataLens Public API > Версионирование

# Версионирование Public API в Yandex DataLens
...
Интерфейс DataLens работает только с последней версией схем, поэтому при редактировании объектов в интерфейсе они будут автоматически сохранены под последней версией.
...
Рекомендуемый сценарий обновления сущностей в Public API:

1. Получите актуальную версию объекта, используя соответствующий метод чтения.
1. Внесите необходимые изменения.
1. Отправьте объект целиком, без фильтрации полей. Сохраните все поля, включая те, которые не описаны в схеме. Это поможет сохранить совместимость с различными версиями API.
