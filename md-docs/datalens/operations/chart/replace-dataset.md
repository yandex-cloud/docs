[Документация Yandex Cloud](../../../index.md) > [Yandex DataLens](../../index.md) > Чарты > Чарты в визарде > Пошаговые инструкции > Замена датасета в чарте

# Замена датасета в чарте Yandex DataLens
...
1. Выберите другой датасет.
   ...
   Новый датасет должен содержать поля с теми же названиями, что и старый. Если после замены датасета чарт отображается с ошибкой `ERR.DS_API.DB.COLUMN_DOES_NOT_EXIST`, воспользуйтесь инструкцией [Обновить поля](../../dataset/create-dataset.md#update-fields).
