...
1. В разделе **Базы данных** выберите подключение **Prometheus**.
1. Выберите режим аутентификации и укажите параметры подключения:
   ...
   - Пароль {#password}
     ...
     * **Имя хоста**. Укажите путь до хоста Prometheus. Вы можете указать несколько хостов через запятую. Если к первому хосту подключиться не получится, DataLens выберет следующий из списка.
     * **Порт**. Укажите порт подключения к Prometheus.
     * **URL путь**. (опционально) Укажите путь к API Prometheus (часть [URL-адреса](https://developer.mozilla.org/ru/docs/Learn/Common_questions/Web_mechanics/What_is_a_URL) без протокола, имени хоста и порта).
     * **Имя пользователя**. Укажите имя пользователя для подключения к Prometheus.
   ...
   - Заголовок {#header}
     ...
     * **Порт**. Укажите порт подключения к Prometheus.
     * **URL путь**. (опционально) Укажите путь к API Prometheus (часть [URL-адреса](https://developer.mozilla.org/ru/docs/Learn/Common_questions/Web_mechanics/What_is_a_URL) без протокола, имени хоста и порта).
     * **Заголовок Authorization**. Укажите токен для авторизации.
     * **Время жизни кеша в секундах**. Укажите время жизни кеша или оставьте значение по умолчанию. Рекомендованное значение — 300 секунд (5 минут).
     * **HTTPS**. Активируйте опцию безопасного подключения, если ваш сервер поддерживает протокол HTTPS.
