[Документация Yandex Cloud](../../../index.md) > [Yandex DataLens](../../index.md) > Подключения > Файлы и сервисы > Создание подключения API Connector
...
1. Укажите параметры подключения:
   ...
   * **Разрешённые методы**. Выберите метод для HTTP-запроса к API. Доступны методы: `GET`, `POST`, `PUT`, `PATCH` и `DELETE`.
...
API может возвращать данные в любом формате. Обработку данных выполнит [Editor](../../charts/editor/index.md), поэтому убедитесь, что код в чарте Editor готов к такому формату. Пример работы с ответом API приведен в руководстве по [созданию таблицы через API](../../charts/editor/quickstart/from-api-connector.md).
