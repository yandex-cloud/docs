...
Назначить права доступа на связанные объекты может только пользователь с правом доступа `{{ permission-admin }}` на них. Если нет доступа на администрирование связанного объекта, то создается запрос на доступ. Запрос прав доступа должен [подтвердить](#confirm-request) пользователь с правом доступа `{{ permission-admin }}` на объект.
...
1. Назначьте права доступа:
   ...
   - На папку и содержимое
     ...
     1. Укажите права доступа и нажмите кнопку **Применить**.
        ...
        Пользователь с правом доступа `{{ permission-admin }}` на родительскую папку может выдать доступ к папке и вложенным объектам.
