[Документация Yandex Cloud](../../index.md) > [Yandex DataLens](../index.md) > [Воркбуки и коллекции](index.md) > Ошибки и предупреждения при экспорте и импорте

# Ошибки и предупреждения при экспорте и импорте в Yandex DataLens
...
Критическая ошибка. В DataLens у каждой сущности есть обязательные поля и их форматы. Ошибка указывает на отсутствие обязательных полей или неправильный тип данных. Исправьте конфигурацию или попробуйте заново экспортировать воркбук.
...
#### Инсталляция DataLens не поддерживает конфигурацию воркбука {#export-installation-error}

Версия чарта или дашборда несовместима с инсталляцией DataLens, в которую производится импорт. Например, экспорт выполнен из новой облачной версии DataLens, а импорт осуществляется в устаревшую версию open source DataLens.
...
Ошибка может возникать при экспорте подключений между разными инсталляциями DataLens, подключений с некорректным типом или подключений, которые более не поддерживаются.
...
#### В воркбуке есть объекты, которые не поддерживаются в вашей инсталляции DataLens {#import-object-not-supported}

Например, при импорте воркбука, содержащего отчет или чарт в Editor, в инсталляцию open source DataLens, где они недоступны.


#### Настройка подключений не завершена, измените или повторно сохраните настройки подключений {#import-connection}

При импорте подключений с использованием сервисного аккаунта или подключения Connection Manager DataLens не может завершить настройку созданного подключения. После импорта повторно сохраните такие подключения без внесения изменений в настройки.
...
#### Инсталляция DataLens не поддерживает конфигурацию воркбука {#import-installation-error}
...
Ошибка может возникать при импорте подключений между разными инсталляциями DataLens, подключений с некорректным типом или подключений, которые более не поддерживаются.
