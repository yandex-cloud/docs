[Документация Yandex Cloud](../../index.md) > [Yandex DataLens](../index.md) > [Воркбуки и коллекции](index.md) > Экспорт и импорт воркбуков

# Экспорт и импорт воркбуков Yandex DataLens

В DataLens можно переносить [воркбуки](index.md) между инсталляциями с помощью инструментов экспорта и импорта. Например, вы сможете экспортировать готовый воркбук, с его настройками и связями, из облачного Yandex DataLens в инсталляцию [open source](https://datalens.tech).
...
* Экспорт может выполнить пользователь с ролью `datalens.admin`.
...
1. Нажмите кнопку **Экспортировать**. Прогресс будет отображаться на индикаторе. Обратите внимание на следующее:
   ...
   * В других версиях DataLens могут не поддерживаться некоторые типы подключений. При импорте такие подключения работать не будут.
...
Вы можете внести изменения в экспортированный файл JSON, например добавить пароль для подключения. Но DataLens не гарантирует успешный импорт и корректную работу воркбука, если в его конфигурацию были внесены ручные изменения. Рекомендуем тщательно проверять все изменения перед импортом для минимизации возможных проблем.
...
По умолчанию пользователи с ролью `datalens.admin` могут экспортировать воркбуки. Эту возможность можно отключить на уровне экземпляра DataLens. После запрета экспорта:
...
Отключить возможность экспорта может только [администратор](../security/roles.md#datalens-admin) экземпляра DataLens (роль `datalens.admin`).
