[Документация Yandex Cloud](../../index.md) > [Yandex DataLens](../index.md) > [Практические руководства](index.md) > Аналитика сервисов > AppMetrica: прямое подключение
...
* Прямой доступ.
  DataLens напрямую подключается к AppMetrica. Этот метод проще и быстрее всего, однако набор поддерживаемых функций [ограничен](../function-ref/availability.md).
* Экспорт в режиме реального времени из AppMetrica в Managed Service for ClickHouse®.
  Доступны сырые данные и есть возможность предагрегации и постобработки данных на уровне БД. Для реализации этого варианта [настройте экспорт](https://appmetrica.yandex.ru/docs/common/cloud/about.html) и [создайте подключение](../operations/connection/create-clickhouse.md) DataLens к БД ClickHouse®.
