[Документация Yandex Cloud](../../index.md) > [Yandex DataLens](../index.md) > [Практические руководства](index.md) > Аналитика сервисов > Битрикс24: прямое подключение
...
В этом руководстве вы создадите подключение к демонстрационному источнику данных Битрикс24 и изучите дашборд и чарты, которые Битрикс24 готовит на базе DataLens. Также вы научитесь работать с [датасетами](../dataset/index.md), [чартами](../concepts/chart/index.md) и [дашбордами](../concepts/dashboard.md).
...
1. Введите название [воркбука](../workbooks-collections/index.md) — `Битрикс24`.
1. Нажмите кнопку **Создать**.
...
1. Чтобы подключиться к демонстрационному источнику данных, укажите:
   ...
   * **Токен** — `wJZIFjdYtbLRtfewYcnq71oSFyEgxlbQ`
   ...
   При подключении к рабочему источнику данных [Битрикс24](../operations/connection/create-bitrix24.md), укажите:

   * **Портал** — адрес вашего портала Битрикс24 в формате `test.bitrix24.ru`.
   * **Токен** — получите секретный ключ в Битрикс24, в разделе **CRM** → **Аналитика** → **BI-аналитика** на вкладке **Yandex DataLens**. Подробнее в [инструкции](https://helpdesk.bitrix24.ru/open/17402692).
...
1. Откройте датасет `Лиды`.
   ...
   * **Имя** — название поля.
   * **Тип** — [тип данных](../dataset/data-types.md) поля.
   * **Источник поля** — обозначение поля в таблице источника или формула для [вычисления](../concepts/calculations/index.md) (обозначены значком ![formula-dataset](../../_assets/console-icons/function.svg)).
   * **Агрегация** — показывает [функцию агрегации данных](../dataset/data-model.md#aggregation), если она применена к полю.
   <br>
...
1. Создайте поле с количеством записей — `ID_COUNT`:
   ...
   1. В столбце **Агрегация** для поля `ID_COUNT` выберите **Количество**. Поле станет [показателем](../concepts/chart/measure-values.md) и поменяет цвет на синий.

   {% note info %}

   Вы можете добавить, переименовать поля, создать [вычисляемые поля](../concepts/calculations/index.md), добавить к ним описания.
...
1. Перейдите в воркбук `Битрикс24` и откройте `Дашборд Битрикс24` или [создайте дашборд](../operations/dashboard/create.md).
