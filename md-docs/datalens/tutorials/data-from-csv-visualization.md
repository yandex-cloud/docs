...
Создайте [датасет](../dataset/index.md) на базе подключения `SuperHeroes`:
...
Для визуализации разделения супергероев по принадлежности к лагерю хороших или плохих создайте [чарт](../concepts/chart/index.md) — [столбчатую диаграмму](../visualization-ref/column-chart.md):
...
Для отображения количества героев каждого пола создайте [кольцевую диаграмму](../visualization-ref/ring-chart.md).
...
Добавьте [селекторы](../dashboard/selector.md), чтобы фильтровать супергероев по цвету глаз (поле `Eye color`) и цвету волос (поле `Hair color`):
