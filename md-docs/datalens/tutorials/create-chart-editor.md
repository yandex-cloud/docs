...
С помощью редактора можно создать отдельные объекты DataLens:
...
В качестве источника данных будет использовано прямое подключение к демонстрационной базе данных ClickHouse® с информацией о продажах товаров в сети московских магазинов.
...
1. Введите название [воркбука](../workbooks-collections/index.md) — `Чарты в Editor`.
1. Нажмите кнопку **Создать**.
...
1. Чтобы получить данные из источника, на вкладке **Source** введите код:
   ...
   * `params` — объект, в который передаются параметры с помощью метода [Editor.getParams()](../charts/editor/methods.md#get-params).
   ...
   * `from_date` и `to_date` — даты начала и окончания периода, получаемые из `date_interval` с помощью метода [Editor.resolveInterval(arg)](../charts/editor/methods.md#resolve-interval).
...
Создайте [дашборд](../concepts/dashboard.md) и добавьте на него чарт:
...
1. На вкладке **Source** сформируйте запрос для получения данных с помощью модуля `buildSource`.
   ...
   1. В объект `params` получите значения параметров с помощью метода [Editor.getParams()](../charts/editor/methods.md#get-params) и для удобства задайте отдельную переменную для каждого параметра.
   ...
   1. Обработайте фильтры и параметры с дашборда:
      ...
      * Все параметры и фильтры приходят в чарт в виде параметров. Обработайте их в одном цикле.
        ...
        * Параметр, указанный в списке `filter_fields` на вкладке `Params`, добавьте в массив `where`:
          ...
          * Значения, начинающиеся на `__` парсятся с помощью метода [Editor.resolveOperation(args)](../charts/editor/methods.md#resolve-oper) для составления выражения. Если значений несколько, они добавляются в виде массива:
          ...
          * Интервальные значения дат обрабатываются с помощью метода [Editor.resolveInterval(arg)](../charts/editor/methods.md#resolve-interval):
          ...
          * Относительные даты обрабатываются с помощью метода [Editor.resolveRelative(arg)](../charts/editor/methods.md#resolve-relative):
        ...
        * Параметр, указанный в списке `params_to_send` на вкладке `Params`, добавьте в массив `field_params`:
          ...
          Чтобы использовать передачу параметров в датасет, включите [параметризацию источников](../dataset/parametrization.md).

          Пример использования параметров в источнике смотрите в практическом руководстве [Параметризация источников в DataLens](data-from-ch-dataset-parametrization.md).
...
Чтобы добавить инерактивные элементы, используйте [Advanced-чарт](../charts/editor/widgets/advanced.md), который позволяет создавать HTML с безопасной разметкой через функцию [Editor.generateHtml(args)](../charts/editor/methods.md#gen-html) и гибкие SVG-визуализации с помощью библиотек `d3`, `d3-chord` и `d3-sankey`.

Создайте [Advanced-чарт](../charts/editor/widgets/advanced.md) и добавьте на него кнопки для выбора метрик и тултипы.
...
1. На вкладке **Prepare** задайте начальные параметры и добавьте код, выполняющий отрисовку элементов и обработку событий чарта.
   ...
   1. В модуле `module.exports` с помощью функции [Editor.wrapFn(args)](../charts/editor/methods.md#wrap) опишите:
