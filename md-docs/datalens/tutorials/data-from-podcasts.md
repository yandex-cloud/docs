...
1. [Настройте подключение](#configure-connection).
   ...
   1. [Изучите статистику подкастов в {{ datalens-short-name }}](#podcasts-in-datalens). 
...
1. [Поделитесь аналитикой по подкасту с другими пользователями](#share-analytics).
   ...
   1. [Предоставьте доступ к {{ datalens-short-name }}](#open-access).
   1. [Поделитесь аналитикой по отдельному подкасту или эпизоду](#share-analytics-on-podcast-or-episode).
      ...
      1. [Предоставьте доступ к вашему экземпляру {{ datalens-short-name }}](#access-to-datalens-instance).
...
1. Откройте [главную страницу сервиса]({{ link-datalens-main-skip-promo }}).
...
В сервисе {{ datalens-full-name }} действует единый тарифный план: бесплатно для индивидуального использования, а для командной работы оплата ведется по количеству [рабочих мест](../pricing.md#seats).

Если необходимо, докупите рабочие места для пользователей, с которыми хотите поделиться аналитикой по подкасту.

Подробнее о [правилах тарификации](../pricing.md) для {{ datalens-short-name }}.
...
1. Убедитесь, что у нового пользователя есть права доступа к дашборду со статистикой по подкастам в зависимости от размещения:
   ...
   - Воркбук {#workbook}
     ...
     1. Нажмите кнопку **Сохранить**. Пользователю или группе будут назначены права на данный объект.
        ...
        Подробнее о [ролях в {{ datalens-short-name }}](../security/roles.md).
...
Данные в {{ datalens-short-name }} доступны только пользователям конкретного экземпляра. Если требуется предоставить публичный неограниченный доступ к аналитике подкастов, [опубликуйте](../concepts/datalens-public.md) дашборд или отдельный чарт.
...
{% cut "Число подписчиков на сервисе больше, чем указано в {{ datalens-short-name }}. Почему?" %}
