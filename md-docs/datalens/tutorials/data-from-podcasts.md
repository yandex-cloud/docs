...
### 1.3. Изучите статистику подкастов в DataLens {#podcasts-in-datalens}
...
1. После создания подключения откроется папка с набором стандартных объектов, на основе которых построена ваша статистика:
   ...
   * ![image](../../_assets/datalens/solution-10/03-datasets.png =20x20) Датасеты — наборы измерений и показателей, на основе которых строятся чарты.
     ...
     * `D01. Прослушивания подкастов` — датасет с данными про прослушиваниям.
     * `D02. Подписчики подкастов` — датасет с данными по подписчикам.
...
1. Откройте [главную страницу сервиса](https://datalens.ru/?skipPromo=true).
...
В сервисе Yandex DataLens действует единый тарифный план: бесплатно для индивидуального использования, а для командной работы оплата ведется по количеству [рабочих мест](../../datalens/pricing.md#seats).

Если необходимо, докупите рабочие места для пользователей, с которыми хотите поделиться аналитикой по подкасту.

Подробнее о [правилах тарификации](../../datalens/pricing.md) для DataLens.

### 3.1. Предоставьте доступ к DataLens {#open-access}

Вы можете предоставить доступ к личному кабинету другим пользователям. Для этого понадобится только аккаунт на Яндексе.

Настройте доступ к Yandex DataLens:

1. Откройте страницу [Управление доступом](https://console.yandex.cloud/iam).
...
1. Убедитесь, что у нового пользователя есть права доступа к дашборду со статистикой по подкастам в зависимости от размещения:

   {% list tabs group=navigation %}

   - Воркбук {#workbook}

     Права на воркбук распространяются на все объекты внутри него. Предоставьте доступ к воркбуку, в котором находится дашборд со статистикой по подкастам:

     1. Вверху воркбука нажмите ![image](../../_assets/console-icons/lock-open.svg) **Доступ**.
     1. Права должны быть у конкретного пользователя или у группы, в которую он входит. Если прав нет, нажмите кнопку **Добавить пользователя**.
     1. В открывшемся окне выберите пользователя или группу, укажите право доступа к воркбуку:

        * `Ограниченный просмотр` — позволяет просматривать вложенные в воркбук чарты и дашборды, а также информацию о назначенных правах доступа к нему.
        * `Просмотр` — позволяет просматривать все вложенные в него объекты, а также информацию о назначенных правах доступа к ним.
        * `Редактирование` — позволяет редактировать воркбук и все вложенные в него объекты.
        * `Администрирование` — позволяет управлять воркбуком, доступом к нему и всеми вложенными в него объектами.

     1. Нажмите кнопку **Сохранить**. Пользователю или группе будут назначены права на данный объект.
        ...
        Подробнее о [ролях в DataLens](../security/roles.md).
   ...
   - Папка {#folder}
     ...
     1. Откройте дашборд.
     1. Вверху нажмите ![image](../../_assets/console-icons/ellipsis.svg) → ![image](../../_assets/console-icons/persons.svg) **Права доступа**.
     1. Права должны быть у группы **Все** или у конкретного пользователя. Если прав нет, нажмите кнопку **Права на дашборд**.
     1. В поле **Добавить участника** введите имя пользователя или группу **Все**.
     1. В разделе **Текущий объект** установите права доступа для дашборда `Аналитика подкастов на Яндекс Музыке`:
     ...
     1. Включите опцию **Связанные объекты**, чтобы предоставить права на другие объекты, связанные с дашбордом (подключение, датасеты и чарты).
        ...
        Права доступа можно настраивать для каждого [объекта](../security/manage-access.md) отдельно.
...
- Воркбук {#workbook}
  ...
  1. Откройте воркбук с дашбордом `Аналитика подкастов на Яндекс Музыке`.
  1. Вверху воркбука нажмите ![image](../../_assets/console-icons/lock-open.svg) **Доступ**.
  1. Нажмите кнопку **Добавить пользователя**.
  1. В открывшемся окне выберите системную группу `All users in organization <название_организации>` и укажите право доступа на воркбук `Ограниченный просмотр`. Общий доступ будет ограничен: другие пользователи смогут посмотреть статистику на дашборде, например, только по отдельным эпизодам, но не смогут поменять эти настройки.  
...
#### 3.2.3. Предоставьте доступ к вашему экземпляру DataLens {#access-to-datalens-instance}

Чтобы предоставить доступ пользователю к вашему экземпляру DataLens:
...
1. Выберите, к каким данным вы хотите ограничить доступ для отдельного пользователя:
   ...
   Подробнее о [правах доступа на уровне данных](../security/row-level-security.md).

   {% note info %}
    
   Указывайте такие же названия, как на сервисе (строчные и заглавные буквы имеют значение при фильтрации).
...
- Воркбук {#workbook}
  ...
  1. Откройте воркбук с дашбордом `Аналитика подкастов на Яндекс Музыке` и вверху справа нажмите ![image](../../_assets/console-icons/lock-open.svg) **Доступ**.
...
- Папка {#folder}
  ...
  1. Чтобы полностью ограничить доступ пользователя к вашему кабинету статистики подкастера:

     1. Откройте страницу [Управление доступом](https://console.yandex.cloud/iam).
     1. Найдите пользователя, нажмите на значок напротив нового пользователя и выберите **Удалить**.

{% endlist %}

## 4. Сделайте публичным дашборд или чарт {#publish-dashboard-chart}

Данные в DataLens доступны только пользователям конкретного экземпляра. Если требуется предоставить публичный неограниченный доступ к аналитике подкастов, [опубликуйте](../concepts/datalens-public.md) дашборд или отдельный чарт.
...
Использование аккаунта на Яндексе в Яндекс Музыке и DataLens обеспечивает высокие меры безопасности данных.
