...
- Интерфейс {{ datalens-short-name }} {#console}
  ...
  1. Выберите коннектор **{{ PG }}**.
      ...
      При подключении к внешнему источнику данных (который не является ресурсом {{ yandex-cloud }}) предоставьте доступ к источнику [для диапазонов IP-адресов сервиса {{ datalens-name }}](../concepts/connection/index.md#changing-connection-ranges).
