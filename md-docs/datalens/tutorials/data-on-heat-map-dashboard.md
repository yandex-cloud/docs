...
Данные в {{ datalens-short-name }} доступны только пользователям конкретного экземпляра. Если требуется предоставить публичный неограниченный доступ к статистике спроса и предложения, опубликуйте дашборд или отдельный чарт в [{{ datalens-short-name }} Public](../concepts/datalens-public.md).
