[Документация Yandex Cloud](../../index.md) > [Yandex DataLens](../index.md) > [Датасеты](index.md) > Настройки датасета

# Настройки датасета в Yandex DataLens
...
Источник данных датасета можно определять произвольными SQL-запросами над подключениями к БД. Текст запроса при обращении к источнику данных исполняется в виде подзапроса. Подробнее о том, как использовать SQL-запросы в датасете, читайте в разделе [Добавить данные](create-dataset.md#add-data).
...
Параметризация источников датасета позволяет динамически подставлять параметры из чарта в SQL-запрос или название таблицы или схемы. Подробнее на странице [Параметризация источников датасета в Yandex DataLens](parametrization.md).
...
* [Вычисляемые поля в Yandex DataLens](../concepts/calculations/index.md)