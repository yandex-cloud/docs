[Документация Yandex Cloud](../../index.md) > [Yandex DataLens](../index.md) > [История изменений](index.md) > Архив > Сентябрь 2024

# История изменений в Yandex DataLens в сентябре 2024
...
В подключениях к [ClickHouse®](../operations/connection/create-clickhouse.md), [PostgreSQL](../operations/connection/create-postgresql.md), [MySQL®](../operations/connection/create-mysql.md), [Greenplum®](../operations/connection/create-greenplum.md) добавлена возможность выбирать особый FQDN, который указывает на текущий мастер или наименее отстающую реплику.

Для обычных FQDN теперь можно указать несколько хостов для подключения.

### Интеграция с Connection Manager {#dash-link-settings}

Добавлена интеграция с сервисом [Connection Manager](../../metadata-hub/concepts/connection-manager.md) для подключений:
...
В DataLens появились [отчеты](../reports/index.md) — многостраничные документы, которые можно экспортировать в PDF и распечатать. На страницы отчета можно добавлять чарты, текстовые блоки, заголовки и изображения.
