...
1. В исходные данные добавьте новое поле для хранения ID пользователя DataLens. По этому полю будет происходить фильтрация всех запросов в источник.
   ...
   Свой ID можно посмотреть [по ссылке](https://center.yandex.cloud/organization/users). Если вам нужен ID другого пользователя, попросите его открыть эту ссылку и передать ID вам.
