...
- После перехода на воркбуки и коллекции {#after-workbooks-collections}
  ...
  Роль `datalens.visitor` предоставляет доступ к сервису {{ datalens-name }}. Просмотр или редактирование [воркбуков и коллекций](../workbooks-collections/index.md) возможны при наличии соответствующих [ролей](#workbooks-collections-roles) для доступа к этим воркбукам и коллекциям.

  #### datalens.creator {#datalens-creator}

  Роль `datalens.creator` предоставляет доступ к сервису {{ datalens-name }} с правами на создание [воркбуков и коллекций](../workbooks-collections/index.md) в корне {{ datalens-name }}. Просмотр или редактирование воркбуков и коллекций, созданных другими пользователями, возможны только при наличии [прав доступа](#workbooks-collections-roles) к этим воркбукам и коллекциям.
  
  Включает разрешения, предоставляемые ролью `datalens.visitor`.

  #### datalens.admin {#datalens-admin}

  Роль `datalens.admin` предоставляет полный доступ к сервису {{ datalens-name }} и всем [воркбукам и коллекциям](../workbooks-collections/index.md) в нем.
...
- До перехода на воркбуки и коллекции {#before-workbooks-collections}
  ...
  Роль `datalens.instances.user` предоставляет доступ к сервису {{ datalens-name }} в качестве пользователя с правами на создание, чтение и изменение [объектов](../concepts/index.md#component-interrelation) согласно правам доступа к ним, а также позволяет просматривать информацию о [каталогах](../../resource-manager/concepts/resources-hierarchy.md#folder).
  
  После назначения сервисной роли вы можете [назначить](../operations/permission/grant.md) пользователю права доступа к объектам и папкам в сервисе {{ datalens-name }}.
  ...
  Роль `datalens.instances.admin` предоставляет доступ к сервису {{ datalens-name }} в качестве администратора экземпляра {{ datalens-name }}. Администратор получает полные права на все [объекты](../concepts/index.md#component-interrelation) и папки в сервисе {{ datalens-name }}, доступ к настройкам {{ datalens-name }}, а также может просматривать информацию о [каталогах](../../resource-manager/concepts/resources-hierarchy.md#folder).
...
Роль `datalens.metaReader` позволяет выполнять запросы в [{{ datalens-name }} Public API](../operations/api-start.md) из раздела [Audit](https://api.datalens.tech/#/Audit), а также запросы для получения сущностей {{ datalens-name }}.
