...
* `custom` — конфигурация параметров ячейки для кросс-фильтрации. Подробнее см. в разделе [Кросс-фильтрация](../cross-filtration.md).
...
* `events` — позволяет задать конфигурацию для кросс-фильтрации для таблицы, которую можно переопределить на урочне конкретной ячейки с помощью поля `custom`. Подробнее см. в разделе [Кросс-фильтрация](../cross-filtration.md).
