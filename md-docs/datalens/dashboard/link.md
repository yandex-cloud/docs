[Документация Yandex Cloud](../../index.md) > [Yandex DataLens](../index.md) > Дашборды > Связь

# Связь в Yandex DataLens
...
Поле, по которому фильтрует селектор, должно присутствовать в датасете, по которому построен чарт. В противном случае связь работать не будет. Подробнее в разделе [Объединение данных Yandex DataLens](../concepts/data-join.md).

Тип связи определяет, как будут отфильтрованы значения в селекторах и чартах. Тип задается в окне связей в режиме редактирования дашборда. Подробнее о том, как настроить связь, читайте в разделе [Настройка связей на дашборде в Yandex DataLens](../operations/dashboard/dashboard-links.md).
...
Нельзя создать связь с виджетами [Текст](widget.md#text) и [Заголовок](widget.md#title).
...
* Установить связи между селектором с типом **На основе датасета** из одного датасета и параметром из другого датасета или чарта на его основе, чтобы значения селектора изменяли значения параметра.
...
Чтобы установить связь с чартом, созданным в Editor, или селектором (JS), добавьте в список параметров виджета ([чарта](../operations/chart/add-parameters.md) или [селектора (JS)](../charts/editor/add-js-selector.md)) на дашборде параметры, указанные на вкладке [Params](../charts/editor/tabs.md#params).
...
Подробнее о том, как создать алиас, читайте в разделе [Создание алиаса в Yandex DataLens](../operations/dashboard/create-alias.md).

Из алиаса, в котором связано более двух полей, можно удалять связи. Удалите поле из алиаса, если связь по нему между датасетами или между виджетами в пределах вкладки дашборда не нужна. Подробнее в разделе [Удаление алиаса в Yandex DataLens](../operations/dashboard/edit-alias.md).