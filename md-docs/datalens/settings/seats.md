...
В разделе **Запросы из непублично встроенных объектов** отображается общее количество запросов из непублично встроенных чартов в текущем месяце для всего экземпляра {{ datalens-short-name }}.

Действует ограничение: 2 000 запросов в месяц из непублично встроенных чартов на одно рабочее место. Это ограничение действует не для каждого отдельно взятого рабочего места, а для всего экземпляра {{ datalens-name }} из расчета:

   > 2 000 запросов × Количество рабочих мест

Считаются только успешно выполненные запросы на получение данных для отрисовки чартов. Если для отрисовки одного чарта выполняется несколько запросов, это учитывается как один запрос. Подробнее см. [{#T}](../pricing.md#packet-queries).
