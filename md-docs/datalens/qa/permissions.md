...
* Пользователю назначена роль `{{ roles-datalens-visitor }}` в [организации](../concepts/organizations.md) для доступа к сервису {{ datalens-short-name }} в ней.
...
Подробнее про права доступа к объектам {{ datalens-short-name }} смотрите в [документации](../security/index.md).
...
После отзыва токена подключения, в которых он использовался, перестанут работать (см. ошибку [{#T}](../troubleshooting/errors/ERR-DS_API-SOURCE_ACCESS_DENIED-INVALID_TOKEN.md)).