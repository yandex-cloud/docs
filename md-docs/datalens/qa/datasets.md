...
### Как с помощью {{ datalens-short-name }} изменить данные в источнике? {#how-to-change-source-data}

С помощью {{ datalens-short-name }} нельзя изменить данные в источнике.

Вы можете обработать полученные из источника данные на стороне {{ datalens-short-name }}, использовав [вычисляемые поля](../concepts/calculations/index.md).
...
Материализация теперь недоступна. Мы рекомендуем работать с базой данных напрямую. Если БД медленно отвечает на аналитические запросы {{ datalens-name }}, рекомендуем сделать [витрины данных](../../glossary/datamart.md), например, на базе [{{ mch-full-name }}](../../managed-clickhouse/index.md). Для загрузки данных можно использовать сервис [{{ data-transfer-full-name }}](../../data-transfer/index.md).
