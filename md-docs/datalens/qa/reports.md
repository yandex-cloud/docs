...
{{ datalens-short-name }} поддерживает селекторы в [отчетах](../reports/index.md), что позволяет настраивать фильтрацию данных. Вы можете добавить в отчет обычный [селектор](../dashboard/selector.md) и [селектор JS](../charts/editor/widgets/controls.md). Перед добавлением селектора убедитесь, что у вас есть право доступа `{{ permission-write }}` или `{{ permission-admin }}` в отчете. Подробнее см. в разделе [{#T}](../security/manage-access.md).
