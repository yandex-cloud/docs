[Документация Yandex Cloud](../../index.md) > [Партнерская программа B2B Tech](../index.md) > Партнерские программы > Реферальная программа
...
Работать с реферальными ссылками и выводить вознаграждения за рефералов можно на [Партнёрском портале](https://partners.yandex.cloud/) в разделе **Реферальная программа**. Чтобы перейти в этот раздел, в верхней части экрана переключитесь на реферальную программу.
...
1. Пройти по реферальной ссылке, зарегистрироваться в Yandex Cloud и создать платежный аккаунт.
   ...
   * Если пользователь закроет вкладку и откроет консоль без использования реферальной ссылки (например, https://console.yandex.cloud), то регистрация не будет зачтена.
...
Размер вознаграждения партнера составляет 10% от суммы продаж рефералам с учетом НДС, но не более 100 000 ₽ в месяц за одного клиента. Вы можете отслеживать потребление привлеченных вами реферальных клиентов в личном кабинете на [Партнёрском портале](https://partners.yandex.cloud/).
