[Документация Yandex Cloud](../../index.md) > [Yandex Virtual Private Cloud](../index.md) > [Пошаговые инструкции](index.md) > Статическая маршрутизация > Перенести таблицу маршрутизации между каталогами
...
- CLI {#cli}
  ...
  1. Переместите таблицу маршрутизации, указав имя или идентификатор таблицы и каталога назначения:
     ...
     Если таблица находится не в текущем каталоге (каталоге по умолчанию), укажите исходный каталог с помощью параметра `--folder-name` или `--folder-id`.
     ...
     ```
     Подробнее о команде `yc vpc route-table move` в [справочнике CLI](../../cli/cli-ref/vpc/cli-ref/route-table/move.md).
