...
* Инфраструктура на стороне [Virtual Private Cloud](../index.md), которая включает в себя:

    * облачную сеть `sample-network`;
    * [подсети](../concepts/network.md#subnet) `vpc-subnet-a` и `vpc-subnet-b` в [зонах доступности](../../overview/concepts/geo-scope.md) `ru-central1-a` и `ru-central1-b`;
...
Настройте инфраструктуру на стороне Virtual Private Cloud. На этом этапе вы создадите [облачную сеть](../concepts/network.md#network), [подсети](../concepts/network.md#subnet), необходимые [группы безопасности](../concepts/security-groups.md) и [виртуальную машину](../../compute/concepts/vm.md).
