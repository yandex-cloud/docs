...
1. В левом меню в секции **Службы** выберите **Сетевое время** → **Общие**. В разделе **Серверы времени** укажите адреса NTP-серверов ([список рекомендуемых NTP-серверов](../../tutorials/infrastructure-management/ntp.md)). Активируйте чекбокс напротив предпочтительного сервера в столбце **Предпочитать**. Нажмите **Сохранить**.
...
* Не назначайте публичные IP-адреса на ВМ в сегментах, где используются таблицы маршрутизации с маршрутом по умолчанию `0.0.0.0/0` через Смарт-Софт TING ([подробности](../concepts/routing.md#restrictions)). Исключение — сегмент управления `mgmt`, где в таблицах маршрутизации не используется маршрут по умолчанию `0.0.0.0/0`. 
