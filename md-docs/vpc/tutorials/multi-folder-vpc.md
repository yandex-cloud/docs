...
1. Создайте [облачную сеть](../concepts/network.md):
   ...
   - Консоль управления {#console}
     ...
     1. Отключите опцию [{{ ui-key.yacloud.vpc.networks.create.field_is-default }}](../operations/subnet-create.md), чтобы создать подсети вручную.
     1. Нажмите кнопку **{{ ui-key.yacloud.vpc.networks.button_create }}**.
...
1. Создайте [подсети](../concepts/network.md#subnet) `subnet-a`, `subnet-b` и `subnet-d` в [зонах доступности](../../overview/concepts/geo-scope.md) `{{ region-id }}-a`, `{{ region-id }}-b` и `{{ region-id }}-d` соответственно:
