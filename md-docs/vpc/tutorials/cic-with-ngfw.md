...
В данном документе рассмотрены настройки [маршрутизации](../concepts/routing.md) в облачной сети и настройки [приватного соединения](../../interconnect/concepts/priv-con.md) {{ interconnect-name }}, которые позволяют обеспечить сетевую связность инфраструктуры клиента с сегментами, размещенными за NGFW.
...
Настройте [таблицы маршрутизации](../concepts/routing.md#rt-vpc) в {{ vpc-short-name }} `interconnect` и примените их к подсетям, в которых находятся сетевые интерфейсы основного и резервного NGFW, в соответствии с таблицами ниже.
