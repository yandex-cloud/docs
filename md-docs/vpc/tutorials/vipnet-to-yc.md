...
ViPNet Coordinator VA устанавливается на [ВМ](../../glossary/vm.md) с четырьмя сетевыми интерфейсами. Один интерфейс получает [публичный](../concepts/address.md#public-addresses) IP-адрес, на нем работает туннель до шлюза на удаленной площадке. Остальным интерфейсам можно выдать [внутренние](../concepts/address.md#internal-addresses) адреса и подключить к разным [подсетям](../concepts/network.md#subnet) одной зоны доступности.
...
Если у вас еще нет [облачной сети](../concepts/network.md#network), создайте ее в каталоге для ViPNet Coordinator VA, например, в каталоге `vipnet-folder`:
...
Создайте [подсеть](../concepts/network.md#subnet) `public-subnet` для внешнего интерфейса ViPNet Coordinator VA:
