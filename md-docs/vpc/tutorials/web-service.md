...
В этом руководстве вы развернете и настроите инфраструктуру базового интернет-сервиса с несколькими виртуальными машинами. Подключение к инфраструктуре будет выполняться с удаленной площадки. Доступ к ВМ будет ограничен с помощью [групп безопасности](../concepts/security-groups.md). Нагрузка по серверам с веб-приложениями будет распределяться [сетевым балансировщиком](../../network-load-balancer/concepts/index.md). 
...
1. [Создайте сеть](../operations/network-create.md) `remote-net`. При создании сети отключите опцию **{{ ui-key.yacloud.vpc.networks.create.field_is-default }}**.
1. [Создайте подсеть](../operations/subnet-create.md) для тестовой ВМ на удаленной площадке со следующими параметрами:
...
- Консоль управления {#console}
  ...
  1. [Создайте облачную сеть](../operations/network-create.md) `network`. При создании сети отключите опцию **{{ ui-key.yacloud.vpc.networks.create.field_is-default }}**.
  1. В сети `network` [создайте подсети](../operations/subnet-create.md) со следующими параметрами:
...
- Консоль управления {#console}
    ...
    1. В блоке **{{ ui-key.yacloud.vpc.network.security-groups.forms.label_section-rules }}** [создайте](../operations/security-group-add-rule.md) правила в соответствии с таблицей:
...
Создайте [таблицу маршрутизации](../concepts/routing.md#rt-vpc) и добавьте в нее [статические маршруты](../concepts/routing.md#static):
...
1. [Добавьте](../operations/security-group-add-rule.md) в группу безопасности `web-service-sg` два новых правила для входящего трафика:
...
1. [Удалите](../operations/address-delete.md) зарезервированные статические публичные IP-адреса.
1. При необходимости удалите [таблицу маршрутизации](../operations/delete-route-table.md), [группы безопасности](../operations/security-group-delete.md), [подсети](../operations/subnet-delete.md) и [сети](../operations/network-delete.md).