...
* [Виртуальная сеть](#overlay) — работает поверх физической сетевой инфраструктуры. Сервисы виртуальной сети [Virtual Private Cloud (VPC)](index.md) предоставляют пользователю:
  * IP-связность между облачными ресурсами.
  * Доступ облачных ресурсов к интернету.

  Виртуальную сеть часто называют термином *Overlay*. В одном [каталоге ресурсов](../../resource-manager/concepts/resources-hierarchy.md#folder) может быть создана одна или несколько [виртуальных сетей](network.md). Виртуальные сети изолированы друг от друга, даже если они находятся в одном каталоге ресурсов.
...
Виртуальная сеть в Yandex Cloud состоит из набора сетевых функций сервиса [Virtual Private Cloud](index.md) и предоставляет пользователям следующие возможности:
...
VRouter обеспечивает работу функции One-to-one NAT для [публичных IP-адресов](address.md#public-addresses) ВМ.

VRouter обеспечивает работу [групп безопасности (SG)](security-groups.md) для всех виртуальных машин физического сервера, на котором он запущен.
...
* Служба DNS. Обрабатывает трафик DNS на втором IP-адресе в облачной подсети (`x.x.x.2`).
* Протокол ICMP.
...
* [NAT-шлюз (NAT-GW)](gateways.md).
* [Сетевой балансировщик нагрузки (NLB)](../../network-load-balancer/concepts/index.md).
* [Cloud Interconnect (CIC)](../../interconnect/concepts/index.md).
